 ...
 */
static ssize_t
build_socks4a_resolve_request(uint8_t **out, const char *username, const char *hostname)
{
    tor_assert(out);
    tor_assert(username);
    tor_assert(hostname);
...
build_socks5_resolve_request(uint8_t **out, const char *hostname, int reverse)
{
    ...
    socks5_client_request_set_command(rq, reverse ? CMD_RESOLVE_PTR : CMD_RESOLVE);
...
cleanup:
    ...
    if (errmsg) {
        tor_free(outbuf);
        log_err(LD_NET, "build_socks5_resolve_request failed with error: %s", errmsg);
    }
...
static ssize_t
build_socks_resolve_request(uint8_t **out, const char *username, const char *hostname, int reverse,
                            int version)
{
    tor_assert(out);
    tor_assert(username);
 ...
 */
static int
parse_socks4a_resolve_response(const char *hostname, const char *response, size_t len,
                               tor_addr_t *addr_out)
{
    int result = 0;
    uint8_t status;
    ...
    ssize_t parsed = socks4_server_reply_parse(&reply, (const uint8_t *)response, len);
 ...
 */
static int
do_resolve(const char *hostname, const tor_addr_t *sockshost, uint16_t socksport, int reverse,
           int version, tor_addr_t *result_addr, char **result_hostname)
{
    ...
    *result_hostname = NULL;
    ...
    socklen = tor_addr_to_sockaddr(sockshost, socksport, (struct sockaddr *)&ss, sizeof(ss));
    ...
    if (version == 5) {
        ...
        if (write_all_to_socket(s, (const char *)buf, encoded_len) != encoded_len) {
        ...
        ssize_t parsed = socks5_server_method_parse(&m, method_buf, sizeof(method_buf));
        ...
        if (method != 0x00) {
            log_err(LD_NET, "Unrecognized socks authentication method: %u", (unsigned)method);
            goto err;
        }
    }

    if ((len = build_socks_resolve_request(&req, "", hostname, reverse, version)) < 0) {
    ...
    if (version == 4) {
        ...
        if (parse_socks4a_resolve_response(hostname, reply_buf, RESPONSE_LEN_4, result_addr) < 0) {
            goto err;
        }
    } else {
        ...
        if (reply_field != 0) {
            log_warn(LD_NET, "Got SOCKS5 status response '%u': %s", (unsigned)reply_field,
        ...
        if (atype == SOCKS5_ATYPE_IPV4) {
            /* IPv4 address */
            tor_addr_from_ipv4h(result_addr, socks5_server_reply_get_bind_addr_ipv4(reply));
...
main(int argc, char **argv)
{
    ...
    log_severity_list_t *severities = tor_malloc_zero(sizeof(log_severity_list_t));
    if (isVerbose)
    ...
    if (n_args == 1) {
        ...
        if (socksport && port_option && socksport != port_option) {
            log_warn(LD_CONFIG, "Conflicting ports; using %d, not %d", (int)socksport,
                     (int)port_option);
        } else if (port_option) {
            socksport = port_option;
        } else if (!socksport) {
    ...
    if (do_resolve(arg[0], &sockshost, socksport, isReverse, isSocks4 ? 4 : 5, &result,
                   &result_hostname))
        return 1;
