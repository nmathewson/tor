    ...
    for (i = 1; i < argc; ++i) {
        if (!strcmp(argv[i], "--help") || !strcmp(argv[i], "-h")) {
            ...
            if (tor_addr_family(&addr) != AF_INET) {
                fprintf(stderr, "%s must resolve to an IPv4 address",
                        addr_arg);
                return 1;
            }
            tor_free(address);
    ...
    if (make_new_id) {
        ...
        if (status != FN_FILE) {
            log_err(LD_GENERAL,
                    ...
                    "for an identity key, use -i.  To generate a new identity "
                    "key, "
...
load_signing_key(void)
{
    ...
    if (!(signing_key = PEM_read_PrivateKey(f, NULL, NULL, NULL))) {
        log_err(LD_GENERAL, "Couldn't read siging key from %s",
                signing_key_file);
        fclose(f);
        return 1;
    }
