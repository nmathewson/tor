...
load_passphrase(void)
{
    ...
    if (n < 0) {
        log_err(LD_GENERAL, "Couldn't read from passphrase fd: %s", strerror(errno));
        return -1;
    }
...
parse_commandline(int argc, char **argv)
{
    ...
    if (!identity_key_file) {
        ...
        log_info(LD_GENERAL, "No identity key file given; defaulting to %s", identity_key_file);
    }
    if (!signing_key_file) {
        signing_key_file = tor_strdup("./authority_signing_key");
        log_info(LD_GENERAL, "No signing key file given; defaulting to %s", signing_key_file);
    }
    if (!certificate_file) {
        certificate_file = tor_strdup("./authority_certificate");
        log_info(LD_GENERAL, "No signing key file given; defaulting to %s", certificate_file);
    }
    if (passphrase_fd >= 0) {
...
load_identity_key(void)
{
    ...
    if (make_new_id) {
        ...
        log_notice(LD_GENERAL, "Generating %d-bit RSA identity key.", IDENTITY_KEY_BITS);
        ...
        if (!(f = start_writing_to_stdio_file(identity_key_file, OPEN_FLAGS_REPLACE | O_TEXT, 0400,
                                              &open_file)))
            return 1;

        /* Write the key to the file.  If passphrase is not set, takes it from
         * the terminal. */
        if (!PEM_write_PKCS8PrivateKey_nid(f, identity_key, NID_pbe_WithSHA1And3_Key_TripleDES_CBC,
                                           passphrase, (int)passphrase_len, NULL, NULL)) {
            log_err(LD_GENERAL, "Couldn't write identity key to %s", identity_key_file);
        ...
        if (!(f = fopen(identity_key_file, "r"))) {
            log_err(LD_GENERAL, "Couldn't open %s for reading: %s", identity_key_file,
                    strerror(errno));
            return 1;
        }
        ...
        if (!identity_key) {
            log_err(LD_GENERAL, "Couldn't read identity key from %s", identity_key_file);
            fclose(f);
            return 1;
        }
        fclose(f);
...
load_signing_key(void)
{
    ...
    if (!(f = fopen(signing_key_file, "r"))) {
        log_err(LD_GENERAL, "Couldn't open %s for reading: %s", signing_key_file, strerror(errno));
        return 1;
    }
...
generate_signing_key(void)
{
    ...
    FILE *f;
    RSA *key;
    log_notice(LD_GENERAL, "Generating %d-bit RSA signing key.", SIGNING_KEY_BITS);
    ...
    if (!(f = start_writing_to_stdio_file(signing_key_file, OPEN_FLAGS_REPLACE | O_TEXT, 0600,
                                          &open_file)))
        return 1;
...
generate_certificate(void)
{
    ...
    tor_snprintf(buf, sizeof(buf),
                 ...
                 "dir-key-crosscert\n"
                 "-----BEGIN ID SIGNATURE-----\n",
                 address ? "\ndir-address " : "", address ? address : "", fingerprint, published,
                 expires, ident, signing);
    tor_free(ident);
    tor_free(signing);

    /* Append a cross-certification */
    RSA *rsa = EVP_PKEY_get1_RSA(signing_key);
    r = RSA_private_encrypt(DIGEST_LEN, (unsigned char *)id_digest, (unsigned char *)signature, rsa,
                            RSA_PKCS1_PADDING);
    RSA_free(rsa);
    ...
    r = RSA_private_encrypt(DIGEST_LEN, (unsigned char *)digest, (unsigned char *)signature, rsa,
                            RSA_PKCS1_PADDING);
    RSA_free(rsa);
    ...
    if (!(f = fopen(certificate_file, "w"))) {
        log_err(LD_GENERAL, "Couldn't open %s for writing: %s", certificate_file, strerror(errno));
        return 1;
    }

    if (fputs(buf, f) < 0) {
        log_err(LD_GENERAL, "Couldn't write to %s: %s", certificate_file, strerror(errno));
        fclose(f);
        return 1;
    }
    fclose(f);
