...
struct or_options_t {
    ...
    struct routerset_t
        *EntryNodes; /**< Structure containing nicknames, digests,
    ...
    struct routerset_t
        *ExcludeNodes; /**< Structure containing nicknames,
                        * digests, country codes and IP address patterns
    ...
    struct config_line_t *ControlPort_lines; /**< Ports to listen on for
                                              * control connections. */
    ...
    int BridgeAuthoritativeDir; /**< Boolean: is this an authoritative
                                 * directory that aggregates bridge
                                 * descriptors? */
    ...
    int ReducedConnectionPadding; /**< Boolean: Should we try to keep
                                    connections open shorter and pad them less
                                    against
     ...
     * rend_service_reveal_startup_time() instead of using this option
     * directly.
     */
     ...
     * Use rend_service_non_anonymous_mode_enabled() instead of using this
     * option directly.
     */
    ...
    struct config_line_t
        *ReachableDirAddresses; /**< IP:ports for Dir conns. */
    ...
    int AutomapHostsOnResolve; /**< If true, when we get a resolve request for
                                * a hostname ending with one of the suffixes in
    ...
    int NewCircuitPeriod; /**< How long do we use a circuit before building
                           * a new one? */
    int MaxCircuitDirtiness; /**< Never use circs that were first used more
                                than this interval ago. */
    uint64_t BandwidthRate; /**< How much bandwidth, on average, are we willing
                             * to use in a second? */
    uint64_t BandwidthBurst; /**< How much bandwidth, at maximum, are we
                              * willing to use in a second? */
    ...
    uint64_t
        PerConnBWBurst; /**< Allowed burst on a single TLS conn, if set. */
    ...
    char *
        Socks5ProxyUsername; /**< Username for SOCKS5 authentication, if any */
    char *
        Socks5ProxyPassword; /**< Password for SOCKS5 authentication, if any */

    char *
        TCPProxy; /**< protocol and hostname:port to use as a proxy, if any. */
    ...
    struct config_line_t *AuthDirBadExit; /**< Address policy for descriptors
                                           * to mark as bad exits. */
    struct config_line_t *AuthDirReject; /**< Address policy for descriptors to
                                          * reject. */
    struct config_line_t *AuthDirInvalid; /**< Address policy for descriptors
                                           * to never mark as valid. */
    /** @name AuthDir...CC
     *
    ...
    char *
        SafeLogging; /**< Contains "relay", "1", "0" (meaning no scrubbing). */
     ...
     * UseEntryGuards_option -- when we're a non-anonymous Single Onion
     * Service, it is always false, otherwise we use the value of
    ...
    int DisableDebuggerAttachment; /**< Currently Linux only specific attempt
                                      to disable ptrace; needs BSD testing. */
    ...
    int EnforceDistinctSubnets; /**< If true, don't allow multiple routers in
                                 * the same network zone in the same circuit.
                                 */
    int AllowNonRFC953Hostnames; /**< If true, we allow connections to
                                  * hostnames with weird characters. */
     ...
     * prefer IPv4. Use node_ipv6_or_preferred() instead of accessing this
     * value directly. */
    ...
    /** If 1, we try to use microdescriptors to build circuits.  If 0, we
     * don't. If -1, Tor decides. */
...
#define MAX_MAX_CLIENT_CIRCUITS_PENDING 1024
    ...
    /** Is this an exit node?  This is a tristate, where "1" means "yes, and
     * use the default exit policy if none is given" and "0" means "no; exit
     * policy is 'reject *'" and "auto" (-1) means "same as 1, but warn the
     * user."
     *
     ...
     * zero, do what the consensus says and fall back to using KIST as if this
     * is set to "10 msec" if the consensus doesn't say anything. */
    ...
    /** For how much time (in seconds) the defense is applicable for a
     * malicious address. A random time delta is added to the defense time of
     * an address which will be between 1 second and half of this value. */
    ...
    /** Boolean: true if having an idle stream is sufficient to prevent a
     *client from becoming dormant.
     **/
