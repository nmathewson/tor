...
setopt_err_t options_init_from_string(const char *cf_defaults, const char *cf, int command,
...
typedef enum { DIRROOT_DATADIR, DIRROOT_CACHEDIR, DIRROOT_KEYDIR } directory_root_t;
...
#define options_get_datadir_fname2_suffix(options, sub1, sub2, suffix) \
    options_get_dir_fname2_suffix((options), DIRROOT_DATADIR, (sub1), (sub2), (suffix))
#define options_get_cachedir_fname2_suffix(options, sub1, sub2, suffix) \
    options_get_dir_fname2_suffix((options), DIRROOT_CACHEDIR, (sub1), (sub2), (suffix))
#define options_get_keydir_fname2_suffix(options, sub1, sub2, suffix) \
    options_get_dir_fname2_suffix((options), DIRROOT_KEYDIR, (sub1), (sub2), (suffix))
...
int port_exists_by_type_addr_port(int listener_type, const tor_addr_t *addr, int port,
                                  int check_wildcard);
int port_exists_by_type_addr32h_port(int listener_type, uint32_t addr_ipv4h, int port,
                                     int check_wildcard);
...
int init_cookie_authentication(const char *fname, const char *header, int cookie_len,
...
int port_cfg_line_extract_addrport(const char *line, char **addrport_out, int *is_unix_out,
                                   const char **rest_out);
...
int port_parse_config(smartlist_t *out, const struct config_line_t *ports, const char *portname,
                      int listener_type, const char *defaultaddr, int defaultport,
...
int pt_parse_transport_line(const or_options_t *options, const char *line, int validate_only,
                            int server);
...
MOCK_DECL(STATIC int, options_act_reversible, (const or_options_t *old_options, char **msg));
struct config_mgr_t;
STATIC const struct config_mgr_t *get_options_mgr(void);

#    define or_options_free(opt) FREE_AND_NULL(or_options_t, or_options_free_, (opt))
...
STATIC int parse_dir_authority_line(const char *line, dirinfo_type_t required_type,
                                    int validate_only);
STATIC int parse_dir_fallback_line(const char *line, int validate_only);

STATIC uint64_t compute_real_max_mem_in_queues(const uint64_t val, int log_guess);
STATIC int open_and_add_file_log(const log_severity_list_t *severity, const char *fname,
                                 int truncate_log);
STATIC int options_init_logs(const or_options_t *old_options, const or_options_t *options,
                             int validate_only);

STATIC int options_create_directories(char **msg_out);
struct log_transaction_t;
STATIC struct log_transaction_t *options_start_log_transaction(const or_options_t *old_options,
                                                               char **msg_out);
...
int options_validate(const or_options_t *old_options, or_options_t *options, char **msg);
#    endif

STATIC int parse_ports(or_options_t *options, int validate_only, char **msg, int *n_ports_out,
