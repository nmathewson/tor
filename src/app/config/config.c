...
set_options(or_options_t *new_val, char **msg)
{
    ...
    /* Issues a CONF_CHANGED event to notify controller of the change. If Tor
     * is just starting up then the old_options will be undefined. */
    ...
    if (old_options != global_options) {
        ...
        /* If we are here it means we've successfully applied the new options
         * and that the global options have been changed to the new values.
         * We'll check if we need to remove or add periodic events. */
                            ...
                            const or_options_t *old_options)
{
    ...
    if (!options->DirAuthorities) {
        ...
        if (!options->AlternateDirAuthority) {
            ...
            /* Only add the default fallback directories when the
             * DirAuthorities, AlternateDirAuthority, and FallbackDir directory
             * config options are set to their defaults, and when
             * UseDefaultFallbackDirs is 1. */
...
options_create_directories(char **msg_out)
{
    ...
    /* We need to handle the group-readable flag for the cache directory and
     * key directory specially, since they may be the same as the data
     * directory */
...
options_start_listener_transaction(const or_options_t *old_options,
                                   char **msg_out)
{
    listener_transaction_t *xn =
        tor_malloc_zero(sizeof(listener_transaction_t));
    ...
    if (!we_are_hibernating()) {
        if (retry_all_listeners(xn->new_listeners, options->DisableNetwork) <
            0) {
...
options_commit_listener_transaction(listener_transaction_t *xn)
{
    ...
    if (xn->set_conn_limit) {
         ...
         * possible sockets, keep this below a limit and set
         * ConnLimit_high_thresh very close to ConnLimit_, but if ConnLimit_ is
         * low, shrink it in proportion.
         *
        ...
        options->ConnLimit_high_thresh =
            options->ConnLimit_ - socks_in_reserve;
          ...
          (const or_options_t *old_options, char **msg))
{
    ...
    listener_transaction =
        options_start_listener_transaction(old_options, msg);
...
options_need_geoip_info(const or_options_t *options, const char **reason_out)
{
    ...
    if (routerset_usage && reason_out) {
        *reason_out =
            ...
            "countries, and we need GEOIP information to figure out which "
            "ones they "
            "are.";
    } else if (bridge_usage && reason_out) {
        *reason_out =
            "We've been configured to see which countries can access "
            "us as a bridge, and we need GEOIP information to tell which "
            "countries "
            "clients are in.";
    }
...
options_act, (const or_options_t *old_options))
{
    ...
    if (init_control_cookie_authentication(options->CookieAuthentication) <
        0) {
        log_warn(LD_CONFIG,
    ...
    /* Update the BridgePassword's hashed version as needed.  We store this as
     * a digest so that we can do side-channel-proof comparisons on it.
     */
    ...
    if (geoip_is_loaded(AF_INET) && options->GeoIPExcludeUnknown) {
        ...
        changed +=
            routerset_add_unknown_ccs(&options->ExcludeExitNodes, is_auto);
    ...
    if (old_options) {
        if (!bool_eq(dirclient_fetches_dir_info_early(options),
            ...
            /* We might need to download a new consensus status later or sooner
...
options_trial_assign(config_line_t *list, unsigned flags, char **msg)
{
    ...
    if ((r = config_assign(get_options_mgr(), trial_options, list, flags,
                           msg)) < 0) {
                   ...
                   char **hostname_out)
{
            ...
            if (!explicit_hostname && tor_addr_is_internal(&myaddr, 0)) {
                ...
                if (get_interface_address6(warn_severity, AF_INET,
                    ...
                    log_fn(notice_severity, LD_CONFIG,
                           ...
                           " Using that.",
                           fmt_addr32(addr));
                    strlcpy(hostname, "<guessed from interfaces>",
                            sizeof(hostname));
                }
            }
        }
    ...
    if (tor_addr_is_internal(&myaddr, 0)) {
        ...
        if (!explicit_ip) {
            /* even if they've set their own authorities, require an explicit
             * IP if they're using an internal address. */
            log_fn(
                warn_severity, LD_CONFIG,
    ...
    *addr_out = addr;
    ...
    if (last_resolved_addr != *addr_out) {
        control_event_server_status(
            LOG_NOTICE, "EXTERNAL_ADDRESS ADDRESS=%s METHOD=%s%s%s",
            addr_string, method_used, hostname_used ? " HOSTNAME=" : "",
            hostname_used ? hostname_used : "");
    }
...
is_local_addr, (const tor_addr_t *addr))
{
    ...
    if (tor_addr_family(addr) == AF_INET) {
         ...
         * that resolve_my_address will never be called at all).  In those
         * cases, last_resolved_addr will be 0, and so checking to see whether
         * ip is on the same /24 as last_resolved_addr will be the same as
...
config_ensure_bandwidth_cap(uint64_t *value, const char *desc, char **msg)
{
    if (*value > ROUTER_MAX_DECLARED_BANDWIDTH) {
        /* This handles an understandable special case where somebody says
         * "2gb" whereas our actual maximum is 2gb-1 (INT_MAX) */
        --*value;
    }
    if (*value > ROUTER_MAX_DECLARED_BANDWIDTH) {
        tor_asprintf(msg, "%s (%" PRIu64 ") must be at most %d", desc,
                     (*value), ROUTER_MAX_DECLARED_BANDWIDTH);
        return -1;
    }
    return 0;
}
...
warn_about_relative_paths(const or_options_t *options)
{
    ...
    for (config_line_t *hs_line = options->RendConfigLines; hs_line;
        ...
        if (!strcasecmp(hs_line->key, "HiddenServiceDir"))
            n += warn_if_option_path_is_relative("HiddenServiceDir",
                                                 hs_line->value);
    }
    return n != 0;
}
...
options_validate_scheduler(or_options_t *options, char **msg)
{
    ...
    if (options->SchedulerTypes_) {
        SMARTLIST_FOREACH(options->SchedulerTypes_, int *, iptr,
                          tor_free(iptr));
        smartlist_free(options->SchedulerTypes_);
    }
...
options_validate_single_onion(or_options_t *options, char **msg)
{
    ...
    if (options->HiddenServiceSingleHopMode &&
        ...
        REJECT("HiddenServiceSingleHopMode does not provide any server "
               "anonymity. "
    ...
    if (options->HiddenServiceNonAnonymousMode &&
        ...
        REJECT("HiddenServiceNonAnonymousMode does not provide any server "
               "anonymity. It must be used with HiddenServiceSingleHopMode "
               "set to "
               "1.");
    }

    /* Now that we've checked that the two options are consistent, we can
     * safely call the rend_service_* functions that abstract these options. */
    ...
    if (rend_service_non_anonymous_mode_enabled(options) && client_port_set) {
        REJECT(
            ...
            "an anonymous client. Please set Socks/Trans/NATD/DNSPort to 0, "
            "or "
    ...
    if (rend_service_allow_non_anonymous_connection(options) &&
        ...
        /* Single Onion services only use entry guards when uploading
         * descriptors; all other connections are one-hop. Further, Single
         * Onions causes the hidden service code to do things which break the
         * path bias detector, and it's far easier to turn off entry guards
         * (and thus the path bias detector with it) than to figure out how to
...
#    endif /* !defined(KERNEL_MAY_SUPPORT_IPFW) */
    ...
    if (options->Socks5ProxyUsername) {
        ...
        if (!options->Socks5ProxyPassword)
            REJECT("Socks5ProxyPassword must be included with "
                   "Socks5ProxyUsername.");
    ...
    if (options->HashedControlPassword) {
        smartlist_t *sl =
            decode_hashed_passwords(options->HashedControlPassword);
        if (!sl) {
    ...
    if (options->HashedControlSessionPassword) {
        ...
        if (!sl) {
            REJECT("Bad HashedControlSessionPassword: wrong length or bad "
                   "encoding");
        } else {
            SMARTLIST_FOREACH(sl, char *, cp, tor_free(cp));
            smartlist_free(sl);
    ...
    if (options->ConstrainedSockets) {
        /* If the user wants to constrain socket buffer use, make sure the
         * desired limit is between MIN|MAX_TCPSOCK_BUFFER in k increments. */
    ...
    if (parse_virtual_addr_network(options->VirtualAddrNetworkIPv4, AF_INET, 1,
                                   msg) < 0)
        return -1;
    if (parse_virtual_addr_network(options->VirtualAddrNetworkIPv6, AF_INET6,
                                   1, msg) < 0)
        return -1;
...
#define CHECK_DEFAULT(arg)                                            \
    STMT_BEGIN                                                        \
        if (!config_is_same(get_options_mgr(), options, dflt_options, \
                            #arg)) {                                  \
...
#    define RAM_IS_VERY_LARGE(x) (0)
            ...
            if (RAM_IS_VERY_LARGE(ram)) {
                /* If we have 8 GB, or more, RAM available, we set the
                 * MaxMemInQueues to 0.4 * RAM. The idea behind this value is
                 * that the amount of RAM is more than enough for a single
                 * relay and should allow the relay operator to run two relays
                 * if they have additional bandwidth available.
                 */
                avail = (ram / 5) * 2;
            } else {
                /* If we have less than 8 GB of RAM available, we use the "old"
                 * default for MaxMemInQueues of 0.75 * RAM.
                 */
                avail = (ram / 4) * 3;
            }

            /* Make sure it's in range from 0.25 GB to 8 GB for 64-bit and 0.25
             * to 2 GB for 32-bit. */
            if (avail > MAX_DEFAULT_MEMORY_QUEUE_SIZE) {
                /* If you want to use more than this much RAM, you need to
...
get_windows_conf_root(void)
{
    ...
    if (is_set)
         ...
         * We would use SHGetSpecialFolder path, but that wasn't added until
         * IE4.
         */
...
load_torrc_from_disk(const config_line_t *cmd_arg, int defaults_file)
{
    ...
    if (*fname_var == NULL) {
        fname =
            find_torrc_filename(cmd_arg, defaults_file, &using_default_torrc,
    ...
    log_debug(LD_CONFIG, "Opening config file \"%s\"",
              fname ? fname : "<NULL>");
    ...
    if (fname == NULL || !(st == FN_FILE || st == FN_EMPTY) ||
        ...
        if (using_default_torrc == 1 || ignore_missing_torrc) {
            ...
            *fname_var = NULL;
            ...
            log_warn(LD_CONFIG, "Unable to open configuration file \"%s\".",
                     fname);
            goto err;
        }
    } else {
...
options_init_from_torrc(int argc, char **argv)
{
    ...
    if (command == CMD_HASH_PASSWORD) {
        ...
        const config_line_t *f_line =
            config_line_find(cmdline_only_options, "-f");
        ...
        if (!cf) {
            if (config_line_find(cmdline_only_options,
                                 "--allow-missing-torrc")) {
                cf = tor_strdup("");
            } else {
                goto err;
    ...
    if (config_line_find(cmdline_only_options, "--no-passphrase")) {
        if (command == CMD_KEYGEN) {
            get_options_mutable()->keygen_force_passphrase =
...

    {
        ...
        if (fd_line) {
            if (get_options()->keygen_force_passphrase ==
                FORCE_PASSPHRASE_OFF) {
                log_err(LD_CONFIG,
                ...
                if (fd < 0 || ok == 0) {
                    log_err(LD_CONFIG, "Invalid --passphrase-fd value %s",
                            escaped(v));
                    retval = -1;
                    goto err;
                }
...

    {
        ...
        if (key_line) {
            if (command != CMD_KEYGEN) {
                ...
                get_options_mutable()->master_key_fname =
                    tor_strdup(key_line->value);
            }
        }
    }
                         ...
                         const char *command_arg, char **msg)
{
    ...
    if (newdefaultoptions == NULL) {
        newdefaultoptions =
            config_dup(get_options_mgr(), global_default_options);
    }
    ...
    if (err < 0) {
        newoptions =
            NULL; // This was already freed in options_validate_and_set.
        goto err;
    }
...
config_register_addressmaps(const or_options_t *options)
{
    ...
    for (opt = options->AddressMap; opt; opt = opt->next) {
        ...
        if (addressmap_register_auto(from, to, 0, ADDRMAPSRC_TORRC, &msg) <
            0) {
            log_warn(LD_CONFIG, "MapAddress '%s' failed: %s. Ignoring.",
                     opt->value, msg);
            goto cleanup;
        }
    ...
    for (opt = options->Logs; opt; opt = opt->next) {
        ...
        if (smartlist_len(elts) == 1 &&
            ...
            if (!validate_only) {
                if (run_as_daemon) {
                    log_warn(LD_CONFIG,
                             "Can't log to %s with RunAsDaemon set; skipping "
                             "stdout",
                             err ? "stderr" : "stdout");
                } else {
        ...
        if (smartlist_len(elts) == 2 &&
            ...
            if (!validate_only) {
                ...
                /* Truncate if TruncateLogFile is set and we haven't seen this
                   option line before. */
                int truncate_log = 0;
...
validate_transport_socks_arguments(const smartlist_t *args)
{
    ...
    SMARTLIST_FOREACH_BEGIN (args, const char *, s) {
        if (!string_is_key_value(LOG_WARN,
                                 s)) { /* items should be k=v items */
                        ...
                        int validate_only, int server)
{
    ...
    if (is_managed) {
        ...
        if (!validate_only && (server || !is_useless_proxy)) {
            ...
            *tmp =
                NULL; /* terminated with NULL, just like execve() likes it */
        ...
        if (tor_addr_port_lookup(addrport, &addr, &port) < 0) {
            log_warn(LD_CONFIG, "Error parsing transport address '%s'",
                     addrport);
            goto err;
        }

        if (!port) {
            log_warn(LD_CONFIG, "Transport address '%s' has no port.",
                     addrport);
            goto err;
        }
        ...
        if (!validate_only) {
            ...
            if (!server) {
                transport_add_from_config(
                    &addr, port, smartlist_get(transport_list, 0), socks_ver);
            }
        }
    }
                         ...
                         int validate_only)
{
    ...
    while (smartlist_len(items)) {
        ...
        if (!strcasecmp(flag, "hs") || !strcasecmp(flag, "no-hs")) {
            ...
            /* obsolete, but may still be contained in DirAuthority lines
            ...
            or_port =
                (uint16_t)tor_parse_long(portstring, 10, 1, 65535, &ok, NULL);
            if (!ok)
                log_warn(LD_CONFIG,
            ...
            weight =
                tor_parse_double(wstring, 0, (double)UINT64_MAX, &ok, NULL);
            if (!ok) {
                log_warn(LD_CONFIG,
    ...
    if (!dir_port) {
        log_warn(LD_CONFIG, "Missing port in DirAuthority address '%s'",
                 addrport);
        goto err;
    }
...
parse_dir_fallback_line(const char *line, int validate_only)
{
    ...
    SMARTLIST_FOREACH_BEGIN (items, const char *, cp) {
        ...
        if (!strcmpstart(cp, "orport=")) {
            orport = (int)tor_parse_long(cp + strlen("orport="), 10, 1, 65535,
                                         &ok, NULL);
            ...
            weight = tor_parse_double(wstring, 0, (double)UINT64_MAX, &num_ok,
                                      NULL);
            if (!num_ok) {
                log_warn(LD_CONFIG, "Invalid weight '%s' on FallbackDir line.",
                         cp);
                weight = 1.0;
            }
        }
    ...
    if (smartlist_len(positional) != 1) {
        log_warn(LD_CONFIG, "Couldn't parse FallbackDir line %s",
                 escaped(line));
        goto end;
    }
    ...
    if (!validate_only) {
        ...
        ds = fallback_dir_server_new(&addr, dirport, orport, ipv6_addrport_ptr,
                                     id, weight);
        if (!ds) {
            log_warn(LD_CONFIG, "Couldn't create FallbackDir %s",
                     escaped(line));
            goto end;
        }
        dir_server_add(ds);
            ...
            if (forbid_nonlocal) {
                if (!warned)
                    log_warn(LD_CONFIG, "You have a ControlPort set to accept "
                                        "unauthenticated connections from a "
                                        "non-local address.  "
                                        "This means that programs not running "
                                        "on your computer "
                                        "can reconfigure your Tor, without "
                                        "even having to guess a "
                                        "password.  That's so bad that I'm "
                                        "closing your ControlPort "
                                        "for you.  If you need to control "
                                        "your Tor remotely, try "
                                        "enabling authentication and using a "
                                        "tool like stunnel or "
                ...
                log_warn(
                    ...
                    "programs not running on your computer can reconfigure "
                    "your "
                    "Tor.  That's pretty bad, since the controller "
                    "protocol isn't encrypted!  Maybe you should just listen "
                    "on "
                  ...
                  const unsigned flags)
{
    ...
    const unsigned is_ext_orport =
        (listener_type == CONN_TYPE_EXT_OR_LISTENER);
    ...
    for (; ports; ports = ports->next) {
        ...
        int no_listen = 0, no_advertise = 0, all_addrs = 0, bind_ipv4_only = 0,
            ...
            prefer_ipv6 = 1, dns_request = 1, onion_traffic = 1,
            cache_ipv4 = 0, use_cached_ipv4 = 0, cache_ipv6 = 0,
            use_cached_ipv6 = 0, prefer_ipv6_automap = 1, world_writable = 0,
        ...
        if (strlen(addrport) == 0) {
            log_warn(LD_CONFIG, "Invalid %sPort line with no address",
                     portname);
            goto err;
        }
        ...
        if (unix_socket_path &&
            ...
            log_warn(LD_CONFIG, "%sPort does not support unix sockets",
                     portname);
            goto err;
        }
        ...
        if (unix_socket_path) {
            ...
            if (ok) {
                ...
                if (ptmp == 0) {
                    log_warn(LD_CONFIG, "%sPort line has address but no port",
                             portname);
                    goto err;
                }
                port = ptmp;
          ...
          all_addrs = 1;
            ...
            SMARTLIST_FOREACH_BEGIN (elts, char *, elt) {
                ...
                if (!strcasecmpstart(elt, "SessionGroup=")) {
                    int group =
                        (int)tor_parse_long(elt + strlen("SessionGroup="), 10,
                                            0, INT_MAX, &ok, NULL);
                ...
                if (takes_hostnames) {
                    if (!strcasecmp(elt, "IPv4Traffic")) {
                         ...
                         * NoDNSRequest, NoIPv4Traffic, NoIPv6Traffic. The
                         * option
                         * NoOnionTrafficOnly is not supported, it's too
                         * confusing. */
                        if (no) {
                            log_warn(LD_CONFIG,
                                     "Unsupported %sPort option 'No%s'. Use "
                                     "DNSRequest, IPv4Traffic, and/or "
                                     "IPv6Traffic instead.",
                                     portname, escaped(elt));
                        } else {
            ...
            int *n_ports_out, int *world_writable_control_socket)
{
    ...
    *n_ports_out = 0;
    ...
    }
    {
        ...
        if (port_parse_config(ports, options->ControlSocket, "ControlSocket",
                              ...
                              control_port_flags | CL_PORT_IS_UNIXSOCKET) <
            0) {
    ...
    *n_ports_out = smartlist_len(ports);
    ...
    if (!validate_only) {
        if (configured_ports) {
            SMARTLIST_FOREACH(configured_ports, port_cfg_t *, p,
                              port_cfg_free(p));
            smartlist_free(configured_ports);
        }
    ...
    SMARTLIST_FOREACH_BEGIN (configured_ports, const port_cfg_t *, cfg) {
        ...
        if (cfg->type == listener_type &&
            ...
            if (tor_addr_is_null(&cfg->addr))
                address = tor_addr_is_v4(&cfg->addr) ? ipv4_localhost
                                                     : ipv6_localhost;
            else
    ...
    SMARTLIST_FOREACH_BEGIN (configured_ports, const port_cfg_t *, cfg) {
        if (cfg->type == listener_type) {
            if (cfg->port == port || (check_wildcard && port == -1)) {
                ...
                const int addr_any_v4 =
                    tor_addr_is_null(&cfg->addr) && addr_v4;
                ...
                const int addr_v6 = tor_addr_family(addr) == AF_INET6 ||
                                    tor_addr_family(addr) == AF_UNSPEC;
                const int addr_any_v6 =
                    tor_addr_is_null(&cfg->addr) && addr_v6;
...
validate_data_directories(or_options_t *options)
{
    ...
    if (options->KeyDirectory_option) {
        options->KeyDirectory =
            get_data_directory(options->KeyDirectory_option);
...
write_configuration_file(const char *fname, const or_options_t *options)
{
    ...
    case FN_DIR:
    default:
        log_warn(LD_CONFIG, "Config file \"%s\" is not a file? Failing.",
                 fname);
        return -1;
    }
    ...
    if (rename_old) {
        ...
        log_notice(LD_CONFIG, "Renaming old configuration file to \"%s\"",
                   fn_tmp);
           ...
           const char *sub1, const char *sub2, const char *suffix))
{
    ...
    if (sub1 == NULL) {
        ...
        tor_asprintf(&fname, "%s" PATH_SEPARATOR "%s%s", rootdir, sub1,
                     suffix);
    } else {
                     ...
                     const char *descr)
{
    ...
    if (write_str_to_file(filename, str, 0) < 0) {
        log_warn(LD_HIST, "Unable to write %s to disk!",
                 descr ? descr : fname);
        return_val = -1;
    }
    tor_free(filename);
    ...
    if (!strcmp(question, "config/names")) {
        ...
        *answer = smartlist_join_strings(sl, "", 0, NULL);
        ...
        SMARTLIST_FOREACH_BEGIN (vars, const config_var_t *, var) {
            if (var->initvalue != NULL) {
                if (strcmp(var->member.name, "DirAuthority") == 0) {
                     ...
                     * count later to decide whether to add the defaults
                     * manually
                     */
                    ++dirauth_lines_seen;
                }
                if (strcmp(var->member.name, "FallbackDir") == 0) {
                    /*
                     * Similarly count fallback lines, so that we can decide
                     * later to add the defaults manually.
                     */
                    ++fallback_lines_seen;
                }
        ...
        if (fallback_lines_seen == 0 &&
             ...
             * is a const char ** NULL-terminated array of fallback config
             * lines.
             */
            const char **i;
                        ...
                        const char *default_fname)
{
    ...
    const char *msg = "";
    int severity =
        options_need_geoip_info(options, &msg) ? LOG_WARN : LOG_INFO;
    int r;
                           ...
                           uint8_t **cookie_out, int *cookie_is_set_out)
{
    ...
    *cookie_out = tor_malloc(cookie_len);
    ...
    /* Success! */
    log_info(LD_GENERAL, "Generated auth cookie file in '%s'.",
             escaped(fname));
    *cookie_is_set_out = 1;
    retval = 0;
