...
static const config_var_t option_vars_[] = {
    ...
    V(AuthDirListBadExits, BOOL, "0"), V(AuthDirMaxServersPerAddr, POSINT, "2"),
    ...
    V(ControlPortWriteToFile, FILENAME, NULL), V(ControlSocket, LINELIST, NULL),
    ...
    V(DormantOnFirstStartup, BOOL, "0"), V(DormantCanceledByStartup, BOOL, "0"),
    ...
    V(ExitRelay, AUTOBOOL, "auto"), VPORT(ExtORPort),
    ...
    V(FetchServerDescriptors, BOOL, "1"), V(FetchHidServDescriptors, BOOL, "1"),
    ...
    V(TruncateLogFile, BOOL, "0"), V_IMMUTABLE(SyslogIdentityTag, STRING, NULL),
    ...
    VAR_NODUMP("__DisablePredictedCircuits", BOOL, DisablePredictedCircuits,
               "0"),
    VAR_NODUMP_IMMUTABLE("__DisableSignalHandlers", BOOL, DisableSignalHandlers,
                         "0"),
...
static int options_check_transition_cb(const void *old, const void *new,
                                       char **msg);
static int parse_ports(or_options_t *options, int validate_only, char **msg_out,
          ...
          (const or_options_t *old_options, char **msg))
{
    ...
    if (running_tor) {
        ...
        if (running_tor && !libevent_initialized) {
             ...
             * completely sensible to hide the libevent setup in the option
             * parsing code!  It also needs to happen before init_keys(), so it
             * needs to happen here too.  How yucky. */
            scheduler_init();
        }
         ...
         * to ports under 1024.)  We don't want to rebind if we're hibernating
         * or shutting down. If networking is disabled, this will close all but
         * the control listeners, but disable those. */
        /* 31851: some listeners are relay-only */
        if (!we_are_hibernating()) {
            if (retry_all_listeners(new_listeners, options->DisableNetwork) <
                0) {
    ...
    if (check_and_create_data_directory(
            ...
            options->DataDirectoryGroupReadable, options->User, msg) < 0) {
        goto done;
    }
    if (check_and_create_data_directory(
            running_tor /* create */, options->KeyDirectory,
            options->KeyDirectoryGroupReadable, options->User, msg) < 0) {
        goto done;
    }
    ...
    if (options->CacheDirectoryGroupReadable != -1) {
        ...
        /* If the user left the value as "auto", and the cache is the same as
         * the datadirectory, use the datadirectory setting.
         */
    ...
    if (check_and_create_data_directory(
            ...
            cache_dir_group_readable, options->User, msg) < 0) {
        goto done;
    }
...
options_need_geoip_info(const or_options_t *options, const char **reason_out)
{
    ...
    if (routerset_usage && reason_out) {
        *reason_out =
            ...
            "countries, and we need GEOIP information to figure out which ones "
            "they "
            "are.";
                   ...
                   char **hostname_out)
{
    ...
    if (tor_addr_is_internal(&myaddr, 0)) {
        ...
        if (!explicit_ip) {
            /* even if they've set their own authorities, require an explicit IP
             * if they're using an internal address. */
            log_fn(
                warn_severity, LD_CONFIG,
...
options_validate_single_onion(or_options_t *options, char **msg)
{
    ...
    if (rend_service_allow_non_anonymous_connection(options) &&
         ...
         * path bias detector, and it's far easier to turn off entry guards (and
...
#define RAM_IS_VERY_LARGE(x) (0)
            ...
            if (RAM_IS_VERY_LARGE(ram)) {
                 ...
                 * that the amount of RAM is more than enough for a single relay
                 * and should allow the relay operator to run two relays if they
                 * have additional bandwidth available.
                 */
                avail = (ram / 5) * 2;
            } else {
 ...
 */
STATIC int
open_and_add_file_log(const log_severity_list_t *severity, const char *filename,
                      int truncate_log)
{
...
parse_dir_fallback_line(const char *line, int validate_only)
{
    ...
    SMARTLIST_FOREACH_BEGIN(items, const char *, cp)
    {
        ...
        if (!strcmpstart(cp, "orport=")) {
            ...
            weight =
                tor_parse_double(wstring, 0, (double)UINT64_MAX, &num_ok, NULL);
            if (!num_ok) {
            ...
            if (forbid_nonlocal) {
                if (!warned)
                    log_warn(
                        ...
                        "can reconfigure your Tor, without even having to "
                        "guess a "
                        "password.  That's so bad that I'm closing your "
                        "ControlPort "
                        "for you.  If you need to control your Tor remotely, "
                        "try "
                        "enabling authentication and using a tool like stunnel "
                        "or "
