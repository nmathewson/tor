 ...
 */
int
resolve_my_address(int warn_severity, const or_options_t *options, uint32_t *addr_out,
                   const char **method_out, char **hostname_out)
{
    ...
    if (tor_inet_aton(hostname, &in) == 0) {
        ...
        if (tor_lookup_hostname(hostname, &addr)) { /* failed to resolve */
            ...
            if (explicit_hostname) {
                log_fn(warn_severity, LD_CONFIG, "Could not resolve local Address '%s'. Failing.",
                       hostname);
                return -1;
            }
    ...
    *addr_out = addr;
    ...
    if (last_resolved_addr != *addr_out) {
        control_event_server_status(LOG_NOTICE, "EXTERNAL_ADDRESS ADDRESS=%s METHOD=%s%s%s",
                                    addr_string, method_used, hostname_used ? " HOSTNAME=" : "",
                                    hostname_used ? hostname_used : "");
    }
...
is_local_addr, (const tor_addr_t *addr))
{
    ...
    if (tor_addr_family(addr) == AF_INET) {
         ...
         */
        if ((last_resolved_addr & (uint32_t)0xffffff00ul) == (ip & (uint32_t)0xffffff00ul))
            return 1;
    }
    return 0;
}
