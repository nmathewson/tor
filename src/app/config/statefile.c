...
or_state_set(or_state_t *new_state)
{
    ...
    if (circuit_build_times_parse_state(get_circuit_build_times_mutable(), global_state) < 0) {
        ret = -1;
    }
...
or_state_load(void)
{
    ...
    if (contents) {
        ...
        if (config_get_lines(contents, &lines, 0) < 0)
            goto done;
        assign_retval = config_assign(get_state_mgr(), new_state, lines, 0, &errmsg);
    ...
    if (badstate && !contents) {
        ...
        if (apparent_skew < 0) {
            ...
            clock_skew_warning(NULL, (long)apparent_skew, 1, LD_GENERAL, "local state file", fname);
        }
    } else {
...
get_transport_in_state_by_name(const char *transport)
{
    ...
    for (line = or_state->TransportProxies; line; line = line->next) {
        ...
        items = smartlist_new();
        smartlist_split_string(items, line->value, NULL, SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, -1);
...
/** Save <b>transport</b> listening on <b>addr</b>:<b>port</b> to
    state */
void
save_transport_to_state(const char *transport, const tor_addr_t *addr, uint16_t port)
{
    ...
    if (transport_line) { /* if transport already exists in state... */
        ...
        if (!strcmp(prev_bindaddr, transport_addrport)) {
            ...
            tor_asprintf(&transport_line->value, "%s %s", transport, fmt_addrport(addr, port));
        }
