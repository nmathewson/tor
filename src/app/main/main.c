...
dumpmemusage(int severity)
{
    ...
    tor_log(severity, LD_GENERAL, "In rephist: %" PRIu64 " used by %d Tors.", (rephist_total_alloc),
            rephist_total_num);
...
dumpstats(int severity)
{
    ...
    SMARTLIST_FOREACH_BEGIN (get_connection_array(), connection_t *, conn) {
        ...
        tor_log(severity, LD_GENERAL,
                "Conn %d (socket %d) type %d (%s), state %d (%s), created %d secs ago", i,
                (int)conn->s, conn->type, conn_type_to_string(conn->type), conn->state,
                conn_state_to_string(conn->type, conn->state),
                (int)(now - conn->timestamp_created));
        if (!connection_is_listener(conn)) {
            tor_log(severity, LD_GENERAL, "Conn %d is to %s:%d.", i, safe_str_client(conn->address),
                    conn->port);
            tor_log(severity, LD_GENERAL,
                    "Conn %d: %d bytes waiting on inbuf (len %d, last read %d secs ago)", i,
                    (int)connection_get_inbuf_len(conn), (int)buf_allocation(conn->inbuf),
                    (int)(now - conn->timestamp_last_read_allowed));
            tor_log(severity, LD_GENERAL,
                    "Conn %d: %d bytes waiting on outbuf "
                    "(len %d, last written %d secs ago)",
                    i, (int)connection_get_outbuf_len(conn), (int)buf_allocation(conn->outbuf),
                    (int)(now - conn->timestamp_last_write_allowed));
            if (conn->type == CONN_TYPE_OR) {
                or_connection_t *or_conn = TO_OR_CONN(conn);
                if (or_conn->tls) {
                    if (tor_tls_get_buffer_sizes(or_conn->tls, &rbuf_cap, &rbuf_len, &wbuf_cap,
                                                 &wbuf_len) == 0) {
                        tor_log(severity, LD_GENERAL,
    ...
    if (elapsed) {
        tor_log(severity, LD_NET, "Average bandwidth: %" PRIu64 "/%d = %d bytes/sec reading",
                (get_bytes_read()), (int)elapsed, (int)(get_bytes_read() / elapsed));
        tor_log(severity, LD_NET, "Average bandwidth: %" PRIu64 "/%d = %d bytes/sec writing",
                (get_bytes_written()), (int)elapsed, (int)(get_bytes_written() / elapsed));
    }
...
handle_signals(void)
{
    ...
    if (enabled)
        SetConsoleCtrlHandler(process_win32_console_ctrl, TRUE);
#endif
}
...
tor_init(int argc, char *argv[])
{
...

    {
        ...
        log_notice(LD_GENERAL,
                   ...
                   version, get_uname(), tor_libevent_get_version_str(), crypto_get_library_name(),
                   crypto_get_library_version_string(),
                   tor_compress_supports_method(ZLIB_METHOD) ? tor_compress_version_str(ZLIB_METHOD)
                                                             : "N/A",
                   tor_compress_supports_method(LZMA_METHOD) ? tor_compress_version_str(LZMA_METHOD)
                                                             : "N/A",
                   tor_compress_supports_method(ZSTD_METHOD) ? tor_compress_version_str(ZSTD_METHOD)
                                                             : "N/A");
...
do_hash_password(void)
{
    ...
    secret_to_key_rfc2440(key + S2K_RFC2440_SPECIFIER_LEN, DIGEST_LEN, get_options()->command_arg,
...
#define RENAME_SUFFIX(name, suffix) \
    sandbox_cfg_allow_rename(&cfg, get_datadir_fname(name suffix), get_datadir_fname(name))
...
#define RENAME_CACHEDIR_SUFFIX(name, suffix) \
    sandbox_cfg_allow_rename(&cfg, get_cachedir_fname(name suffix), get_cachedir_fname(name))

#define RENAME_KEYDIR_SUFFIX(name, suffix) \
    sandbox_cfg_allow_rename(&cfg, get_keydir_fname(name suffix), get_keydir_fname(name))
...
run_tor_main_loop(void)
{
...
#ifdef HAVE_SYSTEMD
    {
        ...
        if (r < 0) {
            log_warn(LD_GENERAL, "Unable to send readiness to systemd: %s", strerror(r));
        } else if (r > 0) {
...
tor_run_main(const tor_main_configuration_t *tor_cfg)
{
    ...
    if (tor_cfg->argc_owned)
        memcpy(argv + tor_cfg->argc, tor_cfg->argv_owned, tor_cfg->argc_owned * sizeof(char *));
