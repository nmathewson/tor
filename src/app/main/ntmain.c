...
struct {
    ...
    (SC_HANDLE hSCManager, LPCSTR lpServiceName, LPCSTR lpDisplayName, DWORD dwDesiredAccess,
    ...
    (LPCSTR lpSystemName, LPCSTR lpAccountName, PSID Sid, LPDWORD cbSid,
     LPTSTR ReferencedDomainName, LPDWORD cchReferencedDomainName, PSID_NAME_USE peUse);
    /** @} */
} service_fns = {0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
...
nt_service_control(DWORD request)
{
    ...
    case SERVICE_CONTROL_SHUTDOWN:
        log_notice(LD_GENERAL, "Got stop/shutdown request; shutting down cleanly.");
...
nt_service_open_scm(void)
{
    ...
    nt_service_loadlibrary();
    if ((hSCManager = service_fns.OpenSCManagerA_fn(NULL, NULL, SC_MANAGER_CREATE_SERVICE)) ==
        NULL) {
...
#    define MAX_SERVICE_WAIT_TIME 10
    ...
    if (service_fns.ControlService_fn(hService, SERVICE_CONTROL_STOP, &service_status)) {
        wait_time = 0;
        while (service_fns.QueryServiceStatus_fn(hService, &service_status) &&
               (service_status.dwCurrentState != SERVICE_STOPPED) &&
...
nt_service_command_line(int *using_default_torrc)
{
    ...
    *using_default_torrc = 1;
    ...
    for (i = 1; i < backup_argc; ++i) {
        if (!strcmp(backup_argv[i], "--options") || !strcmp(backup_argv[i], "-options")) {
            while (++i < backup_argc) {
...
#    endif /* defined(UNICODE) */
    ...
    if (options) {
        tor_asprintf(&command, "\"%s\" --nt-service \"%s\"", tor_exe_ascii, options);
    } else { /* ! options */
...
nt_service_install(int argc, char **argv)
{
    ...
    if (!GetVersionEx((LPOSVERSIONINFO)&info)) {
        ...
        if (info.dwMajorVersion < 5 || (info.dwMajorVersion == 5 && info.dwMinorVersion == 0))
            is_win2k_or_worse = 1;
    }
    ...
    if ((hService = service_fns.CreateServiceA_fn(
             hSCManager, GENSRV_SERVICENAME, GENSRV_DISPLAYNAME, SERVICE_ALL_ACCESS,
             SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START, SERVICE_ERROR_IGNORE, command, NULL,
             NULL, NULL, user_acct, password)) == NULL) {
        errmsg = format_win32_error(GetLastError());
    ...
    service_fns.ChangeServiceConfig2A_fn(hService, SERVICE_CONFIG_DESCRIPTION, &sdBuff);
    ...
    *should_exit = 0;
    ...
    if (argc >= 2) {
        ...
        if (!strcmp(argv[1], "-install") || !strcmp(argv[1], "--install")) {
            nt_service_loadlibrary();
            fprintf(stderr, "The %s option is deprecated; use \"--service install\" instead.",
                    argv[1]);
            *should_exit = 1;
            return nt_service_install(argc, argv);
        }
        if (!strcmp(argv[1], "-remove") || !strcmp(argv[1], "--remove")) {
            nt_service_loadlibrary();
            fprintf(stderr, "The %s option is deprecated; use \"--service remove\" instead.",
                    argv[1]);
            *should_exit = 1;
            return nt_service_remove();
        }
