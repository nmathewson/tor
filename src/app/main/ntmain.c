        ...
        if (result == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) {
            ...
            case CMD_KEY_EXPIRATION:
                log_err(LD_CONFIG, "Unsupported command (--list-fingerprint, "
                                   "--hash-password, --keygen, --dump-config, "
                                   "--verify-config, "
...
nt_service_install(int argc, char **argv)
{
    ...
    if (!user_acct) {
        if (is_win2k_or_worse) {
            /* On Win2k, there is no LocalService account, so we actually need
             * to fall back on NULL (the system account). */
    ...
    if (using_default_torrc)
        printf(
            "IMPORTANT NOTE:\n"
            "    The Tor service will run under the account \"%s\".  This "
            "means\n"
    ...
    *should_exit = 0;
    ...
    if (argc >= 2) {
        ...
        if (!strcmp(argv[1], "-install") || !strcmp(argv[1], "--install")) {
            nt_service_loadlibrary();
            fprintf(stderr,
                    "The %s option is deprecated; use \"--service install\" "
                    "instead.",
                    argv[1]);
            *should_exit = 1;
            return nt_service_install(argc, argv);
        ...
        if (!strcmp(argv[1], "-remove") || !strcmp(argv[1], "--remove")) {
            nt_service_loadlibrary();
            fprintf(stderr,
                    "The %s option is deprecated; use \"--service remove\" "
                    "instead.",
                    argv[1]);
            *should_exit = 1;
            return nt_service_remove();
