    ...
    for (unsigned i = 0; i < n_tor_subsystems; ++i) {
        ...
        if (sys->level < MIN_SUBSYS_LEVEL || sys->level > MAX_SUBSYS_LEVEL) {
            fprintf(stderr,
                    ...
                    sys->name, i, sys->level, MIN_SUBSYS_LEVEL,
                    MAX_SUBSYS_LEVEL);
        ...
        if (sys->level < last_level) {
            fprintf(stderr,
                    ...
                    "Its level is %d; but the previous subsystem's level was "
                    "%d.\n",
...
subsystems_init_upto(int target_level)
{
    ...
    for (unsigned i = 0; i < n_tor_subsystems; ++i) {
        ...
        int r = 0;
        if (sys->initialize) {
            // Note that the logging subsystem is designed so that it does no
            // harm to log a message in an uninitialized state.  These messages
            // will be discarded for now, however.
