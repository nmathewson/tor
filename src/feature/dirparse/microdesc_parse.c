...
microdesc_extract_body(microdesc_t *md, const char *start, const char *s,
                       const char *start_of_next_microdesc, saved_location_t where)
{
...
microdesc_parse_fields(microdesc_t *md, memarea_t *area, const char *s,
                       const char *start_of_next_microdesc, int allow_annotations,
...
smartlist_t *
microdescs_parse_from_string(const char *s, const char *eos, int allow_annotations,
                             saved_location_t where, smartlist_t *invalid_digests_out)
{
    smartlist_t *result;
    microdesc_t *md = NULL;
    memarea_t *area;
    ...
    while (s < eos) {
        ...
        uint8_t md_digest[DIGEST256_LEN];
        {
            const bool body_not_found =
                microdesc_extract_body(md, start, s, start_of_next_microdesc, where) < 0;
        ...
        if (microdesc_parse_fields(md, area, s, start_of_next_microdesc, allow_annotations,
                                   where) == 0) {
