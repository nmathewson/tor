...
{
    /** Reject any certificate at least this big; it is probably an overflow,
     * an attack, a bug, or some other nonsense. */
#define MAX_CERT_SIZE (128 * 1024)
    ...
    if (tok->tp != K_DIR_KEY_CERTIFICATE_VERSION ||
        strcmp(tok->args[0], "3")) {
        log_warn(
            LD_DIR,
    ...
    if (tor_memneq(cert->cache_info.identity_digest, fp_declared,
                   DIGEST_LEN)) {
    ...
    found = 0;
    if (old_cert) {
        /* XXXX We could just compare signed_descriptor_digest, but that
    ...
    if (!found) {
        if (check_signature_token(digest, DIGEST_LEN, tok, cert->identity_key,
                                  0, "key certificate")) {
            goto err;
        }
