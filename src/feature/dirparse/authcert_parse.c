...
authority_cert_t *
authority_cert_parse_from_string(const char *s, size_t maxlen, const char **end_of_string)
{
...
#define MAX_CERT_SIZE (128 * 1024)
    ...
    if (base16_decode(fp_declared, DIGEST_LEN, tok->args[0], strlen(tok->args[0])) != DIGEST_LEN) {
    ...
    if (tok) {
        ...
        if (tor_addr_port_split(LOG_WARN, tok->args[0], &address, &cert->dir_port) < 0 ||
    ...
    old_cert =
        authority_cert_get_by_digests(cert->cache_info.identity_digest, cert->signing_key_digest);
    found = 0;
    if (old_cert) {
    ...
    if (!found) {
        ...
        if (check_signature_token(cert->cache_info.identity_digest, DIGEST_LEN, tok,
                                  cert->signing_key, CST_NO_CHECK_OBJTYPE,
