...
static token_rule_t networkstatus_token_table[] = {
    T1_START("network-status-version", K_NETWORK_STATUS_VERSION, GE(1), NO_OBJ),
    ...
    if ((tok = find_opt_by_keyword(tokens, K_W))) {
        ...
        for (i = 0; i < tok->n_args; ++i) {
            if (!strcmpstart(tok->args[i], "Bandwidth=")) {
                int ok;
                rs->bandwidth_kb =
                    (uint32_t)tor_parse_ulong(strchr(tok->args[i], '=') + 1, 10,
        ...
        if (R + D < S) { // Subcase a
            ...
            if (Rtotal > Stotal) {
                log_warn(LD_DIR,
                         ...
                         " T=%" PRId64 ". "
                         "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                         casename, Rtotal, Stotal, (G), (M), (E), (D), (T), Wgg,
                         Wgd, Wmg, Wme, Wmd, Wee, Wed);
                valid = 0;
            }
            // Rtotal < T/3
            ...
            if (NS >= M) {
                ...
                if (3 * NStotal < T) {
                    log_warn(LD_DIR,
                             ...
                             "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                             casename, NStotal * 3, (T), (G), (M), (E), (D),
                             (T), Wgg, Wgd, Wmg, Wme, Wmd, Wee, Wed);
                    valid = 0;
                }
            }
                                     ...
                                     networkstatus_type_t ns_type)
{
    ...
    if (ns->type != NS_TYPE_CONSENSUS &&
         ...
         * make us invoke undefined behavior whenever we used 1<<flagnum to do a
    ...
    while (eos - s >= 2 && fast_memeq(s, "r ", 2)) {
        if (ns->type != NS_TYPE_CONSENSUS) {
            ...
            if (routerstatus_parse_entry_from_string(rs_area, &s, eos,
                                                     rs_tokens, ns, rs, 0, 0)) {
    ...
    SMARTLIST_FOREACH_BEGIN(footer_tokens, directory_token_t *, _tok)
    {
        ...
        if (networkstatus_get_voter_sig_by_alg(v, sig->alg)) {
            /* We already parsed a vote with this algorithm from this voter. Use
               the first one. */
