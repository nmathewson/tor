                                 ...
                                 routerstatus_t *rs)
{
    ...
    if (!ok) {
        log_warn(LD_DIR, "Invalid GuardFraction %s",
                                     ...
                                     consensus_flavor_t flav)
{
    ...
    if (tor_snprintf(timebuf, sizeof(timebuf), "%s %s", tok->args[3 + offset],
        ...
        log_warn(LD_DIR, "Error parsing time '%s %s' [%d %d]",
                 tok->args[3 + offset], tok->args[4 + offset], offset,
                 (int)flav);
        goto err;
    }
    ...
    rs->or_port = (uint16_t)tor_parse_long(tok->args[6 + offset], 10, 0, 65535,
                                           NULL, NULL);
    rs->dir_port = (uint16_t)tor_parse_long(tok->args[7 + offset], 10, 0,
                                            65535, NULL, NULL);
    ...
    if ((tok = find_opt_by_keyword(tokens, K_W))) {
        ...
        for (i = 0; i < tok->n_args; ++i) {
            if (!strcmpstart(tok->args[i], "Bandwidth=")) {
                ...
                if (!ok) {
                    log_warn(LD_DIR, "Invalid Bandwidth %s",
                             escaped(tok->args[i]));
                    goto err;
                }
                rs->has_bandwidth = 1;
                ...
                if (routerstatus_parse_guardfraction(tok->args[i], vote,
                                                     vote_rs, rs) < 0) {
                    goto err;
                }
            }
    ...
    if (3 * E >= T && 3 * G >= T) {
        ...
        if (fabs(Etotal - Mtotal) > 0.01 * MAX(Etotal, Mtotal)) {
            log_warn(LD_DIR,
                     ...
                     casename, Etotal, Mtotal, (G), (M), (E), (D), (T), Wgg,
        ...
        if (fabs(Etotal - Gtotal) > 0.01 * MAX(Etotal, Gtotal)) {
            log_warn(LD_DIR,
                     ...
                     " T=%" PRId64 ". "
                     "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                     casename, Etotal, Gtotal, (G), (M), (E), (D), (T), Wgg,
                     Wgd, Wmg, Wme, Wmd, Wee, Wed);
            valid = 0;
        }
        if (fabs(Gtotal - Mtotal) > 0.01 * MAX(Gtotal, Mtotal)) {
            log_warn(LD_DIR,
                     ...
                     " T=%" PRId64 ". "
                     "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                     casename, Mtotal, Gtotal, (G), (M), (E), (D), (T), Wgg,
                     Wgd, Wmg, Wme, Wmd, Wee, Wed);
            valid = 0;
        }
    } else if (3 * E < T && 3 * G < T) {
        ...
        if (3 * (S + D) < T) { // Subcase a:
            ...
            if (NS >= M) {
                ...
                // if NS < M, NStotal > T/3 because only one of G or E is
                // scarce
                if (3 * NStotal < T) {
                    log_warn(
                        LD_DIR,
...
extract_shared_random_srvs(networkstatus_t *ns, smartlist_t *tokens)
{
    ...
    if (extract_one_srv(tokens, K_PREVIOUS_SRV, &ns->sr_info.previous_srv) <
        0) {
                                     ...
                                     networkstatus_type_t ns_type)
{
    ...
    if (ns->type == NS_TYPE_VOTE || ns->type == NS_TYPE_OPINION) {
        ...
        if (tok) {
            int num_ok;
            ns->consensus_method = (int)tor_parse_long(tok->args[0], 10, 1,
                                                       INT_MAX, &num_ok, NULL);
            if (!num_ok)
                goto err;
        } else {
...

    {
        ...
        if (package_lst) {
            SMARTLIST_FOREACH(
                package_lst, directory_token_t *, t,
    ...
    if (ns->type != NS_TYPE_CONSENSUS &&
        ...
        /* If we allowed more than 64 flags in votes, then parsing them would
         * make us invoke undefined behavior whenever we used 1<<flagnum to do
         * a bit-shift. This is only for votes and opinions: consensus users
         * don't care about flags they don't recognize, and so don't build a
         * bitfield for them. */
    ...
    SMARTLIST_FOREACH_BEGIN (tokens, directory_token_t *, _tok) {
        ...
        if (tok->tp == K_DIR_SOURCE) {
            ...
            if (ns->type != NS_TYPE_CONSENSUS)
                memcpy(voter->vote_digest, ns_digests.d[DIGEST_SHA1],
                       DIGEST_LEN);

            voter->nickname = tor_strdup(tok->args[0]);
            if (strlen(tok->args[1]) != HEX_DIGEST_LEN ||
                base16_decode(voter->identity_digest,
                              sizeof(voter->identity_digest), tok->args[1],
                              HEX_DIGEST_LEN) !=
            ...
            int ok;
            voter->dir_port = (uint16_t)tor_parse_long(tok->args[4], 10, 0,
                                                       65535, &ok, NULL);
            if (!ok)
                goto err;
            voter->or_port = (uint16_t)tor_parse_long(tok->args[5], 10, 0,
                                                      65535, &ok, NULL);
            if (!ok)
                goto err;
    ...
    while (eos - s >= 2 && fast_memeq(s, "r ", 2)) {
        if (ns->type != NS_TYPE_CONSENSUS) {
            ...
            if (routerstatus_parse_entry_from_string(
                    rs_area, &s, eos, rs_tokens, ns, rs, 0, 0)) {
            ...
            if ((rs = routerstatus_parse_entry_from_string(
                     rs_area, &s, eos, rs_tokens, NULL, NULL,
                     ns->consensus_method, flav))) {
                /* Use exponential-backoff scheduling when downloading
                 * microdescs */
                smartlist_add(ns->routerstatus_list, rs);
            }
        }
    ...
    for (i = 1; i < smartlist_len(ns->routerstatus_list); ++i) {
        ...
        if (ns->type != NS_TYPE_CONSENSUS) {
            vote_routerstatus_t *a =
                smartlist_get(ns->routerstatus_list, i - 1);
        ...
        if (fast_memcmp(rs1->identity_digest, rs2->identity_digest,
                        DIGEST_LEN) >= 0) {
            log_warn(LD_DIR,
    ...
    SMARTLIST_FOREACH_BEGIN (footer_tokens, directory_token_t *, _tok) {
        ...
        if (strlen(sk_hexdigest) != HEX_DIGEST_LEN ||
            base16_decode(sig->signing_key_digest,
                          sizeof(sig->signing_key_digest), sk_hexdigest,
        ...
        if (ns->type != NS_TYPE_CONSENSUS) {
            if (tor_memneq(declared_identity,
                           ns->cert->cache_info.identity_digest, DIGEST_LEN)) {
                log_warn(LD_DIR,
        ...
        if (networkstatus_get_voter_sig_by_alg(v, sig->alg)) {
            /* We already parsed a vote with this algorithm from this voter.
               Use the first one. */
            log_fn(LOG_PROTOCOL_WARN, LD_DIR,
                   "We received a networkstatus "
                   "that contains two signatures from the same voter with the "
                   "same "
        ...
        if (ns->type != NS_TYPE_CONSENSUS) {
            ...
            if (tok->object_size >= INT_MAX ||
                tok->object_size >= SIZE_T_CEILING) {
                tor_free(sig);
                goto err;
            }
...
err:
    ...
    if (footer_tokens) {
        SMARTLIST_FOREACH(footer_tokens, directory_token_t *, t,
                          token_clear(t));
