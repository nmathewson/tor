 ...
 * -1. */
int
router_get_networkstatus_v3_signed_boundaries(const char *s, size_t len, const char **start_out,
                                              const char **end_out)
{
    return router_get_hash_impl_helper(s, len, "network-status-version", "\ndirectory-signature",
                                       ' ', LOG_INFO, start_out, end_out);
}
...
int
router_get_networkstatus_v3_sha3_as_signed(uint8_t *digest_out, const char *s, size_t len)
{
    ...
    tor_assert(start);
    tor_assert(end);
    return crypto_digest256((char *)digest_out, start, end - start, DIGEST_SHA3_256);
}

/** Set <b>digests</b> to all the digests of the consensus document in
 * <b>s</b> */
int
router_get_networkstatus_v3_hashes(const char *s, size_t len, common_digests_t *digests)
{
...
STATIC int
routerstatus_parse_guardfraction(const char *guardfraction_str, networkstatus_t *vote,
                                 vote_routerstatus_t *vote_rs, routerstatus_t *rs)
{
    int ok;
    ...
    guardfraction = (uint32_t)tor_parse_ulong(end_of_header + 1, 10, 0, 100, &ok, NULL);
    if (!ok) {
    ...
    log_debug(LD_GENERAL, "[*] Parsed %s guardfraction '%s' for '%s'.",
              is_consensus ? "consensus" : "vote", guardfraction_str, rs->nickname);
...
routerstatus_parse_entry_from_string(memarea_t *area, const char **s, const char *s_eos,
                                     smartlist_t *tokens, networkstatus_t *vote,
                                     vote_routerstatus_t *vote_rs, int consensus_method,
                                     consensus_flavor_t flav)
{
    ...
    if (tor_snprintf(timebuf, sizeof(timebuf), "%s %s", tok->args[3 + offset],
        ...
        log_warn(LD_DIR, "Error parsing time '%s %s' [%d %d]", tok->args[3 + offset],
                 tok->args[4 + offset], offset, (int)flav);
        goto err;
    }
    ...
    if ((tok = find_opt_by_keyword(tokens, K_W))) {
        ...
        for (i = 0; i < tok->n_args; ++i) {
            if (!strcmpstart(tok->args[i], "Bandwidth=")) {
                int ok;
                rs->bandwidth_kb = (uint32_t)tor_parse_ulong(strchr(tok->args[i], '=') + 1, 10, 0,
                                                             UINT32_MAX, &ok, NULL);
                if (!ok) {
                ...
                int ok;
                vote_rs->measured_bw_kb = (uint32_t)tor_parse_ulong(strchr(tok->args[i], '=') + 1,
                ...
                if (routerstatus_parse_guardfraction(tok->args[i], vote, vote_rs, rs) < 0) {
                    goto err;
                }
            }
        }
    ...
    if (vote_rs) {
        ...
        if (tok) {
            ...
            log_info(LD_BUG,
                     ...
                     rs->nickname, hex_str(rs->identity_digest, DIGEST_LEN), fmt_addr32(rs->addr),
                     rs->or_port);
        }
    }
...
networkstatus_verify_bw_weights(networkstatus_t *ns, int consensus_method)
{
    ...
    if (fabs(Wgg + Wmg - weight_scale) > 0.001 * weight_scale) {
        log_warn(LD_BUG, "Wgg=%f != %" PRId64 " - Wmg=%f", Wgg, (weight_scale), Wmg);
        valid = 0;
    }

    if (fabs(Wee + Wme - weight_scale) > 0.001 * weight_scale) {
        log_warn(LD_BUG, "Wee=%f != %" PRId64 " - Wme=%f", Wee, (weight_scale), Wme);
        valid = 0;
    }

    if (fabs(Wgd + Wmd + Wed - weight_scale) > 0.001 * weight_scale) {
        log_warn(LD_BUG, "Wgd=%f + Wmd=%f + Wed=%f != %" PRId64, Wgd, Wmd, Wed, (weight_scale));
        valid = 0;
    }
    ...
    if (3 * E >= T && 3 * G >= T) {
        ...
        if (fabs(Etotal - Mtotal) > 0.01 * MAX(Etotal, Mtotal)) {
            log_warn(LD_DIR,
                     ...
                     casename, Etotal, Mtotal, (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme, Wmd,
                     Wee, Wed);
            valid = 0;
        }
        if (fabs(Etotal - Gtotal) > 0.01 * MAX(Etotal, Gtotal)) {
            log_warn(LD_DIR,
                     ...
                     "G=%" PRId64 " M=%" PRId64 " E=%" PRId64 " D=%" PRId64 " T=%" PRId64 ". "
                     "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                     casename, Etotal, Gtotal, (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme, Wmd,
                     Wee, Wed);
            valid = 0;
        }
        if (fabs(Gtotal - Mtotal) > 0.01 * MAX(Gtotal, Mtotal)) {
            log_warn(LD_DIR,
                     ...
                     "G=%" PRId64 " M=%" PRId64 " E=%" PRId64 " D=%" PRId64 " T=%" PRId64 ". "
                     "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                     casename, Mtotal, Gtotal, (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme, Wmd,
                     Wee, Wed);
            valid = 0;
        }
    } else if (3 * E < T && 3 * G < T) {
        ...
        if (R + D < S) { // Subcase a
            ...
            if (Rtotal > Stotal) {
                log_warn(LD_DIR,
                         ...
                         "G=%" PRId64 " M=%" PRId64 " E=%" PRId64 " D=%" PRId64 " T=%" PRId64 ". "
                         "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                         casename, Rtotal, Stotal, (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme, Wmd,
                         Wee, Wed);
                valid = 0;
            }
            // Rtotal < T/3
            if (3 * Rtotal > T) {
                log_warn(LD_DIR,
                         ...
                         "%" PRId64 ". G=%" PRId64 " M=%" PRId64 " E=%" PRId64 " D=%" PRId64
                         " T=%" PRId64 ". "
                         "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                         casename, Rtotal * 3, (T), (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme,
                         Wmd, Wee, Wed);
                valid = 0;
            }
            // Stotal < T/3
            if (3 * Stotal > T) {
                log_warn(LD_DIR,
                         ...
                         " T=%" PRId64 ". "
                         "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                         casename, Stotal * 3, (T), (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme,
                         Wmd, Wee, Wed);
                valid = 0;
            }
            // Mtotal > T/3
            if (3 * Mtotal < T) {
                log_warn(LD_DIR,
                         ...
                         "G=%" PRId64 " M=%" PRId64 " E=%" PRId64 " D=%" PRId64 " T=%" PRId64 ". "
                         "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                         casename, Mtotal * 3, (T), (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme,
                         Wmd, Wee, Wed);
                valid = 0;
            }
            ...
            if (D != 0 && 3 * M < T) {
                ...
                if (fabs(Etotal - Mtotal) > 0.01 * MAX(Etotal, Mtotal)) {
                    log_warn(LD_DIR,
                             ...
                             ". "
                             "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                             casename, Etotal, Mtotal, (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme,
                             Wmd, Wee, Wed);
                    valid = 0;
                }
                if (fabs(Etotal - Gtotal) > 0.01 * MAX(Etotal, Gtotal)) {
                    log_warn(LD_DIR,
                             ...
                             ". "
                             "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                             casename, Etotal, Gtotal, (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme,
                             Wmd, Wee, Wed);
                    valid = 0;
                }
                if (fabs(Gtotal - Mtotal) > 0.01 * MAX(Gtotal, Mtotal)) {
                    log_warn(LD_DIR,
                             ...
                             ". "
                             "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                             casename, Mtotal, Gtotal, (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme,
                             Wmd, Wee, Wed);
                    valid = 0;
                }
            } else {
            ...
            if (NS >= M) {
                if (fabs(NStotal - Mtotal) > 0.01 * MAX(NStotal, Mtotal)) {
                    log_warn(LD_DIR,
                             ...
                             ". "
                             "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                             casename, NStotal, Mtotal, (G), (M), (E), (D), (T), Wgg, Wgd, Wmg, Wme,
                             Wmd, Wee, Wed);
                    valid = 0;
                }
            } else {
                ...
                if (3 * NStotal < T) {
                    log_warn(LD_DIR,
                             ...
                             " T=%" PRId64 ". "
                             "Wgg=%f Wgd=%f Wmg=%f Wme=%f Wmd=%f Wee=%f Wed=%f",
                             casename, NStotal * 3, (T), (G), (M), (E), (D), (T), Wgg, Wgd, Wmg,
                             Wme, Wmd, Wee, Wed);
                    valid = 0;
                }
            }
...
extract_shared_random_srvs(networkstatus_t *ns, smartlist_t *tokens)
{
    ...
    if (ns->type == NS_TYPE_VOTE) {
        ...
        tor_assert(voter);
        voter_identity = hex_str(voter->identity_digest, sizeof(voter->identity_digest));
    } else {
    ...
    if (extract_one_srv(tokens, K_PREVIOUS_SRV, &ns->sr_info.previous_srv) < 0) {
        log_warn(LD_DIR, "SR: Unable to parse previous SRV from %s", voter_identity);
...
networkstatus_t *
networkstatus_parse_vote_from_string(const char *s, size_t s_len, const char **eos_out,
                                     networkstatus_type_t ns_type)
{
    ...
    if (router_get_networkstatus_v3_hashes(s, s_len, &ns_digests) ||
        router_get_networkstatus_v3_sha3_as_signed(sha3_as_signed, s, s_len) < 0) {
    ...
    if (tokenize_string(area, s, end_of_header, tokens,
                        (ns_type == NS_TYPE_CONSENSUS) ? networkstatus_consensus_token_table
    ...
    if (ns_type != NS_TYPE_CONSENSUS) {
        ...
        if (!(cert = tor_memstr(s, end_of_header - s, "\ndir-key-certificate-version")))
            goto err;
        ++cert;
        ns->cert = authority_cert_parse_from_string(cert, end_of_header - cert, &end_of_cert);
    ...
    if (ns->type == NS_TYPE_VOTE || ns->type == NS_TYPE_OPINION) {
        ...
        if (tok) {
            int num_ok;
            ns->consensus_method = (int)tor_parse_long(tok->args[0], 10, 1, INT_MAX, &num_ok, NULL);
            if (!num_ok)
                goto err;
        } else {
    ...
    if (ns->valid_after +
            (get_options()->TestingTorNetwork ? MIN_VOTE_INTERVAL_TESTING : MIN_VOTE_INTERVAL) >
        ns->fresh_until) {
        log_warn(LD_DIR, "Vote/consensus freshness interval is too short");
        goto err;
    }
    if (ns->valid_after +
            (get_options()->TestingTorNetwork ? MIN_VOTE_INTERVAL_TESTING : MIN_VOTE_INTERVAL) * 2 >
        ns->valid_until) {
    ...
    SMARTLIST_FOREACH_BEGIN (tokens, directory_token_t *, _tok) {
        ...
        if (tok->tp == K_DIR_SOURCE) {
            ...
            if (strlen(tok->args[1]) != HEX_DIGEST_LEN ||
                base16_decode(voter->identity_digest, sizeof(voter->identity_digest), tok->args[1],
            ...
            if (ns->type != NS_TYPE_CONSENSUS && tor_memneq(ns->cert->cache_info.identity_digest,
            ...
            if (strlen(tok->args[0]) != HEX_DIGEST_LEN ||
                base16_decode(voter->vote_digest, sizeof(voter->vote_digest), tok->args[0],
    ...
    while (eos - s >= 2 && fast_memeq(s, "r ", 2)) {
        if (ns->type != NS_TYPE_CONSENSUS) {
            ...
            if (routerstatus_parse_entry_from_string(rs_area, &s, eos, rs_tokens, ns, rs, 0, 0)) {
            ...
            routerstatus_t *rs;
            if ((rs = routerstatus_parse_entry_from_string(rs_area, &s, eos, rs_tokens, NULL, NULL,
                                                           ns->consensus_method, flav))) {
    ...
    SMARTLIST_FOREACH_BEGIN (footer_tokens, directory_token_t *, _tok) {
        ...
        if (!tok->object_type || strcmp(tok->object_type, "SIGNATURE") || tok->object_size < 128 ||
            tok->object_size > 512) {
        ...
        if (strlen(id_hexdigest) != HEX_DIGEST_LEN ||
            base16_decode(declared_identity, sizeof(declared_identity), id_hexdigest,
        ...
        if (strlen(sk_hexdigest) != HEX_DIGEST_LEN ||
            base16_decode(sig->signing_key_digest, sizeof(sig->signing_key_digest), sk_hexdigest,
        ...
        if (ns->type != NS_TYPE_CONSENSUS) {
            if (check_signature_token(ns_digests.d[DIGEST_SHA1], DIGEST_LEN, tok,
                                      ns->cert->signing_key, 0, "network-status document")) {
                tor_free(sig);
                goto err;
            }
