...
dump_desc_create_dir(void)
{
    ...
    if (!have_dump_desc_dir) {
        ...
        if (check_private_dir(dump_desc_dir, CPD_CREATE, get_options()->User) < 0) {
            log_notice(LD_DIR, "Failed to create descriptor dump directory %s", dump_desc_dir);
            problem_with_dump_desc_dir = 1;
        }
 ...
 */
static void
dump_desc_fifo_add_and_clean(char *filename, const uint8_t *digest_sha256, size_t len)
{
    ...
    while (len > max_len - len_descs_dumped && smartlist_len(descs_dumped) > 0) {
        ...
        if (strcmp(tmp->filename, filename) != 0) {
            ...
            tor_assert(len_descs_dumped >= tmp->len);
            len_descs_dumped -= tmp->len;
            log_info(LD_DIR, "Replacing old descriptor dump %s with new identical one",
                     tmp->filename);
        }
...
dump_desc_populate_one_file, (const char *dirname, const char *f))
{
    ...
    if (!strcmpstart(f, f_pfx)) {
        ...
        if (base16_decode(digest, DIGEST256_LEN, digest_str, strlen(digest_str)) != DIGEST256_LEN) {
     ...
     * filename.
     */
    if (crypto_digest256((char *)content_digest, desc, (size_t)st.st_size, DIGEST_SHA256) < 0) {
        /* Weird, but okay */
        log_info(LD_DIR,
...
dump_desc, (const char *desc, const char *type))
{
    ...
    base16_encode(digest_sha256_hex, sizeof(digest_sha256_hex), (const char *)digest_sha256,
     ...
     */
    tor_asprintf(&debugfile_base, DESC_DUMP_BASE_FILENAME ".%s", digest_sha256_hex);
            ...
            if (!dump_desc_fifo_bump_hash(digest_sha256)) {
                ...
                if (have_dump_desc_dir && !problem_with_dump_desc_dir) {
                    ...
                    log_info(LD_DIR,
                             ...
                             "problem creating the " DESC_DUMP_DATADIR_SUBDIR " subdirectory",
