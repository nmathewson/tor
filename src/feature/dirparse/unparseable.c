          ...
          (const char *dirname, const char *f))
{
    ...
    if (!strcmpstart(f, f_pfx)) {
        ...
        if (base16_decode(digest, DIGEST256_LEN, digest_str,
                          strlen(digest_str)) != DIGEST256_LEN) {
...
dump_desc_compare_fifo_entries(const void **a_v, const void **b_v)
{
    ...
    if ((a != NULL) && (*a != NULL)) {
        if ((b != NULL) && (*b != NULL)) {
            ...
            /*
             * We shouldn't see this, but what the hell, NULLs precede
             * everythin else
             */
            return 1;
        }
...
dump_desc, (const char *desc, const char *type))
{
    ...
    len = strlen(desc);
    if (crypto_digest256((char *)digest_sha256, desc, len, DIGEST_SHA256) <
        0) {
        log_info(
            LD_DIR,
    ...
    if (!(sandbox_is_active() || get_options()->Sandbox)) {
        if (len <= get_options()->MaxUnparseableDescSizeToLog) {
            if (!dump_desc_fifo_bump_hash(digest_sha256)) {
                ...
                if (have_dump_desc_dir && !problem_with_dump_desc_dir) {
                    ...
                    log_info(LD_DIR,
                             "Unable to parse descriptor of type %s with hash "
                             "%s and "
                             "length %lu. See file %s in data directory for "
                             "details.",
                             type, digest_sha256_hex, (unsigned long)len,
                             debugfile_base);
                    dump_desc_fifo_add_and_clean(debugfile, digest_sha256,
                                                 len);
                    /* Since we handed ownership over, don't free debugfile
                     * later */
                    debugfile = NULL;
                } else {
                    ...
                    log_info(
                        LD_DIR,
                        "Unable to parse descriptor of type %s with hash %s "
                        "and "
         ...
         * doesn't have a sensible way to allow filenames according to a
         * pattern match.  (If we ever figure out how to say "allow writes to
         * /regex/", remove this checK).
         */
        log_info(LD_DIR,
                 "Unable to parse descriptor of type %s with hash %s and "
