                ...
                smartlist_t *out, const token_rule_t *table, int flags)
{
    ...
    if (!end) {
        ...
        /* it's only meaningful to check for nuls if we got an end-of-string
         * ptr */
    ...
    for (i = 0; table[i].t; ++i) {
        ...
        if (table[i].pos & AT_START) {
            if (smartlist_len(out) < 1 ||
                (tok = smartlist_get(out, first_nonannotation))->tp !=
                    table[i].v) {
                log_warn(LD_DIR, "Parse error: first item is not %s.",
                         table[i].t);
                return -1;
            }
        }
        if (table[i].pos & AT_END) {
            if (smartlist_len(out) < 1 ||
                (tok = smartlist_get(out, smartlist_len(out) - 1))->tp !=
                    table[i].v) {
                log_warn(LD_DIR, "Parse error: last item is not %s.",
                         table[i].t);
                return -1;
            }
        }
                   ...
                   obj_syntax o_syn)
{
    ...
    case NO_OBJ:
        ...
        if (tok->key) {
            tor_snprintf(ebuf, sizeof(ebuf), "Unexpected public key for %s",
                         kwd);
            RET_ERR(ebuf);
        }
        break;
    ...
    case NEED_SKEY_1024: /* There must be a 1024-bit private key. */
        if (tok->key && crypto_pk_num_bits(tok->key) != PK_BYTES * 8) {
            tor_snprintf(ebuf, sizeof(ebuf),
                         "Wrong size on key for %s: %d bits", kwd,
                         crypto_pk_num_bits(tok->key));
            RET_ERR(ebuf);
        }
        FALLTHROUGH;
...
#define MAX_LINE_LENGTH (128 * 1024)
    ...
    *s = eat_whitespace_eos(*s, eos); /* eat multi-line whitespace */
    ...
    for (i = 0; table[i].t; ++i) {
        if (mem_eq_token(*s, next - *s, table[i].t)) {
            ...
            o_syn = table[i].os;
            *s = eat_whitespace_eos_no_nl(next, eol);
            /* We go ahead whether there are arguments or not, so that
             * tok->args is always set if we want arguments. */
            if (table[i].concat_args) {
                /* The keyword takes the line as a single argument */
                tok->args = ALLOC(sizeof(char *));
                tok->args[0] =
                    STRNDUP(*s, eol - *s); /* Grab everything on line */
                tok->n_args = 1;
            } else {
            ...
            if (tok->n_args < table[i].min_args) {
                tor_snprintf(ebuf, sizeof(ebuf), "Too few arguments to %s",
                             kwd);
                RET_ERR(ebuf);
            } else if (tok->n_args > table[i].max_args) {
                tor_snprintf(ebuf, sizeof(ebuf), "Too many arguments to %s",
                             kwd);
                RET_ERR(ebuf);
            }
            break;
    ...
    *s = eol + 1; /* Set *s to possible start of object data (could be eos) */
    ...
    } else if (!strcmp(tok->object_type,
