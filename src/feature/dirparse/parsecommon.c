 ...
 */
int
tokenize_string(memarea_t *area, const char *start, const char *end, smartlist_t *out,
...
static inline int
get_token_arguments(memarea_t *area, directory_token_t *tok, const char *s, const char *eol)
{
...
token_check_object(memarea_t *area, const char *kwd, directory_token_t *tok, obj_syntax o_syn)
{
    char ebuf[128];
    switch (o_syn) {
    case NO_OBJ:
    ...
    case NEED_SKEY_1024: /* There must be a 1024-bit private key. */
        if (tok->key && crypto_pk_num_bits(tok->key) != PK_BYTES * 8) {
            tor_snprintf(ebuf, sizeof(ebuf), "Wrong size on key for %s: %d bits", kwd,
 ...
 */
directory_token_t *
get_next_token(memarea_t *area, const char **s, const char *eos, const token_rule_t *table)
{
...
#define MAX_LINE_LENGTH (128 * 1024)
    ...
    *s = eol + 1; /* Set *s to possible start of object data (could be eos) */
    ...
    if (!strcmp(tok->object_type, "RSA PUBLIC KEY")) { /* If it's a public key */
        ...
        tok->key = crypto_pk_asn1_decode_private(tok->object_body, tok->object_size, 1024);
        if (!tok->key)
 ...
 */
directory_token_t *
find_by_keyword_(smartlist_t *s, directory_keyword keyword, const char *keyword_as_string)
{
