                   ...
                   obj_syntax o_syn)
{
    ...
    switch (o_syn) {
        case NO_OBJ:
            ...
            if (tok->object_body) {
                tor_snprintf(ebuf, sizeof(ebuf), "Unexpected object for %s",
                             kwd);
                RET_ERR(ebuf);
            }
            if (tok->key) {
        ...
        case NEED_KEY: /* There must be some kind of key. */
            if (!tok->key) {
                tor_snprintf(ebuf, sizeof(ebuf), "Missing public key for %s",
                             kwd);
                RET_ERR(ebuf);
            }
    ...
    *s = eat_whitespace_eos(*s, eos); /* eat multi-line whitespace */
    ...
    for (i = 0; table[i].t; ++i) {
        if (mem_eq_token(*s, next - *s, table[i].t)) {
            ...
            o_syn = table[i].os;
            *s = eat_whitespace_eos_no_nl(next, eol);
            /* We go ahead whether there are arguments or not, so that tok->args
             * is always set if we want arguments. */
