          ...
          (const char *s, int assume_action, int *malformed_list))
{
     ...
     * IPv6 address. But making the buffer shorter might cause valid long
     * lines, which parsed in previous versions, to fail to parse in new
     * versions. (These lines would have to have excessive amounts of
     * whitespace.) */
    ...
    *malformed_list = 0;
    ...
    if ((*s == '*' || *s == '[' || TOR_ISDIGIT(*s)) && assume_action >= 0) {
        if (tor_snprintf(line, sizeof(line), "%s %s",
                         assume_action == ADDR_POLICY_ACCEPT ? "accept"
                                                             : "reject",
                         s) < 0) {
