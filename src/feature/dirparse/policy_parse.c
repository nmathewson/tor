          ...
          (const char *s, int assume_action, int *malformed_list))
{
    ...
    *malformed_list = 0;
    ...
    if ((tok->tp == K_ACCEPT6 || tok->tp == K_REJECT6) && tor_addr_family(&r->addr) != AF_INET6) {
...
router_parse_addr_policy(directory_token_t *tok, unsigned fmt_flags)
{
    ...
    tor_assert(tok->tp == K_REJECT || tok->tp == K_REJECT6 || tok->tp == K_ACCEPT ||
               tok->tp == K_ACCEPT6);
    ...
    if (tor_addr_parse_mask_ports(arg, fmt_flags, &newe.addr, &newe.maskbits, &newe.prt_min,
                                  &newe.prt_max) < 0) {
