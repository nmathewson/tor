                              ...
                              smartlist_t *invalid_digests_out)
{
    ...
    while (1) {
        ...
        int dl_again = 0;
        if (find_start_of_next_router_or_extrainfo(s, eos, &have_extrainfo) <
            0)
            break;
        ...
        if (have_extrainfo && want_extrainfo) {
            ...
            have_raw_digest =
                router_get_router_hash(*s, end - *s, raw_digest) == 0;
        ...
        if (!elt && !dl_again && have_raw_digest && invalid_digests_out) {
            smartlist_add(invalid_digests_out,
                          tor_memdup(raw_digest, DIGEST_LEN));
        }
        if (!elt) {
            *s = end;
            continue;
                               ...
                               int *can_dl_again_out)
{
    ...
    }
    {
        ...
        if (tokenize_string(area, s, end, tokens, routerdesc_token_table,
                            flags)) {
    ...
    router->cache_info.annotations_len =
        s - start_of_annotations + prepend_len;
    ...
    if (!ok) {
        log_warn(LD_DIR, "Invalid bandwidthcapacity %s",
                 escaped(tok->args[1]));
        goto err;
    }
    ...
    if ((tok = find_opt_by_keyword(tokens, K_UPTIME))) {
        ...
        router->uptime =
            tor_parse_long(tok->args[0], 10, 0, LONG_MAX, &ok, NULL);
        if (!ok) {
...

    {
        ...
        if (ed_sig_tok) {
            ...
            if (ed_sig_tok !=
                smartlist_get(tokens, smartlist_len(tokens) - 2)) {
            ...
            tor_cert_t *cert =
                tor_cert_parse((const uint8_t *)ed_cert_tok->object_body,
            ...
            if (master_key_tok) {
                ...
                if (ed25519_public_from_base64(&pkey,
            ...
            ntor_cc_cert =
                tor_cert_parse((const uint8_t *)cc_ntor_tok->object_body,
            ...
            if (ed25519_public_key_from_curve25519_public_key(
                    &ntor_cc_pk, router->onion_curve25519_pkey,
                    ntor_cc_sign_bit) < 0) {
                log_warn(LD_DIR, "Error converting onion key to ed25519");
                goto err;
            }

            if (router_get_hash_impl_helper(
                    s, end - s, "router ", "\nrouter-sig-ed25519", ' ',
            ...
            crypto_digest_add_bytes(d, signed_start,
            ...
            if (tor_cert_get_checkable_sig(&check[0], cert, NULL, &expires) <
                0) {
                log_err(LD_BUG, "Couldn't create 'checkable' for cert.");
                goto err;
            }
            if (tor_cert_get_checkable_sig(&check[1], ntor_cc_cert,
                                           &ntor_cc_pk, &expires) < 0) {
                log_err(LD_BUG,
            ...
            if (check_tap_onion_key_crosscert(
                    ...
                    (int)cc_tap_tok->object_size, rsa_pubkey,
                    &cert->signing_key,
    ...
    if ((tok = find_opt_by_keyword(tokens, K_EXTRA_INFO_DIGEST))) {
        ...
        if (strlen(tok->args[0]) == HEX_DIGEST_LEN) {
            ...
            log_warn(LD_DIR, "Invalid extra info digest %s",
                     escaped(tok->args[0]));
        }
    ...
    can_dl_again = 1;
    if (check_signature_token(digest, DIGEST_LEN, tok, router->identity_pkey,
                              0, "router descriptor") < 0)
        goto err;
