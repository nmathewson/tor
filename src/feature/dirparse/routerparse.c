...
static int
find_start_of_next_router_or_extrainfo(const char **s_ptr, const char *eos, int *is_extrainfo_out)
{
 ...
 */
int
router_parse_list_from_string(const char **s, const char *eos, smartlist_t *dest,
                              saved_location_t saved_location, int want_extrainfo,
                              int allow_annotations, const char *prepend_annotations,
                              smartlist_t *invalid_digests_out)
{
    ...
    while (1) {
        ...
        if (have_extrainfo && want_extrainfo) {
            ...
            extrainfo = extrainfo_parse_entry_from_string(*s, end, saved_location != SAVED_IN_CACHE,
            ...
            router =
                router_parse_entry_from_string(*s, end, saved_location != SAVED_IN_CACHE,
                                               allow_annotations, prepend_annotations, &dl_again);
            if (router) {
                log_debug(LD_DIR, "Read router '%s', purpose '%s'", router_describe(router),
...
int
find_single_ipv6_orport(const smartlist_t *list, tor_addr_t *addr_out, uint16_t *port_out)
{
    int ret = 0;
    ...
    SMARTLIST_FOREACH_BEGIN (list, directory_token_t *, t) {
        ...
        if (tor_addr_parse_mask_ports(t->args[0], 0, &a, &bits, &port_min, &port_max) == AF_INET6 &&
                               ...
                               int *can_dl_again_out)
{
    ...
    if (prepend_annotations) {
        if (tokenize_string(area, prepend_annotations, NULL, tokens, routerdesc_token_table,
                            TS_NOCHECK)) {
    ...
    if (start_of_annotations != s) { /* We have annotations */
        if (allow_annotations) {
            if (tokenize_string(area, start_of_annotations, s, tokens, routerdesc_token_table,
                                TS_NOCHECK)) {
                log_warn(LD_DIR, "Error tokenizing router descriptor (annotations).");
                goto err;
            }
    ...
    if (cache_copy) {
        ...
        char *signed_body = router->cache_info.signed_descriptor_body = tor_malloc(len + 1);
        if (prepend_annotations) {
    ...
    {
        directory_token_t *ed_sig_tok, *ed_cert_tok, *cc_tap_tok, *cc_ntor_tok, *master_key_tok;
        ...
        int n_ed_toks = !!ed_sig_tok + !!ed_cert_tok + !!cc_tap_tok + !!cc_ntor_tok;
        ...
        if (ed_sig_tok) {
            ...
            if (strcmp(cc_ntor_tok->args[0], "0") && strcmp(cc_ntor_tok->args[0], "1")) {
            ...
            tor_cert_t *cert =
                tor_cert_parse((const uint8_t *)ed_cert_tok->object_body, ed_cert_tok->object_size);
            if (!cert) {
            ...
            if (cert->cert_type != CERT_TYPE_ID_SIGNING || !cert->signing_key_included) {
            ...
            if (master_key_tok) {
                ...
                if (fast_memneq(&cert->signing_key.pubkey, pkey.pubkey, ED25519_PUBKEY_LEN)) {
            ...
            ntor_cc_cert =
                tor_cert_parse((const uint8_t *)cc_ntor_tok->object_body, cc_ntor_tok->object_size);
            if (!ntor_cc_cert) {
            ...
            if (tor_cert_get_checkable_sig(&check[1], ntor_cc_cert, &ntor_cc_pk, &expires) < 0) {
                log_err(LD_BUG, "Couldn't create 'checkable' for ntor_cc_cert.");
                goto err;
            }

            if (ed25519_signature_from_base64(&check[2].signature, ed_sig_tok->args[0]) < 0) {
            ...
            rsa_pubkey = router_get_rsa_onion_pkey(router->onion_pkey, router->onion_pkey_len);
            if (check_tap_onion_key_crosscert(
                    (const uint8_t *)cc_tap_tok->object_body, (int)cc_tap_tok->object_size,
                    rsa_pubkey, &cert->signing_key,
    ...
    if (policy_is_reject_star(router->exit_policy, AF_INET, 1) &&
        (!router->ipv6_exit_policy || short_policy_is_reject_star(router->ipv6_exit_policy)))
    ...
    if ((tok = find_opt_by_keyword(tokens, K_EXTRA_INFO_DIGEST))) {
        ...
        if (strlen(tok->args[0]) == HEX_DIGEST_LEN) {
            if (base16_decode(router->cache_info.extra_info_digest, DIGEST_LEN, tok->args[0],
        ...
        if (tok->n_args >= 2) {
            if (digest256_from_base64(router->cache_info.extra_info_digest256, tok->args[1]) < 0) {
                log_warn(LD_DIR, "Invalid extra info digest256 %s", escaped(tok->args[1]));
            }
        }
    }
 ...
 */
extrainfo_t *
extrainfo_parse_entry_from_string(const char *s, const char *end, int cache_copy,
                                  struct digest_ri_map_t *routermap, int *can_dl_again_out)
{
    ...
    if (strlen(tok->args[1]) != HEX_DIGEST_LEN ||
        base16_decode(extrainfo->cache_info.identity_digest, DIGEST_LEN, tok->args[1],
                      HEX_DIGEST_LEN) != DIGEST_LEN) {
...

    {
        ...
        if (ed_sig_tok) {
            ...
            if (ed25519_signature_from_base64(&check[1].signature, ed_sig_tok->args[0]) < 0) {
                log_warn(LD_DIR, "Couldn't decode ed25519 signature");
                goto err;
            }
    ...
    if (routermap &&
        (router = digestmap_get((digestmap_t *)routermap, extrainfo->cache_info.identity_digest))) {
...
find_all_exitpolicy(smartlist_t *s)
{
    ...
    SMARTLIST_FOREACH(s, directory_token_t *, t,
                      if (t->tp == K_ACCEPT || t->tp == K_ACCEPT6 || t->tp == K_REJECT ||
                          t->tp == K_REJECT6) smartlist_add(out, t));
    return out;
}
