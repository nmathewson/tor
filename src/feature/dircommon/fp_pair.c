...
static inline int
fp_pair_map_entries_eq(const fp_pair_map_entry_t *a, const fp_pair_map_entry_t *b)
{
...
HT_PROTOTYPE(fp_pair_map_impl, fp_pair_map_entry_t, node, fp_pair_map_entry_hash,
             fp_pair_map_entries_eq);
HT_GENERATE2(fp_pair_map_impl, fp_pair_map_entry_t, node, fp_pair_map_entry_hash,
             fp_pair_map_entries_eq, 0.6, tor_reallocarray_, tor_free_);
...
void *
fp_pair_map_set_by_digests(fp_pair_map_t *map, const char *first, const char *second, void *val)
{
    fp_pair_t k;
...
void *
fp_pair_map_get_by_digests(const fp_pair_map_t *map, const char *first, const char *second)
{
    fp_pair_t k;
...
fp_pair_map_free_(fp_pair_map_t *map, void (*free_val)(void *))
{
    ...
    if (map) {
        for (ent = HT_START(fp_pair_map_impl, &(map->head)); ent != NULL; ent = next) {
            this = *ent;
...
void
fp_pair_map_iter_get(fp_pair_map_iter_t *iter, fp_pair_t *key_out, void **val_out)
{
    tor_assert(iter);
    tor_assert(*iter);
