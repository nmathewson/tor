...
struct dir_connection_t {
    ...
    /** Which 'resource' did we ask the directory for? This is typically the
     *part of the URL string that defines, relative to the directory conn
     *purpose, what thing we want.  For example, in router descriptor downloads
     *by descriptor digest, it contains "d/", then one or more +-separated
     * fingerprints.
     **/
    ...
    /* Hidden service connection identifier for dir connections: Used by HS
       client-side code to fetch HS descriptors, and by the service-side code
       to upload descriptors. */
    struct hs_ident_dir_conn_t *hs_ident;

    /** If this is a one-hop connection, tracks the state of the directory
     * guard for this connection (if any). */
     ...
     * that's going away and being used on channels instead.  The dirserver
     * still needs this for the incoming side, so it's moved here. */
    uint64_t dirreq_id;
