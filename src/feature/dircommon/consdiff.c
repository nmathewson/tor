            ...
            const smartlist_slice_t *slice1, const smartlist_slice_t *slice2)
{
    ...
    if (slice1->len == 1) {
        const cdline_t *line_common =
            smartlist_get(slice1->list, slice1->offset);
             ...
             bitarray_t *changed1, bitarray_t *changed2)
{
    ...
    if (slice1->len <= 1) {
        ...
        top = smartlist_slice(slice1->list, slice1->offset,
                              slice1->offset + mid);
        ...
        left = smartlist_slice(slice2->list, slice2->offset,
                               slice2->offset + mid2);
            ...
            memarea_t *area)
{
    ...
    /* Initialize the changed bitarrays to zero, so that calc_changes only
     * needs to set the ones that matter and leave the rest untouched.
     */
    bitarray_t *changed1 = bitarray_init_zero(len1);
    bitarray_t *changed2 = bitarray_init_zero(len2);
    int i1 = -1, i2 = -1;
    ...
    /* i1 and i2 are initialized at the first line of each consensus. They
     * never reach past len1 and len2 respectively, since next_router doesn't
     * let that happen. i1 and i2 are advanced by at least one line at each
     * iteration as long as they have not yet reached len1 and len2, so the
     * loop is guaranteed to end, and each pair of (i1,i2) will be inspected at
     * most once.
     */
    while (i1 < len1 || i2 < len2) {
         ...
         */
        if (i1 < len1 || i2 < len2) {
            /* Keep on advancing the lower (by identity hash sorting) position
             * until we have two matching positions. The only other possible
             * outcome is that a lower position reaches the end of the
             * consensus before it can reach a hash that is no longer the lower
             * one. Since there will always be a lower hash for as long as the
             * loop runs, one of the two indexes will always be incremented,
             * thus assuring that the loop must end after a finite number of
             * iterations. If that cannot be because said consensus has already
             * reached the end, both are extended to their respecting ends
             * since we are done.
             */
            int cmp = base64cmp(&iter1.hash, &iter2.hash);
            while (cmp != 0) {
                if (i1 < len1 && cmp < 0) {
                    if (find_next_router_line(cons1, "base", &i1, &iter1) <
                        0) {
                        goto error_cleanup;
                    }
                    if (i1 == len1) {
                        /* We finished the first consensus, so grab all the
                         * remaining lines of the second consensus and finish
                         * up.
                         */
                        i2 = len2;
                        break;
                    ...
                    if (find_next_router_line(cons2, "target", &i2, &iter2) <
                        0) {
                        goto error_cleanup;
                    }
                    if (i2 == len2) {
                        /* We finished the second consensus, so grab all the
                         * remaining lines of the first consensus and finish
                         * up.
                         */
                        i1 = len1;
                        break;
         ...
         * Error if any of the two slices are longer than 10K lines. That
         * should never happen with any pair of real consensuses. Feeding more
         * than 10K lines to calc_changes would be very slow anyway.
         */
#define MAX_LINE_COUNT (10000)
    ...
    while (i1 >= 0 || i2 >= 0) {
        ...
        /* We are at a point were no changed bools are true, so just keep
         * going. */
        ...
        if (action == 'a' || action == 'c') {
            ...
            while (i < diff_len) {
                ...
                if (++i == diff_len) {
                    log_warn(LD_CONSDIFF,
                             "Could not apply consensus diff because "
                             "it has lines to be inserted that don't end with "
                             "a \".\".");
                    goto error_cleanup;
                }
            }
                  ...
                  const consensus_digest_t *digests2, memarea_t *area)
{
    ...
    if (!ed_cons2) {
        ...
        log_warn(LD_BUG | LD_CONSDIFF,
                 "Refusing to generate consensus diff because "
                 "the generated ed diff could not be tested to successfully "
                 "generate "
