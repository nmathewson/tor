 ...
 */
int
add_rsa_fingerprint_to_dir(const char *fp, authdir_config_t *list, rtr_flags_t add_status)
{
    char *fingerprint;
    char d[DIGEST_LEN];
    rtr_flags_t *status;
...
dirserv_load_fingerprint_file(void)
{
    ...
    log_info(LD_GENERAL, "Reloading approved fingerprints from \"%s\"...", fname);
    ...
    for (list = front; list; list = list->next) {
        ...
        if (strlen(fingerprint) == HEX_DIGEST_LEN) {
            rsa_not_ok = add_rsa_fingerprint_to_dir(fingerprint, fingerprint_list_new, add_status);
        }
        ...
        if (strlen(fingerprint) == BASE64_DIGEST256_LEN) {
            if (!digest256_from_base64((char *)ed25519_pubkey_tmp.pubkey, fingerprint)) {
                ed25519_not_ok =
                    add_ed25519_to_dir(&ed25519_pubkey_tmp, fingerprint_list_new, add_status);
            }
        }
...
uint32_t
dirserv_router_get_status(const routerinfo_t *router, const char **msg, int severity)
{
    char d[DIGEST_LEN];
    ...
    r = dirserv_get_status_impl(d, signing_key, router->nickname, router->addr, router->or_port,
    ...
    if (router->cache_info.signing_key_cert) {
        ...
        /* No ed25519 key */
        if (KEYPIN_MISMATCH ==
            keypin_check_lone_rsa((const uint8_t *)router->cache_info.identity_digest)) {
            log_fn(severity, LD_DIR,
...
int
dirserv_would_reject_router(const routerstatus_t *rs, const vote_routerstatus_t *vrs)
{
    uint32_t res;
    struct ed25519_public_key_t pk;
    memcpy(&pk.pubkey, vrs->ed25519_id, ED25519_PUBKEY_LEN);

    res = dirserv_get_status_impl(rs->identity_digest, &pk, rs->nickname, rs->addr, rs->or_port,
                                  NULL, NULL, LOG_DEBUG);
 ...
 */
static uint32_t
dirserv_get_status_impl(const char *id_digest, const ed25519_public_key_t *ed25519_public_key,
                        const char *nickname, uint32_t addr, uint16_t or_port, const char *platform,
                        const char **msg, int severity)
{
    ...
    status_by_digest = digestmap_get(fingerprint_list->status_by_digest, id_digest);
    if (status_by_digest)
        result |= *status_by_digest;

    if (ed25519_public_key) {
        status_by_digest =
            digest256map_get(fingerprint_list->status_by_digest256, ed25519_public_key->pubkey);
        if (status_by_digest)
            result |= *status_by_digest;
    }
    ...
    if (authdir_policy_badexit_address(addr, or_port)) {
        log_fn(severity, LD_DIRSERV, "Marking '%s' as bad exit because of address '%s'", nickname,
               fmt_addr32(addr));
        result |= RTR_BADEXIT;
    }

    if (!authdir_policy_permits_address(addr, or_port)) {
        log_fn(severity, LD_DIRSERV, "Rejecting '%s' because of address '%s'", nickname,
               fmt_addr32(addr));
        if (msg)
    ...
    if (!authdir_policy_valid_address(addr, or_port)) {
        log_fn(severity, LD_DIRSERV, "Not marking '%s' valid because of address '%s'", nickname,
               fmt_addr32(addr));
        result |= RTR_INVALID;
    }
...
dirserv_router_has_valid_address(routerinfo_t *ri)
{
    ...
    if (tor_addr_is_null(&addr) || tor_addr_is_internal(&addr, 0)) {
        log_info(LD_DIRSERV, "Router %s published internal IPv4 address. Refusing.",
                 router_describe(ri));
    ...
    if (!tor_addr_is_null(&ri->ipv6_addr) && tor_addr_is_internal(&ri->ipv6_addr, 0)) {
        log_info(LD_DIRSERV, "Router %s published internal IPv6 address. Refusing.",
                 router_describe(ri));
        return -1; /* it's a private IP, we should reject it */
    }
 ...
 */
int
authdir_wants_to_reject_router(routerinfo_t *ri, const char **msg, int complain, int *valid_out)
{
    ...
    if (ri->cache_info.published_on > now + ROUTER_ALLOW_SKEW) {
        log_fn(severity, LD_DIRSERV,
               ...
               "(%s)",
               router_describe(ri), (int)((ri->cache_info.published_on - now) / 60),
               esc_router_info(ri));
    ...
    if (ri->cache_info.published_on < now - ROUTER_MAX_AGE_TO_PUBLISH) {
        log_fn(severity, LD_DIRSERV,
               ...
               router_describe(ri), (int)((now - ri->cache_info.published_on) / 60),
               esc_router_info(ri));
    ...
    if (dirserv_router_has_valid_address(ri) < 0) {
        log_fn(severity, LD_DIRSERV, "Router %s has invalid address. Not adding (%s).",
...
void
dirserv_set_node_flags_from_authoritative_status(node_t *node, uint32_t authstatus)
{
...
was_router_added_t
dirserv_add_multiple_descriptors(const char *desc, size_t desclen, uint8_t purpose,
                                 const char *source, const char **msg)
{
    ...
    if (tor_snprintf(annotation_buf, sizeof(annotation_buf),
                     ...
                     !general ? router_purpose_to_string(purpose) : "", !general ? "\n" : "") < 0) {
    ...
    s = desc;
    list = smartlist_new();
    if (!router_parse_list_from_string(&s, s + desclen, list, SAVED_NOWHERE, 0, 0, annotation_buf,
                                       NULL)) {
    ...
    s = desc;
    if (!router_parse_list_from_string(&s, s + desclen, list, SAVED_NOWHERE, 1, 0, NULL, NULL)) {
...
dirserv_add_descriptor(routerinfo_t *ri, const char **msg, const char *source)
{
    ...
    routerinfo_t *ri_old;
    char *desc, *nickname;
    const size_t desclen = ri->cache_info.signed_descriptor_len + ri->cache_info.annotations_len;
    const int key_pinning = dirauth_get_options()->AuthDirPinKeys;
    *msg = NULL;
    ...
    if (ri->cache_info.signing_key_cert) {
        ...
        if (ed25519_validate_pubkey(pkey) < 0) {
            log_warn(LD_DIRSERV, "Received bad key from %s (source %s)", router_describe(ri),
                     source);
            routerinfo_free(ri);
            return ROUTER_AUTHDIR_REJECTS;
        }
        ...
        /* Now pin it! */
        keypin_status = keypin_check_and_add((const uint8_t *)ri->cache_info.identity_digest,
                                             pkey->pubkey, !key_pinning);
    } else {
        keypin_status = keypin_check_lone_rsa((const uint8_t *)ri->cache_info.identity_digest);
    ...
    if (!WRA_WAS_ADDED(r)) {
        ...
        log_info(LD_DIRSERV, "Did not add descriptor from '%s' (source: %s): %s.", nickname, source,
                 *msg ? *msg : "(no message)");
    } else {
        smartlist_t *changed;
        ...
        log_info(LD_DIRSERV, "Added descriptor from '%s' (source: %s): %s.", nickname, source,
                 *msg);
    }
    tor_free(desc);
    tor_free(nickname);
...
fail : {
    ...
    download_status_t *dls = router_get_dl_status_by_descriptor_digest(desc_digest);
    if (dls) {
        log_info(LD_GENERAL,
    ...
    *msg = NULL;
    ...
    if (ei->cache_info.signed_descriptor_len > MAX_EXTRAINFO_UPLOAD_SIZE) {
        log_notice(LD_DIR,
                   ...
                   (int)ei->cache_info.signed_descriptor_len, MAX_EXTRAINFO_UPLOAD_SIZE);
        *msg = "Extrainfo document was too large";
        rv = ROUTER_BAD_EI;
        goto fail;
    }

    if ((r = routerinfo_incompatible_with_extrainfo(ri->identity_pkey, ei, &ri->cache_info, msg))) {
        if (r < 0) {
            extrainfo_free(ei);
...
directory_remove_invalid(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (nodes, node_t *, node) {
        ...
        if (r & RTR_REJECT) {
            log_info(LD_DIRSERV, "Router %s is now rejected: %s", description, msg ? msg : "");
