...
dirserv_load_fingerprint_file(void)
{
    ...
    if (!cf) {
        log_warn(LD_FS, "Cannot open fingerprint file '%s'. That's ok.",
                 fname);
        tor_free(fname);
        return 0;
    }
    ...
    for (list = front; list; list = list->next) {
        ...
        if (strlen(fingerprint) == BASE64_DIGEST256_LEN) {
            if (!digest256_from_base64((char *)ed25519_pubkey_tmp.pubkey,
                                       fingerprint)) {
                ed25519_not_ok = add_ed25519_to_dir(
                    &ed25519_pubkey_tmp, fingerprint_list_new, add_status);
            }
        }
    ...
    if (router->cache_info.signing_key_cert) {
        ...
        if (KEYPIN_MISMATCH ==
            ...
            log_fn(severity, LD_DIR,
                   ...
                   "but the <rsa,ed25519> keys don't match what they were "
                   "before.",
                   router_describe(router));
            if (key_pinning) {
                if (msg) {
                    *msg = "Ed25519 identity key or RSA identity key has "
                           "changed.";
                }
                return RTR_REJECT;
            }
                            ...
                            const vote_routerstatus_t *vrs)
{
    ...
    res =
        dirserv_get_status_impl(rs->identity_digest, &pk, rs->nickname,
                        ...
                        const char *platform, const char **msg, int severity)
{
    ...
    if (ed25519_public_key) {
        status_by_digest = digest256map_get(
            fingerprint_list->status_by_digest256, ed25519_public_key->pubkey);
        if (status_by_digest)
            result |= *status_by_digest;
    }
    ...
    if (result & RTR_REJECT) {
        if (msg)
            *msg =
                ...
                "bad-relays@lists.torproject.org mentioning your "
                "fingerprint(s)?";
        return RTR_REJECT;
...
dirserv_router_has_valid_address(routerinfo_t *ri)
{
    ...
    /* We only check internal v6 on non-null addresses because we do not
     * require IPv6 and null IPv6 is normal. */
                               ...
                               int complain, int *valid_out)
{
    ...
    if (ri->cache_info.published_on < now - ROUTER_MAX_AGE_TO_PUBLISH) {
        log_fn(severity, LD_DIRSERV,
               ...
               esc_router_info(ri));
        *msg = "Rejected: Server is expired, or your clock is too far in the "
               "past,"
...
dirserv_add_descriptor(routerinfo_t *ri, const char **msg, const char *source)
{
    ...
    *msg = NULL;
    ...
    if (ri_old &&
        ...
        *msg = "Not replacing router descriptor; no information has changed "
               "since "
    ...
    if (ri->cache_info.signing_key_cert) {
        ...
        keypin_status = keypin_check_and_add(
            (const uint8_t *)ri->cache_info.identity_digest, pkey->pubkey,
            !key_pinning);
    } else {
    ...
    if (keypin_status == KEYPIN_MISMATCH && key_pinning) {
        ...
        *msg =
            ...
            "recorded a different RSA identity for this Ed25519 identity (or "
            "vice "
