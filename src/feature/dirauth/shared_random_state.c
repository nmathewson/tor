...
disk_state_parse_commits(sr_state_t *state, const sr_disk_state_t *disk_state)
{
    ...
    for (line = disk_state->Commit; line; line = line->next) {
        ...
        if (smartlist_len(args) < 3) {
            log_warn(LD_BUG, "SR: Too few arguments in Commit Line: %s", escaped(line->value));
            goto error;
        }
...
disk_state_parse_srv(const char *value, sr_srv_t *dst)
{
    ...
    args = smartlist_new();
    smartlist_split_string(args, value, " ", SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 0);
...
static int
disk_state_parse_sr_values(sr_state_t *state, const sr_disk_state_t *disk_state)
{
    ...
    for (line = disk_state->SharedRandValues; line; line = line->next) {
        ...
        if (disk_state_parse_srv(line->value, srv) < 0) {
            log_warn(LD_BUG, "SR: Broken current SRV line in state %s", escaped(line->value));
            goto bad;
        }
...
disk_state_put_commit_line(const sr_commit_t *commit, config_line_t *line)
{
    ...
    tor_asprintf(&line->value, "%u %s %s %s%s", SR_PROTO_VERSION,
                 ...
                 commit->encoded_commit, reveal_str != NULL ? reveal_str : "");
    if (reveal_str != NULL) {
...
new_protocol_run(time_t valid_after)
{
    ...
    /* Do some logging */
    log_info(LD_DIR, "SR: Protocol run #%" PRIu64 " starting!", sr_state->n_protocol_runs);

    /* Generate fresh commitments for this protocol run */
    our_commitment = sr_generate_our_commit(valid_after, get_my_v3_authority_cert());
    if (our_commitment) {
...
static void
state_query(sr_state_action_t action, sr_state_object_t obj_type, void *data, void **out)
{
    switch (action) {
...
{
    state_query(SR_STATE_ACTION_PUT, SR_STATE_OBJ_VALID_AFTER, (void *)&valid_after, NULL);
}
...
{
    digestmap_t *commits;
    state_query(SR_STATE_ACTION_GET, SR_STATE_OBJ_COMMITS, NULL, (void *)&commits);
    tor_assert(commits);
    return commits;
}
...
sr_state_update(time_t valid_after)
{
    ...
    if (is_phase_transition(next_phase)) {
    ...
    } else if (sr_state->phase == SR_PHASE_COMMIT && digestmap_size(sr_state->commits) == 0) {
        ...
        sr_commit_t *our_commit = sr_generate_our_commit(valid_after, get_my_v3_authority_cert());
        if (our_commit) {
            /* Add our commitment to our state. In case we are unable to create one
             * (highly unlikely), we won't vote for this protocol run since our
             * commitment won't be in our state. */
...
sr_state_get_commit(const char *rsa_identity)
{
    ...
    state_query(SR_STATE_ACTION_GET, SR_STATE_OBJ_COMMIT, (void *)rsa_identity, (void *)&commit);
    return commit;
}
...
sr_state_copy_reveal_info(sr_commit_t *saved_commit, const sr_commit_t *commit)
{
    ...
    memcpy(saved_commit->random_number, commit->random_number, sizeof(saved_commit->random_number));
