...
new_protocol_run(time_t valid_after)
{
    ...
    if (sr_state->phase == SR_PHASE_REVEAL) {
        /* We are about to compute a new shared random value that will be set
         * in our state as the current value so rotate values. */
        state_rotate_srv();
...
sr_state_update(time_t valid_after)
{
    ...
    if (is_phase_transition(next_phase)) {
        if (next_phase == SR_PHASE_COMMIT) {
            /* Going into commit phase means we are starting a new protocol
             * run. */
...
sr_state_add_commit(sr_commit_t *commit)
{
    ...
    state_query(SR_STATE_ACTION_PUT, SR_STATE_OBJ_COMMIT, (void *)commit,
                NULL);
...
sr_state_init(int save_to_disk, int read_from_disk)
{
    ...
    if (ret < 0) {
        ...
        case EINVAL:
             ...
             * since it's obviously unusable and replace it by an new fresh
             * state below. */
        case ENOENT: {
