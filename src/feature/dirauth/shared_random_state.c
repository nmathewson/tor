...
state_query_put_(sr_state_object_t obj_type, void *data)
{
    ...
    switch (obj_type) {
        ...
        case SR_STATE_OBJ_CURSRV:
            /* Check if the new pointer is the same as the old one: if it is,
             * it's probably a bug. The caller may have confused current and
             * previous, or they may have forgotten to sr_srv_dup(). Putting
             * NULL multiple times is allowed. */
        ...
        case SR_STATE_OBJ_VALID_AFTER:
            ...
            break;
        /* It's not allowed to change the phase nor the full commitments map
         * from the state. The phase is decided during a strict process post
         * voting and the commits should be put individually. */
        case SR_STATE_OBJ_PHASE:
        case SR_STATE_OBJ_COMMITS:
        default:
            tor_assert(0);
...
sr_state_update(time_t valid_after)
{
    ...
    if (is_phase_transition(next_phase)) {
        ...
        sr_commit_t *our_commit =
            sr_generate_our_commit(valid_after, get_my_v3_authority_cert());
        if (our_commit) {
            /* Add our commitment to our state. In case we are unable to create
             * one (highly unlikely), we won't vote for this protocol run since
             * our commitment won't be in our state. */
...
sr_state_init(int save_to_disk, int read_from_disk)
{
    ...
    if (ret < 0) {
        switch (-ret) {
            case EINVAL:
                /* We have a state on disk but it contains something we couldn't
                 * parse or an invalid entry in the state file. Let's remove it
                 * since it's
                 * obviously unusable and replace it by an new fresh state
                 * below. */
            case ENOENT: {
                ...
                state_set(new_state);
                /* It's important to set our disk state pointer since the save
                 * call
