...
create_voting_schedule(const or_options_t *options, time_t now, int severity)
{
    ...
    if (consensus) {
        ...
        /* Note down the consensus valid after, so that we detect outdated
         * voting schedules in case of skewed clocks etc. */
        new_voting_schedule->live_consensus_valid_after =
    ...
    end = voting_sched_get_start_of_interval_after(
...
dirauth_get_voting_schedule(void)
{
    ...
    /* Also make sure we are not using an outdated voting schedule. If we have
     * a newer consensus, make sure we recalculate the voting schedule. */
