...
create_voting_schedule(const or_options_t *options, time_t now, int severity)
{
    ...
    start = new_voting_schedule->interval_starts = voting_sched_get_start_of_interval_after(
...
dirauth_get_voting_schedule(void)
{
    ...
    if (fast_mem_is_zero((const char *)&voting_schedule, sizeof(voting_schedule))) {
    ...
    if (ns && ns->valid_after != voting_schedule.live_consensus_valid_after) {
        log_info(LD_DIR, "Voting schedule is outdated: recalculating (%d/%d)", (int)ns->valid_after,
...
dirauth_sched_get_cur_valid_after_time(void)
{
    ...
    return voting_sched_get_start_of_interval_after(next_start - interval - 1, interval, offset);
}
