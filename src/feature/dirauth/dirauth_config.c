    ...
    if (options->V3AuthVotingInterval < MIN_VOTE_INTERVAL) {
        if (options->TestingTorNetwork) {
            if (options->V3AuthVotingInterval < MIN_VOTE_INTERVAL_TESTING) {
                ...
                REJECT(
                    "V3AuthVotingInterval is insanely low."); /* LCOV_EXCL_LINE */
            } else {
                COMPLAIN("V3AuthVotingInterval is very low. "
                         "This may lead to failure to synchronise for a "
                         "consensus.");
            }
        } else {
                                 ...
                                 or_options_t *options, char **msg)
{
    ...
    } else if (((30 * 60) % options->TestingV3AuthInitialVotingInterval) !=
               0) {
        REJECT(
