 ...
 */
int
options_validate_dirauth_mode(const or_options_t *old_options, or_options_t *options, char **msg)
{
    if (BUG(!options))
        return -1;
 ...
 */
int
options_validate_dirauth_schedule(const or_options_t *old_options, or_options_t *options,
                                  char **msg)
{
    (void)old_options;
    ...
    if (options->V3AuthVoteDelay + options->V3AuthDistDelay >= options->V3AuthVotingInterval / 2) {
 ...
 */
int
options_validate_dirauth_testing(const or_options_t *old_options, or_options_t *options, char **msg)
{
    (void)old_options;
    ...
    if (options->TestingV3AuthInitialVotingInterval < MIN_VOTE_INTERVAL_TESTING_INITIAL) {
    ...
    if (options->TestingV3AuthInitialVoteDelay + options->TestingV3AuthInitialDistDelay >=
    ...
    if (options->TestingV3AuthVotingStartOffset >
        MIN(options->TestingV3AuthInitialVotingInterval, options->V3AuthVotingInterval)) {
 ...
 */
int
options_act_dirauth_stats(const or_options_t *old_options, bool *print_notice_out)
{
    ...
    /* If we used to have statistics enabled but we just disabled them,
       stop gathering them.  */
    if (old_options && authdir_mode_bridge(old_options) && !authdir_mode_bridge(options))
...
dirauth_options_pre_normalize(void *arg, char **msg_out)
{
    ...
    if (!options->RecommendedClientVersions)
        options->RecommendedClientVersions = config_lines_dup(options->RecommendedVersions);
    if (!options->RecommendedServerVersions)
        options->RecommendedServerVersions = config_lines_dup(options->RecommendedVersions);

    if (config_ensure_bandwidth_cap(&options->AuthDirFastGuarantee, "AuthDirFastGuarantee",
                                    msg_out) < 0)
        return -1;
    if (config_ensure_bandwidth_cap(&options->AuthDirGuardBWGuarantee, "AuthDirGuardBWGuarantee",
                                    msg_out) < 0)
        return -1;
...
dirauth_options_validate(const void *arg, char **msg)
{
    ...
    if (options->VersioningAuthoritativeDirectory &&
        (!options->RecommendedClientVersions || !options->RecommendedServerVersions)) {
...
const config_format_t dirauth_options_fmt = {
    ...
    .magic = {"dirauth_options_t", DIRAUTH_OPTIONS_MAGIC, offsetof(dirauth_options_t, magic)},
