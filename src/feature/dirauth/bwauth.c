...
int
dirserv_query_measured_bw_cache_kb(const char *node_id, long *bw_kb_out, time_t *as_of_out)
{
 ...
 */
int
dirserv_read_measured_bandwidths(const char *from_file, smartlist_t *routerstatuses,
                                 smartlist_t *bw_file_headers, uint8_t *digest_out)
{
    ...
    if (fp == NULL) {
        log_warn(LD_CONFIG, "Can't open bandwidth file at configured location: %s", from_file);
        goto err;
    }
    ...
    if (!strlen(line) || line[strlen(line) - 1] != '\n') {
        log_warn(LD_DIRSERV, "Long or truncated time in bandwidth file: %s", escaped(line));
    ...
    if (!ok) {
        log_warn(LD_DIRSERV, "Non-integer time in bandwidth file: %s", escaped(line));
        goto continue_digest;
    }
    ...
    if (bw_file_headers)
        smartlist_add_asprintf(bw_file_headers, "timestamp=%lu", (unsigned long)file_time);
    ...
    while (!feof(fp)) {
        ...
        if (tor_getline(&line, &n, fp) >= 0) {
            crypto_digest_add_bytes(digest, (const char *)line, strlen(line));
            if (measured_bw_line_parse(&parsed_line, line, line_is_after_headers) != -1) {
                 ...
                 * then assume this line is a header and add it to bw_file_headers */
                       ...
                       (smartlist_len(bw_file_headers) < MAX_BW_FILE_HEADER_COUNT_IN_VOTE)) {
                line[strlen(line) - 1] = '\0';
 ...
 */
STATIC int
measured_bw_line_parse(measured_bw_line_t *out, const char *orig_line, int line_is_after_headers)
{
    ...
    if (!cp) {
        log_warn(LD_DIRSERV, "Invalid line in bandwidth file: %s", escaped(orig_line));
        tor_free(line);
        return -1;
    }

    if (orig_line[strlen(orig_line) - 1] != '\n') {
        log_warn(LD_DIRSERV, "Incomplete line in bandwidth file: %s", escaped(orig_line));
        tor_free(line);
        return -1;
    }
    ...
    do {
        ...
        if (strcmpstart(cp, "vote=0") == 0) {
            ...
            char *endptr;
            if (got_bw) {
                log_warn(LD_DIRSERV, "Double bw= in bandwidth file line: %s", escaped(orig_line));
                tor_free(line);
                return -1;
            }
            cp += strlen("bw=");
 ...
 */
STATIC int
measured_bw_line_apply(measured_bw_line_t *parsed_line, smartlist_t *routerstatuses)
{
    ...
    if (rs) {
        ...
        log_info(LD_DIRSERV, "Node ID %s not found in routerstatus list", parsed_line->node_hex);
    }
