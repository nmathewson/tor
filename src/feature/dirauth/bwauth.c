                                 ...
                                 uint8_t *digest_out)
{
     ...
     * if there are additional header lines, as introduced in Bandwidth List
     * spec version 1.1.0 */
    ...
    while (!feof(fp)) {
        ...
        if (tor_getline(&line, &n, fp) >= 0) {
            ...
            if (measured_bw_line_parse(&parsed_line, line,
                ...
                /* This condition will be true when the first complete valid bw
                 * line
                 * has been encountered, which means the end of the header
                 * lines. */
                line_is_after_headers = 1;
                ...
                if (measured_bw_line_apply(&parsed_line, routerstatuses) > 0)
                    applied_lines++;
                /* if the terminator is found, it is the end of header lines,
                 * set the flag but do not store anything */
            } else if (strcmp(line, BW_FILE_HEADERS_TERMINATOR) == 0) {
                line_is_after_headers = 1;
                /* if the line was not a correct relay line nor the terminator
                 * and the end of the header lines has not been detected yet
                 * and it is key_value and bw_file_headers did not reach the
                 * maximum number of headers,
                 * then assume this line is a header and add it to
                 * bw_file_headers */
...
err:
    if (line) {
        // we need to raw_free this buffer because we got it from
        // tor_getdelim()
        raw_free(line);
    }
    if (fp)
                       ...
                       int line_is_after_headers)
{
    ...
    if (got_bw && got_node_id) {
        ...
        /* There could be additional header lines, therefore do not give
         * warnings but returns -1 since it's not a complete bw line. */
        log_debug(LD_DIRSERV,
