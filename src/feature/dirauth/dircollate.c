...
static void
ddmap_entry_set_digests(ddmap_entry_t *ent, const uint8_t *rsa_sha1, const uint8_t *ed25519)
{
...
HT_GENERATE2(double_digest_map, ddmap_entry_t, node, ddmap_entry_hash, ddmap_entry_eq, 0.6,
...
static void
dircollator_add_routerstatus(dircollator_t *dc, int vote_num, networkstatus_t *vote,
...
dircollator_free_(dircollator_t *dc)
{
    ...
    for (e = HT_START(double_digest_map, &dc->by_both_ids); e != NULL; e = next) {
        this = *e;
...
dircollator_collate_by_ed25519(dircollator_t *dc)
{
    ...
    HT_FOREACH (iter, double_digest_map, &dc->by_both_ids) {
        ...
        vote_routerstatus_t **vrs_lst2 = digestmap_get(dc->by_rsa_sha1, (char *)ent->d);
        tor_assert(vrs_lst2);
...
dircollator_get_votes_for_router(dircollator_t *dc, int idx)
{
    ...
    return digestmap_get(dc->by_collated_rsa_sha1, smartlist_get(dc->all_rsa_sha1_lst, idx));
}
