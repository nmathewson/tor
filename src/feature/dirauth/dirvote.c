...
static int dirvote_add_signatures_to_all_pending_consensuses(const char *detached_signatures_body,
                                                             const char *source,
                                                             const char **msg_out);
...
authority_cert_dup(authority_cert_t *cert)
{
    ...
    out->cache_info.signed_descriptor_body = tor_strndup(cert->cache_info.signed_descriptor_body,
...
format_protocols_lines_for_vote(const networkstatus_t *v3_ns)
{
    ...
    recommended_relay_protocols_line =
        format_line_if_present("recommended-relay-protocols", v3_ns->recommended_relay_protocols);
    recommended_client_protocols_line =
        format_line_if_present("recommended-client-protocols", v3_ns->recommended_client_protocols);
    required_relay_protocols_line =
        format_line_if_present("required-relay-protocols", v3_ns->required_relay_protocols);
    required_client_protocols_line =
        format_line_if_present("required-client-protocols", v3_ns->required_client_protocols);

    char *result = NULL;
    tor_asprintf(&result, "%s%s%s%s", recommended_relay_protocols_line,
                 recommended_client_protocols_line, required_relay_protocols_line,
                 required_client_protocols_line);
 ...
 * For v3 authorities. */
STATIC char *
format_networkstatus_vote(crypto_pk_t *private_signing_key, networkstatus_t *v3_ns)
{
    ...
    base16_encode(fingerprint, sizeof(fingerprint), v3_ns->cert->cache_info.identity_digest,
                  DIGEST_LEN);

    client_versions_line = format_line_if_present("client-versions", v3_ns->client_versions);
    server_versions_line = format_line_if_present("server-versions", v3_ns->server_versions);
...

    {
        ...
        char *methods = make_consensus_method_list(MIN_SUPPORTED_CONSENSUS_METHOD,
        ...
        if (v3_ns->bw_file_headers) {
            ...
            if (!BUG(smartlist_len(v3_ns->bw_file_headers) > MAX_BW_FILE_HEADER_COUNT_IN_VOTE)) {
                bw_file_headers = smartlist_join_strings(v3_ns->bw_file_headers, " ", 0, NULL);
            ...
            bw_headers_line = format_line_if_present("bandwidth-file-headers", bw_file_headers);
            tor_free(bw_file_headers);
        }
        ...
        if (!tor_digest256_is_zero((const char *)v3_ns->bw_file_digest256)) {
            ...
            digest256_to_base64(b64_digest_bw_file, (const char *)v3_ns->bw_file_digest256);
            ...
            tor_asprintf(&digest_algo_b64_digest_bw_file, "%s=%s",
                         crypto_digest_algorithm_get_name(DIGEST_ALG_BW_FILE), b64_digest_bw_file);
            /* No need for tor_strdup(""), format_line_if_present does it. */
            bw_file_digest =
                format_line_if_present("bandwidth-file-digest", digest_algo_b64_digest_bw_file);
        ...
        smartlist_add_asprintf(
            ...
            ,
            v3_ns->type == NS_TYPE_VOTE ? "vote" : "opinion", methods, published, va, fu, vu,
            v3_ns->vote_seconds, v3_ns->dist_seconds, client_versions_line, server_versions_line,
            protocols_lines, flags, flag_thresholds, params, bw_headers_line ? bw_headers_line : "",
            bw_file_digest ? bw_file_digest : "", voter->nickname, fingerprint, voter->address,
            fmt_addr32(addr), voter->dir_port, voter->or_port, voter->contact,
    ...
    SMARTLIST_FOREACH_BEGIN (v3_ns->routerstatus_list, vote_routerstatus_t *, vrs) {
        ...
        rsf =
            routerstatus_format_entry(&vrs->status, vrs->version, vrs->protocols, NS_V3_VOTE, vrs);
        if (rsf)
    ...
    crypto_digest_smartlist(digest, DIGEST_LEN, chunks, "directory-signature ", DIGEST_SHA1);

    {
        char signing_key_fingerprint[FINGERPRINT_LEN + 1];
        if (crypto_pk_get_fingerprint(private_signing_key, signing_key_fingerprint, 0) < 0) {
    ...
    {
        networkstatus_t *v;
        if (!(v = networkstatus_parse_vote_from_string(status, strlen(status), NULL,
                                                       v3_ns->type))) {
            log_err(LD_BUG,
...
static int
vote_routerstatus_find_microdesc_hash(char *digest256_out, const vote_routerstatus_t *vrs,
...
compare_votes_by_authority_id_(const void **_a, const void **_b)
{
    ...
    return fast_memcmp(get_voter(a)->identity_digest, get_voter(b)->identity_digest, DIGEST_LEN);
}
...
compare_dir_src_ents_by_authority_id_(const void **_a, const void **_b)
{
    ...
    const networkstatus_voter_info_t *a_v = get_voter(a->v), *b_v = get_voter(b->v);
...
compare_vote_rs(const vote_routerstatus_t *a, const vote_routerstatus_t *b)
{
    ...
    if ((r = fast_memcmp(a->status.identity_digest, b->status.identity_digest, DIGEST_LEN)))
        return r;
    if ((r = fast_memcmp(a->status.descriptor_digest, b->status.descriptor_digest, DIGEST_LEN)))
        return r;
...
compute_routerstatus_consensus(smartlist_t *votes, int consensus_method,
                               char *microdesc_digest256_out, tor_addr_port_t *best_alt_orport_out)
{
    ...
    if (cur_n > most_n || (cur && cur_n == most_n && cur->status.published_on > most_published)) {
        most = cur;
    ...
    if (best_alt_orport_out) {
        ...
        SMARTLIST_FOREACH_BEGIN (votes, vote_routerstatus_t *, rs) {
            tor_assert(rs);
            if (compare_vote_rs(most, rs) == 0 && !tor_addr_is_null(&rs->status.ipv6_addr) &&
                rs->status.ipv6_orport) {
                smartlist_add(alt_orports,
                              tor_addr_port_new(&rs->status.ipv6_addr, rs->status.ipv6_orport));
            }
        } SMARTLIST_FOREACH_END (rs);

        smartlist_sort(alt_orports, compare_orports_);
        most_alt_orport = smartlist_get_most_frequent(alt_orports, compare_orports_);
        if (most_alt_orport) {
            memcpy(best_alt_orport_out, most_alt_orport, sizeof(tor_addr_port_t));
            log_debug(LD_DIR, "\"a\" line winner for %s is %s", most->status.nickname,
    ...
    if (microdesc_digest256_out) {
        ...
        SMARTLIST_FOREACH_BEGIN (votes, vote_routerstatus_t *, rs) {
            ...
            if (compare_vote_rs(rs, most))
                continue;
            if (!vote_routerstatus_find_microdesc_hash(d, rs, consensus_method, DIGEST_SHA256))
...
{
    return (method >= MIN_SUPPORTED_CONSENSUS_METHOD) && (method <= MAX_SUPPORTED_CONSENSUS_METHOD);
}
 ...
 */
STATIC int32_t
dirvote_get_intermediate_param_value(const smartlist_t *param_list, const char *keyword,
                                     int32_t default_val)
{
    ...
    SMARTLIST_FOREACH_BEGIN (param_list, const char *, k_v_pair) {
        if (!strcmpstart(k_v_pair, keyword) && k_v_pair[strlen(keyword)] == '=') {
            ...
            int ok;
            value = (int32_t)tor_parse_long(integer_str, 10, INT32_MIN, INT32_MAX, &ok, NULL);
            if (BUG(!ok))
                return default_val;
            ++n_found;
        }
...
#define RANGE_CHECK(a, b, c, d, e, f, g, mx)                                                      \
    ((a) >= 0 && (a) <= (mx) && (b) >= 0 && (b) <= (mx) && (c) >= 0 && (c) <= (mx) && (d) >= 0 && \
     (d) <= (mx) && (e) >= 0 && (e) <= (mx) && (f) >= 0 && (f) <= (mx) && (g) >= 0 && (g) <= (mx))
...
networkstatus_check_weights(int64_t Wgg, int64_t Wgd, int64_t Wmg, int64_t Wme, int64_t Wmd,
                            int64_t Wee, int64_t Wed, int64_t scale, int64_t G, int64_t M,
                            int64_t E, int64_t D, int64_t T, int64_t margin, int do_balance)
{
...
out:
    if (berr) {
        log_info(LD_DIR,
                 "Bw weight mismatch %d. G=%" PRId64 " M=%" PRId64 " E=%" PRId64 " D=%" PRId64
                 " T=%" PRId64 " Wmd=%d Wme=%d Wmg=%d Wed=%d Wee=%d"
                 " Wgd=%d Wgg=%d Wme=%d Wmg=%d",
                 berr, (G), (M), (E), (D), (T), (int)Wmd, (int)Wme, (int)Wmg, (int)Wed, (int)Wee,
 ...
 */
int
networkstatus_compute_bw_weights_v10(smartlist_t *chunks, int64_t G, int64_t M, int64_t E,
                                     int64_t D, int64_t T, int64_t weight_scale)
{
    ...
    if (G <= 0 || M <= 0 || E <= 0 || D <= 0) {
        log_warn(LD_DIR,
                 ...
                 "G=%" PRId64 " M=%" PRId64 " E=%" PRId64 " D=%" PRId64 " T=%" PRId64,
                 (G), (M), (E), (D), (T));
        return 0;
    }
    ...
    if (3 * E >= T && 3 * G >= T) { // E >= T/3 && G >= T/3
        ...
        berr = networkstatus_check_weights(Wgg, Wgd, Wmg, Wme, Wmd, Wee, Wed, weight_scale, G, M, E,
                                           D, T, 10, 1);

        if (berr) {
            log_warn(LD_DIR,
                     "Bw Weights error %d for %s v10. G=%" PRId64 " M=%" PRId64 " E=%" PRId64
                     " D=%" PRId64 " T=%" PRId64 " Wmd=%d Wme=%d Wmg=%d Wed=%d Wee=%d"
                     " Wgd=%d Wgg=%d Wme=%d Wmg=%d weight_scale=%d",
                     berr, casename, (G), (M), (E), (D), (T), (int)Wmd, (int)Wme, (int)Wmg,
                     (int)Wed, (int)Wee, (int)Wgd, (int)Wgg, (int)Wme, (int)Wmg, (int)weight_scale);
            return 0;
        }
        ...
        if (R + D < S) { // Subcase a
            ...
            berr = networkstatus_check_weights(Wgg, Wgd, Wmg, Wme, Wmd, Wee, Wed, weight_scale, G,
                                               M, E, D, T, 10, 1);
            ...
            if (berr) {
                ...
                if (Wmd < 0) { // Can happen if M > T/3
                    ...
                    Wmd = 0;
                    log_warn(LD_DIR, "Too much Middle bandwidth on the network to calculate "
                ...
                berr = networkstatus_check_weights(Wgg, Wgd, Wmg, Wme, Wmd, Wee, Wed, weight_scale,
                                                   G, M, E, D, T, 10, 1);
            }
    ...
    smartlist_add_asprintf(chunks,
                           ...
                           (int)Wmd, (int)Wme, (int)Wmg, (int)weight_scale, (int)weight_scale,
                           (int)weight_scale, (int)Wed, (int)Wee, (int)Wed, (int)Wee,
                           (int)weight_scale, (int)Wgd, (int)Wgg, (int)Wgg, (int)weight_scale,
...
static void
update_total_bandwidth_weights(const routerstatus_t *rs, int is_exit, int is_guard, int64_t *G,
                               int64_t *M, int64_t *E, int64_t *D, int64_t *T)
{
    ...
    if (!rs->has_bandwidth) {
        log_info(LD_BUG, "Missing consensus bandwidth for router %s", rs->nickname);
        return;
    }
...
networkstatus_compute_consensus(smartlist_t *votes, int total_authorities,
                                ...
                                const char *legacy_id_key_digest, crypto_pk_t *legacy_signing_key,
                                consensus_flavor_t flavor)
{
    ...
    if (consensus_method_is_supported(consensus_method)) {
        log_info(LD_DIR, "Generating consensus using method %d.", consensus_method);
    } else {
        log_warn(LD_DIR,
     ...
     * routers we might need to talk about. */
    {
        ...
        SMARTLIST_FOREACH_BEGIN (votes, networkstatus_t *, v) {
            ...
            SMARTLIST_FOREACH(v->known_flags, const char *, cp, smartlist_add_strdup(flags, cp));
        } SMARTLIST_FOREACH_END (v);
        ...
        tor_assert(valid_after + (get_options()->TestingTorNetwork ? MIN_VOTE_INTERVAL_TESTING
                                                                   : MIN_VOTE_INTERVAL) <=
                   fresh_until);
        tor_assert(fresh_until + (get_options()->TestingTorNetwork ? MIN_VOTE_INTERVAL_TESTING
                                                                   : MIN_VOTE_INTERVAL) <=
                   valid_until);
        ...
        server_versions =
            compute_consensus_versions_list(combined_server_versions, n_versioning_servers);
        client_versions =
...

    {
        ...
        smartlist_add_asprintf(chunks,
                               ...
                               flavor == FLAV_NS ? "" : " ", flavor == FLAV_NS ? "" : flavor_name);
        ...
        smartlist_add_asprintf(chunks,
                               ...
                               "%s" /* packages */
                               "known-flags %s\n",
                               va_buf, fu_buf, vu_buf, vote_seconds, dist_seconds, client_versions,
                               server_versions, packages, flaglist);
    ...
    param_list = dirvote_compute_params(votes, consensus_method, total_authorities);
    ...
    /* Add the authority sections. */
    {
        ...
        SMARTLIST_FOREACH_BEGIN (dir_sources, const dir_src_ent_t *, e) {
            ...
            base16_encode(votedigest, sizeof(votedigest), voter->vote_digest, DIGEST_LEN);

            smartlist_add_asprintf(chunks, "dir-source %s%s %s %s %s %d %d\n", voter->nickname,
    ...
    /* Add the actual router entries. */
    {
        ...
        uint32_t *bandwidths_kb = tor_calloc(smartlist_len(votes), sizeof(uint32_t));
         ...
         * them.*/
        SMARTLIST_FOREACH_BEGIN (votes, networkstatus_t *, v) {
            flag_map[v_sl_idx] = tor_calloc(smartlist_len(v->known_flags), sizeof(int));
        ...
        /* Named and Unnamed get treated specially */
        {
            SMARTLIST_FOREACH_BEGIN (votes, networkstatus_t *, v) {
                ...
                SMARTLIST_FOREACH_BEGIN (v->routerstatus_list, vote_routerstatus_t *, rs) {
                    if ((rs->flags & nf) != 0) {
        ...
        for (i = 0; i < num_routers; ++i) {
            vote_routerstatus_t **vrs_lst = dircollator_get_votes_for_router(collator, i);
            ...
            for (int voter_idx = 0; voter_idx < smartlist_len(votes); ++voter_idx) {
                ...
                if (named_flag[voter_idx] >= 0 &&
                    ...
                    if (chosen_name && strcmp(chosen_name, rs->status.nickname)) {
                        log_notice(LD_DIR, "Conflict on naming for router: %s vs %s", chosen_name,
                                   rs->status.nickname);
                        naming_conflict = 1;
                    }
                ...
                if (rs->ed25519_reflects_consensus) {
                    ...
                    if (ed_consensus_val) {
                        tor_assert(
                            fast_memeq(ed_consensus_val, rs->ed25519_id, ED25519_PUBKEY_LEN));
                    } else {
            ...
            rs = compute_routerstatus_consensus(matching_descs, consensus_method, microdesc_digest,
                                                &alt_orport);
            /* Copy bits of that into rs_out. */
            memset(&rs_out, 0, sizeof(rs_out));
            tor_assert(fast_memeq(current_rsa_id, rs->status.identity_digest, DIGEST_LEN));
            memcpy(rs_out.identity_digest, current_rsa_id, DIGEST_LEN);
            memcpy(rs_out.descriptor_digest, rs->status.descriptor_digest, DIGEST_LEN);
            ...
            {
                update_total_bandwidth_weights(&rs_out, is_exit, is_guard, &G, &M, &E, &D, &T);
            }
             ...
             */
            {
                ...
                SMARTLIST_FOREACH_BEGIN (matching_descs, vote_routerstatus_t *, vsr) {
                     ...
                     * proper descriptor */
                    tor_assert(fast_memeq(rs_out.identity_digest, vsr->status.identity_digest,
                                          DIGEST_LEN));
                    if (vsr->status.has_exitsummary &&
                        fast_memeq(rs_out.descriptor_digest, vsr->status.descriptor_digest,
                                   DIGEST_LEN)) {
            ...
            if (flavor == FLAV_MICRODESC && tor_digest256_is_zero(microdesc_digest)) {
            ...
            smartlist_add(chunks, smartlist_join_strings(chosen_flags, " ", 0, NULL));
...

    {
        ...
        if (bw_weight_param) {
            ...
            if (eq) {
                ...
                log_warn(LD_DIR, "Bad element '%s' in bw weight param", escaped(bw_weight_param));
                weight_scale = BW_WEIGHT_SCALE;
            }
        }

        added_weights = networkstatus_compute_bw_weights_v10(chunks, G, M, E, D, T, weight_scale);
    }
    ...
    /* Add a signature. */
    {
        ...
        digest_algorithm_t digest_alg = flavor == FLAV_NS ? DIGEST_SHA1 : DIGEST_SHA256;
        ...
        /* And the signature. */
        if (!(signature = router_get_dirobj_signature(digest, digest_len, signing_key))) {
        ...
        if (legacy_id_key_digest && legacy_signing_key) {
            smartlist_add_strdup(chunks, "directory-signature ");
            base16_encode(fingerprint, sizeof(fingerprint), legacy_id_key_digest, DIGEST_LEN);
            crypto_pk_get_fingerprint(legacy_signing_key, signing_key_fingerprint, 0);
            if (flavor == FLAV_NS) {
                smartlist_add_asprintf(chunks, "%s %s\n", fingerprint, signing_key_fingerprint);
            } else {
                smartlist_add_asprintf(chunks, "%s %s %s\n", algname, fingerprint,
            ...
            if (!(signature =
                      router_get_dirobj_signature(digest, digest_len, legacy_signing_key))) {
                log_warn(LD_BUG, "Couldn't sign consensus networkstatus.");
                goto done;
            }
            smartlist_add(chunks, signature);
    ...
    {
        networkstatus_t *c;
        if (!(c = networkstatus_parse_vote_from_string(result, strlen(result), NULL,
                                                       NS_TYPE_CONSENSUS))) {
...
compute_consensus_package_lines(smartlist_t *votes)
{
    ...
    STRMAP_FOREACH(package_status, key, const char **, values)
    {
        ...
        const char *most_frequent = smartlist_get_most_frequent_string_(entries, &count);
 ...
 */
STATIC int
networkstatus_add_detached_signatures(networkstatus_t *target, ns_detached_signatures_t *sigs,
                                      const char *source, int severity, const char **msg_out)
{
    ...
    SMARTLIST_FOREACH_BEGIN (siglist, document_signature_t *, sig) {
        ...
        base16_encode(voter_identity, sizeof(voter_identity), sig->identity_digest, DIGEST_LEN);
        log_info(LD_DIR, "Looking at signature from %s using %s", voter_identity, algorithm);
        ...
        if (old_sig && old_sig->good_signature) {
            log_info(LD_DIR, "We already have a good signature from %s using %s", voter_identity,
                     algorithm);
            continue;
        }
        ...
        if (!sig->good_signature && !sig->bad_signature) {
            cert = authority_cert_get_by_digests(sig->identity_digest, sig->signing_key_digest);
            if (cert) {
        ...
        if (sig->good_signature || !old_sig || old_sig->bad_signature) {
            log_info(LD_DIR, "Adding signature from %s with %s", voter_identity, algorithm);
            tor_log(severity, LD_DIR, "Added a signature for %s from %s.", target_voter->nickname,
                    source);
            ++r;
            if (old_sig) {
...
static char *
networkstatus_format_signatures(networkstatus_t *consensus, int for_detached_signatures)
{
    ...
    SMARTLIST_FOREACH_BEGIN (consensus->voters, networkstatus_voter_info_t *, v) {
        SMARTLIST_FOREACH_BEGIN (v->sigs, document_signature_t *, sig) {
            ...
            if (flavor == FLAV_NS) {
                ...
                smartlist_add_asprintf(elements, "%s%s%s %s %s %s\n-----BEGIN SIGNATURE-----\n",
                                       keyword, for_detached_signatures ? " " : "",
                                       for_detached_signatures ? flavor_name : "", digest_name, id,
                                       sk);
            }
 ...
 * <b>pending</b>. */
static char *
get_detached_signatures_from_pending_consensuses(pending_consensus_t *pending, int n_flavors)
{
    int flav;
    char *signatures;
...
#define ENDIF \
    ...
    ENDIF
    IF_TIME_FOR_NEXT_ACTION(fetch_missing_signatures, have_fetched_missing_signatures)
    {
    ...
    IF_TIME_FOR_NEXT_ACTION(interval_starts, have_published_consensus)
    {
        ...
        sr_act_post_consensus(networkstatus_get_latest_consensus_by_flavor(FLAV_NS));
        /* XXXX We will want to try again later if we haven't got enough
...
dirvote_perform_vote(void)
{
    ...
    tor_free(contents);
    if (!pending_vote) {
        log_warn(LD_DIR, "Couldn't store my own vote! (I told myself, '%s'.)", msg);
        return -1;
    }

    directory_post_to_dirservers(DIR_PURPOSE_UPLOAD_VOTE, ROUTER_PURPOSE_GENERAL, V3_DIRINFO,
                                 pending_vote->vote_body->dir, pending_vote->vote_body->dir_len, 0);
...
dirvote_fetch_missing_votes(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (router_get_trusted_dir_servers(), dir_server_t *, ds) {
        if (!(ds->type & V3_DIRINFO))
            continue;
        if (!dirvote_get_vote(ds->v3_identity_digest, DGV_BY_ID | DGV_INCLUDE_PENDING)) {
    ...
    directory_get_from_all_authorities(DIR_PURPOSE_FETCH_STATUS_VOTE, 0, resource);
    tor_free(resource);
...
dirvote_fetch_missing_signatures(void)
{
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        ...
        if (!consensus || networkstatus_check_consensus_signature(consensus, -1) == 1) {
...
dirvote_clear_votes(int all_votes)
{
    ...
    if (pending_consensus_signature_list) {
        SMARTLIST_FOREACH(pending_consensus_signature_list, char *, cp, tor_free(cp));
...
list_v3_auth_ids(void)
{
    ...
    SMARTLIST_FOREACH(
        ...
        if ((ds->type & V3_DIRINFO) && !tor_digest_is_zero(ds->v3_identity_digest))
            smartlist_add(known_v3_keys, tor_strdup(hex_str(ds->v3_identity_digest, DIGEST_LEN))));
...
again:
    vote = networkstatus_parse_vote_from_string(vote_body, strlen(vote_body), &end_of_vote,
                                                NS_TYPE_VOTE);
    if (!end_of_vote)
        end_of_vote = vote_body + strlen(vote_body);
    if (!vote) {
        log_warn(LD_DIR, "Couldn't parse vote: length was %d", (int)strlen(vote_body));
    ...
    if (!ds) {
        ...
        log_warn(LD_DIR,
                 ...
                 vi->nickname, vi->address, hex_str(vi->identity_digest, DIGEST_LEN), keys);
        tor_free(keys);
    ...
    if (time_posted && time_posted > voting_schedule.fetch_missing_votes) {
        ...
        log_warn(LD_DIR,
                 "Rejecting posted vote from %s received at %s; "
    ...
    SMARTLIST_FOREACH_BEGIN (pending_vote_list, pending_vote_t *, v) {
        if (fast_memeq(v->vote->cert->cache_info.identity_digest,
            ...
            if (fast_memeq(vi_old->vote_digest, vi->vote_digest, DIGEST_LEN)) {
                ...
                log_info(LD_DIR, "Discarding a vote we already have (from %s).", vi->address);
                if (*status_out < 200)
                    *status_out = 200;
                goto discard;
                ...
                cached_dir_decref(v->vote_body);
                networkstatus_vote_free(v->vote);
                v->vote_body = new_cached_dir(tor_strndup(vote_body, end_of_vote - vote_body),
                                              vote->published);
                v->vote = vote;
    ...
    pending_vote->vote_body =
        new_cached_dir(tor_strndup(vote_body, end_of_vote - vote_body), vote->published);
...
dirvote_compute_consensuses(void)
{
...

    {
        ...
        for (flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
            ...
            consensus_body = networkstatus_compute_consensus(votes, n_voters, my_cert->identity_key,
                                                             get_my_v3_authority_signing_key(),
                                                             legacy_id_digest, legacy_sign, flav);

            if (!consensus_body) {
                log_warn(LD_DIR, "Couldn't generate a %s consensus at all!", flavor_name);
                continue;
            }
            consensus = networkstatus_parse_vote_from_string(consensus_body, strlen(consensus_body),
                                                             NULL, NS_TYPE_CONSENSUS);
            if (!consensus) {
                log_warn(LD_DIR, "Couldn't parse %s consensus we generated!", flavor_name);
    ...
    if (pending_consensus_signature_list) {
        ...
        SMARTLIST_FOREACH_BEGIN (pending_consensus_signature_list, char *, sig) {
            const char *msg = NULL;
            int r = dirvote_add_signatures_to_all_pending_consensuses(sig, "pending", &msg);
            if (r >= 0)
                n_sigs += r;
            else
                log_warn(LD_DIR, "Could not add queued signature to new consensus: %s", msg);
            tor_free(sig);
        } SMARTLIST_FOREACH_END (sig);
        if (n_sigs)
            log_notice(LD_DIR,
    ...
    directory_post_to_dirservers(DIR_PURPOSE_UPLOAD_SIGNATURES, ROUTER_PURPOSE_GENERAL, V3_DIRINFO,
                                 pending_consensus_signatures, strlen(pending_consensus_signatures),
                                 0);
...
static int
dirvote_add_signatures_to_pending_consensus(pending_consensus_t *pc, ns_detached_signatures_t *sigs,
                                            const char *source, int severity, const char **msg_out)
{
    const char *flavor_name;
    int r = -1;
    ...
    *msg_out = NULL;
    ...
    r = networkstatus_add_detached_signatures(pc->consensus, sigs, source, severity, msg_out);
    if (r >= 0) {
        log_info(LD_DIR, "Added %d signatures to consensus.", r);
    } else {
        log_fn(LOG_PROTOCOL_WARN, LD_DIR, "Unable to add signatures to consensus: %s",
    ...
    if (r >= 1) {
        ...
        {
            networkstatus_t *v = networkstatus_parse_vote_from_string(pc->body, strlen(pc->body),
                                                                      NULL, NS_TYPE_CONSENSUS);
            tor_assert(v);
            networkstatus_vote_free(v);
        }
...
static int
dirvote_add_signatures_to_all_pending_consensuses(const char *detached_signatures_body,
                                                  const char *source, const char **msg_out)
{
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        ...
        if (!pc->consensus)
            continue;
        res = dirvote_add_signatures_to_pending_consensus(pc, sigs, source, severity, msg_out);
        if (res < 0)
            errors++;
        else
            n_added += res;
    ...
    if (n_added && pending_consensuses[FLAV_NS].consensus) {
        char *new_detached = get_detached_signatures_from_pending_consensuses(pending_consensuses,
                                                                              N_CONSENSUS_FLAVORS);
        if (new_detached) {
            tor_free(pending_consensus_signatures);
 ...
 * Return negative on failure, nonnegative on success. */
int
dirvote_add_signatures(const char *detached_signatures_body, const char *source, const char **msg)
{
    if (pending_consensuses[FLAV_NS].consensus) {
        log_notice(LD_DIR,
                   ...
                   source);
        return dirvote_add_signatures_to_all_pending_consensuses(detached_signatures_body, source,
                                                                 msg);
    } else {
        log_notice(LD_DIR,
                   "Got a signature from %s. "
        ...
        smartlist_add_strdup(pending_consensus_signature_list, detached_signatures_body);
...
dirvote_publish_consensus(void)
{
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        ...
        if (networkstatus_set_current_consensus(pending->body, strlen(pending->body), name, 0,
                                                NULL))
    ...
    if (by_id) {
        if (pending_vote_list && include_pending) {
            SMARTLIST_FOREACH(pending_vote_list, pending_vote_t *, pv,
                              if (fast_memeq(get_voter(pv->vote)->identity_digest, fp,
...
dirvote_create_microdescriptor(const routerinfo_t *ri, int consensus_method)
{
    ...
    smartlist_t *chunks = smartlist_new();
    char *output = NULL;
    crypto_pk_t *rsa_pubkey = router_get_rsa_onion_pkey(ri->onion_pkey, ri->onion_pkey_len);
...

    {
        ...
        if (ri->cache_info.signing_key_cert &&
            ...
            keytype = "ed25519";
            ed25519_public_to_base64(idbuf, &ri->cache_info.signing_key_cert->signing_key);
        } else {
            keytype = "rsa1024";
    ...
    {
        smartlist_t *lst =
            microdescs_parse_from_string(output, output + strlen(output), 0, SAVED_NOWHERE, NULL);
 ...
 * on success. */
static ssize_t
dirvote_format_microdesc_vote_line(char *out_buf, size_t out_buf_len, const microdesc_t *md,
                                   int consensus_method_low, int consensus_method_high)
{
    ssize_t ret = -1;
    char d64[BASE64_DIGEST256_LEN + 1];
    char *microdesc_consensus_methods =
        make_consensus_method_list(consensus_method_low, consensus_method_high, ",");
    tor_assert(microdesc_consensus_methods);

    digest256_to_base64(d64, md->digest);

    if (tor_snprintf(out_buf, out_buf_len, "m %s sha256=%s\n", microdesc_consensus_methods, d64) <
        0)
        goto out;
...
static const struct consensus_method_range_t {
    ...
    {MIN_SUPPORTED_CONSENSUS_METHOD, MIN_METHOD_FOR_CANONICAL_FAMILIES_IN_MICRODESCS - 1},
    {MIN_METHOD_FOR_CANONICAL_FAMILIES_IN_MICRODESCS, MAX_SUPPORTED_CONSENSUS_METHOD},
    {-1, -1}};
                                        ...
                                        smartlist_t *microdescriptors_out)
{
    ...
    for (cmr = microdesc_consensus_methods; cmr->low != -1 && cmr->high != -1; cmr++) {
        microdesc_t *md = dirvote_create_microdescriptor(ri, cmr->low);
        if (md) {
            microdesc_vote_line_t *e = tor_malloc_zero(sizeof(microdesc_vote_line_t));
            e->md = md;
            e->low = cmr->low;
            e->high = cmr->high;
            e->next = entries;
    ...
    for (ep = entries; ep; ep = ep->next) {
        while (ep->next && fast_memeq(ep->md->digest, ep->next->md->digest, DIGEST256_LEN) &&
    ...
    while ((ep = entries)) {
        char buf[128];
        vote_microdesc_hash_t *h;
        if (dirvote_format_microdesc_vote_line(buf, sizeof(buf), ep->md, ep->low, ep->high) >= 0) {
...
dirvote_clear_commits(networkstatus_t *ns)
{
    ...
    if (ns->sr_info.commits) {
        SMARTLIST_FOREACH(ns->sr_info.commits, sr_commit_t *, c, sr_commit_free(c));
...
dirvote_dirreq_get_status_vote(const char *url, smartlist_t *items, smartlist_t *dir_items)
{
    ...
    if (!strcmp(url, "consensus")) {
        ...
        if (!strcmpstart(url, "d/")) {
            ...
            flags = DGV_BY_ID | (current ? DGV_INCLUDE_PREVIOUS : DGV_INCLUDE_PENDING);
        }
...
dirserv_get_bandwidth_for_router_kb(const routerinfo_t *ri)
{
    ...
    if (ri) {
         ...
         */
        if (dirserv_query_measured_bw_cache_kb(ri->cache_info.identity_digest, &mbw_kb, NULL)) {
            /* Got one! */
...
compare_routerinfo_by_ip_and_bw_(const void **a, const void **b)
{
    ...
    first_is_auth = router_digest_is_trusted_dir(first->cache_info.identity_digest);
    second_is_auth = router_digest_is_trusted_dir(second->cache_info.identity_digest);
    ...
    return fast_memcmp(first->cache_info.identity_digest, second->cache_info.identity_digest,
                       DIGEST_LEN);
}
...
format_recommended_version_list(const config_line_t *ln, int warn)
{
    ...
    for (; ln; ln = ln->next) {
        smartlist_split_string(versions, ln->value, ",", SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 0);
    }
    ...
    SMARTLIST_FOREACH_BEGIN (versions, char *, v) {
        if (strchr(v, ' ')) {
            ...
            smartlist_split_string(more_versions, v, NULL, SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK,
                                   0);
            tor_free(v);
        }
    } SMARTLIST_FOREACH_END (v);
...
clear_status_flags_on_sybil(routerstatus_t *rs)
{
    rs->is_authority = rs->is_exit = rs->is_stable = rs->is_fast = rs->is_flagged_running =
        rs->is_named = rs->is_valid = rs->is_hs_dir = rs->is_v2_dir = rs->is_possible_guard = 0;
 ...
 * authorities) */
networkstatus_t *
dirserv_generate_networkstatus_vote_obj(crypto_pk_t *private_key, authority_cert_t *cert)
{
    const or_options_t *options = get_options();
    ...
    if (d_options->VersioningAuthoritativeDirectory) {
        client_versions = format_recommended_version_list(d_options->RecommendedClientVersions, 0);
        server_versions = format_recommended_version_list(d_options->RecommendedServerVersions, 0);
    }
     ...
     * means. */
    SMARTLIST_FOREACH(rl->routers, routerinfo_t *, ri, { dirserv_set_router_is_running(ri, now); });
    ...
    SMARTLIST_FOREACH_BEGIN (routers, routerinfo_t *, ri) {
        ...
        if (ri->protocol_list && protover_contains_long_protocol_names(ri->protocol_list)) {
            continue;
        }
        if (ri->cache_info.published_on >= cutoff) {
            ...
            rs = &vrs->status;
            dirauth_set_routerstatus_from_routerinfo(rs, node, ri, now, listbadexits);
            ...
            vrs->microdesc = dirvote_format_all_microdesc_vote_lines(ri, now, microdescriptors);
    ...
    {
        smartlist_t *added =
            microdescs_add_list_to_cache(get_microdesc_cache(), microdescriptors, SAVED_NOWHERE, 0);
    ...
    if (options->V3BandwidthsFile) {
        ...
        dirserv_read_measured_bandwidths(options->V3BandwidthsFile, routerstatuses, bw_file_headers,
                                         bw_file_digest256);
    ...
    v3_out->published = now;
    {
        ...
        v3_out->valid_after = voting_sched_get_start_of_interval_after(
            now, (int)last_consensus_interval, options->TestingV3AuthVotingStartOffset);
    ...
    v3_out->valid_until = v3_out->valid_after + (timing.vote_interval * timing.n_intervals_valid);
    ...
    v3_out->recommended_relay_protocols = tor_strdup(DIRVOTE_RECCOMEND_RELAY_PROTO);
    v3_out->recommended_client_protocols = tor_strdup(DIRVOTE_RECCOMEND_CLIENT_PROTO);

    v3_out->required_relay_protocols = tor_strdup(DIRVOTE_REQUIRE_RELAY_PROTO);
    v3_out->required_client_protocols = tor_strdup(DIRVOTE_REQUIRE_CLIENT_PROTO);
    ...
    tor_assert_nonfatal(protover_all_supported(v3_out->recommended_relay_protocols, NULL));
    tor_assert_nonfatal(protover_all_supported(v3_out->recommended_client_protocols, NULL));
    ...
    if (d_options->ConsensusParams) {
        ...
        smartlist_split_string(v3_out->net_params, d_options->ConsensusParams, NULL, 0, 0);
