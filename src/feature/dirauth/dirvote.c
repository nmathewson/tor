...
static int dirvote_add_signatures_to_pending_consensus(
    pending_consensus_t *pc, ns_detached_signatures_t *sigs, const char *source,
    int severity, const char **msg_out);
                               ...
                               tor_addr_port_t *best_alt_orport_out)
{
    ...
    if (best_alt_orport_out) {
        ...
        if (most_alt_orport) {
            ...
            log_debug(
                LD_DIR, "\"a\" line winner for %s is %s", most->status.nickname,
    ...
    if (3 * E >= T && 3 * G >= T) { // E >= T/3 && G >= T/3
        ...
        if (berr) {
            log_warn(LD_DIR,
                     ...
                     (int)Wme, (int)Wmg, (int)Wed, (int)Wee, (int)Wgd, (int)Wgg,
                     (int)Wme, (int)Wmg, (int)weight_scale);
            return 0;
        }
        ...
        if (R + D < S) { // Subcase a
            ...
            berr =
                networkstatus_check_weights(Wgg, Wgd, Wmg, Wme, Wmd, Wee, Wed,
                                            weight_scale, G, M, E, D, T, 10, 1);
            ...
            if (berr != BW_WEIGHTS_NO_ERROR &&
                ...
                log_warn(LD_DIR,
                         ...
                         " M=%" PRId64 " E=%" PRId64 " D=%" PRId64 " T=%" PRId64
                         " Wmd=%d Wme=%d Wmg=%d Wed=%d Wee=%d"
                         " Wgd=%d Wgg=%d Wme=%d Wmg=%d weight_scale=%d",
                         berr, casename, (G), (M), (E), (D), (T), (int)Wmd,
...
update_total_bandwidth_weights(const routerstatus_t *rs, int is_exit,
                               int is_guard, int64_t *G, int64_t *M, int64_t *E,
                               int64_t *D, int64_t *T)
{
                                ...
                                consensus_flavor_t flavor)
{
    ...
    /* Add the actual router entries. */
    {
         ...
         * members for known_flags, so no value will be greater than 63, so it's
         * safe to do UINT64_C(1) << index on these values.  But note also that
         * named_flag and unnamed_flag are initialized to -1, so we need to
         * check that they're actually set before doing UINT64_C(1) << index
         * with them.*/
        SMARTLIST_FOREACH_BEGIN(votes, networkstatus_t *, v)
        {
            flag_map[v_sl_idx] =
        ...
        for (i = 0; i < num_routers; ++i) {
            ...
            rs =
                compute_routerstatus_consensus(matching_descs, consensus_method,
...
dirserv_get_bandwidth_for_router_kb(const routerinfo_t *ri)
{
    ...
    if (ri) {
         ...
         * as_of_out here, on the theory that a stale measured bandwidth is
         * still better to trust than an advertised one.
         */
                                        ...
                                        authority_cert_t *cert)
{
    ...
    if (options->VersioningAuthoritativeDir) {
        client_versions = format_recommended_version_list(
            options->RecommendedClientVersions, 0);
        server_versions = format_recommended_version_list(
            options->RecommendedServerVersions, 0);
    }
    ...
    if (options->ConsensusParams) {
        ...
        smartlist_split_string(v3_out->net_params, options->ConsensusParams,
                               NULL, 0, 0);
