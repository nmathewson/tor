                          ...
                          networkstatus_t *v3_ns)
{
...

    {
        ...
        /* XXXX Abstraction violation: should be pulling a field out of
         * v3_ns.*/
        ...
        if (v3_ns->bw_file_headers) {
            ...
            if (!BUG(smartlist_len(v3_ns->bw_file_headers) >
                ...
                bw_file_headers = smartlist_join_strings(
                    v3_ns->bw_file_headers, " ", 0, NULL);
                if (BUG(strlen(bw_file_headers) >
            ...
            if (!bw_file_headers) {
                /* If parsing failed, add a bandwidth header line with no
                 * entries */
         ...
         * v3_ns->b64_digest_bw_file will contain the digest when
         * V3BandwidthsFile is configured and the bandwidth file could be read,
         * even if it was not parseable.
         */
        ...
        smartlist_add_asprintf(
            ...
            ,
            v3_ns->type == NS_TYPE_VOTE ? "vote" : "opinion", methods,
            published, va, fu, vu, v3_ns->vote_seconds, v3_ns->dist_seconds,
        ...
        if (!tor_digest_is_zero(voter->legacy_id_digest)) {
            ...
            base16_encode(fpbuf, sizeof(fpbuf), voter->legacy_id_digest,
                          DIGEST_LEN);
    ...
    SMARTLIST_FOREACH_BEGIN (v3_ns->routerstatus_list, vote_routerstatus_t *,
        ...
        rsf = routerstatus_format_entry(&vrs->status, vrs->version,
                                        vrs->protocols, NS_V3_VOTE, vrs);
        if (rsf)
                                      ...
                                      int method, digest_algorithm_t alg)
{
    ...
    for (h = vrs->microdesc; h; h = h->next) {
        ...
        size_t num_len;
        /* cp looks like \d+(,\d+)* (digesttype=val )+ .  Let's hunt for mstr
         * in the first part. */
        while (1) {
            ...
            if (num_len == mlen && fast_memeq(mstr, cp, mlen)) {
                ...
                /* XXXX ignores extraneous stuff if the digest is too long.
...
compare_vote_rs(const vote_routerstatus_t *a, const vote_routerstatus_t *b)
{
    ...
    if ((r = fast_memcmp(a->status.descriptor_digest,
         ...
         * the descriptor digests matched, so somebody is making SHA1
         * collisions.
         */
                               ...
                               tor_addr_port_t *best_alt_orport_out)
{
    ...
    if (cur_n > most_n || (cur && cur_n == most_n &&
                           cur->status.published_on > most_published)) {
        most = cur;
        // most_n = cur_n; // unused after this point.
        // most_published = cur->status.published_on; // unused after this
        // point.
    }
    ...
    if (best_alt_orport_out) {
        ...
        SMARTLIST_FOREACH_BEGIN (votes, vote_routerstatus_t *, rs) {
            ...
            if (compare_vote_rs(most, rs) == 0 &&
                ...
                smartlist_add(alt_orports,
                              tor_addr_port_new(&rs->status.ipv6_addr,
                                                rs->status.ipv6_orport));
            }
        } SMARTLIST_FOREACH_END (rs);
        ...
        if (most_alt_orport) {
            memcpy(best_alt_orport_out, most_alt_orport,
                   sizeof(tor_addr_port_t));
            log_debug(
                LD_DIR, "\"a\" line winner for %s is %s",
    ...
    if (microdesc_digest256_out) {
        ...
        if (best_microdesc_digest)
            memcpy(microdesc_digest256_out, best_microdesc_digest,
                   DIGEST256_LEN);
                                     ...
                                     const char *keyword, int32_t default_val)
{
    ...
    SMARTLIST_FOREACH_BEGIN (param_list, const char *, k_v_pair) {
        if (!strcmpstart(k_v_pair, keyword) &&
            k_v_pair[strlen(keyword)] == '=') {
            const char *integer_str = &k_v_pair[strlen(keyword) + 1];
            int ok;
            value = (int32_t)tor_parse_long(integer_str, 10, INT32_MIN,
                                            INT32_MAX, &ok, NULL);
            if (BUG(!ok))
                return default_val;
            ++n_found;
...
dirvote_compute_params(smartlist_t *votes, int method, int total_authorities)
{
    ...
    SMARTLIST_FOREACH_BEGIN (param_list, const char *, param) {
        ...
        vals[i++] = (int32_t)tor_parse_long(eq + 1, 10, INT32_MIN, INT32_MAX,
                                            &ok, NULL);
        ...
        if (!next_param || strncmp(next_param, param, cur_param_len)) {
            ...
            if (i > total_authorities / 2 || i >= MIN_VOTES_FOR_PARAM) {
                ...
                tor_snprintf(out_string + cur_param_len, 64, "%ld",
                             (long)median);
                            ...
                            int do_balance)
{
    ...
    if (do_balance) {
        ...
        if (!CHECK_EQ(Wgg * G + Wgd * D, Wee * E + Wed * D,
                      (margin * T) / 3)) {
        ...
        if (!CHECK_EQ(Wgg * G + Wgd * D,
                      M * scale + Wmd * D + Wme * E + Wmg * G,
                      (margin * T) / 3)) {
    ...
    if (3 * E >= T && 3 * G >= T) { // E >= T/3 && G >= T/3
        ...
        if (berr) {
            log_warn(LD_DIR,
                     ...
                     berr, casename, (G), (M), (E), (D), (T), (int)Wmd,
                     (int)Wme, (int)Wmg, (int)Wed, (int)Wee, (int)Wgd,
                     (int)Wgg, (int)Wme, (int)Wmg, (int)weight_scale);
            return 0;
        }
        ...
        if (R + D < S) { // Subcase a
            ...
            if (berr) {
                ...
                if (Wmd < 0) { // Can happen if M > T/3
                    ...
                    log_warn(
                        LD_DIR,
                        "Too much Middle bandwidth on the network to "
                        "calculate "
                ...
                berr = networkstatus_check_weights(Wgg, Wgd, Wmg, Wme, Wmd,
                                                   Wee, Wed, weight_scale, G,
                                                   M, E, D, T, 10, 1);
            }
            if (berr != BW_WEIGHTS_NO_ERROR &&
                berr != BW_WEIGHTS_BALANCE_MID_ERROR) {
                log_warn(LD_DIR,
                         "Bw Weights error %d for %s v10. G=%" PRId64
                         " M=%" PRId64 " E=%" PRId64 " D=%" PRId64
                                ...
                                consensus_flavor_t flavor)
{
     ...
     * routers we might need to talk about. */
    {
        ...
        server_versions = compute_consensus_versions_list(
            combined_server_versions, n_versioning_servers);
        client_versions = compute_consensus_versions_list(
            combined_client_versions, n_versioning_clients);
...

    {
        ...
        smartlist_add_asprintf(chunks,
                               ...
                               "%s" /* packages */
                               "known-flags %s\n",
                               va_buf, fu_buf, vu_buf, vote_seconds,
                               dist_seconds, client_versions, server_versions,
                               packages, flaglist);
...

    {
        ...
        for (idx = 0; idx < 4; ++idx) {
            char *proto_line =
                compute_nth_protocol_set(idx, num_dirauth, votes);
            if (BUG(!proto_line))
                continue;
...

    {
         ...
         * instance, if we have 9 dirauth, the default value is 6. The
         * following calculation will round it down. */
        ...
        char *srv_lines =
            sr_get_string_for_consensus(votes, num_srv_agreements);
        if (srv_lines != NULL) {
    ...
    /* Add the authority sections. */
    {
        ...
        SMARTLIST_FOREACH_BEGIN (dir_sources, const dir_src_ent_t *, e) {
            ...
            base16_encode(fingerprint, sizeof(fingerprint), e->digest,
                          DIGEST_LEN);
...

    {
        ...
        /* XXXX Extract this code into a common function.  Or don't!  see
         * #19011 */
        if (params) {
        ...
        if (max_unmeasured_param) {
            ...
            if (eq) {
                max_unmeasured_bw_kb = (uint32_t)tor_parse_ulong(
                    eq + 1, 10, 1, UINT32_MAX, &ok, NULL);
                if (!ok) {
                    log_warn(LD_DIR,
    ...
    /* Add the actual router entries. */
    {
        ...
        int *n_voter_flags; /* n_voter_flags[j] is the number of flags that
                             * votes[j] knows about. */
        int *n_flag_voters; /* n_flag_voters[f] is the number of votes that
                             * care about flags[f]. */
        ...
        /* Build the flag indexes. Note that no vote can have more than 64
         * members for known_flags, so no value will be greater than 63, so
         * it's safe to do UINT64_C(1) << index on these values.  But note also
         * that named_flag and unnamed_flag are initialized to -1, so we need
         * to check that they're actually set before doing UINT64_C(1) << index
         * with them.*/
        SMARTLIST_FOREACH_BEGIN (votes, networkstatus_t *, v) {
            flag_map[v_sl_idx] =
            ...
            SMARTLIST_FOREACH_BEGIN (votes, networkstatus_t *, v) {
                ...
                SMARTLIST_FOREACH_BEGIN (v->routerstatus_list,
                    ...
                    if ((rs->flags & nf) != 0) {
                        ...
                        if (!d) {
                            /* We have no name officially mapped to this
                             * digest. */
                            ...
                            /* It's already a conflict, or it's already this
                             * ID. */
                        }
                    }
                } SMARTLIST_FOREACH_END (rs);
                ...
                SMARTLIST_FOREACH_BEGIN (v->routerstatus_list,
                    ...
                    if ((rs->flags & uf) != 0) {
                        ...
                        if (d == conflict || d == unknown) {
                            ...
                            strmap_set_lc(name_to_id_map, rs->status.nickname,
                                          unknown);
                        } else if (fast_memeq(d, rs->status.identity_digest,
                                              DIGEST_LEN)) {
                            /* Authorities disagree about this nickname. */
                            strmap_set_lc(name_to_id_map, rs->status.nickname,
                                          conflict);
        ...
        for (i = 0; i < num_routers; ++i) {
            ...
            for (int voter_idx = 0; voter_idx < smartlist_len(votes);
                 ++voter_idx) {
                if (vrs_lst[voter_idx] == NULL)
                    continue; /* This voter had nothing to say about this
                                 entry. */
                ...
                if (named_flag[voter_idx] >= 0 &&
                    ...
                    if (chosen_name &&
                        strcmp(chosen_name, rs->status.nickname)) {
                        log_notice(LD_DIR,
            ...
            tor_assert(fast_memeq(current_rsa_id, rs->status.identity_digest,
                                  DIGEST_LEN));
            ...
            if (chosen_name && !naming_conflict) {
                ...
                strlcpy(rs_out.nickname, rs->status.nickname,
            ...
            SMARTLIST_FOREACH_BEGIN (flags, const char *, fl) {
                if (!strcmp(fl, "Named")) {
                    ...
                    if (flag_counts[fl_sl_idx] >
                        n_flag_voters[fl_sl_idx] / 2) {
                        smartlist_add(chosen_flags, (char *)fl);
                        if (!strcmp(fl, "Exit"))
                            is_exit = 1;
            ...
            if (is_guard && num_guardfraction_inputs > 2) {
                ...
                rs_out.guardfraction_percentage = median_uint32(
                    measured_guardfraction, num_guardfraction_inputs);
            ...
            /* Update total bandwidth weights with the bandwidths of this
             * router. */
            {
                update_total_bandwidth_weights(&rs_out, is_exit, is_guard, &G,
                                               &M, &E, &D, &T);
            }
             ...
             */
            {
                ...
                SMARTLIST_FOREACH_BEGIN (matching_descs, vote_routerstatus_t *,
                                         vsr) {
                    ...
                    if (vsr->status.has_exitsummary &&
                        ...
                        } else if (strcmp(chosen_exitsummary,
                                          vsr->status.exitsummary)) {
                            /* Great.  There's disagreement among the voters.
                ...
                if (exitsummary_disagreement) {
                    ...
                    base16_encode(id, sizeof(dd), rs_out.identity_digest,
                                  DIGEST_LEN);
                    base16_encode(dd, sizeof(dd), rs_out.descriptor_digest,
                                  DIGEST_LEN);
                    log_warn(LD_DIR,
                             "The voters disagreed on the exit policy summary "
                             " for router %s with descriptor %s.  This really "
                             "shouldn't"
                             " have happened.",
                             id, dd);
                    ...
                    smartlist_clear(exitsummaries);
                    SMARTLIST_FOREACH(
                        matching_descs, vote_routerstatus_t *, vsr, {
                            if (vsr->status.has_exitsummary)
                                smartlist_add(exitsummaries,
                                              vsr->status.exitsummary);
                        });
                    smartlist_sort_strings(exitsummaries);
                    chosen_exitsummary =
...

    {
        ...
        if (bw_weight_param) {
            ...
            if (eq) {
                weight_scale =
                    tor_parse_long(eq + 1, 10, 1, INT32_MAX, &ok, NULL);
                if (!ok) {
        ...
        added_weights = networkstatus_compute_bw_weights_v10(
            chunks, G, M, E, D, T, weight_scale);
    }
    ...
    /* Add a signature. */
    {
        ...
        if (legacy_id_key_digest && legacy_signing_key) {
            ...
            if (!(signature = router_get_dirobj_signature(
                      digest, digest_len, legacy_signing_key))) {
                log_warn(LD_BUG, "Couldn't sign consensus networkstatus.");
                goto done;
            }
            smartlist_add(chunks, signature);
                                      ...
                                      const char **msg_out)
{
    ...
    SMARTLIST_FOREACH_BEGIN (siglist, document_signature_t *, sig) {
        ...
        log_info(LD_DIR, "Looking at signature from %s using %s",
                 voter_identity, algorithm);
        /* If the target doesn't know about this voter, then forget it. */
        if (!target_voter) {
            log_info(LD_DIR, "We do not know any voter with ID %s",
                     voter_identity);
            continue;
        }
        ...
        /* If the target already has a good signature from this voter, then
         * skip this one. */
        ...
        if (!sig->good_signature && !sig->bad_signature) {
            ...
            if (cert) {
                /* Not checking the return value here, since we are going to
                 * look at the status of sig->good_signature in a moment. */
                (void)networkstatus_check_document_signature(target, sig,
                                                             cert);
            }
        }
        ...
        if (sig->good_signature || !old_sig || old_sig->bad_signature) {
            log_info(LD_DIR, "Adding signature from %s with %s",
                     voter_identity, algorithm);
        ...
        SMARTLIST_FOREACH_BEGIN (v->sigs, document_signature_t *, sig) {
            ...
            if (flavor == FLAV_NS) {
                ...
                smartlist_add_asprintf(
                    ...
                    for_detached_signatures ? flavor_name : "", digest_name,
                    id, sk);
            }
...
dirvote_act(const or_options_t *options, time_t now)
{
     ...
     * recalculate the timings to match our vote. The reason to do that is if
     * we have a voting schedule initialized 1 minute ago, the voting timings
     * might not be aligned to what we should expect with "now". This is
     * especially true for TestingTorNetwork using smaller timings.  */
...
dirvote_perform_vote(void)
{
    ...
    directory_post_to_dirservers(
        DIR_PURPOSE_UPLOAD_VOTE, ROUTER_PURPOSE_GENERAL, V3_DIRINFO,
        ...
        if (fast_memeq(v->vote->cert->cache_info.identity_digest,
            ...
            if (fast_memeq(vi_old->vote_digest, vi->vote_digest, DIGEST_LEN)) {
                ...
                v->vote_body = new_cached_dir(
                    tor_strndup(vote_body, end_of_vote - vote_body),
                    vote->published);
                v->vote = vote;
                if (end_of_vote &&
...
dirvote_compute_consensuses(void)
{
...

    {
        ...
        for (flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
            ...
            consensus = networkstatus_parse_vote_from_string(
                consensus_body, strlen(consensus_body), NULL,
                NS_TYPE_CONSENSUS);
            if (!consensus) {
    ...
    if (pending_consensus_signature_list) {
        ...
        SMARTLIST_FOREACH_BEGIN (pending_consensus_signature_list, char *,
                                 sig) {
            const char *msg = NULL;
            int r = dirvote_add_signatures_to_all_pending_consensuses(
                sig, "pending", &msg);
            if (r >= 0)
                n_sigs += r;
            else
...
dirvote_publish_consensus(void)
{
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        ...
        tor_assert(name);
        if (!pending->consensus || networkstatus_check_consensus_signature(
                                       pending->consensus, 1) < 0) {
...

    {
        ...
        if (ri->cache_info.signing_key_cert &&
            ...
            ed25519_public_to_base64(
                idbuf, &ri->cache_info.signing_key_cert->signing_key);
        } else {
            keytype = "rsa1024";
                                        ...
                                        smartlist_t *microdescriptors_out)
{
    ...
    /* Format them into vote_microdesc_hash_t, and add to
...
extract_shared_random_commits(networkstatus_t *ns, const smartlist_t *tokens)
{
     ...
     * for forward compatibility, it's the parse commit job to inform us if
    ...
    SMARTLIST_FOREACH_BEGIN (commits, directory_token_t *, tok) {
        ...
        if (commit == NULL) {
            /* Get voter identity so we can warn that this dirauth vote
             * contains commit we can't parse. */
            ...
            /* Commitment couldn't be parsed. Continue onto the next commit
             * because this one could be unsupported for instance. */
            continue;
        }
...
dirserv_get_bandwidth_for_router_kb(const routerinfo_t *ri)
{
    ...
    if (ri) {
        /*
         * * First try to see if we have a measured bandwidth; don't bother
         * with as_of_out here, on the theory that a stale measured bandwidth
         * is still better to trust than an advertised one.
         */
        ...
        if (strchr(v, ' ')) {
            if (warn)
                log_warn(LD_DIRSERV,
                         ...
                         "(These are supposed to be comma-separated; I'll "
                         "pretend you "
                         "used commas instead.)",
                         escaped(v));
...
routers_make_ed_keys_unique(smartlist_t *routers)
{
    ...
    SMARTLIST_FOREACH_BEGIN (routers, routerinfo_t *, ri) {
        ...
        if ((ri2 = digest256map_get(by_ed_key, pk))) {
            /* Duplicate; must omit one.  Set the omit_from_vote flag in
             * whichever one has the earlier published_on. */
                                        ...
                                        authority_cert_t *cert)
{
    ...
    const int vote_on_reachability =
    ...
    if (options->V3BandwidthsFile) {
        ...
        /*
         * No bandwidths file; clear the measured bandwidth cache in case we
         * had one last time around.
         */
    ...
    SMARTLIST_FOREACH_BEGIN (routers, routerinfo_t *, ri) {
        ...
        if (ri->cache_info.published_on >= cutoff) {
            ...
            vrs->microdesc = dirvote_format_all_microdesc_vote_lines(
                ri, now, microdescriptors);
    ...
    tor_assert(
        protover_all_supported(v3_out->required_client_protocols, NULL));
