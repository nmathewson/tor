 ...
 */
void
dirserv_orconn_tls_done(const tor_addr_t *addr, uint16_t or_port, const char *digest_rcvd,
                        const ed25519_public_key_t *ed_id_rcvd)
{
    ...
    if (dirauth_get_options()->AuthDirTestEd25519LinkKeys &&
        node_supports_ed25519_link_authentication(node, 1) && ri->cache_info.signing_key_cert) {
         ...
         * needs to match. */
        const ed25519_public_key_t *expected_id = &ri->cache_info.signing_key_cert->signing_key;
    ...
    if (router_has_orport(ri, &orport)) {
        ...
        if (!authdir_mode_bridge(get_options()) || ri->purpose == ROUTER_PURPOSE_BRIDGE) {
            char addrstr[TOR_ADDR_BUF_LEN];
            /* This is a bridge or we're not a bridge authority --
               mark it as reachable.  */
            log_info(LD_DIRSERV, "Found router %s to be reachable at %s:%d. Yay.",
                     router_describe(ri), tor_addr_to_str(addrstr, addr, sizeof(addrstr), 1),
                     ri->or_port);
...
int
dirserv_should_launch_reachability_test(const routerinfo_t *ri, const routerinfo_t *ri_old)
{
...
dirserv_single_reachability_test(time_t now, routerinfo_t *router)
{
    ...
    if (dirauth_options->AuthDirTestEd25519LinkKeys &&
        node_supports_ed25519_link_authentication(node, 1) && router->cache_info.signing_key_cert) {
    ...
    chan = channel_tls_connect(&router_addr, router->or_port, router->cache_info.identity_digest,
                               ed_id_key);
    if (chan)
