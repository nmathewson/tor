                        ...
                        const ed25519_public_key_t *ed_id_rcvd)
{
    ...
    if (dirauth_get_options()->AuthDirTestEd25519LinkKeys &&
        ...
        /* We allow the node to have an ed25519 key if we haven't been told one
         * in the routerinfo, but if we *HAVE* been told one in the routerinfo,
         * it needs to match. */
        ...
        if (!ed_id_rcvd || !ed25519_pubkey_eq(ed_id_rcvd, expected_id)) {
            log_info(LD_DIRSERV,
                     ...
                     fmt_addr(addr), or_port,
                     hex_str(digest_rcvd, DIGEST_LEN));
    ...
    if (router_has_orport(ri, &orport)) {
        ...
        if (!authdir_mode_bridge(get_options()) ||
            ...
            if (tor_addr_family(addr) == AF_INET) {
                rep_hist_note_router_reachable(digest_rcvd, addr, or_port,
                                               now);
                                        ...
                                        const routerinfo_t *ri_old)
{
    if (!authdir_mode_handles_descs(get_options(), ri->purpose))
        return 0;
    if (!ri_old) {
        /* New router: Launch an immediate reachability test, so we will have
         * an opinion soon in case we're generating a consensus soon */
        return 1;
    }
...
dirserv_single_reachability_test(time_t now, routerinfo_t *router)
{
    ...
    if (dirauth_get_options()->AuthDirHasIPv6Connectivity == 1 &&
        ...
        char addrstr[TOR_ADDR_BUF_LEN];
        log_debug(
            LD_OR, "Testing reachability of %s at %s:%u.", router->nickname,
