...
static smartlist_t *
detached_get_signatures(ns_detached_signatures_t *sigs, const char *flavor_name)
{
...
networkstatus_parse_detached_signatures(const char *s, const char *eos)
{
    ...
    SMARTLIST_FOREACH_BEGIN (tokens, directory_token_t *, _tok) {
        ...
        if (base16_decode(digests->d[alg], digest_length, hexdigest, strlen(hexdigest)) !=
            (int)digest_length) {
    ...
    SMARTLIST_FOREACH_BEGIN (tokens, directory_token_t *, _tok) {
        ...
        if (strlen(id_hexdigest) != HEX_DIGEST_LEN ||
            base16_decode(id_digest, sizeof(id_digest), id_hexdigest, HEX_DIGEST_LEN) !=
                sizeof(id_digest)) {
            log_warn(LD_DIR,
        ...
        SMARTLIST_FOREACH(siglist, document_signature_t *, dsig, {
            if (dsig->alg == alg && tor_memeq(id_digest, dsig->identity_digest, DIGEST_LEN) &&
...
ns_detached_signatures_free_(ns_detached_signatures_t *s)
{
    ...
    if (s->signatures) {
        ...
        {
            SMARTLIST_FOREACH(sigs, document_signature_t *, sig, document_signature_free(sig));
            smartlist_free(sigs);
        }
        STRMAP_FOREACH_END;
