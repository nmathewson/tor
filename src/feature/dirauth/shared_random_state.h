...
typedef struct sr_state_t {
    ...
    /** Indicate if the state contains an SRV that was _just_ generated. This
     * is used during voting so that we know whether to use the super majority
     * rule or not when deciding on keeping it for the consensus. It is
     * _always_ set to 0 post consensus.
     *
