                                    ...
                                    int need_uptime, int need_capacity)
{
    if (need_uptime) {
        if (!enough_mtbf_info) {
             ...
             * "use uptime if we don't have mtbf data" to "don't advertise
             * Stable on v3 if we don't have enough mtbf data."  Or maybe not,
             * since if we ever hit a point where we need to reset a lot of
             * authorities at once, none of them would be in a position to
             * declare Stable.
             */
            ...
            double mtbf = rep_hist_get_stability(
    ...
    if (!ri->bandwidthcapacity) {
        if (get_options()->TestingTorNetwork) {
            if (dirauth_get_options()->TestingMinExitFlagThreshold > 0) {
                /* If we're in a TestingTorNetwork, and
                 * TestingMinExitFlagThreshold is, then require
                 * bandwidthcapacity */
                return 0;
            }
        } else {
            /* If we're not in a TestingTorNetwork, then require
             * bandwidthcapacity */
            return 0;
        }
    }
                                ...
                                time_t now)
{
    ...
    if (get_uptime() >
        ...
        uptime =
            MIN(rep_hist_get_uptime(router->cache_info.identity_digest, now),
                                ...
                                int require_mbw)
{
    ...
    if (options->TestingTorNetwork) {
        min_bw_kb =
            (int64_t)dirauth_options->TestingMinExitFlagThreshold / 1000;
    }
...
dirserv_compute_performance_thresholds(digestmap_t *omit_as_sybil)
{
    ...
    if (n_active) {
        ...
        fast_bandwidth_kb =
            find_nth_uint32(bandwidths_kb, n_active, n_active / 8);
    ...
    SMARTLIST_FOREACH_BEGIN (nodelist, node_t *, node) {
        if (router_counts_toward_thresholds(node, now, omit_as_sybil,
            ...
            if (tk < guard_tk)
                continue;
            wfus[n_familiar++] =
                rep_hist_get_weighted_fractional_uptime(id, now);
        }
    } SMARTLIST_FOREACH_END (node);
    if (n_familiar)
...
running_long_enough_to_decide_unreachable(void)
{
    ...
    return time_of_process_start +
               opts->TestingAuthDirTimeToLearnReachability <
           approx_time();
}
...
dirserv_set_router_is_running(routerinfo_t *router, time_t now)
{
    /*XXXX This function is a mess.  Separate out the part that calculates
      whether it's reachable and the part that tells rephist that the router
      was unreachable.
     */
    int answer;
    const or_options_t *options = get_options();
    ...
    if (!answer && running_long_enough_to_decide_unreachable()) {
        /* Not considered reachable. tell rephist about that.
           ...
           been down since at least that time after we last successfully
           reached it.
                                         ...
                                         int listbadexits)
{
    ...
    if (node->is_fast && node->is_stable &&
        ...
        double wfu =
            rep_hist_get_weighted_fractional_uptime(node->identity, now);
