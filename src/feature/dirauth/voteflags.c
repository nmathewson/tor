 ...
 */
static int
dirserv_thinks_router_is_unreliable(time_t now, const routerinfo_t *router, int need_uptime,
                                    int need_capacity)
{
    if (need_uptime) {
        if (!enough_mtbf_info) {
            ...
            if ((unsigned)uptime < stable_uptime && (unsigned)uptime < UPTIME_TO_GUARANTEE_STABLE)
                return 1;
        } else {
            double mtbf = rep_hist_get_stability(router->cache_info.identity_digest, now);
 ...
 */
static int
dirserv_thinks_router_is_hs_dir(const routerinfo_t *router, const node_t *node, time_t now)
{
    long uptime;
    ...
    return (router->wants_to_be_hs_dir && router->supports_tunnelled_dir_requests &&
...
static int
router_counts_toward_thresholds(const node_t *node, time_t now, const digestmap_t *omit_as_sybil,
                                int require_mbw)
{
    /* Have measured bw? */
    ...
    return node->ri && router_is_active(node->ri, node, now) &&
           ...
           (dirserv_get_credible_bandwidth_kb(node->ri) >= min_bw_kb) && (have_mbw || !require_mbw);
}
...
dirserv_compute_performance_thresholds(digestmap_t *omit_as_sybil)
{
    ...
    bandwidths_excluding_exits_kb = tor_calloc(smartlist_len(nodelist), sizeof(uint32_t));
    ...
    SMARTLIST_FOREACH_BEGIN (nodelist, node_t *, node) {
        ...
        if (router_counts_toward_thresholds(node, now, omit_as_sybil, require_mbw)) {
    ...
    if (n_active) {
        ...
        guard_bandwidth_including_exits_kb = third_quartile_uint32(bandwidths_kb, n_active);
...
#define ABSOLUTE_MIN_VALUE_FOR_FAST_FLAG 4
        ...
        min_fast =
            networkstatus_get_param(NULL, "FastFlagMinThreshold", ABSOLUTE_MIN_VALUE_FOR_FAST_FLAG,
                                    ABSOLUTE_MIN_VALUE_FOR_FAST_FLAG, INT32_MAX);
        if (options->TestingTorNetwork) {
            min_fast = (int32_t)dirauth_options->TestingMinFastFlagThreshold;
        }
        max_fast =
            networkstatus_get_param(NULL, "FastFlagMaxThreshold", INT32_MAX, min_fast, INT32_MAX);
    ...
    if (n_active_nonexit) {
        guard_bandwidth_excluding_exits_kb = find_nth_uint32(
            bandwidths_excluding_exits_kb, n_active_nonexit, n_active_nonexit * 3 / 4);
    }
    ...
    log_info(LD_DIRSERV,
             ...
             (unsigned long)fast_bandwidth_kb, guard_wfu * 100, (unsigned long)guard_tk,
             (unsigned long)guard_bandwidth_including_exits_kb,
             (unsigned long)guard_bandwidth_excluding_exits_kb, enough_mtbf_info ? "" : " don't");
...
{
    char *result = NULL;
    const int measured_threshold = dirauth_get_options()->MinMeasuredBWsForAuthToIgnoreAdvertised;
    const int enough_measured_bw = dirserv_get_last_n_measured_bws() > measured_threshold;
    ...
    tor_asprintf(&result,
                 ...
                 (unsigned long)stable_uptime, (unsigned long)stable_mtbf,
                 (unsigned long)fast_bandwidth_kb * 1000, guard_wfu * 100, (unsigned long)guard_tk,
                 (unsigned long)guard_bandwidth_including_exits_kb * 1000,
                 (unsigned long)guard_bandwidth_excluding_exits_kb * 1000, enough_mtbf_info ? 1 : 0,
...
running_long_enough_to_decide_unreachable(void)
{
    ...
    return time_of_process_start + opts->TestingAuthDirTimeToLearnReachability < approx_time();
}
...
dirserv_set_router_is_running(routerinfo_t *router, time_t now)
{
    ...
    } else if (router->is_hibernating && (router->cache_info.published_on +
    ...
    if (!answer && running_long_enough_to_decide_unreachable()) {
         ...
         */
        time_t when = now;
        if (node->last_reachable && node->last_reachable + REACHABILITY_TEST_CYCLE_PERIOD < now)
...
static int
should_publish_node_ipv6(const node_t *node, const routerinfo_t *ri, time_t now)
{
    const dirauth_options_t *options = dirauth_get_options();

    return options->AuthDirHasIPv6Connectivity == 1 && !tor_addr_is_null(&ri->ipv6_addr) &&
           ((node->last_reachable6 >= now - REACHABLE_TIMEOUT) || router_is_me(ri));
}
 ...
 */
void
dirauth_set_routerstatus_from_routerinfo(routerstatus_t *rs, node_t *node, const routerinfo_t *ri,
                                         time_t now, int listbadexits)
{
    ...
    if (node->is_fast && node->is_stable && ri->supports_tunnelled_dir_requests &&
        ((bw_opt && routerbw_kb >= bw_opt / 1000) ||
         routerbw_kb >=
             MIN(guard_bandwidth_including_exits_kb, guard_bandwidth_excluding_exits_kb))) {
    ...
    /* Set rs->is_staledesc. */
    rs->is_staledesc = (ri->cache_info.published_on + DESC_IS_STALE_INTERVAL) < now;
...
dirserv_set_routerstatus_testing(routerstatus_t *rs)
{
    ...
    if (routerset_contains_routerstatus(options->TestingDirAuthVoteExit, rs, 0)) {
        rs->is_exit = 1;
    } else if (options->TestingDirAuthVoteExitIsStrict) {
        rs->is_exit = 0;
    }

    if (routerset_contains_routerstatus(options->TestingDirAuthVoteGuard, rs, 0)) {
        rs->is_possible_guard = 1;
    } else if (options->TestingDirAuthVoteGuardIsStrict) {
        rs->is_possible_guard = 0;
    }

    if (routerset_contains_routerstatus(options->TestingDirAuthVoteHSDir, rs, 0)) {
        rs->is_hs_dir = 1;
