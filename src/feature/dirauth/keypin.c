...
static int keypin_check_and_add_impl(const uint8_t *rsa_id_digest, const uint8_t *ed25519_id_key,
...
HT_GENERATE2(rsamap, keypin_ent_st, rsamap_node, keypin_ent_hash_rsa, keypin_ents_eq_rsa, 0.6,
             tor_reallocarray, tor_free_);

HT_PROTOTYPE(edmap, keypin_ent_st, edmap_node, keypin_ent_hash_ed, keypin_ents_eq_ed);
HT_GENERATE2(edmap, keypin_ent_st, edmap_node, keypin_ent_hash_ed, keypin_ents_eq_ed, 0.6,
             tor_reallocarray, tor_free_);
 ...
 */
int
keypin_check_and_add(const uint8_t *rsa_id_digest, const uint8_t *ed25519_id_key,
                     const int replace_existing_entry)
{
    return keypin_check_and_add_impl(rsa_id_digest, ed25519_id_key, 0, replace_existing_entry);
}
 ...
 */
static int
keypin_check_and_add_impl(const uint8_t *rsa_id_digest, const uint8_t *ed25519_id_key,
                          const int do_not_add, const int replace)
{
    ...
    if (!replace) {
        ...
        if (ent) {
            ...
            tor_assert(fast_memeq(ent->ed25519_key, ed25519_id_key, sizeof(ent->ed25519_key)));
 ...
 * <b>ed25519_id_key</b>. */
static int
keypin_journal_append_entry(const uint8_t *rsa_id_digest, const uint8_t *ed25519_id_key)
{
    ...
    digest256_to_base64(line + BASE64_DIGEST_LEN + 1, (const char *)ed25519_id_key);
...
keypin_parse_journal_line(const char *cp)
{
    ...
    if (base64_decode((char *)ent->rsa_id, sizeof(ent->rsa_id), cp, BASE64_DIGEST_LEN) !=
            DIGEST_LEN ||
...
keypin_clear(void)
{
    ...
    if (bad_entries) {
        log_warn(LD_BUG, "Found %d discrepencies in the keypin database.", bad_entries);
    }
}
