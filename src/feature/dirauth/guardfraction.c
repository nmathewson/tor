 ...
 */
static int
guardfraction_line_apply(const char *guard_id, uint32_t guardfraction_percentage,
                         smartlist_t *vote_routerstatuses)
{
    ...
    vrs =
        smartlist_bsearch(vote_routerstatuses, guard_id, compare_digest_to_vote_routerstatus_entry);
 ...
 */
static int
guardfraction_file_parse_guard_line(const char *guard_line, smartlist_t *vote_routerstatuses,
                                    char **err_msg)
{
    ...
    smartlist_split_string(sl, guard_line, " ", SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 3);
    ...
    guardfraction = (uint32_t)tor_parse_long(inputs_tmp, 10, 0, 100, &num_ok, NULL);
    if (!num_ok) {
    ...
    if (vote_routerstatuses) {
        retval = guardfraction_line_apply(guard_id, guardfraction, vote_routerstatuses);
    } else {
 ...
 */
static int
guardfraction_file_parse_inputs_line(const char *inputs_line, int *total_consensuses,
                                     int *total_days, char **err_msg)
{
    int retval = -1;
    char *inputs_tmp = NULL;
    int num_ok = 1;
    ...
    smartlist_split_string(sl, inputs_line, " ", SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 3);
    ...
    inputs_tmp = smartlist_get(sl, 0);
    *total_consensuses = (int)tor_parse_long(inputs_tmp, 10, 0, INT_MAX, &num_ok, NULL);
    if (!num_ok) {
                                         ...
                                         smartlist_t *vote_routerstatuses)
{
    ...
    for (line = front; line; line = line->next) {
        ...
        if (!strcmp(line->key, GUARDFRACTION_VERSION)) {
            ...
            version = (unsigned int)tor_parse_long(line->value, 10, 0, INT_MAX, &num_ok, NULL);
            ...
            if (parse_iso_time(line->value, &file_written_at) < 0) {
                log_warn(LD_CONFIG, "Guardfraction:%d: Bad date '%s'. Ignoring", current_line_n,
                         line->value);
            ...
            if (guardfraction_file_parse_inputs_line(line->value, &total_consensuses, &total_days,
                                                     &err_msg) < 0) {
            ...
            ret_tmp =
                guardfraction_file_parse_guard_line(line->value, vote_routerstatuses, &err_msg);
            ...
            log_warn(LD_CONFIG, "Unknown guardfraction line %d (%s %s)", current_line_n, line->key,
                     line->value);
        }
    }
...
int
dirserv_read_guardfraction_file(const char *fname, smartlist_t *vote_routerstatuses)
{
    ...
    return dirserv_read_guardfraction_file_from_str(guardfraction_file_str, vote_routerstatuses);
}
