                                         ...
                                         smartlist_t *vote_routerstatuses)
{
    ...
    for (line = front; line; line = line->next) {
        ...
        if (!strcmp(line->key, GUARDFRACTION_VERSION)) {
            ...
            if (!num_ok || version != 1) {
                log_warn(LD_GENERAL, "Got unknown guardfraction version %d.",
                         version);
                goto done;
            }
            ...
            if (guardfraction_file_parse_inputs_line(
                    line->value, &total_consensuses, &total_days, &err_msg) <
                0) {
                log_warn(LD_CONFIG, "Guardfraction:%d: %s", current_line_n,
                         err_msg);
                tor_free(err_msg);
                continue;
            }
            ...
            /* Successfully parsed guard line. Check if it was applied
             * properly. */
            guards_read_n++;
            if (ret_tmp > 0) {
                guards_applied_n++;
                                ...
                                smartlist_t *vote_routerstatuses)
{
    ...
    guardfraction_file_str =
        read_file_to_str(fname, RFTS_IGNORE_MISSING, NULL);
    if (!guardfraction_file_str) {
        log_warn(LD_FS, "Cannot open guardfraction file '%s'. Failing.",
                 fname);
        return -1;
    }
