...
commit_new(const char *rsa_identity)
{
    ...
    base16_encode(commit->rsa_identity_hex, sizeof(commit->rsa_identity_hex), commit->rsa_identity,
...
commit_log(const sr_commit_t *commit)
{
    ...
    log_debug(LD_DIR, "SR: Commit: [TS: %" PRIu64 "] [Encoded: %s]", commit->commit_ts,
              commit->encoded_commit);
    log_debug(LD_DIR, "SR: Reveal: [TS: %" PRIu64 "] [Encoded: %s]", commit->reveal_ts,
              safe_str(commit->encoded_reveal));
}
...
verify_commit_and_reveal(const sr_commit_t *commit)
{
    ...
    log_debug(LD_DIR, "SR: Validating commit from authority %s", sr_commit_get_rsa_fpr(commit));
     ...
     * the reveal we just received. */
    {
        ...
        if (crypto_digest256(received_hashed_reveal, commit->encoded_reveal, SR_REVEAL_BASE64_LEN,
                             commit->alg) < 0) {
...
{
    return !fast_mem_is_zero(commit->encoded_reveal, sizeof(commit->encoded_reveal));
}
...
commit_decode(const char *encoded, sr_commit_t *commit)
{
    ...
    decoded_len = base64_decode(b64_decoded, sizeof(b64_decoded), encoded, strlen(encoded));
    if (decoded_len < 0) {
    ...
    if (decoded_len != SR_COMMIT_LEN) {
        log_warn(LD_BUG,
                 ...
                 sr_commit_get_rsa_fpr(commit), decoded_len, (unsigned)SR_COMMIT_LEN);
        goto error;
    }
    ...
    memcpy(commit->hashed_reveal, b64_decoded + offset, sizeof(commit->hashed_reveal));
...
reveal_decode(const char *encoded, sr_commit_t *commit)
{
    ...
    if (decoded_len != SR_REVEAL_LEN) {
        log_warn(LD_BUG,
                 ...
                 sr_commit_get_rsa_fpr(commit), decoded_len, (unsigned)SR_REVEAL_LEN);
        goto error;
    }

    commit->reveal_ts = tor_ntohll(get_uint64(b64_decoded));
    /* Copy the last part, the random value. */
    memcpy(commit->random_number, b64_decoded + 8, sizeof(commit->random_number));
...
get_srv_element_from_commit(const sr_commit_t *commit)
{
    ...
    tor_asprintf(&element, "%s%s", sr_commit_get_rsa_fpr(commit), commit->encoded_reveal);
    return element;
}
...
static sr_srv_t *
generate_srv(const char *hashed_reveals, uint64_t reveal_num, const sr_srv_t *previous_srv)
{
...
compare_reveal_(const void **_a, const void **_b)
{
    ...
    return fast_memcmp(a->hashed_reveal, b->hashed_reveal, sizeof(a->hashed_reveal));
}
...
get_vote_line_from_commit(const sr_commit_t *commit, sr_phase_t phase)
{
    ...
    case SR_PHASE_COMMIT:
        tor_asprintf(&vote_line, "%s %u %s %s %s\n", commit_ns_str, SR_PROTO_VERSION,
                     crypto_digest_algorithm_get_name(commit->alg), sr_commit_get_rsa_fpr(commit),
                     commit->encoded_commit);
        break;
    case SR_PHASE_REVEAL: {
        /* Send a reveal value for this commit if we have one. */
        const char *reveal_str = commit->encoded_reveal;
        if (fast_mem_is_zero(commit->encoded_reveal, sizeof(commit->encoded_reveal))) {
            reveal_str = "";
        }
...
srv_to_ns_string(const sr_srv_t *srv, const char *key)
{
    ...
    sr_srv_encode(srv_hash_encoded, sizeof(srv_hash_encoded), srv);
    tor_asprintf(&srv_str, "%s %" PRIu64 " %s\n", key, srv->num_reveals, srv_hash_encoded);
...
STATIC int
commitments_are_the_same(const sr_commit_t *commit_one, const sr_commit_t *commit_two)
{
...
commit_is_authoritative(const sr_commit_t *commit, const char *voter_key)
{
    ...
    return fast_memeq(commit->rsa_identity, voter_key, sizeof(commit->rsa_identity));
}
 ...
 * if not. */
STATIC int
should_keep_commit(const sr_commit_t *commit, const char *voter_key, sr_phase_t phase)
{
    ...
    log_debug(LD_DIR, "SR: Inspecting commit from %s (voter: %s)?", sr_commit_get_rsa_fpr(commit),
...
should_keep_srv(int n_agreements)
{
    ...
    if (sr_state_srv_is_fresh()) {
        ...
        if (n_agreements < num_srv_agreements_from_vote) {
            log_notice(LD_DIR, "SR: New SRV didn't reach agreement [%d/%d]!", n_agreements,
                       num_srv_agreements_from_vote);
            return 0;
        }
    }
...
get_majority_srv_from_votes(const smartlist_t *votes, int current)
{
...

    {
        ...
        log_debug(LD_DIR, "SR: Chosen SRV by majority: %s (%d votes)", encoded, count);
    }
...
sr_generate_our_commit(time_t timestamp, const authority_cert_t *my_rsa_cert)
{
    ...
    if (reveal_encode(commit, commit->encoded_reveal, sizeof(commit->encoded_reveal)) < 0) {
    ...
    if (crypto_digest256(commit->hashed_reveal, commit->encoded_reveal, SR_REVEAL_BASE64_LEN,
                         commit->alg) < 0) {
        goto error;
    }

    /* Now get the base64 blob that corresponds to our commit. */
    if (commit_encode(commit, commit->encoded_commit, sizeof(commit->encoded_commit)) < 0) {
...
sr_compute_srv(void)
{
...

    {
        ...
        SMARTLIST_FOREACH(chunks, char *, s, tor_free(s));
        smartlist_free(chunks);
        if (crypto_digest256(hashed_reveals, reveals, strlen(reveals), SR_DIGEST_ALG) < 0) {
            goto end;
        }
        current_srv = generate_srv(hashed_reveals, reveal_num, sr_state_get_previous_srv());
...
sr_parse_commit(const smartlist_t *args)
{
    ...
    if (version > SR_PROTO_VERSION) {
        log_info(LD_DIR, "SR: Commit version %" PRIu32 " (%s) is not supported.", version,
                 escaped(value));
        goto error;
    }
    ...
    if (alg != SR_DIGEST_ALG) {
        log_warn(LD_BUG, "SR: Commit algorithm %s is not recognized.", escaped(value));
        goto error;
    }
    ...
    if (base16_decode(digest, DIGEST_LEN, rsa_identity_fpr, HEX_DIGEST_LEN) < 0) {
        log_warn(LD_DIR, "SR: RSA fingerprint %s not decodable", escaped(rsa_identity_fpr));
        goto error;
    }
...
sr_get_string_for_vote(void)
{
    ...
    {
        char *srv_lines =
            get_ns_str_from_sr_values(sr_state_get_previous_srv(), sr_state_get_current_srv());
        if (srv_lines) {
 ...
 * */
char *
sr_get_string_for_consensus(const smartlist_t *votes, int32_t num_srv_agreements)
{
    char *srv_str;
    const dirauth_options_t *options = dirauth_get_options();
...
sr_act_post_consensus(const networkstatus_t *consensus)
{
     ...
     * authority. */
    if (!sr_state_is_initialized() || !authdir_mode_v3(options) || authdir_mode_bridge(options)) {
        return;
    }
