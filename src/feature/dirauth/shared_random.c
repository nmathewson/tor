...
commit_decode(const char *encoded, sr_commit_t *commit)
{
    ...
    if (strlen(encoded) > SR_COMMIT_BASE64_LEN) {
        /* This means that if we base64 decode successfully the reveiced
         * commit, we'll end up with a bigger decoded commit thus unusable. */
        goto error;
    }
                   ...
                   sr_phase_t phase)
{
    ...
    case SR_PHASE_COMMIT:
        ...
        if (saved_commit) {
            /*  Receiving known commits should happen naturally since commit
               phase lasts multiple rounds. However if the commitment value
               changes during commit phase, it might be a bug so log more
               loudly. */
...
sr_parse_commit(const smartlist_t *args)
{
    ...
    if (version > SR_PROTO_VERSION) {
        log_info(LD_DIR,
                 "SR: Commit version %" PRIu32 " (%s) is not supported.",
...
sr_handle_received_commits(smartlist_t *commits, crypto_pk_t *voter_key)
{
    ...
    SMARTLIST_FOREACH_BEGIN (commits, sr_commit_t *, commit) {
        ...
        /* Ok, we have a valid commit now that we are about to put in our
         * state. so flag it valid from now on. */
        commit->valid = 1;
...
sr_get_string_for_vote(void)
{
    ...
    {
        char *srv_lines = get_ns_str_from_sr_values(
            sr_state_get_previous_srv(), sr_state_get_current_srv());
        if (srv_lines) {
...
sr_act_post_consensus(const networkstatus_t *consensus)
{
    ...
    if (consensus) {
         ...
         * voting do not really matter. Now that all the votes are in, we use
         * the majority's opinion on which are the active SRVs. */
        sr_state_clean_srvs();
