...
get_min_filtered_sample_size(void)
{
    return networkstatus_get_param(NULL, "guard-min-filtered-sample-size",
                                   DFLT_MIN_FILTERED_SAMPLE_SIZE, 1,
                                   INT32_MAX);
}
/**
...
get_n_primary_guards(void)
{
    ...
    /* otherwise check for consensus parameter and if that's not set either,
...
{
    int32_t pct = networkstatus_get_param(
        NULL, "guard-extreme-restriction-percent",
                               ...
                               const tor_addr_port_t *bridge_addrport)
{
    ...
    if (bridge_addrport)
        guard->bridge_addr =
            tor_memdup(bridge_addrport, sizeof(*bridge_addrport));
                                    ...
                                    const uint8_t *rsa_id_digest)
{
    ...
    if (tor_digest_is_zero(g->identity)) {
        ...
        base16_encode(old_id, sizeof(old_id), g->identity,
                      sizeof(g->identity));
        log_warn(
            LD_BUG,
        ...
        return; // redundant, but let's be clear: we're not making this
                // persistent.
    }
...
get_max_sample_size(guard_selection_t *gs, int n_guards)
{
    ...
    /* If we are in bridge mode, expand our sample set as needed without
     * worrying about max size. We should respect the user's wishes to use many
     * bridges if
...
entry_guards_expand_sample(guard_selection_t *gs)
{
    ...
    while (n_usable_filtered_guards < min_filtered_sample) {
        ...
        added_guard =
            select_and_add_guard_item_for_sample(gs, eligible_guards);
        if (!added_guard)
        ...
        if (guard->currently_listed == 0 &&
            ...
            /*
              "We have a live consensus, and {IS_LISTED} is false, and
               {FIRST_UNLISTED_AT} is over
              get_remove_unlisted_guards_after_days() days in the past."
            */
            log_info(LD_GUARD,
...
sampled_guards_update_from_consensus(guard_selection_t *gs)
{
    ...
    if (n_changes) {
        ...
        /* We don't need to rebuild the confirmed list right here -- we may
         * have removed confirmed guards above, but we can't have added any new
         * confirmed guards.
         */
        entry_guards_changed_for_guard_selection(gs);
    }
                          ...
                          entry_guard_t *guard)
{
    ...
    if (gs->type == GS_TYPE_BRIDGE) {
        ...
        const node_t *node = node_get_by_id(guard->identity);
        if (node == NULL) {
            // This can happen when currently_listed is true, and we're not
            // updating it because we don't have a live consensus.
            return 0;
        }
...
guard_in_node_family(const entry_guard_t *guard, const node_t *node)
{
    ...
    if (guard_node) {
         ...
         *
         * (Strictly speaking, I believe this check is unnecessary, since we
         * only use it to avoid the exit's family when building circuits, and
         * we don't build multihop circuits until we have a routerinfo_t for
         * the bridge... at which point, we'll also have a node_t for the
                                       ...
                                       unsigned flags)
{
    ...
    log_info(LD_GUARD,
             "  (After filters [%x], we have %d guards to consider.)", flags,
...
entry_guards_update_confirmed(guard_selection_t *gs)
{
    ...
    smartlist_sort(gs->confirmed_entry_guards,
...
get_retry_schedule(time_t failing_since, time_t now, int is_primary)
{
    ...
    for (i = 0; i < ARRAY_LENGTH(delays); ++i) {
        if (tdiff <= delays[i].maximum) {
            return is_primary ? delays[i].primary_delay
                              : delays[i].nonprimary_delay;
        }
    }
                               ...
                               unsigned *state_out)
{
    ...
    /* "Otherwise, if the ordered intersection of {CONFIRMED_GUARDS}
        ...
        false." */
    chosen_guard =
        select_confirmed_guard_for_circuit(gs, usage, rst, state_out);
    if (chosen_guard)
        return chosen_guard;
    ...
    chosen_guard =
        select_filtered_guard_for_circuit(gs, usage, rst, state_out);
                                ...
                                unsigned old_state)
{
    ...
    case GUARD_CIRC_STATE_USABLE_IF_NO_BETTER_GUARD:
        if (guard->is_primary) {
            /* XXXX #20832 -- I don't actually like this logic. It seems to
             * make us a little more susceptible to evil-ISP attacks.  The
             * mitigations I'm thinking of, however, aren't local to this
             * point, so I'll leave it alone. */
...
entry_guard_cancel(circuit_guard_state_t **guard_state_p)
{
    ...
    /* XXXX prop271 -- last_tried_to_connect_at will be erroneous here, but
     * this function will only get called in "bug" cases anyway. */
    guard->is_pending = 0;
                                      ...
                                      smartlist_t *newly_complete_out)
{
    ...
    SMARTLIST_FOREACH_BEGIN (all_circuits, origin_circuit_t *, circ) {
        ...
        if (state->state == GUARD_CIRC_STATE_WAITING_FOR_BETTER_GUARD) {
            ...
            if (!best_waiting_circuit ||
                circ_state_has_higher_priority(circ, NULL,
     ...
     * this circuit, so that we don't allow any circuit without those
     * restrictions to block it. */
...
entry_guard_encode_for_state(entry_guard_t *guard)
{
    ...
    if (guard->confirmed_idx >= 0) {
        ...
        smartlist_add_asprintf(result, "confirmed_idx=%d",
                               guard->confirmed_idx);
    }
...
#undef FIELD
    ...
    if (base16_decode(guard->identity, sizeof(guard->identity), rsa_id,
        ...
        log_warn(LD_CIRC, "Unable to decode guard identity %s",
                 escaped(rsa_id));
        goto err;
    }
    ...
    if (bridge_addr) {
        ...
        int r = tor_addr_port_parse(LOG_WARN, bridge_addr, &res.addr,
                                    &res.port, -1);
        if (r == 0)
...
#undef HANDLE_TIME
    ...
    if (smartlist_len(extra) > 0) {
        guard->extra_state_fields =
            smartlist_join_strings(extra, " ", 0, NULL);
    }
...
#define PB_FIELD(field)                                                   \
    do {                                                                  \
        if (pb_##field) {                                                 \
            ...
            if (!ok) {                                                    \
                log_warn(LD_CIRC, "Guard has invalid pb_%s %s", #field,   \
                         pb_##field);                                     \
            } else {                                                      \
                guard->pb.field = r;                                      \
            }                                                             \
...
pathbias_check_use_success_count(entry_guard_t *node)
{
    ...
    if (node->pb.use_attempts > EPSILON &&
        ...
        log_info(
            LD_GENERAL, "Path use bias is too high (%f/%f); disabling node %s",
...
pathbias_check_close_success_count(entry_guard_t *node)
{
    ...
    if (node->pb.circ_attempts > EPSILON &&
        ...
        node->pb.path_bias_disabled = 1;
        log_info(
            LD_GENERAL, "Path bias is too high (%f/%f); disabling node %s",
            node->pb.circ_successes, node->pb.circ_attempts, node->nickname);
    }
}
                    ...
                    circuit_guard_state_t **guard_state_out)
{
    ...
    /* Only apply restrictions if we have a specific exit node in mind, and
     * only if we are not doing vanguard circuits: we don't want to apply guard
