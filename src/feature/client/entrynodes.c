...
static void entry_guard_set_filtered_flags(const or_options_t *options, guard_selection_t *gs,
                                           entry_guard_t *guard);
...
should_apply_guardfraction(const networkstatus_t *ns)
{
    ...
    if (options->UseGuardFraction == -1) {
        return networkstatus_get_param(ns, "UseGuardFraction", 0, /* default to "off" */
                                       0, 1);
    }
 ...
 */
STATIC guard_selection_t *
get_guard_selection_by_name(const char *name, guard_selection_type_t type, int create_if_absent)
{
    if (!guard_contexts) {
...
create_initial_guard_context(void)
{
    ...
    const char *name = choose_guard_selection(
        get_options(),
        networkstatus_get_reasonably_live_consensus(approx_time(), usable_consensus_flavor()), NULL,
        &type);
...
{
    int32_t pct = networkstatus_get_param(NULL, "guard-max-sample-threshold-percent",
...
{
    return (int)networkstatus_get_param(NULL, "guard-max-sample-size", DFLT_MAX_SAMPLE_SIZE, 1,
                                        INT32_MAX);
}
/**
...
{
    return networkstatus_get_param(NULL, "guard-remove-unlisted-guards-after-days",
...
get_guard_lifetime(void)
{
    ...
    int32_t days;
    days =
        networkstatus_get_param(NULL, "guard-lifetime-days", DFLT_GUARD_LIFETIME_DAYS, 1, 365 * 10);
    return days * 86400;
}
/**
...
get_guard_confirmed_min_lifetime(void)
{
    ...
    days = networkstatus_get_param(NULL, "guard-confirmed-min-lifetime-days",
                                   DFLT_GUARD_CONFIRMED_MIN_LIFETIME_DAYS, 1, 365 * 10);
    return days * 86400;
}
/**
...
get_n_primary_guards(void)
{
    ...
    return networkstatus_get_param(NULL, "guard-n-primary-guards", DFLT_N_PRIMARY_GUARDS, 1,
                                   INT32_MAX);
}
/**
...
get_internet_likely_down_interval(void)
{
    return networkstatus_get_param(NULL, "guard-internet-likely-down-interval",
                                   DFLT_INTERNET_LIKELY_DOWN_INTERVAL, 1, INT32_MAX);
}
/**
...
{
    return networkstatus_get_param(NULL, "guard-nonprimary-guard-connect-timeout",
...
get_nonprimary_guard_idle_timeout(void)
{
    return networkstatus_get_param(NULL, "guard-nonprimary-guard-idle-timeout",
                                   DFLT_NONPRIMARY_GUARD_IDLE_TIMEOUT, 1, INT32_MAX);
}
/**
...
{
    int32_t pct = networkstatus_get_param(NULL, "guard-meaningful-restriction-percent",
...
{
    int32_t pct = networkstatus_get_param(NULL, "guard-extreme-restriction-percent",
 ...
 */
STATIC const char *
choose_guard_selection(const or_options_t *options, const networkstatus_t *live_ns,
                       const guard_selection_t *old_selection, guard_selection_type_t *type_out)
{
    tor_assert(options);
    tor_assert(type_out);
...
update_guard_selection_choice(const or_options_t *options)
{
    ...
    const char *new_name = choose_guard_selection(
        options,
        networkstatus_get_reasonably_live_consensus(approx_time(), usable_consensus_flavor()),
    ...
    log_notice(LD_GUARD, "Switching to guard context \"%s\" (was using \"%s\")", new_name,
               cur_name);
...
node_is_possible_guard(const node_t *node)
{
    ...
    tor_assert(node);
    return (node->is_possible_guard && node->is_stable && node->is_fast && node->is_valid &&
...
static entry_guard_t *
get_sampled_guard_for_bridge(guard_selection_t *gs, const bridge_info_t *bridge)
{
...
get_bridge_info_for_guard(const entry_guard_t *guard)
{
    ...
    return get_configured_bridge_by_exact_addr_port_digest(
        &guard->bridge_addr->addr, guard->bridge_addr->port, (const char *)identity);
}
...
{
    log_info(LD_GUARD, "Adding %s to the entry guard sample set.", node_describe(node));
 ...
 */
static entry_guard_t *
entry_guard_add_to_sample_impl(guard_selection_t *gs, const uint8_t *rsa_id_digest,
                               const char *nickname, const tor_addr_port_t *bridge_addrport)
{
 ...
 */
static entry_guard_t *
entry_guard_add_bridge_to_sample(guard_selection_t *gs, const bridge_info_t *bridge)
{
 ...
 */
static entry_guard_t *
get_sampled_guard_by_bridge_addr(guard_selection_t *gs, const tor_addr_port_t *addrport)
{
    if (!gs)
        return NULL;
    if (BUG(!addrport))
 ...
 */
void
entry_guard_learned_bridge_identity(const tor_addr_port_t *addrport, const uint8_t *rsa_id_digest)
{
 ...
 */
STATIC int
num_reachable_filtered_guards(const guard_selection_t *gs, const entry_guard_restriction_t *rst)
{
 ...
 */
static smartlist_t *
get_eligible_guards(const or_options_t *options, guard_selection_t *gs, int *n_guards_out)
{
...
static entry_guard_t *
select_and_add_guard_item_for_sample(guard_selection_t *gs, smartlist_t *eligible_guards)
{
...
reasonably_live_consensus_is_missing(const guard_selection_t *gs)
{
    ...
    return networkstatus_get_reasonably_live_consensus(approx_time(), usable_consensus_flavor()) ==
           NULL;
}
...
static void
remove_guard_from_confirmed_and_primary_lists(guard_selection_t *gs, entry_guard_t *guard)
{
    if (guard->is_primary) {
        guard->is_primary = 0;
...
sampled_guards_update_consensus_presence(guard_selection_t *gs)
{
    ...
    const time_t unlisted_since_slop = get_remove_unlisted_guards_after_seconds() / 5;
 ...
 */
static size_t
sampled_guards_prune_obsolete_entries(guard_selection_t *gs, const time_t remove_if_unlisted_since,
        ...
        if (guard->currently_listed == 0 && guard->unlisted_since_date < remove_if_unlisted_since) {
            ...
            */
            log_info(LD_GUARD,
                     ...
                     "for over %d days",
                     entry_guard_describe(guard), get_remove_unlisted_guards_after_days());
            rmv = 1;
...
sampled_guards_update_from_consensus(guard_selection_t *gs)
{
    ...
    n_changes += sampled_guards_prune_obsolete_entries(
        gs, remove_if_unlisted_since, maybe_remove_if_sampled_before, remove_if_confirmed_before);
 ...
 * connect to. */
static int
bridge_passes_guard_filter(const or_options_t *options, const bridge_info_t *bridge)
{
    tor_assert(bridge);
    if (!bridge)
        return 0;
...
static int
entry_guard_passes_filter(const or_options_t *options, guard_selection_t *gs, entry_guard_t *guard)
{
...
guard_in_node_family(const entry_guard_t *guard, const node_t *node)
{
    ...
    if (guard_node) {
        ...
        if (get_options()->EnforceDistinctSubnets && guard->bridge_addr) {
            ...
            if (addrs_in_same_network_family(&node_addr, &guard->bridge_addr->addr)) {
                return 1;
            }
        }
        return 0;
...
static int
guard_obeys_exit_restriction(const entry_guard_t *guard, const entry_guard_restriction_t *rst)
{
 ...
 * flags on <b>guard</b>. */
void
entry_guard_set_filtered_flags(const or_options_t *options, guard_selection_t *gs,
                               entry_guard_t *guard)
{
 ...
 **/
STATIC entry_guard_t *
sample_reachable_filtered_entry_guards(guard_selection_t *gs, const entry_guard_restriction_t *rst,
                                       unsigned flags)
{
    tor_assert(gs);
    ...
    log_info(LD_GUARD, "  (After filters [%x], we have %d guards to consider.)", flags,
...
make_guard_confirmed(guard_selection_t *gs, entry_guard_t *guard)
{
    ...
    log_info(LD_GUARD, "Marking %s as a confirmed guard (index %d)", entry_guard_describe(guard),
...
entry_guards_update_primary(guard_selection_t *gs)
{
    ...
    SMARTLIST_FOREACH_BEGIN (old_primary_guards, entry_guard_t *, guard) {
        ...
        if (smartlist_len(new_primary_guards) < N_PRIMARY_GUARDS && guard->is_filtered_guard) {
            guard->is_primary = 1;
            smartlist_add(new_primary_guards, guard);
            SMARTLIST_DEL_CURRENT_KEEPORDER(old_primary_guards, guard);
        } else {
    ...
    while (smartlist_len(new_primary_guards) < N_PRIMARY_GUARDS) {
        entry_guard_t *guard = sample_reachable_filtered_entry_guards(
            gs, NULL, SAMPLE_EXCLUDE_CONFIRMED | SAMPLE_EXCLUDE_PRIMARY | SAMPLE_NO_UPDATE_PRIMARY);
        if (!guard)
            break;
        guard->is_primary = 1;
        smartlist_add(new_primary_guards, guard);
    ...
    SMARTLIST_FOREACH(gs->sampled_entry_guards, entry_guard_t *, guard, {
        tor_assert_nonfatal(
            bool_eq(guard->is_primary, smartlist_contains(new_primary_guards, guard)));
    });
#endif /* 1 */

    const int any_change = !smartlist_ptrs_eq(gs->primary_entry_guards, new_primary_guards);
    if (any_change) {
        ...
        SMARTLIST_FOREACH_BEGIN (new_primary_guards, entry_guard_t *, g) {
            log_info(LD_GUARD, "  %d/%d: %s%s%s", g_sl_idx + 1, n, entry_guard_describe(g),
...
entry_guard_consider_retry(entry_guard_t *guard)
{
    ...
    if (BUG(last_attempt == 0) || now >= last_attempt + delay) {
        ...
        log_info(LD_GUARD,
                 ...
                 guard->is_primary ? "primary " : "", guard->confirmed_idx >= 0 ? "confirmed " : "",
...
select_primary_guard_for_circuit(guard_selection_t *gs, guard_usage_t usage,
                                 const entry_guard_restriction_t *rst, unsigned *state_out)
{
                                  ...
                                  const entry_guard_restriction_t *rst, unsigned *state_out)
{
    ...
    chosen_guard = sample_reachable_filtered_entry_guards(
        gs, rst,
        SAMPLE_EXCLUDE_CONFIRMED | SAMPLE_EXCLUDE_PRIMARY | SAMPLE_EXCLUDE_PENDING | flags);
    if (!chosen_guard) {
        return NULL;
    }
...
entry_guards_note_guard_failure(guard_selection_t *gs, entry_guard_t *guard)
{
    ...
    log_info(LD_GUARD, "Recorded failure for %s%sguard %s", guard->is_primary ? "primary " : "",
             guard->confirmed_idx >= 0 ? "confirmed " : "", entry_guard_describe(guard));
}
 ...
 **/
STATIC unsigned
entry_guards_note_guard_success(guard_selection_t *gs, entry_guard_t *guard, unsigned old_state)
{
    tor_assert(gs);
    ...
    log_info(LD_GUARD, "Recorded success for %s%sguard %s", guard->is_primary ? "primary " : "",
             guard->confirmed_idx >= 0 ? "confirmed " : "", entry_guard_describe(guard));
...
entry_guard_pick_for_circuit(guard_selection_t *gs, guard_usage_t usage,
                             entry_guard_restriction_t *rst, const node_t **chosen_node_out,
                             circuit_guard_state_t **guard_state_out)
{
    ...
    *guard_state_out = NULL;
    ...
    if (!node)
        goto fail;
    if (BUG(usage != GUARD_USAGE_DIRGUARD && !node_has_preferred_descriptor(node, 1)))
        goto fail;
...
entry_guard_succeeded(circuit_guard_state_t **guard_state_p)
{
    ...
    unsigned newstate =
        entry_guards_note_guard_success(guard->in_selection, guard, (*guard_state_p)->state);
 ...
 */
static int
circ_state_has_higher_priority(origin_circuit_t *a, const entry_guard_restriction_t *rst,
                               origin_circuit_t *b)
{
 ...
 */
int
entry_guards_upgrade_waiting_circuits(guard_selection_t *gs, const smartlist_t *all_circuits_in,
...
#undef FIELD
    ...
    if (base16_decode(guard->identity, sizeof(guard->identity), rsa_id, strlen(rsa_id)) !=
        DIGEST_LEN) {
    ...
    if (nickname) {
        ...
        guard->nickname[0] = '$';
        base16_encode(guard->nickname + 1, sizeof(guard->nickname) - 1, guard->identity,
                      DIGEST_LEN);
    }
...
#define HANDLE_TIME(field)                                                                     \
    do {                                                                                       \
        if (field) {                                                                           \
            ...
            if (r < 0) {                                                                       \
                log_warn(LD_CIRC, "Unable to parse %s %s from guard", #field, escaped(field)); \
                field##_time = -1;                                                             \
            }                                                                                  \
        }                                                                                      \
    } while (0)
...
entry_guard_t *
entry_guard_get_by_id_digest_for_guard_selection(guard_selection_t *gs, const char *digest)
{
...
get_guard_state_for_bridge_desc_fetch(const char *digest)
{
    ...
    guard = entry_guard_get_by_id_digest_for_guard_selection(get_guard_selection_info(), digest);
    if (!guard) {
        return NULL;
    }
    ...
    guard_state = circuit_guard_state_new(guard, GUARD_CIRC_STATE_USABLE_ON_COMPLETION, NULL);
...
pathbias_check_use_success_count(entry_guard_t *node)
{
    ...
    if (node->pb.use_attempts > EPSILON &&
        ...
        log_info(LD_GENERAL, "Path use bias is too high (%f/%f); disabling node %s",
 ...
 * */
int
getinfo_helper_entry_guards(control_connection_t *conn, const char *question, char **answer,
                            const char **errmsg)
{
    guard_selection_t *gs = get_guard_selection_info();
 ...
 */
void
guard_get_guardfraction_bandwidth(guardfraction_bandwidth_t *guardfraction_bw, int orig_bandwidth,
                    ...
                    circuit_guard_state_t **guard_state_out)
{
    ...
    if (entry_guard_pick_for_circuit(get_guard_selection_info(), GUARD_USAGE_TRAFFIC, rst, &r,
                                     guard_state_out) < 0) {
        tor_assert(r == NULL);
    }
    return r;
...
const node_t *
guards_choose_dirguard(uint8_t dir_purpose, circuit_guard_state_t **guard_state_out)
{
    const node_t *r = NULL;
    entry_guard_restriction_t *rst = NULL;
    ...
    if (entry_guard_pick_for_circuit(get_guard_selection_info(), GUARD_USAGE_DIRGUARD, rst, &r,
                                     guard_state_out) < 0) {
        tor_assert(r == NULL);
    }
    return r;
...
char *
guard_selection_get_err_str_if_dir_info_missing(guard_selection_t *gs, int using_mds,
                                                int num_present, int num_usable)
{
    ...
    tor_asprintf(&ret_str,
                 ...
                 n_missing_descriptors, num_primary_to_check, using_mds ? "micro" : "", num_present,
                 num_usable);
...
char *
entry_guards_get_err_str_if_dir_info_missing(int using_mds, int num_present, int num_usable)
{
    return guard_selection_get_err_str_if_dir_info_missing(get_guard_selection_info(), using_mds,
