...
get_max_sample_size(guard_selection_t *gs, int n_guards)
{
     ...
     * bridges if that's what they have specified in their configuration file.
     */
    if (using_bridges)
        return INT_MAX;
...
guard_in_node_family(const entry_guard_t *guard, const node_t *node)
{
    ...
    if (guard_node) {
         ...
         * only use it to avoid the exit's family when building circuits, and we
         * don't build multihop circuits until we have a routerinfo_t for the
    ...
    switch (old_state) {
        ...
        case GUARD_CIRC_STATE_USABLE_IF_NO_BETTER_GUARD:
            if (guard->is_primary) {
                ...
                /* This guard may have become primary by virtue of being
                 * confirmed. If so, the circuit for it is now complete.
                 */
                new_state = GUARD_CIRC_STATE_COMPLETE;
            } else {
...
#undef FIELD
    ...
    if (bridge_addr) {
        ...
        int r = tor_addr_port_parse(LOG_WARN, bridge_addr, &res.addr, &res.port,
                                    -1);
        if (r == 0)
