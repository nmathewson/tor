...
addressmap_virtaddress_remove(const char *address, addressmap_entry_t *ent)
{
    if (ent && ent->new_address &&
        ...
        /*log_fn(LOG_NOTICE,"remove reverse mapping for
         * %s",ent->new_address);*/
        if (ve) {
            ...
            if (!ve->ipv4_address && !ve->ipv6_address &&
                !ve->hostname_address) {
                tor_free(ve);
...
addressmap_clear_invalid_automaps(const or_options_t *options)
{
    ...
    STRMAP_FOREACH_MODIFY(addressmap, src_address, addressmap_entry_t *, ent)
    {
        ...
        if (!remove_this) {
            remove_this =
                !addressmap_address_should_automap(src_address, options);
        }
    ...
    for (rewrites = 0; rewrites < 16; rewrites++) {
        ...
        if (!ent || !ent->new_address) {
            ...
            if (ent->src_wildcard && !ent->dst_wildcard &&
                ...
                /* This is a rule like "rewrite *.example.com to example.com",
                 * and we just got "example.com". Instead of calling it an
                 * infinite loop, call it complete. */
                goto done;
            }
            exact_match = 1;
        }
                           ...
                           int validate_only, char **msg)
{
    ...
    virtual_addr_conf_t *conf =
        ipv6 ? &virtaddr_conf_ipv6 : &virtaddr_conf_ipv4;

    if (!val || val[0] == '\0') {
        if (msg)
            tor_asprintf(
                msg, "Value not present (%s) after VirtualAddressNetwork%s",
...
address_is_in_virtual_range(const char *address)
{
    ...
    if (!strcasecmpend(address, ".virtual")) {
        ...
        const virtual_addr_conf_t *conf = (tor_addr_family(&addr) == AF_INET6)
                                              ? &virtaddr_conf_ipv6
                                              : &virtaddr_conf_ipv4;
        if (tor_addr_compare_masked(&addr, &conf->addr, conf->bits,
                                    CMP_EXACT) == 0)
            return 1;
    }
    return 0;
    ...
    if (type == RESOLVED_TYPE_HOSTNAME) {
        ...
        while (attempts--) {
            ...
            if (!strmap_get(addressmap, buf)) {
                /* XXXX This code is to make sure I didn't add an undecorated
                 * version by mistake. I hope it's needless. */
                char tmp[TOR_ADDR_BUF_LEN];
                tor_addr_to_str(tmp, &addr, sizeof(tmp), 0);
                if (strmap_get(addressmap, tmp)) {
                    // LCOV_EXCL_START
                    log_warn(LD_BUG,
                             "%s wasn't in the addressmap, but %s was.", buf,
                             tmp);
                    continue;
                    // LCOV_EXCL_STOP
                }
            ...
            if (!sl) {
                ...
                if (want_expiry) {
                    if (val->expires < 3 || val->expires == TIME_MAX)
                        smartlist_add_asprintf(sl, "%s%s %s%s NEVER", src_wc,
                                               key, dst_wc, val->new_address);
                    else {
                        char isotime[ISO_TIME_LEN + 1];
                        format_iso_time(isotime, val->expires);
                        smartlist_add_asprintf(sl, "%s%s %s%s \"%s\"", src_wc,
                                               key, dst_wc, val->new_address,
                                               isotime);
                    }
                } else {
                    smartlist_add_asprintf(sl, "%s%s %s%s", src_wc, key,
                                           dst_wc, val->new_address);
                }
            }
        }
