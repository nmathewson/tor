...
addressmap_clear_excluded_trackexithosts(const or_options_t *options)
{
    ...
    STRMAP_FOREACH_MODIFY(addressmap, address, addressmap_entry_t *, ent)
    {
        ...
        tor_free(nodename);
        if (!node || (allow_nodes && !routerset_contains_node(allow_nodes, node)) ||
...
int
addressmap_address_should_automap(const char *address, const or_options_t *options)
{
 ...
 */
int
addressmap_rewrite(char *address, size_t maxlen, unsigned flags, time_t *expires_out,
                   addressmap_entry_source_t *exit_source_out)
{
    ...
    for (rewrites = 0; rewrites < 16; rewrites++) {
        ...
        if (!ent || !ent->new_address) {
            ...
            if (ent->src_wildcard && !ent->dst_wildcard && !strcasecmp(address, ent->new_address)) {
        ...
        default:
            log_warn(LD_BUG, "Unknown addrmap source value %d. Ignoring it.", (int)ent->source);
            goto done;
        }
    ...
    log_warn(LD_CONFIG, "Loop detected: we've rewritten %s 16 times! Using it as-is.",
             escaped_safe_str_client(address));
...
int
addressmap_rewrite_reverse(char *address, size_t maxlen, unsigned flags, time_t *expires_out)
{
    char *s, *cp;
    addressmap_entry_t *ent;
    int r = 0;
    ...
    if (ent) {
        ...
        log_info(LD_APP, "Rewrote reverse lookup %s -> %s", escaped_safe_str_client(s), cp);
        tor_free(cp);
        strlcpy(address, ent->new_address, maxlen);
        r = 1;
    }
                    ...
                    const int wildcard_new_addr)
{
    ...
    ent = strmap_get(addressmap, address);
    if (!new_address || (!strcasecmp(address, new_address) && wildcard_addr == wildcard_new_addr)) {
    ...
    log_info(LD_CONFIG, "Addressmap: (re)mapped '%s' to '%s'", safe_str_client(address),
             safe_str_client(ent->new_address));
...
client_dns_incr_failures(const char *address)
{
    ...
    log_info(LD_APP, "Address %s now has %d resolve failures.", safe_str_client(address),
 ...
 */
static void
client_dns_set_addressmap_impl(entry_connection_t *for_conn, const char *address, const char *name,
                               const char *exitname, int ttl)
{
    ...
    addressmap_register(extendedaddress, extendedval, time(NULL) + ttl, ADDRMAPSRC_DNS, 0, 0);
 ...
 */
void
client_dns_set_addressmap(entry_connection_t *for_conn, const char *address, const tor_addr_t *val,
                          const char *exitname, int ttl)
{
    tor_addr_t addr_tmp;
 ...
 */
void
client_dns_set_reverse_addressmap(entry_connection_t *for_conn, const char *address, const char *v,
                                  const char *exitname, int ttl)
{
    char *s = NULL;
    {
...
int
parse_virtual_addr_network(const char *val, sa_family_t family, int validate_only, char **msg)
{
    ...
    if (tor_addr_parse_mask_ports(val, 0, &addr, &bits, NULL, NULL) < 0) {
        if (msg)
            tor_asprintf(msg, "Error parsing VirtualAddressNetwork%s %s", ipv6 ? "IPv6" : "", val);
        return -1;
    }
...
address_is_in_virtual_range(const char *address)
{
    ...
    if (!strcasecmpend(address, ".virtual")) {
        ...
        const virtual_addr_conf_t *conf =
            (tor_addr_family(&addr) == AF_INET6) ? &virtaddr_conf_ipv6 : &virtaddr_conf_ipv4;
...
get_random_virtual_addr(const virtual_addr_conf_t *conf, tor_addr_t *addr_out)
{
    ...
    tor_assert(tor_addr_compare_masked(addr_out, &conf->addr, conf->bits, CMP_EXACT) == 0);
}
...
addressmap_get_virtual_address(int type)
{
    ...
    if (type == RESOLVED_TYPE_HOSTNAME) {
        ...
        const virtual_addr_conf_t *conf = ipv6 ? &virtaddr_conf_ipv6 : &virtaddr_conf_ipv4;
        ...
        while (attempts--) {
            ...
            if (!strmap_get(addressmap, buf)) {
                ...
                if (strmap_get(addressmap, tmp)) {
                    // LCOV_EXCL_START
                    log_warn(LD_BUG, "%s wasn't in the addressmap, but %s was.", buf, tmp);
                    continue;
                    // LCOV_EXCL_STOP
                }
    ...
    if (*addrp) {
        ...
        if (ent && ent->new_address && !strcasecmp(new_address, ent->new_address)) {
            ...
            log_warn(LD_BUG,
                     ...
                     safe_str_client(new_address), safe_str_client(*addrp), safe_str_client(*addrp),
 ...
 */
void
addressmap_get_mappings(smartlist_t *sl, time_t min_expires, time_t max_expires, int want_expiry)
{
    strmap_iter_t *iter;
    const char *key;
    void *val_;
        ...
        if (val->expires >= min_expires && val->expires <= max_expires) {
            if (!sl) {
                ...
                if (want_expiry) {
                    ...
                    smartlist_add_asprintf(sl, "%s%s %s%s", src_wc, key, dst_wc, val->new_address);
                }
            }
        }
