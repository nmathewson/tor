...
typedef enum { GUARD_USAGE_TRAFFIC = 0, GUARD_USAGE_DIRGUARD = 1 } guard_usage_t;
...
int entry_guards_upgrade_waiting_circuits(guard_selection_t *gs, const smartlist_t *all_circuits,
...
STATIC guard_selection_type_t guard_selection_infer_type(guard_selection_type_t type_in,
                                                         const char *name);
STATIC guard_selection_t *guard_selection_new(const char *name, guard_selection_type_t type);
STATIC guard_selection_t *get_guard_selection_by_name(const char *name, guard_selection_type_t type,
...
MOCK_DECL(STATIC int, entry_guard_is_listed, (guard_selection_t * gs, const entry_guard_t *guard));
STATIC const char *choose_guard_selection(const or_options_t *options, const networkstatus_t *ns,
...
#    define entry_guard_free(e) FREE_AND_NULL(entry_guard_t, entry_guard_free_, (e))
...
STATIC entry_guard_t *select_entry_guard_for_circuit(guard_selection_t *gs, guard_usage_t usage,
                                                     const entry_guard_restriction_t *rst,
                                                     unsigned *state_out);
STATIC void mark_primary_guards_maybe_reachable(guard_selection_t *gs);
STATIC unsigned entry_guards_note_guard_success(guard_selection_t *gs, entry_guard_t *guard,
                                                unsigned old_state);
...
int entry_guards_parse_state_for_guard_selection(guard_selection_t *gs, or_state_t *state, int set,
                                                 char **msg);
...
char *guard_selection_get_err_str_if_dir_info_missing(guard_selection_t *gs, int using_mds,
...
void guard_get_guardfraction_bandwidth(guardfraction_bandwidth_t *guardfraction_bw,
                                       int orig_bandwidth, uint32_t guardfraction_percentage);
