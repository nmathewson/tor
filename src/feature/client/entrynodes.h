...
typedef struct guard_pathbias_t {
    ...
    unsigned int path_bias_warned : 1; /**< Did we alert the user about path
                                        * bias for this node already? */
    unsigned int path_bias_extreme : 1; /**< Did we alert the user about path
                                         * bias for this node already? */
    unsigned int path_bias_disabled : 1; /**< Have we disabled this node
                                          * because of path bias issues? */
    unsigned int
        path_bias_use_noticed : 1; /**< Did we alert the user about path
                                    * use bias for this node already? */
    unsigned int
        path_bias_use_extreme : 1; /**< Did we alert the user about path
                                    * use bias for this node already? */
    ...
    double timeouts; /**< Number of 'right-censored' circuit timeouts for this
                      * guard. */
    double
        use_attempts; /**< Number of circuits we tried to use with streams */
...
struct entry_guard_t {
     ...
     * appear earlier on the confirmed list.  If the confirmed list is
     * compacted, this field corresponds to the index of this guard on the
     * confirmed list.
     *
...
typedef enum guard_selection_type_t {
    ...
    GS_TYPE_NORMAL = 1,
    /** Use the normal guard selection algorithm, taking our sample from the
     * configured bridges, and allowing it to grow as large as all the
     * configured bridges */
    GS_TYPE_BRIDGE,
    /** Use the normal guard selection algorithm, taking our sample from the
...
struct guard_selection_t {
    ...
    /**
     * A value of 1 means that primary_entry_guards is up-to-date with respect
     * to the consensus and status info that we currently have; 0 means we need
     * to recalculate it before using primary_entry_guards or the is_primary
     * flag on any guard.
     */
