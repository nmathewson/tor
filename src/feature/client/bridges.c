    ...
    SMARTLIST_FOREACH_BEGIN (bridge_list, bridge_info_t *, bridge) {
        if (tor_digest_is_zero(bridge->identity)) {
            SMARTLIST_FOREACH_BEGIN (orports, tor_addr_port_t *, ap) {
                if (tor_addr_compare(&bridge->addr, &ap->addr, CMP_EXACT) ==
                        0 &&
                    bridge->port == ap->port)
                    return bridge;
...
{
    tor_assert(addr);
    return get_configured_bridge_by_addr_port_digest(addr, port, digest) ? 1
                                                                         : 0;
}
...
get_configured_bridge_by_routerinfo(const routerinfo_t *ri)
{
    ...
    bi = get_configured_bridge_by_orports_digest(
        ri->cache_info.identity_digest, orports);
                                       ...
                                       const uint16_t port, const char *digest)
{
    ...
    if (tor_addr_port_is_valid_ipv4h(ipv4_addr, port, 0)) {
        ...
        bridge_info_t *bridge = get_configured_bridge_by_addr_port_digest(
            &node_ipv4, port, digest);
                        ...
                        const char *digest, const ed25519_public_key_t *ed_id)
{
  ...
  if (bridge && ed_id &&
    ...
    if (learned) {
        ...
        if (transport_name)
            tor_asprintf(&transport_info, " (with transport '%s')",
                         transport_name);
        ...
        if (tor_addr_eq(&bridge->addr, addr) && (bridge->port == port)) {
            ...
            if (!bridge_has_digest(bridge, digest) ||
                ...
                log_warn(
                    ...
                    " wanted, please change your configuration file "
                    "accordingly.",
    ...
    *transport = NULL;
    ...
    SMARTLIST_FOREACH_BEGIN (bridge_list, const bridge_info_t *, bridge) {
        if (tor_addr_eq(&bridge->addr, addr) &&
            ...
            if (bridge
                    ->transport_name) { /* it also uses pluggable transports */
                *transport = transport_get_by_name(bridge->transport_name);
                if (*transport ==
                    NULL) { /* it uses pluggable transports, but
        ...
        if ((!tor_addr_compare(&bridge->addr, &addr, CMP_EXACT) &&
            ...
            if (tor_addr_family(&bridge->addr) == AF_INET) {
                ...
                log_info(LD_DIR,
                         ...
                         rs->nickname,
                         fmt_addrport(&bridge->addr, rs->or_port));
...
learned_bridge_descriptor(routerinfo_t *ri, int from_cache)
{
    ...
    if (get_options()->UseBridges) {
        ...
        if (bridge) { /* if we actually want to use this one */
            ...
            if (!from_cache) {
                /* This schedules the re-fetch at a constant interval, which
                 * produces a pattern of bridge traffic. But it's better than
                 * trying all
            ...
            rewrite_node_address_for_bridge(bridge, node);
            if (tor_digest_is_zero(bridge->identity)) {
                memcpy(bridge->identity, ri->cache_info.identity_digest,
                       DIGEST_LEN);
            ...
            log_notice(LD_DIR, "new bridge descriptor '%s' (%s): %s",
                       ri->nickname, from_cache ? "cached" : "fresh",
                       router_describe(ri));
            /* If we didn't have a reachable bridge before this one, try
             * directory documents again. */
            if (first) {
