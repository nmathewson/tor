...
mark_bridge_list(void)
{
    ...
    SMARTLIST_FOREACH(bridge_list, bridge_info_t *, b, b->marked_for_removal = 1);
}
...
get_configured_bridge_by_orports_digest(const char *digest, const smartlist_t *orports)
{
    if (!bridge_list)
        return NULL;
...
bridge_info_t *
get_configured_bridge_by_addr_port_digest(const tor_addr_t *addr, uint16_t port, const char *digest)
{
    if (!bridge_list)
        return NULL;
    SMARTLIST_FOREACH_BEGIN (bridge_list, bridge_info_t *, bridge) {
        if ((tor_digest_is_zero(bridge->identity) || digest == NULL) &&
            !tor_addr_compare(&bridge->addr, addr, CMP_EXACT) && bridge->port == port)
            return bridge;
        if (digest && tor_memeq(bridge->identity, digest, DIGEST_LEN))
            return bridge;
    } SMARTLIST_FOREACH_END (bridge);
 ...
 */
bridge_info_t *
get_configured_bridge_by_exact_addr_port_digest(const tor_addr_t *addr, uint16_t port,
                                                const char *digest)
{
    if (!bridge_list)
        return NULL;
    SMARTLIST_FOREACH_BEGIN (bridge_list, bridge_info_t *, bridge) {
        if (!tor_addr_compare(&bridge->addr, addr, CMP_EXACT) && bridge->port == port) {
            if (digest && tor_memeq(bridge->identity, digest, DIGEST_LEN))
                return bridge;
 ...
 * address/port matches only. */
int
addr_is_a_configured_bridge(const tor_addr_t *addr, uint16_t port, const char *digest)
{
    tor_assert(addr);
...
get_configured_bridge_by_routerinfo(const routerinfo_t *ri)
{
    ...
    bi = get_configured_bridge_by_orports_digest(ri->cache_info.identity_digest, orports);
 ...
 */
static int
bridge_exists_with_ipv4h_addr_and_port(const uint32_t ipv4_addr, const uint16_t port,
                                       const char *digest)
{
    ...
    if (tor_addr_port_is_valid_ipv4h(ipv4_addr, port, 0)) {
        ...
        bridge_info_t *bridge = get_configured_bridge_by_addr_port_digest(&node_ipv4, port, digest);
 ...
 */
static int
bridge_exists_with_ipv6_addr_and_port(const tor_addr_t *ipv6_addr, const uint16_t port,
                                      const char *digest)
{
    if (!tor_addr_port_is_valid(ipv6_addr, port, 0))
        return 0;

    bridge_info_t *bridge = get_configured_bridge_by_addr_port_digest(ipv6_addr, port, digest);
...
node_is_a_configured_bridge(const node_t *node)
{
     ...
     */
    if (node->ri) {
        if (bridge_exists_with_ipv4h_addr_and_port(node->ri->addr, node->ri->or_port,
                                                   node->identity))
            return 1;

        if (bridge_exists_with_ipv6_addr_and_port(&node->ri->ipv6_addr, node->ri->ipv6_orport,
                                                  node->identity))
            return 1;
    } else if (node->rs) {
        if (bridge_exists_with_ipv4h_addr_and_port(node->rs->addr, node->rs->or_port,
                                                   node->identity))
            return 1;

        if (bridge_exists_with_ipv6_addr_and_port(&node->rs->ipv6_addr, node->rs->ipv6_orport,
                                                  node->identity))
            return 1;
    } else if (node->md) {
        if (bridge_exists_with_ipv6_addr_and_port(&node->md->ipv6_addr, node->md->ipv6_orport,
                                                  node->identity))
            return 1;
    }
 ...
 */
void
learned_router_identity(const tor_addr_t *addr, uint16_t port, const char *digest,
                        const ed25519_public_key_t *ed_id)
{
    // XXXX prop220 use ed_id here, once there is some way to specify
    (void)ed_id;
    int learned = 0;
    bridge_info_t *bridge = get_configured_bridge_by_exact_addr_port_digest(addr, port, digest);
  ...
  if (bridge && ed_id &&
    ...
    if (learned) {
        ...
        const char *transport_name = find_transport_name_by_bridge_addrport(addr, port);
        if (transport_name)
            tor_asprintf(&transport_info, " (with transport '%s')", transport_name);

        // XXXX prop220 log both fingerprints.
        log_notice(LD_DIR, "Learned fingerprint %s for bridge %s%s.", hex_str(digest, DIGEST_LEN),
                   fmt_addrport(addr, port), transport_info ? transport_info : "");
        tor_free(transport_info);
        entry_guard_learned_bridge_identity(&bridge->addrport_configured, (const uint8_t *)digest);
    }
}
 ...
 */
STATIC void
bridge_resolve_conflicts(const tor_addr_t *addr, uint16_t port, const char *digest,
    ...
    SMARTLIST_FOREACH_BEGIN (bridge_list, bridge_info_t *, bridge) {
        ...
        if (tor_addr_eq(&bridge->addr, addr) && (bridge->port == port)) {
            ...
            if (!bridge_has_digest(bridge, digest) ||
                ...
                tor_asprintf(&bridge_description_new, "%s:%s:%s", fmt_addrport(addr, port),
                             digest ? hex_str(digest, DIGEST_LEN) : "",
                             transport_name ? transport_name : "");
                tor_asprintf(
                    &bridge_description_old, "%s:%s:%s", fmt_addrport(&bridge->addr, bridge->port),
                ...
                log_warn(LD_GENERAL,
                         ...
                         bridge_description_new, bridge_description_old, bridge_description_new);
...
transport_is_needed, (const char *transport_name))
{
    ...
    SMARTLIST_FOREACH_BEGIN (bridge_list, const bridge_info_t *, bridge) {
        if (bridge->transport_name && !strcmp(bridge->transport_name, transport_name))
            return 1;
    } SMARTLIST_FOREACH_END (bridge);
...
bridge_add_from_config(bridge_line_t *bridge_line)
{
    ...
    { /* Log the bridge we are about to register: */
        log_debug(LD_GENERAL, "Registering bridge at %s (transport: %s) (%s)",
                  ...
                  bridge_line->transport_name ? bridge_line->transport_name : "no transport",
        ...
        if (bridge_line->socks_args) { /* print socks arguments */
            ...
            log_debug(LD_GENERAL,
                      "Bridge uses %d SOCKS arguments:", smartlist_len(bridge_line->socks_args));
            SMARTLIST_FOREACH(bridge_line->socks_args, const char *, arg,
                              log_debug(LD_CONFIG, "%d: %s", ++i, arg));
        }
    }

    bridge_resolve_conflicts(&bridge_line->addr, bridge_line->port, bridge_line->digest,
                                 ...
                                 const transport_t **transport)
{
    *transport = NULL;
    ...
    SMARTLIST_FOREACH_BEGIN (bridge_list, const bridge_info_t *, bridge) {
        if (tor_addr_eq(&bridge->addr, addr) && (bridge->port == port)) { /* bridge matched */
...
{
    bridge_info_t *bridge = get_configured_bridge_by_addr_port_digest(addr, port, NULL);
...
launch_direct_bridge_descriptor_fetch(bridge_info_t *bridge)
{
    ...
    if (connection_get_by_type_addr_port_purpose(CONN_TYPE_DIR, &bridge->addr, bridge->port,
     ...
     * it. */
    if (!fascist_firewall_allows_address_addr(&bridge->addr, bridge->port, FIREWALL_OR_CONNECTION,
                                              0, 0)) {
    ...
    directory_request_t *req = directory_request_new(DIR_PURPOSE_FETCH_SERVERDESC);
...
fetch_bridge_descriptors(const or_options_t *options, time_t now)
{
    ...
    SMARTLIST_FOREACH_BEGIN (bridge_list, bridge_info_t *, bridge) {
        ...
        can_use_bridge_authority = !tor_digest_is_zero(bridge->identity) && num_bridge_auths;
        ask_bridge_directly = !can_use_bridge_authority || !options->UpdateBridgesFromAuthority;
        log_debug(LD_DIR, "ask_bridge_directly=%d (%d, %d, %d)", ask_bridge_directly,
                  tor_digest_is_zero(bridge->identity), !options->UpdateBridgesFromAuthority,
                  !num_bridge_auths);

        if (ask_bridge_directly && !fascist_firewall_allows_address_addr(
                                       &bridge->addr, bridge->port, FIREWALL_OR_CONNECTION, 0, 0)) {
            log_notice(LD_DIR,
                       "Bridge at '%s' isn't reachable by our "
                       "firewall policy. %s.",
                       fmt_addrport(&bridge->addr, bridge->port),
                       can_use_bridge_authority ? "Asking bridge authority instead" : "Skipping");
        ...
        if (ask_bridge_directly) {
            ...
            base16_encode(resource + 3, HEX_DIGEST_LEN + 1, bridge->identity, DIGEST_LEN);
            memcpy(resource + 3 + HEX_DIGEST_LEN, ".z", 3);
            log_info(LD_DIR, "Fetching bridge info '%s' from bridge authority.", resource);
            directory_get_from_dirserver(DIR_PURPOSE_FETCH_SERVERDESC, ROUTER_PURPOSE_BRIDGE,
...
rewrite_node_address_for_bridge(const bridge_info_t *bridge, node_t *node)
{
    ...
    if (node->ri) {
        ...
        if ((!tor_addr_compare(&bridge->addr, &addr, CMP_EXACT) && bridge->port == ri->or_port) ||
        ...
        if (!tor_addr_is_null(&ri->ipv6_addr)) {
            ...
            log_notice(LD_CONFIG,
                       ...
                       options->ClientPreferIPv6ORPort == -1 ? "the configured Bridge address"
    ...
    if (node->rs) {
        ...
        if ((!tor_addr_compare(&bridge->addr, &addr, CMP_EXACT) && bridge->port == rs->or_port) ||
