...
proxy_needs_restart(const managed_proxy_t *mp)
{
    ...
    /* If the PT proxy config has changed, then all existing pluggable
     * transports should be restarted.
     */
    ...
    if (smartlist_len(mp->transports_to_launch) !=
        smartlist_len(mp->transports))
        goto needs_restart;
...
launch_managed_proxy(managed_proxy_t *mp)
{
    ...
    process_set_stdout_read_callback(mp->process,
                                     managed_proxy_stdout_callback);
    process_set_stderr_read_callback(mp->process,
...
register_client_proxy(const managed_proxy_t *mp)
{
    ...
    SMARTLIST_FOREACH_BEGIN (mp->transports, transport_t *, t) {
        ...
        case -1:
            ...
            break;
        case 0:
            log_info(LD_GENERAL, "Successfully registered transport %s",
                     t->name);
            control_event_transport_launched("client", t->name, &t->addr,
                                             t->port);
            break;
        case 1:
            log_info(LD_GENERAL, "Successfully registered transport %s",
...
managed_proxy_destroy(managed_proxy_t *mp, int also_terminate_process)
{
    ...
    if (also_terminate_process && mp->process) {
        /* Note that we do not call process_free(mp->process) here because we
         * let the exit handler in managed_proxy_exit_callback() return `true`
         * which makes the process subsystem deallocate the process_t. */
        process_set_data(mp->process, NULL);
        process_terminate(mp->process);
    }
...
handle_proxy_line(const char *line, managed_proxy_t *mp)
{
    ...
    if (!strcmpstart(line, PROTO_ENV_ERROR)) {
         ...
         * string to make sure we can later extend this big if/else-if table
         * with something that begins with "LOG" without having to get the
         * order right.
         * */
    ...
    log_notice(LD_GENERAL, "Unknown line received by managed proxy (%s).",
               line);
    return;
...
parse_version(const char *line, managed_proxy_t *mp)
{
    ...
    if (strcmp("1",
               line + strlen(PROTO_NEG_SUCCESS) + 1)) { /* hardcoded temp */
        log_warn(LD_CONFIG,
    ...
    mp->conf_protocol =
        PROTO_VERSION_ONE; /* temp. till more versions appear */
    return 0;
}
...
parse_status_line(const char *line, managed_proxy_t *mp)
{
    ...
    /* We have checked that TRANSPORT is there, we can now emit the STATUS
     * event via the control port. */
...
get_transport_options_for_server_proxy(const managed_proxy_t *mp)
{
    ...
    /** Loop over the transports of the proxy. If we have options for
        ...
        string. */
    SMARTLIST_FOREACH_BEGIN (mp->transports_to_launch, const char *,
                             transport) {
        ...
        SMARTLIST_FOREACH_BEGIN (options_tmp_sl, const char *, options) {
            ...
            smartlist_add_asprintf(string_sl, "%s:%s", transport,
                                   escaped_opts);
...
create_managed_proxy_environment(const managed_proxy_t *mp)
{
    ...
    if (mp->is_server) {
...

        {
            ...
            smartlist_add_asprintf(envs, "TOR_PT_SERVER_BINDADDR=%s",
                                   bindaddr_tmp);
            tor_free(bindaddr_tmp);
        }
...
get_transport_proxy_ports(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (managed_proxy_list, const managed_proxy_t *, mp) {
        ...
        tor_assert(mp->transports);
        SMARTLIST_FOREACH(
            mp->transports, const transport_t *, t,
