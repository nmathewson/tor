...
STATIC transport_t *
transport_new(const tor_addr_t *addr, uint16_t port, const char *name, int socks_ver,
...
mark_transport_list(void)
{
    ...
    SMARTLIST_FOREACH(transport_list, transport_t *, t, t->marked_for_removal = 1);
}
...
transport_resolve_conflicts(const transport_t *t)
{
    ...
    if (t_tmp) { /* same name */
        if (tor_addr_eq(&t->addr, &t_tmp->addr) && (t->port == t_tmp->port)) {
            ...
            char *new_transport_addrport = tor_strdup(fmt_addrport(&t->addr, t->port));
          ...
          (const tor_addr_t *addr, uint16_t port, const char *name, int socks_ver))
{
    ...
    case -1:
    default:
        log_notice(LD_GENERAL, "Could not add transport %s at %s. Skipping.", t->name,
                   fmt_addrport(&t->addr, t->port));
        transport_free(t);
        return -1;
    case 1:
        log_info(LD_GENERAL, "Successfully registered transport %s at %s.", t->name,
                 fmt_addrport(&t->addr, t->port));
...
launch_managed_proxy(managed_proxy_t *mp)
{
    ...
    if (process_exec(mp->process) != PROCESS_STATUS_RUNNING) {
        log_warn(LD_CONFIG, "Managed proxy at '%s' failed at launch.", mp->argv[0]);
        return -1;
    }

    log_info(LD_CONFIG, "Managed proxy at '%s' has spawned with PID '%" PRIu64 "'.", mp->argv[0],
...
pt_configure_remaining_proxies(void)
{
    ...
    log_debug(LD_CONFIG, "Configuring remaining managed proxies (%d)!", unconfigured_proxies_n);
    ...
    SMARTLIST_FOREACH_BEGIN (tmp, managed_proxy_t *, mp) {
        tor_assert(mp->conf_state != PT_PROTO_BROKEN && mp->conf_state != PT_PROTO_FAILED_LAUNCH);
        ...
        if (mp->was_around_before_config_read) {
            ...
            if (proxy_needs_restart(mp)) {
                log_info(LD_GENERAL, "Preparing managed proxy '%s' for restart.", mp->argv[0]);
    ...
    if (options->Socks4Proxy || options->Socks5Proxy || options->HTTPSProxy) {
        ...
        if (options->Socks4Proxy) {
            ...
            if (!options->Socks5ProxyUsername && !options->Socks5ProxyPassword) {
                ...
                tor_asprintf(&uri, "socks5://%s:%s@%s:%d", options->Socks5ProxyUsername,
            ...
            if (!options->HTTPSProxyAuthenticator) {
                ...
                tor_asprintf(&uri, "http://%s@%s:%d", options->HTTPSProxyAuthenticator, addr,
...
handle_finished_proxy(managed_proxy_t *mp)
{
    ...
    default:
        log_warn(LD_CONFIG, "Unexpected state '%d' of managed proxy '%s'.", (int)mp->conf_state,
                 mp->argv[0]);
        tor_assert(0);
    }
...
{
    return (mp->conf_state == PT_PROTO_CONFIGURED || mp->conf_state == PT_PROTO_BROKEN ||
            mp->conf_state == PT_PROTO_FAILED_LAUNCH);
}
...
{
    log_info(LD_GENERAL, "Got a line from managed proxy '%s': (%s)", mp->argv[0], line);
...
parse_version(const char *line, managed_proxy_t *mp)
{
    if (strlen(line) < (strlen(PROTO_NEG_SUCCESS) + 2)) {
        log_warn(LD_CONFIG, "Managed proxy sent us malformed %s line.", PROTO_NEG_SUCCESS);
        return -1;
    }
...
parse_method_line_helper(const char *line, managed_proxy_t *mp, int is_smethod)
{
    ...
    transport = transport_new(&tor_addr, port, transport_name, socks_ver, args_string);

    smartlist_add(mp->transports, transport);

    /** Logs info about line parsing success for client or server */
    if (is_smethod) {
        log_info(LD_CONFIG, "Server transport %s at %s:%d.", transport_name, address, (int)port);
    } else {
        log_info(LD_CONFIG,
...
parse_log_line(const char *line, managed_proxy_t *mp)
{
    ...
    if (!values) {
        log_warn(LD_PT, "Managed proxy \"%s\" wrote an invalid LOG message: %s", mp->argv[0], data);
        goto done;
    }
    ...
    tor_log(log_severity, LD_PT, "Managed proxy \"%s\": %s", mp->argv[0], message->value);
...
create_managed_proxy_environment(const managed_proxy_t *mp)
{
    ...
    {
        char *transports_to_launch = smartlist_join_strings(mp->transports_to_launch, ",", 0, NULL);

        smartlist_add_asprintf(
            envs, mp->is_server ? "TOR_PT_SERVER_TRANSPORTS=%s" : "TOR_PT_CLIENT_TRANSPORTS=%s",
    ...
    if (mp->is_server) {
        ...
        {
            char *server_transport_options = get_transport_options_for_server_proxy(mp);
...
STATIC managed_proxy_t *
managed_proxy_create(const smartlist_t *with_transport_list, char **proxy_argv, int is_server)
{
    ...
    SMARTLIST_FOREACH_BEGIN (managed_proxy_list, const managed_proxy_t *, mp) {
        ...
        SMARTLIST_FOREACH_BEGIN (mp->transports, const transport_t *, t) {
            ...
            if (tor_addr_is_null(&t->addr) &&
                router_pick_published_address(get_options(), &external_ip_address, 0) >= 0) {
                tor_addr_t addr;
            ...
            smartlist_add_asprintf(string_chunks, "transport %s %s%s", t->name, addrport,
...
STATIC void
managed_proxy_stdout_callback(process_t *process, const char *line, size_t size)
{
    tor_assert(process);
    tor_assert(line);
 ...
 * <b>line</b> and the length of our line is given in <b>size</b>. */
STATIC void
managed_proxy_stderr_callback(process_t *process, const char *line, size_t size)
{
    tor_assert(process);
    tor_assert(line);
    ...
    log_info(LD_PT, "Managed proxy at '%s' reported via standard error: %s", mp->argv[0], line);
}
