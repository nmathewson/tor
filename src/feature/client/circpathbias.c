...
entry_guard_inc_circ_attempt_count(entry_guard_t *guard)
{
    ...
    log_info(LD_CIRC, "Got success count %f/%f for guard %s", pb->circ_successes, pb->circ_attempts,
...
#define DFLT_PATH_BIAS_MIN_CIRC 150
    if (options->PathBiasCircThreshold >= 5)
        ...
        return networkstatus_get_param(NULL, "pb_mincircs", DFLT_PATH_BIAS_MIN_CIRC, 5, INT32_MAX);
}
...
#define DFLT_PATH_BIAS_NOTICE_PCT 70
    if (options->PathBiasNoticeRate >= 0.0)
        ...
        return networkstatus_get_param(NULL, "pb_noticepct", DFLT_PATH_BIAS_NOTICE_PCT, 0, 100) /
               100.0;
}
...
#define DFLT_PATH_BIAS_WARN_PCT 50
    if (options->PathBiasWarnRate >= 0.0)
        ...
        return networkstatus_get_param(NULL, "pb_warnpct", DFLT_PATH_BIAS_WARN_PCT, 0, 100) / 100.0;
}
...
#define DFLT_PATH_BIAS_EXTREME_PCT 30
    if (options->PathBiasExtremeRate >= 0.0)
        ...
        return networkstatus_get_param(NULL, "pb_extremepct", DFLT_PATH_BIAS_EXTREME_PCT, 0, 100) /
               100.0;
}
...
#define DFLT_PATH_BIAS_DROP_GUARDS 0
    if (options->PathBiasDropGuards >= 0)
        ...
        return networkstatus_get_param(NULL, "pb_dropguards", DFLT_PATH_BIAS_DROP_GUARDS, 0, 1);
}
...
#define DFLT_PATH_BIAS_SCALE_THRESHOLD 300
    if (options->PathBiasScaleThreshold >= 10)
        ...
        return networkstatus_get_param(NULL, "pb_scalecircs", DFLT_PATH_BIAS_SCALE_THRESHOLD, 10,
                                       INT32_MAX);
}
...
pathbias_get_scale_ratio(const or_options_t *options)
{
     ...
     */
    int denominator = networkstatus_get_param(NULL, "pb_scalefactor", 2, 2, INT32_MAX);
...
#define DFLT_PATH_BIAS_MIN_USE 20
    if (options->PathBiasUseThreshold >= 3)
        ...
        return networkstatus_get_param(NULL, "pb_minuse", DFLT_PATH_BIAS_MIN_USE, 3, INT32_MAX);
}
...
#define DFLT_PATH_BIAS_NOTICE_USE_PCT 80
    if (options->PathBiasNoticeUseRate >= 0.0)
        ...
        return networkstatus_get_param(NULL, "pb_noticeusepct", DFLT_PATH_BIAS_NOTICE_USE_PCT, 0,
                                       100) /
               100.0;
}
...
#define DFLT_PATH_BIAS_EXTREME_USE_PCT 60
    if (options->PathBiasExtremeUseRate >= 0.0)
        ...
        return networkstatus_get_param(NULL, "pb_extremeusepct", DFLT_PATH_BIAS_EXTREME_USE_PCT, 0,
                                       100) /
               100.0;
}
...
#define DFLT_PATH_BIAS_SCALE_USE_THRESHOLD 100
    if (options->PathBiasScaleUseThreshold >= 10)
        ...
        return networkstatus_get_param(NULL, "pb_scaleuse", DFLT_PATH_BIAS_SCALE_USE_THRESHOLD, 10,
                                       INT32_MAX);
}
...
#define PATHBIAS_COUNT_INTERVAL (600)
    ...
    if (get_options()->UseEntryGuards == 0 || circ->base_.purpose == CIRCUIT_PURPOSE_TESTING ||
        ...
        if (circ->pathbias_shouldcount == PATHBIAS_SHOULDCOUNT_COUNTED &&
            ...
            log_info(LD_BUG,
                     ...
                     circ->global_identifier, circuit_purpose_to_string(circ->base_.purpose),
...
#define CIRC_ATTEMPT_NOTICE_INTERVAL (600)
    static ratelim_t circ_attempt_notice_limit = RATELIM_INIT(CIRC_ATTEMPT_NOTICE_INTERVAL);
    char *rate_msg = NULL;
    ...
    if (pathbias_is_new_circ_attempt(circ)) {
        ...
        if (circ->has_opened && circ->path_state != PATH_STATE_BUILD_ATTEMPTED) {
            if ((rate_msg = rate_limit_log(&circ_attempt_notice_limit, approx_time()))) {
                log_info(LD_BUG,
                         "Opened circuit %d is in strange path state %s. "
                         "Circuit is a %s currently %s.%s",
                         circ->global_identifier, pathbias_state_to_string(circ->path_state),
                         circuit_purpose_to_string(circ->base_.purpose),
                         circuit_state_to_string(circ->base_.state), rate_msg);
                tor_free(rate_msg);
            }
        ...
        if (!circ->has_opened) {
            ...
            if (circ->cpath && circ->cpath->extend_info) {
                guard = entry_guard_get_by_id_digest(circ->cpath->extend_info->identity_digest);
        ...
        if (guard) {
            ...
            if (pb->circ_attempts < pb->circ_successes) {
                log_notice(LD_BUG,
                           ...
                           "for guard %s",
                           pb->circ_successes, pb->circ_attempts, entry_guard_describe(guard));
            }
...
pathbias_count_use_attempt(origin_circuit_t *circ)
{
    ...
    if (circ->path_state < PATH_STATE_BUILD_SUCCEEDED) {
        ...
        entry_guard_t *guard =
            entry_guard_get_by_id_digest(circ->cpath->extend_info->identity_digest);
        if (guard) {
            guard_pathbias_t *pb = entry_guard_get_pathbias_state(guard);
        ...
        if (guard) {
            ...
            if (pb->use_attempts < pb->use_successes) {
                log_notice(LD_BUG,
                           ...
                           "for guard %s",
                           pb->use_successes, pb->use_attempts, entry_guard_describe(guard));
            }

            log_debug(LD_CIRC, "Marked circuit %d (%f/%f) as used successfully for guard %s",
                      circ->global_identifier, pb->use_successes, pb->use_attempts,
                      entry_guard_describe(guard));
        }
    }
...
pathbias_send_usable_probe(circuit_t *circ)
{
    ...
    if (cpath_layer->state != CPATH_STATE_OPEN) {
        ...
        log_info(LD_CIRC,
                 ...
                 "Opened %d, len %d",
                 ocirc->global_identifier, ocirc->has_opened, ocirc->build_state->desired_path_len);
        return -1;
    }

    /* We already went down this road. */
    if (circ->purpose == CIRCUIT_PURPOSE_PATH_BIAS_TESTING && ocirc->pathbias_probe_id) {
        log_info(LD_CIRC,
    ...
    if (circ->n_chan == NULL ||
        ...
        log_info(LD_CIRC, "Skipping pathbias probe for circuit %d: Channel is not open.",
                 ocirc->global_identifier);
        return -1;
    }
    ...
    crypto_rand((char *)&ocirc->pathbias_probe_nonce, sizeof(ocirc->pathbias_probe_nonce));
    ...
    log_info(LD_CIRC, "Sending pathbias testing cell to %s:25 on stream %d for circ %d.",
             probe_nonce, ocirc->pathbias_probe_id, ocirc->global_identifier);
    tor_free(probe_nonce);

    /* Send a test relay cell */
    if (relay_send_command_from_edge(ocirc->pathbias_probe_id, circ, RELAY_COMMAND_BEGIN, payload,
...
pathbias_check_probe_response(circuit_t *circ, const cell_t *cell)
{
    ...
    reason = rh.length > 0 ? get_uint8(cell->payload + RELAY_HEADER_SIZE) : END_STREAM_REASON_MISC;

    if (rh.command == RELAY_COMMAND_END && reason == END_STREAM_REASON_EXITPOLICY &&
        ...
        if (rh.length < 9) { /* reason+ipv4+dns_ttl */
            log_notice(LD_PROTOCOL, "Short path bias probe response length field (%d).", rh.length);
            return -END_CIRC_REASON_TORPROTOCOL;
        }
        ...
        if (ipv4_host == ocirc->pathbias_probe_nonce) {
            ...
            log_info(LD_CIRC, "Got valid path bias probe back for circ %d, stream %d.",
            ...
            log_notice(LD_CIRC,
                       ...
                       "stream %d.",
                       ipv4_host, ocirc->pathbias_probe_nonce, ocirc->global_identifier,
    ...
    case PATH_STATE_BUILD_SUCCEEDED:
        if (reason & END_CIRC_REASON_FLAG_REMOTE) {
            ...
            log_info(LD_CIRC,
                     ...
                     circuit_state_to_string(circ->state), ocirc->build_state->desired_path_len);
            pathbias_count_collapse(ocirc);
        } else if ((reason & ~END_CIRC_REASON_FLAG_REMOTE) == END_CIRC_REASON_CHANNEL_CLOSED &&
                   circ->n_chan && circ->n_chan->reason_for_closing != CHANNEL_CLOSE_REQUESTED) {
            ...
            log_info(LD_CIRC,
                     ...
                     circ->purpose, ocirc->has_opened, circuit_state_to_string(circ->state),
 ...
 */
static int
pathbias_count_circs_in_states(entry_guard_t *guard, path_state_t from, path_state_t to)
{
    int open_circuits = 0;
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        ...
        if (ocirc->path_state >= from && ocirc->path_state <= to && pathbias_should_count(ocirc) &&
            fast_memeq(entry_guard_get_rsa_id_digest(guard),
                       ocirc->cpath->extend_info->identity_digest, DIGEST_LEN)) {
            log_debug(LD_CIRC, "Found opened circuit %d in path_state %s", ocirc->global_identifier,
...
pathbias_get_use_success_count(entry_guard_t *guard)
{
    ...
    return pb->use_successes + pathbias_count_circs_in_states(guard, PATH_STATE_USE_ATTEMPTED,
                                                              PATH_STATE_USE_SUCCEEDED);
}
            ...
            if (pathbias_get_dropguards(options)) {
                if (!pb->path_bias_disabled) {
                    log_warn(LD_CIRC,
                             ...
                             tor_lround(pb->unusable_circuits), tor_lround(pb->collapsed_circuits),
                ...
                log_warn(
                    ...
                    "and %ld timed out. "
                    "For reference, your timeout cutoff is %ld seconds.",
                    entry_guard_describe(guard), tor_lround(pathbias_get_use_success_count(guard)),
                    tor_lround(pb->use_attempts),
                    tor_lround(pathbias_get_close_success_count(guard)),
                    tor_lround(pb->circ_attempts), tor_lround(pb->circ_successes),
                    tor_lround(pb->unusable_circuits), tor_lround(pb->collapsed_circuits),
                    tor_lround(pb->timeouts), tor_lround(get_circuit_build_close_time_ms() / 1000));
            }
...
pathbias_scale_close_rates(entry_guard_t *guard)
{
    ...
    if (pb->circ_attempts > pathbias_get_scale_threshold(options)) {
        ...
        int opened_attempts = pathbias_count_circs_in_states(guard, PATH_STATE_BUILD_ATTEMPTED,
                                                             PATH_STATE_BUILD_ATTEMPTED);
        int opened_built = pathbias_count_circs_in_states(guard, PATH_STATE_BUILD_SUCCEEDED,
        ...
        log_info(LD_CIRC,
                 ...
                 "%s",
                 pb->circ_successes, pb->successful_circuits_closed, pb->circ_attempts,
                 opened_built, opened_attempts, entry_guard_describe(guard));
        ...
        if (counts_are_sane && pb->circ_attempts < pb->circ_successes) {
            log_notice(LD_BUG,
                       ...
                       "for guard %s",
                       pb->circ_successes, pb->circ_attempts, opened_built, opened_attempts,
                       entry_guard_describe(guard));
        }
    }
}
...
pathbias_scale_use_rates(entry_guard_t *guard)
{
    ...
    if (pb->use_attempts > pathbias_get_scale_use_threshold(options)) {
        double scale_ratio = pathbias_get_scale_ratio(options);
        int opened_attempts = pathbias_count_circs_in_states(guard, PATH_STATE_USE_ATTEMPTED,
                                                             PATH_STATE_USE_SUCCEEDED);
        /* Verify that the counts are sane before and after scaling */
        ...
        log_info(LD_CIRC, "Scaled pathbias use counts to %f/%f (%d open) for guard %s",
                 pb->use_successes, pb->use_attempts, opened_attempts, entry_guard_describe(guard));
