...
evdns_server_callback(struct evdns_server_request *req, void *data_)
{
    ...
    if ((addrlen = evdns_server_request_get_requesting_addr(req, (struct sockaddr *)&addr,
    ...
    if (q->type == EVDNS_TYPE_A || q->type == EVDNS_TYPE_AAAA || q->type == EVDNS_QTYPE_ALL) {
...
dnsserv_launch_request(const char *name, int reverse, control_connection_t *control_conn)
{
    ...
    strlcpy(entry_conn->socks_request->address, name, sizeof(entry_conn->socks_request->address));
 ...
 * DNS poisoning. */
static const char *
evdns_get_orig_address(const struct evdns_server_request *req, int rtype, const char *addr)
{
    int i, type;
...
void
dnsserv_resolved(entry_connection_t *conn, int answer_type, size_t answer_len, const char *answer,
                 int ttl)
{
...
dnsserv_configure_listener(connection_t *conn)
{
    ...
    listener_conn->dns_server_port =
        tor_evdns_add_server_port(conn->s, 0, evdns_server_callback, listener_conn);
}
