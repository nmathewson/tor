...
evdns_server_callback(struct evdns_server_request *req, void *data_)
{
    ...
    /* Now, let's find the first actual question of a type we can answer in
     * this DNS request.  It makes us a little noncompliant to act like this;
     * we should fix that eventually if it turns out to make a difference for
     * anybody. */
    if (req->nquestions == 0) {
        log_info(LD_APP,
    ...
    entry_conn->entry_cfg.isolation_flags =
        listener->entry_cfg.isolation_flags;
                       ...
                       control_connection_t *control_conn)
{
    ...
    if (control_conn->base_.socket_family == AF_UNIX) {
        ...
        TO_CONN(conn)->address =
            tor_addr_to_str_dup(&control_conn->base_.addr);
    }
