...
trusted_dir_server_new(const char *nickname, const char *address,
                       ...
                       const tor_addr_port_t *ipv6_addrport, const char *digest,
                       const char *v3_auth_digest, dirinfo_type_t type,
                       double weight)
{
    uint32_t a;
    tor_addr_t addr;
    ...
    if (!address) { /* The address is us; we should guess. */
        if (resolve_my_address(LOG_WARN, get_options(), &a, NULL, &hostname) <
            0) {
            log_warn(
                LD_CONFIG,
    ...
    result =
        dir_server_new(1, nickname, &addr, hostname, dir_port, or_port,
