...
routerset_parse(routerset_t *target, const char *s, const char *description)
{
    ...
    int malformed_list;
    smartlist_split_string(list, s, ",", SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 0);
    SMARTLIST_FOREACH_BEGIN (list, char *, nick) {
        ...
        if (is_legal_hexdigest(nick)) {
            ...
            added_countries = 1;
                   ...
                   (p = router_parse_addr_policy_item_from_string(nick, ADDR_POLICY_REJECT,
                                                                  &malformed_list))) {
...
{
    return smartlist_len(set->country_names) == 0 && smartlist_len(set->policies) == 0;
}
 ...
 * from addr.) */
STATIC int
routerset_contains(const routerset_t *set, const tor_addr_t *addr, uint16_t orport,
                   const char *nickname, const char *id_digest, country_t country)
{
    if (!set || !set->list)
        return 0;
    ...
    if (id_digest && digestmap_get(set->digests, id_digest))
        return 4;
    if (addr &&
        compare_tor_addr_to_addr_policy(addr, orport, set->policies) == ADDR_POLICY_REJECTED)
        return 3;
    if (set->countries) {
        if (country < 0 && addr)
            country = geoip_get_country_by_addr(addr);

        if (country >= 0 && country < set->n_countries && bitarray_is_set(set->countries, country))
            return 2;
    }
    return 0;
}
...
routerset_add_unknown_ccs(routerset_t **setp, int only_if_some_cc_set)
{
    ...
    add_unknown =
        !smartlist_contains_string_case(set->country_names, "??") && geoip_get_country("??") >= 0;
    add_a1 =
        !smartlist_contains_string_case(set->country_names, "a1") && geoip_get_country("A1") >= 0;
...
{
    return routerset_contains(set, &ei->addr, ei->port, ei->nickname, ei->identity_digest,
                              -1 /*country*/);
}
 ...
 * look up the country. */
int
routerset_contains_router(const routerset_t *set, const routerinfo_t *ri, country_t country)
{
    tor_addr_t addr;
    tor_addr_from_ipv4h(&addr, ri->addr);
    return routerset_contains(set, &addr, ri->or_port, ri->nickname, ri->cache_info.identity_digest,
                              country);
}
 ...
 * look up the country. */
int
routerset_contains_routerstatus(const routerset_t *set, const routerstatus_t *rs, country_t country)
{
    tor_addr_t addr;
    tor_addr_from_ipv4h(&addr, rs->addr);
    return routerset_contains(set, &addr, rs->or_port, rs->nickname, rs->identity_digest, country);
}
...
routerset_contains_bridge(const routerset_t *set, const bridge_info_t *bridge)
{
    ...
    tor_assert(addrport);
    return routerset_contains(set, &addrport->addr, addrport->port, NULL, id, -1);
}
...
routerset_free_(routerset_t *routerset)
{
    ...
    SMARTLIST_FOREACH(routerset->policies, addr_policy_t *, p, addr_policy_free(p));
 ...
 */
static int
routerset_kv_parse(void *target, const config_line_t *line, char **errmsg, const void *params)
{
    (void)params;
 ...
 **/
static const var_type_fns_t routerset_type_fns = {.kv_parse = routerset_kv_parse,
 ...
 **/
const var_type_def_t ROUTERSET_type_defn = {.name = "RouterList", .fns = &routerset_type_fns};
