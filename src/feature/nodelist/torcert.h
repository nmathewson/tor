...
tor_cert_t *tor_cert_create(const ed25519_keypair_t *signing_key, uint8_t cert_type,
                            const ed25519_public_key_t *signed_key, time_t now, time_t lifetime,
                            uint32_t flags);
...
int tor_cert_get_checkable_sig(ed25519_checkable_t *checkable_out, const tor_cert_t *out,
...
ssize_t tor_make_rsa_ed25519_crosscert(const ed25519_public_key_t *ed_key,
                                       const crypto_pk_t *rsa_key, time_t expires, uint8_t **cert);
...
int or_handshake_certs_rsa_ok(int severity, or_handshake_certs_t *certs, struct tor_tls_t *tls,
                              time_t now);
int or_handshake_certs_ed25519_ok(int severity, or_handshake_certs_t *certs, struct tor_tls_t *tls,
                                  time_t now);
void or_handshake_certs_check_both(int severity, or_handshake_certs_t *certs, struct tor_tls_t *tls,
                                   time_t now, const ed25519_public_key_t **ed_id_out,
