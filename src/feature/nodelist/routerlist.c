...
router_rebuild_store(int flags, desc_store_t *store)
{
    ...
    if (store->type == EXTRAINFO_STORE)
        ...
        had_any = (smartlist_len(routerlist->routers) + smartlist_len(routerlist->old_routers)) > 0;
    ...
    if (store->type == EXTRAINFO_STORE) {
        eimap_iter_t *iter;
        for (iter = eimap_iter_init(routerlist->extra_info_map); !eimap_iter_done(iter);
...
router_reload_router_list_impl(desc_store_t *store)
{
    ...
    if (store->mmap) {
        ...
        if (extrainfo)
            router_load_extrainfo_from_string(
                store->mmap->data, store->mmap->data + store->mmap->size, SAVED_IN_CACHE, NULL, 0);
        else
            router_load_routers_from_string(store->mmap->data,
                                            store->mmap->data + store->mmap->size, SAVED_IN_CACHE,
                                            NULL, 0, NULL);
    }
    ...
    if (contents) {
        if (extrainfo)
            router_load_extrainfo_from_string(contents, NULL, SAVED_IN_JOURNAL, NULL, 0);
        else
            router_load_routers_from_string(contents, NULL, SAVED_IN_JOURNAL, NULL, 0, NULL);
...
routers_have_same_or_addrs(const routerinfo_t *r1, const routerinfo_t *r2)
{
    return r1->addr == r2->addr && r1->or_port == r2->or_port &&
           tor_addr_eq(&r1->ipv6_addr, &r2->ipv6_addr) && r1->ipv6_orport == r2->ipv6_orport;
}
 ...
 */
void
router_add_running_nodes_to_smartlist(smartlist_t *sl, int need_uptime, int need_capacity,
                                      int need_guard, int need_desc, int pref_addr, int direct_conn)
{
    const int check_reach = !router_skip_or_reachability(get_options(), pref_addr);
    /* XXXX MOVE */
...
hex_digest_nickname_decode(const char *hexdigest, char *digest_out,
                           char *nickname_qualifier_char_out, char *nickname_out)
{
    size_t len;
    ...
    if (len < HEX_DIGEST_LEN) {
        return -1;
               ...
               (hexdigest[HEX_DIGEST_LEN] == '=' || hexdigest[HEX_DIGEST_LEN] == '~') &&
    ...
    if (base16_decode(digest_out, DIGEST_LEN, hexdigest, HEX_DIGEST_LEN) != DIGEST_LEN)
        return -1;
    return 0;
}
 ...
 */
static const char *
signed_descriptor_get_body_impl(const signed_descriptor_t *desc, int with_annotations)
{
    const char *r = NULL;
    ...
    if (!r) /* no mmap, or not in cache. */
        r = desc->signed_descriptor_body + (with_annotations ? 0 : desc->annotations_len);
...
routerlist_free_(routerlist_t *rl)
{
    ...
    SMARTLIST_FOREACH(rl->old_routers, signed_descriptor_t *, sd, signed_descriptor_free(sd));
...
dump_routerlist_mem_usage(int severity)
{
    ...
    tor_log(severity, LD_DIR,
            ...
            smartlist_len(routerlist->routers), (livedescs), smartlist_len(routerlist->old_routers),
            (olddescs));
}
...
routerlist_insert(routerlist_t *rl, routerinfo_t *ri)
{
    ...
    sd_old =
        sdmap_set(rl->desc_digest_map, ri->cache_info.signed_descriptor_digest, &(ri->cache_info));
    if (sd_old) {
    ...
    if (!tor_digest_is_zero(ri->cache_info.extra_info_digest))
        sdmap_set(rl->desc_by_eid_map, ri->cache_info.extra_info_digest, &ri->cache_info);
          ...
          (routerlist_t * rl, extrainfo_t *ei, int warn_if_incompatible))
{
    ...
    if (!sd) {
        ...
        r = ROUTER_BAD_EI;
        log_fn_ratelim(&no_sd_ratelim, severity, LD_BUG, "No entry found in extrainfo map.");
        goto done;
    }
    if (tor_memneq(ei->cache_info.signed_descriptor_digest, sd->extra_info_digest, DIGEST_LEN)) {
    ...
    if (routerinfo_incompatible_with_extrainfo(ri->identity_pkey, ei, sd,
        ...
        r = (ri->cache_info.extrainfo_is_bogus) ? ROUTER_BAD_EI : ROUTER_NOT_IN_CONSENSUS;
...
routerlist_insert_old(routerlist_t *rl, routerinfo_t *ri)
{
    ...
    if (should_cache_old_descriptors() && ri->purpose == ROUTER_PURPOSE_GENERAL &&
...
routerlist_remove(routerlist_t *rl, routerinfo_t *ri, int make_old, time_t now)
{
    ...
    if (make_old && should_cache_old_descriptors() && ri->purpose == ROUTER_PURPOSE_GENERAL) {
        signed_descriptor_t *sd;
        ...
        sd_tmp = sdmap_remove(rl->desc_digest_map, ri->cache_info.signed_descriptor_digest);
        tor_assert(sd_tmp == &(ri->cache_info));
        rl->desc_store.bytes_dropped += ri->cache_info.signed_descriptor_len;
        ei_tmp = eimap_remove(rl->extra_info_map, ri->cache_info.extra_info_digest);
        if (ei_tmp) {
            rl->extrainfo_store.bytes_dropped += ei_tmp->cache_info.signed_descriptor_len;
            extrainfo_free(ei_tmp);
        }
...
static void
routerlist_replace(routerlist_t *rl, routerinfo_t *ri_old, routerinfo_t *ri_new)
{
    int idx;
    ...
    if (tor_memneq(ri_old->cache_info.identity_digest, ri_new->cache_info.identity_digest,
                   DIGEST_LEN)) {
        /* digests don't match; digestmap_set won't replace */
        rimap_remove(rl->identity_map, ri_old->cache_info.identity_digest);
    }
    ri_tmp = rimap_set(rl->identity_map, ri_new->cache_info.identity_digest, ri_new);
    ...
    if (should_cache_old_descriptors() && ri_old->purpose == ROUTER_PURPOSE_GENERAL &&
        !same_descriptors) {
        ...
        if (!same_descriptors) {
            ...
            sdmap_remove(rl->desc_digest_map, ri_old->cache_info.signed_descriptor_digest);

            if (tor_memneq(ri_old->cache_info.extra_info_digest,
                           ri_new->cache_info.extra_info_digest, DIGEST_LEN)) {
                ei_tmp = eimap_remove(rl->extra_info_map, ri_old->cache_info.extra_info_digest);
                if (ei_tmp) {
                    rl->extrainfo_store.bytes_dropped += ei_tmp->cache_info.signed_descriptor_len;
                    extrainfo_free(ei_tmp);
                }
...
routerlist_reparse_old(routerlist_t *rl, signed_descriptor_t *sd)
{
    ...
    ri = router_parse_entry_from_string(
        body, body + sd->signed_descriptor_len + sd->annotations_len, 0, 1, NULL, NULL);
    if (!ri)
        return NULL;
 ...
 */
was_router_added_t
router_add_to_routerlist(routerinfo_t *router, const char **msg, int from_cache, int from_fetch)
{
    ...
    networkstatus_t *consensus = networkstatus_get_latest_consensus_by_flavor(FLAV_NS);
    int in_consensus = 0;
    ...
    if (sdmap_get(routerlist->desc_digest_map, router->cache_info.signed_descriptor_digest)) {
        ...
        const int was_bridge = old_router && old_router->purpose == ROUTER_PURPOSE_BRIDGE;

        if (routerinfo_is_a_configured_bridge(router) && router->purpose == ROUTER_PURPOSE_BRIDGE &&
            !was_bridge) {
            log_info(LD_DIR,
            ...
            log_info(LD_DIR, "Dropping descriptor that we already have for router %s",
                     router_describe(router));
    ...
    if (authdir) {
        ...
        if (!signed_desc_digest_is_recognized(&router->cache_info) &&
            ...
            log_info(LD_DIR, "Received a no-longer-recognized descriptor for router %s",
                     router_describe(router));
            *msg = "Router descriptor is not referenced by any network-status.";

            /* Only journal this desc if we want to keep old descriptors */
            if (!from_cache && should_cache_old_descriptors())
                signed_desc_append_to_journal(&router->cache_info, &routerlist->desc_store);
    ...
    if (consensus) {
        routerstatus_t *rs = networkstatus_vote_find_mutable_entry(consensus, id_digest);
        if (rs && tor_memeq(rs->descriptor_digest, router->cache_info.signed_descriptor_digest,
                            DIGEST_LEN)) {
            in_consensus = 1;
        }
    }

    if (router->purpose == ROUTER_PURPOSE_GENERAL && consensus && !in_consensus && !authdir) {
    ...
    if (router->purpose == ROUTER_PURPOSE_BRIDGE && from_cache && !authdir_mode_bridge(options) &&
        !routerinfo_is_a_configured_bridge(router)) {
        log_info(LD_DIR,
    ...
    if (old_router) {
        if (!in_consensus &&
            (router->cache_info.published_on <= old_router->cache_info.published_on)) {
            /* Same key, but old.  This one is not listed in the consensus. */
            log_debug(LD_DIR, "Not-new descriptor for router %s", router_describe(router));
            ...
            *msg = "Router descriptor was not new.";
            ...
            log_debug(LD_DIR, "Replacing entry for router %s", router_describe(router));
 ...
 */
was_router_added_t
router_add_extrainfo_to_routerlist(extrainfo_t *ei, const char **msg, int from_cache,
                                   int from_fetch)
{
    ...
    if (WRA_WAS_ADDED(inserted) && !from_cache)
        signed_desc_append_to_journal(&ei->cache_info, &routerlist->extrainfo_store);
 ...
 */
static void
routerlist_remove_old_cached_routers_with_id(time_t now, time_t cutoff, int lo, int hi,
                                             digestset_t *retain)
{
    int i, n = hi - lo + 1;
    ...
    for (i = lo; i <= hi; ++i) {
        ...
        if (r->last_listed_as_valid_until >= now ||
            (retain && digestset_probably_contains(retain, r->signed_descriptor_digest))) {
            must_keep[i - lo] = 1;
        }
        if (i < hi) {
...
routerlist_remove_old_routers(void)
{
    ...
    */
    ...
    if (consensus) {
        ...
        for (i = 0; i < smartlist_len(routerlist->routers); ++i) {
            ...
            if (router->cache_info.published_on <= cutoff &&
                ...
                !digestset_probably_contains(retain, router->cache_info.signed_descriptor_digest)) {
                /* Too old: remove it.  (If we're a cache, just move it into
                 * old_routers.) */
                log_info(LD_DIR, "Forgetting obsolete (too old) routerinfo for router %s",
                         router_describe(router));
    ...
    log_info(LD_DIR, "We have %d live routers and %d old router descriptors.",
             smartlist_len(routerlist->routers), smartlist_len(routerlist->old_routers));
 ...
 */
int
router_load_single_router(const char *s, uint8_t purpose, int cache, const char **msg)
{
    routerinfo_t *ri;
    was_router_added_t r;
    smartlist_t *lst;
    ...
    *msg = NULL;
    ...
    if (!(ri = router_parse_entry_from_string(s, NULL, 1, 0, annotation_buf, NULL))) {
 ...
 */
int
router_load_routers_from_string(const char *s, const char *eos, saved_location_t saved_location,
                                ...
                                const char *prepend_annotations)
{
    ...
    router_parse_list_from_string(&s, eos, routers, saved_location, 0, allow_annotations,
                                  prepend_annotations, invalid_digests);
    ...
    SMARTLIST_FOREACH_BEGIN (routers, routerinfo_t *, ri) {
        ...
        if (requested_fingerprints) {
            base16_encode(fp, sizeof(fp),
                          descriptor_digests ? ri->cache_info.signed_descriptor_digest
...
void
router_load_extrainfo_from_string(const char *s, const char *eos, saved_location_t saved_location,
                                  smartlist_t *requested_fingerprints, int descriptor_digests)
{
    ...
    router_parse_list_from_string(&s, eos, extrainfo_list, saved_location, 1, 0, NULL,
                                  invalid_digests);
    ...
    SMARTLIST_FOREACH_BEGIN (extrainfo_list, extrainfo_t *, ei) {
        ...
        if (WRA_WAS_ADDED(added) && requested_fingerprints) {
            char fp[HEX_DIGEST_LEN + 1];
            base16_encode(fp, sizeof(fp),
                          descriptor_digests ? ei->cache_info.signed_descriptor_digest
    ...
    SMARTLIST_FOREACH_BEGIN (invalid_digests, const uint8_t *, bad_digest) {
        ...
        signed_descriptor_t *sd = router_get_by_extrainfo_digest((char *)bad_digest);
        if (sd) {
            log_info(LD_GENERAL,
                     "Marking extrainfo with descriptor %s as "
                     "unparseable, and therefore undownloadable",
...
signed_desc_digest_is_recognized(signed_descriptor_t *desc)
{
    ...
    if (consensus) {
        ...
        if (rs && tor_memeq(rs->descriptor_digest, desc->signed_descriptor_digest, DIGEST_LEN))
            return 1;
    }
    return 0;
}
 ...
 */
void
list_pending_downloads(digestmap_t *result, digest256map_t *result256, int purpose,
                       const char *prefix)
{
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn) {
        if (conn->type == CONN_TYPE_DIR && conn->purpose == purpose && !conn->marked_for_close) {
            const char *resource = TO_DIR_CONN(conn)->requested_resource;
            if (!strcmpstart(resource, prefix))
                dir_split_resource_into_fingerprints(resource + p_len, tmp, NULL, flags);
        }
...
{
    int purpose = extrainfo ? DIR_PURPOSE_FETCH_EXTRAINFO : DIR_PURPOSE_FETCH_SERVERDESC;
           ...
           int pds_flags))
{
    ...
    if (source) {
        ...
        directory_get_from_dirserver(purpose, ROUTER_PURPOSE_GENERAL, resource, pds_flags,
 ...
 */
void
launch_descriptor_downloads(int purpose, smartlist_t *downloadable, const routerstatus_t *source,
                            time_t now)
{
    const or_options_t *options = get_options();
    const char *descname;
    ...
    if (!dirclient_fetches_dir_info_early(options)) {
        if (n_downloadable >= MAX_DL_TO_DELAY) {
            log_debug(LD_DIR, "There are enough downloadable %ss to launch requests.", descname);
    ...
    if (!authdir_mode(options)) {
         ...
         */
        pds_flags |=
            fetch_microdesc ? PDS_NO_EXISTING_MICRODESC_FETCH : PDS_NO_EXISTING_SERVERDESC_FETCH;
    }
    ...
    log_info(LD_DIR, "Launching %d request%s for %d %s%s, %d at a time",
             CEIL_DIV(n_downloadable, n_per_request), req_plural, n_downloadable, descname,
             rtr_plural, n_per_request);
    smartlist_sort_digests(downloadable);
    for (i = 0; i < n_downloadable; i += n_per_request) {
        initiate_descriptor_downloads(source, purpose, downloadable, i, i + n_per_request,
                                      pds_flags);
    }
...
void
update_consensus_router_descriptor_downloads(time_t now, int is_vote, networkstatus_t *consensus)
{
    const or_options_t *options = get_options();
    ...
    int n_delayed = 0, n_have = 0, n_would_reject = 0, n_wouldnt_use = 0, n_inprogress = 0,
        n_in_oldrouters = 0;
    ...
    SMARTLIST_FOREACH_BEGIN (consensus->routerstatus_list, void *, rsp) {
        ...
        if ((sd = router_get_by_descriptor_digest(rs->descriptor_digest))) {
            ...
            if (!(ri = router_get_by_id_digest(rs->identity_digest)) ||
                tor_memneq(ri->cache_info.signed_descriptor_digest, sd->signed_descriptor_digest,
                           DIGEST_LEN)) {
        ...
        if (is_vote && source) {
            ...
            log_info(LD_DIR, "Learned about %s (%s vs %s) from %s's vote (%s)",
                     routerstatus_describe(rs), time_bufnew, oldrouter ? time_bufold : "none",
    ...
    log_info(LD_DIR,
             ...
             smartlist_len(downloadable), n_delayed, n_have, n_in_oldrouters, n_would_reject,
             n_wouldnt_use, n_inprogress);

    launch_descriptor_downloads(DIR_PURPOSE_FETCH_SERVERDESC, downloadable, source, now);
 ...
 * address has changed. */
static void
launch_dummy_descriptor_download_as_needed(time_t now, const or_options_t *options)
{
    ...
    if (!options->Address && server_mode(options) &&
        ...
        directory_get_from_dirserver(DIR_PURPOSE_FETCH_SERVERDESC, ROUTER_PURPOSE_GENERAL,
                                     "authority.z", PDS_RETRY_IF_NO_SERVERS, DL_WANT_ANY_DIRSERVER);
    }
}
...
update_extrainfo_downloads(time_t now)
{
    ...
    log_info(LD_DIR,
             ...
             n_no_ei, n_have, n_delay, n_pending, smartlist_len(wanted), n_bogus[0], n_bogus[1]);
    ...
    for (i = 0; i < smartlist_len(wanted); i += max_dl_per_req) {
        initiate_descriptor_downloads(NULL, DIR_PURPOSE_FETCH_EXTRAINFO, wanted, i,
                                      i + max_dl_per_req,
...
router_differences_are_cosmetic(const routerinfo_t *r1, const routerinfo_t *r2)
{
    ...
    if (r1->addr != r2->addr || strcasecmp(r1->nickname, r2->nickname) ||
        r1->or_port != r2->or_port || !tor_addr_eq(&r1->ipv6_addr, &r2->ipv6_addr) ||
        ...
        (r1->supports_tunnelled_dir_requests != r2->supports_tunnelled_dir_requests))
        return 0;
    ...
    if ((r1->bandwidthrate != r2->bandwidthrate) || (r1->bandwidthburst != r2->bandwidthburst))
        return 0;
    ...
    if (time_difference > ROUTER_ALLOW_UPTIME_DRIFT && time_difference > r1->uptime * .05 &&
 ...
 **/
int
routerinfo_incompatible_with_extrainfo(const crypto_pk_t *identity_pkey, extrainfo_t *ei,
                                       signed_descriptor_t *sd, const char **msg)
{
    ...
    digest256_matches = tor_memeq(ei->digest256, sd->extra_info_digest256, DIGEST256_LEN);
    digest256_matches |= fast_mem_is_zero(sd->extra_info_digest256, DIGEST256_LEN);

    /* The identity must match exactly to have been generated at the same time
     * by the same router. */
    if (tor_memneq(sd->identity_digest, ei->cache_info.identity_digest, DIGEST_LEN)) {
        if (msg)
    ...
    if (ei->pending_sig) {
        ...
        if (crypto_pk_public_checksig(identity_pkey, signed_digest, sizeof(signed_digest),
                                      ei->pending_sig, ei->pending_sig_len) != DIGEST_LEN ||
            tor_memneq(signed_digest, ei->cache_info.signed_descriptor_digest, DIGEST_LEN)) {
            ei->bad_sig = 1;
...
int
routerstatus_version_supports_extend2_cells(const routerstatus_t *rs, int allow_unknown_versions)
{
    if (!rs) {
...
routerlist_assert_ok(const routerlist_t *rl)
{
    ...
    EIMAP_FOREACH (rl->extra_info_map, d, ei) {
        ...
        if (sd) {
            tor_assert(tor_memeq(ei->cache_info.signed_descriptor_digest, sd->extra_info_digest,
                                 DIGEST_LEN));
        }
    }
    DIGESTMAP_FOREACH_END;
