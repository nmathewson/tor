...
#define SDMAP_FOREACH(map, keyvar, valvar)              \
    DIGESTMAP_FOREACH (sdmap_to_digestmap(map), keyvar, \
                       signed_descriptor_t *, valvar)
...
router_rebuild_store(int flags, desc_store_t *store)
{
    ...
    if (!store->mmap) {
        if (errno == ERANGE) {
            ...
            log_warn(LD_FS, "Unable to mmap new descriptor file at '%s'.",
                     fname);
        }
    }
...
router_reload_router_list_impl(desc_store_t *store)
{
    ...
    if (store->mmap) {
        ...
        if (extrainfo)
            router_load_extrainfo_from_string(
                store->mmap->data, store->mmap->data + store->mmap->size,
    ...
    if (file_status(fname) == FN_FILE)
        contents =
            read_file_to_str(fname, RFTS_BIN | RFTS_IGNORE_MISSING, &st);
    if (contents) {
        if (extrainfo)
                                      ...
                                      int direct_conn)
{
    ...
    SMARTLIST_FOREACH_BEGIN (nodelist_get_list(), const node_t *, node) {
        ...
        if (node->rs &&
            !routerstatus_version_supports_extend2_cells(node->rs, 1))
            continue;
                                ...
                                int with_annotations)
{
    ...
    tor_assert(r);
    if (!with_annotations) {
        if (fast_memcmp("router ", r, 7) &&
            fast_memcmp("extra-info ", r, 11)) {
          ...
          (routerlist_t * rl, extrainfo_t *ei, int warn_if_incompatible))
{
    ...
    signed_descriptor_t *sd = sdmap_get(
        rl->desc_by_eid_map, ei->cache_info.signed_descriptor_digest);
    extrainfo_t *ei_tmp;
    ...
    if (!ri) {
        /* This router is unknown; we can't even verify the signature. Give
         * up.*/
        r = ROUTER_NOT_IN_CONSENSUS;
        goto done;
    }
    if (!sd) {
        /* The extrainfo router doesn't have a known routerdesc to attach it
         * to. This just won't work. */
        ;
    ...
    if (routerinfo_incompatible_with_extrainfo(ri->identity_pkey, ei, sd,
        ...
        base16_encode(d1, sizeof(d1), ri->cache_info.identity_digest,
                      DIGEST_LEN);
        base16_encode(d2, sizeof(d2), ei->cache_info.identity_digest,
                      DIGEST_LEN);
                   ...
                   routerinfo_t *ri_new)
{
    ...
    ri_tmp = rimap_set(rl->identity_map, ri_new->cache_info.identity_digest,
                       ri_new);
...
routerlist_reparse_old(routerlist_t *rl, signed_descriptor_t *sd)
{
    ...
    ri = router_parse_entry_from_string(
        body, body + sd->signed_descriptor_len + sd->annotations_len, 0, 1,
        NULL, NULL);
    if (!ri)
        return NULL;
                         ...
                         int from_cache, int from_fetch)
{
    ...
    if (sdmap_get(routerlist->desc_digest_map,
         ...
         * bridge descriptor, replace it. If we had a bridge descriptor before
         * and the new one is not a bridge descriptor, don't replace it. */
    ...
    if (authdir) {
        ...
        if (!signed_desc_digest_is_recognized(&router->cache_info) &&
            ...
            /* We asked for it, so some networkstatus must have listed it when
             * we
             * did.  Save it if we're a cache in case somebody else asks for
             * it. */
            log_info(
                LD_DIR,
    ...
    if (old_router) {
        if (!in_consensus && (router->cache_info.published_on <=
            ...
            *msg = authdir_believes_valid ? "Valid server updated"
                                          : ("Invalid server updated. (This "
                                             "dirserver is marking your "
...
compare_old_routers_by_identity_(const void **_a, const void **_b)
{
    int i;
    const signed_descriptor_t *r1 = *_a, *r2 = *_b;
    if ((i = fast_memcmp(r1->identity_digest, r2->identity_digest,
                         DIGEST_LEN)))
        return i;
    return (int)(r1->published_on - r2->published_on);
}
                                             ...
                                             int hi, digestset_t *retain)
{
    ...
    for (i = lo; i <= hi; ++i) {
        ...
        if (r->last_listed_as_valid_until >= now ||
            (retain && digestset_probably_contains(
                           retain, r->signed_descriptor_digest))) {
            must_keep[i - lo] = 1;
        }
        if (i < hi) {
        ...
        if (!must_keep[i - lo] && r->published_on < cutoff &&
            n_rmv < n_extra) {
            ++n_rmv;
    ...
    if (n_rmv < n_extra) {
         ...
         * the duration of liveness, and remove the ones we're not already
         *going to remove based on how long they were alive.
         **/
...
routerlist_remove_old_routers(void)
{
    ...
    */
    ...
    /* Iterate through the list from back to front, so when we remove
     * descriptors we don't mess up groups we haven't gotten to. */
    for (i = smartlist_len(routerlist->old_routers) - 1; i >= 0; --i) {
        ...
        if (tor_memneq(cur_id, r->identity_digest, DIGEST_LEN)) {
            routerlist_remove_old_cached_routers_with_id(now, cutoff, i + 1,
                                                         hi, retain);
            cur_id = r->identity_digest;
            hi = i;
        }
    ...
    if (hi >= 0)
        routerlist_remove_old_cached_routers_with_id(now, cutoff, 0, hi,
                                                     retain);
    // routerlist_assert_ok(routerlist);
                                  ...
                                  int descriptor_digests)
{
    ...
    SMARTLIST_FOREACH_BEGIN (extrainfo_list, extrainfo_t *, ei) {
        ...
        was_router_added_t added = router_add_extrainfo_to_routerlist(
            ei, &msg, from_cache, !from_cache);
        if (WRA_WAS_ADDED(added) && requested_fingerprints) {
            ...
            base16_encode(fp, sizeof(fp),
                          ...
                          DIGEST_LEN);
            smartlist_string_remove(requested_fingerprints, fp);
            /* We silently let relays stuff us with extrainfos we didn't ask
             * for, so long as we would have wanted them anyway.  Since we
             * always fetch all the extrainfos we want, and we never actually
             * act on them inside Tor, this should be harmless. */
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn) {
        if (conn->type == CONN_TYPE_DIR && conn->purpose == purpose &&
            ...
            if (!strcmpstart(resource, prefix))
                dir_split_resource_into_fingerprints(resource + p_len, tmp,
                                                     NULL, flags);
        }
                                             ...
                                             networkstatus_t *consensus)
{
    ...
    if (is_vote) {
        ...
        networkstatus_voter_info_t *voter =
            smartlist_get(consensus->voters, 0);
        tor_assert(voter);
    ...
    SMARTLIST_FOREACH_BEGIN (consensus->routerstatus_list, void *, rsp) {
        ...
        if ((sd = router_get_by_descriptor_digest(rs->descriptor_digest))) {
            ...
            if (!(ri = router_get_by_id_digest(rs->identity_digest)) ||
                ...
                /* We have a descriptor with this digest, but either there is
                 * no entry in routerlist with the same ID (!ri), or there is
                 * one, but the identity digest differs (memneq).
                 */
        ...
        if (is_vote && source) {
            ...
            if (oldrouter)
                format_iso_time(time_bufold,
                                oldrouter->cache_info.published_on);
        ...
        SMARTLIST_FOREACH_BEGIN (no_longer_old, signed_descriptor_t *, sd) {
            ...
            if (WRA_WAS_OUTDATED(r)) {
                ...
                if (r == ROUTER_CERTS_EXPIRED) {
                    ...
                    format_iso_time(time_cert_expires,
                                           ...
                                           const or_options_t *options)
{
    ...
    if (!options->Address && server_mode(options) &&
        ...
        directory_get_from_dirserver(
            DIR_PURPOSE_FETCH_SERVERDESC, ROUTER_PURPOSE_GENERAL,
        ...
        for (i = 0; i < smartlist_len(lst); ++i) {
            ...
            if (sd2 != sd) {
                if (sd2 != NULL) {
                    ...
                    base16_encode(d1, sizeof(d1), sd->identity_digest,
                                  DIGEST_LEN);
                    base16_encode(d2, sizeof(d2), sd2->identity_digest,
                                  DIGEST_LEN);
                    base16_encode(d3, sizeof(d3), d, DIGEST_LEN);
                    base16_encode(d4, sizeof(d3), sd2->extra_info_digest,
                                  DIGEST_LEN);

                    log_info(LD_DIR,
                             "Found an entry in %s with mismatched "
                             "router_get_by_extrainfo_digest() value. This "
                             "has ID %s "
                             "but the entry in the map has ID %s. This has EI "
                             "digest "
...
err:
    if (digest_matches) {
        /* This signature was okay, and the digest was right: This is indeed
         * the corresponding extrainfo.  But insanely, it doesn't match the
         * routerinfo that lists it.  Don't try to fetch this one again. */
