...
HT_GENERATE2(microdesc_map, microdesc_t, node, microdesc_hash_, microdesc_eq_, 0.6,
...
microdesc_note_outdated_dirserver(const char *relay_digest)
{
     ...
     * them. */
    if (!networkstatus_get_reasonably_live_consensus(approx_time(), FLAV_MICRODESC)) {
        return;
    }
    ...
    if (BUG(smartlist_len(outdated_dirserver_list) > TOO_MANY_OUTDATED_DIRSERVERS)) {
        microdesc_reset_outdated_dirservers_list();
    }

    /* Turn the binary relay digest to a hex since smartlists have better support
     * for strings than digests. */
    base16_encode(relay_hexdigest, sizeof(relay_hexdigest), relay_digest, DIGEST_LEN);
...
dump_microdescriptor(int fd, microdesc_t *md, size_t *annotation_len_out)
{
    ...
    if (md->last_listed) {
        ...
        if (write_all_to_fd(fd, annotation, strlen(annotation)) < 0) {
            log_warn(LD_DIR, "Couldn't write microdescriptor annotation: %s", strerror(errno));
            return -1;
        }
        r += strlen(annotation);
    ...
    warn_if_nul_found(md->body, md->bodylen, (int64_t)md->off, "dumping a microdescriptor");
    written = write_all_to_fd(fd, md->body, md->bodylen);
    if (written != (ssize_t)md->bodylen) {
        written = written < 0 ? 0 : written;
        log_warn(LD_DIR, "Couldn't dump microdescriptor (wrote %ld out of %lu): %s", (long)written,
                 (unsigned long)md->bodylen, strerror(errno));
        return -1;
    }
    r += md->bodylen;
    return r;
...
smartlist_t *
microdescs_add_to_cache(microdesc_cache_t *cache, const char *s, const char *eos,
                        saved_location_t where, int no_save, time_t listed_at,
                        smartlist_t *requested_digests256)
{
    ...
    descriptors = microdescs_parse_from_string(s, eos, allow_annotations, where, invalid_digests);
    if (listed_at != (time_t)-1) {
        SMARTLIST_FOREACH(descriptors, microdesc_t *, md, md->last_listed = listed_at);
    }
    if (requested_digests256) {
        ...
        SMARTLIST_FOREACH_BEGIN (descriptors, microdesc_t *, md) {
            if (digest256map_get(requested, (const uint8_t *)md->digest)) {
                digest256map_set(requested, (const uint8_t *)md->digest, DIGEST_RECEIVED);
            } else {
 ...
 * not add. */
smartlist_t *
microdescs_add_list_to_cache(microdesc_cache_t *cache, smartlist_t *descriptors,
                             saved_location_t where, int no_save)
{
    ...
    if (where == SAVED_NOWHERE && !no_save) {
        fd = start_writing_to_file(cache->journal_fname, OPEN_FLAGS_APPEND | O_BINARY, 0600,
                                   &open_file);
        if (fd < 0) {
            log_warn(LD_DIR, "Couldn't append to journal in %s: %s", cache->journal_fname,
                     strerror(errno));
        }
    }
    ...
    if (fd >= 0) {
        if (finish_writing_to_file(open_file) < 0) {
            log_warn(LD_DIR, "Error appending to microdescriptor file: %s", strerror(errno));
...
static void
warn_if_nul_found(const char *inp, size_t len, int64_t offset, const char *activity)
{
    const char *nul_found = memchr(inp, 0, len);
    if (BUG(nul_found)) {
        log_warn(LD_BUG,
                 "Found unexpected NUL while %s, offset %" PRId64 "at position %" TOR_PRIuSZ
                 "/%" TOR_PRIuSZ ".",
...
microdesc_cache_reload(microdesc_cache_t *cache)
{
    ...
    if (mm) {
        ...
        added = microdescs_add_to_cache(cache, mm->data, mm->data + mm->size, SAVED_IN_CACHE, 0, -1,
                                        NULL);
        if (added) {
    ...
    if (journal_content) {
        ...
        warn_if_nul_found(journal_content, (size_t)st.st_size, 0, "reading microdesc journal");
        added = microdescs_add_to_cache(cache, journal_content, journal_content + st.st_size,
    ...
    log_info(LD_DIR, "Reloaded microdescriptor cache. Found %d descriptors.", total);
...
microdesc_cache_clean(microdesc_cache_t *cache, time_t cutoff, int force)
{
    ...
    if (!force && !networkstatus_get_reasonably_live_consensus(now, FLAV_MICRODESC))
        return;
        ...
        if (is_old && !held_by_nodes) {
            ...
            if (is_old) {
                ...
                log_warn(LD_BUG,
                         ...
                         (int)((now - (*mdp)->last_listed) / 3600), (int)((now - cutoff) / 3600),
                         held_by_nodes, smartlist_len(nodes), networkstatus_age / 3600, ht_badness);

                SMARTLIST_FOREACH_BEGIN (nodes, const node_t *, node) {
                    const char *rs_match = "No RS";
                    const char *rs_present = "";
                    if (node->rs) {
                        if (tor_memeq(node->rs->descriptor_digest, (*mdp)->digest, DIGEST256_LEN)) {
                    ...
                    log_warn(LD_BUG, "  [%d]: ID=%s. md=%p, rs=%p, ri=%p. %s.%s", node_sl_idx,
                             hex_str(node->identity, DIGEST_LEN), node->md, node->rs, node->ri,
                             rs_match, rs_present);
    ...
    if (dropped) {
        log_info(LD_DIR, "Removed %d/%d microdescriptors as old.", dropped, dropped + kept);
...
microdesc_cache_rebuild(microdesc_cache_t *cache, int force)
{
    ...
    fd = start_writing_to_file(cache->cache_fname, OPEN_FLAGS_REPLACE | O_BINARY, 0600, &open_file);
    if (fd < 0)
        return -1;
    ...
    if (cache->cache_content) {
        ...
        if (res != 0) {
            log_warn(LD_FS, "Failed to unmap old microdescriptor cache while rebuilding");
        }
        cache->cache_content = NULL;
    }

    if (finish_writing_to_file(open_file) < 0) {
        log_warn(LD_DIR, "Error rebuilding microdescriptor cache: %s", strerror(errno));
    ...
    if (!cache->cache_content && smartlist_len(wrote)) {
        log_err(LD_DIR, "Couldn't map file that we just wrote to %s!", cache->cache_fname);
        smartlist_free(wrote);
        return -1;
    }
    SMARTLIST_FOREACH_BEGIN (wrote, microdesc_t *, md) {
        ...
        if (PREDICT_UNLIKELY(md->bodylen < 9 || fast_memneq(md->body, "onion-key", 9) != 0)) {
...
update_microdesc_downloads(time_t now)
{
    ...
    missing = microdesc_list_missing_digest256(consensus, get_microdesc_cache(), 1, pending);
...
update_microdescs_from_networkstatus(time_t now)
{
    ...
    microdesc_t *md;
    networkstatus_t *ns = networkstatus_get_reasonably_live_consensus(now, FLAV_MICRODESC);
