...
microdesc_note_outdated_dirserver(const char *relay_digest)
{
    ...
    /* If we have a reasonably live consensus, then most of our dirservers
     * should still be caching all the microdescriptors in it. Reasonably live
    ...
    /* Turn the binary relay digest to a hex since smartlists have better
     * support for strings than digests. */
    ...
    if (router_get_trusteddirserver_by_digest(relay_digest)) {
        log_info(LD_GENERAL, "Auth %s gave us outdated dirinfo.",
                 relay_hexdigest);
        return;
    }
                        ...
                        time_t listed_at, smartlist_t *requested_digests256)
{
    ...
    descriptors = microdescs_parse_from_string(s, eos, allow_annotations,
    ...
    if (requested_digests256) {
        ...
        /* Set requested[d] to DIGEST_INVALID for every md we requested which
         * we will never be able to parse.  Remove the ones we didn't request
         * from invalid_digests.
         */
        ...
        /* Update requested[d] to 2 for the mds we asked for and got. Delete
         * the ones we never requested from the 'descriptors' smartlist.
         */
        ...
        /* Remove the ones we got or the invalid ones from
                             ...
                             int no_save)
{
    ...
    if (where == SAVED_NOWHERE && !no_save) {
        fd = start_writing_to_file(cache->journal_fname,
                                   OPEN_FLAGS_APPEND | O_BINARY, 0600,
                                   &open_file);
        if (fd < 0) {
    ...
    for (mdp = HT_START(microdesc_map, &cache->map); mdp != NULL;) {
        ...
        if (is_old && !held_by_nodes) {
            ...
            if (is_old) {
                ...
                const int ht_badness =
                    HT_REP_IS_BAD_(microdesc_map, &cache->map);
                if (ns) {
                ...
                log_warn(
                    ...
                    "(last listed %d hours ago vs %d hour cutoff), but is "
                    "still "
                    ...
                    smartlist_len(nodes), networkstatus_age / 3600,
                    ht_badness);
                ...
                SMARTLIST_FOREACH_BEGIN (nodes, const node_t *, node) {
                    ...
                    if (node->rs) {
                        ...
                        if (ns) {
                            ...
                            SMARTLIST_FOREACH(
                                ns->routerstatus_list, routerstatus_t *, rs, {
                                    if (rs == node->rs) {
                                        rs_present =
                    ...
                    log_warn(
                        LD_BUG, "  [%d]: ID=%s. md=%p, rs=%p, ri=%p. %s.%s",
                        node_sl_idx, hex_str(node->identity, DIGEST_LEN),
...
microdesc_cache_rebuild(microdesc_cache_t *cache, int force)
{
    ...
    fd = start_writing_to_file(
        cache->cache_fname, OPEN_FLAGS_REPLACE | O_BINARY, 0600, &open_file);
    if (fd < 0)
        return -1;
...
microdesc_free_(microdesc_t *md, const char *fname, int lineno)
{
    ...
    /* Make sure that the microdesc was really removed from the appropriate
       data structures. */
    if (md->held_in_map) {
        microdesc_cache_t *cache = get_microdesc_cache_noload();
                                 ...
                                 int downloadable_only, digest256map_t *skip)
{
    ...
    SMARTLIST_FOREACH_BEGIN (ns->routerstatus_list, routerstatus_t *, rs) {
        ...
        if (downloadable_only &&
            ...
            continue;
        if (skip &&
            digest256map_get(skip, (const uint8_t *)rs->descriptor_digest))
            continue;
...
update_microdesc_downloads(time_t now)
{
    ...
    missing = microdesc_list_missing_digest256(
        consensus, get_microdesc_cache(), 1, pending);
    digest256map_free(pending, NULL);

    launch_descriptor_downloads(DIR_PURPOSE_FETCH_MICRODESC, missing, NULL,
                                now);
