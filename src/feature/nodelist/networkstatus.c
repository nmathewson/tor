...
static download_status_t consensus_dl_status[N_CONSENSUS_FLAVORS] = {
    {0, 0, 0, DL_SCHED_CONSENSUS, DL_WANT_ANY_DIRSERVER, DL_SCHED_INCREMENT_FAILURE, 0, 0},
    {0, 0, 0, DL_SCHED_CONSENSUS, DL_WANT_ANY_DIRSERVER, DL_SCHED_INCREMENT_FAILURE, 0, 0},
};
...
static download_status_t consensus_bootstrap_dl_status[N_CONSENSUS_BOOTSTRAP_SCHEDULES] = {
    {0, 0, 0, DL_SCHED_CONSENSUS, DL_WANT_AUTHORITY, DL_SCHED_INCREMENT_ATTEMPT, 0, 0},
    /* During bootstrap, DL_WANT_ANY_DIRSERVER means "use fallbacks". */
    {0, 0, 0, DL_SCHED_CONSENSUS, DL_WANT_ANY_DIRSERVER, DL_SCHED_INCREMENT_ATTEMPT, 0, 0},
};
...
static int networkstatus_check_required_protocols(const networkstatus_t *ns, int client_mode,
                                                  char **warning_out);
static int reload_consensus_from_file(const char *fname, const char *flavor, unsigned flags,
...
{
    SMARTLIST_FOREACH(nodelist_get_list(), node_t *, node, node->name_lookup_warned = 0);
...
static tor_mmap_t *
networkstatus_map_cached_consensus_impl(int flav, const char *flavorname, int unverified_consensus)
{
    char *filename = networkstatus_get_cache_fname(flav, flavorname, unverified_consensus);
...
router_reload_consensus_networkstatus(void)
{
    ...
    for (flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
        ...
        reload_consensus_from_file(fname, flavor, flags | NSSET_WAS_WAITING_FOR_CERTS, NULL);
        tor_free(fname);
    }
...
networkstatus_vote_free_(networkstatus_t *ns)
{
    ...
    if (ns->routerstatus_list) {
        if (ns->type == NS_TYPE_VOTE || ns->type == NS_TYPE_OPINION) {
            ...
            SMARTLIST_FOREACH(ns->routerstatus_list, routerstatus_t *, rs, routerstatus_free(rs));
        }
...
networkstatus_get_voter_by_id(networkstatus_t *vote, const char *identity)
{
    ...
    SMARTLIST_FOREACH(vote->voters, networkstatus_voter_info_t *, voter,
                      if (fast_memeq(voter->identity_digest, identity, DIGEST_LEN)) return voter);
    return NULL;
}
...
document_signature_t *
networkstatus_get_voter_sig_by_alg(const networkstatus_voter_info_t *voter, digest_algorithm_t alg)
{
    if (!voter->sigs)
        return NULL;
    SMARTLIST_FOREACH(voter->sigs, document_signature_t *, sig, if (sig->alg == alg) return sig);
    return NULL;
}
...
int
networkstatus_check_document_signature(const networkstatus_t *consensus, document_signature_t *sig,
                                       const authority_cert_t *cert)
{
    ...
    if (tor_memneq(sig->signing_key_digest, key_digest, DIGEST_LEN) ||
        tor_memneq(sig->identity_digest, cert->cache_info.identity_digest, DIGEST_LEN))
        return -1;
    ...
    if (crypto_pk_public_checksig(cert->signing_key, signed_digest, signed_digest_len,
                                  sig->signature, sig->signature_len) < dlen ||
...
networkstatus_check_consensus_signature(networkstatus_t *consensus, int warn)
{
    ...
    SMARTLIST_FOREACH_BEGIN (consensus->voters, networkstatus_voter_info_t *, voter) {
        int good_here = 0;
        int bad_here = 0;
        int unknown_here = 0;
        ...
        SMARTLIST_FOREACH_BEGIN (voter->sigs, document_signature_t *, sig) {
            if (!sig->good_signature && !sig->bad_signature && sig->signature) {
                ...
                int is_v3_auth =
                    trusteddirserver_get_by_v3_auth_digest(sig->identity_digest) != NULL;
                authority_cert_t *cert =
                    authority_cert_get_by_digests(sig->identity_digest, sig->signing_key_digest);
                tor_assert(tor_memeq(sig->identity_digest, voter->identity_digest, DIGEST_LEN));
    ...
    if (warn >= 0) {
        ...
        });
        {
            ...
            smartlist_add_asprintf(sl,
                                   ...
                                   "This %s one has %d (%s).",
                                   n_required, networkstatus_get_flavor_name(consensus->flavor),
                                   n_good, tmp);
            tor_free(tmp);
            if (n_no_signature) {
                tmp = smartlist_join_strings(list_no_signature, " ", 0, NULL);
                smartlist_add_asprintf(sl, "%d (%s) of the authorities we know didn't sign it.",
                                       n_no_signature, tmp);
                tor_free(tmp);
            }
            if (n_unknown) {
 ...
 * digest in the key to the identity digest of a routerstatus_t. */
int
compare_digest_to_vote_routerstatus_entry(const void *_key, const void **_member)
{
    const char *key = _key;
...
{
    return smartlist_bsearch(ns->routerstatus_list, digest, compare_digest_to_routerstatus_entry);
}
...
int
networkstatus_vote_find_entry_idx(networkstatus_t *ns, const char *digest, int *found_out)
{
    return smartlist_bsearch_idx(ns->routerstatus_list, digest,
                                 compare_digest_to_routerstatus_entry, found_out);
}
...
router_get_consensus_status_by_descriptor_digest(networkstatus_t *consensus, const char *digest)
{
    return router_get_mutable_consensus_status_by_descriptor_digest(consensus, digest);
}
...
router_get_descriptor_digests_in_consensus(networkstatus_t *consensus)
{
    ...
    for (i = digestmap_iter_init(consensus->desc_digest_map); !(digestmap_iter_done(i));
...
router_get_dl_status_by_descriptor_digest, (const char *d))
{
    ...
    if (!current_ns_consensus)
        return NULL;
    if ((rs = router_get_mutable_consensus_status_by_descriptor_digest(current_ns_consensus, d)))
        return &rs->dl_status;
...
router_get_mutable_consensus_status_by_id(const char *digest)
{
    ...
    return smartlist_bsearch(rslist, digest, compare_digest_to_routerstatus_entry);
}
...
{
    if (authdir_mode_v3(options) || directory_caches_unknown_auth_certs((options))) {
...
static int
check_consensus_waiting_for_certs(int flavor, time_t now, download_status_t *dls)
{
...
update_consensus_networkstatus_downloads(time_t now)
{
    ...
    const int we_are_bootstrapping = networkstatus_consensus_is_bootstrapping(now);
    const int use_multi_conn = networkstatus_consensus_can_use_multiple_directories(options);
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        ...
        if (we_are_bootstrapping && use_multi_conn) {
            max_in_progress_conns = options->ClientBootstrapConsensusMaxInProgressTries;
        }
        if (connection_dir_count_by_purpose_and_resource(DIR_PURPOSE_FETCH_CONSENSUS, resource) >=
        ...
        if (we_are_bootstrapping && use_multi_conn && i == usable_consensus_flavor()) {
            ...
            directory_get_from_dirserver(DIR_PURPOSE_FETCH_CONSENSUS, ROUTER_PURPOSE_GENERAL,
 ...
 */
static void
update_consensus_bootstrap_attempt_downloads(time_t now, download_status_t *dls,
                                             download_want_authority_t want_authority)
{
    const char *resource = networkstatus_get_flavor_name(usable_consensus_flavor());
    ...
    while (download_status_is_ready(dls, now)) {
        log_info(LD_DIR,
                 ...
                 "download.",
                 resource, (want_authority == DL_WANT_AUTHORITY ? "authority" : "mirror"));

        directory_get_from_dirserver(DIR_PURPOSE_FETCH_CONSENSUS, ROUTER_PURPOSE_GENERAL, resource,
 ...
 */
static void
update_consensus_bootstrap_multiple_downloads(time_t now, const or_options_t *options)
{
    ...
    if (networkstatus_consensus_can_use_extra_fallbacks(options)) {
        ...
        if (!check_consensus_waiting_for_certs(usable_flavor, now, dls_f)) {
            /* During bootstrap, DL_WANT_ANY_DIRSERVER means "use fallbacks". */
            update_consensus_bootstrap_attempt_downloads(now, dls_f, DL_WANT_ANY_DIRSERVER);
        }
    }

    /* Now try an authority. */
    download_status_t *dls_a = &consensus_bootstrap_dl_status[CONSENSUS_BOOTSTRAP_SOURCE_AUTHORITY];

    if (!check_consensus_waiting_for_certs(usable_flavor, now, dls_a)) {
        update_consensus_bootstrap_attempt_downloads(now, dls_a, DL_WANT_AUTHORITY);
    }
}
...
update_consensus_networkstatus_fetch_time_impl(time_t now, int flav)
{
    ...
    if (c && c->valid_after <= now && now <= c->valid_until) {
        ...
        log_debug(LD_DIR,
                  ...
                  (long)c->fresh_until, (long)start, dl_interval, (long)c->valid_until);
        ...
        time_to_download_next_consensus[flav] = start + crypto_rand_int((int)dl_interval);
        {
            ...
            log_info(LD_DIR,
                     ...
                     flavor, (c->fresh_until > now) ? "will be" : "was", tbuf1, tbuf2, tbuf3);
        }
    } else {
        time_to_download_next_consensus[flav] = now;
        log_info(LD_DIR, "No live %s consensus; we should fetch one immediately.", flavor);
    }
}
...
update_certificate_downloads(time_t now)
{
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        if (consensus_waiting_for_certs[i].consensus)
            authority_certs_fetch_missing(consensus_waiting_for_certs[i].consensus, now, NULL);
    }
...
networkstatus_get_dl_status_by_flavor, (consensus_flavor_t flavor))
{
    ...
    const int we_are_bootstrapping = networkstatus_consensus_is_bootstrapping(time(NULL));

    if ((int)flavor <= N_CONSENSUS_FLAVORS) {
        dl =
            &((we_are_bootstrapping ? consensus_bootstrap_dl_status : consensus_dl_status)[flavor]);
    }
 ...
 */
int
networkstatus_consensus_reasonably_live(const networkstatus_t *consensus, time_t now)
{
    if (BUG(!consensus))
        return 0;

    return networkstatus_valid_after_is_reasonably_live(consensus->valid_after, now) &&
           networkstatus_valid_until_is_reasonably_live(consensus->valid_until, now);
}
...
{
    networkstatus_t *consensus = networkstatus_get_latest_consensus_by_flavor(flavor);
...
networkstatus_consensus_is_bootstrapping, (time_t now))
{
    ...
    if (networkstatus_get_reasonably_live_consensus(now, usable_consensus_flavor())) {
        return 0;
    }
...
int
networkstatus_consensus_can_use_multiple_directories(const or_options_t *options)
{
...
networkstatus_consensus_is_already_downloading(const char *resource)
{
    ...
    smartlist_t *fetching_conns = connection_dir_list_by_purpose_resource_and_state(
...
routerstatus_has_visibly_changed(const routerstatus_t *a, const routerstatus_t *b)
{
    ...
    return strcmp(a->nickname, b->nickname) ||
           ...
           a->addr != b->addr || a->or_port != b->or_port || a->dir_port != b->dir_port ||
           a->is_authority != b->is_authority || a->is_exit != b->is_exit ||
           a->is_stable != b->is_stable || a->is_fast != b->is_fast ||
           a->is_flagged_running != b->is_flagged_running || a->is_named != b->is_named ||
           a->is_unnamed != b->is_unnamed || a->is_valid != b->is_valid ||
           a->is_possible_guard != b->is_possible_guard || a->is_bad_exit != b->is_bad_exit ||
           a->is_hs_dir != b->is_hs_dir || a->is_staledesc != b->is_staledesc ||
           a->has_bandwidth != b->has_bandwidth || a->published_on != b->published_on ||
...
static void
notify_control_networkstatus_changed(const networkstatus_t *old_c, const networkstatus_t *new_c)
{
    smartlist_t *changed;
    if (old_c == new_c)
        return;
    ...
    SMARTLIST_FOREACH_JOIN(old_c->routerstatus_list, const routerstatus_t *, rs_old,
...
static void
notify_before_networkstatus_changes(const networkstatus_t *old_c, const networkstatus_t *new_c)
{
...
static void
networkstatus_copy_old_consensus_info(networkstatus_t *new_c, const networkstatus_t *old_c)
{
    if (old_c == new_c)
        return;
                           ...
                           STMT_NIL)
    {
        ...
        if (tor_memeq(rs_old->descriptor_digest, rs_new->descriptor_digest, DIGEST256_LEN)) {
 ...
 */
int
networkstatus_set_current_consensus_from_ns(networkstatus_t *c, const char *flavor)
{
 ...
 **/
static int
reload_consensus_from_file(const char *fname, const char *flavor, unsigned flags,
                           const char *source_dir)
{
    ...
    int rv = networkstatus_set_current_consensus(map->data, map->size, flavor, flags, source_dir);
#ifdef _WIN32
    if (rv < 0 && tor_memstr(map->data, map->size, "\r\n")) {
        ...
        rv = networkstatus_set_current_consensus(content, strlen(content), flavor, flags,
                                                 source_dir);
        tor_free(content);
    }
#endif /* defined(_WIN32) */
    if (rv < -1) {
        log_warn(LD_GENERAL, "Couldn't set consensus from cache file %s", escaped(fname));
    }
    tor_munmap_file(map);
    return rv;
}
...
handle_missing_protocol_warning_impl(const networkstatus_t *c, int is_client)
{
    ...
    int should_exit = networkstatus_check_required_protocols(c, is_client, &protocol_warning);
    if (protocol_warning) {
        tor_log(should_exit ? LOG_ERR : LOG_WARN, LD_GENERAL, "%s", protocol_warning);
    }
    if (should_exit) {
 ...
 * warn. */
static void
handle_missing_protocol_warning(const networkstatus_t *c, const or_options_t *options)
{
 ...
 */
int
networkstatus_set_current_consensus(const char *consensus, size_t consensus_len, const char *flavor,
                                    unsigned flags, const char *source_dir)
{
    ...
    if (flav != usable_consensus_flavor() && !we_want_to_fetch_flavor(options, flav)) {
        /* This consensus is totally boring to us: we won't use it, we didn't want
         * it, and we won't serve it.  Drop it. */
        goto done;
    }

    if (from_cache && !accept_obsolete && c->valid_until < now - OLD_ROUTER_DESC_MAX_AGE) {
    ...
    if (current_digests && tor_memeq(&c->digests, current_digests, sizeof(c->digests))) {
    ...
    if (current_valid_after && c->valid_after <= current_valid_after) {
         ...
         * even if it's great. */
        log_info(LD_DIR, "Got a %s consensus at least as old as the one we have", flavor);
        goto done;
    }
    ...
    if ((r = networkstatus_check_consensus_signature(c, 1)) < 0) {
        if (r == -1) {
            ...
            if (!was_waiting_for_certs) {
                log_info(LD_DIR, "Not enough certificates to check networkstatus consensus");
            }
            if (!current_valid_after || c->valid_after > current_valid_after) {
            ...
            if (was_waiting_for_certs && (r < -1) && from_cache) {
                if (unlink(unverified_fname) != 0) {
                    log_debug(LD_FS, "Failed to unlink %s: %s", unverified_fname, strerror(errno));
                }
            }
            goto done;
        }
    ...
    if (is_usable_flavor) {
        notify_before_networkstatus_changes(networkstatus_get_latest_consensus(), c);
    }
    if (flav == FLAV_NS) {
        if (current_ns_consensus) {
            networkstatus_copy_old_consensus_info(c, current_ns_consensus);
    ...
    if (is_usable_flavor) {
        ...
        circuit_build_times_new_consensus_params(get_circuit_build_times_mutable(), c);
    ...
    if (we_want_to_fetch_flavor(options, flav)) {
        if (dir_server_mode(get_options())) {
            dirserv_set_cached_consensus_networkstatus(consensus, consensus_len, flavor,
                                                       &c->digests, c->digest_sha3_as_signed,
                                                       c->valid_after);
...
networkstatus_note_certs_arrived(const char *source_dir)
{
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        ...
        if (networkstatus_check_consensus_signature(waiting->consensus, 0) >= 0) {
            ...
            reload_consensus_from_file(fname, flavor_name, NSSET_WAS_WAITING_FOR_CERTS, source_dir);
            tor_free(fname);
        }
    }
}
...
routers_update_all_from_networkstatus(time_t now, int dir_version)
{
    ...
    networkstatus_t *consensus = networkstatus_get_reasonably_live_consensus(now, FLAV_NS);
    ...
    SMARTLIST_FOREACH(rl->routers, routerinfo_t *, ri, ri->cache_info.routerlist_index = ri_sl_idx);
    if (rl->old_routers)
    ...
    if (!have_warned_about_old_version) {
        ...
        if (status == VS_RECOMMENDED) {
            ...
            log_info(LD_GENERAL, "The directory authorities don't recommend any versions.");
            ...
            log_warn(LD_GENERAL,
                     ...
                     "authorities. Recommended versions are: %s",
                     VERSION, status == VS_OLD ? "obsolete" : "not recommended", recommended);
 ...
 * <b>routers</b>. */
void
routers_update_status_from_consensus_networkstatus(smartlist_t *routers, int reset_failures)
{
    const or_options_t *options = get_options();
    ...
    SMARTLIST_FOREACH_JOIN(
        ns->routerstatus_list, routerstatus_t *, rs, routers, routerinfo_t *, router,
        tor_memcmp(rs->identity_digest, router->cache_info.identity_digest, DIGEST_LEN), {})
    {
        /* Is it the same descriptor, or only the same identity? */
        if (tor_memeq(router->cache_info.signed_descriptor_digest, rs->descriptor_digest,
                      DIGEST_LEN)) {
...
signed_descs_update_status_from_consensus_networkstatus(smartlist_t *descs)
{
    ...
    SMARTLIST_FOREACH(descs, signed_descriptor_t *, d, {
        const routerstatus_t *rs = digestmap_get(ns->desc_digest_map, d->signed_descriptor_digest);
        if (rs) {
 ...
 */
void
set_routerstatus_from_routerinfo(routerstatus_t *rs, const node_t *node, const routerinfo_t *ri)
{
...
networkstatus_getinfo_by_purpose(const char *purpose_string, time_t now)
{
    ...
    if (purpose == ROUTER_PURPOSE_UNKNOWN) {
        log_info(LD_DIR, "Unrecognized purpose '%s' when listing router statuses.", purpose_string);
        return NULL;
    }
...
static int32_t
get_net_param_from_list(smartlist_t *net_params, const char *param_name, int32_t default_val,
...
networkstatus_get_param,
          (const networkstatus_t *ns, const char *param_name, int32_t default_val, int32_t min_val,
           int32_t max_val))
{
    ...
    return get_net_param_from_list(ns->net_params, param_name, default_val, min_val, max_val);
}
...
networkstatus_get_overridable_param(const networkstatus_t *ns, int32_t torrc_value,
                                    const char *param_name, int32_t default_val, int32_t min_val,
                                    int32_t max_val)
{
    if (torrc_value >= min_val && torrc_value <= max_val)
        return torrc_value;
    else
        return networkstatus_get_param(ns, param_name, default_val, min_val, max_val);
}
...
{
    return networkstatus_get_param(ns, "bwweightscale", BW_WEIGHT_SCALE, BW_MIN_WEIGHT_SCALE,
                                   BW_MAX_WEIGHT_SCALE);
}
...
int32_t
networkstatus_get_bw_weight(networkstatus_t *ns, const char *weight_name, int32_t default_val)
{
    int32_t param;
    int max;
    if (!ns) /* if they pass in null, go find it ourselves */
    ...
    param = get_net_param_from_list(ns->weight_params, weight_name, default_val, -1,
                                    BW_MAX_WEIGHT_SCALE);
    if (param > max) {
        log_warn(LD_DIR,
...
int
getinfo_helper_networkstatus(control_connection_t *conn, const char *question, char **answer,
                             const char **errmsg)
{
...
int
networkstatus_check_required_protocols(const networkstatus_t *ns, int client_mode,
                                       char **warning_out)
{
    ...
    const bool consensus_postdates_this_release = ns->valid_after >= tor_get_approx_release_date();
 ...
 * previous interval. */
time_t
voting_sched_get_start_of_interval_after(time_t now, int interval, int offset)
{
    struct tm tm;
