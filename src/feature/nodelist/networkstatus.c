...
networkstatus_vote_free_(networkstatus_t *ns)
{
    ...
    if (ns->voters) {
        SMARTLIST_FOREACH_BEGIN (ns->voters, networkstatus_voter_info_t *,
                                 voter) {
...
networkstatus_check_consensus_signature(networkstatus_t *consensus, int warn)
{
    ...
    SMARTLIST_FOREACH_BEGIN (consensus->voters, networkstatus_voter_info_t *,
        ...
        SMARTLIST_FOREACH_BEGIN (voter->sigs, document_signature_t *, sig) {
            if (!sig->good_signature && !sig->bad_signature &&
                ...
                tor_assert(tor_memeq(sig->identity_digest,
                ...
                if (!is_v3_auth) {
                    ...
                    ++missing_key_here;
                    if (authority_cert_dl_looks_uncertain(
                            sig->identity_digest))
                        ++dl_failed_key_here;
                    continue;
                }
                if (networkstatus_check_document_signature(consensus, sig,
                                                           cert) < 0) {
                    smartlist_add(need_certs_from, voter);
                    ++missing_key_here;
                    if (authority_cert_dl_looks_uncertain(
    ...
    if (warn > 1 || (warn >= 0 && (n_good + n_missing_key - n_dl_failed_key <
                                   n_required))) {
        severity = LOG_WARN;
    } else {
        severity = LOG_INFO;
    ...
    if (warn >= 0) {
        ...
        SMARTLIST_FOREACH(
            need_certs_from, networkstatus_voter_info_t *, voter, {
                tor_log(
                    severity, LD_DIR,
    ...
    if (waiting->consensus) {
        ...
        if (waiting->set_at + DELAY_WHILE_FETCHING_CERTS > now &&
            ...
            if (!waiting->dl_failed) {
                if (waiting->set_at + MIN_DELAY_FOR_FETCH_CERT_STATUS_FAILURE >
                    now) {
...
update_consensus_networkstatus_downloads(time_t now)
{
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        ...
        if (we_are_bootstrapping && use_multi_conn &&
            ...
            if (check_consensus_waiting_for_certs(i, now,
                                              ...
                                              const or_options_t *options)
{
     ...
     * we'll fail both the authority and fallback schedules. This is better
     * than failing only one of the schedules, and having the other continue
     * unchecked.
     */
    ...
    if (networkstatus_consensus_can_use_extra_fallbacks(options)) {
        ...
        if (!check_consensus_waiting_for_certs(usable_flavor, now, dls_f)) {
             ...
             */
            update_consensus_bootstrap_attempt_downloads(
                now, dls_f, DL_WANT_ANY_DIRSERVER);
        }
    }
    ...
    if (c && c->valid_after <= now && now <= c->valid_until) {
        ...
        if (dirclient_fetches_dir_info_early(options)) {
            ...
            if (dirclient_fetches_dir_info_later(options)) {
                /* Give all the *clients* enough time to download the
                 * consensus. */
                start = (time_t)(start + dl_interval + min_sec_before_caching);
                /* But try to get it before ours actually expires. */
                dl_interval =
                    (c->valid_until - start) - min_sec_before_caching;
            }
        }
...
should_delay_dir_fetches(const or_options_t *options, const char **msg_out)
{
    ...
    if (options->UseBridges) {
        /* If we know that none of our bridges can possibly work, avoid
         * fetching directory documents. But if some of them might work, try
         * again. */
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        if (consensus_waiting_for_certs[i].consensus)
            authority_certs_fetch_missing(
                consensus_waiting_for_certs[i].consensus, now, NULL);
    }
                                 ...
                                 const routerstatus_t *b)
{
    ...
    return strcmp(a->nickname, b->nickname) ||
           fast_memneq(a->descriptor_digest, b->descriptor_digest,
                       DIGEST_LEN) ||
                                     ...
                                     const networkstatus_t *new_c)
{
    ...
    SMARTLIST_FOREACH_JOIN(old_c->routerstatus_list, const routerstatus_t *,
                           rs_old, new_c->routerstatus_list,
                           const routerstatus_t *, rs_new,
...
handle_missing_protocol_warning_impl(const networkstatus_t *c, int is_client)
{
    ...
    int should_exit = networkstatus_check_required_protocols(
                                    ...
                                    unsigned flags, const char *source_dir)
{
    ...
    if (from_cache && !was_waiting_for_certs) {
        /* We previously stored this; check _now_ to make sure that
         * version-kills really work. This happens even before we check
         * signatures: we did so before when we stored this to disk. This does
         * mean an attacker who can write to the datadir can make us not start:
         * such an attacker could already harm us by replacing our guards,
    ...
    if (flav != usable_consensus_flavor() &&
        ...
        /* This consensus is totally boring to us: we won't use it, we didn't
         * want it, and we won't serve it.  Drop it. */
        goto done;
    }
    ...
    if ((r = networkstatus_check_consensus_signature(c, 1)) < 0) {
        if (r == -1) {
            ...
            if (!was_waiting_for_certs) {
                log_info(LD_DIR, "Not enough certificates to check "
            ...
            if (!current_valid_after || c->valid_after > current_valid_after) {
                ...
                waiting->dl_failed = 0;
                if (!from_cache) {
                    write_bytes_to_file(unverified_fname, consensus,
                                        consensus_len, 1);
                }
                if (dl_certs)
                    authority_certs_fetch_missing(c, now, source_dir);
                /* This case is not a success or a failure until we get the
                 * certs or fail to get the certs. */
                result = 0;
            } else {
                ...
                if (was_waiting_for_certs && from_cache)
                    if (unlink(unverified_fname) != 0) {
                        log_debug(LD_FS, "Failed to unlink %s: %s",
    ...
    /* Before we switch to the new consensus, notify that we are about to
     * change it using the old consensus and the new one. */
    if (is_usable_flavor) {
    ...
    if (flav == FLAV_NS) {
        if (current_ns_consensus) {
            networkstatus_copy_old_consensus_info(c, current_ns_consensus);
            networkstatus_vote_free(current_ns_consensus);
            /* Defensive programming : we should set current_ns_consensus very
             * soon but we're about to call some stuff in the meantime, and
             * leaving this dangling pointer around has proven to be trouble.
             */
    ...
    if (is_usable_flavor) {
        /* Notify that we just changed the consensus so the current global
         * value can be looked at. */
        notify_after_networkstatus_changes();

        /* The "current" consensus has just been set and it is a usable flavor
         * so the first thing we need to do is recalculate the voting schedule
         * static object so we can use the timings in there needed by some
         * subsystems such as hidden service and shared random. */
...
networkstatus_note_certs_arrived(const char *source_dir)
{
    ...
    for (i = 0; i < N_CONSENSUS_FLAVORS; ++i) {
        ...
        if (!waiting->consensus)
            continue;
        if (networkstatus_check_consensus_signature(waiting->consensus, 0) >=
            0) {
            char *fname = networkstatus_get_cache_fname(i, flavor_name, 1);
            reload_consensus_from_file(
                fname, flavor_name, NSSET_WAS_WAITING_FOR_CERTS, source_dir);
            tor_free(fname);
        }
    }
}
...
routers_update_all_from_networkstatus(time_t now, int dir_version)
{
    ...
    if (!have_warned_about_old_version) {
        ...
        const char *recommended = is_server ? consensus->server_versions
                                                   ...
                                                   int reset_failures)
{
                           ...
                           {})
    {
        ...
        if (tor_memeq(router->cache_info.signed_descriptor_digest,
            ...
            if (ns->valid_until >
                router->cache_info.last_listed_as_valid_until)
                router->cache_info.last_listed_as_valid_until =
                    ns->valid_until;
        }
        ...
        if (authdir) {
            ...
            routerinfo_t *old_router = router_get_mutable_by_digest(
                router->cache_info.identity_digest);
            if (old_router != router) {
                router->needs_retest_if_added =
                    dirserv_should_launch_reachability_test(router,
                                                            old_router);
            }
        }
        if (reset_failures) {
                        ...
                        int32_t default_val, int32_t min_val, int32_t max_val)
{
    ...
    SMARTLIST_FOREACH_BEGIN (net_params, const char *, p) {
        if (!strcmpstart(p, param_name) && p[name_len] == '=') {
            int ok = 0;
            long v = tor_parse_long(p + name_len + 1, 10, INT32_MIN, INT32_MAX,
                                    &ok, NULL);
            if (ok) {
                res = (int32_t)v;
                break;
                            ...
                            int32_t default_val)
{
    ...
    param = get_net_param_from_list(ns->weight_params, weight_name,
                                    default_val, -1, BW_MAX_WEIGHT_SCALE);
    if (param > max) {
        log_warn(LD_DIR,
