...
static int compute_weighted_bandwidths(const smartlist_t *sl, bandwidth_weight_rule_t rule,
                                       double **bandwidths_out, double *total_bandwidth_out);
static const routerstatus_t *router_pick_trusteddirserver_impl(const smartlist_t *sourcelist,
                                                               dirinfo_type_t auth, int flags,
                                                               int *n_busy_out);
static const routerstatus_t *router_pick_dirserver_generic(smartlist_t *sourcelist,
...
router_pick_directory_server(dirinfo_type_t type, int flags)
{
    ...
    if (busy) {
        ...
        tor_assert((flags & (PDS_NO_EXISTING_SERVERDESC_FETCH | PDS_NO_EXISTING_MICRODESC_FETCH)));
        return NULL;
    }
 ...
 */
const routerstatus_t *
router_pick_dirserver_generic(smartlist_t *sourcelist, dirinfo_type_t type, int flags)
{
    const routerstatus_t *choice;
    int busy = 0;
...
#define RETRY_ALTERNATE_IP_VERSION(retry_label)                                       \
    STMT_BEGIN                                                                        \
        if (result == NULL && try_ip_pref && options->ClientUseIPv4 &&                \
            fascist_firewall_use_ipv6(options) && !server_mode(options) && !n_busy) { \
            n_excluded = 0;                                                           \
            n_busy = 0;                                                               \
            try_ip_pref = 0;                                                          \
            goto retry_label;                                                         \
...
#define RETRY_WITHOUT_EXCLUDE(retry_label)                                                       \
    STMT_BEGIN                                                                                   \
        if (result == NULL && try_excluding && !options->StrictNodes && n_excluded && !n_busy) { \
            try_excluding = 0;                                                                   \
            n_excluded = 0;                                                                      \
            n_busy = 0;                                                                          \
            try_ip_pref = 1;                                                                     \
...
router_picked_poor_directory_log(const routerstatus_t *rs)
{
    ...
    if (!rs) {
        ...
        log_info(LD_DIR,
                 ...
                 routerstatus_describe(rs), fmt_addr32(rs->addr), rs->or_port, rs->dir_port,
                 fmt_addr(&rs->ipv6_addr), rs->ipv6_orport, rs->dir_port);
    }
}
 ...
 */
STATIC int
router_is_already_dir_fetching(const tor_addr_port_t *ap, int serverdesc, int microdesc)
{
     ...
     */
    if (serverdesc && (connection_get_by_type_addr_port_purpose(CONN_TYPE_DIR, &ap->addr, ap->port,
                                                                DIR_PURPOSE_FETCH_SERVERDESC) ||
                       connection_get_by_type_addr_port_purpose(CONN_TYPE_DIR, &ap->addr, ap->port,
                                                                DIR_PURPOSE_FETCH_EXTRAINFO))) {
        return 1;
    }

    if (microdesc && (connection_get_by_type_addr_port_purpose(CONN_TYPE_DIR, &ap->addr, ap->port,
 ...
 */
static int
router_is_already_dir_fetching_(uint32_t ipv4_addr, const tor_addr_t *ipv6_addr, uint16_t dir_port,
...
router_pick_directory_server_impl(dirinfo_type_t type, int flags, int *n_busy_out)
{
...
retry_search:
    ...
    SMARTLIST_FOREACH_BEGIN (nodelist_get_list(), const node_t *, node) {
        ...
        if (try_excluding &&
            routerset_contains_routerstatus(options->ExcludeNodes, status, country)) {
            ++n_excluded;
            continue;
        }

        if (router_is_already_dir_fetching_(status->addr, &status->ipv6_addr, status->dir_port,
        ...
        if (!fascistfirewall || skip_or_fw ||
            fascist_firewall_allows_node(node, FIREWALL_OR_CONNECTION, try_ip_pref))
            smartlist_add(is_trusted ? trusted_tunnel : is_overloaded ? overloaded_tunnel : tunnel,
                          (void *)node);
        else if (!must_have_or && (skip_dir_fw || fascist_firewall_allows_node(
                                                      node, FIREWALL_DIR_CONNECTION, try_ip_pref)))
            smartlist_add(is_trusted ? trusted_direct : is_overloaded ? overloaded_direct : direct,
                          (void *)node);
...
STATIC void
scale_array_elements_to_u64(uint64_t *entries_out, const double *entries_in, int n_entries,
                            uint64_t *total_out)
{
    double total = 0.0;
...
choose_array_element_by_weight(const uint64_t *entries, int n_entries)
{
    ...
    return select_array_member_cumulative_timei(entries, n_entries, total, rand_val);
}
 ...
 */
static const node_t *
smartlist_choose_node_by_bandwidth_weights(const smartlist_t *sl, bandwidth_weight_rule_t rule)
{
    ...
    scale_array_elements_to_u64(bandwidths_u64, bandwidths_dbl, smartlist_len(sl), NULL);

    {
        int idx = choose_array_element_by_weight(bandwidths_u64, smartlist_len(sl));
...
log_buggy_rs_source(const routerstatus_t *rs)
{
    ...
    if ((m = rate_limit_log(&buggy_rs_ratelim, approx_time()))) {
        log_warn(LD_BUG,
                 ...
                 rs, rs->has_guardfraction, rs->guardfraction_percentage, rs->is_possible_guard, m);
        tor_free(m);
        networkstatus_t *ns;
        int in_ns_count = 0;
...
static int
compute_weighted_bandwidths(const smartlist_t *sl, bandwidth_weight_rule_t rule,
                            double **bandwidths_out, double *total_bandwidth_out)
{
    ...
    tor_assert(rule == NO_WEIGHTING || rule == WEIGHT_FOR_EXIT || rule == WEIGHT_FOR_GUARD ||
               rule == WEIGHT_FOR_MID || rule == WEIGHT_FOR_DIR);
    ...
    *bandwidths_out = NULL;
    ...
    if (Wg < 0 || Wm < 0 || We < 0 || Wd < 0 || Wgb < 0 || Wmb < 0 || Wdb < 0 || Web < 0) {
        log_debug(LD_CIRC, "Got negative bandwidth weights. Defaulting to naive selection"
                           " algorithm.");
        Wg = Wm = We = Wd = weight_scale;
        Wgb = Wmb = Web = Wdb = weight_scale;
    }
        ...
        if (node->rs) {
            if (!node->rs->has_bandwidth) {
                ...
                if (!warned_missing_bw) {
                    log_warn(LD_BUG, "Consensus is missing some bandwidths. Using a naive "
    ...
    log_debug(LD_CIRC,
              ...
              bandwidth_weight_rule_to_string(rule), Wg, Wm, We, Wd, total_bandwidth);
...
double
frac_nodes_with_descriptors(const smartlist_t *sl, bandwidth_weight_rule_t rule,
                            int for_direct_conn)
{
    double *bandwidths = NULL;
    ...
    if (compute_weighted_bandwidths(sl, rule, &bandwidths, &total) < 0 || total <= 0.0) {
        int n_with_descs = 0;
...
const node_t *
node_sl_choose_by_bandwidth(const smartlist_t *sl, bandwidth_weight_rule_t rule)
{ /*XXXX MOVE */
...
nodelist_subtract(smartlist_t *sl, const smartlist_t *excluded)
{
    ...
    SMARTLIST_FOREACH_BEGIN (excluded, const node_t *, node) {
        ...
        if (BUG(idx < 0) || BUG(idx >= nodelist_len) || BUG(node != smartlist_get(nodelist, idx))) {
            goto internal_error;
        }
 ...
 */
const node_t *
router_choose_random_node(smartlist_t *excludedsmartlist, routerset_t *excludedset,
...
{ /* XXXX MOVE */
    ...
    rule = weight_for_exit ? WEIGHT_FOR_EXIT : (need_guard ? WEIGHT_FOR_GUARD : WEIGHT_FOR_MID);
    ...
    router_add_running_nodes_to_smartlist(sl, need_uptime, need_capacity, need_guard, need_desc,
    ...
    if (excludedset) {
        ...
        log_debug(LD_CIRC, "We removed excludedset, leaving %d nodes.", smartlist_len(sl));
    }
    ...
    if (!choice && (need_uptime || need_capacity || need_guard || pref_addr)) {
        ...
        log_info(LD_CIRC,
                 ...
                 need_guard ? ", guard" : "");
        flags &= ~(CRN_NEED_UPTIME | CRN_NEED_CAPACITY | CRN_NEED_GUARD | CRN_PREF_ADDR);
        choice = router_choose_random_node(excludedsmartlist, excludedset, flags);
    }
    smartlist_free(excludednodes);
    if (!choice) {
        log_warn(LD_CIRC, "No available nodes when trying to choose node. Failing.");
    }
    return choice;
}
...
{
    return router_pick_dirserver_generic(router_get_trusted_dir_servers_mutable(), type, flags);
}
...
{
    return router_pick_dirserver_generic(router_get_fallback_dir_servers_mutable(), type, flags);
}
 ...
 */
static const routerstatus_t *
router_pick_trusteddirserver_impl(const smartlist_t *sourcelist, dirinfo_type_t type, int flags,
                                  int *n_busy_out)
{
    const or_options_t *options = get_options();
    smartlist_t *direct, *tunnel;
    ...
    const double auth_weight =
        (sourcelist == router_get_fallback_dir_servers()) ? options->DirAuthorityFallbackRate : 1.0;
...
retry_search:
    ...
    SMARTLIST_FOREACH_BEGIN (sourcelist, const dir_server_t *, d) {
        ...
        if (requireother && me && router_digest_is_me(d->digest))
            continue;
        if (try_excluding &&
            routerset_contains_routerstatus(options->ExcludeNodes, &d->fake_status, -1)) {
            ++n_excluded;
            continue;
        }
         ...
         */
        if (!fascistfirewall || skip_or_fw ||
            fascist_firewall_allows_dir_server(d, FIREWALL_OR_CONNECTION, try_ip_pref))
            smartlist_add(is_overloaded ? overloaded_tunnel : tunnel, (void *)d);
        else if (!must_have_or && (skip_dir_fw || fascist_firewall_allows_dir_server(
    ...
    {
        const dir_server_t *selection = dirserver_choose_by_weight(pick_from, auth_weight);
