                               ...
                               int microdesc)
{
     ...
     */
    if (serverdesc && (connection_get_by_type_addr_port_purpose(
                           CONN_TYPE_DIR, &ap->addr, ap->port,
    ...
    if (microdesc && (connection_get_by_type_addr_port_purpose(
                         CONN_TYPE_DIR, &ap->addr, ap->port,
                                  ...
                                  int *n_busy_out)
{
    ...
    const int no_microdesc_fetching =
        (flags & PDS_NO_EXISTING_MICRODESC_FETCH);
...
retry_search:
    ...
    SMARTLIST_FOREACH_BEGIN (nodelist_get_list(), const node_t *, node) {
        ...
        if (try_excluding && routerset_contains_routerstatus(
                                 options->ExcludeNodes, status, country)) {
            ++n_excluded;
            continue;
        }
    ...
    if (smartlist_len(tunnel)) {
        ...
        result =
            node_sl_choose_by_bandwidth(overloaded_tunnel, WEIGHT_FOR_DIR);
        ...
        result =
            node_sl_choose_by_bandwidth(overloaded_direct, WEIGHT_FOR_DIR);
    } else {
...
log_buggy_rs_source(const routerstatus_t *rs)
{
    ...
    if ((m = rate_limit_log(&buggy_rs_ratelim, approx_time()))) {
        ...
        if ((ns = networkstatus_get_latest_consensus_by_flavor(
                 FLAV_MICRODESC))) {
            int pos = smartlist_pos(ns->routerstatus_list, rs);
            if (pos >= 0) {
                ++in_ns_count;
                            ...
                            double *total_bandwidth_out)
{
    ...
    *bandwidths_out = NULL;
    ...
    if (rule == WEIGHT_FOR_GUARD) {
        ...
        Wm = networkstatus_get_bw_weight(NULL, "Wem",
                                         -1); /* Odd exit policies */
        Wd = networkstatus_get_bw_weight(NULL, "Wed", -1);
        Wg = networkstatus_get_bw_weight(NULL, "Weg",
                                         -1); /* Odd exit policies */
    ...
    if (Wg < 0 || Wm < 0 || We < 0 || Wd < 0 || Wgb < 0 || Wmb < 0 ||
        Wdb < 0 || Web < 0) {
        log_debug(
            LD_CIRC,
    ...
    SMARTLIST_FOREACH_BEGIN (sl, const node_t *, node) {
        ...
        if (node->rs) {
            if (!node->rs->has_bandwidth) {
                /* This should never happen, unless all the authorities
                 * downgrade
                 * to 0.2.0 or rogue routerstatuses get inserted into our
                 * consensus. */
        ...
        /* These should be impossible; but overflows here would be bad, so
         * let's make sure. */
        if (this_bw < 0)
            this_bw = 0;
        if (weight < 0.0)
         ...
         *    measured bandwidth of N in the consensus.  Then instead of
         * choosing N for position p proportionally to Wpf*B or Wpn*B, clients
         * should choose N proportionally to F*Wpf*B + (1-F)*Wpn*B.
         */
        if (node->rs && node->rs->has_guardfraction &&
            ...
            log_debug(LD_GENERAL,
                      ...
                      bandwidth_weight_rule_to_string(rule));
        } else { /* no guardfraction information. calculate the weight
                    normally. */
...
routerlist_add_node_and_family(smartlist_t *sl, const routerinfo_t *router)
{
    ...
    if (node == NULL) {
        ...
        memcpy(fake_node.identity, router->cache_info.identity_digest,
               DIGEST_LEN);
        node = &fake_node;
    }
...
nodelist_subtract(smartlist_t *sl, const smartlist_t *excluded)
{
     ...
     * node before we use it.  If we fail, we fall back to
     * smartlist_subtract().
     */
...
{ /* XXXX MOVE */
    ...
    SMARTLIST_FOREACH_BEGIN (node_list, const node_t *, node) {
        if (node_allows_single_hop_exits(node)) {
            ...
            /* Exclude relays that do not support to rendezvous for a hidden
             * service version 3. */
            smartlist_add(excludednodes, (node_t *)node);
        }
    } SMARTLIST_FOREACH_END (node);
    ...
    if (!choice && (need_uptime || need_capacity || need_guard || pref_addr)) {
        ...
        flags &= ~(CRN_NEED_UPTIME | CRN_NEED_CAPACITY | CRN_NEED_GUARD |
                   CRN_PREF_ADDR);
        choice =
            router_choose_random_node(excludedsmartlist, excludedset, flags);
    }
...
retry_search:
    ...
    SMARTLIST_FOREACH_BEGIN (sourcelist, const dir_server_t *, d) {
        ...
        if (router_is_already_dir_fetching_(
                d->addr, &d->ipv6_addr, d->dir_port, no_serverdesc_fetching,
