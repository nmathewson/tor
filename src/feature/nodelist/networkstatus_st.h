...
struct networkstatus_t {
    ...
    unsigned int
        has_measured_bws : 1; /**< True iff this networkstatus contains
                               * measured= bandwidth values. */

    time_t published; /**< Vote only: Time when vote was written. */
    time_t
        valid_after; /**< Time after which this vote or consensus applies. */
    ...
    /** List of flags that this vote/consensus applies to routers.  If a flag
     * is not listed here, the voter has no opinion on what its value should
     * be. */
