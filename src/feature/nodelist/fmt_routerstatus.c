                          ...
                          const vote_routerstatus_t *vrs)
{
    ...
    smartlist_add_asprintf(
        ...
        rs->is_bad_exit ? " BadExit" : "", rs->is_exit ? " Exit" : "",
        rs->is_fast ? " Fast" : "", rs->is_possible_guard ? " Guard" : "",
        rs->is_hs_dir ? " HSDir" : "",
        rs->is_flagged_running ? " Running" : "",
        rs->is_stable ? " Stable" : "", rs->is_staledesc ? " StaleDesc" : "",
        rs->is_v2_dir ? " V2Dir" : "", rs->is_valid ? " Valid" : "");
...
#define V_LINE_OVERHEAD 7
    ...
    if (format != NS_V2) {
        const routerinfo_t *desc =
            router_get_by_id_digest(rs->identity_digest);
        uint32_t bw_kb;
        ...
        if (format != NS_CONTROL_PORT) {
            ...
            if (!desc) {
                ...
                base16_encode(dd, sizeof(dd), rs->descriptor_digest,
                              DIGEST_LEN);
                log_warn(LD_BUG,
            ...
            if (tor_memneq(desc->cache_info.signed_descriptor_digest,
                ...
                base16_encode(rl_d, sizeof(rl_d),
                              desc->cache_info.signed_descriptor_digest,
                              DIGEST_LEN);
                base16_encode(rs_d, sizeof(rs_d), rs->descriptor_digest,
                              DIGEST_LEN);
                base16_encode(id, sizeof(id), rs->identity_digest, DIGEST_LEN);
                log_err(LD_BUG,
        ...
        if (format == NS_V3_VOTE && vrs && vrs->has_measured_bw) {
            smartlist_add_asprintf(chunks, " Measured=%d",
                                   vrs->measured_bw_kb);
        }
        ...
        if (format == NS_V3_VOTE && vrs) {
            if (fast_mem_is_zero((char *)vrs->ed25519_id,
                                 ED25519_PUBKEY_LEN)) {
