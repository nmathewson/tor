                        ...
                        const tor_addr_t *addr, uint32_t addr32h)
{
    ...
    if (!id_digest) {
        /* strlcpy() returns the length of the source string it attempted to
         * copy, ignoring any required truncation due to the buffer length. */
...
node_describe(const node_t *node)
{
    ...
    if (node->rs) {
         ...
         * microdescs. This code can be removed when 0.2.9 is no longer
         * supported, and the MIN_METHOD_FOR_NO_A_LINES_IN_MICRODESC macro is
         * removed. */
