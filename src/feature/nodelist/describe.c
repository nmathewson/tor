...
router_describe(const routerinfo_t *ri)
{
    ...
    return format_node_description(buf, ri->cache_info.identity_digest, ri->nickname,
                                   &ri->ipv6_addr, ri->addr);
}
...
node_describe(const node_t *node)
{
    ...
    return format_node_description(buf, node->identity, nickname, ipv6_addr, addr32h);
}
...
routerstatus_describe(const routerstatus_t *rs)
{
    ...
    return format_node_description(buf, rs->identity_digest, rs->nickname, &rs->ipv6_addr,
                                   rs->addr);
}
...
extend_info_describe(const extend_info_t *ei)
{
    ...
    return format_node_description(buf, ei->identity_digest, ei->nickname, &ei->addr, 0);
}
...
router_get_verbose_nickname(char *buf, const routerinfo_t *router)
{
...

    {
        ...
        base16_encode(hex_digest, sizeof(hex_digest), router->cache_info.identity_digest,
                      DIGEST_LEN);
