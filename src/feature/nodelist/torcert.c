                   ...
                   time_t now, time_t lifetime, uint32_t flags)
{
    ...
    tor_assert(
        cert); // Unlike Tor's, Trunnel's "new" functions can return NULL.
                  ...
                  time_t now)
{
    ...
    if (ed25519_checksig_batch(&okay, &checkable, 1) < 0) {
        ...
        cert->sig_ok = 1;
        /* Only copy the checkable public key when it is different from the
         * signing key of the certificate to avoid undefined behavior. */
                          ...
                          tor_tls_t *tls, time_t now)
{
    ...
    if (certs->started_here) {
        ...
        if (!tor_tls_cert_is_valid(LOG_PROTOCOL_WARN, auth_cert, id_cert, now,
                                   1))
            ERR("The authentication certificate was not valid");
        if (!tor_tls_cert_is_valid(LOG_PROTOCOL_WARN, id_cert, id_cert, now,
                                   1))
            ERR("The ID certificate was not valid");
    }
...
#define ADDCERT(cert, pk)                                               \
    ...
    if (certs->started_here) {
            ...
            ERR("No Ed25519 link key");
        {
            ...
            if (BUG(!peer_cert)) {
                /* This is a bug, because if we got to this point, we are a
                 * connection that was initiated here, and we completed a TLS
                 * handshake. The other side *must* have given us a
                 * certificate! */
    ...
    if (rsa_ed25519_crosscert_check(
            certs->ed_rsa_crosscert, certs->ed_rsa_crosscert_len, rsa_id_key,
