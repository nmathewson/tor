 ...
 */
static tor_cert_t *
tor_cert_sign_impl(const ed25519_keypair_t *signing_key, uint8_t cert_type, uint8_t signed_key_type,
                   const uint8_t signed_key_info[32], time_t now, time_t lifetime, uint32_t flags)
{
...
tor_cert_create(const ed25519_keypair_t *signing_key, uint8_t cert_type,
                const ed25519_public_key_t *signed_key, time_t now, time_t lifetime, uint32_t flags)
{
    return tor_cert_sign_impl(signing_key, cert_type, SIGNED_KEY_TYPE_ED25519, signed_key->pubkey,
                              now, lifetime, flags);
}
 ...
 */
int
tor_cert_get_checkable_sig(ed25519_checkable_t *checkable_out, const tor_cert_t *cert,
                           const ed25519_public_key_t *pubkey, time_t *expiration_out)
{
    if (!pubkey) {
        if (cert->signing_key_included)
    ...
    memcpy(checkable_out->signature.sig, cert->encoded + signed_len, ED25519_SIG_LEN);
 ...
 */
int
tor_cert_checksig(tor_cert_t *cert, const ed25519_public_key_t *pubkey, time_t now)
{
...
ssize_t
tor_make_rsa_ed25519_crosscert(const ed25519_public_key_t *ed_key, const crypto_pk_t *rsa_key,
                               time_t expires, uint8_t **cert)
{
    ...
    crypto_digest_add_bytes(d, RSA_ED_CROSSCERT_PREFIX, strlen(RSA_ED_CROSSCERT_PREFIX));
    ...
    int siglen =
        crypto_pk_private_sign(rsa_key, (char *)rsa_ed_crosscert_getarray_sig(cc),
...
rsa_ed25519_crosscert_check,
          (const uint8_t *crosscert, const size_t crosscert_len, const crypto_pk_t *rsa_id_key,
           const ed25519_public_key_t *master_key, const time_t reject_if_expired_before))
{
...
#define ERR(code, s)                                                                              \
    do {                                                                                          \
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Received a bad RSA->Ed25519 crosscert: %s", (s)); \
        rv = (code);                                                                              \
        goto err;                                                                                 \
    } while (0)
    ...
    if (tor_memneq(rsa_ed_crosscert_getarray_ed_key(cc), master_key->pubkey, ED25519_PUBKEY_LEN)) {
    ...
    if (reject_if_expired_before < 0 || expiration_time < (uint64_t)reject_if_expired_before) {
    ...
    int signed_len = crypto_pk_public_checksig(rsa_id_key, (char *)signed_, sizeof(signed_),
                                               (char *)sig, siglen);
...
int
or_handshake_certs_rsa_ok(int severity, or_handshake_certs_t *certs, tor_tls_t *tls, time_t now)
{
 ...
 * <b>severity</b> */
int
or_handshake_certs_ed25519_ok(int severity, or_handshake_certs_t *certs, tor_tls_t *tls, time_t now)
{
...
#define ADDCERT(cert, pk)                                                                 \
    do {                                                                                  \
        ...
        if (tor_cert_get_checkable_sig(&check[n_checkable++], cert, pk, &expiration) < 0) \
    ...
    if (certs->started_here) {
            ...
            ERR("No Ed25519 link key");
        {
            ...
            const common_digests_t *peer_cert_digests = tor_x509_cert_get_cert_digests(peer_cert);
    ...
    if (rsa_ed25519_crosscert_check(certs->ed_rsa_crosscert, certs->ed_rsa_crosscert_len,
                                    rsa_id_key, &certs->ed_id_sign->signing_key, now) < 0) {
           ...
           const ed25519_public_key_t *master_id_pkey, const uint8_t *rsa_id_digest))
{
    ...
    int cc_len = crypto_pk_public_checksig(onion_pkey, (char *)cc, crypto_pk_keysize(onion_pkey),
    ...
    if (tor_memneq(cc, rsa_id_digest, DIGEST_LEN) ||
        tor_memneq(cc + DIGEST_LEN, master_id_pkey->pubkey, ED25519_PUBKEY_LEN)) {
 ...
 */
void
or_handshake_certs_check_both(int severity, or_handshake_certs_t *certs, tor_tls_t *tls, time_t now,
...
tor_cert_encode_ed22519(const tor_cert_t *cert, char **cert_str_out)
{
    ...
    ed_cert_b64_len = base64_encode_size(cert->encoded_len, BASE64_ENCODE_MULTILINE) + 1;
    ed_cert_b64 = tor_malloc_zero(ed_cert_b64_len);

    /* Base64 encode the encoded certificate. */
    if (base64_encode(ed_cert_b64, ed_cert_b64_len, (const char *)cert->encoded, cert->encoded_len,
