...
{
    size_t alloc_len = offsetof(nodefamily_t, family_members) + NODEFAMILY_ARRAY_SIZE(n_members);
...
{
    return (unsigned)siphash24g(nf->family_members, NODEFAMILY_ARRAY_SIZE(nf->n_members));
}
...
nodefamily_eq(const nodefamily_t *a, const nodefamily_t *b)
{
    return (a->n_members == b->n_members) &&
           fast_memeq(a->family_members, b->family_members, NODEFAMILY_ARRAY_SIZE(a->n_members));
}

static HT_HEAD(nodefamily_map, nodefamily_t) the_node_families = HT_INITIALIZER();

HT_PROTOTYPE(nodefamily_map, nodefamily_t, ht_ent, nodefamily_hash, nodefamily_eq);
HT_GENERATE2(nodefamily_map, nodefamily_t, ht_ent, nodefamily_hash, node_family_eq, 0.6,
 ...
 **/
char *
nodefamily_canonicalize(const char *s, const uint8_t *rsa_id_self, unsigned flags)
{
    smartlist_t *sl = smartlist_new();
    smartlist_t *result_members = smartlist_new();
    smartlist_split_string(sl, s, NULL, SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 0);
    nodefamily_t *nf = nodefamily_from_members(sl, rsa_id_self, flags, result_members);

    char *formatted = nodefamily_format(nf);
    smartlist_split_string(result_members, formatted, NULL, SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK,
                           0);
 ...
 **/
nodefamily_t *
nodefamily_from_members(const smartlist_t *members, const uint8_t *rsa_id_self, unsigned flags,
                        smartlist_t *unrecognized_out)
{
    ...
    SMARTLIST_FOREACH_BEGIN (members, const char *, cp) {
        ...
        if (bad_element) {
            ...
            log_fn(severity, LD_GENERAL, "Bad element %s while parsing a node family.",
                   escaped(cp));
            ++n_bad_elements;
        } else {
    ...
    qsort(tmp->family_members, n_members, NODEFAMILY_MEMBER_LEN, compare_members);
    ...
    if (found) {
        ...
        if (n_members_alloc != n_members) {
            ...
            memcpy(tmp2->family_members, tmp->family_members, n_members * NODEFAMILY_MEMBER_LEN);
            tor_free(tmp);
            tmp = tmp2;
        }
...
nodefamily_contains_rsa_id(const nodefamily_t *family, const uint8_t *rsa_id)
{
    ...
    for (i = 0; i < family->n_members; ++i) {
        ...
        if (ptr[0] == NODEFAMILY_BY_RSA_ID && fast_memeq(ptr + 1, rsa_id, DIGEST_LEN)) {
            return true;
        }
    }
    return false;
