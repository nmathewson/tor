...
nodefamily_parse(const char *s, const uint8_t *rsa_id_self, unsigned flags)
{
    ...
    smartlist_split_string(sl, s, NULL, SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK,
                           0);
    nodefamily_t *result =
        nodefamily_from_members(sl, rsa_id_self, flags, NULL);
                        ...
                        unsigned flags, smartlist_t *unrecognized_out)
{
    ...
    SMARTLIST_FOREACH_BEGIN(members, const char *, cp)
    {
        ...
        if (is_legal_nickname(cp)) {
            ...
            tor_assert(strlen(cp) <
                       DIGEST_LEN); // guaranteed by is_legal_nickname
            ...
            char nn_char = 0;
            if (hex_digest_nickname_decode(cp, digest_buf, &nn_char, nn_buf) ==
                0) {
                bad_element = false;
        ...
        if (bad_element) {
            const int severity =
                (flags & NF_WARN_MALFORMED) ? LOG_WARN : LOG_INFO;
    ...
    for (i = 0; i < n_members - 1; ++i) {
        ...
        if (fast_memeq(thisptr, nextptr, NODEFAMILY_MEMBER_LEN)) {
            memmove(thisptr, nextptr,
                    (n_members - i - 1) * NODEFAMILY_MEMBER_LEN);
            --n_members;
            --i;
        }
    }
...
nodefamily_contains_nickname(const nodefamily_t *family, const char *name)
{
    ...
    for (i = 0; i < family->n_members; ++i) {
        const uint8_t *ptr = NODEFAMILY_MEMBER_PTR(family, i);
        // note that the strcasecmp() is safe because there is always at least
        // one NUL in the encoded nickname, because all legal nicknames are less
        // than DIGEST_LEN bytes long.
    ...
    for (i = 0; i < family->n_members; ++i) {
        ...
        switch (ptr[0]) {
            case NODEFAMILY_BY_NICKNAME:
                node =
                    node_get_by_nickname((char *)ptr + 1, NNF_NO_WARN_UNNAMED);
                break;
    ...
    for (i = 0; i < family->n_members; ++i) {
        ...
        switch (ptr[0]) {
            ...
            case NODEFAMILY_BY_RSA_ID: {
                ...
                buf[0] = '$';
                base16_encode(buf + 1, sizeof(buf) - 1, (char *)ptr + 1,
                              DIGEST_LEN);
                tor_strupper(buf);
