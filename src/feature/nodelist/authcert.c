    ...
    for (s = contents; *s; s = eos) {
        ...
        if (already_have_cert(cert)) {
            ...
            if (!from_store) {
                 ...
                 * authority_cert_dl_failed() needs to know whether the download
                 * was by fp or (fp,sk) pair to twiddle the right bit in the
                 * download map.
                 */
                              ...
                              const char *dir_hint)
{
    ...
    if (status) {
                                ...
                                voter)
        {
            ...
            /*
             * If we don't know *any* cert for this authority, and a download by
             * ID is pending or we added it to missing_id_digests above, skip
             * this one for now to avoid duplicate downloads.
             */
