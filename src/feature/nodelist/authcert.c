                                    ...
                                    int flush, const char *source_dir)
{
    ...
    for (s = contents; *s; s = eos) {
        ...
        if (already_have_cert(cert)) {
            ...
            /*
             * A duplicate on download should be treated as a failure, so we
             * call authority_cert_dl_failed() to reset the download status to
             * make sure we can't try again.  Since we've implemented the fp-sk
             * mechanism to download certs by signing key, this should be much
             * rarer than it was and is perhaps cause for concern.
             */
            if (!from_store) {
                ...
                /*
                 * This is where we care about the source;
                 * authority_cert_dl_failed() needs to know whether the
                 * download was by fp or (fp,sk) pair to twiddle the right bit
                 * in the download map.
                 */
        ...
        if (ds && cert->cache_info.published_on > ds->addr_current_at) {
            /* Check to see whether we should update our view of the
             * authority's address. */
     ...
     * This avoids retrying a directory that's serving bad or entirely
    ...
    DIGESTMAP_FOREACH (trusted_dir_certs, key, cert_list_t *, cl) {
        ...
        SMARTLIST_FOREACH_BEGIN (cl->certs, authority_cert_t *, cert) {
            ...
            authority_cert_t *next_cert =
                smartlist_get(cl->certs, cert_sl_idx + 1);
            const time_t next_cert_published =
                next_cert->cache_info.published_on;
            ...
            if (cert->expires + DEAD_CERT_LIFETIME < now) {
                /* Certificate has been expired for at least
                 * DEAD_CERT_LIFETIME. Remove it. */
                should_remove = 1;
                                    ...
                                    const routerstatus_t *rs)
{
    ...
    if (options->UseBridges) {
        if (node && !node_is_a_configured_bridge(node)) {
            /* If we're using bridges, and node is not a bridge, use a 3-hop
             * path. */
            get_via_tor = 1;
        } else if (!node) {
    ...
    if (options->UseBridges && node && node->ri && !get_via_tor) {
        ...
        /* And if we've just downloaded a consensus from a directory, re-use
         * that directory */
        req = directory_request_new(DIR_PURPOSE_FETCH_CERTIFICATE);
    ...
    if (req) {
        /* We've set up a request object -- fill in the other request fields,
                              ...
                              const char *dir_hint)
{
    ...
    SMARTLIST_FOREACH_BEGIN (router_get_trusted_dir_servers(), dir_server_t *,
        ...
        SMARTLIST_FOREACH_BEGIN (cl->certs, authority_cert_t *, cert) {
            if (now < cert->expires) {
                ...
                /* No sense trying to download it specifically by signing key
                 * hash */
                download_status_reset_by_sk_in_cl(cl,
    ...
    if (status) {
        SMARTLIST_FOREACH_BEGIN (status->voters, networkstatus_voter_info_t *,
            ...
            if (!smartlist_len(voter->sigs))
                continue; /* This authority never signed this consensus, so
                           * don't go looking for a cert with key digest
                           * 0000000000. */
            if (!keep_unknown && !trusteddirserver_get_by_v3_auth_digest(
                                     voter->identity_digest))
                continue; /* We don't want unknown certs, and we don't know
                           * this authority.*/

            /*
             * If we don't know *any* cert for this authority, and a download
             * by ID is pending or we added it to missing_id_digests above,
             * skip this one for now to avoid duplicate downloads.
             */
            ...
            SMARTLIST_FOREACH_BEGIN (voter->sigs, document_signature_t *,
                                     sig) {
                ...
                if (download_status_is_ready_by_sk_in_cl(
                    ...
                    memcpy(fp_tmp->first, voter->identity_digest,
                           sizeof(fp_tmp->first));
     ...
     * As soon as the directory fails to provide additional certificates, we
     * try another, randomly selected directory. This avoids continual retries.
     * (We only ever have one outstanding request per certificate.)
     */
    if (dir_hint) {
        if (options->UseBridges) {
            /* Bridge clients try the nodelist. If the dir_hint is from an
             * authority, or something else fetched over tor, we won't find the
             * node here, but we will find the rs. */
