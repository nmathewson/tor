...
#define DSMAP_FOREACH(map, keyvar, valvar) \
    DIGESTMAP_FOREACH (dsmap_to_digestmap(map), keyvar, download_status_t *, valvar)
 ...
 */
static int
download_status_is_ready_by_sk_in_cl(cert_list_t *cl, const char *digest, time_t now)
{
    int rv = 0;
    download_status_t *dlstatus = NULL;
...
cert_list_free_(cert_list_t *cl)
{
    ...
    SMARTLIST_FOREACH(cl->certs, authority_cert_t *, cert, authority_cert_free(cert));
...
trusted_dirs_reload_certs(void)
{
    ...
    if (!contents)
        return 0;
    r = trusted_dirs_load_certs_from_string(contents, TRUSTED_DIRS_CERTS_SRC_FROM_STORE, 1, NULL);
    tor_free(contents);
    return r;
}
 ...
 */
int
trusted_dirs_load_certs_from_string(const char *contents, int source, int flush,
                                    const char *source_dir)
{
    ...
    for (s = contents; *s; s = eos) {
        authority_cert_t *cert = authority_cert_parse_from_string(s, strlen(s), &eos);
        cert_list_t *cl;
        if (!cert) {
            failure_code = -1;
            break;
        ...
        if (already_have_cert(cert)) {
            ...
            if (!from_store) {
                ...
                if (source == TRUSTED_DIRS_CERTS_SRC_DL_BY_ID_DIGEST) {
                    authority_cert_dl_failed(cert->cache_info.identity_digest, NULL, 404);
        ...
        if (ds) {
            ...
            log_info(LD_DIR,
                     ...
                     adding ? "Adding" : "Not adding", from_store ? "cached" : "downloaded",
                     hex_str(cert->signing_key_digest, DIGEST_LEN));
            if (!adding) {
                authority_cert_free(cert);
                continue;
...
authority_cert_get_by_sk_digest(const char *sk_digest)
{
    ...
    if ((c = get_my_v3_authority_cert()) && tor_memeq(c->signing_key_digest, sk_digest, DIGEST_LEN))
        return c;
    if ((c = get_my_v3_legacy_cert()) && tor_memeq(c->signing_key_digest, sk_digest, DIGEST_LEN))
        return c;
...
authority_cert_get_by_digests(const char *id_digest, const char *sk_digest)
{
    ...
    if (!trusted_dir_certs || !(cl = digestmap_get(trusted_dir_certs, id_digest)))
        return NULL;
    SMARTLIST_FOREACH(cl->certs, authority_cert_t *, cert,
                      if (tor_memeq(cert->signing_key_digest, sk_digest, DIGEST_LEN)) return cert;);
...
authority_cert_get_all(smartlist_t *certs_out)
{
    ...
    DIGESTMAP_FOREACH (trusted_dir_certs, key, cert_list_t *, cl) {
        SMARTLIST_FOREACH(cl->certs, authority_cert_t *, c, smartlist_add(certs_out, c));
    }
    DIGESTMAP_FOREACH_END;
}
...
void
authority_cert_dl_failed(const char *id_digest, const char *signing_key_digest, int status)
{
    cert_list_t *cl;
    download_status_t *dlstatus = NULL;
    ...
    if (!signing_key_digest) {
        ...
        if (dlstatus) {
             ...
             */
            base16_encode(id_digest_str, sizeof(id_digest_str), id_digest, DIGEST_LEN);
            base16_encode(sk_digest_str, sizeof(sk_digest_str), signing_key_digest, DIGEST_LEN);
            log_warn(LD_BUG,
...
static void
authority_certs_fetch_resource_impl(const char *resource, const char *dir_hint, const node_t *node,
                                    const routerstatus_t *rs)
{
    const or_options_t *options = get_options();
    int get_via_tor = purpose_needs_anonymity(DIR_PURPOSE_FETCH_CERTIFICATE, 0, resource);
...
authority_certs_fetch_missing(networkstatus_t *status, time_t now, const char *dir_hint)
{
     ...
     * duplicate effort.
     */
    list_pending_downloads(pending_id, NULL, DIR_PURPOSE_FETCH_CERTIFICATE, "fp/");
    ...
    if (status) {
        SMARTLIST_FOREACH_BEGIN (status->voters, networkstatus_voter_info_t *, voter) {
            ...
            if (!keep_unknown && !trusteddirserver_get_by_v3_auth_digest(voter->identity_digest))
            ...
            if (smartlist_len(cl->certs) == 0) {
                 ...
                 */
                if (smartlist_contains_digest(missing_id_digests, voter->identity_digest))
                    continue;
            }

            SMARTLIST_FOREACH_BEGIN (voter->sigs, document_signature_t *, sig) {
                authority_cert_t *cert =
                    authority_cert_get_by_digests(voter->identity_digest, sig->signing_key_digest);
                if (cert) {
                    if (now < cert->expires)
                        download_status_reset_by_sk_in_cl(cl, sig->signing_key_digest);
                    continue;
                }
                if (download_status_is_ready_by_sk_in_cl(cl, sig->signing_key_digest, now) &&
                     ...
                     * old results and we call twice in the param list.
                     */
                    base16_encode(id_digest_str, sizeof(id_digest_str), voter->identity_digest,
                                  DIGEST_LEN);
                    base16_encode(sk_digest_str, sizeof(sk_digest_str), sig->signing_key_digest,
                                  DIGEST_LEN);
                    ...
                    memcpy(fp_tmp->second, sig->signing_key_digest, sizeof(fp_tmp->second));
    ...
    if (dir_hint) {
        ...
        if (!rs) {
            ...
            const dir_server_t *ds = router_get_fallback_dirserver_by_digest(dir_hint);
            if (ds) {
                rs = &ds->fake_status;
            }
        }
        ...
        if (!node && !rs) {
            log_warn(LD_BUG,
                     ...
                     options->UseBridges ? "no node and " : "", hex_str(dir_hint, DIGEST_LEN));
        }
    }
    ...
    if (smartlist_len(missing_cert_digests) > 0) {
        ...
        SMARTLIST_FOREACH_BEGIN (missing_cert_digests, const fp_pair_t *, d) {
            ...
            base16_encode(id_digest_str, sizeof(id_digest_str), d->first, DIGEST_LEN);
            base16_encode(sk_digest_str, sizeof(sk_digest_str), d->second, DIGEST_LEN);
...
list_pending_fpsk_downloads(fp_pair_map_t *result)
{
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn) {
        if (conn->type == CONN_TYPE_DIR && conn->purpose == DIR_PURPOSE_FETCH_CERTIFICATE &&
