...
static void count_usable_descriptors(int *num_present, int *num_usable, smartlist_t *descs_out,
...
HT_GENERATE2(nodelist_map, node_t, ht_ent, node_id_hash, node_id_eq, 0.6, tor_reallocarray_,
             tor_free_);
...
HT_GENERATE2(nodelist_ed_map, node_t, ed_ht_ent, node_ed_id_hash, node_ed_id_eq, 0.6,
...
node_remove_from_ed25519_map(node_t *node)
{
    ...
    int rv = 0;
    node_t *search = HT_FIND(nodelist_ed_map, &the_nodelist->nodes_by_ed_id, node);
    if (BUG(search != node)) {
...
node_log_dup_ed_id(const node_t *old, const node_t *node, const char *ed_id)
{
    ...
    tor_asprintf(&s, "Reused ed25519_id %s: old %s new %s", ed_id, olddesc, node_describe(node));
...
node_add_to_ed25519_map(node_t *node)
{
    ...
    if (old) {
        ...
        base32_encode(ed_id, sizeof(ed_id), (const char *)key->pubkey, sizeof(key->pubkey));
        if (BUG(old == node)) {
            /* Actual bug: all callers of this function call
             * node_remove_from_ed25519_map first. */
            log_err(LD_BUG, "Unexpectedly found deleted node with ed25519_id %s", ed_id);
        } else {
...
node_set_hsdir_index(node_t *node, const networkstatus_t *ns)
{
    ...
    hs_build_hsdir_index(node_identity_pk, fetch_srv, fetch_tp, node->hsdir_index.fetch);
...
{
    if (BUG(!addr) || tor_addr_is_null(addr) || tor_addr_is_v4(addr) || !the_nodelist ||
        !the_nodelist->node_addrs) {
        return;
    }
...
nodelist_set_consensus(networkstatus_t *ns)
{
    ...
    int estimated_addresses =
        smartlist_len(ns->routerstatus_list) * get_estimated_address_per_node();
    estimated_addresses +=
        (get_n_authorities(V3_DIRINFO & BRIDGE_DIRINFO) * get_estimated_address_per_node());
...
nodelist_ensure_freshness(networkstatus_t *ns)
{
    ...
    if (the_nodelist->live_consensus_valid_after != ns->valid_after) {
        log_info(LD_GENERAL, "Nodelist was not fresh: rebuilding. (%d / %d)",
                 (int)the_nodelist->live_consensus_valid_after, (int)ns->valid_after);
...
static const protover_summary_flags_t zero_protover_flags = {0, 0, 0, 0, 0, 0, 0, 0, 0};
...
node_supports_ed25519_link_authentication,
          (const node_t *node, int compatible_with_us))
{
    if (!node_get_ed25519_id(node))
        return 0;
...
node_supports_establish_intro_dos_extension(const node_t *node)
{
    ...
    return node_get_protover_summary_flags(node)->supports_establish_intro_dos_extension;
}
...
node_get_link_specifier_smartlist, (const node_t *node, bool direct_conn))
{
     ...
     * address (or both). */
    if (BUG(!tor_addr_is_v4(&ap.addr)) || BUG(!tor_addr_port_is_valid_ap(&ap, 0))) {
        return lspecs;
    }
    ...
    link_specifier_set_ls_len(ls, sizeof(ap.addr.addr.in_addr) + sizeof(ap.port));
...
link_specifier_smartlist_free_(smartlist_t *ls_list)
{
    ...
    SMARTLIST_FOREACH(ls_list, link_specifier_t *, lspec, link_specifier_free(lspec));
...
{
    if (node->ri && node->ri->declared_family && smartlist_len(node->ri->declared_family)) {
        return true;
    }
...
nodes_in_same_family(const node_t *node1, const node_t *node2)
{
    ...
    if (node_family_contains(node1, node2) && node_family_contains(node2, node1)) {
        return 1;
    }
...
{ /*XXXX MOVE*/
    ...
    SMARTLIST_FOREACH(nodelist_get_list(), const node_t *, node, {
        if (node_get_addr_ipv4h(node) == addr && node->is_running &&
            compare_tor_addr_to_node_policy(&a, port, node) == ADDR_POLICY_ACCEPTED &&
 ...
 */
int
node_is_unreliable(const node_t *node, int need_uptime, int need_capacity, int need_guard)
{
...
int
router_exit_policy_all_nodes_reject(const tor_addr_t *addr, uint16_t port, int need_uptime)
{
...
router_set_status(const char *digest, int up)
{
    ...
    SMARTLIST_FOREACH(router_get_fallback_dir_servers(), dir_server_t *, d,
                      if (tor_memeq(d->digest, digest, DIGEST_LEN)) d->is_running = up);
 ...
 */
static void
count_usable_descriptors(int *num_present, int *num_usable, smartlist_t *descs_out,
                         const networkstatus_t *consensus, time_t now, routerset_t *in_set,
                         usable_descriptor_t exit_only)
{
    ...
    *num_present = 0, *num_usable = 0;
    ...
    log_debug(LD_DIR, "%d usable, %d present (%s%s%s%s%s).", *num_usable, *num_present,
              md ? "microdesc" : "desc",
 ...
 * paths. */
static double
compute_frac_paths_available(const networkstatus_t *consensus, const or_options_t *options,
                             time_t now, int *num_present_out, int *num_usable_out,
                             char **status_out)
{
    ...
    count_usable_descriptors(num_present_out, num_usable_out, mid, consensus, now,
                             options->MiddleNodes, USABLE_DESCRIPTOR_ALL);
    log_debug(LD_NET, "%s: %d present, %d usable", "mid", np, nu);

    if (options->EntryNodes) {
        count_usable_descriptors(&np, &nu, guards, consensus, now, options->EntryNodes,
    ...
    have_consensus_path = ((np > 0) ? CONSENSUS_PATH_EXIT : CONSENSUS_PATH_INTERNAL);
    ...
    if (options->ExitNodes) {
        ...
        count_usable_descriptors(&np, &nu, myexits, consensus, now, options->ExitNodes,
                                 USABLE_DESCRIPTOR_EXIT_POLICY_AND_FLAG);
        ...
        count_usable_descriptors(&np, &nu, myexits_unflagged, consensus, now, options->ExitNodes,
        ...
        f_myexit_unflagged = frac_nodes_with_descriptors(myexits_unflagged, WEIGHT_FOR_EXIT, 0);

        log_debug(LD_NET, "f_exit: %.2f, f_myexit: %.2f, f_myexit_unflagged: %.2f", f_exit,
...
count_loading_descriptors_progress(void)
{
    ...
    return BOOTSTRAP_STATUS_LOADING_DESCRIPTORS +
           (int)(fraction *
                 (BOOTSTRAP_STATUS_ENOUGH_DIRINFO - 1 - BOOTSTRAP_STATUS_LOADING_DESCRIPTORS));
}
...
static double
get_frac_paths_needed_for_circs(const or_options_t *options, const networkstatus_t *ns)
{
#define DFLT_PCT_USABLE_NEEDED 60
    if (options->PathsNeededToBuildCircuits >= 0.0) {
        return options->PathsNeededToBuildCircuits;
    } else {
        return networkstatus_get_param(ns, "min_paths_for_circs_pct", DFLT_PCT_USABLE_NEEDED, 25,
                                       95) /
               100.0;
    }
}
...
update_router_have_minimum_dir_info(void)
{
    ...
    {
        char *status = NULL;
        double paths = compute_frac_paths_available(consensus, options, now, &num_present,
                                                    &num_usable, &status);
        ...
        if (paths < get_frac_paths_needed_for_circs(options, consensus)) {
            tor_snprintf(dir_info_status, sizeof(dir_info_status),
                         ...
                         using_md ? "micro" : "", num_present, num_usable, (int)(paths * 100),
                         status);
            tor_free(status);
            res = 0;
    ...
    { /* Check entry guard dirinfo status */
        char *guard_error =
            entry_guards_get_err_str_if_dir_info_missing(using_md, num_present, num_usable);
        if (guard_error) {
