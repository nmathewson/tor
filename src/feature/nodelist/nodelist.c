...
node_add_to_ed25519_map(node_t *node)
{
    ...
    if (old) {
        ...
        if (BUG(old == node)) {
            ...
            log_err(LD_BUG,
                    "Unexpectedly found deleted node with ed25519_id %s",
                    ed_id);
        } else {
...
node_set_hsdir_index(node_t *node, const networkstatus_t *ns)
{
    ...
    uint8_t *fetch_srv = NULL, *store_first_srv = NULL,
            *store_second_srv = NULL;
    ...
    /* We always use the old SRV for storing the first descriptor and the
     * latest SRV for storing the second descriptor */
    ...
    /* If we are in the time segment between TP#N and SRV#N+1, the fetch index
       is the same as the second store index */
    if (hs_in_period_between_tp_and_srv(ns, now)) {
        memcpy(node->hsdir_index.store_second, node->hsdir_index.fetch,
               sizeof(node->hsdir_index.store_second));
    } else {
        hs_build_hsdir_index(node_identity_pk, store_second_srv,
                             store_second_tp, node->hsdir_index.store_second);
    }
...
nodelist_set_consensus(networkstatus_t *ns)
{
    ...
    SMARTLIST_FOREACH_BEGIN (ns->routerstatus_list, routerstatus_t *, rs) {
        ...
        if (ns->flavor == FLAV_MICRODESC) {
            if (node->md == NULL ||
                tor_memneq(node->md->digest, rs->descriptor_digest,
                           DIGEST256_LEN)) {
                node_remove_from_ed25519_map(node);
                if (node->md)
                    node->md->held_by_nodes--;
                node->md = microdesc_cache_lookup_by_digest256(
    ...
    if (!authdir) {
        SMARTLIST_FOREACH_BEGIN (the_nodelist->nodes, node_t *, node) {
            /* We have no routerstatus for this router. Clear flags so we can
             * skip it, maybe.*/
            if (!node->rs) {
                tor_assert(node->ri); /* if it had only an md, or nothing,
                                       * purge would have removed it. */
    ...
    /* If the consensus is live, note down the consensus valid-after that
     * formed the nodelist. */
...
nodelist_assert_ok(void)
{
    ...
    if (rl) {
        SMARTLIST_FOREACH_BEGIN (rl->routers, routerinfo_t *, ri) {
            const node_t *node =
                node_get_by_id(ri->cache_info.identity_digest);
...
node_get_by_hex_id(const char *hex_id, unsigned flags)
{
    ...
    if (hex_digest_nickname_decode(hex_id, digest_buf, &nn_char, nn_buf) ==
        0) {
    ...
    /* Okay, so the name is not canonical for anybody. */
    {
        ...
        if (smartlist_len(matches) > 1 && warn_if_unnamed) {
            ...
            if (!node->name_lookup_warned) {
                ...
                log_warn(
                    LD_CONFIG,
                    "You specified a relay \"%s\" by name, but nicknames can "
                    "be "
          ...
          (const node_t *node))
{
    ...
    if (ri_pk && md_pk) {
        if (ed25519_pubkey_eq(ri_pk, md_pk)) {
            ...
            /* This can happen if the relay gets flagged NoEdConsensus which
             * will be
...
node_get_protover_summary_flags(const node_t *node)
{
    if (node->rs) {
        ...
        /* This should be impossible: every node should have a routerstatus or
         * a router descriptor or both. But just in case we've messed up
         * somehow, return a nice empty set of flags to indicate "this node
         * supports nothing." */
...
node_get_link_specifier_smartlist,
          (const node_t *node, bool direct_conn))
{
    link_specifier_t *ls;
    tor_addr_port_t ap;
...
#define RETURN_IPV4_AP(r, port_field, ap_out)                              \
    STMT_BEGIN                                                             \
        if (r &&                                                           \
            tor_addr_port_is_valid_ipv4h((r)->addr, (r)->port_field, 0)) { \
...
node_get_prim_orport(const node_t *node, tor_addr_port_t *ap_out)
{
    ...
    /* Clear the address, as a safety precaution if calling functions ignore
     * the return value */
...
node_lookup_declared_family(smartlist_t *out, const node_t *node)
{
    if (node->ri && node->ri->declared_family &&
        smartlist_len(node->ri->declared_family)) {
        SMARTLIST_FOREACH_BEGIN (node->ri->declared_family, const char *,
                                 name) {
    ...
    *num_present = 0, *num_usable = 0;
    ...
    SMARTLIST_FOREACH_BEGIN (consensus->routerstatus_list, routerstatus_t *,
        ...
        if (client_would_use_router(rs, now)) {
            ...
            if (md)
                present =
                    NULL != microdesc_cache_lookup_by_digest256(NULL, digest);
            else
    ...
    log_debug(
        LD_DIR, "%d usable, %d present (%s%s%s%s%s).", *num_usable,
        *num_present, md ? "microdesc" : "desc",
                             ...
                             char **status_out)
{
    ...
    if (old_have_consensus_path != have_consensus_path) {
        if (have_consensus_path == CONSENSUS_PATH_INTERNAL) {
            ...
            /* However, exit nodes can reachability self-test using this
             * consensus, join the network, and appear in a later consensus.
             * This will allow the network to build exit paths, such as paths
             * for world wide web
    ...
    log_debug(LD_NET, "f_guard: %.2f, f_mid: %.2f, f_exit: %.2f", f_guard,
              f_mid, f_exit);
    ...
    if (options->ExitNodes) {
        ...
        /* Now compute the nodes in the ExitNodes option where we know their
         * exit policy permits something. */
        ...
        /* If our ExitNodes list has eliminated every possible Exit node, and
         * there were some possible Exit nodes, then instead consider nodes
         * that permit exiting to some ports. */
    ...
    if (router_have_consensus_path() != CONSENSUS_PATH_EXIT) {
        /* If the exit bandwidth weight fraction is not zero, we need to wait
         * for descriptors for those exits. (The bandwidth weight fraction does
         * not check for descriptors.) If the exit bandwidth fraction is zero,
         * there are no exits in the consensus at all. So it is safe to replace
         * f_exit with f_mid.
         *
