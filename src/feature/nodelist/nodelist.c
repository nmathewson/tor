...
HT_GENERATE2(nodelist_ed_map, node_t, ed_ht_ent, node_ed_id_hash, node_ed_id_eq,
...
node_get_ed25519_id(const node_t *node)
{
    ...
    if (ri_pk && md_pk) {
        if (ed25519_pubkey_eq(ri_pk, md_pk)) {
             ...
             * will be triggered on all relays of the network. Thus a protocol
             * warning. */
                             ...
                             char **status_out)
{
    ...
    if (options->ExitNodes) {
         ...
         * there were some possible Exit nodes, then instead consider nodes that
         * permit exiting to some ports. */
