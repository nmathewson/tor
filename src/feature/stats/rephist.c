...
void
rep_hist_note_router_reachable(const char *id, const tor_addr_t *at_addr, const uint16_t at_port,
                               time_t when)
{
    ...
    port_changed = at_port && hist->last_reached_port && at_port != hist->last_reached_port;
...
#define SUBTRACT_CLAMPED(var, penalty)                     \
    do {                                                   \
        ...
        log_info(LD_HIST,
                 ...
                 hex_str(id, DIGEST_LEN), tbuf, hist->weighted_uptime, hist->total_weighted_time);
    }
    if (!hist->start_of_downtime) {
...
rep_hist_downrate_old_runs(time_t now)
{
    ...
    log_info(LD_HIST, "Discounting all old stability info by a factor of %f", alpha);
    ...
    for (orhist_it = digestmap_iter_init(history_map); !digestmap_iter_done(orhist_it);
        ...
        hist->weighted_run_length = (unsigned long)(hist->weighted_run_length * alpha);
        hist->total_run_weights *= alpha;

        hist->weighted_uptime = (unsigned long)(hist->weighted_uptime * alpha);
        hist->total_weighted_time = (unsigned long)(hist->total_weighted_time * alpha);
    }
...
rep_hist_dump_stats(time_t now, int severity)
{
    ...
    for (orhist_it = digestmap_iter_init(history_map); !digestmap_iter_done(orhist_it);
        ...
        stability = (long)s;
        tor_log(severity, LD_HIST, "OR %s [%s]: wmtbf %lu:%02lu:%02lu", name1, hexdigest1,
...
rep_history_clean(time_t before)
{
    ...
    while (!digestmap_iter_done(orhist_it)) {
        ...
        should_remove = authority ? (or_history->total_run_weights < STABILITY_EPSILON &&
...
rep_hist_record_mtbf_data(time_t now, int missing_means_down)
{
...

    {
        ...
        f = start_writing_to_stdio_file(filename, OPEN_FLAGS_REPLACE | O_TEXT, 0600, &open_file);
        tor_free(filename);
        if (!f)
            return -1;
    }
...
rep_hist_load_mtbf_data(time_t now)
{
...

    {
        ...
        if (smartlist_len(lines) > 4) {
            ...
            if (!strcmpstart(firstline, "format "))
                format =
                    tor_parse_long(firstline + strlen("format "), 10, -1, LONG_MAX, NULL, NULL);
        }
    }
    ...
    for (; i < smartlist_len(lines); ++i) {
        ...
        if (format == 1) {
            n = tor_sscanf(line, "%40s %ld %lf S=%10s %8s", hexbuf, &wrl, &trw, mtbf_timebuf,
                           mtbf_timebuf + 11);
            if (n != 3 && n != 5) {
            ...
            if (mtbf_idx >= 0) {
                ...
                n = tor_sscanf(mtbfline, "+MTBF %lu %lf S=%10s %8s", &wrl, &trw, mtbf_timebuf,
                               mtbf_timebuf + 11);
                if (n == 2 || n == 4) {
                    have_mtbf = 1;
                } else {
            ...
            if (wfu_idx >= 0) {
                ...
                n = tor_sscanf(wfuline, "+WFU %lu %lu S=%10s %8s", &wt_uptime, &total_wt_time,
...
#define MAX_HIST_VALUE_LEN (21 * NUM_TOTALS)
    ...
    for (r = 0; r < 4; ++r) {
        ...
        tor_snprintf(cp, len - (cp - buf), "%s %s (%d s) ", desc, t, NUM_SECS_BW_SUM_INTERVAL);
        cp += strlen(cp);
...
static void
rep_hist_update_bwhist_state_section(or_state_t *state, const bw_array_t *b, smartlist_t **s_values,
                                     smartlist_t **s_maxima, time_t *s_begins, int *s_interval)
{
    int i, j;
    uint64_t maxval;
...
#define UPDATE(arrname, st)                                                             \
    rep_hist_update_bwhist_state_section(                                               \
        state, (arrname), &state->BWHistory##st##Values, &state->BWHistory##st##Maxima, \
        &state->BWHistory##st##Ends, &state->BWHistory##st##Interval)
...
rep_hist_load_bwhist_state_section(bw_array_t *b, const smartlist_t *s_values,
                                   const smartlist_t *s_maxima, const time_t s_begins,
                                   const int s_interval)
{
    ...
    uint64_t v, mv;
    int i, ok, ok_m = 0;
    int have_maxima = s_maxima && s_values && (smartlist_len(s_values) == smartlist_len(s_maxima));
    ...
    if (s_values && s_begins >= now - NUM_SECS_BW_SUM_INTERVAL * NUM_TOTALS) {
        ...
        SMARTLIST_FOREACH_BEGIN (s_values, const char *, cp) {
            ...
            if (maxstr && !ok_m) {
                retval = -1;
                log_notice(LD_HIST, "Could not parse maximum '%s' into a number.'", maxstr);
            }
...
#define LOAD(arrname, st)                                                          \
    if (rep_hist_load_bwhist_state_section(                                        \
            (arrname), state->BWHistory##st##Values, state->BWHistory##st##Maxima, \
            state->BWHistory##st##Ends, state->BWHistory##st##Interval) < 0)       \
    all_ok = 0
...
rep_hist_format_exit_stats(time_t now)
{
    ...
    uint64_t cur_bytes = 0, other_read = 0, other_written = 0, total_read = 0, total_written = 0;
    ...
    for (j = 0; j < top_elements; j++) {
        ...
        if (exit_bytes_written[cur_port] > 0) {
            uint64_t num = round_uint64_to_next_multiple_of(exit_bytes_written[cur_port],
        ...
        if (exit_bytes_read[cur_port] > 0) {
            uint64_t num = round_uint64_to_next_multiple_of(exit_bytes_read[cur_port],
                                                            EXIT_STATS_ROUND_UP_BYTES);
            num /= 1024;
    ...
    other_written = round_uint64_to_next_multiple_of(other_written, EXIT_STATS_ROUND_UP_BYTES);
    other_written /= 1024;
    smartlist_add_asprintf(written_strings, "other=%" PRIu64, (other_written));
    other_read = round_uint64_to_next_multiple_of(other_read, EXIT_STATS_ROUND_UP_BYTES);
    other_read /= 1024;
    smartlist_add_asprintf(read_strings, "other=%" PRIu64, (other_read));
    other_streams = round_uint32_to_next_multiple_of(other_streams, EXIT_STATS_ROUND_UP_STREAMS);
    ...
    tor_asprintf(&result,
                 ...
                 t, (unsigned)(now - start_of_exit_stats_interval), written_string, read_string,
                 streams_string);
 ...
 * circuit. */
void
rep_hist_add_buffer_stats(double mean_num_cells_in_queue, double mean_time_cells_in_queue,
                          uint32_t processed_cells)
{
...
rep_hist_buffer_stats_add_circ(circuit_t *circ, time_t end_of_interval)
{
    ...
    if (!orcirc->processed_cells)
        return;
    start_of_interval = (circ->timestamp_created.tv_sec > start_of_buffer_stats_interval)
    ...
    mean_num_cells_in_queue =
        (double)orcirc->total_cell_waiting_time / (double)interval_length / 1000.0 / 2.0;
    mean_time_cells_in_queue =
        (double)orcirc->total_cell_waiting_time / (double)orcirc->processed_cells;
    orcirc->total_cell_waiting_time = 0;
    orcirc->processed_cells = 0;
    rep_hist_add_buffer_stats(mean_num_cells_in_queue, mean_time_cells_in_queue, processed_cells);
}
...
rep_hist_reset_buffer_stats(time_t now)
{
    if (!circuits_for_buffer_stats)
        circuits_for_buffer_stats = smartlist_new();
    SMARTLIST_FOREACH(circuits_for_buffer_stats, circ_buffer_stats_t *, stats, tor_free(stats));
...
#define SHARES 10
    ...
    smartlist_t *processed_cells_strings, *queued_cells_strings, *time_in_queue_strings;
    ...
    for (i = 0; i < SHARES; i++) {
        smartlist_add_asprintf(processed_cells_strings, "%" PRIu64,
                               !circs_in_share[i] ? 0 : (processed_cells[i] / circs_in_share[i]));
    }
    for (i = 0; i < SHARES; i++) {
        smartlist_add_asprintf(
            queued_cells_strings, "%.2f",
            circs_in_share[i] == 0 ? 0.0 : queued_cells[i] / (double)circs_in_share[i]);
    }
    for (i = 0; i < SHARES; i++) {
        smartlist_add_asprintf(
            time_in_queue_strings, "%.0f",
            circs_in_share[i] == 0 ? 0.0 : time_in_queue[i] / (double)circs_in_share[i]);
    }

    /* Join all observations in single strings. */
    processed_cells_string = smartlist_join_strings(processed_cells_strings, ",", 0, NULL);
    queued_cells_string = smartlist_join_strings(queued_cells_strings, ",", 0, NULL);
    time_in_queue_string = smartlist_join_strings(time_in_queue_strings, ",", 0, NULL);
    ...
    tor_asprintf(&result,
                 ...
                 t, (unsigned)(now - start_of_buffer_stats_interval), processed_cells_string,
                 queued_cells_string, time_in_queue_string, CEIL_DIV(number_of_circuits, SHARES));
...
HT_GENERATE2(bidimap, bidi_map_entry_t, node, bidi_map_ent_hash, bidi_map_ent_eq, 0.6,
 ...
 * for this connection. */
void
rep_hist_note_or_conn_bytes(uint64_t conn_id, size_t num_read, size_t num_written, time_t when)
{
    ...
    if (when >= bidi_next_interval) {
        ...
        log_info(LD_GENERAL,
                 ...
                 below_threshold, mostly_read, mostly_written, both_read_and_written);
    }
...
rep_hist_format_conn_stats(time_t now)
{
    ...
    tor_asprintf(&result, "conn-bi-direct %s (%d s) %d,%d,%d,%d\n", written,
                 (unsigned)(now - start_of_conn_stats_interval), below_threshold, mostly_read,
                 mostly_written, both_read_and_written);
    return result;
}
...
rep_hist_stored_maybe_new_hs(const crypto_pk_t *pubkey)
{
    ...
    if (!digestmap_get(hs_stats->onions_seen_this_period, pubkey_hash)) {
        digestmap_set(hs_stats->onions_seen_this_period, pubkey_hash, (void *)(uintptr_t)1);
    }
}
...
rep_hist_format_hs_stats(time_t now)
{
    ...
    uint64_t rounded_cells_seen =
        round_uint64_to_next_multiple_of(hs_stats->rp_relay_cells_seen, REND_CELLS_BIN_SIZE);
    rounded_cells_seen = MIN(rounded_cells_seen, INT64_MAX);
    obfuscated_cells_seen = add_laplace_noise((int64_t)rounded_cells_seen, crypto_rand_double(),
                                              REND_CELLS_DELTA_F, REND_CELLS_EPSILON);

    uint64_t rounded_onions_seen = round_uint64_to_next_multiple_of(
        (size_t)digestmap_size(hs_stats->onions_seen_this_period), ONIONS_SEEN_BIN_SIZE);
    rounded_onions_seen = MIN(rounded_onions_seen, INT64_MAX);
    obfuscated_onions_seen = add_laplace_noise((int64_t)rounded_onions_seen, crypto_rand_double(),
    ...
    tor_asprintf(&hs_stats_string,
                 ...
                 "epsilon=%.2f bin_size=%d\n",
                 t, (unsigned)(now - start_of_hs_stats_interval), (obfuscated_cells_seen),
                 REND_CELLS_DELTA_F, REND_CELLS_EPSILON, REND_CELLS_BIN_SIZE,
                 (obfuscated_onions_seen), ONIONS_SEEN_DELTA_F, ONIONS_SEEN_EPSILON,
...
rep_hist_hs_stats_write(time_t now)
{
    ...
    /* Try to write to disk. */
    if (!check_or_create_data_subdir("stats")) {
        write_to_data_subdir("stats", "hidserv-stats", str, "hidden service stats");
    }
...
#define ROUND_AND_SET_COUNT(x) (x) = round_uint64_to_next_multiple_of((x), ROUND_CELL_COUNTS_TO)
...
rep_hist_get_padding_count_lines(void)
{
    ...
    if (!padding_published.read_cell_count || !padding_published.write_cell_count) {
        return NULL;
    }
    ...
    tor_asprintf(
        ...
        " bin-size=%" PRIu64 " write-drop=%" PRIu64 " write-pad=%" PRIu64 " write-total=%" PRIu64
        " read-drop=%" PRIu64 " read-pad=%" PRIu64 " read-total=%" PRIu64
        " enabled-read-pad=%" PRIu64 " enabled-read-total=%" PRIu64 " enabled-write-pad=%" PRIu64
        " enabled-write-total=%" PRIu64 " max-chanpad-timers=%" PRIu64 "\n",
        padding_published.first_published_at, REPHIST_CELL_PADDING_COUNTS_INTERVAL,
        (uint64_t)ROUND_CELL_COUNTS_TO, (padding_published.write_drop_cell_count),
        (padding_published.write_pad_cell_count), (padding_published.write_cell_count),
        (padding_published.read_drop_cell_count), (padding_published.read_pad_cell_count),
        (padding_published.read_cell_count), (padding_published.enabled_read_pad_cell_count),
        (padding_published.enabled_read_cell_count),
        (padding_published.enabled_write_pad_cell_count),
        (padding_published.enabled_write_cell_count), (padding_published.maximum_chanpad_timers));
...
rep_hist_free_all(void)
{
    ...
    if (circuits_for_buffer_stats) {
        SMARTLIST_FOREACH(circuits_for_buffer_stats, circ_buffer_stats_t *, s, tor_free(s));
