                               ...
                               const uint16_t at_port, time_t when)
{
    ...
    if (hist->start_of_downtime) {
        ...
        if ((ns = networkstatus_get_latest_consensus())) {
            ...
            /* on average, a descriptor addr change takes .5 intervals to make
             * it into a consensus, and half a liveness period to make it to
             * clients. */
...
get_stability(or_history_t *hist, time_t when)
{
    ...
    if (hist->start_of_run) {
        /* We're currently in a run.  Let total and total_weights hold the
         * values they would hold if the current run were to end now. */
...
get_weighted_fractional_uptime(or_history_t *hist, time_t when)
{
    ...
    if (!total) {
        /* Avoid calling anybody's uptime infinity (which should be impossible
         * if the code is working), or NaN (which can happen for any router we
         * haven't observed up or down yet). */
        return 0.0;
    }
...
rep_history_clean(time_t before)
{
    ...
    while (!digestmap_iter_done(orhist_it)) {
        ...
        should_remove =
            authority ? (or_history->total_run_weights < STABILITY_EPSILON &&
...
rep_hist_load_mtbf_data(time_t now)
{
    ...
    for (i = 1; i < smartlist_len(lines); ++i) {
        ...
        if (!strcmpstart(line, "tracked-since ")) {
            if (parse_iso_time(line + strlen("tracked-since "),
                               &tracked_since) < 0)
                log_warn(LD_HIST,
    ...
    for (; i < smartlist_len(lines); ++i) {
        ...
        if (format == 1) {
            ...
            if (mtbf_idx >= 0) {
                ...
                n = tor_sscanf(mtbfline, "+MTBF %lu %lf S=%10s %8s", &wrl,
                               &trw, mtbf_timebuf, mtbf_timebuf + 11);
                if (n == 2 || n == 4) {
                    have_mtbf = 1;
                } else {
            ...
            if (wfu_idx >= 0) {
                ...
                if (n == 2 || n == 4) {
                    ...
                    log_warn(LD_HIST, "Couldn't scan +WFU line %s",
                             escaped(wfuline));
                }
            }
            if (wfu_idx > i)
        ...
        if (have_mtbf) {
            if (mtbf_timebuf[0]) {
                mtbf_timebuf[10] = ' ';
                if (parse_possibly_bad_iso_time(mtbf_timebuf, &start_of_run) <
                    0)
        ...
        if (have_wfu) {
            if (wfu_timebuf[0]) {
                wfu_timebuf[10] = ' ';
                if (parse_possibly_bad_iso_time(wfu_timebuf,
                                                &start_of_downtime) < 0)
                    log_warn(LD_HIST, "Couldn't parse time %s",
                             escaped(wfu_timebuf));
            }
 ...
 */
struct bw_array_t {
    /** Observation array: Total number of bytes transferred in each of the
     * last
...
add_obs(bw_array_t *b, time_t when, uint64_t n)
{
    ...
    while (when > b->cur_obs_time) {
        /* Doing this one second at a time is potentially inefficient, if we
           start with a state file that is very old.  Fortunately, it doesn't
           seem to show up in profiles, so we can just ignore it for now. */
        advance_obs(b);
    }
...
rep_hist_fill_bandwidth_history(char *buf, size_t len, const bw_array_t *b)
{
    ...
    if (b->num_maxes_set <= b->next_max_idx) {
        /* We haven't been through the circular array yet; time starts at
         * i=0.*/
        i = 0;
    } else {
    ...
    if (options->RelayBandwidthRate) {
        /* We don't want to report that we used more bandwidth than the max
         * we're willing to relay; otherwise everybody will know how much
         * traffic we used ourself. */
                                     ...
                                     int *s_interval)
{
    ...
    if (!server_mode(get_options())) {
        ...
        if (*s_begins != 0 || *s_interval != 900) {
            ...
            time_t save_at =
                get_options()->AvoidDiskWrites ? now + 3600 : now + 600;
    ...
    if (s_values && s_begins >= now - NUM_SECS_BW_SUM_INTERVAL * NUM_TOTALS) {
        ...
        SMARTLIST_FOREACH_BEGIN (s_values, const char *, cp) {
            ...
            if (start < now) {
                ...
                /* This is potentially inefficient, but since we don't do it
                 * very often it should be ok. */
...
rep_hist_format_exit_stats(time_t now)
{
     ...
     *        volume(j) > top_bytes[k] AND j is not in
     * top_ports[0..top_elements]
     *
    ...
    for (j = 0; j < top_elements; j++) {
        ...
        if (exit_bytes_written[cur_port] > 0) {
            ...
            num /= 1024;
            smartlist_add_asprintf(written_strings, "%d=%" PRIu64, cur_port,
                                   (num));
        ...
        if (exit_bytes_read[cur_port] > 0) {
            ...
            num /= 1024;
            smartlist_add_asprintf(read_strings, "%d=%" PRIu64, cur_port,
                                   (num));
    ...
    other_written = round_uint64_to_next_multiple_of(
        other_written, EXIT_STATS_ROUND_UP_BYTES);
    other_written /= 1024;
...
rep_hist_exit_stats_write(time_t now)
{
    ...
    if (!check_or_create_data_subdir("stats")) {
        write_to_data_subdir("stats", "exit-stats", str,
                             "exit port statistics");
    }
...
rep_hist_buffer_stats_add_circ(circuit_t *circ, time_t end_of_interval)
{
    ...
    rep_hist_add_buffer_stats(mean_num_cells_in_queue,
                              mean_time_cells_in_queue, processed_cells);
}
...
rep_hist_buffer_stats_write(time_t now)
{
    ...
    /* Try to write to disk. */
    if (!check_or_create_data_subdir("stats")) {
        write_to_data_subdir("stats", "buffer-stats", str,
                             "buffer statistics");
    }
...
rep_hist_conn_stats_write(time_t now)
{
    ...
    /* Try to write to disk. */
    if (!check_or_create_data_subdir("stats")) {
        write_to_data_subdir("stats", "conn-stats", str,
                             "connection statistics");
    }
