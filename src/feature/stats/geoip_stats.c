...
clientmap_entry_hash(const clientmap_entry_t *a)
{
    ...
    if (a->transport_name)
        h +=
            (unsigned)siphash24g(a->transport_name, strlen(a->transport_name));
...
clientmap_entry_free_(clientmap_entry_t *ent)
{
    ...
    /* This entry is about to be freed so pass it to the DoS subsystem to see
     * if any actions can be taken about it. */
                       ...
                       const char *transport_name, time_t now)
{
    ...
    if (action == GEOIP_CLIENT_CONNECT) {
        /* Only remember statistics if the DoS mitigation subsystem is enabled.
         * If not, only if as entry guard or as bridge. */
        if (!dos_enabled()) {
...
{
    clientmap_HT_FOREACH_FN(&client_history, remove_old_client_helper_,
                            &cutoff);
}
...
geoip_client_cache_handle_oom(time_t now, size_t min_remove_bytes)
{
    ...
    /* Set k to the initial cutoff of an entry. We then going to move it by
     * step to try to remove as much as we can. */
    ...
    do {
         ...
         * mitigation subsystem if by just filling the geoip cache, it was
         * enough to trigger the OOM and clean every single entries. */
...
geoip_get_transport_history(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (transports_used, const char *, transport_name) {
        void *transport_count_ptr =
            strmap_get(transport_counts, transport_name);
        uintptr_t transport_count = (uintptr_t)transport_count_ptr;

        log_debug(LD_GENERAL,
                  "We got %" PRIu64 " clients with transport '%s'.",
    ...
    log_debug(LD_GENERAL, "Final bridge-ip-transports string: '%s'",
              the_string);
...
#define MIN_DIR_REQ_RESPONSES 16
    if (complete >= MIN_DIR_REQ_RESPONSES) {
        ...
        SMARTLIST_FOREACH_BEGIN (dirreq_completed, dirreq_map_entry_t *, ent) {
            ...
            bytes_per_second =
                (uint32_t)(1000 * ent->response_size / time_diff_);
...
geoip_get_request_history(void)
{
    ...
    entries = smartlist_new();
    SMARTLIST_FOREACH_BEGIN (geoip_get_countries(), const geoip_country_t *,
                             c) {
        uint32_t tot = 0;
        c_hist_t *ent;
...
geoip_dirreq_stats_write(time_t now)
{
    ...
    if (!check_or_create_data_subdir("stats")) {
        write_to_data_subdir("stats", "dirreq-stats", str,
                             "dirreq statistics");
...
validate_bridge_stats(const char *stats_str, time_t now)
{
    ...
    if (!tmp) {
        ...
        tmp =
            find_str_at_start_of_line(stats_str, BRIDGE_TRANSPORTS_EMPTY_LINE);
        if (!tmp)
            return 0;
    }
