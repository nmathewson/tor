...
increment_v3_ns_request(country_t country)
{
    ...
    if ((size_t)country >= n_v3_ns_requests_len) {
        ...
        n_v3_ns_requests = tor_reallocarray(n_v3_ns_requests, new_len, sizeof(uint32_t));
...
clientmap_entries_eq(const clientmap_entry_t *a, const clientmap_entry_t *b)
{
    ...
    return !tor_addr_compare(&a->addr, &b->addr, CMP_EXACT) && a->action == b->action;
}

HT_PROTOTYPE(clientmap, clientmap_entry_t, node, clientmap_entry_hash, clientmap_entries_eq);
HT_GENERATE2(clientmap, clientmap_entry_t, node, clientmap_entry_hash, clientmap_entries_eq, 0.6,
...
{
    tor_assert(ent);
    return (sizeof(clientmap_entry_t) + (ent->transport_name ? strlen(ent->transport_name) : 0));
}
                    ...
                    const char *transport_name)
{
    ...
    tor_assert(action == GEOIP_CLIENT_CONNECT || action == GEOIP_CLIENT_NETWORKSTATUS);
    tor_assert(addr);
...
HT_GENERATE2(dirreqmap, dirreq_map_entry_t, node, dirreq_map_ent_hash, dirreq_map_ent_eq, 0.6,
             tor_reallocarray_, tor_free_);
...
static void
dirreq_map_put_(dirreq_map_entry_t *entry, dirreq_type_t type, uint64_t dirreq_id)
{
...
void
geoip_start_dirreq(uint64_t dirreq_id, size_t response_size, dirreq_type_t type)
{
...
void
geoip_change_dirreq_state(uint64_t dirreq_id, dirreq_type_t type, dirreq_state_t new_state)
{
    dirreq_map_entry_t *ent;
    if (!get_options()->DirReqStatistics)
        return;
    ...
    if (new_state - 1 != ent->state)
        return;
    ent->state = new_state;
    if ((type == DIRREQ_DIRECT && new_state == DIRREQ_FLUSHING_DIR_CONN_FINISHED) ||
        (type == DIRREQ_TUNNELED && new_state == DIRREQ_CHANNEL_BUFFER_FLUSHED)) {
...
geoip_get_transport_history(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (transports_used, const char *, transport_name) {
        ...
        smartlist_add_asprintf(
            string_chunks, "%s=%" PRIu64, transport_name,
            (round_uint64_to_next_multiple_of((uint64_t)transport_count, granularity)));
...
#define MIN_DIR_REQ_RESPONSES 16
    if (complete >= MIN_DIR_REQ_RESPONSES) {
        ...
        SMARTLIST_FOREACH_BEGIN (dirreq_completed, dirreq_map_entry_t *, ent) {
            ...
            uint32_t time_diff_ = (uint32_t)tv_mdiff(&ent->request_time, &ent->completion_time);
            if (time_diff_ == 0)
        ...
        buf_add_printf(buf,
                       ...
                       dltimes[0], dltimes[1 * complete / 10 - 1], dltimes[2 * complete / 10 - 1],
                       dltimes[1 * complete / 4 - 1], dltimes[3 * complete / 10 - 1],
                       dltimes[4 * complete / 10 - 1], dltimes[5 * complete / 10 - 1],
                       dltimes[6 * complete / 10 - 1], dltimes[7 * complete / 10 - 1],
                       dltimes[3 * complete / 4 - 1], dltimes[8 * complete / 10 - 1],
                       dltimes[9 * complete / 10 - 1], dltimes[complete - 1]);
        tor_free(dltimes);
    }

    result = buf_extract(buf, NULL);

    SMARTLIST_FOREACH(dirreq_completed, dirreq_map_entry_t *, ent, tor_free(ent));
...
int
geoip_get_client_history(geoip_client_action_t action, char **country_str, char **ipver_str)
{
    unsigned granularity = IP_GRANULARITY;
    ...
    if (ipver_str) {
        ...
        smartlist_add_asprintf(chunks, "v4=%u", round_to_next_multiple_of(ipv4_count, granularity));
        smartlist_add_asprintf(chunks, "v6=%u", round_to_next_multiple_of(ipv6_count, granularity));
...
#define RESPONSE_GRANULARITY 8
    for (i = 0; i < GEOIP_NS_RESPONSE_NUM; i++) {
        ns_v3_responses[i] =
            round_uint32_to_next_multiple_of(ns_v3_responses[i], RESPONSE_GRANULARITY);
    }
#undef RESPONSE_GRANULARITY
    ...
    tor_asprintf(&result,
                 ...
                 v3_ips_string ? v3_ips_string : "", v3_reqs_string ? v3_reqs_string : "",
                 ns_v3_responses[GEOIP_SUCCESS], ns_v3_responses[GEOIP_REJECT_NOT_ENOUGH_SIGS],
                 ns_v3_responses[GEOIP_REJECT_UNAVAILABLE], ns_v3_responses[GEOIP_REJECT_NOT_FOUND],
                 ns_v3_responses[GEOIP_REJECT_NOT_MODIFIED], ns_v3_responses[GEOIP_REJECT_BUSY],
...
validate_bridge_stats(const char *stats_str, time_t now)
{
    ...
    if (parse_iso_time(stats_end_str, &stats_end_time) < 0)
        return 0;
    if (stats_end_time < now - (25 * 60 * 60) || stats_end_time > now + (1 * 60 * 60))
        return 0;
...
geoip_format_bridge_stats(time_t now)
{
    ...
    tor_asprintf(&out,
                 ...
                 written, duration, country_data ? country_data : "", ipver_data ? ipver_data : "",
...
format_bridge_stats_controller(time_t now)
{
    ...
    tor_asprintf(&out, "TimeStarted=\"%s\" CountrySummary=%s IPVersions=%s", started,
                 country_data ? country_data : "", ipver_data ? ipver_data : "");
    tor_free(country_data);
    tor_free(ipver_data);
    return out;
}
...
geoip_bridge_stats_write(time_t now)
{
    ...
    /* Write it to disk. */
    if (!check_or_create_data_subdir("stats")) {
        write_to_data_subdir("stats", "bridge-stats", bridge_stats_extrainfo, "bridge statistics");
...
geoip_format_entry_stats(time_t now)
{
    ...
    tor_asprintf(&result,
                 ...
                 "entry-ips %s\n",
                 t, (unsigned)(now - start_of_entry_stats_interval), data ? data : "");
    tor_free(data);
    return result;
}
