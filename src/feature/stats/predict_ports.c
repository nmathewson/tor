...
add_predicted_port(time_t now, uint16_t port)
{
    ...
    if (!any_predicted_circuits(now)) {
        prediction_timeout = (time_t)channelpadding_get_circuits_available_timeout();
    }
...
rep_hist_remove_predicted_ports(const smartlist_t *rmv_ports)
{
    ...
    SMARTLIST_FOREACH(rmv_ports, const uint16_t *, p, bitarray_set(remove_ports, *p));
    SMARTLIST_FOREACH_BEGIN (predicted_ports_list, predicted_port_t *, pp) {
...
rep_hist_circbuilding_dormant(time_t now)
{
    ...
    if (server_mode(options) &&
        (!check_whether_orport_reachable(options) || !circuit_enough_testing_circs()))
        return 0;
...
predicted_ports_free_all(void)
{
    if (!predicted_ports_list)
        return;
    predicted_ports_total_alloc -= smartlist_len(predicted_ports_list) * sizeof(predicted_port_t);
