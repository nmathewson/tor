            ...
            if (generate) {
                if (!have_lockfile()) {
                    if (try_locking(get_options(), 0) < 0) {
                        /* Make sure that --list-fingerprint only creates new
                         * keys if there is no possibility for a deadlock. */
                        tor_log(severity, LD_FS,
                                "Another Tor process has locked \"%s\". "
                                "Not writing any new keys.",
                                fname);
                        /*XXXX The 'other process' might make a key in a second
                         * or two; maybe we should wait for it. */
                        goto error;
                    }
                }
                log_info(LD_GENERAL,
                      ...
                      const or_options_t *options)
{
    ...
    if (try_to_load && (!have_secret || split)) {
        ...
        if (found_public) {
             ...
             * public key file! */
            if (have_secret) {
                if (ed25519_pubkey_write_to_file(&keypair->pubkey, public_fname,
                                                 tag) < 0) {
        ...
        if (have_encrypted_secret_file) {
            ...
            tor_log(
                ...
                (flags & INIT_ED_KEY_SUGGEST_KEYGEN)
                    ? "If you're keeping your master secret key offline, you "
                      "will "
                      "need to run 'tor --keygen' to generate new signing keys."
                    : "Did you forget to copy it over when you copied the rest "
                      "of the "
