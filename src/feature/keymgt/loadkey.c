            ...
            if (!have_lockfile()) {
                if (try_locking(get_options(), 0) < 0) {
                    ...
                    tor_log(severity, LD_FS,
                            ...
                            fname);
                    /*XXXX The 'other process' might make a key in a second or
                     * two; maybe we should wait for it. */
                    goto error;
                }
            }
            log_info(LD_GENERAL,
...
read_encrypted_secret_key(ed25519_secret_key_t *out, const char *fname)
{
    ...
    while (1) {
        ...
        /* Otherwise, passphrase is bad, so try again till user does ctrl-c or
         * gets it right. */
    }
...
write_encrypted_secret_key(const ed25519_secret_key_t *key, const char *fname)
{
    ...
    if (crypto_write_tagged_contents_to_file(fname, ENC_KEY_HEADER,
                                             ENC_KEY_TAG, encrypted_key,
                                             encrypted_len) < 0)
        goto done;
    r = 1;
done:
                      ...
                      const or_options_t *options)
{
    ...
    if (explicit_fname) {
        ...
        tor_asprintf(&encrypted_secret_fname, "%s_secret_key_encrypted",
                     fname);
    }
    ...
    if (have_secret) {
        ...
        if (ed25519_public_key_generate(&keypair->pubkey, &keypair->seckey) <
            0) {
    ...
    if (try_to_load && (!have_secret || split)) {
        ...
        if (found_public) {
            if (have_secret) {
                ...
                if (!ed25519_pubkey_eq(&keypair->pubkey, &pubkey_tmp)) {
                    tor_log(severity, LD_OR,
                            ...
                            "to restore from backup, make sure you didn't mix "
                            "up the "
                            "key files. If you are absolutely sure that %s is "
                            "the right "
                            "key for this relay, delete %s or move it out of "
                            "the way.",
            ...
            /* We have no public key file, but we do have a secret key, make
             * the public key file! */
            if (have_secret) {
                if (ed25519_pubkey_write_to_file(&keypair->pubkey,
                                                 public_fname, tag) < 0) {
                    tor_log(severity, LD_OR, "Couldn't repair %s",
                            public_fname);
                    goto err;
                } else {
        ...
        if (have_encrypted_secret_file) {
            ...
            tor_log(severity, LD_OR,
                    ...
                    (flags & INIT_ED_KEY_SUGGEST_KEYGEN)
                        ? "If you're keeping your master secret key offline, "
                          "you will "
                          "need to run 'tor --keygen' to generate new signing "
                          "keys."
                        : "Did you forget to copy it over when you copied the "
                          "rest of the "
    ...
    if (!have_secret && !found_public) {
        tor_free(keypair);
        keypair =
            ed_key_new(signing_key, flags, now, lifetime, cert_type, &cert);
        if (!keypair) {
    ...
    if (created_sk ||
        ...
        if (write_secret_key(&keypair->seckey, encrypt_key, secret_fname, tag,
            ...
            (split && ed25519_pubkey_write_to_file(&keypair->pubkey,
                                                   public_fname, tag) < 0) ||
           ...
           time_t lifetime, uint8_t cert_type, struct tor_cert_st **cert_out)
{
    ...
    if (flags & INIT_ED_KEY_INCLUDE_SIGNING_KEY_IN_CERT)
        cert_flags |= CERT_FLAG_INCLUDE_SIGNING_KEY;
    tor_cert_t *cert = tor_cert_create(
        signing_key, cert_type, &keypair->pubkey, now, lifetime, cert_flags);
    if (!cert)
        goto err;
