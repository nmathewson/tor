...
bool
rend_cache_intro_failure_exists(const char *service_id, const uint8_t *intro_identity)
{
...
rend_cache_failure_clean(time_t now)
{
    ...
    STRMAP_FOREACH_MODIFY(rend_cache_failure, key, rend_cache_failure_t *, ent)
    {
        ...
        DIGESTMAP_FOREACH_MODIFY (ent->intro_failures, ip_key, rend_cache_failure_intro_t *,
                                  ip_ent) {
...
{
    rend_cache_failure_intro_t *ent_dup = rend_cache_failure_intro_entry_new(entry->failure_type);
                        ...
                        rend_intro_point_failure_t failure)
{
    ...
    old_entry = digestmap_set(fail_entry->intro_failures, (char *)identity, entry);
...
STATIC void
validate_intro_point_failure(const rend_service_descriptor_t *desc, const char *service_id)
{
...
void
rend_cache_intro_failure_note(rend_intro_point_failure_t failure, const uint8_t *identity,
...
rend_cache_lookup_v2_desc_as_dir(const char *desc_id, const char **desc)
{
    ...
    if (base32_decode(desc_id_digest, DIGEST_LEN, desc_id, REND_DESC_ID_V2_LEN_BASE32) !=
        DIGEST_LEN) {
...
rend_cache_store_v2_desc_as_dir(const char *desc)
{
    ...
    tor_assert(rend_cache_v2_dir);
    tor_assert(desc);
    while (rend_parse_v2_service_descriptor(&parsed, desc_id, &intro_content, &intro_size,
                                            &encoded_size, &next_desc, current_desc, 1) >= 0) {
        number_parsed++;
    ...
    log_info(LD_REND, "Parsed %d and added %d descriptor%s.", number_parsed, number_stored,
             number_stored != 1 ? "s" : "");
    return 0;
}
...
rend_cache_store_v2_desc_as_service(const char *desc)
{
    ...
    if (rend_parse_v2_service_descriptor(&parsed, desc_id, &intro_content, &intro_size,
                                         &encoded_size, &next_desc, desc, 0) < 0) {
    ...
    e = (rend_cache_entry_t *)strmap_get_lc(rend_cache_local_service, service_id);
    if (e && e->parsed->timestamp > parsed->timestamp) {
        log_info(LD_REND,
    ...
    e->len = encoded_size;
    rend_cache_increment_allocation(rend_cache_entry_allocation(e));
    log_debug(LD_REND, "Successfully stored rend desc '%s', len %d.", safe_str_client(service_id),
              (int)encoded_size);
    return 0;
 ...
 */
int
rend_cache_store_v2_desc_as_client(const char *desc, const char *desc_id_base32,
                                   const rend_data_t *rend_query, rend_cache_entry_t **entry)
{
    ...
    if (base32_decode(want_desc_id, sizeof(want_desc_id), desc_id_base32, strlen(desc_id_base32)) !=
        sizeof(want_desc_id)) {
    ...
    if (rend_data->onion_address[0] != '\0' && strcmp(rend_data->onion_address, service_id)) {
        log_warn(LD_REND,
    ...
    if (intro_content && intro_size > 0) {
        ...
        if (rend_data->auth_type != REND_NO_AUTH &&
            !safe_mem_is_zero(rend_data->descriptor_cookie, sizeof(rend_data->descriptor_cookie))) {
            char *ipos_decrypted = NULL;
            size_t ipos_decrypted_size;
            if (rend_decrypt_introduction_points(&ipos_decrypted, &ipos_decrypted_size,
                                                 rend_data->descriptor_cookie, intro_content,
                                                 intro_size) < 0) {
        ...
        n_intro_points = rend_parse_introduction_points(parsed, intro_content, intro_size);
        if (n_intro_points <= 0) {
