                                 ...
                                 int as_hsdir)
{
    ...
    *encoded_size_out = eos - desc;
    ...
    for (i = 0; i < smartlist_len(versions); i++) {
        version = (int)tor_parse_long(smartlist_get(versions, i), 10, 0,
                                      INT_MAX, &num_ok, NULL);
    ...
    if (ipos_encrypted[0] == (int)REND_BASIC_AUTH) {
        ...
        for (pos = 2; pos < 2 + client_entries_len;
            ...
            if (tor_memeq(ipos_encrypted + pos, client_id,
                ...
                len = ipos_encrypted_size - 2 - client_entries_len -
                      CIPHER_IV_LEN;
                               ...
                               size_t intro_points_encoded_size)
{
    ...
    while (!fast_memcmpstart(current_ipo, end_of_intro_points - current_ipo,
        ...
        memarea_clear(area);
        /* Tokenize string. */
        if (tokenize_string(area, current_ipo, eos, tokens, ipo_token_table,
                            0)) {
...
rend_parse_client_keys(strmap_t *parsed_clients, const char *ckstr)
{
    ...
    while (!strcmpstart(current_entry, "client-name ")) {
        ...
        tor_assert(tok->n_args == 1);
        if (rend_auth_decode_cookie(tok->args[0],
                                    parsed_entry->descriptor_cookie, NULL,
                                    &err_msg) < 0) {
            tor_assert(err_msg);
