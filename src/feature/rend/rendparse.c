...
static token_rule_t desc_token_table[] = {
    T1_START("rendezvous-service-descriptor", R_RENDEZVOUS_SERVICE_DESCRIPTOR, EQ(1), NO_OBJ),
 ...
 */
int
rend_parse_v2_service_descriptor(rend_service_descriptor_t **parsed_out, char *desc_id_out,
                                 char **intro_points_encrypted_out,
                                 size_t *intro_points_encrypted_size_out, size_t *encoded_size_out,
                                 const char **next_out, const char *desc, int as_hsdir)
{
    ...
    *encoded_size_out = eos - desc;
    ...
    if (base32_decode(desc_id_out, DIGEST_LEN, tok->args[0], REND_DESC_ID_V2_LEN_BASE32) !=
        DIGEST_LEN) {
        log_warn(LD_REND, "Descriptor ID has wrong length or illegal characters: %s", tok->args[0]);
        goto err;
    }
    ...
    if (base32_decode(secret_id_part, DIGEST_LEN, tok->args[0], 32) != DIGEST_LEN) {
        log_warn(LD_REND, "Secret ID part has wrong length or illegal characters: %s",
                 tok->args[0]);
        goto err;
    }
    ...
    tor_assert(tok->n_args == 1);
    if (parse_iso_time_(tok->args[0], &result->timestamp, strict_time_fmt, 0) < 0) {
...
int
rend_decrypt_introduction_points(char **ipos_decrypted, size_t *ipos_decrypted_size,
                                 const char *descriptor_cookie, const char *ipos_encrypted,
                                 size_t ipos_encrypted_size)
{
    ...
    if (ipos_encrypted[0] == (int)REND_BASIC_AUTH) {
        ...
        for (pos = 2; pos < 2 + client_entries_len; pos += REND_BASIC_AUTH_CLIENT_ENTRY_LEN) {
            if (tor_memeq(ipos_encrypted + pos, client_id, REND_BASIC_AUTH_CLIENT_ID_LEN)) {
                ...
                if (crypto_cipher_decrypt(cipher, session_key,
                                          ipos_encrypted + pos + REND_BASIC_AUTH_CLIENT_ID_LEN,
                                          CIPHER_KEY_LEN) < 0) {
        ...
        declen = crypto_cipher_decrypt_with_iv(descriptor_cookie, dec,
                                               ipos_encrypted_size - CIPHER_IV_LEN - 1,
                               ...
                               size_t intro_points_encoded_size)
{
    ...
    while (
        !fast_memcmpstart(current_ipo, end_of_intro_points - current_ipo, "introduction-point ")) {
        /* Determine end of string. */
        const char *eos =
        ...
        if (base32_decode(info->identity_digest, DIGEST_LEN, tok->args[0],
            ...
            log_warn(LD_REND, "Identity digest has wrong length or illegal characters: %s",
                     tok->args[0]);
        ...
        info->nickname[0] = '$';
        base16_encode(info->nickname + 1, sizeof(info->nickname) - 1, info->identity_digest,
                      DIGEST_LEN);
        /* Parse IP address. */
...
rend_parse_client_keys(strmap_t *parsed_clients, const char *ckstr)
{
    ...
    while (!strcmpstart(current_entry, "client-name ")) {
        ...
        tor_assert(tok->n_args == 1);
        if (rend_auth_decode_cookie(tok->args[0], parsed_entry->descriptor_cookie, NULL, &err_msg) <
            0) {
            tor_assert(err_msg);
