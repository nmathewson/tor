                        ...
                        uint8_t replica)
{
    ...
    rend_get_descriptor_id_bytes(desc_id_out, service_id_binary,
                                 secret_id_part);
    return 0;
}
...
rend_encode_v2_intro_points(char **encoded, rend_service_descriptor_t *desc)
{
    ...
    for (i = 0; i < smartlist_len(desc->intro_nodes); i++) {
        ...
        res = tor_snprintf(unenc + unenc_written, unenc_len - unenc_written,
                           ...
                           "onion-key\n%s"
                           "service-key\n%s",
                           id_base32, address, info->port, onion_key,
                           service_key);
        tor_free(address);
        tor_free(onion_key);
...
rend_encode_v2_descriptors(smartlist_t *descs_out,
                           ...
                           crypto_pk_t *client_key, smartlist_t *client_cookies)
{
    ...
    if (smartlist_len(desc->intro_nodes) > 0) {
        ...
        switch (auth_type) {
            ...
            case REND_BASIC_AUTH:
                if (rend_encrypt_v2_intro_points_basic(
                        &ipos_encrypted, &ipos_encrypted_len, ipos,
                        client_cookies) < 0) {
                    log_warn(LD_REND,
            ...
            case REND_STEALTH_AUTH:
                if (rend_encrypt_v2_intro_points_stealth(
                        &ipos_encrypted, &ipos_encrypted_len, ipos,
                        descriptor_cookie) < 0) {
                    log_warn(LD_REND,
                             "Encrypting of introduction points did not "
    ...
    for (k = 0; k < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; k++) {
        ...
        for (i = 0; i < 8; i++) {
            if (desc->protocols & 1 << i) {
                tor_snprintf(protocol_versions_string +
        ...
        desc_len =
            2000 + smartlist_len(desc->intro_nodes) * 1000; /* far too long,
                                                               but okay.*/
        enc->desc_str = desc_str = tor_malloc_zero(desc_len);
        result =
            tor_snprintf(desc_str, desc_len,
...
rend_get_service_id(crypto_pk_t *pk, char *out)
{
    ...
    if (crypto_pk_get_digest(pk, buf) < 0)
        return -1;
    base32_encode(out, REND_SERVICE_ID_LEN_BASE32 + 1, buf,
                  REND_SERVICE_ID_LEN);
    return 0;
}
                        ...
                        int command, size_t length, const uint8_t *payload)
{
    ...
    switch (command) {
        ...
        case RELAY_COMMAND_INTRODUCE_ACK:
            if (origin_circ)
                r = hs_client_receive_introduce_ack(origin_circ, payload,
                                                    length);
            break;
        ...
        case RELAY_COMMAND_INTRO_ESTABLISHED:
            if (origin_circ)
                r = hs_service_receive_intro_established(origin_circ, payload,
                                                         length);
            break;
        case RELAY_COMMAND_RENDEZVOUS_ESTABLISHED:
            if (origin_circ)
                r = hs_client_receive_rendezvous_acked(origin_circ, payload,
                                                       length);
            break;
        default:
            tor_fragile_assert();
                        ...
                        rend_auth_type_t *auth_type_out, char **err_msg_out)
{
    ...
    if (auth_type_out) {
        auth_type_val =
            (descriptor_cookie_decoded[REND_DESC_COOKIE_LEN] >> 4) + 1;
        ...
        *auth_type_out =
            auth_type_val == 1 ? REND_BASIC_AUTH : REND_STEALTH_AUTH;
    }
...
err:
    ...
    memwipe(descriptor_cookie_base64ext, 0,
            sizeof(descriptor_cookie_base64ext));
    return res;
}
