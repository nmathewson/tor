...
rend_encode_v2_intro_points(char **encoded, rend_service_descriptor_t *desc)
{
    ...
    unenc_len =
        smartlist_len(desc->intro_nodes) * 1000; /* too long, but ok. */
                           ...
                           smartlist_t *client_cookies)
{
    ...
    for (k = 0; k < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; k++) {
        ...
        get_secret_id_part_bytes(secret_id_part, time_period,
                                 descriptor_cookie, k);
                        ...
                        int command, size_t length, const uint8_t *payload)
{
    ...
    if (r == -2)
        log_info(LD_PROTOCOL,
                 "Dropping cell (type %d) for wrong circuit type.", command);
}
