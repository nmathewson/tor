...
static void
get_secret_id_part_bytes(char *secret_id_part, uint32_t time_period, const char *descriptor_cookie,
                         uint8_t replica)
{
    crypto_digest_t *digest = crypto_digest_new();
...
get_time_period(time_t now, uint8_t deviation, const char *service_id)
{
    ...
    return (uint32_t)(now + ((uint8_t)*service_id) * REND_TIME_PERIOD_V2_DESC_VALIDITY / 256) /
...
get_seconds_valid(time_t now, const char *service_id)
{
    uint32_t result =
        REND_TIME_PERIOD_V2_DESC_VALIDITY -
        ((uint32_t)(now + ((uint8_t)*service_id) * REND_TIME_PERIOD_V2_DESC_VALIDITY / 256) %
         REND_TIME_PERIOD_V2_DESC_VALIDITY);
    return result;
}
...
int
rend_compute_v2_desc_id(char *desc_id_out, const char *service_id, const char *descriptor_cookie,
                        time_t now, uint8_t replica)
{
    ...
    get_secret_id_part_bytes(secret_id_part, time_period, descriptor_cookie, replica);
...
rend_encode_v2_intro_points(char **encoded, rend_service_descriptor_t *desc)
{
    ...
    for (i = 0; i < smartlist_len(desc->intro_nodes); i++) {
        ...
        if (!intro_key ||
            crypto_pk_write_public_key_to_string(intro_key, &service_key, &service_key_len) < 0) {
...
static int
rend_encrypt_v2_intro_points_basic(char **encrypted_out, size_t *encrypted_len_out,
                                   const char *encoded, smartlist_t *client_cookies)
{
    ...
    client_entries_len =
        client_blocks * REND_BASIC_AUTH_CLIENT_MULTIPLE * REND_BASIC_AUTH_CLIENT_ENTRY_LEN;
    ...
    enclen =
        crypto_cipher_encrypt_with_iv(session_key, enc + 2 + client_entries_len,
    ...
    SMARTLIST_FOREACH_BEGIN (client_cookies, const char *, cookie) {
        ...
        if (crypto_cipher_encrypt(cipher, client_part + REND_BASIC_AUTH_CLIENT_ID_LEN, session_key,
                                  CIPHER_KEY_LEN) < 0) {
    ...
    for (i = (smartlist_len(client_cookies) - 1) % REND_BASIC_AUTH_CLIENT_MULTIPLE;
...
static int
rend_encrypt_v2_intro_points_stealth(char **encrypted_out, size_t *encrypted_len_out,
                                     const char *encoded, const char *descriptor_cookie)
{
    int r = -1, enclen;
    char *enc;
    tor_assert(encoded);
    ...
    enclen = crypto_cipher_encrypt_with_iv(
        descriptor_cookie, enc + 1, CIPHER_IV_LEN + strlen(encoded), encoded, strlen(encoded));
    if (enclen < 0) {
        log_warn(LD_REND, "Could not encrypt introduction point string.");
        goto done;
    }
...
rend_desc_v2_is_parsable(rend_encoded_v2_service_descriptor_t *desc)
{
    ...
    int res = rend_parse_v2_service_descriptor(&test_parsed, test_desc_id, &test_intro_content,
                                               &test_intro_size, &test_encoded_size, &test_next,
                                               desc->desc_str, 1);
...
void
rend_encoded_v2_service_descriptor_free_(rend_encoded_v2_service_descriptor_t *desc)
{
    if (!desc)
        return;
...
int
rend_encode_v2_descriptors(smartlist_t *descs_out, rend_service_descriptor_t *desc, time_t now,
                           uint8_t period, rend_auth_type_t auth_type, crypto_pk_t *client_key,
                           smartlist_t *client_cookies)
{
    ...
    if (smartlist_len(desc->intro_nodes) > 0) {
        ...
        case REND_BASIC_AUTH:
            if (rend_encrypt_v2_intro_points_basic(&ipos_encrypted, &ipos_encrypted_len, ipos,
                                                   client_cookies) < 0) {
        ...
        case REND_STEALTH_AUTH:
            if (rend_encrypt_v2_intro_points_stealth(&ipos_encrypted, &ipos_encrypted_len, ipos,
                                                     descriptor_cookie) < 0) {
                log_warn(LD_REND, "Encrypting of introduction points did not "
                                  "succeed.");
                tor_free(ipos);
            ...
            break;
        default:
            log_warn(LD_REND | LD_BUG, "Unrecognized authorization type %d", (int)auth_type);
            tor_free(ipos);
            return -1;
        }
    ...
    for (k = 0; k < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; k++) {
        ...
        base32_encode(secret_id_part_base32, sizeof(secret_id_part_base32), secret_id_part,
                      DIGEST_LEN);
        ...
        if (crypto_pk_write_public_key_to_string(service_key, &permanent_key, &permanent_key_len) <
            0) {
        ...
        result = tor_snprintf(desc_str, desc_len,
                              ...
                              "publication-time %s\n"
                              "protocol-versions %s\n",
                              desc_id_base32, permanent_key, secret_id_part_base32, published,
        ...
        if (router_append_dirobj_signature(desc_str + written, desc_len - written, desc_digest,
                                           DIGEST_LEN, service_key) < 0) {
...
void
rend_process_relay_cell(circuit_t *circ, const crypt_path_t *layer_hint, int command, size_t length,
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        ...
        if (!layer_hint || layer_hint != origin_circ->cpath->prev) {
            log_fn(LOG_PROTOCOL_WARN, LD_APP,
                   "Relay cell (rend purpose %d) from wrong hop on origin circ", command);
            origin_circ = NULL;
        }
    } else {
 ...
 */
int
hid_serv_get_responsible_directories(smartlist_t *responsible_dirs, const char *id)
{
...
rend_auth_encode_cookie(const uint8_t *cookie_in, rend_auth_type_t auth_type)
{
    ...
    re = base64_encode(cookie_out, REND_DESC_COOKIE_LEN_EXT_BASE64 + 1,
                       (const char *)extended_cookie, REND_DESC_COOKIE_LEN_EXT, 0);
 ...
 */
int
rend_auth_decode_cookie(const char *cookie_in, uint8_t *cookie_out, rend_auth_type_t *auth_type_out,
                        char **err_msg_out)
{
    ...
    if (len == REND_DESC_COOKIE_LEN_BASE64) {
        ...
        tor_snprintf(descriptor_cookie_base64ext, sizeof(descriptor_cookie_base64ext),
                     "%sA=", descriptor_cookie);
        descriptor_cookie = descriptor_cookie_base64ext;
    } else if (len != REND_DESC_COOKIE_LEN_EXT_BASE64) {
        tor_asprintf(&err_msg, "Authorization cookie has wrong length: %s", escaped(cookie_in));
        goto err;
    }

    decoded_len =
        base64_decode((char *)descriptor_cookie_decoded, sizeof(descriptor_cookie_decoded),
                      descriptor_cookie, REND_DESC_COOKIE_LEN_EXT_BASE64);
    if (decoded_len != REND_DESC_COOKIE_LEN && decoded_len != REND_DESC_COOKIE_LEN_EXT) {
    ...
    if (auth_type_out) {
        ...
        if (auth_type_val < 1 || auth_type_val > 2) {
            tor_asprintf(&err_msg, "Authorization cookie type is unknown: %s", escaped(cookie_in));
            goto err;
        }
 ...
 */
void
assert_circ_anonymity_ok(const origin_circuit_t *circ, const or_options_t *options)
{
    tor_assert(options);
    tor_assert(circ);
...
rend_circuit_pk_digest_eq(const origin_circuit_t *ocirc, const uint8_t *digest)
{
    ...
    rend_pk_digest = rend_data_get_pk_digest(ocirc->rend_data, &rend_pk_digest_len);
