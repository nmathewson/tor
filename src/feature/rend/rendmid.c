                                ...
                                size_t request_len)
{
    ...
    hs_circuitmap_register_intro_circ_v2_relay_side(circ,
                                                    (uint8_t *)pk_digest);
                          ...
                          size_t request_len)
{
     ...
     */
    if (request_len <
        (DIGEST_LEN + (MAX_NICKNAME_LEN + 1) + REND_COOKIE_LEN +
         DH1024_KEY_LEN + CIPHER_KEY_LEN + PKCS1_OAEP_PADDING_OVERHEAD)) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
                    ...
                    size_t request_len)
{
    ...
    if (!rend_circ) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
               "Rejecting RENDEZVOUS1 cell with unrecognized rendezvous "
               "cookie %s.",
               hexid);
        reason = END_CIRC_REASON_TORPROTOCOL;
        goto err;
