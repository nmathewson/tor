 ...
 */
int
rend_mid_establish_intro_legacy(or_circuit_t *circ, const uint8_t *request, size_t request_len)
{
    crypto_pk_t *pk = NULL;
    ...
    if (tor_memneq(expected_digest, request + 2 + asn1len, DIGEST_LEN)) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Hash of session info was not as expected.");
        reason = END_CIRC_REASON_TORPROTOCOL;
        goto err;
    }
    ...
    c = NULL;
    while ((c = hs_circuitmap_get_intro_circ_v2_relay_side((const uint8_t *)pk_digest))) {
        log_info(LD_REND, "Replacing old circuit for service %s", safe_str(serviceid));
    ...
    log_info(LD_REND, "Established introduction point on circuit %u for service %s",
 ...
 */
int
rend_mid_introduce_legacy(or_circuit_t *circ, const uint8_t *request, size_t request_len)
{
    or_circuit_t *intro_circ;
    char serviceid[REND_SERVICE_ID_LEN_BASE32 + 1];
    char nak_body[1];

    log_info(LD_REND, "Received an INTRODUCE1 request on circuit %u", (unsigned)circ->p_circ_id);
     ...
     */
    if (request_len < (DIGEST_LEN + (MAX_NICKNAME_LEN + 1) + REND_COOKIE_LEN + DH1024_KEY_LEN +
     ...
     * PK. */
    intro_circ = hs_circuitmap_get_intro_circ_v2_relay_side((const uint8_t *)request);
    if (!intro_circ) {
        log_info(LD_REND,
    ...
    log_info(LD_REND,
             ...
             safe_str(serviceid), (unsigned)circ->p_circ_id, (unsigned)intro_circ->p_circ_id);

    /* Great.  Now we just relay the cell down the circuit. */
    if (relay_send_command_from_edge(0, TO_CIRCUIT(intro_circ), RELAY_COMMAND_INTRODUCE2,
    ...
    if (relay_send_command_from_edge(0, TO_CIRCUIT(circ), RELAY_COMMAND_INTRODUCE_ACK, NULL, 0,
                                     NULL)) {
...
err:
    ...
    nak_body[0] = 1;
    if (relay_send_command_from_edge(0, TO_CIRCUIT(circ), RELAY_COMMAND_INTRODUCE_ACK, nak_body, 1,
                                     NULL)) {
 ...
 */
int
rend_mid_establish_rendezvous(or_circuit_t *circ, const uint8_t *request, size_t request_len)
{
    char hexid[9];
    ...
    if (channel_is_client(circ->p_chan) && dos_should_refuse_single_hop_client()) {
    ...
    if (circ->base_.n_chan) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Tried to establish rendezvous on non-edge circuit");
        goto err;
    }

    if (request_len != REND_COOKIE_LEN) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Invalid length on ESTABLISH_RENDEZVOUS.");
        goto err;
    }
    ...
    /* Acknowledge the request. */
    if (relay_send_command_from_edge(0, TO_CIRCUIT(circ), RELAY_COMMAND_RENDEZVOUS_ESTABLISHED, "",
                                     0, NULL) < 0) {
 ...
 */
int
rend_mid_rendezvous(or_circuit_t *circ, const uint8_t *request, size_t request_len)
{
    ...
    if (request_len < REND_COOKIE_LEN) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
               "Rejecting RENDEZVOUS1 cell with bad length (%d) on circuit %u.", (int)request_len,
               (unsigned)circ->p_circ_id);
        reason = END_CIRC_REASON_TORPROTOCOL;
        goto err;
    }
    ...
    if (!rend_circ) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
               "Rejecting RENDEZVOUS1 cell with unrecognized rendezvous cookie %s.", hexid);
        reason = END_CIRC_REASON_TORPROTOCOL;
        goto err;
    }
    ...
    if (relay_send_command_from_edge(0, TO_CIRCUIT(rend_circ), RELAY_COMMAND_RENDEZVOUS2,
                                     (char *)(request + REND_COOKIE_LEN),
                                     request_len - REND_COOKIE_LEN, NULL)) {
        log_warn(LD_GENERAL, "Unable to send RENDEZVOUS2 cell to client on circuit %u.",
    ...
    /* Join the circuits. */
    log_info(LD_REND, "Completing rendezvous: circuit %u joins circuit %u (cookie %s)",
    ...
    circuit_change_purpose(TO_CIRCUIT(rend_circ), CIRCUIT_PURPOSE_REND_ESTABLISHED);
