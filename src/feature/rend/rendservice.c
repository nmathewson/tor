...
rend_service_prune_list_impl_(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN(new_service_list, rend_service_t *, new)
    {
        SMARTLIST_FOREACH_BEGIN(old_service_list, rend_service_t *, old)
        {
             ...
             * points from current services to newly configured one that already
             * exists. The same directory means it's the same service. */
                                                ...
                                                const or_options_t *options)
{
    ...
    switch (file_status(poison_fname)) {
        ...
        case FN_EMPTY: /* single onion poison file already exists. NOP. */
            ...
            break;
        case FN_NOENT:
            fd = tor_open_cloexec(poison_fname, O_RDWR | O_CREAT | O_TRUNC,
                                  0600);
            if (fd < 0) {
 ...
 * and 0 for failure. */
static int
rend_check_authorization(rend_service_t *service, const char *descriptor_cookie,
                         size_t cookie_len)
{
                                  ...
                                  const uint8_t *request, size_t request_len)
{
    ...
    if (crypto_dh_compute_secret(LOG_PROTOCOL_WARN, dh,
                                 (char *)(parsed_req->dh), DH1024_KEY_LEN, keys,
...
static ssize_t (*intro_version_handlers[])(rend_intro_cell_t *, const uint8_t *,
                                           size_t, char **) = {
    ...
    if (valid_ip_circuits > service->n_intro_points_wanted) {
        ...
        if (options->ExcludeNodes) {
            /* XXXX in some future version, we can test whether the transition
               is allowed or not given the actual nodes in the circuit.  But for
               now, this case, we might as well close the thing. */
...
rend_consider_services_intro_points(time_t now)
{
    ...
    SMARTLIST_FOREACH_BEGIN(rend_service_list, rend_service_t *, service)
    {
        ...
        if (intro_nodes_len == 0) {
             ...
             * and use the first n_intro_points_wanted that complete. See
             * proposal #155, section 4 for the rationale of this which is
             * purely for performance.
             *
             * The ones after the first n_intro_points_to_open will be converted
             * to 'general' internal circuits in
...
rend_consider_services_upload(time_t now)
{
    ...
    for (i = 0; i < smartlist_len(rend_service_list); ++i) {
        ...
        if (!service->next_upload_time) { /* never been uploaded yet */
             ...
             * that
             * the descriptor is stable before being published. See comment
             * below. */
