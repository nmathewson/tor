...
rend_get_service_list(const smartlist_t *substitute_service_list)
{
    ...
    return rend_get_service_list_mutable(
...
{
    return rend_service_is_ephemeral(s) ? "[EPHEMERAL]"
                                        : escaped(s->directory);
}
...
rend_service_prune_list_impl_(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (new_service_list, rend_service_t *, new) {
        SMARTLIST_FOREACH_BEGIN (old_service_list, rend_service_t *, old) {
            /* Skip ephemeral services as we only want to copy introduction
             * points from current services to newly configured one that
             * already exists.
             * The same directory means it's the same service. */
            if (rend_service_is_ephemeral(new) ||
                rend_service_is_ephemeral(old) ||
    ...
    while ((ocirc = circuit_get_next_intro_circ(ocirc, false))) {
        ...
        SMARTLIST_FOREACH_BEGIN (surviving_services, const rend_service_t *,
                                 s) {
        ...
        log_info(LD_REND, "Closing intro point %s for service %s.",
                 safe_str_client(
                     extend_info_describe(ocirc->build_state->chosen_exit)),
                           ...
                           hs_service_config_t *config)
{
    ...
    service->max_streams_per_circuit =
        (int)config->max_streams_per_rdv_circuit;
                    ...
                    hs_service_config_t *config)
{
    ...
    for (line = line_; line; line = line->next) {
        ...
        if (!strcasecmp(line->key, "HiddenServiceAuthorizeClient")) {
            /* Parse auth type and comma-separated list of client names and add
             * a rend_authorized_client_t for each client to the service's list
            ...
            if (smartlist_len(type_names_split) < 2) {
                log_warn(LD_CONFIG,
                         ...
                         service->auth_type == REND_BASIC_AUTH ? "basic"
                                                               : "stealth");
                SMARTLIST_FOREACH(type_names_split, char *, cp, tor_free(cp));
                smartlist_free(type_names_split);
                continue;
                           ...
                           smartlist_t *auth_clients, char **service_id_out)
{
    *service_id_out = NULL;
    ...
    /* Enforcing pk/id uniqueness should be done by rend_service_load_keys(),
     * but it's not, see #14828.
     */
...
rend_service_update_descriptor(rend_service_t *service)
{
    ...
    for (i = 0; i < smartlist_len(service->intro_nodes); ++i) {
         ...
         * checked every second in rend_consider_services_intro_points(), so
    ...
    if (have_intro != service->n_intro_points_wanted) {
        ...
        log_fn(severity, LD_REND,
               ...
               service->service_id, service->n_intro_points_wanted,
               have_intro);
...
rend_service_load_auth_keys(rend_service_t *s, const char *hfname)
{
    ...
    if (!(hfile = start_writing_to_stdio_file(
        ...
        log_warn(LD_CONFIG, "Could not open hostname file %s",
                 escaped(hfname));
        goto err;
    }
    ...
    SMARTLIST_FOREACH_BEGIN (s->clients, rend_authorized_client_t *, client) {
        ...
        if (parsed) {
            ...
            crypto_rand((char *)client->descriptor_cookie,
                        REND_DESC_COOKIE_LEN);
        }
        ...
        if (client->client_key) {
            ...
            if (crypto_pk_write_private_key_to_string(
                    client->client_key, &client_key_out, &len) != 0) {
                log_warn(LD_BUG,
                         "Internal error: "
            ...
            if (rend_get_service_id(client->client_key, service_id) < 0) {
                ...
                /*
                 * len is string length, not buffer length, but last byte is
                 * NUL anyway.
                 */
...
{
    /* We'll connect directly all reachable addresses, whether preferred or
     * not. The prefer_ipv6 argument to fascist_firewall_allows_address_addr is
                                  ...
                                  const uint8_t *request, size_t request_len)
{
    ...
    char keys[DIGEST_LEN +
              CPATH_KEY_MATERIAL_LEN]; /* Holds KH, Df, Db, Kf, Kb */
    int i;
    crypto_dh_t *dh = NULL;
    ...
    if (!service->accepted_intro_dh_parts) {
        service->accepted_intro_dh_parts = replaycache_new(
            REND_REPLAY_TIME_INTERVAL, REND_REPLAY_TIME_INTERVAL);
    }
    ...
    if (service->clients) {
        if (parsed_req->version == 3 && parsed_req->u.v3.auth_len > 0) {
            if (rend_check_authorization(
                    service, (const char *)parsed_req->u.v3.auth_data,
    ...
    if (crypto_dh_compute_secret(
            LOG_PROTOCOL_WARN, dh, (char *)(parsed_req->dh), DH1024_KEY_LEN,
    ...
    for (i = 0; i < max_rend_failures; i++) {
         ...
         *
         * We only use a one-hop path on the first attempt. If the first
         * attempt fails, we use a 3-hop path for reachability / reliability.
...
log_error:
    if (!err_msg) {
        if (stage_descr) {
            tor_asprintf(&err_msg, "unknown %s error for INTRODUCE2",
                         stage_descr);
        } else {
                               ...
                               uint8_t type, char **err_msg_out)
{
    ...
    if (request_len < (DIGEST_LEN + REND_COOKIE_LEN + (MAX_NICKNAME_LEN + 1) +
                       DH1024_KEY_LEN + 42)) {
        if (err_msg_out) {
            tor_asprintf(&err_msg, "got a truncated INTRODUCE%d cell",
                         (int)type);
        }
        goto err;
    }
                                   ...
                                   char **err_msg_out)
{
    ...
    if (ver_invariant_len < REND_COOKIE_LEN + DH1024_KEY_LEN) {
        tor_asprintf(&err_msg,
                     "decrypted plaintext of INTRODUCE%d cell was truncated "
                     "(%ld bytes)",
        ...
        memcpy(intro->rc, intro->plaintext + ver_specific_len,
               REND_COOKIE_LEN);
        memcpy(intro->dh,
                                    ...
                                    rend_intro_point_t *intro)
{
    ...
    if (rend_service_allow_non_anonymous_connection(options) &&
         ...
         * We've either just chosen it from the consensus, or we've just
         * reviewed our intro points to see which ones are still valid, and
         * deleted the ones that aren't in the consensus any more. */
        const node_t *node = node_get_by_id(launch_ei->identity_digest);
        if (BUG(!node)) {
            /* The service has kept an intro point after it went missing from
             * the consensus. If we did anything else here, it would be a
             * consensus distinguisher. Which are less of an issue for single
             * onion services, but still a bug. */
            return -1;
        }
        ...
        if (rend_service_use_direct_connection_node(options, node)) {
            ...
            if (BUG(!direct_ei)) {
                /* rend_service_use_direct_connection_node and
                 * extend_info_from_node disagree about which addresses on this
                 * node are permitted. This should never happen. Avoiding the
                 * connection is a safe response. */
                return -1;
            }
            flags = flags | CIRCLAUNCH_ONEHOP_TUNNEL;
            launch_ei = direct_ei;
    ...
    /* launch_ei is either intro->extend_info, or has been replaced with a
     * valid extend_info for single onion service direct connection. */
    ...
    /* We must have the same exit node even if cannibalized or direct
     * connection.
     */
    tor_assert(tor_memeq(intro->extend_info->identity_digest,
...
rend_service_intro_has_opened(origin_circuit_t *circuit)
{
    ...
    if (!service) {
        log_warn(
            LD_REND, "Unrecognized service ID %s on introduction circuit %u.",
    ...
    if (valid_ip_circuits > service->n_intro_points_wanted) {
        ...
        if (options->ExcludeNodes) {
            /* XXXX in some future version, we can test whether the transition
               is allowed or not given the actual nodes in the circuit.  But
               for now, this case, we might as well close the thing. */
    ...
    /* Send the ESTABLISH_INTRO cell */
    {
        ...
        if (relay_send_command_from_edge(0, TO_CIRCUIT(circuit),
                                         RELAY_COMMAND_ESTABLISH_INTRO, buf,
                                         len, circuit->cpath->prev) < 0) {
            log_info(LD_GENERAL,
                     "Couldn't send introduction request for service %s on "
                     "circuit %u",
...
{
    tor_assert(service);
    tor_assert(TO_CIRCUIT(circ)->purpose ==
                         ...
                         const char *service_id, int seconds_valid)
{
    ...
    for (i = 0; i < smartlist_len(descs); i++) {
        ...
        if (hs_dirs && smartlist_len(hs_dirs) > 0) {
            ...
            if (hid_serv_get_responsible_directories(responsible_dirs,
                ...
                control_event_hs_descriptor_upload(service_id, "UNKNOWN",
                                                   "UNKNOWN", NULL);
                goto done;
            }
        }
        for (j = 0; j < smartlist_len(responsible_dirs); j++) {
            ...
            if (smartlist_contains_digest(renddesc->successful_uploads,
                ...
                /* Don't upload descriptor if we succeeded in doing so last
                 * time. */
                continue;
            ...
            rend_data = rend_data_client_create(service_id, desc->desc_id,
                                                NULL, REND_NO_AUTH);
            ...
            log_info(LD_REND,
                     ...
                     safe_str_client(desc_id_base32), seconds_valid,
                     hs_dir->nickname, hs_dir_ip, hs_dir->or_port);
            control_event_hs_descriptor_upload(
                service_id, hs_dir->identity_digest, desc_id_base32, NULL);
            tor_free(hs_dir_ip);
    ...
    if (!failed_upload) {
        if (renddesc->successful_uploads) {
            SMARTLIST_FOREACH(renddesc->successful_uploads, char *, c,
                              tor_free(c););
    ...
    if (c && smartlist_len(c->routerstatus_list) > 0) {
        ...
        for (j = 0; j < num_descs; j++) {
            ...
            seconds_valid = rend_encode_v2_descriptors(
                descs, service->desc, now, 0, service->auth_type, client_key,
                client_cookies);
            if (seconds_valid < 0) {
            ...
            if (get_options()->PublishHidServDescriptors) {
                /* Post the current descriptors to the hidden service
                 * directories. */
            ...
            if (seconds_valid < REND_TIME_PERIOD_OVERLAPPING_V2_DESCS) {
                seconds_valid = rend_encode_v2_descriptors(
                    descs, service->desc, now, 1, service->auth_type,
                ...
                if (get_options()->PublishHidServDescriptors) {
                    directory_post_to_hs_dir(service->desc, descs, NULL,
                            ...
                            smartlist_t *retry_nodes, time_t now)
{
    ...
    SMARTLIST_FOREACH_BEGIN (service->intro_nodes, rend_intro_point_t *,
        ...
        if (intro_circ == NULL) {
            ...
            /* We've lost the circuit for this intro point, flag it so it can
             * be accounted for when considiring uploading a descriptor. */
            ...
            /* The intro point is still alive so let's try to use it again
             * because we have a published descriptor containing it. Keep the
             * intro point in the intro_nodes list because it's still valid, we
             * are rebuilding a circuit to it. */
            if (retry_nodes) {
                smartlist_add(retry_nodes, intro);
            }
        }
        /* else, the circuit is valid so in both cases, node being alive or
         * not, we leave the circuit and intro point object as is. Closing the
...
rend_max_intro_circs_per_period(unsigned int n_intro_points_wanted)
{
     ...
     * allow that twice because once every 24h or so, we can do it twice for
     * two descriptors that is the current one and the next one. So (3 + 2) * 2
...
rend_consider_services_intro_points(time_t now)
{
    ...
    SMARTLIST_FOREACH_BEGIN (rend_service_list, rend_service_t *, service) {
        ...
        SMARTLIST_FOREACH_BEGIN (retry_nodes, rend_intro_point_t *, intro) {
            ...
            if (r < 0) {
                log_warn(
                    ...
                    safe_str_client(extend_info_describe(intro->extend_info)),
                    safe_str_client(service->service_id));
                /* Unable to launch a circuit to that intro point, remove it
                 * from the valid list so we can create a new one. */
                smartlist_remove(service->intro_nodes, intro);
                rend_intro_point_free(intro);
                continue;
            }
         ...
         * was originally empty for performance reasons. */
        if (intro_nodes_len >= service->n_intro_points_wanted) {
            continue;
        }

        /* Number of intro points we want to open which is the wanted amount
         * minus the current amount of valid nodes. We know that this won't
         * underflow because of the check above. */
        n_intro_points_to_open =
            service->n_intro_points_wanted - intro_nodes_len;
        if (intro_nodes_len == 0) {
            /* We want to end up with n_intro_points_wanted intro points, but
             * if we have no intro points at all (chances are they all cycled
             * or we are starting up), we launch NUM_INTRO_POINTS_EXTRA extra
             * circuits and use the first n_intro_points_wanted that complete.
             * See proposal #155, section 4 for the rationale of this which is
             * purely for performance.
             *
             * The ones after the first n_intro_points_to_open will be
             * converted to 'general' internal circuits in
             * rend_service_intro_has_opened(), and then we'll drop them from
             * the list of intro points. */
        ...
        for (i = 0; i < (int)n_intro_points_to_open; i++) {
            ...
            node =
                router_choose_random_node(exclude_nodes, options->ExcludeNodes,
            ...
            /* Add the chosen node to the exclusion list in order to avoid
             * picking it again in the next iteration. */
            smartlist_add(exclude_nodes, (void *)node);
            intro = tor_malloc_zero(sizeof(rend_intro_point_t));
            /* extend_info is for clients, so we want the multi-hop primary
             * ORPort, even if we are a single onion service and intend to
             * connect to it directly ourselves. */
            ...
            if (r < 0) {
                log_warn(
                    ...
                    safe_str_client(extend_info_describe(intro->extend_info)),
                    safe_str_client(service->service_id));
                /* This funcion will be called again by the main loop so this
                 * intro point without a intro circuit will be retried on or
                 * removed after a maximum number of attempts. */
            }
        }
...
rend_consider_services_upload(time_t now)
{
    ...
    for (i = 0; i < smartlist_len(rend_service_list); ++i) {
        ...
        if (!service->next_upload_time) { /* never been uploaded yet */
            /* The fixed lower bound of rendinitialpostdelay seconds ensures
             * that the descriptor is stable before being published. See
             * comment below. */
             ...
             * startup time, as their IP address is publicly discoverable
             * anyway.
             */
        ...
        if (intro_points_ready &&
             ...
             * descriptor and ours has been stable for rendinitialpostdelay
...
rend_consider_descriptor_republication(void)
{
    ...
    for (i = 0; i < smartlist_len(rend_service_list); ++i) {
        ...
        if (service->desc && !service->desc->all_uploads_performed) {
            /* If we failed in uploading a descriptor last time, try again
             * *without* updating the descriptor's contents. */
            upload_service_descriptor(service);
        }
    }
}
        ...
        for (j = 0; j < smartlist_len(service->intro_nodes); ++j) {
            ...
            if (!circ) {
                tor_log(severity, LD_GENERAL,
                        "  Intro point %d at %s: no circuit", j, safe_name);
                continue;
            }
            tor_log(severity, LD_GENERAL,
                    "  Intro point %d at %s: circuit is %s", j, safe_name,
