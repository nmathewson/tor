...
static rend_intro_point_t *find_expiring_intro_point(struct rend_service_t *service,
...
static ssize_t rend_service_parse_intro_for_v0_or_v1(rend_intro_cell_t *intro, const uint8_t *buf,
                                                     size_t plaintext_len, char **err_msg_out);
static ssize_t rend_service_parse_intro_for_v2(rend_intro_cell_t *intro, const uint8_t *buf,
                                               size_t plaintext_len, char **err_msg_out);
static ssize_t rend_service_parse_intro_for_v3(rend_intro_cell_t *intro, const uint8_t *buf,
                                               size_t plaintext_len, char **err_msg_out);

static int rend_service_check_private_dir(const or_options_t *options, const rend_service_t *s,
                                          int create);
static const smartlist_t *rend_get_service_list(const smartlist_t *substitute_service_list);
static smartlist_t *rend_get_service_list_mutable(smartlist_t *substitute_service_list);
...
rend_service_free_staging_list(void)
{
    if (rend_service_staging_list) {
        SMARTLIST_FOREACH(rend_service_staging_list, rend_service_t *, ptr, rend_service_free(ptr));
...
{
    if (rend_service_list) {
        SMARTLIST_FOREACH(rend_service_list, rend_service_t *, ptr, rend_service_free(ptr));
 ...
 * -1 if not.*/
static int
rend_validate_service(const smartlist_t *service_list, const rend_service_t *service)
{
    ...
    if (service->max_streams_close_circuit < 0 || service->max_streams_close_circuit > 1) {
        log_warn(LD_CONFIG,
...
rend_add_service(smartlist_t *service_list, rend_service_t *service)
{
    ...
    log_debug(LD_REND, "Configuring service with directory %s", rend_service_escaped_dir(service));
...
#ifdef HAVE_SYS_UN_H
            log_debug(LD_REND, "Service maps port %d to socket at \"%s\"", p->virtual_port,
                      p->unix_addr);
#else
            log_warn(LD_BUG,
...
rend_service_parse_port_config(const char *string, const char *sep, char **err_msg_out)
{
    smartlist_t *sl;
    int virtport;
    int realport = 0;
    ...
    sl = smartlist_new();
    smartlist_split_string(sl, string, sep, SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 2);
    ...
    if (smartlist_len(sl) == 1) {
        ...
        int is_unix;
        ret = port_cfg_line_extract_addrport(addrport_element, &addrport, &is_unix, &rest);
...
rend_service_prune_list(void)
{
    ...
    if (old_service_list) {
        ...
        SMARTLIST_FOREACH(old_service_list, rend_service_t *, s, rend_service_free(s));
...
static void
service_config_shadow_copy(rend_service_t *service, hs_service_config_t *config)
{
    tor_assert(service);
    tor_assert(config);
    ...
    if (BUG(config->max_streams_per_rdv_circuit > HS_CONFIG_MAX_STREAMS_PER_RDV_CIRCUIT)) {
...
int
rend_config_service(const hs_opts_t *hs_opts, const or_options_t *options,
                    hs_service_config_t *config)
{
    ...
    if (hs_opts->HiddenServiceNumIntroductionPoints > NUM_INTRO_POINTS_MAX) {
        log_warn(LD_CONFIG,
                 ...
                 NUM_INTRO_POINTS_MAX, hs_opts->HiddenServiceNumIntroductionPoints);
        goto err;
    }
    ...
    if (hs_opts->HiddenServiceAuthorizeClient) {
        ...
        smartlist_split_string(type_names_split, hs_opts->HiddenServiceAuthorizeClient, " ", 0, 2);
        ...
        smartlist_split_string(clients, smartlist_get(type_names_split, 1), ",", SPLIT_SKIP_SPACE,
                               0);
        SMARTLIST_FOREACH(type_names_split, char *, cp, tor_free(cp));
        smartlist_free(type_names_split);
        ...
        if ((service->auth_type == REND_BASIC_AUTH && smartlist_len(service->clients) > 512) ||
            (service->auth_type == REND_STEALTH_AUTH && smartlist_len(service->clients) > 16)) {
            log_warn(LD_CONFIG,
                     "HiddenServiceAuthorizeClient contains %d "
...
rend_service_add_ephemeral(crypto_pk_t *pk, smartlist_t *ports, int max_streams_per_circuit,
                           ...
                           smartlist_t *auth_clients, char **service_id_out)
{
    *service_id_out = NULL;
    ...
    if (s->auth_type != REND_NO_AUTH && (!s->clients || smartlist_len(s->clients) == 0)) {
...
rend_service_del_ephemeral(const char *service_id)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        if (!circ->marked_for_close && (circ->purpose == CIRCUIT_PURPOSE_S_ESTABLISH_INTRO ||
                                        circ->purpose == CIRCUIT_PURPOSE_S_INTRO)) {
            origin_circuit_t *oc = TO_ORIGIN_CIRCUIT(circ);
            if (oc->rend_data == NULL || !rend_circuit_pk_digest_eq(oc, (uint8_t *)s->pk_digest)) {
                continue;
            }
    ...
    tor_assert_nonfatal(intro_period_elapsed >= 0);
    {
        ...
        if ((msg = rate_limit_log(&rlimit, approx_time()))) {
            log_fn(severity, LD_REND,
                   ...
                   service->service_id, exceeded_limit ? "exceeded launch limit with" : "launched",
...
rend_service_update_descriptor(rend_service_t *service)
{
    ...
    if (have_intro != service->n_intro_points_wanted) {
        ...
        if (have_intro < service->n_intro_points_wanted || have_intro > NUM_INTRO_POINTS_MAX) {
            severity = LOG_WARN;
        } else {
...
STATIC int
rend_service_verify_single_onion_poison(const rend_service_t *s, const or_options_t *options)
{
    /* Passing a NULL service is a bug */
    if (BUG(!s)) {
        return -1;
    ...
    if (service_is_single_onion_poisoned(s) != rend_service_non_anonymous_mode_enabled(options)) {
        return -1;
    }
                                                ...
                                                const or_options_t *options)
{
    ...
    case FN_DIR:
    case FN_ERROR:
        log_warn(LD_FS, "Can't read single onion poison file \"%s\"", poison_fname);
        goto done;
    case FN_FILE: /* single onion poison file already exists. NOP. */
    case FN_EMPTY: /* single onion poison file already exists. NOP. */
        log_debug(LD_FS, "Tried to re-poison a single onion poisoned file \"%s\"", poison_fname);
        break;
    case FN_NOENT:
        fd = tor_open_cloexec(poison_fname, O_RDWR | O_CREAT | O_TRUNC, 0600);
        if (fd < 0) {
            log_warn(LD_FS, "Could not create single onion poison file %s", poison_fname);
            goto done;
        }
        close(fd);
        break;
...
STATIC int
rend_service_poison_new_single_onion_dir(const rend_service_t *s, const or_options_t *options)
{
    /* Passing a NULL service is a bug */
    if (BUG(!s)) {
        return -1;
...
rend_service_load_all_keys(const smartlist_t *service_list)
{
    ...
    SMARTLIST_FOREACH_BEGIN (s_list, rend_service_t *, s) {
        if (s->private_key)
            continue;
        log_info(LD_REND, "Loading hidden-service keys from %s", rend_service_escaped_dir(s));
 ...
 * service */
void
rend_services_add_filenames_to_lists(smartlist_t *open_lst, smartlist_t *stat_lst)
{
    if (!rend_service_list)
        return;
 ...
 * Return 0 on success, -1 on failure. */
static int
rend_service_check_private_dir(const or_options_t *options, const rend_service_t *s, int create)
{
    /* Passing a NULL service is a bug */
    if (BUG(!s)) {
        return -1;
    ...
    if (hs_check_service_private_dir(options->User, s->directory, s->dir_group_readable, create) <
        0) {
        return -1;
    }
    ...
    if (rend_service_verify_single_onion_poison(s, options) < 0) {
        ...
        log_warn(
            ...
            "This is not allowed.",
            rend_service_non_anonymous_mode_enabled(options) ? 1 : 0, rend_service_escaped_dir(s),
            rend_service_non_anonymous_mode_enabled(options) ? "an anonymous" : "a non-anonymous");
        return -1;
    }
...
rend_service_load_auth_keys(rend_service_t *s, const char *hfname)
{
    ...
    if (!(cfile = start_writing_to_stdio_file(cfname, OPEN_FLAGS_REPLACE | O_TEXT, 0600,
                                              &open_cfile))) {
        log_warn(LD_CONFIG, "Could not open client_keys file %s", escaped(cfname));
        goto err;
    }

    if (!(hfile = start_writing_to_stdio_file(hfname, OPEN_FLAGS_REPLACE | O_TEXT, 0600,
                                              &open_hfile))) {
    ...
    SMARTLIST_FOREACH_BEGIN (s->clients, rend_authorized_client_t *, client) {
        ...
        if (parsed) {
            memcpy(client->descriptor_cookie, parsed->descriptor_cookie, REND_DESC_COOKIE_LEN);
        } else {
        ...
        if (base64_encode(desc_cook_out, 3 * REND_DESC_COOKIE_LEN_BASE64 + 1,
                          (char *)client->descriptor_cookie, REND_DESC_COOKIE_LEN, 0) < 0) {
        ...
        if (client->client_key) {
            ...
            if (crypto_pk_write_private_key_to_string(client->client_key, &client_key_out, &len) !=
                0) {
        ...
        if (fputs(buf, cfile) < 0) {
            log_warn(LD_FS, "Could not append client entry to file: %s", strerror(errno));
            goto err;
        }

        /* Add line to hostname file. This is not the same encoding as in
         * client_keys. */
        char *encoded_cookie = rend_auth_encode_cookie(client->descriptor_cookie, s->auth_type);
        if (!encoded_cookie) {
            log_warn(LD_BUG, "Could not base64-encode descriptor cookie.");
            goto err;
        }
        tor_snprintf(buf, sizeof(buf), "%s.onion %s # client: %s\n", service_id, encoded_cookie,
                     client->client_name);
        memwipe(encoded_cookie, 0, strlen(encoded_cookie));
        tor_free(encoded_cookie);

        if (fputs(buf, hfile) < 0) {
            log_warn(LD_FS, "Could not append host entry to file: %s", strerror(errno));
            goto err;
        }
 ...
 * and 0 for failure. */
static int
rend_check_authorization(rend_service_t *service, const char *descriptor_cookie, size_t cookie_len)
{
    ...
    SMARTLIST_FOREACH(service->clients, rend_authorized_client_t *, client, {
        if (tor_memeq(client->descriptor_cookie, descriptor_cookie, REND_DESC_COOKIE_LEN)) {
            auth_client = client;
            break;
        }
    });
    if (!auth_client) {
        ...
        base64_encode(descriptor_cookie_base64, sizeof(descriptor_cookie_base64), descriptor_cookie,
    ...
    /* Allow the request. */
    log_info(LD_REND, "Client %s authorized for service %s.", auth_client->client_name,
             service->service_id);
    return 1;
}
...
static int
rend_service_use_direct_connection(const or_options_t *options, const extend_info_t *ei)
{
    ...
    return (
        ...
        fascist_firewall_allows_address_addr(&ei->addr, ei->port, FIREWALL_OR_CONNECTION, 0, 0));
}

/* Like rend_service_use_direct_connection, but to a node. */
static int
rend_service_use_direct_connection_node(const or_options_t *options, const node_t *node)
{
    /* We'll connect directly all reachable addresses, whether preferred or not.
     */
 ...
 */
int
rend_service_receive_introduction(origin_circuit_t *circuit, const uint8_t *request,
                                  size_t request_len)
{
    /* Global status stuff */
    int status = 0, result;
    ...
    if (circuit->base_.purpose != CIRCUIT_PURPOSE_S_INTRO) {
        log_warn(LD_PROTOCOL, "Got an INTRODUCE2 over a non-introduction circuit %u.",
    ...
    base32_encode(serviceid, REND_SERVICE_ID_LEN_BASE32 + 1, rend_pk_digest, REND_SERVICE_ID_LEN);
    ...
    log_info(LD_REND, "Received INTRODUCE2 cell for service %s on circ %u.", escaped(serviceid),
             (unsigned)circuit->base_.n_circ_id);
    ...
    parsed_req = rend_service_begin_parse_intro(request, request_len, 2, &err_msg);
    if (!parsed_req) {
        goto log_error;
    } else if (err_msg) {
        log_info(LD_REND, "%s on circ %u.", err_msg, (unsigned)circuit->base_.n_circ_id);
        tor_free(err_msg);
    }
    ...
    replay = replaycache_add_test_and_elapsed(intro_point->accepted_intro_rsa_parts,
                                              parsed_req->ciphertext,
    ...
    if (options->StrictNodes && routerset_contains_extendinfo(options->ExcludeNodes, rp)) {
     ...
     * part 1. */
    replay = replaycache_add_test_and_elapsed(service->accepted_intro_dh_parts, parsed_req->dh,
                                              DH1024_KEY_LEN, &elapsed);
    ...
    if (service->clients) {
        if (parsed_req->version == 3 && parsed_req->u.v3.auth_len > 0) {
            if (rend_check_authorization(service, (const char *)parsed_req->u.v3.auth_data,
    ...
    if (crypto_dh_compute_secret(LOG_PROTOCOL_WARN, dh, (char *)(parsed_req->dh), DH1024_KEY_LEN,
    ...
    for (i = 0; i < max_rend_failures; i++) {
        ...
        launched = circuit_launch_by_extend_info(CIRCUIT_PURPOSE_S_CONNECT_REND, rp, flags);
    ...
    launched->rend_data = rend_data_service_create(service->service_id, rend_pk_digest,
    ...
    dh = NULL;
    if (cpath_init_circuit_crypto(cpath, keys + DIGEST_LEN, sizeof(keys) - DIGEST_LEN, 1, 0) < 0)
        goto err;
...
log_error:
    ...
    log_fn(err_msg_severity, LD_REND, "%s on circ %u", err_msg, (unsigned)circuit->base_.n_circ_id);
err:
    status = -1;
    if (dh)
        crypto_dh_free(dh);
...
find_rp_for_intro(const rend_intro_cell_t *intro, char **err_msg_out)
{
    ...
    if (intro->version == 0 || intro->version == 1) {
        ...
        if (!node) {
            if (err_msg_out) {
                tor_asprintf(&err_msg, "Couldn't find router %s named in INTRODUCE2 cell",
        ...
        const int allow_direct = rend_service_allow_non_anonymous_connection(get_options());
        ...
        if (err_msg_out) {
            tor_asprintf(&err_msg, "Unknown version %d in INTRODUCE2 cell", (int)(intro->version));
        }
    ...
    if (!extend_info_addr_is_allowed(&rp->addr)) {
        if (err_msg_out) {
            tor_asprintf(&err_msg, "Relay IP in INTRODUCE2 cell is private address.");
        }
        extend_info_free(rp);
        rp = NULL;
        goto err;
...
rend_intro_cell_t *
rend_service_begin_parse_intro(const uint8_t *request, size_t request_len, uint8_t type,
                               char **err_msg_out)
{
    ...
    if (request_len <
        (DIGEST_LEN + REND_COOKIE_LEN + (MAX_NICKNAME_LEN + 1) + DH1024_KEY_LEN + 42)) {
        if (err_msg_out) {
...
static ssize_t
rend_service_parse_intro_for_v0_or_v1(rend_intro_cell_t *intro, const uint8_t *buf,
                                      size_t plaintext_len, char **err_msg_out)
{
    ...
    if ((intro->version == 0 && !is_legal_nickname(rp_nickname)) ||
        ...
        if (err_msg_out) {
            tor_asprintf(err_msg_out, "bad nickname in INTRODUCE%d cell", (int)(intro->type));
        }
        goto err;
    }
...
static ssize_t
rend_service_parse_intro_for_v2(rend_intro_cell_t *intro, const uint8_t *buf, size_t plaintext_len,
                                char **err_msg_out)
{
    unsigned int klen;
    ...
    extend_info->onion_key = crypto_pk_asn1_decode((const char *)(buf + 7 + DIGEST_LEN + 2), klen);
    ...
    if (128 != crypto_pk_keysize(extend_info->onion_key)) {
        if (err_msg_out) {
            tor_asprintf(err_msg_out, "invalid onion key size in version %d INTRODUCE%d cell",
                         intro->version, (intro->type));
        }
...
static ssize_t
rend_service_parse_intro_for_v3(rend_intro_cell_t *intro, const uint8_t *buf, size_t plaintext_len,
                                char **err_msg_out)
{
    ...
    if (intro->u.v3.auth_type == REND_BASIC_AUTH || intro->u.v3.auth_type == REND_STEALTH_AUTH) {
        if (intro->u.v3.auth_len != REND_DESC_COOKIE_LEN) {
            if (err_msg_out) {
                tor_asprintf(err_msg_out,
                             "wrong auth data size %d for INTRODUCE%d cell, "
                             "should be %d",
                             (int)(intro->u.v3.auth_len), (int)(intro->type), REND_DESC_COOKIE_LEN);
            }
    ...
    v2_ver_specific_len =
        rend_service_parse_intro_for_v2(intro, buf + adjust, plaintext_len - adjust, err_msg_out);
...
static ssize_t (*intro_version_handlers[])(rend_intro_cell_t *, const uint8_t *, size_t,
                                           char **) = {
    rend_service_parse_intro_for_v0_or_v1, rend_service_parse_intro_for_v0_or_v1,
    rend_service_parse_intro_for_v2, rend_service_parse_intro_for_v3};
...
int
rend_service_decrypt_intro(rend_intro_cell_t *intro, crypto_pk_t *key, char **err_msg_out)
{
    char *err_msg = NULL;
    ...
    if (tor_memneq(key_digest, intro->pk, DIGEST_LEN)) {
        if (err_msg_out) {
            base32_encode(service_id, REND_SERVICE_ID_LEN_BASE32 + 1, (char *)(intro->pk),
                          REND_SERVICE_ID_LEN);
            tor_asprintf(&err_msg, "got an INTRODUCE%d cell for the wrong service (%s)",
    ...
    result = crypto_pk_obsolete_private_hybrid_decrypt(
        key, (char *)buf, sizeof(buf), (const char *)(intro->ciphertext), intro->ciphertext_len,
        PK_PKCS1_OAEP_PADDING, 1);
    if (result < 0) {
        if (err_msg_out) {
            tor_asprintf(&err_msg, "couldn't decrypt INTRODUCE%d cell", (int)(intro->type));
        }
        status = -6;
        goto err;
    }
...
err:
    if (err_msg_out && !err_msg) {
        tor_asprintf(&err_msg, "unknown INTRODUCE%d error decrypting encrypted part",
...
int
rend_service_parse_intro_plaintext(rend_intro_cell_t *intro, char **err_msg_out)
{
    char *err_msg = NULL;
    ...
    if (!intro) {
        if (err_msg_out) {
            err_msg = tor_strdup("rend_service_parse_intro_plaintext() called with NULL "
                                 "rend_intro_cell_t");
        }
    ...
    ver_specific_len =
        intro_version_handlers[version](intro, intro->plaintext, intro->plaintext_len, &err_msg);
    ...
    if (ver_invariant_len < REND_COOKIE_LEN + DH1024_KEY_LEN) {
        tor_asprintf(&err_msg, "decrypted plaintext of INTRODUCE%d cell was truncated (%ld bytes)",
                     (int)(intro->type), (long)(intro->plaintext_len));
        status = -5;
        goto err;
    } else if (ver_invariant_len > REND_COOKIE_LEN + DH1024_KEY_LEN) {
        tor_asprintf(&err_msg, "decrypted plaintext of INTRODUCE%d cell was too long (%ld bytes)",
                     (int)(intro->type), (long)(intro->plaintext_len));
        status = -6;
        goto err;
    } else {
        memcpy(intro->rc, intro->plaintext + ver_specific_len, REND_COOKIE_LEN);
        memcpy(intro->dh, intro->plaintext + ver_specific_len + REND_COOKIE_LEN, DH1024_KEY_LEN);
    }
...
int
rend_service_validate_intro_late(const rend_intro_cell_t *intro, char **err_msg_out)
{
    int status = 0;
    ...
    if (intro->version == 3 && intro->parsed) {
        if (!(intro->u.v3.auth_type == REND_NO_AUTH || intro->u.v3.auth_type == REND_BASIC_AUTH ||
              intro->u.v3.auth_type == REND_STEALTH_AUTH)) {
            /* This is an informative message, not an error, as in the old code */
            if (err_msg_out)
                tor_asprintf(err_msg_out, "unknown authorization type %d", intro->u.v3.auth_type);
        }
    }
...
rend_service_relaunch_rendezvous(origin_circuit_t *oldcirc)
{
    ...
    newcirc =
        circuit_launch_by_extend_info(CIRCUIT_PURPOSE_S_CONNECT_REND, oldstate->chosen_exit, flags);
    ...
    newstate->service_pending_final_cpath_ref = oldstate->service_pending_final_cpath_ref;
 ...
 */
static int
rend_service_launch_establish_intro(rend_service_t *service, rend_intro_point_t *intro)
{
     ...
     * function.)
     */
    if (rend_service_allow_non_anonymous_connection(options) && intro->circuit_retries == 0) {
    ...
    tor_assert(
        tor_memeq(intro->extend_info->identity_digest, launch_ei->identity_digest, DIGEST_LEN));

    log_info(LD_REND, "Launching circuit to introduction point %s%s%s for service %s",
    ...
    launched = circuit_launch_by_extend_info(CIRCUIT_PURPOSE_S_ESTABLISH_INTRO, launch_ei, flags);

    if (!launched) {
        log_info(LD_REND, "Can't launch circuit to establish introduction at %s%s%s.",
                 safe_str_client(extend_info_describe(intro->extend_info)),
                 direct_ei ? " via direct address " : "",
                 direct_ei ? safe_str_client(extend_info_describe(direct_ei)) : "");
    ...
    launched->rend_data =
        rend_data_service_create(service->service_id, service->pk_digest, NULL, service->auth_type);
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        if (!circ->marked_for_close && circ->state == CIRCUIT_STATE_OPEN &&
            (circ->purpose == CIRCUIT_PURPOSE_S_ESTABLISH_INTRO ||
             circ->purpose == CIRCUIT_PURPOSE_S_INTRO)) {
            origin_circuit_t *oc = TO_ORIGIN_CIRCUIT(circ);
            if (oc->rend_data && rend_circuit_pk_digest_eq(oc, (uint8_t *)service->pk_digest)) {
                num_ipos++;
            }
        }
    } SMARTLIST_FOREACH_END (circ);
 ...
 */
ssize_t
rend_service_encode_establish_intro_cell(char *cell_body_out, size_t cell_body_out_len,
                                         crypto_pk_t *intro_key, const char *rend_circ_nonce)
{
    int retval = -1;
    int r;
    int len = 0;
    ...
    r = crypto_pk_asn1_encode(intro_key, cell_body_out + 2, RELAY_PAYLOAD_SIZE - 2);
    if (r < 0) {
    ...
    if (crypto_digest(cell_body_out + len, auth, DIGEST_LEN + 9))
        goto err;
    len += 20;
    r = crypto_pk_private_sign_digest(intro_key, cell_body_out + len, cell_body_out_len - len,
                                      cell_body_out, len);
    if (r < 0) {
...
rend_service_intro_has_opened(origin_circuit_t *circuit)
{
    ...
    if (valid_ip_circuits > service->n_intro_points_wanted) {
        ...
        if (options->ExcludeNodes) {
            ...
            if (circuit_should_use_vanguards(TO_CIRCUIT(circuit)->purpose)) {
                circuit_change_purpose(TO_CIRCUIT(circuit), CIRCUIT_PURPOSE_HS_VANGUARDS);
            } else {
    ...
    log_info(LD_REND, "Established circuit %u as introduction point for service %s",
    ...
    {
        ssize_t len;
        len = rend_service_encode_establish_intro_cell(buf, sizeof(buf), circuit->intro_key,
        ...
        if (relay_send_command_from_edge(0, TO_CIRCUIT(circuit), RELAY_COMMAND_ESTABLISH_INTRO, buf,
                                         len, circuit->cpath->prev) < 0) {
            log_info(LD_GENERAL, "Couldn't send introduction request for service %s on circuit %u",
 ...
 * now out-of-date. */
int
rend_service_intro_established(origin_circuit_t *circuit, const uint8_t *request,
                               size_t request_len)
{
    rend_service_t *service;
    ...
    const char *rend_pk_digest = (char *)rend_data_get_pk_digest(circuit->rend_data, NULL);
    ...
    log_info(LD_REND, "Received INTRO_ESTABLISHED cell on circuit %u for service %s",
             (unsigned)circuit->base_.n_circ_id, serviceid);
...
rend_service_rendezvous_has_opened(origin_circuit_t *circuit)
{
    ...
    if (crypto_dh_get_public(hop->rend_dh_handshake_state, buf + REND_COOKIE_LEN, DH1024_KEY_LEN) <
        0) {
        log_warn(LD_GENERAL, "Couldn't get DH public key.");
        reason = END_CIRC_REASON_INTERNAL;
        goto err;
    }
    memcpy(buf + REND_COOKIE_LEN + DH1024_KEY_LEN, hop->rend_circ_nonce, DIGEST_LEN);

    /* Send the cell */
    if (relay_send_command_from_edge(0, TO_CIRCUIT(circuit), RELAY_COMMAND_RENDEZVOUS1, buf,
...
find_intro_circuit(rend_intro_point_t *intro, const char *pk_digest)
{
    ...
    tor_assert(intro);
    while ((circ = circuit_get_next_by_pk_and_purpose(circ, (uint8_t *)pk_digest,
...
find_expiring_intro_point(rend_service_t *service, origin_circuit_t *circ)
{
    ...
    SMARTLIST_FOREACH(
        ...
        if (crypto_pk_eq_keys(intro_point->intro_key, circ->intro_key)) { return intro_point; });
 ...
 */
void
directory_post_to_hs_dir(rend_service_descriptor_t *renddesc, smartlist_t *descs,
                         smartlist_t *hs_dirs, const char *service_id, int seconds_valid)
{
    ...
    for (i = 0; i < smartlist_len(descs); i++) {
        ...
        if (hs_dirs && smartlist_len(hs_dirs) > 0) {
            ...
            if (hid_serv_get_responsible_directories(responsible_dirs, desc->desc_id) < 0) {
                log_warn(LD_REND, "Could not determine the responsible hidden service "
                                  "directories to post descriptors to.");
                control_event_hs_descriptor_upload(service_id, "UNKNOWN", "UNKNOWN", NULL);
                goto done;
            }
        }
        for (j = 0; j < smartlist_len(responsible_dirs); j++) {
            ...
            if (smartlist_contains_digest(renddesc->successful_uploads, hs_dir->identity_digest))
            ...
            rend_data = rend_data_client_create(service_id, desc->desc_id, NULL, REND_NO_AUTH);
            directory_request_t *req = directory_request_new(DIR_PURPOSE_UPLOAD_RENDDESC_V2);
            directory_request_set_routerstatus(req, hs_dir);
            directory_request_set_indirection(req, DIRIND_ANONYMOUS);
            directory_request_set_payload(req, desc->desc_str, strlen(desc->desc_str));
            directory_request_set_rend_query(req, rend_data);
            directory_initiate_request(req);
            directory_request_free(req);

            rend_data_free(rend_data);
            base32_encode(desc_id_base32, sizeof(desc_id_base32), desc->desc_id, DIGEST_LEN);
            ...
            log_info(LD_REND,
                     ...
                     "%s:%d.",
                     safe_str_client(service_id), safe_str_client(desc_id_base32), seconds_valid,
                     hs_dir->nickname, hs_dir_ip, hs_dir->or_port);
            control_event_hs_descriptor_upload(service_id, hs_dir->identity_digest, desc_id_base32,
                                               NULL);
            tor_free(hs_dir_ip);
...
upload_service_descriptor(rend_service_t *service)
{
    ...
    if (c && smartlist_len(c->routerstatus_list) > 0) {
         ...
         * type 'stealth'. */
        num_descs = service->auth_type == REND_STEALTH_AUTH ? smartlist_len(service->clients) : 1;
        for (j = 0; j < num_descs; j++) {
            ...
            seconds_valid = rend_encode_v2_descriptors(
                descs, service->desc, now, 0, service->auth_type, client_key, client_cookies);
            if (seconds_valid < 0) {
            ...
            if (get_options()->PublishHidServDescriptors) {
                ...
                directory_post_to_hs_dir(service->desc, descs, NULL, serviceid, seconds_valid);
            }
            ...
            if (seconds_valid - REND_TIME_PERIOD_OVERLAPPING_V2_DESCS > rendpostperiod)
            ...
            if (seconds_valid < REND_TIME_PERIOD_OVERLAPPING_V2_DESCS) {
                seconds_valid = rend_encode_v2_descriptors(
                    descs, service->desc, now, 1, service->auth_type, client_key, client_cookies);
                if (seconds_valid < 0) {
                    log_warn(LD_BUG, "Internal error: couldn't encode service "
                                     "descriptor; not uploading.");
                    smartlist_free(descs);
...
intro_point_should_expire_now(rend_intro_point_t *intro, time_t now)
{
    ...
    if (intro->time_to_expire == -1) {
        ...
        int intro_point_lifetime_seconds = crypto_rand_int_range(INTRO_POINT_LIFETIME_MIN_SECONDS,
...
static void
remove_invalid_intro_points(rend_service_t *service, smartlist_t *exclude_nodes,
                            smartlist_t *retry_nodes, time_t now)
{
    tor_assert(service);

    /* Remove any expired nodes that doesn't have a circuit. */
    SMARTLIST_FOREACH_BEGIN (service->expiring_nodes, rend_intro_point_t *, intro) {
        origin_circuit_t *intro_circ = find_intro_circuit(intro, service->pk_digest);
        if (intro_circ) {
            continue;
        }
    ...
    SMARTLIST_FOREACH_BEGIN (service->intro_nodes, rend_intro_point_t *, intro) {
        ...
        if (intro_circ == NULL) {
            ...
            if (node == NULL || intro->circuit_retries >= MAX_INTRO_POINT_CIRCUIT_RETRIES) {
                rend_intro_point_free(intro);
        ...
        for (i = 0; i < (int)n_intro_points_to_open; i++) {
            ...
            if (allow_direct && !node) {
                log_info(LD_REND,
                         ...
                         safe_str_client(service->service_id));
                node = router_choose_random_node(exclude_nodes, options->ExcludeNodes, flags);
            }
            ...
            if (!node) {
                log_warn(LD_REND,
                         ...
                         "wanted %u.",
                         smartlist_len(service->intro_nodes), safe_str_client(service->service_id),
            ...
            intro->max_introductions = crypto_rand_int_range(
                INTRO_POINT_MIN_LIFETIME_INTRODUCTIONS, INTRO_POINT_MAX_LIFETIME_INTRODUCTIONS);
            smartlist_add(service->intro_nodes, intro);
            log_info(LD_REND, "Picked router %s as an intro point for %s.",
                     safe_str_client(node_describe(node)), safe_str_client(service->service_id));
...
rend_consider_services_upload(time_t now)
{
    ...
    int rendinitialpostdelay = (options->TestingTorNetwork ? MIN_REND_INITIAL_POST_DELAY_TESTING
    ...
    for (i = 0; i < smartlist_len(rend_service_list); ++i) {
        ...
        if (intro_points_ready &&
            (service->next_upload_time < now ||
             (service->desc_is_dirty && service->desc_is_dirty < now - rendinitialpostdelay))) {
...
rend_service_dump_stats(int severity)
{
    ...
    for (int i = 0; i < smartlist_len(rend_service_list); ++i) {
        ...
        for (int j = 0; j < smartlist_len(service->intro_nodes); ++j) {
            ...
            if (!circ) {
                tor_log(severity, LD_GENERAL, "  Intro point %d at %s: no circuit", j, safe_name);
                continue;
            }
            tor_log(severity, LD_GENERAL, "  Intro point %d at %s: circuit is %s", j, safe_name,
 ...
 */
int
rend_service_set_connection_addr_port(edge_connection_t *conn, origin_circuit_t *circ)
{
    rend_service_t *service;
    char serviceid[REND_SERVICE_ID_LEN_BASE32 + 1];
    const char *rend_pk_digest;
...
#define MAX_STREAM_WARN_INTERVAL 600
        static struct ratelim_t stream_ratelim = RATELIM_INIT(MAX_STREAM_WARN_INTERVAL);
    ...
    log_info(LD_REND, "No virtual port mapping exists for port %d on service %s", conn->base_.port,
             serviceid);
...
rend_service_non_anonymous_mode_consistent(const or_options_t *options)
{
    ...
    return (!!options->HiddenServiceSingleHopMode == !!options->HiddenServiceNonAnonymousMode);
}
