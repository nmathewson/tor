...
static extend_info_t *rend_client_get_random_intro_impl(const rend_cache_entry_t *rend_query,
...
rend_client_send_establish_rendezvous(origin_circuit_t *circ)
{
    ...
    if (relay_send_command_from_edge(0, TO_CIRCUIT(circ), RELAY_COMMAND_ESTABLISH_RENDEZVOUS,
 ...
 */
int
rend_client_send_introduction(origin_circuit_t *introcirc, origin_circuit_t *rendcirc)
{
    ...
    if (!intro_key) {
        log_info(LD_REND,
                 ...
                 safe_str_client(onion_address),
                 safe_str_client(extend_info_describe(introcirc->build_state->chosen_exit)),
    ...
    if (entry->parsed->protocols & (1 << 3) || entry->parsed->protocols & (1 << 2)) {
        ...
        klen = crypto_pk_asn1_encode(extend_info->onion_key, tmp + v3_shift + 7 + DIGEST_LEN + 2,
        ...
        if (BUG(strlen(rendcirc->build_state->chosen_exit->nickname) > MAX_NICKNAME_LEN)) {
            goto perm_err;
        }
    ...
    if (crypto_dh_get_public(cpath->rend_dh_handshake_state, tmp + dh_offset, DH1024_KEY_LEN) < 0) {
     ...
     * rendezvous circuit. */
    memcpy(introcirc->rend_data->rend_cookie, rendcirc->rend_data->rend_cookie, REND_COOKIE_LEN);

    log_info(LD_REND, "Sending an INTRODUCE1 cell");
    if (relay_send_command_from_edge(0, TO_CIRCUIT(introcirc), RELAY_COMMAND_INTRODUCE1, payload,
    ...
    circuit_change_purpose(TO_CIRCUIT(introcirc), CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT);
    /* Set timestamp_dirty, because circuit_expire_building expects it
 ...
 */
int
rend_client_introduction_acked(origin_circuit_t *circ, const uint8_t *request, size_t request_len)
{
    const or_options_t *options = get_options();
    ...
    if (request_len == 0) {
        ...
        if (rendcirc) { /* remember the ack */
            assert_circ_anonymity_ok(rendcirc, options);
            circuit_change_purpose(TO_CIRCUIT(rendcirc), CIRCUIT_PURPOSE_C_REND_READY_INTRO_ACKED);
        ...
        circuit_change_purpose(TO_CIRCUIT(circ), CIRCUIT_PURPOSE_C_INTRODUCE_ACKED);
        ...
        if (rend_client_report_intro_point_failure(circ->build_state->chosen_exit, circ->rend_data,
                          ...
                          routerstatus_t *rs_hsdir)
{
    ...
    if (!rs_hsdir) {
        ...
        if (!hs_dir) {
            ...
            const char *query_response = (rate_limited) ? "QUERY_RATE_LIMITED" : "QUERY_NO_HSDIR";
            control_event_hsv2_descriptor_failed(rend_query, NULL, query_response);
            control_event_hs_descriptor_content(rend_data_get_address(rend_query), desc_id_base32,
                                                NULL, NULL);
            return 0;
        }
    }
     ...
     * on the control port. */
    hsdir_fp = tor_memdup(hs_dir->identity_digest, sizeof(hs_dir->identity_digest));
    ...
    if (rend_data->auth_type != REND_NO_AUTH) {
        if (base64_encode(descriptor_cookie_base64, sizeof(descriptor_cookie_base64),
        ...
        strlcpy(descriptor_cookie_base64, "(none)", sizeof(descriptor_cookie_base64));
    }
    ...
    directory_request_t *req = directory_request_new(DIR_PURPOSE_FETCH_RENDDESC_V2);
    ...
    control_event_hs_descriptor_requested(rend_data->onion_address, rend_data->auth_type,
                                          hs_dir->identity_digest, desc_id_base32, NULL);
    return 1;
}
...
static int
fetch_v2_desc_by_descid(const char *desc_id, const rend_data_t *rend_query, smartlist_t *hsdirs)
{
    int ret;
...
fetch_v2_desc_by_addr(rend_data_t *rend_query, smartlist_t *hsdirs)
{
    ...
    while (tries_left > 0) {
        ...
        ret = rend_compute_v2_desc_id(
            descriptor_id, rend_data->onion_address,
            rend_data->auth_type == REND_STEALTH_AUTH ? rend_data->descriptor_cookie : NULL,
...
rend_client_cancel_descriptor_fetches(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (connection_array, connection_t *, conn) {
        if (conn->type == CONN_TYPE_DIR && conn->purpose == DIR_PURPOSE_FETCH_RENDDESC_V2) {
            ...
            if (!rd) {
                log_warn(LD_BUG | LD_REND, "Marking for close dir conn fetching rendezvous "
 ...
 */
int
rend_client_report_intro_point_failure(extend_info_t *failed_intro, rend_data_t *rend_data,
                                       unsigned int failure_type)
{
    ...
    if (r < 0) {
        ...
        switch (-r) {
        case EINVAL:
            log_warn(LD_BUG, "Malformed service ID %s.", escaped_safe_str_client(onion_address));
            return -1;
        case ENOENT:
    ...
    for (i = 0; i < smartlist_len(ent->parsed->intro_nodes); i++) {
        ...
        if (tor_memeq(failed_intro->identity_digest, intro->extend_info->identity_digest,
                      DIGEST_LEN)) {
            switch (failure_type) {
            default:
                log_warn(LD_BUG, "Unknown failure type %u. Removing intro point.", failure_type);
                tor_fragile_assert();
                FALLTHROUGH;
            case INTRO_POINT_FAILURE_GENERIC:
                rend_cache_intro_failure_note(
                    failure_type, (uint8_t *)failed_intro->identity_digest, onion_address);
    ...
    if (!rend_client_any_intro_points_usable(ent)) {
        log_info(LD_REND, "No more intro points remain for %s. Re-fetching descriptor.",
                 escaped_safe_str_client(onion_address));
        rend_client_refetch_v2_renddesc(rend_data);
    ...
    log_info(LD_REND, "%d options left for %s.", smartlist_len(ent->parsed->intro_nodes),
             escaped_safe_str_client(onion_address));
    return 1;
}
...
int
rend_client_receive_rendezvous(origin_circuit_t *circ, const uint8_t *request, size_t request_len)
{
    if (request_len != DH1024_KEY_LEN + DIGEST_LEN) {
        log_warn(LD_PROTOCOL, "Incorrect length (%d) on RENDEZVOUS2 cell.", (int)request_len);
        goto err;
    }
...
rend_client_desc_trynow(const char *query)
{
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, base_conn) {
        if (base_conn->type != CONN_TYPE_AP || base_conn->state != AP_CONN_STATE_RENDDESC_WAIT ||
...
rend_client_note_connection_attempt_ended(const rend_data_t *rend_data)
{
    ...
    if (cache_entry != NULL) {
        SMARTLIST_FOREACH(cache_entry->parsed->intro_nodes, rend_intro_point_t *, ip,
                          ip->timed_out = 0;);
    }
    ...
    if (have_onion) {
        ...
        for (replica = 0; replica < ARRAY_LENGTH(rend_data_v2->descriptor_id); replica++) {
...
rend_client_get_random_intro(const rend_data_t *rend_query)
{
    ...
    if (ret < 0 || !rend_client_any_intro_points_usable(entry)) {
        log_warn(LD_REND, "Query '%s' didn't have valid rend desc in cache. Failing.",
                 safe_str_client(onion_address));
...
rend_client_get_random_intro_impl(const rend_cache_entry_t *entry, const int strict,
                                  const int warnings)
{
    int i;
...
again:
    ...
    if (strict && routerset_contains_extendinfo(options->ExcludeNodes, intro->extend_info)) {
        n_excluded++;
        smartlist_del(usable_nodes, i);
        goto again;
    }
...
#define rend_service_authorization_free(val) \
    FREE_AND_NULL(rend_service_authorization_t, rend_service_authorization_free_, (val))
...
int
rend_parse_service_authorization(const or_options_t *options, int validate_only)
{
    config_line_t *line;
    int res = -1;
    ...
    for (line = options->HidServAuth; line; line = line->next) {
        ...
        if (strlen(onion_address) != REND_SERVICE_ADDRESS_LEN ||
            ...
            log_warn(LD_CONFIG, "Onion address has wrong format: '%s'", onion_address);
            goto err;
        }
        ...
        if (rend_auth_decode_cookie(descriptor_cookie, auth->descriptor_cookie, &auth->auth_type,
                                    &err_msg) < 0) {
            tor_assert(err_msg);
...
rend_client_circuit_cleanup_on_free(const circuit_t *circ)
{
    ...
    case CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT: {
        ...
        rend_client_report_intro_point_failure(ocirc->build_state->chosen_exit, ocirc->rend_data,
