                              ...
                              origin_circuit_t *rendcirc)
{
    ...
    intro_key = NULL;
    SMARTLIST_FOREACH(
        entry->parsed->intro_nodes, rend_intro_point_t *, intro, {
    ...
    if (entry->parsed->protocols & (1 << 3)) {
        ...
        if (tmp[1] != REND_NO_AUTH) {
            ...
            memcpy(tmp + 4,
                   TO_REND_DATA_V2(introcirc->rend_data)->descriptor_cookie,
    ...
    if (entry->parsed->protocols & (1 << 3) ||
        ...
        /* Some compilers are smart enough to work out that nickname can be
         * more than 19 characters, when it's a hexdigest. They warn that
         * strncpy() will truncate hexdigests without NUL-terminating them. But
         * we only put hexdigests in HSDir and general circuit exits. */
    ...
    if (relay_send_command_from_edge(
            0, TO_CIRCUIT(introcirc), RELAY_COMMAND_INTRODUCE1, payload,
...
perm_err:
    if (!introcirc->base_.marked_for_close)
        circuit_mark_for_close(TO_CIRCUIT(introcirc),
                               ...
                               size_t request_len)
{
    ...
    if (request_len == 0) {
        /* It's an ACK; the introduction point relayed our introduction
         * request. */
        ...
        /* Save the rend data digest to a temporary object so that we don't
         * access it after we mark the circuit for close. */
        const uint8_t *rend_digest_tmp = NULL;
        size_t digest_len;
        uint8_t *cached_rend_digest = NULL;
        rend_digest_tmp =
            rend_data_get_pk_digest(circ->rend_data, &digest_len);
                          ...
                          routerstatus_t *rs_hsdir)
{
    ...
    if (!rs_hsdir) {
        ...
        /* Determine responsible dirs. Even if we can't get all we want, work
         * with the ones we have. If it's empty, we'll notice in
         * hs_pick_hsdir(). */
        smartlist_t *responsible_dirs = smartlist_new();
        hid_serv_get_responsible_directories(responsible_dirs, desc_id);

        hs_dir =
            hs_pick_hsdir(responsible_dirs, desc_id_base32, &rate_limited);
        if (!hs_dir) {
            ...
            control_event_hs_descriptor_content(
                rend_data_get_address(rend_query), desc_id_base32, NULL, NULL);
            return 0;
        }
    }
    ...
    if (rend_data->auth_type != REND_NO_AUTH) {
        if (base64_encode(
            ...
            control_event_hsv2_descriptor_failed(rend_query, hsdir_fp,
                                                 "BAD_DESC");
            control_event_hs_descriptor_content(
                rend_data_get_address(rend_query), desc_id_base32, hsdir_fp,
                NULL);
            return 0;
        }
        /* Remove == signs. */
    ...
    control_event_hs_descriptor_requested(
        rend_data->onion_address, rend_data->auth_type,
                                       ...
                                       unsigned int failure_type)
{
    ...
    if (!rend_client_any_intro_points_usable(ent)) {
         ...
         */
        while (
            (conn = connection_get_by_type_state_rendquery(
...
rend_client_note_connection_attempt_ended(const rend_data_t *rend_data)
{
    ...
    if (cache_entry != NULL) {
        SMARTLIST_FOREACH(cache_entry->parsed->intro_nodes,
                          rend_intro_point_t *, ip, ip->timed_out = 0;);
    }
                                  ...
                                  const int strict, const int warnings)
{
    ...
    /* Get service ID so we can use it to query the failure cache. If we fail
     * to parse it, this cache entry is no good. */
...
again:
     ...
     * Clients used to try to get the TAP onion key from the consensus, but
     * this meant that hidden services could discover which consensus clients
     * have. */
...
{
    extend_info_t *extend_info = rend_client_get_random_intro_impl(
        entry, get_options()->StrictNodes, 0);
...
rend_client_circuit_cleanup_on_free(const circuit_t *circ)
{
    ...
    case CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT: {
        ...
        log_info(
            ...
            has_timed_out ? "Recording timeout."
    ...
    case CIRCUIT_PURPOSE_C_INTRODUCING: {
        /* Ignore if we were introducing and it timed out, we didn't pick an
         * exit point yet (IP) or the reason indicate that it was a redundant
         * IP. */
