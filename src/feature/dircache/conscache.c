 ...
 */
int
consensus_cache_register_with_sandbox(consensus_cache_t *cache, struct sandbox_cfg_elem_t **cfg)
{
#ifdef MUST_UNMAP_TO_UNLINK
...
consensus_cache_add(consensus_cache_t *cache, const config_line_t *labels, const uint8_t *data,
                    size_t datalen)
{
    char *fname = NULL;
    int r = storage_dir_save_labeled_to_file(cache->dir, labels, data, datalen, &fname);
    if (r < 0 || fname == NULL) {
        return NULL;
    }
    consensus_cache_entry_t *ent = tor_malloc_zero(sizeof(consensus_cache_entry_t));
    ent->magic = CCE_MAGIC;
    ent->fname = fname;
 ...
 */
consensus_cache_entry_t *
consensus_cache_find_first(consensus_cache_t *cache, const char *key, const char *value)
{
 ...
 */
void
consensus_cache_find_all(smartlist_t *out, consensus_cache_t *cache, const char *key,
                         const char *value)
{
    SMARTLIST_FOREACH_BEGIN (cache->entries, consensus_cache_entry_t *, ent) {
        if (ent->can_remove == 1) {
 ...
 */
void
consensus_cache_filter_list(smartlist_t *lst, const char *key, const char *value)
{
    if (BUG(lst == NULL))
 ...
 */
const char *
consensus_cache_entry_get_value(const consensus_cache_entry_t *ent, const char *key)
{
 ...
 */
int
consensus_cache_entry_get_body(const consensus_cache_entry_t *ent, const uint8_t **body_out,
                               size_t *sz_out)
{
    if (BUG(ent->magic != CCE_MAGIC))
    ...
    SMARTLIST_FOREACH_BEGIN (fnames, const char *, fname) {
        ...
        if (!map) {
            ...
            if (errno == ERANGE || errno == EINVAL) {
                ...
                log_warn(LD_FS, "Unable to map file %s from consensus cache: %s", escaped(fname),
                         strerror(errno));
            }
            continue;
        }
 ...
 */
static void
consensus_cache_entry_map(consensus_cache_t *cache, consensus_cache_entry_t *ent)
{
    if (ent->map)
        return;

    ent->map = storage_dir_map_labeled(cache->dir, ent->fname, NULL, &ent->body, &ent->bodylen);
    ent->unused_since = TIME_MAX;
}
