                          ...
                          const networkstatus_t *as_parsed)
{
    ...
    return consensus_queue_compression_work(consensus, consensus_len,
                                            as_parsed);
}
...
consdiffmgr_cleanup(void)
{
    ...
    // 1. Delete any consensus or diff or anything whose valid_after is too
    // old.
    ...
    // 2. Delete all diffs that lead to a consensus whose valid-after is not
    // the latest.
    ...
    // 3. Delete all consensuses except the most recent that are compressed
    // with an un-preferred method.
    for (int flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
        const char *flavname = networkstatus_get_flavor_name(flav);
        /* Determine the most recent consensus of this flavor */
        consensus_cache_find_all(consensuses, cdm_cache_get(), LABEL_DOCTYPE,
        ...
        SMARTLIST_FOREACH_BEGIN (consensuses, consensus_cache_entry_t *, ent) {
            const char *lv_sha3_uncompressed = consensus_cache_entry_get_value(
...
consdiffmgr_validate(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (objects, consensus_cache_entry_t *, obj) {
        ...
        int r =
            cdm_entry_get_sha3_value(sha3_expected, obj, LABEL_SHA3_DIGEST);
        if (r == -1) {
        ...
        if (r == 0) {
            crypto_digest256((char *)sha3_received, (const char *)body,
                             bodylen, DIGEST_SHA3_256);
        }
...
consdiffmgr_rescan_flavor_(consensus_flavor_t flavor)
{
    ...
    log_info(
        ...
        smartlist_len(matches), flavname, smartlist_len(compute_diffs_from));
    ...
    SMARTLIST_FOREACH_BEGIN (compute_diffs_from, consensus_cache_entry_t *,
                             c) {
...
consdiffmgr_consensus_load(void)
{
    ...
    for (int flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
        ...
        consensus_cache_find_all(matches, cdm_cache_get(), LABEL_FLAVOR,
                                 flavname);
        consensus_cache_filter_list(matches, LABEL_DOCTYPE, DOCTYPE_CONSENSUS);
        consensus_cache_entry_t *most_recent =
...
consdiffmgr_diffs_load(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (diffs, consensus_cache_entry_t *, diff) {
        ...
        if (cdm_entry_get_sha3_value(from_sha3, diff, LABEL_FROM_SHA3_DIGEST) <
            0)
            continue;
        if (cdm_entry_get_sha3_value(to_sha3, diff, LABEL_TARGET_SHA3_DIGEST) <
            0)
            continue;
...
consdiffmgr_ensure_space_for_files(int n)
{
    ...
    const int n_to_remove =
        n - consensus_cache_get_n_filenames_available(cache);
    if (n_to_remove <= 0) {
        // okay, finally!
        return 0;
    }
                  ...
                  size_t len, const config_line_t *labels_in)
{
    ...
    for (i = 0; i < n_methods; ++i) {
        ...
        size_t sz;
        if (0 ==
            tor_compress(&result, &sz, (const char *)input, len, method)) {
               ...
               const compressed_result_t *results, const char *description)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (body_out && bodylen_out && labels) {
            ...
            consensus_cache_entry_t *ent = consensus_cache_add(
                cdm_cache_get(), labels, body_out, bodylen_out);
            if (ent == NULL) {
...
consensus_diff_worker_threadfn(void *state_, void *work_)
{
    ...
    if (!lv_from_digest) {
         ...
         * from an older version of master to a newer one.  The older ones
         * didn't annotate their stored consensus objects with
         * sha3-digest-as-signed.
         */
        return WQ_RPL_REPLY; // LCOV_EXCL_LINE
    }
    ...
    char *consensus_diff;
    {
        ...
        consensus_diff = consensus_diff_generate(
            diff_from_nt, diff_from_nt_len, diff_to_nt, diff_to_nt_len);
        tor_free(owned1);
        tor_free(owned2);
    }
    if (!consensus_diff) {
    ...
    if (lv_to_valid_until)
        config_line_prepend(&common_labels, LABEL_VALID_UNTIL,
                            lv_to_valid_until);
    if (lv_to_fresh_until)
        config_line_prepend(&common_labels, LABEL_FRESH_UNTIL,
                            lv_to_fresh_until);
    if (lv_to_signatories)
        config_line_prepend(&common_labels, LABEL_SIGNATORIES,
                            lv_to_signatories);
...
consensus_diff_worker_replyfn(void *work_)
{
    ...
    for (u = 0; u < ARRAY_LENGTH(handles); ++u) {
        ...
        if (cache) {
            ...
            cdm_diff_ht_set_status(flav, from_sha3, to_sha3, method,
                                   this_status, h);
        } else {
                                           ...
                                           smartlist_t *out)
{
    ...
    if (s == NULL)
        return -1;
    smartlist_split_string(out, s, ",", SPLIT_SKIP_SPACE | SPLIT_STRIP_SPACE,
                           0);
    return 0;
}
