...
static consensus_cache_entry_handle_t
    *latest_consensus[N_CONSENSUS_FLAVORS][ARRAY_LENGTH(compress_consensus_with)];
...
static int consensus_queue_compression_work(const char *consensus, size_t consensus_len,
...
cdm_diff_eq(const cdm_diff_t *diff1, const cdm_diff_t *diff2)
{
    return fast_memeq(diff1->from_sha3, diff2->from_sha3, DIGEST256_LEN) &&
           diff1->flavor == diff2->flavor && diff1->compress_method == diff2->compress_method;
}

HT_PROTOTYPE(cdm_diff_ht, cdm_diff_t, node, cdm_diff_hash, cdm_diff_eq);
HT_GENERATE2(cdm_diff_ht, cdm_diff_t, node, cdm_diff_hash, cdm_diff_eq, 0.6, tor_reallocarray,
             tor_free_);
...
static cdm_diff_t *
cdm_diff_new(consensus_flavor_t flav, const uint8_t *from_sha3, const uint8_t *target_sha3,
 ...
 */
static int
cdm_diff_ht_check_and_note_pending(consensus_flavor_t flav, const uint8_t *from_sha3,
 ...
 */
static void
cdm_diff_ht_set_status(consensus_flavor_t flav, const uint8_t *from_sha3, const uint8_t *to_sha3,
                       compress_method_t method, int status, consensus_cache_entry_handle_t *handle)
{
    if (handle == NULL) {
    ...
    memset(&search, 0, sizeof(cdm_diff_t));
    search.flavor = flav;
    search.compress_method = method, memcpy(search.from_sha3, from_sha3, DIGEST256_LEN);
    ent = HT_FIND(cdm_diff_ht, &cdm_diff_ht, &search);
    if (!ent) {
 ...
 */
static void
cdm_diff_ht_purge(consensus_flavor_t flav, const uint8_t *unless_target_sha3_matches)
{
    cdm_diff_t **diff, **next;
    for (diff = HT_START(cdm_diff_ht, &cdm_diff_ht); diff; diff = next) {
        cdm_diff_t *this = *diff;

        if ((*diff)->cdm_diff_status == CDM_DIFF_PRESENT && flav == (*diff)->flavor) {
            ...
            if (unless_target_sha3_matches &&
                fast_memneq(unless_target_sha3_matches, (*diff)->target_sha3, DIGEST256_LEN)) {
...
cdm_cache_init(void)
{
    ...
    consdiffmgr_rescan_ev = mainloop_event_postloop_new(consdiffmgr_rescan_cb, NULL);
 ...
 */
static void
cdm_labels_prepend_sha3(config_line_t **labels, const char *label, const uint8_t *body,
                        size_t bodylen)
{
    uint8_t sha3_digest[DIGEST256_LEN];
    char hexdigest[HEX_DIGEST256_LEN + 1];
    crypto_digest256((char *)sha3_digest, (const char *)body, bodylen, DIGEST_SHA3_256);
    base16_encode(hexdigest, sizeof(hexdigest), (const char *)sha3_digest, sizeof(sha3_digest));
...
STATIC int
cdm_entry_get_sha3_value(uint8_t *digest_out, consensus_cache_entry_t *ent, const char *label)
{
    if (ent == NULL)
        return -1;
...
cdm_cache_lookup_consensus(consensus_flavor_t flavor, time_t valid_after)
{
    ...
    consensus_cache_find_all(matches, cdm_cache_get(), LABEL_VALID_AFTER, formatted_time);
...
get_max_age_to_cache(void)
{
    ...
    return 3600 * networkstatus_get_param(NULL, MAX_AGE_TO_CACHE_NAME, DEFAULT_MAX_AGE_TO_CACHE,
 ...
 * testing. */
int
consdiffmgr_add_consensus_nulterm(const char *consensus, const networkstatus_t *as_parsed)
{
                          ...
                          const networkstatus_t *as_parsed)
{
    ...
    consensus_cache_entry_t *entry = cdm_cache_lookup_consensus(flavor, valid_after);
    if (entry) {
 ...
 */
consdiff_status_t
consdiffmgr_find_consensus(struct consensus_cache_entry_t **entry_out, consensus_flavor_t flavor,
                           compress_method_t method)
{
 ...
 */
consdiff_status_t
consdiffmgr_find_diff_from(consensus_cache_entry_t **entry_out, consensus_flavor_t flavor,
                           int digest_type, const uint8_t *digest, size_t digestlen,
                           compress_method_t method)
{
...
consdiffmgr_cleanup(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (objects, consensus_cache_entry_t *, ent) {
        const char *lv_valid_after = consensus_cache_entry_get_value(ent, LABEL_VALID_AFTER);
        if (!lv_valid_after) {
            log_debug(LD_DIRSERV, "Ignoring entry because it had no %s label", LABEL_VALID_AFTER);
            continue;
        }
    ...
    for (int flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
        ...
        consensus_cache_find_all(consensuses, cdm_cache_get(), LABEL_DOCTYPE, DOCTYPE_CONSENSUS);
        consensus_cache_filter_list(consensuses, LABEL_FLAVOR, flavname);
        consensus_cache_entry_t *most_recent = sort_and_find_most_recent(consensuses);
        if (most_recent == NULL)
            continue;
        const char *most_recent_sha3 =
            consensus_cache_entry_get_value(most_recent, LABEL_SHA3_DIGEST_UNCOMPRESSED);
        if (BUG(most_recent_sha3 == NULL))
            continue; // LCOV_EXCL_LINE

        /* consider all such-flavored diffs, and look to see if they match. */
        consensus_cache_find_all(diffs, cdm_cache_get(), LABEL_DOCTYPE, DOCTYPE_CONSENSUS_DIFF);
...
consdiffmgr_validate(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (objects, consensus_cache_entry_t *, obj) {
        ...
        if (r == 0) {
            crypto_digest256((char *)sha3_received, (const char *)body, bodylen, DIGEST_SHA3_256);
        }
...
consdiffmgr_rescan_flavor_(consensus_flavor_t flavor)
{
    ...
    //    to diff to it.
    const char *methodname = compression_method_get_name(RETAIN_CONSENSUS_COMPRESSED_WITH_METHOD);
    ...
    diffs = smartlist_new();
    consensus_cache_find_all(diffs, cdm_cache_get(), LABEL_VALID_AFTER, most_recent_valid_after);
    consensus_cache_filter_list(diffs, LABEL_DOCTYPE, DOCTYPE_CONSENSUS_DIFF);
    consensus_cache_filter_list(diffs, LABEL_FLAVOR, flavname);
    have_diff_from = strmap_new();
    SMARTLIST_FOREACH_BEGIN (diffs, consensus_cache_entry_t *, diff) {
        const char *va = consensus_cache_entry_get_value(diff, LABEL_FROM_VALID_AFTER);
        if (BUG(va == NULL))
            continue; // LCOV_EXCL_LINE
    ...
    log_info(LD_DIRSERV,
             ...
             smartlist_len(matches) - smartlist_len(compute_diffs_from), smartlist_len(matches),
             flavname, smartlist_len(compute_diffs_from));
    ...
    SMARTLIST_FOREACH_BEGIN (compute_diffs_from, consensus_cache_entry_t *, c) {
        ...
        if (cdm_entry_get_sha3_value(this_sha3, c, LABEL_SHA3_DIGEST_AS_SIGNED) < 0) {
            // Not actually a bug, since we might be running with a directory
            // with stale files from before the #22143 fixes.
            continue;
        }
        if (cdm_diff_ht_check_and_note_pending(flavor, this_sha3, most_recent_sha3)) {
...
consdiffmgr_consensus_load(void)
{
    ...
    for (int flav = 0; flav < N_CONSENSUS_FLAVORS; ++flav) {
        ...
        if (BUG(most_recent_sha3 == NULL))
            continue; // LCOV_EXCL_LINE
        consensus_cache_filter_list(matches, LABEL_SHA3_DIGEST_UNCOMPRESSED, most_recent_sha3);
        ...
        SMARTLIST_FOREACH_BEGIN (matches, consensus_cache_entry_t *, ent) {
            const char *lv_compression =
                consensus_cache_entry_get_value(ent, LABEL_COMPRESSION_TYPE);
            compress_method_t method = compression_method_get_by_name(lv_compression);
            int pos = consensus_compression_method_pos(method);
            if (pos < 0)
                continue;
...
consdiffmgr_diffs_load(void)
{
    ...
    consensus_cache_find_all(diffs, cdm_cache_get(), LABEL_DOCTYPE, DOCTYPE_CONSENSUS_DIFF);
    SMARTLIST_FOREACH_BEGIN (diffs, consensus_cache_entry_t *, diff) {
        const char *lv_flavor = consensus_cache_entry_get_value(diff, LABEL_FLAVOR);
        if (!lv_flavor)
            continue;
        int flavor = networkstatus_parse_flavor_name(lv_flavor);
        if (flavor < 0)
            continue;
        const char *lv_compression = consensus_cache_entry_get_value(diff, LABEL_COMPRESSION_TYPE);
        ...
        cdm_diff_ht_set_status(flavor, from_sha3, to_sha3, method, CDM_DIFF_PRESENT,
...
consdiffmgr_set_cache_flags(void)
{
    ...
    smartlist_t *objects = smartlist_new();
    consensus_cache_find_all(objects, cdm_cache_get(), LABEL_DOCTYPE, DOCTYPE_CONSENSUS);
    SMARTLIST_FOREACH_BEGIN (objects, consensus_cache_entry_t *, ent) {
        consensus_cache_entry_mark_for_aggressive_release(ent);
    } SMARTLIST_FOREACH_END (ent);
    smartlist_free(objects);
 ...
 */
static int
compress_multiple(compressed_result_t *results_out, int n_methods, const compress_method_t *methods,
                  const uint8_t *input, size_t len, const config_line_t *labels_in)
{
    int rv = 0;
    int i;
    for (i = 0; i < n_methods; ++i) {
        ...
        if (0 == tor_compress(&result, &sz, (const char *)input, len, method)) {
            ...
            cdm_labels_prepend_sha3(&results_out[i].labels, LABEL_SHA3_DIGEST, results_out[i].body,
                                    results_out[i].bodylen);
            config_line_prepend(&results_out[i].labels, LABEL_COMPRESSION_TYPE, methodname);
        } else {
            rv = -1;
        }
    }
...
store_multiple(consensus_cache_entry_handle_t **handles_out, int n,
               const compress_method_t *methods, const compressed_result_t *results,
               const char *description)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (body_out && bodylen_out && labels) {
            ...
            log_info(LD_DIRSERV, "Adding %s, compressed with %s", description, methodname);
            ...
            if (ent == NULL) {
                ...
                log_fn_ratelim(&cant_store_ratelim, LOG_WARN, LD_FS,
                               "Unable to store object %s compressed with %s.", description,
                               methodname);
                continue;
            }
                      ...
                      consensus_cache_entry_t *ent)
{
    ...
    *owned_out = NULL;
    ...
    const char *lv_compression = consensus_cache_entry_get_value(ent, LABEL_COMPRESSION_TYPE);
    compress_method_t method = NO_METHOD;
    ...
    if (method == NO_METHOD) {
        ...
        *outlen = bodylen;
        ...
        rv = tor_uncompress(owned_out, outlen, (const char *)body, bodylen, method, 1, LOG_WARN);
        *out = *owned_out;
    }
    return rv;
}
...
consensus_diff_worker_threadfn(void *state_, void *work_)
{
    ...
    const char *lv_from_digest =
        consensus_cache_entry_get_value(job->diff_from, LABEL_SHA3_DIGEST_AS_SIGNED);
    const char *lv_from_flavor = consensus_cache_entry_get_value(job->diff_from, LABEL_FLAVOR);
    const char *lv_to_flavor = consensus_cache_entry_get_value(job->diff_to, LABEL_FLAVOR);
    const char *lv_to_digest =
        consensus_cache_entry_get_value(job->diff_to, LABEL_SHA3_DIGEST_UNCOMPRESSED);
    ...
    if (BUG(!lv_to_valid_after) || BUG(!lv_from_valid_after) || BUG(!lv_from_flavor) ||
        BUG(!lv_to_flavor)) {
        return WQ_RPL_REPLY; // LCOV_EXCL_LINE
    }
    ...
    char *consensus_diff;
    {
        ...
        if (uncompress_or_set_ptr(&diff_from_nt, &diff_from_nt_len, &owned1, job->diff_from) < 0) {
            return WQ_RPL_REPLY;
        }
        if (uncompress_or_set_ptr(&diff_to_nt, &diff_to_nt_len, &owned2, job->diff_to) < 0) {
            tor_free(owned1);
            return WQ_RPL_REPLY;
        }
        ...
        consensus_diff =
            consensus_diff_generate(diff_from_nt, diff_from_nt_len, diff_to_nt, diff_to_nt_len);
        tor_free(owned1);
        tor_free(owned2);
    }
    if (!consensus_diff) {
    ...
    cdm_labels_prepend_sha3(&common_labels, LABEL_SHA3_DIGEST_UNCOMPRESSED, job->out[0].body,
                            job->out[0].bodylen);
    config_line_prepend(&common_labels, LABEL_FROM_VALID_AFTER, lv_from_valid_after);
    ...
    cdm_labels_prepend_sha3(&job->out[0].labels, LABEL_SHA3_DIGEST, job->out[0].body,
                            job->out[0].bodylen);

    compress_multiple(job->out + 1, n_diff_compression_methods() - 1, compress_diffs_with + 1,
...
#define consensus_diff_worker_job_free(job) \
    FREE_AND_NULL(consensus_diff_worker_job_t, consensus_diff_worker_job_free_, (job))
...
consensus_diff_worker_replyfn(void *work_)
{
    ...
    const char *lv_to_digest =
        consensus_cache_entry_get_value(job->diff_to, LABEL_SHA3_DIGEST_UNCOMPRESSED);
    const char *lv_flavor = consensus_cache_entry_get_value(job->diff_to, LABEL_FLAVOR);
    ...
    int flav = -1;
    int cache = 1;
    if (BUG(cdm_entry_get_sha3_value(from_sha3, job->diff_from, LABEL_SHA3_DIGEST_AS_SIGNED) < 0))
        cache = 0;
    if (BUG(cdm_entry_get_sha3_value(to_sha3, job->diff_to, LABEL_SHA3_DIGEST_UNCOMPRESSED) < 0))
        cache = 0;
    ...
    tor_snprintf(description, sizeof(description), "consensus diff from %s to %s", lv_from_digest,
                 lv_to_digest);

    int status = store_multiple(handles, n_diff_compression_methods(), compress_diffs_with,
                                job->out, description);
 ...
 */
static int
consensus_diff_queue_diff_work(consensus_cache_entry_t *diff_from, consensus_cache_entry_t *diff_to)
{
    tor_assert(in_main_thread());
...
#define consensus_compress_worker_job_free(job) \
    FREE_AND_NULL(consensus_compress_worker_job_t, consensus_compress_worker_job_free_, (job))
...
consensus_compress_worker_threadfn(void *state_, void *work_)
{
    ...
    cdm_labels_prepend_sha3(&labels, LABEL_SHA3_DIGEST_UNCOMPRESSED, (const uint8_t *)consensus,
                            bodylen);
    {
        const char *start, *end;
        if (router_get_networkstatus_v3_signed_boundaries(consensus, bodylen, &start, &end) < 0) {
            start = consensus;
            end = consensus + bodylen;
        }
        cdm_labels_prepend_sha3(&labels, LABEL_SHA3_DIGEST_AS_SIGNED, (const uint8_t *)start,
                                end - start);
    }
    config_line_prepend(&labels, LABEL_FLAVOR, flavname);
    config_line_prepend(&labels, LABEL_DOCTYPE, DOCTYPE_CONSENSUS);

    compress_multiple(job->out, n_consensus_compression_methods(), compress_consensus_with,
...
consensus_compress_worker_replyfn(void *work_)
{
    ...
    consensus_cache_entry_handle_t *handles[ARRAY_LENGTH(compress_consensus_with)];
    memset(handles, 0, sizeof(handles));

    store_multiple(handles, n_consensus_compression_methods(), compress_consensus_with, job->out,
                   "consensus");
    mark_cdm_cache_dirty();
                                 ...
                                 const networkstatus_t *as_parsed)
{
    ...
    if (as_parsed->voters) {
        ...
        SMARTLIST_FOREACH_BEGIN (as_parsed->voters, networkstatus_voter_info_t *, vi) {
...
int
consensus_cache_entry_get_voter_id_digests(const consensus_cache_entry_t *ent, smartlist_t *out)
{
    tor_assert(ent);
    tor_assert(out);
    const char *s;
...
int
consensus_cache_entry_get_fresh_until(const consensus_cache_entry_t *ent, time_t *out)
{
    tor_assert(ent);
    tor_assert(out);
    const char *s;
...
int
consensus_cache_entry_get_valid_until(const consensus_cache_entry_t *ent, time_t *out)
{
    tor_assert(ent);
    tor_assert(out);
 ...
 * *<b>out</b> and return 0, or return -1 if no such time was recorded. */
int
consensus_cache_entry_get_valid_after(const consensus_cache_entry_t *ent, time_t *out)
{
    tor_assert(ent);
    tor_assert(out);
