...
consdiff_status_t consdiffmgr_find_diff_from(struct consensus_cache_entry_t **entry_out,
                                             consensus_flavor_t flavor, int digest_type,
                                             const uint8_t *digest, size_t digestlen,
                                             enum compress_method_t method);

int consensus_cache_entry_get_voter_id_digests(const struct consensus_cache_entry_t *ent,
                                               smartlist_t *out);
...
STATIC struct consensus_cache_entry_t *cdm_cache_lookup_consensus(consensus_flavor_t flavor,
                                                                  time_t valid_after);
STATIC int cdm_entry_get_sha3_value(uint8_t *digest_out, struct consensus_cache_entry_t *ent,
                                    const char *label);
STATIC int uncompress_or_set_ptr(const char **out, size_t *outlen, char **owned_out,
                                 struct consensus_cache_entry_t *ent);
#endif /* defined(CONSDIFFMGR_PRIVATE) */

#ifdef TOR_UNIT_TESTS
int consdiffmgr_add_consensus_nulterm(const char *consensus, const networkstatus_t *as_parsed);
#endif
