 ...
 */
int
directory_too_idle_to_fetch_descriptors(const or_options_t *options, time_t now)
{
    ...
    if (!strcmp(key, "all")) {
        ...
        SMARTLIST_FOREACH_BEGIN(rl->routers, const routerinfo_t *, r)
        {
            ...
            spooled = spooled_resource_new(
                source, (const uint8_t *)r->cache_info.identity_digest,
                DIGEST_LEN);
                               ...
                               time_t *published_out)
{
    if (spooled->spool_eagerly) {
        ...
        size_t result =
            compressed ? cached->dir_compressed_len : cached->dir_len;
        return result;
    }
}
...
spooled_resource_flush_some(spooled_resource_t *spooled, dir_connection_t *conn)
{
    if (spooled->spool_eagerly) {
        ...
        if (cached == NULL && cce == NULL) {
            /* The cached_dir_t hasn't been materialized yet. So let's look it
             * up. */
            cached = spooled->cached_dir_ref =
        ...
        connection_dir_buf_add(ptr + spooled->cached_dir_offset, bytes, conn,
                               0);
                                            ...
                                            int *n_expired_out)
{
    ...
    SMARTLIST_FOREACH_BEGIN(spool, spooled_resource_t *, spooled)
    {
        ...
        size_t sz = spooled_resource_estimate_size(spooled, conn, compression,
                                                   &published);
        if (published < cutoff) {
            ++n_expired;
...
connection_dirserv_flushed_some(dir_connection_t *conn)
{
    ...
    if (conn->compress_state) {
        /* Flush the compression state: there could be more bytes pending in
         * there, and we don't want to omit bytes. */
