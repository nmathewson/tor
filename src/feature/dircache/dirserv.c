...
static int spooled_resource_lookup_body(const spooled_resource_t *spooled, int conn_is_encrypted,
                                        const uint8_t **body_out, size_t *size_out,
...
directory_caches_dir_info(const or_options_t *options)
{
    ...
    return !router_my_exit_policy_is_reject_star() && should_refuse_unknown_exits(options);
}
...
new_cached_dir(char *s, time_t published)
{
    ...
    if (tor_compress(&(d->dir_compressed), &(d->dir_compressed_len), d->dir, d->dir_len,
                     ZLIB_METHOD)) {
...
void
dirserv_set_cached_consensus_networkstatus(const char *networkstatus, size_t networkstatus_len,
                                           const char *flavor_name, const common_digests_t *digests,
                                           const uint8_t *sha3_as_signed, time_t published)
{
    ...
    new_networkstatus =
        new_cached_dir(tor_memdup_nulterm(networkstatus, networkstatus_len), published);
    memcpy(&new_networkstatus->digests, digests, sizeof(common_digests_t));
    memcpy(&new_networkstatus->digest_sha3_as_signed, sha3_as_signed, DIGEST256_LEN);
    old_networkstatus = strmap_set(cached_consensuses, flavor_name, new_networkstatus);
...
int
dir_split_resource_into_spoolable(const char *resource, dir_spool_source_t source,
                                  smartlist_t *spool_out, int *compressed_out, int flags)
{
    smartlist_t *fingerprints = smartlist_new();

    tor_assert(flags & (DSR_HEX | DSR_BASE64));
    const size_t digest_len = (flags & DSR_DIGEST256) ? DIGEST256_LEN : DIGEST_LEN;

    int r = dir_split_resource_into_fingerprints(resource, fingerprints, compressed_out, flags);
    /* This is not a very efficient implementation XXXX */
    SMARTLIST_FOREACH_BEGIN (fingerprints, uint8_t *, digest) {
        spooled_resource_t *spooled = spooled_resource_new(source, digest, digest_len);
        if (spooled)
 ...
 */
int
dirserv_get_routerdesc_spool(smartlist_t *spool_out, const char *key, dir_spool_source_t source,
    ...
    *msg_out = NULL;
    ...
    if (!strcmp(key, "all")) {
        ...
        SMARTLIST_FOREACH_BEGIN (rl->routers, const routerinfo_t *, r) {
            ...
            spooled = spooled_resource_new(source, (const uint8_t *)r->cache_info.identity_digest,
                                           DIGEST_LEN);
        ...
        if (ri)
            smartlist_add(spool_out,
                          spooled_resource_new(
                              source, (const uint8_t *)ri->cache_info.identity_digest, DIGEST_LEN));
    } else if (!strcmpstart(key, "d/")) {
        key += strlen("d/");
        dir_split_resource_into_spoolable(key, source, spool_out, NULL, DSR_HEX | DSR_SORT_UNIQ);
    ...
    if (!conn_is_encrypted) {
        ...
        SMARTLIST_FOREACH_BEGIN (spool_out, spooled_resource_t *, spooled) {
            ...
            size_t bodylen = 0;
            int r = spooled_resource_lookup_body(spooled, conn_is_encrypted, &body, &bodylen, NULL);
...
spooled_resource_t *
spooled_resource_new(dir_spool_source_t source, const uint8_t *digest, size_t digestlen)
{
...
spooled_resource_new_from_cache_entry(consensus_cache_entry_t *entry)
{
    ...
    int r = consensus_cache_entry_get_body(entry, &spooled->cce_body, &spooled->cce_len);
    if (r == 0) {
        return spooled;
    } else {
        spooled_resource_free(spooled);
 ...
 */
static size_t
spooled_resource_estimate_size(const spooled_resource_t *spooled, dir_connection_t *conn,
                               int compressed, time_t *published_out)
{
    if (spooled->spool_eagerly) {
        const uint8_t *body = NULL;
        size_t bodylen = 0;
        int r = spooled_resource_lookup_body(spooled, connection_dir_is_encrypted(conn), &body,
                                             &bodylen, published_out);
...
typedef enum { SRFS_ERR = -1, SRFS_MORE = 0, SRFS_DONE } spooled_resource_flush_status_t;
...
spooled_resource_flush_some(spooled_resource_t *spooled, dir_connection_t *conn)
{
    if (spooled->spool_eagerly) {
        ...
        if (cached == NULL && cce == NULL) {
            ...
            cached = spooled->cached_dir_ref = spooled_resource_lookup_cached_dir(spooled, NULL);
            if (!cached) {
                /* Absent objects count as done. */
                return SRFS_DONE;
            }
 ...
 */
static cached_dir_t *
spooled_resource_lookup_cached_dir(const spooled_resource_t *spooled, time_t *published_out)
{
 ...
 * On failure return -1. */
static int
spooled_resource_lookup_body(const spooled_resource_t *spooled, int conn_is_encrypted,
                             const uint8_t **body_out, size_t *size_out, time_t *published_out)
{
    ...
    case DIR_SPOOL_MICRODESC: {
        microdesc_t *md = microdesc_cache_lookup_by_digest256(get_microdesc_cache(),
...
void
dirserv_spool_remove_missing_and_guess_size(dir_connection_t *conn, time_t cutoff, int compression,
                                            size_t *size_out, int *n_expired_out)
{
    if (BUG(!conn))
        return;
    ...
    SMARTLIST_FOREACH_BEGIN (spool, spooled_resource_t *, spooled) {
        ...
        size_t sz = spooled_resource_estimate_size(spooled, conn, compression, &published);
        if (published < cutoff) {
            ++n_expired;
...
get_signed_descriptor_by_fp(const uint8_t *fp, int extrainfo)
{
    if (router_digest_is_me((const char *)fp)) {
        ...
        if (ri) {
            if (extrainfo)
                return extrainfo_get_by_descriptor_digest(ri->cache_info.extra_info_digest);
            else
...
connection_dirserv_flushed_some(dir_connection_t *conn)
{
    ...
    while (connection_get_outbuf_len(TO_CONN(conn)) < DIRSERV_BUFFER_MIN &&
        ...
        spooled_resource_t *spooled = smartlist_get(conn->spool, smartlist_len(conn->spool) - 1);
...
dir_conn_clear_spool(dir_connection_t *conn)
{
    if (!conn || !conn->spool)
        return;
    SMARTLIST_FOREACH(conn->spool, spooled_resource_t *, s, spooled_resource_free(s));
    smartlist_free(conn->spool);
    conn->spool = NULL;
}
