           ...
           size_t req_body_len))
{
    ...
    if ((header = http_get_header(headers, "If-Modified-Since: "))) {
        ...
        if (parse_http_time(header, &tm) == 0) {
            if (tor_timegm(&tm, &if_modified_since) < 0) {
                ...
                log_debug(LD_DIRSERV, "If-Modified-Since is '%s'.",
                          escaped(header));
            }
        }
    ...
    url_mem = url;
    {
        ...
        zlib_compressed_in_url =
            url_len > 2 && !strcmp(url + url_len - 2, ".z");
...
parse_or_diff_from_header(smartlist_t **digests_out, const char *headers)
{
    ...
    *digests_out = smartlist_new();
    ...
    SMARTLIST_FOREACH_BEGIN (hex_digests, const char *, hex) {
        ...
        if (!parse_one_diff_hash(
                digest, hex, "X-Or-Diff-From-Consensus header", "ignoring")) {
               ...
               compress_method_t *compression_used_out)
{
    ...
    SMARTLIST_FOREACH_BEGIN (digests, const uint8_t *, diff_from) {
        if (consdiffmgr_find_diff_from(
                &result, flav, DIGEST_SHA3_256, diff_from, DIGEST256_LEN,
                             ...
                             const get_handler_args_t *args)
{
    ...
    if (cached_consensus && have_valid_after &&
        ...
        warn_consensus_is_not_reasonably_live(cached_consensus, req.flavor,
                                              now, 1);
        geoip_note_ns_response(GEOIP_REJECT_NOT_FOUND);
        goto done;
    ...
    // compressed. Decompress them using `compression_used`. See fallback code
    // in find_best_consensus() and find_best_diff().
...
handle_get_status_vote(dir_connection_t *conn, const get_handler_args_t *args)
{
    const char *url = args->url;
    {
        ...
        if (smartlist_len(items)) {
            ...
            /* We're taking cached_dir_t objects. We only have them
...
handle_get_microdesc(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    int clear_spool = 1;
    {
        ...
        dir_split_resource_into_spoolable(
            url + strlen("/tor/micro/d/"), DIR_SPOOL_MICRODESC, conn->spool,
            NULL, DSR_DIGEST256 | DSR_BASE64 | DSR_SORT_UNIQ);
...
handle_get_descriptor(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    if (!strcmpstart(url, "/tor/server/") ||
        ...
        if (!strcmpstart(url, "d/")) {
            source = is_extra ? DIR_SPOOL_EXTRA_BY_DIGEST
        ...
        int n_expired = 0;
        dirserv_spool_remove_missing_and_guess_size(
            conn, publish_cutoff, compress_method != NO_METHOD, &size_guess,
            &n_expired);
        ...
        if (get_options()->BridgeAuthoritativeDir &&
            ...
            SMARTLIST_FOREACH_BEGIN (conn->spool, spooled_resource_t *,
                                     spooled) {
                const routerinfo_t *router =
                    router_get_by_id_digest((const char *)spooled->digest);
                /* router can be NULL here when the bridge auth is asked for
                 * its own descriptor. */
...
handle_get_keys(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    const time_t if_modified_since = args->if_modified_since;
    {
        ...
        if (connection_dir_is_global_write_low(
                TO_CONN(conn), compress_method != NO_METHOD ? len / 2 : len)) {
            write_short_http_response(conn, 503,
                                      "Directory busy, try again later");
            goto keys_done;
        }
        ...
        if (compress_method != NO_METHOD) {
            conn->compress_state = tor_compress_new(
                1, compress_method, choose_compression_level(len));
        }
                            ...
                            const get_handler_args_t *args)
{
    ...
    if (!connection_dir_is_anonymous(conn)) {
        write_short_http_response(
            conn, 503, "Rejecting single hop HS v3 descriptor request");
        goto done;
    }
    ...
    if (options->V3BandwidthsFile) {
        ...
        if (bandwidth != NULL) {
            ...
            write_http_response_header(
                conn, compress_method != NO_METHOD ? -1 : len, compress_method,
...
handle_get_robots(dir_connection_t *conn, const get_handler_args_t *args)
{
    (void)args;
    {
        ...
        write_http_response_header(conn, len, NO_METHOD,
...
handle_post_hs_descriptor(const char *url, const char *body)
{
     ...
     * Use the end position just after the version and check for the command.
     */
           ...
           size_t body_len))
{
    ...
    if (connection_dir_is_encrypted(conn) &&
        ...
        if (rend_cache_store_v2_desc_as_dir(body) < 0) {
            ...
            write_short_http_response(
                conn, 400, "Invalid v2 service descriptor rejected");
        } else {
            write_short_http_response(conn, 200,
    ...
    if (!strcmpstart(url, "/tor/hs/")) {
        if (!connection_dir_is_anonymous(conn)) {
            write_short_http_response(
                conn, 503, "Rejecting single hop HS descriptor post");
            goto done;
        }
