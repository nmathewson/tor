...
write_http_response_headers(dir_connection_t *conn, ssize_t length,
                            compress_method_t method, const char *extra_headers,
                            long cache_lifetime)
{
    ...
    if (!strcmpstart(url, "/tor/server/") ||
        ...
        if (get_options()->BridgeAuthoritativeDir &&
            ...
            SMARTLIST_FOREACH_BEGIN(conn->spool, spooled_resource_t *, spooled)
            {
                ...
                /* router can be NULL here when the bridge auth is asked for its
                 * own descriptor. */
    ...
    if (connection_dir_is_encrypted(conn)) {
        ...
        if (rend_valid_descriptor_id(query)) {
            ...
            switch (rend_cache_lookup_v2_desc_as_dir(query, &descp)) {
                case 1: /* valid */
                    write_http_response_header(conn, strlen(descp), NO_METHOD,
                                               0);
