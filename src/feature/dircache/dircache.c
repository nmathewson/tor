 ...
 */
static void
write_short_http_response(dir_connection_t *conn, int status, const char *reason_phrase)
{
    char *buf = NULL;
...
static void
write_http_response_header_impl(dir_connection_t *conn, ssize_t length, const char *type,
                                const char *encoding, const char *extra_headers,
                                long cache_lifetime)
{
 ...
 * encoding */
static void
write_http_response_headers(dir_connection_t *conn, ssize_t length, compress_method_t method,
                            const char *extra_headers, long cache_lifetime)
{
    write_http_response_header_impl(conn, length, "text/plain", compression_method_get_name(method),
                                    extra_headers, cache_lifetime);
}

/** As write_http_response_headers, but assumes extra_headers is NULL */
static void
write_http_response_header(dir_connection_t *conn, ssize_t length, compress_method_t method,
                           long cache_lifetime)
{
...
static compress_method_t srv_meth_pref_precompressed[] = {LZMA_METHOD, ZSTD_METHOD, ZLIB_METHOD,
                                                          GZIP_METHOD, NO_METHOD};

/** Array of compression methods to use (if supported) for serving
 * streamed data, ordered from best to worst. */
static compress_method_t srv_meth_pref_streaming_compression[] = {ZSTD_METHOD, ZLIB_METHOD,
                                                                  GZIP_METHOD, NO_METHOD};
 ...
 */
static int
client_likes_consensus(const struct consensus_cache_entry_t *ent, const char *want_url)
{
 ...
 */
static void
warn_consensus_is_not_reasonably_live(const struct consensus_cache_entry_t *consensus,
                                      const char *flavor, time_t now, bool is_too_new)
{
#define NOT_REASONABLY_LIVE_WARNING_INTERVAL (60 * 60)
    static ratelim_t warned[2] = {RATELIM_INIT(NOT_REASONABLY_LIVE_WARNING_INTERVAL),
    ...
    if (dupes) {
        ...
        log_warn(LD_DIRSERV,
                 ...
                 flavor ? flavor : "", flavor ? " " : "", is_too_new ? "new" : "old",
                 is_too_new ? "after" : "until", timestamp,
 ...
 */
static int
parse_one_diff_hash(uint8_t *digest, const char *hex, const char *location, const char *action)
{
    if (base16_decode((char *)digest, DIGEST256_LEN, hex, strlen(hex)) == DIGEST256_LEN) {
        return 0;
    } else {
        log_fn(LOG_PROTOCOL_WARN, LD_DIR, "%s contained bogus digest %s; %s.", location,
               escaped(hex), action);
        return -1;
    }
}
...
parse_or_diff_from_header(smartlist_t **digests_out, const char *headers)
{
    ...
    *digests_out = smartlist_new();
    smartlist_split_string(hex_digests, hdr, " ", SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, -1);
    SMARTLIST_FOREACH_BEGIN (hex_digests, const char *, hex) {
        uint8_t digest[DIGEST256_LEN];
        if (!parse_one_diff_hash(digest, hex, "X-Or-Diff-From-Consensus header", "ignoring")) {
...
find_best_diff(const smartlist_t *digests, int flav, unsigned compression_methods,
               compress_method_t *compression_used_out)
{
    ...
    SMARTLIST_FOREACH_BEGIN (digests, const uint8_t *, diff_from) {
        ...
        for (u = 0; u < ARRAY_LENGTH(srv_meth_pref_precompressed); ++u) {
            ...
            if (consdiffmgr_find_diff_from(&result, flav, DIGEST_SHA3_256, diff_from, DIGEST256_LEN,
...
static struct consensus_cache_entry_t *
find_best_consensus(int flav, unsigned compression_methods, compress_method_t *compression_used_out)
{
    struct consensus_cache_entry_t *result = NULL;
    unsigned u;
    ...
    for (u = 0; u < ARRAY_LENGTH(srv_meth_pref_precompressed); ++u) {
        ...
        if (consdiffmgr_find_consensus(&result, flav, method) == CONSDIFF_AVAILABLE) {
            tor_assert_nonfatal(result);
            *compression_used_out = method;
            return result;
        }
    ...
    if (consdiffmgr_find_consensus(&result, flav, FALLBACK_COMPRESS_METHOD) == CONSDIFF_AVAILABLE) {
        tor_assert_nonfatal(result);
        *compression_used_out = FALLBACK_COMPRESS_METHOD;
        return result;
    }
...
static int
digest_list_contains_best_consensus(consensus_flavor_t flavor, const smartlist_t *digests)
{
 ...
 */
static int
parse_consensus_request(parsed_consensus_request_t *out, const get_handler_args_t *args)
{
    ...
    const char CONSENSUS_FLAVORED_PREFIX[] = "/tor/status-vote/current/consensus-";
    ...
    if (diff_hash_in_url) {
        ...
        out->diff_only = 1;
        int ok = !parse_one_diff_hash(diff_from, diff_hash_in_url, "URL", "rejecting");
        tor_free(diff_hash_in_url);
        if (ok) {
            smartlist_add(out->diff_from_digests, tor_memdup(diff_from, DIGEST256_LEN));
        } else {
            return -1;
        }
    } else {
 ...
 */
static int
handle_get_current_consensus(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    if (!cached_consensus) {
        cached_consensus =
            find_best_consensus(req.flav, args->compression_supported, &compression_used);
    }

    time_t valid_after, fresh_until, valid_until;
    int have_valid_after = 0, have_fresh_until = 0, have_valid_until = 0;
    if (cached_consensus) {
        have_valid_after = !consensus_cache_entry_get_valid_after(cached_consensus, &valid_after);
        have_fresh_until = !consensus_cache_entry_get_fresh_until(cached_consensus, &fresh_until);
        have_valid_until = !consensus_cache_entry_get_valid_until(cached_consensus, &valid_until);
    }
    ...
    if (cached_consensus && have_valid_after &&
        ...
        warn_consensus_is_not_reasonably_live(cached_consensus, req.flavor, now, 1);
        geoip_note_ns_response(GEOIP_REJECT_NOT_FOUND);
        goto done;
        ...
        warn_consensus_is_not_reasonably_live(cached_consensus, req.flavor, now, 0);
        geoip_note_ns_response(GEOIP_REJECT_NOT_FOUND);
        goto done;
    }
    ...
    dirserv_spool_remove_missing_and_guess_size(
        conn, if_modified_since, compress_method != NO_METHOD, &size_guess, &n_expired);
    ...
    if (connection_dir_is_global_write_low(TO_CONN(conn), size_guess)) {
        log_debug(LD_DIRSERV, "Client asked for network status lists, but we've been "
    ...
    if (tor_addr_parse(&addr, (TO_CONN(conn))->address) >= 0) {
        geoip_note_client_seen(GEOIP_CLIENT_NETWORKSTATUS, &addr, NULL, time(NULL));
        ...
        if (conn->dirreq_id)
            ...
            geoip_start_dirreq(TO_CONN(conn)->global_identifier, size_guess, DIRREQ_DIRECT);
    }
    ...
    write_http_response_headers(conn, -1,
                                compress_method == NO_METHOD ? NO_METHOD : compression_used,
                                vary_header, smartlist_len(conn->spool) == 1 ? lifetime : 0);

    if (compress_method == NO_METHOD && smartlist_len(conn->spool))
        conn->compress_state = tor_compress_new(0, compression_used, HIGH_COMPRESSION);
...
handle_get_status_vote(dir_connection_t *conn, const get_handler_args_t *args)
{
    const char *url = args->url;
    {
        ...
        tor_assert_nonfatal(smartlist_len(items) == 0 || smartlist_len(dir_items) == 0);
        ...
        const compress_method_t compress_method =
            find_best_compression_method(args->compression_supported & mask, streaming);

        SMARTLIST_FOREACH(dir_items, cached_dir_t *, d,
                          body_len +=
                          compress_method != NO_METHOD ? d->dir_compressed_len : d->dir_len);
        ...
        write_http_response_header(conn, body_len ? body_len : -1, compress_method, lifetime);

        if (smartlist_len(items)) {
            if (compress_method != NO_METHOD) {
                conn->compress_state =
                    tor_compress_new(1, compress_method, choose_compression_level(estimated_len));
            }

            SMARTLIST_FOREACH(items, const char *, c,
                              connection_dir_buf_add(c, strlen(c), conn, c_sl_idx == c_sl_len - 1));
        } else {
            SMARTLIST_FOREACH(dir_items, cached_dir_t *, d,
                              connection_buf_add(
...
handle_get_microdesc(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    int clear_spool = 1;
    {
        ...
        dir_split_resource_into_spoolable(url + strlen("/tor/micro/d/"), DIR_SPOOL_MICRODESC,
                                          conn->spool, NULL,
                                          DSR_DIGEST256 | DSR_BASE64 | DSR_SORT_UNIQ);

        size_t size_guess = 0;
        dirserv_spool_remove_missing_and_guess_size(conn, 0, compress_method != NO_METHOD,
                                                    &size_guess, NULL);
        ...
        if (connection_dir_is_global_write_low(TO_CONN(conn), size_guess)) {
            log_info(LD_DIRSERV, "Client asked for server descriptors, but we've been "
                                 "writing too many bytes lately. Sending 503 Dir busy.");
            write_short_http_response(conn, 503, "Directory busy, try again later");
            goto done;
        }
        ...
        clear_spool = 0;
        write_http_response_header(conn, -1, compress_method, MICRODESC_CACHE_LIFETIME);

        if (compress_method != NO_METHOD)
            conn->compress_state =
                tor_compress_new(1, compress_method, choose_compression_level(size_guess));
...
handle_get_descriptor(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    if (!strcmpstart(url, "/tor/server/") ||
        ...
        if (!strcmpstart(url, "d/")) {
            source = is_extra ? DIR_SPOOL_EXTRA_BY_DIGEST : DIR_SPOOL_SERVER_BY_DIGEST;
        } else {
        ...
        conn->spool = smartlist_new();
        res = dirserv_get_routerdesc_spool(conn->spool, url, source,
                                           connection_dir_is_encrypted(conn), &msg);
        ...
        int n_expired = 0;
        dirserv_spool_remove_missing_and_guess_size(
            conn, publish_cutoff, compress_method != NO_METHOD, &size_guess, &n_expired);
        ...
        if (get_options()->BridgeAuthoritativeDir && source == DIR_SPOOL_SERVER_BY_FP) {
            SMARTLIST_FOREACH_BEGIN (conn->spool, spooled_resource_t *, spooled) {
                const routerinfo_t *router = router_get_by_id_digest((const char *)spooled->digest);
...
handle_get_keys(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    const time_t if_modified_since = args->if_modified_since;
    {
        ...
        if (!strcmp(url, "/tor/keys/all")) {
            ...
            dir_split_resource_into_fingerprints(url + strlen("/tor/keys/fp/"), fps, NULL,
            ...
            smartlist_t *fps = smartlist_new();
            dir_split_resource_into_fingerprints(url + strlen("/tor/keys/sk/"), fps, NULL,
                                                 DSR_HEX | DSR_SORT_UNIQ);
            SMARTLIST_FOREACH(fps, char *, d, {
        ...
        len = 0;
        SMARTLIST_FOREACH(certs, authority_cert_t *, c, len += c->cache_info.signed_descriptor_len);
        ...
        write_http_response_header(conn, compress_method != NO_METHOD ? -1 : len, compress_method,
                                   60 * 60);
        if (compress_method != NO_METHOD) {
            conn->compress_state =
...
handle_get_hs_descriptor_v3(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    if (!connection_dir_is_anonymous(conn)) {
        write_short_http_response(conn, 503, "Rejecting single hop HS v3 descriptor request");
        goto done;
    }
 ...
 */
static int
handle_get_networkstatus_bridges(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    if (options->BridgeAuthoritativeDir && options->BridgePassword_AuthDigest_ &&
        ...
        if (!header || tor_memneq(digest, options->BridgePassword_AuthDigest_, DIGEST256_LEN)) {
            write_short_http_response(conn, 404, "Not found");
            tor_free(header);
            goto done;
        }
...
static int
handle_get_next_bandwidth(dir_connection_t *conn, const get_handler_args_t *args)
{
    ...
    if (options->V3BandwidthsFile) {
        char *bandwidth = read_file_to_str(options->V3BandwidthsFile, RFTS_IGNORE_MISSING, NULL);
        if (bandwidth != NULL) {
            ...
            if (compress_method != NO_METHOD) {
                conn->compress_state =
                    tor_compress_new(1, compress_method, choose_compression_level(len / 2));
...
STATIC int
parse_hs_version_from_post(const char *url, const char *prefix, const char **end_pos)
{
    int ok;
          ...
          (dir_connection_t * conn, const char *headers, const char *body, size_t body_len))
{
    ...
    if (connection_dir_is_encrypted(conn) && !strcmpstart(url, "/tor/rendezvous2/publish")) {
        if (rend_cache_store_v2_desc_as_dir(body) < 0) {
            log_warn(LD_REND, "Rejected v2 rend descriptor (body size %d) from %s.", (int)body_len,
                     conn->base_.address);
            write_short_http_response(conn, 400, "Invalid v2 service descriptor rejected");
        } else {
    ...
    if (!strcmpstart(url, "/tor/hs/")) {
        if (!connection_dir_is_anonymous(conn)) {
            write_short_http_response(conn, 503, "Rejecting single hop HS descriptor post");
            goto done;
        }
    ...
    if (authdir_mode(options) && !strcmp(url, "/tor/")) { /* server descriptor post */
        const char *msg = "[None]";
        uint8_t purpose =
            authdir_mode_bridge(options) ? ROUTER_PURPOSE_BRIDGE : ROUTER_PURPOSE_GENERAL;
        was_router_added_t r =
            dirserv_add_multiple_descriptors(body, body_len, purpose, conn->base_.address, &msg);
        tor_assert(msg);
    ...
    if (authdir_mode_v3(options) && !strcmp(url, "/tor/post/vote")) { /* v3 networkstatus vote */
        const char *msg = "OK";
        int status;
        if (dirvote_add_vote(body, approx_time(), &msg, &status)) {
            ...
            tor_assert(msg);
            log_warn(LD_DIRSERV, "Rejected vote from %s (\"%s\").", conn->base_.address, msg);
    ...
    if (authdir_mode_v3(options) &&
        ...
        if (dirvote_add_signatures(body, conn->base_.address, &msg) >= 0) {
            ...
            log_warn(LD_DIR, "Unable to store signatures posted by %s: %s", conn->base_.address,
                     msg ? msg : "???");
            write_short_http_response(conn, 400, msg ? msg : "Unable to store signatures");
        }
        goto done;
    }
...
http_set_address_origin(const char *headers, connection_t *conn)
{
    ...
    if (fwd) {
        tor_addr_t toraddr;
        if (tor_addr_parse(&toraddr, fwd) == -1 || tor_addr_is_internal(&toraddr, 0)) {
...
directory_handle_command(dir_connection_t *conn)
{
    ...
    switch (connection_fetch_from_buf_http(TO_CONN(conn), &headers, MAX_HEADERS_SIZE, &body,
                                           &body_len, MAX_DIR_UL_SIZE, 0)) {
    case -1: /* overflow */
        log_warn(LD_DIRSERV, "Request too large from address '%s' to DirPort. Closing.",
    ...
    if (!strncasecmp(headers, "GET", 3))
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Got headers %s with unknown command. Closing.",
               escaped(headers));
        r = -1;
    }
