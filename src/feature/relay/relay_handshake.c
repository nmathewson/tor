...
static void
add_certs_cell_cert_helper(certs_cell_t *certs_cell, uint8_t cert_type, const uint8_t *cert_encoded,
                           size_t cert_len)
{
...
static void
add_x509_cert(certs_cell_t *certs_cell, uint8_t cert_type, const tor_x509_cert_t *cert)
{
    if (NULL == cert)
        return;
...
static void
add_ed25519_cert(certs_cell_t *certs_cell, uint8_t cert_type, const tor_cert_t *cert)
{
    if (NULL == cert)
        return;

    add_certs_cell_cert_helper(certs_cell, cert_type, cert->encoded, cert->encoded_len);
}
...
connection_or_send_certs_cell(or_connection_t *conn)
{
    ...
    if (tor_tls_get_my_certs(conn_in_server_mode, &global_link_cert, &id_cert) < 0)
        return -1;
    ...
    add_ed25519_cert(certs_cell, CERTTYPE_ED_ID_SIGN, get_master_signing_key_cert());
    if (conn_in_server_mode) {
        tor_assert_nonfatal(conn->handshake_state->own_link_cert ||
                            certs_cell_ed25519_disabled_for_testing);
        add_ed25519_cert(certs_cell, CERTTYPE_ED_SIGN_LINK, conn->handshake_state->own_link_cert);
    } else {
        add_ed25519_cert(certs_cell, CERTTYPE_ED_SIGN_AUTH, get_current_auth_key_cert());
    }
    ...
    /* And finally the crosscert. */
    {
        ...
        if (crosscert) {
            add_certs_cell_cert_helper(certs_cell, CERTTYPE_RSA1024_ID_EDID, crosscert,
                                       crosscert_len);
        }
    }
...
int
authchallenge_type_is_better(uint16_t challenge_type_a, uint16_t challenge_type_b)
{
...
connection_or_send_auth_challenge_cell(or_connection_t *conn)
{
    ...
    auth_challenge_cell_set_n_methods(ac, auth_challenge_cell_getlen_methods(ac));

    cell = var_cell_new(auth_challenge_cell_encoded_len(ac));
    ssize_t len = auth_challenge_cell_encode(cell->payload, cell->payload_len, ac);
 ...
 */
var_cell_t *
connection_or_compute_authenticate_cell_body(or_connection_t *conn, const int authtype,
                                             ...
                                             const ed25519_keypair_t *ed_signing_key, int server)
{
...

    {
        ...
        their_digests = tor_x509_cert_get_id_digests(conn->handshake_state->certs->id_cert);
...

    {
        ...
        if (!cert) {
            log_warn(LD_OR, "Unable to find cert when making %s data.", authtype_str);
            goto err;
        }

        memcpy(auth->scert, tor_x509_cert_get_cert_digests(cert)->d[DIGEST_SHA256], 32);
    ...
    if (old_tlssecrets_algorithm) {
        ...
        char label[128];
        tor_snprintf(label, sizeof(label), "EXPORTER FOR TOR TLS CLIENT BINDING %s", authtype_str);
    ...
    if (ed_signing_key && is_ed) {
        ...
        int siglen = crypto_pk_private_sign(signing_key, (char *)auth1_getarray_sig(auth),
