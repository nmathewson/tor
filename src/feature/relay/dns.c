...
{
    return (unsigned)siphash24g((const uint8_t *)a->address,
                                strlen(a->address));
}
...
evdns_log_cb(int warn, const char *msg)
{
    ...
    if (!strcmpstart(msg, "Nameserver ") &&
        ...
        control_event_server_status(
            LOG_NOTICE, "NAMESERVER_STATUS NS=%s STATUS=DOWN ERR=%s", ns,
            escaped(err));
        tor_free(ns);
    ...
    if (query_type == DNS_PTR) {
        ...
        if (dns_result == DNS_ERR_NONE && answer_addr &&
            ...
            memcpy(&resolve->result_ipv6.addr_ipv6,
                   tor_addr_to_in6(answer_addr), sizeof(struct in6_addr));
    ...
    switch (r) {
        case 1:
            ...
            if (is_resolve) {
                ...
                /* Add to the n_streams list; the calling function will send
                 * back a connected cell. */
                exitconn->next_stream = oncirc->n_streams;
                oncirc->n_streams = exitconn;
            }
            break;
        case 0:
            /* The request is pending: add the connection into the linked list
             * of resolving_streams on this circuit. */
        ...
        case -1:
            ...
            if (!made_connection_pending && !exitconn->base_.marked_for_close) {
                /* If we made the connection pending, then we freed it already
                 * in dns_cancel_pending_resolve().  If we marked it for close,
                 * it'll get freed from the main loop.  Otherwise, can free it
                 * now. */
           ...
           cached_resolve_t **resolve_out))
{
    ...
    *made_connection_pending_out = 0;
        ...
        switch (resolve->state) {
            ...
            case CACHE_STATE_CACHED:
                ...
                *resolve_out = resolve;
                ...
                return set_exitconn_info_from_resolve(exitconn, resolve,
                                                      hostname_out);
    ...
    strlcpy(resolve->address, exitconn->base_.address,
            sizeof(resolve->address));
           ...
           char **hostname_out))
{
    ...
    if (ipv4_ok && ipv6_ok && is_resolve) {
        ...
        if (ipv4_allowed && !ipv6_allowed) {
            ...
            /* Our exit policy would permit both.  Answer with whichever the
             * user prefers */
...
configured_nameserver_address(const size_t idx)
{
    ...
    if (evdns_base_get_nameserver_addr(the_evdns_base, (int)idx,
        ...
        if (tor_addr_from_sockaddr(tor_addr, (const struct sockaddr *)&sa,
                                   NULL) == 0) {
            return tor_addr;
        }
        tor_free(tor_addr);
...
configure_nameservers(int force)
{
    ...
    if (conf_fname) {
        ...
        if (!missing_resolv_conf) {
            log_info(LD_EXIT, "Parsing resolver configuration in '%s'",
                     conf_fname);
            if ((r = evdns_base_resolv_conf_parse(
                     the_evdns_base, flags,
                ...
                if (r !=
                    6) // "r = 6" means "no DNS servers were in resolv.conf" -
                    goto err; // in which case we expect libevent to add
               ...
               void *arg)
{
    ...
    if (result != DNS_ERR_SHUTDOWN)
        dns_found_answer(string_address, orig_query_type, result, &addr,
                         hostname, ttl);
                   ...
                   const tor_addr_t *ptr_address)
{
    ...
    switch (query_type) {
        ...
        case DNS_PTR:
            ...
            break;
        default:
            log_warn(LD_BUG, "Called with unexpectd query type %d",
                     (int)query_type);
            break;
    }
    ...
    if (*ip > 5 && n_wildcard_requests > 10) {
        ...
        if (!smartlist_contains_string(dns_wildcard_list, id)) {
            tor_log(dns_wildcard_notice_given ? LOG_INFO : LOG_NOTICE, LD_EXIT,
                    "Your DNS provider has given \"%s\" as an answer for %d "
                    "different "
                    "invalid addresses. Apparently they are hijacking DNS "
                    "failures. "
                    "I'll try to correct for this by treating future "
                    "occurrences of "
                    "\"%s\" as 'not found'.",
                    id, *ip, id);
...
add_wildcarded_test_address(const char *address)
{
    ...
    if (n > n_test_addrs / 2) {
        tor_log(
            dns_wildcarded_test_address_notice_given ? LOG_INFO : LOG_NOTICE,
            LD_EXIT,
    ...
    if (result == DNS_ERR_NONE && count) {
        ...
        if (type == DNS_IPv4_A) {
            ...
            for (i = 0; i < count; ++i) {
                ...
                tor_inet_ntop(AF_INET6, &addrs[i], answer_buf,
                              sizeof(answer_buf));
                wildcard_increment_answer(answer_buf);
            }
        }
...
dns_launch_wildcard_checks(void)
{
    ...
    for (ipv6 = 0; ipv6 <= 1; ++ipv6) {
        for (i = 0; i < N_WILDCARD_CHECKS; ++i) {
             ...
             * attempt to 'comply' with rfc2606, refrain from giving A records
             * for these.  This is the standards-compliance equivalent of making
             * sure that your crackhouse's elevator inspection certificate is up
             * to date.
             */
            ...
            /* Try some random .com/org/net domains. This will work fine so long
             * as not too many resolve to the same place. */
...
{
    return dns_wildcard_list &&
           smartlist_contains_string(dns_wildcard_list, ip);
}
...
dump_dns_mem_usage(int severity)
{
    ...
    /* Print out the count and estimated size of our &cache_root.  It
       undercounts hostnames in cached reverse resolves.
     */
