...
static void dns_found_answer(const char *address, uint8_t query_type, int dns_answer,
...
HT_GENERATE2(cache_map, cached_resolve_t, node, cached_resolve_hash, cached_resolves_eq, 0.6,
...
evdns_log_cb(int warn, const char *msg)
{
    ...
    if (!strcmpstart(msg, "Resolve requested for") && get_options()->SafeLogging) {
    ...
    if (!strcmpstart(msg, "Nameserver ") && (cp = strstr(msg, " has failed: "))) {
        ...
        severity = LOG_INFO;
        control_event_server_status(LOG_NOTICE, "NAMESERVER_STATUS NS=%s STATUS=DOWN ERR=%s", ns,
                                    escaped(err));
        tor_free(ns);
    } else if (!strcmpstart(msg, "Nameserver ") && (cp = strstr(msg, " is back up"))) {
        char *ns = tor_strndup(msg + 11, cp - (msg + 11));
        severity = (all_down && warn) ? LOG_NOTICE : LOG_INFO;
        all_down = 0;
        control_event_server_status(LOG_NOTICE, "NAMESERVER_STATUS NS=%s STATUS=UP", ns);
        tor_free(ns);
...
static void
cached_resolve_add_answer(cached_resolve_t *resolve, int query_type, int dns_result,
                          const tor_addr_t *answer_addr, const char *answer_hostname, uint32_t ttl)
{
    if (query_type == DNS_PTR) {
        ...
        if (dns_result == DNS_ERR_NONE && answer_addr && tor_addr_family(answer_addr) == AF_INET) {
        ...
        if (dns_result == DNS_ERR_NONE && answer_addr && tor_addr_family(answer_addr) == AF_INET6) {
...
set_expiry(cached_resolve_t *resolve, time_t expires)
{
    ...
    smartlist_pqueue_add(cached_resolve_pqueue, compare_cached_resolves_by_expiry_,
...
purge_expired_resolves(time_t now)
{
    ...
    while (smartlist_len(cached_resolve_pqueue)) {
        ...
        if (resolve->expire > now)
            break;
        smartlist_pqueue_pop(cached_resolve_pqueue, compare_cached_resolves_by_expiry_,
        ...
        if (resolve->state == CACHE_STATE_PENDING) {
            ...
            log_debug(LD_EXIT, "Forgetting old cached resolve (address %s, expires %lu)",
        ...
        if (resolve->pending_connections) {
            log_debug(LD_EXIT, "Closing pending connections on timed-out DNS resolve!");
        ...
        if (resolve->state == CACHE_STATE_CACHED || resolve->state == CACHE_STATE_PENDING) {
            removed = HT_REMOVE(cache_map, &cache_root, resolve);
            if (removed != resolve) {
                log_err(LD_BUG,
                        "The expired resolve we purged didn't match any in"
                        " the cache. Tried to purge %s (%p); instead got %s (%p).",
                        resolve->address, (void *)resolve, removed ? removed->address : "NULL",
                        (void *)removed);
            }
...
dns_resolve(edge_connection_t *exitconn)
{
    ...
    r = dns_resolve_impl(exitconn, is_resolve, oncirc, &hostname, &made_connection_pending,
                         &resolve);
    ...
    case -1:
        ...
        if (is_resolve) {
            send_resolved_cell(
                exitconn, (r == -1) ? RESOLVED_TYPE_ERROR : RESOLVED_TYPE_ERROR_TRANSIENT, NULL);
        }
...
dns_resolve_impl,
          (edge_connection_t * exitconn, int is_resolve, or_circuit_t *oncirc, char **hostname_out,
           int *made_connection_pending_out, cached_resolve_t **resolve_out))
{
    ...
    *made_connection_pending_out = 0;
    ...
    if (tor_addr_parse(&addr, exitconn->base_.address) >= 0) {
        if (tor_addr_family(&addr) == AF_INET || tor_addr_family(&addr) == AF_INET6) {
    ...
    if (address_is_invalid_destination(exitconn->base_.address, 0)) {
        tor_log(LOG_PROTOCOL_WARN, LD_EXIT, "Rejecting invalid destination address %s",
     ...
     * connection.
     */
    if ((r = tor_addr_parse_PTR_name(&addr, exitconn->base_.address, AF_UNSPEC, 0)) != 0) {
        if (r == 1) {
            is_reverse = 1;
    ...
    *made_connection_pending_out = 1;
    ...
    log_debug(LD_EXIT, "Launching %s.", escaped_safe_str(exitconn->base_.address));
    assert_cache_ok();
          ...
          (edge_connection_t * exitconn, const cached_resolve_t *resolve, char **hostname_out))
{
    ...
    if (answer_with_ipv4) {
        if (resolve->res_status_ipv4 == RES_STATUS_DONE_OK) {
            tor_addr_from_ipv4h(&exitconn->base_.addr, resolve->result_ipv4.addr_ipv4);
        } else {
        ...
        if (resolve->res_status_ipv6 == RES_STATUS_DONE_OK) {
            tor_addr_from_in6(&exitconn->base_.addr, &resolve->result_ipv6.addr_ipv6);
        } else {
...
dns_cancel_pending_resolve, (const char *address))
{
    ...
    if (resolve->state != CACHE_STATE_PENDING) {
         ...
         * ignore it. */
        if (resolve->pending_connections) {
            log_warn(LD_BUG, "Address %s is not pending but has pending connections!",
    ...
    if (tmp != resolve) {
        log_err(LD_BUG,
                ...
                " the cache. Tried to purge %s (%p); instead got %s (%p).",
                resolve->address, (void *)resolve, tmp ? tmp->address : "NULL", (void *)tmp);
    }
    tor_assert(tmp == resolve);
 ...
 */
static void
dns_found_answer(const char *address, uint8_t query_type, int dns_answer, const tor_addr_t *addr,
                 const char *hostname, uint32_t ttl)
{
    ...
    if (!resolve) {
        ...
        if (!is_test_addr)
            log_info(LD_EXIT, "Resolved unasked address %s; ignoring.", escaped_safe_str(address));
        return;
    }
    ...
    cached_resolve_add_answer(resolve, query_type, dns_answer, addr, hostname, ttl);
    ...
    while (resolve->pending_connections) {
        ...
        if (r < 0) {
            ...
            if (pendconn->base_.purpose == EXIT_PURPOSE_CONNECT) {
                ...
                send_resolved_cell(
                    pendconn, r == -1 ? RESOLVED_TYPE_ERROR : RESOLVED_TYPE_ERROR_TRANSIENT, NULL);
...
make_pending_resolve_cached(cached_resolve_t *resolve)
{
    ...
    {
        cached_resolve_t *new_resolve = tor_memdup(resolve, sizeof(cached_resolve_t));
        uint32_t ttl = UINT32_MAX;
        new_resolve->expire = 0; /* So that set_expiry won't croak. */
        if (resolve->res_status_hostname == RES_STATUS_DONE_OK)
            new_resolve->result_ptr.hostname = tor_strdup(resolve->result_ptr.hostname);
...
configured_nameserver_address(const size_t idx)
{
    ...
    if (evdns_base_get_nameserver_addr(the_evdns_base, (int)idx, (struct sockaddr *)&sa, sa_len) >
        0) {
        tor_addr_t *tor_addr = tor_malloc(sizeof(tor_addr_t));
        if (tor_addr_from_sockaddr(tor_addr, (const struct sockaddr *)&sa, NULL) == 0) {
            return tor_addr;
        }
        tor_free(tor_addr);
    }
...
configure_nameservers(int force)
{
    ...
    if (conf_fname) {
        ...
        if (stat_res) {
            log_warn(LD_EXIT, "Unable to stat resolver configuration in '%s': %s", conf_fname,
                     strerror(errno));
            missing_resolv_conf = 1;
        } else if (!force && resolv_conf_fname && !strcmp(conf_fname, resolv_conf_fname) &&
        ...
        if (flags & DNS_OPTION_HOSTSFILE) {
            ...
            evdns_base_load_hosts(the_evdns_base, sandbox_intern_string("/etc/hosts"));
        }
 ...
 */
static void
evdns_callback(int result, char type, int count, int ttl, void *addresses, void *arg)
{
    char *arg_ = arg;
    ...
    if (result == DNS_ERR_NONE) {
        if (type == DNS_IPv4_A && count) {
            ...
            escaped_address = esc_for_log(string_address);
            log_debug(LD_EXIT, "eventdns said that %s resolves to %s", safe_str(escaped_address),
    ...
    if (orig_query_type && type && orig_query_type != type) {
        log_warn(LD_BUG, "Weird; orig_query_type == %d but type == %d", (int)orig_query_type,
                 (int)type);
    }
    if (result != DNS_ERR_SHUTDOWN)
        dns_found_answer(string_address, orig_query_type, result, &addr, hostname, ttl);
...
static int
launch_one_resolve(const char *address, uint8_t query_type, const tor_addr_t *ptr_address)
{
    const int options = get_options()->ServerDNSSearchDomains ? 0 : DNS_QUERY_NO_SEARCH;
    ...
    switch (query_type) {
    case DNS_IPv4_A:
        req = evdns_base_resolve_ipv4(the_evdns_base, address, options, evdns_callback, addr);
        break;
    case DNS_IPv6_AAAA:
        req = evdns_base_resolve_ipv6(the_evdns_base, address, options, evdns_callback, addr);
        ++n_ipv6_requests_made;
        break;
    case DNS_PTR:
        if (tor_addr_family(ptr_address) == AF_INET)
            req = evdns_base_resolve_reverse(the_evdns_base, tor_addr_to_in(ptr_address),
                                             DNS_QUERY_NO_SEARCH, evdns_callback, addr);
        else if (tor_addr_family(ptr_address) == AF_INET6)
            req = evdns_base_resolve_reverse_ipv6(the_evdns_base, tor_addr_to_in6(ptr_address),
                                                  DNS_QUERY_NO_SEARCH, evdns_callback, addr);
        else
...
launch_resolve, (cached_resolve_t * resolve))
{
    ...
    if (r == 0) {
        log_info(LD_EXIT, "Launching eventdns request for %s", escaped_safe_str(resolve->address));
...
add_wildcarded_test_address(const char *address)
{
    ...
    if (n > n_test_addrs / 2) {
        tor_log(dns_wildcarded_test_address_notice_given ? LOG_INFO : LOG_NOTICE, LD_EXIT,
...
static void
evdns_wildcard_check_callback(int result, char type, int count, int ttl, void *addresses, void *arg)
{
    (void)ttl;
    ++n_wildcard_requests;
 ...
 * <b>suffix</b> */
static void
launch_wildcard_check(int min_len, int max_len, int is_ipv6, const char *suffix)
{
    char *addr;
    ...
    if (options->ServerDNSTestAddresses) {
        ...
        SMARTLIST_FOREACH_BEGIN (options->ServerDNSTestAddresses, const char *, address) {
            if (launch_one_resolve(address, DNS_IPv4_A, NULL) < 0) {
                log_info(LD_EXIT, "eventdns rejected test address %s", escaped_safe_str(address));
            }
...
dns_launch_correctness_checks(void)
{
    ...
    if (!launch_event)
        launch_event = tor_evtimer_new(tor_libevent_get_base(), launch_test_addresses, NULL);
    timeout.tv_sec = 30;
    timeout.tv_usec = 0;
...
dns_reset_correctness_checks(void)
{
    ...
    if (dns_wildcarded_test_address_list) {
        SMARTLIST_FOREACH(dns_wildcarded_test_address_list, char *, cp, tor_free(cp));
...
assert_resolve_ok(cached_resolve_t *resolve)
{
    ...
    if (resolve->state == CACHE_STATE_PENDING || resolve->state == CACHE_STATE_DONE) {
#if 0
...
{
    return sizeof(struct cached_resolve_t) * dns_cache_entry_count() + HT_MEM_USAGE(&cache_root);
}
...
assert_cache_ok_(void)
{
    ...
    smartlist_pqueue_assert_ok(cached_resolve_pqueue, compare_cached_resolves_by_expiry_,
                               offsetof(cached_resolve_t, minheap_idx));
