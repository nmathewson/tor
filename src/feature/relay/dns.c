...
evdns_log_cb(int warn, const char *msg)
{
    ...
    if (!strcmpstart(msg, "Nameserver ") &&
        ...
        log_fn_ratelim(
            &mismatch_limit, severity, LD_EXIT,
           ...
           char **hostname_out))
{
    ...
    if (answer_with_ipv4) {
        if (resolve->res_status_ipv4 == RES_STATUS_DONE_OK) {
            ...
            r = evdns_err_is_transient(resolve->result_ipv4.err_ipv4) ? -2
                                                                      : -1;
        }
        ...
        if (resolve->res_status_ipv6 == RES_STATUS_DONE_OK) {
            ...
            r = evdns_err_is_transient(resolve->result_ipv6.err_ipv6) ? -2
                                                                      : -1;
        }
...
make_pending_resolve_cached(cached_resolve_t *resolve)
{
    ...
    assert_resolve_ok(resolve);
    assert_cache_ok();
    /* The resolve will eventually just hit the time-out in the expiry queue
     * and expire. See fd0bafb0dedc7e2 for a brief explanation of how this got
     * that way.  XXXXX we could do better!*/
...
launch_resolve, (cached_resolve_t * resolve))
{
    ...
    if (r == 0) {
        ...
        if (r == 0 && get_options()->IPv6Exit) {
            ...
            if (launch_one_resolve(resolve->address, DNS_IPv6_AAAA, NULL) <
                0) {
...
dns_launch_wildcard_checks(void)
{
    ...
    for (ipv6 = 0; ipv6 <= 1; ++ipv6) {
        for (i = 0; i < N_WILDCARD_CHECKS; ++i) {
             ...
             * for these.  This is the standards-compliance equivalent of
             * making sure that your crackhouse's elevator inspection
            ...
            /* Try some random .com/org/net domains. This will work fine so
             * long as not too many resolve to the same place. */
