...
pt_get_bindaddr_from_config(const char *transport)
{
    ...
    for (cl = options->ServerTransportListenAddr; cl; cl = cl->next) {
        char *bindaddr = get_bindaddr_from_transport_listen_line(cl->value, transport);
        if (bindaddr)
            return bindaddr;
    }
...
pt_get_options_for_server_transport(const char *transport)
{
    ...
    for (cl = options->ServerTransportOptions; cl; cl = cl->next) {
        smartlist_t *options_sl = get_options_from_transport_options_line(cl->value, transport);
        if (options_sl)
            return options_sl;
    }
 ...
 */
int
options_validate_server_transport(const or_options_t *old_options, or_options_t *options,
                                  char **msg)
{
    (void)old_options;
    ...
    for (cl = options->ServerTransportOptions; cl; cl = cl->next) {
        ...
        smartlist_t *options_sl = get_options_from_transport_options_line(cl->value, NULL);
        if (!options_sl)
...
options_act_server_transport(const or_options_t *old_options)
{
    ...
    if (running_tor && init_ext_or_cookie_authentication(!!options->ExtORPort_lines) < 0) {
