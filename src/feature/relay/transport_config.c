                                        ...
                                        const char *transport)
{
    ...
    SMARTLIST_FOREACH_BEGIN (items, const char *, option) {
        ...
        log_debug(LD_CONFIG, "Added %s to the list of options",
                  escaped(option));
                                  ...
                                  or_options_t *options, char **msg)
{
    ...
    if (options->ServerTransportListenAddr &&
        !options->ServerTransportPlugin) {
    ...
    for (cl = options->ServerTransportListenAddr; cl; cl = cl->next) {
        ...
        char *bindaddr =
            get_bindaddr_from_transport_listen_line(cl->value, NULL);
        if (!bindaddr)
