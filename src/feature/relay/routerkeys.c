...
#define EXPIRES_SOON(cert, interval) \
    ...
    {
        uint32_t flags = (INIT_ED_KEY_SPLIT | INIT_ED_KEY_EXTRA_STRONG |
        ...
        if (!id) {
            if (need_new_signing_key) {
                if (offline_master)
                    FAIL("Can't load master identity key; OfflineMasterKey is "
                         "set.");
                else
                ...
                fname = options_get_keydir_fname(
                    options, "ed25519_master_id_public_key");
                if (ed25519_pubkey_write_to_file(&id->pubkey, fname, "type0") <
                    0) {
                    log_warn(
                        LD_OR,
    ...
    if (want_new_signing_key && sign_signing_key_with_id) {
        uint32_t flags = (INIT_ED_KEY_CREATE | INIT_ED_KEY_REPLACE |
                          INIT_ED_KEY_EXTRA_STRONG | INIT_ED_KEY_NEEDCERT |
                          INIT_ED_KEY_INCLUDE_SIGNING_KEY_IN_CERT);
        char *fname = options_get_keydir_fname(options, "ed25519_signing");
        ed25519_keypair_free(sign);
        tor_cert_free(sign_cert);
        sign = ed_key_init_from_file(
            fname, flags, LOG_WARN, sign_signing_key_with_id, now,
...
generate_ed_link_cert(const or_options_t *options, time_t now, int force)
{
    ...
    if (force == 0 && link_cert_cert &&
        ...
        fast_memeq(digests->d[DIGEST_SHA256],
                             ...
                             const crypto_pk_t *rsa_id_key, int *len_out)
{
    ...
    *len_out = 0;
    ...
    memcpy(signed_data + DIGEST_LEN, master_id_key->pubkey,
           ED25519_PUBKEY_LEN);
