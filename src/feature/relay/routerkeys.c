...
#define EXPIRES_SOON(cert, interval) (!(cert) || HAPPENS_SOON((cert)->valid_until, (interval)))
    ...
    /* First try to get the signing key to see how it is. */
    {
        ...
        sign = ed_key_init_from_file(
            fname, INIT_ED_KEY_NEEDCERT | INIT_ED_KEY_INCLUDE_SIGNING_KEY_IN_CERT, LOG_INFO, NULL,
            0, 0, CERT_TYPE_ID_SIGNING, &sign_cert, options);
        tor_free(fname);
    ...
    if (use_signing) {
        ...
        } else if (!ed25519_pubkey_eq(&use_signing->pubkey, &master_signing_key->pubkey) ||
                   !tor_memeq(use_signing->seckey.seckey, master_signing_key->seckey.seckey,
                              ED25519_SECKEY_LEN)) {
    ...
    const int offline_master = options->OfflineMasterKey && options->command != CMD_KEYGEN;
    ...
    const int want_new_signing_key =
        need_new_signing_key || EXPIRES_SOON(check_signing_cert, options->TestingSigningKeySlop);
    ...
    {
        uint32_t flags = (INIT_ED_KEY_SPLIT | INIT_ED_KEY_EXTRA_STRONG | INIT_ED_KEY_NO_REPAIR);
        ...
        id = ed_key_init_from_file(fname, flags, LOG_WARN, NULL, 0, 0, 0, NULL, options);
        tor_free(fname);
        if (!id) {
            if (need_new_signing_key) {
                if (offline_master)
                ...
                memcpy(&id->pubkey, &check_signing_cert->signing_key, sizeof(ed25519_public_key_t));
                fname = options_get_keydir_fname(options, "ed25519_master_id_public_key");
    ...
    if (master_identity_key && !ed25519_pubkey_eq(&id->pubkey, &master_identity_key->pubkey)) {
    ...
    if (want_new_signing_key && sign_signing_key_with_id) {
        uint32_t flags = (INIT_ED_KEY_CREATE | INIT_ED_KEY_REPLACE | INIT_ED_KEY_EXTRA_STRONG |
                          INIT_ED_KEY_NEEDCERT | INIT_ED_KEY_INCLUDE_SIGNING_KEY_IN_CERT);
        char *fname = options_get_keydir_fname(options, "ed25519_signing");
        ed25519_keypair_free(sign);
        tor_cert_free(sign_cert);
        sign = ed_key_init_from_file(fname, flags, LOG_WARN, sign_signing_key_with_id, now,
                                     options->SigningKeyLifetime, CERT_TYPE_ID_SIGNING, &sign_cert,
                                     options);
        tor_free(fname);
        if (!sign)
    ...
    if (server_mode(options) &&
        (!rsa_ed_crosscert || HAPPENS_SOON(rsa_ed_crosscert_expiration, 30 * 86400))) {
        uint8_t *crosscert;
    ...
    if (!current_auth_key || signing_key_changed ||
        ...
        auth = ed_key_new(use_signing, INIT_ED_KEY_NEEDCERT, now, options->TestingAuthKeyLifetime,
...
generate_ed_link_cert(const or_options_t *options, time_t now, int force)
{
    ...
    if (force == 0 && link_cert_cert &&
        ...
        fast_memeq(digests->d[DIGEST_SHA256], link_cert_cert->signed_key.pubkey, DIGEST256_LEN)) {
        return 0;
    }
    ...
    link_cert = tor_cert_create(get_master_signing_keypair(), CERT_TYPE_SIGNING_LINK, &dummy_key,
                                now, options->TestingLinkCertLifetime, 0);
...
{
    if (!master_identity_key || !master_signing_key || !current_auth_key || !link_cert_cert ||
    ...
    if (!fast_memeq(digests->d[DIGEST_SHA256], link_cert_cert->signed_key.pubkey, DIGEST256_LEN)) {
        return 1;
    }
...
#    define MAKECERT(cert, signing, signed_, type, flags)                                  \
        cert = tor_cert_create(signing, type, &signed_->pubkey, time(NULL), 86400, flags); \
        if (!cert) {                                                                       \
            log_warn(LD_BUG, "Couldn't make a %s certificate!", #cert);                    \
            goto err;                                                                      \
        }

    MAKECERT(signing_key_cert, master_identity_key, master_signing_key, CERT_TYPE_ID_SIGNING,
             CERT_FLAG_INCLUDE_SIGNING_KEY);
    MAKECERT(auth_key_cert, master_signing_key, current_auth_key, CERT_TYPE_SIGNING_AUTH, 0);
    ...
    rsa_ed_crosscert_len = tor_make_rsa_ed25519_crosscert(
        &master_identity_key->pubkey, rsa_identity_key, time(NULL) + 86400, &rsa_ed_crosscert);
 ...
 */
static void
log_ed_cert_expiration(const tor_cert_t *cert, const char *description, const char *fname)
{
    char expiration[ISO_TIME_LEN + 1];

    if (BUG(!cert)) { /* If the specified key hasn't been loaded */
        log_warn(LD_OR, "No %s key loaded; can't get certificate expiration.", description);
    } else {
        format_local_iso_time(expiration, cert->valid_until);
        log_notice(LD_OR, "The %s certificate stored in %s is valid until %s.", description, fname,
                   expiration);
...
{
    return id && master_identity_key && ed25519_pubkey_eq(id, &master_identity_key->pubkey);
}
...
make_ntor_onion_key_crosscert(const curve25519_keypair_t *onion_key,
                              const ed25519_public_key_t *master_id_key, time_t now,
                              time_t lifetime, int *sign_out)
{
    tor_cert_t *cert = NULL;
    ed25519_keypair_t ed_onion_key;

    if (ed25519_keypair_from_curve25519_keypair(&ed_onion_key, sign_out, onion_key) < 0)
        goto end;

    cert = tor_cert_create(&ed_onion_key, CERT_TYPE_ONION_ID, master_id_key, now, lifetime, 0);
                             ...
                             const crypto_pk_t *rsa_id_key, int *len_out)
{
    ...
    *len_out = 0;
    ...
    int r = crypto_pk_private_sign(onion_key, (char *)signature, sizeof(signature),
