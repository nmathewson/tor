...
have_room_for_onionskin(uint16_t type)
{
    ...
    tap_usec = estimated_usec_for_onionskins(ol_entries[ONION_HANDSHAKE_TYPE_TAP],
                                             ONION_HANDSHAKE_TYPE_TAP) /
               num_cpus;

    /* How long would it take to process all the NTor cells in the queue? */
    ntor_usec = estimated_usec_for_onionskins(ol_entries[ONION_HANDSHAKE_TYPE_NTOR],
    ...
    if (type == ONION_HANDSHAKE_TYPE_NTOR &&
        (ntor_usec + tap_during_ntor_usec) / 1000 > (uint64_t)options->MaxOnionQueueDelay)
        return 0;

    if (type == ONION_HANDSHAKE_TYPE_TAP &&
        (tap_usec + ntor_during_tap_usec) / 1000 > (uint64_t)options->MaxOnionQueueDelay)
        return 0;
...
onion_pending_add(or_circuit_t *circ, create_cell_t *onionskin)
{
    ...
    if (onionskin->handshake_type > MAX_ONION_HANDSHAKE_TYPE) {
        ...
        log_warn(LD_BUG, "Handshake %d out of range! Dropping.", onionskin->handshake_type);
        return -1;
        /* LCOV_EXCL_STOP */
    }
...
#define WARN_TOO_MANY_CIRC_CREATIONS_INTERVAL (60)
        static ratelim_t last_warned = RATELIM_INIT(WARN_TOO_MANY_CIRC_CREATIONS_INTERVAL);
        char *m;
    ...
    log_info(LD_OR, "New create (%s). Queues now ntor=%d and tap=%d.",
             ...
             ol_entries[ONION_HANDSHAKE_TYPE_NTOR], ol_entries[ONION_HANDSHAKE_TYPE_TAP]);
...
#define MAX_NUM_NTORS_PER_TAP 100000
    ...
    int result = networkstatus_get_param(NULL, "NumNTorsPerTAP", DEFAULT_NUM_NTORS_PER_TAP,
                                         MIN_NUM_NTORS_PER_TAP, MAX_NUM_NTORS_PER_TAP);
    tor_assert(result > 0);
    return result;
}
...
decide_next_handshake_type(void)
{
    ...
    if (!ol_entries[ONION_HANDSHAKE_TYPE_TAP]) {
        ...
        if (ol_entries[ONION_HANDSHAKE_TYPE_NTOR] && recently_chosen_ntors <= num_ntors_per_tap())
...
onion_queue_entry_remove(onion_queue_t *victim)
{
    if (victim->handshake_type > MAX_ONION_HANDSHAKE_TYPE) {
        /* LCOV_EXCL_START
         * We should have rejected this far before this point */
        log_warn(LD_BUG, "Handshake %d out of range! Dropping.", victim->handshake_type);
        /* XXX leaks */
        return;
        /* LCOV_EXCL_STOP */
    }
