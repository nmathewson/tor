 ...
 *  message. */
STATIC int
connection_write_ext_or_command(connection_t *conn, uint16_t command, const char *body,
                                size_t bodylen)
{
    char header[4];
...
init_ext_or_cookie_authentication(int is_enabled)
{
    ...
    retval = init_cookie_authentication(fname, EXT_OR_PORT_AUTH_COOKIE_HEADER,
                                        ...
                                        &ext_or_auth_cookie, &ext_or_auth_cookie_is_set);
    tor_free(fname);
    return retval;
}
...
/* DOCDOC */
STATIC int
handle_client_auth_nonce(const char *client_nonce, size_t client_nonce_len, char **client_hash_out,
                         char **reply_out, size_t *reply_len_out)
{
    ...
    { /* set up macs */
        size_t hmac_s_msg_len =
            strlen(EXT_OR_PORT_AUTH_SERVER_TO_CLIENT_CONST) + 2 * EXT_OR_PORT_AUTH_NONCE_LEN;
        size_t hmac_c_msg_len =
            strlen(EXT_OR_PORT_AUTH_CLIENT_TO_SERVER_CONST) + 2 * EXT_OR_PORT_AUTH_NONCE_LEN;
        ...
        memcpy(hmac_s_msg + strlen(EXT_OR_PORT_AUTH_SERVER_TO_CLIENT_CONST), client_nonce,
        ...
        memcpy(hmac_c_msg + strlen(EXT_OR_PORT_AUTH_CLIENT_TO_SERVER_CONST), client_nonce,
               EXT_OR_PORT_AUTH_NONCE_LEN);
        memcpy(hmac_c_msg + strlen(EXT_OR_PORT_AUTH_CLIENT_TO_SERVER_CONST) +
                   EXT_OR_PORT_AUTH_NONCE_LEN,
               server_nonce, EXT_OR_PORT_AUTH_NONCE_LEN);

        crypto_hmac_sha256(server_hash, (char *)ext_or_auth_cookie, EXT_OR_PORT_AUTH_COOKIE_LEN,
                           hmac_s_msg, hmac_s_msg_len);

        crypto_hmac_sha256(correct_client_hash, (char *)ext_or_auth_cookie,
                           EXT_OR_PORT_AUTH_COOKIE_LEN, hmac_c_msg, hmac_c_msg_len);
    ...
    { /* debug logging */ /* XXX disable this codepath if not logging on debug?*/
        ...
        base16_encode(server_hash_encoded, sizeof(server_hash_encoded), server_hash,
                      sizeof(server_hash));
        base16_encode(server_nonce_encoded, sizeof(server_nonce_encoded), server_nonce,
                      sizeof(server_nonce));
        base16_encode(client_nonce_encoded, sizeof(client_nonce_encoded), client_nonce,
                      EXT_OR_PORT_AUTH_NONCE_LEN);

        log_debug(LD_GENERAL, "server_hash: '%s'\nserver_nonce: '%s'\nclient_nonce: '%s'",
...
connection_ext_or_auth_handle_client_nonce(connection_t *conn)
{
    ...
    if (connection_buf_get_bytes(client_nonce, EXT_OR_PORT_AUTH_NONCE_LEN, conn) < 0)
        return -1;
...
connection_ext_or_auth_handle_client_hash(connection_t *conn)
{
    ...
    if (connection_buf_get_bytes(provided_client_hash, EXT_OR_PORT_AUTH_HASH_LEN, conn) < 0)
        return -1;

    if (tor_memneq(TO_OR_CONN(conn)->ext_or_auth_correct_client_hash, provided_client_hash,
...
static int
connection_ext_or_handle_cmd_useraddr(connection_t *conn, const char *payload, uint16_t len)
{
    /* Copy address string. */
    tor_addr_t addr;
    uint16_t port;
 ...
 *  Return 0 on success and -1 on error. */
static int
connection_ext_or_handle_cmd_transport(or_connection_t *conn, const char *payload, uint16_t len)
{
    char *transport_str;
    if (memchr(payload, '\0', len)) {
...
connection_ext_or_process_inbuf(or_connection_t *or_conn)
{
    ...
    while (1) {
        ...
        if (command->cmd == EXT_OR_CMD_TB_DONE) {
            ...
            if (connection_ext_or_handle_cmd_useraddr(conn, command->body, command->len) < 0)
                goto err;
        } else if (command->cmd == EXT_OR_CMD_TB_TRANSPORT) {
            if (connection_ext_or_handle_cmd_transport(or_conn, command->body, command->len) < 0)
                goto err;
        } else {
...
connection_ext_or_start_auth(or_connection_t *or_conn)
{
    ...
    log_debug(LD_GENERAL, "ExtORPort authentication: Sending supported authentication types");
