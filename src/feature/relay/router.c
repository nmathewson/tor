...
init_curve25519_keypair_from_file(curve25519_keypair_t *keys_out,
                                  const char *fname, int generate, int severity,
                                  const char *tag)
{
    switch (file_status(fname)) {
        ...
        case FN_EMPTY:
            if (generate) {
                ...
                if (curve25519_keypair_generate(keys_out, 1) < 0)
                    goto error;
                if (curve25519_keypair_write_to_file(keys_out, fname, tag) <
                    0) {
                    tor_log(severity, LD_FS,
        ...
        case FN_FILE: {
            char *tag_in = NULL;
            if (curve25519_keypair_read_from_file(keys_out, &tag_in, fname) <
                0) {
            ...
            if (!tag_in || strcmp(tag_in, tag)) {
                tor_log(severity, LD_GENERAL,
                        "Unexpected tag %s on private key.", escaped(tag_in));
                tor_free(tag_in);
                goto error;
            }
            tor_free(tag_in);
                             ...
                             const ed25519_keypair_t *signing_keypair)
{
...

    {
        char *sig;
        if (!(sig =
                  router_get_dirobj_signature(digest, DIGEST_LEN, ident_key))) {
...
static int
extrainfo_dump_to_string_header_helper(smartlist_t *chunks,
