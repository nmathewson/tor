...
assert_identity_keys_ok(void)
{
    ...
    if (public_server_mode(get_options())) {
        ...
        if (server_identitykey)
            tor_assert(
                !crypto_pk_eq_keys(client_identitykey, server_identitykey));
    }
}
...
rotate_onion_key(void)
{
    ...
    if (curve25519_keypair_write_to_file(&new_curve25519_keypair, fname,
                                         "onion") < 0) {
        log_err(LD_FS, "Couldn't write curve25519 onion key to \"%s\".",
                fname);
        goto error;
    }
    ...
    or_state_mark_dirty(state,
                        get_options()->AvoidDiskWrites ? now + 3600 : 0);
    goto done;
error:
...
{
    int grace_period;
    grace_period = networkstatus_get_param(NULL, "onion-key-grace-period-days",
...
router_initialize_tls_context(void)
{
    ...
    if (!lifetime) { /* we should guess a good ssl cert lifetime */
        ...
        if (crypto_rand_int(2)) {
             ...
             * one second before midnight. (Some CAs wobble their expiry times
             * a bit in practice, perhaps to reduce collision attacks; see
             * ticket 8443 for details about observed certs in the wild.) */
            lifetime--;
        }
    }
    ...
    return tor_tls_context_init(
        flags, get_tlsclient_identity_key(),
...
router_write_fingerprint(int hashed)
{
    ...
    if (!hashed) {
        if (crypto_pk_get_fingerprint(get_server_identity_key(), fingerprint,
                                      0) < 0) {
...
init_keys(void)
{
    ...
    if (options->command == CMD_RUN_TOR) {
        ...
        if (state->LastRotatedOnionKey > 100 &&
            state->LastRotatedOnionKey < now) {
            /* We allow for some parsing slop, but we don't want to risk
             * accepting values in the distant future.  If we did, we might
             * never rotate the onion key. */
            onionkey_set_at = state->LastRotatedOnionKey;
        } else {
            /* We have no LastRotatedOnionKey set; either we just created the
             * key or it's a holdover from 0.1.2.4-alpha-dev or earlier.  In
             * either case, start the clock ticking now so that we will
             * eventually rotate it even if we don't stay up for the full
             * lifetime of an onion key. */
            state->LastRotatedOnionKey = onionkey_set_at = now;
            or_state_mark_dirty(
                state, options->AvoidDiskWrites ? time(NULL) + 3600 : 0);
        }
    }
    ...
    if (authdir_mode_v3(options)) {
        ...
        if (mydesc) {
            ...
            ri =
                router_parse_entry_from_string(mydesc, NULL, 1, 0, NULL, NULL);
            if (!ri) {
                log_err(LD_GENERAL,
            ...
            if (!WRA_WAS_ADDED(added)) {
                if (!WRA_WAS_OUTDATED(added)) {
                    ...
                    /* If the descriptor was outdated, that's ok. This can
                     * happen when some config options are toggled that affect
                     * workers, but we don't really need new keys yet so the
                     * descriptor doesn't change and the old one is still
                     * fresh. */
                    log_info(
                        LD_GENERAL,
    ...
    if (!ds) {
        ds = trusted_dir_server_new(options->Nickname, NULL,
                                    ...
                                    NULL, digest, v3_digest, type, 0.0);
        if (!ds) {
            log_err(LD_GENERAL,
...
router_should_be_dirserver(const or_options_t *options, int dir_port)
{
    ...
    if (accounting_is_enabled(options) &&
        ...
        /* Don't spend bytes for directory traffic if we could end up
         * hibernating, but allow DirPort otherwise. Some relay operators set
         * AccountingMax because they're confused or to get statistics.
         * Directory traffic has a much larger effect on output than input so
         * there is no reason to turn it off if using AccountingRule in. */
...
router_upload_dir_desc_to_dirservers(int force)
{
    ...
    if (ei) {
        memcpy(msg + desc_len, ei->cache_info.signed_descriptor_body,
               extra_len);
    }
...
router_compare_to_my_exit_policy(const tor_addr_t *addr, uint16_t port)
{
     ...
     * bit unusual, in that it contains IPv6 and IPv6 entries.  We don't want
     * to look at router_get_my_routerinfo()->ipv6_exit_policy, since that's a
     * port summary. */
...
get_my_declared_family(const or_options_t *options)
{
        ...
        if (!member) {
            ...
            if (!smartlist_contains_string(warned_family, name) &&
                ...
                if (is_legal)
                    log_warn(
                        LD_CONFIG,
                        "There is a router named %s in my declared family, "
                        "but "
            ...
            fp[0] = '$';
            base16_encode(fp + 1, HEX_DIGEST_LEN + 1, member->identity,
                          DIGEST_LEN);
    ...
    if (smartlist_len(result) == 1) {
        ...
        if (singleton[0] == '$') {
            char d[DIGEST_LEN];
            int n = base16_decode(d, sizeof(d), singleton + 1,
                                  strlen(singleton + 1));
            if (n == DIGEST_LEN &&
          ...
          (routerinfo_t * *ri_out))
{
    ...
    /* For now, at most one IPv6 or-address is being advertised. */
    {
        ...
        SMARTLIST_FOREACH_BEGIN (get_configured_ports(), const port_cfg_t *,
                                 p) {
            if (p->type == CONN_TYPE_OR_LISTENER &&
                !p->server_cfg.no_advertise && !p->server_cfg.bind_ipv4_only &&
                tor_addr_family(&p->addr) == AF_INET6) {
                /* Like IPv4, if the relay is configured using the default
                 * authorities, disallow internal IPs. Otherwise, allow them.
                 */
                const int default_auth =
                    using_default_dir_authorities(options);
                if (!tor_addr_is_internal(&p->addr, 0) || !default_auth) {
                    ...
                    log_warn(
                        ...
                        "Try specifying a globally reachable address "
                        "explicitly.",
                        tor_addr_to_str(addrbuf, &p->addr, sizeof(addrbuf),
                                        1));
                }
            }
    ...
    if (dns_seems_to_be_broken() || has_dns_init_failed()) {
        ...
        policies_parse_exit_policy_from_options(
            options, ri->addr, &ri->ipv6_addr, &ri->exit_policy);
    }
...
router_build_fresh_descriptor(routerinfo_t **r, extrainfo_t **e)
{
    ...
    if (ei) {
        if (BUG(routerinfo_incompatible_with_extrainfo(
                ri->identity_pkey, ei, &ri->cache_info, NULL))) {
            result = TOR_ROUTERINFO_ERROR_INTERNAL_BUG;
            goto err;
        }
    }
...
mark_my_descriptor_dirty_if_too_old(time_t now)
{
    ...
    const char *retry_fast_reason =
        NULL; /* Set if we should retry frequently */
                             ...
                             const ed25519_keypair_t *signing_keypair)
{
    ...
    if (emit_ed_sigs) {
        if (!router->cache_info.signing_key_cert->signing_key_included ||
            !ed25519_pubkey_eq(
                &router->cache_info.signing_key_cert->signed_key,
    ...
    if (ntor_keypair && router->cache_info.signing_key_cert &&
        ...
        int sign = 0;
        char buf[256];
        /* XXXX Base the expiration date on the actual onion key expiration
         * time?*/
    ...
    const ed25519_keypair_t *signing_keypair, int emit_ed_sigs)
{
    ...
    /* This is the first chunk in the file. If the file is too big, other
     * chunks are removed. So we must only add one chunk here. */
    tor_asprintf(&pre, "extra-info %s %s\n%spublished %s\n",
                 extrainfo->nickname, identity, ed_cert_line, published);
                         ...
                         const ed25519_keypair_t *signing_keypair)
{
    ...
    /* This is one of the three final chunks in the file. If the file is too
     * big, other chunks are removed. So we must only add one chunk here. */
    smartlist_add_strdup(chunks, "router-signature\n");
    ...
    while (strlen(s) > MAX_EXTRAINFO_UPLOAD_SIZE - DIROBJ_MAX_SIG_LEN) {
         ...
         * extra-info line and one for the router-signature), we can keep
         * removing things. If emit_ed_sigs is true, we also keep 2 additional
         * chunks at the end for the ed25519 signature. */
        const int required_chunks = emit_ed_sigs ? 4 : 2;
        if (smartlist_len(chunks) > required_chunks) {
            /* We remove the next-to-last or 4th-last element (remember, len-1
             * is the
             * last element), since we need to keep the router-signature
             * elements. */
