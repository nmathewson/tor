...
expire_old_onion_keys(void)
{
    ...
    if (file_status(fname) == FN_FILE) {
        if (tor_unlink(fname) != 0) {
            log_warn(LD_FS, "Couldn't unlink old onion key file %s: %s", fname, strerror(errno));
        }
    }
    tor_free(fname);
...
set_server_identity_key(crypto_pk_t *k)
{
    ...
    if (crypto_pk_get_digest(server_identitykey, server_identitykey_digest) < 0) {
...
rotate_onion_key(void)
{
    ...
    if (curve25519_keypair_write_to_file(&new_curve25519_keypair, fname, "onion") < 0) {
    ...
    onionkey = prkey;
    memcpy(&last_curve25519_onion_key, &curve25519_onion_key, sizeof(curve25519_keypair_t));
    memcpy(&curve25519_onion_key, &new_curve25519_keypair, sizeof(curve25519_keypair_t));
    now = time(NULL);
...
static int
init_curve25519_keypair_from_file(curve25519_keypair_t *keys_out, const char *fname, int generate,
                                  int severity, const char *tag)
{
    ...
    case FN_FILE: {
        ...
        if (!tag_in || strcmp(tag_in, tag)) {
            tor_log(severity, LD_GENERAL, "Unexpected tag %s on private key.", escaped(tag_in));
            tor_free(tag_in);
            goto error;
        }
        tor_free(tag_in);
...
static int
load_authority_keyset(int legacy, crypto_pk_t **key_out, authority_cert_t **cert_out)
{
    int r = -1;
    ...
    fname = get_keydir_fname(legacy ? "legacy_signing_key" : "authority_signing_key");
    ...
    tor_free(fname);
    fname = get_keydir_fname(legacy ? "legacy_certificate" : "authority_certificate");
    cert = read_file_to_str(fname, 0, NULL);
    if (!cert) {
        log_warn(LD_DIR, "Signing key found, but no certificate found in %s", fname);
        goto done;
    }
...
{
    if (load_authority_keyset(0, &authority_signing_key, &authority_key_certificate) < 0)
        return -1;

    if (get_options()->V3AuthUseLegacyKey &&
        load_authority_keyset(1, &legacy_signing_key, &legacy_key_certificate) < 0)
        return -1;
...
{
    return networkstatus_get_param(NULL, "onion-key-rotation-days", DEFAULT_ONION_KEY_LIFETIME_DAYS,
...
router_initialize_tls_context(void)
{
    ...
    return tor_tls_context_init(flags, get_tlsclient_identity_key(),
                                server_mode(options) ? get_server_identity_key() : NULL,
...
router_write_fingerprint(int hashed)
{
    ...
    log_info(LD_GENERAL, "Dumping %sfingerprint to \"%s\"...", hashed ? "hashed " : "", keydir);
    if (!hashed) {
        ...
        if (crypto_pk_get_hashed_fingerprint(get_server_identity_key(), fingerprint) < 0) {
    ...
    if (!cp || strcmp(cp, fingerprint_line)) {
        if (write_str_to_file(keydir, fingerprint_line, 0)) {
            log_err(LD_FS, "Error writing %sfingerprint line to file", hashed ? "hashed " : "");
            goto done;
        }
    }

    log_notice(LD_GENERAL, "Your Tor %s identity key fingerprint is '%s %s'",
               hashed ? "bridge's hashed" : "server's", options->Nickname, fingerprint);
...
init_keys(void)
{
    ...
    if (authdir_mode_v3(options)) {
        ...
        if (cert) {
            if (crypto_pk_get_digest(get_my_v3_authority_cert()->identity_key, v3_digest) < 0) {
...

    {
        ...
        if (fast_mem_is_zero((const char *)last_curve25519_onion_key.pubkey.public_key,
                             CURVE25519_PUBKEY_LEN) &&
            file_status(keydir) == FN_FILE) {
            /* Load keys from non-empty files only.
             * Missing old keys won't be replaced with freshly generated keys. */
            init_curve25519_keypair_from_file(&last_curve25519_onion_key, keydir, 0, LOG_ERR,
                                              "onion");
        }
        tor_free(keydir);
    }
    ...
    if (authdir_mode_v3(options)) {
         ...
         * recognized. */
        if (dirserv_add_own_fingerprint(get_server_identity_key(), get_master_identity_key())) {
    ...
    type = ((options->V3AuthoritativeDir ? (V3_DIRINFO | MICRODESC_DIRINFO | EXTRAINFO_DIRINFO)
                                         : NO_DIRINFO) |
            (options->BridgeAuthoritativeDir ? BRIDGE_DIRINFO : NO_DIRINFO));

    ds = router_get_trusteddirserver_by_digest(digest);
    if (!ds) {
        ds = trusted_dir_server_new(
            options->Nickname, NULL, router_get_advertised_dir_port(options, 0),
            router_get_advertised_or_port(options), NULL, digest, v3_digest, type, 0.0);
        if (!ds) {
    ...
    if (cert) { /* add my own cert to the list of known certs */
        ...
        if (trusted_dirs_load_certs_from_string(cert->cache_info.signed_descriptor_body,
...
router_should_be_dirserver(const or_options_t *options, int dir_port)
{
    ...
    if (accounting_is_enabled(options) && get_options()->AccountingRule != ACCT_IN) {
        ...
        log_info(LD_GENERAL,
                 ...
                 dir_port ? "dirport" : "begindir", effective_bw, (options->AccountingMax),
                 interval_length);
                             ...
                             int supports_tunnelled_dir_requests)
{
    ...
    if (dir_port && !router_get_advertised_dir_port(options, dir_port))
        return 0;
    if (supports_tunnelled_dir_requests && !router_get_advertised_or_port(options))
        return 0;
 ...
 */
static int
router_should_advertise_begindir(const or_options_t *options, int supports_tunnelled_dir_requests)
{
    /* dir_port is not relevant, pass 0 */
    return decide_to_advertise_dir_impl(options, 0, supports_tunnelled_dir_requests);
}
 ...
 */
uint16_t
router_get_advertised_or_port_by_af(const or_options_t *options, sa_family_t family)
{
    int port = get_first_advertised_port_by_type_af(CONN_TYPE_OR_LISTENER, family);
    (void)options;

    /* If the port is in 'auto' mode, we have to use
       router_get_listener_port_by_type(). */
    if (port == CFG_AUTO_PORT)
        return router_get_active_listener_port_by_type_af(CONN_TYPE_OR_LISTENER, family);
...
void
router_get_advertised_ipv6_or_ap(const or_options_t *options, tor_addr_port_t *ipv6_ap_out)
{
    ...
    const tor_addr_t *addr = get_first_advertised_addr_by_type_af(CONN_TYPE_OR_LISTENER, AF_INET6);
    const uint16_t port = router_get_advertised_or_port_by_af(options, AF_INET6);
...
router_get_advertised_dir_port(const or_options_t *options, uint16_t dirport)
{
    ...
    if (dirport_configured == CFG_AUTO_PORT)
        return router_get_active_listener_port_by_type_af(CONN_TYPE_DIR_LISTENER, AF_INET);
...
router_upload_dir_desc_to_dirservers(int force)
{
    ...
    directory_post_to_dirservers(DIR_PURPOSE_UPLOAD_DIR,
                                 (auth & BRIDGE_DIRINFO) ? ROUTER_PURPOSE_BRIDGE
...
router_compare_to_my_exit_policy(const tor_addr_t *addr, uint16_t port)
{
    ...
    if ((tor_addr_family(addr) == AF_INET || tor_addr_family(addr) == AF_INET6)) {
        return compare_tor_addr_to_addr_policy(addr, port, me->exit_policy) != ADDR_POLICY_ACCEPTED;
#if 0
...
{
    return (server_identitykey && tor_memeq(server_identitykey_digest, digest, DIGEST_LEN));
}
...
router_extrainfo_digest_is_me(const char *digest)
{
    ...
    return tor_memeq(digest, ei->cache_info.signed_descriptor_digest, DIGEST_LEN);
}
...
static void
router_check_descriptor_address_port_consistency(uint32_t ipv4h_desc_addr, int listener_type)
{
    tor_assert(listener_type == CONN_TYPE_OR_LISTENER || listener_type == CONN_TYPE_DIR_LISTENER);

    /* The first advertised Port may be the magic constant CFG_AUTO_PORT.
     */
    int port_v4_cfg = get_first_advertised_port_by_type_af(listener_type, AF_INET);
    if (port_v4_cfg != 0 &&
        !port_exists_by_type_addr32h_port(listener_type, ipv4h_desc_addr, port_v4_cfg, 1)) {
        const tor_addr_t *port_addr = get_first_advertised_addr_by_type_af(listener_type, AF_INET);
        ...
        const char *listener_str = (listener_type == CONN_TYPE_OR_LISTENER ? "OR" : "Dir");
        log_warn(LD_CONFIG,
                 ...
                 "NoAdvertise'.",
                 listener_str, port_addr_str, desc_addr_str, listener_str, listener_str,
                 listener_str);
    }
}
...
{
    router_check_descriptor_address_port_consistency(ipv4h_desc_addr, CONN_TYPE_OR_LISTENER);
    router_check_descriptor_address_port_consistency(ipv4h_desc_addr, CONN_TYPE_DIR_LISTENER);
}
...
get_my_declared_family(const or_options_t *options)
{
    ...
    for (family = options->MyFamily; family; family = family->next) {
        ...
        if (!member) {
            ...
            if (!smartlist_contains_string(warned_family, name) && !is_legal_hexdigest(name)) {
                if (is_legal)
                    log_warn(LD_CONFIG,
            ...
            if (!is_legal_hexdigest(name) && !smartlist_contains_string(warned_family, name)) {
     ...
     * on clients. */
    nodefamily_t *nf = nodefamily_from_members(declared_family, router_get_my_id_digest(),
                                               NF_WARN_MALFORMED, NULL);
    ...
    smartlist_split_string(result, s, NULL, SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 0);
    tor_free(s);
    ...
    if (smartlist_len(result) == 1) {
        ...
        if (singleton[0] == '$') {
            ...
            if (n == DIGEST_LEN && fast_memeq(d, router_get_my_id_digest(), DIGEST_LEN)) {
                is_me = true;
            }
        }
        if (!is_me) {
...
router_build_fresh_unsigned_routerinfo, (routerinfo_t * *ri_out))
{
    ...
    ri->supports_tunnelled_dir_requests = directory_permits_begindir_requests(options);
    ri->cache_info.published_on = time(NULL);
    /* get_onion_key() must invoke from main thread */
    router_set_rsa_onion_pkey(get_onion_key(), &ri->onion_pkey, &ri->onion_pkey_len);

    ri->onion_curve25519_pkey =
        tor_memdup(&get_current_curve25519_keypair()->pubkey, sizeof(curve25519_public_key_t));
    ...
    if (BUG(crypto_pk_get_digest(ri->identity_pkey, ri->cache_info.identity_digest) < 0)) {
    ...
    ri->policy_is_reject_star = policy_is_reject_star(ri->exit_policy, AF_INET, 1) &&
...
router_build_fresh_unsigned_extrainfo(const routerinfo_t *ri)
{
    ...
    memcpy(ei->cache_info.identity_digest, ri->cache_info.identity_digest, DIGEST_LEN);
...
router_dump_and_sign_extrainfo_descriptor_body(extrainfo_t *ei)
{
    ...
    if (extrainfo_dump_to_string(&ei->cache_info.signed_descriptor_body, ei,
                                 get_server_identity_key(), get_master_signing_keypair()) < 0) {
 ...
 */
STATIC void
router_update_routerinfo_from_extrainfo(routerinfo_t *ri, const extrainfo_t *ei)
{
    if (BUG(!ei)) {
    ...
    memcpy(ri->cache_info.extra_info_digest, ei->cache_info.signed_descriptor_digest, DIGEST_LEN);
...
router_dump_and_sign_routerinfo_descriptor_body(routerinfo_t *ri)
{
    ...
    if (!(ri->cache_info.signed_descriptor_body = router_dump_router_to_string(
              ri, get_server_identity_key(), get_onion_key(), get_current_curve25519_keypair(),
              get_master_signing_keypair()))) {
        log_warn(LD_BUG, "Couldn't generate router descriptor.");
        return TOR_ROUTERINFO_ERROR_CANNOT_GENERATE;
    }

    ri->cache_info.signed_descriptor_len = strlen(ri->cache_info.signed_descriptor_body);
...
router_build_fresh_descriptor(routerinfo_t **r, extrainfo_t **e)
{
    ...
    if (ei) {
        if (BUG(routerinfo_incompatible_with_extrainfo(ri->identity_pkey, ei, &ri->cache_info,
                                                       NULL))) {
            result = TOR_ROUTERINFO_ERROR_INTERNAL_BUG;
            goto err;
        }
...
check_descriptor_bandwidth_changed(time_t now)
{
    ...
    if ((prev != cur && (!prev || !cur)) || cur > (prev * BANDWIDTH_CHANGE_FACTOR) ||
        ...
        if (last_changed + MAX_BANDWIDTH_CHANGE_FREQ < now || !prev) {
            log_info(LD_GENERAL, "Measured bandwidth has changed; rebuilding descriptor.");
 ...
 * <b>prev</b> to <b>cur</b>. */
void
log_addr_has_changed(int severity, const tor_addr_t *prev, const tor_addr_t *cur,
                     const char *source)
{
    ...
    if (!tor_addr_is_null(prev))
        ...
        log_notice(LD_GENERAL, "Guessed our IP address as %s (source: %s).", addrbuf_cur, source);
}
...
check_descriptor_ipaddress_changed(time_t now)
{
    ...
    if (prev != cur) {
        ...
        tor_asprintf(&source, "METHOD=%s%s%s", method, hostname ? " HOSTNAME=" : "",
                     hostname ? hostname : "");
...
router_dump_router_to_string(routerinfo_t *router, const crypto_pk_t *ident_key,
                             const crypto_pk_t *tap_key, const curve25519_keypair_t *ntor_keypair,
                             const ed25519_keypair_t *signing_keypair)
{
    ...
    char *output = NULL;
    const int emit_ed_sigs = signing_keypair && router->cache_info.signing_key_cert;
    ...
    if (emit_ed_sigs) {
        ...
        ed25519_public_to_base64(ed_fp_base64, &router->cache_info.signing_key_cert->signing_key);
    ...
    if (crypto_pk_write_public_key_to_string(rsa_pubkey, &onion_pkey, &onion_pkeylen) < 0) {
    ...
    if (tap_key && router->cache_info.signing_key_cert &&
        ...
        int tap_cc_len = 0;
        uint8_t *tap_cc =
            make_tap_onion_key_crosscert(tap_key, &router->cache_info.signing_key_cert->signing_key,
    ...
    smartlist_add_asprintf(
        ...
        ed_cert_line ? ed_cert_line : "", extra_or_address ? extra_or_address : "",
        router->platform, proto_line, published, fingerprint, get_uptime(),
        (int)router->bandwidthrate, (int)router->bandwidthburst, (int)router->bandwidthcapacity,
    ...
    if (router->onion_curve25519_pkey) {
        char kbuf[128];
        base64_encode(kbuf, sizeof(kbuf), (const char *)router->onion_curve25519_pkey->public_key,
    ...
    if (router_should_advertise_begindir(options, router->supports_tunnelled_dir_requests)) {
    ...
    if (emit_ed_sigs) {
        ...
        crypto_digest_smartlist_prefix(digest, DIGEST256_LEN, ED_DESC_SIGNATURE_PREFIX, chunks, "",
                                       DIGEST_SHA256);
        ed25519_signature_t sig;
        char buf[ED25519_SIG_BASE64_LEN + 1];
        if (ed25519_sign(&sig, (const uint8_t *)digest, DIGEST256_LEN, signing_keypair) < 0)
            goto err;
 ...
 */
char *
router_dump_exit_policy_to_string(const routerinfo_t *router, int include_ipv4, int include_ipv6)
{
    if ((!router->exit_policy) || (router->policy_is_reject_star)) {
        return tor_strdup("reject *:*");
    }

    return policy_dump_to_string(router->exit_policy, include_ipv4, include_ipv6);
}
 ...
 * failure. */
static int
load_stats_file(const char *filename, const char *end_line, time_t now, char **out)
{
    int r = -1;
...
static int
extrainfo_dump_to_string_header_helper(smartlist_t *chunks, const extrainfo_t *extrainfo,
 ...
 * Can not fail. */
static void
extrainfo_dump_to_string_stats_helper(smartlist_t *chunks, int write_stats_to_extrainfo)
{
    const or_options_t *options = get_options();
    char *contents = NULL;
    time_t now = time(NULL);
    ...
    if (options->ExtraInfoStatistics && write_stats_to_extrainfo) {
        ...
        if (options->DirReqStatistics && load_stats_file("stats" PATH_SEPARATOR "dirreq-stats",
                                                         "dirreq-stats-end", now, &contents) > 0) {
            smartlist_add(chunks, contents);
        }
        if (options->HiddenServiceStatistics &&
            load_stats_file("stats" PATH_SEPARATOR "hidserv-stats", "hidserv-stats-end", now,
                            &contents) > 0) {
            smartlist_add(chunks, contents);
        }
        if (options->EntryStatistics && load_stats_file("stats" PATH_SEPARATOR "entry-stats",
                                                        "entry-stats-end", now, &contents) > 0) {
            smartlist_add(chunks, contents);
        }
        if (options->CellStatistics && load_stats_file("stats" PATH_SEPARATOR "buffer-stats",
                                                       "cell-stats-end", now, &contents) > 0) {
            smartlist_add(chunks, contents);
        }
        if (options->ExitPortStatistics && load_stats_file("stats" PATH_SEPARATOR "exit-stats",
                                                           "exit-stats-end", now, &contents) > 0) {
            smartlist_add(chunks, contents);
        }
        if (options->ConnDirectionStatistics &&
            load_stats_file("stats" PATH_SEPARATOR "conn-stats", "conn-bi-direct", now, &contents) >
                0) {
            smartlist_add(chunks, contents);
        }
 ...
 * Returns 0 on success, negative on failure. */
static int
extrainfo_dump_to_string_ed_sig_helper(smartlist_t *chunks,
                                       const ed25519_keypair_t *signing_keypair)
{
    ...
    smartlist_add_strdup(chunks, "router-sig-ed25519 ");
    crypto_digest_smartlist_prefix(sha256_digest, DIGEST256_LEN, ED_DESC_SIGNATURE_PREFIX, chunks,
                                   "", DIGEST_SHA256);
    if (ed25519_sign(&ed_sig, (const uint8_t *)sha256_digest, DIGEST256_LEN, signing_keypair) < 0)
        goto err;
 ...
 * Returns 0 on success, negative on failure. */
static int
extrainfo_dump_to_string_rsa_sig_helper(smartlist_t *chunks, crypto_pk_t *ident_key,
                                        const char *extrainfo_string)
{
    ...
    if (router_get_extrainfo_hash(extrainfo_string, strlen(extrainfo_string), digest) < 0 ||
        router_append_dirobj_signature(sig, sizeof(sig), digest, DIGEST_LEN, ident_key) < 0) {
...
int
extrainfo_dump_to_string(char **s_out, extrainfo_t *extrainfo, crypto_pk_t *ident_key,
                         const ed25519_keypair_t *signing_keypair)
{
    int result;
    ...
    const int emit_ed_sigs = signing_keypair && extrainfo->cache_info.signing_key_cert;
    int rv = 0;

    rv = extrainfo_dump_to_string_header_helper(chunks, extrainfo, signing_keypair, emit_ed_sigs);
    if (rv < 0)
        goto err;
    ...
    if (!ei_tmp) {
        if (write_stats_to_extrainfo) {
            ...
            result = extrainfo_dump_to_string(s_out, extrainfo, ident_key, signing_keypair);
            goto done;
        } else {
...
void
router_set_rsa_onion_pkey(const crypto_pk_t *pk, char **onion_pkey_out, size_t *onion_pkey_len_out)
{
    int len;
    char buf[1024];
