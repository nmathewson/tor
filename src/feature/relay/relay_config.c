                       ...
                       smartlist_t *ports_out, int *have_low_ports_out)
{
    ...
    if (port_parse_config(ports, options->ExtORPort_lines, "ExtOR",
                          ...
                          CL_PORT_SERVER_OPTIONS | CL_PORT_WARN_NONLOCAL) <
        0) {
                            ...
                            or_options_t *options, char **msg)
{
    ...
    if (server_mode(options) && !options->ContactInfo)
        log_notice(LD_CONFIG, "Your ContactInfo config option is not set. "
                              "Please consider setting it, so we can contact "
                              "you if your server is "
                                ...
                                or_options_t *options, char **msg)
{
    ...
    if (options->BridgeDistribution) {
        ...
        if (check_bridge_distribution_setting(options->BridgeDistribution) <
            0) {
    ...
    if (options->PublishServerDescriptor)
        SMARTLIST_FOREACH(
            options->PublishServerDescriptor, const char *, pubdes, {
                if (!strcmp(pubdes, "1") || !strcmp(pubdes, "0"))
                    if (smartlist_len(options->PublishServerDescriptor) > 1) {
                        COMPLAIN(
                            "You have passed a list of multiple arguments to "
                            "the "
                            "PublishServerDescriptor option that includes 0 "
                            "or 1. "
                            "0 or 1 should only be used as the sole argument. "
                            "This configuration will be rejected in a future "
                            "release.");
                        break;
                    }
            });
            ...
            if (!is_legal_nickname_or_hexdigest(s)) {
                ...
                if (s[0] != '$') {
                    ...
                    if (is_legal_nickname_or_hexdigest(prepended)) {
                        // The nickname is valid when it's prepended, set it as
                ...
                if (!normalized) {
                    tor_asprintf(msg, "Invalid nickname '%s' in %s line", s,
                                 name);
                    valid_line = 0;
                    break;
                }
                            ...
                            or_options_t *options, char **msg)
{
    ...
    if (server_mode(options) && options->RendConfigLines)
        log_warn(
            ...
            "That's not very secure: you should probably run your hidden "
            "service "
    ...
    if (options->ConstrainedSockets) {
        if (options->DirPort_set) {
            /* Providing cached directory entries while system TCP buffers are
             * scarce
             * will exacerbate the socket errors.  Suggest that this be
             * disabled. */
            COMPLAIN(
                        ...
                        bool *print_notice_out)
{
    ...
    if (options->CellStatistics || options->DirReqStatistics ||
        ...
        if ((!old_options || !old_options->EntryStatistics) &&
            ...
            /* If we get here, we've started recording bridge info when we
             * didn't do so before.  Note that "should_record_bridge_info()"
             * will always be false at this point, because of the earlier block
             * that cleared EntryStatistics when public_server_mode() was
             * false. We're leaving it in as defensive programming. */
...
options_act_relay_dos(const or_options_t *old_options)
{
    ...
    if (public_server_mode(options)) {
         ...
         * HUP, this is safe to call as it will load data from the current
         * options or/and the consensus. */
        dos_init();
