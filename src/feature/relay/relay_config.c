...
static int
check_server_ports(const smartlist_t *ports, const or_options_t *options, int *n_low_ports_out)
{
    if (BUG(!ports))
        return -1;
    ...
    if (n_orport_advertised && !n_orport_advertised_ipv4 && !options->BridgeRelay) {
 ...
 **/
int
port_parse_ports_relay(or_options_t *options, char **msg, smartlist_t *ports_out,
                       int *have_low_ports_out)
{
    ...
    if (port_parse_config(ports, options->ORPort_lines, "OR", CONN_TYPE_OR_LISTENER, "0.0.0.0", 0,
                          CL_PORT_SERVER_OPTIONS) < 0) {
        *msg = tor_strdup("Invalid ORPort configuration");
        goto err;
    }
    if (port_parse_config(ports, options->ExtORPort_lines, "ExtOR", CONN_TYPE_EXT_OR_LISTENER,
                          "127.0.0.1", 0, CL_PORT_SERVER_OPTIONS | CL_PORT_WARN_NONLOCAL) < 0) {
        *msg = tor_strdup("Invalid ExtORPort configuration");
        goto err;
    }
    if (port_parse_config(ports, options->DirPort_lines, "Dir", CONN_TYPE_DIR_LISTENER, "0.0.0.0",
                          0, CL_PORT_SERVER_OPTIONS) < 0) {
...
port_update_port_set_relay(or_options_t *options, const smartlist_t *ports)
{
     ...
     * out of an integer. */
    options->ORPort_set = !!port_count_real_listeners(ports, CONN_TYPE_OR_LISTENER, 0);
    options->DirPort_set = !!port_count_real_listeners(ports, CONN_TYPE_DIR_LISTENER, 0);
    options->ExtORPort_set = !!port_count_real_listeners(ports, CONN_TYPE_EXT_OR_LISTENER, 0);
}
 ...
 */
int
options_validate_relay_os(const or_options_t *old_options, or_options_t *options, char **msg)
{
    (void)old_options;
 ...
 */
int
options_validate_relay_info(const or_options_t *old_options, or_options_t *options, char **msg)
{
    (void)old_options;
    ...
    if (server_mode(options) && !options->ContactInfo) {
        log_warn(LD_CONFIG, "Your ContactInfo config option is not set. Please strongly "
 ...
 */
int
options_validate_publish_server(const or_options_t *old_options, or_options_t *options, char **msg)
{
    (void)old_options;
    ...
    if ((options->BridgeRelay || options->PublishServerDescriptor_ & BRIDGE_DIRINFO) &&
 ...
 */
int
options_validate_relay_padding(const or_options_t *old_options, or_options_t *options, char **msg)
{
    (void)old_options;
 ...
 */
int
options_validate_relay_bandwidth(const or_options_t *old_options, or_options_t *options, char **msg)
{
    (void)old_options;
     ...
     * in relay mode. */
    if (config_ensure_bandwidth_cap(&options->MaxAdvertisedBandwidth, "MaxAdvertisedBandwidth",
                                    msg) < 0)
        return -1;
    if (config_ensure_bandwidth_cap(&options->RelayBandwidthRate, "RelayBandwidthRate", msg) < 0)
        return -1;
    if (config_ensure_bandwidth_cap(&options->RelayBandwidthBurst, "RelayBandwidthBurst", msg) < 0)
        return -1;
    if (config_ensure_bandwidth_cap(&options->PerConnBWRate, "PerConnBWRate", msg) < 0)
        return -1;
    if (config_ensure_bandwidth_cap(&options->PerConnBWBurst, "PerConnBWBurst", msg) < 0)
        return -1;
    ...
    if (server_mode(options)) {
        ...
        if (options->BandwidthRate < required_min_bw) {
            ...
            tor_asprintf(msg,
                         ...
                         "For %sservers, it must be at least %u.",
                         (int)options->MaxAdvertisedBandwidth, optbridge, required_min_bw / 2);
            return -1;
        }
        if (options->RelayBandwidthRate && options->RelayBandwidthRate < required_min_bw) {
            tor_asprintf(msg,
                         "RelayBandwidthRate is set to %d bytes/second. "
                         "For %sservers, it must be at least %u.",
                         (int)options->RelayBandwidthRate, optbridge, required_min_bw);
            return -1;
        }
    }
 ...
 */
int
options_validate_relay_accounting(const or_options_t *old_options, or_options_t *options,
                                  char **msg)
{
    (void)old_options;
    ...
    if (options->AccountingMax) {
        ...
        } else if (config_count_key(options->RendConfigLines, "HiddenServiceDir") > 1) {
 ...
 */
static int
normalize_nickname_list(config_line_t **normalized_out, const config_line_t *lst, const char *name,
                        char **msg)
{
    if (!lst)
        return 0;
...
STATIC int
have_enough_mem_for_dircache(const or_options_t *options, size_t total_mem, char **msg)
{
    *msg = NULL;
    ...
    if (total_mem == 0) {
        if (get_total_system_memory(&total_mem) < 0) {
            total_mem =
                options->MaxMemInQueues >= SIZE_MAX ? SIZE_MAX : (size_t)options->MaxMemInQueues;
        }
    }
    if (options->DirCache) {
 ...
 */
int
options_validate_relay_mode(const or_options_t *old_options, or_options_t *options, char **msg)
{
    (void)old_options;
    ...
    if (server_mode(options) && options->RendConfigLines)
        log_warn(LD_CONFIG, "Tor is currently configured as a relay and a hidden service. "
    ...
    if (normalize_nickname_list(&options->MyFamily, options->MyFamily_lines, "MyFamily", msg))
        return -1;
 ...
 */
int
options_validate_relay_testing(const or_options_t *old_options, or_options_t *options, char **msg)
{
    (void)old_options;
...
static int
options_transition_affects_workers(const or_options_t *old_options, const or_options_t *new_options)
{
...
options_act_relay(const or_options_t *old_options)
{
    ...
    if (transition_affects_workers ||
        (authdir_mode_v3(options) && (!old_options || !authdir_mode_v3(old_options)))) {
        if (init_keys() < 0) {
    ...
    if (old_options) {
        if (transition_affects_workers) {
            log_info(LD_GENERAL, "Worker-related options changed. Rotating workers.");
            const int server_mode_turned_on = server_mode(options) && !server_mode(old_options);
 ...
 */
int
options_act_relay_stats(const or_options_t *old_options, bool *print_notice_out)
{
    ...
    if (options->CellStatistics || options->DirReqStatistics || options->EntryStatistics ||
        options->ExitPortStatistics || options->ConnDirectionStatistics ||
        ...
        if ((!old_options || !old_options->DirReqStatistics) && options->DirReqStatistics) {
        ...
        if ((!old_options || !old_options->ExitPortStatistics) && options->ExitPortStatistics) {
    ...
    if (old_options && old_options->HiddenServiceStatistics && !options->HiddenServiceStatistics)
        rep_hist_hs_stats_term();
    if (old_options && old_options->ExitPortStatistics && !options->ExitPortStatistics)
        rep_hist_exit_stats_term();
    if (old_options && old_options->ConnDirectionStatistics && !options->ConnDirectionStatistics)
...
options_act_relay_dir(const or_options_t *old_options)
{
    ...
    if (options->DirPortFrontPage) {
        ...
        if (!global_dirfrontpagecontents) {
            log_warn(LD_CONFIG, "DirPortFrontPage file '%s' not found. Continuing anyway.",
