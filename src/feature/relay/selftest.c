...
#define SELF_EXCLUDED_WARN_INTERVAL 3600
            ...
            log_fn_ratelim(
                &warning_limit, LOG_WARN, LD_CIRC,
...
extend_info_from_router(const routerinfo_t *r)
{
    ...
    info = extend_info_new(r->nickname, r->cache_info.identity_digest,
                           ed_id_key, rsa_pubkey, r->onion_curve25519_pkey,
                           &ap.addr, ap.port);
...
router_do_reachability_checks(int test_or, int test_dir)
{
    ...
    if (router_should_check_reachability(test_or, test_dir)) {
        if (test_or &&
            (!orport_reachable || !circuit_enough_testing_circs())) {
...
router_perform_bandwidth_test(int num_circs, time_t now)
{
    ...
    int max_cells =
        num_cells < CIRCWINDOW_START ? num_cells : CIRCWINDOW_START;
