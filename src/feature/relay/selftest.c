...
router_should_check_reachability(int test_or, int test_dir)
{
    ...
    if (routerset_contains_router(options->ExcludeNodes, me, -1) && options->StrictNodes) {
...
extend_info_from_router(const routerinfo_t *r)
{
    ...
    info = extend_info_new(r->nickname, r->cache_info.identity_digest, ed_id_key, rsa_pubkey,
                           r->onion_curve25519_pkey, &ap.addr, ap.port);
    ...
    if (router_should_check_reachability(test_or, test_dir)) {
        if (test_or && (!orport_reachable || !circuit_enough_testing_circs())) {
            ...
            log_info(LD_CIRC, "Testing %s of my ORPort: %s:%d.",
                     !orport_reachable ? "reachability" : "bandwidth", fmt_addr32(me->addr),
                     me->or_port);
        ...
        if (test_dir && !check_whether_dirport_reachable(options) &&
            !connection_get_by_type_addr_port_purpose(CONN_TYPE_DIR, &addr, me->dir_port,
            ...
            directory_request_set_directory_id_digest(req, me->cache_info.identity_digest);
...
inform_testing_reachability(void)
{
    ...
    control_event_server_status(LOG_NOTICE, "CHECKING_REACHABILITY ORADDRESS=%s:%d", address,
                                me->or_port);
    if (me->dir_port) {
        tor_snprintf(dirbuf, sizeof(dirbuf), " and DirPort %s:%d", address, me->dir_port);
        control_event_server_status(LOG_NOTICE, "CHECKING_REACHABILITY DIRADDRESS=%s:%d", address,
                                    me->dir_port);
    }
    log_notice(LD_OR,
               ...
               address, me->or_port, me->dir_port ? dirbuf : "", me->dir_port ? "are" : "is",
...
router_orport_found_reachable(void)
{
    ...
    if (!can_reach_or_port && me) {
        ...
        control_event_server_status(LOG_NOTICE, "REACHABILITY_SUCCEEDED ORADDRESS=%s:%d", address,
                                    me->or_port);
        tor_free(address);
    }
}
...
router_dirport_found_reachable(void)
{
    ...
    if (!can_reach_dir_port && me) {
        ...
        control_event_server_status(LOG_NOTICE, "REACHABILITY_SUCCEEDED DIRADDRESS=%s:%d", address,
                                    me->dir_port);
        tor_free(address);
    }
}
...
{
    int num_cells = (int)(get_options()->BandwidthRate * 10 / CELL_MAX_NETWORK_SIZE);
    ...
    while ((circ = circuit_get_next_by_pk_and_purpose(circ, NULL, CIRCUIT_PURPOSE_TESTING))) {
        ...
        while (i-- > 0) {
            if (relay_send_command_from_edge(0, TO_CIRCUIT(circ), RELAY_COMMAND_DROP, NULL, 0,
                                             circ->cpath->prev) < 0) {
