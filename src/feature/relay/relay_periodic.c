...
check_dns_honesty_callback(time_t now, const or_options_t *options)
{
     ...
     * to us. */
    if (net_is_disabled() || !public_server_mode(options) || router_my_exit_policy_is_reject_star())
...
check_for_reachability_bw_callback(time_t now, const or_options_t *options)
{
    ...
    if (server_mode(options) && (have_completed_a_circuit() || !any_predicted_circuits(now)) &&
        !net_is_disabled()) {
...
reachability_warnings_callback(time_t now, const or_options_t *options)
{
    ...
    if (server_mode(options) && !net_is_disabled() && have_completed_a_circuit()) {
        ...
        if (me && !check_whether_orport_reachable(options)) {
            ...
            log_warn(LD_CONFIG,
                     ...
                     address, me->or_port);
            control_event_server_status(LOG_WARN, "REACHABILITY_FAILED ORADDRESS=%s:%d", address,
                                        me->or_port);
            tor_free(address);
        }
        ...
        if (me && !check_whether_dirport_reachable(options)) {
            ...
            log_warn(LD_CONFIG,
                     ...
                     "your firewalls, ports, address, /etc/hosts file, etc.",
                     address, me->dir_port);
            control_event_server_status(LOG_WARN, "REACHABILITY_FAILED DIRADDRESS=%s:%d", address,
                                        me->dir_port);
            tor_free(address);
        }
    }
