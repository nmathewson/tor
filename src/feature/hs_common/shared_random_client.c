...
sr_srv_encode(char *dst, size_t dst_len, const sr_srv_t *srv)
{
    ...
    ret = base64_encode(buf, sizeof(buf), (const char *)srv->value, sizeof(srv->value), 0);
...
sr_parse_srv(const smartlist_t *args)
{
    ...
    num_reveals = tor_parse_uint64(smartlist_get(args, 0), 10, 0, UINT64_MAX, &ok, NULL);
    if (!ok) {
        goto end;
    }
    ...
    ret = base64_decode((char *)srv->value, sizeof(srv->value), value, SR_SRV_VALUE_BASE64_LEN - 1);
...
sr_state_get_start_time_of_current_protocol_run(void)
{
    ...
    */
    ...
    log_debug(LD_GENERAL,
              ...
              "Time elapsed: %u (%d)",
              (unsigned)beginning_of_curr_round, (unsigned)time_elapsed_since_start_of_run,
              voting_interval);
...
{
    time_t start_time_of_current_run = sr_state_get_start_time_of_current_protocol_run();
