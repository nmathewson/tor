...
replaycache_add_and_test_internal(time_t present, replaycache_t *r,
                                  const void *data, size_t len, time_t *elapsed)
{
    int rv = 0;
            ...
            if (elapsed) {
                if (present >= *access_time) {
                    *elapsed = present - *access_time;
                    ...
                    /* We shouldn't really be seeing hits from the future,
                     * but... */
                    *elapsed = 0;
                }
            }
 ...
 */
int
replaycache_add_test_and_elapsed(replaycache_t *r, const void *data, size_t len,
                                 time_t *elapsed)
{
