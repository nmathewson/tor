                                       ...
                                       int reason)
{
    ...
    smartlist_t *conns = connection_list_by_type_state(
        CONN_TYPE_AP, AP_CONN_STATE_RENDDESC_WAIT);
    ...
    SMARTLIST_FOREACH_BEGIN(conns, connection_t *, base_conn)
    {
         ...
         * which we tried to get the descriptor */
        if (!edge_conn->hs_ident ||
...
retry_all_socks_conn_waiting_for_desc(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN(conns, connection_t *, base_conn)
    {
        ...
        if (status == HS_CLIENT_FETCH_HAVE_DESC) {
             ...
             * usuable. Once it came back to life, the intro point failure cache
             * was cleaned up and thus the descriptor became usable again
                                   ...
                                   const hs_descriptor_t *desc)
{
                            ...
                            hs_desc_intro_point_t *, ip)
    {
        SMARTLIST_FOREACH_BEGIN(ip->link_specifiers, const link_specifier_t *,
                                lspec)
        {
            /* Not all tor node have an ed25519 identity key so we still rely on
             * the legacy identity digest. */
...
send_introduce1(origin_circuit_t *intro_circ, origin_circuit_t *rend_circ)
{
    ...
    if (hs_circ_send_introduce1(intro_circ, rend_circ, ip,
        ...
        if (TO_CIRCUIT(intro_circ)->marked_for_close) {
             ...
             * that same intro point, we'll note down the intro point failure so
...
client_get_random_intro(const ed25519_public_key_t *service_pk)
{
    ...
    while (smartlist_len(usable_ips) != 0) {
        ...
        if (routerset_contains_extendinfo(options->ExcludeNodes, ei)) {
            ...
            if (ei_excluded) {
                /* If something was already here free it. After the loop is gone
                 * we will examine the last excluded intro point, and that's
                 * fine since that's random anyway */
                     ...
                     size_t payload_len)
{
    ...
    switch (status) {
        ...
        /* It is possible that the intro point can send us an unknown status
         * code for the NACK that we do not know about like a new code for
         * instance. Just fallthrough so we can note down the NACK and
         * re-extend. */
        default:
            handle_introduce_ack_bad(circ, status);
            /* We are going to see if we have to close the circuits (IP and RP)
             * or we can re-extend to a new intro point. */
...
hs_config_client_authorization(const or_options_t *options, int validate_only)
{
    ...
    SMARTLIST_FOREACH_BEGIN(file_list, char *, filename)
    {
        ...
        if (auth) {
            ...
            if (hs_parse_address(auth->onion_address, &identity_pk, NULL,
                                 NULL) < 0) {
                log_warn(LD_REND,
