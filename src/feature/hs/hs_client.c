...
find_entry_conns(const ed25519_public_key_t *service_identity_pk)
{
    ...
    conns = connection_list_by_type_state(CONN_TYPE_AP, AP_CONN_STATE_RENDDESC_WAIT);
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, base_conn) {
        ...
        if (!edge_conn->hs_ident ||
            (service_identity_pk &&
             !ed25519_pubkey_eq(service_identity_pk, &edge_conn->hs_ident->identity_pk))) {
            continue;
        }
...
{
    smartlist_t *conns = connection_list_by_type_purpose(CONN_TYPE_DIR, DIR_PURPOSE_FETCH_HSDESC);
...
flag_all_conn_wait_desc(const ed25519_public_key_t *service_identity_pk)
{
    ...
    smartlist_t *conns = connection_list_by_type_state(CONN_TYPE_AP, AP_CONN_STATE_CIRCUIT_WAIT);
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn) {
        ...
        if (edge_conn->hs_ident &&
            ed25519_pubkey_eq(&edge_conn->hs_ident->identity_pk, service_identity_pk)) {
...
purge_hid_serv_request(const ed25519_public_key_t *identity_pk)
{
    ...
    hs_build_blinded_pubkey(identity_pk, NULL, 0, hs_get_time_period_num(0), &blinded_pk);
                                       ...
                                       hs_client_fetch_status_t status, int reason)
{
    ...
    if (count > 0) {
        ...
        log_notice(LD_REND,
                   ...
                   count, safe_str_client(onion_address), stream_end_reason_to_string(reason),
                               ...
                               const routerstatus_t *hsdir)
{
    ...
    /* Get blinded pubkey */
    hs_build_blinded_pubkey(onion_identity_pk, NULL, 0, current_time_period, &blinded_pubkey);
    /* ...and base64 it. */
    ...
    hs_ident_dir_conn_init(onion_identity_pk, &blinded_pubkey, &hs_conn_dir_ident);
    ...
    log_info(LD_REND,
             ...
             safe_str_client(base64_blinded_pubkey), safe_str_client(routerstatus_describe(hsdir)));

    /* Fire a REQUESTED event on the control port. */
    hs_control_desc_event_requested(onion_identity_pk, base64_blinded_pubkey, hsdir);
...
pick_hsdir_v3(const ed25519_public_key_t *onion_identity_pk)
{
    ...
    hs_get_responsible_hsdirs(&blinded_pubkey, current_time_period, 0, 1, responsible_hsdirs);
...
find_desc_intro_point_by_ident(const hs_ident_circuit_t *ident, const hs_descriptor_t *desc)
{
    ...
    SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points, const hs_desc_intro_point_t *, ip) {
        if (ed25519_pubkey_eq(&ident->intro_auth_pk, &ip->auth_key_cert->signed_key)) {
            intro_point = ip;
            break;
        }
...
find_desc_intro_point_by_legacy_id(const char *legacy_id, const hs_descriptor_t *desc)
{
    ...
    SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points, hs_desc_intro_point_t *, ip) {
        SMARTLIST_FOREACH_BEGIN (ip->link_specifiers, const link_specifier_t *, lspec) {
            ...
            if (fast_memneq(legacy_id, link_specifier_getconstarray_un_legacy_id(lspec),
                            DIGEST_LEN)) {
                break;
            }
            /* Found it. */
...
send_introduce1(origin_circuit_t *intro_circ, origin_circuit_t *rend_circ)
{
    ...
    if (desc == NULL || !hs_client_any_intro_points_usable(service_identity_pk, desc)) {
        log_info(LD_REND, "Request to %s %s. Trying to fetch a new descriptor.",
                 safe_str_client(onion_address),
                 (desc) ? "didn't have usable intro points" : "didn't have a descriptor");
    ...
    if (hs_circ_send_introduce1(intro_circ, rend_circ, ip, &desc->subcredential) < 0) {
    ...
    ed25519_pubkey_copy(&rend_circ->hs_ident->intro_auth_pk, &intro_circ->hs_ident->intro_auth_pk);

    /* Now, we wait for an ACK or NAK on this circuit. */
    circuit_change_purpose(TO_CIRCUIT(intro_circ), CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT);
...
setup_intro_circ_auth_key(origin_circuit_t *circ)
{
    ...
    /* We will go over every intro point and try to find which one is linked to
     * that circuit. Those lists are small so it's not that expensive. */
    ip = find_desc_intro_point_by_legacy_id(circ->build_state->chosen_exit->identity_digest, desc);
    if (ip) {
        /* We got it, copy its authentication key to the identifier. */
        ed25519_pubkey_copy(&circ->hs_ident->intro_auth_pk, &ip->auth_key_cert->signed_key);
        goto end;
    }
...
client_rendezvous_circ_has_opened(origin_circuit_t *circ)
{
    ...
    if (!TO_CIRCUIT(circ)->marked_for_close) {
        hs_circuitmap_register_rend_circ_client_side(circ, circ->hs_ident->rendezvous_cookie);
    }
}
 ...
 */
static int
intro_point_is_usable(const ed25519_public_key_t *service_pk, const hs_desc_intro_point_t *ip)
{
    ...
    state = hs_cache_client_intro_state_find(service_pk, &ip->auth_key_cert->signed_key);
    if (state == NULL) {
...
client_get_random_intro(const ed25519_public_key_t *service_pk)
{
    ...
    hs_build_address(service_pk, desc ? desc->plaintext_data.version : HS_VERSION_THREE,
                     onion_address);
...
intro_points_all_timed_out(const ed25519_public_key_t *service_pk)
{
    ...
    SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points, const hs_desc_intro_point_t *, ip) {
        const hs_cache_intro_state_t *state =
            hs_cache_client_intro_state_find(service_pk, &ip->auth_key_cert->signed_key);
...
socks_mark_rend_circuit_timed_out(const origin_circuit_t *rend_circ)
{
     ...
     * the error. */
    for (edge_connection_t *edge = rend_circ->p_streams; edge; edge = edge->next_stream) {
        entry_connection_t *entry = EDGE_TO_ENTRY_CONN(edge);
        if (entry->socks_request) {
            entry->socks_request->socks_extended_error_code = SOCKS5_HS_REND_FAILED;
        }
    }
}
...
static void
socks_mark_introduction_failed(entry_connection_t *conn, const ed25519_public_key_t *identity_pk)
{
...
close_or_reextend_intro_circ(origin_circuit_t *intro_circ)
{
    ...
    if (!hs_client_any_intro_points_usable(&intro_circ->hs_ident->identity_pk, desc)) {
        goto close;
    }
...
close:
    ...
    if (!TO_CIRCUIT(intro_circ)->marked_for_close) {
        circuit_change_purpose(TO_CIRCUIT(intro_circ), CIRCUIT_PURPOSE_C_INTRODUCE_ACKED);
        circuit_mark_for_close(TO_CIRCUIT(intro_circ), END_CIRC_REASON_FINISHED);
    }
    /* Close the related rendezvous circuit. */
    rend_circ = hs_circuitmap_get_rend_circ_client_side(intro_circ->hs_ident->rendezvous_cookie);
...
handle_introduce_ack_success(origin_circuit_t *intro_circ)
{
    ...
    circuit_change_purpose(TO_CIRCUIT(rend_circ), CIRCUIT_PURPOSE_C_REND_READY_INTRO_ACKED);
    /* Set timestamp_dirty, because circuit_expire_building expects it to
...
handle_introduce_ack_bad(origin_circuit_t *circ, int status)
{
    ...
    log_info(LD_REND, "Received INTRODUCE_ACK nack by %s. Reason: %u",
             safe_str_client(extend_info_describe(circ->build_state->chosen_exit)), status);
    ...
    hs_cache_client_intro_state_note(&circ->hs_ident->identity_pk, &circ->hs_ident->intro_auth_pk,
                                     INTRO_POINT_FAILURE_GENERIC);
}
 ...
 * introduction point. */
static int
handle_introduce_ack(origin_circuit_t *circ, const uint8_t *payload, size_t payload_len)
{
    int status, ret = -1;
...
STATIC int
handle_rendezvous2(origin_circuit_t *circ, const uint8_t *payload, size_t payload_len)
{
    int ret = -1;
    ...
    if (hs_cell_parse_rendezvous2(payload, payload_len, handshake_info, sizeof(handshake_info)) <
        0) {
        goto err;
    }
    ...
    if (hs_ntor_client_get_rendezvous1_keys(&ident->intro_auth_pk, &ident->rendezvous_client_kp,
    ...
    if (hs_circuit_setup_e2e_rend_circ(circ, keys.ntor_key_seed, sizeof(keys.ntor_key_seed), 0) <
        0) {
                        ...
                        hs_client_fetch_status_t *status_out)
{
    ...
    /* Check if fetching a desc for this HS is useful to us right now */
    {
        ...
        if (cached_desc && hs_client_any_intro_points_usable(identity_pk, cached_desc)) {
...
{
    DIGEST256MAP_FOREACH_MODIFY (client_auths, key, hs_client_service_authorization_t *, auth) {
...
client_desc_has_arrived(const smartlist_t *entry_conns)
{
    ...
    SMARTLIST_FOREACH_BEGIN (entry_conns, entry_connection_t *, entry_conn) {
        const hs_descriptor_t *desc;
        edge_connection_t *edge_conn = ENTRY_TO_EDGE_CONN(entry_conn);
        const ed25519_public_key_t *identity_pk = &edge_conn->hs_ident->identity_pk;
...
static void
client_desc_missing_bad_client_auth(const smartlist_t *entry_conns, hs_desc_decode_status_t status)
{
    tor_assert(entry_conns);
...
static void
client_dir_fetch_200(dir_connection_t *dir_conn, const smartlist_t *entry_conns, const char *body)
{
    ...
    decode_status = hs_cache_store_as_client(body, &dir_conn->hs_ident->identity_pk);
    ...
    case HS_DESC_DECODE_BAD_CLIENT_AUTH:
        ...
        if (decode_status == HS_DESC_DECODE_OK) {
            ...
            log_info(LD_REND,
                     ...
                     decode_status == HS_DESC_DECODE_NEED_CLIENT_AUTH ? "missing" : "new");
        }
        /* Fire control port RECEIVED event. */
        hs_control_desc_event_received(dir_conn->hs_ident, dir_conn->identity_digest);
        hs_control_desc_event_content(dir_conn->hs_ident, dir_conn->identity_digest, body);
        break;
    ...
    default:
        ...
        hs_control_desc_event_failed(dir_conn->hs_ident, dir_conn->identity_digest, "BAD_DESC");
...
static void
client_dir_fetch_404(dir_connection_t *dir_conn, const smartlist_t *entry_conns)
{
    tor_assert(entry_conns);
    ...
    /* Fire control port FAILED event. */
    hs_control_desc_event_failed(dir_conn->hs_ident, dir_conn->identity_digest, "NOT_FOUND");
    hs_control_desc_event_content(dir_conn->hs_ident, dir_conn->identity_digest, NULL);
...
client_dir_fetch_400(dir_connection_t *dir_conn, const char *reason)
{
    ...
    /* Fire control port FAILED event. */
    hs_control_desc_event_failed(dir_conn->hs_ident, dir_conn->identity_digest, "QUERY_REJECTED");
    hs_control_desc_event_content(dir_conn->hs_ident, dir_conn->identity_digest, NULL);
}

/** Called when we get an unexpected directory fetch status code. */
static void
client_dir_fetch_unexpected(dir_connection_t *dir_conn, const char *reason, const int status_code)
{
    tor_assert(dir_conn);
    ...
    log_warn(LD_REND,
             ...
             status_code, escaped(reason), TO_CONN(dir_conn)->address, TO_CONN(dir_conn)->port);
    /* Fire control port FAILED event. */
    hs_control_desc_event_failed(dir_conn->hs_ident, dir_conn->identity_digest, "UNEXPECTED");
    hs_control_desc_event_content(dir_conn->hs_ident, dir_conn->identity_digest, NULL);
}
 ...
 */
static int
store_permanent_client_auth_credentials(const hs_client_service_authorization_t *creds)
{
    const or_options_t *options = get_options();
    char *full_fname = NULL;
    ...
    full_fname = get_client_auth_creds_filename(creds->onion_address, options->ClientOnionAuthDir);

    /* Encode client private key */
    base32_encode(priv_key_b32, sizeof(priv_key_b32), (char *)creds->enc_seckey.secret_key,
                  sizeof(creds->enc_seckey.secret_key));

    /* Get the full file contents and write it to disk! */
    tor_asprintf(&file_contents, "%s:descriptor:x25519:%s", creds->onion_address, priv_key_b32);
...
hs_client_register_auth_credentials(hs_client_service_authorization_t *creds)
{
    ...
    if (hs_parse_address(creds->onion_address, &service_identity_pk, NULL, NULL) < 0) {
 ...
 */
static hs_client_service_authorization_t *
get_creds_from_client_auth_filename(const char *filename, const or_options_t *options)
{
    ...
    client_key_file_path = hs_path_from_filename(options->ClientOnionAuthDir, filename);
...
remove_client_auth_creds_file(const char *filename)
{
    ...
    creds_file_path = hs_path_from_filename(options->ClientOnionAuthDir, filename);
    if (tor_unlink(creds_file_path) != 0) {
        log_warn(LD_REND, "Failed to remove client auth file (%s).", creds_file_path);
        goto end;
    }

    log_warn(LD_REND, "Successfuly removed client auth file (%s).", creds_file_path);
 ...
 */
static void
find_and_remove_client_auth_creds_file(const hs_client_service_authorization_t *cred)
{
...
hs_client_circuit_cleanup_on_free(const circuit_t *circ)
{
    ...
    case CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT:
        log_info(LD_REND,
                 ...
                 safe_str_client(build_state_get_exit_nickname(orig_circ->build_state)), failure);
        hs_cache_client_intro_state_note(&orig_circ->hs_ident->identity_pk,
                                         &orig_circ->hs_ident->intro_auth_pk, failure);
        break;
...
hs_client_decode_descriptor(const char *desc_str, const ed25519_public_key_t *service_identity_pk,
                            hs_descriptor_t **desc)
{
    ...
    {
        uint64_t current_time_period = hs_get_time_period_num(0);
        hs_build_blinded_pubkey(service_identity_pk, NULL, 0, current_time_period, &blinded_pubkey);
        hs_get_subcredential(service_identity_pk, &blinded_pubkey, &subcredential);
    }

    /* Parse descriptor */
    ret = hs_desc_decode_descriptor(desc_str, &subcredential, client_auth_sk, desc);
...
int
hs_client_send_introduce1(origin_circuit_t *intro_circ, origin_circuit_t *rend_circ)
{
...
int
hs_client_receive_rendezvous_acked(origin_circuit_t *circ, const uint8_t *payload,
                                   size_t payload_len)
{
    tor_assert(circ);
    tor_assert(payload);
...
parse_auth_file_content(const char *client_key_str)
{
    ...
    if (base32_decode((char *)auth->enc_seckey.secret_key, sizeof(auth->enc_seckey.secret_key),
                      seckey_b32, strlen(seckey_b32)) != sizeof(auth->enc_seckey.secret_key)) {
        log_warn(LD_REND,
                 "Client authorization encoded base32 private key "
                 "can't be decoded: %s",
                 seckey_b32);
...
void
hs_client_dir_fetch_done(dir_connection_t *dir_conn, const char *reason, const char *body,
...
hs_client_get_random_intro_from_edge(const edge_connection_t *edge_conn)
{
    ...
    return (edge_conn->hs_ident) ? client_get_random_intro(&edge_conn->hs_ident->identity_pk)
...
int
hs_client_receive_introduce_ack(origin_circuit_t *circ, const uint8_t *payload, size_t payload_len)
{
    int ret = -1;
 ...
 * on error. */
int
hs_client_receive_rendezvous2(origin_circuit_t *circ, const uint8_t *payload, size_t payload_len)
{
    int ret = -1;
