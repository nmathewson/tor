...
find_entry_conns(const ed25519_public_key_t *service_identity_pk)
{
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, base_conn) {
        ...
        /* Only consider the entry connections that matches the service for
         * which we just fetched its descriptor. */
...
{
    smartlist_t *conns = connection_list_by_type_purpose(
        CONN_TYPE_DIR, DIR_PURPOSE_FETCH_HSDESC);
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn) {
        const hs_ident_dir_conn_t *ident = TO_DIR_CONN(conn)->hs_ident;
        if (BUG(ident == NULL)) {
            /* A directory connection fetching a service descriptor can't have
             * an empty hidden service identifier. */
            continue;
        }
        log_debug(LD_REND,
...
flag_all_conn_wait_desc(const ed25519_public_key_t *service_identity_pk)
{
    ...
    smartlist_t *conns = connection_list_by_type_state(
        CONN_TYPE_AP, AP_CONN_STATE_CIRCUIT_WAIT);
...
retry_all_socks_conn_waiting_for_desc(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (entry_conns, entry_connection_t *, entry_conn) {
         ...
         * However, the refetch process can cleanup all SOCKS request to the
         * same service if an internal error happens. Thus, we can end up with
         * closed connections in our list. */
        if (base_conn->marked_for_close) {
            continue;
        }

        /* XXX: There is an optimization we could do which is that for a
         * service key, we could check if we can fetch and remember that
         * decision. */
        ...
        if (status == HS_CLIENT_FETCH_HAVE_DESC) {
            /* This is a rare case where a SOCKS connection is in state waiting
             * for a descriptor but we do have it in the cache.
             *
             * This can happen is tor comes back from suspend where it
             * previously had the descriptor but the intro points were not
             * usuable. Once it came back to life, the intro point failure
             * cache was cleaned up and thus the descriptor became usable again
             * leaving us in this code path.
             *
             * We'll mark the connection as waiting for a circuit so the
             * descriptor can be retried. This is safe because a connection in
             * state waiting for a descriptor can not be in the entry
...
note_connection_attempt_succeeded(const hs_ident_edge_conn_t *hs_conn_ident)
{
     ...
     * We don't try to do it here because we still need to keep intact the
     * intro point state for future connections. Even though we are able to
     * connect to the service, doesn't mean we should reset the timed out intro
     * points.
     *
                                   ...
                                   const hs_descriptor_t *desc)
{
    ...
    SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points,
        ...
        SMARTLIST_FOREACH_BEGIN (ip->link_specifiers, const link_specifier_t *,
                                 lspec) {
            /* Not all tor node have an ed25519 identity key so we still rely
             * on the legacy identity digest. */
...
send_introduce1(origin_circuit_t *intro_circ, origin_circuit_t *rend_circ)
{
    ...
    /* For logging purposes. There will be a time where the hs_ident will have
     * a version number but for now there is none because it's all v3. */
    ...
    if (ip == NULL) {
         ...
         * this introduction circuit open and waiting for the rendezvous
         * circuit to be ready. Which results in this situation where we can't
         * find the corresponding intro point within the descriptor of the
         * service. */
        log_info(LD_REND,
    ...
    if (hs_circ_send_introduce1(intro_circ, rend_circ, ip,
        ...
        if (TO_CIRCUIT(intro_circ)->marked_for_close) {
            /* If the introduction circuit was closed, we were unable to send
             * the cell for some reasons. In any case, the intro circuit has to
             * be closed by the above function. We'll return a transient error
             * so tor can recover and pick a new intro point. To avoid picking
             * that same intro point, we'll note down the intro point failure
             * so it doesn't get reused. */
            hs_cache_client_intro_state_note(
                service_identity_pk, &intro_circ->hs_ident->intro_auth_pk,
                INTRO_POINT_FAILURE_GENERIC);
        }
        /* It is also possible that the rendezvous circuit was closed due to
         * being unable to use the rendezvous point node_t so in that case, we
         * also want to recover and let tor pick a new one. */
        goto tran_err;
    }

    /* Cell has been sent successfully. Copy the introduction point
     * authentication and encryption key in the rendezvous circuit identifier
     * so we can compute the ntor keys when we receive the RENDEZVOUS2 cell. */
...
perm_err:
     ...
     * because we weren't able to send the cell. Make sure we don't double
     * close it which would result in a warning. */
    if (!TO_CIRCUIT(intro_circ)->marked_for_close) {
...
setup_intro_circ_auth_key(origin_circuit_t *circ)
{
    ...
    if (desc == NULL) {
        /* There is a very small race window between the opening of this
         * circuit and the client descriptor cache that gets purged (NEWNYM) or
         * the cleaned up because it expired. Mark the circuit for close so a
         * new descriptor fetch can occur. */
...
desc_intro_point_to_extend_info(const hs_desc_intro_point_t *ip)
{
    ...
    ei =
        hs_get_extend_info_from_lspecs(ip->link_specifiers, &ip->onion_key, 0);
...
client_get_random_intro(const ed25519_public_key_t *service_pk)
{
    ...
    while (smartlist_len(usable_ips) != 0) {
        ...
        /* We need to make sure we have a usable intro points which is in a
         * good state in our cache. */
        ...
        if (ei == NULL) {
            /* We can get here for instance if the intro point is a private
             * address and we aren't allowed to extend to those. */
            log_info(
                LD_REND,
        ...
        if (routerset_contains_extendinfo(options->ExcludeNodes, ei)) {
            /* If this pick is in the ExcludeNodes list, we keep its reference
             * so if we ever end up not being able to pick anything else and
             * StrictNodes is unset, we'll use it. */
            if (ei_excluded) {
                /* If something was already here free it. After the loop is
                 * gone we will examine the last excluded intro point, and
                 * that's fine since that's random anyway */
...
intro_points_all_timed_out(const ed25519_public_key_t *service_pk)
{
    ...
    if (BUG(!desc)) {
        /* We can't introduce without a descriptor so ending up here means
         * somehow between the introduction failure and this, the cache entry
         * was removed which shouldn't be possible in theory. */
        goto end;
    }
    ...
    SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points,
        ...
        const hs_cache_intro_state_t *state = hs_cache_client_intro_state_find(
            service_pk, &ip->auth_key_cert->signed_key);
        if (!state || !state->timed_out) {
            /* No state or if this intro point has not timed out, we are done
             * since clearly not all of them have timed out. */
            goto end;
        }
    } SMARTLIST_FOREACH_END (ip);
...
close:
    /* Change the intro circuit purpose before so we don't report an intro
     * point failure again triggering an extra descriptor fetch. The circuit
     * can already be closed on failure to re-extend. */
    if (!TO_CIRCUIT(intro_circ)->marked_for_close) {
        circuit_change_purpose(TO_CIRCUIT(intro_circ),
    ...
    if (rend_circ) {
        circuit_mark_for_close(TO_CIRCUIT(rend_circ),
                               END_CIRC_REASON_FINISHED);
    }
...
handle_introduce_ack_bad(origin_circuit_t *circ, int status)
{
    ...
    /* Note down this failure in the intro point failure cache. Depending on
                     ...
                     size_t payload_len)
{
    ...
    default:
        ...
        /* We are going to see if we have to close the circuits (IP and RP) or
         * we can re-extend to a new intro point. */
...
{
    /* If the map is not allocated, we can assume that we do not have any
...
client_desc_has_arrived(const smartlist_t *entry_conns)
{
    ...
    SMARTLIST_FOREACH_BEGIN (entry_conns, entry_connection_t *, entry_conn) {
        ...
        /* We were just called because we stored the descriptor for this
         * service
        ...
        if (!hs_client_any_intro_points_usable(identity_pk, desc)) {
            log_info(LD_REND, "Hidden service descriptor is unusable. "
                              "Closing streams.");
            /* Report the extended socks error code that we were unable to
             * introduce to the service. */
            socks_mark_introduction_failed(entry_conn, identity_pk);

            connection_mark_unattached_ap(entry_conn,
                                          END_STREAM_REASON_RESOLVEFAILED);
            /* We are unable to use the descriptor so remove the directory
             * request from the cache so the next connection can try again. */
    ...
    case HS_DESC_DECODE_BAD_CLIENT_AUTH:
        ...
        if (decode_status == HS_DESC_DECODE_OK) {
            ...
            log_info(LD_REND,
                     ...
                     decode_status == HS_DESC_DECODE_NEED_CLIENT_AUTH
                         ? "missing"
                         : "new");
        }
...
hs_client_register_auth_credentials(hs_client_service_authorization_t *creds)
{
    ...
    /* If we reach this point, the credentials will be stored one way or
     * another: Make them permanent if the user asked us to. */
...
hs_client_circuit_cleanup_on_close(const circuit_t *circ)
{
    ...
    case CIRCUIT_PURPOSE_C_REND_JOINED:
        /* Report extended SOCKS error code when a rendezvous circuit times
         * out. This MUST be done on_close() because it is possible the entry
                            ...
                            hs_descriptor_t **desc)
{
    ...
    /* Parse descriptor */
    ret = hs_desc_decode_descriptor(desc_str, &subcredential, client_auth_sk,
                                    desc);
...
hs_client_refetch_hsdesc(const ed25519_public_key_t *identity_pk)
{
    ...
    if (fetch_status_should_close_socks(status)) {
        close_all_socks_conns_waiting_for_desc(
            identity_pk, status, END_STREAM_REASON_RESOLVEFAILED);
                                   ...
                                   const uint8_t *payload, size_t payload_len)
{
     ...
     * Waiting any longer opens us up to attacks from malicious hidden
     * services. They could induce the client to attempt to connect to their
     * hidden service and never reply to the client's rend requests */
...
hs_config_client_authorization(const or_options_t *options, int validate_only)
{
    ...
    SMARTLIST_FOREACH_BEGIN (file_list, const char *, filename) {
        ...
        if (hs_parse_address(auth->onion_address, &identity_pk, NULL, NULL) <
            0) {
            log_warn(LD_REND,
...
hs_client_reextend_intro_circuit(origin_circuit_t *circ)
{
    ...
    if (circ->remaining_relay_early_cells) {
        ...
        if (ret == 0) {
            /* We were able to extend so update the timestamp so we avoid
             * expiring this circuit too early. The intro circuit is short live
             * so the linkability issue is minimized, we just need the circuit
             * to hold a bit longer so we can introduce. */
            TO_CIRCUIT(circ)->timestamp_dirty = time(NULL);
        }
    } else {
...
hs_client_close_intro_circuits_from_desc(const hs_descriptor_t *desc)
{
    ...
    /* We iterate over all client intro circuits because they aren't kept in
     * the HS circuitmap. That is probably something we want to do one day. */
