                      ...
                      uint8_t *mac_out, size_t mac_out_len)
{
    ...
    /* Second, put the CLIENT_PK + ENCRYPTED_DATA but ommit the MAC field
     * (which is junk at this point). */
                                   ...
                                   const uint8_t *onion_pk)
{
    ...
    trn_cell_introduce_encrypted_set_onion_key_len(cell,
                                                   CURVE25519_PUBKEY_LEN);
                              ...
                              const hs_cell_introduce1_data_t *data)
{
    ...
    compute_introduce_mac(encoded_cell, encoded_cell_len, encrypted,
                          encrypted_len, keys.mac_key, sizeof(keys.mac_key),
                          mac, sizeof(mac));
                                    ...
                                    trn_cell_extension_t *extensions)
{
    ...
    build_establish_intro_dos_param(
        dos_ext, TRUNNEL_DOS_PARAM_TYPE_INTRO2_BURST_PER_SEC,
    ...
    /* We've just add an extension field to the cell extensions so increment
     * the total number. */
                              ...
                              uint8_t *cell_out)
{
     ...
     * AUTH_KEY to match */
    {
        ...
        /* We do this call _after_ setting the length because it's reallocated
         * at that point only. */
    ...
    /* Calculate HANDSHAKE_AUTH field (MAC). */
    {
        ...
        crypto_mac_sha3_256(mac, sizeof(mac), (uint8_t *)circ_nonce,
                            DIGEST_LEN, tmp_cell_enc,
    ...
    /* Calculate the cell signature SIG. */
    {
        ...
        if (ed25519_sign_prefixed(
                &sig, tmp_cell_enc, tmp_cell_enc_len - tmp_cell_sig_offset,
...
hs_cell_parse_intro_established(const uint8_t *payload, size_t payload_len)
{
    ...
    if (ret >= 0) {
        /* On success, we do not keep the cell, we just notify the caller that
                                   ...
                                   size_t encrypted_section_len)
{
    ...
    for (unsigned i = 0; i < data->n_subcredentials; ++i) {
        ...
        /* Time-invariant conditional copy: if the MAC is what we expected,
         * then set intro_keys_result to intro_keys[i]. Otherwise, don't: but
         * don't leak which one it was! */
        bool equal =
            tor_memeq(mac, encrypted_section + mac_offset, sizeof(mac));
                         ...
                         const hs_service_t *service)
{
...

    {
        ...
        /* It's symmetric encryption so it's correct to use the ENCRYPTED
         * length for decryption. Computes the length of ENCRYPTED_DATA meaning
         * removing the CLIENT_PK and MAC length. */
