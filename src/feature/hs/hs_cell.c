...
compute_introduce_mac(const uint8_t *encoded_cell, size_t encoded_cell_len,
                      const uint8_t *encrypted, size_t encrypted_len, const uint8_t *mac_key,
                      size_t mac_key_len, uint8_t *mac_out, size_t mac_out_len)
{
    size_t offset = 0;
    size_t mac_msg_len;
    ...
    crypto_mac_sha3_256(mac_out, mac_out_len, mac_key, mac_key_len, mac_msg, mac_msg_len);
...
get_introduce2_key_material(const ed25519_public_key_t *auth_key,
                            const curve25519_keypair_t *enc_key, size_t n_subcredentials,
                            const hs_subcredential_t *subcredentials,
                            const uint8_t *encrypted_section, curve25519_public_key_t *client_pk)
{
    ...
    if (hs_ntor_service_get_introduce1_keys_multi(auth_key, enc_key, client_pk, n_subcredentials,
                   ...
                   size_t encrypted_section_len)
{
    ...
    cipher = crypto_cipher_new_with_bits((char *)enc_key, CURVE25519_PUBKEY_LEN * 8);
    tor_assert(cipher);

    /* This is symmetric encryption so can't be bigger than the encrypted
     * section length. */
    decrypted = tor_malloc_zero(encrypted_section_len);
    if (crypto_cipher_decrypt(cipher, (char *)decrypted, (const char *)encrypted_section,
...
parse_introduce2_encrypted(const uint8_t *decrypted_data, size_t decrypted_len,
                           const origin_circuit_t *circ, const hs_service_t *service)
{
    ...
    if (trn_cell_introduce_encrypted_parse(&enc_cell, decrypted_data, decrypted_len) < 0) {
        log_info(LD_REND,
    ...
    if (trn_cell_introduce_encrypted_getlen_onion_key(enc_cell) != CURVE25519_PUBKEY_LEN) {
        log_info(LD_REND,
...
static ssize_t
build_legacy_establish_intro(const char *circ_nonce, crypto_pk_t *enc_key, uint8_t *cell_out)
{
    ssize_t cell_len;
    ...
    cell_len = rend_service_encode_establish_intro_cell((char *)cell_out, RELAY_PAYLOAD_SIZE,
                                                        enc_key, circ_nonce);
    return cell_len;
}
...
static int
parse_introduce2_cell(const hs_service_t *service, const origin_circuit_t *circ,
 ...
 * INTRODUCE1 cell. */
static void
introduce1_set_encrypted_onion_key(trn_cell_introduce_encrypted_t *cell, const uint8_t *onion_pk)
{
    tor_assert(cell);
    tor_assert(onion_pk);
    /* There is only one possible key type for a non legacy cell. */
    trn_cell_introduce_encrypted_set_onion_key_type(cell, TRUNNEL_HS_INTRO_ONION_KEY_TYPE_NTOR);
 ...
 * INTRODUCE1 cell. */
static void
introduce1_set_encrypted_link_spec(trn_cell_introduce_encrypted_t *cell, const smartlist_t *lspecs)
{
    tor_assert(cell);
    tor_assert(lspecs);
                                 ...
                                 trn_cell_introduce_encrypted_t *enc_cell)
{
    ...
    ssize_t full_len =
        trn_cell_introduce1_encoded_len(cell) + trn_cell_introduce_encrypted_encoded_len(enc_cell);
                              ...
                              const hs_cell_introduce1_data_t *data)
{
     ...
     * computation on it. */
    encoded_cell_len = trn_cell_introduce1_encode(encoded_cell, sizeof(encoded_cell), cell);
    /* We have a much more serious issue if this isn't true. */
    tor_assert(encoded_cell_len > 0);

    encoded_enc_cell_len =
        trn_cell_introduce_encrypted_encode(encoded_enc_cell, sizeof(encoded_enc_cell), enc_cell);
    /* We have a much more serious issue if this isn't true. */
    tor_assert(encoded_enc_cell_len > 0);

    /* Get the key material for the encryption. */
    if (hs_ntor_client_get_introduce1_keys(data->auth_pk, data->enc_pk, data->client_kp,
                                           data->subcredential, &keys) < 0) {
        tor_assert_unreached();
    }

    /* Prepare cipher with the encryption key just computed. */
    cipher = crypto_cipher_new_with_bits((const char *)keys.enc_key, sizeof(keys.enc_key) * 8);
    tor_assert(cipher);

    /* Compute the length of the ENCRYPTED section which is the CLIENT_PK,
     * ENCRYPTED_DATA and MAC length. */
    encrypted_len = sizeof(data->client_kp->pubkey) + encoded_enc_cell_len + sizeof(mac);
    ...
    crypto_cipher_encrypt(cipher, (char *)encrypted + offset, (const char *)encoded_enc_cell,
                          encoded_enc_cell_len);
    crypto_cipher_free(cipher);
    ...
    compute_introduce_mac(encoded_cell, encoded_cell_len, encrypted, encrypted_len, keys.mac_key,
                          sizeof(keys.mac_key), mac, sizeof(mac));
    ...
    memcpy(trn_cell_introduce1_getarray_encrypted(cell), encrypted, encrypted_len);
...
static void
introduce1_set_encrypted(trn_cell_introduce1_t *cell, const hs_cell_introduce1_data_t *data)
{
    trn_cell_introduce_encrypted_t *enc_cell;
    ...
    memcpy(trn_cell_introduce_encrypted_getarray_rend_cookie(enc_cell), data->rendezvous_cookie,
           REND_COOKIE_LEN);
...
static void
introduce1_set_auth_key(trn_cell_introduce1_t *cell, const hs_cell_introduce1_data_t *data)
{
    tor_assert(cell);
    tor_assert(data);
    /* There is only one possible type for a non legacy cell. */
    trn_cell_introduce1_set_auth_key_type(cell, TRUNNEL_HS_INTRO_AUTH_KEY_TYPE_ED25519);
...
static void
introduce1_set_legacy_id(trn_cell_introduce1_t *cell, const hs_cell_introduce1_data_t *data)
{
    tor_assert(cell);
    tor_assert(data);
 ...
 * value. */
static void
build_establish_intro_dos_param(trn_cell_extension_dos_t *dos_ext, uint8_t param_type,
                                uint64_t param_value)
{
    trn_cell_extension_dos_param_t *dos_param = trn_cell_extension_dos_param_new();
                                    ...
                                    trn_cell_extension_t *extensions)
{
    ...
    trn_cell_extension_field_set_field_type(field, TRUNNEL_CELL_EXTENSION_TYPE_DOS);
    ...
    build_establish_intro_dos_param(dos_ext, TRUNNEL_DOS_PARAM_TYPE_INTRO2_RATE_PER_SEC,
                                    service_config->intro_dos_rate_per_sec);
    /* Build DoS parameter INTRO2 burst per second. */
    build_establish_intro_dos_param(dos_ext, TRUNNEL_DOS_PARAM_TYPE_INTRO2_BURST_PER_SEC,
    ...
    ret = trn_cell_extension_dos_encode(field_array, trn_cell_extension_field_getlen_field(field),
                                        dos_ext);
    if (BUG(ret <= 0)) {
        goto err;
    }
     ...
     * total number. */
    trn_cell_extension_set_num(extensions, trn_cell_extension_get_num(extensions) + 1);
                                 ...
                                 const hs_service_intro_point_t *ip)
{
    ...
    if (service_config->has_dos_defense_enabled && ip->support_intro2_dos_defense) {
...
ssize_t
hs_cell_build_establish_intro(const char *circ_nonce, const hs_service_config_t *service_config,
                              const hs_service_intro_point_t *ip, uint8_t *cell_out)
{
    ssize_t cell_len = -1;
    ...
    if (ip->base.is_only_legacy) {
        ...
        cell_len = build_legacy_establish_intro(circ_nonce, ip->legacy_key, cell_out);
    ...
    trn_cell_establish_intro_set_auth_key_type(cell, TRUNNEL_HS_INTRO_AUTH_KEY_TYPE_ED25519);
    ...
    /* Calculate HANDSHAKE_AUTH field (MAC). */
    {
        ...
        ssize_t tmp_cell_mac_offset =
            sig_len + sizeof(cell->sig_len) + trn_cell_establish_intro_getlen_handshake_mac(cell);
        ...
        tmp_cell_enc_len =
            trn_cell_establish_intro_encode(tmp_cell_enc, sizeof(tmp_cell_enc), cell);
        ...
        crypto_mac_sha3_256(mac, sizeof(mac), (uint8_t *)circ_nonce, DIGEST_LEN, tmp_cell_enc,
                            tmp_cell_enc_len - tmp_cell_mac_offset);
    ...
    /* Calculate the cell signature SIG. */
    {
        ...
        if (ed25519_sign_prefixed(&sig, tmp_cell_enc, tmp_cell_enc_len - tmp_cell_sig_offset,
    ...
    cell_len = trn_cell_establish_intro_encode(cell_out, RELAY_PAYLOAD_SIZE, cell);
...
get_introduce2_keys_and_verify_mac(hs_cell_introduce2_data_t *data,
                                   const uint8_t *encrypted_section, size_t encrypted_section_len)
{
    ...
    for (unsigned i = 0; i < data->n_subcredentials; ++i) {
         ...
         * ENCRYPTED section. */
        compute_introduce_mac(data->payload, data->payload_len - encrypted_section_len,
        ...
        memcpy_if_true_timei(equal, intro_keys_result, &intro_keys[i], sizeof(*intro_keys_result));
    }
...
ssize_t
hs_cell_parse_introduce2(hs_cell_introduce2_data_t *data, const origin_circuit_t *circ,
                         const hs_service_t *service)
{
    ...
    if (parse_introduce2_cell(service, circ, data->payload, data->payload_len, &cell) < 0) {
        goto done;
    }
    ...
    intro_keys = get_introduce2_keys_and_verify_mac(data, encrypted_section, encrypted_section_len);
    if (!intro_keys) {
        log_warn(LD_REND,
...

    {
        ...
        const uint8_t *encrypted_data = encrypted_section + sizeof(data->client_pk);
        ...
        decrypted = decrypt_introduce2(intro_keys->enc_key, encrypted_data, encrypted_data_len);
        if (decrypted == NULL) {
            log_info(LD_REND,
        ...
        enc_cell = parse_introduce2_encrypted(decrypted, encrypted_data_len, circ, service);
    ...
    memcpy(data->onion_pk.public_key,
           trn_cell_introduce_encrypted_getconstarray_onion_key(enc_cell), CURVE25519_PUBKEY_LEN);
    ...
    for (size_t idx = 0; idx < trn_cell_introduce_encrypted_get_nspec(enc_cell); idx++) {
        link_specifier_t *lspec = trn_cell_introduce_encrypted_get_nspecs(enc_cell, idx);
        if (BUG(!lspec)) {
            goto done;
        }
...
ssize_t
hs_cell_build_rendezvous1(const uint8_t *rendezvous_cookie, size_t rendezvous_cookie_len,
                          const uint8_t *rendezvous_handshake_info,
                          size_t rendezvous_handshake_info_len, uint8_t *cell_out)
{
    ssize_t cell_len;
    ...
    memcpy(trn_cell_rendezvous1_getarray_rendezvous_cookie(cell), rendezvous_cookie,
           rendezvous_cookie_len);
    /* Set the HANDSHAKE_INFO. */
    trn_cell_rendezvous1_setlen_handshake_info(cell, rendezvous_handshake_info_len);
    memcpy(trn_cell_rendezvous1_getarray_handshake_info(cell), rendezvous_handshake_info,
...
ssize_t
hs_cell_build_introduce1(const hs_cell_introduce1_data_t *data, uint8_t *cell_out)
{
    ssize_t cell_len;
...
ssize_t
hs_cell_build_establish_rendezvous(const uint8_t *rendezvous_cookie, uint8_t *cell_out)
{
    tor_assert(rendezvous_cookie);
    tor_assert(cell_out);
...
int
hs_cell_parse_rendezvous2(const uint8_t *payload, size_t payload_len, uint8_t *handshake_info,
                          size_t handshake_info_len)
{
     ...
     * our code flow. */
    tor_assert(trn_cell_rendezvous2_getlen_handshake_info(cell) == handshake_info_len);
    memcpy(handshake_info, trn_cell_rendezvous2_getconstarray_handshake_info(cell),
           handshake_info_len);
    ret = 0;
