 ...
 * <b>auth_key_out</b>. */
STATIC void
get_auth_key_from_cell(ed25519_public_key_t *auth_key_out, unsigned int cell_type, const void *cell)
{
    size_t auth_key_len;
...
verify_establish_intro_cell(const trn_cell_establish_intro_t *cell,
                            const uint8_t *circuit_key_material, size_t circuit_key_material_len)
{
    ...
    if (trn_cell_establish_intro_getlen_auth_key(cell) != ED25519_PUBKEY_LEN ||
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "ESTABLISH_INTRO auth key length is invalid");
        return -1;
    }
    ...
    /* Verify the sig */
    {
        ...
        if (trn_cell_establish_intro_getlen_sig(cell) != sizeof(sig_struct.sig) ||
            ...
            log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "ESTABLISH_INTRO sig len is invalid");
            return -1;
        }
        ...
        int sig_mismatch = ed25519_checksig_prefixed(&sig_struct, msg, sig_msg_len,
                                                     ESTABLISH_INTRO_SIG_PREFIX, &auth_key);
        if (sig_mismatch) {
            log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "ESTABLISH_INTRO signature not as expected");
            return -1;
        }
    }
    ...
    /* Verify the MAC */
    {
        ...
        crypto_mac_sha3_256(mac, sizeof(mac), circuit_key_material, circuit_key_material_len, msg,
                            auth_msg_len);
...
hs_intro_send_intro_established_cell, (or_circuit_t * circ))
{
    ...
    result_len = trn_cell_intro_established_encode(encoded_cell, encoded_len, cell);
    tor_assert(encoded_len == result_len);

    ret = relay_send_command_from_edge(0, TO_CIRCUIT(circ), RELAY_COMMAND_INTRO_ESTABLISHED,
...
STATIC bool
cell_dos_extension_parameters_are_valid(uint64_t intro2_rate_per_sec, uint64_t intro2_burst_per_sec)
{
    bool ret = false;
...
static void
handle_establish_intro_cell_dos_extension(const trn_cell_extension_field_t *field,
                                          or_circuit_t *circ)
{
    ssize_t ret;
    ...
    ret = trn_cell_extension_dos_parse(&dos, trn_cell_extension_field_getconstarray_field(field),
    ...
    if (!cell_dos_extension_parameters_are_valid(intro2_rate_per_sec, intro2_burst_per_sec)) {
        circ->introduce2_dos_defense_enabled = 0;
        log_info(LD_REND, "Disabling INTRO2 DoS defenses on circuit id %u", circ->p_circ_id);
        goto end;
    }
    ...
    token_bucket_ctr_init(&circ->introduce2_bucket, (uint32_t)intro2_rate_per_sec,
                          (uint32_t)intro2_burst_per_sec, (uint32_t)approx_time());
    log_info(LD_REND, "Intro point DoS defenses enabled. Rate is %" PRIu64 " and Burst is %" PRIu64,
             intro2_rate_per_sec, intro2_burst_per_sec);
...
static void
handle_establish_intro_cell_extensions(const trn_cell_establish_intro_t *parsed_cell,
                                       or_circuit_t *circ)
{
                                     ...
                                     const trn_cell_establish_intro_t *parsed_cell)
{
    ...
    ed25519_public_key_t auth_key;
    get_auth_key_from_cell(&auth_key, RELAY_COMMAND_ESTABLISH_INTRO, parsed_cell);
...
static int
handle_establish_intro(or_circuit_t *circ, const uint8_t *request, size_t request_len)
{
    ...
    log_info(LD_REND, "Received an ESTABLISH_INTRO request on circuit %" PRIu32, circ->p_circ_id);
    ...
    /* Parse the cell */
    ssize_t parsing_result = trn_cell_establish_intro_parse(&parsed_cell, request, request_len);
    if (parsing_result < 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Rejecting %s ESTABLISH_INTRO cell.",
               parsing_result == -1 ? "invalid" : "truncated");
        goto err;
    }

    cell_ok = verify_establish_intro_cell(parsed_cell, (uint8_t *)circ->rend_circ_nonce,
                                          sizeof(circ->rend_circ_nonce));
    if (cell_ok < 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Failed to verify ESTABLISH_INTRO cell.");
        goto err;
    }
...
static int
circuit_is_suitable_intro_point(const or_circuit_t *circ, const char *log_cell_type_str)
{
    tor_assert(circ);
...
int
hs_intro_received_establish_intro(or_circuit_t *circ, const uint8_t *request, size_t request_len)
{
    tor_assert(circ);
    tor_assert(request);
    ...
    default:
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Unrecognized AUTH_KEY_TYPE %u.", first_byte);
        goto err;
    }
...
send_introduce_ack_cell(or_circuit_t *circ, uint16_t status)
{
    ...
    ret =
        relay_send_command_from_edge(CONTROL_CELL_ID, TO_CIRCUIT(circ), RELAY_COMMAND_INTRODUCE_ACK,
                                     (char *)encoded_cell, encoded_len, NULL);
    /* On failure, the above function will close the circuit. */
...
validate_introduce1_parsed_cell(const trn_cell_introduce1_t *cell)
{
    ...
    if (trn_cell_introduce1_get_auth_key_type(cell) != TRUNNEL_HS_INTRO_AUTH_KEY_TYPE_ED25519) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
 ...
 */
STATIC int
handle_introduce1(or_circuit_t *client_circ, const uint8_t *request, size_t request_len)
{
    int ret = -1;
    ...
    ssize_t cell_size = trn_cell_introduce1_parse(&parsed_cell, request, request_len);
    if (cell_size < 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
    ...
    if (relay_send_command_from_edge(CONTROL_CELL_ID, TO_CIRCUIT(service_circ),
                                     RELAY_COMMAND_INTRODUCE2, (char *)request, request_len,
                                     NULL)) {
...
int
hs_intro_received_introduce1(or_circuit_t *circ, const uint8_t *request, size_t request_len)
{
    int ret;
