                       ...
                       unsigned int cell_type, const void *cell)
{
    ...
    default:
        /* Getting here is really bad as it means we got a unknown cell type
         * from this file where every call has an hardcoded value. */
                            ...
                            size_t circuit_key_material_len)
{
    ...
    if (trn_cell_establish_intro_getlen_auth_key(cell) != ED25519_PUBKEY_LEN ||
        trn_cell_establish_intro_get_auth_key_len(cell) !=
            ED25519_PUBKEY_LEN) {
    ...
    /* Verify the sig */
    {
        ...
        /* Make sure the signature length is of the right size. For EXTRA
         * safety, we check both the size of the array and the length which
         * must be the same. Safety first!*/
        if (trn_cell_establish_intro_getlen_sig(cell) !=
                sizeof(sig_struct.sig) ||
            trn_cell_establish_intro_get_sig_len(cell) !=
                sizeof(sig_struct.sig)) {
            log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
        ...
        int sig_mismatch =
            ed25519_checksig_prefixed(&sig_struct, msg, sig_msg_len,
...
hs_intro_send_intro_established_cell, (or_circuit_t * circ))
{
    ...
    ret = relay_send_command_from_edge(
        0, TO_CIRCUIT(circ), RELAY_COMMAND_INTRO_ESTABLISHED,
                                        ...
                                        uint64_t intro2_burst_per_sec)
{
    ...
    /* Check that received value is not below the minimum. Don't check if
       minimum is set to 0, since the param is a positive value and gcc will
       complain. */
    ...
    /* In a rate limiting scenario, burst can never be smaller than the rate.
     * At best it can be equal. */
    ...
    const trn_cell_extension_field_t *field, or_circuit_t *circ)
{
    ...
    for (size_t i = 0; i < trn_cell_extension_dos_get_n_params(dos); i++) {
        ...
        case TRUNNEL_DOS_PARAM_TYPE_INTRO2_RATE_PER_SEC:
            intro2_rate_per_sec =
                trn_cell_extension_dos_param_get_value(param);
            break;
    ...
    /* A value of 0 is valid in the sense that we accept it but we still
     * disable the defenses so return false. */
    ...
    or_circuit_t *circ, const trn_cell_establish_intro_t *parsed_cell)
{
    ...
    /* Setup INTRODUCE2 defenses on the circuit. Must be done before parsing
     * the cell extension that can possibly change the defenses' values. */
...
send_introduce_ack_cell(or_circuit_t *circ, uint16_t status)
{
    ...
    encoded_cell = tor_malloc_zero(encoded_len);
    result_len =
        trn_cell_introduce_ack_encode(encoded_cell, encoded_len, cell);
    tor_assert(encoded_len == result_len);

    ret = relay_send_command_from_edge(
        CONTROL_CELL_ID, TO_CIRCUIT(circ), RELAY_COMMAND_INTRODUCE_ACK,
        (char *)encoded_cell, encoded_len, NULL);
    /* On failure, the above function will close the circuit. */
...
validate_introduce1_parsed_cell(const trn_cell_introduce1_t *cell)
{
    ...
    /* The auth key of an INTRODUCE1 should be of type ed25519 thus leading to
     * a known fixed length as well. */
                  ...
                  size_t request_len)
{
    ...
    {
        ed25519_public_key_t auth_key;
        get_auth_key_from_cell(&auth_key, RELAY_COMMAND_INTRODUCE1,
                               parsed_cell);
                             ...
                             size_t request_len)
{
    ...
    if (!circuit_is_suitable_for_introduce1(circ)) {
        /* We do not send a NACK because the circuit is not suitable for any
         * kind of response or transmission as it's a violation of the
         * protocol. */
        goto err;
    }
