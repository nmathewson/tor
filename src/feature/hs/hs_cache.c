...
cache_lookup_v3_as_dir(const char *query, const char **desc_out)
{
    ...
    if (ed25519_public_from_base64(&blinded_key, query) < 0) {
        log_info(LD_REND, "Unable to decode the v3 HSDir query %s.", safe_str_client(query));
        goto err;
    }
...
cache_clean_v3_as_dir(time_t now, time_t global_cutoff)
{
    ...
    DIGEST256MAP_FOREACH_MODIFY (hs_cache_v3_dir, key, hs_cache_dir_descriptor_t *, entry) {
        size_t entry_size;
 ...
 * untouched. */
int
hs_cache_lookup_as_dir(uint32_t version, const char *query, const char **desc_out)
{
    int found;
...
static hs_cache_client_descriptor_t *
cache_client_desc_new(const char *desc_str, const ed25519_public_key_t *service_identity_pk,
...
#define cache_client_intro_state_free(val) \
    FREE_AND_NULL(hs_cache_client_intro_state_t, cache_client_intro_state_free_, (val))
...
static void
cache_client_intro_state_note(hs_cache_intro_state_t *state, rend_intro_point_failure_t failure)
{
    tor_assert(state);
    switch (failure) {
...
cache_client_intro_state_add(const ed25519_public_key_t *service_pk,
                             const ed25519_public_key_t *auth_key, hs_cache_intro_state_t **state)
{
...
static void
cache_client_intro_state_clean(time_t cutoff, hs_cache_client_intro_state_t *cache)
{
    tor_assert(cache);

    DIGEST256MAP_FOREACH_MODIFY (cache->intro_points, key, hs_cache_intro_state_t *, entry) {
        if (entry->created_ts <= cutoff) {
 ...
 * expired. */
static int
cached_client_descriptor_has_expired(time_t now, const hs_cache_client_descriptor_t *cached_desc)
{
...
cache_clean_v3_as_client(time_t now)
{
    ...
    DIGEST256MAP_FOREACH_MODIFY (hs_cache_v3_client, key, hs_cache_client_descriptor_t *, entry) {
        size_t entry_size;
...
hs_cache_store_as_client(const char *desc_str, const ed25519_public_key_t *identity_pk)
{
    hs_desc_decode_status_t ret;
    hs_cache_client_descriptor_t *client_desc = NULL;
...
hs_cache_client_intro_state_clean(time_t now)
{
    ...
    DIGEST256MAP_FOREACH_MODIFY (hs_cache_client_intro_state, key, hs_cache_client_intro_state_t *,
                                 cache) {
...
hs_cache_client_new_auth_parse(const ed25519_public_key_t *service_pk)
{
    ...
    if (hs_client_decode_descriptor(cached_desc->encoded_desc, service_pk, &cached_desc->desc) ==
        HS_DESC_DECODE_OK) {
        ret = true;
    }
...
{
    return (unsigned)networkstatus_get_param(NULL, "HSV3MaxDescriptorSize", HS_DESC_MAX_LEN, 1,
                                             INT32_MAX);
}
...
hs_cache_free_all(void)
{
    ...
    digest256map_free(hs_cache_client_intro_state, cache_client_intro_state_free_void);
