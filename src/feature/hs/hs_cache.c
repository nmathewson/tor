...
{
    return (sizeof(*entry) +
            hs_desc_plaintext_obj_size(entry->plaintext_data) +
...
hs_cache_store_as_dir(const char *desc)
{
    ...
    /* Create a new cache object. This can fail if the descriptor plaintext
     * data
...
hs_cache_clean_as_dir(time_t now)
{
    ...
    /* Now, clean the v3 cache. Set the cutoff to 0 telling the cleanup
     * function to compute the cutoff by itself using the lifetime value. */
                      ...
                      hs_desc_decode_status_t *decode_status_out)
{
    ...
    if (ret == HS_DESC_DECODE_OK) {
        ...
        if (BUG(desc != NULL)) {
            /* We are not suppose to have a descriptor if the decoding code is
             * not indicating success. Just in case, bail early to recover. */
            goto end;
        }
    }
    ...
    /* Set expiration time for this cached descriptor to be the start of the
     * next time period since that's when clients need to start using the next
     * blinded pk of the service (and hence will need its next descriptor). */
                             ...
                             hs_cache_intro_state_t **state)
{
    ...
    if (cache == NULL) {
        ...
        digest256map_set(hs_cache_client_intro_state, service_pk->pubkey,
                         cache);
    }
...
cache_store_as_client(hs_cache_client_descriptor_t *client_desc)
{
    ...
    if (cache_entry != NULL) {
        /* If the current or the new cache entry don't have a decrypted
         * descriptor (missing client authorization), we always replace the
         * current one with the new one. Reason is that we can't inspect the
         * revision counter within the plaintext data so we blindly replace. */
        ...
        /* We just removed an old descriptor and will replace it. We'll close
         * all intro circuits related to this old one so we don't have
         * leftovers. We leave the rendezvous circuits opened because they
         * could be in use. */
    ...
    time_t now, const hs_cache_client_descriptor_t *cached_desc)
{
     ...
     * protocol as well (e.g. to build the blinded key and compute time
     * periods). */
    const networkstatus_t *ns = networkstatus_get_live_consensus(now);
    /* If we don't have a recent consensus, consider this entry expired since
     * we will want to fetch a new HS desc when we get a live consensus. */
    if (!ns) {
        return 1;
    }
...
cache_clean_v3_as_client(time_t now)
{
    ...
    DIGEST256MAP_FOREACH_MODIFY (hs_cache_v3_client, key,
        ...
        /* We just removed an old descriptor. We need to close all intro
         * circuits if the descriptor is decrypted so we don't have leftovers
         * that can be selected while lacking a descriptor. Circuits are
         * selected by intro authentication key thus we need the descriptor. We
        ...
        /* Update our OOM. We didn't use the remove() function because we are
         * in a loop so we have to explicitly decrement. */
        rend_cache_decrement_allocation(entry_size);
        /* Logging. */
        {
            char key_b64[BASE64_DIGEST256_LEN + 1];
                         ...
                         const ed25519_public_key_t *identity_pk)
{
    ...
    if (!client_desc) {
        ...
        log_debug(LD_GENERAL, "Failed to parse HSDesc: %s.",
                  escaped(desc_str));
        goto err;
    }
