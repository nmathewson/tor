...
cache_store_as_client(hs_cache_client_descriptor_t *client_desc)
{
    ...
    if (cache_entry != NULL) {
        /* Signalling an undecrypted descriptor. We'll always replace the one
         * we have with the new one just fetched. */
...
cache_clean_v3_as_client(time_t now)
{
    ...
    DIGEST256MAP_FOREACH_MODIFY (hs_cache_v3_client, key,
         ...
         * circuits so we don't have leftovers that can be selected while
         * lacking a descriptor. We leave the rendezvous circuits opened
         * because they could be in use. */
