...
typedef struct hs_service_intro_point_t {
    ...
    /** Legacy key if that intro point doesn't support v3. This should be used
     * if the base object legacy flag is set. */
     ...
     * incremented every time we do a circuit relaunch on this intro point
     * which is triggered when the circuit dies but the node is still in the
     * consensus. After MAX_INTRO_POINT_CIRCUIT_RETRIES, we give up on it. */
    uint32_t circuit_retries;

    /** Replay cache recording the encrypted part of an INTRODUCE2 cell that
     * the circuit associated with this intro point has received. This is used
...
typedef struct hs_service_intropoints_t {
    ...
    /** Contains node's identity key digest that were introduction point for
     * this descriptor but were retried to many times. We keep those so we
     * avoid re-picking them over and over for a circuit retry period.
...
typedef struct hs_service_keys_t {
    ...
    /** True iff the key is kept offline which means the identity_sk MUST not
...
typedef struct hs_service_config_t {
    ...
    /** Path on the filesystem where the service persistent data is stored.
     * NULL
    ...
    /** List of hs_service_authorized_client_t's of clients that may access
     * this service. Specified by HiddenServiceAuthorizeClient option. */
    smartlist_t *clients;
    ...
    /** If true, allow group read permissions on the directory_path. Specified
     * by HiddenServiceDirGroupReadable option. */
    ...
    /** If set, contains the Onion Balance master ed25519 public key (taken
     * from an .onion addresses) that this tor instance serves as backend. */
...
typedef struct hs_service_state_t {
    ...
    /** Replay cache tracking the REND_COOKIE found in INTRODUCE2 cell to
     * detect repeats. Clients may send INTRODUCE1 cells for the same
     * rendezvous point through two or more different introduction points; when
     * they do, this keeps us from launching multiple simultaneous attempts to
     * connect to the same rend point. */
    replaycache_t *replay_cache_rend_cookie;

    /** When is the next time we should rotate our descriptors. This is has to
     * be done at the start time of the next SRV protocol run. */
...
#    define service_intro_point_free(ip)                                   \
        FREE_AND_NULL(hs_service_intro_point_t, service_intro_point_free_, \
                      (ip))
