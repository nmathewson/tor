    ...
    for (int i = 0; optlist[i]; i++) {
        ...
        for (line = line_; line; line = line->next) {
            if (!strcasecmp(line->key, SECTION_HEADER)) {
                 ...
                 * (This shouldn't be possible, now that we have partitioned
...
config_service_v3(const hs_opts_t *hs_opts, hs_service_config_t *config)
{
    ...
    if (hs_opts->HiddenServiceExportCircuitID) {
        ...
        config->circuit_id_protocol = helper_parse_circuit_id_protocol(
            ...
            hs_opts->HiddenServiceExportCircuitID, &ok);
        if (!ok) {
            goto err;
        }
    }
               ...
               smartlist_t *service_list)
{
    ...
    if (ret < 0) {
        log_warn(LD_REND, "Can't parse configuration for onion service: %s",
                 msg);
        goto err;
    }
