        ...
        for (line = line_; line; line = line->next) {
            ...
            if (!strcasecmp(line->key, opt)) {
                ...
                if (!strcasecmp(line->key, "HiddenServiceAuthorizeClient")) {
                     ...
                     * alternatives. If more such special cases appear, it would
                     * be good to generalize the exception mechanism here. */
                    log_warn(
                        LD_CONFIG,
    ...
    for (line = line_; line; line = line->next) {
        ...
        if (!strcasecmp(line->key, "HiddenServiceEnableIntroDoSDefense")) {
            config->has_dos_defense_enabled = (unsigned int)helper_parse_uint64(
                line->key, line->value, HS_CONFIG_V3_DOS_DEFENSE_DEFAULT, 1,
                &ok);
            if (!ok || dos_enabled) {
                if (dos_enabled) {
                    dup_opt_seen = line->key;
...
hs_config_service_all(const or_options_t *options, int validate_only)
{
    ...
    if (!validate_only) {
         ...
         * purge the staging list so it's not kept in memory as valid service.
         */
