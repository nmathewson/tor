...
stage_services(smartlist_t *service_list)
{
     ...
     * only >= v3 service. And remember, v2 has a different object type which
     * is shadow copied from an hs_service_t type. */
                             ...
                             const hs_service_t *service)
{
    ...
    SMARTLIST_FOREACH_BEGIN (service_list, const hs_service_t *, s) {
        if (!strcmp(s->config.directory_path,
                    ...
                    uint64_t max, int *ok)
{
    ...
    *ok = 0;
    ...
    if (!*ok) {
        log_warn(LD_CONFIG,
                 "%s must be between %" PRIu64 " and %" PRIu64 ", not %s.",
                 opt, min, max, value);
        goto err;
    }
        ...
        for (line = line_; line; line = line->next) {
            ...
            if (!strcasecmp(line->key, opt)) {
                ...
                if (!strcasecmp(line->key, "HiddenServiceAuthorizeClient")) {
                    /* Special case this v2 option so that we can offer
                     * alternatives. If more such special cases appear, it
                     * would be good to generalize the exception mechanism
                     * here. */
                    log_warn(LD_CONFIG,
                             "For v3 onion service client authorization, "
                             "please read the 'CLIENT AUTHORIZATION' section "
                             "in the "
                             "manual.");
                }
    ...
    for (line = line_; line; line = line->next) {
        ...
        if (!strcasecmp(line->key, "HiddenServiceEnableIntroDoSDefense")) {
            config->has_dos_defense_enabled =
                (unsigned int)helper_parse_uint64(
                    line->key, line->value, HS_CONFIG_V3_DOS_DEFENSE_DEFAULT,
                    1, &ok);
            if (!ok || dos_enabled) {
                if (dos_enabled) {
                    dup_opt_seen = line->key;
                       ...
                       hs_service_t *service)
{
    ...
    /* The first line starts with HiddenServiceDir so we consider what's next
     * is the configuration of the service. */
    for (line = line_; line; line = line->next) {
        int ok = 0;
        ...
        if (!strcasecmp(line->key, "HiddenServiceAllowUnknownPorts")) {
            config->allow_unknown_ports = (unsigned int)helper_parse_uint64(
                line->key, line->value, 0, 1, &ok);
        ...
        if (!strcasecmp(line->key, "HiddenServiceDirGroupReadable")) {
            config->dir_group_readable = (unsigned int)helper_parse_uint64(
                line->key, line->value, 0, 1, &ok);
        ...
        if (!strcasecmp(line->key, "HiddenServiceMaxStreams")) {
            config->max_streams_per_rdv_circuit = helper_parse_uint64(
               ...
               smartlist_t *service_list)
{
    ...
    /* We make sure that this set of options for a service are valid that is
     * for instance an option only for v2 is not used for v3. */
...
hs_config_service_all(const or_options_t *options, int validate_only)
{
    ...
    for (line = options->RendConfigLines; line; line = line->next) {
        ...
        /* Try to configure this service now. On success, it will be added to
         * the list and validated against the service in that same list. */
    ...
    if (!validate_only) {
        ...
        /* We've just validated that we were able to build a clean working list
         * of services. We don't need those objects anymore. */
         ...
         * through the prunning process. In validation mode, we thus have to
         * purge
