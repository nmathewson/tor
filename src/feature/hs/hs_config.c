...
static int
service_is_duplicate_in_list(const smartlist_t *service_list, const hs_service_t *service)
{
    int ret = 0;
...
{
    if (i < low || i > high) {
        log_warn(LD_CONFIG, "%s must be between %d and %d, not %d.", name, low, high, i);
        return true;
    }
    return false;
}
...
static int
config_has_invalid_options(const config_line_t *line_, const hs_service_t *service)
{
    int ret = 0;
    const char **optlist;
    ...
    const char *opts_exclude_v2[] = {
        ...
        "HiddenServiceEnableIntroDoSRatePerSec", "HiddenServiceEnableIntroDoSBurstPerSec",
        "HiddenServiceOnionBalanceInstance",     NULL /* End marker. */
    };
...
config_service_v3(const hs_opts_t *hs_opts, hs_service_config_t *config)
{
    ...
    if (hs_opts->HiddenServiceExportCircuitID) {
        ...
        config->circuit_id_protocol = helper_parse_circuit_id_protocol(
            "HiddenServcieExportCircuitID", hs_opts->HiddenServiceExportCircuitID, &ok);
        if (!ok) {
            goto err;
        }
    }
    ...
    config->has_dos_defense_enabled = hs_opts->HiddenServiceEnableIntroDoSDefense;
    ...
    config->intro_dos_rate_per_sec = hs_opts->HiddenServiceEnableIntroDoSRatePerSec;
    ...
    config->intro_dos_burst_per_sec = hs_opts->HiddenServiceEnableIntroDoSBurstPerSec;
...
static int
config_generic_service(const hs_opts_t *hs_opts, const or_options_t *options, hs_service_t *service)
{
    ...
    } else if (CHECK_OOB(hs_opts, HiddenServiceVersion, HS_VERSION_MIN, HS_VERSION_MAX)) {
        goto err;
    } else {
    ...
    if (CHECK_OOB(hs_opts, HiddenServiceMaxStreams, 0, HS_CONFIG_MAX_STREAMS_PER_RDV_CIRCUIT)) {
        goto err;
    }
    config->max_streams_per_rdv_circuit = hs_opts->HiddenServiceMaxStreams;

    /* Maximum amount of streams before we close the circuit. */
    config->max_streams_close_circuit = hs_opts->HiddenServiceMaxStreamsCloseCircuit;
...
static int
config_service(config_line_t *line, const or_options_t *options, smartlist_t *service_list)
{
    int ret;
    ...
    validation_status_t vs = config_validate(get_hs_opts_mgr(), NULL, hs_opts, &msg);
    if (vs < 0) {
    ...
    if (hs_check_service_private_dir(options->User, service->config.directory_path,
...
hs_config_service_all(const or_options_t *options, int validate_only)
{
    ...
    if (options->RendConfigLines && strcasecmp(options->RendConfigLines->key, SECTION_HEADER)) {
        log_warn(LD_CONFIG, "%s with no preceding %s directive", options->RendConfigLines->key,
                 SECTION_HEADER);
        goto err;
    }
