...
void
hs_control_desc_event_requested(const ed25519_public_key_t *onion_pk, const char *base64_blinded_pk,
                                const routerstatus_t *hsdir_rs)
{
    ...
    /* Trigger the event. */
    control_event_hs_descriptor_requested(onion_address, REND_NO_AUTH, hsdir_rs->identity_digest,
                                          base64_blinded_pk,
...
void
hs_control_desc_event_failed(const hs_ident_dir_conn_t *ident, const char *hsdir_id_digest,
                             const char *reason)
{
    char onion_address[HS_SERVICE_ADDR_LEN_BASE32 + 1];
    ...
    control_event_hsv3_descriptor_failed(onion_address, base64_blinded_pk, hsdir_id_digest, reason);
}
 ...
 * None can be NULL. */
void
hs_control_desc_event_received(const hs_ident_dir_conn_t *ident, const char *hsdir_id_digest)
{
    char onion_address[HS_SERVICE_ADDR_LEN_BASE32 + 1];
    char base64_blinded_pk[ED25519_BASE64_LEN + 1];
    ...
    control_event_hsv3_descriptor_received(onion_address, base64_blinded_pk, hsdir_id_digest);
}
...
void
hs_control_desc_event_created(const char *onion_address, const ed25519_public_key_t *blinded_pk)
{
    char base64_blinded_pk[ED25519_BASE64_LEN + 1];
...
void
hs_control_desc_event_upload(const char *onion_address, const char *hsdir_id_digest,
                             const ed25519_public_key_t *blinded_pk, const uint8_t *hsdir_index)
{
    char base64_blinded_pk[ED25519_BASE64_LEN + 1];
    ...
    control_event_hs_descriptor_upload(onion_address, hsdir_id_digest, base64_blinded_pk,
                                       hex_str((const char *)hsdir_index, DIGEST256_LEN));
}
 ...
 * None can be NULL. */
void
hs_control_desc_event_uploaded(const hs_ident_dir_conn_t *ident, const char *hsdir_id_digest)
{
    char onion_address[HS_SERVICE_ADDR_LEN_BASE32 + 1];
 ...
 * can be NULL. */
void
hs_control_desc_event_content(const hs_ident_dir_conn_t *ident, const char *hsdir_id_digest,
                              const char *body)
{
    char onion_address[HS_SERVICE_ADDR_LEN_BASE32 + 1];
    char base64_blinded_pk[ED25519_BASE64_LEN + 1];
    ...
    control_event_hs_descriptor_content(onion_address, base64_blinded_pk, hsdir_id_digest, body);
}
 ...
 * on success. */
int
hs_control_hspost_command(const char *body, const char *onion_address, const smartlist_t *hsdirs_rs)
{
    int ret = -1;
    ...
    if (hsdirs_rs == NULL) {
        ...
        hs_get_responsible_hsdirs(&plaintext.blinded_pubkey, hs_get_time_period_num(0),
 ...
 */
void
hs_control_hsfetch_command(const ed25519_public_key_t *onion_identity_pk, const smartlist_t *hsdirs)
{
