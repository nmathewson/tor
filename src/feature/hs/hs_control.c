                                ...
                                const routerstatus_t *hsdir_rs)
{
    ...
    /* Get the node from the routerstatus object to get the HSDir index used
     * for this request. We can't have a routerstatus entry without a node and
     * we can't pick a node without an hsdir_index. */
                              ...
                              const ed25519_public_key_t *blinded_pk)
{
    ...
    /* Version 3 doesn't use the replica number in its descriptor ID
     * computation
                          ...
                          const smartlist_t *hsdirs_rs)
{
    ...
    if (hsdirs_rs == NULL) {
        ...
        hs_get_responsible_hsdirs(&plaintext.blinded_pubkey,
                                  ...
                                  0, /* Always the current descriptor which
                                      * uses the first hsdir index. */
