...
circuit_purpose_is_correct_for_rend(unsigned int circ_purpose, int is_service_side)
{
    if (is_service_side) {
        if (circ_purpose != CIRCUIT_PURPOSE_S_CONNECT_REND) {
            log_warn(LD_BUG, "HS e2e circuit setup with wrong purpose (%d)", circ_purpose);
            return 0;
        }
    }
    ...
    if (!is_service_side) {
        if (circ_purpose != CIRCUIT_PURPOSE_C_REND_READY &&
            ...
            log_warn(LD_BUG, "Client e2e circuit setup with wrong purpose (%d)", circ_purpose);
            return 0;
        }
    }
...
static crypt_path_t *
create_rend_cpath(const uint8_t *ntor_key_seed, size_t seed_len, int is_service_side)
{
    uint8_t keys[HS_NTOR_KEY_EXPANSION_KDF_OUT_LEN];
    crypt_path_t *cpath = NULL;

    /* Do the key expansion */
    if (hs_ntor_circuit_key_expansion(ntor_key_seed, seed_len, keys, sizeof(keys)) < 0) {
        goto err;
    }
    ...
    if (cpath_init_circuit_crypto(cpath, (char *)keys, sizeof(keys), is_service_side, 1) < 0) {
        tor_free(cpath);
        goto err;
    }
...
create_rend_cpath_legacy(origin_circuit_t *circ, const uint8_t *rend_cell_body)
{
    ...
    /* ... and set up cpath. */
    if (cpath_init_circuit_crypto(hop, keys + DIGEST_LEN, sizeof(keys) - DIGEST_LEN, 0, 0) < 0)
        goto err;
...
static void
finalize_rend_circuit(origin_circuit_t *circ, crypt_path_t *hop, int is_service_side)
{
    tor_assert(circ);
    tor_assert(hop);
    ...
    int new_circ_purpose =
        is_service_side ? CIRCUIT_PURPOSE_S_REND_JOINED : CIRCUIT_PURPOSE_C_REND_JOINED;
...
register_intro_circ(const hs_service_intro_point_t *ip, origin_circuit_t *circ)
{
    ...
    if (ip->base.is_only_legacy) {
        hs_circuitmap_register_intro_circ_v2_service_side(circ, ip->legacy_key_digest);
    } else {
                                       ...
                                       const hs_service_descriptor_t *desc)
{
    ...
    DIGEST256MAP_FOREACH (desc->intro_points.map, key, const hs_service_intro_point_t *, ip) {
        ...
        tor_assert(ed25519_pubkey_eq(&service->keys.identity_pk, &ocirc->hs_ident->identity_pk));
                             ...
                             const hs_ntor_rend_cell_keys_t *keys)
{
    ...
    memcpy(ident->rendezvous_cookie, rendezvous_cookie, sizeof(ident->rendezvous_cookie));
    ...
    memcpy(handshake_info + CURVE25519_PUBKEY_LEN, keys->rend_cell_auth_mac, DIGEST256_LEN);
    tor_assert(sizeof(ident->rendezvous_handshake_info) == sizeof(handshake_info));
    ...
    tor_assert(sizeof(ident->rendezvous_ntor_key_seed) == sizeof(keys->ntor_key_seed));
...
static hs_ident_circuit_t *
create_intro_circuit_identifier(const hs_service_t *service, const hs_service_intro_point_t *ip)
{
    hs_ident_circuit_t *ident;
                     ...
                     origin_circuit_t *circ)
{
    ...
    cell_len = hs_cell_build_establish_intro(circ->cpath->prev->rend_circ_nonce, &service->config,
                                             ip, payload);
    if (cell_len < 0) {
        log_warn(LD_REND,
                 "Unable to encode ESTABLISH_INTRO cell for service %s "
                 "on circuit %u. Closing circuit.",
                 safe_str_client(service->onion_address), TO_CIRCUIT(circ)->n_circ_id);
        goto err;
    }

    /* Send the cell on the circuit. */
    if (relay_send_command_from_edge(CONTROL_CELL_ID, TO_CIRCUIT(circ),
                                     RELAY_COMMAND_ESTABLISH_INTRO, (char *)payload, cell_len,
                                     circ->cpath->prev) < 0) {
        log_info(LD_REND,
           ...
           const hs_cell_introduce2_data_t *data))
{
    ...
    if (info == NULL) {
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_REND,
               ...
               "%s service %s.",
               get_service_anonymity_string(service), safe_str_client(service->onion_address));
        goto end;
    }
    ...
    for (int i = 0; i < MAX_REND_FAILURES; i++) {
        ...
        circ = circuit_launch_by_extend_info(CIRCUIT_PURPOSE_S_CONNECT_REND, info, circ_flags);
        if (circ != NULL) {
    ...
    if (circ == NULL) {
        log_warn(LD_REND,
                 ...
                 "for %s service %s",
                 safe_str_client(extend_info_describe(info)), get_service_anonymity_string(service),
    ...
    log_info(LD_REND,
             ...
             safe_str_client(hex_str((const char *)data->rendezvous_cookie, REND_COOKIE_LEN)),
             get_service_anonymity_string(service), safe_str_client(service->onion_address));
    tor_assert(circ->build_state);
    ...
    /* Create circuit identifier and key material. */
    {
        ...
        if (hs_ntor_service_get_rendezvous1_keys(&ip->auth_key_kp.pubkey, &ip->enc_key_kp,
                                                 &ephemeral_kp, &data->client_pk, &keys) < 0) {
        ...
        circ->hs_ident = create_rp_circuit_identifier(service, data->rendezvous_cookie,
...
can_relaunch_service_rendezvous_point(const origin_circuit_t *circ)
{
    ...
    if (circ->build_state->failure_count > max_rend_failures ||
        ...
        log_info(LD_REND,
                 ...
                 "Giving up building.",
                 safe_str_client(extend_info_describe(circ->build_state->chosen_exit)),
                 circ->build_state->failure_count, (long int)circ->build_state->expiry_time);
        goto disallow;
    }
...
retry_service_rendezvous_point(const origin_circuit_t *circ)
{
    ...
    new_circ =
        circuit_launch_by_extend_info(CIRCUIT_PURPOSE_S_CONNECT_REND, bstate->chosen_exit, flags);
    if (new_circ == NULL) {
...
hs_circ_service_get_established_intro_circ(const hs_service_intro_point_t *ip)
{
    ...
    return (circ && TO_CIRCUIT(circ)->purpose == CIRCUIT_PURPOSE_S_INTRO) ? circ : NULL;
}
...
int
hs_circ_launch_intro_point(hs_service_t *service, const hs_service_intro_point_t *ip,
                           extend_info_t *ei, bool direct_conn)
{
    ...
    log_info(LD_REND, "Launching a circuit to intro point %s for service %s.",
             safe_str_client(extend_info_describe(ei)), safe_str_client(service->onion_address));
    ...
    circ = circuit_launch_by_extend_info(CIRCUIT_PURPOSE_S_ESTABLISH_INTRO, ei, circ_flags);
    if (circ == NULL) {
        goto end;
    }
 ...
 * opened. */
int
hs_circ_service_intro_has_opened(hs_service_t *service, hs_service_intro_point_t *ip,
...
void
hs_circ_service_rp_has_opened(const hs_service_t *service, origin_circuit_t *circ)
{
    size_t payload_len;
    ...
    payload_len = hs_cell_build_rendezvous1(
        circ->hs_ident->rendezvous_cookie, sizeof(circ->hs_ident->rendezvous_cookie),
    ...
    if (payload_len < HS_LEGACY_RENDEZVOUS_CELL_SIZE) {
        crypto_rand((char *)payload + payload_len, HS_LEGACY_RENDEZVOUS_CELL_SIZE - payload_len);
        payload_len = HS_LEGACY_RENDEZVOUS_CELL_SIZE;
    }

    if (relay_send_command_from_edge(CONTROL_CELL_ID, TO_CIRCUIT(circ), RELAY_COMMAND_RENDEZVOUS1,
    ...
    if (hs_circuit_setup_e2e_rend_circ(circ, circ->hs_ident->rendezvous_ntor_key_seed,
...
int
hs_circ_handle_intro_established(const hs_service_t *service, const hs_service_intro_point_t *ip,
                                 origin_circuit_t *circ, const uint8_t *payload, size_t payload_len)
{
    ...
    if (!ip->base.is_only_legacy && hs_cell_parse_intro_established(payload, payload_len) < 0) {
        log_warn(LD_REND,
...
int
hs_circ_handle_introduce2(const hs_service_t *service, const origin_circuit_t *circ,
                          hs_service_intro_point_t *ip, const hs_subcredential_t *subcredential,
                          const uint8_t *payload, size_t payload_len)
{
    ...
    if (get_subcredential_for_handling_intro2_cell(service, &data, subcredential)) {
        goto done;
    }
    ...
    if (replaycache_add_test_and_elapsed(service->state.replay_cache_rend_cookie,
...
int
hs_circuit_setup_e2e_rend_circ(origin_circuit_t *circ, const uint8_t *ntor_key_seed,
                               size_t seed_len, int is_service_side)
{
    if (BUG(!circuit_purpose_is_correct_for_rend(TO_CIRCUIT(circ)->purpose, is_service_side))) {
        return -1;
    }

    crypt_path_t *hop = create_rend_cpath(ntor_key_seed, seed_len, is_service_side);
    if (!hop) {
 ...
 * other side. */
int
hs_circuit_setup_e2e_rend_circ_legacy_client(origin_circuit_t *circ, const uint8_t *rend_cell_body)
{
    if (BUG(!circuit_purpose_is_correct_for_rend(TO_CIRCUIT(circ)->purpose, 0))) {
        return -1;
    }
...
int
hs_circ_send_introduce1(origin_circuit_t *intro_circ, origin_circuit_t *rend_circ,
                        const hs_desc_intro_point_t *ip, const hs_subcredential_t *subcredential)
{
    int ret = -1;
    ssize_t payload_len;
    uint8_t payload[RELAY_PAYLOAD_SIZE] = {0};
    ...
    memcpy(intro_circ->hs_ident->rendezvous_cookie, rend_circ->hs_ident->rendezvous_cookie,
    ...
    if (relay_send_command_from_edge(CONTROL_CELL_ID, TO_CIRCUIT(intro_circ),
                                     RELAY_COMMAND_INTRODUCE1, (const char *)payload, payload_len,
...
hs_circ_send_establish_rendezvous(origin_circuit_t *circ)
{
    ...
    cell_len = hs_cell_build_establish_rendezvous(circ->hs_ident->rendezvous_cookie, cell);
    if (BUG(cell_len < 0)) {
        goto err;
    }

    if (relay_send_command_from_edge(CONTROL_CELL_ID, TO_CIRCUIT(circ),
                                     RELAY_COMMAND_ESTABLISH_RENDEZVOUS, (const char *)cell,
                                     cell_len, circ->cpath->prev) < 0) {
