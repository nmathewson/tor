...
hs_circ_service_get_established_intro_circ(const hs_service_intro_point_t *ip)
{
    ...
    if (ip->base.is_only_legacy) {
        circ =
            hs_circuitmap_get_intro_circ_v2_service_side(ip->legacy_key_digest);
    } else {
                                 ...
                                 origin_circuit_t *circ)
{
    ...
    if (num_intro_circ > num_needed_circ) {
         ...
         * added here. I can only assume in case our ExcludeNodes list changes
         * but in that case, all circuit are flagged unusable (config.c).
         * --dgoulet */
