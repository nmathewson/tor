                                    ...
                                    int is_service_side)
{
    ...
    if (!is_service_side) {
        if (circ_purpose != CIRCUIT_PURPOSE_C_REND_READY &&
            ...
            log_warn(LD_BUG,
                     "Client e2e circuit setup with wrong purpose (%d)",
                     circ_purpose);
            return 0;
        }
    }
                     ...
                     origin_circuit_t *circ)
{
    ...
    cell_len = hs_cell_build_establish_intro(
        circ->cpath->prev->rend_circ_nonce, &service->config, ip, payload);
    if (cell_len < 0) {
        log_warn(LD_REND,
           ...
           const hs_cell_introduce2_data_t *data))
{
    ...
    info =
        hs_get_extend_info_from_lspecs(data->link_specifiers, &data->onion_pk,
    ...
    for (int i = 0; i < MAX_REND_FAILURES; i++) {
        ...
        circ = circuit_launch_by_extend_info(CIRCUIT_PURPOSE_S_CONNECT_REND,
                                             info, circ_flags);
        if (circ != NULL) {
    ...
    /* Create circuit identifier and key material. */
    {
        ...
        /* No need for extra strong, this is only for this circuit life time.
         * This key will be used for the RENDEZVOUS1 cell that will be sent on
         * the circuit once opened. */
        curve25519_keypair_generate(&ephemeral_kp, 0);
        if (hs_ntor_service_get_rendezvous1_keys(
                &ip->auth_key_kp.pubkey, &ip->enc_key_kp, &ephemeral_kp,
...
can_relaunch_service_rendezvous_point(const origin_circuit_t *circ)
{
    ...
    /* We check failure_count >= hs_get_service_max_rend_failures()-1 below,
     * and the -1 is because we increment the failure count for our current
     * failure *after* this clause. */
    int max_rend_failures = hs_get_service_max_rend_failures() - 1;

    /* A failure count that has reached maximum allowed or circuit that
     * expired, we skip relaunching. */
...
cleanup_on_close_client_circ(circuit_t *circ)
{
    ...
    /* It is possible the circuit has an HS purpose but no identifier
     * (rend_data or hs_ident). Thus possible that this passes through. */
}
...
hs_circ_service_get_intro_circ(const hs_service_intro_point_t *ip)
{
    ...
    if (ip->base.is_only_legacy) {
        return hs_circuitmap_get_intro_circ_v2_service_side(
...
hs_circ_service_get_established_intro_circ(const hs_service_intro_point_t *ip)
{
    ...
    if (ip->base.is_only_legacy) {
        circ = hs_circuitmap_get_intro_circ_v2_service_side(
            ip->legacy_key_digest);
    } else {
                           ...
                           extend_info_t *ei, bool direct_conn)
{
     ...
     * be launched, we still want to respect the retry period to avoid stress
     * on the circuit subsystem. */
                                 ...
                                 origin_circuit_t *circ)
{
    ...
    /* Cound opened circuits that have sent ESTABLISH_INTRO cells or are
     * already established introduction circuits */
    ...
    if (num_intro_circ > num_needed_circ) {
        ...
        /* XXX: Legacy code checks options->ExcludeNodes and if not NULL it
         * just closes the circuit. I have NO idea why it does that so it
         * hasn't been added here. I can only assume in case our ExcludeNodes
         * list changes but in that case, all circuit are flagged unusable
         * (config.c). --dgoulet */
        ...
        /* This return value indicate to the caller that the IP object should
         * be removed from the service because it's corresponding circuit has
         * just been repurposed. */
        ret = 1;
        goto done;
    }
    ...
    /* Time to send an ESTABLISH_INTRO cell on this circuit. On error, this
     * call makes sure the circuit gets closed. */
                              ...
                              origin_circuit_t *circ)
{
    ...
    /* Pad the payload with random bytes so it matches the size of a legacy
     * cell which is normally always bigger. Also, the size of a legacy cell is
...
get_subcredential_for_handling_intro2_cell(
    const hs_service_t *service, hs_cell_introduce2_data_t *data,
                          ...
                          const uint8_t *payload, size_t payload_len)
{
    ...
    if (replaycache_add_test_and_elapsed(
        ...
        /* A Tor client will send a new INTRODUCE1 cell with the same
         * REND_COOKIE as its previous one if its intro circ times out while in
         * state CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT. If we received the first
         * INTRODUCE1 cell (the intro-point relay converts it into an
         * INTRODUCE2 cell), we are already trying to connect to that rend
         * point (and may have already succeeded); drop this cell. */
        log_info(LD_REND,
                        ...
                        const hs_subcredential_t *subcredential)
{
    ...
    /* It is undefined behavior in hs_cell_introduce1_data_clear() if
     * intro1_data has been declared on the stack but not initialized. Here, we
     * set it to 0.
     */
    memset(&intro1_data, 0, sizeof(hs_cell_introduce1_data_t));
    ...
    if (setup_introduce1_data(ip, exit_node, subcredential, &intro1_data) <
        0) {
        log_warn(LD_REND,
    ...
    /* Final step before we encode a cell, we setup the circuit identifier
     * which will generate both the rendezvous cookie and client keypair for
     * this connection. Those are put in the ident. */
...
hs_circ_cleanup_on_free(circuit_t *circ)
{
    ...
    /* We have no assurance that the given HS circuit has been closed before
     * and thus removed from the HS map. This actually happens in unit tests.
     */
    if (circ->hs_token) {
        hs_circuitmap_remove_circuit(circ);
    }
...
hs_circ_cleanup_on_repurpose(circuit_t *circ)
{
     ...
     * the on_free actions since we don't treat a repurpose as something we
     * need to report in the client cache failure. */
...
hs_circ_is_rend_sent_in_intro1(const origin_circuit_t *circ)
{
    ...
    if (circ->rend_data) {
        ...
        /* A circuit with an HS purpose without an hs_ident or rend_data in
         * theory can not happen. In case, scream loudly and return false to
         * the caller that the rendezvous was not sent in the INTRO1 cell. */
