                   ...
                   size_t secret_data_len, uint8_t **secret_input_out)
{
    ...
    size_t secret_input_len =
        secret_data_len + DIGEST256_LEN + sizeof(uint64_t);
              ...
              int is_superencrypted_layer)
{
    ...
    if (is_superencrypted_layer) {
        crypto_xof_add_bytes(xof,
                             (const uint8_t *)str_enc_const_superencryption,
...
encode_link_specifiers(const smartlist_t *specs)
{
...

    {
        ...
        ret = base64_encode(encoded_b64, encoded_b64_len,
                            (const char *)encoded, encoded_len, 0);
                ...
                int is_superencrypted_layer)
{
    ...
    /* If we are encrypting the middle layer of the descriptor, we need to
                        ...
                        int is_superencrypted_layer)
{
    ...
    /* KDF construction resulting in a key from which the secret key, IV and
     * MAC key are extracted which is what we need for the encryption. */
...
get_inner_encrypted_layer_plaintext(const hs_descriptor_t *desc)
{
    ...
    /* Build the start of the section prior to the introduction points. */
    {
        ...
        if (desc->encrypted_data.intro_auth_types &&
            ...
            char *buf = smartlist_join_strings(
                desc->encrypted_data.intro_auth_types, " ", 0, NULL);
                           ...
                           char **encrypted_blob_out)
{
     ...
     * We then encrypt it and use it to create the middle layer of the
     * descriptor (layer1).  Finally we superencrypt the middle layer and
               ...
               const uint8_t *descriptor_cookie, char **encoded_out)
{
    ...
    /* Sign all fields of the descriptor with our short term signing key. */
    {
        ...
        if (ed25519_sign_prefixed(&sig, (const uint8_t *)encoded_str,
                                  encoded_len, str_desc_sig_prefix,
                                  signing_kp) < 0) {
...
decode_create2_list(hs_desc_encrypted_data_t *desc, const char *list)
{
    ...
    SMARTLIST_FOREACH_BEGIN (tokens, char *, s) {
        ...
        if (!ok) {
            log_warn(LD_REND, "Unparseable value %s in create2 list",
                     escaped(s));
            continue;
        }
        switch (type) {
...
cert_is_valid(tor_cert_t *cert, uint8_t type, const char *log_obj_type)
{
    ...
    if (cert == NULL) {
        log_warn(LD_REND, "Certificate for %s couldn't be parsed.",
                 log_obj_type);
        goto err;
    }
    ...
    if (tor_cert_checksig(cert, &cert->signing_key, approx_time()) < 0) {
        if (cert->cert_expired) {
            ...
            log_fn(LOG_PROTOCOL_WARN, LD_REND,
                   "Invalid signature for %s: %s (%s)", log_obj_type,
                          ...
                          uint8_t **descriptor_cookie_out)
{
    ...
    tor_assert(
        !fast_mem_is_zero((char *)desc->subcredential.subcred, DIGEST256_LEN));

    /* Catch potential code-flow cases of an unitialized private key sneaking
     * into this function. */
    if (BUG(fast_mem_is_zero((char *)client_auth_sk,
    ...
    keystream_length = build_descriptor_cookie_keys(
        &desc->subcredential, client_auth_sk,
           ...
           bool is_superencrypted_layer, char **decrypted_out))
{
    ...
    /* KDF construction resulting in a key from which the secret key, IV and
     * MAC key are extracted which is what we need for the decryption. */
    build_secret_key_iv_mac(desc, secret_data, secret_data_len, salt,
     ...
     *
     * This is a critical check that is making sure the computed MAC matches
...

    {
        ...
        cipher = crypto_cipher_new_with_iv_and_bits(
            secret_key, secret_iv, HS_DESC_ENCRYPTED_BIT_SIZE);
                       ...
                       char **decrypted_out)
{
    ...
    if (client_auth_sk) {
        SMARTLIST_FOREACH_BEGIN (desc->superencrypted_data.clients,
                                 hs_desc_authorized_client_t *, client) {
            /* If we can decrypt the descriptor cookie successfully, we will
             * use that descriptor cookie and break from the loop. */
    ...
    encrypted_len = decrypt_desc_layer(desc, descriptor_cookie, false,
                                       &encrypted_plaintext);
                          ...
                          const smartlist_t *tokens)
{
    ...
    SMARTLIST_FOREACH_BEGIN (onion_keys, directory_token_t *, tok) {
        ...
        if (!strcmp(tok->args[0], "ntor")) {
            if (curve25519_public_from_base64(onion_key_out, tok->args[1]) <
                0) {
                log_warn(LD_REND,
                    ...
                    hs_desc_encrypted_data_t *desc_enc, const char *data)
{
     ...
     */
    {
        /* Split the descriptor string using the intro point header as
         * delimiter */
        smartlist_split_string(chunked_desc, data, str_intro_point_start, 0,
                               0);

        /* Check if there are actually any intro points included. The first
         * chunk should be other descriptor fields (e.g. create2-formats), so
         * it's not an intro point. */
    ...
    /* Take the intro point substrings, and prepare them for parsing */
    {
        ...
        SMARTLIST_FOREACH_BEGIN (chunked_desc, char *, chunk) {
            ...
            smartlist_add_asprintf(intro_points, "%s %s", str_intro_point,
                                   chunk);
    ...
    SMARTLIST_FOREACH_BEGIN (intro_points, const char *, intro_point) {
        hs_desc_intro_point_t *ip =
            decode_introduction_point(desc, intro_point);
        if (!ip) {
            /* Malformed introduction point section. We'll ignore this
             * introduction point and continue parsing. New or unknown fields
             * are possible for forward compatibility. */
            continue;
        }
                         ...
                         const char *encoded_desc, size_t encoded_len)
{
    ...
    /* Copy the superencrypted blob to the descriptor object so we can handle
     * it latter if needed. */
                         ...
                         hs_desc_encrypted_data_t *desc_encrypted_out)
{
    ...
    if (!message_len) {
         ...
         * configured that didn't work or we do not have any configured for
         * this onion address so likely the descriptor is for authorized client
         * only, we are not. */
        if (client_auth_sk) {
                         ...
                         hs_desc_encrypted_data_t *desc_encrypted)
{
     ...
     * error. The superencrypted parsing should never succeed in the first
     * place without an encrypted section. */
    tor_assert(desc->superencrypted_data.encrypted_blob);
    /* Let's make sure we have a supported version as well. By correctly
     * parsing the plaintext, this should not fail. */
                          ...
                          hs_descriptor_t **desc_out)
{
    ...
    ret =
        hs_desc_decode_encrypted(desc, client_auth_sk, &desc->encrypted_data);
    if (ret != HS_DESC_DECODE_OK) {
        goto err;
    }
    ...
    const uint8_t *descriptor_cookie, hs_desc_authorized_client_t *client_out)
{
    ...
    keystream_length = build_descriptor_cookie_keys(
        subcredential, auth_ephemeral_sk, client_auth_pk, &keystream);
    tor_assert(keystream_length > 0);
