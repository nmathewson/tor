...
build_kdf_key(const hs_descriptor_t *desc, const uint8_t *secret_data,
              ...
              uint8_t *key_out, size_t key_out_len, int is_superencrypted_layer)
{
    uint8_t *secret_input = NULL;
    size_t secret_input_len;
    crypto_xof_t *xof;
...
build_secret_key_iv_mac(const hs_descriptor_t *desc, const uint8_t *secret_data,
                        size_t secret_data_len, const uint8_t *salt,
                        size_t salt_len, uint8_t *key_out, size_t key_len,
                        uint8_t *iv_out, size_t iv_len, uint8_t *mac_out,
                        size_t mac_len, int is_superencrypted_layer)
{
    size_t offset = 0;
...
encrypt_descriptor_data(const hs_descriptor_t *desc, const uint8_t *secret_data,
                        size_t secret_data_len, const char *plaintext,
                           ...
                           char **encrypted_blob_out)
{
     ...
     * descriptor (layer1).  Finally we superencrypt the middle layer and return
     * it to our caller. */
                       ...
                       char **decrypted_out)
{
    ...
    if (client_auth_sk) {
        SMARTLIST_FOREACH_BEGIN(desc->superencrypted_data.clients,
                                hs_desc_authorized_client_t *, client)
        {
            /* If we can decrypt the descriptor cookie successfully, we will use
             * that descriptor cookie and break from the loop. */
...
static int (*decode_encrypted_handlers[])(
 ...
 * indexed by the version number so v3 callback is at index 3 in the array. */
static int (*decode_superencrypted_handlers[])(
    const hs_descriptor_t *desc,
 ...
 * indexed by the version number so v3 callback is at index 3 in the array. */
static int (*decode_plaintext_handlers[])(smartlist_t *tokens,
...
hs_desc_build_authorized_client(
    const uint8_t *subcredential, const curve25519_public_key_t *client_auth_pk,
