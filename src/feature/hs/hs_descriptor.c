...
static token_rule_t hs_desc_encrypted_v3_token_table[] = {
    ...
    T01(str_single_onion, R3_SINGLE_ONION_SERVICE, ARGS, NO_OBJ), END_OF_TABLE};
...
static void
build_mac(const uint8_t *mac_key, size_t mac_key_len, const uint8_t *salt, size_t salt_len,
          const uint8_t *encrypted, size_t encrypted_len, uint8_t *mac_out, size_t mac_len)
{
...
static size_t
build_secret_input(const hs_descriptor_t *desc, const uint8_t *secret_data, size_t secret_data_len,
                   uint8_t **secret_input_out)
{
    ...
    set_uint64(secret_input + offset, tor_htonll(desc->plaintext_data.revision_counter));
...
static void
build_kdf_key(const hs_descriptor_t *desc, const uint8_t *secret_data, size_t secret_data_len,
              const uint8_t *salt, size_t salt_len, uint8_t *key_out, size_t key_out_len,
              int is_superencrypted_layer)
{
    ...
    secret_input_len = build_secret_input(desc, secret_data, secret_data_len, &secret_input);
...
static void
build_secret_key_iv_mac(const hs_descriptor_t *desc, const uint8_t *secret_data,
                        size_t secret_data_len, const uint8_t *salt, size_t salt_len,
                        uint8_t *key_out, size_t key_len, uint8_t *iv_out, size_t iv_len,
                        uint8_t *mac_out, size_t mac_len, int is_superencrypted_layer)
{
    size_t offset = 0;
    ...
    build_kdf_key(desc, secret_data, secret_data_len, salt, salt_len, kdf_key, sizeof(kdf_key),
...
encode_link_specifiers(const smartlist_t *specs)
{
...

    {
        ...
        ret = base64_encode(encoded_b64, encoded_b64_len, (const char *)encoded, encoded_len, 0);
...
encode_legacy_key(const hs_desc_intro_point_t *ip)
{
    ...
    /* Encode cross cert. */
    if (base64_encode(b64_cert, sizeof(b64_cert), (const char *)ip->legacy.cert.encoded,
    ...
    if (crypto_pk_write_public_key_to_string(ip->legacy.key, &key_str, &key_str_len) < 0) {
...
static char *
encode_intro_point(const ed25519_public_key_t *sig_key, const hs_desc_intro_point_t *ip)
{
...
compute_padded_plaintext_length(size_t plaintext_len)
{
    ...
    plaintext_padded_len = CEIL_DIV(plaintext_len, padding_block_length) * padding_block_length;
 ...
 * padded buffer. */
STATIC size_t
build_plaintext_padding(const char *plaintext, size_t plaintext_len, uint8_t **padded_out)
{
    size_t padded_len;
    uint8_t *padded;
...
static size_t
build_encrypted(const uint8_t *key, const uint8_t *iv, const char *plaintext, size_t plaintext_len,
                uint8_t **encrypted_out, int is_superencrypted_layer)
{
    ...
    /* If we are encrypting the middle layer of the descriptor, we need to first
       pad the plaintext */
    if (is_superencrypted_layer) {
        encrypted_len = build_plaintext_padding(plaintext, plaintext_len, &padded_plaintext);
    ...
    cipher = crypto_cipher_new_with_iv_and_bits(key, iv, HS_DESC_ENCRYPTED_BIT_SIZE);
    ...
    /* This can't fail. */
    crypto_cipher_encrypt(cipher, (char *)encrypted, (const char *)padded_plaintext, encrypted_len);
...
static size_t
encrypt_descriptor_data(const hs_descriptor_t *desc, const uint8_t *secret_data,
                        size_t secret_data_len, const char *plaintext, char **encrypted_out,
                        int is_superencrypted_layer)
{
    char *final_blob;
    ...
    build_secret_key_iv_mac(desc, secret_data, secret_data_len, salt, sizeof(salt), secret_key,
                            sizeof(secret_key), secret_iv, sizeof(secret_iv), mac_key,
                            sizeof(mac_key), is_superencrypted_layer);

    /* Build the encrypted part that is do the actual encryption. */
    encrypted_len = build_encrypted(secret_key, secret_iv, plaintext, strlen(plaintext), &encrypted,
                                    is_superencrypted_layer);
    ...
    /* Build the MAC. */
    build_mac(mac_key, sizeof(mac_key), salt, sizeof(salt), encrypted, encrypted_len, mac,
              sizeof(mac));
...
#define ASSERT_AND_BASE64(field)                                                     \
    STMT_BEGIN                                                                       \
        tor_assert(!fast_mem_is_zero((char *)client->field, sizeof(client->field))); \
        ret = base64_encode_nopad(field##_b64, sizeof(field##_b64), client->field,   \
                                  sizeof(client->field));                            \
        tor_assert(ret > 0);                                                         \
    STMT_END
    ...
    tor_asprintf(&auth_client_str, "%s %s %s %s", str_desc_auth_client, client_id_b64, iv_b64,
...
get_all_auth_client_lines(const hs_descriptor_t *desc)
{
    ...
    tor_assert(smartlist_len(desc->superencrypted_data.clients) % HS_DESC_AUTH_CLIENT_MULTIPLE ==
               0);

    /* Make a line for each client */
    SMARTLIST_FOREACH_BEGIN (desc->superencrypted_data.clients, const hs_desc_authorized_client_t *,
                             client) {
        char *auth_client_str = NULL;
...
get_inner_encrypted_layer_plaintext(const hs_descriptor_t *desc)
{
    ...
    /* Build the start of the section prior to the introduction points. */
    {
        ...
        if (desc->encrypted_data.intro_auth_types &&
            ...
            char *buf = smartlist_join_strings(desc->encrypted_data.intro_auth_types, " ", 0, NULL);
    ...
    SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points, const hs_desc_intro_point_t *, ip) {
        char *encoded_ip = encode_intro_point(&desc->plaintext_data.signing_pubkey, ip);
        if (encoded_ip == NULL) {
 ...
 * string. Can not fail. */
static char *
get_outer_encrypted_layer_plaintext(const hs_descriptor_t *desc, const char *layer2_b64_ciphertext)
{
    ...
    { /* Print ephemeral x25519 key */
        ...
        tor_assert(!fast_mem_is_zero((char *)ephemeral_pubkey->public_key, CURVE25519_PUBKEY_LEN));
...
static char *
encrypt_desc_data_and_base64(const hs_descriptor_t *desc, const uint8_t *secret_data,
                             ...
                             int is_superencrypted_layer)
{
    ...
    enc_len = encrypt_descriptor_data(desc, secret_data, secret_data_len, encoded_str,
    ...
    ret_len = base64_encode(enc_b64, enc_b64_len, encrypted_blob, enc_len, BASE64_ENCODE_MULTILINE);
...
static size_t
build_secret_data(const ed25519_public_key_t *blinded_pubkey, const uint8_t *descriptor_cookie,
                  uint8_t **secret_data_out)
{
    ...
    if (descriptor_cookie) {
        ...
        memcpy(secret_data + ED25519_PUBKEY_LEN, descriptor_cookie, HS_DESC_DESCRIPTOR_COOKIE_LEN);
    } else {
...
static int
encode_superencrypted_data(const hs_descriptor_t *desc, const uint8_t *descriptor_cookie,
                           char **encrypted_blob_out)
{
    ...
    secret_data_len =
        build_secret_data(&desc->plaintext_data.blinded_pubkey, descriptor_cookie, &secret_data);

    /* Encrypt and b64 the inner layer */
    layer2_b64_ciphertext =
        encrypt_desc_data_and_base64(desc, secret_data, secret_data_len, layer2_str, 0);
    ...
    layer1_b64_ciphertext = encrypt_desc_data_and_base64(
        desc, desc->plaintext_data.blinded_pubkey.pubkey, ED25519_PUBKEY_LEN, layer1_str, 1);
...
static int
desc_encode_v3(const hs_descriptor_t *desc, const ed25519_keypair_t *signing_kp,
               const uint8_t *descriptor_cookie, char **encoded_out)
{
    ...
    /* Build the non-encrypted values. */
    {
        ...
        if (desc->plaintext_data.signing_key_cert->cert_type != CERT_TYPE_SIGNING_HS_DESC) {
            log_err(LD_BUG,
        ...
        if (tor_cert_encode_ed22519(desc->plaintext_data.signing_key_cert, &encoded_cert) < 0) {
            /* The function will print error logs. */
            goto err;
        }
        /* Create the hs descriptor line. */
        smartlist_add_asprintf(lines, "%s %" PRIu32, str_hs_desc, desc->plaintext_data.version);
    ...
    /* Build the superencrypted data section. */
    {
        ...
        if (encode_superencrypted_data(desc, descriptor_cookie, &enc_b64_blob) < 0) {
            goto err;
        }
        smartlist_add_asprintf(lines,
                               "%s\n"
...
static int
decode_auth_client(const directory_token_t *tok, hs_desc_authorized_client_t *client)
{
    int ret = -1;
    ...
    if (base64_decode((char *)client->client_id, sizeof(client->client_id), tok->args[0],
                      strlen(tok->args[0])) != sizeof(client->client_id)) {
        goto done;
    }
    if (base64_decode((char *)client->iv, sizeof(client->iv), tok->args[1], strlen(tok->args[1])) !=
        sizeof(client->iv)) {
        goto done;
    }
    if (base64_decode((char *)client->encrypted_cookie, sizeof(client->encrypted_cookie),
                      tok->args[2], strlen(tok->args[2])) != sizeof(client->encrypted_cookie)) {
        goto done;
    }
...
decode_link_specifiers(const char *encoded)
{
    ...
    decoded_len = base64_decode((char *)decoded, encoded_len, encoded, encoded_len);
    if (decoded_len < 0) {
        goto err;
    }

    if (link_specifier_list_parse(&specs, decoded, (size_t)decoded_len) < decoded_len) {
        goto err;
    }
    tor_assert(specs);
...
cert_is_valid(tor_cert_t *cert, uint8_t type, const char *log_obj_type)
{
    ...
    if (cert->cert_type != type) {
        log_warn(LD_REND, "Invalid cert type %02x for %s.", cert->cert_type, log_obj_type);
        goto err;
    }
    ...
    if (tor_cert_checksig(cert, &cert->signing_key, approx_time()) < 0) {
        if (cert->cert_expired) {
            ...
            log_fn(LOG_PROTOCOL_WARN, LD_REND, "Invalid signature for %s: %s (%s)", log_obj_type,
                   tor_cert_describe_signature_status(cert), expiration_str);
        } else {
...
static int
cert_parse_and_validate(tor_cert_t **cert_out, const char *data, size_t data_len,
                        unsigned int cert_type_wanted, const char *err_msg)
{
    tor_cert_t *cert;
...
build_descriptor_cookie_keys(const hs_subcredential_t *subcredential,
                             const curve25519_secret_key_t *sk, const curve25519_public_key_t *pk,
                             uint8_t **keys_out)
{
 ...
 * NULL. */
static int
decrypt_descriptor_cookie(const hs_descriptor_t *desc, const hs_desc_authorized_client_t *client,
                          ...
                          uint8_t **descriptor_cookie_out)
{
    ...
    tor_assert(!fast_mem_is_zero((char *)&desc->superencrypted_data.auth_ephemeral_pubkey,
                                 sizeof(desc->superencrypted_data.auth_ephemeral_pubkey)));
    tor_assert(!fast_mem_is_zero((char *)desc->subcredential.subcred, DIGEST256_LEN));
    ...
    keystream_length =
        build_descriptor_cookie_keys(&desc->subcredential, client_auth_sk,
    ...
    /* This creates a cipher for AES. It can't fail. */
    cipher =
        crypto_cipher_new_with_iv_and_bits(cookie_key, client->iv, HS_DESC_COOKIE_KEY_BIT_SIZE);
    descriptor_cookie = tor_malloc_zero(HS_DESC_DESCRIPTOR_COOKIE_LEN);
    /* This can't fail. */
    crypto_cipher_decrypt(cipher, (char *)descriptor_cookie, (const char *)client->encrypted_cookie,
           ...
           bool is_superencrypted_layer, char **decrypted_out))
{
    ...
    encrypted_len = encrypted_blob_size - (HS_DESC_ENCRYPTED_SALT_LEN + DIGEST256_LEN);
    ...
    build_secret_key_iv_mac(desc, secret_data, secret_data_len, salt, HS_DESC_ENCRYPTED_SALT_LEN,
                            secret_key, sizeof(secret_key), secret_iv, sizeof(secret_iv), mac_key,
                            sizeof(mac_key), is_superencrypted_layer);

    /* Build MAC. */
    build_mac(mac_key, sizeof(mac_key), salt, HS_DESC_ENCRYPTED_SALT_LEN, encrypted, encrypted_len,
              our_mac, sizeof(our_mac));
    memwipe(mac_key, 0, sizeof(mac_key));
...

    {
        ...
        cipher =
            crypto_cipher_new_with_iv_and_bits(secret_key, secret_iv, HS_DESC_ENCRYPTED_BIT_SIZE);
        /* Extra byte for the NUL terminated byte. */
        decrypted = tor_malloc_zero(encrypted_len + 1);
        crypto_cipher_decrypt(cipher, (char *)decrypted, (const char *)encrypted, encrypted_len);
        crypto_cipher_free(cipher);
    }
...
desc_decrypt_superencrypted(const hs_descriptor_t *desc, char **decrypted_out)
{
    ...
    superencrypted_len = decrypt_desc_layer(desc, NULL, true, &superencrypted_plaintext);
...
static size_t
desc_decrypt_encrypted(const hs_descriptor_t *desc, const curve25519_secret_key_t *client_auth_sk,
                       char **decrypted_out)
{
    ...
    if (client_auth_sk) {
        SMARTLIST_FOREACH_BEGIN (desc->superencrypted_data.clients, hs_desc_authorized_client_t *,
                                 client) {
            /* If we can decrypt the descriptor cookie successfully, we will use that
             * descriptor cookie and break from the loop. */
            if (!decrypt_descriptor_cookie(desc, client, client_auth_sk, &descriptor_cookie)) {
                break;
            }
        } SMARTLIST_FOREACH_END (client);
    }

    encrypted_len = decrypt_desc_layer(desc, descriptor_cookie, false, &encrypted_plaintext);
                        ...
                        hs_desc_intro_point_t *ip, const hs_descriptor_t *desc)
{
    ...
    if (rsa_ed25519_crosscert_check(ip->legacy.cert.encoded, ip->legacy.cert.len, ip->legacy.key,
...
static int
set_intro_point_onion_key(curve25519_public_key_t *onion_key_out, const smartlist_t *tokens)
{
    int retval = -1;
...
decode_introduction_point(const hs_descriptor_t *desc, const char *start)
{
    ...
    if (cert_parse_and_validate(&ip->auth_key_cert, tok->object_body, tok->object_size,
                                CERT_TYPE_AUTH_HS_IP_KEY, "introduction point auth-key") < 0) {
        goto err;
    }
    /* Validate authentication certificate with descriptor signing key. */
    if (tor_cert_checksig(ip->auth_key_cert, &desc->plaintext_data.signing_pubkey, 0) < 0) {
    ...
    if (cert_parse_and_validate(&ip->enc_key_cert, tok->object_body, tok->object_size,
                                CERT_TYPE_CROSS_HS_IP_KEYS,
                                "introduction point enc-key-cert") < 0) {
        goto err;
    }
    if (tor_cert_checksig(ip->enc_key_cert, &desc->plaintext_data.signing_pubkey, 0) < 0) {
...
static void
decode_intro_points(const hs_descriptor_t *desc, hs_desc_encrypted_data_t *desc_enc,
                    const char *data)
{
...
STATIC int
desc_sig_is_valid(const char *b64_sig, const ed25519_public_key_t *signing_pubkey,
                  const char *encoded_desc, size_t encoded_len)
{
    ...
    if (ed25519_checksig_prefixed(&sig, (const uint8_t *)encoded_desc, sig_start - encoded_desc,
                                  str_desc_sig_prefix, signing_pubkey) != 0) {
                         ...
                         const char *encoded_desc, size_t encoded_len)
{
    ...
    tor_assert(tok->n_args == 1);
    desc->lifetime_sec = (uint32_t)tor_parse_ulong(tok->args[0], 10, 0, UINT32_MAX, &ok, NULL);
    if (!ok) {
    ...
    if (cert_parse_and_validate(&desc->signing_key_cert, tok->object_body, tok->object_size,
                                CERT_TYPE_SIGNING_HS_DESC, "service descriptor signing key") < 0) {
        goto err;
    }
    ...
    tor_assert(tok->n_args == 1);
    desc->revision_counter = tor_parse_uint64(tok->args[0], 10, 0, UINT64_MAX, &ok, NULL);
    if (!ok) {
    ...
    if (!desc_sig_is_valid(tok->args[0], &desc->signing_pubkey, encoded_desc, encoded_len)) {
        goto err;
    }
                              ...
                              hs_desc_superencrypted_data_t *desc_superencrypted_out)
{
    ...
    tor_assert(tok->n_args >= 1);
    if (curve25519_public_from_base64(&superencrypted->auth_ephemeral_pubkey, tok->args[0]) < 0) {
    ...
    superencrypted->encrypted_blob = tor_memdup(tok->object_body, tok->object_size);
...
static hs_desc_decode_status_t
desc_decode_encrypted_v3(const hs_descriptor_t *desc, const curve25519_secret_key_t *client_auth_sk,
                         hs_desc_encrypted_data_t *desc_encrypted_out)
{
    ...
    if (smartlist_len(desc_encrypted_out->intro_points) > HS_CONFIG_V3_MAX_INTRO_POINTS) {
        log_warn(LD_REND,
                 "Service descriptor contains too many introduction "
                 "points. Maximum allowed is %d but we have %d",
                 HS_CONFIG_V3_MAX_INTRO_POINTS, smartlist_len(desc_encrypted_out->intro_points));
        goto err;
    }
                         ...
                         hs_desc_encrypted_data_t *desc_encrypted)
{
    ...
    ret = decode_encrypted_handlers[version](desc, client_auth_sk, desc_encrypted);
    if (ret < 0) {
        goto err;
    }
...
static hs_desc_decode_status_t (*decode_superencrypted_handlers[])(
    const hs_descriptor_t *desc, hs_desc_superencrypted_data_t *desc_superencrypted) = {
    /* v0 */ NULL,
    /* v1 */ NULL,
    /* v2 */ NULL,
 ...
 * indexed by the version number so v3 callback is at index 3 in the array. */
static hs_desc_decode_status_t (*decode_plaintext_handlers[])(smartlist_t *tokens,
...
hs_desc_decode_status_t
hs_desc_decode_plaintext(const char *encoded, hs_desc_plaintext_data_t *plaintext)
{
    ...
    if (encoded_len >= hs_cache_get_max_descriptor_size()) {
        log_warn(LD_REND, "Service descriptor is too big (%lu bytes)", (unsigned long)encoded_len);
        goto err;
    }
    ...
    if (tokenize_string(area, encoded, encoded + encoded_len, tokens, hs_desc_v3_token_table, 0) <
        0) {
    ...
    tor_assert(tok->n_args == 1);
    plaintext->version = (uint32_t)tor_parse_ulong(tok->args[0], 10, 0, UINT32_MAX, &ok, NULL);
    if (!ok) {
        log_warn(LD_REND, "Service descriptor has unparseable version %s", escaped(tok->args[0]));
        goto err;
    }
    ...
    /* Run the version specific plaintext decoder. */
    ret = decode_plaintext_handlers[plaintext->version](tokens, plaintext, encoded, encoded_len);
 ...
 * set to NULL. */
hs_desc_decode_status_t
hs_desc_decode_descriptor(const char *encoded, const hs_subcredential_t *subcredential,
                          const curve25519_secret_key_t *client_auth_sk, hs_descriptor_t **desc_out)
{
    ...
    if (BUG(!subcredential || fast_mem_is_zero((char *)subcredential, DIGEST256_LEN))) {
...
static int (*encode_handlers[])(const hs_descriptor_t *desc, const ed25519_keypair_t *signing_kp,
                                const uint8_t *descriptor_cookie, char **encoded_out) = {
    /* v0 */ NULL,
    /* v1 */ NULL,
    /* v2 */ NULL,
           ...
           const uint8_t *descriptor_cookie, char **encoded_out))
{
    ...
    ret = encode_handlers[version](desc, signing_kp, descriptor_cookie, encoded_out);
    if (ret < 0) {
        goto err;
    }
     ...
     * cookie will be NULL. */
    if (!descriptor_cookie) {
        ret = hs_desc_decode_descriptor(*encoded_out, &desc->subcredential, NULL, NULL);
...
{
    tor_assert(data);
    return (sizeof(*data) + sizeof(*data->signing_key_cert) + data->superencrypted_blob_size);
}
...
hs_desc_encrypted_obj_size(const hs_desc_encrypted_data_t *data)
{
    ...
    if (data->intro_auth_types) {
        intro_size += smartlist_len(data->intro_auth_types) * sizeof(intro_auth_types);
    }
    if (data->intro_points) {
        /* XXX could follow pointers here and get more accurate size */
        intro_size += smartlist_len(data->intro_points) * sizeof(hs_desc_intro_point_t);
    }
...
hs_desc_obj_size(const hs_descriptor_t *data)
{
    ...
    return (hs_desc_plaintext_obj_size(&data->plaintext_data) +
            hs_desc_encrypted_obj_size(&data->encrypted_data) + sizeof(data->subcredential));
}
...
hs_desc_intro_point_free_(hs_desc_intro_point_t *ip)
{
    ...
    if (ip->link_specifiers) {
        SMARTLIST_FOREACH(ip->link_specifiers, link_specifier_t *, ls, link_specifier_free(ls));
        smartlist_free(ip->link_specifiers);
    }
...
{
    hs_desc_authorized_client_t *client_auth = tor_malloc_zero(sizeof(*client_auth));

    crypto_rand((char *)client_auth->client_id, sizeof(client_auth->client_id));
    crypto_rand((char *)client_auth->iv, sizeof(client_auth->iv));
    crypto_rand((char *)client_auth->encrypted_cookie, sizeof(client_auth->encrypted_cookie));
                                ...
                                hs_desc_authorized_client_t *client_out)
{
    ...
    tor_assert(subcredential);
    tor_assert(!fast_mem_is_zero((char *)auth_ephemeral_sk, sizeof(*auth_ephemeral_sk)));
    tor_assert(!fast_mem_is_zero((char *)client_auth_pk, sizeof(*client_auth_pk)));
    tor_assert(!fast_mem_is_zero((char *)descriptor_cookie, HS_DESC_DESCRIPTOR_COOKIE_LEN));
    ...
    keystream_length =
        build_descriptor_cookie_keys(subcredential, auth_ephemeral_sk, client_auth_pk, &keystream);
    tor_assert(keystream_length > 0);
    ...
    /* This creates a cipher for AES. It can't fail. */
    cipher =
        crypto_cipher_new_with_iv_and_bits(cookie_key, client_out->iv, HS_DESC_COOKIE_KEY_BIT_SIZE);
    /* This can't fail. */
    crypto_cipher_encrypt(cipher, (char *)client_out->encrypted_cookie,
                          (const char *)descriptor_cookie, HS_DESC_DESCRIPTOR_COOKIE_LEN);
...
hs_descriptor_clear_intro_points(hs_descriptor_t *desc)
{
    ...
    if (ips) {
        SMARTLIST_FOREACH(ips, hs_desc_intro_point_t *, ip, hs_desc_intro_point_free(ip));
        smartlist_clear(ips);
    }
}
