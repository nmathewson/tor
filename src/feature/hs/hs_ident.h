...
typedef struct hs_ident_circuit_t {
    /** (All circuit) The public key used to uniquely identify the service. It
     * is the one found in the onion address. */
    ed25519_public_key_t identity_pk;

    /** (All circuit) Introduction point authentication key. It's also needed
     * on the rendezvous circuit for the ntor handshake. It's used as the
     * unique key of the introduction point so it should not be shared between
     * multiple intro points. */
    ...
    /** (Only client rendezvous circuit) Client ephemeral keypair needed for
     * the e2e encryption with the service. */
    curve25519_keypair_t rendezvous_client_kp;

    /** (Only rendezvous circuit) The NTOR_KEY_SEED needed for key derivation
     * for the e2e encryption with the client on the circuit. */
...
typedef struct hs_ident_dir_conn_t {
    /** The public key used to uniquely identify the service. It is the one
     * found in the onion address. */
    ed25519_public_key_t identity_pk;

    /** The blinded public key used to uniquely identify the descriptor that
     * this directory connection identifier is for. Only used by the
     * service-side code to fine control descriptor uploads. */
...
typedef struct hs_ident_edge_conn_t {
    ...
    /** The original virtual port that was used by the client to access the
     * onion service, regardless of the internal port forwarding that might
     * have happened on the service-side. */
