...
typedef struct hs_ident_circuit_t {
     ...
     * the rendezvous circuit for the ntor handshake. It's used as the unique
     * key of the introduction point so it should not be shared between multiple
     * intro points. */
...
typedef struct hs_ident_edge_conn_t {
     ...
     * onion service, regardless of the internal port forwarding that might have
