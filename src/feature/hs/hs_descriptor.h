...
#define hs_desc_superencrypted_data_free(desc) \
    FREE_AND_NULL(hs_desc_superencrypted_data_t, hs_desc_superencrypted_data_free_, (desc))
...
int hs_desc_decode_descriptor(const char *encoded, const hs_subcredential_t *subcredential,
...
#define hs_desc_authorized_client_free(client) \
    FREE_AND_NULL(hs_desc_authorized_client_t, hs_desc_authorized_client_free_, (client))
...
STATIC size_t build_plaintext_padding(const char *plaintext, size_t plaintext_len,
                                      uint8_t **padded_out);
/* Decoding. */
...
STATIC int desc_sig_is_valid(const char *b64_sig, const ed25519_public_key_t *signing_pubkey,
