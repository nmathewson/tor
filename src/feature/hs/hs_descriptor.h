...
typedef struct hs_desc_intro_point_t {
    ...
    /** Certificate cross certifying the descriptor signing key by the
     * encryption curve25519 key. This certificate contains the signing key and
     * is of type CERT_TYPE_CROSS_HS_IP_KEYS [0B]. */
    tor_cert_t *enc_key_cert;

    /** (Optional): If this introduction point is a legacy one that is version
     * <= 0.2.9.x (HSIntro=3), we use this extra key for the intro point to be
     * able to relay the cells to the service correctly. */
    struct {
        /** RSA public key. */
        crypto_pk_t *key;

        /** Cross certified cert with the descriptor signing key (RSA->Ed).
         * Because of the cross certification API, we need to keep the
         * certificate binary blob and its length in order to properly encode
         * it after. */
        struct {
            uint8_t *encoded;
            size_t len;
    ...
    /** True iff the introduction point has passed the cross certification.
...
typedef struct hs_desc_encrypted_data_t {
    /** Bitfield of CREATE2 cell supported formats. The only currently
     * supported format is ntor. */
...
typedef struct hs_desc_plaintext_data_t {
    ...
    /** Signing public key which is used to sign the descriptor. Same public
     * key as in the signing key certificate. */
...
#define hs_desc_plaintext_data_free(desc)                                 \
    FREE_AND_NULL(hs_desc_plaintext_data_t, hs_desc_plaintext_data_free_, \
                  (desc))
...
#define hs_desc_encrypted_data_free(desc)                                 \
    FREE_AND_NULL(hs_desc_encrypted_data_t, hs_desc_encrypted_data_free_, \
                  (desc))
