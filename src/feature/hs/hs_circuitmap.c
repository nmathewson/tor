...
static int
hs_circuits_have_same_token(const circuit_t *first_circuit, const circuit_t *second_circuit)
{
    ...
    return tor_memeq(first_token->token, second_token->token, first_token->token_len);
}
...
hs_circuit_hash_token(const circuit_t *circuit)
{
    ...
    return (unsigned)siphash24g(circuit->hs_token->token, circuit->hs_token->token_len);
}
...
HT_GENERATE2(hs_circuitmap_ht, circuit_t, hs_circuitmap_node, hs_circuit_hash_token,
             hs_circuits_have_same_token, 0.6, tor_reallocarray, tor_free_);
...
static void
hs_circuitmap_register_circuit(circuit_t *circ, hs_token_type_t type, size_t token_len,
                               const uint8_t *token)
{
...
static circuit_t *
hs_circuitmap_get_circuit_impl(hs_token_type_t type, size_t token_len, const uint8_t *token,
                               uint8_t wanted_circ_purpose)
{
    ...
    if (!found_circ || found_circ->purpose != wanted_circ_purpose || found_circ->marked_for_close) {
        return NULL;
    }
...
hs_circuitmap_get_origin_circuit(hs_token_type_t type, size_t token_len, const uint8_t *token,
                                 uint8_t wanted_circ_purpose)
{
    circuit_t *circ;
    tor_assert(token);
    ...
    circ = hs_circuitmap_get_circuit_impl(type, token_len, token, wanted_circ_purpose);
    if (!circ) {
        return NULL;
    }
...
static or_circuit_t *
hs_circuitmap_get_or_circuit(hs_token_type_t type, size_t token_len, const uint8_t *token,
                             uint8_t wanted_circ_purpose)
{
    circuit_t *circ;
    tor_assert(token);
...
hs_circuitmap_get_all_intro_circ_relay_side(void)
{
    ...
    HT_FOREACH (iter, hs_circuitmap_ht, the_hs_circuitmap) {
        ...
        if (CIRCUIT_IS_ORIGIN(circ) || circ->purpose != CIRCUIT_PURPOSE_INTRO_POINT ||
 ...
 * circuitmap. */
or_circuit_t *
hs_circuitmap_get_intro_circ_v3_relay_side(const ed25519_public_key_t *auth_key)
{
    return hs_circuitmap_get_or_circuit(HS_TOKEN_INTRO_V3_RELAY_SIDE, ED25519_PUBKEY_LEN,
                                        auth_key->pubkey, CIRCUIT_PURPOSE_INTRO_POINT);
}
...
{
    return hs_circuitmap_get_or_circuit(HS_TOKEN_INTRO_V2_RELAY_SIDE, REND_TOKEN_LEN, digest,
...
{
    return hs_circuitmap_get_or_circuit(HS_TOKEN_REND_RELAY_SIDE, REND_TOKEN_LEN, cookie,
 ...
 * circuitmap. */
void
hs_circuitmap_register_rend_circ_relay_side(or_circuit_t *circ, const uint8_t *cookie)
{
    hs_circuitmap_register_circuit(TO_CIRCUIT(circ), HS_TOKEN_REND_RELAY_SIDE, REND_TOKEN_LEN,
                                   cookie);
}
/** Public function: Register v2 intro circuit with key <b>digest</b> to the
 * circuitmap. */
void
hs_circuitmap_register_intro_circ_v2_relay_side(or_circuit_t *circ, const uint8_t *digest)
{
    hs_circuitmap_register_circuit(TO_CIRCUIT(circ), HS_TOKEN_INTRO_V2_RELAY_SIDE, REND_TOKEN_LEN,
                                   digest);
}
...
{
    hs_circuitmap_register_circuit(TO_CIRCUIT(circ), HS_TOKEN_INTRO_V3_RELAY_SIDE,
...
origin_circuit_t *
hs_circuitmap_get_intro_circ_v3_service_side(const ed25519_public_key_t *auth_key)
{
    origin_circuit_t *circ = NULL;

    /* Check first for established intro circuits */
    circ = hs_circuitmap_get_origin_circuit(HS_TOKEN_INTRO_V3_SERVICE_SIDE, ED25519_PUBKEY_LEN,
                                            auth_key->pubkey, CIRCUIT_PURPOSE_S_INTRO);
    if (circ) {
        return circ;
    }
    ...
    circ = hs_circuitmap_get_origin_circuit(HS_TOKEN_INTRO_V3_SERVICE_SIDE, ED25519_PUBKEY_LEN,
                                            auth_key->pubkey, CIRCUIT_PURPOSE_S_ESTABLISH_INTRO);
...
hs_circuitmap_get_intro_circ_v2_service_side(const uint8_t *digest)
{
    ...
    /* Check first for established intro circuits */
    circ = hs_circuitmap_get_origin_circuit(HS_TOKEN_INTRO_V2_SERVICE_SIDE, REND_TOKEN_LEN, digest,
...
hs_circuitmap_get_rend_circ_client_side(const uint8_t *cookie)
{
    ...
    circ = hs_circuitmap_get_origin_circuit(HS_TOKEN_REND_CLIENT_SIDE, REND_TOKEN_LEN, cookie,
 ...
 * circuitmap. */
void
hs_circuitmap_register_intro_circ_v2_service_side(origin_circuit_t *circ, const uint8_t *digest)
{
    hs_circuitmap_register_circuit(TO_CIRCUIT(circ), HS_TOKEN_INTRO_V2_SERVICE_SIDE, REND_TOKEN_LEN,
                                   digest);
}
...
hs_circuitmap_register_intro_circ_v3_service_side(origin_circuit_t *circ,
                                                  const ed25519_public_key_t *auth_key)
{
    hs_circuitmap_register_circuit(TO_CIRCUIT(circ), HS_TOKEN_INTRO_V3_SERVICE_SIDE,
                                   ED25519_PUBKEY_LEN, auth_key->pubkey);
}
 ...
 * circuitmap. */
void
hs_circuitmap_register_rend_circ_service_side(origin_circuit_t *circ, const uint8_t *cookie)
{
    hs_circuitmap_register_circuit(TO_CIRCUIT(circ), HS_TOKEN_REND_SERVICE_SIDE, REND_TOKEN_LEN,
                                   cookie);
}
...
void
hs_circuitmap_register_rend_circ_client_side(origin_circuit_t *or_circ, const uint8_t *cookie)
{
    ...
    hs_circuitmap_register_circuit(circ, HS_TOKEN_REND_CLIENT_SIDE, REND_TOKEN_LEN, cookie);
}
...
hs_circuitmap_remove_circuit(circuit_t *circ)
{
    ...
    if (tmp) {
        ...
        log_warn(LD_BUG, "Could not find circuit (%u) in circuitmap.", circ->n_circ_id);
    }
