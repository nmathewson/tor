...
hs_circuitmap_register_impl(circuit_t *circ, hs_token_t *token)
{
    ...
    /* Kill old circuits with the same token. We want new intro/rend circuits
       to take precedence over old ones, so that HSes and clients and
...
hs_circuitmap_get_all_intro_circ_relay_side(void)
{
    ...
    HT_FOREACH (iter, hs_circuitmap_ht, the_hs_circuitmap) {
        ...
        /* An origin circuit or purpose is wrong or the hs token is not set to
         * be a v2 or v3 intro relay side type, we ignore the circuit. Else, we
         * have a match so add it to our list. */
    ...
    const ed25519_public_key_t *auth_key)
{
    ...
    circ = hs_circuitmap_get_origin_circuit(
        HS_TOKEN_INTRO_V3_SERVICE_SIDE, ED25519_PUBKEY_LEN, auth_key->pubkey,
                                              ...
                                              const uint8_t *cookie)
{
    hs_circuitmap_register_circuit(
        TO_CIRCUIT(circ), HS_TOKEN_REND_SERVICE_SIDE, REND_TOKEN_LEN, cookie);
}
