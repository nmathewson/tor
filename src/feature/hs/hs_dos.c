...
get_intro2_rate_consensus_param(const networkstatus_t *ns)
{
    return networkstatus_get_param(ns, "HiddenServiceEnableIntroDoSRatePerSec",
                                   HS_DOS_INTRODUCE_DEFAULT_CELL_RATE_PER_SEC, 0, INT32_MAX);
}
...
get_intro2_burst_consensus_param(const networkstatus_t *ns)
{
    return networkstatus_get_param(ns, "HiddenServiceEnableIntroDoSBurstPerSec",
                                   HS_DOS_INTRODUCE_DEFAULT_CELL_BURST_PER_SEC, 0, INT32_MAX);
}
...
set_consensus_parameters(const networkstatus_t *ns)
{
    ...
    consensus_param_introduce_burst_per_sec = get_intro2_burst_consensus_param(ns);
    consensus_param_introduce_defense_enabled = get_intro2_enable_consensus_param(ns);
...
hs_dos_setup_default_intro2_defenses(or_circuit_t *circ)
{
    ...
    circ->introduce2_dos_defense_enabled = consensus_param_introduce_defense_enabled;
    token_bucket_ctr_init(&circ->introduce2_bucket, consensus_param_introduce_rate_per_sec,
...
hs_dos_can_send_intro2(or_circuit_t *s_intro_circ)
{
    ...
    token_bucket_ctr_refill(&s_intro_circ->introduce2_bucket, (uint32_t)approx_time());
