...
get_intro2_burst_consensus_param(const networkstatus_t *ns)
{
    return networkstatus_get_param(
        ns, "HiddenServiceEnableIntroDoSBurstPerSec",
...
hs_dos_can_send_intro2(or_circuit_t *s_intro_circ)
{
    ...
    if (BUG(TO_CIRCUIT(s_intro_circ)->purpose !=
     ...
     *
     * First, the INTRODUCE2 bucket will be refilled (if any). Then,
     * decremented because we are about to send or not the cell we just got.
     * Finally, evaluate if we can send it based on our token bucket state. */
