                         ...
                         const ed25519_public_key_t *auth_key)
{
     ...
     * will have a different intro auth key for each descriptor since we
...
close_service_rp_circuits(hs_service_t *service)
{
    ...
    while ((ocirc = circuit_get_next_service_rp_circ(ocirc))) {
        ...
        if (ocirc->hs_ident != NULL &&
             ...
             * circuit is considered old/uneeded. When freed, it is removed from
             * the hs circuitmap. */
...
register_all_services(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN(hs_service_staging_list, hs_service_t *, snew)
    {
        ...
        if (BUG(register_service(new_service_map, snew) < 0)) {
             ...
             * register a service means we failed to validate correctly. In that
             * case, don't break tor and ignore the service but tell user. */
...
build_all_descriptors(time_t now)
{
    FOR_EACH_SERVICE_BEGIN(service)
    {
         ...
         * happens when we rotate meaning that we are guaranteed to have a new
         * SRV at that point for the next time period. */
    ...
    FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
    {
                                    ...
                                    hs_service_intro_point_t *, ip)
        {
            if (should_remove_intro_point(ip, now)) {
                ...
                /* Remove intro point from descriptor map and add it to the list
                 * of ips to free for which we'll also try to close the intro
                 * circuit. */
                MAP_DEL_CURRENT(key);
...
should_rotate_descriptors(hs_service_t *service, time_t now)
{
    ...
    if (ns->valid_after >= service->state.next_rotation_time) {
         ...
         * the next SRV period using the consensus valid after time so it should
         ...
         * so this is why we BUG() on both with extra logging to try to
         * understand how this can possibly happens. We'll simply ignore and tor
         * should recover from this by skipping rotation and building the
    ...
    FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
    {
                                    ...
                                    hs_service_intro_point_t *, ip)
        {
            ...
            if (ei == NULL) {
                 ...
                 * info out of it. In this case, we remove the intro point and a
                 * new one will be picked at the next main loop callback. */
                MAP_DEL_CURRENT(key);
                service_intro_point_free(ip);
                continue;
            }
...
run_upload_descriptor_event(time_t now)
{
    ...
    FOR_EACH_SERVICE_BEGIN(service)
    {
        FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
        {
             ...
             * update the service's descriptor mutable fields in order to upload
...
static int
service_handle_intro_established(origin_circuit_t *circ, const uint8_t *payload,
                                 size_t payload_len)
{
    hs_service_t *service = NULL;
    hs_service_intro_point_t *ip = NULL;
...
hs_service_intro_circ_has_closed(origin_circuit_t *circ)
{
    ...
    if (ip == NULL) {
        /* The introduction point object has already been removed probably by
         * our cleanup process so ignore. */
        goto end;
    }
