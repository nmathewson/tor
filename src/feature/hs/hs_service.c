...
describe_intro_point(const hs_service_intro_point_t *ip)
{
    ...
    /* For now, we only print the identity digest but we could improve this
     * with much more information such as the ed25519 identity has well. */
    buf[0] = '$';
    if (legacy_id) {
...
{
    /* The [0, 2147483647] range is quite large to accommodate anything we
...
{
    /* The [0, 128] range bounds the number of extra introduction point
     * allowed. Above 128 intro points, it's getting a bit crazy. */
                         ...
                         const ed25519_public_key_t *auth_key)
{
     ...
     *
     * Even if we use the same node as intro point in both descriptors, the
     * node will have a different intro auth key for each descriptor since we
     * generate a new one everytime we pick an intro point.
     *
                                 ...
                                 unsigned int direct_conn)
{
    ...
    if (node == NULL) {
         ...
         * from the consensus. In that case, the intro point will be removed
         * from the descriptor during the scheduled events. */
        goto end;
    }

    /* In the case of a direct connection (single onion service), it is
     * possible our firewall policy won't allow it so this can return a NULL
     * value. */
...
close_service_rp_circuits(hs_service_t *service)
{
    ...
    while ((ocirc = circuit_get_next_service_rp_circ(ocirc))) {
        ...
        if (ocirc->hs_ident != NULL &&
             ...
             * circuit is considered old/uneeded. When freed, it is removed
             * from the hs circuitmap. */
            circuit_mark_for_close(TO_CIRCUIT(ocirc),
...
close_intro_circuits(hs_service_intropoints_t *intro_points)
{
    ...
    DIGEST256MAP_FOREACH (intro_points->map, key,
        ...
        if (ocirc) {
            /* Reason is FINISHED because service has been removed and thus the
             * circuit is considered old/uneeded. When freed, the circuit is
             * removed from the HS circuitmap. */
            circuit_mark_for_close(TO_CIRCUIT(ocirc),
                                   END_CIRC_REASON_FINISHED);
        }
    }
...
move_ephemeral_services(hs_service_ht *src, hs_service_ht *dst)
{
    ...
    /* Iterate over the map to find ephemeral service and move them to the
     * other map. We loop using this method to have a safe removal process. */
...
register_all_services(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (hs_service_staging_list, hs_service_t *, snew) {
        ...
        if (s) {
            /* Pass ownership of the descriptors from s (the current service)
             * to snew (the newly configured one). */
            move_descriptors(s, snew);
            move_hs_state(s, snew);
            /* Remove the service from the global map because after this, we
             * need to go over the remaining service in that map that aren't
             * surviving the reload to close their circuits. */
        ...
        if (BUG(register_service(new_service_map, snew) < 0)) {
            /* This should never happen because prior to registration, we
             * validate every service against the entire set. Not being able to
             * register a service means we failed to validate correctly. In
             * that case, don't
    ...
    /* Time to make the switch. We'll clear the staging list because its
     * content has now changed ownership to the map. */
...
write_address_to_file(const hs_service_t *service, const char *fname_)
{
    ...
    /* Construct the full address with the onion tld and write the hostname
     * file to disk. */
...
load_service_keys(hs_service_t *service)
{
    ...
    fname =
        hs_path_from_filename(config->directory_path, fname_keyfile_prefix);
    ...
    if (!kp) {
        ...
        kp = ed_key_init_from_file(fname, key_flags, LOG_WARN, NULL, 0, 0, 0,
                                   NULL, NULL);
        if (!kp) {
            log_warn(LD_REND, "Unable to generate keys and save in %s.",
                     fname);
            goto end;
        }
    }
...
parse_authorized_client(const char *client_key_str)
{
     ...
     * have that so we don't successfully decode a value with a different
     * length and end up in trouble when copying the decoded key into a fixed
     * length buffer. */
                                       ...
                                       const hs_service_config_t *config2)
{
    ...
    SMARTLIST_FOREACH(
        config1->clients, hs_service_authorized_client_t *, client,
        smartlist_add(sl1, service_authorized_client_dup(client)));

    SMARTLIST_FOREACH(
        config2->clients, hs_service_authorized_client_t *, client,
...
remove_expired_failing_intro(hs_service_t *service, time_t now)
{
    ...
    FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
    {
        DIGESTMAP_FOREACH_MODIFY (desc->intro_points.failed_id, key, time_t *,
                                  t) {
                       ...
                       hs_desc_intro_point_t *desc_ip)
{
    ...
    /* Encryption key and its cross certificate. */
    {
        ...
        ed25519_public_key_from_curve25519_public_key(
            &ed25519_pubkey, &ip->enc_key_kp.pubkey, 0);
        desc_ip->enc_key_cert = tor_cert_create(
            signing_kp, CERT_TYPE_CROSS_HS_IP_KEYS, &ed25519_pubkey,
            nearest_hour, HS_DESC_CERT_LIFETIME,
                        ...
                        hs_service_descriptor_t *desc, time_t now)
{
    ...
    DIGEST256MAP_FOREACH (desc->intro_points.map, key,
        ...
        if (!hs_circ_service_get_established_intro_circ(ip)) {
             ...
             * because their circuit has not opened and they haven't been
             * removed yet even though we have enough intro circuits.
             *
             * Due to #31561, it can stay in that list until rotation so this
                                  ...
                                  hs_service_descriptor_t *desc)
{
    ...
    if (config->is_client_auth_enabled) {
        SMARTLIST_FOREACH_BEGIN (config->clients,
            ...
            /* Prepare the client for descriptor and then add to the list in
             * the superencrypted part of the descriptor */
            hs_desc_build_authorized_client(
                &desc->desc->subcredential, &client->client_pk,
                &desc->auth_ephemeral_kp.seckey, desc->descriptor_cookie,
                desc_client);
...
build_descriptors_for_new_service(hs_service_t *service, time_t now)
{
     ...
     * period is thus TP#1 and the next is TP#2 which for both we have access
     * to their SRVs.
     *
...
build_all_descriptors(time_t now)
{
    FOR_EACH_SERVICE_BEGIN(service)
    {
        /* A service booting up will have both descriptors to NULL. No other
         * cases makes both descriptor non existent. */
        if (service->desc_current == NULL && service->desc_next == NULL) {
            build_descriptors_for_new_service(service, now);
            continue;
        }

        /* Reaching this point means we are pass bootup so at runtime. We
         * should *never* have an empty current descriptor. If the next
         * descriptor is empty, we'll try to build it for the next time period.
         * This only happens when we rotate meaning that we are guaranteed to
         * have a new SRV at that point for the next time period. */
...
pick_needed_intro_points(hs_service_t *service, hs_service_descriptor_t *desc)
{
    ...
    for (i = 0; i < num_needed_ip; i++) {
        ...
        /* This function will add the picked intro point node to the exclude
         * nodes list so we don't pick the same one at the next iteration. */
        ip = pick_intro_point(service->config.is_single_onion, exclude_nodes);
        if (ip == NULL) {
            /* If we end up unable to pick an introduction point it is because
             * we can't find suitable node and calling this again is highly
             * unlikely to give us a valid node all of the sudden. */
            log_info(LD_REND,
     ...
     * circuits to be the number of configured intro points circuits and not
     * the number of intro points object that we have. */
    ...
    if (num_intro_points < service->config.num_intro_points) {
        ...
        if (num_new_intro_points != 0) {
            log_info(LD_REND,
                     ...
                     num_intro_points);
            /* We'll build those introduction point into the descriptor once we
             * have confirmation that the circuits are opened and ready.
             * However, indicate that this descriptor should be uploaded from
             * now on. */
...
update_all_descriptors_intro_points(time_t now)
{
    FOR_EACH_SERVICE_BEGIN(service)
    {
        /* We'll try to update each descriptor that is if certain conditions
         * apply in order for the descriptor to be updated. */
        FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
        {
...
should_remove_intro_point(hs_service_intro_point_t *ip, time_t now)
{
     ...
     * if a circuit (established or pending) exists. In that case, we should
     * not remove it because it might simply be valid and opened at the
     * previous scheduled event for the last retry. */
...
{
    /* List of intro points to close. We can't mark the intro circuits for
     * close in the modify loop because doing so calls back into the HS
     * subsystem and we need to keep that code path outside of the service/desc
     * loop so those maps don't get modified during the close making us in a
     * possible use-after-free situation. */
    ...
    FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
    {
        ...
        DIGEST256MAP_FOREACH_MODIFY (desc->intro_points.map, key,
            ...
            if (should_remove_intro_point(ip, now)) {
                /* We've retried too many times, remember it as a failed intro
                 * point so we don't pick it up again for
                 * INTRO_CIRC_RETRY_PERIOD sec. */
                if (ip->circuit_retries > MAX_INTRO_POINT_CIRCUIT_RETRIES) {
                    remember_failing_intro_point(ip, desc, approx_time());
                }

                /* Remove intro point from descriptor map and add it to the
                 * list of ips to free for which we'll also try to close the
                 * intro circuit. */
                MAP_DEL_CURRENT(key);
    ...
    SMARTLIST_FOREACH_BEGIN (ips_to_free, hs_service_intro_point_t *, ip) {
        ...
        /* XXX: Legacy code does NOT close circuits like this: it keeps the
         * circuit open until a new descriptor is uploaded and then closed all
         * expiring intro point circuit. Here, we close immediately and because
         * we just discarded the intro point, a new one will be selected, a new
...
should_rotate_descriptors(hs_service_t *service, time_t now)
{
    ...
    if (ns->valid_after >= service->state.next_rotation_time) {
        /* In theory, we should never get here with no descriptors. We can
         * never have a NULL current descriptor except when tor starts up. The
         * next descriptor can be NULL after a rotation but we build a new one
         * right after.
         *
         * So, when tor starts, the next rotation time is set to the start of
         * the next SRV period using the consensus valid after time so it
         * should always be set to a future time value. This means that we
         * should never reach this point at bootup that is this check
         * safeguards tor in never allowing a rotation if the valid after time
         * is smaller than the next rotation time.
         *
         * This is all good in theory but we've had a NULL descriptor issue
         * here so this is why we BUG() on both with extra logging to try to
         * understand how this can possibly happens. We'll simply ignore and
         * tor should recover from this by skipping rotation and building the
         * missing descriptors just after this. */
...
rotate_service_descriptors(hs_service_t *service)
{
    if (service->desc_current) {
        ...
        /* We don't need this one anymore, we won't serve any clients coming
         * with this service descriptor. */
...
{
    /* XXX We rotate all our service descriptors at once. In the future it
     * might be wise, to rotate service descriptors independently to hide that
     * all those descriptors are on the same tor instance */

    FOR_EACH_SERVICE_BEGIN(service)
    {
        /* Note for a service booting up: Both descriptors are NULL in that
         * case so this function might return true if we are in the timeframe
         * for a rotation leading to basically swapping two NULL pointers which
         * is harmless. However, the side effect is that triggering a rotation
         * will update the service state and avoid doing anymore rotations
         * after the two descriptors have been built. */
...
{
    /* Note that nothing here opens circuit(s) nor uploads descriptor(s). We
     * are simply moving things around or removing unneeded elements. */

    FOR_EACH_SERVICE_BEGIN(service)
    {
        /* If the service is starting off, set the rotation time. We can't do
         * that at configure time because the get_options() needs to be set for
         * setting that time that uses the voting interval. */
        if (service->state.next_rotation_time == 0) {
            /* Set the next rotation time of the descriptors. If it's Oct 25th
             * 23:47:00, the next rotation time is when the next SRV is
             * computed which is at Oct 26th 00:00:00 that is in 13 minutes. */
            set_rotation_time(service);
        }
        ...
        /* Remove expired failing intro point from the descriptor failed list.
         * We reset them at each INTRO_CIRC_RETRY_PERIOD. */
        remove_expired_failing_intro(service, now);

        /* At this point, the service is now ready to go through the scheduled
         * events guaranteeing a valid state. Intro points might be missing
         * from the descriptors after the cleanup but the update/build process
         * will make sure we pick those missing ones. */
    }
    FOR_EACH_SERVICE_END;
}
...
run_build_descriptor_event(time_t now)
{
     ...
     * points. Missing introduction points will be picked in this function
     * which is useful for newly built descriptors. */
...
launch_intro_point_circuits(hs_service_t *service)
{
    ...
    FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
    {
        ...
        DIGEST256MAP_FOREACH_MODIFY (desc->intro_points.map, key,
            ...
            /* If we can't connect directly to the intro point, get an
             * extend_info for a multi-hop path instead. */
            ...
            if (ei == NULL) {
                /* This is possible if we can get a node_t but not the extend
                 * info out of it. In this case, we remove the intro point and
                 * a new one will be picked at the next main loop callback. */
                MAP_DEL_CURRENT(key);
                service_intro_point_free(ip);
                continue;
            }
...
can_service_launch_intro_circuit(hs_service_t *service, time_t now)
{
    ...
    /* Rate limit log that we've reached our circuit creation limit. */
    {
        char *msg;
        time_t elapsed_time =
            now - service->state.intro_circ_retry_started_time;
...
run_build_circuit_event(time_t now)
{
    ...
    FOR_EACH_SERVICE_BEGIN(service)
    {
        /* For introduction circuit, we need to make sure we don't stress too
         * much circuit creation so make sure this service is respecting that
         * limit. */
                                ...
                                bool is_current)
{
     ...
     * start of the SR protocol run that is relevant to this descriptor. This
     * is guaranteed to be a positive value since we need the SRV to start
     * making a descriptor (so that we know where to upload it).
     *
     ...
     * the current SRV for uploading the descriptor.  and hence we use the
     * start time of the current protocol run.
     */
    if (is_current) {
                         ...
                         hs_service_descriptor_t *desc)
{
     ...
     * short time frame which can lead to a race where the second upload
     * arrives before the first one leading to a 400 malformed descriptor
     * response from the directory. Closing all pending requests avoids that.
     */
    close_directory_connections(service, desc);

    /* Get our list of responsible HSDir. */
    responsible_dirs = smartlist_new();
    /* The parameter 0 means that we aren't a client so tell the function to
     * use the spread store consensus paremeter. */
    ...
    SMARTLIST_FOREACH_BEGIN (responsible_dirs, const routerstatus_t *,
        ...
        /* Getting responsible hsdir implies that the node_t object exists for
         * the routerstatus_t found in the consensus else we have a problem. */
...
{
    /* Writing the log every minute shouldn't be too annoying for log rate
     * limit since this can be emitted every second for each descriptor.
     *
                                 ...
                                 time_t now)
{
    ...
    /* If this descriptors has missing intro points that is that it couldn't
     * get them all when it was time to pick them, it means that we should
     * upload instead of waiting an arbitrary amount of time breaking the
     * service. Else, if we have no missing intro points, we use the value
    ...
    /* Do we know enough router descriptors to have adequate vision of the
       HSDir hash ring? */
...
refresh_service_descriptor(const hs_service_t *service,
                           hs_service_descriptor_t *desc, time_t now)
{
    /* There are few fields that we consider "mutable" in the descriptor
     * meaning we need to update them regurlarly over the lifetime fo the
     * descriptor. The rest are set once and should not be modified.
     *
...
run_upload_descriptor_event(time_t now)
{
    ...
    FOR_EACH_SERVICE_BEGIN(service)
    {
        FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
        {
            /* If we were asked to re-examine the hash ring, and it changed,
               then schedule an upload */
            ...
            /* We are about to upload so we need to do one last step which is
             * to update the service's descriptor mutable fields in order to
             * upload a coherent descriptor. */
                          ...
                          size_t payload_len)
{
    ...
    /* The following will parse, decode and launch the rendezvous point
     * circuit. Both current and legacy cells are handled. */
    if (hs_circ_handle_introduce2(service, circ, ip,
                                  &desc->desc->subcredential, payload,
                                  payload_len) < 0) {
        goto err;
    }
...
hs_service_lookup_current_desc(const ed25519_public_key_t *pk)
{
    ...
    if (service && service->desc_current) {
        char *encoded_desc = NULL;
        /* No matter what is the result (which should never be a failure),
         * return the encoded variable, if success it will contain the right
         * thing else it will be NULL. */
                              ...
                              edge_connection_t *conn)
{
    ...
    if (service == NULL) {
         ...
         * service so no danger. Attempting to bruteforce the entire key space
         * by opening circuits to learn which service is being hosted here is
         * impractical. */
        goto err_close;
    }

    /* Enforce the streams-per-circuit limit, and refuse to provide a mapping
     * if this circuit will exceed the limit. */
...
#define MAX_STREAM_WARN_INTERVAL 600
    ...
    /* Find a virtual port of that service mathcing the one in the connection
     * if successful, set the address in the connection. */
    if (hs_set_conn_addr_port(service->config.ports, conn) < 0) {
        ...
        if (service->config.allow_unknown_ports) {
            /* Service explicitly allow connection to unknown ports so close
             * right away because we do not care about port mapping. */
            goto err_close;
        }
        /* If the service didn't explicitly allow it, we do NOT close the
         * circuit here to raise the bar in terms of performance for port
         * mapping. */
        goto err_no_close;
    }
...
hs_service_dir_info_changed(void)
{
    if (hs_service_get_num_services() > 0) {
        ...
        static struct ratelim_t dir_info_changed_ratelim =
            RATELIM_INIT(30 * 60);
...
hs_service_load_all_keys(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (hs_service_staging_list, hs_service_t *,
                             service) {
...
hs_service_dump_stats(int severity)
{
    ...
    FOR_EACH_SERVICE_BEGIN(hs)
    {
        tor_log(severity, LD_GENERAL,
                "Service configured in %s:", service_escaped_dir(hs));
        FOR_EACH_DESCRIPTOR_BEGIN(hs, desc)
        {
            DIGEST256MAP_FOREACH (desc->intro_points.map, key,
                ...
                tor_log(severity, LD_GENERAL,
                        "  Intro point %s: circuit is %s", nickname,
