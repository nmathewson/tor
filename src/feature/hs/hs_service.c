...
static void set_descriptor_revision_counter(hs_service_descriptor_t *hs_desc, time_t now,
                                            bool is_current);
...
hs_service_ht_eq(const hs_service_t *first, const hs_service_t *second)
{
    ...
    tor_assert(second);
    /* Simple key compare. */
    return ed25519_pubkey_eq(&first->keys.identity_pk, &second->keys.identity_pk);
}
...
HT_GENERATE2(hs_service_ht, hs_service_t, hs_service_node, hs_service_ht_hash, hs_service_ht_eq,
...
remove_service(hs_service_ht *map, hs_service_t *service)
{
    ...
    if (BUG(service == NULL) || BUG(ed25519_public_key_is_zero(&service->keys.identity_pk))) {
        return;
    }
...
service_clear_config(hs_service_config_t *config)
{
    ...
    if (config->ob_master_pubkeys) {
        SMARTLIST_FOREACH(config->ob_master_pubkeys, ed25519_public_key_t *, k, tor_free(k));
...
describe_intro_point(const hs_service_intro_point_t *ip)
{
    ...
    SMARTLIST_FOREACH_BEGIN (ip->base.link_specifiers, const link_specifier_t *, lspec) {
        if (link_specifier_get_ls_type(lspec) == LS_LEGACY_ID) {
            legacy_id = (const char *)link_specifier_getconstarray_un_legacy_id(lspec);
            break;
        }
...
get_intro_point_min_introduce2(void)
{
    ...
    return networkstatus_get_param(NULL, "hs_intro_min_introduce2",
                                   INTRO_POINT_MIN_LIFETIME_INTRODUCTIONS, 0, INT32_MAX);
}
...
get_intro_point_max_introduce2(void)
{
    ...
    return networkstatus_get_param(NULL, "hs_intro_max_introduce2",
                                   INTRO_POINT_MAX_LIFETIME_INTRODUCTIONS, 0, INT32_MAX);
}
...
#define MIN_INTRO_POINT_LIFETIME_TESTING 10
    ...
    /* The [0, 2147483647] range is quite large to accommodate anything we decide
     * in the future. */
    return networkstatus_get_param(NULL, "hs_intro_min_lifetime", INTRO_POINT_LIFETIME_MIN_SECONDS,
                                   0, INT32_MAX);
}
...
#define MAX_INTRO_POINT_LIFETIME_TESTING 30
    ...
    /* The [0, 2147483647] range is quite large to accommodate anything we decide
     * in the future. */
    return networkstatus_get_param(NULL, "hs_intro_max_lifetime", INTRO_POINT_LIFETIME_MAX_SECONDS,
                                   0, INT32_MAX);
}
...
get_intro_point_num_extra(void)
{
    ...
    return networkstatus_get_param(NULL, "hs_intro_num_extra", NUM_INTRO_POINTS_EXTRA, 0, 128);
}
...
service_free_all(void)
{
    ...
    if (hs_service_staging_list) {
        ...
        SMARTLIST_FOREACH(hs_service_staging_list, hs_service_t *, s, hs_service_free(s));
...
service_intro_point_new(const node_t *node)
{
    ...
    { /* Set introduce2 max cells limit */
        ...
        ip->introduce2_max = crypto_rand_int_range(min_introduce2_cells, max_introduce2_cells);
    }
    { /* Set intro point lifetime */
        ...
        ip->time_to_expire = approx_time() + crypto_rand_int_range(intro_point_min_lifetime,
    ...
    if (!node_supports_ed25519_hs_intro(node)) {
        ...
        if (crypto_pk_get_digest(ip->legacy_key, (char *)ip->legacy_key_digest) < 0) {
            goto err;
        }
    }

    /* Flag if this intro point supports the INTRO2 dos defenses. */
    ip->support_intro2_dos_defense = node_supports_establish_intro_dos_extension(node);

    /* Finally, copy onion key from the node. */
    memcpy(&ip->onion_key, node_get_curve25519_onion_key(node), sizeof(ip->onion_key));
...
STATIC void
service_intro_point_remove(const hs_service_t *service, const hs_service_intro_point_t *ip)
{
    tor_assert(service);
    tor_assert(ip);
...
STATIC hs_service_intro_point_t *
service_intro_point_find(const hs_service_t *service, const ed25519_public_key_t *auth_key)
{
    ...
    FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
    {
        if ((ip = digest256map_get(desc->intro_points.map, auth_key->pubkey)) != NULL) {
            break;
        }
    }
    FOR_EACH_DESCRIPTOR_END;
...
service_desc_find_by_intro(const hs_service_t *service, const hs_service_intro_point_t *ip)
{
    ...
    FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
    {
        if (digest256map_get(desc->intro_points.map, ip->auth_key_kp.pubkey.pubkey)) {
            descp = desc;
            break;
        }
    }
...
get_objects_from_ident(const hs_ident_circuit_t *ident, hs_service_t **service,
                       hs_service_intro_point_t **ip, hs_service_descriptor_t **desc)
{
    hs_service_t *s;
...
get_node_from_intro_point(const hs_service_intro_point_t *ip)
{
    ...
    return node_get_by_id((const char *)link_specifier_getconstarray_un_legacy_id(ls));
}
...
get_extend_info_from_intro_point(const hs_service_intro_point_t *ip, unsigned int direct_conn)
{
          ...
          (const hs_service_descriptor_t *desc))
{
    ...
    DIGEST256MAP_FOREACH (desc->intro_points.map, key, const hs_service_intro_point_t *, ip) {
...
static void
close_directory_connections(const hs_service_t *service, const hs_service_descriptor_t *desc)
{
    unsigned int count = 0;
    ...
    dir_conns = connection_list_by_type_purpose(CONN_TYPE_DIR, DIR_PURPOSE_UPLOAD_HSDESC);
    SMARTLIST_FOREACH_BEGIN (dir_conns, connection_t *, conn) {
        dir_connection_t *dir_conn = TO_DIR_CONN(conn);
        if (ed25519_pubkey_eq(&dir_conn->hs_ident->identity_pk, &service->keys.identity_pk) &&
            ed25519_pubkey_eq(&dir_conn->hs_ident->blinded_pk, &desc->blinded_kp.pubkey)) {
...
close_service_rp_circuits(hs_service_t *service)
{
    ...
    while ((ocirc = circuit_get_next_service_rp_circ(ocirc))) {
        ...
        if (ocirc->hs_ident != NULL &&
            ed25519_pubkey_eq(&ocirc->hs_ident->identity_pk, &service->keys.identity_pk)) {
...
close_intro_circuits(hs_service_intropoints_t *intro_points)
{
    ...
    DIGEST256MAP_FOREACH (intro_points->map, key, const hs_service_intro_point_t *, ip) {
...
{
    return (s->config.is_ephemeral) ? "[EPHEMERAL]" : escaped(s->config.directory_path);
}
...
write_address_to_file(const hs_service_t *service, const char *fname_)
{
    ...
    if (write_str_to_file(fname, addr_buf, 0) < 0) {
        log_warn(LD_REND, "Could not write onion address to hostname file %s", escaped(fname));
        goto end;
    }
...
load_service_keys(hs_service_t *service)
{
    ...
    kp = ed_key_init_from_file(fname, INIT_ED_KEY_SPLIT, LOG_INFO, NULL, 0, 0, 0, NULL, NULL);
    if (!kp) {
        ...
        uint32_t key_flags = INIT_ED_KEY_CREATE | INIT_ED_KEY_EXTRA_STRONG | INIT_ED_KEY_SPLIT;
        kp = ed_key_init_from_file(fname, key_flags, LOG_WARN, NULL, 0, 0, 0, NULL, NULL);
        if (!kp) {
    ...
    memcpy(&service->keys.identity_sk, &kp->seckey, sizeof(service->keys.identity_sk));
    ...
    hs_build_address(&service->keys.identity_pk, (uint8_t)service->config.version,
...
client_filename_is_valid(const char *filename)
{
    ...
    if (!strcmpend(filename, valid_extension) && strlen(filename) != strlen(valid_extension)) {
        ret = 1;
    } else {
        ret = 0;
    }
...
parse_authorized_client(const char *client_key_str)
{
    ...
    if (strcmp(auth_type, "descriptor")) {
        log_warn(LD_REND, "Client authorization auth type '%s' not supported.", auth_type);
        goto err;
    }

    /* Currently, the only supported key type is "x25519". */
    if (strcmp(key_type, "x25519")) {
        log_warn(LD_REND, "Client authorization key type '%s' not supported.", key_type);
        goto err;
    }
    ...
    if (base32_decode((char *)client->client_pk.public_key, sizeof(client->client_pk.public_key),
                      pubkey_b32, strlen(pubkey_b32)) != sizeof(client->client_pk.public_key)) {
        log_warn(LD_REND, "Client authorization public key cannot be decoded: %s", pubkey_b32);
        goto err;
    }
...
load_client_keys(hs_service_t *service)
{
    ...
    client_keys_dir_path = hs_path_from_filename(config->directory_path, dname_client_pubkeys);
    if (BUG(hs_check_service_private_dir(get_options()->User, client_keys_dir_path,
                                         config->dir_group_readable, 1) < 0)) {
        goto end;
    }
    ...
    SMARTLIST_FOREACH_BEGIN (file_list, const char *, filename) {
        ...
        client_key_file_path = hs_path_from_filename(client_keys_dir_path, filename);
...
service_authorized_client_dup(const hs_service_authorized_client_t *client)
{
    ...
    memcpy(client_dup->client_pk.public_key, client->client_pk.public_key, CURVE25519_PUBKEY_LEN);
                              ...
                              const hs_service_authorized_client_t *client2)
{
    ...
    /* Currently, the public key is the only component of
     * hs_service_authorized_client_t. */
    return tor_memcmp(client1->client_pk.public_key, client2->client_pk.public_key,
                                       ...
                                       const hs_service_config_t *config2)
{
    ...
    for (i = 0; i < smartlist_len(sl1); i++) {
         ...
         * differ. */
        if (service_authorized_client_cmp(smartlist_get(sl1, i), smartlist_get(sl2, i))) {
            goto done;
        }
    }
...
move_descriptors(hs_service_t *src, hs_service_t *dst)
{
    ...
    int client_auth_changed = !service_authorized_client_config_equal(&src->config, &dst->config);
    if (client_auth_changed && dst->desc_current) {
        /* We have to clear the superencrypted content first. */
        hs_desc_superencrypted_data_free_contents(&dst->desc_current->desc->superencrypted_data);
    ...
    if (client_auth_changed && dst->desc_next) {
        /* We have to clear the superencrypted content first. */
        hs_desc_superencrypted_data_free_contents(&dst->desc_next->desc->superencrypted_data);
...
static void
setup_intro_point_exclude_list(const hs_service_descriptor_t *desc, smartlist_t *node_list)
{
    tor_assert(desc);
 ...
 * desc failed id map. */
static void
remember_failing_intro_point(const hs_service_intro_point_t *ip, hs_service_descriptor_t *desc,
                             time_t now)
{
...
static int
setup_desc_intro_point(const ed25519_keypair_t *signing_kp, const hs_service_intro_point_t *ip,
                       time_t now, hs_desc_intro_point_t *desc_ip)
{
    ...
    desc_ip->auth_key_cert =
        tor_cert_create(signing_kp, CERT_TYPE_AUTH_HS_IP_KEY, &ip->auth_key_kp.pubkey, nearest_hour,
    ...
    SMARTLIST_FOREACH_BEGIN (ip->base.link_specifiers, const link_specifier_t *, ls) {
        if (BUG(!ls)) {
            goto done;
        }
    ...
    if (ip->base.is_only_legacy) {
        ...
        ssize_t cert_len = tor_make_rsa_ed25519_crosscert(&signing_kp->pubkey, desc_ip->legacy.key,
    ...
    /* Encryption key and its cross certificate. */
    {
        ...
        memcpy(&desc_ip->enc_key, &ip->enc_key_kp.pubkey, sizeof(desc_ip->enc_key));
        /* The following can't fail. */
        ed25519_public_key_from_curve25519_public_key(&ed25519_pubkey, &ip->enc_key_kp.pubkey, 0);
        desc_ip->enc_key_cert =
            tor_cert_create(signing_kp, CERT_TYPE_CROSS_HS_IP_KEYS, &ed25519_pubkey, nearest_hour,
                            HS_DESC_CERT_LIFETIME, CERT_FLAG_INCLUDE_SIGNING_KEY);
...
static void
build_desc_intro_points(const hs_service_t *service, hs_service_descriptor_t *desc, time_t now)
{
...
build_desc_signing_key_cert(hs_service_descriptor_t *desc, time_t now)
{
    ...
    plaintext->signing_key_cert =
        tor_cert_create(&desc->blinded_kp, CERT_TYPE_SIGNING_HS_DESC, &desc->signing_kp.pubkey, now,
                        HS_DESC_CERT_LIFETIME, CERT_FLAG_INCLUDE_SIGNING_KEY);
...
static int
build_service_desc_encrypted(const hs_service_t *service, hs_service_descriptor_t *desc)
{
    hs_desc_encrypted_data_t *encrypted;
 ...
 * else -1 on error. */
static int
build_service_desc_superencrypted(const hs_service_t *service, hs_service_descriptor_t *desc)
{
    const hs_service_config_t *config;
    int i;
    ...
    memcpy(&superencrypted->auth_ephemeral_pubkey, &desc->auth_ephemeral_kp.pubkey,
           sizeof(curve25519_public_key_t));

    /* Test that subcred is not zero because we might use it below */
    if (BUG(fast_mem_is_zero((char *)desc->desc->subcredential.subcred, DIGEST256_LEN))) {
        return -1;
    }
    ...
    if (config->is_client_auth_enabled) {
        SMARTLIST_FOREACH_BEGIN (config->clients, hs_service_authorized_client_t *, client) {
            ...
            hs_desc_build_authorized_client(&desc->desc->subcredential, &client->client_pk,
    ...
    if (num_clients == 0) {
        ...
        num_clients_to_add =
            HS_DESC_AUTH_CLIENT_MULTIPLE - (num_clients % HS_DESC_AUTH_CLIENT_MULTIPLE);
    }

    for (i = 0; i < num_clients_to_add; i++) {
        hs_desc_authorized_client_t *desc_client = hs_desc_build_fake_authorized_client();
        smartlist_add(superencrypted->clients, desc_client);
    }
...
static void
build_service_desc_plaintext(const hs_service_t *service, hs_service_descriptor_t *desc)
{
    hs_desc_plaintext_data_t *plaintext;

    tor_assert(service);
    tor_assert(desc);
    tor_assert(!fast_mem_is_zero((char *)&desc->blinded_kp, sizeof(desc->blinded_kp)));
    tor_assert(!fast_mem_is_zero((char *)&desc->signing_kp, sizeof(desc->signing_kp)));
...
generate_ope_cipher_for_desc(const hs_service_descriptor_t *hs_desc)
{
    ...
    crypto_digest_add_bytes(digest, (char *)eph_privkey->seckey, sizeof(eph_privkey->seckey));
...
static int
build_service_desc_keys(const hs_service_t *service, hs_service_descriptor_t *desc)
{
    int ret = -1;
    ed25519_keypair_t kp;

    tor_assert(desc);
    tor_assert(!fast_mem_is_zero((char *)&service->keys.identity_pk, ED25519_PUBKEY_LEN));
    ...
    hs_build_blinded_keypair(&kp, NULL, 0, desc->time_period_num, &desc->blinded_kp);
    ...
    log_info(LD_GENERAL, "Getting OPE for TP#%u", (unsigned)desc->time_period_num);
    ...
    crypto_strongest_rand(desc->descriptor_cookie, sizeof(desc->descriptor_cookie));
                         ...
                         hs_service_descriptor_t **desc_out)
{
     ...
     * decodable after. */
    if (BUG(service_encode_descriptor(service, desc, &desc->signing_kp, &encoded_desc) < 0)) {
        goto err;
    }
    ...
    *desc_out = desc;
    ...
    hs_control_desc_event_created(service->onion_address, &desc->blinded_kp.pubkey);
...
build_all_descriptors(time_t now)
{
    FOR_EACH_SERVICE_BEGIN(service)
    {
        ...
        if (service->desc_next == NULL) {
            build_service_descriptor(service, hs_get_next_time_period_num(0), &service->desc_next);
            log_info(LD_REND,
...
pick_needed_intro_points(hs_service_t *service, hs_service_descriptor_t *desc)
{
    ...
    num_needed_ip = service->config.num_intro_points - digest256map_size(desc->intro_points.map);
    ...
    DIGEST256MAP_FOREACH (desc->intro_points.map, key, hs_service_intro_point_t *, ip) {
...
STATIC void
service_desc_schedule_upload(hs_service_descriptor_t *desc, time_t now, int descriptor_changed)
...
static void
update_service_descriptor_intro_points(hs_service_t *service, hs_service_descriptor_t *desc,
                                       time_t now)
{
    ...
    if (num_intro_points < service->config.num_intro_points) {
        unsigned int num_new_intro_points = pick_needed_intro_points(service, desc);
        if (num_new_intro_points != 0) {
            log_info(LD_REND,
                     ...
                     (desc == service->desc_current) ? "current" : "next", num_intro_points);
        ...
        if ((num_new_intro_points + num_intro_points) < service->config.num_intro_points) {
...
should_remove_intro_point(hs_service_intro_point_t *ip, time_t now)
{
    ...
    if (ret) {
        log_info(LD_REND,
                 ...
                 has_expired ? " has expired" : (has_no_node) ? " fell off the consensus" : "",
                 ip->circuit_retries);
    }
    return ret;
}
...
cleanup_intro_points(hs_service_t *service, time_t now)
{
    ...
    FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
    {
        ...
        DIGEST256MAP_FOREACH_MODIFY (desc->intro_points.map, key, hs_service_intro_point_t *, ip) {
...
set_rotation_time(hs_service_t *service)
{
    ...
    service->state.next_rotation_time =
        sr_state_get_start_time_of_current_protocol_run() + sr_state_get_protocol_run_duration();

    {
        char fmt_time[ISO_TIME_LEN + 1];
        format_local_iso_time(fmt_time, service->state.next_rotation_time);
        log_info(LD_REND, "Next descriptor rotation time set to %s for %s", fmt_time,
                 safe_str_client(service->onion_address));
    }
}
    ...
    if (ns->valid_after >= service->state.next_rotation_time) {
        ...
        if (BUG(service->desc_current == NULL || service->desc_next == NULL)) {
            log_warn(LD_BUG,
                     ...
                     (long)service->state.next_rotation_time, (long)now, (long)ns->valid_after);
            goto no_rotation;
        }
        goto rotation;
    }
...
rotate_all_descriptors(time_t now)
{
    ...
    FOR_EACH_SERVICE_BEGIN(service)
    {
        ...
        log_info(LD_REND, "Time to rotate our descriptors (%p / %p) for %s", service->desc_current,
                 service->desc_next, safe_str_client(service->onion_address));
...
get_max_intro_circ_per_period(const hs_service_t *service)
{
    ...
    tor_assert(service);
    tor_assert(service->config.num_intro_points <= HS_CONFIG_V3_MAX_INTRO_POINTS);
...
can_service_launch_intro_circuit(hs_service_t *service, time_t now)
{
    ...
    if (service->state.num_intro_circ_launched <= get_max_intro_circ_per_period(service)) {
        goto allow;
    }
...
run_build_circuit_event(time_t now)
{
    ...
    if (router_have_consensus_path() == CONSENSUS_PATH_UNKNOWN || !have_completed_a_circuit()) {
        return;
    }
 ...
 * is false. */
static void
upload_descriptor_to_hsdir(const hs_service_t *service, hs_service_descriptor_t *desc,
                           const node_t *hsdir)
{
    ...
    hs_service_upload_desc_to_dir(encoded_desc, service->config.version, &service->keys.identity_pk,
    ...
    /* Logging so we know where it was sent. */
    {
        ...
        const uint8_t *idx =
            (is_next_desc) ? hsdir->hsdir_index.store_second : hsdir->hsdir_index.store_first;
        ...
        log_info(LD_REND,
                 ...
                 safe_str_client(service->onion_address), (is_next_desc) ? "next" : "current",
                 desc->desc->plaintext_data.revision_counter, safe_str_client(node_describe(hsdir)),
...
static void
set_descriptor_revision_counter(hs_service_descriptor_t *hs_desc, time_t now, bool is_current)
{
    ...
    rev_counter = crypto_ope_encrypt(hs_desc->ope_cipher, (int)seconds_since_start_of_srv);

    /* The OPE module returns CRYPTO_OPE_ERROR in case of errors. */
    tor_assert_nonfatal(rev_counter < CRYPTO_OPE_ERROR);

    log_info(LD_REND, "Encrypted revision counter %d to %" PRIu64, (int)seconds_since_start_of_srv,
             rev_counter);
...
STATIC void
upload_descriptor_to_all(const hs_service_t *service, hs_service_descriptor_t *desc)
{
     ...
     * descriptor. */
    desc->next_upload_time = (time(NULL) + crypto_rand_int_range(HS_SERVICE_NEXT_UPLOAD_TIME_MIN,
...
STATIC int
service_desc_hsdirs_changed(const hs_service_t *service, const hs_service_descriptor_t *desc)
{
...
static void
log_cant_upload_desc(const hs_service_t *service, const hs_service_descriptor_t *desc,
                     const char *msg, const log_desc_upload_reason_t reason)
{
    ...
    static ratelim_t limits[2][LOG_DESC_UPLOAD_REASON_MAX + 1] = {
        {RATELIM_INIT(60), RATELIM_INIT(60), RATELIM_INIT(60 * 10), RATELIM_INIT(60),
         RATELIM_INIT(60)},
        {RATELIM_INIT(60), RATELIM_INIT(60), RATELIM_INIT(60 * 10), RATELIM_INIT(60),
         RATELIM_INIT(60)},
    };
    ...
    log_fn_ratelim(rlim, LOG_INFO, LD_REND, "Service %s can't upload its %s descriptor: %s",
                   safe_str_client(service->onion_address), (is_next_desc) ? "next" : "current",
                   msg);
}
...
static int
should_service_upload_descriptor(const hs_service_t *service, const hs_service_descriptor_t *desc,
                                 time_t now)
{
    char *msg = NULL;
    ...
    if (desc->missing_intro_points && num_intro_points == 0) {
        ...
        log_cant_upload_desc(service, desc, msg, LOG_DESC_UPLOAD_REASON_MISSING_IPS);
        goto cannot;
    }
    ...
    if (count_ip_established != num_intro_points) {
        ...
        log_cant_upload_desc(service, desc, msg, LOG_DESC_UPLOAD_REASON_IP_NOT_ESTABLISHED);
        goto cannot;
    }
    ...
    if (desc->next_upload_time > now) {
        ...
        log_cant_upload_desc(service, desc, msg, LOG_DESC_UPLOAD_REASON_NOT_TIME);
        goto cannot;
    }
    ...
    if (!networkstatus_get_live_consensus(now)) {
        ...
        log_cant_upload_desc(service, desc, msg, LOG_DESC_UPLOAD_REASON_NO_LIVE_CONSENSUS);
        goto cannot;
    }
    ...
    if (!router_have_minimum_dir_info()) {
        ...
        log_cant_upload_desc(service, desc, msg, LOG_DESC_UPLOAD_REASON_NO_DIRINFO);
        goto cannot;
    }
...
static void
refresh_service_descriptor(const hs_service_t *service, hs_service_descriptor_t *desc, time_t now)
{
    ...
    FOR_EACH_SERVICE_BEGIN(service)
    {
        FOR_EACH_DESCRIPTOR_BEGIN(service, desc)
        {
            ...
            log_info(LD_REND,
                     ...
                     safe_str_client(service->onion_address),
                     digest256map_size(desc->intro_points.map), service->config.num_intro_points,
...
service_intro_circ_has_opened(origin_circuit_t *circ)
{
    ...
    if (ip == NULL) {
        log_warn(LD_REND,
                 ...
                 "for service %s",
                 TO_CIRCUIT(circ)->n_circ_id, safe_str_client(service->onion_address));
        goto err;
    }
...
service_rendezvous_circ_has_opened(origin_circuit_t *circ)
{
    ...
    if (service == NULL) {
        log_warn(LD_REND,
                 ...
                 safe_str_client(ed25519_fmt(&circ->hs_ident->identity_pk)),
                 TO_CIRCUIT(circ)->n_circ_id,
                 hex_str((const char *)circ->hs_ident->rendezvous_cookie, REND_COOKIE_LEN));
        goto err;
    }
...
static int
service_handle_intro_established(origin_circuit_t *circ, const uint8_t *payload, size_t payload_len)
{
    hs_service_t *service = NULL;
    hs_service_intro_point_t *ip = NULL;
    ...
    if (hs_circ_handle_intro_established(service, ip, circ, payload, payload_len) < 0) {
        goto err;
    }
...
static int
service_handle_introduce2(origin_circuit_t *circ, const uint8_t *payload, size_t payload_len)
{
    hs_service_t *service = NULL;
    hs_service_intro_point_t *ip = NULL;
    hs_service_descriptor_t *desc = NULL;
    ...
    if (hs_circ_handle_introduce2(service, circ, ip, &desc->desc->subcredential, payload,
                                  payload_len) < 0) {
        goto err;
    }
...
service_key_on_disk(const char *directory_path)
{
    ...
    kp = ed_key_init_from_file(fname, INIT_ED_KEY_SPLIT, LOG_DEBUG, NULL, 0, 0, 0, NULL, NULL);
    if (kp) {
        ret = 1;
    }
...
static int
service_encode_descriptor(const hs_service_t *service, const hs_service_descriptor_t *desc,
                          const ed25519_keypair_t *signing_kp, char **encoded_out)
{
    int ret;
    ...
    ret = hs_desc_encode_descriptor(desc->desc, signing_kp, descriptor_cookie, encoded_out);
...
hs_service_add_ephemeral(ed25519_secret_key_t *sk, smartlist_t *ports,
                         int max_streams_per_rdv_circuit, int max_streams_close_circuit,
                         char **address_out)
{
    ...
    if (ed25519_public_key_generate(&service->keys.identity_pk, &service->keys.identity_sk) < 0) {
...
hs_service_del_ephemeral(const char *address)
{
    ...
    log_info(LD_CONFIG, "Removed ephemeral v3 hidden service: %s", safe_str_client(address));
    return 0;
...
hs_service_lookup_current_desc(const ed25519_public_key_t *pk)
{
    ...
    if (service && service->desc_current) {
        ...
        service_encode_descriptor(service, service->desc_current,
                                  &service->desc_current->signing_kp, &encoded_desc);
        return encoded_desc;
    }
 ...
 * security reasons. */
int
hs_service_set_conn_addr_port(const origin_circuit_t *circ, edge_connection_t *conn)
{
    hs_service_t *service = NULL;
    ...
    if (service == NULL) {
        log_warn(LD_REND,
                 ...
                 ed25519_fmt(&circ->hs_ident->identity_pk), TO_CIRCUIT(circ)->n_circ_id);
    ...
    if (service->config.max_streams_per_rdv_circuit > 0 &&
        (circ->hs_ident->num_rdv_streams >= service->config.max_streams_per_rdv_circuit)) {
#define MAX_STREAM_WARN_INTERVAL 600
        ...
        log_fn_ratelim(&stream_ratelim, LOG_WARN, LD_REND,
                       ...
                       circ->hs_ident->num_rdv_streams, service->config.max_streams_per_rdv_circuit,
                       service->config.max_streams_close_circuit ? "Closing circuit"
...
void
hs_service_lists_fnames_for_sandbox(smartlist_t *file_list, smartlist_t *dir_list)
{
...
int
hs_service_receive_introduce2(origin_circuit_t *circ, const uint8_t *payload, size_t payload_len)
{
    int ret = -1;
...
int
hs_service_receive_intro_established(origin_circuit_t *circ, const uint8_t *payload,
                                     size_t payload_len)
{
    int ret = -1;
...
hs_service_load_all_keys(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (hs_service_staging_list, hs_service_t *, service) {
        ...
        log_info(LD_REND, "Loading v3 onion service keys from %s", service_escaped_dir(service));
...
hs_service_dump_stats(int severity)
{
    ...
    {
        tor_log(severity, LD_GENERAL, "Service configured in %s:", service_escaped_dir(hs));
        FOR_EACH_DESCRIPTOR_BEGIN(hs, desc)
        {
            DIGEST256MAP_FOREACH (desc->intro_points.map, key, hs_service_intro_point_t *, ip) {
                ...
                if (!circ) {
                    tor_log(severity, LD_GENERAL, "  Intro point at %s: no circuit", nickname);
                    continue;
                }
                tor_log(severity, LD_GENERAL, "  Intro point %s: circuit is %s", nickname,
