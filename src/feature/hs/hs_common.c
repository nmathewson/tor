...
hs_get_service_max_rend_failures(void)
{
    return networkstatus_get_param(
        NULL, "hs_service_max_rdv_failures", MAX_REND_FAILURES_DEFAULT,
...
hs_get_start_time_of_next_time_period(time_t now)
{
    ...
    unsigned int time_period_rotation_offset = sr_state_get_phase_duration();
    return (time_t)(start_of_next_tp_in_mins * 60 +
...
get_disaster_srv(uint64_t time_period_num, uint8_t *srv_out)
{
    if (time_period_num == cached_time_period_nums[0]) {
        ...
        compute_disaster_srv(time_period_num,
                             cached_disaster_srv[replace_idx]);
                      ...
                      uint8_t *checksum_out, uint8_t *version_out)
{
    ...
    if (checksum_out) {
        ...
        memcpy(checksum_out, address + offset,
    ...
    if (chosen_port) {
        if (!(chosen_port->is_unix_addr)) {
            ...
            if (set_unix_port(conn, chosen_port)) {
                /* Simply impossible to end up here else we were able to add a
                 * Unix port without AF_UNIX support... ? */
                tor_assert(0);
            }
        }
    }
...
hs_address_is_valid(const char *address)
{
    ...
    /* Validate that this pubkey does not have a torsion component. We need to
     * do this on the prop224 client-side so that attackers can't give
     * equivalent forms of an onion address to users. */
          ...
          (const networkstatus_t *consensus, time_t now))
{
    ...
    /* Get start time of next TP and of current SRV protocol run, and check if
     * we are between them. */
                     ...
                     uint8_t *hsdir_index_out)
{
    ...
    digest = crypto_digest256_new(DIGEST_SHA3_256);
    crypto_digest_add_bytes(digest, HSDIR_INDEX_PREFIX,
                            HSDIR_INDEX_PREFIX_LEN);
    crypto_digest_add_bytes(digest, (const char *)identity_pk->pubkey,
                            ED25519_PUBKEY_LEN);
...
node_has_hsdir_index(const node_t *node)
{
     ...
     * always use the consensus-indexed node's keys to build the hash ring,
     * even if some of the consensus-indexed nodes are also bridges. */
                          ...
                          int for_fetching, smartlist_t *responsible_dirs)
{
     ...
     * hsdir_index already computed for them for this consensus. */
    {
        SMARTLIST_FOREACH_BEGIN (c->routerstatus_list, const routerstatus_t *,
                                 rs) {
            /* Even though this node_t object won't be modified and should be
             * const, we can't add const object in a smartlist_t. */
    ...
    for (int replica = 1; replica <= hs_get_hsdir_n_replicas(); replica++) {
        ...
        /* Number of node to add to the responsible dirs list depends on if we
         * are trying to fetch or store. A client always fetches. */
         ...
         * we are looking for so start at the first element. */
        ...
        while (n_added < n_to_add) {
            ...
            if (idx == start) {
                /* We've gone over the whole list, stop and avoid infinite
                 * loop. */
                break;
            }
        }
                                ...
                                const char *req_key_str, time_t now, int set)
{
    ...
    if (set) {
        ...
        *last_request_ptr = now;
        ...
        last_request_ptr =
            strmap_get(last_hid_serv_requests, hsdir_desc_comb_id);
    }
...
hs_purge_hid_serv_from_last_hid_serv_requests(const char *req_key_str)
{
    ...
    for (iter = strmap_iter_init(last_hid_serv_requests);
        ...
        /* This strmap contains variable-sized elements so this is a basic
         * length check on the strings we are about to compare. The key is
         * variable sized since it's composed as follows: key =
         * base32(hsdir_identity) + base32(req_key_str) where 'req_key_str' is
         * the descriptor ID of the HS in the v2 case, or the ed25519 blinded
         * public key of the HS in the v3 case. */
              ...
              bool *is_rate_limited_out)
{
    ...
    excluded_some = smartlist_len(usable_responsible_dirs) <
    ...
    if (!hs_dir) {
        ...
        /* Remember that we are requesting a descriptor from this hidden
         * service directory now. */
                               ...
                               int direct_conn)
{
    ...
    SMARTLIST_FOREACH_BEGIN (lspecs, const link_specifier_t *, ls) {
        ...
        case LS_IPV4:
            /* Skip if we already seen a v4. If direct_conn is true, we skip
             * this block because fascist_firewall_choose_address_ls() will set
             * ap. If direct_conn is false, set ap to the first IPv4 address
             * and port in the link specifiers.*/
