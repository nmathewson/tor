...
time_t
hs_lookup_last_hid_serv_request(routerstatus_t *hs_dir, const char *req_key_str,
                                time_t now, int set)
{
    ...
    SMARTLIST_FOREACH_BEGIN(lspecs, const link_specifier_t *, ls)
    {
        switch (link_specifier_get_ls_type(ls)) {
            case LS_IPV4:
                 ...
                 * this block because fascist_firewall_choose_address_ls() will
                 * set ap. If direct_conn is false, set ap to the first IPv4
                 * address and port in the link specifiers.*/
            ...
            case LS_LEGACY_ID:
                ...
                if (link_specifier_getlen_un_legacy_id(ls) <
                    sizeof(legacy_id)) {
                    break;
                }
