...
static const char *str_ed25519_basepoint = "(15112221349535400772501151409588531511"
 ...
 * next hsdir_index. */
static int
compare_digest_to_store_first_hsdir_index(const void *_key, const void **_member)
{
    const char *key = _key;
    const node_t *node = *_member;
 ...
 * next hsdir_index. */
static int
compare_digest_to_store_second_hsdir_index(const void *_key, const void **_member)
{
    const char *key = _key;
    const node_t *node = *_member;
...
compare_node_fetch_hsdir_index(const void **a, const void **b)
{
    ...
    return tor_memcmp(node1->hsdir_index.fetch, node2->hsdir_index.fetch, DIGEST256_LEN);
}
...
compare_node_store_first_hsdir_index(const void **a, const void **b)
{
    const node_t *node1 = *a;
    const node_t *node2 = *b;
    return tor_memcmp(node1->hsdir_index.store_first, node2->hsdir_index.store_first,
                      DIGEST256_LEN);
}
...
compare_node_store_second_hsdir_index(const void **a, const void **b)
{
    const node_t *node1 = *a;
    const node_t *node2 = *b;
    return tor_memcmp(node1->hsdir_index.store_second, node2->hsdir_index.store_second,
                      DIGEST256_LEN);
}
...
hs_check_service_private_dir(const char *username, const char *path,
                             unsigned int dir_group_readable, unsigned int create)
{
...
{
    return networkstatus_get_param(NULL, "hs_service_max_rdv_failures", MAX_REND_FAILURES_DEFAULT,
...
get_time_period_length(void)
{
    ...
    int32_t time_period_length =
        networkstatus_get_param(NULL, "hsdir_interval", HS_TIME_PERIOD_LENGTH_DEFAULT,
...
rend_data_dup(const rend_data_t *data)
{
    ...
    switch (data->version) {
    case HS_VERSION_TWO: {
        rend_data_v2_t *v2_data = tor_memdup(TO_REND_DATA_V2(data), sizeof(*v2_data));
...
compute_desc_id(rend_data_t *rend_data)
{
    ...
    case HS_VERSION_TWO: {
        ...
        for (replica = 0; replica < ARRAY_LENGTH(v2_data->descriptor_id); replica++) {
            ret = rend_compute_v2_desc_id(v2_data->descriptor_id[replica], v2_data->onion_address,
 ...
 * rend_data_t. */
rend_data_t *
rend_data_service_create(const char *onion_address, const char *pk_digest, const uint8_t *cookie,
...
rend_data_t *
rend_data_client_create(const char *onion_address, const char *desc_id, const char *cookie,
                        rend_auth_type_t auth_type)
{
    /* Create a rend_data_t object for version 2. */
    rend_data_t *rend_data = rend_data_alloc(HS_VERSION_TWO);
 ...
 * non NULL. */
const char *
rend_data_get_desc_id(const rend_data_t *rend_data, uint8_t replica, size_t *len_out)
{
    tor_assert(rend_data);
...
compute_disaster_srv(uint64_t time_period_num, uint8_t *srv_out)
{
    ...
    crypto_digest_add_bytes(digest, HS_SRV_DISASTER_PREFIX, HS_SRV_DISASTER_PREFIX_LEN);
...
static void
build_blinded_key_param(const ed25519_public_key_t *pubkey, const uint8_t *secret,
                        size_t secret_len, uint64_t period_num, uint64_t period_length,
                        uint8_t *param_out)
{
    size_t offset = 0;
    ...
    crypto_digest_add_bytes(digest, str_ed25519_basepoint, strlen(str_ed25519_basepoint));
...
static void
build_hs_checksum(const ed25519_public_key_t *key, uint8_t version, uint8_t *checksum_out)
{
    size_t offset = 0;
...
static void
build_hs_address(const ed25519_public_key_t *key, const uint8_t *checksum, uint8_t version,
                 char *addr_out)
{
    size_t offset = 0;
...
static void
hs_parse_address_impl(const char *address, ed25519_public_key_t *key_out, uint8_t *checksum_out,
                      uint8_t *version_out)
{
    size_t offset = 0;
...
hs_get_subcredential(const ed25519_public_key_t *identity_pk,
                     const ed25519_public_key_t *blinded_pk, hs_subcredential_t *subcred_out)
{
    ...
    crypto_digest_add_bytes(digest, (const char *)identity_pk->pubkey, ED25519_PUBKEY_LEN);
    ...
    digest = crypto_digest256_new(DIGEST_SHA3_256);
    crypto_digest_add_bytes(digest, HS_SUBCREDENTIAL_PREFIX, HS_SUBCREDENTIAL_PREFIX_LEN);
    crypto_digest_add_bytes(digest, (const char *)credential, sizeof(credential));
    crypto_digest_add_bytes(digest, (const char *)blinded_pk->pubkey, ED25519_PUBKEY_LEN);
    crypto_digest_get_digest(digest, (char *)subcred_out->subcred, SUBCRED_LEN);
    crypto_digest_free(digest);
...
int
hs_parse_address_no_log(const char *address, ed25519_public_key_t *key_out, uint8_t *checksum_out,
                        uint8_t *version_out, const char **errmsg)
{
 ...
 * failure. */
int
hs_parse_address(const char *address, ed25519_public_key_t *key_out, uint8_t *checksum_out,
                 uint8_t *version_out)
{
    const char *errmsg = NULL;
    int ret = hs_parse_address_no_log(address, key_out, checksum_out, version_out, &errmsg);
    if (ret < 0) {
        log_warn(LD_REND, "Service address %s failed to be parsed: %s", escaped_safe_str(address),
                 errmsg);
    }
    return ret;
}
...
hs_address_is_valid(const char *address)
{
    ...
    if (tor_memcmp(checksum, target_checksum, sizeof(checksum))) {
        log_warn(LD_REND, "Service address %s invalid checksum.", escaped_safe_str(address));
        goto invalid;
    }
    ...
    if (ed25519_validate_pubkey(&service_pubkey) < 0) {
        log_warn(LD_REND, "Service address %s has bad pubkey .", escaped_safe_str(address));
        goto invalid;
    }
 ...
 * */
void
hs_build_address(const ed25519_public_key_t *key, uint8_t version, char *addr_out)
{
    ...
    base32_encode(addr_out, HS_SERVICE_ADDR_LEN_BASE32 + 1, address, sizeof(address));
 ...
 * key of the service. */
void
hs_build_blinded_pubkey(const ed25519_public_key_t *pk, const uint8_t *secret, size_t secret_len,
                        uint64_t time_period_num, ed25519_public_key_t *blinded_pk_out)
{
    ...
    build_blinded_key_param(pk, secret, secret_len, time_period_num, get_time_period_length(),
                            param);
...
void
hs_build_blinded_keypair(const ed25519_keypair_t *kp, const uint8_t *secret, size_t secret_len,
                         uint64_t time_period_num, ed25519_keypair_t *blinded_kp_out)
{
    /* Our blinding key API requires a 32 bytes parameter. */
    uint8_t param[DIGEST256_LEN];
...
hs_service_requires_uptime_circ(const smartlist_t *ports)
{
    ...
    SMARTLIST_FOREACH_BEGIN (ports, rend_service_port_config_t *, p) {
        if (smartlist_contains_int_as_string(get_options()->LongLivedPorts, p->virtual_port)) {
            return 1;
        }
    } SMARTLIST_FOREACH_END (p);
    return 0;
...
void
hs_build_hs_index(uint64_t replica, const ed25519_public_key_t *blinded_pk, uint64_t period_num,
...
void
hs_build_hsdir_index(const ed25519_public_key_t *identity_pk, const uint8_t *srv_value,
                     uint64_t period_num, uint8_t *hsdir_index_out)
{
    crypto_digest_t *digest;
...
hs_get_hsdir_n_replicas(void)
{
    ...
    return networkstatus_get_param(NULL, "hsdir_n_replicas", HS_DEFAULT_HSDIR_N_REPLICAS, 1, 16);
}
...
hs_get_hsdir_spread_fetch(void)
{
    ...
    return networkstatus_get_param(NULL, "hsdir_spread_fetch", HS_DEFAULT_HSDIR_SPREAD_FETCH, 1,
                                   128);
}
...
{
    /* The [1,128] range is a specification requirement. */
    return networkstatus_get_param(NULL, "hsdir_spread_store", HS_DEFAULT_HSDIR_SPREAD_STORE, 1,
                                   128);
}
...
node_has_hsdir_index(const node_t *node)
{
    ...
    if (BUG(fast_mem_is_zero((const char *)node->hsdir_index.fetch, DIGEST256_LEN))) {
        return 0;
    }
    if (BUG(fast_mem_is_zero((const char *)node->hsdir_index.store_first, DIGEST256_LEN))) {
        return 0;
    }
    if (BUG(fast_mem_is_zero((const char *)node->hsdir_index.store_second, DIGEST256_LEN))) {
        return 0;
    }
...
void
hs_get_responsible_hsdirs(const ed25519_public_key_t *blinded_pk, uint64_t time_period_num,
                          ...
                          smartlist_t *responsible_dirs)
{
    ...
    for (int replica = 1; replica <= hs_get_hsdir_n_replicas(); replica++) {
        ...
        int n_to_add = (for_fetching) ? hs_get_hsdir_spread_fetch() : hs_get_hsdir_spread_store();
        ...
        start = idx = smartlist_bsearch_idx(sorted_nodes, hs_index, cmp_fct, &found);
...
time_t
hs_lookup_last_hid_serv_request(routerstatus_t *hs_dir, const char *req_key_str, time_t now,
                                int set)
{
    ...
    /* Create the key */
    base32_encode(hsdir_id_base32, sizeof(hsdir_id_base32), hs_dir->identity_digest, DIGEST_LEN);
    ...
    if (set) {
        ...
        *last_request_ptr = now;
        oldptr = strmap_set(last_hid_serv_requests, hsdir_desc_comb_id, last_request_ptr);
        tor_free(oldptr);
    } else {
...
hs_clean_last_hid_serv_requests(time_t now)
{
    ...
    strmap_t *last_hid_serv_requests = get_last_hid_serv_requests();
    for (iter = strmap_iter_init(last_hid_serv_requests); !strmap_iter_done(iter);) {
        const char *key;
        void *val;
        time_t *ent;
...
hs_purge_hid_serv_from_last_hid_serv_requests(const char *req_key_str)
{
    ...
    for (iter = strmap_iter_init(last_hid_serv_requests); !strmap_iter_done(iter);) {
        ...
        if (tor_memeq(key + REND_DESC_ID_V2_LEN_BASE32, req_key_str, strlen(req_key_str))) {
            iter = strmap_iter_next_rmv(last_hid_serv_requests, iter);
            tor_free(val);
        } else {
            iter = strmap_iter_next(last_hid_serv_requests, iter);
...
routerstatus_t *
hs_pick_hsdir(smartlist_t *responsible_dirs, const char *req_key_str, bool *is_rate_limited_out)
{
    ...
    excluded_some = smartlist_len(usable_responsible_dirs) < smartlist_len(responsible_dirs);
 ...
 */
extend_info_t *
hs_get_extend_info_from_lspecs(const smartlist_t *lspecs, const curve25519_public_key_t *onion_key,
                               int direct_conn)
{
    ...
    SMARTLIST_FOREACH_BEGIN (lspecs, const link_specifier_t *, ls) {
        ...
        case LS_LEGACY_ID:
            ...
            memcpy(legacy_id, link_specifier_getconstarray_un_legacy_id(ls), sizeof(legacy_id));
            have_legacy_id = 1;
            break;
        case LS_ED25519_ID:
    ...
    info = extend_info_new(NULL, legacy_id, (have_ed25519_id) ? &ed25519_pk : NULL, NULL, onion_key,
                           &ap.addr, ap.port);
done:
    return info;
}
...
link_specifier_dup(const link_specifier_t *src)
{
    ...
    ssize_t encoded_len_data = link_specifier_encode(buf, encoded_len_alloc, src);
