...
get_onion_public_key(const char *value, ed25519_public_key_t *pkey_out)
{
     ...
     * strlcpy() in order to accomodate the NUL byte and sizeof() counts the
     * NUL byte so we need to remove them from the equation. */
                       ...
                       hs_subcredential_t **subcredentials_out)
{
     ...
     * instance descriptor plus three for the onionbalance frontend service:
     * the previous one (-1), the current one (0) and the next one (1) for each
     ...
     *
     * As a slight optimization, statistically, the current time period (0)
     * will be the one to work first so we'll put them first in the array to
    ...
    log_info(LD_REND, "Refreshing %u onionbalance keys (TP #%d).", idx,
             (int)tp);
...
hs_ob_refresh_keys(hs_service_t *service)
{
     ...
     * own descriptors which should always include the latest subcredentials
