...
static const config_var_t config_vars[] = {V(MasterOnionAddress, LINELIST, NULL),
...
ob_option_parse(hs_service_config_t *config, const ob_options_t *opts)
{
    ...
    for (line = opts->MasterOnionAddress; line; line = line->next) {
        ...
        if (!get_onion_public_key(line->value, pubkey)) {
            log_warn(LD_REND, "OnionBalance: MasterOnionAddress %s is invalid", line->value);
            tor_free(pubkey);
            goto end;
        }
        smartlist_add(config->ob_master_pubkeys, pubkey);
        log_notice(LD_REND, "OnionBalance: MasterOnionAddress %s registered", line->value);
    }
    /* Success. */
    ret = 1;
...
hs_ob_parse_config_file(hs_service_config_t *config)
{
    ...
    if (!content) {
        log_warn(LD_FS, "OnionBalance: Unable to read config file %s", escaped(config_file_path));
        goto end;
    }
...
STATIC size_t
compute_subcredentials(const hs_service_t *service, hs_subcredential_t **subcredentials_out)
{
    ...
    subcreds = tor_calloc((num_steps * num_pkeys) + 2, sizeof(hs_subcredential_t));

    /* For each master pubkey we add 3 subcredentials: */
    for (unsigned int i = 0; i < num_steps; i++) {
        SMARTLIST_FOREACH_BEGIN (service->config.ob_master_pubkeys, const ed25519_public_key_t *,
                                 pkey) {
    ...
    memcpy(&subcreds[idx++], &service->desc_next->desc->subcredential, sizeof(hs_subcredential_t));
