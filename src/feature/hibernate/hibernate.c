...
accounting_parse_options(const or_options_t *options, int validate_only)
{
    ...
    if (smartlist_len(items) != (idx + 1)) {
        log_warn(LD_CONFIG, "Accounting unit '%s' requires %d argument%s.", s,
                 idx, (idx > 1) ? "s" : "");
        goto err;
    }
...
configure_accounting(time_t now)
{
    ...
    if (!interval_start_time) {
        ...
        if (-0.50 <= delta && delta <= 0.50) {
             ...
             * remembered.  That's fine; we might lose some bytes we could
             * otherwise have written, but better to err on the side of obeying
             * accounting settings. */
            log_info(LD_ACCT,
...
#define MIN_TIME_FOR_MEASUREMENT (1800)
    ...
    if (soft_limit_hit_at > interval_start_time && n_bytes_at_soft_limit &&
         ...
         * considering the entirety of the last interval, since we likely
         * started using bytes very slowly once we hit our soft limit. */
        expected =
         ...
         * never hit our soft limit, or we're using a state file from a Tor
         * that doesn't know to store soft-limit info.  Just take rate at which
...
accounting_set_wakeup_time(void)
{
    ...
    if (time_to_consider <= 0) {
        ...
        /* XXX can we simplify this just by picking a random
         * (non-deterministic) time to be up? If we go down and come up, then
         * we pick a new one. Is that good enough? -RD */

        /* This is not a perfectly unbiased conversion, but it is good enough:
         * in the worst case, the first half of the day is 0.06 percent
         * likelier to be chosen than the last half. */
...
read_bandwidth_usage(void)
{
    ...
    /* Older versions of Tor (before 0.2.2.17-alpha or so) didn't generate
     * these fields. If you switch back and forth, you might get an
...

    {
        ...
        log_info(
            LD_ACCT,
            "Successfully read bandwidth accounting info from state written "
            "at %s "
            "for interval starting at %s.  We have been active for %lu "
            "seconds in "
...
#define SOFT_LIM_MINUTES (3 * 60)
     ...
     * We want to stop accepting connections when ALL of the following are
     * true:
    ...
    if (expected_bandwidth_usage) {
        ...
        if (acct_max > expected_usage &&
            acct_max - expected_usage > soft_limit)
...
hibernate_begin(hibernate_state_t new_state, time_t now)
{
    ...
    if (new_state == HIBERNATE_STATE_EXITING) {
...
#ifdef HAVE_SYSTEMD
        /* tell systemd that we may need more than the default 90 seconds to
         * shut down so they don't kill us. add some extra time to actually
         * finish shutting down, otherwise systemd will kill us immediately
         * after the EXTEND_TIMEOUT_USEC expires. this is an *upper* limit; tor
         * will probably only take one or two more seconds, but assume that
         * maybe we got swapped out and it takes a little while longer.
         *
         * as of writing, this is a no-op with all-defaults: ShutdownWaitLength
         * is 30 seconds, so this will extend the timeout to 60 seconds.
         ...
         * ShutdownWaitLength to more than that, but use a longer type so we
         * don't need to think about UB on overflow
         */
...
hibernate_go_dormant(time_t now)
{
    ...
    while ((conn = connection_get_by_type(CONN_TYPE_OR)) ||
        ...
        if (CONN_IS_EDGE(conn)) {
            connection_edge_end(TO_EDGE_CONN(conn),
...
hibernate_schedule_wakeup_event(time_t now, time_t end_time)
{
    ...
    if (!wakeup_event) {
        wakeup_event =
            mainloop_event_postloop_new(wakeup_event_callback, NULL);
    }
...
consider_hibernation(time_t now)
{
    ...
    if (hibernate_state == HIBERNATE_STATE_DORMANT) {
        ...
        if (hibernate_end_time > now && accounting_enabled) {
            /* If we're hibernating, don't wake up until it's time, regardless
             * of whether we're in a new interval. */
            return;
        } else {
    ...
    /* Else, we aren't hibernating. See if it's time to start hibernating, or
     * to go dormant. */
    ...
    if (hibernate_state == HIBERNATE_STATE_LOWBANDWIDTH) {
        if (!accounting_enabled) {
            ...
            /* The hibernation period ended while we were still in
             * lowbandwidth.*/
            hibernate_end_time_elapsed(now);
        }
    }
    ...
    if (!strcmp(question, "accounting/enabled")) {
        ...
        *answer = tor_strdup(hibernate_state_to_string(hibernate_state));
        ...
        if (get_options()->AccountingRule == ACCT_SUM) {
            ...
            tor_asprintf(answer, "%" PRIu64 " %" PRIu64, (total_left),
                         (total_left));
        } else if (get_options()->AccountingRule == ACCT_IN) {
            ...
            tor_asprintf(answer, "%" PRIu64 " %" PRIu64, (limit),
                         (write_left));
        } else {
            ...
            if (n_bytes_written_in_interval < limit)
                write_left = limit - n_bytes_written_in_interval;
            tor_asprintf(answer, "%" PRIu64 " %" PRIu64, (read_left),
                         (write_left));
        }
