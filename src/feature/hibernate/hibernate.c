...
accounting_parse_options(const or_options_t *options, int validate_only)
{
    ...
    if (!cp || *cp != ':') {
        log_warn(LD_CONFIG, "Accounting start time not parseable: not in HH:MM format");
        goto err;
    }
...
edge_of_accounting_period_containing(time_t now, int get_end)
{
    ...
    before =
        tm.tm_hour < cfg_start_hour || (tm.tm_hour == cfg_start_hour && tm.tm_min < cfg_start_min);
    ...
    case UNIT_MONTH: {
        ...
        if (tm.tm_mday < cfg_start_day || (tm.tm_mday == cfg_start_day && before)) {
            --tm.tm_mon;
        }
...
update_expected_bandwidth(void)
{
    ...
    uint64_t max_configured =
        (options->RelayBandwidthRate > 0 ? options->RelayBandwidthRate : options->BandwidthRate) *
        60;
...
#define MIN_TIME_FOR_MEASUREMENT (1800)
    ...
    if (soft_limit_hit_at > interval_start_time && n_bytes_at_soft_limit &&
        ...
        expected = n_bytes_at_soft_limit / (soft_limit_hit_at - interval_start_time);
        expected /= 60;
...
accounting_set_wakeup_time(void)
{
    ...
    time_to_exhaust_bw = (get_options()->AccountingMax / expected_bandwidth_usage) * 60;
    if (time_to_exhaust_bw > INT_MAX) {
        time_to_exhaust_bw = INT_MAX;
        time_to_consider = 0;
    } else {
        time_to_consider = accounting_get_interval_length() - (int)time_to_exhaust_bw;
    }
    ...
    if (time_to_consider <= 0) {
        ...
        interval_wakeup_time = interval_start_time + (get_uint32(digest) % time_to_consider);
    }
...
accounting_record_bandwidth_usage(time_t now, or_state_t *state)
{
    ...
    state->AccountingBytesWrittenInInterval = ROUND_UP(n_bytes_written_in_interval);
    ...
    or_state_mark_dirty(state, now + (get_options()->AvoidDiskWrites ? 7200 : 60));
...
read_bandwidth_usage(void)
{
...

    {
        ...
        log_info(LD_ACCT,
                 ...
                 (unsigned long)(expected_bandwidth_usage * 1024 / 60), (n_bytes_read_in_interval),
...
#define SOFT_LIM_MINUTES (3 * 60)
    ...
    if (expected_bandwidth_usage) {
        const uint64_t expected_usage = expected_bandwidth_usage * SOFT_LIM_MINUTES;
...
hibernate_begin(hibernate_state_t new_state, time_t now)
{
    ...
    if (new_state == HIBERNATE_STATE_EXITING && hibernate_state != HIBERNATE_STATE_LIVE) {
        log_notice(LD_GENERAL, "SIGINT received %s; exiting now.",
                   hibernate_state == HIBERNATE_STATE_EXITING ? "a second time"
                                                              : "while hibernating");
        tor_shutdown_event_loop_and_exit(0);
        return;
    }

    if (new_state == HIBERNATE_STATE_LOWBANDWIDTH && hibernate_state == HIBERNATE_STATE_LIVE) {
    ...
    or_state_mark_dirty(get_or_state(), get_options()->AvoidDiskWrites ? now + 600 : 0);
}
...
consider_hibernation(time_t now)
{
     ...
     * go dormant. */
    if (hibernate_state == HIBERNATE_STATE_LIVE || hibernate_state == HIBERNATE_STATE_INITIAL) {
        if (hibernate_soft_limit_reached()) {
            log_notice(LD_ACCT, "Bandwidth soft limit reached; commencing hibernation. "
                                "No new connections will be accepted");
            hibernate_begin(HIBERNATE_STATE_LOWBANDWIDTH, now);
        } else if (accounting_enabled && now < interval_wakeup_time) {
            format_local_iso_time(buf, interval_wakeup_time);
            log_notice(LD_ACCT, "Commencing hibernation. We will wake up at %s local time.", buf);
            hibernate_go_dormant(now);
 ...
 * NULL. */
int
getinfo_helper_accounting(control_connection_t *conn, const char *question, char **answer,
                          const char **errmsg)
{
    (void)conn;
    (void)errmsg;
