...
accounting_parse_options(const or_options_t *options, int validate_only)
{
    ...
    switch (unit) {
        ...
        case UNIT_DAY:
            d = 0;
            break;
            /* Coverity dislikes unreachable default cases; some compilers warn
             * on
...
edge_of_accounting_period_containing(time_t now, int get_end)
{
    ...
    switch (cfg_unit) {
        ...
        case UNIT_WEEK: {
            ...
            /* If we are on the right day, but the changeover hasn't happened
             * yet, then subtract a whole week. */
            if (delta == 0 && before)
                delta = 7;
            tm.tm_mday -= delta;
            if (get_end)
...
read_bandwidth_usage(void)
{
...

    {
        ...
        log_info(
            LD_ACCT,
            "Successfully read bandwidth accounting info from state written at "
            "%s "
            "for interval starting at %s.  We have been active for %lu seconds "
            "in "
...
hibernate_begin(hibernate_state_t new_state, time_t now)
{
    ...
    if (new_state == HIBERNATE_STATE_EXITING) {
         ...
         * is 30 seconds, so this will extend the timeout to 60 seconds. default
         * systemd DefaultTimeoutStopSec is 90 seconds, so systemd will wait (up
         * to) 90 seconds anyways.
         *
