...
static int control_setconf_helper(control_connection_t *conn, const control_cmd_args_t *args,
                                  int use_defaults);
 ...
 **/
static int
kvline_check_keyword_args(const control_cmd_args_t *result, const control_cmd_syntax_t *syntax,
                          char **error_out)
{
    ...
    for (line = result->kwargs; line; line = line->next) {
        if (!string_array_contains_keyword(syntax->allowed_keywords, line->key)) {
            tor_asprintf(error_out, "Unrecognized keyword argument %s", escaped(line->key));
            return -1;
        }
    }
...
control_cmd_parse_args(const char *command, const control_cmd_syntax_t *syntax, size_t body_len,
                       const char *body, char **error_out)
{
    *error_out = NULL;
    ...
    if (syntax->want_cmddata) {
        ...
        ++eol;
        result->cmddata_len = read_escaped_data(eol, (body + body_len) - eol, &result->cmddata);
    } else {
        if (eol && (eol + 1) != body + body_len) {
    ...
    smartlist_split_string(result->args, cmdline, " ", SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK,
    ...
    if (n_args < syntax->min_args) {
        ...
        tor_asprintf(error_out, "Cannot accept more than %u argument(s)", syntax->max_args);
        goto err;
    }
...
static int
handle_control_setconf(control_connection_t *conn, const control_cmd_args_t *args)
{
 ...
 * Modifies the contents of body. */
static int
handle_control_resetconf(control_connection_t *conn, const control_cmd_args_t *args)
{
...
static int
handle_control_getconf(control_connection_t *conn, const control_cmd_args_t *args)
{
    ...
    SMARTLIST_FOREACH_BEGIN (questions, const char *, q) {
        if (!option_is_recognized(q)) {
            control_reply_add_printf(unrecognized, 552, "Unrecognized configuration key \"%s\"", q);
        } else {
            ...
            while (answer) {
                config_line_t *next;
                control_reply_add_one_kv(answers, 250, KV_RAW, answer->key, answer->value);
                next = answer->next;
                tor_free(answer->key);
...
static int
handle_control_loadconf(control_connection_t *conn, const control_cmd_args_t *args)
{
    setopt_err_t retval;
    char *errstring = NULL;

    retval = options_init_from_string(NULL, args->cmddata, CMD_RUN_TOR, NULL, &errstring);

    if (retval != SETOPT_OK)
        log_warn(LD_CONTROL, "Controller gave us config file that didn't validate: %s", errstring);
...
static int
handle_control_setevents(control_connection_t *conn, const control_cmd_args_t *args)
{
    int event_code;
...
static int
handle_control_saveconf(control_connection_t *conn, const control_cmd_args_t *args)
{
    bool force = config_lines_contain_flag(args->kwargs, "FORCE");
    const or_options_t *options = get_options();
    if ((!force && options->IncludeUsed) || options_save_current() < 0) {
        control_write_endreply(conn, 551, "Unable to write configuration to disk.");
    } else {
        send_control_done(conn);
    }
    return 0;
 ...
 * may not be reported.) */
static int
handle_control_signal(control_connection_t *conn, const control_cmd_args_t *args)
{
    int sig = -1;
    int i;
 ...
 * closes. */
static int
handle_control_takeownership(control_connection_t *conn, const control_cmd_args_t *args)
{
    (void)args;
 ...
 * closes. */
static int
handle_control_dropownership(control_connection_t *conn, const control_cmd_args_t *args)
{
    (void)args;
 ...
 */
static int
control_setconf_helper(control_connection_t *conn, const control_cmd_args_t *args, int use_defaults)
{
    setopt_err_t opt_err;
    char *errstring = NULL;
    const unsigned flags = CAL_CLEAR_FIRST | (use_defaults ? CAL_USE_DEFAULTS : 0);
...
#undef SEND_ERRMSG
        log_warn(LD_CONTROL, "Controller gave us config lines that didn't validate: %s", errstring);
        config_free_lines(lines);
        tor_free(errstring);
        return 0;
    }
...
static int
handle_control_mapaddress(control_connection_t *conn, const control_cmd_args_t *args)
{
    smartlist_t *reply;
    char *r;
    size_t sz;
    ...
    for (line = args->kwargs; line; line = line->next) {
        ...
        const char *to = line->value;
        {
            if (address_is_invalid_mapaddress_target(to)) {
                smartlist_add_asprintf(reply, "512-syntax error: invalid address '%s'", to);
                log_warn(LD_CONTROL, "Skipping invalid argument '%s' in MapAddress msg", to);
            } else if (!strcmp(from, ".") || !strcmp(from, "0.0.0.0") || !strcmp(from, "::")) {
                const char type =
                    !strcmp(from, ".")
                        ? RESOLVED_TYPE_HOSTNAME
                        : (!strcmp(from, "0.0.0.0") ? RESOLVED_TYPE_IPV4 : RESOLVED_TYPE_IPV6);
                const char *address = addressmap_register_virtual_address(type, tor_strdup(to));
                if (!address) {
                    smartlist_add_asprintf(reply, "451-resource exhausted: skipping '%s=%s'", from,
                                           to);
                    log_warn(LD_CONTROL, "Unable to allocate address for '%s' in MapAddress msg",
                             safe_str_client(to));
                } else {
                ...
                const char *msg;
                if (addressmap_register_auto(from, to, 1, ADDRMAPSRC_CONTROLLER, &msg) < 0) {
                    smartlist_add_asprintf(reply,
                                           "512-syntax error: invalid address mapping "
                                           " '%s=%s': %s",
                                           from, to, msg);
                    log_warn(LD_CONTROL, "Skipping invalid argument '%s=%s' in MapAddress msg: %s",
                             from, to, msg);
                } else {
...
static const control_cmd_syntax_t extendcircuit_syntax = {.min_args = 1,
...
static int
handle_control_extendcircuit(control_connection_t *conn, const control_cmd_args_t *args)
{
    ...
    */
    ...
    if (purpose_line) {
        ...
        if (intended_purpose == CIRCUIT_PURPOSE_UNKNOWN) {
            control_printf_endreply(conn, 552, "Unknown purpose \"%s\"", purpose_line->value);
            goto done;
        }
    }
    ...
    SMARTLIST_FOREACH(nodes, const node_t *, node, {
        ...
        if (!info) {
            ...
            log_warn(LD_CONTROL, "controller tried to connect to a node that lacks a suitable "
    ...
    if (zero_circ) {
        ...
        if (circ->base_.state == CIRCUIT_STATE_OPEN ||
            ...
            if ((err_reason = circuit_send_next_onion_skin(circ)) < 0) {
                log_info(LD_CONTROL, "send_next_onion_skin failed; circuit marked for closing.");
                circuit_mark_for_close(TO_CIRCUIT(circ), -err_reason);
    ...
    control_printf_endreply(conn, 250, "EXTENDED %lu", (unsigned long)circ->global_identifier);
...
static int
handle_control_setcircuitpurpose(control_connection_t *conn, const control_cmd_args_t *args)
{
    origin_circuit_t *circ = NULL;
    uint8_t new_purpose;
    const char *circ_id = smartlist_get(args->args, 0);
...

    {
        ...
        if (new_purpose == CIRCUIT_PURPOSE_UNKNOWN) {
            control_printf_endreply(conn, 552, "Unknown purpose \"%s\"", purp->value);
            goto done;
        }
    }
...
static const control_cmd_syntax_t attachstream_syntax = {.min_args = 2,
                                                         .max_args = 2,
                                                         .accept_keywords = true,
...
static int
handle_control_attachstream(control_connection_t *conn, const control_cmd_args_t *args)
{
    ...
    if (!(ap_conn = get_stream(stream_id))) {
        ...
        if (hoparg) {
            hop = (int)tor_parse_ulong(hoparg->value, 10, 0, INT_MAX, &hop_line_ok, NULL);
    ...
    if (ENTRY_TO_CONN(ap_conn)->state != AP_CONN_STATE_CONTROLLER_WAIT &&
        ...
        control_write_endreply(conn, 555, "Connection is not managed by controller.");
        return 0;
    }
    ...
    if (circ && (circ->base_.state != CIRCUIT_STATE_OPEN)) {
        control_write_endreply(conn, 551, "Can't attach stream to non-open origin circuit");
        return 0;
    }
    /* Is this a single hop circuit? */
    if (circ && (circuit_get_cpath_len(circ) < 2 || hop == 1)) {
        control_write_endreply(conn, 551, "Can't attach stream to this one-hop circuit.");
        return 0;
    }
...
static int
handle_control_postdescriptor(control_connection_t *conn, const control_cmd_args_t *args)
{
    const char *msg = NULL;
    ...
    if (line) {
        ...
        if (purpose == ROUTER_PURPOSE_UNKNOWN) {
            control_printf_endreply(conn, 552, "Unknown purpose \"%s\"", line->value);
            goto done;
        }
    }
    ...
    if (line) {
        if (!strcasecmp(line->value, "no"))
            ...
            control_printf_endreply(conn, 552, "Unknown cache request \"%s\"", line->value);
            goto done;
        }
    }
...
static int
handle_control_redirectstream(control_connection_t *conn, const control_cmd_args_t *cmd_args)
{
    entry_connection_t *ap_conn = NULL;
    char *new_addr = NULL;
    uint16_t new_port = 0;
    ...
    if (!(ap_conn = get_stream(smartlist_get(args, 0))) || !ap_conn->socks_request) {
        control_printf_endreply(conn, 552, "Unknown stream \"%s\"", (char *)smartlist_get(args, 0));
    } else {
        int ok = 1;
        if (smartlist_len(args) > 2) { /* they included a port too */
            new_port = (uint16_t)tor_parse_ulong(smartlist_get(args, 2), 10, 1, 65535, &ok, NULL);
        }
        if (!ok) {
    ...
    strlcpy(ap_conn->socks_request->address, new_addr, sizeof(ap_conn->socks_request->address));
    if (new_port)
...
static const control_cmd_syntax_t closestream_syntax = {
    .min_args = 2, .max_args = UINT_MAX, /* XXXX This is the original behavior, but
 ...
 * and report success or failure. */
static int
handle_control_closestream(control_connection_t *conn, const control_cmd_args_t *cmd_args)
{
    entry_connection_t *ap_conn = NULL;
    uint8_t reason = 0;
    int ok;
    ...
    if (!(ap_conn = get_stream(smartlist_get(args, 0))))
        ...
        reason = (uint8_t)tor_parse_ulong(smartlist_get(args, 1), 10, 0, 255, &ok, NULL);
        if (!ok) {
...
static const control_cmd_syntax_t closecircuit_syntax = {
    .min_args = 1, .max_args = 1, .accept_keywords = true, .kvline_flags = KV_OMIT_VALS,
 ...
 * and report success or failure. */
static int
handle_control_closecircuit(control_connection_t *conn, const control_cmd_args_t *args)
{
    const char *circ_id = smartlist_get(args->args, 0);
    origin_circuit_t *circ = NULL;
...
static int
handle_control_resolve(control_connection_t *conn, const control_cmd_args_t *args)
{
    smartlist_t *failed;
    int is_reverse = 0;
...
get_authmethods(const or_options_t *options)
{
    ...
    char *methods;
    int passwd =
        (options->HashedControlPassword != NULL || options->HashedControlSessionPassword != NULL);
...
static int
handle_control_protocolinfo(control_connection_t *conn, const control_cmd_args_t *cmd_args)
{
...
static int
handle_control_usefeature(control_connection_t *conn, const control_cmd_args_t *cmd_args)
{
    const smartlist_t *args = cmd_args->args;
    int bad = 0;
...
static int
handle_control_dropguards(control_connection_t *conn, const control_cmd_args_t *args)
{
...
static int
handle_control_hsfetch(control_connection_t *conn, const control_cmd_args_t *args)

{
    ...
    } else if (strcmpstart(arg1, v2_str) == 0 && rend_valid_descriptor_id(arg1 + v2_str_len) &&
    ...
    if (version == HS_VERSION_TWO) {
        rend_query = rend_data_client_create(hsaddress, desc_id, NULL, REND_NO_AUTH);
        if (rend_query == NULL) {
...
static int
handle_control_hspost(control_connection_t *conn, const control_cmd_args_t *args)
{
    ...
    if (!rend_parse_v2_service_descriptor(&parsed, desc->desc_id, &intro_content, &intro_size,
                                          &encoded_size, &next_desc, desc->desc_str, 1)) {
        /* Post the descriptor. */
...
add_onion_helper_add_service(int hs_version, add_onion_secret_key_t *pk, smartlist_t *port_cfgs,
                             int max_streams, int max_streams_close_circuit, int auth_type,
                             smartlist_t *auth_clients, char **address_out)
{
    ...
    switch (hs_version) {
    case HS_VERSION_TWO:
        ret = rend_service_add_ephemeral(pk->v2, port_cfgs, max_streams, max_streams_close_circuit,
                                         auth_type, auth_clients, address_out);
        break;
    case HS_VERSION_THREE:
        ret = hs_service_add_ephemeral(pk->v3, port_cfgs, max_streams, max_streams_close_circuit,
                                       address_out);
        break;
    default:
...
static const char *add_onion_keywords[] = {"Port", "Flags", "MaxStreams", "ClientAuth", NULL};
static const control_cmd_syntax_t add_onion_syntax = {
    .min_args = 1, .max_args = 1, .accept_keywords = true, .allowed_keywords = add_onion_keywords};

/** Called when we get a ADD_ONION command; parse the body, and set up
 * the new ephemeral Onion Service. */
static int
handle_control_add_onion(control_connection_t *conn, const control_cmd_args_t *args)
{
    ...
    for (arg = args->kwargs; arg; arg = arg->next) {
        if (!strcasecmp(arg->key, "Port")) {
            ...
            rend_service_port_config_t *cfg = rend_service_parse_port_config(arg->value, ",", NULL);
            if (!cfg) {
    ...
    if (smartlist_len(port_cfgs) == 0) {
        ...
        goto out;
    } else if ((auth_type == REND_BASIC_AUTH && smartlist_len(auth_clients) > 512) ||
               (auth_type == REND_STEALTH_AUTH && smartlist_len(auth_clients) > 16)) {
        control_write_endreply(conn, 512, "Too many auth clients");
        goto out;
    } else if (non_anonymous != rend_service_non_anonymous_mode_enabled(get_options())) {
    ...
    if (add_onion_helper_keyarg(onionkey, discard_pk, &key_new_alg, &key_new_blob, &pk, &hs_version,
                                conn) < 0) {
        goto out;
    }
    ...
    int ret = add_onion_helper_add_service(hs_version, &pk, port_cfgs, max_streams,
                                           max_streams_close_circuit, auth_type, auth_clients,
                                           &service_id);
    ...
    case RSAE_OKAY: {
        ...
        if (key_new_alg) {
            ...
            control_printf_midreply(conn, 250, "PrivateKey=%s:%s", key_new_alg, key_new_blob);
        }
        if (auth_created_clients) {
            SMARTLIST_FOREACH(auth_created_clients, rend_authorized_client_t *, ac, {
                char *encoded = rend_auth_encode_cookie(ac->descriptor_cookie, auth_type);
                tor_assert(encoded);
                control_printf_midreply(conn, 250, "ClientAuth=%s:%s", ac->client_name, encoded);
 ...
 */
STATIC int
add_onion_helper_keyarg(const char *arg, int discard_pk, const char **key_new_alg_out,
                        char **key_new_blob_out, add_onion_secret_key_t *decoded_key,
                        int *hs_version, control_connection_t *conn)
{
    ...
    if (!strcasecmp(key_type_rsa1024, key_type)) {
        ...
        *hs_version = HS_VERSION_TWO;
        ...
        if (base64_decode((char *)sk->seckey, sizeof(sk->seckey), key_blob, strlen(key_blob)) !=
            sizeof(sk->seckey)) {
            tor_free(sk);
        ...
        *hs_version = HS_VERSION_THREE;
        ...
        if (!strcasecmp(key_type_rsa1024, key_blob)) {
            ...
            if (crypto_pk_generate_key(pk)) {
                control_printf_endreply(conn, 551, "Failed to generate %s key", key_type_rsa1024);
                goto err;
            }
            if (!discard_pk) {
                if (crypto_pk_base64_encode_private(pk, &key_new_blob)) {
                    crypto_pk_free(pk);
                    control_printf_endreply(conn, 551, "Failed to encode %s key", key_type_rsa1024);
                    goto err;
                }
            ...
            *hs_version = HS_VERSION_TWO;
            ...
            if (!discard_pk) {
                ...
                if (base64_encode(key_new_blob, len, (const char *)sk->seckey, sizeof(sk->seckey),
                                  0) != (len - 1)) {
                    tor_free(sk);
                    tor_free(key_new_blob);
...
add_onion_helper_clientauth(const char *arg, int *created, control_connection_t *conn)
{
    int ok = 0;
    ...
    rend_authorized_client_t *client = tor_malloc_zero(sizeof(rend_authorized_client_t));
    ...
    if (smartlist_len(auth_args) == 2) {
        ...
        if (rend_auth_decode_cookie(smartlist_get(auth_args, 1), client->descriptor_cookie, NULL,
                                    &decode_err_msg) < 0) {
...
static int
handle_control_del_onion(control_connection_t *conn, const control_cmd_args_t *cmd_args)
{
    int hs_version = 0;
     ...
     * should be returned.
     */
    smartlist_t *services[2] = {conn->ephemeral_onion_services, detached_onion_services};
    ...
    if (onion_services == NULL) {
        ...
        if (ret < 0) {
             ...
             */
            log_warn(LD_BUG, "Failed to remove Onion Service %s.", escaped(service_id));
            tor_fragile_assert();
        }
 ...
 */
static int
handle_control_obsolete(control_connection_t *conn, const control_cmd_args_t *args)
{
    (void)args;
 ...
 */
static int
handle_single_control_command(const control_cmd_def_t *def, control_connection_t *conn,
                              uint32_t cmd_data_len, char *args)
{
    ...
    parsed_args = control_cmd_parse_args(conn->current_cmd, def->syntax, cmd_data_len, args, &err);
    if (!parsed_args) {
        control_printf_endreply(conn, 512, "Bad arguments to %s: %s", conn->current_cmd,
                                err ? err : "");
        tor_free(err);
    } else {
        if (BUG(err))
 ...
 */
int
handle_control_command(control_connection_t *conn, uint32_t cmd_data_len, char *args)
{
    tor_assert(conn);
    tor_assert(args);
    ...
    control_printf_endreply(conn, 510, "Unrecognized command \"%s\"", conn->current_cmd);
