                       ...
                       size_t body_len, const char *body, char **error_out)
{
    *error_out = NULL;
    ...
    if (n_args > syntax->max_args) {
        /* We have extra arguments after the positional arguments, and we
           didn't treat them as an error, so they must count as keyword
           arguments: Either K=V pairs, or flags, or both. */
    ...
    SMARTLIST_FOREACH_BEGIN (questions, const char *, q) {
        if (!option_is_recognized(q)) {
            control_reply_add_printf(
                unrecognized, 552, "Unrecognized configuration key \"%s\"", q);
        } else {
        ...
        const char *to = line->value;
        {
            if (address_is_invalid_mapaddress_target(to)) {
                smartlist_add_asprintf(
                    reply, "512-syntax error: invalid address '%s'", to);
                log_warn(LD_CONTROL,
                ...
                const char type =
                    !strcmp(from, ".")
                        ...
                        : (!strcmp(from, "0.0.0.0") ? RESOLVED_TYPE_IPV4
                                                    : RESOLVED_TYPE_IPV6);
                const char *address =
                    addressmap_register_virtual_address(type, tor_strdup(to));
                if (!address) {
                    smartlist_add_asprintf(
                        reply, "451-resource exhausted: skipping '%s=%s'",
                        from, to);
                    log_warn(LD_CONTROL,
                             "Unable to allocate address for '%s' in "
                             "MapAddress msg",
                             safe_str_client(to));
                } else {
                ...
                const char *msg;
                if (addressmap_register_auto(
                        from, to, 1, ADDRMAPSRC_CONTROLLER, &msg) < 0) {
                    smartlist_add_asprintf(
                        reply,
                        ...
                        from, to, msg);
                    log_warn(LD_CONTROL,
                             "Skipping invalid argument '%s=%s' in MapAddress "
                             "msg: %s",
                             from, to, msg);
                } else {
                             ...
                             const control_cmd_args_t *args)
{
    ...
    */
    ...
    const config_line_t *purpose_line =
        config_line_find_case(kwargs, "PURPOSE");
    ...
    if (zero_circ) {
        ...
        if (circ->base_.state == CIRCUIT_STATE_OPEN ||
            ...
            if ((err_reason = circuit_send_next_onion_skin(circ)) < 0) {
                log_info(LD_CONTROL, "send_next_onion_skin failed; circuit "
                                     "marked for closing.");
                circuit_mark_for_close(TO_CIRCUIT(circ), -err_reason);
                                 ...
                                 const control_cmd_args_t *args)
{
    ...
    {
        const config_line_t *purp =
            config_line_find_case(args->kwargs, "PURPOSE");
        if (!purp) {
                            ...
                            const control_cmd_args_t *args)
{
    ...
    if (circ && hop > 0) {
        ...
        if (!cpath) {
            control_printf_endreply(conn, 551, "Circuit doesn't have %d hops.",
                                    hop);
            return 0;
        }
    }
                       ...
                       const control_cmd_args_t *args)
{
    ...
    {
        const config_line_t *modearg =
            config_line_find_case(args->kwargs, "mode");
    ...
    if (rend_valid_v2_service_id(arg1)) {
        ...
        version = HS_VERSION_TWO;
               ...
               base32_decode(digest, sizeof(digest), arg1 + v2_str_len,
                   ...
                   REND_DESC_ID_V2_LEN_BASE32) {
        /* We have a well formed version 2 descriptor ID. Keep the decoded
         * value of the id. */
        desc_id = digest;
        version = HS_VERSION_TWO;
                      ...
                      const control_cmd_args_t *args)
{
    ...
    /* Handle the v3 case. */
    if (onion_address) {
        if (hs_control_hspost_command(encoded_desc, onion_address, hs_dirs) <
            0) {
    ...
    if (!rend_parse_v2_service_descriptor(
        ...
        if (!rend_get_service_id(parsed->pk, serviceid)) {
             ...
             * because after that 650 event(s) are possible so better to have
             * the 250 OK before them to avoid out of order replies. */
            send_control_done(conn);
                         ...
                         const control_cmd_args_t *args)
{
    ...
    for (arg = args->kwargs; arg; arg = arg->next) {
        if (!strcasecmp(arg->key, "Port")) {
            ...
            int ok = 0;
            max_streams =
                (int)tor_parse_long(arg->value, 10, 0, 65535, &ok, NULL);
            if (!ok) {
             ...
             *   * 'DiscardPK' - If tor generates the keypair, do not include
             * it in the response.
             *   * 'Detach' - Do not tie this onion service to any particular
             * control connection.
             *   * 'MaxStreamsCloseCircuit' - Close the circuit if MaxStreams
             * is exceeded.
             *   * 'BasicAuth' - Client authorization using the 'basic' method.
             *   * 'NonAnonymous' - Add a non-anonymous Single Onion Service.
             * If this flag is present, tor must be in non-anonymous hidden
             * service mode. If this flag is absent, tor must be in anonymous
             * hidden service mode.
             */
            ...
            smartlist_split_string(flags, arg->value, ",", SPLIT_IGNORE_BLANK,
                                   0);
            ...
            if (auth_clients != NULL) {
                int bad = 0;
                SMARTLIST_FOREACH_BEGIN (auth_clients,
                                         rend_authorized_client_t *, ac) {
    ...
    if (smartlist_len(port_cfgs) == 0) {
         ...
         * (I've deliberately written them out in full here to aid
         * searchability.)
         */
        control_printf_endreply(conn, 512,
        ...
        if (auth_created_clients) {
            SMARTLIST_FOREACH(
                auth_created_clients, rend_authorized_client_t *, ac, {
                    char *encoded = rend_auth_encode_cookie(
                        ac->descriptor_cookie, auth_type);
                    tor_assert(encoded);
    ...
    if (!strcasecmp(key_type_rsa1024, key_type)) {
        ...
        *hs_version = HS_VERSION_TWO;
        ...
        if (base64_decode((char *)sk->seckey, sizeof(sk->seckey), key_blob,
            ...
            tor_free(sk);
            control_write_endreply(conn, 512,
                                   "Failed to decode ED25519-V3 key");
            goto err;
        }
        decoded_key->v3 = sk;
        ...
        if (!strcasecmp(key_type_rsa1024, key_blob)) {
            ...
            if (!discard_pk) {
                if (crypto_pk_base64_encode_private(pk, &key_new_blob)) {
                    crypto_pk_free(pk);
                    control_printf_endreply(conn, 551,
                                            "Failed to encode %s key",
                                            key_type_rsa1024);
                    goto err;
                }
                         ...
                         const control_cmd_args_t *cmd_args)
{
    ...
    if (onion_services == NULL) {
        ...
        case HS_VERSION_THREE:
            ...
            break;
        default:
            /* The ret value will be -1 thus hitting the warning below. This
             * should never happen because of the check at the start of the
             * function. */
            break;
        }
        if (ret < 0) {
                       ...
                       char *args)
{
    ...
    for (unsigned i = 0; i < N_CONTROL_COMMANDS; ++i) {
        ...
        if (!strcasecmp(conn->current_cmd, def->name)) {
            return handle_single_control_command(def, conn, cmd_data_len,
                                                 args);
        }
    }
