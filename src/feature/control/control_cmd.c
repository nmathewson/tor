    ...
    *error_out = NULL;
    ...
    if (n_args > syntax->max_args) {
        /* We have extra arguments after the positional arguments, and we didn't
           treat them as an error, so they must count as keyword arguments:
           Either K=V pairs, or flags, or both. */
    ...
    for (line = args->kwargs; line; line = line->next) {
        ...
        const char *to = line->value;
        {
            if (address_is_invalid_mapaddress_target(to)) {
                ...
                if (!address) {
                    smartlist_add_asprintf(
                        reply, "451-resource exhausted: skipping '%s=%s'", from,
                        to);
                    log_warn(
                        LD_CONTROL,
                             ...
                             smartlist_t *auth_clients, char **address_out)
{
    ...
    switch (hs_version) {
        case HS_VERSION_TWO:
            ret = rend_service_add_ephemeral(
                pk->v2, port_cfgs, max_streams, max_streams_close_circuit,
                         ...
                         const control_cmd_args_t *args)
{
    ...
    for (arg = args->kwargs; arg; arg = arg->next) {
        if (!strcasecmp(arg->key, "Port")) {
             ...
             *   * 'DiscardPK' - If tor generates the keypair, do not include it
             * in the response.
             ...
             *   * 'NonAnonymous' - Add a non-anonymous Single Onion Service. If
             * this flag is present, tor must be in non-anonymous hidden service
             * mode. If this flag is absent, tor must be in anonymous hidden
             * service mode.
             */
    ...
    switch (ret) {
        ...
        case RSAE_BADPRIVKEY:
            control_write_endreply(conn, 551,
                                   "Failed to generate onion address");
            break;
            ...
            if (!discard_pk) {
                if (crypto_pk_base64_encode_private(pk, &key_new_blob)) {
                    ...
                    control_printf_endreply(
                        conn, 551, "Failed to encode %s key", key_type_rsa1024);
                    goto err;
                }
    ...
    if (onion_services == NULL) {
        ...
        switch (hs_version) {
            ...
            default:
                 ...
                 * should
                 * never happen because of the check at the start of the
                 * function. */
                break;
        }
        if (ret < 0) {
...
handle_single_control_command(const control_cmd_def_t *def,
                              control_connection_t *conn, uint32_t cmd_data_len,
                              char *args)
{
    int rv = 0;
