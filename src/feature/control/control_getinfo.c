...
static int
getinfo_helper_misc(control_connection_t *conn, const char *question, char **answer,
                    const char **errmsg)
{
    (void)conn;
...
static char *
munge_extrainfo_into_routerinfo(const char *ri_body, const signed_descriptor_t *ri,
...
static int
getinfo_helper_listeners(control_connection_t *control_conn, const char *question, char **answer,
                         const char **errmsg)
{
    int type;
    smartlist_t *res;
...
STATIC int
getinfo_helper_current_time(control_connection_t *control_conn, const char *question, char **answer,
                            const char **errmsg)
{
    (void)control_conn;
    (void)errmsg;
...
STATIC int
getinfo_helper_current_consensus(consensus_flavor_t flavor, char **answer, const char **errmsg)
{
 ...
 */
static int
controller_get_routerdescs(smartlist_t *descs_out, const char *key, const char **msg)
{
    *msg = NULL;
    ...
    if (!strcmp(key, "/tor/server/all")) {
        ...
        dir_split_resource_into_fingerprints(key, digests, NULL, DSR_HEX | DSR_SORT_UNIQ);
...
STATIC int
getinfo_helper_dir(control_connection_t *control_conn, const char *question, char **answer,
                   const char **errmsg)
{
    (void)control_conn;
    if (!strcmpstart(question, "desc/id/")) {
        ...
        const node_t *node = node_get_by_nickname(question + strlen("desc/name/"), 0);
        if (node)
            ri = node->ri;
        if (ri) {
            const char *body = signed_descriptor_get_body(&ri->cache_info);
        ...
        if (routerlist && routerlist->routers) {
            SMARTLIST_FOREACH(routerlist->routers, const routerinfo_t *, ri, {
                const char *body = signed_descriptor_get_body(&ri->cache_info);
                if (body)
                    smartlist_add(sl, tor_strndup(body, ri->cache_info.signed_descriptor_len));
            });
        }
        ...
        if (routerlist && routerlist->routers) {
            SMARTLIST_FOREACH_BEGIN (routerlist->routers, const routerinfo_t *, ri) {
                ...
                signed_descriptor_t *ei =
                    extrainfo_get_by_descriptor_digest(ri->cache_info.extra_info_digest);
                if (ei && body) {
                    smartlist_add(sl, munge_extrainfo_into_routerinfo(body, &ri->cache_info, ei));
                } else if (body) {
                    smartlist_add(sl, tor_strndup(body, ri->cache_info.signed_descriptor_len));
                }
        ...
        *answer = smartlist_join_strings(microdescs, "", 0, NULL);
        ...
        const routerinfo_t *ri = NULL;
        const node_t *node = node_get_by_hex_id(question + strlen("desc-annotations/id/"), 0);
        if (node)
            ri = node->ri;
        if (ri) {
            const char *annotations = signed_descriptor_get_annotations(&ri->cache_info);
            if (annotations)
        ...
        *answer = tor_strdup("");
        ...
        int consensus_result = getinfo_helper_current_consensus(FLAV_NS, answer, errmsg);
        if (consensus_result < 0) {
            return -1;
        }
    } else if (!strcmp(question, "dir/status-vote/current/consensus-microdesc")) {
        int consensus_result = getinfo_helper_current_consensus(FLAV_MICRODESC, answer, errmsg);
        if (consensus_result < 0) {
            return -1;
        }
        ...
        if (strlen(question) == HEX_DIGEST_LEN) {
            ...
            if (base16_decode(d, sizeof(d), question, strlen(question)) == sizeof(d)) {
...
download_status_to_string(const download_status_t *dl)
{
    ...
    if (dl) {
        ...
        tor_asprintf(&rv,
                     ...
                     "last-delay-used %d\n",
                     tbuf, dl->n_download_failures, dl->n_download_attempts, schedule_str,
                     want_authority_str, increment_on_str, backoff_str, dl->last_backoff_position,
                     dl->last_delay_used);
    }
...
STATIC void
getinfo_helper_downloads_networkstatus(const char *flavor, download_status_t **dl_to_emit,
                                       const char **errmsg)
{
    /*
    ...
    if (strcmp(flavor, "ns") == 0) {
        ...
        *dl_to_emit = networkstatus_get_dl_status_by_flavor_bootstrap(FLAV_MICRODESC);
    } else if (strcmp(flavor, "microdesc/running") == 0) {
        *dl_to_emit = networkstatus_get_dl_status_by_flavor_running(FLAV_MICRODESC);
    } else {
...
STATIC void
getinfo_helper_downloads_cert(const char *fp_sk_req, download_status_t **dl_to_emit,
    ...
    if (strcmp(fp_sk_req, "fps") == 0) {
        *digest_list = list_authority_ids_with_downloads();
        ...
        if (sk_req) {
            ...
            if (base16_decode(id_digest, DIGEST_LEN, fp_sk_req, sk_req - fp_sk_req) == DIGEST_LEN) {
                /* Skip past the '/' */
                ++sk_req;
                if (strcmp(sk_req, "sks") == 0) {
                    ...
                    *digest_list = list_sk_digests_for_authority_id(id_digest);
                    ...
                    if (base16_decode(sk_digest, DIGEST_LEN, sk_req, strlen(sk_req)) ==
                        DIGEST_LEN) {
                        *dl_to_emit = download_status_for_authority_id_and_sk(id_digest, sk_digest);
                        if (!(*dl_to_emit)) {
            ...
            if (strlen(fp_sk_req) == HEX_DIGEST_LEN) {
                if (base16_decode(id_digest, DIGEST_LEN, fp_sk_req, strlen(fp_sk_req)) ==
                    DIGEST_LEN) {
...
STATIC void
getinfo_helper_downloads_desc(const char *desc_req, download_status_t **dl_to_emit,
                              smartlist_t **digest_list, const char **errmsg)
{
    ...
    if (strcmp(desc_req, "descs") == 0) {
        *digest_list = router_get_descriptor_digests();
        ...
        if (base16_decode(desc_digest, DIGEST_LEN, desc_req, strlen(desc_req)) == DIGEST_LEN) {
...
STATIC void
getinfo_helper_downloads_bridge(const char *bridge_req, download_status_t **dl_to_emit,
                                smartlist_t **digest_list, const char **errmsg)
{
    ...
    if (strcmp(bridge_req, "bridges") == 0) {
        *digest_list = list_bridge_identities();
        ...
        if (base16_decode(bridge_digest, DIGEST_LEN, bridge_req, strlen(bridge_req)) ==
            DIGEST_LEN) {
            /* Okay we got a digest-shaped thing; try asking for it */
...
STATIC int
getinfo_helper_downloads(control_connection_t *control_conn, const char *question, char **answer,
                         const char **errmsg)
{
    ...
    *errmsg = NULL;
    ...
    if (!strcmpstart(question, "downloads/networkstatus/")) {
        getinfo_helper_downloads_networkstatus(question + strlen("downloads/networkstatus/"),
                                               &dl_to_emit, errmsg);
        /* Certificates? */
    } else if (!strcmpstart(question, "downloads/cert/")) {
        getinfo_helper_downloads_cert(question + strlen("downloads/cert/"), &dl_to_emit,
                                      &digest_list, errmsg);
        /* Router descriptors? */
    } else if (!strcmpstart(question, "downloads/desc/")) {
        getinfo_helper_downloads_desc(question + strlen("downloads/desc/"), &dl_to_emit,
                                      &digest_list, errmsg);
        /* Bridge descriptors? */
    } else if (!strcmpstart(question, "downloads/bridge/")) {
        getinfo_helper_downloads_bridge(question + strlen("downloads/bridge/"), &dl_to_emit,
                                        &digest_list, errmsg);
    } else {
...
static int
getinfo_helper_events(control_connection_t *control_conn, const char *question, char **answer,
                      const char **errmsg)
{
    ...
    if (!strcmp(question, "circuit-status")) {
        ...
        SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ_) {
            ...
            smartlist_add_asprintf(status, "%lu %s%s%s", (unsigned long)circ->global_identifier,
        ...
        *answer = smartlist_join_strings(status, "\r\n", 0, NULL);
        ...
        SMARTLIST_FOREACH_BEGIN (conns, connection_t *, base_conn) {
            ...
            case AP_CONN_STATE_CIRCUIT_WAIT:
                if (conn->socks_request && SOCKS_COMMAND_IS_RESOLVE(conn->socks_request->command))
                    state = "NEWRESOLVE";
                else
                    state = "NEW";
                break;
            ...
            case AP_CONN_STATE_OPEN:
                ...
                break;
            default:
                log_warn(LD_BUG, "Asked for stream in unknown state %d", base_conn->state);
                continue;
            }
            ...
            smartlist_add_asprintf(
                status, "%lu %s %lu %s", (unsigned long)base_conn->global_identifier, state,
        ...
        *answer = smartlist_join_strings(mappings, "\r\n", 0, NULL);
        ...
        if (!strcmp(question, "status/circuit-established")) {
            ...
            *answer = tor_strdup(router_have_minimum_dir_info() ? "1" : "0");
             ...
             * control-spec.txt. */
            *answer = tor_strdup(directories_have_accepted_server_descriptor() ? "1" : "0");
        } else if (!strcmp(question, "status/reachability-succeeded/or")) {
            *answer = tor_strdup(check_whether_orport_reachable(options) ? "1" : "0");
        } else if (!strcmp(question, "status/reachability-succeeded/dir")) {
            *answer = tor_strdup(check_whether_dirport_reachable(options) ? "1" : "0");
        } else if (!strcmp(question, "status/reachability-succeeded")) {
            tor_asprintf(answer, "OR=%d DIR=%d", check_whether_orport_reachable(options) ? 1 : 0,
...
STATIC int
getinfo_helper_onions(control_connection_t *control_conn, const char *question, char **answer,
                      const char **errmsg)
{
 ...
 * liveness. */
static int
getinfo_helper_liveness(control_connection_t *control_conn, const char *question, char **answer,
                        const char **errmsg)
{
    (void)control_conn;
    (void)errmsg;
 ...
 * value. */
static int
getinfo_helper_sr(control_connection_t *control_conn, const char *question, char **answer,
                  const char **errmsg)
{
    (void)control_conn;
    (void)errmsg;
...
static const getinfo_item_t getinfo_items[] = {
    ...
    ITEM("config-text", misc, "Return the string that would be written by a saveconf command."),
    ...
    ITEM("accounting/interval-start", accounting, "Time when the accounting period starts."),
    ITEM("accounting/interval-end", accounting, "Time when the accounting period ends."),
    ITEM("accounting/interval-wake", accounting, "Time to wake up in this accounting period."),
    ITEM("helper-nodes", entry_guards, NULL), /* deprecated */
    ITEM("entry-guards", entry_guards, "Which nodes are we using as entry guards?"),
    ITEM("fingerprint", misc, NULL),
    PREFIX("config/", config, "Current configuration values."),
    DOC("config/names", "List of configuration options, types, and documentation."),
    ...
    PREFIX("downloads/networkstatus/", downloads, "Download statuses for networkstatus objects"),
    DOC("downloads/networkstatus/ns", "Download status for current-mode networkstatus download"),
    ...
    DOC("downloads/networkstatus/microdesc", "Download status for current-mode microdesc download"),
    ...
    PREFIX("downloads/cert/", downloads,
           ...
           "signing key"),
    DOC("downloads/cert/fps", "List of authority fingerprints for which any download statuses "
                              "exist"),
    ...
    DOC("downloads/desc/descs", "Return a list of known router descriptor digests"),
    DOC("downloads/desc/<desc>", "Return a download status for a given descriptor digest"),
    ...
    DOC("downloads/bridge/bridges",
        ...
        "statuses"),
    DOC("downloads/bridge/<desc>", "Return a download status for a given bridge identity digest"),
    ITEM("info/names", misc, "List of GETINFO options, types, and documentation."),
    ITEM("events/names", misc, "Events that the controller can ask for with SETEVENTS."),
    ...
    ITEM("desc/all-recent", dir, "All non-expired, non-superseded router descriptors."),
    ITEM("desc/download-enabled", dir, "Do we try to download router descriptors?"),
    ...
    ITEM("md/download-enabled", dir, "Do we try to download microdescriptors?"),
    PREFIX("extra-info/digest/", dir, "Extra-info documents by digest."),
    PREFIX("hs/client/desc/id", dir, "Hidden Service descriptor in client's cache by onion."),
    PREFIX("hs/service/desc/id/", dir, "Hidden Service descriptor in services's cache by onion."),
    PREFIX("net/listeners/", listeners, "Bound addresses by type"),
    ITEM("ns/all", networkstatus, "Brief summary of router status (v2 directory format)"),
    PREFIX("ns/id/", networkstatus, "Brief summary of router status by ID (v2 directory format)."),
    PREFIX("ns/name/", networkstatus,
           "Brief summary of router status by nickname (v2 directory format)."),
    PREFIX("ns/purpose/", networkstatus,
           "Brief summary of router status by purpose (v2 directory format)."),
    PREFIX("consensus/", networkstatus, "Information about and from the ns consensus."),
    ITEM("network-status", dir, "Brief summary of router status (v1 directory format)"),
    ITEM("network-liveness", liveness, "Current opinion on whether the network is live"),
    ITEM("circuit-status", events, "List of current circuits originating here."),
    ITEM("stream-status", events, "List of current streams."),
    ITEM("orconn-status", events, "A list of current OR connections."),
    ITEM("dormant", misc, "Is Tor dormant (not building circuits because it's idle)?"),
    ...
    DOC("address-mappings/config", "Current address mappings from configuration."),
    DOC("address-mappings/control", "Current address mappings from controller."),
    PREFIX("status/", events, NULL),
    DOC("status/circuit-established", "Whether we think client functionality is working."),
    DOC("status/enough-dir-info",
        "Whether we have enough up-to-date directory information to build "
        "circuits."),
    DOC("status/bootstrap-phase", "The last bootstrap phase status event that Tor sent."),
    DOC("status/clients-seen", "Breakdown of client countries seen by a bridge."),
    ...
    PREFIX("dir/status/", dir, "v2 networkstatus docs as retrieved from a DirPort."),
    ...
    ITEM("onions/current", onions, "Onion services owned by the current control connection."),
    ITEM("onions/detached", onions, "Onion services detached from the control connection."),
...
list_getinfo_options(void)
{
    ...
    for (i = 0; getinfo_items[i].varname; ++i) {
        ...
        smartlist_add_asprintf(lines, "%s%s -- %s\n", getinfo_items[i].varname,
                               getinfo_items[i].is_prefix ? "*" : "", getinfo_items[i].desc);
    }
 ...
 * internal error. */
static int
handle_getinfo_helper(control_connection_t *control_conn, const char *question, char **answer,
                      const char **err_out)
{
    int i;
...
int
handle_control_getinfo(control_connection_t *conn, const control_cmd_args_t *args)
{
    ...
    SMARTLIST_FOREACH_BEGIN (questions, const char *, q) {
        ...
        if (!ans) {
            if (errmsg) {
                ...
                control_reply_add_printf(unrecognized, 552, "Unrecognized key \"%s\"", q);
            }
        } else {
