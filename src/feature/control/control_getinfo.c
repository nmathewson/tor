        ...
        *answer = tor_strdup("");
        ...
        if (strlen(question) == HEX_DIGEST_LEN) {
            ...
            if (base16_decode(d, sizeof(d), question, strlen(question)) ==
                 ...
                 * affecting other parts of the code, especially since the rules
                 * for using our own extrainfo (including when it might be
                 * freed) are different from those for using one we have
                 * downloaded. */
                       ...
                       const control_cmd_args_t *args)
{
    ...
    SMARTLIST_FOREACH_BEGIN(questions, const char *, q)
    {
        ...
        if (!ans) {
            if (errmsg) /* use provided error message */
                ...
                smartlist_add_asprintf(unrecognized, "Unrecognized key \"%s\"",
                                       q);
        } else {
    ...
    if (smartlist_len(unrecognized)) {
        ...
        control_write_endreply(conn, 552,
                               (char *)smartlist_get(unrecognized, i));
        goto done;
    }
