    ...
    if (!strcmp(question, "version")) {
        ...
        *answer = tor_strdup(dormant ? "1" : "0");
        ...
        for (i = 0; control_event_table[i].event_name != NULL; ++i) {
            smartlist_add(event_names,
                          (char *)control_event_table[i].event_name);
        }
                         ...
                         const char **errmsg)
{
    ...
    SMARTLIST_FOREACH_BEGIN (get_connection_array(), connection_t *, conn) {
        ...
        if (getsockname(conn->s, (struct sockaddr *)&ss, &ss_len) < 0) {
            smartlist_add_asprintf(res, "%s:%d", conn->address,
                                   (int)conn->port);
        } else {
    ...
    if (!strcmp(key, "/tor/server/all")) {
        ...
        SMARTLIST_FOREACH_BEGIN (digests, const char *, d) {
            if (router_digest_is_me(d)) {
                ...
                /* Don't actually serve a descriptor that everyone will think
                 * is expired.  This is an (ugly) workaround to keep buggy
                 * 0.1.1.10 Tors from downloading descriptors that they will
                 * throw away.
                 */
                   ...
                   char **answer, const char **errmsg)
{
    ...
    if (!strcmpstart(question, "desc/id/")) {
        ...
        const node_t *node =
            node_get_by_hex_id(question + strlen("desc/id/"), 0);
        if (node)
            ri = node->ri;
        if (ri) {
        ...
        *answer = smartlist_join_strings(sl, "", 0, NULL);
        ...
        smartlist_t *sl = smartlist_new();
        if (routerlist && routerlist->routers) {
            SMARTLIST_FOREACH_BEGIN (routerlist->routers, const routerinfo_t *,
                                     ri) {
                const char *body = signed_descriptor_get_body(&ri->cache_info);
                signed_descriptor_t *ei = extrainfo_get_by_descriptor_digest(
                    ri->cache_info.extra_info_digest);
                if (ei && body) {
                    smartlist_add(sl, munge_extrainfo_into_routerinfo(
                                          body, &ri->cache_info, ei));
                } else if (body) {
                    smartlist_add(
                        sl, tor_strndup(body,
                                        ri->cache_info.signed_descriptor_len));
        ...
        *answer = smartlist_join_strings(microdescs, "", 0, NULL);
        ...
        const node_t *node =
            node_get_by_hex_id(question + strlen("md/id/"), 0);
        ...
        if (ri) {
            ...
            if (annotations)
                *answer =
                    tor_strndup(annotations, ri->cache_info.annotations_len);
        }
        ...
        *answer = tor_strdup("");
        ...
        if (strlen(question) == HEX_DIGEST_LEN) {
            ...
            if (base16_decode(d, sizeof(d), question, strlen(question)) ==
                sizeof(d)) {
                /* XXXX this test should move into
                 * extrainfo_get_by_descriptor_digest, but I don't want to risk
                 * affecting other parts of the code, especially since the
                 * rules for using our own extrainfo (including when it might
                 * be freed) are different from those for using one we have
                 * downloaded. */
...
download_status_to_string(const download_status_t *dl)
{
    ...
    if (dl) {
        ...
        tor_asprintf(&rv,
                     ...
                     backoff_str, dl->last_backoff_position,
                     dl->last_delay_used);
    }
            ...
            if (base16_decode(id_digest, DIGEST_LEN, fp_sk_req,
                ...
                if (strcmp(sk_req, "sks") == 0) {
                    ...
                    *digest_list = list_sk_digests_for_authority_id(id_digest);
                    if (!(*digest_list)) {
                        *errmsg = "Failed to get list of signing key digests "
                                  "for this "
                                  "authority identity digest";
                    }
                } else {
                    /* We've got a signing key digest */
                    if (base16_decode(sk_digest, DIGEST_LEN, sk_req,
                                      strlen(sk_req)) == DIGEST_LEN) {
                        *dl_to_emit = download_status_for_authority_id_and_sk(
                            id_digest, sk_digest);
                        if (!(*dl_to_emit)) {
                            *errmsg = "Failed to get download status for this "
                                      "identity/"
                              ...
                              smartlist_t **digest_list, const char **errmsg)
{
    ...
    if (strcmp(desc_req, "descs") == 0) {
        *digest_list = router_get_descriptor_digests();
        ...
        if (base16_decode(desc_digest, DIGEST_LEN, desc_req,
                          strlen(desc_req)) == DIGEST_LEN) {
    ...
    *errmsg = NULL;
    ...
    if (!strcmpstart(question, "downloads/networkstatus/")) {
        getinfo_helper_downloads_networkstatus(
            question + strlen("downloads/networkstatus/"), &dl_to_emit,
            errmsg);
        /* Certificates? */
                      ...
                      char **answer, const char **errmsg)
{
    ...
    if (!strcmp(question, "circuit-status")) {
        ...
        *answer = smartlist_join_strings(status, "\r\n", 0, NULL);
        ...
        SMARTLIST_FOREACH_BEGIN (conns, connection_t *, base_conn) {
            ...
            smartlist_add_asprintf(
                ...
                origin_circ ? (unsigned long)origin_circ->global_identifier
                            : 0ul,
                buf);
        ...
        *answer = smartlist_join_strings(mappings, "\r\n", 0, NULL);
        ...
        /* Note that status/ is not a catch-all for events; there's only
         * supposed to be a status GETINFO if there's a corresponding STATUS
         * event. */
        if (!strcmp(question, "status/circuit-established")) {
            ...
            *answer = control_event_boot_last_msg();
            ...
            if (c) {
                recommended =
                    is_server ? c->server_versions : c->client_versions;
