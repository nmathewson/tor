 ...
 */
void
control_vprintf_reply(control_connection_t *conn, int code, int c, const char *fmt, va_list ap)
{
    char *buf = NULL;
    int len;
 ...
 */
void
control_write_reply_line(control_connection_t *conn, const control_reply_line_t *line, bool lastone)
{
 ...
 */
void
control_reply_add_one_kv(smartlist_t *reply, int code, int flags, const char *key, const char *val)
{
...
{
    SMARTLIST_FOREACH(reply, control_reply_line_t *, line, control_reply_line_free(line));
