    ...
    for (cl = passwords; cl; cl = cl->next) {
        ...
        if (!strcmpstart(hashed, "16:")) {
            if (base16_decode(decoded, sizeof(decoded), hashed + 3,
                ...
                strlen(hashed + 3) !=
                    (S2K_RFC2440_SPECIFIER_LEN + DIGEST_LEN) * 2) {
                goto err;
            }
        } else {
            if (base64_decode(decoded, sizeof(decoded), hashed,
                              strlen(hashed)) !=
                S2K_RFC2440_SPECIFIER_LEN + DIGEST_LEN) {
                goto err;
            }
        ...
        smartlist_add(
            sl, tor_memdup(decoded, S2K_RFC2440_SPECIFIER_LEN + DIGEST_LEN));
    }
                             ...
                             const control_cmd_args_t *args)
{
...

    {
        ...
        memcpy(tmp + AUTHENTICATION_COOKIE_LEN, client_nonce,
               client_nonce_len);
        memcpy(tmp + AUTHENTICATION_COOKIE_LEN + client_nonce_len,
               server_nonce, SAFECOOKIE_SERVER_NONCE_LEN);

        crypto_hmac_sha256(
            server_hash, SAFECOOKIE_SERVER_TO_CONTROLLER_CONSTANT,
            strlen(SAFECOOKIE_SERVER_TO_CONTROLLER_CONSTANT), tmp, tmp_len);

        crypto_hmac_sha256(
            client_hash, SAFECOOKIE_CONTROLLER_TO_SERVER_CONSTANT,
                            ...
                            const control_cmd_args_t *args)
{
    ...
    if (args->kwargs == NULL) {
        ...
        control_write_endreply(conn, 512,
                               "Too many arguments to AUTHENTICATE.");
        connection_mark_for_close(TO_CONN(conn));
        return 0;
    } else if (strcmp(args->kwargs->key, "")) {
        control_write_endreply(
            conn, 512, "AUTHENTICATE does not accept keyword arguments.");
        connection_mark_for_close(TO_CONN(conn));
        return 0;
    ...
    if (conn->safecookie_client_hash != NULL) {
        ...
        if (tor_memneq(conn->safecookie_client_hash, password,
                       DIGEST256_LEN)) {
            log_warn(LD_CONTROL,
    ...
    if (options->HashedControlPassword ||
        ...
        if (bad) {
            if (!also_cookie) {
                ...
                errstr = "Couldn't decode HashedControlPassword value in "
                         "configuration.";
                goto err;
            }
            bad_password = 1;
            ...
            SMARTLIST_FOREACH(sl, char *, expected, {
                secret_to_key_rfc2440(received, DIGEST_LEN, password,
            ...
            if (used_quoted_string)
                ...
                errstr =
                    ...
                    "If so, the standard requires that you put it in double "
                    "quotes.";
            bad_password = 1;
            if (!also_cookie)
                goto err;
