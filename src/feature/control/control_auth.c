...
decode_hashed_passwords(config_line_t *passwords)
{
    ...
    for (cl = passwords; cl; cl = cl->next) {
        ...
        smartlist_add(sl, tor_memdup(decoded, S2K_RFC2440_SPECIFIER_LEN + DIGEST_LEN));
    }
...
const control_cmd_syntax_t authchallenge_syntax = {.min_args = 1,
                                                   .max_args = 1,
...
int
handle_control_authchallenge(control_connection_t *conn, const control_cmd_args_t *args)
{
    char *client_nonce;
    ...
    if (args->kwargs == NULL || args->kwargs->next != NULL) {
        control_write_endreply(conn, 512, "Wrong number of arguments for AUTHCHALLENGE");
        goto fail;
    }
    ...
    if (contains_quote) {
        ...
        if (base16_decode(client_nonce, client_nonce_len, hex_nonce, strlen(hex_nonce)) !=
    ...
    {
        size_t tmp_len =
            (AUTHENTICATION_COOKIE_LEN + client_nonce_len + SAFECOOKIE_SERVER_NONCE_LEN);
        ...
        crypto_hmac_sha256(server_hash, SAFECOOKIE_SERVER_TO_CONTROLLER_CONSTANT,
                           strlen(SAFECOOKIE_SERVER_TO_CONTROLLER_CONSTANT), tmp, tmp_len);

        crypto_hmac_sha256(client_hash, SAFECOOKIE_CONTROLLER_TO_SERVER_CONSTANT,
                           strlen(SAFECOOKIE_CONTROLLER_TO_SERVER_CONSTANT), tmp, tmp_len);
    ...
    control_printf_endreply(conn, 250, "AUTHCHALLENGE SERVERHASH=%s SERVERNONCE=%s",
...
const control_cmd_syntax_t authenticate_syntax = {.max_args = 0,
                                                  .accept_keywords = true,
                                                  .kvline_flags = KV_OMIT_KEYS | KV_QUOTED_QSTRING,
                                                  .store_raw_body = true};
 ...
 */
int
handle_control_authenticate(control_connection_t *conn, const control_cmd_args_t *args)
{
    ...
    if (args->kwargs == NULL) {
        ...
        control_write_endreply(conn, 512, "AUTHENTICATE does not accept keyword arguments.");
        connection_mark_for_close(TO_CONN(conn));
        return 0;
    ...
    if (conn->safecookie_client_hash != NULL) {
        ...
        if (tor_memneq(conn->safecookie_client_hash, password, DIGEST256_LEN)) {
            log_warn(LD_CONTROL, "Got incorrect safe cookie authentication response");
    ...
    if (options->HashedControlPassword || options->HashedControlSessionPassword) {
        ...
        if (bad) {
            if (!also_cookie) {
                log_warn(LD_BUG, "Couldn't decode HashedControlPassword: invalid base16");
            ...
            SMARTLIST_FOREACH(sl, char *, expected, {
                secret_to_key_rfc2440(received, DIGEST_LEN, password, password_len, expected);
                if (tor_memeq(expected + S2K_RFC2440_SPECIFIER_LEN, received, DIGEST_LEN))
                    goto ok;
            });
            SMARTLIST_FOREACH(sl, char *, str, tor_free(str));
            smartlist_free(sl);
