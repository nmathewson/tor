...
clear_circ_bw_fields(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        ...
        ocirc->n_delivered_written_circ_bw = ocirc->n_delivered_read_circ_bw =
            0;
...
control_per_second_events(void)
{
    ...
    control_event_bandwidth_used((uint32_t)bytes_read,
                                   ...
                                   const struct timeval *tv)
{
...

    {
        ...
        send_control_event(EVENT_CIRCUIT_STATUS_MINOR,
                           ...
                           (unsigned long)circ->global_identifier, event_desc,
                           sp, circdesc, event_tail);
        tor_free(circdesc);
    }
                            ...
                            int reason_code)
{
    ...
    if (reason_code &&
        (tp == STREAM_EVENT_FAILED || tp == STREAM_EVENT_CLOSED ||
        ...
        if (reason_code & END_STREAM_REASON_FLAG_REMOTE)
            ...
            tor_snprintf(reason_buf, sizeof(reason_buf), " REASON=%s",
                         reason_str);
        tor_free(r);
    ...
    if (tp == STREAM_EVENT_NEW || tp == STREAM_EVENT_NEW_RESOLVE) {
        ...
        if (strcmp(ENTRY_TO_CONN(conn)->address, "(Tor_internal)") != 0) {
            tor_snprintf(addrport_buf, sizeof(addrport_buf),
                         " SOURCE_ADDR=%s:%d", ENTRY_TO_CONN(conn)->address,
                         ENTRY_TO_CONN(conn)->port);
        } else {
            /*
                             ...
                             int reason)
{
    ...
    if (conn->chan) {
        ncircs =
            circuit_count_pending_on_channel(TLS_CHAN_TO_BASE(conn->chan));
    } else {
        ncircs = 0;
    }
...
sum_up_cell_stats_by_command(circuit_t *circ, cell_stats_t *cell_stats)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circ->testing_cell_stats,
        ...
        if (!ent->removed && !ent->exitward) {
            ...
            cell_stats->total_time_appward[ent->command] +=
                ent->waiting_time * 10;
        } else {
            cell_stats->removed_cells_exitward[ent->command] += 1;
            cell_stats->total_time_exitward[ent->command] +=
                ent->waiting_time * 10;
        }
...
{
    return control_event_networkstatus_changed_helper(statuses, EVENT_NS,
                                                      "NS");
}
...
control_event_status(int type, int severity, const char *format, va_list args)
{
    ...
    if (tor_snprintf(format_buf, sizeof(format_buf), "650 %s %s", status,
                     sev) < 0) {
