...
static void send_control_event(uint16_t event, const char *format, ...) CHECK_PRINTF(2, 3);
...
control_adjust_event_log_severity(void)
{
    ...
    if (min_log_event <= max_log_event)
        change_callback_log_severity(event_to_log_severity(min_log_event),
                                     event_to_log_severity(max_log_event), control_event_logmsg);
    else
...
control_initialize_event_queue(void)
{
    ...
    if (flush_queued_events_event == NULL) {
        ...
        if (b) {
            flush_queued_events_event = mainloop_event_new(flush_queued_events_cb, NULL);
...
queued_events_flush_all(int force)
{
    ...
    SMARTLIST_FOREACH_BEGIN (queued_events, queued_event_t *, ev) {
        ...
        SMARTLIST_FOREACH_BEGIN (controllers, control_connection_t *, control_conn) {
...
int
control_event_circuit_status(origin_circuit_t *circ, circuit_status_event_t tp, int reason_code)
{
    const char *status;
    char reasons[64] = "";
    ...
    if (tp == CIRC_EVENT_FAILED || tp == CIRC_EVENT_CLOSED) {
        ...
        if (reason_code > 0 && reason_code & END_CIRC_REASON_FLAG_REMOTE) {
            tor_snprintf(reasons, sizeof(reasons), " REASON=DESTROYED REMOTE_REASON=%s",
                         reason_str);
        } else {
...

    {
        ...
        send_control_event(EVENT_CIRCUIT_STATUS, "650 CIRC %lu %s%s%s%s\r\n",
                           (unsigned long)circ->global_identifier, status, sp, circdesc, reasons);
        tor_free(circdesc);
    }
 ...
 * interested control connections. */
static int
control_event_circuit_status_minor(origin_circuit_t *circ, circuit_status_minor_event_t e,
                                   int purpose, const struct timeval *tv)
{
    ...
    case CIRC_MINOR_EVENT_PURPOSE_CHANGED:
...

        {
            ...
            const char *hs_state_str = circuit_purpose_to_controller_hs_state_string(purpose);
    ...
    case CIRC_MINOR_EVENT_CANNIBALIZED:
...

        {
            ...
            tor_snprintf(event_tail, sizeof(event_tail), " OLD_PURPOSE=%s%s%s OLD_TIME_CREATED=%s",
                         circuit_purpose_to_controller_string(purpose),
                         (hs_state_str != NULL) ? " OLD_HS_STATE=" : "",
...

    {
        char *circdesc = circuit_describe_status_for_controller(circ);
        const char *sp = strlen(circdesc) ? " " : "";
        send_control_event(EVENT_CIRCUIT_STATUS_MINOR, "650 CIRC_MINOR %lu %s%s%s%s\r\n",
                           (unsigned long)circ->global_identifier, event_desc, sp, circdesc,
                           event_tail);
        tor_free(circdesc);
    }
...
{
    return control_event_circuit_status_minor(circ, CIRC_MINOR_EVENT_PURPOSE_CHANGED, old_purpose,
                                              NULL);
}
...
{
    return control_event_circuit_status_minor(circ, CIRC_MINOR_EVENT_CANNIBALIZED, old_purpose,
                                              old_tv_created);
}
...
int
control_event_stream_status(entry_connection_t *conn, stream_status_event_t tp, int reason_code)
{
    char reason_buf[64];
    ...
    if (reason_code && (tp == STREAM_EVENT_FAILED || tp == STREAM_EVENT_CLOSED ||
        ...
        if (reason_code & END_STREAM_REASON_FLAG_REMOTE)
            tor_snprintf(reason_buf, sizeof(reason_buf), " REASON=END REMOTE_REASON=%s",
                         reason_str);
        else
        ...
        case REMAP_STREAM_SOURCE_EXIT:
            ...
            break;
        default:
            tor_snprintf(reason_buf, sizeof(reason_buf), " REASON=UNKNOWN_%d", reason_code);
...

    {
        ...
        send_control_event(EVENT_STREAM_STATUS, "650 STREAM %" PRIu64 " %s %lu %s%s%s%s%s%s\r\n",
 ...
 */
int
control_event_or_conn_status(or_connection_t *conn, or_conn_status_event_t tp, int reason)
{
    int ncircs = 0;
    const char *status;
    char name[128];
    ...
    send_control_event(EVENT_OR_CONN_STATUS, "650 ORCONN %s %s%s%s%s ID=%" PRIu64 "\r\n", name,
...
control_event_stream_bandwidth(edge_connection_t *edge_conn)
{
    ...
    if (EVENT_IS_INTERESTING(EVENT_STREAM_BANDWIDTH_USED)) {
        ...
        send_control_event(EVENT_STREAM_BANDWIDTH_USED, "650 STREAM_BW %" PRIu64 " %lu %lu %s\r\n",
                           (edge_conn->base_.global_identifier), (unsigned long)edge_conn->n_read,
...
control_event_stream_bandwidth_used(void)
{
    if (EVENT_IS_INTERESTING(EVENT_STREAM_BANDWIDTH_USED)) {
        ...
        SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn) {
            ...
            format_iso_time_nospace_usec(tbuf, &now);
            send_control_event(
                EVENT_STREAM_BANDWIDTH_USED, "650 STREAM_BW %" PRIu64 " %lu %lu %s\r\n",
                (edge_conn->base_.global_identifier), (unsigned long)edge_conn->n_read,
                (unsigned long)edge_conn->n_written, tbuf);
...
control_event_circ_bandwidth_used_for_circ(origin_circuit_t *ocirc)
{
    ...
    send_control_event(EVENT_CIRC_BANDWIDTH_USED,
                       ...
                       ocirc->global_identifier, (unsigned long)ocirc->n_read_circ_bw,
...
control_event_conn_bandwidth(connection_t *conn)
{
    ...
    if (!get_options()->TestingEnableConnBwEvent || !EVENT_IS_INTERESTING(EVENT_CONN_BW))
        return 0;
    ...
    send_control_event(EVENT_CONN_BW,
                       ...
                       (unsigned long)conn->n_read_conn_bw, (unsigned long)conn->n_written_conn_bw);
...
sum_up_cell_stats_by_command(circuit_t *circ, cell_stats_t *cell_stats)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circ->testing_cell_stats, const testing_cell_stats_entry_t *, ent) {
                             ...
                             const uint64_t *include_if_non_zero, const uint64_t *number_to_include)
{
    ...
    for (i = 0; i <= CELL_COMMAND_MAX_; i++) {
        if (include_if_non_zero[i] > 0) {
            smartlist_add_asprintf(key_value_strings, "%s:%" PRIu64, cell_command_to_string(i),
                                   (number_to_include[i]));
        }
    }
...
void
format_cell_stats(char **event_string, circuit_t *circ, cell_stats_t *cell_stats)
{
    smartlist_t *event_parts = smartlist_new();
    if (CIRCUIT_IS_ORIGIN(circ)) {
        origin_circuit_t *ocirc = TO_ORIGIN_CIRCUIT(circ);
        smartlist_add_asprintf(event_parts, "ID=%lu", (unsigned long)ocirc->global_identifier);
        ...
        append_cell_stats_by_command(event_parts, "InboundAdded", cell_stats->added_cells_appward,
                                     cell_stats->added_cells_appward);
        append_cell_stats_by_command(event_parts, "InboundRemoved",
                                     cell_stats->removed_cells_appward,
                                     cell_stats->removed_cells_appward);
        append_cell_stats_by_command(event_parts, "InboundTime", cell_stats->removed_cells_appward,
                                     cell_stats->total_time_appward);
    }
    if (circ->n_chan) {
        smartlist_add_asprintf(event_parts, "OutboundQueue=%lu", (unsigned long)circ->n_circ_id);
        smartlist_add_asprintf(event_parts, "OutboundConn=%" PRIu64,
                               (circ->n_chan->global_identifier));
        append_cell_stats_by_command(event_parts, "OutboundAdded", cell_stats->added_cells_exitward,
...
control_event_circuit_cell_stats(void)
{
    ...
    char *event_string;
    if (!get_options()->TestingEnableCellStatsEvent || !EVENT_IS_INTERESTING(EVENT_CELL_STATS))
        return 0;
...
{
    int i;
    int idx = (next_measurement_idx + N_BW_EVENTS_TO_CACHE - n_measurements) % N_BW_EVENTS_TO_CACHE;
    ...
    for (i = 0; i < n_measurements; ++i) {
        ...
        smartlist_add_asprintf(elements, "%u,%u", (unsigned)bwe->n_read, (unsigned)bwe->n_written);
...
control_event_logmsg(int severity, log_domain_mask_t domain, const char *msg)
{
    ...
    if (domain == LD_BUG && EVENT_IS_INTERESTING(EVENT_STATUS_GENERAL) && severity <= LOG_NOTICE) {
 ...
 */
int
control_event_address_mapped(const char *from, const char *to, time_t expires, const char *error,
                             const int cached)
{
    ...
    if (expires < 3 || expires == TIME_MAX)
        send_control_event(EVENT_ADDRMAP,
                           ...
                           "CACHED=\"%s\"\r\n",
                           from, to, error ? error : "", error ? " " : "", cached ? "YES" : "NO");
    else {
        ...
        send_control_event(EVENT_ADDRMAP,
                           ...
                           from, to, buf, error ? error : "", error ? " " : "", buf2,
                           cached ? "YES" : "NO");
    }
...
control_event_network_liveness_update(int liveness)
{
    if (liveness > 0) {
        if (get_cached_network_liveness() <= 0) {
            ...
            send_control_event_string(EVENT_NETWORK_LIVENESS, "650 NETWORK_LIVENESS UP\r\n");
        }
        ...
        if (get_cached_network_liveness() > 0) {
            ...
            send_control_event_string(EVENT_NETWORK_LIVENESS, "650 NETWORK_LIVENESS DOWN\r\n");
        }
...
static int
control_event_networkstatus_changed_helper(smartlist_t *statuses, uint16_t event,
...
control_event_newconsensus(const networkstatus_t *consensus)
{
    if (!control_event_is_interesting(EVENT_NEWCONSENSUS))
        return 0;
    return control_event_networkstatus_changed_helper(consensus->routerstatus_list,
                                                      EVENT_NEWCONSENSUS, "NEWCONSENSUS");
}
...
control_event_status(int type, int severity, const char *format, va_list args)
{
    ...
    if (tor_snprintf(format_buf, sizeof(format_buf), "650 %s %s", status, sev) < 0) {
...
int
control_event_guard(const char *nickname, const char *digest, const char *status)
{
...
control_event_conf_changed(const config_line_t *changes)
{
    ...
    send_control_event(EVENT_CONF_CHANGED, "650-CONF_CHANGED\r\n%s\r\n650 OK\r\n", result);
    tor_free(result);
 ...
 */
void
control_event_transport_launched(const char *mode, const char *transport_name, tor_addr_t *addr,
                                 uint16_t port)
{
    send_control_event(EVENT_TRANSPORT_LAUNCHED, "650 TRANSPORT_LAUNCHED %s %s %s %u\r\n", mode,
 ...
 */
void
control_event_hs_descriptor_requested(const char *onion_address, rend_auth_type_t auth_type,
                                      ...
                                      const char *hsdir_index)
{
    ...
    send_control_event(EVENT_HS_DESC, "650 HS_DESC REQUESTED %s %s %s %s%s\r\n",
                       ...
                       rend_auth_type_to_string(auth_type), node_describe_longname_by_id(id_digest),
 ...
 */
void
control_event_hs_descriptor_created(const char *onion_address, const char *desc_id, int replica)
{
    ...
    send_control_event(EVENT_HS_DESC, "650 HS_DESC CREATED %s UNKNOWN UNKNOWN %s%s\r\n",
 ...
 */
void
control_event_hs_descriptor_upload(const char *onion_address, const char *id_digest,
                                   const char *desc_id, const char *hsdir_index)
{
    char *hsdir_index_field = NULL;
    ...
    send_control_event(EVENT_HS_DESC, "650 HS_DESC UPLOAD %s UNKNOWN %s %s%s\r\n", onion_address,
 ...
 */
static void
event_hs_descriptor_receive_end(const char *action, const char *onion_address, const char *desc_id,
                                rend_auth_type_t auth_type, const char *hsdir_id_digest,
                                const char *reason)
{
    ...
    send_control_event(EVENT_HS_DESC, "650 HS_DESC %s %s %s %s%s%s\r\n", action,
                       ...
                       hsdir_id_digest ? node_describe_longname_by_id(hsdir_id_digest) : "UNKNOWN",
 ...
 */
void
control_event_hs_descriptor_upload_end(const char *action, const char *onion_address,
                                       const char *id_digest, const char *reason)
{
    char *reason_field = NULL;
    ...
    send_control_event(EVENT_HS_DESC, "650 HS_DESC %s %s UNKNOWN %s%s\r\n", action,
                       rend_hsaddress_str_or_unknown(onion_address),
                       node_describe_longname_by_id(id_digest), reason_field ? reason_field : "");
...
get_desc_id_from_query(const rend_data_t *rend_data, const char *hsdir_fp)
{
    ...
    for (replica = 0; replica < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; replica++) {
 ...
 */
void
control_event_hsv2_descriptor_received(const char *onion_address, const rend_data_t *rend_data,
                                       const char *hsdir_id_digest)
{
    ...
    event_hs_descriptor_receive_end("RECEIVED", onion_address, desc_id_field,
                                    TO_REND_DATA_V2(rend_data)->auth_type, hsdir_id_digest, NULL);
 ...
 * Called when we successfully received a hidden service descriptor. */
void
control_event_hsv3_descriptor_received(const char *onion_address, const char *desc_id,
                                       const char *hsdir_id_digest)
{
    char *desc_id_field = NULL;
    ...
    event_hs_descriptor_receive_end("RECEIVED", onion_address, desc_id_field, REND_NO_AUTH,
 ...
 */
void
control_event_hs_descriptor_uploaded(const char *id_digest, const char *onion_address)
{
    if (BUG(!id_digest)) {
        return;
    }
 ...
 */
void
control_event_hsv2_descriptor_failed(const rend_data_t *rend_data, const char *hsdir_id_digest,
                                     const char *reason)
{
    char *desc_id_field = NULL;
    const char *desc_id;
    ...
    event_hs_descriptor_receive_end("FAILED", rend_data_get_address(rend_data), desc_id_field,
 ...
 * field. */
void
control_event_hsv3_descriptor_failed(const char *onion_address, const char *desc_id,
                                     const char *hsdir_id_digest, const char *reason)
{
    ...
    event_hs_descriptor_receive_end("FAILED", onion_address, desc_id_field, REND_NO_AUTH,
...
void
control_event_hs_descriptor_content(const char *onion_address, const char *desc_id,
                                    const char *hsdir_id_digest, const char *content)
{
    static const char *event_name = "HS_DESC_CONTENT";
    char *esc_content = NULL;

    if (!onion_address || !desc_id) {
        log_warn(LD_BUG, "Called with onion_address==%p, desc_id==%p, ", onion_address, desc_id);
        return;
    }
    ...
    send_control_event(EVENT_HS_DESC_CONTENT, "650+%s %s %s %s\r\n%s650 OK\r\n", event_name,
 ...
 */
void
control_event_hs_descriptor_upload_failed(const char *id_digest, const char *onion_address,
                                          const char *reason)
{
    if (BUG(!id_digest)) {
        return;
    }
    control_event_hs_descriptor_upload_end("FAILED", onion_address, id_digest, reason);
}
...
control_events_free_all(void)
{
    ...
    if (queued_events) {
        SMARTLIST_FOREACH(queued_events, queued_event_t *, ev, queued_event_free(ev));
        smartlist_free(queued_events);
    }
