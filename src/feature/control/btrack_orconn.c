...
bto_update_best(const bt_orconn_t *bto, bto_best_t *best, const char *type)
{
    ...
    best->gid = bto->gid;
    if (bto->state > best->state) {
        log_info(LD_BTRACK, "ORCONN BEST_%s state %d->%d gid=%" PRIu64, type, best->state,
                 bto->state, bto->gid);
...
bto_state_rcvr(const msg_t *msg, const orconn_state_msg_t *arg)
{
    ...
    log_debug(LD_BTRACK, "ORCONN gid=%" PRIu64 " chan=%" PRIu64 " proxy_type=%d state=%d", arg->gid,
              arg->chan, arg->proxy_type, arg->state);
...
bto_status_rcvr(const msg_t *msg, const orconn_status_msg_t *arg)
{
    ...
    case OR_CONN_EVENT_CLOSED:
        log_info(LD_BTRACK, "ORCONN DELETE gid=%" PRIu64 " status=%d reason=%d", arg->gid,
...
bto_chan_rcvr(const msg_t *msg, const ocirc_chan_msg_t *arg)
{
    ...
    if (!bto->is_orig || (bto->is_onehop && !arg->onehop)) {
        log_debug(LD_BTRACK, "ORCONN LAUNCH chan=%" PRIu64 " onehop=%d", arg->chan, arg->onehop);
    }
    bto->is_orig = true;
    if (!arg->onehop)
        bto->is_onehop = false;
