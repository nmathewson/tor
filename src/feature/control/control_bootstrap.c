                             ...
                             int progress)
{
    ...
    tor_snprintf(buf, sizeof(buf),
                 "BOOTSTRAP PROGRESS=%d TAG=%s SUMMARY=\"%s\"", status, tag,
                 summary);
                                ...
                                const connection_t *conn, int dowarn)
{
    ...
    tor_assert(bootstrap_status_to_string(bootstrap_phase, &tag, &summary) ==
               0);
          ...
          (const char *warn, int reason, or_connection_t *or_conn))
{
    ...
    control_event_bootstrap_problem(
        warn, orconn_end_reason_to_control_string(reason), TO_CONN(or_conn),
        dowarn);
}
...
control_event_bootstrap_reset(void)
{
    ...
    memset(last_sent_bootstrap_message, 0,
