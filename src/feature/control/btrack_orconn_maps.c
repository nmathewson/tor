...
HT_GENERATE2(bto_gid_ht, bt_orconn_t, node, bto_gid_hash_, bto_gid_eq_, 0.6, tor_reallocarray_,
             tor_free_);
...
HT_GENERATE2(bto_chan_ht, bt_orconn_t, chan_node, bto_chan_hash_, bto_chan_eq_, 0.6,
...
bto_delete(uint64_t gid)
{
    ...
    if (!bto) {
        ...
        log_debug(LD_BTRACK, "tried to delete unregistered ORCONN gid=%" PRIu64, gid);
        return;
    }
...
bto_update(bt_orconn_t *bto, const bt_orconn_t *key)
{
    ...
    if (!bto->gid && key->gid) {
        ...
        log_debug(LD_BTRACK, "ORCONN chan=%" PRIu64 " newgid=%" PRIu64, key->chan, key->gid);
        bto->gid = key->gid;
    ...
    if (!bto->chan && key->chan) {
        ...
        log_debug(LD_BTRACK, "ORCONN gid=%" PRIu64 " newchan=%" PRIu64, bto->gid, key->chan);
        bto->chan = key->chan;
