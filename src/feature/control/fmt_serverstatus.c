...
list_single_server_status(const routerinfo_t *desc, int is_live)
{
    ...
    *cp++ = '$';
    base16_encode(cp, HEX_DIGEST_LEN + 1, desc->cache_info.identity_digest, DIGEST_LEN);
 ...
 */
int
list_server_status_v1(smartlist_t *routers, char **router_status_out, int for_controller)
{
    ...
    SMARTLIST_FOREACH_BEGIN (routers, routerinfo_t *, ri) {
        ...
        if (for_controller) {
            ...
            smartlist_add(rs_entries, list_single_server_status(ri, node->is_running));
        }
