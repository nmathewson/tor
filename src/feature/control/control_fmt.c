...
write_stream_target_to_buf(entry_connection_t *conn, char *buf, size_t len)
{
    ...
    if (conn->chosen_exit_name)
        if (tor_snprintf(buf2, sizeof(buf2), ".%s.exit",
...
orconn_target_get_name(char *name, size_t len, or_connection_t *conn)
{
    ...
    if (node) {
        ...
        tor_snprintf(name, len, "%s:%d", conn->base_.address,
                     conn->base_.port);
    }
}
...
circuit_describe_status_for_controller(origin_circuit_t *circ)
{
...

    {
        ...
        if (*vpath) {
            ...
            tor_free(vpath); /* empty path; don't put an extra space in the
                                result */
        }
    }

    {
        ...
        if (smartlist_len(flaglist)) {
            ...
            smartlist_add_asprintf(descparts, "BUILD_FLAGS=%s",
                                   flaglist_joined);
    ...
    if (circ->rend_data != NULL || circ->hs_ident != NULL) {
        ...
        if (circ->rend_data) {
            ...
            hs_build_address(&circ->hs_ident->identity_pk, HS_VERSION_THREE,
                             addr);
            onion_address = addr;
        }
...
entry_connection_describe_status_for_controller(const entry_connection_t *conn)
{
    ...
    if (conn->socks_request != NULL) {
        // Show username and/or password if available; used by
        // IsolateSOCKSAuth.
    ...
    // Show session group; used for stream isolation of multiple listener
    // ports.
