                                    ...
                                    control_connection_t *conn)
{
    ...
    if (base64_decode((char *)privkey->secret_key, sizeof(privkey->secret_key),
        ...
        control_printf_endreply(conn, 512,
                                "Failed to decode x25519 private key");
        goto err;
    }

    if (fast_mem_is_zero((const char *)privkey->secret_key,
                         sizeof(privkey->secret_key))) {
        control_printf_endreply(conn, 553, "Invalid private key \"%s\"",
                                key_blob);
        goto err;
    }
                                     ...
                                     const control_cmd_args_t *args)
{
    ...
    for (const config_line_t *line = args->kwargs; line; line = line->next) {
        if (!strcasecmpstart(line->key, "Flags")) {
            smartlist_split_string(flags, line->value, ",", SPLIT_IGNORE_BLANK,
                                   0);
                                      ...
                                      const control_cmd_args_t *args)
{
    ...
    /* We are gonna put all the credential strings into a smartlist, and sort
       it before printing, so that we can get a guaranteed order of printing.
     */
    ...
    if (argc >= 1) {
        ...
        if (!hs_address_is_valid(hsaddress)) {
            control_printf_endreply(conn, 512, "Invalid v3 addr \"%s\"",
                                    hsaddress);
            goto err;
        }
    }
    ...
    if (hsaddress) {
        control_printf_midreply(conn, 250, "ONION_CLIENT_AUTH_VIEW %s",
                                hsaddress);
    } else {
    ...
    while (!digest256map_iter_done(itr)) {
        ...
        /* If a specific HS address was requested, only print creds for that
         * one */
