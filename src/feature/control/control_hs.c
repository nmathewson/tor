...
parse_private_key_from_control_port(const char *client_privkey_str,
                                    curve25519_secret_key_t *privkey, control_connection_t *conn)
{
    int retval = -1;
    ...
    smartlist_split_string(key_args, client_privkey_str, ":", SPLIT_IGNORE_BLANK, 0);
    ...
    if (strcasecmp(key_type, "x25519")) {
        control_printf_endreply(conn, 552, "Unrecognized key type \"%s\"", key_type);
        goto err;
    }

    if (base64_decode((char *)privkey->secret_key, sizeof(privkey->secret_key), key_blob,
                      strlen(key_blob)) != sizeof(privkey->secret_key)) {
        control_printf_endreply(conn, 512, "Failed to decode x25519 private key");
        goto err;
    }

    if (fast_mem_is_zero((const char *)privkey->secret_key, sizeof(privkey->secret_key))) {
        control_printf_endreply(conn, 553, "Invalid private key \"%s\"", key_blob);
        goto err;
    }
 ...
 */
int
handle_control_onion_client_auth_add(control_connection_t *conn, const control_cmd_args_t *args)
{
    int retval = -1;
    ...
    if (argc < 2) {
        control_printf_endreply(conn, 512, "Incomplete ONION_CLIENT_AUTH_ADD command");
        goto err;
    }
    ...
    if (parse_private_key_from_control_port(client_privkey, &creds->enc_seckey, conn) < 0) {
        goto err;
    }
    ...
    case REGISTER_FAIL_PERMANENT_STORAGE:
        control_printf_endreply(conn, 553, "Unable to store creds for \"%s\"", hsaddress);
        break;
 ...
 */
int
handle_control_onion_client_auth_remove(control_connection_t *conn, const control_cmd_args_t *args)
{
    int retval = -1;
    ...
    int argc = smartlist_len(args->args);
    if (argc < 1) {
        control_printf_endreply(conn, 512, "Incomplete ONION_CLIENT_AUTH_REMOVE command");
        goto err;
    }
...
static char *
encode_client_auth_cred_for_control_port(hs_client_service_authorization_t *cred)
{
    ...
    if (base64_encode(x25519_b64, sizeof(x25519_b64), (char *)cred->enc_seckey.secret_key,
                      sizeof(cred->enc_seckey.secret_key), 0) < 0) {
        tor_assert_nonfatal_unreached();
        goto err;
    }

    smartlist_add_asprintf(control_line, "CLIENT %s x25519:%s", cred->onion_address, x25519_b64);
 ...
 */
int
handle_control_onion_client_auth_view(control_connection_t *conn, const control_cmd_args_t *args)
{
    int retval = -1;
