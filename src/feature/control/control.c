...
control_ports_write_to_file(void)
{
    ...
    if (write_str_to_file(options->ControlPortWriteToFile, joined, 0) < 0) {
        log_warn(LD_CONTROL, "Writing %s failed: %s", options->ControlPortWriteToFile,
                 strerror(errno));
    }
#ifndef _WIN32
    if (options->ControlPortFileGroupReadable) {
        if (chmod(options->ControlPortWriteToFile, 0640)) {
            log_warn(LD_FS, "Unable to make %s group-readable.", options->ControlPortWriteToFile);
        }
    }
...
{
    log_notice(LD_CONTROL, "Owning controller %s has %s -- exiting now.", owner_type, loss_manner);
...
is_valid_initial_command(control_connection_t *conn, const char *cmd)
{
    ...
    if (!strcasecmp(cmd, "PROTOCOLINFO"))
        return (!conn->have_sent_protocolinfo && conn->safecookie_client_hash == NULL);
 ...
 **/
STATIC char *
control_split_incoming_command(char *incoming_cmd, size_t *data_len, char **current_cmd_out)
{
...
again:
    while (1) {
        ...
        do {
            ...
            r = connection_buf_get_line(TO_CONN(conn),
                                        conn->incoming_cmd + conn->incoming_cmd_cur_len, &data_len);
            if (r == 0)
    ...
    args = control_split_incoming_command(conn->incoming_cmd, &data_len, &conn->current_cmd);
...
monitor_owning_controller_process(const char *process_spec)
{
    ...
    if (owning_controller_process_spec != NULL) {
        if ((process_spec != NULL) && !strcmp(process_spec, owning_controller_process_spec)) {
    ...
    owning_controller_process_monitor =
        tor_process_monitor_new(tor_libevent_get_base(), owning_controller_process_spec, LD_CONTROL,
