...
int
getinfo_helper_geoip(control_connection_t *control_conn, const char *question, char **answer,
                     const char **errmsg)
{
    (void)control_conn;
    if (!strcmpstart(question, "ip-to-country/")) {
        ...
        if (!strcmp(question, "ipv4-available") || !strcmp(question, "ipv6-available")) {
