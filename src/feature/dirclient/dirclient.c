...
static void dir_routerdesc_download_failed(smartlist_t *failed, int status_code,
                                           int router_purpose,
                                           int was_extrainfo,
        ...
        if (options->UseBridges && !(type & BRIDGE_DIRINFO)) {
            ...
            if (prefer_authority || (type & BRIDGE_DIRINFO)) {
                ...
                if (rs == NULL &&
                     ...
                     * currently fetching server descriptors from, and we got no
                     * match.  Did we get no match because all the authorities
                     * have connections fetching server descriptors (in which
                     * case we should just return,) or because all the
                     * authorities are down or on fire or unreachable or
    ...
    if (!anonymized_connection && !use_begindir) {
        ...
        switch (connection_connect(TO_CONN(conn), conn->base_.address, &addr,
            ...
            case 0:
                ...
                connection_watch_events(TO_CONN(conn),
...
connection_dir_client_reached_eof(dir_connection_t *conn)
{
    ...
    if (conn->guard_state) {
         ...
         * do not, however, delay use of the circuit here, since it's just for a
        ...
        case 200: {
            ...
            if (rejected_hdr) {
                if (!strcmp(rejected_hdr, "Yes")) {
                     ...
                     * sooner. -NM */
                    /* XXXX++ On further thought, the task above implies that
                     * we're basing our regenerate-descriptor time on when we
                     * uploaded the last descriptor, not on the published time
                     * of the last descriptor.  If those are different, that's a
                     * bad thing to do. -NM */
                }
                                ...
                                const response_handler_args_t *args)
{
    ...
    switch (status_code) {
        case 200:
            ...
            if (hs_cache_store_as_client(body, &conn->hs_ident->identity_pk) <
                0) {
                log_info(LD_REND, "Failed to store hidden service descriptor");
                /* Fire control port FAILED event. */
                hs_control_desc_event_failed(conn->hs_ident,
                                             conn->identity_digest, "BAD_DESC");
                hs_control_desc_event_content(conn->hs_ident,
                ...
                hs_control_desc_event_received(conn->hs_ident,
                                               conn->identity_digest);
                hs_control_desc_event_content(conn->hs_ident,
        ...
        case 404:
             ...
             * connection_about_to_close_connection() tries to clean this conn
             * up. */
            log_info(LD_REND,
...
#define SEND_HS_DESC_FAILED_CONTENT()                                          \
    ...
    switch (status_code) {
        case 200: {
            ...
            if (rend_cache_store_v2_desc_as_client(
                    body, conn->requested_resource, conn->rend_data, &entry) <
                0) {
                ...
                control_event_hsv2_descriptor_received(
                    service_id, conn->rend_data, conn->identity_digest);
                control_event_hs_descriptor_content(
                    service_id, conn->requested_resource, conn->identity_digest,
                    body);
    ...
    switch (status_code) {
        case 200:
            log_info(LD_REND,
                     ...
                     escaped(reason));
            hs_control_desc_event_uploaded(conn->hs_ident,
                                           conn->identity_digest);
            break;
        case 400:
