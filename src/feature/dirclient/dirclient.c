...
static void dir_routerdesc_download_failed(smartlist_t *failed, int status_code, int router_purpose,
...
static void connection_dir_close_consensus_fetches(dir_connection_t *except_this_one,
                                                   const char *resource);
 ...
 */
void
directory_post_to_dirservers(uint8_t dir_purpose, uint8_t router_purpose, dirinfo_type_t type,
                             const char *payload, size_t payload_len, size_t extrainfo_len)
{
    const or_options_t *options = get_options();
    dir_indirection_t indirection;
    const smartlist_t *dirservers = router_get_trusted_dir_servers();
    int found = 0;
    const int exclude_self =
        (dir_purpose == DIR_PURPOSE_UPLOAD_VOTE || dir_purpose == DIR_PURPOSE_UPLOAD_SIGNATURES);
    ...
    SMARTLIST_FOREACH_BEGIN (dirservers, dir_server_t *, ds) {
        ...
        if (extrainfo_len && router_supports_extrainfo(ds->digest, 1)) {
            ...
            log_info(LD_DIR, "Uploading an extrainfo too (length %d)", (int)extrainfo_len);
        }
        if (purpose_needs_anonymity(dir_purpose, router_purpose, NULL)) {
            indirection = DIRIND_ANONYMOUS;
        } else if (!fascist_firewall_allows_dir_server(ds, FIREWALL_DIR_CONNECTION, 0)) {
...
directory_pick_generic_dirserver(dirinfo_type_t type, int pds_flags, uint8_t dir_purpose,
 ...
 */
static void
dir_consensus_request_set_additional_headers(directory_request_t *req, const char *resource)
{
    ...
    const char TRY_DIFF_FOR_CONSENSUS_NEWER_NAME[] = "try-diff-for-consensus-newer-than";
    ...
    int32_t max_age_for_diff =
        3600 * networkstatus_get_param(
                   NULL, TRY_DIFF_FOR_CONSENSUS_NEWER_NAME, DEFAULT_TRY_DIFF_FOR_CONSENSUS_NEWER,
                   MIN_TRY_DIFF_FOR_CONSENSUS_NEWER, MAX_TRY_DIFF_FOR_CONSENSUS_NEWER);
    ...
    if (or_diff_from_is_set) {
        ...
        base16_encode(hex, sizeof(hex), (const char *)or_diff_from, sizeof(or_diff_from));
...
directory_get_from_dirserver,
          (uint8_t dir_purpose, uint8_t router_purpose, const char *resource, int pds_flags,
           download_want_authority_t want_authority))
{
    const routerstatus_t *rs = NULL;
    const or_options_t *options = get_options();
    int prefer_authority =
        (dirclient_fetches_from_authorities(options) || want_authority == DL_WANT_AUTHORITY);
    int require_authority = 0;
    int get_via_tor = purpose_needs_anonymity(dir_purpose, router_purpose, resource);
    ...
    if (!get_via_tor) {
        if (options->UseBridges && !(type & BRIDGE_DIRINFO)) {
            ...
            if (node && node->ri) {
                ...
                fascist_firewall_choose_address_node(node, FIREWALL_OR_CONNECTION, 0, &or_ap);
                directory_request_set_or_addr_port(req, &or_ap);
                directory_request_set_directory_id_digest(req, ri->cache_info.identity_digest);
                directory_request_set_router_purpose(req, router_purpose);
            ...
            if (!rs && !(type & BRIDGE_DIRINFO)) {
                rs = directory_pick_generic_dirserver(type, pds_flags, dir_purpose, &guard_state);
                if (!rs)
    ...
    if (rs) {
        const dir_indirection_t indirection = get_via_tor ? DIRIND_ANONYMOUS : DIRIND_ONEHOP;
        directory_request_t *req = directory_request_new(dir_purpose);
        directory_request_set_routerstatus(req, rs);
        directory_request_set_router_purpose(req, router_purpose);
        directory_request_set_indirection(req, indirection);
 ...
 */
static int
directory_choose_address_routerstatus(const routerstatus_t *status, dir_indirection_t indirection,
                                      tor_addr_port_t *use_or_ap, tor_addr_port_t *use_dir_ap)
{
    ...
    if (indirection == DIRIND_ANONYMOUS) {
         ...
         */
        fascist_firewall_choose_address_rs(status, FIREWALL_OR_CONNECTION, 0, use_or_ap);
    ...
    if (indirection == DIRIND_DIRECT_CONN || indirection == DIRIND_ANON_DIRPORT ||
        (indirection == DIRIND_ONEHOP && !dirclient_must_use_begindir(options))) {
        fascist_firewall_choose_address_rs(status, FIREWALL_DIR_CONNECTION, 0, use_dir_ap);
    ...
    if (!have_or && !have_dir) {
        ...
        log_info(LD_BUG,
                 ...
                 routerstatus_describe(status), fmt_addr32(status->addr), status->or_port,
                 status->dir_port, fmt_addr(&status->ipv6_addr), status->ipv6_orport,
                 status->dir_port);
        if (!logged_backtrace) {
...
{
    if (conn->requested_resource && !strcmpstart(conn->requested_resource, "authority")) {
...
connection_dir_client_request_failed(dir_connection_t *conn)
{
    ...
    if (conn->base_.purpose == DIR_PURPOSE_FETCH_SERVERDESC ||
        ...
        log_info(LD_DIR, "Giving up downloading votes from '%s'", conn->base_.address);
...
connection_dir_download_cert_failed(dir_connection_t *conn, int status)
{
    ...
    if (!strcmpstart(conn->requested_resource, fp_pfx)) {
        ...
        dir_split_resource_into_fingerprints(conn->requested_resource + strlen(fp_pfx), failed,
                                             NULL, DSR_HEX);
        ...
        dir_split_resource_into_fingerprint_pairs(conn->requested_resource + strlen(fpsk_pfx),
                                                  failed);
 ...
 */
static int
directory_command_should_use_begindir(const or_options_t *options, const directory_request_t *req,
                                      const char **reason)
{
    ...
    *reason = NULL;
    ...
    if (indirection == DIRIND_ONEHOP) {
        ...
        if (!fascist_firewall_allows_address_addr(or_addr, or_port, FIREWALL_OR_CONNECTION, 0, 0)) {
 ...
 */
void
directory_request_set_or_addr_port(directory_request_t *req, const tor_addr_port_t *p)
{
 ...
 */
void
directory_request_set_dir_addr_port(directory_request_t *req, const tor_addr_port_t *p)
{
 ...
 */
void
directory_request_set_directory_id_digest(directory_request_t *req, const char *digest)
{
 ...
 */
void
directory_request_set_router_purpose(directory_request_t *req, uint8_t router_purpose)
{
    tor_assert(router_purpose == ROUTER_PURPOSE_GENERAL || router_purpose == ROUTER_PURPOSE_BRIDGE);
 ...
 */
void
directory_request_set_indirection(directory_request_t *req, dir_indirection_t indirection)
{
 ...
 */
void
directory_request_set_payload(directory_request_t *req, const char *payload, size_t payload_len)
{
 ...
 */
void
directory_request_set_if_modified_since(directory_request_t *req, time_t if_modified_since)
{
 ...
 */
void
directory_request_add_header(directory_request_t *req, const char *key, const char *val)
{
 ...
 */
void
directory_request_set_rend_query(directory_request_t *req, const rend_data_t *query)
{
    if (query) {
 ...
 */
void
directory_request_upload_set_hs_ident(directory_request_t *req, const hs_ident_dir_conn_t *ident)
{
    if (ident) {
 ...
 */
void
directory_request_fetch_set_hs_ident(directory_request_t *req, const hs_ident_dir_conn_t *ident)
{
    if (ident) {
...
void
directory_request_set_guard_state(directory_request_t *req, circuit_guard_state_t *state)
{
...
directory_request_dir_contact_info_specified(const directory_request_t *req)
{
     ...
     * is set */
    return (req->or_addr_port.port || req->dir_addr_port.port || !tor_digest_is_zero(req->digest));
}
 ...
 */
void
directory_request_set_routerstatus(directory_request_t *req, const routerstatus_t *status)
{
...

{
    ...
    if (options->ExcludeNodes && options->StrictNodes &&
        ...
        log_warn(LD_DIR,
                 ...
                 routerstatus_describe(status), dir_conn_purpose_to_string(req->dir_purpose));
        return -1;
    }
     ...
     */
    if (directory_choose_address_routerstatus(status, req->indirection, &use_or_ap, &use_dir_ap) <
        0) {
        return -1;
    }
...
directory_initiate_request, (directory_request_t * request))
{
    ...
    if (request->routerstatus) {
        tor_assert_nonfatal(!directory_request_dir_contact_info_specified(request));
    ...
    log_debug(LD_DIR, "anonymized %d, use_begindir %d.", anonymized_connection, use_begindir);

    log_debug(LD_DIR, "Initiating %s", dir_conn_purpose_to_string(dir_purpose));

    if (purpose_needs_anonymity(dir_purpose, router_purpose, resource)) {
        tor_assert(anonymized_connection || rend_non_anonymous_mode_enabled(options));
    }
    ...
    if (!port || tor_addr_is_null(&addr)) {
        static int logged_backtrace = 0;
        log_warn(LD_DIR, "Cannot make an outgoing %sconnection without a remote %sPort.",
    ...
    if (!anonymized_connection && !use_begindir) {
        ...
        switch (
            connection_connect(TO_CONN(conn), conn->base_.address, &addr, port, &socket_error)) {
        case -1:
         ...
         * hook up both sides
         */
        linked_conn = connection_ap_make_link(TO_CONN(conn), conn->base_.address, conn->base_.port,
                                              digest, SESSION_GROUP_DIRCONN, iso_flags,
...
directory_get_consensus_url(const char *resource)
{
...

    {
        ...
        SMARTLIST_FOREACH_BEGIN (router_get_trusted_dir_servers(), dir_server_t *, ds) {
            ...
            base16_encode(hex, 2 * CONDITIONAL_CONSENSUS_FPR_LEN + 1, ds->v3_identity_digest,
                          CONDITIONAL_CONSENSUS_FPR_LEN);
            smartlist_add(authority_digests, hex);
        } SMARTLIST_FOREACH_END (ds);
        smartlist_sort(authority_digests, compare_strs_);
        authority_id_list = smartlist_join_strings(authority_digests, "+", 0, NULL);

        tor_asprintf(&url, "/tor/status-vote/current/consensus%s%s/%s.z", hyphen, flavor,
                     authority_id_list);
 ...
 */
static void
copy_ipv6_address(char *destination, const char *source, size_t len, int decorate)
{
 ...
 */
static void
directory_send_command(dir_connection_t *conn, const int direct, const directory_request_t *req)
{
    tor_assert(req);
    ...
    if (strchr(conn->base_.address, ':')) {
        copy_ipv6_address(decorated_address, conn->base_.address, sizeof(decorated_address), 1);
    } else {
    ...
    if (conn->base_.port == 80) {
        ...
        tor_snprintf(hoststring, sizeof(hoststring), "%s:%d", decorated_address, conn->base_.port);
    }
    ...
    case DIR_PURPOSE_FETCH_CONSENSUS:
        ...
        log_info(LD_DIR, "Downloading consensus from %s using %s", hoststring, url);
        break;
    case DIR_PURPOSE_FETCH_CERTIFICATE:
        tor_assert(resource);
        tor_assert(!payload);
    ...
    log_debug(LD_DIR,
              ...
              conn->base_.address, conn->base_.port, conn->base_.purpose, (total_request_len),
...
static int
load_downloaded_routers(const char *body, smartlist_t *which, int descriptor_digests,
                        int router_purpose, const char *source)
{
    char buf[256];
    ...
    added =
        router_load_routers_from_string(body, NULL, SAVED_NOWHERE, which, descriptor_digests, buf);
    if (added && general)
...
static int
dir_client_decompress_response_body(char **bodyp, size_t *bodylenp, dir_connection_t *conn,
                                    compress_method_t compression, int anonymized_connection)
{
    int rv = 0;
    ...
    tor_log(severity, LD_HTTP,
            ...
            (compression > 0 && guessed > 0 && want_to_try_both) ? "  Trying both." : "");
    ...
    if (anonymized_connection && !allowed_anonymous_connection_compression_method(compression)) {
    ...
    if (tor_compress_supports_method(compression)) {
        tor_uncompress(&new_body, &new_len, body, body_len, compression, !allow_partial,
                       LOG_PROTOCOL_WARN);
        if (new_body) {
    ...
    if (anonymized_connection && !allowed_anonymous_connection_compression_method(guessed)) {
    ...
    if (tor_compress_supports_method(guessed) && compression != guessed) {
        tor_uncompress(&new_body, &new_len, body, body_len, guessed, !allow_partial, LOG_INFO);
        tried_both = 1;
    }
    ...
    if (!plausible && !new_body) {
        log_fn(LOG_PROTOCOL_WARN, LD_HTTP,
               "Unable to decompress HTTP body (tried %s%s%s, server '%s:%d').", description1,
               tried_both ? " and " : "", tried_both ? description2 : "", conn->base_.address,
               conn->base_.port);
        rv = -1;
        goto done;
    }
    ...
    for (int bootstrapped = 0; bootstrapped < 2; ++bootstrapped) {
        ...
        for (int i = 0; i < DIR_PURPOSE_MAX_; ++i) {
            ...
            if (options->SafeLogging_ != SAFELOG_SCRUB_NONE &&
                ...
                continue;
            if (first_time) {
                log_notice(LD_NET, "While %sbootstrapping, fetched this many bytes: ",
                           bootstrapped ? "not " : "");
                first_time = false;
            }
            log_notice(LD_NET, "    %" PRIu64 " (%s)", n, dir_conn_purpose_to_string(i));
        }
    }
}
...
connection_dir_client_reached_eof(dir_connection_t *conn)
{
    ...
    log_debug(LD_DIR,
              "Downloaded %" TOR_PRIuSZ " bytes on connection of purpose "
              "%s; bootstrap %d%%",
              received_bytes, dir_conn_purpose_to_string(conn->base_.purpose),
    ...
    switch (connection_fetch_from_buf_http(TO_CONN(conn), &headers, MAX_HEADERS_SIZE, &body,
                                           &body_len, MAX_DIR_DL_SIZE, allow_partial)) {
    case -1: /* overflow */
        log_warn(LD_PROTOCOL, "'fetch' response too large (server '%s:%d'). Closing.",
                 conn->base_.address, conn->base_.port);
        return -1;
    case 0:
        log_info(LD_HTTP, "'fetch' response not all here, but we're at eof. Closing.");
        return -1;
        /* case 1, fall through */
    }

    if (parse_http_response(headers, &status_code, &date_header, &compression, &reason) < 0) {
        log_warn(LD_HTTP, "Unparseable headers (server '%s:%d'). Closing.", conn->base_.address,
                 conn->base_.port);
    ...
    if (date_header > 0) {
        ...
        if (labs(apparent_skew) > ALLOW_DIRECTORY_TIME_SKEW) {
            ...
            clock_skew_warning(TO_CONN(conn), apparent_skew, trusted, LD_HTTP, "directory",
                               "DIRSERV");
 ...
 **/
STATIC int
handle_response_fetch_consensus(dir_connection_t *conn, const response_handler_args_t *args)
{
    ...
    if (looks_like_a_consensus_diff(body, body_len)) {
        ...
        new_consensus = consensus_diff_apply(consensus_body, consensus_body_len, body, body_len);
        tor_munmap_file(mapped_consensus);
        ...
        log_info(LD_DIR,
                 ...
                 (int)body_len, conn->base_.address, conn->base_.port, (int)strlen(new_consensus));
    ...
    if ((r = networkstatus_set_current_consensus(consensus, strlen(consensus), flavname, 0,
 ...
 **/
static int
handle_response_fetch_certificate(dir_connection_t *conn, const response_handler_args_t *args)
{
    tor_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_CERTIFICATE);
    const int status_code = args->status_code;
    const char *reason = args->reason;
    ...
    if (src_code != -1) {
        if (trusted_dirs_load_certs_from_string(body, src_code, 1, conn->identity_digest) < 0) {
 ...
 **/
static int
handle_response_fetch_status_vote(dir_connection_t *conn, const response_handler_args_t *args)
{
                                          ...
                                          const response_handler_args_t *args)
{
    ...
    const char *msg = NULL;
    log_info(LD_DIR, "Got detached signatures (body size %d) from server %s:%d", (int)body_len,
             conn->base_.address, conn->base_.port);
    if (status_code != 200) {
        log_warn(LD_DIR,
    ...
    if (dirvote_add_signatures(body, conn->base_.address, &msg) < 0) {
        log_warn(LD_DIR, "Problem adding detached signatures from %s:%d: %s", conn->base_.address,
                 conn->base_.port, msg ? msg : "???");
    }
 ...
 **/
static int
handle_response_fetch_desc(dir_connection_t *conn, const response_handler_args_t *args)
{
    tor_assert(conn->base_.purpose == DIR_PURPOSE_FETCH_SERVERDESC ||
               conn->base_.purpose == DIR_PURPOSE_FETCH_EXTRAINFO);
    const int status_code = args->status_code;
    ...
    log_info(LD_DIR, "Received %s (body size %d) from server '%s:%d'",
             was_ei ? "extra server info" : "server info", (int)body_len, conn->base_.address,
             conn->base_.port);
    if (conn->requested_resource && (!strcmpstart(conn->requested_resource, "d/") ||
                                     !strcmpstart(conn->requested_resource, "fp/"))) {
        which = smartlist_new();
        dir_split_resource_into_fingerprints(
            conn->requested_resource + (descriptor_digests ? 2 : 3), which, NULL, 0);
        n_asked_for = smartlist_len(which);
    }
    if (status_code != 200) {
        int dir_okay =
            status_code == 404 || (status_code == 400 && !strcmp(reason, "Servers unavailable."));
        ...
        if (!which) {
            ...
            dir_routerdesc_download_failed(which, status_code, conn->router_purpose, was_ei,
                                           descriptor_digests);
            SMARTLIST_FOREACH(which, char *, cp, tor_free(cp));
            smartlist_free(which);
        }
    ...
    if (which ||
        (conn->requested_resource &&
         (!strcmpstart(conn->requested_resource, "all") ||
          (!strcmpstart(conn->requested_resource, "authority") && get_options()->UseBridges)))) {
        /* as we learn from them, we remove them from 'which' */
        if (was_ei) {
            router_load_extrainfo_from_string(body, NULL, SAVED_NOWHERE, which, descriptor_digests);
        } else {
            // router_load_routers_from_string(body, NULL, SAVED_NOWHERE, which,
            //                       descriptor_digests, conn->router_purpose);
            if (load_downloaded_routers(body, which, descriptor_digests, conn->router_purpose,
                                        conn->base_.address)) {
                time_t now = approx_time();
    ...
    if (which) { /* mark remaining ones as failed */
        log_info(LD_DIR, "Received %d/%d %s requested from %s:%d",
                 ...
                 was_ei ? "extra-info documents" : "router descriptors", conn->base_.address,
                 (int)conn->base_.port);
        if (smartlist_len(which)) {
            dir_routerdesc_download_failed(which, status_code, conn->router_purpose, was_ei,
                                           descriptor_digests);
 ...
 **/
STATIC int
handle_response_fetch_microdesc(dir_connection_t *conn, const response_handler_args_t *args)
{
    ...
    tor_assert(conn->requested_resource && !strcmpstart(conn->requested_resource, "d/"));
    tor_assert_nonfatal(!fast_mem_is_zero(conn->identity_digest, DIGEST_LEN));
    which = smartlist_new();
    dir_split_resource_into_fingerprints(conn->requested_resource + 2, which, NULL,
    ...
    if (status_code != 200) {
        log_info(LD_DIR,
                 ...
                 "soon.",
                 status_code, escaped(reason), conn->base_.address, (int)conn->base_.port,
                 conn->requested_resource);
        ...
        smartlist_t *mds;
        time_t now = approx_time();
        mds = microdescs_add_to_cache(get_microdesc_cache(), body, body + body_len, SAVED_NOWHERE,
                                      0, now, which);
        if (smartlist_len(which)) {
 ...
 **/
static int
handle_response_upload_dir(dir_connection_t *conn, const response_handler_args_t *args)
{
    ...
    switch (status_code) {
    case 200: {
        dir_server_t *ds = router_get_trusteddirserver_by_digest(conn->identity_digest);
        char *rejected_hdr = http_get_header(headers, "X-Descriptor-Not-New: ");
        if (rejected_hdr) {
            if (!strcmp(rejected_hdr, "Yes")) {
                log_info(LD_GENERAL, "Authority '%s' declined our descriptor (not new)",
                         ds->nickname);
        ...
        log_info(LD_GENERAL, "eof (status 200) after uploading server "
                             "descriptor: finished.");
        control_event_server_status(LOG_NOTICE, "ACCEPTED_SERVER_DESCRIPTOR DIRAUTH=%s:%d",
                                    conn->base_.address, conn->base_.port);
    ...
    case 400:
        ...
        control_event_server_status(LOG_WARN, "BAD_SERVER_DESCRIPTOR DIRAUTH=%s:%d REASON=\"%s\"",
 ...
 **/
static int
handle_response_upload_vote(dir_connection_t *conn, const response_handler_args_t *args)
{
    ...
    switch (status_code) {
    case 200: {
        log_notice(LD_DIR, "Uploaded a vote to dirserver %s:%d", conn->base_.address,
                   conn->base_.port);
    } break;
    case 400:
        log_warn(LD_DIR,
 ...
 **/
static int
handle_response_upload_signatures(dir_connection_t *conn, const response_handler_args_t *args)
{
    ...
    switch (status_code) {
    case 200: {
        log_notice(LD_DIR, "Uploaded signature(s) to dirserver %s:%d", conn->base_.address,
                   conn->base_.port);
    } break;
    case 400:
        log_warn(LD_DIR,
 ...
 **/
STATIC int
handle_response_fetch_hsdesc_v3(dir_connection_t *conn, const response_handler_args_t *args)
{
    const int status_code = args->status_code;
    const char *reason = args->reason;
    const char *body = args->body;
    ...
    log_info(LD_REND, "Received v3 hsdesc (body size %d, status %d (%s))", (int)body_len,
 ...
 **/
static int
handle_response_fetch_renddesc_v2(dir_connection_t *conn, const response_handler_args_t *args)
{
...
#define SEND_HS_DESC_FAILED_CONTENT()                                            \
    (control_event_hs_descriptor_content(rend_data_get_address(conn->rend_data), \
                                         conn->requested_resource, conn->identity_digest, NULL))
    ...
    case 200: {
        ...
        if (rend_cache_store_v2_desc_as_client(body, conn->requested_resource, conn->rend_data,
                                               &entry) < 0) {
 ...
 **/
static int
handle_response_upload_renddesc_v2(dir_connection_t *conn, const response_handler_args_t *args)
{
...
#define SEND_HS_DESC_UPLOAD_FAILED_EVENT(reason)                      \
    (control_event_hs_descriptor_upload_failed(conn->identity_digest, \
                                               rend_data_get_address(conn->rend_data), reason))
 ...
 **/
static int
handle_response_upload_hsdesc(dir_connection_t *conn, const response_handler_args_t *args)
{
    const int status_code = args->status_code;
    const char *reason = args->reason;
    ...
    case 400:
        log_fn(LOG_PROTOCOL_WARN, LD_REND,
               ...
               escaped(reason), conn->base_.address, conn->base_.port);
        hs_control_desc_event_failed(conn->hs_ident, conn->identity_digest, "UPLOAD_REJECTED");
        break;
    default:
        log_warn(LD_REND,
                 "Uploading hidden service descriptor: http "
                 ...
                 "'%s:%d').",
                 status_code, escaped(reason), conn->base_.address, conn->base_.port);
        hs_control_desc_event_failed(conn->hs_ident, conn->identity_digest, "UNEXPECTED");
        break;
    }
...
connection_dir_reached_eof(dir_connection_t *conn)
{
    ...
    if (conn->base_.state != DIR_CONN_STATE_CLIENT_READING) {
        log_info(LD_HTTP, "conn reached eof, not reading. [state=%d] Closing.", conn->base_.state);
...
static compress_method_t client_meth_pref[] = {LZMA_METHOD, ZSTD_METHOD, ZLIB_METHOD, GZIP_METHOD,
                                               NO_METHOD};
...
static compress_method_t client_meth_allowed_anonymous_compression[] = {ZLIB_METHOD, GZIP_METHOD,
                                                                        NO_METHOD};
...
allowed_anonymous_connection_compression_method(compress_method_t method)
{
    ...
    for (u = 0; u < ARRAY_LENGTH(client_meth_allowed_anonymous_compression); ++u) {
        compress_method_t allowed_method = client_meth_allowed_anonymous_compression[u];
 ...
 */
static void
connection_dir_close_consensus_fetches(dir_connection_t *except_this_one, const char *resource)
{
    smartlist_t *conns_to_close =
        connection_dir_list_by_purpose_and_resource(DIR_PURPOSE_FETCH_CONSENSUS, resource);
 ...
 */
static void
dir_routerdesc_download_failed(smartlist_t *failed, int status_code, int router_purpose,
                               int was_extrainfo, int was_descriptor_digests)
{
    char digest[DIGEST_LEN];
 ...
 * appropriate. */
static void
dir_microdesc_download_failed(smartlist_t *failed, int status_code, const char *dir_id)
{
    networkstatus_t *consensus = networkstatus_get_latest_consensus_by_flavor(FLAV_MICRODESC);
    routerstatus_t *rs;
    ...
    SMARTLIST_FOREACH_BEGIN (failed, const char *, d) {
        ...
        { /* Increment the failure count for this md fetch */
            ...
            log_info(LD_DIR, "Failed to download md %s from %s", buf, hex_str(dir_id, DIGEST_LEN));
