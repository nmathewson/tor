                             ...
                             size_t payload_len, size_t extrainfo_len)
{
    ...
    /* This tries dirservers which we believe to be down, but ultimately,
     * that's harmless, and we may as well err on the side of getting things
     * uploaded.
     */
                                 ...
                                 circuit_guard_state_t **guard_state_out)
{
    ...
    if (should_use_directory_guards(options)) {
        const node_t *node =
            guards_choose_dirguard(dir_purpose, guard_state_out);
        if (node)
            rs = node->rs;
    } else {
                                             ...
                                             const char *resource)
{
    ...
    if (flav != -1) {
        ...
        if (cd) {
            if_modified_since =
                cd->published + DEFAULT_IF_MODIFIED_SINCE_DELAY;
    ...
    if (or_diff_from_is_set) {
        ...
        base16_encode(hex, sizeof(hex), (const char *)or_diff_from,
                      sizeof(or_diff_from));
        directory_request_add_header(req, X_OR_DIFF_FROM_CONSENSUS_HEADER,
                                     hex);
    }
}
           ...
           int pds_flags, download_want_authority_t want_authority))
{
    ...
    dirinfo_type_t type =
        dir_fetch_type(dir_purpose, router_purpose, resource);
    ...
    if (!get_via_tor) {
        if (options->UseBridges && !(type & BRIDGE_DIRINFO)) {
             ...
             */
            const node_t *node =
                guards_choose_dirguard(dir_purpose, &guard_state);
            if (node && node->ri) {
                ...
                fascist_firewall_choose_address_node(
                    node, FIREWALL_OR_CONNECTION, 0, &or_ap);
                ...
                if (dir_purpose == DIR_PURPOSE_FETCH_CONSENSUS)
                    dir_consensus_request_set_additional_headers(req,
                                                                 resource);
            ...
            if (prefer_authority || (type & BRIDGE_DIRINFO)) {
                ...
                if (rs == NULL &&
                    ...
                    /* We don't want to fetch from any authorities that we're
                     * currently fetching server descriptors from, and we got
                     * no match.  Did we get no match because all the
                     * authorities have connections fetching server descriptors
                     * (in which case we should just return,) or because all
                     * the authorities are down or on fire or unreachable or
                     * something (in which case we should go on with our
                     * fallback code)? */
                    pds_flags &= ~(PDS_NO_EXISTING_SERVERDESC_FETCH |
                                   PDS_NO_EXISTING_MICRODESC_FETCH);
                    rs = router_pick_trusteddirserver(type, pds_flags);
            ...
            if (!rs && !(type & BRIDGE_DIRINFO)) {
                rs = directory_pick_generic_dirserver(
                    type, pds_flags, dir_purpose, &guard_state);
                if (!rs)
                                      ...
                                      tor_addr_port_t *use_dir_ap)
{
    ...
    if (indirection == DIRIND_ANONYMOUS) {
        if (status->addr) {
             ...
             * to extend to this address, always use the primary (IPv4) OR
             * address */
...
{
    if (conn->guard_state) {
        /* We haven't seen a success on this guard state, so consider it to
         * have failed. */
    ...
    if (!entry_list_is_constrained(get_options()))
        router_set_status(conn->identity_digest,
    ...
    *reason = NULL;
    ...
    if (indirection == DIRIND_ONEHOP) {
        ...
        if (!fascist_firewall_allows_address_addr(
                or_addr, or_port, FIREWALL_OR_CONNECTION, 0, 0)) {
...

{
     ...
     * directory server, we have selected a server that has at least one
     * address allowed by ClientUseIPv4/6 and Reachable{"",OR,Dir}Addresses.
     * This selection uses the preference in ClientPreferIPv6{OR,Dir}Port, if
     * possible. (If UseBridges is set, clients always use IPv6, and prefer it
     * by default.)
     *
     * Now choose an address that we can use to connect to the directory
     * server.
     */
                       ...
                       const directory_request_t *req)
{
    ...
    if (strchr(conn->base_.address, ':')) {
        ...
        strlcpy(decorated_address, conn->base_.address,
                                    ...
                                    int anonymized_connection)
{
    ...
    want_to_try_both =
        (compression == UNKNOWN_METHOD || guessed != compression);
    if (want_to_try_both) {
...
dirclient_dump_total_dls(void)
{
    ...
    for (int bootstrapped = 0; bootstrapped < 2; ++bootstrapped) {
        ...
        for (int i = 0; i < DIR_PURPOSE_MAX_; ++i) {
            ...
            log_notice(LD_NET, "    %" PRIu64 " (%s)", n,
                       dir_conn_purpose_to_string(i));
        }
    }
}
...
connection_dir_client_reached_eof(dir_connection_t *conn)
{
    ...
    tor_log(LOG_DEBUG, LD_DIR,
            ...
            escaped(reason), conn->base_.purpose, (received_bytes),
#ifdef MEASUREMENTS_21206
    ...
    if (conn->guard_state) {
        /* we count the connection as successful once we can read from it.  We
         * do not, however, delay use of the circuit here, since it's just for
         * a one-hop directory request. */
    ...
    if (dir_client_decompress_response_body(
            &body, &body_len, conn, compression, anonymized_connection) < 0) {
        rv = -1;
        goto done;
    }
                                ...
                                const response_handler_args_t *args)
{
    ...
    if (looks_like_a_consensus_diff(body, body_len)) {
        ...
        new_consensus = consensus_diff_apply(
            consensus_body, consensus_body_len, body, body_len);
        tor_munmap_file(mapped_consensus);
                                          ...
                                          const response_handler_args_t *args)
{
    ...
    if (status_code != 200) {
        log_warn(LD_DIR,
                 "Received http status code %d (%s) from server '%s:%d' while "
                 "fetching "
                                ...
                                const response_handler_args_t *args)
{
    ...
    if (status_code != 200) {
        ...
        smartlist_t *mds;
        time_t now = approx_time();
        mds = microdescs_add_to_cache(get_microdesc_cache(), body,
                                      body + body_len, SAVED_NOWHERE, 0, now,
                                      which);
        if (smartlist_len(which)) {
                           ...
                           const response_handler_args_t *args)
{
    ...
    case 200: {
        ...
        char *rejected_hdr =
            http_get_header(headers, "X-Descriptor-Not-New: ");
        if (rejected_hdr) {
            if (!strcmp(rejected_hdr, "Yes")) {
                 ...
                 * basing our regenerate-descriptor time on when we uploaded
                 * the last descriptor, not on the published time of the last
                              ...
                              const response_handler_args_t *args)
{
    ...
    log_info(LD_REND,
             ...
             "(%s))",
             status_code, escaped(reason));
    /* For this directory response, it MUST have an hidden service identifier
     * on this connection. */
    tor_assert(conn->hs_ident);
    switch (status_code) {
    case 200:
...
connection_dir_reached_eof(dir_connection_t *conn)
{
    ...
    if (conn->base_.state != DIR_CONN_STATE_CLIENT_READING) {
        ...
        connection_close_immediate(
            TO_CONN(conn)); /* error: give up on flushing */
        connection_mark_for_close(TO_CONN(conn));
        return -1;
    }
                              ...
                              const char *dir_id)
{
    ...
    SMARTLIST_FOREACH_BEGIN (failed, const char *, d) {
        rs = router_get_mutable_consensus_status_by_descriptor_digest(
            consensus, d);
        if (!rs)
            continue;
        dls = &rs->dl_status;
        ...
        { /* Increment the failure count for this md fetch */
            ...
            download_status_increment_failure(dls, status_code, buf, server,
                                              now);
        }
    } SMARTLIST_FOREACH_END (d);
}
