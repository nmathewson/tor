        ...
        if (!networkstatus_consensus_can_use_multiple_directories(options)) {
            ...
            if (networkstatus_consensus_is_bootstrapping(time(NULL))) {
                ...
                if (!networkstatus_consensus_can_use_extra_fallbacks(options)) {
                    ...
                    return options->ClientBootstrapConsensusAuthorityOnlyDownloadInitialDelay;
                } else if (dls->want_authority) {
                    /* A bootstrapping client with extra fallback directories, but
                     * connecting to an authority */
                    return options->ClientBootstrapConsensusAuthorityDownloadInitialDelay;
                } else {
 ...
 * than the high bound. */
STATIC void
next_random_exponential_delay_range(int *low_bound_out, int *high_bound_out, int delay,
                                    int base_delay)
{
...
next_random_exponential_delay(int delay, int base_delay)
{
    ...
    next_random_exponential_delay_range(&low_bound, &high_bound, delay, base_delay);
...
STATIC int
download_status_schedule_get_delay(download_status_t *dls, int min_delay, time_t now)
{
    tor_assert(dls);
    ...
    uint8_t dls_schedule_position =
        (dls->increment_on == DL_SCHED_INCREMENT_ATTEMPT ? dls->n_download_attempts
...
download_status_log_helper(const char *item, int was_schedule_incremented,
                           const char *increment_action, const char *not_incremented_response,
                           uint8_t dls_n_download_increments, int increment,
                           time_t dls_next_attempt_at, time_t now)
{
    if (item) {
        if (!was_schedule_incremented)
            log_debug(LD_DIR, "%s %s %d time(s); I'll try again %s.", item, increment_action,
                      (int)dls_n_download_increments, not_incremented_response);
        else if (increment == 0)
            ...
            log_debug(LD_DIR, "%s %s %d time(s); I'll try again in %d seconds.", item,
                      increment_action, (int)dls_n_download_increments,
                      (int)(dls_next_attempt_at - now));
        else
            log_debug(LD_DIR, "%s %s %d time(s); Giving up for a while.", item, increment_action,
...
time_t
download_status_increment_failure(download_status_t *dls, int status_code, const char *item,
                                  int server, time_t now)
{
    ...
    download_status_log_helper(item, !dls->increment_on, "failed", "concurrently",
                               dls->n_download_failures, increment,
 ...
 * concurrent attempt). */
time_t
download_status_increment_attempt(download_status_t *dls, const char *item, time_t now)
{
    int delay = -1;
    int min_delay = 0;
    ...
    download_status_log_helper(item, dls->increment_on, "attempted", "on failure",
                               dls->n_download_attempts, delay,
                               download_status_get_next_attempt_at(dls), now);
