...
find_dl_min_delay(const download_status_t *dls, const or_options_t *options)
{
    ...
    switch (dls->schedule) {
        ...
        case DL_SCHED_CONSENSUS:
            if (!networkstatus_consensus_can_use_multiple_directories(
                    options)) {
                /* A public relay */
                ...
                if (networkstatus_consensus_is_bootstrapping(time(NULL))) {
                    ...
                    if (!networkstatus_consensus_can_use_extra_fallbacks(
                            options)) {
                        /* A bootstrapping client without extra fallback
                         * directories */
                        return options
                            ->ClientBootstrapConsensusAuthorityOnlyDownloadInitialDelay;
                    } else if (dls->want_authority) {
                        /* A bootstrapping client with extra fallback
                         * directories, but connecting to an authority */
                        return options
                            ->ClientBootstrapConsensusAuthorityDownloadInitialDelay;
                    } else {
                        /* A bootstrapping client connecting to extra fallback
                         * directories
                         */
                        return options
                    ...
                    /* A client with a reasonably live consensus, with or
        ...
        case DL_SCHED_BRIDGE:
            if (options->UseBridges && num_bridges_usable(0) > 0) {
                /* A bridge client that is sure that one or more of its bridges
                 * are running can afford to wait longer to update bridge
                 * descriptors. */
                return options->TestingBridgeDownloadInitialDelay;
            } else {
                /* A bridge client which might have no running bridges, must try
                 * to get bridge descriptors straight away. */
                                  ...
                                  const char *item, int server, time_t now)
{
    ...
    if (dls->increment_on == DL_SCHED_INCREMENT_FAILURE) {
        ...
        /* only return a failure retry time if this schedule increments on
         * failures
         */
