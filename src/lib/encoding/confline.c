 ...
 * <b>last</b>. */
int
config_get_lines_aux(const char *string, config_line_t **result, int extended, int allow_include,
                     int *has_include, struct smartlist_t *opened_lst, int recursion_level,
    ...
    do {
        ...
        if (k && v) {
            ...
            if (allow_include && !strcmp(k, "%include") && handle_include) {
                ...
                if (handle_include(v, recursion_level, extended, &include_list, &list_last,
                                   opened_lst) < 0) {
                    log_warn(LD_CONFIG,
...
{
    return config_get_lines_aux(string, result, extended, 0, NULL, NULL, 1, NULL, NULL);
}
 ...
 */
const char *
parse_config_line_from_str_verbose(const char *line, char **key_out, char **value_out,
                                   const char **err_out)
{
    /*
    ...
    *key_out = *value_out = NULL;
    ...
    key = line;
    while (*line && !TOR_ISSPACE(*line) && *line != '#' && !(line[0] == '\\' && line[1] == '\n'))
        ++line;
