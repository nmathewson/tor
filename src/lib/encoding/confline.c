                     ...
                     config_line_t **last, include_handler_fn handle_include)
{
    ...
    do {
        k = v = NULL;
        string =
            parse_config_line_from_str_verbose(string, &k, &v, &parse_err);
        if (!string) {
        ...
        if (k && v) {
            ...
            if (allow_include && !strcmp(k, "%include") && handle_include) {
                ...
                *next = include_list;
                ...
                /* This list can get long, so we keep a pointer to the end of
                 * it rather than using config_line_append over and over and
                 * getting n^2 performance. */
