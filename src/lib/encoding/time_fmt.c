...
tor_timegm(const struct tm *tm, time_t *time_out)
{
    ...
    *time_out = 0;
    ...
    if (invalid_year || tm->tm_mon < 0 || tm->tm_mon > 11 || tm->tm_mday < 1 || tm->tm_mday > dpm ||
...
parse_rfc1123_time(const char *buf, time_t *t)
{
    ...
    if (tor_sscanf(buf, "%3s, %2u %3s %u %2u:%2u:%2u GMT", weekday, &tm_mday, month, &tm_year,
    ...
    if (invalid_year || tm_mday < 1 || tm_mday > dpm || tm_hour > 23 || tm_min > 59 ||
        tm_sec > 60) {
        char *esc = esc_for_log(buf);
        log_warn(LD_GENERAL, "Got invalid RFC1123 time %s", esc);
        tor_free(esc);
...
parse_iso_time_(const char *cp, time_t *t, int strict, int nospace)
{
    ...
    n_fields = tor_sscanf(cp, "%u-%2u-%2u%c%2u:%2u:%2u%c", &year, &month, &day, &separator_char,
...
parse_http_time(const char *date, struct tm *tm)
{
    ...
    if ((cp = strchr(date, ','))) {
        ...
        if (*cp != ' ')
            return -1;
        ++cp;
        if (tor_sscanf(cp, "%2u %3s %4u %2u:%2u:%2u GMT", &tm_mday, month, &tm_year, &tm_hour,
                       &tm_min, &tm_sec) == 6) {
            /* rfc1123-date */
            tm_year -= 1900;
        } else if (tor_sscanf(cp, "%2u-%3s-%2u %2u:%2u:%2u GMT", &tm_mday, month, &tm_year,
        ...
        if (tor_sscanf(date, "%3s %3s %2u %2u:%2u:%2u %4u", wkday, month, &tm_mday, &tm_hour,
                       &tm_min, &tm_sec, &tm_year) == 7) {
            tm_year -= 1900;
        } else {
            return -1;
        }
    ...
    if (tm->tm_year < 0 || tm->tm_mon < 0 || tm->tm_mon > 11 || tm->tm_mday < 1 ||
        tm->tm_mday > 31 || tm->tm_hour < 0 || tm->tm_hour > 23 || tm->tm_min < 0 ||
...
format_time_interval(char *out, size_t out_len, long interval)
{
    ...
    if (day) {
        return tor_snprintf(out, out_len, "%ld days, %ld hours, %ld minutes", day, hour, min);
    } else if (hour) {
