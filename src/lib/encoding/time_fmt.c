...
tor_timegm(const struct tm *tm, time_t *time_out)
{
    ...
    /* Initialize time_out to 0 for now, to avoid bad usage in case this
       function fails and the caller ignores the return value. */
    tor_assert(time_out);
    *time_out = 0;
    ...
    if (invalid_year || tm->tm_mon < 0 || tm->tm_mon > 11 || tm->tm_mday < 1 ||
        ...
        tm->tm_min < 0 || tm->tm_min > 59 || tm->tm_sec < 0 ||
        tm->tm_sec > 60) {
...
parse_rfc1123_time(const char *buf, time_t *t)
{
    ...
    if (m < 0) {
        char *esc = esc_for_log(buf);
        log_warn(LD_GENERAL, "Got invalid RFC1123 time %s: No such month",
                 esc);
        tor_free(esc);
        return -1;
    }
    ...
    if (tm.tm_year < 1970) {
        ...
        char *esc = esc_for_log(buf);
        log_warn(LD_GENERAL, "Got invalid RFC1123 time %s. (Before 1970)",
                 esc);
        tor_free(esc);
        return -1;
        /* LCOV_EXCL_STOP */
...
parse_iso_time_(const char *cp, time_t *t, int strict, int nospace)
{
    ...
    n_fields =
        tor_sscanf(cp, "%u-%2u-%2u%c%2u:%2u:%2u%c", &year, &month, &day,
...
parse_http_time(const char *date, struct tm *tm)
{
    ...
    if ((cp = strchr(date, ','))) {
        ...
        if (tor_sscanf(cp, "%2u %3s %4u %2u:%2u:%2u GMT", &tm_mday, month,
            ...
            /* rfc1123-date */
            tm_year -= 1900;
        } else if (tor_sscanf(cp, "%2u-%3s-%2u %2u:%2u:%2u GMT", &tm_mday,
                              month, &tm_year, &tm_hour, &tm_min,
                              &tm_sec) == 6) {
            /* rfc850-date */
        } else {
            return -1;
        ...
        if (tor_sscanf(date, "%3s %3s %2u %2u:%2u:%2u %4u", wkday, month,
...
format_time_interval(char *out, size_t out_len, long interval)
{
    ...
    if (day) {
        ...
        return tor_snprintf(out, out_len, "%ld minutes, %ld seconds", min,
                            sec);
    } else {
