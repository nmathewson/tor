...
#    endif /* defined(VER_NT_SERVER) */
            ...
            for (i = 0; win_version_table[i].major > 0; ++i) {
                if (win_version_table[i].major == info.dwMajorVersion &&
                    ...
                    if (is_server) {
                        ...
                        /* Use client versions for clients, and when we don't
                         * know if it is a client or a server. */
            ...
            if (plat) {
                ...
                if (info.dwMajorVersion > win_version_table[0].major ||
                    ...
                    tor_snprintf(
                        uname_result, sizeof(uname_result),
             ...
             *
             * Microsoft's API documentation says that on Windows 8.1 and
             * later, GetVersionEx returns Windows 8 (6.2) for applications
             * without an app compatibility manifest (including tor's default
             * build).
             *
            ...
            if (info.dwMajorVersion > 6 ||
                ...
                /* When GetVersionEx() returns Windows 8, the actual OS may be
                 * any later version. */
                strlcat(uname_result, " [or later]", sizeof(uname_result));
            }
            /* When we don't know if the OS is a client or server version, we
             * use the client version, and this qualifier. */
            if (!is_server && !is_client) {
                strlcat(uname_result, " [client or server]",
                        sizeof(uname_result));
            }
#else /* !defined(_WIN32) */
