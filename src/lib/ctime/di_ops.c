...
tor_memeq(const void *a, const void *b, size_t sz)
{
    ...
    while (sz--) {
        /* Set byte_diff to all of those bits that are different in *ba and
         * *bb, and advance both ba and bb. */
                                     ...
                                     uint64_t total, uint64_t rand_val)
{
    ...
    for (i = 0; i < n_entries; ++i) {
        ...
        if (gt_i64_timei(total_so_far, rand_val)) {
            i_chosen = i;
            n_chosen++;
            /* Set rand_val to INT64_MAX rather than stopping the loop. This
             * way, the time we spend in the loop does not leak which element
             * we chose. */
            rand_val = INT64_MAX;
        }
    }
