...
#        define CHUNK_SET_SENTINEL(chunk, alloclen)                        \
            do {                                                           \
                ...
                DBG_S(uint8_t *b =                                         \
                          &((uint8_t *)(chunk))[(alloclen)-SENTINEL_LEN]); \
...
buf_pullup(buf_t *buf, size_t bytes, const char **head_out, size_t *len_out)
{
    ...
    if (buf->head->memlen >= capacity) {
        /* We don't need to grow the first chunk, but we might need to repack
         * it.*/
...
buf_move_to_buf(buf_t *buf_out, buf_t *buf_in, size_t *buf_flushlen)
{
    ...
    while (len) {
        /* This isn't the most efficient implementation one could imagine,
         * since it does two copies instead of 1, but I kinda doubt that this
