...
#    define USE_EVP_AES_CTR
    ...
    (defined(__i386) || defined(__i386__) || defined(_M_IX86) || defined(__x86_64) || \
     defined(__x86_64__) || defined(_M_AMD64) || defined(_M_X64) || defined(__INTEL__))
...
aes_crypt_inplace(aes_cnt_cipher_t *cipher_, char *data, size_t len)
{
    ...
    EVP_EncryptUpdate(cipher, (unsigned char *)data, &outl, (unsigned char *)data, (int)len);
}
int
...
#    else
    ...
    if (e) {
        log_info(LD_CRYPTO, "AES engine \"%s\" found; using EVP_* functions.", ENGINE_get_name(e));
        should_use_EVP = 1;
    } else {
...
static void aes_set_key(aes_cnt_cipher_t *cipher, const uint8_t *key, int key_bits);
...
aes_crypt_inplace(aes_cnt_cipher_t *cipher, char *data, size_t len)
{
    ...
    if (cipher->using_evp) {
         ...
         */
        CRYPTO_ctr128_encrypt((const unsigned char *)data, (unsigned char *)data, len,
                              &cipher->key.evp, cipher->ctr_buf.buf, cipher->buf, &cipher->pos,
                              evp_block128_fn);
    } else {
        AES_ctr128_encrypt((const unsigned char *)data, (unsigned char *)data, len,
                           &cipher->key.aes, cipher->ctr_buf.buf, cipher->buf, &cipher->pos);
    }
}
