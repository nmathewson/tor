...
#ifdef ENABLE_NSS
  /* Copy into a temporary ctx, since DigestFinal (alters) the context */
  /* XXXX verify that this is needed */
  PK11Context *t = PK11_CloneContext(digest->d.ctx);
  unsigned rlen = 0;
  SECStatus s;
  s = PK11_DigestFinal(t, r, &rlen, sizeof(r));
  tor_assert(s == SECSuccess);
  PK11_DestroyContext(t, PR_TRUE);
...
#ifdef ENABLE_NSS
  if (library_supports_digest(digest->algorithm)) {
    SECStatus s = PK11_SaveContext(digest->d.ctx,
                   ...
                   const char *msg, size_t msg_len)
{
  ...
  if (BUG(s != SECSuccess || len != DIGEST256_LEN))
    goto done;
  ok = 1;
