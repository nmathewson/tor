                                         ...
                                         int force)
{
    ...
    if (!force && fromlen + overhead <= pkeylen) {
        ...
        return crypto_pk_public_encrypt(env, to, tolen, from, fromlen,
                                        padding);
    }
    ...
    r = crypto_cipher_encrypt(cipher, to + outlen,
                              from + pkeylen - overhead - CIPHER_KEY_LEN,
                              symlen);
                                          ...
                                          int warnOnFailure)
{
    ...
    if (fromlen <= pkeylen) {
        return crypto_pk_private_decrypt(env, to, tolen, from, fromlen,
                                         padding, warnOnFailure);
    }

    buf = tor_malloc(pkeylen);
    outlen = crypto_pk_private_decrypt(env, buf, pkeylen, from, pkeylen,
                                       padding, warnOnFailure);
    if (outlen < 0) {
                                  ...
                                  bool private_key)
{
    ...
    if (pem_encode(result, resultlen, (const unsigned char *)buf, n, tag) <
        0) {
        goto done;
    }
 ...
 */
int
crypto_pk_read_private_key1024_from_string(crypto_pk_t *env, const char *src,
                                           ssize_t len)
{
    return crypto_pk_read_from_string_generic(env, src, len, LOG_INFO, true,
                                              1024);
