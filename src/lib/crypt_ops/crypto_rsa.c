 ...
 */
int
crypto_pk_obsolete_public_hybrid_encrypt(crypto_pk_t *env, char *to, size_t tolen, const char *from,
                                         size_t fromlen, int padding, int force)
{
    ...
    r = crypto_cipher_encrypt(cipher, to + outlen, from + pkeylen - overhead - CIPHER_KEY_LEN,
                              symlen);
 ...
 */
int
crypto_pk_obsolete_private_hybrid_decrypt(crypto_pk_t *env, char *to, size_t tolen,
                                          ...
                                          int warnOnFailure)
{
    ...
    outlen = crypto_pk_private_decrypt(env, buf, pkeylen, from, pkeylen, padding, warnOnFailure);
    if (outlen < 0) {
        log_fn(warnOnFailure ? LOG_WARN : LOG_DEBUG, LD_CRYPTO, "Error decrypting public-key data");
        goto err;
    }
    if (outlen < CIPHER_KEY_LEN) {
        log_fn(warnOnFailure ? LOG_WARN : LOG_INFO, LD_CRYPTO, "No room for a symmetric key");
        goto err;
    }
 ...
 */
int
crypto_pk_private_sign_digest(crypto_pk_t *env, char *to, size_t tolen, const char *from,
                              size_t fromlen)
{
    int r;
    char digest[DIGEST_LEN];
 ...
 */
static int
crypto_pk_write_to_string_generic(crypto_pk_t *env, char **dest, size_t *len, bool private_key)
{
    const int factor =
        private_key ? PRIVATE_ASN_MAX_OVERHEAD_FACTOR : PUBLIC_ASN_MAX_OVERHEAD_FACTOR;
 ...
 */
int
crypto_pk_write_public_key_to_string(crypto_pk_t *env, char **dest, size_t *len)
{
 ...
 */
int
crypto_pk_write_private_key_to_string(crypto_pk_t *env, char **dest, size_t *len)
{
 ...
 */
static int
crypto_pk_read_from_string_generic(crypto_pk_t *env, const char *src, size_t len, int severity,
                                   bool private_key, int max_bits)
{
    ...
    if (n < 0) {
        log_fn(severity, LD_CRYPTO, "Error decoding PEM wrapper while reading %s", ktype);
        goto done;
    }

    crypto_pk_t *pk = private_key ? crypto_pk_asn1_decode_private((const char *)buf, n, max_bits)
                                  : crypto_pk_asn1_decode((const char *)buf, n);
    if (!pk) {
        log_fn(severity, LD_CRYPTO, "Error decoding ASN.1 while reading %s", ktype);
        goto done;
    }
 ...
 */
int
crypto_pk_read_public_key_from_string(crypto_pk_t *env, const char *src, size_t len)
{
    return crypto_pk_read_from_string_generic(env, src, len, LOG_INFO, false, -1);
}
 ...
 */
int
crypto_pk_read_private_key_from_string(crypto_pk_t *env, const char *src, ssize_t len)
{
    return crypto_pk_read_from_string_generic(env, src, len, LOG_INFO, true, -1);
}
 ...
 */
int
crypto_pk_read_private_key1024_from_string(crypto_pk_t *env, const char *src, ssize_t len)
{
    return crypto_pk_read_from_string_generic(env, src, len, LOG_INFO, true, 1024);
}
...
crypto_pk_read_private_key_from_filename(crypto_pk_t *env, const char *keyfile)
{
    ...
    if (!buf) {
        log_warn(LD_CRYPTO, "Unable to read file for private key in %s", escaped(keyfile));
        return -1;
    }
    if (st.st_size > MAX_PRIVKEY_FILE_LEN) {
        log_warn(LD_CRYPTO, "Private key file %s was far too large.", escaped(keyfile));
        tor_free(buf);
        return -1;
    }

    int rv = crypto_pk_read_from_string_generic(env, buf, (ssize_t)st.st_size, LOG_WARN, true, -1);
    if (rv < 0) {
        log_warn(LD_CRYPTO, "Unable to decode private key from file %s", escaped(keyfile));
    }
