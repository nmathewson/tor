...
typedef struct {
    ...
    int (*pubkey_from_curve25519_pubkey)(unsigned char *,
                                        ...
                                        const curve25519_keypair_t *inp)
{
    ...
    crypto_digest_get_digest(ctx, (char *)sha512_output,
...
ed25519_impl_spot_check, (void))
{
     ...
     * self-test code that sanity-checks the internals.  If present, use that
     * to screen out catastrophic errors like massive compiler failure.
     */
