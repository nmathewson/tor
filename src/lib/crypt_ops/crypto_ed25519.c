...
typedef struct {
    ...
    int (*sign)(unsigned char *, const unsigned char *, size_t, const unsigned char *,
                const unsigned char *);
 ...
 */
int
ed25519_secret_key_from_seed(ed25519_secret_key_t *seckey_out, const uint8_t *seed)
{
 ...
 */
int
ed25519_public_key_generate(ed25519_public_key_t *pubkey_out, const ed25519_secret_key_t *seckey)
{
...
ed25519_keypair_generate(ed25519_keypair_t *keypair_out, int extra_strong)
{
    if (ed25519_secret_key_generate(&keypair_out->seckey, extra_strong) < 0)
        return -1;
    if (ed25519_public_key_generate(&keypair_out->pubkey, &keypair_out->seckey) < 0)
        return -1;
    return 0;
}
 ...
 */
int
ed25519_sign(ed25519_signature_t *signature_out, const uint8_t *msg, size_t len,
...
ed25519_sign_prefixed,
          (ed25519_signature_t * signature_out, const uint8_t *msg, size_t msg_len,
           const char *prefix_str, const ed25519_keypair_t *keypair))
{
    ...
    retval = ed25519_sign(signature_out, prefixed_msg, prefixed_msg_len, keypair);
...
MOCK_IMPL(int,
ed25519_checksig,
          (const ed25519_signature_t *signature, const uint8_t *msg, size_t len,
           const ed25519_public_key_t *pubkey))
{
    return get_ed_impl()->open(signature->sig, msg, len, pubkey->pubkey) < 0 ? -1 : 0;
}
 ...
 */
int
ed25519_checksig_prefixed(const ed25519_signature_t *signature, const uint8_t *msg, size_t msg_len,
                          const char *prefix_str, const ed25519_public_key_t *pubkey)
{
    int retval;
    size_t prefixed_msg_len;
    uint8_t *prefixed_msg;
 ...
 */
int
ed25519_keypair_from_curve25519_keypair(ed25519_keypair_t *out, int *signbit_out,
                                        const curve25519_keypair_t *inp)
{
    ...
    *signbit_out = out->pubkey.pubkey[31] >> 7;
    ...
    ed25519_public_key_from_curve25519_public_key(&pubkey_check, &inp->pubkey, *signbit_out);
...
ed25519_public_key_from_curve25519_public_key(ed25519_public_key_t *pubkey,
                                              const curve25519_public_key_t *pubkey_in, int signbit)
{
    return get_ed_impl()->pubkey_from_curve25519_pubkey(pubkey->pubkey, pubkey_in->public_key,
                                                        signbit);
}
...
int
ed25519_keypair_blind(ed25519_keypair_t *out, const ed25519_keypair_t *inp, const uint8_t *param)
{
    ed25519_public_key_t pubkey_check;

    get_ed_impl()->blind_secret_key(out->seckey.seckey, inp->seckey.seckey, param);
 ...
 */
int
ed25519_public_blind(ed25519_public_key_t *out, const ed25519_public_key_t *inp,
                     const uint8_t *param)
{
 ...
 */
int
ed25519_seckey_write_to_file(const ed25519_secret_key_t *seckey, const char *filename,
                             const char *tag)
{
    return crypto_write_tagged_contents_to_file(filename, "ed25519v1-secret", tag, seckey->seckey,
                              ...
                              const char *filename)
{
    ...
    len = crypto_read_tagged_contents_from_file(filename, "ed25519v1-secret", tag_out,
                                                seckey_out->seckey, sizeof(seckey_out->seckey));
 ...
 */
int
ed25519_pubkey_write_to_file(const ed25519_public_key_t *pubkey, const char *filename,
                             const char *tag)
{
    return crypto_write_tagged_contents_to_file(filename, "ed25519v1-public", tag, pubkey->pubkey,
                              ...
                              const char *filename)
{
    ...
    len = crypto_read_tagged_contents_from_file(filename, "ed25519v1-public", tag_out,
                                                pubkey_out->pubkey, sizeof(pubkey_out->pubkey));
...
int
ed25519_pubkey_eq(const ed25519_public_key_t *key1, const ed25519_public_key_t *key2)
{
    tor_assert(key1);
    tor_assert(key2);
 ...
 */
void
ed25519_pubkey_copy(ed25519_public_key_t *dest, const ed25519_public_key_t *src)
{
    tor_assert(dest);
    tor_assert(src);
...
{
    static const uint8_t alicesk[32] = {0xc5, 0xaa, 0x8d, 0xf4, 0x3f, 0x9f, 0x83, 0x7b,
                                        0xed, 0xb7, 0x44, 0x2f, 0x31, 0xdc, 0xb7, 0xb1,
                                        0x66, 0xd3, 0x85, 0x35, 0x07, 0x6f, 0x09, 0x4b,
                                        0x85, 0xce, 0x3a, 0x2e, 0x0b, 0x44, 0x58, 0xf7};
    static const uint8_t alicepk[32] = {0xfc, 0x51, 0xcd, 0x8e, 0x62, 0x18, 0xa1, 0xa3,
...
ed25519_point_is_identity_element(const uint8_t *point)
{
    ...
    static const uint8_t ed25519_identity[32] = {0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
ed25519_validate_pubkey(const ed25519_public_key_t *pubkey)
{
    ...
    if (get_ed_impl()->ed25519_scalarmult_with_group_order(result, pubkey->pubkey) < 0) {
