           ...
           const ed25519_keypair_t *keypair))
{
    ...
    prefixed_msg =
        get_prefixed_msg(msg, msg_len, prefix_str, &prefixed_msg_len);
    if (BUG(!prefixed_msg)) {
...
{
    return get_ed_impl()->open(signature->sig, msg, len, pubkey->pubkey) < 0
               ? -1
               : 0;
}
                          ...
                          const ed25519_public_key_t *pubkey)
{
    ...
    retval =
        ed25519_checksig(signature, prefixed_msg, prefixed_msg_len, pubkey);
    tor_free(prefixed_msg);
           ...
           int n_checkable))
{
    ...
    if (impl->open_batch == NULL) {
        /* No batch verification implementation available, fake it by checking
         * the each signature individually.
         */
        res = 0;
        for (i = 0; i < n_checkable; ++i) {
            const ed25519_checkable_t *ch = &checkable[i];
            int r =
                ed25519_checksig(&ch->signature, ch->msg, ch->len, ch->pubkey);
            if (r < 0)
                --res;
            if (okay_out)
                okay_out[i] = (r == 0);
...
MOCK_IMPL(STATIC int, ed25519_impl_spot_check, (void))
{
    ...
    /* Some implementations (eg: The modified Ed25519-donna) have handy
     * self-test code that sanity-checks the internals.  If present, use that to
     * screen out catastrophic errors like massive compiler failure.
     */
...
ed25519_validate_pubkey(const ed25519_public_key_t *pubkey)
{
    ...
    /* For any point on the curve, doing l*point should give the identity
     * element (where l is the group order). Do the computation and check that
     * the identity element is returned. */
    if (get_ed_impl()->ed25519_scalarmult_with_group_order(
            result, pubkey->pubkey) < 0) {
