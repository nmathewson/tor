...
crypto_fast_rng_new_from_seed(const uint8_t *seed)
{
    ...
    if (inherit == INHERIT_RES_KEEP) {
        /* This value will neither be dropped nor zeroed after fork, so we need
         * to check our pid to make sure we are not sharing it across a fork.
         * This can be expensive if the pid value isn't cached, sadly.
         */
        result->owner = getpid();
    }
#elif defined(_WIN32)
                              ...
                              const size_t n)
{
    ...
    if (rng->owner) {
         ...
         * INHERIT_RES_KEEP.
         *
         * If the result was INHERIT_RES_DROP, then any attempt to access the
         * rng memory after forking will crash.
         *
         * If the result was INHERIT_RES_ZERO, then forking will set the
         * bytes_left and n_till_reseed fields to zero.  This function will
         * call crypto_fast_rng_refill(), which will in turn reseed the PRNG.
         *
         * So we only need to do this test in the case when mmap_anonymous()
         * returned INHERIT_KEEP.  We avoid doing it needlessly, since getpid()
         * is often a system call, and that can be slow.
         */
