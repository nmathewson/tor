...
crypto_fast_rng_new_from_seed(const uint8_t *seed)
{
     ...
     */
    crypto_fast_rng_t *result =
        tor_mmap_anonymous(sizeof(*result), ANONMAP_PRIVATE | ANONMAP_NOINHERIT, &inherit);
 ...
 **/
static void
crypto_fast_rng_getbytes_impl(crypto_fast_rng_t *rng, uint8_t *out, const size_t n)
{
#ifdef CHECK_PID
    if (rng->owner) {
    ...
    while (bytes_to_yield) {
        ...
        uint8_t *copy_from = rng->buf.bytes + (sizeof(rng->buf.bytes) - rng->bytes_left);
