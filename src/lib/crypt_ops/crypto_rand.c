...
crypto_strongest_rand_syscall(uint8_t *out, size_t out_len)
{
     ...
     * fails the crypto_strongest_rand_raw() caller will log with a
     * warning-level message and let crypto_strongest_rand() error out and
     * finally terminating Tor with an assertion error.
     */
    ...
    for (int i = 0; i < max_attempts; i++) {
        ...
        if (crypto_strongest_rand_syscall(out, out_len) != 0) {
            ...
            if (crypto_strongest_rand_fallback(out, out_len) != 0) {
                /* Welp, we tried.  Hopefully the calling code terminates the
                 * process since we're basically boned without good entropy.
                 */
                log_warn(
                    LD_CRYPTO,
...
#define DLEN DIGEST512_LEN
    ...
    while (out_len) {
        ...
        if (out_len >= DLEN) {
            crypto_digest512((char *)out, (char *)inp, sizeof(inp),
                             DIGEST_SHA512);
            out += DLEN;
            out_len -= DLEN;
        } else {
            crypto_digest512((char *)tmp, (char *)inp, sizeof(inp),
                             DIGEST_SHA512);
...
crypto_seed_openssl_rng(void)
{
     ...
     * entropy than we do.  We'll try calling that, *and* calling our own
     * entropy functions.  If one succeeds, we'll accept the RNG as seeded. */
