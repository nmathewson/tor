...
crypto_strongest_rand_fallback(uint8_t *out, size_t out_len)
{
    ...
    return -1;
#else /* !defined(_WIN32) */
    static const char *filenames[] = {"/dev/srandom", "/dev/urandom", "/dev/random", NULL};
    int fd, i;
    size_t n;
    ...
    for (i = 0; filenames[i]; ++i) {
        ...
        if (n != out_len) {
            ...
            log_notice(LD_CRYPTO, "Error reading from entropy source %s (read only %lu bytes).",
 ...
 **/
char *
crypto_random_hostname(int min_rand_len, int max_rand_len, const char *prefix, const char *suffix)
{
    ...
    base32_encode(result + prefixlen, resultlen - prefixlen, rand_bytes, rand_bytes_len);
    tor_free(rand_bytes);
