...
#else
        ret =
            (sha3_256((uint8_t *)digest, DIGEST256_LEN, (const uint8_t *)m, len)
             > -1);
...
#else
        ret =
            (sha3_512((uint8_t *)digest, DIGEST512_LEN, (const uint8_t *)m, len)
             > -1);
#endif /* defined(OPENSSL_HAS_SHA3) */
    }
                   ...
                   const char *msg, size_t msg_len)
{
    ...
    rv = HMAC(
        EVP_sha256(), key, (int)key_len, (unsigned char *)msg, (int)msg_len,
