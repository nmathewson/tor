                 ...
                 digest_algorithm_t algorithm)
{
    ...
    if (algorithm == DIGEST_SHA256) {
        ...
        ret =
            EVP_Digest(m, len, (uint8_t *)digest, &dlen, EVP_sha3_256(), NULL);
#else
                 ...
                 digest_algorithm_t algorithm)
{
    ...
    if (algorithm == DIGEST_SHA512) {
        ret = (SHA512((const unsigned char *)m, len,
                      (unsigned char *)digest) != NULL);
    } else {
#ifdef OPENSSL_HAS_SHA3
        unsigned int dlen = DIGEST512_LEN;
        ret =
            EVP_Digest(m, len, (uint8_t *)digest, &dlen, EVP_sha3_512(), NULL);
#else
...
crypto_digest_get_digest(crypto_digest_t *digest, char *out, size_t out_len)
{
    ...
    if (digest->algorithm == DIGEST_SHA3_256 ||
        ...
        goto done;
#else /* !defined(OPENSSL_HAS_SHA3) */
        /* Tiny-Keccak handles copying into a temporary ctx, and also can
         * handle short output buffers by truncating appropriately. */
    ...
    case DIGEST_SHA3_512:
    default:
        log_warn(LD_BUG, "Handling unexpected algorithm %d",
                 digest->algorithm);
...
{
    crypto_digest_smartlist_prefix(digest_out, len_out, NULL, lst, append,
                                   alg);
}
