...
crypto_digest512(char *digest, const char *m, size_t len, digest_algorithm_t algorithm)
{
    ...
    if (algorithm == DIGEST_SHA512) {
        ret = (SHA512((const unsigned char *)m, len, (unsigned char *)digest) != NULL);
    } else {
#ifdef OPENSSL_HAS_SHA3
...
crypto_digest_get_digest(crypto_digest_t *digest, char *out, size_t out_len)
{
    ...
    if (digest->algorithm == DIGEST_SHA3_256 || digest->algorithm == DIGEST_SHA3_512) {
#ifdef OPENSSL_HAS_SHA3
        unsigned dlen = (unsigned)crypto_digest_algorithm_get_length(digest->algorithm);
...
#ifdef OPENSSL_HAS_SHA3
    if (from->algorithm == DIGEST_SHA3_256 || from->algorithm == DIGEST_SHA3_512) {
