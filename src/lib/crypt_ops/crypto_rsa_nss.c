...
crypto_pk_generate_key_with_bits, (crypto_pk_t * key, int bits))
{
    ...
    seckey = PK11_GenerateKeyPair(slot, CKM_RSA_PKCS_KEY_PAIR_GEN, &params, &pubkey,
                                  PR_FALSE /*isPerm */, PR_FALSE /*isSensitive*/, NULL);
...
static const SECItem oaep_item = {
    .type = siBuffer, .data = (unsigned char *)&oaep_params, .len = sizeof(oaep_params)};
 ...
 */
int
crypto_pk_public_encrypt(crypto_pk_t *env, char *to, size_t tolen, const char *from, size_t fromlen,
                         int padding)
{
    tor_assert(env);
    tor_assert(to);
    ...
    SECStatus s =
        PK11_PubEncrypt(env->pubkey, m, item, (unsigned char *)to, &result_len, (unsigned int)tolen,
                        (const unsigned char *)from, (unsigned int)fromlen, NULL);
    if (s != SECSuccess) {
 ...
 */
int
crypto_pk_private_decrypt(crypto_pk_t *key, char *to, size_t tolen, const char *from,
                          size_t fromlen, int padding, int warnOnFailure)
{
    tor_assert(key);
    tor_assert(to);
    tor_assert(from);
    ...
    CK_MECHANISM_TYPE m = padding_to_mechanism(padding, &item);
    SECStatus s =
        PK11_PrivDecrypt(key->seckey, m, item, (unsigned char *)to, &result_len,
...
crypto_pk_public_checksig,
          (const crypto_pk_t *key, char *to, size_t tolen, const char *from, size_t fromlen))
{
    tor_assert(key);
    tor_assert(to);
    tor_assert(from);
    ...
    SECItem dsig = {.type = siBuffer, .data = (unsigned char *)to, .len = (unsigned int)tolen};
    SECStatus s;
 ...
 */
int
crypto_pk_private_sign(const crypto_pk_t *key, char *to, size_t tolen, const char *from,
                       size_t fromlen)
{
    tor_assert(key);
    tor_assert(to);
    ...
    SECItem sig = {.type = siBuffer, .data = (unsigned char *)to, .len = (unsigned int)tolen};
    SECItem hash = {.type = siBuffer, .data = (unsigned char *)from, .len = (unsigned int)fromlen};
...
crypto_pk_asn1_decode(const char *str, size_t len)
{
    ...
    if (len >= INT_MAX)
        return NULL;
    CERTSubjectPublicKeyInfo info = {.algorithm = {.algorithm = {.type = siDEROID,
 ...
 */
int
crypto_pk_asn1_encode_private(const crypto_pk_t *pk, char *dest, size_t destlen)
{
...
crypto_pk_asn1_decode_private(const char *str, size_t len, int max_bits)
{
    ...
    SECKEYPrivateKeyInfo info = {.algorithm = {.algorithm = {.type = siBuffer,
                                                             .data = (unsigned char *)RSA_OID,
                                                             .len = sizeof(RSA_OID)}},
                                 .privateKey = {
                                     .type = siBuffer,
    ...
    s = PK11_ImportPrivateKeyInfoAndReturnKey(
        slot, &info, NULL /* nickname */, NULL /* publicValue */, PR_FALSE /* isPerm */,
        PR_FALSE /* isPrivate */, KU_ALL /* keyUsage */, &seckey, NULL);
