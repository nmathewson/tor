...
memwipe(void *mem, uint8_t byte, size_t sz)
{
     ...
     * have this function call "memset".  A smart compiler could inline it,
     * then eliminate dead memsets, and declare itself to be clever. */
     ...
     * the caller should have been using memset(), and the memset() wouldn't
     *get eliminated.  In other words, this is here so that we won't break
     *anything if somebody accidentally calls memwipe() instead of memset().
     **/
    memset(mem, byte, sz);
}
