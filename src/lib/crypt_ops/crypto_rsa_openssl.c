...
crypto_pk_copy_full(crypto_pk_t *env)
{
    ...
    if (!new_key) {
        ...
        crypto_openssl_log_errors(LOG_ERR, privatekey ? "Duplicating a private key"
                         ...
                         int padding)
{
    ...
    r = RSA_public_encrypt((int)fromlen, (unsigned char *)from, (unsigned char *)to, env->key,
 ...
 */
int
crypto_pk_private_decrypt(crypto_pk_t *env, char *to, size_t tolen, const char *from,
                          size_t fromlen, int padding, int warnOnFailure)
{
    ...
    r = RSA_private_decrypt((int)fromlen, (unsigned char *)from, (unsigned char *)to, env->key,
                            crypto_get_rsa_padding(padding));
          ...
          (const crypto_pk_t *env, char *to, size_t tolen, const char *from, size_t fromlen))
{
    ...
    tor_assert(fromlen < INT_MAX);
    tor_assert(tolen >= crypto_pk_keysize(env));
    r = RSA_public_decrypt((int)fromlen, (unsigned char *)from, (unsigned char *)to, env->key,
                           RSA_PKCS1_PADDING);
 ...
 */
int
crypto_pk_private_sign(const crypto_pk_t *env, char *to, size_t tolen, const char *from,
                       size_t fromlen)
{
    int r;
    tor_assert(env);
    ...
    r = RSA_private_encrypt((int)fromlen, (unsigned char *)from, (unsigned char *)to,
 ...
 */
int
crypto_pk_asn1_encode_private(const crypto_pk_t *pk, char *dest, size_t dest_len)
{
    int len;
    unsigned char *buf = NULL;
