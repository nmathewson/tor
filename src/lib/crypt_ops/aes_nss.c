...
aes_new_cipher(const uint8_t *key, const uint8_t *iv, int key_bits)
{
    ...
    SECItem keyItem = {.type = siBuffer, .data = (unsigned char *)key, .len = (key_bits / 8)};
    CK_AES_CTR_PARAMS params;
    params.ulCounterBits = 128;
    memcpy(params.cb, iv, 16);
    SECItem ivItem = {.type = siBuffer, .data = (unsigned char *)&params, .len = sizeof(params)};
    ...
    keyObj = PK11_ImportSymKey(slot, ckm, PK11_OriginUnwrap, CKA_ENCRYPT, &keyItem, NULL);
    if (!keyObj)
        goto err;
