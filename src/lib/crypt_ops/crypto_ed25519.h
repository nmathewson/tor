...
typedef struct ed25519_secret_key_t {
     ...
     * expands it into a 64-byte H(k), using the first 32 bytes for a
     * multiplier of the base point, and second 32 bytes as an input to a hash
     * function for deriving r.  But because we implement key blinding, we need
     * to store keys in the 64-byte expanded form. */
