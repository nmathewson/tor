...
crypto_dh_init_nss(void)
{
    ...
    int r;
    r = base16_decode((char *)tls_dh_prime_data, sizeof(tls_dh_prime_data), TLS_DH_PRIME,
                      strlen(TLS_DH_PRIME));
    tor_assert(r == DH1024_KEY_LEN);
    r = base16_decode((char *)circuit_dh_prime_data, sizeof(circuit_dh_prime_data), OAKLEY_PRIME_2,
...
ssize_t
crypto_dh_handshake(int severity, crypto_dh_t *dh, const char *pubkey, size_t pubkey_len,
                    unsigned char *secret_out, size_t secret_bytes_out)
{
    ...
    PK11SymKey *sym =
        PK11_PubDerive(dh->seckey, &peer_key, PR_FALSE, NULL, NULL, CKM_DH_PKCS_DERIVE,
