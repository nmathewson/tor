 ...
 **/
static int
digest_nss_internal(SECOidTag alg, char *digest, unsigned len_out, const char *msg, size_t msg_len)
{
...
int
crypto_digest256(char *digest, const char *m, size_t len, digest_algorithm_t algorithm)
{
    tor_assert(m);
    tor_assert(digest);
    ...
    if (algorithm == DIGEST_SHA256) {
        ...
        ret = (sha3_256((uint8_t *)digest, DIGEST256_LEN, (const uint8_t *)m, len) > -1);
    }
 ...
 * into <b>digest</b>.  Return 0 on success, -1 on failure. */
int
crypto_digest512(char *digest, const char *m, size_t len, digest_algorithm_t algorithm)
{
    tor_assert(m);
    tor_assert(digest);
    ...
    if (algorithm == DIGEST_SHA512) {
        ...
        ret = (sha3_512((uint8_t *)digest, DIGEST512_LEN, (const uint8_t *)m, len) > -1);
    }
...
#define END_OF_FIELD(f) (offsetof(crypto_digest_t, f) + STRUCT_FIELD_SIZE(crypto_digest_t, f))
    switch (alg) {
    case DIGEST_SHA1:
        FALLTHROUGH;
    case DIGEST_SHA256:
...
crypto_digest_add_bytes(crypto_digest_t *digest, const char *data, size_t len)
{
    ...
    case DIGEST_SHA512:
        ...
        SECStatus s = PK11_DigestOp(digest->d.ctx, (const unsigned char *)data, (unsigned int)len);
...
crypto_digest_get_digest(crypto_digest_t *digest, char *out, size_t out_len)
{
    ...
    if (digest->algorithm == DIGEST_SHA3_256 || digest->algorithm == DIGEST_SHA3_512) {
    ...
    unsigned rlen;
    unsigned char *saved = PK11_SaveContextAlloc(digest->d.ctx, buf, sizeof(buf), &saved_len);
    tor_assert(saved);
 ...
 */
void
crypto_digest_checkpoint(crypto_digest_checkpoint_t *checkpoint, const crypto_digest_t *digest)
{
    const size_t bytes = crypto_digest_alloc_bytes(digest->algorithm);
    tor_assert(bytes <= sizeof(checkpoint->mem));
    if (library_supports_digest(digest->algorithm)) {
        unsigned char *allocated;
        allocated = PK11_SaveContextAlloc(digest->d.ctx, (unsigned char *)checkpoint->mem,
...
void
crypto_digest_restore(crypto_digest_t *digest, const crypto_digest_checkpoint_t *checkpoint)
{
    const size_t bytes = crypto_digest_alloc_bytes(digest->algorithm);
    if (library_supports_digest(digest->algorithm)) {
        SECStatus s = PK11_RestoreContext(digest->d.ctx, (unsigned char *)checkpoint->mem,
 ...
 * <b>out_len</b> must be \<= DIGEST512_LEN. */
void
crypto_digest_smartlist(char *digest_out, size_t len_out, const smartlist_t *lst,
...
void
crypto_digest_smartlist_prefix(char *digest_out, size_t len_out, const char *prepend,
                               const smartlist_t *lst, const char *append, digest_algorithm_t alg)
{
    crypto_digest_t *d = crypto_digest_new_internal(alg);
    if (prepend)
        crypto_digest_add_bytes(d, prepend, strlen(prepend));
    SMARTLIST_FOREACH(lst, const char *, cp, crypto_digest_add_bytes(d, cp, strlen(cp)));
    if (append)
 ...
 */
void
crypto_hmac_sha256(char *hmac_out, const char *key, size_t key_len, const char *msg, size_t msg_len)
{
    ...
    if (!slot)
        goto done;
    symKey = PK11_ImportSymKey(slot, CKM_SHA256_HMAC, PK11_OriginUnwrap, CKA_SIGN, &keyItem, NULL);
    if (!symKey)
        goto done;

    hmac = PK11_CreateContextBySymKey(CKM_SHA256_HMAC, CKA_SIGN, symKey, &paramItem);
    if (!hmac)
        goto done;
