...
crypto_rand_uint(unsigned limit)
{
    ...
    IMPLEMENT_RAND_UNSIGNED(unsigned, UINT_MAX, limit, crypto_rand((char *)&val, sizeof(val)));
}
...
crypto_rand_uint64(uint64_t max)
{
    ...
    IMPLEMENT_RAND_UNSIGNED(uint64_t, UINT64_MAX, max, crypto_rand((char *)&val, sizeof(val)));
}
 ...
 */
uint64_t
crypto_fast_rng_uint64_range(crypto_fast_rng_t *rng, uint64_t min, uint64_t max)
{
