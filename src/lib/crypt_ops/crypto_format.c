...
ssize_t
crypto_read_tagged_contents_from_file(const char *fname, const char *typestring, char **tag_out,
                                      uint8_t *data_out, ssize_t data_out_len)
{
    ...
    *tag_out =
        tor_strndup(prefix + 5 + strlen(typestring), strlen(prefix) - 8 - strlen(typestring));
...
curve25519_public_to_base64(char *output, const curve25519_public_key_t *pkey)
{
    char buf[128];
    int n =
        base64_encode(buf, sizeof(buf), (const char *)pkey->public_key, CURVE25519_PUBKEY_LEN, 0);
...
ed25519_signature_from_base64(ed25519_signature_t *sig, const char *input)
{
    if (strlen(input) != ED25519_SIG_BASE64_LEN)
        return -1;
    char decoded[128];
    int n = base64_decode(decoded, sizeof(decoded), input, ED25519_SIG_BASE64_LEN);
...
{
    char buf[256];
    int n = base64_encode_nopad(buf, sizeof(buf), (const uint8_t *)digest, DIGEST_LEN);
    /* These asserts should always succeed, unless there is a bug in
     * base64_encode_nopad(). */
...
{
    char buf[256];
    int n = base64_encode_nopad(buf, sizeof(buf), (const uint8_t *)digest, DIGEST256_LEN);
    /* These asserts should always succeed, unless there is a bug in
     * base64_encode_nopad(). */
