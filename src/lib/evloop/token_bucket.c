 ...
 */
void
token_bucket_raw_reset(token_bucket_raw_t *bucket, const token_bucket_cfg_t *cfg)
{
...
void
token_bucket_raw_adjust(token_bucket_raw_t *bucket, const token_bucket_cfg_t *cfg)
{
 ...
 */
int
token_bucket_raw_refill_steps(token_bucket_raw_t *bucket, const token_bucket_cfg_t *cfg,
...
rate_per_sec_to_rate_per_step(uint32_t rate)
{
    ...
    */
    ...
    uint32_t val = (uint32_t)(monotime_coarse_stamp_units_to_approx_msec(units) / 1000);
    return val ? val : 1;
}
 ...
 */
void
token_bucket_rw_init(token_bucket_rw_t *bucket, uint32_t rate, uint32_t burst, uint32_t now_ts)
{
 ...
 */
void
token_bucket_rw_adjust(token_bucket_rw_t *bucket, uint32_t rate, uint32_t burst)
{
    token_bucket_cfg_init(&bucket->cfg, rate_per_sec_to_rate_per_step(rate), burst);
...
token_bucket_rw_refill(token_bucket_rw_t *bucket, uint32_t now_ts)
{
    ...
    int flags = 0;
    if (token_bucket_raw_refill_steps(&bucket->read_bucket, &bucket->cfg, elapsed_steps))
        flags |= TB_READ;
    if (token_bucket_raw_refill_steps(&bucket->write_bucket, &bucket->cfg, elapsed_steps))
        flags |= TB_WRITE;
 ...
 */
int
token_bucket_rw_dec(token_bucket_rw_t *bucket, ssize_t n_read, ssize_t n_written)
{
    int flags = 0;
 ...
 * full. */
void
token_bucket_ctr_init(token_bucket_ctr_t *bucket, uint32_t rate, uint32_t burst, uint32_t now_ts)
{
 ...
 * <b>bucket</b>. */
void
token_bucket_ctr_adjust(token_bucket_ctr_t *bucket, uint32_t rate, uint32_t burst)
{
