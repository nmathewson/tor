    ...
    for (i = WORKQUEUE_PRIORITY_FIRST; i <= WORKQUEUE_PRIORITY_LAST; ++i) {
        ...
        if (!TOR_TAILQ_EMPTY(this_queue)) {
            ...
            if (!crypto_fast_rng_one_in_n(get_thread_fast_rng(),
                ...
                /* Usually we'll just break now, so that we can get out of the
                 * loop and use the queue where we found work. But with a small
                 * probability, we'll keep looking for lower priority work, so
                 * that we don't ignore our low-priority queues entirely. */
                break;
            }
        }
    }
...
worker_thread_main(void *thread_)
{
    ...
    while (1) {
        ...
        /* At this point the lock is held, and there is no work in this
         * thread's queue. */
...
threadpool_queue_work(threadpool_t *pool,
                      ...
                      void (*reply_fn)(void *), void *arg)
{
    return threadpool_queue_work_priority(pool, WQ_PRI_HIGH, fn, reply_fn,
                                          arg);
}
...
threadpool_start_threads(threadpool_t *pool, int n)
{
    ...
    while (pool->n_threads < n) {
        ...
        int32_t chance =
            (pool->n_threads & 1) ? CHANCE_STRICT : CHANCE_PERMISSIVE;
