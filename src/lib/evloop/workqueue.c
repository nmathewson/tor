...
workqueue_entry_new(workqueue_reply_t (*fn)(void *, void *), void (*reply_fn)(void *), void *arg)
{
...
worker_thread_extract_next_work(workerthread_t *thread)
{
    ...
    for (i = WORKQUEUE_PRIORITY_FIRST; i <= WORKQUEUE_PRIORITY_LAST; ++i) {
        ...
        if (!TOR_TAILQ_EMPTY(this_queue)) {
            queue = this_queue;
            if (!crypto_fast_rng_one_in_n(get_thread_fast_rng(), thread->lower_priority_chance)) {
    ...
    while (1) {
        ...
        while (worker_thread_has_work(thread)) {
            ...
            if (thread->in_pool->generation != thread->generation) {
                ...
                workqueue_reply_t (*update_fn)(void *, void *) = thread->in_pool->update_fn;
...
workerthread_new(int32_t lower_priority_chance, void *state, threadpool_t *pool,
...
threadpool_queue_work_priority(threadpool_t *pool, workqueue_priority_t prio,
                               ...
                               void *arg)
{
    tor_assert(((int)prio) >= WORKQUEUE_PRIORITY_FIRST && ((int)prio) <= WORKQUEUE_PRIORITY_LAST);
...
workqueue_entry_t *
threadpool_queue_work(threadpool_t *pool, workqueue_reply_t (*fn)(void *, void *),
...
threadpool_queue_update(threadpool_t *pool, void *(*dup_fn)(void *),
                        workqueue_reply_t (*fn)(void *, void *), void (*free_fn)(void *), void *arg)
{
    int i, n_threads;
...
threadpool_start_threads(threadpool_t *pool, int n)
{
    ...
    if (pool->n_threads < n)
        pool->threads = tor_reallocarray(pool->threads, sizeof(workerthread_t *), n);
    ...
    while (pool->n_threads < n) {
        ...
        workerthread_t *thr = workerthread_new(chance, state, pool, pool->reply_queue);
 ...
 */
threadpool_t *
threadpool_new(int n_threads, replyqueue_t *replyqueue, void *(*new_thread_state_fn)(void *),
...
threadpool_register_reply_event(threadpool_t *tp, void (*cb)(threadpool_t *tp))
{
    ...
    tp->reply_event = tor_event_new(base, tp->reply_queue->alert.read_fd, EV_READ | EV_PERSIST,
                                    reply_event_cb, tp);
...
replyqueue_process(replyqueue_t *queue)
{
    ...
    if (r < 0) {
        ...
        log_fn_ratelim(&warn_limit, LOG_WARN, LD_GENERAL, "Failure from drain_fd: %s",
