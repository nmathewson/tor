...
libevent_logging_callback(int severity, const char *msg)
{
    ...
    case _EVENT_LOG_ERR:
        ...
        break;
    default:
        log_warn(LD_NOCB | LD_GENERAL, "Message [%d] from libevent: %s", severity, buf);
        break;
    }
}
...
tor_libevent_initialize(tor_libevent_cfg_t *torcfg)
{
    ...
    rescan_mainloop_ev = event_new(the_event_base, -1, 0, rescan_mainloop_cb, the_event_base);
    ...
    log_info(LD_GENERAL, "Initialized libevent version %s using method %s. Good.",
                   ...
                   void (*cb)(periodic_timer_t *timer, void *data), void *data)
{
    ...
    if (!(timer->ev = tor_event_new(base, -1, EV_PERSIST, periodic_timer_cb, timer))) {
        tor_free(timer);
        return NULL;
    }
    timer->cb = cb;
...
mainloop_event_new_impl(int postloop, void (*cb)(mainloop_event_t *, void *), void *userdata)
{
    tor_assert(cb);
    ...
    mev->ev =
        tor_event_new(base, -1, 0, postloop ? mainloop_event_postloop_cb : mainloop_event_cb, mev);
    tor_assert(mev->ev);
    mev->cb = cb;
 ...
 */
mainloop_event_t *
mainloop_event_postloop_new(void (*cb)(mainloop_event_t *, void *), void *userdata)
{
...
void
tor_libevent_exit_loop_after_delay(struct event_base *base, const struct timeval *delay)
{
