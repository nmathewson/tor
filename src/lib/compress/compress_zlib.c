...
#define A_FEW_KILOBYTES 2048
    ...
    if (inflate_) {
        /* From zconf.h:
           ...
           for small objects."
        */
        return sizeof(tor_zlib_compress_state_t) + sizeof(struct z_stream_s) + (1 << 15) +
               A_FEW_KILOBYTES;
    } else {
        /* Also from zconf.h:
                      ...
                      compression_level_t compression_level)
{
    ...
    if (compress_) {
        if (deflateInit2(&out->stream, Z_BEST_COMPRESSION, Z_DEFLATED, bits, memlevel,
...
tor_zlib_compress_process(tor_zlib_compress_state_t *state, char **out, size_t *out_len,
