...
tor_zstd_state_size_precalc_fake(int compress, int preset)
{
    ...
    // The Zstandard library provides a number of functions that would be
    // useful here, but they are, unfortunately, still considered experimental
    // and are thus only available in libzstd if we link against the library
    // statically.
    //
    ...
    //
    // - We cannot use ZSTD_sizeof_CStream() and ZSTD_sizeof_DStream() since
    // they
    //   are unexposed.
    //
    ...
    if (compress) {
        ...
        // simplified function to calculate the estimated amount of bytes used
        // in this struct. memory_usage += (preset - 0.5) * 1024 * 1024;
                          ...
                          int finish)
{
    ...
    if (state->compress) {
        if (!state->have_called_end)
            retval =
                ZSTD_compressStream(state->u.compress_stream, &output, &input);
        else
            retval = 0;
    } else {
        retval =
    ...
    *in_len = input.size - input.pos;
    ...
    if (!finish) {
        // The caller says we're not done with the input, so no need to write
        // an epilogue.
        ...
        *out_len = output.size - output.pos;
        ...
        if (retval == 0)
            return TOR_COMPRESS_DONE;
        // Don't check out_len, it might have some space left if the next
        // output chunk is larger than the remaining space
        else if (*in_len > 0)
            return TOR_COMPRESS_BUFFER_FULL;
        else
            return TOR_COMPRESS_OK;
