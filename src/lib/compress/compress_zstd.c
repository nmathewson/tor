...
{
    tor_snprintf(buf, buflen, "%u.%u.%u", version_number / 10000 % 100, version_number / 100 % 100,
                 version_number % 100);
}
#endif /* defined(HAVE_ZSTD) */
...
tor_zstd_get_version_str(void)
{
    ...
    tor_zstd_format_version(version_str, sizeof(version_str), ZSTD_versionNumber());
...
tor_zstd_compress_new(int compress, compress_method_t method, compression_level_t level)
{
...
tor_zstd_compress_process(tor_zstd_compress_state_t *state, char **out, size_t *out_len,
                          const char **in, size_t *in_len, int finish)
{
    ...
    if (state->compress) {
        ...
        retval = ZSTD_decompressStream(state->u.decompress_stream, &output, &input);
    }
    ...
    *in_len = input.size - input.pos;
    ...
    if (!state->compress &&
        tor_compress_is_compression_bomb(state->input_so_far, state->output_so_far)) {
    ...
    if (ZSTD_isError(retval)) {
        log_warn(LD_GENERAL, "Zstandard %s didn't finish: %s.",
                 state->compress ? "compression" : "decompression", ZSTD_getErrorName(retval));
        return TOR_COMPRESS_ERROR;
    }
...
tor_zstd_warn_if_version_mismatched(void)
{
    ...
    if (!tor_zstd_can_use_static_apis()) {
        ...
        tor_zstd_format_version(header_version, sizeof(header_version), ZSTD_VERSION_NUMBER);
        tor_zstd_format_version(runtime_version, sizeof(runtime_version), ZSTD_versionNumber());
