                  ...
                  int protocol_warn_level)
{
    ...
    *out = outptr = tor_malloc(out_remaining);
    ...
    while (1) {
        switch (tor_compress_process(stream, &outptr, &out_remaining, &in,
                                     &in_len, finish)) {
            case TOR_COMPRESS_DONE:
                if (in_len == 0 || compress) {
                    goto done;
                } else {
                    // More data is present, and we're decompressing.  So we may
                    // need to reinitialize the stream if we are handling
                    // multiple concatenated inputs.
                    tor_compress_free(stream);
                    stream =
                        tor_compress_new(compress, method, compression_level);
                    if (stream == NULL) {
                        log_warn(LD_GENERAL, "NULL stream while %scompressing",
                                 compress ? "" : "de");
                        goto err;
            ...
            case TOR_COMPRESS_BUFFER_FULL: {
                ...
                if (!compress &&
                    ...
                    // This should already have been caught down in the backend
                    // logic.
                    // LCOV_EXCL_START
                     ...
                     const char **in, size_t *in_len, int finish)
{
    ...
    if (*out_len == 0 && (*in_len > 0 || finish)) {
        // If we still have input data, but no space for output data, we might
        // as well return early and let the caller do the reallocation of the
        // out variable.
    ...
    switch (state->method) {
        case GZIP_METHOD:
        case ZLIB_METHOD:
            rv = tor_zlib_compress_process(state->u.zlib_state, out, out_len,
                                           in, in_len, finish);
            break;
        case LZMA_METHOD:
            rv = tor_lzma_compress_process(state->u.lzma_state, out, out_len,
                                           in, in_len, finish);
            break;
        case ZSTD_METHOD:
            rv = tor_zstd_compress_process(state->u.zstd_state, out, out_len,
                                           in, in_len, finish);
            break;
        case NO_METHOD:
