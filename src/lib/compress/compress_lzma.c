...
tor_lzma_compress_new(int compress, compress_method_t method, compression_level_t level)
{
    ...
    if (compress) {
        ...
        if (retval != LZMA_OK) {
            // LCOV_EXCL_START
            log_warn(LD_GENERAL, "Error from LZMA encoder: %s (%u).", lzma_error_str(retval),
                     retval);
            goto err;
            // LCOV_EXCL_STOP
        }
        ...
        if (retval != LZMA_OK) {
            // LCOV_EXCL_START
            log_warn(LD_GENERAL, "Error from LZMA decoder: %s (%u).", lzma_error_str(retval),
                     retval);
            goto err;
            // LCOV_EXCL_STOP
        }
...
tor_lzma_compress_process(tor_lzma_compress_state_t *state, char **out, size_t *out_len,
    ...
    *in_len = state->stream.avail_in;
    ...
    default:
        log_warn(LD_GENERAL, "LZMA %s didn't finish: %s.",
                 state->compress ? "compression" : "decompression", lzma_error_str(retval));
        return TOR_COMPRESS_ERROR;
    }
#else /* !defined(HAVE_LZMA) */
    (void)state;
