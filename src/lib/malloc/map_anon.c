...
nodump_mem(void *mem, size_t sz)
{
    ...
    if (rv == 0) {
        ...
        tor_log_err_sigsafe("Unexpected error from madvise: ", strerror(errno), NULL);
        return -1;
    }
...
noinherit_mem(void *mem, size_t sz, inherit_res_t *inherit_result_out)
{
    ...
    if (errno == ENOSYS || errno == EINVAL) {
        ...
        tor_log_err_sigsafe("Unexpected error from minherit: ", strerror(errno), NULL);
        return -1;
    }
 ...
 */
void *
tor_mmap_anonymous(size_t sz, unsigned flags, inherit_res_t *inherit_result_out)
{
    void *ptr;
...
#if defined(_WIN32)
    HANDLE mapping =
        CreateFileMapping(INVALID_HANDLE_VALUE, NULL, /*attributes*/
                          PAGE_READWRITE, HIGH_SIZE_T_BYTES(sz), sz & 0xffffffff, NULL /* name */);
