...
tor_malloc_(size_t size)
{
    ...
    if (PREDICT_UNLIKELY(result == NULL)) {
         ...
         * spawn_exit, but that's ok, since the parent will run out of memory
         * soon anyway. */
...
tor_malloc_zero_(size_t size)
{
     ...
     * malloc+memset?  Perhaps libc's calloc knows some nifty optimization
     * trick we don't!"  Indeed it does, but its optimizations are only a big
     * win when we're allocating something very big (it knows if it just got
     * the memory from the OS in a pre-zeroed state).  We don't want to use
     * tor_malloc_zero for big stuff, so we don't bother with calloc. */
