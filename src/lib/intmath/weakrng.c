...
tor_weak_random(tor_weak_rng_t *rng)
{
     ...
     * range 0..INT32_MAX. We don't want to use the platform rand() or
     * random(), since some platforms have bad weak RNGs that only return
     * values in the range 0..INT16_MAX, which just isn't enough. */
...
{
    /* We don't want to just do tor_weak_random() % top, since random() is
     * often implemented with an LCG whose modulus is a power of 2, and those
     * are cyclic in their low-order bits. */
    int divisor, result;
    raw_assert(top > 0);
