...
static void config_mgr_register_fmt(config_mgr_t *mgr,
                                    const config_format_t *fmt, int object_idx);
    ...
    while (*value) {
        switch (*value) {
            ...
            case '#':
                 ...
                 * using quotes, not otherwise, so they don't trigger escaping
                 * on their own. */
                return 1;
            default:
 ...
 **/
validation_status_t
config_validate(const config_mgr_t *mgr, const void *old_options, void *options,
                char **msg_out)
{
