...
config_new(const config_mgr_t *mgr)
{
    ...
    if (suitep) {
        *suitep = config_suite_new();
        SMARTLIST_FOREACH_BEGIN (mgr->subconfigs, const config_format_t *,
                                 fmt) {
...
{
    SMARTLIST_FOREACH_BEGIN (mgr->all_abbrevs, const config_abbrev_t *,
                             abbrev) {
...
config_find_deprecation(const config_mgr_t *mgr, const char *key)
{
    ...
    SMARTLIST_FOREACH_BEGIN (mgr->all_deprecations,
                             const config_deprecation_t *, d) {
                    ...
                    bool allow_truncated, int *idx_out)
{
    ...
    if (!keylen)
        return NULL; /* if they say "--" on the command line, it's not an
                        option */
...
config_var_is_gettable(const config_var_t *var)
{
     ...
     * they have been gettable for a long time, and making them ungettable
     * could have compatibility effects.  For now, let's leave them alone.
     */
...
config_mark_lists_fragile(const config_mgr_t *mgr, void *options)
{
    ...
    SMARTLIST_FOREACH_BEGIN (mgr->all_vars, const managed_var_t *, mv) {
        void *object =
            config_mgr_get_obj_mutable(mgr, options, mv->object_idx);
...
config_value_needs_escape(const char *value)
{
    ...
    while (*value) {
        ...
        case '\n':
        case '#':
            /* Note: quotes and backspaces need special handling when we are
             * using quotes, not otherwise, so they don't trigger escaping on
             * their own. */
            return 1;
        default:
...
config_dup(const config_mgr_t *mgr, const void *old)
{
    ...
    SMARTLIST_FOREACH_BEGIN (mgr->all_vars, managed_var_t *, mv) {
        if (!config_var_needs_copy(mv->cvar)) {
            // Something else will copy this option, or it doesn't need
            // copying.
            continue;
        }
        const void *oldobj = config_mgr_get_obj(mgr, old, mv->object_idx);
        void *newobj =
            config_mgr_get_obj_mutable(mgr, newopts, mv->object_idx);
                       ...
                       void *options, char **msg_out)
{
    ...
    if (old_options) {
        if (config_check_immutable_flags(fmt, old_options, options, msg_out) <
            0) {
    ...
    SMARTLIST_FOREACH_BEGIN (mgr->all_vars, managed_var_t *, mv) {
        ...
        for (; line; line = line->next) {
            if (!strcmpstart(line->key, "__")) {
                /* This check detects "hidden" variables inside LINELIST_V
                 * structures.
                 */
                continue;
            }
