...
static void config_reset(const config_mgr_t *fmt, void *options, const managed_var_t *var,
                         int use_defaults);
...
static void
config_mgr_register_fmt(config_mgr_t *mgr, const config_format_t *fmt, int object_idx)
{
    int i;

    tor_assertf(!mgr->frozen, "Tried to add a format to a configuration manager after "
                              "it had been frozen.");
    ...
    tor_assertf(fmt != mgr->toplevel && !smartlist_contains(mgr->subconfigs, fmt),
...
config_mgr_get_obj_mutable(const config_mgr_t *mgr, void *toplevel, int idx)
{
    ...
    tor_assertf(idx >= 0 && idx < smartlist_len(mgr->subconfigs), "Index %d is out of range.", idx);
    config_suite_t **suite = config_mgr_get_suite_ptr(mgr, toplevel);
    tor_assert(suite);
    tor_assert(smartlist_len(mgr->subconfigs) == smartlist_len((*suite)->configs));
...
config_mgr_freeze(config_mgr_t *mgr)
{
    ...
    memcpy(&mgr->toplevel_magic, &mgr->toplevel->magic, sizeof(struct_magic_decl_t));
    uint64_t magic_input[3] = {mgr->toplevel_magic.magic_val, (uint64_t)(uintptr_t)mgr,
                               ++mgr_count};
    mgr->toplevel_magic.magic_val = (uint32_t)siphash24g(magic_input, sizeof(magic_input));
    mgr->frozen = true;
}
...
config_mgr_list_vars(const config_mgr_t *mgr)
{
    ...
    tor_assert(mgr);
    SMARTLIST_FOREACH(mgr->all_vars, managed_var_t *, mv, smartlist_add(result, (void *)mv->cvar));
    return result;
}
...
config_mgr_assert_magic_ok(const config_mgr_t *mgr, const void *options)
{
    ...
    tor_assert(smartlist_len((*suitep)->configs) == smartlist_len(mgr->subconfigs));
...
const char *
config_expand_abbrev(const config_mgr_t *mgr, const char *option, int command_line,
                     int warn_obsolete)
{
...
config_find_deprecation(const config_mgr_t *mgr, const char *key)
{
    ...
    SMARTLIST_FOREACH_BEGIN (mgr->all_deprecations, const config_deprecation_t *, d) {
...
config_mgr_find_var(const config_mgr_t *mgr, const char *key, bool allow_truncated, int *idx_out)
{
 ...
 */
static int
config_assign_value(const config_mgr_t *mgr, void *options, config_line_t *c, char **msg)
{
 ...
 */
static int
config_assign_line(const config_mgr_t *mgr, void *options, config_line_t *c, unsigned flags,
                   bitarray_t *options_seen, char **msg)
{
    ...
    if (!mvar) {
        ...
        if (fmt->extra) {
            ...
            log_info(LD_CONFIG, "Found unrecognized option '%s'; saving it.", c->key);
    ...
    if (warn_deprecations && (deprecation_msg = config_find_deprecation(mgr, cvar->member.name))) {
    ...
    if (!strlen(c->value)) {
        ...
        if (!clear_first) {
            if (!config_var_is_replaced_on_set(cvar) && c->command != CONFIG_LINE_CLEAR) {
                /* We got an empty linelist from the torrc or command line.
                   As a special case, call this an error. Warn and ignore. */
                log_warn(LD_CONFIG, "Linelist option '%s' has no value. Skipping.", c->key);
 ...
 * Called from config_assign(). */
STATIC void
config_reset_line(const config_mgr_t *mgr, void *options, const char *key, int use_defaults)
{
    const managed_var_t *var;
...
config_line_t *
config_get_assigned_option(const config_mgr_t *mgr, const void *options, const char *key,
                           int escape_val)
{
    const managed_var_t *var;
    ...
    if (!config_var_is_gettable(var->cvar)) {
        log_warn(LD_CONFIG, "Option '%s' is obsolete or unfetchable. Failing.", key);
        return NULL;
    }
              ...
              unsigned config_assign_flags, char **msg)
{
    ...
    while (list) {
        int r;
        if ((r = config_assign_line(mgr, options, list, config_assign_flags, options_seen, msg))) {
...
static void
config_reset(const config_mgr_t *mgr, void *options, const managed_var_t *var, int use_defaults)
{
    config_line_t *c;
    char *msg = NULL;
    CONFIG_CHECK(mgr, options);
...
config_free_(const config_mgr_t *mgr, void *options)
{
    ...
    if (mgr->toplevel->extra) {
        config_line_t **linep = STRUCT_VAR_P(options, mgr->toplevel->extra->offset);
 ...
 */
int
config_is_same(const config_mgr_t *mgr, const void *o1, const void *o2, const char *name)
{
    CONFIG_CHECK(mgr, o1);
    CONFIG_CHECK(mgr, o2);
 ...
 **/
config_line_t *
config_get_changes(const config_mgr_t *mgr, const void *options1, const void *options2)
{
    ...
    SMARTLIST_FOREACH_BEGIN (mgr->all_vars, managed_var_t *, mv) {
        ...
        config_line_t *line = config_get_assigned_option(mgr, options2, varname, 1);
 ...
 */
static int
config_check_immutable_flags(const config_format_t *fmt, const void *old_options,
                             const void *new_options, char **msg_out)
{
    tor_assert(fmt);
...
config_validate_single(const config_format_t *fmt, const void *old_options, void *options,
                       char **msg_out)
{
    tor_assert(fmt);
    tor_assert(options);
 ...
 **/
validation_status_t
config_validate(const config_mgr_t *mgr, const void *old_options, void *options, char **msg_out)
{
 ...
 */
char *
config_dump(const config_mgr_t *mgr, const void *default_options, const void *options, int minimal,
