 ...
 **/
int
typed_var_assign(void *target, const char *value, char **errmsg, const var_type_def_t *def)
{
    if (BUG(!def))
 ...
 */
config_line_t *
typed_var_kvencode(const char *key, const void *value, const var_type_def_t *def)
{
    if (BUG(!def))
        return NULL; // LCOV_EXCL_LINE
...
typed_var_copy(void *dest, const void *src, const var_type_def_t *def)
{
    ...
    if (BUG(rv < 0)) {
        // LCOV_EXCL_START
        log_warn(LD_BUG, "Encoded value %s was not parseable as a %s: %s", escaped(enc), def->name,
                 err ? err : "");
        // LCOV_EXCL_STOP
    }
    tor_free(err);
