                ...
                const void *params)
{
    ...
    *v = config_parse_units(value, table, &ok, &msg);
    if (!ok) {
        tor_asprintf(errmsg,
                     "Provided value is malformed or out of bounds: %s", msg);
        tor_free(msg);
        return -1;
    }
    if (BUG(msg)) {
             ...
             const void *params)
{
    ...
    *v = strtod(value, &endptr);
    if (endptr == value || *endptr != '\0') {
        // Either there are no converted characters, or there were some
        // characters that didn't get converted.
        tor_asprintf(errmsg, "Could not convert %s to a number.",
                     escaped(value));
        return -1;
    }
    if (errno == ERANGE) {
...
enum_parse(void *target, const char *value, char **errmsg, const void *params_)
{
    ...
    tor_asprintf(errmsg, "Unrecognized value %s. %s", value,
                 params->allowed_val_string);
    return -1;
}
...
static const enumeration_params_t enum_params_bool = {
    "Allowed values are 0 and 1.", enum_table_bool};
...
static const enumeration_params_t enum_params_autobool = {
    "Allowed values are 0, 1, and auto.", enum_table_autobool};
