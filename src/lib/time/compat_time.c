...
#    ifdef MONOTIME_COARSE_FN_IS_DIFFERENT
    memcpy(&initialized_at_coarse_saved, &initialized_at_coarse, sizeof(monotime_coarse_t));
...
#    ifdef MONOTIME_COARSE_FN_IS_DIFFERENT
    memcpy(&initialized_at_coarse, &initialized_at_coarse_saved, sizeof(monotime_coarse_t));
#    endif
}
...
#    ifdef TOR_UNIT_TESTS
    if (monotime_mocking_enabled) {
        out->abstime_ = (mock_time_nsec * mach_time_info.denom) / mach_time_info.numer;
        return;
    }
#    endif /* defined(TOR_UNIT_TESTS) */
...
#        ifdef TOR_UNIT_TESTS
    if (monotime_mocking_enabled) {
        out->abstime_ = (mock_time_nsec_coarse * mach_time_info.denom) / mach_time_info.numer;
        return;
    }
#        endif /* defined(TOR_UNIT_TESTS) */
...
monotime_diff_nsec(const monotime_t *start, const monotime_t *end)
{
    ...
    const int64_t diff_nsec = (diff_ticks * mach_time_info.numer) / mach_time_info.denom;
    return diff_nsec;
}

int32_t
monotime_coarse_diff_msec32_(const monotime_coarse_t *start, const monotime_coarse_t *end)
{
    if (BUG(mach_time_info.denom == 0)) {
        monotime_init();
    }
    ...
    if (diff_microticks >= mach_time_msec_cvt_threshold) {
        return (diff_microticks / mach_time_info_msec_cvt.denom) * mach_time_info_msec_cvt.numer;
    } else {
        return (diff_microticks * mach_time_info_msec_cvt.numer) / mach_time_info_msec_cvt.denom;
    }
}
...
monotime_diff_nsec(const monotime_t *start, const monotime_t *end)
{
    ...
    const int64_t diff_nsec = diff_sec * ONE_BILLION + (end->ts_.tv_nsec - start->ts_.tv_nsec);
...
monotime_init_internal(void)
{
    ...
    if (h) {
        GetTickCount64_fn =
            (GetTickCount64_fn_t)(void (*)(void))GetProcAddress(h, "GetTickCount64");
    }
...
#    ifdef TOR_UNIT_TESTS
    if (monotime_mocking_enabled) {
        out->pcount_ = (mock_time_nsec * nsec_per_tick_denom) / nsec_per_tick_numer;
        return;
    }
#    endif /* defined(TOR_UNIT_TESTS) */
...
int64_t
monotime_coarse_diff_msec(const monotime_coarse_t *start, const monotime_coarse_t *end)
{
...
int64_t
monotime_coarse_diff_usec(const monotime_coarse_t *start, const monotime_coarse_t *end)
{
...
void
monotime_coarse_add_msec(monotime_coarse_t *out, const monotime_coarse_t *val, uint32_t msec)
{
...
monotime_coarse_stamp_units_to_approx_msec(uint64_t units)
{
    ...
    return (abstime_diff * mach_time_info.numer) / (mach_time_info.denom * ONE_MILLION);
}
uint64_t
monotime_msec_to_approx_coarse_stamp_units(uint64_t msec)
{
    uint64_t abstime_val =
        (((uint64_t)msec) * ONE_MILLION * mach_time_info.denom) / mach_time_info.numer;
