...
monotime_init_internal(void)
{
    ...
    }
    {
        ...
        // For any value above this amount, we should divide before
        // multiplying, to avoid overflow.  For a value below this, we should
        // multiply before dividing, to improve accuracy.
        mach_time_msec_cvt_threshold =
            INT32_MAX / mach_time_info_msec_cvt.numer;
    }
}
...
monotime_add_msec(monotime_t *out, const monotime_t *val, uint32_t msec)
{
    ...
    const uint64_t ticks =
        (nsec * mach_time_info.denom) / mach_time_info.numer;
...
monotime_init_internal(void)
{
    ...
    if (h) {
        GetTickCount64_fn = (GetTickCount64_fn_t)(
            void (*)(void))GetProcAddress(h, "GetTickCount64");
    }
