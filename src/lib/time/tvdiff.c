...
tv_mdiff(const struct timeval *start, const struct timeval *end)
{
    ...
    /* Subtract and round */
    mdiff = secdiff * 1000 +
            /* We add a million usec here to ensure that the result is
             * positive, so that the round-towards-zero behavior of the
             * division will give the right result for rounding to the nearest
             * msec. Later we subtract 1000 in order to get the correct result.
             * We'll never get an overflow here, because we check that both
             * usecs are between 0 and TV_USEC_PER_SEC. */
