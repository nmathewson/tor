...
alloc_chunk(size_t sz)
{
    ...
    tor_assert(res->next_mem + res->mem_size + SENTINEL_LEN == ((char *)res) + chunk_size);
...
memarea_alloc(memarea_t *area, size_t sz)
{
    ...
    const size_t space_remaining = (chunk->U_MEM + chunk->mem_size) - chunk->next_mem;
...
memarea_assert_ok(memarea_t *area)
{
    ...
    for (chunk = area->first; chunk; chunk = chunk->next_chunk) {
        CHECK_SENTINEL(chunk);
        tor_assert(chunk->next_mem >= chunk->U_MEM);
        tor_assert(chunk->next_mem <= (char *)realign_pointer(chunk->U_MEM + chunk->mem_size));
    }
}
