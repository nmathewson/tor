...
static dispatch_typefns_t nop_typefns = {.free_fn = type_free_nop, .fmt_fn = type_fmt_nop};
...
dtbl_entry_from_lst(smartlist_t *receivers)
{
    ...
    dtbl_entry_t *ent;
    ent = tor_malloc_zero(offsetof(dtbl_entry_t, rcv) + sizeof(dispatch_rcv_t) * n_recv);
...
dispatch_new(const dispatch_cfg_t *cfg)
{
    ...
    const size_t n_msgs = MAX(smartlist_len(cfg->type_by_msg), smartlist_len(cfg->recv_by_msg)) + 1;

    /* Any channel that any message has counts towards the number of channels. */
    const size_t n_chans = (size_t)MAX(1, max_in_u16_sl(cfg->chan_by_msg, 0)) + 1;

    /* Any type that a message has, or that has functions, counts towards
     * the number of types. */
    const size_t n_types =
        (size_t)MAX(max_in_u16_sl(cfg->type_by_msg, 0), smartlist_len(cfg->fns_by_type)) + 1;
