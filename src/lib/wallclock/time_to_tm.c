 ...
 * <b>resultbuf</b>. */
static struct tm *
correct_tm(int islocal, const time_t *timep, struct tm *resultbuf, struct tm *r, char **err_out)
{
    const char *outcome;
    ...
    if (err_out) {
        tor_asprintf(err_out, "%s(%" PRId64 ") failed with error %s: %s",
                     islocal ? "localtime" : "gmtime", timep ? ((int64_t)*timep) : 0,
                     strerror(errno), outcome);
    }
    return r;
}
