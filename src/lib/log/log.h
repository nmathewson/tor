...
MOCK_DECL(int, add_file_log, (const log_severity_list_t *severity, const char *filename, int fd));

#ifdef HAVE_SYSLOG_H
int add_syslog_log(const log_severity_list_t *severity, const char *syslog_identity_tag);
#endif // HAVE_SYSLOG_H.
#ifdef HAVE_ANDROID_LOG_H
int add_android_log(const log_severity_list_t *severity, const char *android_identity_tag);
...
void tor_log(int severity, log_domain_mask_t domain, const char *format, ...) CHECK_PRINTF(3, 4);
...
void log_fn_(int severity, log_domain_mask_t domain, const char *funcname, const char *format, ...)
    CHECK_PRINTF(4, 5);
struct ratelim_t;
void log_fn_ratelim_(struct ratelim_t *ratelim, int severity, log_domain_mask_t domain,
                     const char *funcname, const char *format, ...) CHECK_PRINTF(5, 6);

int log_message_is_interesting(int severity, log_domain_mask_t domain);
void tor_log_string(int severity, log_domain_mask_t domain, const char *function,
                    const char *string);
...
#    define log_fn(severity, domain, args...) log_fn_(severity, domain, __FUNCTION__, args)
...
#    define log_debug(domain, args...)                          \
        ...
        STMT_END
#    define log_info(domain, args...) log_fn_(LOG_INFO, domain, __FUNCTION__, args)
#    define log_notice(domain, args...) log_fn_(LOG_NOTICE, domain, __FUNCTION__, args)
#    define log_warn(domain, args...) log_fn_(LOG_WARN, domain, __FUNCTION__, args)
 ...
 */
#    define log_fn_ratelim(ratelim, severity, domain, args, ...) \
        log_fn_ratelim_(ratelim, severity, domain, __FUNCTION__, args, ##__VA_ARGS__)
...
MOCK_DECL(STATIC void, logv,
          (int severity, log_domain_mask_t domain, const char *funcname, const char *suffix,
           const char *format, va_list ap)CHECK_PRINTF(5, 0));
