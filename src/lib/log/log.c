...
typedef struct logfile_t {
    ...
    int needs_close; /**< Boolean: true if the stream gets closed on shutdown.
                      */
    int is_temporary; /**< Boolean: close after initializing logging
                         subsystem.*/
...
#ifdef HAVE_SYSLOG_H
#    ifdef MAXLINE
        /* Some syslog implementations have limits on the length of what you
         * can pass them, and some very old ones do not detect overflow so
         * well. Regrettably, they call their maximum line length MAXLINE. */
#        if MAXLINE < 64
        ...
        if (domain & LD_NOCB) {
            if (!*callbacks_deferred && pending_cb_messages) {
                smartlist_add(pending_cb_messages,
                              pending_log_message_new(severity, domain, NULL,
                                                      msg_after_prefix));
           ...
           const char *suffix, const char *format, va_list ap))
{
    ...
    if (queue_startup_messages &&
        ...
        end_of_prefix = format_msg(buf, sizeof(buf), domain, severity,
                                   funcname, suffix, format, ap, &msg_len);
        formatted = 1;
...
tor_log_update_sigsafe_err_fds(void)
{
    ...
    /* log_fds and err_fds contain matching entries: log_fds are the fds used
     * by the log module, and err_fds are the fds used by the err module. For
     * stdio logs, the log_fd and err_fd values are identical, and the err
     * module closes the fd on shutdown. For file logs, the err_fd is a dup()
     * of the log_fd, and the log and err modules both close their respective
     * fds on shutdown. (Once all fds representing a file are closed, the
    ...
    for (lf = logfiles; lf; lf = lf->next) {
        /* Don't try callback to the control port, syslogs, android logs, or
         * any other non-file descriptor log: We can't call arbitrary functions
        ...
        if (lf->severities->masks[SEVERITY_MASK_IDX(LOG_ERR)] &
            ...
            log_fds[n_fds] = lf->fd;
            if (lf->needs_close) {
                /* File log fds are duplicated, because close_log() closes the
                 * log module's fd, and tor_log_close_sigsafe_err_fds() closes
                 * the err module's fd. Both refer to the same file. */
    ...
    if (!found_real_stderr &&
         ...
         * That's ok, max_fds is large enough that most tor instances don't
         * exceed it. */
        ...
        SMARTLIST_FOREACH_BEGIN (messages, pending_log_message_t *, msg) {
            ...
            for (lf = logfiles; lf; lf = lf->next) {
                if (!lf->callback || lf->seems_dead ||
                    !(lf->severities->masks[SEVERITY_MASK_IDX(severity)] &
                      domain)) {
                    continue;
                }
...
domain_to_string(log_domain_mask_t domain, char *buf, size_t buflen)
{
    ...
    while (1) {
        ...
        if ((unsigned)bit >= ARRAY_LENGTH(domain_list) - 1 ||
            ...
            tor_snprintf(buf, buflen, "<BUG:Unknown domain %lx>",
                         (long)domain);
        ...
        if (n >= buflen) {
            tor_snprintf(buf, buflen, "<BUG:Truncating domain %lx>",
                         (long)domain);
            return buf + strlen(buf);
        }
        cp += n;
                          ...
                          log_severity_list_t *severity_out)
{
    ...
    while (*cfg) {
        ...
        if (*cfg == '[') {
            ...
            smartlist_split_string(domains_list, domains_str, ",",
                                   SPLIT_SKIP_SPACE, -1);
            ...
            SMARTLIST_FOREACH_BEGIN (domains_list, const char *, domain) {
                if (!strcmp(domain, "*")) {
                    ...
                    if (!d) {
                        log_warn(LD_CONFIG, "No such logging domain as %s",
                                 domain);
                        err = 1;
                    } else {
                        if (negate)
