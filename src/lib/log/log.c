...
static inline char *format_msg(char *buf, size_t buf_len, log_domain_mask_t domain, int severity,
                               const char *funcname, const char *suffix, const char *format,
                               va_list ap, size_t *msg_len_out) CHECK_PRINTF(7, 0);
 ...
 */
void
tor_log_string(int severity, log_domain_mask_t domain, const char *function, const char *string)
{
...
log_prefix_(char *buf, size_t buf_len, int severity)
{
    ...
    n = strftime(buf, buf_len, "%b %d %H:%M:%S", tor_localtime_r_msg(&t, &tm, NULL));
    r = tor_snprintf(buf + n, buf_len - n, ".%.3i [%s] ", ms, sev_to_string(severity));
 ...
 */
static inline char *
format_msg(char *buf, size_t buf_len, log_domain_mask_t domain, int severity, const char *funcname,
           const char *suffix, const char *format, va_list ap, size_t *msg_len_out)
{
    size_t n;
    int r;
    char *end_of_prefix;
    ...
    if (funcname && should_log_function_name(domain, severity)) {
        r = tor_snprintf(buf + n, buf_len - n, pretty_fn_has_parens ? "%s: " : "%s(): ", funcname);
        if (r < 0)
            n = strlen(buf);
        else
            n += r;
...
pending_log_message_new(int severity, log_domain_mask_t domain, const char *fullmsg,
                        const char *shortmsg)
{
 ...
 */
static inline int
logfile_wants_message(const logfile_t *lf, int severity, log_domain_mask_t domain)
{
...
static inline void
logfile_deliver(logfile_t *lf, const char *buf, size_t msg_len, const char *msg_after_prefix,
                log_domain_mask_t domain, int severity, int *callbacks_deferred)
{
    if (lf->is_syslog) {
#ifdef HAVE_SYSLOG_H
#    ifdef MAXLINE
...
logv,
          (int severity, log_domain_mask_t domain, const char *funcname, const char *suffix,
           const char *format, va_list ap))
{
    ...
    if ((!(domain & LD_NOCB)) && pending_cb_messages && smartlist_len(pending_cb_messages))
        flush_pending_log_callbacks();

    if (queue_startup_messages && pending_startup_messages_len < MAX_STARTUP_MSG_LEN) {
        end_of_prefix =
            format_msg(buf, sizeof(buf), domain, severity, funcname, suffix, format, ap, &msg_len);
        formatted = 1;
    ...
    for (lf = logfiles; lf; lf = lf->next) {
        ...
        if (!formatted) {
            end_of_prefix = format_msg(buf, sizeof(buf), domain, severity, funcname, suffix, format,
                                       ap, &msg_len);
            formatted = 1;
        }
...
tor_log_update_sigsafe_err_fds(void)
{
    ...
    for (lf = logfiles; lf; lf = lf->next) {
         ...
         * signal handler.
         */
        if (lf->is_temporary || logfile_is_external(lf) || lf->seems_dead || lf->fd < 0)
            continue;
        if (lf->severities->masks[SEVERITY_MASK_IDX(LOG_ERR)] & (LD_BUG | LD_GENERAL)) {
    ...
    if (!found_real_stderr && int_array_contains(fds, n_fds, STDOUT_FILENO)) {
...
void
log_fn_(int severity, log_domain_mask_t domain, const char *fn, const char *format, ...)
{
    va_list ap;
    if (severity > log_global_min_severity_)
        return;
...
void
log_fn_ratelim_(ratelim_t *ratelim, int severity, log_domain_mask_t domain, const char *fn,
                const char *format, ...)
{
    va_list ap;
    char *m;
...
logs_free_all(void)
{
    ...
    SMARTLIST_FOREACH(messages, pending_log_message_t *, msg, { pending_log_message_free(msg); });
 ...
 */
void
set_log_severity_config(int loglevelMin, int loglevelMax, log_severity_list_t *severity_out)
{
    int i;
...
flush_log_messages_from_startup(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (pending_startup_messages, pending_log_message_t *, msg) {
        int callbacks_deferred = 0;
        for (lf = logfiles; lf; lf = lf->next) {
            ...
            logfile_deliver(lf, msg->fullmsg, strlen(msg->fullmsg), msg->msg, msg->domain,
                            msg->severity, &callbacks_deferred);
        }
        pending_log_message_free(msg);
    } SMARTLIST_FOREACH_END (msg);
 ...
 */
MOCK_IMPL(int,
add_file_log, (const log_severity_list_t *severity, const char *filename, int fd))
{
    logfile_t *lf;
...
static const char *domain_list[] = {
    "GENERAL",  "CRYPTO",  "NET",     "CONFIG",    "FS",        "PROTOCOL", "MM",      "HTTP",
    "APP",      "CONTROL", "CIRC",    "REND",      "BUG",       "DIR",      "DIRSERV", "OR",
    "EDGE",     "ACCT",    "HIST",    "HANDSHAKE", "HEARTBEAT", "CHANNEL",  "SCHED",   "GUARD",
    "CONSDIFF", "DOS",     "PROCESS", "PT",        "BTRACK",    "MESG",     NULL};
...
domain_to_string(log_domain_mask_t domain, char *buf, size_t buflen)
{
    ...
    while (1) {
        ...
        size_t n;
        if ((unsigned)bit >= ARRAY_LENGTH(domain_list) - 1 || bit >= N_LOGGING_DOMAINS) {
 ...
 */
int
parse_log_severity_config(const char **cfg_ptr, log_severity_list_t *severity_out)
{
    ...
    while (*cfg) {
        ...
        if (*cfg == '[') {
            ...
            smartlist_split_string(domains_list, domains_str, ",", SPLIT_SKIP_SPACE, -1);
