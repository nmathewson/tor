...
#define tor_assert_unreached()                                                                  \
    STMT_BEGIN                                                                                  \
        {                                                                                       \
            tor_assertion_failed_(SHORT_FILE__, __LINE__, __func__, "line should be unreached", \
                                  NULL);                                                        \
            tor_abort_();                                                                       \
        }                                                                                       \
    STMT_END
...
#    define tor_assertf_nonfatal(cond, fmt, ...)                                                   \
        STMT_BEGIN                                                                                 \
            if (ASSERT_PREDICT_UNLIKELY_(cond)) {                                                  \
                ...
                tor_bug_occurred_(SHORT_FILE__, __LINE__, __func__, #cond, 0, fmt, ##__VA_ARGS__); \
            }                                                                                      \
        STMT_END
...
#    define IF_BUG_ONCE(cond) \
        IF_BUG_ONCE__(ASSERT_PREDICT_UNLIKELY_(cond), IF_BUG_ONCE_VARNAME__(__LINE__))
...
void tor_assertion_failed_(const char *fname, unsigned int line, const char *func, const char *expr,
                           const char *fmt, ...) CHECK_PRINTF(5, 6);
void tor_bug_occurred_(const char *fname, unsigned int line, const char *func, const char *expr,
                       int once, const char *fmt, ...) CHECK_PRINTF(6, 7);
