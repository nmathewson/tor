...
tor_cond_wait(tor_cond_t *cond, tor_mutex_t *mutex, const struct timeval *tv)
{
    ...
    if (tv == NULL) {
        while (1) {
            ...
            if (r == EINTR) {
                /* EINTR should be impossible according to POSIX, but POSIX,
                 * like the Pirate's Code, is apparently treated "more like what
...
#endif
        const int ret2 = pthread_attr_setdetachstate(&attr_detached,
