...
spawn_exit(void)
{
    _endthread();
    // LCOV_EXCL_START
    // we should never get here. my compiler thinks that _endthread returns,
    // this is an attempt to fool it.
    tor_assert(0);
...
tor_threadlocal_get(tor_threadlocal_t *threadlocal)
{
    ...
    if (value == NULL) {
        ...
        if (err != ERROR_SUCCESS) {
            ...
            log_err(LD_GENERAL, "Error retrieving thread-local value: %s",
                    msg);
            tor_free(msg);
