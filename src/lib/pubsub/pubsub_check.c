...
pubsub_cfg_dump(const pubsub_cfg_t *cfg, int severity, const char *prefix)
{
    ...
    tor_log(severity, LD_MESG, "%s%s %s: %s{%s} on %s (%s) <%u %u %u %u %x> [%s:%d]", prefix,
            get_subsys_id_name(cfg->subsys), cfg->is_publish ? "PUB" : "SUB",
            get_message_id_name(cfg->msg), get_msg_type_id_name(cfg->type),
            get_channel_id_name(cfg->channel), format_flags(cfg->flags), cfg->subsys, cfg->msg,
            cfg->type, cfg->channel, cfg->flags, cfg->added_by_file, cfg->added_by_line);
}
 ...
 **/
static void
get_message_bitarray(const pubsub_adjmap_t *map, const smartlist_t *items, bitarray_t **out)
{
 ...
 **/
static int
lint_message_graph(const pubsub_adjmap_t *map, message_id_t msg, const smartlist_t *pub,
                   const smartlist_t *sub)
{
    ...
    for (unsigned i = 0; i < map->n_subsystems; ++i) {
        if (bitarray_is_set(published_by, i) && bitarray_is_set(subscribed_by, i)) {
 ...
 **/
static int
lint_message_consistency(message_id_t msg, const smartlist_t *pub, const smartlist_t *sub)
{
...
lint_message(const pubsub_adjmap_t *map, message_id_t msg)
{
    ...
    if (n_pub == 0) {
        log_warn(LD_MESG | LD_BUG, "Message \"%s\" has subscribers, but no publishers.",
                 get_message_id_name(msg));
        ok = false;
    } else if (n_sub == 0) {
        log_warn(LD_MESG | LD_BUG, "Message \"%s\" has publishers, but no subscribers.",
                 get_message_id_name(msg));
        ok = false;
    }
    ...
    if (!ok) {
         ...
         * this message */
        if (pub) {
            SMARTLIST_FOREACH(pub, pubsub_cfg_t *, cfg, pubsub_cfg_dump(cfg, LOG_WARN, "   "));
        }
        if (sub) {
            SMARTLIST_FOREACH(sub, pubsub_cfg_t *, cfg, pubsub_cfg_dump(cfg, LOG_WARN, "   "));
        }
    }
