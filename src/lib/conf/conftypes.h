...
typedef enum config_type_t {
     ...
     *
     * Reported as "ignored" when its type is queried. For use with options
     *used by disabled modules.
     **/
    CONFIG_TYPE_IGNORE,
     ...
     * enumeration, since those types are abstractions defined at a higher
     *level than this module.  (For example, parsing a routerset_t is
     *higher-level than this module.)  To handle this, we use
     *CONFIG_TYPE_EXTENDED for those types, and give a definition for them in
     *the struct_member_t.type_def.
     **/
...
typedef struct struct_member_t {
     ...
     * type_def be set in all cases. If we do, we will also need a new
     * mechanism to enforce consistency between configuration variable types
     * and their corresponding structures, since our current design in
...
typedef struct config_format_t {
    ...
    const config_var_t *vars; /**< List of variables we recognize, their
                               * default values, and where we stick them in the
                               * structure. */
