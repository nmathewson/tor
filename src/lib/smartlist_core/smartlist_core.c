...
#    define MAX_CAPACITY (int)((SIZE_MAX / (sizeof(void *))))
    ...
    if (size > (size_t)sl->capacity) {
        ...
        sl->list =
            tor_reallocarray(sl->list, sizeof(void *), ((size_t)higher));
...
smartlist_grow(smartlist_t *sl, size_t new_size)
{
    ...
    if (new_size > (size_t)sl->num_used) {
        /* This memset() should be a no-op: everything else in the smartlist
         * code tries to make sure that unused entries are always NULL.  Still,
         * that is meant as a safety mechanism, so let's clear the memory here.
         */
