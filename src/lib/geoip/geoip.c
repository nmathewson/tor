...
static void
geoip_add_entry(const tor_addr_t *low, const tor_addr_t *high, const char *country)
{
    intptr_t idx;
    void *idxplus1_;
...
geoip_ipv6_compare_entries_(const void **_a, const void **_b)
{
    ...
    return fast_memcmp(a->ip_low.s6_addr, b->ip_low.s6_addr, sizeof(struct in6_addr));
}
...
geoip_ipv6_compare_key_to_entry_(const void *_key, const void **_member)
{
    ...
    if (fast_memcmp(addr->s6_addr, entry->ip_low.s6_addr, sizeof(struct in6_addr)) < 0)
        return -1;
    else if (fast_memcmp(addr->s6_addr, entry->ip_high.s6_addr, sizeof(struct in6_addr)) > 0)
        return 1;
    else
        return 0;
}
...
init_geoip_countries(void)
{
    ...
    strlcpy(geoip_unresolved->countrycode, "??", sizeof(geoip_unresolved->countrycode));
...
geoip_load_file(sa_family_t family, const char *filename, int severity)
{
    ...
    log_notice(LD_GENERAL, "Parsing GEOIP %s file %s.", (family == AF_INET) ? "IPv4" : "IPv6",
               filename);
    while (!feof(f)) {
        char buf[512];
