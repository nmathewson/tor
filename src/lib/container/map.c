...
/** Helper: compare digestmap_entry_t objects by key value. */
static inline int
digest256map_entries_eq(const digest256map_entry_t *a, const digest256map_entry_t *b)
{
...
HT_GENERATE2(strmap_impl, strmap_entry_t, node, strmap_entry_hash, strmap_entries_eq, 0.6,
             tor_reallocarray_, tor_free_);

HT_PROTOTYPE(digestmap_impl, digestmap_entry_t, node, digestmap_entry_hash, digestmap_entries_eq);
HT_GENERATE2(digestmap_impl, digestmap_entry_t, node, digestmap_entry_hash, digestmap_entries_eq,
             0.6, tor_reallocarray_, tor_free_);

HT_PROTOTYPE(digest256map_impl, digest256map_entry_t, node, digest256map_entry_hash,
             digest256map_entries_eq);
HT_GENERATE2(digest256map_impl, digest256map_entry_t, node, digest256map_entry_hash,
...
#define IMPLEMENT_MAP_FNS(maptype, keytype, prefix)                                            \
    ...
    {                                                                                          \
        ...
        HT_FIND_OR_INSERT_(                                                                    \
            prefix##_impl, node, prefix##_entry_hash, &(map->head), prefix##_entry_t, &search, \
            ptr,                                                                               \
            {                                                                                  \
                /* we found an entry. */                                                       \
                oldval = (*ptr)->val;                                                          \
            ...
            {                                                                                  \
                ...
                prefix##_entry_t *newent = tor_malloc_zero(sizeof(prefix##_entry_t));          \
    ...
    {                                                                                          \
        ...
        if (!map)                                                                              \
            return;                                                                            \
        for (ent = HT_START(prefix##_impl, &map->head); ent != NULL; ent = next) {             \
            this = *ent;                                                                       \
    ...
    prefix##_iter_t *prefix##_iter_next_rmv(maptype *map, prefix##_iter_t *iter)               \
    {                                                                                          \
        prefix##_entry_t *rmv;                                                                 \
        tor_assert(map);                                                                       \
        tor_assert(iter);                                                                      \
     ...
     * to by iter. */                                                                          \
    void prefix##_iter_get(prefix##_iter_t *iter, const keytype *keyp, void **valp)            \
    {                                                                                          \
        tor_assert(iter);                                                                      \
        tor_assert(*iter);                                                                     \
        tor_assert(keyp);                                                                      \
