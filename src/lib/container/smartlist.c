    ...
    while (lo <= hi) {
        ...
        if (cmp == 0) {
            ...
            *found_out = 1;
            ...
            /*
             * Since lo <= mid <= hi, hi can only decrease on each iteration
             * (by being set to mid - 1) and hi is initially len - 1, mid < len
             * should always hold, and this is not symmetric with the left end
             * of list mid > 0 test below.  A key greater than the right end of
             * the list should eventually lead to lo == hi == mid == len - 1,
             * and then we set lo to len below and fall out to the same exit we
             * hit for a key in the middle of the list but not matching.  Thus,
             * we just assert for consistency here rather than handle a mid ==
             * len case.
             */
            tor_assert(mid < len);
            ...
            if (mid > 0) {
                /* Normal case, move hi to the element immediately before
                 * sl[mid] */
                hi = mid - 1;
            } else {
                ...
                tor_assert(mid == 0);
                /*
                 * We were at the beginning of the list and concluded that
                 * every element e compares e > key.
                 */
                *found_out = 0;
                return 0;
            }
                  ...
                  ptrdiff_t idx_field_offset, int idx)
{
    while (1) {
        if (!IDX_MAY_HAVE_CHILDREN(idx)) {
             ...
             * violate the heap property by being greater than a child (since
             * it doesn't have any). */
            return;
        }
