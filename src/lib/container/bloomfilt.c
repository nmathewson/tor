...
{
    /* The probability of false positives is about P=(1 - exp(-kn/m))^k, where
     * k is the number of hash functions per entry, m is the bits in the array,
     * and n is the number of elements inserted.  For us, k==4,
     * n<=max_elements, and m==n_bits= approximately max_elements*32.  This
     * gives P<(1-exp(-4*n/(32*n)))^4 == (1-exp(1/-8))^4 == .00019
     *
