...
#define DECLARE_TYPED_DIGESTMAP_FNS(prefix, maptype, valtype)                  \
    ...
    ATTR_UNUSED static inline maptype *prefix##new (void)                      \
    {                                                                          \
        return (maptype *)digestmap_new();                                     \
    }                                                                          \
    ATTR_UNUSED static inline digestmap_t *prefix##to_digestmap(maptype *map)  \
    {                                                                          \
        return (digestmap_t *)map;                                             \
    }                                                                          \
    ATTR_UNUSED static inline valtype *prefix##get(maptype *map,               \
                                                   const char *key)            \
    {                                                                          \
        return (valtype *)digestmap_get((digestmap_t *)map, key);              \
    }                                                                          \
    ATTR_UNUSED static inline valtype *prefix##set(                            \
        maptype *map, const char *key, valtype *val)                           \
    {                                                                          \
        return (valtype *)digestmap_set((digestmap_t *)map, key, val);         \
    }                                                                          \
    ATTR_UNUSED static inline valtype *prefix##remove(maptype *map,            \
                                                      const char *key)         \
    {                                                                          \
        return (valtype *)digestmap_remove((digestmap_t *)map, key);           \
    }                                                                          \
    ATTR_UNUSED static inline void prefix##f##ree_(maptype *map,               \
                                                   void (*free_val)(void *))   \
    {                                                                          \
        digestmap_free_((digestmap_t *)map, free_val);                         \
    }                                                                          \
    ATTR_UNUSED static inline int prefix##isempty(maptype *map)                \
    {                                                                          \
        return digestmap_isempty((digestmap_t *)map);                          \
    }                                                                          \
    ATTR_UNUSED static inline int prefix##size(maptype *map)                   \
    {                                                                          \
        return digestmap_size((digestmap_t *)map);                             \
    }                                                                          \
    ATTR_UNUSED static inline prefix##iter_t *prefix##iter_init(maptype *map)  \
    {                                                                          \
        return (prefix##iter_t *)digestmap_iter_init((digestmap_t *)map);      \
    }                                                                          \
    ATTR_UNUSED static inline prefix##iter_t *prefix##iter_next(               \
        maptype *map, prefix##iter_t *iter)                                    \
    {                                                                          \
