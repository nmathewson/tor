...
string_is_C_identifier(const char *string)
{
    ...
    for (iter = 0; iter < length; iter++) {
        if (iter == 0) {
            ...
            if (!(TOR_ISALPHA(string[iter]) || TOR_ISDIGIT(string[iter]) || string[iter] == '_'))
                return 0;
        }
    }
...
string_is_utf8(const char *str, size_t len)
{
    ...
    if (!str) {
        ...
        if (len) {
            ...
            tor_log_err_sigsafe("BUG: string_is_utf8() called with NULL str but non-zero len.");
...
{
    if (str && len >= 3 && (!strcmpstart(str, "\uFEFF") || !strcmpstart(str, "\uFFFE"))) {
        return false;
    }
