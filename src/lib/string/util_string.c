           ...
           size_t nlen)
{
    ...
    while ((p = memchr(p, first, last_possible_start + 1 - p))) {
        ...
        if (++p > last_possible_start) {
            /* This comparison shouldn't be necessary, since if p was
             * previously equal to last_possible_start, the next memchr call
             * would be "memchr(p, first, 0)", which will return NULL. But it
             * clarifies the logic. */
            return NULL;
        }
    }
...
validate_char(const uint8_t *c, uint8_t len)
{
    ...
    if (len == 2 && codepoint <= 0x7f)
        return false; // Invalid, overly long encoding, should have fit in 1
                      // byte.

    if (len == 3 && codepoint <= 0x7ff)
        return false; // Invalid, overly long encoding, should have fit in 2
                      // bytes.

    if (len == 4 && codepoint <= 0xffff)
        return false; // Invalid, overly long encoding, should have fit in 3
                      // bytes.
...
string_is_utf8(const char *str, size_t len)
{
    ...
    if (!str) {
        ...
        if (len) {
            ...
            tor_log_err_sigsafe("BUG: string_is_utf8() called with NULL str "
                                "but non-zero len.");
