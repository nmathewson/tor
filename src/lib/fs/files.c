...
struct open_file_t {
    ...
    unsigned
        rename_on_close : 1; /**< Are we using the temporary file or not? */
                      ...
                      open_file_t **data_out)
{
    ...
    if (append) {
        if (tor_fd_seekend(new_file->fd) < 0) {
            log_warn(LD_FS, "Couldn't seek to end of file \"%s\": %s",
    ...
    if (file_data->rename_on_close) {
        ...
        if (!abort_write) {
            ...
            if (replace_file(file_data->tempname, file_data->filename)) {
                log_warn(LD_FS, "Error replacing \"%s\": %s",
                         file_data->filename, strerror(errno));
                abort_write = r = -1;
            }
        }
        if (abort_write) {
...
read_file_to_str_until_eof(int fd, size_t max_bytes_to_read, size_t *sz_out)
{
    ...
    do {
         ...
         * performance here, we should be doubling.  But in practice we
         * shouldn't be using this function on big files anyway. */
        string_max = pos + 1024;
