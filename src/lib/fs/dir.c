          ...
          (const char *dirname, cpd_check_t check, const char *effective_user))
{
    ...
    if ((check & (CPD_GROUP_OK | CPD_GROUP_READ)) &&
        ...
        process_groupname =
            gr ? tor_strdup(gr->gr_name) : tor_strdup("<unknown>");
    ...
    if ((st.st_mode & unwanted_bits & check_bits_filter) != 0) {
        ...
        new_mode &=
            ~unwanted_bits; /* Clear the bits that we didn't want set...*/
...
#    endif
    if (INVALID_HANDLE_VALUE ==
        (handle = FindFirstFile(tpattern, &findData))) {
        tor_free(pattern);
        return NULL;
    }
...
#    endif /* defined(UNICODE) */
        ...
        if (!FindNextFile(handle, &findData)) {
            ...
            if ((err = GetLastError()) != ERROR_NO_MORE_FILES) {
                ...
                log_warn(LD_FS, "Error reading directory '%s': %s", dirname,
                         errstr);
                tor_free(errstr);
            }
            break;
