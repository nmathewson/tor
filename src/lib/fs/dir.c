          ...
          (const char *dirname, cpd_check_t check, const char *effective_user))
{
    ...
    if (fd == -1) {
        if (errno != ENOENT) {
            ...
            log_warn(LD_FS, "Directory %s cannot be read: %s", dirname, strerror(errno));
            return -1;
        }
        ...
        if (check & CPD_CREATE) {
            ...
            if (r) {
                log_warn(LD_FS, "Error creating directory %s: %s", dirname, strerror(errno));
                return -1;
            }
            ...
            if (fd == -1) {
                log_warn(LD_FS, "Could not reopen recently created directory %s: %s", dirname,
                         strerror(errno));
                return -1;
            } else {
                close(fd);
    ...
    if (st.st_uid != running_uid) {
...

        {
            ...
            process_ownername =
                pw_running ? tor_strdup(pw_running->pw_name) : tor_strdup("<unknown>");
        }

        {
            const struct passwd *pw_stat = tor_getpwuid(st.st_uid);
            file_ownername = pw_stat ? tor_strdup(pw_stat->pw_name) : tor_strdup("<unknown>");
        }

        log_warn(LD_FS,
                 "%s is not owned by this user (%s, %d) but by "
                 "%s (%d). Perhaps you are running Tor as the wrong user?",
                 dirname, process_ownername, (int)running_uid, file_ownername, (int)st.st_uid);
    ...
    if ((check & (CPD_GROUP_OK | CPD_GROUP_READ)) && (st.st_gid != running_gid) &&
        (st.st_gid != 0)) {
        struct group *gr;
        ...
        log_warn(LD_FS,
                 ...
                 dirname, process_groupname, (int)running_gid, gr ? gr->gr_name : "<unknown>",
                 (int)st.st_gid);
    ...
    if ((st.st_mode & unwanted_bits & check_bits_filter) != 0) {
        ...
        if (check & CPD_CHECK_MODE_ONLY) {
            log_warn(LD_FS, "Permissions on directory %s are too permissive.", dirname);
            close(fd);
            return -1;
        }
        ...
        if (fchmod(fd, new_mode)) {
            log_warn(LD_FS, "Could not chmod directory %s: %s", dirname, strerror(errno));
            close(fd);
            return -1;
        } else {
            close(fd);
