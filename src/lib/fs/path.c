...
get_unquoted_path(const char *path)
{
    ...
    char *unquoted_path =
        tor_malloc(len - has_start_quote - has_end_quote + 1);
...
#    ifdef HAVE_PWD_H
            ...
            if (!(home = get_user_homedir(username))) {
                log_warn(LD_CONFIG, "Couldn't get homedir for \"%s\"",
                         username);
                tor_free(username);
                return NULL;
            }
...
get_parent_directory(char *fname)
{
    ...
    tor_assert(fname);
#ifdef _WIN32
    /* If we start with, say, c:, then don't consider that the start of the
     * path
     */
    if (fname[0] && fname[1] == ':') {
        fname += 2;
    ...
    while (--cp >= fname) {
        ...
        if (is_sep) {
            if (cp == fname) {
                /* This is the first separator in the file name; don't remove
                 * it! */
                cp[1] = '\0';
                return 0;
            }
