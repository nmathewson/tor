...
tor_mmap_file, (const char *filename))
{
    ...
    if (fd < 0) {
        ...
        log_fn(severity, LD_FS, "Could not open \"%s\" for mmap(): %s",
...
#    endif
    ...
    if (real_size > SIZE_MAX) {
        log_warn(LD_FS, "File \"%s\" is too big to map; not trying.",
                 filename);
        goto err;
    }
    res->size = real_size;
    ...
    if (res->mmap_handle == NULL)
        goto win_err;
    res->data =
        (char *)MapViewOfFile(res->mmap_handle, FILE_MAP_READ, 0, 0, 0);
    if (!res->data)
        goto win_err;
...
tor_munmap_file, (tor_mmap_t * handle))
{
    ...
    if (handle->data) {
        /* This is an ugly cast, but without it, "data" in struct tor_mmap_t
           would have to be redefined as non-const. */
