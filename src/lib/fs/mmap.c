...
tor_mmap_file, (const char *filename))
{
    ...
    if (fd < 0) {
        ...
        log_fn(severity, LD_FS, "Could not open \"%s\" for mmap(): %s", filename, strerror(errno));
        errno = save_errno;
        return NULL;
    }
    ...
    if (result != 0) {
        int save_errno = errno;
        log_warn(LD_FS, "Couldn't fstat opened descriptor for \"%s\" during mmap: %s", filename,
                 strerror(errno));
        close(fd);
        errno = save_errno;
        return NULL;
    ...
    if (string == MAP_FAILED) {
        int save_errno = errno;
        log_warn(LD_FS, "Could not mmap file \"%s\": %s", filename, strerror(errno));
        errno = save_errno;
        return NULL;
    }
...
tor_munmap_file, (tor_mmap_t * handle))
{
    ...
    if (res == 0) {
        ...
        log_warn(LD_FS, "Failed to munmap() in tor_munmap_file(): %s", strerror(errno));
        res = -1;
    }
...
#    endif
    file_handle = CreateFile(tfilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
    ...
    res->mmap_handle =
        CreateFileMapping(file_handle, NULL, PAGE_READONLY, size_high, size_low, NULL);
