...
storage_dir_register_with_sandbox(storage_dir_t *d, sandbox_cfg_t **cfg)
{
    ...
    for (idx = FNAME_MIN_NUM; idx < FNAME_MIN_NUM + d->max_files; ++idx) {
        ...
        problems += sandbox_cfg_allow_rename(cfg, tor_strdup(tmppath), tor_strdup(path));
...
static int
storage_dir_save_chunks_to_file(storage_dir_t *d, const smartlist_t *chunks, int binary,
                                char **fname_out)
{
    ...
    SMARTLIST_FOREACH(chunks, const sized_chunk_t *, ch, total_length += ch->len);
 ...
 * -1. */
int
storage_dir_save_bytes_to_file(storage_dir_t *d, const uint8_t *data, size_t length, int binary,
                               char **fname_out)
{
 ...
 */
int
storage_dir_save_string_to_file(storage_dir_t *d, const char *str, int binary, char **fname_out)
{
    return storage_dir_save_bytes_to_file(d, (const uint8_t *)str, strlen(str), binary, fname_out);
}
 ...
 */
int
storage_dir_save_labeled_to_file(storage_dir_t *d, const config_line_t *labels, const uint8_t *data,
                                 size_t length, char **fname_out)
{
    /*
 ...
 */
tor_mmap_t *
storage_dir_map_labeled(storage_dir_t *dir, const char *fname, config_line_t **labels_out,
 ...
 * data. */
uint8_t *
storage_dir_read_labeled(storage_dir_t *dir, const char *fname, config_line_t **labels_out,
                         size_t *sz_out)
{
    const uint8_t *data = NULL;
    tor_mmap_t *m = storage_dir_map_labeled(dir, fname, labels_out, &data, sz_out);
    if (m == NULL)
        return NULL;
...
storage_dir_remove_file(storage_dir_t *d, const char *fname)
{
    ...
    if (unlink(ipath) == 0) {
        ...
        log_warn(LD_FS, "Unable to unlink %s while removing file: %s", escaped(path),
                 strerror(errno));
        tor_free(path);
        return;
    }
