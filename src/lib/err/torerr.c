...
tor_log_set_sigsafe_err_fds(const int *fds, int n)
{
    ...
    /* Clear the entire array. This code mitigates against some race
     * conditions, but there are still some races here:
...
tor_log_flush_sigsafe_err_fds(void)
{
    ...
    for (i = 0; i < n_fds; ++i) {
        /* This function is called on error and on shutdown, so we don't log,
         * or take any other action, if fsync() fails. */
        (void)fsync(fds[i]);
    }
                              ...
                              const char *msg)
{
    ...
    tor_log_err_sigsafe("INTERNAL ERROR: Raw assertion failed in ",
                        get_tor_backtrace_version(), " at ", file, ":",
                        linebuf, ": ", expr, "\n", NULL);
    if (msg) {
     ...
     * an abort. This issue is probably caused by a race condition between
     * write buffer cache flushing, and process termination. So we write an
     * extra newline, to make sure that the message always gets through. */
    tor_log_err_sigsafe_write("\n");
}
