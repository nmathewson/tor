...
#if defined(HAVE_EXECINFO_H) && defined(HAVE_BACKTRACE) && defined(HAVE_BACKTRACE_SYMBOLS_FD) && \
    defined(HAVE_SIGACTION) && defined(HAVE_PTHREAD_H)
...
#        if defined(__linux__)
    const size_t n = 1;
#        elif defined(__darwin__) || defined(__APPLE__) || defined(OpenBSD) || defined(__FreeBSD__)
    const size_t n = 2;
#        else
    const size_t n = 1;
 ...
 * logger". */
void
log_backtrace_impl(int severity, log_domain_mask_t domain, const char *msg, tor_log_fn logger)
{
    size_t depth;
    char **symbols;
    size_t i;
...
static void crash_handler(int sig, siginfo_t *si, void *ctx_) __attribute__((noreturn));
...
static int trap_signals[] = {SIGSEGV, SIGILL, SIGFPE, SIGBUS, SIGSYS, SIGIO, -1};
...
log_backtrace_impl(int severity, log_domain_mask_t domain, const char *msg, tor_log_fn logger)
{
    logger(severity, domain, "%s: %s. (Stack trace not available)", bt_version, msg);
}
