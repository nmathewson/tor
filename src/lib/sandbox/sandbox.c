...
#    define SCMP_CMP_STR(a, b, c) ((struct scmp_arg_cmp){(a), (b), (intptr_t)(void *)(c), 0})
...
#    define SCMP_CMP_MASKED(a, b, c) SCMP_CMP4((a), SCMP_CMP_MASKED_EQ, ~(scmp_datum_t)(b), (c))

/** Variable used for storing all syscall numbers that will be allowed with the
 * stage 1 general Tor sandbox.
 */
static int filter_nopar_gen[] = {
    SCMP_SYS(access), SCMP_SYS(brk), SCMP_SYS(clock_gettime), SCMP_SYS(close), SCMP_SYS(clone),
    SCMP_SYS(dup), SCMP_SYS(epoll_create), SCMP_SYS(epoll_wait),
...
#    ifdef __NR_fstat64
    SCMP_SYS(fstat64),
#    endif
    SCMP_SYS(fsync), SCMP_SYS(futex), SCMP_SYS(getdents), SCMP_SYS(getdents64), SCMP_SYS(getegid),
...
#    ifdef __NR_sigreturn
    SCMP_SYS(sigreturn),
#    endif
    SCMP_SYS(stat), SCMP_SYS(uname), SCMP_SYS(wait4), SCMP_SYS(write), SCMP_SYS(writev),
...
#        endif
#    endif
    SCMP_SYS(recvmsg), SCMP_SYS(recvfrom), SCMP_SYS(sendto), SCMP_SYS(unlink), SCMP_SYS(poll)};

/* These macros help avoid the error where the number of filters we add on a
 * single rule don't match the arg_cnt param. */
#    define seccomp_rule_add_0(ctx, act, call) seccomp_rule_add((ctx), (act), (call), 0)
#    define seccomp_rule_add_1(ctx, act, call, f1) seccomp_rule_add((ctx), (act), (call), 1, (f1))
...
sb_time(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    (void)filter;
#    ifdef __NR_time
    return seccomp_rule_add_1(ctx, SCMP_ACT_ALLOW, SCMP_SYS(time), SCMP_CMP(0, SCMP_CMP_EQ, 0));
#    else
    return 0;
...
#    ifdef __i386__
    rc =
        seccomp_rule_add_1(ctx, SCMP_ACT_ALLOW, SCMP_SYS(socketcall), SCMP_CMP(0, SCMP_CMP_EQ, 18));
    if (rc) {
        return rc;
    }
#    endif /* defined(__i386__) */
...
sb_chmod(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    for (elem = filter; elem != NULL; elem = elem->next) {
        ...
        if (param != NULL && param->prot == 1 && param->syscall == SCMP_SYS(chmod)) {
...
sb_chown(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    for (elem = filter; elem != NULL; elem = elem->next) {
        ...
        if (param != NULL && param->prot == 1 && param->syscall == SCMP_SYS(chown)) {
...
sb_rename(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    for (elem = filter; elem != NULL; elem = elem->next) {
        ...
        if (param != NULL && param->prot == 1 && param->syscall == SCMP_SYS(rename)) {
...
sb_openat(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    for (elem = filter; elem != NULL; elem = elem->next) {
        ...
        if (param != NULL && param->prot == 1 && param->syscall == SCMP_SYS(openat)) {
            rc = seccomp_rule_add_3(
                ctx, SCMP_ACT_ALLOW, SCMP_SYS(openat), SCMP_CMP(0, SCMP_CMP_EQ, AT_FDCWD),
...
#    endif
    ...
    rc =
        seccomp_rule_add_2(ctx, SCMP_ACT_ALLOW, SCMP_SYS(socket), SCMP_CMP(0, SCMP_CMP_EQ, PF_FILE),
    ...
    for (i = 0; i < 2; ++i) {
        ...
        for (j = 0; j < 3; ++j) {
            ...
            const int protocol = (j == 0) ? IPPROTO_TCP : (j == 1) ? IPPROTO_IP : IPPROTO_UDP;
...
#    ifdef ENABLE_NSS
    rc = seccomp_rule_add_3(
        ctx, SCMP_ACT_ALLOW, SCMP_SYS(socket), SCMP_CMP(0, SCMP_CMP_EQ, PF_INET),
        SCMP_CMP(1, SCMP_CMP_EQ, SOCK_STREAM), SCMP_CMP(2, SCMP_CMP_EQ, IPPROTO_IP));
    if (rc)
        return rc;
#    endif
    ...
    rc = seccomp_rule_add_3(
        ...
        SCMP_CMP_MASKED(1, SOCK_CLOEXEC | SOCK_NONBLOCK, SOCK_STREAM), SCMP_CMP(2, SCMP_CMP_EQ, 0));
    if (rc)
        return rc;

    rc = seccomp_rule_add_3(
        ctx, SCMP_ACT_ALLOW, SCMP_SYS(socket), SCMP_CMP(0, SCMP_CMP_EQ, PF_UNIX),
        SCMP_CMP_MASKED(1, SOCK_CLOEXEC | SOCK_NONBLOCK, SOCK_DGRAM), SCMP_CMP(2, SCMP_CMP_EQ, 0));
    if (rc)
        return rc;

    rc = seccomp_rule_add_3(
        ctx, SCMP_ACT_ALLOW, SCMP_SYS(socket), SCMP_CMP(0, SCMP_CMP_EQ, PF_NETLINK),
        SCMP_CMP_MASKED(1, SOCK_CLOEXEC, SOCK_RAW), SCMP_CMP(2, SCMP_CMP_EQ, 0));
    if (rc)
        return rc;
...
#        include <netinet/tcp.h>
    rc = seccomp_rule_add_2(ctx, SCMP_ACT_ALLOW, SCMP_SYS(getsockopt),
                            SCMP_CMP(1, SCMP_CMP_EQ, SOL_TCP), SCMP_CMP(2, SCMP_CMP_EQ, TCP_INFO));
    if (rc)
        return rc;
#    endif /* defined(HAVE_KIST_SUPPORT) */
...
sb_fcntl64(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    rc =
        seccomp_rule_add_2(ctx, SCMP_ACT_ALLOW, SCMP_SYS(fcntl64),
                           SCMP_CMP(1, SCMP_CMP_EQ, F_SETFD), SCMP_CMP(2, SCMP_CMP_EQ, FD_CLOEXEC));
    if (rc)
        return rc;
...
sb_flock(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    rc =
        seccomp_rule_add_1(ctx, SCMP_ACT_ALLOW, SCMP_SYS(flock), SCMP_CMP(1, SCMP_CMP_EQ, LOCK_UN));
    if (rc)
        return rc;
...
sb_futex(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    rc = seccomp_rule_add_1(
        ...
        SCMP_CMP(1, SCMP_CMP_EQ, FUTEX_WAIT_BITSET_PRIVATE | FUTEX_CLOCK_REALTIME));
    if (rc)
        return rc;
...
sb_stat64(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    for (elem = filter; elem != NULL; elem = elem->next) {
        ...
        if (param != NULL && param->prot == 1 &&
            (param->syscall == SCMP_SYS(open) || param->syscall == SCMP_SYS(stat64))) {
...
#    ifdef __NR_kill
    ...
    return seccomp_rule_add_1(ctx, SCMP_ACT_ALLOW, SCMP_SYS(kill), SCMP_CMP(1, SCMP_CMP_EQ, 0));
#    else
    return 0;
...
#    ifdef __NR_mmap2
    sb_mmap2,
#    endif
    sb_chown,        sb_chmod,          sb_open,       sb_openat,     sb_rename,
#    ifdef __NR_fcntl64
    sb_fcntl64,
#    endif
    sb_epoll_ctl,    sb_prctl,          sb_mprotect,   sb_flock,      sb_futex,  sb_mremap,
#    ifdef __NR_stat64
    sb_stat64,
#    endif
...
/* DOCDOC */
static int
prot_strings_helper(strmap_t *locations, char **pr_mem_next_p, size_t *pr_mem_left_p,
                    char **value_p)
{
    char *param_val;
    size_t param_size;
...
prot_strings(scmp_filter_ctx ctx, sandbox_cfg_t *cfg)
{
    ...
    pr_mem_base = (char *)mmap(NULL, MALLOC_MP_LIM + pr_mem_size, PROT_READ | PROT_WRITE,
                               MAP_PRIVATE | MAP_ANON, -1, 0);
    if (pr_mem_base == MAP_FAILED) {
        log_err(LD_BUG, "(Sandbox) failed allocate protected memory! mmap: %s", strerror(errno));
        ret = -1;
        goto out;
    }
    ...
    for (el = cfg; el != NULL; el = el->next) {
        if (prot_strings_helper(locations, &pr_mem_next, &pr_mem_left, &el->param->value) < 0) {
            ret = -2;
            goto out;
        }
        if (prot_strings_helper(locations, &pr_mem_next, &pr_mem_left, &el->param->value2) < 0) {
            ret = -2;
            goto out;
        }
        el->param->prot = 1;
    ...
    if (mprotect(pr_mem_base, MALLOC_MP_LIM + pr_mem_size, PROT_READ)) {
        log_err(LD_BUG, "(Sandbox) failed to protect memory! mprotect: %s", strerror(errno));
        ret = -3;
        goto out;
    }
    ...
    ret = seccomp_rule_add_3(
        ctx, SCMP_ACT_ALLOW, SCMP_SYS(mprotect), SCMP_CMP(0, SCMP_CMP_LT, (intptr_t)pr_mem_base),
        SCMP_CMP(1, SCMP_CMP_LE, MALLOC_MP_LIM), SCMP_CMP(2, SCMP_CMP_EQ, PROT_READ | PROT_WRITE));
    if (ret) {
    ...
    ret = seccomp_rule_add_3(
        ...
        SCMP_CMP(0, SCMP_CMP_GT, (intptr_t)pr_mem_base + pr_mem_size + MALLOC_MP_LIM),
        SCMP_CMP(1, SCMP_CMP_LE, MALLOC_MP_LIM), SCMP_CMP(2, SCMP_CMP_EQ, PROT_READ | PROT_WRITE));
    if (ret) {
...
get_syscall_name(int syscall_num)
{
...

    {
        ...
        format_dec_number_sigsafe(syscall_num, syscall_name_buf, sizeof(syscall_name_buf));
...
#    endif /* defined(USE_BACKTRACE) */
    ...
    tor_log_err_sigsafe("(Sandbox) Caught a bad syscall attempt (syscall ", syscall_name, ")\n",
                        NULL);
...
sandbox_init(sandbox_cfg_t *cfg)
{
    ...
    (void)cfg;
    log_warn(LD_GENERAL, "This version of Tor was built without support for sandboxing. To "
...
#else
    (void)cfg;
    log_warn(LD_GENERAL, "Currently, sandboxing is only implemented on Linux. The feature "
