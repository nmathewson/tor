...
#    ifdef __NR_fstat64
    SCMP_SYS(fstat64),
#    endif
    SCMP_SYS(fsync), SCMP_SYS(futex), SCMP_SYS(getdents), SCMP_SYS(getdents64),
    SCMP_SYS(getegid),
...
sb_rt_sigaction(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    unsigned i;
    int rc;
    int param[] = {SIGINT, SIGTERM, SIGPIPE, SIGUSR1, SIGUSR2,
                   SIGHUP, SIGCHLD, SIGSEGV, SIGILL,  SIGFPE,
                   SIGBUS, SIGSYS,  SIGIO,
#    ifdef SIGXFSZ
                   SIGXFSZ
#    endif
...
#    endif /* defined(__i386__) */
    ...
    rc = seccomp_rule_add_1(
        ctx, SCMP_ACT_ALLOW, SCMP_SYS(accept4),
...
{
    if (use_openat) {
        return seccomp_rule_add_2(
            ctx, SCMP_ACT_ALLOW, SCMP_SYS(openat),
...
sb_chmod(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    for (elem = filter; elem != NULL; elem = elem->next) {
        ...
        if (param != NULL && param->prot == 1 &&
            ...
            rc =
                seccomp_rule_add_1(ctx, SCMP_ACT_ALLOW, SCMP_SYS(chmod),
...
sb_chown(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    for (elem = filter; elem != NULL; elem = elem->next) {
        ...
        if (param != NULL && param->prot == 1 &&
            ...
            rc =
                seccomp_rule_add_1(ctx, SCMP_ACT_ALLOW, SCMP_SYS(chown),
                                   SCMP_CMP_STR(0, SCMP_CMP_EQ, param->value));
            if (rc != 0) {
                log_err(LD_BUG,
...
sb_rename(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    for (elem = filter; elem != NULL; elem = elem->next) {
        ...
        if (param != NULL && param->prot == 1 &&
            ...
            rc = seccomp_rule_add_2(
                ctx, SCMP_ACT_ALLOW, SCMP_SYS(rename),
                SCMP_CMP_STR(0, SCMP_CMP_EQ, param->value),
...
#    endif
    ...
    rc = seccomp_rule_add_2(
        ctx, SCMP_ACT_ALLOW, SCMP_SYS(socket),
...
#    endif
    ...
    rc = seccomp_rule_add_2(
        ctx, SCMP_ACT_ALLOW, SCMP_SYS(socketpair),
        SCMP_CMP(0, SCMP_CMP_EQ, PF_FILE),
...
sb_stat64(scmp_filter_ctx ctx, sandbox_cfg_t *filter)
{
    ...
    for (elem = filter; elem != NULL; elem = elem->next) {
        ...
        if (param != NULL && param->prot == 1 &&
            ...
            rc =
                seccomp_rule_add_1(ctx, SCMP_ACT_ALLOW, SCMP_SYS(stat64),
                                   SCMP_CMP_STR(0, SCMP_CMP_EQ, param->value));
            if (rc != 0) {
                log_err(LD_BUG,
                    ...
                    size_t *pr_mem_left_p, char **value_p)
{
    ...
    if (location) {
        ...
        *value_p = location;
        ...
        strmap_set(locations, location,
                   location); /* good real estate advice */
...
prot_strings(scmp_filter_ctx ctx, sandbox_cfg_t *cfg)
{
    ...
    /*
     * Setting sandbox restrictions so the string memory cannot be tampered
     * with
     */
