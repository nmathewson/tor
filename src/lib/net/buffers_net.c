 ...
 * otherwise. */
static inline int
read_to_chunk(buf_t *buf, chunk_t *chunk, tor_socket_t fd, size_t at_most, int *reached_eof,
              int *error, bool is_socket)
{
    ssize_t read_result;
    ...
    if (read_result < 0) {
        ...
        *reached_eof = 1;
        ...
        log_debug(LD_NET, "Read %ld bytes. %d on inbuf.", (long)read_result, (int)buf->datalen);
...
static int
buf_read_from_fd(buf_t *buf, int fd, size_t at_most, int *reached_eof, int *socket_error,
                 bool is_socket)
{
    ...
    while (at_most > total_read) {
        ...
        r = read_to_chunk(buf, chunk, fd, readlen, reached_eof, socket_error, is_socket);
        check();
        if (r < 0)
            return r; /* Error */
 ...
 */
static inline int
flush_chunk(tor_socket_t fd, buf_t *buf, chunk_t *chunk, size_t sz, size_t *buf_flushlen,
            bool is_socket)
{
 ...
 */
static int
buf_flush_to_fd(buf_t *buf, int fd, size_t sz, size_t *buf_flushlen, bool is_socket)
{
    /* XXXX It's stupid to overload the return values for these functions:
 ...
 */
int
buf_flush_to_socket(buf_t *buf, tor_socket_t s, size_t sz, size_t *buf_flushlen)
{
 ...
 */
int
buf_read_from_socket(buf_t *buf, tor_socket_t s, size_t at_most, int *reached_eof,
                     int *socket_error)
{
 ...
 */
int
buf_read_from_pipe(buf_t *buf, int fd, size_t at_most, int *reached_eof, int *socket_error)
{
