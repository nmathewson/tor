        ...
        for (i = 0; i < 8; ++i) {
            if (words[i] == 0 && longestGapPos == i) {
                ...
                *cp++ = ':';
                ...
                tor_snprintf(cp, sizeof(buf) - (cp - buf), "%x",
                             (unsigned)words[i]);
                cp += strlen(cp);
                if (i != 7)
                    *cp++ = ':';
...
tor_inet_pton(int af, const char *src, void *dst)
{
    if (af == AF_INET) {
        ...
        if (dot == src)
            ...
            if (tor_sscanf(eow, "%3u.%3u.%3u.%3u%c", &byte1, &byte2, &byte3,
                           &byte4, &more) != 4)
                return 0;
        ...
        while (src < eow) {
            ...
            if (TOR_ISXDIGIT(*src)) {
                ...
                if (next == NULL || next == src) {
                    /* The 'next == src' error case can happen on versions of
                     * openbsd which treat "0xfoo" as an error, rather than as
                     * "0" followed by "xfoo". */
                    return 0;
                }
