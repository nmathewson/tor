...
tor_inet_ntoa(const struct in_addr *in, char *buf, size_t buf_len)
{
    ...
    return tor_snprintf(buf, buf_len, "%d.%d.%d.%d", (int)(uint8_t)((a >> 24) & 0xff),
    ...
    if (af == AF_INET) {
        ...
        if (words[0] == 0 && words[1] == 0 && words[2] == 0 && words[3] == 0 && words[4] == 0 &&
            ...
            if (words[5] == 0) {
                ...
                tor_snprintf(buf, sizeof(buf), "::%x:%d.%d.%d.%d", words[5], addr->s6_addr[12],
                             addr->s6_addr[13], addr->s6_addr[14], addr->s6_addr[15]);
            }
...
tor_inet_pton(int af, const char *src, void *dst)
{
    if (af == AF_INET) {
        ...
        if (dot == src)
            ...
            if (tor_sscanf(eow, "%3u.%3u.%3u.%3u%c", &byte1, &byte2, &byte3, &byte4, &more) != 4)
                return 0;
        ...
        if (setWords > 8 || (setWords == 8 && gapPos != -1) || (setWords < 8 && gapPos == -1))
            return 0;
