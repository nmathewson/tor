...
check_network_configuration(bool server_mode)
{
    ...
    if (server_mode) {
        ...
        if (sysctlbyname("net.inet.ip.random_id", &random_id_state, &state_size, NULL, 0)) {
...
MOCK_IMPL(tor_socket_t,
tor_connect_socket,
          (tor_socket_t sock, const struct sockaddr *address, socklen_t address_len))
{
...
tor_socket_t
tor_open_socket_with_extensions(int domain, int type, int protocol, int cloexec, int nonblock)
{
    tor_socket_t s;
    ...
    int ext_flags = (cloexec ? SOCK_CLOEXEC : 0) | (nonblock ? SOCK_NONBLOCK : 0);
 ...
 * counts the number of open sockets. */
tor_socket_t
tor_accept_socket_nonblocking(tor_socket_t sockfd, struct sockaddr *addr, socklen_t *len)
{
 ...
 * if the corresponding extension should be used.*/
tor_socket_t
tor_accept_socket_with_extensions(tor_socket_t sockfd, struct sockaddr *addr, socklen_t *len,
...
MOCK_IMPL(int,
tor_getsockname,
          (tor_socket_t sock, struct sockaddr *address, socklen_t *address_len))
{
