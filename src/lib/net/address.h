...
socklen_t tor_addr_to_sockaddr(const tor_addr_t *a, uint16_t port, struct sockaddr *sa_out,
                               socklen_t len);
...
int tor_addr_compare_masked(const tor_addr_t *addr1, const tor_addr_t *addr2, maskbits_t mask,
...
int tor_addr_is_internal_(const tor_addr_t *ip, int for_listening, const char *filename,
                          int lineno);
...
int tor_addr_parse_PTR_name(tor_addr_t *result, const char *address, int family,
                            int accept_regular);
...
int tor_addr_parse_mask_ports(const char *s, unsigned flags, tor_addr_t *addr_out,
                              maskbits_t *mask_out, uint16_t *port_min_out, uint16_t *port_max_out);
...
#define tor_addr_port_is_valid(addr, port, for_listening) \
    (tor_addr_is_valid((addr), (for_listening)) && tor_port_is_valid((port), (for_listening)))
...
int tor_addr_port_parse(int severity, const char *addrport, tor_addr_t *address_out,
                        uint16_t *port_out, int default_port);
...
#ifdef ADDRESS_PRIVATE
MOCK_DECL(struct smartlist_t *, get_interface_addresses_raw, (int severity, sa_family_t family));
MOCK_DECL(int, get_interface_address6_via_udp_socket_hack,
          (int severity, sa_family_t family, tor_addr_t *addr));

#    ifdef HAVE_IFADDRS_TO_SMARTLIST
STATIC struct smartlist_t *ifaddrs_to_smartlist(const struct ifaddrs *ifa, sa_family_t family);
STATIC struct smartlist_t *get_interface_addresses_ifaddrs(int severity, sa_family_t family);
#    endif /* defined(HAVE_IFADDRS_TO_SMARTLIST) */

#    ifdef HAVE_IP_ADAPTER_TO_SMARTLIST
STATIC struct smartlist_t *ip_adapter_addresses_to_smartlist(const IP_ADAPTER_ADDRESSES *addresses);
STATIC struct smartlist_t *get_interface_addresses_win32(int severity, sa_family_t family);
#    endif /* defined(HAVE_IP_ADAPTER_TO_SMARTLIST) */

#    ifdef HAVE_IFCONF_TO_SMARTLIST
STATIC struct smartlist_t *ifreq_to_smartlist(const uint8_t *ifr, size_t buflen);
STATIC struct smartlist_t *get_interface_addresses_ioctl(int severity, sa_family_t family);
