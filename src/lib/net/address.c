...
tor_addr_to_str(char *dest, const tor_addr_t *addr, size_t len, int decorate)
{
    ...
    switch (tor_addr_family(addr)) {
        ...
        case AF_INET6:
            ...
            if (decorate)
                ptr = tor_inet_ntop(AF_INET6, &addr->addr.in6_addr, dest + 1,
                                    len - 2);
            else
                        ...
                        int accept_regular)
{
    ...
    if (!strcasecmpend(address, ".ip6.arpa")) {
        ...
        for (int i = 0; i < 16; ++i) {
            n0 = hex_decode_digit(
                *cp++); /* The low-order nybble appears first. */
            if (*cp++ != '.')
                return -1; /* Then a dot. */
            n1 = hex_decode_digit(
                *cp++); /* The high-order nybble appears first. */
            if (*cp++ != '.')
                return -1;        /* Then another dot. */
            if (n0 < 0 || n1 < 0) /* Both nybbles must be hex. */
                return -1;

            /* We don't check the length of the string in here.  But that's
             * okay, since we already know that the string ends with
             * ".ip6.arpa", and there is no way to frameshift .ip6.arpa so it
             * fits into the pattern of hexdigit, period, hexdigit, period that
             * we enforce above.
             */
                          ...
                          uint16_t *port_max_out)
{
    ...
    /** Longest possible length for an address, mask, and port-range
     * combination. Includes IP, [], /mask, :, ports */
#define MAX_ADDRESS_LENGTH                                                     \
        ...
        if (mask) { /* the caller (tried to) specify a mask */
            ...
            if (!*endptr) { /* strtol converted everything, so it was an integer
                ...
                if ((bits < 0 || bits > 128) ||
                    ...
                    log_warn(LD_GENERAL,
                             "Bad number of mask bits (%d) on address range; "
                             "rejecting.",
                             bits);
                    goto err;
                }
                ...
                if (tor_inet_pton(AF_INET, mask, &v4mask) > 0) {
                    ...
                    if (bits < 0) {
                        log_warn(LD_GENERAL,
                                 "IPv4-style mask %s is not a prefix address; "
                                 "rejecting.",
                                 escaped(mask));
                        goto err;
                    }
            ...
            if (family == AF_INET6 && v4map) {
                ...
                bits =
                    96 + bits % 32; /* map v4-mapped masks onto 96-128 bits */
            }
            if (any_flag) {
                log_warn(LD_GENERAL,
            ...
            if (any_flag)
                bits = 0; /* This is okay whether it's V6 or V4 (FIX V4-mapped
                             V6!) */
    ...
    if (port_min_out) {
        ...
        if (parse_port_range(port, port_min_out, port_max_out) < 0) {
            ...
            port_max_out =
                NULL; /* caller specified one port, so set this back */
            goto err;
        }
    } else {
        if (port) {
    ...
    if (family1 == family2) {
        ...
        switch (family1) {
            ...
            case AF_UNIX:
                 ...
                 * 2 AF_UNIX addresses is when dealing with ISO_CLIENTADDR
                 * (which is disabled for AF_UNIX SocksPorts anyway), this just
                 * does a pointer comparison.
                 *
                 * See: #20261.
                 */
                if (addr1 < addr2)
...
#define FLAGS                                                                  \
    ...
    if (res != NO_ERROR) {
        log_fn(severity, LD_NET, "GetAdaptersAddresses failed (result: %lu)",
               res);
        goto done;
    }
...
get_interface_addresses_ioctl(int severity, sa_family_t family)
{
    ...
    do {
        ...
        /* Ensure we have least IFREQ_SIZE bytes unused at the end. Otherwise,
         * we don't know if we got everything during ioctl. */
          ...
          (int severity, sa_family_t family, tor_addr_t *addr))
{
    ...
    if (tor_connect_socket(sock, (struct sockaddr *)&target_addr, addr_len) <
        ...
        log_fn(severity, LD_NET, "connect() failed: %s",
               tor_socket_strerror(e));
        goto err;
    }
 ...
 * Return 0 on success, -1 on failure. */
int
tor_addr_port_parse(int severity, const char *addrport, tor_addr_t *address_out,
                    ...
                    uint16_t *port_out)
{
    ...
    if (colon) {
        ...
        if (!port_) {
            log_fn(severity, LD_GENERAL, "Port %s out of range",
                   escaped(colon + 1));
            ok = 0;
        }
        if (!port_out) {
            ...
            log_fn(severity, LD_GENERAL,
                   "Port %s given on %s when not required", escaped(colon + 1),
                   esc_addrport);
