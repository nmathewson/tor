...
#    ifdef HAVE_GETRESUID
    if (getresuid(&ruid, &euid, &suid) != 0) {
        log_warn(LD_GENERAL, "Error getting changed UIDs: %s",
                 strerror(errno));
        return -1;
    } else {
...
#    ifdef HAVE_GETRESGID
    if (getresgid(&rgid, &egid, &sgid) != 0) {
        log_warn(LD_GENERAL, "Error getting changed GIDs: %s",
                 strerror(errno));
        return -1;
    } else {
        log_fn(CREDENTIAL_LOG_LEVEL, LD_GENERAL,
...
#    endif /* defined(HAVE_GETRESGID) */
    ...
    while ((ngids = getgroups(sup_gids_size, sup_gids)) < 0 &&
           errno == EINVAL && sup_gids_size < NGROUPS_MAX) {
        sup_gids_size *= 2;
...
drop_capabilities(int pre_setuid)
{
    ...
    const cap_value_t caplist[] = {CAP_NET_BIND_SERVICE, CAP_SETUID,
                                   CAP_SETGID};
...
#    endif /* defined(HAVE_LINUX_CAPABILITIES) */
    ...
    */
    ...
    /* We've properly switched egid, gid, euid, uid, and supplementary groups
     * if we're here. */
#    ifdef HAVE_LINUX_CAPABILITIES
    if (keep_bindlow) {
...
#    if !defined(CYGWIN) && !defined(__CYGWIN__)
    /* If we tried to drop privilege to a group/user other than root, attempt
     * to restore root (E)(U|G)ID, and abort if the operation succeeds */
