...
#    ifdef HAVE_GETRESUID
    if (getresuid(&ruid, &euid, &suid) != 0) {
        ...
        log_fn(CREDENTIAL_LOG_LEVEL, LD_GENERAL, "UID is %u (real), %u (effective), %u (saved)",
...
#    else /* !defined(HAVE_GETRESUID) */
    ...
    log_fn(CREDENTIAL_LOG_LEVEL, LD_GENERAL, "UID is %u (real), %u (effective), unknown (saved)",
...
#    ifdef HAVE_GETRESGID
    if (getresgid(&rgid, &egid, &sgid) != 0) {
        ...
        log_fn(CREDENTIAL_LOG_LEVEL, LD_GENERAL, "GID is %u (real), %u (effective), %u (saved)",
...
#    else /* !defined(HAVE_GETRESGID) */
    ...
    egid = getegid();
    (void)sgid;
    log_fn(CREDENTIAL_LOG_LEVEL, LD_GENERAL, "GID is %u (real), %u (effective), unknown (saved)",
...
drop_capabilities(int pre_setuid)
{
    ...
    if (prctl(PR_SET_KEEPCAPS, keepcaps) < 0) {
        log_warn(LD_CONFIG, "Unable to call prctl() %s: %s", where, strerror(errno));
        return -1;
    }

    cap_t caps = cap_get_proc();
    if (!caps) {
        log_warn(LD_CONFIG, "Unable to call cap_get_proc() %s: %s", where, strerror(errno));
        return -1;
    }
    cap_clear(caps);
    ...
    cap_free(caps);
    if (r < 0) {
        log_warn(LD_CONFIG, "No permission to set capabilities %s: %s", where, strerror(errno));
        return -1;
    }
...
#    endif /* defined(HAVE_LINUX_CAPABILITIES) */
    ...
    if (setgroups(1, &pw->pw_gid)) {
        log_warn(LD_GENERAL, "Error setting groups to gid %d: \"%s\".", (int)pw->pw_gid,
                 strerror(errno));
    ...
    if (setegid(pw->pw_gid)) {
        log_warn(LD_GENERAL, "Error setting egid to %d: %s", (int)pw->pw_gid, strerror(errno));
        return -1;
    }

    if (setgid(pw->pw_gid)) {
        log_warn(LD_GENERAL, "Error setting gid to %d: %s", (int)pw->pw_gid, strerror(errno));
        return -1;
    }

    if (setuid(pw->pw_uid)) {
        log_warn(LD_GENERAL, "Error setting configured uid to %s (%d): %s", user, (int)pw->pw_uid,
                 strerror(errno));
        return -1;
    }

    if (seteuid(pw->pw_uid)) {
        log_warn(LD_GENERAL, "Error setting configured euid to %s (%d): %s", user, (int)pw->pw_uid,
                 strerror(errno));
        return -1;
    }
