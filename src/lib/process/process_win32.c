...
process_win32_exec(process_t *process)
{
    ...
    if (!process_win32_create_pipe(&stdout_pipe_read, &stdout_pipe_write, &security_attributes,
                                   PROCESS_WIN32_PIPE_TYPE_READER)) {
        return PROCESS_STATUS_ERROR;
    }

    /* Create our standard error pipe. */
    if (!process_win32_create_pipe(&stderr_pipe_read, &stderr_pipe_write, &security_attributes,
                                   PROCESS_WIN32_PIPE_TYPE_READER)) {
        return PROCESS_STATUS_ERROR;
    }

    /* Create out standard in pipe. */
    if (!process_win32_create_pipe(&stdin_pipe_read, &stdin_pipe_write, &security_attributes,
    ...
    ret = CreateProcessA(NULL, joined_argv, NULL, NULL, TRUE, CREATE_NO_WINDOW,
                         env->windows_environment_block[0] == '\0' ? NULL
    ...
    if (!ret) {
        log_warn(LD_PROCESS, "CreateProcessA() failed: %s", format_win32_error(GetLastError()));
...
process_win32_terminate(process_t *process)
{
    ...
    if (!ret) {
        log_warn(LD_PROCESS, "TerminateProcess() failed: %s", format_win32_error(GetLastError()));
        return false;
    }
...
process_win32_write(struct process_t *process, buf_t *buffer)
{
    ...
    const size_t write_size = MIN(buffer_size, sizeof(win32_process->stdin_handle.buffer));
    ...
    /* Schedule our write. */
    ret = WriteFileEx(win32_process->stdin_handle.pipe, win32_process->stdin_handle.buffer,
                      write_size, &win32_process->stdin_handle.overlapped,
    ...
    if (!ret) {
        ...
        if (error_code == ERROR_HANDLE_EOF || error_code == ERROR_BROKEN_PIPE) {
            ...
            log_warn(LD_PROCESS, "WriteFileEx() failed: %s", format_win32_error(error_code));
        }
...
process_win32_timer_start(void)
{
    ...
    periodic_timer =
        periodic_timer_new(tor_libevent_get_base(), &interval, process_win32_timer_callback, NULL);
}
...
process_win32_timer_test_process(process_t *process)
{
    ...
    ret = GetExitCodeProcess(win32_process->process_information.hProcess, &exit_code);

    if (!ret) {
        log_warn(LD_PROCESS, "GetExitCodeProcess() failed: %s", format_win32_error(GetLastError()));
        return false;
    }
...
STATIC bool
process_win32_create_pipe(HANDLE *read_pipe, HANDLE *write_pipe, SECURITY_ATTRIBUTES *attributes,
                          process_win32_pipe_type_t pipe_type)
{
    ...
    tor_snprintf(pipe_name, sizeof(pipe_name), "\\\\.\\Pipe\\Tor-Process-Pipe-%lu-%lu", process_id,
                 counter++);
    ...
    if (read_handle == INVALID_HANDLE_VALUE) {
        log_warn(LD_PROCESS, "CreateNamedPipeA() failed: %s", format_win32_error(GetLastError()));
        return false;
    }

    /* Create our file in the pipe namespace. */
    write_handle = CreateFileA(pipe_name, GENERIC_WRITE, 0, attributes, OPEN_EXISTING,
                               (FILE_ATTRIBUTE_NORMAL | write_mode), NULL);

    if (write_handle == INVALID_HANDLE_VALUE) {
        log_warn(LD_PROCESS, "CreateFileA() failed: %s", format_win32_error(GetLastError()));
...
STATIC VOID WINAPI
process_win32_stdout_read_done(DWORD error_code, DWORD byte_count, LPOVERLAPPED overlapped)
{
    ...
    if (process_win32_handle_read_completion(&win32_process->stdout_handle, error_code,
                                             byte_count)) {
 ...
 * there is data available for it to read from us. */
STATIC VOID WINAPI
process_win32_stderr_read_done(DWORD error_code, DWORD byte_count, LPOVERLAPPED overlapped)
{
    tor_assert(overlapped);
    tor_assert(overlapped->hEvent);
    ...
    if (process_win32_handle_read_completion(&win32_process->stderr_handle, error_code,
                                             byte_count)) {
        /* Schedule our next read. */
        process_notify_event_stderr(process);
    }
...
STATIC VOID WINAPI
process_win32_stdin_write_done(DWORD error_code, DWORD byte_count, LPOVERLAPPED overlapped)
{
    tor_assert(overlapped);
    tor_assert(overlapped->hEvent);
    ...
    if (error_code == 0) {
        ...
        memset(win32_process->stdin_handle.buffer, 0, sizeof(win32_process->stdin_handle.buffer));

        /* Schedule the next write. */
        process_notify_event_stdin(process);
    } else if (error_code == ERROR_HANDLE_EOF || error_code == ERROR_BROKEN_PIPE) {
         ...
         * reached EOF */
        log_warn(LD_PROCESS, "Error in I/O completion routine from WriteFileEx(): %s",
                 format_win32_error(error_code));
        win32_process->stdin_handle.reached_eof = true;
    }
}
                               ...
                               LPOVERLAPPED_COMPLETION_ROUTINE callback)
{
    ...
    ret = ReadFileEx(handle->pipe, handle->buffer, sizeof(handle->buffer), &handle->overlapped,
                     callback);
    ...
    if (!ret) {
        ...
        if (error_code == ERROR_HANDLE_EOF || error_code == ERROR_BROKEN_PIPE) {
            ...
            log_warn(LD_PROCESS, "ReadFileEx() failed: %s", format_win32_error(error_code));
        }
...
STATIC bool
process_win32_handle_read_completion(process_win32_handle_t *handle, DWORD error_code,
                                     DWORD byte_count)
{
    tor_assert(handle);
    ...
    if (error_code == 0) {
        ...
        /* An error happened: We warn the user and mark our handle as having
         * reached EOF */
        log_warn(LD_PROCESS, "Error in I/O completion routine from ReadFileEx(): %s",
                 format_win32_error(error_code));
...
format_win_cmdline_argument(const char *arg)
{
    ...
    const size_t formatted_arg_len = smartlist_len(arg_chars) + (need_quotes ? 2 : 0) + 1;
