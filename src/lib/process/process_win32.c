...
process_win32_exec(process_t *process)
{
    ...
    if (!ret) {
         ...
         * first do the fork() call in the Tor process and then the child
...
process_win32_write(struct process_t *process, buf_t *buffer)
{
     ...
     * before we call WriteFileEx() because WriteFileEx() does not reset the
...
process_win32_timer_callback(periodic_timer_t *timer, void *data)
{
    ...
    do {
        ...
        SMARTLIST_FOREACH_BEGIN(processes, process_t *, process)
        {
             ...
             * process_get_all_processes(). We start the loop over again until
             * we have a succesful run over the entire list where the list was
             * not modified. */
