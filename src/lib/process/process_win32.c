...
process_win32_exec(process_t *process)
{
    ...
    /* Windows expects argv to be a whitespace delimited string, so join argv
     * up
     */
    char *joined_argv = tor_join_win_cmdline((const char **)argv);

    /* Create the child process */
    ret = CreateProcessA(NULL, joined_argv, NULL, NULL, TRUE, CREATE_NO_WINDOW,
    ...
    if (!ret) {
        ...
        /* In the Unix backend, we do not get an error in the Tor process when
         * a child process fails to spawn its target executable since we need
         * to first do the fork() call in the Tor process and then the child
         * process is responsible for doing the call to execve().
         *
         ...
         * happen on Unix. For example: when the target executable does not
         * exist on the file system.
         *
         * To have somewhat feature compatibility between the Unix and the
         * Windows backend, we here notify the process_t owner that the process
         * have exited (even though it never managed to run) to ensure that the
         * exit callback is executed.
         */
     ...
     * that we can figure out which process_t that was responsible for the
     * event.
     *
     * Warning, here be dragons:
     *
     *   MSDN says that the hEvent member of the overlapped structure is unused
     *   for ReadFileEx() and WriteFileEx, which allows us to store a pointer
     * to our process state there.
     */
...
process_win32_write(struct process_t *process, buf_t *buffer)
{
    ...
    /* Because of the slightly weird API for WriteFileEx() we must set this to
     * 0 before we call WriteFileEx() because WriteFileEx() does not reset the
     * last error itself when it's succesful. See comment below after the call
     * to GetLastError(). */
    SetLastError(0);
     ...
     * should check GetLastError() after a call to WriteFileEx() even though
     * the `ret` return value was successful. If everything is good,
     * GetLastError() returns `ERROR_SUCCESS` and nothing happens.
     *
...
process_win32_read_stdout(struct process_t *process, buf_t *buffer)
{
    ...
    return process_win32_read_from_handle(
        &win32_process->stdout_handle, buffer, process_win32_stdout_read_done);
}
...
process_win32_read_stderr(struct process_t *process, buf_t *buffer)
{
    ...
    return process_win32_read_from_handle(
        &win32_process->stderr_handle, buffer, process_win32_stderr_read_done);
}
...
process_win32_trigger_completion_callbacks(void)
{
     ...
     * the process to enter alertable state, where the Windows kernel will
     * notify us about completed I/O requests from ReadFileEx() and
     * WriteFileEX(), which will cause our completion callbacks to be executed.
     *
     * This function returns 0 if the time interval expired or
     * WAIT_IO_COMPLETION if one or more I/O callbacks were executed. */
    ret = SleepEx(0, TRUE);
...
process_win32_timer_callback(periodic_timer_t *timer, void *data)
{
     ...
     * is important here that we make sure to not touch the list of processes
     * if the call to process_win32_timer_test_process() returns true. */
    bool done;
    ...
    do {
        ...
        SMARTLIST_FOREACH_BEGIN (processes, process_t *, process) {
            /* If process_win32_timer_test_process() returns true, it means
             * that smartlist_remove() might have been called on the list
             * returned by process_get_all_processes(). We start the loop over
             * again until we have a succesful run over the entire list where
...
process_win32_timer_test_process(process_t *process)
{
     ...
     * make sure that our ReadFileEx() calls for the process have *all*
     * returned and both standard out and error have been marked as EOF before
     * we try to see if the process terminated.
     *
     ...
     * standard in's WriteFileEx() function is only called on-demand when we
     * have something to write and is thus usually not awaiting to finish any
     * operations. If we WriteFileEx() to a file that has terminated we'll
     * simply get an error from ReadFileEx() or its completion routine and move
     * on with life.  */
     ...
     * exit_callback might decide to process_free() our process handle it is
     * very important that we do not touch the process_t after the call to
                          ...
                          process_win32_pipe_type_t pipe_type)
{
    ...
    tor_snprintf(pipe_name, sizeof(pipe_name),
                 "\\\\.\\Pipe\\Tor-Process-Pipe-%lu-%lu", process_id,
                 counter++);
    ...
    read_handle = CreateNamedPipeA(
        pipe_name, (PIPE_ACCESS_INBOUND | read_mode),
        (PIPE_TYPE_BYTE | PIPE_WAIT), 1, size, size, 1000, attributes);
                               ...
                               LPOVERLAPPED overlapped)
{
    ...
    if (error_code == 0) {
        /** Our data have been succesfully written. Clear our state and
                               ...
                               LPOVERLAPPED_COMPLETION_ROUTINE callback)
{
    ...
    /* Ask the Windows kernel to read data from our pipe into our buffer and
     * call the callback function when it is done. */
                                     ...
                                     DWORD error_code, DWORD byte_count)
{
    ...
    if (error_code == 0) {
        ...
        /* This cast should be safe since byte_count should never be larger
         * than BUFFER_SIZE. */
        ...
        return true;
               ...
               error_code == ERROR_BROKEN_PIPE) {
        /* Our ReadFileEx() finished, but we reached the end of our file.  We
         * mark our handle as having reached EOF and returns. */
        tor_assert(byte_count == 0);
