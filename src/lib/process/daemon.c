...
start_daemon(void)
{
    ...
    if (pid) { /* Parent */
        ...
        /*
         * Fork one more time, so the parent (the session group leader) can
         * exit. This means that we, as a non-session group leader, can never
         * regain a controlling terminal.   This part is recommended by
         * Stevens's _Advanced Programming in the Unix Environment_.
         */
        if (fork() != 0) {
