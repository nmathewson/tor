...
process_exec(process_t *process)
{
    ...
    if (status != PROCESS_STATUS_RUNNING) {
        log_warn(LD_PROCESS, "Failed to start process: %s", process_get_command(process));
    }
...
void
process_set_stdout_read_callback(process_t *process, process_read_callback_t callback)
{
    tor_assert(process);
 ...
 * the callback to only contain complete "\n" or "\r\n" terminated lines. */
void
process_set_stderr_read_callback(process_t *process, process_read_callback_t callback)
{
    tor_assert(process);
...
process_reset_environment(process_t *process, const smartlist_t *env)
{
    ...
    SMARTLIST_FOREACH(env, char *, x, smartlist_add(process->environment, tor_strdup(x)));
}
...
process_notify_event_stdout(process_t *process)
{
    ...
    if (ret > 0)
        process_read_data(process, process->stdout_buffer, process->stdout_read_callback);
}
...
process_notify_event_stderr(process_t *process)
{
    ...
    if (ret > 0)
        process_read_data(process, process->stderr_buffer, process->stderr_read_callback);
}
...
process_notify_event_exit(process_t *process, process_exit_code_t exit_code)
{
    ...
    log_debug(LD_PROCESS, "Process terminated with exit code: %" PRIu64, exit_code);
...
process_read_stdout, (process_t * process, buf_t *buffer))
{
    tor_assert(process);
    tor_assert(buffer);
...
MOCK_IMPL(STATIC int,
process_read_stderr, (process_t * process, buf_t *buffer))
{
    tor_assert(process);
    tor_assert(buffer);
...
process_write_stdin, (process_t * process, buf_t *buffer))
{
    tor_assert(process);
    tor_assert(buffer);
...
STATIC void
process_read_data(process_t *process, buf_t *buffer, process_read_callback_t callback)
{
    tor_assert(process);
    tor_assert(buffer);
...
STATIC void
process_read_buffer(process_t *process, buf_t *buffer, process_read_callback_t callback)
{
    tor_assert(process);
    tor_assert(buffer);
...
STATIC void
process_read_lines(process_t *process, buf_t *buffer, process_read_callback_t callback)
{
    tor_assert(process);
    tor_assert(buffer);
