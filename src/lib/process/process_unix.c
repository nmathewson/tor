...
#    endif /* defined(HAVE_SYS_PRCTL_H) && defined(__linux__) */
        ...
        /* Close all other fds, including the read end of the pipe.  XXX: We
         * should now be doing enough FD_CLOEXEC setting to make this needless.
         */
...
process_unix_write(process_t *process, buf_t *buffer)
{
    ...
    /* We don't have any data to write, but the kernel is currently notifying
     * us about whether we are able to write or not. Tell the kernel to stop
    ...
    return buf_flush_to_pipe(
        buffer, process_get_unix_process(process)->stdin_handle.fd,
...
process_unix_waitpid_callback(int status, void *data)
{
     ...
     * process_notify_event_exit() on it, to allow users to process_free() it
                         ...
                         buf_t *buffer)
{
    ...
    ret =
        buf_read_from_pipe(buffer, handle->fd, PROCESS_MAX_READ, &eof, &error);
