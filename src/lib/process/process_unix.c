...
#    ifdef _SC_OPEN_MAX
    if (-1 == max_fd) {
        ...
        if (max_fd == -1) {
            ...
            log_warn(LD_PROCESS, "Cannot find maximum file descriptor, assuming: %d", max_fd);
        }
    }
...
#    endif /* defined(HAVE_SYS_PRCTL_H) && defined(__linux__) */
    ...
    if (-1 == pid) {
        log_warn(LD_PROCESS, "Failed to create child process: %s", strerror(errno));
    ...
    unix_process->waitpid = set_waitpid_callback(pid, process_unix_waitpid_callback, process);
    ...
    if (-1 == retval) {
        log_warn(LD_PROCESS, "Failed to close write end of standard out pipe: %s", strerror(errno));
    }
    ...
    if (-1 == retval) {
        log_warn(LD_PROCESS, "Failed to close write end of standard error pipe: %s",
                 strerror(errno));
    }
    ...
    if (-1 == retval) {
        log_warn(LD_PROCESS, "Failed to close read end of standard in pipe: %s", strerror(errno));
    }

    /* Setup our handles. */
    process_unix_setup_handle(process, &unix_process->stdout_handle, EV_READ | EV_PERSIST,
                              stdout_read_callback);

    process_unix_setup_handle(process, &unix_process->stderr_handle, EV_READ | EV_PERSIST,
                              stderr_read_callback);

    process_unix_setup_handle(process, &unix_process->stdin_handle, EV_WRITE | EV_PERSIST,
...
process_unix_write(process_t *process, buf_t *buffer)
{
    ...
    return buf_flush_to_pipe(buffer, process_get_unix_process(process)->stdin_handle.fd,
...
process_unix_read_stdout(process_t *process, buf_t *buffer)
{
    ...
    return process_unix_read_handle(process, &unix_process->stdout_handle, buffer);
}
...
process_unix_read_stderr(process_t *process, buf_t *buffer)
{
    ...
    return process_unix_read_handle(process, &unix_process->stderr_handle, buffer);
}
 ...
 * given <b>handle</b>. */
STATIC void
process_unix_setup_handle(process_t *process, process_unix_handle_t *handle, short flags,
                          event_callback_fn callback)
{
    ...
    if (fcntl(handle->fd, F_SETFL, O_NONBLOCK) < 0) {
        log_warn(LD_PROCESS, "Unable mark Unix handle as non-blocking: %s", strerror(errno));
    }

    /* Setup libevent event. */
    handle->event = tor_event_new(tor_libevent_get_base(), handle->fd, flags, callback, process);
}

/** This function reads data from the given <b>handle</b> and puts it into
 * <b>buffer</b>. Returns the number of bytes read this way. */
STATIC int
process_unix_read_handle(process_t *process, process_unix_handle_t *handle, buf_t *buffer)
{
    tor_assert(process);
    tor_assert(handle);
    tor_assert(buffer);
