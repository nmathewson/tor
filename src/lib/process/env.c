...
process_environment_make(struct smartlist_t *env_vars)
{
    ...
    /* Now copy the strings into the environment blocks. */
    {
        ...
        for (i = 0; i < n_env_vars; ++i) {
            ...
            if (prev_env_var != NULL && environment_variable_names_equal(s, prev_env_var)) {
                log_warn(LD_GENERAL,
...
void
set_environment_variable_in_smartlist(struct smartlist_t *env_vars, const char *new_var,
