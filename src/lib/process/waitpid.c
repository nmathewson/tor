...
clear_waitpid_callback(waitpid_callback_t *ent)
{
    ...
    if (ent->running) {
        ...
        if (old_ent != ent) {
            log_warn(LD_BUG, "Couldn't remove waitpid monitor for pid %u.", (unsigned)ent->pid);
            return;
        }
    }
...
notify_waitpid_callback_by_pid(pid_t pid, int status)
{
    ...
    log_info(LD_GENERAL, "Child process %u has exited; running callback.", (unsigned)pid);
