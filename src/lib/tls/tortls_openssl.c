    ...
    case SSL_ERROR_SYSCALL:
        ...
        if (r == 0) {
            tor_log(severity, LD_NET,
                    "TLS error: unexpected close while %s (%s)", doing,
    ...
    if (!tls_library_is_initialized) {
        ...
        if (version >= OPENSSL_V_SERIES(1, 0, 1)) {
            /* Warn if we could *almost* be running with much faster ECDH.
               ...
               (We could be looking at OPENSSL_NO_EC_NISTP_64_GCC_128 instead
               of doing this test, but that gives compile-time options, not
               runtime behavior.)
            */
                    ...
                    unsigned flags, int is_client)
{
  ...
  if (!(result->ctx = SSL_CTX_new(TLSv1_method())))
     ...
     * renegotiation as authenticating any earlier-received data.
     */
    {
        SSL_CTX_set_options(result->ctx,
...
find_cipher_by_id(const SSL *ssl, const SSL_METHOD *m, uint16_t cipher)
{
    ...
    (void)m;
    {
        ...
        cipherid[2] = 0; /* If ssl23_get_cipher_by_char finds no cipher
                          * starting with a two-byte 'cipherid', it may look
                          * for a v2 cipher with the appropriate 3 bytes. */
...
tor_tls_server_info_callback(const SSL *ssl, int type, int val)
{
    ...
    if (tor_tls_client_is_using_v2_ciphers(ssl)) {
        if (tls->wasV2Handshake)
            return; /* We already turned this stuff off for the first
                     * handshake; This is a renegotiation. */

        /* Yes, we're casting away the const from ssl.  This is very naughty of
         * us. Let's hope openssl doesn't notice! */

        /* Set SSL_MODE_NO_AUTO_CHAIN to keep from sending back any extra
         * certs. */
...
tor_tls_finish_handshake(tor_tls_t *tls)
{
    ...
    if (tls->isServer) {
        ...
        if (tor_tls_client_is_using_v2_ciphers(tls->ssl)) {
            /* This check is redundant, but back when we did it in the
             * callback, we might have not been able to look up the tor_tls_t
             * if the code was buggy.  Fixing that. */
            if (!tls->wasV2Handshake) {
...
tor_tls_get_n_raw_bytes(tor_tls_t *tls, size_t *n_read, size_t *n_written)
{
     ...
     * supposed to use this form of the version macro, but the OpenSSL
     * developers introduced major API changes in the pre-release stage.
     */
...
#define TLSSECRET_MAGIC "Tor V3 handshake TLS cross-certification"
    ...
    {
        size_t r = SSL_get_server_random(ssl, buf + client_random_len,
                                         server_random_len);
           ...
           size_t context_len, const char *label))
{
    ...
    if (r != 1 && strlen(label) > 12 &&
        ...
        if (!openssl_bug_7712_is_present) {
             ...
             */
            r = SSL_export_keying_material(tls->ssl, secrets_out,
                                           DIGEST256_LEN, "short", 5, context,
                                           context_len, 1);
            if (r == 1) {
                /* A short label succeeds, but a long label fails. This was
                 * openssl issue 7712. */
                openssl_bug_7712_is_present = 1;
                log_warn(LD_GENERAL,
                         "Detected OpenSSL bug 7712: disabling TLS 1.3 on "
                         "future connections. A fix is expected to appear in "
                         "OpenSSL "
                         "1.1.1b.");
            }
        }
