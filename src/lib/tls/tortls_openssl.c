...
tor_tls_allocate_tor_tls_object_ex_data_index(void)
{
    if (tor_tls_object_ex_data_index == -1) {
        tor_tls_object_ex_data_index = SSL_get_ex_new_index(0, NULL, NULL, NULL, NULL);
...
void
tor_tls_log_one_error(tor_tls_t *tls, unsigned long err, int severity, int domain,
                      const char *doing)
{
    ...
    if (!func)
        func = "(null)";
    if (doing) {
        tor_log(severity, domain, "TLS error while %s%s%s: %s (in %s:%s:%s)", doing,
                addr ? " with " : "", addr ? addr : "", msg, lib, func, state);
    } else {
        tor_log(severity, domain, "TLS error%s%s: %s (in %s:%s:%s)", addr ? " with " : "",
                addr ? addr : "", msg, lib, func, state);
    }
}
 ...
 */
int
tor_tls_get_error(tor_tls_t *tls, int r, int extra, const char *doing, int severity, int domain)
{
    ...
    case SSL_ERROR_SYSCALL:
        ...
        if (r == 0) {
            tor_log(severity, LD_NET, "TLS error: unexpected close while %s (%s)", doing,
                    SSL_state_string_long(tls->ssl));
            tor_error = TOR_TLS_ERROR_IO;
        } else {
            int e = tor_socket_errno(tls->socket);
            tor_log(severity, LD_NET,
                    "TLS error: <syscall error while %s> (errno=%d: %s; state=%s)", doing, e,
                    tor_socket_strerror(e), SSL_state_string_long(tls->ssl));
    ...
    case SSL_ERROR_ZERO_RETURN:
        ...
        tor_log(severity, LD_NET, "TLS connection closed while %s in state %s", doing,
                SSL_state_string_long(tls->ssl));
        tls_log_errors(tls, severity, domain, doing);
    ...
    if (!tls_library_is_initialized) {
        ...
        if (version >= OPENSSL_V_SERIES(1, 0, 1)) {
            ...
            */
            ...
            const int warn = (m == EC_GFp_simple_method() || m == EC_GFp_mont_method() ||
 ...
 */
tor_tls_context_t *
tor_tls_context_new(crypto_pk_t *identity, unsigned int key_lifetime, unsigned flags, int is_client)
{
    EVP_PKEY *pkey = NULL;
    ...
    if (!is_client) {
        if (tor_tls_context_init_certificates(result, identity, key_lifetime, flags) < 0) {
            goto error;
        }
    }
 ...
 * *we* don't recognize. */
STATIC uint16_t v2_cipher_list[] = {0xc00a, /* TLS1_TXT_ECDHE_ECDSA_WITH_AES_256_CBC_SHA */
 ...
 **/
int
tor_tls_classify_client_ciphers(const SSL *ssl, STACK_OF(SSL_CIPHER) * peer_ciphers)
{
    int i, res;
    tor_tls_t *tor_tls;
    ...
    for (i = 0; i < sk_SSL_CIPHER_num(peer_ciphers); ++i) {
        ...
        if (strcmp(ciphername, TLS1_TXT_DHE_RSA_WITH_AES_128_SHA) &&
            ...
            strcmp(ciphername, SSL3_TXT_EDH_RSA_DES_192_CBC3_SHA) && strcmp(ciphername, "(NONE)")) {
...
tor_tls_session_secret_cb(SSL *ssl, void *secret, int *secret_len,
                          ...
                          CONST_IF_OPENSSL_1_1_API SSL_CIPHER **cipher, void *arg)
{
    (void)secret;
    (void)secret_len;
    (void)peer_ciphers;
    ...
    if (tor_tls_classify_client_ciphers(ssl, peer_ciphers) == CIPHERS_UNRESTRICTED) {
...
tor_tls_new(tor_socket_t sock, int isServer)
{
    ...
    if (!SSL_set_cipher_list(result->ssl, isServer ? SERVER_CIPHER_LIST : CLIENT_CIPHER_LIST)) {
    ...
    {
        int set_worked = SSL_set_ex_data(result->ssl, tor_tls_object_ex_data_index, result);
        if (!set_worked) {
            log_warn(LD_BUG, "Couldn't set the tls for an SSL*; connection will fail");
        }
    }
 ...
 */
void
tor_tls_set_renegotiate_callback(tor_tls_t *tls, void (*cb)(tor_tls_t *, void *arg), void *arg)
{
...
tor_tls_write(tor_tls_t *tls, const char *cp, size_t n)
{
    ...
    if (tls->wantwrite_n) {
        ...
        log_debug(LD_NET, "resuming pending-write, (%d to flush, reusing %d)", (int)n,
                  (int)tls->wantwrite_n);
        n = tls->wantwrite_n;
        tls->wantwrite_n = 0;
    }
...
tor_tls_handshake(tor_tls_t *tls)
{
    ...
    if (ERR_peek_error() != 0) {
        tls_log_errors(tls, tls->isServer ? LOG_INFO : LOG_WARN, LD_HANDSHAKE, "handshaking");
        return TOR_TLS_ERROR_MISC;
    }
    if (r == TOR_TLS_DONE) {
...
tor_tls_get_own_cert, (tor_tls_t * tls))
{
    ...
    tls_log_errors(tls, LOG_WARN, LD_HANDSHAKE, "getting own-connection certificate");
    if (!cert)
        return NULL;
...
tls_get_write_overhead_ratio, (void))
{
    ...
    return ((double)total_bytes_written_by_tls) / ((double)total_bytes_written_over_tls);
}
...
check_no_tls_errors_(const char *fname, int line)
{
    if (ERR_peek_error() == 0)
        return;
    log_warn(LD_CRYPTO, "Unhandled OpenSSL errors found at %s:%d: ", tor_fix_source_file(fname),
             line);
...
#define TLSSECRET_MAGIC "Tor V3 handshake TLS cross-certification"
    ...
    */
    crypto_hmac_sha256((char *)secrets_out, (char *)master_key, master_key_len, (char *)buf, len);
...
tor_tls_export_key_material,
          (tor_tls_t * tls, uint8_t *secrets_out, const uint8_t *context, size_t context_len,
           const char *label))
{
    tor_assert(tls);
    tor_assert(tls->ssl);

    int r = SSL_export_keying_material(tls->ssl, secrets_out, DIGEST256_LEN, label, strlen(label),
    ...
    if (r != 1 && strlen(label) > 12 && SSL_version(tls->ssl) >= TLS1_3_VERSION) {
        if (!openssl_bug_7712_is_present) {
             ...
             */
            r = SSL_export_keying_material(tls->ssl, secrets_out, DIGEST256_LEN, "short", 5,
                                           context, context_len, 1);
            if (r == 1) {
...
int
tor_tls_get_buffer_sizes(tor_tls_t *tls, size_t *rbuf_capacity, size_t *rbuf_bytes,
