...
int tor_tls_context_init(unsigned flags, crypto_pk_t *client_identity, crypto_pk_t *server_identity,
...
void tor_tls_set_renegotiate_callback(tor_tls_t *tls, void (*cb)(tor_tls_t *, void *arg),
                                      void *arg);
...
int tor_tls_get_buffer_sizes(tor_tls_t *tls, size_t *rbuf_capacity, size_t *rbuf_bytes,
...
void tor_tls_log_one_error(tor_tls_t *tls, unsigned long err, int severity, int domain,
                           const char *doing);
#else /* !defined(ENABLE_OPENSSL) */
#    define check_no_tls_errors() STMT_NIL
#endif /* defined(ENABLE_OPENSSL) */

int tor_tls_get_my_certs(int server, const struct tor_x509_cert_t **link_cert_out,
