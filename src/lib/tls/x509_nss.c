...
tor_tls_create_certificate_internal(crypto_pk_t *rsa, crypto_pk_t *rsa_sign, CERTName *subject_dn,
                                    CERTName *issuer_dn, time_t start_time, time_t end_time)
{
    ...
    request = CERT_CreateCertificateRequest(subject_dn, subject_spki, NULL /* attributes */);
    if (!request)
        goto err;
    ...
    *cert->version.data = 2; /* 2 means version 3. */
    ...
    SECOidTag oid_tag = SEC_GetSignatureAlgorithmOidTag(privkey_type, SEC_OID_SHA256);
    ...
    void *tmp;
    tmp = SEC_ASN1EncodeItem(cert->arena, &der, cert, SEC_ASN1_GET(CERT_CertificateTemplate));
    if (!tmp)
        goto err;
  ...
  s = SEC_DerSignDataWithAlgorithmID(cert->arena,
...
#if 1
    {
        ...
        SECKEYPublicKey *issuer_pk = (SECKEYPublicKey *)crypto_pk_get_nss_pubkey(rsa_sign);
        SECStatus cert_ok =
            CERT_VerifySignedDataWithPublicKey(&result_cert->signatureWrap, issuer_pk, NULL);
...
tor_tls_create_certificate,
          (crypto_pk_t * rsa, crypto_pk_t *rsa_sign, const char *cname, const char *cname_sign,
           unsigned int cert_lifetime))
{
    ...
    tor_tls_pick_certificate_lifetime(time(NULL), cert_lifetime, &start_time, &end_time);

    result = tor_tls_create_certificate_internal(rsa, rsa_sign, subject_dn, issuer_dn, start_time,
                                                 end_time);
err:
...
void
tor_x509_cert_get_der(const tor_x509_cert_t *cert, const uint8_t **encoded_out, size_t *size_out)
{
    tor_assert(cert);
...
tor_x509_cert_decode_internal(const uint8_t *certificate, int certificate_len)
{
    ...
    SECItem der = {.type = siBuffer, .data = (unsigned char *)certificate, .len = certificate_len};
    CERTCertDBHandle *certdb = CERT_GetDefaultCertDB();
    tor_assert(certdb);
    return CERT_NewTempCertificate(certdb, &der, NULL /* nickname */, PR_FALSE, /* isPerm */
                                   PR_TRUE /* CopyDER */);
}
...
{
    CERTCertificate *cert = tor_x509_cert_decode_internal(certificate, (int)certificate_len);
    if (!cert) {
...
tor_tls_cert_is_valid(int severity, const tor_x509_cert_t *cert,
                      const tor_x509_cert_t *signing_cert, time_t now, int check_rsa_1024)
{
    int result = 0;
    ...
    if (pk == NULL) {
        log_fn(severity, LD_CRYPTO, "Invalid certificate: could not extract issuer key");
        goto fail;
    }

    SECStatus s = CERT_VerifySignedDataWithPublicKey(&cert->cert->signatureWrap, pk, NULL);
    if (s != SECSuccess) {
        log_fn(severity, LD_CRYPTO, "Invalid certificate: could not validate signature.");
        goto fail;
    }

    if (tor_x509_check_cert_lifetime_internal(severity, cert->cert, now, TOR_X509_PAST_SLOP,
                                              TOR_X509_FUTURE_SLOP) < 0)
        goto fail;

    if (check_rsa_1024) {
        /* We require that this is a 1024-bit RSA key, for legacy reasons .:p */
        if (SECKEY_GetPublicKeyType(pk) != rsaKey || SECKEY_PublicKeyStrengthInBits(pk) != 1024) {
...
static void
log_cert_lifetime(int severity, const char *status, time_t now, PRTime notBefore, PRTime notAfter)
{
    ...
    log_fn(severity, LD_GENERAL, "(The certificate is valid from %s until %s. Your time is %s.)",
           nbbuf, nabuf, nowbuf);
}

int
tor_x509_check_cert_lifetime_internal(int severity, const tor_x509_cert_impl_t *cert, time_t now,
                                      int past_tolerance, int future_tolerance)
{
    ...
    if (r != SECSuccess) {
        log_fn(severity, LD_CRYPTO, "Couldn't get validity times from certificate");
        return -1;
    }
...
tor_x509_cert_t *
tor_x509_cert_replace_expiration(const tor_x509_cert_t *inp, time_t new_expiration_time,
                                 crypto_pk_t *signing_key)
{
    ...
    newcert =
        tor_tls_create_certificate_internal(subject_key, signing_key, &inp->cert->subject,
                                            &inp->cert->issuer, start_time, new_expiration_time);
