                                    ...
                                    time_t start_time, time_t end_time)
{
  ...
  s = SEC_DerSignDataWithAlgorithmID(cert->arena,
    ...
    result_cert =
        tor_x509_cert_decode_internal(signed_der.data, signed_der.len);
...
tor_tls_cert_get_key(tor_x509_cert_t *cert)
{
    ...
    SECKEYPublicKey *pub =
        SECKEY_ExtractPublicKey(spki); // we own this pointer
    if (pub == NULL)
        return NULL;
                      ...
                      int check_rsa_1024)
{
    ...
    SECStatus s = CERT_VerifySignedDataWithPublicKey(
        &cert->cert->signatureWrap, pk, NULL);
    if (s != SECSuccess) {
        log_fn(severity, LD_CRYPTO,
    ...
    if (check_rsa_1024) {
        ...
        unsigned min_bits =
            (SECKEY_GetPublicKeyType(pk) == ecKey) ? 128 : 1024;
                                      ...
                                      int future_tolerance)
{
    ...
    t *= PRTIME_PER_SEC;
    if (notAfter < t) {
        log_cert_lifetime(severity, "already expired", now, notBefore,
                          notAfter);
        return -1;
    }
