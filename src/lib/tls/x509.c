...
void
tor_tls_pick_certificate_lifetime(time_t now, unsigned int cert_lifetime, time_t *start_time_out,
                                  time_t *end_time_out)
{
    ...
    if (cert_lifetime <= min_real_lifetime + start_granularity) {
        ...
        earliest_start_time = now + min_real_lifetime + start_granularity - cert_lifetime;
    }
