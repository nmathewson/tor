...
int
tor_tls_context_init(unsigned flags, crypto_pk_t *client_identity, crypto_pk_t *server_identity,
                     unsigned int key_lifetime)
{
    ...
    if (is_public_server) {
        ...
        rv1 =
            tor_tls_context_init_one(&server_tls_context, server_identity, key_lifetime, flags, 0);
        ...
        if (server_identity != NULL) {
            rv1 = tor_tls_context_init_one(&server_tls_context, server_identity, key_lifetime,
                                           flags, 0);
            if (rv1 < 0)
                tls_log_errors(NULL, LOG_WARN, LD_CRYPTO, "constructing a server TLS context");
        } else {
        ...
        rv2 =
            tor_tls_context_init_one(&client_tls_context, client_identity, key_lifetime, flags, 1);
        if (rv2 < 0)
            tls_log_errors(NULL, LOG_WARN, LD_CRYPTO, "constructing a client TLS context");
    }
...
tor_tls_context_init_one(tor_tls_context_t **ppcontext, crypto_pk_t *identity,
                         unsigned int key_lifetime, unsigned int flags, int is_client)
{
    tor_tls_context_t *new_ctx = tor_tls_context_new(identity, key_lifetime, flags, is_client);
 ...
 */
int
tor_tls_context_init_certificates(tor_tls_context_t *result, crypto_pk_t *identity,
...
const subsys_fns_t sys_tortls = {
    .name = "tortls", SUBSYS_DECLARE_LOCATION(), .level = -50, .shutdown = subsys_tortls_shutdown};
