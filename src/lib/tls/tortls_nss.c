                    ...
                    unsigned flags, int is_client)
{
    ...
    if (!is_client) {
        if (tor_tls_context_init_certificates(ctx, identity, key_lifetime,
                                              flags) < 0) {
            goto err;
        }
    }
    ...
    s = SSL_OptionSet(ctx->ctx,
                      is_client ? SSL_HANDSHAKE_AS_CLIENT
    ...
    // Only support strong ciphers.
    {
        ...
        for (i = 0; i < n_ciphers; ++i) {
            ...
            if (BUG(info.cipherSuite != ciphers[i]))
                goto err;
            int disable =
                info.effectiveKeyBits < 128 || info.macBits < 128 ||
            ...
            if (ciphersuite_has_nss_export_bug(&info)) {
                /* SSL_ExportKeyingMaterial will fail; we can't use this
                 * cipher.
                 */
                disable = 1;
            }
...
tls_log_errors(tor_tls_t *tls, int severity, int domain, const char *doing)
{
    ...
    if (doing) {
        ...
        log_fn(severity, domain, "TLS error %s%s%s: %s", name, string, with,
               addr);
    }
}
...
{
    /* XXX We don't currently have a way to measure this in NSS; we could do
     * that
