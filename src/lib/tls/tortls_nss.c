...
ciphersuite_has_nss_export_bug(const SSLCipherSuiteInfo *info)
{
     ...
     * name.  */
    if (strstr(info->cipherSuiteName, "_SHA384") || strstr(info->cipherSuiteName, "_SHA512")) {
        return true;
    }
...
tor_tls_context_new(crypto_pk_t *identity, unsigned int key_lifetime, unsigned flags, int is_client)
{
    ...
    if (!is_client) {
        if (tor_tls_context_init_certificates(ctx, identity, key_lifetime, flags) < 0) {
            goto err;
        }
    }
    ...
    if (!is_client) {
        s = SSL_ConfigServerCert(ctx->ctx, ctx->my_link_cert->cert,
    ...
    s = SSL_OptionSet(ctx->ctx, is_client ? SSL_HANDSHAKE_AS_CLIENT : SSL_HANDSHAKE_AS_SERVER,
                      PR_TRUE);
    if (s != SECSuccess)
        goto err;
    ...
    // Only support strong ciphers.
    {
        ...
        for (i = 0; i < n_ciphers; ++i) {
            ...
            int disable = info.effectiveKeyBits < 128 || info.macBits < 128 ||
                          !we_like_ssl_cipher(info.symCipher) || !we_like_ssl_kea(info.keaType) ||
            ...
            s = SSL_CipherPrefSet(ctx->ctx, ciphers[i], disable ? PR_FALSE : PR_TRUE);
            if (s != SECSuccess)
                goto err;
        }
    }
...
tls_log_errors(tor_tls_t *tls, int severity, int domain, const char *doing)
{
    ...
    addr = addr ? addr : "";
    if (doing) {
        log_fn(severity, domain, "TLS error %s while %s%s%s: %s", name, doing, with, addr, string);
    } else {
...
tor_tls_release_socket(tor_tls_t *tls)
{
     ...
     */
    tor_socket_t sock = tor_open_socket_nonblocking(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (!SOCKET_OK(sock)) {
...
MOCK_IMPL(int,
tor_tls_cert_matches_key, (const tor_tls_t *tls, const struct tor_x509_cert_t *cert))
{
    tor_assert(tls);
    tor_assert(cert);
    int rv = 0;
    ...
    rv = SECOID_CompareAlgorithmID(&peer_info->algorithm, &cert_info->algorithm) == 0 &&
         SECITEM_ItemsAreEqual(&peer_info->subjectPublicKey, &cert_info->subjectPublicKey);
           ...
           const char *label))
{
    ...
    s = SSL_ExportKeyingMaterial(tls->ssl, label, (unsigned)strlen(label), PR_TRUE, context,
                                 (unsigned)context_len, secrets_out, DIGEST256_LEN);
    if (s != SECSuccess) {
        tls_log_errors(tls, LOG_WARN, LD_CRYPTO, "exporting key material for a TLS handshake");
    }
...
tor_tls_get_ciphersuite_name(tor_tls_t *tls)
{
    ...
    SECStatus s = SSL_GetChannelInfo(tls->ssl, &channel_info, sizeof(channel_info));
    if (s != SECSuccess)
        return NULL;

    s = SSL_GetCipherSuiteInfo(channel_info.cipherSuite, &cipher_info, sizeof(cipher_info));
    if (s != SECSuccess)
        return NULL;
...
static SECStatus
always_accept_cert_cb(void *arg, PRFileDesc *ssl, PRBool checkSig, PRBool isServer)
{
    (void)arg;
    (void)ssl;
    (void)checkSig;
