...
struct tor_tls_t {
    uint32_t magic;
    tor_tls_context_t
        *context; /** A link to the context object for this tls. */
    ...
    unsigned int
        isServer : 1; /**< True iff this is a server-side connection */
    unsigned int wasV2Handshake : 1; /**< True iff the original handshake for
                                      * this connection used the updated
                                      * version of the connection protocol
                                      * (client sends different cipher list,
                                      * server sends only one certificate). */
