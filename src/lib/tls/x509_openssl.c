...
#define SERIAL_NUMBER_SIZE 8
    ...
    tor_tls_pick_certificate_lifetime(now, cert_lifetime, &start_time, &end_time);
...
static void
log_cert_lifetime(int severity, const X509 *cert, const char *problem, time_t now)
{
    BIO *bio = NULL;
    BUF_MEM *buf;
    ...
    if (n > 0) {
        tor_log(severity, LD_GENERAL,
                "(certificate lifetime runs from %s through %s. Your time is %s.)", s1, s2, mytime);
    } else {
        tor_log(severity, LD_GENERAL,
 ...
 * and return -1. */
int
tor_x509_check_cert_lifetime_internal(int severity, const X509 *cert, time_t now,
