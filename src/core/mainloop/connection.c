        ...
        case CONN_TYPE_AP:
            ...
            switch (TO_CONN(listener)->type) {
                ...
                case CONN_TYPE_AP_TRANS_LISTENER:
                    ...
                    return connection_ap_process_transparent(
                        TO_ENTRY_CONN(conn));
                case CONN_TYPE_AP_NATD_LISTENER:
                    TO_ENTRY_CONN(conn)->is_transparent_ap = 1;
...
connection_connect_log_client_use_ip_version(const connection_t *conn)
{
    ...
    if ((must_ipv4 && tor_addr_family(&real_addr) == AF_INET6) ||
        ...
        log_info(LD_BUG,
                 ...
                 fmt_addr(&real_addr), options->ClientUseIPv4 == 0 ? "4" : "6");
        if (!logged_backtrace) {
    ...
    switch (type) {
        ...
        case PROXY_SOCKS4: {
            ...
            if (socks_args_string) { /* place the SOCKS args string: */
                ...
                tor_assert(buf_size >= SOCKS4_STANDARD_BUFFER_SIZE +
        ...
        case PROXY_SOCKS5: {
            ...
            if ((options->Socks5ProxyUsername) ||
                (conn_get_proxy_type(conn) == PROXY_PLUGGABLE &&
                 (get_socks_args_by_bridge_addrport(&conn->addr,
                                                    conn->port)))) {
...
connection_read_proxy_handshake(connection_t *conn)
{
    ...
    switch (conn->proxy_state) {
        ...
        case PROXY_SOCKS4_WANT_CONNECT_OK:
            ret = connection_fetch_from_buf_socks_client(
                conn, conn->proxy_state, &reason);
            if (ret == 1)
                conn->proxy_state = PROXY_CONNECTED;
            break;
            ...
            if (ret == 1) {
                ...
                if (conn_get_proxy_type(conn) == PROXY_PLUGGABLE) {
                    ...
                    if (strlen(socks_args_string) >
                        ...
                        psize = strlen(socks_args_string) -
                                MAX_SOCKS5_AUTH_FIELD_SIZE;
                    } else {
                        user = socks_args_string;
        ...
        switch (result) {
            ...
            case TOR_TLS_WANTREAD:
                if (conn->in_connection_handle_write) {
                     ...
                     * started, and SSL_read returned WANTWRITE.  But now
                     * SSL_read is saying WANTREAD again.  Stop waiting for
                     * write events now, or else we'll busy-loop until data
                     * arrives for us to read.
        ...
        conn, (conn->type == type && (!state || state == conn->state)) &&
                  ((type == CONN_TYPE_DIR && TO_DIR_CONN(conn)->rend_data &&
                   ...
                   (CONN_IS_EDGE(conn) && TO_EDGE_CONN(conn)->rend_data &&
                    !rend_cmp_service_ids(
                        rendquery, rend_data_get_address(
