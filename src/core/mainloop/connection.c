...
static connection_t *connection_listener_new_for_port(const port_cfg_t *port, int *defer,
                                                      int *addr_in_use);
...
const tor_addr_t *conn_get_outbound_address(sa_family_t family, const or_options_t *options,
...
conn_state_to_string(int type, int state)
{
    ...
    tor_snprintf(buf, sizeof(buf), "unknown state [%d] on unknown [%s] connection", state,
...
{
    control_connection_t *control_conn = tor_malloc_zero(sizeof(control_connection_t));
    connection_init(time(NULL), TO_CONN(control_conn), CONN_TYPE_CONTROL, socket_family);
    return control_conn;
}
...
{
    listener_connection_t *listener_conn = tor_malloc_zero(sizeof(listener_connection_t));
...
connection_free_minimal(connection_t *conn)
{
    ...
    if (conn->linked) {
        log_info(LD_GENERAL,
                 ...
                 conn_type_to_string(conn->type), conn_state_to_string(conn->type, conn->state),
                 (int)connection_get_inbuf_len(conn), (int)connection_get_outbuf_len(conn));
    }
    ...
    if (!connection_is_listener(conn)) {
        ...
        if (conn->socket_family == AF_UNIX) {
            ...
            if (unlink(conn->address) < 0 && errno != ENOENT) {
                log_warn(LD_NET, "Could not unlink %s: %s", conn->address, strerror(errno));
            }
        }
    }
    ...
    if (conn->type == CONN_TYPE_OR && !tor_digest_is_zero(TO_OR_CONN(conn)->identity_digest)) {
...
connection_free_, (connection_t * conn))
{
    ...
    if (BUG(conn->linked_conn)) {
        ...
        if (!conn->linked_conn->marked_for_close && conn->linked_conn->reading_from_linked_conn)
    ...
    /* DEBUGGING */
    if (conn->type == CONN_TYPE_AP) {
        connection_ap_warn_and_unmark_if_pending_circ(TO_ENTRY_CONN(conn), "connection_free");
    }
#endif /* 1 */
...
connection_close_immediate(connection_t *conn)
{
    ...
    if (conn->outbuf_flushlen) {
        log_info(LD_NET, "fd %d, type %s, state %s, %d bytes on outbuf.", (int)conn->s,
                 conn_type_to_string(conn->type), conn_state_to_string(conn->type, conn->state),
...
create_unix_sockaddr(const char *listenaddress, char **readable_address, socklen_t *len_out)
{
    ...
    if (strlcpy(sockaddr->sun_path, listenaddress, sizeof(sockaddr->sun_path)) >=
        ...
        log_warn(LD_CONFIG, "Unix socket path '%s' is too long to fit.", escaped(listenaddress));
        tor_free(sockaddr);
        return NULL;
    }
...
create_unix_sockaddr(const char *listenaddress, char **readable_address, socklen_t *len_out)
{
    ...
    log_fn(LOG_ERR, LD_BUG, "Unix domain sockets not supported, yet we tried to create one.");
    *len_out = 0;
    tor_fragile_assert();
    return NULL;
}
...
#define WARN_TOO_MANY_CONNS_INTERVAL (6 * 60 * 60)
    ...
    if ((m = rate_limit_log(&last_warned, approx_time()))) {
        ...
        log_warn(LD_NET,
                 ...
                 n_conns, m);
        tor_free(m);
        control_event_general_status(LOG_WARN, "TOO_MANY_CONNECTIONS CURRENT=%d", n_conns);
    }
}
...
static int
check_location_for_unix_socket(const or_options_t *options, const char *path, int purpose,
...
make_socket_reuseable(tor_socket_t sock)
{
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void *)&one, (socklen_t)sizeof(one)) == -1) {
        return -1;
    }
    return 0;
...
#    ifdef SO_EXCLUSIVEADDRUSE
     ...
     * on win32. */
    if (setsockopt(sock, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (void *)&one, (socklen_t)sizeof(one))) {
        return -1;
    }
    return 0;
 ...
 */
static connection_t *
connection_listener_new(const struct sockaddr *listensockaddr, socklen_t socklen, int type,
                        const char *address, const port_cfg_t *port_cfg, int *addr_in_use)
{
    ...
    if (listensockaddr->sa_family == AF_INET || listensockaddr->sa_family == AF_INET6) {
        ...
        if (make_socket_reuseable(s) < 0) {
            log_warn(LD_NET, "Error setting SO_REUSEADDR flag on %s: %s", conn_type_to_string(type),
        ...
        if (options->TransProxyType_parsed == TPT_TPROXY && type == CONN_TYPE_AP_TRANS_LISTENER) {
            int one = 1;
            if (setsockopt(s, SOL_IP, IP_TRANSPARENT, (void *)&one, (socklen_t)sizeof(one)) < 0) {
                const char *extra = "";
                int e = tor_socket_errno(s);
                if (e == EPERM)
                    extra = "TransTPROXY requires root privileges or similar"
                            " capabilities.";
                log_warn(LD_NET, "Error setting IP_TRANSPARENT flag: %s.%s", tor_socket_strerror(e),
                         extra);
            }
        }
        ...
        if (listensockaddr->sa_family == AF_INET6) {
             ...
             * IPv4 connections. */
            if (setsockopt(s, IPPROTO_IPV6, IPV6_V6ONLY, (void *)&one, (socklen_t)sizeof(one)) <
                0) {
                int e = tor_socket_errno(s);
                log_warn(LD_NET, "Error setting IPV6_V6ONLY flag: %s", tor_socket_strerror(e));
        ...
        if (usePort != 0) {
            ...
            if (getsockname(s, (struct sockaddr *)&ss, &ss_len) < 0) {
                log_warn(LD_NET, "getsockname() couldn't learn address for %s: %s",
                         conn_type_to_string(type), tor_socket_strerror(tor_socket_errno(s)));
                gotPort = 0;
            }
...
#    ifdef HAVE_PWD_H
        if (options->User) {
            ...
            struct stat st;
            if (pw == NULL) {
                log_warn(LD_NET, "Unable to chown() %s socket: user %s not found.", address,
                         options->User);
                goto err;
            } else if (fstat(s, &st) == 0 && st.st_uid == pw->pw_uid && st.st_gid == pw->pw_gid) {
                /* No change needed */
            } else if (chown(sandbox_intern_string(address), pw->pw_uid, pw->pw_gid) < 0) {
...
#    endif /* !defined(__APPLE__) */
        ...
        log_err(LD_BUG, "Got unexpected address family %d.", listensockaddr->sa_family);
        tor_assert(0);
    }
    ...
    log_fn(usePort == gotPort ? LOG_DEBUG : LOG_NOTICE, LD_NET, "%s listening on port %u.",
    ...
    if (conn->socket_family == AF_UNIX) {
        log_notice(LD_NET, "Opened %s on %s", conn_type_to_string(type), conn->address);
    } else {
 ...
 */
static connection_t *
connection_listener_new_for_port(const port_cfg_t *port, int *defer, int *addr_in_use)
{
    connection_t *conn;
    ...
    if (port->is_unix_addr) {
        listensockaddr =
            (struct sockaddr *)create_unix_sockaddr(port->unix_addr, &address, &listensocklen);
    } else {
        listensockaddr = tor_malloc(sizeof(struct sockaddr_storage));
        listensocklen = tor_addr_to_sockaddr(&port->addr, real_port, listensockaddr,
                                             sizeof(struct sockaddr_storage));
        address = tor_addr_to_str_dup(&port->addr);
    }

    if (listensockaddr) {
        conn = connection_listener_new(listensockaddr, listensocklen, port->type, address, port,
                                       addr_in_use);
...
check_sockaddr(const struct sockaddr *sa, int len, int level)
{
    ...
    if (sa->sa_family == AF_INET) {
        ...
        if (fast_mem_is_zero((void *)sin6->sin6_addr.s6_addr, 16) || sin6->sin6_port == 0) {
            log_fn(level, LD_NET, "Address for new connection has address/port equal to zero.");
            ok = 0;
        }
...
check_sockaddr_family_match(sa_family_t got, connection_t *listener)
{
    if (got != listener->socket_family) {
        log_info(LD_BUG,
                 ...
                 conn_type_to_string(listener->type), (int)listener->socket_family, (int)got);
        return -1;
    }
    return 0;
}
...
connection_handle_listener_read(connection_t *conn, int new_type)
{
    ...
    if (!SOCKET_OK(news)) { /* accept() error */
        ...
        log_warn(LD_NET, "accept() failed: %s. Closing listener.", tor_socket_strerror(e));
        connection_mark_for_close(conn);
        /* Tell the OOS handler about this too */
        connection_check_oos(get_n_open_sockets(), 0);
        return -1;
    }
    log_debug(LD_NET, "Connection accepted on socket %d (child of fd %d).", (int)news,
              (int)conn->s);
    ...
    if (conn->socket_family == AF_INET || conn->socket_family == AF_INET6 ||
        ...
        if (check_sockaddr(remote, remotelen, LOG_INFO) < 0) {
            log_info(LD_NET, "accept() returned a strange address; closing connection.");
            tor_close_socket(news);
            return 0;
        }
        ...
        if (new_type == CONN_TYPE_AP && conn->socket_family != AF_UNIX) {
            log_info(LD_NET, "New SOCKS connection opened from %s.", fmt_and_decorate_addr(&addr));
        }
        if (new_type == CONN_TYPE_AP && conn->socket_family == AF_UNIX) {
 ...
 */
int
connection_init_accepted_conn(connection_t *conn, const listener_connection_t *listener)
{
    int rv;
    ...
    case CONN_TYPE_OR:
        ...
        break;
    case CONN_TYPE_AP:
        memcpy(&TO_ENTRY_CONN(conn)->entry_cfg, &listener->entry_cfg, sizeof(entry_port_cfg_t));
...
connection_connect_sockaddr,
          (connection_t * conn, const struct sockaddr *sa, socklen_t sa_len,
           const struct sockaddr *bindaddr, socklen_t bindaddr_len, int *socket_error))
{
    tor_socket_t s;
    int inprogress = 0;
    const or_options_t *options = get_options();
    ...
    const int proto = (sa->sa_family == AF_INET6 || sa->sa_family == AF_INET) ? IPPROTO_TCP : 0;
    ...
    if (bindaddr && bind(s, bindaddr, bindaddr_len) < 0) {
        *socket_error = tor_socket_errno(s);
        log_warn(LD_NET, "Error binding network socket: %s", tor_socket_strerror(*socket_error));
        tor_close_socket(s);
        return -1;
    }
    ...
    if (connect(s, sa, sa_len) < 0) {
        ...
        if (!ERRNO_IS_CONN_EINPROGRESS(e)) {
            /* yuck. kill it. */
            *socket_error = e;
            log_info(LD_NET, "connect() to socket failed: %s", tor_socket_strerror(e));
            tor_close_socket(s);
            return -1;
        } else {
            inprogress = 1;
...
connection_connect_log_client_use_ip_version(const connection_t *conn)
{
    ...
    const int pref_ipv6 =
        (conn->type == CONN_TYPE_OR ? fascist_firewall_prefer_ipv6_orport(options)
    ...
    if ((!pref_ipv6 && tor_addr_family(&real_addr) == AF_INET6) ||
        ...
        log_info(LD_NET,
                 ...
                 options->ClientUseIPv4, fascist_firewall_use_ipv6(options), options->ClientUseIPv6,
                 options->UseBridges);
    }
}
 ...
 **/
const tor_addr_t *
conn_get_outbound_address(sa_family_t family, const or_options_t *options, unsigned int conn_type)
{
    ...
    if (conn_type == CONN_TYPE_EXIT) {
        if (!tor_addr_is_null(&options->OutboundBindAddresses[OUTBOUND_ADDR_EXIT][fam_index])) {
            ext_addr = &options->OutboundBindAddresses[OUTBOUND_ADDR_EXIT][fam_index];
        } else if (!tor_addr_is_null(
                       &options->OutboundBindAddresses[OUTBOUND_ADDR_EXIT_AND_OR][fam_index])) {
            ext_addr = &options->OutboundBindAddresses[OUTBOUND_ADDR_EXIT_AND_OR][fam_index];
        }
    } else { // All non-exit connections
        if (!tor_addr_is_null(&options->OutboundBindAddresses[OUTBOUND_ADDR_OR][fam_index])) {
 ...
 */
int
connection_connect(connection_t *conn, const char *address, const tor_addr_t *addr, uint16_t port,
                   int *socket_error)
{
    struct sockaddr_storage addrbuf;
    ...
    if (!tor_addr_is_loopback(addr)) {
        const tor_addr_t *ext_addr = NULL;
        ext_addr = conn_get_outbound_address(tor_addr_family(addr), get_options(), conn->type);
        if (ext_addr) {
            memset(&bind_addr_ss, 0, sizeof(bind_addr_ss));
            bind_addr_len = tor_addr_to_sockaddr(ext_addr, 0, (struct sockaddr *)&bind_addr_ss,
                                                 sizeof(bind_addr_ss));
            if (bind_addr_len == 0) {
                log_warn(LD_NET,
    ...
    log_debug(LD_NET, "Connecting to %s:%u.", escaped_safe_str_client(address), port);

    return connection_connect_sockaddr(conn, dest_addr, dest_addr_len, bind_addr, bind_addr_len,
                                       socket_error);
}
 ...
 */
int
connection_connect_unix(connection_t *conn, const char *socket_path, int *socket_error)
{
    ...
    log_debug(LD_NET, "Connecting to AF_UNIX socket at %s.", escaped_safe_str_client(socket_path));

    return connection_connect_sockaddr(conn, (struct sockaddr *)&dest_addr, sizeof(dest_addr), NULL,
                                       0, socket_error);
}
...
connection_socks4_proxy_connect(connection_t *conn)
{
    ...
    { /* If we are here because we are trying to connect to a
        ...
        if (conn_get_proxy_type(conn) == PROXY_PLUGGABLE) {
            socks_args_string = pt_get_socks_args_for_proxy_addrport(&conn->addr, conn->port);
...
connection_proxy_connect(connection_t *conn, int type)
{
    ...
    if (ret == 0) {
        log_debug(LD_NET, "set state %s", connection_proxy_state_to_string(conn->proxy_state));
    }
...
connection_read_https_proxy_response(connection_t *conn)
{
    ...
    case -1: /* overflow */
        log_warn(LD_PROTOCOL, "Your https proxy sent back an oversized response. Closing.");
        return -1;
    case 0:
    ...
    if (parse_http_response(headers, &status_code, &date_header, NULL, &reason) < 0) {
        log_warn(LD_NET, "Unparseable headers from proxy (connecting to '%s'). Closing.",
                 conn->address);
        tor_free(headers);
        return -1;
    }
    ...
    if (status_code == 200) {
        log_info(LD_NET, "HTTPS connect to '%s' successful! (200 %s) Starting TLS.", conn->address,
                 escaped(reason));
        tor_free(reason);
        return 1;
    }
...
static int
connection_fetch_from_buf_socks_client(connection_t *conn, int state, char **reason)
{
...
connection_read_proxy_handshake(connection_t *conn)
{
    ...
    log_debug(LD_NET, "enter state %s", connection_proxy_state_to_string(conn->proxy_state));
    ...
    case PROXY_SOCKS4_WANT_CONNECT_OK:
        ret = connection_fetch_from_buf_socks_client(conn, conn->proxy_state, &reason);
        if (ret == 1)
            conn->proxy_state = PROXY_CONNECTED;
        break;
    ...
    case PROXY_SOCKS5_WANT_AUTH_METHOD_RFC1929:
        ...
        if (ret == 1) {
            ...
            tor_assert(usize <= MAX_SOCKS5_AUTH_FIELD_SIZE && psize <= MAX_SOCKS5_AUTH_FIELD_SIZE);
    ...
    log_debug(LD_NET, "leaving state %s", connection_proxy_state_to_string(conn->proxy_state));

    if (ret < 0) {
        if (reason) {
            log_warn(LD_NET, "Proxy Client: unable to connect to %s:%d (%s)", conn->address,
                     conn->port, escaped(reason));
            tor_free(reason);
        } else {
            log_warn(LD_NET, "Proxy Client: unable to connect to %s:%d", conn->address, conn->port);
        }
    } else if (ret == 1) {
        log_info(LD_NET, "Proxy Client: connection to %s:%d successful", conn->address, conn->port);
    }
 ...
 **/
static int
retry_listener_ports(smartlist_t *old_conns, const smartlist_t *ports, smartlist_t *new_conns,
                     smartlist_t *replacements, int control_listeners_only)
{
    ...
    SMARTLIST_FOREACH_BEGIN (old_conns, connection_t *, conn) {
        ...
        SMARTLIST_FOREACH_BEGIN (launch, const port_cfg_t *, wanted) {
            ...
            if (wanted->is_unix_addr) {
                if (conn->socket_family == AF_UNIX && !strcmp(wanted->unix_addr, conn->address)) {
                    found_port = wanted;
                    break;
                }
            } else {
                 ...
                 */
                const int port_matches = (wanted->port == CFG_AUTO_PORT || port_matches_exact);
                ...
                const int may_need_rebind =
                    ...
                    port_matches_exact &&
                    bool_neq(tor_addr_is_null(&wanted->addr), tor_addr_is_null(&conn->addr));
...
retry_all_listeners(smartlist_t *new_conns, int close_all_noncontrol)
{
    ...
    const uint16_t old_or_port_ipv6 = router_get_advertised_or_port_by_af(options, AF_INET6);
    ...
    if (retry_listener_ports(listeners, get_configured_ports(), new_conns, replacements,
    ...
    SMARTLIST_FOREACH_BEGIN (replacements, listener_replacement_t *, r) {
        ...
        connection_t *new_conn = connection_listener_new_for_port(r->new_port, &skip, &addr_in_use);
        connection_t *old_conn = r->old_conn;

        if (skip) {
            log_debug(LD_NET, "Skipping creating new listener for %s:%d", old_conn->address,
                      old_conn->port);
            continue;
        }
        ...
        if (addr_in_use) {
            new_conn = connection_listener_new_for_port(r->new_port, &skip, &addr_in_use);
        }
        ...
        log_notice(LD_NET,
                   ...
                   "(replaced by %s:%d)",
                   conn_type_to_string(old_conn->type), old_conn->address, old_conn->port,
                   new_conn->address, new_conn->port);
    ...
    if (old_or_port != router_get_advertised_or_port(options) ||
        old_or_port_ipv6 != router_get_advertised_or_port_by_af(options, AF_INET6) ||
...
connection_mark_all_noncontrol_connections(void)
{
    SMARTLIST_FOREACH_BEGIN (get_connection_array(), connection_t *, conn) {
        ...
        case CONN_TYPE_CONTROL:
            break;
        case CONN_TYPE_AP:
            connection_mark_unattached_ap(TO_ENTRY_CONN(conn), END_STREAM_REASON_HIBERNATING);
            break;
        case CONN_TYPE_OR: {
...
{
    if (conn->type == CONN_TYPE_OR &&
        connection_or_client_used(TO_OR_CONN(conn)) + CLIENT_IDLE_TIME_FOR_PRIORITY < now)
        return 1;
...
static ssize_t
connection_bucket_get_share(int base, int priority, ssize_t global_bucket_val, ssize_t conn_bucket)
{
    ssize_t at_most;
...
connection_bucket_read_limit(connection_t *conn, time_t now)
{
    ...
    return connection_bucket_get_share(base, priority, global_bucket_val, conn_bucket);
}
...
connection_bucket_write_limit(connection_t *conn, time_t now)
{
    ...
    if (connection_speaks_cells(conn)) {
        ...
        or_connection_t *or_conn = TO_OR_CONN(conn);
        if (conn->state == OR_CONN_STATE_OPEN)
            conn_bucket = MIN(conn_bucket, token_bucket_rw_get_write(&or_conn->bucket));
        base = get_cell_network_size(or_conn->wide_circ_ids);
    }
...
{
    size_t smaller_bucket = MIN(token_bucket_rw_get_write(&global_bucket),
...
static void
record_num_bytes_transferred_impl(connection_t *conn, time_t now, size_t num_read,
                                  size_t num_written)
{
    ...
    if (conn->type == CONN_TYPE_OR)
        rep_hist_note_or_conn_bytes(conn->global_identifier, num_read, num_written, now);
    ...
    if (accounting_is_enabled(get_options())) {
        if (now > last_recorded_accounting_at && last_recorded_accounting_at) {
            accounting_add_bytes(num_read, num_written, (int)(now - last_recorded_accounting_at));
        } else {
...
static void
connection_buckets_decrement(connection_t *conn, time_t now, size_t num_read, size_t num_written)
{
...
connection_consider_empty_read_buckets(connection_t *conn)
{
    ...
    } else if (connection_speaks_cells(conn) && conn->state == OR_CONN_STATE_OPEN &&
...
connection_bucket_init(void)
{
    ...
    if (options->RelayBandwidthRate) {
        token_bucket_rw_init(&global_relayed_bucket, (int32_t)options->RelayBandwidthRate,
                             (int32_t)options->RelayBandwidthBurst, now_ts);
    } else {
        token_bucket_rw_init(&global_relayed_bucket, (int32_t)options->BandwidthRate,
                             (int32_t)options->BandwidthBurst, now_ts);
    }
...
connection_bucket_adjust(const or_options_t *options)
{
    ...
    if (options->RelayBandwidthRate) {
        token_bucket_rw_adjust(&global_relayed_bucket, (int32_t)options->RelayBandwidthRate,
                               (int32_t)options->RelayBandwidthBurst);
    } else {
        token_bucket_rw_adjust(&global_relayed_bucket, (int32_t)options->BandwidthRate,
                               (int32_t)options->BandwidthBurst);
    }
}
...
reenable_blocked_connection_init(const or_options_t *options)
{
    if (!reenable_blocked_connections_ev) {
        reenable_blocked_connections_ev = mainloop_event_new(reenable_blocked_connections_cb, NULL);
        reenable_blocked_connections_is_scheduled = 0;
    }
...
reenable_blocked_connection_schedule(void)
{
    ...
    mainloop_event_schedule(reenable_blocked_connections_ev, &reenable_blocked_connections_delay);
...
loop_again:
    ...
    if (CONN_IS_EDGE(conn) && try_to_read != max_to_read) {
        ...
        if (!conn->marked_for_close && connection_is_reading(conn) && !conn->inbuf_reached_eof &&
            max_to_read > 0)
    ...
    if (!conn->marked_for_close && conn->inbuf_reached_eof && connection_reached_eof(conn) < 0) {
        return -1;
    }
    return 0;
}
 ...
 */
static int
connection_buf_read_from_socket(connection_t *conn, ssize_t *max_to_read, int *socket_error)
{
    int result;
...
again:
    ...
    if (connection_speaks_cells(conn) && conn->state > OR_CONN_STATE_PROXY_HANDSHAKING) {
        ...
        log_debug(LD_GENERAL, "After TLS read of %d: %ld read, %ld written", result, (long)n_read,
                  (long)n_written);
    } else if (conn->linked) {
        if (conn->linked_conn) {
        ...
        if (!conn->linked_conn ||
            (conn->linked_conn->marked_for_close && buf_datalen(conn->linked_conn->outbuf) == 0))
        ...
        int reached_eof = 0;
        CONN_LOG_PROTECT(conn, result = buf_read_from_socket(conn->inbuf, conn->s, at_most,
 ...
 * appropriate. */
int
connection_fetch_from_buf_http(connection_t *conn, char **headers_out, size_t max_headerlen,
                               char **body_out, size_t *body_used, size_t max_bodylen,
                               int force_complete)
{
    return fetch_from_buf_http(conn->inbuf, headers_out, max_headerlen, body_out, body_used,
                               max_bodylen, force_complete);
}
...
update_send_buffer_size(tor_socket_t sock)
{
    ...
    if (!WSAIoctl(sock, SIO_IDEAL_SEND_BACKLOG_QUERY, NULL, 0, &isb, sizeof(isb), &bytesReturned,
                  NULL, NULL)) {
...
connection_handle_write_impl(connection_t *conn, int force)
{
        ...
        if (e) {
            ...
            if (!ERRNO_IS_CONN_EINPROGRESS(e)) {
                ...
                if (CONN_IS_EDGE(conn))
                    connection_edge_end_errno(TO_EDGE_CONN(conn));
                if (conn->type == CONN_TYPE_OR)
                    connection_or_notify_error(TO_OR_CONN(conn), errno_to_orconn_end_reason(e),
                                               tor_socket_strerror(e));
    ...
    max_to_write =
        force ? (ssize_t)conn->outbuf_flushlen : connection_bucket_write_limit(conn, now);
    ...
    if (connection_speaks_cells(conn) && conn->state > OR_CONN_STATE_PROXY_HANDSHAKING) {
        ...
        result = buf_flush_to_tls(conn->outbuf, or_conn->tls, max_to_write, &conn->outbuf_flushlen);
        ...
        case TOR_TLS_CLOSE:
            log_info(LD_NET, result != TOR_TLS_CLOSE ? "tls error. breaking."
                                                     : "TLS connection closed on flush");
            /* Don't flush; connection is dead. */
            connection_or_notify_error(or_conn, END_OR_CONN_REASON_MISC,
                                       result != TOR_TLS_CLOSE ? "TLS error in during flush"
        ...
        tor_tls_get_n_raw_bytes(or_conn->tls, &n_read, &n_written);
        log_debug(LD_GENERAL, "After TLS write of %d: %ld read, %ld written", result, (long)n_read,
                  (long)n_written);
        ...
        CONN_LOG_PROTECT(conn, result = buf_flush_to_socket(conn->outbuf, conn->s, max_to_write,
    ...
    if (!connection_wants_to_flush(conn) && !dont_stop_writing) { /* it's done flushing */
...
connection_write_to_buf_failed(connection_t *conn)
{
    if (CONN_IS_EDGE(conn)) {
        ...
        log_warn(LD_NET, "write_to_buf failed. Closing circuit (fd %d).", (int)conn->s);
        ...
        log_warn(LD_NET, "write_to_buf failed. Closing connection (fd %d).", (int)conn->s);
        connection_mark_for_close(conn);
    }
}
          ...
          (const char *string, size_t len, connection_t *conn, int zlib))
{
    ...
    if (zlib) {
        ...
        int done = zlib < 0;
        CONN_LOG_PROTECT(
            conn, r = buf_add_compress(conn->outbuf, dir_conn->compress_state, string, len, done));
 ...
 */
void
connection_dir_buf_add(const char *string, size_t len, dir_connection_t *dir_conn, int done)
{
...
void
connection_buf_add_compress(const char *string, size_t len, dir_connection_t *conn, int done)
{
...
{
    CONN_GET_ALL_TEMPLATE(conn, (conn->type == type && conn->purpose == purpose));
}
...
{
    CONN_GET_TEMPLATE(conn, (conn->type == type && tor_addr_eq(&conn->addr, addr) &&
...
{
    DIR_CONN_LIST_TEMPLATE(conn, conn->purpose == purpose, dirconn,
 ...
 * freed if their connections close. */
smartlist_t *
connection_dir_list_by_purpose_resource_and_state(int purpose, const char *resource, int state)
{
    DIR_CONN_LIST_TEMPLATE(conn, conn->purpose == purpose && conn->state == state, dirconn,
                           0 == strcmp_opt(resource, dirconn->requested_resource));
}
...
{
    CONN_GET_TEMPLATE(conn, conn != TO_CONN(this_conn) && conn->type == CONN_TYPE_OR);
}
...
any_other_active_or_conns(const or_connection_t *this_conn)
{
    ...
    if (conn != NULL) {
        log_debug(LD_DIR, "%s: Found an OR connection: %s", __func__, conn->address);
        return 1;
    }
...
{
    if (conn->type == CONN_TYPE_OR_LISTENER || conn->type == CONN_TYPE_EXT_OR_LISTENER ||
        conn->type == CONN_TYPE_AP_LISTENER || conn->type == CONN_TYPE_AP_TRANS_LISTENER ||
        conn->type == CONN_TYPE_AP_DNS_LISTENER || conn->type == CONN_TYPE_AP_NATD_LISTENER ||
        conn->type == CONN_TYPE_AP_HTTP_CONNECT_LISTENER || conn->type == CONN_TYPE_DIR_LISTENER ||
...
alloc_http_authenticator(const char *authenticator)
{
    ...
    const size_t base64_authenticator_length = base64_encode_size(authenticator_length, 0) + 1;
    char *base64_authenticator = tor_malloc(base64_authenticator_length);
    if (base64_encode(base64_authenticator, base64_authenticator_length, authenticator,
                      authenticator_length, 0) < 0) {
...
connection_flushed_some(connection_t *conn)
{
    ...
    if (conn->type == CONN_TYPE_DIR && conn->state == DIR_CONN_STATE_SERVER_WRITING) {
    ...
    if (smartlist_len(conns) > 0) {
        ...
        for (i = CONN_TYPE_MIN_; i <= CONN_TYPE_MAX_; ++i) {
            ...
            if (conn_counts_by_type[i] > 0) {
                log_info(LD_NET, "%s: %d conns", conn_type_to_string(i), conn_counts_by_type[i]);
            }
        }
...
kill_conn_list_for_oos, (smartlist_t * conns))
{
    ...
    log_notice(LD_NET, "OOS handler marked %d connections", smartlist_len(conns));
}
...
connection_check_oos(int n_socks, int failed)
{
    ...
    if (n_socks >= get_options()->ConnLimit_high_thresh &&
        get_options()->ConnLimit_high_thresh != 0 && get_options()->ConnLimit_ != 0) {
    ...
    if (target_n_socks > 0) {
        ...
        if (moribund_socks < n_socks - target_n_socks) {
            ...
            if (conns) {
                ...
                log_notice(LD_NET, "OOS handler killed %d conns", smartlist_len(conns));
...
connection_dump_buffer_mem_stats(int severity)
{
    ...
    for (i = CONN_TYPE_MIN_; i <= CONN_TYPE_MAX_; ++i) {
        ...
        tor_log(severity, LD_GENERAL,
                ...
                n_conns_by_type[i], conn_type_to_string(i), (used_by_type[i]), (alloc_by_type[i]));
    }
}
...
assert_connection_ok(connection_t *conn, time_t now)
{
    ...
    if (conn->outbuf_flushlen > 0) {
         ...
         * */
        tor_assert((conn->type == CONN_TYPE_EXIT && conn->state == EXIT_CONN_STATE_RESOLVING) ||
 ...
 */
int
get_proxy_addrport(tor_addr_t *addr, uint16_t *port, int *proxy_type, int *is_pt_out,
...
log_failed_proxy_connection(connection_t *conn)
{
    ...
    if (get_proxy_addrport(&proxy_addr, &proxy_port, &proxy_type, &is_pt, conn) != 0)
    ...
    log_warn(LD_NET,
             ...
             proxy_type_to_string(proxy_type), fmt_addrport(&proxy_addr, proxy_port));
}
...
connection_free_all(void)
{
    ...
    SMARTLIST_FOREACH(conns, connection_t *, conn,
                      if (conn->type == CONN_TYPE_CONTROL) TO_CONTROL_CONN(conn)->event_mask = 0);
    ...
    SMARTLIST_FOREACH(conns, connection_t *, conn, connection_free_minimal(conn));
...
MOCK_IMPL(void,
clock_skew_warning,
          (const connection_t *conn, long apparent_skew, int trusted, log_domain_mask_t domain,
           const char *received, const char *source))
{
    ...
    if (trusted) {
        control_event_general_status(LOG_WARN, "CLOCK_SKEW SKEW=%ld SOURCE=%s", apparent_skew,
                                     ext_source);
        tor_asprintf(&warn, "Clock skew %ld in %s from %s", apparent_skew, received, source);
