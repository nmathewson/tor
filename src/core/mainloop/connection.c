...
dir_connection_new(int socket_family)
{
    ...
    connection_init(time(NULL), TO_CONN(dir_conn), CONN_TYPE_DIR,
                    socket_family);
    return dir_conn;
}
...
entry_connection_new(int type, int socket_family)
{
    ...
    connection_init(time(NULL), ENTRY_TO_CONN(entry_conn), type,
                    socket_family);
...
connection_free_minimal(connection_t *conn)
{
    ...
    if (connection_speaks_cells(conn)) {
        ...
        if (or_conn->tls) {
            if (!SOCKET_OK(conn->s)) {
                /* The socket has been closed by somebody else; we must tell
                 * the TLS object not to close it. */
                tor_tls_release_socket(or_conn->tls);
            } else {
                /* The tor_tls_free() call below will close the socket; we must
                 * tell the code below not to close it a second time. */
    ...
    if (conn->type == CONN_TYPE_CONTROL) {
        ...
        if (control_conn->ephemeral_onion_services) {
            SMARTLIST_FOREACH(control_conn->ephemeral_onion_services, char *,
                              cp, {
          ...
          (connection_t * conn, int line, const char *file))
{
    ...
    if (conn->marked_for_close) {
        log_warn(LD_BUG,
                 ...
                 " (first at %s:%d)",
                 file, line, conn->marked_for_close_file,
    ...
    if (conn->type == CONN_TYPE_OR) {
         ...
         * this so we can find things that call this wrongly when the asserts
         * hit.
         */
        log_debug(
            LD_CHANNEL,
                     ...
                     socklen_t *len_out)
{
    ...
    if (strlcpy(sockaddr->sun_path, listenaddress,
...
#    ifdef SO_EXCLUSIVEADDRUSE
     ...
     * didn't set REUSEADDR_. Use EXCLUSIVEADDRUSE to prevent this
                        ...
                        const port_cfg_t *port_cfg, int *addr_in_use)
{
    ...
    if (listensockaddr->sa_family == AF_INET ||
        ...
        if (listensockaddr->sa_family == AF_INET6) {
            int one = 1;
            /* We need to set IPV6_V6ONLY so that this socket can't get used
        ...
        if (is_stream) {
            if (tor_listen(s) < 0) {
                log_warn(LD_NET, "Could not listen on %s:%u: %s", address,
                         usePort, tor_socket_strerror(tor_socket_errno(s)));
                goto err;
            }
        }
        ...
        if (check_location_for_unix_socket(
                options, address,
        ...
        if (bind(s, listensockaddr, (socklen_t)sizeof(struct sockaddr_un)) ==
            -1) {
...
#    endif /* !defined(__APPLE__) */
    ...
    memcpy(&lis_conn->entry_cfg, &port_cfg->entry_cfg,
           sizeof(entry_port_cfg_t));

    if (port_cfg->entry_cfg.isolation_flags) {
        lis_conn->entry_cfg.isolation_flags =
            port_cfg->entry_cfg.isolation_flags;
        if (port_cfg->entry_cfg.session_group >= 0) {
            lis_conn->entry_cfg.session_group =
                port_cfg->entry_cfg.session_group;
        } else {
            /* This can wrap after around INT_MAX listeners are opened.  But I
             * don't believe that matters, since you would need to open a
             * ridiculous number of listeners while keeping the early ones open
             * before you ever hit this.  An OR with a dozen ports open, for
             * example, would have to close and re-open its listeners every
             * second for 4 years nonstop.
             */
                                 ...
                                 int *addr_in_use)
{
    ...
    if (listensockaddr) {
        conn = connection_listener_new(listensockaddr, listensocklen,
                                       port->type, address, port, addr_in_use);
...
check_sockaddr(const struct sockaddr *sa, int len, int level)
{
    ...
    if (sa->sa_family == AF_INET) {
        ...
        if (len != sizeof(struct sockaddr_in)) {
            log_fn(level, LD_NET,
                   "Length of address not as expected: %d vs %d", len,
...
check_sockaddr_family_match(sa_family_t got, connection_t *listener)
{
    if (got != listener->socket_family) {
        log_info(LD_BUG,
                 ...
                 (int)listener->socket_family, (int)got);
        return -1;
    }
    return 0;
}
...
connection_handle_listener_read(connection_t *conn, int new_type)
{
    ...
    if (options->ConstrainedSockets)
        set_constrained_socket_buffers(news,
                                       (int)options->ConstrainedSockSize);
    ...
    if (conn->socket_family == AF_INET || conn->socket_family == AF_INET6 ||
        ...
        if (new_type == CONN_TYPE_OR) {
            /* Assess with the connection DoS mitigation subsystem if this
             * address can open a new connection. */
            if (dos_conn_addr_get_defense_type(&addr) ==
                              ...
                              const listener_connection_t *listener)
{
    ...
    case CONN_TYPE_AP:
        ...
        TO_ENTRY_CONN(conn)->socks_request->listener_type =
            listener->base_.type;
...
connection_connect_log_client_use_ip_version(const connection_t *conn)
{
    ...
    /* OR conns keep the original address in real_addr, as addr gets
     * overwritten with the descriptor address */
    if (conn->type == CONN_TYPE_OR) {
    ...
    if ((must_ipv4 && tor_addr_family(&real_addr) == AF_INET6) ||
        ...
        log_info(
            LD_BUG, "Outgoing %s connection to %s violated ClientUseIPv%s 0.",
                          ...
                          unsigned int conn_type)
{
    ...
    if (conn_type == CONN_TYPE_EXIT) {
        if (!tor_addr_is_null(
                &options
                     ->OutboundBindAddresses[OUTBOUND_ADDR_EXIT][fam_index])) {
            ext_addr =
                &options->OutboundBindAddresses[OUTBOUND_ADDR_EXIT][fam_index];
        } else if (!tor_addr_is_null(
                       &options
                            ->OutboundBindAddresses[OUTBOUND_ADDR_EXIT_AND_OR]
                                                   [fam_index])) {
            ext_addr =
        ...
        if (!tor_addr_is_null(&options->OutboundBindAddresses[OUTBOUND_ADDR_OR]
                                                             [fam_index])) {
            ext_addr =
                   ...
                   const tor_addr_t *addr, uint16_t port, int *socket_error)
{
    ...
    if (!tor_addr_is_loopback(addr)) {
        ...
        if (ext_addr) {
            ...
            bind_addr_len = tor_addr_to_sockaddr(
                ext_addr, 0, (struct sockaddr *)&bind_addr_ss,
                sizeof(bind_addr_ss));
            if (bind_addr_len == 0) {
                log_warn(
                    LD_NET,
    ...
    dest_addr_len =
        tor_addr_to_sockaddr(addr, port, dest_addr, sizeof(addrbuf));
...
conn_get_proxy_type(const connection_t *conn)
{
    ...
    if (options->ClientTransportPlugin) {
        ...
        int r;
        r = get_transport_by_bridge_addrport(&conn->addr, conn->port,
                                             &transport);
        if (r == 0 && transport)
...
connection_read_https_proxy_response(connection_t *conn)
{
    ...
    if (parse_http_response(headers, &status_code, &date_header, NULL,
                            &reason) < 0) {
        log_warn(
            LD_NET,
...
connection_read_proxy_handshake(connection_t *conn)
{
    ...
    case PROXY_SOCKS5_WANT_AUTH_METHOD_RFC1929:
        ...
        if (ret == 1) {
            ...
            if (conn_get_proxy_type(conn) == PROXY_PLUGGABLE) {
                socks_args_string = pt_get_socks_args_for_proxy_addrport(
                    &conn->addr, conn->port);
                if (!socks_args_string) {
                    log_warn(LD_NET,
                ...
                log_debug(LD_NET, "PT SOCKS5 arguments: %s",
                          socks_args_string);
                tor_assert(strlen(socks_args_string) > 0);
                tor_assert(strlen(socks_args_string) <=
                ...
                if (strlen(socks_args_string) > MAX_SOCKS5_AUTH_FIELD_SIZE) {
                    ...
                    psize =
                        strlen(socks_args_string) - MAX_SOCKS5_AUTH_FIELD_SIZE;
                } else {
                    user = socks_args_string;
                     ...
                     int control_listeners_only)
{
    ...
    /* Iterate through old_conns, comparing it to launch: remove from both
     * lists each pair of elements that corresponds to the same port. */
    SMARTLIST_FOREACH_BEGIN (old_conns, connection_t *, conn) {
        ...
        SMARTLIST_FOREACH_BEGIN (launch, const port_cfg_t *, wanted) {
            ...
            if (wanted->is_unix_addr) {
                ...
                const int may_need_rebind =
                    tor_addr_family(&wanted->addr) ==
                        tor_addr_family(&conn->addr) &&
                    port_matches_exact &&
        ...
        if (found_port) {
            ...
            //    conn_type_to_string(found_port->type), conn->address,
            //    conn->port);
    ...
    SMARTLIST_FOREACH_BEGIN (launch, const port_cfg_t *, port) {
        int skip = 0;
        connection_t *conn =
            connection_listener_new_for_port(port, &skip, NULL);
...
retry_all_listeners(smartlist_t *new_conns, int close_all_noncontrol)
{
    ...
    SMARTLIST_FOREACH_BEGIN (replacements, listener_replacement_t *, r) {
        ...
        if (addr_in_use) {
            new_conn = connection_listener_new_for_port(r->new_port, &skip,
                                                        &addr_in_use);
        }
                                  ...
                                  size_t num_read, size_t num_written)
{
     ...
     * accounting:
     *  - counting linked connections would double-count BEGINDIR bytes,
     * because they are sent as Dir bytes on the linked connection, and OR
     * bytes on the OR connection;
...
loop_again:
    ...
    if (connection_buf_read_from_socket(conn, &max_to_read, &socket_error) <
        0) {
        ...
        connection_close_immediate(
            conn); /* Don't flush; connection is dead. */
        /*
    ...
    if (connection_speaks_cells(conn) &&
        ...
        case TOR_TLS_WANTREAD:
            if (conn->in_connection_handle_write) {
                 ...
                 * we're waiting for a TLS renegotiation, the renegotiation
                 * started, and SSL_read returned WANTWRITE.  But now SSL_read
                 * is saying WANTREAD again.  Stop waiting for write events
                 * now, or else we'll busy-loop until data arrives for us to
                 * read.
        ...
        // log_notice(LD_GENERAL, "Moved %d bytes on an internal link!",
        // result);
        /* If the other side has disappeared, or if it's been marked for close
         * and
        ...
        int reached_eof = 0;
        CONN_LOG_PROTECT(
            conn, result = buf_read_from_socket(conn->inbuf, conn->s, at_most,
...
update_send_buffer_size(tor_socket_t sock)
{
    ...
    if (!WSAIoctl(sock, SIO_IDEAL_SEND_BACKLOG_QUERY, NULL, 0, &isb,
        ...
        setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (const char *)&isb,
                   sizeof(isb));
    }
#else /* !defined(_WIN32) */
    (void)sock;
...
connection_handle_write_impl(connection_t *conn, int force)
{
    ...
    if (connection_speaks_cells(conn) &&
         ...
         * single set of TLS operations. But it looks kinda ugly. If we
         * refactor the *_buf_tls functions, we should make them return ssize_t
         * or size_t or something. */
        result = (int)(initial_size - buf_datalen(conn->outbuf));
    } else {
        CONN_LOG_PROTECT(conn, result = buf_flush_to_socket(
                                   conn->outbuf, conn->s, max_to_write,
                                   &conn->outbuf_flushlen));
        if (result < 0) {
            if (CONN_IS_EDGE(conn))
                connection_edge_end_errno(TO_EDGE_CONN(conn));
            if (conn->type == CONN_TYPE_AP) {
                /* writing failed; we couldn't send a SOCKS reply if we wanted
                 * to */
                TO_ENTRY_CONN(conn)->socks_request->has_finished = 1;
            }
                                       ...
                                       const char *rendquery)
{
    ...
    CONN_GET_TEMPLATE(
        conn,
        (conn->type == type && (!state || state == conn->state)) &&
...
{
    CONN_GET_TEMPLATE(conn, conn != TO_CONN(this_conn) &&
                                conn->type == CONN_TYPE_OR);
}
...
client_check_address_changed(tor_socket_t sock)
{
    ...
    if (tor_addr_eq(&iface_addr, *last_interface_ip_ptr)) {
        ...
        smartlist_add(outgoing_addrs,
                      tor_memdup(&out_addr, sizeof(tor_addr_t)));
    } else {
        ...
        SMARTLIST_FOREACH(outgoing_addrs, tor_addr_t *, a_ptr,
                          tor_free(a_ptr));
...
connection_finished_connecting(connection_t *conn)
{
    ...
    if (!server_mode(get_options())) {
        /* See whether getsockname() says our address changed.  We need to do
         * this now that the connection has finished, because getsockname() on
         * Windows won't work until then. */
...
{
    if (conn != NULL &&
        (conn->conn_array_index < 0 || conn->marked_for_close)) {
        return 1;
    } else {
        return 0;
    }
...
connection_check_oos(int n_socks, int failed)
{
    ...
    if (n_socks >= get_options()->ConnLimit_high_thresh &&
        ...
        /*
         * If we're not at the limit but we hit a socket exhaustion error, try
         * to drop some (but not as aggressively as ConnLimit_low_threshold,
         * which is 3/4 of ConnLimit_)
         */
    ...
    if (target_n_socks > 0) {
         ...
         * to get rid of here but don't immediately close get counted as
         * moribund on subsequent invocations so we don't try to kill too many
         * things if connection_check_oos() gets called multiple times.
         */
...
connection_dump_buffer_mem_stats(int severity)
{
    ...
    for (i = CONN_TYPE_MIN_; i <= CONN_TYPE_MAX_; ++i) {
        if (!n_conns_by_type[i])
            continue;
        tor_log(severity, LD_GENERAL,
                "  For %d %s connections: %" PRIu64 " used/%" PRIu64
                " allocated",
...
assert_connection_ok(connection_t *conn, time_t now)
{
    ...
    /* XXXX check: read_blocked_on_bw, write_blocked_on_bw, s,
     * conn_array_index, marked_for_close. */
                   ...
                   int *is_pt_out, const connection_t *conn)
{
    ...
    *is_pt_out = 0;
    /* Client Transport Plugins can use another proxy, but that should be
     * hidden from the rest of tor (as the plugin is responsible for dealing
     * with the proxy), check it first, then check the rest of the proxy types
     * to allow the config to have unused ClientTransportPlugin entries.
     */
    if (options->ClientTransportPlugin) {
        const transport_t *transport = NULL;
        int r;
           ...
           log_domain_mask_t domain, const char *received, const char *source))
{
    ...
    if (conn)
        tor_asprintf(&ext_source, "%s:%s:%d", source, conn->address,
                     conn->port);
    else
