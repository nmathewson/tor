...
cpu_init(void)
{
    ...
    if (!threadpool) {
        /*
          ...
          half are strict (when it comes to running lower-priority tasks). So
          we always make sure we have at least two threads, so that there will
          be at least one thread of each kind.
        */
...
estimated_usec_for_onionskins(uint32_t n_requests, uint16_t onionskin_type)
{
    ...
    if (PREDICT_UNLIKELY(onionskins_n_processed[onionskin_type] < 100)) {
         ...
         * measured in onionskin_usec_internal, and they won't take anything
         * near 1 sec each, and we won't have anything like 1 million queued
...
cpuworker_onion_handshake_replyfn(void *work_)
{
    ...
    if (rpl.timed && rpl.success &&
        ...
        if (usec_roundtrip >= 0 &&
            ...
            if (onionskins_n_processed[rpl.handshake_type] >= 500000) {
                /* Scale down every 500000 handshakes.  On a busy server,
    ...
    if (circ->base_.magic == DEAD_CIRCUIT_MAGIC) {
         ...
         * pending. Instead, it got left for us to free so that we wouldn't
         * freak out when the job->circ field wound up pointing to nothing. */
        log_debug(LD_OR,
