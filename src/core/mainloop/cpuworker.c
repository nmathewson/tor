...
cpu_init(void)
{
    ...
    if (!threadpool) {
        ...
        */
        ...
        threadpool =
            threadpool_new(n_threads, replyqueue, worker_state_new, worker_state_free_void, NULL);
...
cpuworkers_rotate_keyinfo(void)
{
    ...
    if (threadpool_queue_update(threadpool, worker_state_new, update_state_threadfn,
...
static int
get_overhead_for_onionskins(uint32_t *usec_out, double *frac_out, uint16_t onionskin_type)
{
    uint64_t overhead;
    ...
    *frac_out = 0.0;
    ...
    overhead = onionskins_usec_roundtrip[onionskin_type] - onionskins_usec_internal[onionskin_type];
 ...
 * log it. */
void
cpuworker_log_onionskin_overhead(int severity, int onionskin_type, const char *onionskin_type_name)
{
    uint32_t overhead;
    double relative_overhead;
    int r;

    r = get_overhead_for_onionskins(&overhead, &relative_overhead, onionskin_type);
    if (!overhead || r < 0)
        return;
...
cpuworker_onion_handshake_replyfn(void *work_)
{
    ...
    if (rpl.timed && rpl.success && rpl.handshake_type <= MAX_ONION_HANDSHAKE_TYPE) {
        ...
        if (usec_roundtrip >= 0 && usec_roundtrip < MAX_BELIEVABLE_ONIONSKIN_DELAY) {
    ...
    log_debug(LD_OR, "Unpacking cpuworker reply %p, circ=%p, success=%d", job, circ, rpl.success);
    ...
    if (onionskin_answer(circ, &rpl.created_cell, (const char *)rpl.keys, sizeof(rpl.keys),
...
cpuworker_onion_handshake_threadfn(void *state_, void *work_)
{
    ...
    n = onion_skin_server_handshake(cc->handshake_type, cc->onionskin, cc->handshake_len,
                                    onion_keys, cell_out->reply, rpl.keys, CPATH_KEY_MATERIAL_LEN,
...
cpuworker_queue_work,
          (workqueue_priority_t priority, workqueue_reply_t (*fn)(void *, void *),
           void (*reply_fn)(void *), void *arg))
{
    tor_assert(threadpool);

    return threadpool_queue_work_priority(threadpool, priority, fn, reply_fn, arg);
}
...
assign_onionskin_to_cpuworker(or_circuit_t *circ, create_cell_t *onionskin)
{
    ...
    ++total_pending_tasks;
    queue_entry =
        threadpool_queue_work_priority(threadpool, WQ_PRI_HIGH, cpuworker_onion_handshake_threadfn,
    ...
    log_debug(LD_OR, "Queued task %p (qe=%p, circ=%p)", job, queue_entry, job->circ);
