...
{
    tor_assert(conn);
    tor_assert(SOCKET_OK(conn->s) || conn->linked ||
    ...
    if (SOCKET_OK(conn->s) || conn->linked) {
        conn->read_event = tor_event_new(tor_libevent_get_base(), conn->s, EV_READ | EV_PERSIST,
                                         conn_read_callback, conn);
        conn->write_event = tor_event_new(tor_libevent_get_base(), conn->s, EV_WRITE | EV_PERSIST,
...
connection_remove(connection_t *conn)
{
    ...
    log_debug(LD_NET, "removing socket %d (type %s), n_conns now %d", (int)conn->s,
              conn_type_to_string(conn->type), smartlist_len(connection_array));
...
connection_check_event(connection_t *conn, struct event *ev)
{
    ...
    if (bad) {
        log_warn(LD_BUG,
                 ...
                 conn_state_to_string(conn->type, conn->state), (int)conn->s, (int)conn->linked,
                 (conn->type == CONN_TYPE_AP && TO_EDGE_CONN(conn)->is_dns_request),
...
connection_start_writing, (connection_t * conn))
{
    ...
    if (conn->linked) {
        ...
        if (conn->linked_conn && connection_should_read_from_linked_conn(conn->linked_conn))
...
connection_should_read_from_linked_conn(connection_t *conn)
{
    if (conn->linked && conn->reading_from_linked_conn) {
        if (!conn->linked_conn ||
            (conn->linked_conn->writing_to_linked_conn && buf_datalen(conn->linked_conn->outbuf)))
            return 1;
    }
    return 0;
}
...
static void
tor_shutdown_event_loop_for_restart_cb(evutil_socket_t fd, short event, void *arg)
{
    (void)fd;
    (void)event;
    (void)arg;
...
tor_shutdown_event_loop_and_exit(int exitcode)
{
    ...
    shutdown_did_not_work_event =
        tor_evtimer_new(tor_libevent_get_base(), shutdown_did_not_work_callback, NULL);
...
conn_write_callback(evutil_socket_t fd, short events, void *_conn)
{
    ...
    LOG_FN_CONN(conn, (LOG_DEBUG, LD_NET, "socket %d wants to write.", (int)conn->s));
    ...
    if (connection_handle_write(conn, 0) < 0) {
        if (!conn->marked_for_close) {
            ...
            log_fn(LOG_WARN, LD_BUG, "unhandled error on write for %s connection (fd %d); removing",
                   conn_type_to_string(conn->type), (int)conn->s);
            tor_fragile_assert();
            if (CONN_IS_EDGE(conn)) {
    ...
    if ((SOCKET_OK(conn->s) || conn->linked_conn) && connection_wants_to_flush(conn)) {
        ...
        if (!conn->hold_open_until_flushed)
            log_info(LD_NET,
                     ...
                     conn_type_to_string(conn->type), conn->state, (int)conn->outbuf_flushlen,
                     conn->marked_for_close_file, conn->marked_for_close);
        if (conn->linked_conn) {
            retval =
                buf_move_to_buf(conn->linked_conn->inbuf, conn->outbuf, &conn->outbuf_flushlen);
            if (retval >= 0) {
            ...
            log_debug(LD_GENERAL,
                      ...
                      retval, (int)connection_get_outbuf_len(conn), (int)conn->outbuf_flushlen,
            ...
            retval = buf_flush_to_socket(conn->outbuf, conn->s, sz, &conn->outbuf_flushlen);
        }
        if (retval >= 0 && /* Technically, we could survive things like
                              TLS_WANT_WRITE here. But don't bother for now. */
            conn->hold_open_until_flushed && connection_wants_to_flush(conn)) {
            if (retval > 0) {
                LOG_FN_CONN(conn, (LOG_INFO, LD_NET, "Holding conn (fd %d) open for more flushing.",
                                   (int)conn->s));
        ...
        if (connection_wants_to_flush(conn)) {
            log_fn(LOG_INFO, LD_NET,
                   ...
                   (int)connection_get_outbuf_len(conn), escaped_safe_str_client(conn->address),
...
directory_all_unreachable_cb(mainloop_event_t *event, void *arg)
{
    ...
    while ((conn = connection_get_by_type_state(CONN_TYPE_AP, AP_CONN_STATE_CIRCUIT_WAIT))) {
        ...
        connection_mark_unattached_ap(entry_conn, END_STREAM_REASON_NET_UNREACHABLE);
    }
...
directory_all_unreachable(time_t now)
{
    ...
    if (!directory_all_unreachable_cb_event) {
        directory_all_unreachable_cb_event = mainloop_event_new(directory_all_unreachable_cb, NULL);
...
directory_info_has_arrived(time_t now, int from_cache, int suppress_logs)
{
    ...
    if (!router_have_minimum_dir_info()) {
        int quiet =
            suppress_logs || from_cache || dirclient_too_idle_to_fetch_descriptors(options, now);
...
run_connection_housekeeping(int i, time_t now)
{
    ...
    int past_keepalive = now >= conn->timestamp_last_write_allowed + options->KeepalivePeriod;
    ...
    if (conn->type == CONN_TYPE_DIR &&
        ((DIR_CONN_IS_SERVER(conn) &&
          conn->timestamp_last_write_allowed + options->TestingDirConnectionMaxStall < now) ||
         (!DIR_CONN_IS_SERVER(conn) &&
          conn->timestamp_last_read_allowed + options->TestingDirConnectionMaxStall < now))) {
        log_info(LD_DIR, "Expiring wedged directory conn (fd %d, purpose %d)", (int)conn->s,
                 conn->purpose);
    ...
    if (channel_is_bad_for_new_circs(TLS_CHAN_TO_BASE(or_conn->chan)) && !have_any_circuits) {
        /* It's bad for new circuits, and has no unmarked circuits on it:
         * mark it now. */
        log_info(LD_OR, "Expiring non-used OR connection to fd %d (%s:%d) [Too old].", (int)conn->s,
                 conn->address, conn->port);
        if (conn->state == OR_CONN_STATE_CONNECTING)
            connection_or_connect_failed(TO_OR_CONN(conn), END_OR_CONN_REASON_TIMEOUT,
                                         "Tor gave up on the connection");
        connection_or_close_normally(TO_OR_CONN(conn), 1);
    } else if (!connection_state_is_open(conn)) {
        if (past_keepalive) {
            /* We never managed to actually get this connection open and happy. */
            log_info(LD_OR, "Expiring non-open OR connection to fd %d (%s:%d).", (int)conn->s,
                     conn->address, conn->port);
            connection_or_close_normally(TO_OR_CONN(conn), 0);
        }
    } else if (we_are_hibernating() && !have_any_circuits && !connection_get_outbuf_len(conn)) {
        ...
        log_info(LD_OR,
                 ...
                 (chan->global_identifier), (int)conn->s, conn->address, conn->port,
                 (int)(now - chan->timestamp_last_had_circuits), or_conn->idle_timeout,
                 or_conn->is_canonical ? "" : "non");
        connection_or_close_normally(TO_OR_CONN(conn), 0);
    } else if (now >= or_conn->timestamp_lastempty + options->KeepalivePeriod * 10 &&
               now >= conn->timestamp_last_write_allowed + options->KeepalivePeriod * 10) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
               "Expiring stuck OR connection to fd %d (%s:%d). (%d bytes to "
               "flush; %d seconds since last write)",
               (int)conn->s, conn->address, conn->port, (int)connection_get_outbuf_len(conn),
               (int)(now - conn->timestamp_last_write_allowed));
        connection_or_close_normally(TO_OR_CONN(conn), 0);
    } else if (past_keepalive && !connection_get_outbuf_len(conn)) {
        /* send a padding cell */
        log_fn(LOG_DEBUG, LD_OR, "Sending keepalive to (%s:%d)", conn->address, conn->port);
...
do_signewnym(time_t now)
{
    if (time_of_last_signewnym + MAX_SIGNEWNYM_RATE > now) {
        ...
        log_notice(LD_CONTROL, "Rate limiting NEWNYM request: delaying by %d second(s)",
                   (int)(delay_sec));
    } else {
        signewnym_impl(now);
    }
...
get_my_roles(const or_options_t *options)
{
    ...
    int is_hidden_service = !!hs_service_get_num_services() || !!rend_num_services();
    ...
    int is_client = options_any_client_port_set(options) || options->ControlPort_set ||
                    options->OwningControllerFD != UINT64_MAX;

    int is_net_participant = is_participating_on_network() || is_relay || is_hidden_service;
...
schedule_rescan_periodic_events, (void))
{
    if (!rescan_periodic_events_ev) {
        rescan_periodic_events_ev = mainloop_event_new(rescan_periodic_events_cb, NULL);
    }
...
check_expired_networkstatus_callback(time_t now, const or_options_t *options)
{
     ...
     */
    if (ns && !networkstatus_consensus_reasonably_live(ns, now) && router_have_minimum_dir_info()) {
...
fetch_networkstatus_callback(time_t now, const or_options_t *options)
{
    ...
    const int prefer_mirrors = !dirclient_fetches_from_authorities(get_options());
...
#define NUM_IDLE_SECONDS_BEFORE_WARN 3600
    ...
    if (seconds_elapsed < -NUM_JUMPED_SECONDS_BEFORE_WARN) {
        ...
        const int32_t monotime_msec_passed =
            monotime_coarse_diff_msec32(&last_updated, &current_second_last_changed);
...
initialize_mainloop_events(void)
{
    ...
    if (!postloop_cleanup_ev) {
        postloop_cleanup_ev = mainloop_event_postloop_new(postloop_cleanup_cb, NULL);
    }
}
...
do_main_loop(void)
{
    ...
    initialize_periodic_events_event =
        tor_evtimer_new(tor_libevent_get_base(), initialize_periodic_events_cb, NULL);
    ...
    if (sd_watchdog_enabled(1, &watchdog_delay) > 0) {
        if (!systemd_watchdog_timer) {
            ...
            systemd_watchdog_timer = periodic_timer_new(tor_libevent_get_base(), &watchdog,
                                                        systemd_watchdog_callback, NULL);
...
#ifdef ENABLE_RESTART_DEBUGGING
    {
        ...
        if (first_time && getenv("TOR_DEBUG_RESTART")) {
            ...
            long sec;
            int sec_ok = 0;
            if (sec_str && (sec = tor_parse_long(sec_str, 10, 0, INT_MAX, &sec_ok, NULL)) &&
                sec_ok) {
            ...
            tor_shutdown_event_loop_for_restart_event = tor_evtimer_new(
                tor_libevent_get_base(), tor_shutdown_event_loop_for_restart_cb, NULL);
...
run_main_loop_once(void)
{
    ...
    if (loop_result < 0) {
        ...
        if (e != EINTR && !ERRNO_IS_EINPROGRESS(e)) {
            log_err(LD_NET, "libevent call with %s failed: %s [%d]", tor_libevent_get_method(),
