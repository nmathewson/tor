...
connection_add_impl(connection_t *conn, int is_connecting)
{
    ...
    tor_assert(conn->conn_array_index ==
    ...
    if (SOCKET_OK(conn->s) || conn->linked) {
        conn->read_event =
            tor_event_new(tor_libevent_get_base(), conn->s,
                          EV_READ | EV_PERSIST, conn_read_callback, conn);
        conn->write_event =
            tor_event_new(tor_libevent_get_base(), conn->s,
                          EV_WRITE | EV_PERSIST, conn_write_callback, conn);
...
connection_unregister_events(connection_t *conn)
{
    ...
    if (conn->write_event) {
        if (event_del(conn->write_event))
            log_warn(LD_BUG, "Error removing write event for %d",
                     (int)conn->s);
...
connection_remove(connection_t *conn)
{
    ...
    if (current_index ==
        smartlist_len(connection_array) - 1) { /* at the end */
...
schedule_active_linked_connections_cb(mainloop_event_t *event, void *arg)
{
     ...
     * sent incrementally by small chunks unless spool_eagerly is true. For
     * that to happen, we need to induce the activation of the read event so it
     * can be flushed. */
...
connection_is_reading(connection_t *conn)
{
    ...
    return conn->reading_from_linked_conn ||
           (conn->read_event &&
            event_pending(conn->read_event, EV_READ, NULL));
}
...
connection_check_event(connection_t *conn, struct event *ev)
{
    ...
    if (conn->type == CONN_TYPE_AP && TO_EDGE_CONN(conn)->is_dns_request) {
        /* DNS requests which we launch through the dnsserv.c module do not
         * have any underlying socket or any underlying linked connection, so
         * they shouldn't have any attached events either.
         */
        bad = ev != NULL;
    } else {
        /* Everything else should have an underlying socket, or a linked
         * connection (which is also tracked with a read_event/write_event
         * pair).
         */
        bad = ev == NULL;
    }
...
tor_shutdown_event_loop_and_exit(int exitcode)
{
    ...
    /* Die with an assertion failure in ten seconds, if for some reason we
     * don't exit normally. */
...
conn_read_callback(evutil_socket_t fd, short event, void *_conn)
{
    ...
    if (conn->marked_for_close && connection_is_reading(conn)) {
        /* Libevent says we can read, but we are marked for close so we will
         * never try to read again. We will try to close the connection below
         * inside of close_closeable_connections(), but let's make sure not to
         * cause Libevent to spin on conn_read_callback() while we wait for the
...
conn_write_callback(evutil_socket_t fd, short events, void *_conn)
{
    ...
    LOG_FN_CONN(
        conn, (LOG_DEBUG, LD_NET, "socket %d wants to write.", (int)conn->s));
...
conn_close_if_marked(int i)
{
    ...
    if ((SOCKET_OK(conn->s) || conn->linked_conn) &&
        ...
        if (conn->linked_conn) {
            ...
            if (retval >= 0) {
                /* The linked conn will notice that it has data when it notices
                 * that we're gone. */
                connection_start_reading_from_linked_conn(conn->linked_conn);
            }
            ...
            if (conn->state == OR_CONN_STATE_OPEN) {
                retval = buf_flush_to_tls(conn->outbuf, TO_OR_CONN(conn)->tls,
                                          sz, &conn->outbuf_flushlen);
            } else
            ...
            if (retval > 0) {
                LOG_FN_CONN(conn,
                            (LOG_INFO, LD_NET,
                             ...
                             (int)conn->s));
                conn->timestamp_last_write_allowed = now; /* reset so we can
                                                           * flush more */
            } else if (sz == 0) {
                /* Also, retval==0.  If we get here, we didn't want to write
                 * anything (because of rate-limiting) and we didn't. */

                /* Connection must flush before closing, but it's being
                 * rate-limited. Let's remove from Libevent, and mark it as
                 * blocked on bandwidth so it will be re-added on next token
                 * bucket refill. Prevents busy Libevent loops where we keep
                 * ending up here and returning 0 until we are no longer
                 * blocked on bandwidth.
                 */
                ...
                /* The connection is being held due to write rate limit and
                 * thus will flush its data later. We need to stop reading
                 * because this connection is about to be closed once flushed.
                 * It should not
...
run_connection_housekeeping(int i, time_t now)
{
    ...
    if (conn->type == CONN_TYPE_DIR &&
         ...
         * consider parsing partial serverdesc responses. */
    ...
    if (channel_is_bad_for_new_circs(TLS_CHAN_TO_BASE(or_conn->chan)) &&
        ...
        /* We're hibernating or shutting down, there's no circuits, and nothing
         * to flush.*/
        log_info(LD_OR,
    ...
    if (time_of_last_signewnym + MAX_SIGNEWNYM_RATE > now) {
        ...
        if (!signewnym_is_pending) {
            ...
            if (!handle_deferred_signewnym_ev) {
                handle_deferred_signewnym_ev = mainloop_event_postloop_new(
                    handle_deferred_signewnym_cb, NULL);
            }
...
mainloop_schedule_postloop_cleanup(void)
{
    if (PREDICT_UNLIKELY(postloop_cleanup_ev == NULL)) {
        // (It's possible that we can get here if we decide to close a
        // connection in the earliest stages of our configuration, before we
        // create events.)
        return;
    }
...
mainloop_schedule_shutdown(int delay_sec)
{
    ...
    if (!scheduled_shutdown_ev) {
        scheduled_shutdown_ev =
            mainloop_event_new(scheduled_shutdown_cb, NULL);
    }
...
second_elapsed_callback(time_t now, const or_options_t *options)
{
    ...
    if (options->UseBridges && !net_is_disabled()) {
        /* Note: this check uses net_is_disabled(), not
...
check_expired_networkstatus_callback(time_t now, const or_options_t *options)
{
    ...
    /* Use reasonably live consensuses until they are no longer reasonably
     * live.
     */
...
heartbeat_callback(time_t now, const or_options_t *options)
{
    ...
    if (log_heartbeat(now) == 0) {
        ...
        /* If we couldn't write the heartbeat log message, try again in the
...
#define NUM_IDLE_SECONDS_BEFORE_WARN 3600
    ...
    if (seconds_elapsed < -NUM_JUMPED_SECONDS_BEFORE_WARN) {
         ...
         * possibilities.  On some systems, this means we have been suspended
         * or sleeping.  Everywhere, it can mean that the wall-clock time has
         * been changed -- for example, with settimeofday().
         *
         * On the other hand, if the monotonic time matches with the wall-clock
         * time, we've probably just been idle for a while, with no events
         * firing. we tolerate much more of that.
         */
...
ip_address_changed(int on_client_conn)
{
    ...
    if (on_client_conn) {
        ...
        if (server) {
            ...
            /* All relays include their IP addresses as their ORPort addresses
             * in their descriptor. Exit relays also incorporate interface
             * addresses in their exit policies, when
...
do_main_loop(void)
{
    ...
    if (sd_watchdog_enabled(1, &watchdog_delay) > 0) {
        if (!systemd_watchdog_timer) {
            ...
            /* The manager will "act on" us if we don't send them a
             * notification every 'watchdog_delay' microseconds.  So, send
             * notifications twice that often.  */
            watchdog_delay /= 2;
            watchdog.tv_sec = watchdog_delay / 1000000;
            watchdog.tv_usec = watchdog_delay % 1000000;

            systemd_watchdog_timer =
                periodic_timer_new(tor_libevent_get_base(), &watchdog,
...
#ifdef ENABLE_RESTART_DEBUGGING
    {
        ...
        if (first_time && getenv("TOR_DEBUG_RESTART")) {
            ...
            event_add(tor_shutdown_event_loop_for_restart_event,
                      &restart_after);
        }
    }
#endif /* defined(ENABLE_RESTART_DEBUGGING) */
...
run_main_loop_once(void)
{
    ...
    if (get_options()->MainloopStats) {
        /* We always enforce that EVLOOP_ONCE is passed to event_base_loop() if
         * we are collecting main loop statistics. */
        called_loop_once = 1;
    } else {
        called_loop_once = 0;
    }
     ...
     * catch, but more likely, it's just an interrupted poll() call or
     * something, and we should try again. */
    if (loop_result < 0) {
        ...
        if (e != EINTR && !ERRNO_IS_EINPROGRESS(e)) {
            ...
            log_fn_ratelim(
                &libevent_error_ratelim, LOG_WARN, LD_NET,
