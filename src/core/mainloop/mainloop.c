...
connection_add_impl(connection_t *conn, int is_connecting)
{
    ...
    if (SOCKET_OK(conn->s) || conn->linked) {
        conn->read_event = tor_event_new(
            tor_libevent_get_base(),
            conn->s, EV_READ | EV_PERSIST, conn_read_callback,
            conn);
        conn->write_event = tor_event_new(
            tor_libevent_get_base(),
            conn->s, EV_WRITE | EV_PERSIST, conn_write_callback,
            conn);
    ...
    if (conn->linked) {
        ...
        if (event_del(conn->read_event)) {
            log_warn(LD_NET, "Error from libevent setting read event state for %d "
                     "to unwatched: %s",
                     (int)conn->s,
                     tor_socket_strerror(tor_socket_errno(
                                             conn->s)));
        }
    }
}
    ...
    if ((SOCKET_OK(conn->s) || conn->linked_conn) &&
        ...
        if (conn->linked_conn) {
            ...
            if (conn->state == OR_CONN_STATE_OPEN)
                retval = buf_flush_to_tls(conn->outbuf, TO_OR_CONN(
                                              conn)->tls, sz,
                                          &conn->outbuf_flushlen);
            else
        ...
        if (!signewnym_is_pending) {
            ...
            if (!handle_deferred_signewnym_ev)
                handle_deferred_signewnym_ev =
                    mainloop_event_postloop_new(handle_deferred_signewnym_cb,
                                                NULL);
...
STATIC periodic_event_item_t mainloop_periodic_events[] = {
    ...
    CALLBACK(retry_listeners,             NET_PARTICIPANT,
             FL(NEED_NET)),
    ...
    CALLBACK(launch_descriptor_fetches,   NET_PARTICIPANT,
             FL(NEED_NET)),
    ...
    CALLBACK(clean_caches,                NET_PARTICIPANT,
             FL(RUN_ON_DISABLE)),
    CALLBACK(save_state,                  NET_PARTICIPANT,
             FL(RUN_ON_DISABLE)),
    CALLBACK(write_stats_file,            NET_PARTICIPANT,
             FL(RUN_ON_DISABLE)),
    CALLBACK(prune_old_routers,           NET_PARTICIPANT,
             FL(RUN_ON_DISABLE)),
...
initialize_mainloop_events(void)
{
    if (!schedule_active_linked_connections_event)
        schedule_active_linked_connections_event =
            mainloop_event_postloop_new(schedule_active_linked_connections_cb,
                                        NULL);
