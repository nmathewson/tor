    ...
    if ((SOCKET_OK(conn->s) || conn->linked_conn) &&
        ...
        if (retval >= 0 && /* Technically, we could survive things like
            ...
            if (retval > 0) {
                 ...
                 * ending up here and returning 0 until we are no longer blocked
                 * on bandwidth.
                 */
                ...
                /* The connection is being held due to write rate limit and thus
                 * will flush its data later. We need to stop reading because
                 * this connection is about to be closed once flushed. It should
                 * not process anything more coming in at this stage. */
                connection_stop_reading(conn);
            }
            return 0;
        }
...
do_main_loop(void)
{
    ...
    if (sd_watchdog_enabled(1, &watchdog_delay) > 0) {
        if (!systemd_watchdog_timer) {
             ...
             * every 'watchdog_delay' microseconds.  So, send notifications
             * twice that often.  */
            watchdog_delay /= 2;
