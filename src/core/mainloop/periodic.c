...
periodic_event_dispatch(mainloop_event_t *ev, void *data)
{
    ...
    if (!periodic_event_is_enabled(event)) {
        /* The event got disabled from inside its callback, or before: no need
         * to reschedule. */
        return;
    }
...
safe_timer_diff(time_t now, time_t next)
{
    if (next > now) {
         ...
         * Bang. We cannot extrapolate past a singularity, but there was
         *probably nothing that could run Tor then, either.
         **/
