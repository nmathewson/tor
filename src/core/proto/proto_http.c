...
{
    if (buf_peek_startswith(buf, "CONNECT ") || buf_peek_startswith(buf, "DELETE ") ||
 ...
 */
int
fetch_from_buf_http(buf_t *buf, char **headers_out, size_t max_headerlen, char **body_out,
                    size_t *body_used, size_t max_bodylen, int force_complete)
{
    const char *headers;
    ...
    if (crlf_offset > (int)max_headerlen || (crlf_offset < 0 && buf_datalen(buf) > max_headerlen)) {
 ...
 */
STATIC int
buf_http_find_content_length(const char *headers, size_t headerlen, size_t *result_out)
{
