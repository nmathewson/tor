...
static socks_result_t parse_socks(const char *data, size_t datalen, socks_request_t *req,
                                  int log_sockstype, int safe_socks, size_t *drain_out);
static int parse_socks_client(const uint8_t *data, size_t datalen, int state, char **reason,
                              ssize_t *drain_out);
/**
...
static void
log_unsafe_socks_warning(int socks_protocol, const char *address, uint16_t port, int safe_socks)
{
    ...
    control_event_client_status(LOG_WARN, "DANGEROUS_SOCKS PROTOCOL=SOCKS%d ADDRESS=%s:%d",
...
parse_socks4_request(const uint8_t *raw_data, socks_request_t *req, size_t datalen, int *is_socks4a,
                     size_t *drain_out)
{
    ...
    *drain_out = 0;
    ...
    ssize_t parsed = socks4_client_request_parse(&trunnel_req, raw_data, datalen);
    ...
    *is_socks4a = (dest_ip >> 8) == 0;
    ...
    if (*is_socks4a) {
        ...
        const char *hostname = (char *)raw_data + SOCKS4_NETWORK_LEN + usernamelen + 1;
 ...
 */
static socks_result_t
process_socks4_request(const socks_request_t *req, int is_socks4a, int log_sockstype,
                       int safe_socks)
{
    ...
    if (req->command != SOCKS_COMMAND_CONNECT && req->command != SOCKS_COMMAND_RESOLVE) {
         ...
         * socks4.) */
        log_warn(LD_APP, "socks4: command %d not recognized. Rejecting.", req->command);
        return SOCKS_RESULT_INVALID;
    }
 ...
 */
static socks_result_t
parse_socks5_methods_request(const uint8_t *raw_data, socks_request_t *req, size_t datalen,
                             int *have_user_pass, int *have_no_auth, size_t *drain_out)
{
    socks_result_t res = SOCKS_RESULT_DONE;
    socks5_client_version_t *trunnel_req;

    ssize_t parsed = socks5_client_version_parse(&trunnel_req, raw_data, datalen);
 ...
 */
static socks_result_t
process_socks5_methods_request(socks_request_t *req, int have_user_pass, int have_no_auth)
{
    socks_result_t res = SOCKS_RESULT_DONE;
    ...
    if (errmsg) {
        ...
        ssize_t encoded = socks5_server_method_encode(req->reply, sizeof(req->reply), trunnel_resp);
 ...
 */
static socks_result_t
parse_socks5_userpass_auth(const uint8_t *raw_data, socks_request_t *req, size_t datalen,
                           size_t *drain_out)
{
    socks_result_t res = SOCKS_RESULT_DONE;
    socks5_client_userpass_auth_t *trunnel_req = NULL;
    ssize_t parsed = socks5_client_userpass_auth_parse(&trunnel_req, raw_data, datalen);
    tor_assert(drain_out);
    *drain_out = 0;
    ...
    *drain_out = (size_t)parsed;
    ...
    uint8_t usernamelen = socks5_client_userpass_auth_get_username_len(trunnel_req);
    uint8_t passwordlen = socks5_client_userpass_auth_get_passwd_len(trunnel_req);
    const char *username = socks5_client_userpass_auth_getconstarray_username(trunnel_req);
    const char *password = socks5_client_userpass_auth_getconstarray_passwd(trunnel_req);
...
{
    socks_result_t res = SOCKS_RESULT_DONE;
    socks5_server_userpass_auth_t *trunnel_resp = socks5_server_userpass_auth_new();
    tor_assert(trunnel_resp);
    ...
    if (errmsg) {
        log_warn(LD_APP, "socks5: server userpass auth validation failed: %s", errmsg);
        res = SOCKS_RESULT_INVALID;
        goto end;
    }

    ssize_t encoded =
        socks5_server_userpass_auth_encode(req->reply, sizeof(req->reply), trunnel_resp);
 ...
 */
static socks_result_t
parse_socks5_client_request(const uint8_t *raw_data, socks_request_t *req, size_t datalen,
                            size_t *drain_out)
{
    socks_result_t res = SOCKS_RESULT_DONE;
    tor_addr_t destaddr;
    ...
    *drain_out = (size_t)parsed;
    ...
    case 4: {
        const uint8_t *ipv6 = socks5_client_request_getarray_dest_addr_ipv6(trunnel_req);
 ...
 */
static socks_result_t
process_socks5_client_request(socks_request_t *req, int log_sockstype, int safe_socks)
{
    socks_result_t res = SOCKS_RESULT_DONE;
    tor_addr_t tmpaddr;
    ...
    if (req->command == SOCKS_COMMAND_RESOLVE_PTR && tor_addr_parse(&tmpaddr, req->address) < 0) {
                     ...
                     int log_sockstype, int safe_socks, size_t *drain_out)
{
    ...
    if (socks_version == SOCKS_VER_4) {
        ...
        int is_socks4a = 0;
        res = parse_socks4_request((const uint8_t *)raw_data, req, datalen, &is_socks4a, drain_out);
        ...
        if (!req->got_auth && (raw_data[0] == 1 || req->auth_type == SOCKS_USER_PASS)) {
            ...
            res = parse_socks5_methods_request(raw_data, req, datalen, &have_user_pass,
                                               &have_no_auth, drain_out);
 ...
 */
int
fetch_from_buf_socks(buf_t *buf, socks_request_t *req, int log_sockstype, int safe_socks)
{
    int res = 0;
    ...
    do {
        ...
        socks_res = parse_socks(head, datalen, req, log_sockstype, safe_socks, &n_drain);
 ...
 */
static void
socks_request_set_socks5_error(socks_request_t *req, socks5_reply_status_t reason)
{
    ...
    ssize_t encoded = socks5_server_reply_encode(req->reply, sizeof(req->reply), trunnel_resp);
    if (encoded < 0) {
...
static int
parse_socks(const char *data, size_t datalen, socks_request_t *req, int log_sockstype,
            int safe_socks, size_t *drain_out)
{
    ...
    if (first_octet == SOCKS_VER_5 || first_octet == SOCKS_VER_4 ||
        ...
        return handle_socks_message((const uint8_t *)data, datalen, req, log_sockstype, safe_socks,
                                    drain_out);
    }
    ...
    case 'P': /* put/post */
    case 'C': /* connect */
        strlcpy((char *)req->reply, SOCKS_PROXY_IS_NOT_AN_HTTP_PROXY_MSG, MAX_SOCKS_REPLY_LEN);
    ...
    default: /* version is not socks4 or socks5 */
                 ...
                 *(data));
        {
            ...
            control_event_client_status(LOG_WARN, "SOCKS_UNKNOWN_PROTOCOL DATA=\"%s\"",
                                        escaped(tmp));
            tor_free(tmp);
        }
        return -1;
...
static int
parse_socks_client(const uint8_t *data, size_t datalen, int state, char **reason,
                   ssize_t *drain_out)
{
