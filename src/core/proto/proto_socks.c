...
static void
log_unsafe_socks_warning(int socks_protocol, const char *address, uint16_t port,
                         int safe_socks)
{
    ...
    control_event_client_status(
        LOG_WARN, "DANGEROUS_SOCKS PROTOCOL=SOCKS%d ADDRESS=%s:%d",
                     ...
                     size_t *drain_out)
{
    ...
    if (socks_version == SOCKS_VER_4) {
        ...
        res =
            process_socks4_request(req, is_socks4a, log_sockstype, safe_socks);
        ...
        if (!req->got_auth &&
            ...
            res =
                parse_socks5_client_request(raw_data, req, datalen, drain_out);
            if (res != SOCKS_RESULT_DONE) {
                socks_request_set_socks5_error(req, SOCKS5_GENERAL_ERROR);
                goto end;
            }
                     ...
                     int safe_socks)
{
    ...
    do {
        ...
        socks_res = parse_socks(head, datalen, req, log_sockstype, safe_socks,
                                &n_drain);
                               ...
                               socks5_reply_status_t reason)
{
    ...
    ssize_t encoded = socks5_server_reply_encode(req->reply, sizeof(req->reply),
                                                 trunnel_resp);
    if (encoded < 0) {
    ...
    switch (first_octet) { /* which version of socks? */
        ...
        default: /* version is not socks4 or socks5 */
                     ...
                     *(data));
            {
                ...
                control_event_client_status(
                    LOG_WARN, "SOCKS_UNKNOWN_PROTOCOL DATA=\"%s\"",
                    escaped(tmp));
                tor_free(tmp);
            }
            return -1;
    ...
    *drain_out = 0;
    ...
    switch (state) {
        ...
        case PROXY_SOCKS5_WANT_AUTH_METHOD_RFC1929:
            /* we have a username and password. return 1 if we can proceed
             * without providing authentication, or 2 otherwise. */
            switch (data[1]) {
                case 0x00:
                    log_info(LD_NET,
