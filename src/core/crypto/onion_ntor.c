...
#define PROTOID_LEN 24
    PROTOID ":mac", PROTOID ":key_extract", PROTOID ":verify", PROTOID ":key_expand"};
...
#define SECRET_INPUT_LEN \
    (CURVE25519_PUBKEY_LEN * 3 + CURVE25519_OUTPUT_LEN * 2 + DIGEST_LEN + PROTOID_LEN)
#define AUTH_INPUT_LEN \
    (DIGEST256_LEN + DIGEST_LEN + CURVE25519_PUBKEY_LEN * 3 + PROTOID_LEN + SERVER_STR_LEN)
                                 ...
                                 uint8_t *handshake_reply_out, uint8_t *key_out, size_t key_out_len)
{
    ...
    h_tweak(handshake_reply_out + CURVE25519_PUBKEY_LEN, s.auth_input, sizeof(s.auth_input),
            T->t_mac);

    /* Generate the key material */
    crypto_expand_key_material_rfc5869_sha256(
        s.secret_input, sizeof(s.secret_input), (const uint8_t *)T->t_key, strlen(T->t_key),
...
onion_skin_ntor_client_handshake(const ntor_handshake_state_t *handshake_state,
                                 const uint8_t *handshake_reply, uint8_t *key_out,
