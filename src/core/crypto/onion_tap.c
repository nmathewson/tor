 ...
 */
int
onion_skin_TAP_create(crypto_pk_t *dest_router_key, crypto_dh_t **handshake_state_out,
                      char *onion_skin_out) /* TAP_ONIONSKIN_CHALLENGE_LEN bytes */
{
    ...
    *handshake_state_out = NULL;
    ...
    if (crypto_pk_obsolete_public_hybrid_encrypt(dest_router_key, onion_skin_out,
                                                 TAP_ONIONSKIN_CHALLENGE_LEN, challenge,
                                                 DH1024_KEY_LEN, PK_PKCS1_OAEP_PADDING, 1) < 0)
        goto err;
...
onion_skin_TAP_server_handshake(
    ...
    const char *onion_skin, crypto_pk_t *private_key, crypto_pk_t *prev_private_key,
    ...
    char *handshake_reply_out, char *key_out, size_t key_out_len)
{
    ...
    for (i = 0; i < 2; ++i) {
        ...
        if (!k)
            break;
        len = crypto_pk_obsolete_private_hybrid_decrypt(k, challenge, TAP_ONIONSKIN_CHALLENGE_LEN,
                                                        onion_skin, TAP_ONIONSKIN_CHALLENGE_LEN,
                                                        PK_PKCS1_OAEP_PADDING, 0);
        if (len > 0)
            break;
    }
    if (len < 0) {
        log_info(LD_PROTOCOL, "Couldn't decrypt onionskin: client may be using old onion key");
        goto err;
    } else if (len != DH1024_KEY_LEN) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Unexpected onionskin length after decryption: %ld",
               (long)len);
        goto err;
    }
    ...
    len = crypto_dh_compute_secret(LOG_PROTOCOL_WARN, dh, challenge, DH1024_KEY_LEN, key_material,
                                   key_material_len);
    if (len < 0) {
                                ...
                                char *key_out, size_t key_out_len, const char **msg_out)
{
    ...
    key_material_len = DIGEST_LEN + key_out_len;
    key_material = tor_malloc(key_material_len);
    len = crypto_dh_compute_secret(LOG_PROTOCOL_WARN, handshake_state, handshake_reply,
