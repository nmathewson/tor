...
#define REND_SECRET_HS_INPUT_LEN \
    (CURVE25519_OUTPUT_LEN * 2 + ED25519_PUBKEY_LEN + CURVE25519_PUBKEY_LEN * 3 + PROTOID_LEN)
/* Length of auth_input = verify | AUTH_KEY | B | Y | X | PROTOID | "Server" */
#define REND_AUTH_INPUT_LEN \
    (DIGEST256_LEN + ED25519_PUBKEY_LEN + CURVE25519_PUBKEY_LEN * 3 + PROTOID_LEN + SERVER_STR_LEN)
                             ...
                             hs_ntor_rend_cell_keys_t *hs_ntor_rend_cell_keys_out)
{
    ...
    crypto_mac_sha3_256(ntor_key_seed, sizeof(ntor_key_seed), rend_secret_hs_input,
                        REND_SECRET_HS_INPUT_LEN, (const uint8_t *)T_HSENC, strlen(T_HSENC));
    ...
    crypto_mac_sha3_256(ntor_verify, sizeof(ntor_verify), rend_secret_hs_input,
                        REND_SECRET_HS_INPUT_LEN, (const uint8_t *)T_HSVERIFY, strlen(T_HSVERIFY));
    ...
    /* Append Y */
    APPEND(ptr, service_ephemeral_rend_pubkey->public_key, CURVE25519_PUBKEY_LEN);
    /* Append X */
    ...
    crypto_mac_sha3_256(rend_cell_auth, sizeof(rend_cell_auth), rend_auth_input,
                        sizeof(rend_auth_input), (const uint8_t *)T_HSMAC, strlen(T_HSMAC));
    bad |= safe_mem_is_zero(ntor_verify, DIGEST256_LEN);

    { /* Get the computed RENDEZVOUS1 material! */
        memcpy(&hs_ntor_rend_cell_keys_out->rend_cell_auth_mac, rend_cell_auth, DIGEST256_LEN);
        memcpy(&hs_ntor_rend_cell_keys_out->ntor_key_seed, ntor_key_seed, DIGEST256_LEN);
    }
...
#define INTRO_SECRET_HS_INPUT_LEN                                                                 \
    (CURVE25519_OUTPUT_LEN + ED25519_PUBKEY_LEN + CURVE25519_PUBKEY_LEN + CURVE25519_PUBKEY_LEN + \
     PROTOID_LEN)
...
static void
get_introduce1_key_material(const uint8_t *secret_input, const hs_subcredential_t *subcredential,
                            hs_ntor_intro_cell_keys_t *hs_ntor_intro_cell_keys_out)
{
    ...
    { /* Get the keys */
        ...
        memcpy(&hs_ntor_intro_cell_keys_out->mac_key, keystream + CIPHER256_KEY_LEN, DIGEST256_LEN);
    }
 ...
 */
static void
get_intro_secret_hs_input(const uint8_t *dh_result, const ed25519_public_key_t *intro_auth_pubkey,
                          const curve25519_public_key_t *client_ephemeral_enc_pubkey,
                          const curve25519_public_key_t *intro_enc_pubkey,
 ...
 * DIGEST256_LEN). */
int
hs_ntor_client_get_introduce1_keys(const ed25519_public_key_t *intro_auth_pubkey,
                                   const curve25519_public_key_t *intro_enc_pubkey,
                                   ...
                                   hs_ntor_intro_cell_keys_t *hs_ntor_intro_cell_keys_out)
{
    ...
    /* Calculate EXP(B,x) */
    curve25519_handshake(dh_result, &client_ephemeral_enc_keypair->seckey, intro_enc_pubkey);
    bad |= safe_mem_is_zero(dh_result, CURVE25519_OUTPUT_LEN);

    /* Get intro_secret_hs_input */
    get_intro_secret_hs_input(dh_result, intro_auth_pubkey, &client_ephemeral_enc_keypair->pubkey,
                              intro_enc_pubkey, secret_input);
    bad |= safe_mem_is_zero(secret_input, CURVE25519_OUTPUT_LEN);

    /* Get ENC_KEY and MAC_KEY! */
    get_introduce1_key_material(secret_input, subcredential, hs_ntor_intro_cell_keys_out);
...
int
hs_ntor_client_get_rendezvous1_keys(const ed25519_public_key_t *intro_auth_pubkey,
                                    const curve25519_keypair_t *client_ephemeral_enc_keypair,
                                    const curve25519_public_key_t *intro_enc_pubkey,
                                    const curve25519_public_key_t *service_ephemeral_rend_pubkey,
                                    hs_ntor_rend_cell_keys_t *hs_ntor_rend_cell_keys_out)
{
    ...
    /* Compute EXP(B, x) */
    curve25519_handshake(dh_result2, &client_ephemeral_enc_keypair->seckey, intro_enc_pubkey);
    bad |= safe_mem_is_zero(dh_result2, CURVE25519_OUTPUT_LEN);

    /* Get rend_secret_hs_input */
    get_rend_secret_hs_input(dh_result1, dh_result2, intro_auth_pubkey, intro_enc_pubkey,
    ...
    bad |= get_rendezvous1_key_material(
        rend_secret_hs_input, intro_auth_pubkey, intro_enc_pubkey, service_ephemeral_rend_pubkey,
        &client_ephemeral_enc_keypair->pubkey, hs_ntor_rend_cell_keys_out);
...
int
hs_ntor_service_get_introduce1_keys(const ed25519_public_key_t *intro_auth_pubkey,
                                    const curve25519_keypair_t *intro_enc_keypair,
                                    const curve25519_public_key_t *client_ephemeral_enc_pubkey,
                                    const hs_subcredential_t *subcredential,
                                    hs_ntor_intro_cell_keys_t *hs_ntor_intro_cell_keys_out)
{
    return hs_ntor_service_get_introduce1_keys_multi(intro_auth_pubkey, intro_enc_keypair,
                                                     client_ephemeral_enc_pubkey, 1, subcredential,
...
hs_ntor_service_get_introduce1_keys_multi(
    ...
    const struct curve25519_public_key_t *client_ephemeral_enc_pubkey, size_t n_subcredentials,
    ...
    hs_ntor_intro_cell_keys_t *hs_ntor_intro_cell_keys_out)
{
    ...
    /* Compute EXP(X, b) */
    curve25519_handshake(dh_result, &intro_enc_keypair->seckey, client_ephemeral_enc_pubkey);
    bad |= safe_mem_is_zero(dh_result, CURVE25519_OUTPUT_LEN);

    /* Get intro_secret_hs_input */
    get_intro_secret_hs_input(dh_result, intro_auth_pubkey, client_ephemeral_enc_pubkey,
 ...
 * <b>client_ephemeral_enc_pubkey</b> is X (CLIENT_PK in INTRODUCE2 cell) */
int
hs_ntor_service_get_rendezvous1_keys(const ed25519_public_key_t *intro_auth_pubkey,
                                     const curve25519_keypair_t *intro_enc_keypair,
                                     ...
                                     hs_ntor_rend_cell_keys_t *hs_ntor_rend_cell_keys_out)
{
    ...
    /* Compute EXP(X, b) */
    curve25519_handshake(dh_result2, &intro_enc_keypair->seckey, client_ephemeral_enc_pubkey);
    bad |= safe_mem_is_zero(dh_result2, CURVE25519_OUTPUT_LEN);

    /* Get rend_secret_hs_input */
    get_rend_secret_hs_input(dh_result1, dh_result2, intro_auth_pubkey, &intro_enc_keypair->pubkey,
                             client_ephemeral_enc_pubkey, &service_ephemeral_rend_keypair->pubkey,
                             rend_secret_hs_input);

    /* Get NTOR_KEY_SEED and AUTH_INPUT_MAC! */
    bad |= get_rendezvous1_key_material(rend_secret_hs_input, intro_auth_pubkey,
                                        &intro_enc_keypair->pubkey,
                                        &service_ephemeral_rend_keypair->pubkey,
                                        client_ephemeral_enc_pubkey, hs_ntor_rend_cell_keys_out);
...
int
hs_ntor_client_rendezvous2_mac_is_good(const hs_ntor_rend_cell_keys_t *hs_ntor_rend_cell_keys,
                                       const uint8_t *rcvd_mac)
{
    tor_assert(rcvd_mac);
    tor_assert(hs_ntor_rend_cell_keys);

    return tor_memeq(hs_ntor_rend_cell_keys->rend_cell_auth_mac, rcvd_mac, DIGEST256_LEN);
}
...
int
hs_ntor_circuit_key_expansion(const uint8_t *ntor_key_seed, size_t seed_len, uint8_t *keys_out,
                              size_t keys_out_len)
{
    uint8_t *ptr;
    ...
    /* Generate the keys */
    crypto_xof(keys_out, HS_NTOR_KEY_EXPANSION_KDF_OUT_LEN, kdf_input, sizeof(kdf_input));
