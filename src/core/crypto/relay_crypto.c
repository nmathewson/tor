 ...
 * SENDME cell digest. */
void
relay_crypto_record_sendme_digest(relay_crypto_t *crypto, bool is_foward_digest)
{
    ...
    crypto_digest_get_digest(digest, (char *)crypto->sendme_digest, sizeof(crypto->sendme_digest));
}
 ...
 */
int
relay_decrypt_cell(circuit_t *circ, cell_t *cell, cell_direction_t cell_direction,
                   crypt_path_t **layer_hint, char *recognized)
{
    ...
    if (cell_direction == CELL_DIRECTION_IN) {
        if (CIRCUIT_IS_ORIGIN(circ)) { /* We're at the beginning of the circuit.
            ...
            log_fn(LOG_PROTOCOL_WARN, LD_OR, "Incoming cell at client not recognized. Closing.");
            return -1;
        } else {
 ...
 */
void
relay_encrypt_cell_outbound(cell_t *cell, origin_circuit_t *circ, crypt_path_t *layer_hint)
{
 ...
 */
int
relay_crypto_init(relay_crypto_t *crypto, const char *key_data, size_t key_data_len, int reverse,
                  int is_hs_v3)
{
    ...
    tor_assert(crypto);
    tor_assert(key_data);
    tor_assert(!(crypto->f_crypto || crypto->b_crypto || crypto->f_digest || crypto->b_digest));
    ...
    crypto->f_crypto = crypto_cipher_new_with_bits(key_data + (2 * digest_len), cipher_key_bits);
    if (!crypto->f_crypto) {
    ...
    crypto->b_crypto =
        crypto_cipher_new_with_bits(key_data + (2 * digest_len) + cipher_key_len, cipher_key_bits);
    if (!crypto->b_crypto) {
