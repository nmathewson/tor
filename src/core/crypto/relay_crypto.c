                   ...
                   char *recognized)
{
    ...
    if (cell_direction == CELL_DIRECTION_IN) {
        if (CIRCUIT_IS_ORIGIN(circ)) { /* We're at the beginning of the circuit.
            ...
            do { /* Remember: cpath is in forward order, that is, first hop
                    first. */
                tor_assert(thishop);
                ...
                relay_header_unpack(&rh, cell->payload);
                if (rh.recognized == 0) {
                    /* it's possibly recognized. have to check digest to be
                     * sure. */
                    if (relay_digest_matches(cpath_get_incoming_digest(thishop),
                                             cell)) {
                        *recognized = 1;
                        *layer_hint = thishop;
                        return 0;
                  ...
                  size_t key_data_len, int reverse, int is_hs_v3)
{
    ...
    crypto_digest_add_bytes(crypto->b_digest, key_data + digest_len,
                            digest_len);
