...
static const char *private_nets[] = {
    ...
    "172.16.0.0/12", "[::]/8",         "[fc00::]/7",  "[fe80::]/10",    "[fec0::]/10",
    "[ff00::]/8",    "[::]/127",       NULL};

static int policies_parse_exit_policy_internal(config_line_t *cfg, smartlist_t **dest,
                                               int ipv6_exit, int rejectprivate,
                                               const smartlist_t *configured_addresses,
                                               int reject_interface_addresses,
                                               int reject_configured_port_addresses,
                                               int add_default_policy, int add_reduced_policy);
...
policy_expand_private(smartlist_t **policy)
{
    ...
    SMARTLIST_FOREACH_BEGIN (*policy, addr_policy_t *, p) {
        ...
        for (i = 0; private_nets[i]; ++i) {
            ...
            if (tor_addr_parse_mask_ports(private_nets[i], 0, &newpolicy.addr, &newpolicy.maskbits,
                                          &port_min, &port_max) < 0) {
...
parse_addr_policy(config_line_t *cfg, smartlist_t **dest, int assume_action)
{
    ...
    for (; cfg; cfg = cfg->next) {
        ...
        SMARTLIST_FOREACH_BEGIN (entries, const char *, ent) {
            ...
            malformed_list = 0;
            item = router_parse_addr_policy_item_from_string(ent, assume_action, &malformed_list);
            if (item) {
...
parse_reachable_addresses(void)
{
    ...
    if (parse_addr_policy(options->ReachableORAddresses ? options->ReachableORAddresses
                                                        : options->ReachableAddresses,
    ...
    if (parse_addr_policy(options->ReachableDirAddresses ? options->ReachableDirAddresses
                                                         : options->ReachableAddresses,
                          &reachable_dir_addr_policy, ADDR_POLICY_ACCEPT)) {
        if (options->ReachableDirAddresses)
            log_warn(LD_CONFIG, "Error parsing ReachableDirAddresses entry; ignoring.");
        ret = -1;
    }
    ...
    if (!ret && (options->ReachableDirAddresses || options->ReachableORAddresses ||
                 options->ReachableAddresses)) {
 ...
 */
static int
addr_policy_permits_tor_addr(const tor_addr_t *addr, uint16_t port, smartlist_t *policy)
{
...
STATIC int
fascist_firewall_allows_address(const tor_addr_t *addr, uint16_t port, smartlist_t *firewall_policy,
...
fascist_firewall_use_ipv6(const or_options_t *options)
{
    ...
    return (options->ClientUseIPv6 == 1 || options->ClientUseIPv4 == 0 ||
            options->ClientPreferIPv6ORPort == 1 || options->ClientPreferIPv6DirPort == 1 ||
                                     ...
                                     int pref_ipv6)
{
    if (fw_connection == FIREWALL_OR_CONNECTION) {
        return fascist_firewall_allows_address(addr, port, reachable_or_addr_policy, pref_only,
                                               pref_ipv6);
    } else if (fw_connection == FIREWALL_DIR_CONNECTION) {
        return fascist_firewall_allows_address(addr, port, reachable_dir_addr_policy, pref_only,
                                               pref_ipv6);
    } else {
 ...
 */
static int
fascist_firewall_allows_address_ap(const tor_addr_port_t *ap, firewall_connection_t fw_connection,
                                   int pref_only, int pref_ipv6)
{
    tor_assert(ap);
    return fascist_firewall_allows_address_addr(&ap->addr, ap->port, fw_connection, pref_only,
                                                pref_ipv6);
}
 ...
 */
static int
fascist_firewall_allows_address_ipv4h(uint32_t ipv4h_or_addr, uint16_t ipv4_or_port,
                                      firewall_connection_t fw_connection, int pref_only,
                                      int pref_ipv6)
{
    tor_addr_t ipv4_or_addr;
    tor_addr_from_ipv4h(&ipv4_or_addr, ipv4h_or_addr);
    return fascist_firewall_allows_address_addr(&ipv4_or_addr, ipv4_or_port, fw_connection,
                                                pref_only, pref_ipv6);
}
 ...
 */
static int
fascist_firewall_allows_base(uint32_t ipv4h_addr, uint16_t ipv4_orport, uint16_t ipv4_dirport,
                             const tor_addr_t *ipv6_addr, uint16_t ipv6_orport,
                             uint16_t ipv6_dirport, firewall_connection_t fw_connection,
                             int pref_only, int pref_ipv6)
{
    if (fascist_firewall_allows_address_ipv4h(
            ipv4h_addr, (fw_connection == FIREWALL_OR_CONNECTION ? ipv4_orport : ipv4_dirport),
            fw_connection, pref_only, pref_ipv6)) {
        return 1;
    }

    if (fascist_firewall_allows_address_addr(
            ipv6_addr, (fw_connection == FIREWALL_OR_CONNECTION ? ipv6_orport : ipv6_dirport),
            fw_connection, pref_only, pref_ipv6)) {
        return 1;
    }
...
static int
fascist_firewall_allows_ri_impl(const routerinfo_t *ri, firewall_connection_t fw_connection,
                                int pref_only, int pref_ipv6)
{
    if (!ri) {
        return 0;
    ...
    return fascist_firewall_allows_base(ri->addr, ri->or_port, ri->dir_port, &ri->ipv6_addr,
                                        ri->ipv6_orport, ri->dir_port, fw_connection, pref_only,
                                        pref_ipv6);
}

/** Like fascist_firewall_allows_rs, but takes pref_ipv6. */
static int
fascist_firewall_allows_rs_impl(const routerstatus_t *rs, firewall_connection_t fw_connection,
                                int pref_only, int pref_ipv6)
{
    if (!rs) {
        return 0;
    ...
    /* Assume IPv4 and IPv6 DirPorts are the same */
    return fascist_firewall_allows_base(rs->addr, rs->or_port, rs->dir_port, &rs->ipv6_addr,
                                        rs->ipv6_orport, rs->dir_port, fw_connection, pref_only,
                                        pref_ipv6);
}
...
int
fascist_firewall_allows_rs(const routerstatus_t *rs, firewall_connection_t fw_connection,
                           int pref_only)
{
    if (!rs) {
        return 0;
    ...
    const or_options_t *options = get_options();
    int pref_ipv6 =
        (fw_connection == FIREWALL_OR_CONNECTION ? fascist_firewall_prefer_ipv6_orport(options)
                                                 : fascist_firewall_prefer_ipv6_dirport(options));

    return fascist_firewall_allows_rs_impl(rs, fw_connection, pref_only, pref_ipv6);
}
 ...
 */
static int
fascist_firewall_allows_md_impl(const microdesc_t *md, firewall_connection_t fw_connection,
                                int pref_only, int pref_ipv6)
{
    if (!md) {
        return 0;
    ...
    return fascist_firewall_allows_address_addr(&md->ipv6_addr, md->ipv6_orport, fw_connection,
                                                pref_only, pref_ipv6);
}
...
int
fascist_firewall_allows_node(const node_t *node, firewall_connection_t fw_connection, int pref_only)
{
    if (!node) {
        return 0;
    }
    ...
    const int pref_ipv6 = (fw_connection == FIREWALL_OR_CONNECTION ? node_ipv6_or_preferred(node)
                                                                   : node_ipv6_dir_preferred(node));

    /* Sometimes, the rs is missing the IPv6 address info, and we need to go
     * all the way to the md */
    if (node->ri &&
        fascist_firewall_allows_ri_impl(node->ri, fw_connection, pref_only, pref_ipv6)) {
        return 1;
    } else if (node->rs &&
               fascist_firewall_allows_rs_impl(node->rs, fw_connection, pref_only, pref_ipv6)) {
        return 1;
    } else if (node->md &&
               fascist_firewall_allows_md_impl(node->md, fw_connection, pref_only, pref_ipv6)) {
        return 1;
    } else {
...
int
fascist_firewall_allows_dir_server(const dir_server_t *ds, firewall_connection_t fw_connection,
                                   int pref_only)
{
    if (!ds) {
        return 0;
    ...
    return fascist_firewall_allows_rs(&ds->fake_status, fw_connection, pref_only);
}
...
fascist_firewall_choose_address_impl(const tor_addr_port_t *a, const tor_addr_port_t *b, int want_a,
                                     firewall_connection_t fw_connection, int pref_only,
                                     int pref_ipv6)
{
    ...
    if (fascist_firewall_allows_address_ap(b, fw_connection, pref_only, pref_ipv6)) {
        use_b = b;
    }
 ...
 * Otherwise, return NULL. */
STATIC const tor_addr_port_t *
fascist_firewall_choose_address(const tor_addr_port_t *a, const tor_addr_port_t *b, int want_a,
                                firewall_connection_t fw_connection, int pref_only, int pref_ipv6)
{
    const tor_addr_port_t *pref =
        fascist_firewall_choose_address_impl(a, b, want_a, fw_connection, 1, pref_ipv6);
    if (pref_only || pref) {
        ...
        return fascist_firewall_choose_address_impl(a, b, want_a, fw_connection, 0, pref_ipv6);
    }
}
...
static void
fascist_firewall_choose_address_base(const tor_addr_t *ipv4_addr, uint16_t ipv4_orport,
                                     uint16_t ipv4_dirport, const tor_addr_t *ipv6_addr,
                                     uint16_t ipv6_orport, uint16_t ipv6_dirport,
                                     firewall_connection_t fw_connection, int pref_only,
                                     int pref_ipv6, tor_addr_port_t *ap)
{
    ...
    ipv4_ap.port = (fw_connection == FIREWALL_OR_CONNECTION ? ipv4_orport : ipv4_dirport);

    tor_addr_port_t ipv6_ap;
    tor_addr_copy(&ipv6_ap.addr, ipv6_addr);
    ipv6_ap.port = (fw_connection == FIREWALL_OR_CONNECTION ? ipv6_orport : ipv6_dirport);

    result = fascist_firewall_choose_address(&ipv4_ap, &ipv6_ap, want_ipv4, fw_connection,
                                             pref_only, pref_ipv6);
...
static void
fascist_firewall_choose_address_ipv4h(uint32_t ipv4h_addr, uint16_t ipv4_orport,
                                      uint16_t ipv4_dirport, const tor_addr_t *ipv6_addr,
                                      uint16_t ipv6_orport, uint16_t ipv6_dirport,
                                      firewall_connection_t fw_connection, int pref_only,
                                      int pref_ipv6, tor_addr_port_t *ap)
{
    ...
    fascist_firewall_choose_address_base(&ipv4_addr, ipv4_orport, ipv4_dirport, ipv6_addr,
                                         ipv6_orport, ipv6_dirport, fw_connection, pref_only,
                                         pref_ipv6, ap);
}
 ...
 */
void
fascist_firewall_choose_address_rs(const routerstatus_t *rs, firewall_connection_t fw_connection,
                                   int pref_only, tor_addr_port_t *ap)
{
    ...
    if (node) {
        ...
        fascist_firewall_choose_address_ipv4h(rs->addr, rs->or_port, rs->dir_port, &rs->ipv6_addr,
                                              rs->ipv6_orport, rs->dir_port, fw_connection,
 ...
 */
void
fascist_firewall_choose_address_ls(const smartlist_t *lspecs, int pref_only, tor_addr_port_t *ap)
{
    ...
    SMARTLIST_FOREACH_BEGIN (lspecs, const link_specifier_t *, ls) {
        ...
        case LS_IPV6:
            ...
            if (have_v6)
                continue;
            tor_addr_from_ipv6_bytes(&addr_v6, link_specifier_getconstarray_un_ipv6_addr(ls));
    ...
    fascist_firewall_choose_address_base(&addr_v4, port_v4, 0, &addr_v6, port_v6, 0,
                                         FIREWALL_OR_CONNECTION, pref_only, pref_ipv6, ap);
}
 ...
 * for pref_ipv6. */
void
fascist_firewall_choose_address_node(const node_t *node, firewall_connection_t fw_connection,
                                     int pref_only, tor_addr_port_t *ap)
{
    tor_assert(ap);
    ...
    const int pref_ipv6_node =
        (fw_connection == FIREWALL_OR_CONNECTION ? node_ipv6_or_preferred(node)
                                                 : node_ipv6_dir_preferred(node));
    ...
    /* Assume the IPv6 OR and Dir addresses are the same. */
    fascist_firewall_choose_address_base(&ipv4_or_ap.addr, ipv4_or_ap.port, ipv4_dir_ap.port,
                                         &ipv6_or_ap.addr, ipv6_or_ap.port, ipv6_dir_ap.port,
                                         fw_connection, pref_only, pref_ipv6_node, ap);
}
                                           ...
                                           tor_addr_port_t *ap)
{
    ...
    fascist_firewall_choose_address_rs(&ds->fake_status, fw_connection, pref_only, ap);
}
...
validate_addr_policies(const or_options_t *options, char **msg)
{
    ...
    *msg = NULL;
    ...
    if (parse_addr_policy(options->AuthDirReject, &addr_policy, ADDR_POLICY_REJECT))
        REJECT("Error in AuthDirReject entry.");
    if (parse_addr_policy(options->AuthDirInvalid, &addr_policy, ADDR_POLICY_REJECT))
        REJECT("Error in AuthDirInvalid entry.");
    if (parse_addr_policy(options->AuthDirBadExit, &addr_policy, ADDR_POLICY_REJECT))
        REJECT("Error in AuthDirBadExit entry.");

    if (parse_addr_policy(options->ReachableAddresses, &addr_policy, ADDR_POLICY_ACCEPT))
        REJECT("Error in ReachableAddresses entry.");
    if (parse_addr_policy(options->ReachableORAddresses, &addr_policy, ADDR_POLICY_ACCEPT))
        REJECT("Error in ReachableORAddresses entry.");
    if (parse_addr_policy(options->ReachableDirAddresses, &addr_policy, ADDR_POLICY_ACCEPT))
 ...
 */
static int
load_policy_from_option(config_line_t *config, const char *option_name, smartlist_t **policy,
                        int assume_action)
{
    int r;
...
{
    int ret = 0;
    if (load_policy_from_option(options->SocksPolicy, "SocksPolicy", &socks_policy, -1) < 0)
        ret = -1;
    if (load_policy_from_option(options->DirPolicy, "DirPolicy", &dir_policy, -1) < 0)
        ret = -1;
    if (load_policy_from_option(options->AuthDirReject, "AuthDirReject", &authdir_reject_policy,
                                ADDR_POLICY_REJECT) < 0)
        ret = -1;
    if (load_policy_from_option(options->AuthDirInvalid, "AuthDirInvalid", &authdir_invalid_policy,
                                ADDR_POLICY_REJECT) < 0)
        ret = -1;
    if (load_policy_from_option(options->AuthDirBadExit, "AuthDirBadExit", &authdir_badexit_policy,
                                ADDR_POLICY_REJECT) < 0)
        ret = -1;
...
HT_GENERATE2(policy_map, policy_map_ent_t, node, policy_hash, policy_eq, 0.6, tor_reallocarray_,
             tor_free_);
                                      ...
                                      const smartlist_t *policy)
{
    ...
    SMARTLIST_FOREACH_BEGIN (policy, addr_policy_t *, tmpe) {
        ...
        /* Address is known */
        if (!tor_addr_compare_masked(addr, &tmpe->addr, tmpe->maskbits, CMP_EXACT)) {
...
static addr_policy_result_t
compare_known_tor_addr_to_addr_policy_noport(const tor_addr_t *addr, const smartlist_t *policy)
{
...
static addr_policy_result_t
compare_unknown_tor_addr_to_addr_policy(uint16_t port, const smartlist_t *policy)
{
    /* We look to see if there's a definite match.  If so, we return that
       match's value, unless there's an intervening possible match that says
       something different. */
...
addr_policy_append_reject_addr(smartlist_t **dest, const tor_addr_t *addr)
{
    ...
    log_debug(LD_CONFIG, "Adding a reject ExitPolicy 'reject %s:*'", fmt_addr(addr));
}
 ...
 */
static void
addr_policy_append_reject_addr_filter(smartlist_t **dest, const tor_addr_t *addr, int ipv4_rules,
                                      int ipv6_rules)
{
    tor_assert(dest);
    tor_assert(addr);
 ...
 * list as needed. */
void
addr_policy_append_reject_addr_list(smartlist_t **dest, const smartlist_t *addrs)
{
    tor_assert(dest);
    tor_assert(addrs);
...
static void
addr_policy_append_reject_addr_list_filter(smartlist_t **dest, const smartlist_t *addrs,
 ...
 */
void
policies_parse_exit_policy_reject_private(smartlist_t **dest, int ipv6_exit,
                                          const smartlist_t *configured_addresses,
                                          int reject_interface_addresses,
                                          int reject_configured_port_addresses)
{
    ...
    if (configured_addresses) {
        addr_policy_append_reject_addr_list_filter(dest, configured_addresses, 1, ipv6_exit);
    }
 ...
 */
static int
policies_parse_exit_policy_internal(config_line_t *cfg, smartlist_t **dest, int ipv6_exit,
                                    int rejectprivate, const smartlist_t *configured_addresses,
                                    int reject_interface_addresses,
                                    ...
                                    int add_reduced_policy)
{
     ...
     * on this exit relay */
    policies_parse_exit_policy_reject_private(dest, ipv6_exit, configured_addresses,
 ...
 */
int
policies_parse_exit_policy(config_line_t *cfg, smartlist_t **dest, exit_policy_parser_cfg_t options,
                           const smartlist_t *configured_addresses)
{
    ...
    int reject_local_interfaces = (options & EXIT_POLICY_REJECT_LOCAL_INTERFACES) ? 1 : 0;
    int add_reduced = (options & EXIT_POLICY_ADD_REDUCED) ? 1 : 0;

    return policies_parse_exit_policy_internal(cfg, dest, ipv6_enabled, reject_private,
                                               configured_addresses, reject_local_interfaces,
                                               reject_local_interfaces, add_default, add_reduced);
}
 ...
 */
int
policies_parse_exit_policy_from_options(const or_options_t *or_options, uint32_t local_address,
                                        const tor_addr_t *ipv6_local_address, smartlist_t **result)
{
    ...
    if (or_options->ExitRelay == 0 || policy_using_default_exit_options(or_options)) {
    ...
    if (or_options->ExitPolicyRejectLocalInterfaces) {
        policies_copy_outbound_addresses_to_smartlist(configured_addresses, or_options);
    }
...
exit_policy_is_general_exit_helper(smartlist_t *policy, int port)
{
    ...
    SMARTLIST_FOREACH_BEGIN (policy, addr_policy_t *, p) {
        ...
        for (i = ((mask & ip) >> 24); i <= (~((mask & ip) ^ mask) >> 24); ++i) {
            ...
            if (tor_addr_is_internal(&addr, 0) && !get_options()->DirAllowPrivateAddresses) {
...
int
policy_is_reject_star(const smartlist_t *policy, sa_family_t family, int default_reject)
{
    if (!policy)
        return default_reject;
    SMARTLIST_FOREACH_BEGIN (policy, const addr_policy_t *, p) {
        if (p->policy_type == ADDR_POLICY_ACCEPT &&
            (tor_addr_family(&p->addr) == family || tor_addr_family(&p->addr) == AF_UNSPEC)) {
            return 0;
        } else if (p->policy_type == ADDR_POLICY_REJECT && p->prt_min <= 1 && p->prt_max == 65535 &&
                   p->maskbits == 0 &&
...
int
policy_write_item(char *buf, size_t buflen, const addr_policy_t *policy, int format_for_desc)
{
    size_t written = 0;
    ...
    result = tor_snprintf(buf, buflen, "%s%s %s", is_accept ? "accept" : "reject",
    ...
    if (policy->prt_min <= 1 && policy->prt_max == 65535) {
        ...
        result = tor_snprintf(buf + written, buflen - written, ":%d-%d", policy->prt_min,
                              policy->prt_max);
        if (result < 0)
            return -1;
        written += result;
...
#define REJECT_CUTOFF_COUNT_IPV4 (UINT64_C(1) << (IPV4_BITS - REJECT_CUTOFF_SCALE_IPV4 - 7))
 ...
 */
#define REJECT_CUTOFF_COUNT_IPV6 (UINT64_C(1) << (IPV6_BITS - REJECT_CUTOFF_SCALE_IPV6 - 16))
 ...
 */
static void
policy_summary_accept(smartlist_t *summary, uint16_t prt_min, uint16_t prt_max, sa_family_t family)
{
    tor_assert_nonfatal_once(family == AF_INET || family == AF_INET6);
    uint64_t family_reject_count =
        ((family == AF_INET) ? REJECT_CUTOFF_COUNT_IPV4 : REJECT_CUTOFF_COUNT_IPV6);
...
static void
policy_summary_reject(smartlist_t *summary, maskbits_t maskbits, uint16_t prt_min, uint16_t prt_max,
                      sa_family_t family)
{
    tor_assert_nonfatal_once(family == AF_INET || family == AF_INET6);
     ...
     */
    int scale = ((family == AF_INET) ? REJECT_CUTOFF_SCALE_IPV4 : REJECT_CUTOFF_SCALE_IPV6);
...
policy_summary_add_item(smartlist_t *summary, addr_policy_t *p)
{
    if (p->policy_type == ADDR_POLICY_ACCEPT) {
        ...
        for (i = 0; private_nets[i]; ++i) {
            tor_addr_t addr;
            maskbits_t maskbits;
            if (tor_addr_parse_mask_ports(private_nets[i], 0, &addr, &maskbits, NULL, NULL) < 0) {
                tor_assert(0);
            }
            if (tor_addr_compare(&p->addr, &addr, CMP_EXACT) == 0 && p->maskbits == maskbits) {
                is_private = 1;
                break;
            }
        }
        ...
        if (!is_private) {
            policy_summary_reject(summary, p->maskbits, p->prt_min, p->prt_max, p->addr.family);
        }
    } else
        tor_assert(0);
}
...
parse_short_policy(const char *summary)
{
    ...
    if (n_entries == 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_DIR, "Found no port-range entries in summary %s",
               escaped(orig_summary));
        return NULL;
    }
    ...
    {
        size_t size =
            offsetof(short_policy_t, entries) + sizeof(short_policy_entry_t) * (n_entries);
...
short_policy_is_reject_star(const short_policy_t *policy)
{
    ...
    return (policy->is_accept == 0 && policy->n_entries == 1 && policy->entries[0].min_port == 1 &&
 ...
 * interpretation. */
addr_policy_result_t
compare_tor_addr_to_node_policy(const tor_addr_t *addr, uint16_t port, const node_t *node)
{
    if (node->rejects_all)
        return ADDR_POLICY_REJECTED;
    ...
    if (node->ri) {
        ...
        if (node->md->exit_policy == NULL)
            ...
            return compare_tor_addr_to_short_policy(addr, port, node->md->exit_policy);
    } else {
 ...
 */
char *
policy_dump_to_string(const smartlist_t *policy_list, int include_ipv4, int include_ipv6)
{
 ...
 * about "exit-policy/..." */
int
getinfo_helper_policies(control_connection_t *conn, const char *question, char **answer,
                        const char **errmsg)
{
    (void)conn;
    (void)errmsg;
    if (!strcmp(question, "exit-policy/default")) {
        ...
        *answer = smartlist_join_strings(private_policy_strings, ",", 0, NULL);
        ...
        if (!options->ExitPolicyRejectPrivate && !options->ExitPolicyRejectLocalInterfaces) {
            *answer = tor_strdup("");
            return 0;
        }
        ...
        if (options->ExitPolicyRejectLocalInterfaces) {
            policies_copy_outbound_addresses_to_smartlist(configured_addresses, options);
        }

        policies_parse_exit_policy_reject_private(
            &private_policy_list, options->IPv6Exit, configured_addresses,
            options->ExitPolicyRejectLocalInterfaces, options->ExitPolicyRejectLocalInterfaces);
