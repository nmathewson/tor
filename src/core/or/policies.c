...
policy_expand_unspec(smartlist_t **policy)
{
    ...
    SMARTLIST_FOREACH_BEGIN (*policy, addr_policy_t *, p) {
        ...
        if (family == AF_INET6 || family == AF_INET || p->is_private) {
            ...
            if (p->maskbits != 0) {
                log_warn(LD_BUG, "AF_UNSPEC policy with maskbits==%d",
                         p->maskbits);
            ...
            log_warn(LD_BUG, "Funny-looking address policy with family %d",
                     family);
            smartlist_add(tmp, p);
        }
    } SMARTLIST_FOREACH_END (p);
    ...
    for (; cfg; cfg = cfg->next) {
        ...
        SMARTLIST_FOREACH_BEGIN (entries, const char *, ent) {
            ...
            item = router_parse_addr_policy_item_from_string(
                ent, assume_action, &malformed_list);
            if (item) {
                ...
                /* the error is minor: don't add the item, but keep processing
                 * the rest of the policies in the list */
                log_debug(LD_CONFIG,
                             ...
                             int pref_only)
{
    ...
    if (node->ri && fascist_firewall_allows_ri_impl(node->ri, fw_connection,
        ...
        return 1;
               ...
               fascist_firewall_allows_rs_impl(node->rs, fw_connection,
                                               pref_only, pref_ipv6)) {
        return 1;
    } else if (node->md &&
               fascist_firewall_allows_md_impl(node->md, fw_connection,
                                               pref_only, pref_ipv6)) {
        return 1;
    } else {
        /* If we know nothing, assume it's unreachable, we'll never get an
                                ...
                                int pref_only, int pref_ipv6)
{
    ...
    if (pref_only || pref) {
        /* If there is a preferred address, use it. If we can only use
         * preferred addresses, and neither address is preferred, pref will be
         * NULL, and we want to return NULL, so return it. */
        return pref;
    } else {
        /* If there's no preferred address, and we can return addresses that
         * are not preferred, use an address that's allowed */
        return fascist_firewall_choose_address_impl(
            a, b, want_a, fw_connection, 0, pref_ipv6);
    }
}
                                   ...
                                   int pref_only, tor_addr_port_t *ap)
{
    ...
    if (node) {
        fascist_firewall_choose_address_node(node, fw_connection, pref_only,
                                             ap);
    } else {
        ...
        fascist_firewall_choose_address_ipv4h(
            ...
            rs->ipv6_orport, rs->dir_port, fw_connection, pref_only, pref_ipv6,
            ap);
    }
}
                                   ...
                                   tor_addr_port_t *ap)
{
    ...
    SMARTLIST_FOREACH_BEGIN (lspecs, const link_specifier_t *, ls) {
        ...
        case LS_IPV4:
            ...
            break;
        case LS_IPV6:
            /* Skip if we already seen a v6, or deliberately skip it if we're
             * not a direct connection. */
            if (have_v6)
                continue;
            tor_addr_from_ipv6_bytes(
                &addr_v6, link_specifier_getconstarray_un_ipv6_addr(ls));
     ...
     */
    fascist_firewall_choose_address_base(&addr_v4, port_v4, 0, &addr_v6,
                                         port_v6, 0, FIREWALL_OR_CONNECTION,
                                           ...
                                           int pref_only, tor_addr_port_t *ap)
{
     ...
     * addresses/ports in both fake_status and dir_server_t, this works fine.
     * (See #17867.)
     * This function relies on fascist_firewall_choose_address_rs looking up
     * the
        ...
        if (!tor_addr_compare_masked(addr, &tmpe->addr, tmpe->maskbits,
            ...
            if (tmpe->prt_min <= 1 && tmpe->prt_max >= 65535) {
                ...
                if (tmpe->policy_type == ADDR_POLICY_ACCEPT) {
                    /* If we already hit a clause that might trigger a
                     * 'reject', than we can't be sure of this certain
                     * 'accept'.*/
...
append_exit_policy_string(smartlist_t **policy, const char *more)
{
    ...
    if (parse_addr_policy(&tmp, policy, -1) < 0) {
        log_warn(LD_BUG, "Unable to parse internally generated policy %s",
                 more);
    }
}
                                           ...
                                           int ipv4_rules, int ipv6_rules)
{
    ...
    SMARTLIST_FOREACH_BEGIN (addrs, tor_addr_t *, addr) {
        addr_policy_append_reject_addr_filter(dest, addr, ipv4_rules,
                                              ipv6_rules);
    } SMARTLIST_FOREACH_END (addr);
}
...
exit_policy_remove_redundancies(smartlist_t *dest)
{
    ...
    for (i = 0; i < smartlist_len(dest) - 1; ++i) {
        ap = smartlist_get(dest, i);
        for (j = i + 1; j < smartlist_len(dest); ++j) {
            // tor_assert(j > i); // j starts out at i+1; j only increases; i
            // only
            //                    // decreases.
            tmp = smartlist_get(dest, j);
    ...
    int reject_interface_addresses, int reject_configured_port_addresses)
{
    ...
    if (reject_configured_port_addresses) {
        ...
        SMARTLIST_FOREACH_BEGIN (port_addrs, port_cfg_t *, port) {
            ...
            if (!port->is_unix_addr) {
                addr_policy_append_reject_addr_filter(dest, &port->addr, 1,
                                                      ipv6_exit);
            }
    ...
    if (reject_interface_addresses) {
        ...
        addr_policy_append_reject_addr_list_filter(dest, public_addresses, 1,
                                                   0);
        ...
        if (ipv6_exit) {
            ...
            addr_policy_append_reject_addr_list_filter(dest, public_addresses,
                                                       0, 1);
            interface_address6_list_free(public_addresses);
        }
    }
...
#define REDUCED_EXIT_POLICY                                                  \
    ...
    "accept *:749,accept *:873,accept *:902-904,accept *:981,accept "        \
    "*:989-995,"                                                             \
                                    ...
                                    int add_reduced_policy)
{
    ...
    /* Consider rejecting IPv4 and IPv6 advertised relay addresses, outbound
     * bind addresses, publicly routable addresses, and configured port
     * addresses on this exit relay */
    policies_parse_exit_policy_reject_private(
                                        ...
                                        smartlist_t **result)
{
    ...
    if (or_options->ExitPolicyRejectPrivate) {
        ...
        policies_copy_addr_to_smartlist(configured_addresses,
                                        ipv6_local_address);
    }
...
exit_policy_is_general_exit_helper(smartlist_t *policy, int port)
{
    ...
    SMARTLIST_FOREACH_BEGIN (policy, addr_policy_t *, p) {
        ...
        for (i = ((mask & ip) >> 24); i <= (~((mask & ip) ^ mask) >> 24);
             ++i) {
            tor_addr_t addr;
                  ...
                  int format_for_desc)
{
    ...
    if (policy->prt_min <= 1 && policy->prt_max == 65535) {
        ...
        result = tor_snprintf(buf + written, buflen - written, ":%d",
                              policy->prt_min);
        if (result < 0)
            return -1;
        written += result;
                      ...
                      uint16_t prt_min, uint16_t prt_max, sa_family_t family)
{
    ...
    if (maskbits > (addrbits - scale)) {
        ...
        /* The address range is so small, we'd need billions of them to reach
         * the rejection limit. So we ignore this range in the reject count. */
        return;
    }
    ...
    if (addrbits - scale - maskbits >= 64) {
        tor_assert_nonfatal_once(family == AF_INET6);
        /* The address range is so large, it's an automatic rejection for all
         * ports in the range. */
        count = UINT64_MAX;
    } else {
    ...
    while (i < smartlist_len(summary) && AT(i)->prt_max <= prt_max) {
        if (AT(i)->reject_count <= UINT64_MAX - count) {
            ...
            /* IPv4 would require a 4-billion address redundant policy to get
             * here, but IPv6 just needs to have ::/0 */
            if (family == AF_INET) {
...
policy_summary_add_item(smartlist_t *summary, addr_policy_t *p)
{
    if (p->policy_type == ADDR_POLICY_ACCEPT) {
        ...
        for (i = 0; private_nets[i]; ++i) {
            tor_addr_t addr;
            maskbits_t maskbits;
            if (tor_addr_parse_mask_ports(private_nets[i], 0, &addr, &maskbits,
                                          NULL, NULL) < 0) {
                tor_assert(0);
            }
    ...
    while (1) {
        ...
        if (last || AT(i)->accepted != AT(i + 1)->accepted) {
            ...
            if (start_prt == AT(i)->prt_max)
                ...
                tor_snprintf(buf, sizeof(buf), "%d-%d", start_prt,
                             AT(i)->prt_max);
...
parse_short_policy(const char *summary)
{
    ...
    for (; *summary; summary = next) {
        if (n_entries == MAX_EXITPOLICY_SUMMARY_LEN) {
            log_fn(LOG_PROTOCOL_WARN, LD_DIR,
                   "Impossibly long policy summary %s", escaped(orig_summary));
            return NULL;
        }
        ...
        case '\0':
            ...
            break;
        case '-':
            high = (unsigned)tor_parse_ulong(next + 1, 10, low, 65535, &ok,
                                             &next);
            if (!ok)
                goto bad_ent;
...
write_short_policy(const short_policy_t *policy)
{
    ...
    smartlist_add_asprintf(sl, "%s",
                           policy->is_accept ? "accept " : "reject ");
                      ...
                      int include_ipv6)
{
    ...
    SMARTLIST_FOREACH_BEGIN (policy_list, addr_policy_t *, tmpe) {
        ...
        bytes_written_to_pbuf =
            policy_write_item(pbuf, POLICY_BUF_LEN, tmpe, 1);
    ...
    if (!strcmp(question, "exit-policy/default")) {
        *answer = tor_strdup(DEFAULT_EXIT_POLICY);
        ...
        while (*priv != NULL) {
            ...
            smartlist_add_asprintf(private_policy_strings, "reject %s:*",
                                   *priv);
            priv++;
        }
    ...
    if (!HT_EMPTY(&policy_root)) {
        ...
        HT_FOREACH (ent, policy_map, &policy_root) {
            ...
            if (policy_write_item(buf, sizeof(buf), (*ent)->policy, 0) >= 0)
                log_warn(LD_MM, "  %d [%d]: %s", n, (*ent)->policy->refcnt,
                         buf);
        }
    }
