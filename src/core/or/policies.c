                                ...
                                int pref_only, int pref_ipv6)
{
    ...
    if (pref_only || pref) {
         ...
         * addresses, and neither address is preferred, pref will be NULL, and
         * we want to return NULL, so return it. */
        return pref;
    } else {
    ...
    SMARTLIST_FOREACH_BEGIN(lspecs, const link_specifier_t *, ls)
    {
        switch (link_specifier_get_ls_type(ls)) {
            case LS_IPV4:
                ...
                break;
            case LS_IPV6:
                /* Skip if we already seen a v6, or deliberately skip it if
                 * we're not a direct connection. */
                if (have_v6)
                    continue;
                tor_addr_from_ipv6_bytes(
                    &addr_v6,
                    (const char *)link_specifier_getconstarray_un_ipv6_addr(
                        ls));
                                     ...
                                     int pref_only, tor_addr_port_t *ap)
{
     ...
     * fascist_firewall_choose_address_rs() on the fake (hard-coded)
     * routerstatus for the node. If it is not hard-coded, check that the node
     * has a microdescriptor, full descriptor (routerinfo), or is one of our
     * configured bridges before calling this function. */
        ...
        if (!tor_addr_compare_masked(addr, &tmpe->addr, tmpe->maskbits,
            ...
            if (tmpe->prt_min <= 1 && tmpe->prt_max >= 65535) {
                ...
                if (tmpe->policy_type == ADDR_POLICY_ACCEPT) {
                    /* If we already hit a clause that might trigger a 'reject',
                     * than we can't be sure of this certain 'accept'.*/
