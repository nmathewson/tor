 ...
 */
static int
circuit_is_acceptable(const origin_circuit_t *origin_circ, const entry_connection_t *conn,
                      int must_be_open, uint8_t purpose, int need_uptime, int need_internal,
                      time_t now)
{
    ...
    if (purpose == CIRCUIT_PURPOSE_C_GENERAL || purpose == CIRCUIT_PURPOSE_C_HSDIR_GET ||
        purpose == CIRCUIT_PURPOSE_S_HSDIR_POST || purpose == CIRCUIT_PURPOSE_HS_VANGUARDS ||
    ...
    if (purpose == CIRCUIT_PURPOSE_C_GENERAL || purpose == CIRCUIT_PURPOSE_S_HSDIR_POST ||
        ...
        if (build_state->onehop_tunnel) {
            ...
            if (build_state->chosen_exit) {
                ...
                if (tor_digest_is_zero(digest)) {
                    ...
                    if (family < 0 || !tor_addr_eq(&build_state->chosen_exit->addr, &addr) ||
        ...
        if (origin_circ->prepend_policy) {
            if (tor_addr_parse(&addr, conn->socks_request->address) != -1) {
                int r = compare_tor_addr_to_addr_policy(&addr, conn->socks_request->port,
                  ...
                  const entry_connection_t *conn)
{
    ...
    case CIRCUIT_PURPOSE_C_GENERAL:
        ...
        if (b->timestamp_dirty) {
            ...
            if (a->timestamp_dirty || timercmp(&a->timestamp_began, &b->timestamp_began, OP_GT))
                return 1;
    ...
    a_bits = connection_edge_update_circuit_isolation(conn, (origin_circuit_t *)oa, 1);
    b_bits = connection_edge_update_circuit_isolation(conn, (origin_circuit_t *)ob, 1);
...
circuit_get_best(const entry_connection_t *conn, int must_be_open, uint8_t purpose, int need_uptime,
                 int need_internal)
{
    ...
    tor_assert(purpose == CIRCUIT_PURPOSE_C_GENERAL || purpose == CIRCUIT_PURPOSE_HS_VANGUARDS ||
               purpose == CIRCUIT_PURPOSE_C_HSDIR_GET || purpose == CIRCUIT_PURPOSE_S_HSDIR_POST ||
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        ...
        if (!circuit_is_acceptable(origin_circ, conn, must_be_open, purpose, need_uptime,
                                   need_internal, (time_t)now.tv_sec))
            continue;
...
count_pending_general_client_circuits(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        if (circ->marked_for_close || circ->state == CIRCUIT_STATE_OPEN ||
            !CIRCUIT_PURPOSE_COUNTS_TOWARDS_MAXPENDING(circ->purpose) || !CIRCUIT_IS_ORIGIN(circ))
            continue;
...
circuit_expire_building(void)
{
    ...
    struct timeval general_cutoff, begindir_cutoff, fourhop_cutoff, close_cutoff,
        extremely_old_cutoff, hs_extremely_old_cutoff, cannibalized_cutoff, c_intro_cutoff,
        s_intro_cutoff, stream_cutoff;
...
#define SET_CUTOFF(target, msec)                  \
    ...
    SET_CUTOFF(cannibalized_cutoff, MAX(get_circuit_build_close_time_ms() * (4 / 6.0),
    ...
    SET_CUTOFF(hs_extremely_old_cutoff,
               MAX(get_circuit_build_close_time_ms() * 2 + 1000, options->SocksTimeout * 1000));
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, victim) {
        ...
        if (build_state && build_state->onehop_tunnel)
            ...
            cutoff = close_cutoff;
        else if (TO_ORIGIN_CIRCUIT(victim)->has_opened && victim->state != CIRCUIT_STATE_OPEN)
      ...
      if (!victim->timestamp_dirty)
        ...
        if (victim->state == CIRCUIT_STATE_OPEN) {
            ...
            case CIRCUIT_PURPOSE_C_REND_READY:
                 ...
                 */
                if (TO_ORIGIN_CIRCUIT(victim)->rend_data || TO_ORIGIN_CIRCUIT(victim)->hs_ident ||
            ...
            if (circuit_timeout_want_to_count_circ(TO_ORIGIN_CIRCUIT(victim)) &&
                ...
                log_info(LD_CIRC, "Deciding to count the timeout for circuit %" PRIu32,
                         TO_ORIGIN_CIRCUIT(victim)->global_identifier);

                /* Circuits are allowed to last longer for measurement.
                 * Switch their purpose and wait. */
                if (victim->purpose != CIRCUIT_PURPOSE_C_MEASURE_TIMEOUT) {
                    circuit_build_times_mark_circ_as_measurement_only(TO_ORIGIN_CIRCUIT(victim));
                    continue;
                }
                ...
                if (timercmp(&victim->timestamp_began, &extremely_old_cutoff, OP_LT)) {
                    log_notice(LD_CIRC,
                               ...
                               (long)(now.tv_sec - victim->timestamp_began.tv_sec), victim->purpose,
                               circuit_purpose_to_string(victim->purpose));
        ...
        if (victim->n_chan)
            log_info(LD_CIRC,
                     ...
                     (unsigned)victim->n_circ_id, TO_ORIGIN_CIRCUIT(victim)->has_opened,
                     victim->state, circuit_state_to_string(victim->state), victim->purpose,
                     TO_ORIGIN_CIRCUIT(victim)->build_state
                         ? TO_ORIGIN_CIRCUIT(victim)->build_state->desired_path_len
                         : -1);
        else
            log_info(LD_CIRC, "Abandoning circ %u %u (state %d,%d:%s, purpose %d, len %d)",
                     TO_ORIGIN_CIRCUIT(victim)->global_identifier, (unsigned)victim->n_circ_id,
...
{
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_origin_circuit_list(), origin_circuit_t *, circ) {
...
#define MAX_ANCIENT_ONEHOP_CIRCUITS_TO_LOG 10
    ...
    SMARTLIST_FOREACH_BEGIN (log_these, const origin_circuit_t *, ocirc) {
        ...
        if (circ->timestamp_dirty) {
            ...
            tor_asprintf(&dirty, "Dirty since %s (%ld seconds vs %ld-second cutoff)", dirty_since,
                         (long)(now - circ->timestamp_dirty), (long)options->MaxCircuitDirtiness);
        } else {
        ...
        log_notice(LD_HEARTBEAT,
                   ...
                   circ->marked_for_close ? "Marked" : "Not marked", circ->package_window,
        ...
        for (conn = ocirc->p_streams; conn; conn = conn->next_stream) {
            ...
            log_notice(LD_HEARTBEAT,
                       ...
                       conn_type_to_string(c->type), conn_state_to_string(c->type, c->state),
                       c->purpose, c->marked_for_close ? "Marked" : "Not marked",
                       c->marked_for_close_file ? c->marked_for_close_file : "--",
                       c->marked_for_close, c->hold_open_until_flushed ? "" : "not ");
        }
 ...
 */
int
circuit_stream_is_being_handled(entry_connection_t *conn, uint16_t port, int min)
{
    const node_t *exitnode;
    int num = 0;
    time_t now = time(NULL);
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        if (CIRCUIT_IS_ORIGIN(circ) && !circ->marked_for_close &&
            ...
            if (origin_circ->isolation_values_set &&
                (conn == NULL || !connection_edge_compatible_with_circuit(conn, origin_circ)))
                continue;
...
circuit_is_available_for_use(const circuit_t *circ)
{
    ...
    if (circ->purpose != CIRCUIT_PURPOSE_C_GENERAL && circ->purpose != CIRCUIT_PURPOSE_HS_VANGUARDS)
...
{
    return (!circuit_all_predicted_ports_handled(now, needs_uptime, needs_capacity) &&
...
STATIC int
needs_hs_client_circuits(time_t now, int *needs_uptime, int *needs_capacity, int num_internal,
                         int num_uptime_internal)
{
    int used_internal_recently = rep_hist_get_predicted_internal(now, needs_uptime, needs_capacity);
    int requires_uptime =
        num_uptime_internal < SUFFICIENT_UPTIME_INTERNAL_HS_CLIENTS && needs_uptime;
...
{
    if (router_have_consensus_path() != CONSENSUS_PATH_UNKNOWN) {
        if (num < networkstatus_get_param(NULL, "cbtmaxopencircs", DFLT_CBT_UNUSED_OPEN_CIRCS,
...
circuit_predict_and_launch_new(void)
{
    ...
    if (needs_exit_circuits(now, &port_needs_uptime, &port_needs_capacity)) {
        ...
        log_info(LD_CIRC, "Have %d clean circs (%d internal), need another exit circ.", num,
                 num_internal);
        circuit_launch(CIRCUIT_PURPOSE_C_GENERAL, flags);
        return;
    }
    ...
    if (needs_hs_client_circuits(now, &hidserv_needs_uptime, &hidserv_needs_capacity, num_internal,
                                 num_uptime_internal)) {
    ...
    if (needs_circuits_for_build(num)) {
        ...
        log_info(LD_CIRC, "Have %d clean circs need another buildtime test circ.", num);
        circuit_launch(CIRCUIT_PURPOSE_C_GENERAL, flags);
        return;
    }
}
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        ...
        if (circ->timestamp_dirty &&
            ...
            log_debug(LD_CIRC,
                      ...
                      "purpose %d)",
                      (unsigned)circ->n_circ_id, (long)(now.tv_sec - circ->timestamp_dirty),
                      circ->purpose);
            ...
            if (timercmp(&circ->timestamp_began, &cutoff, OP_LT)) {
                if (circ->purpose == CIRCUIT_PURPOSE_C_GENERAL ||
                    ...
                    log_info(LD_CIRC,
                             "Closing circuit %" PRIu32 " that has been unused for %ld msec.",
...
circuit_expire_old_circuits_serverside(time_t now)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
         ...
         */
        if (or_circ->p_chan && channel_is_client(or_circ->p_chan) && !circ->n_chan &&
            !or_circ->n_streams && !or_circ->resolving_streams && !or_circ->rend_splice &&
...
circuit_enough_testing_circs(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        if (!circ->marked_for_close && CIRCUIT_IS_ORIGIN(circ) &&
            circ->purpose == CIRCUIT_PURPOSE_TESTING && circ->state == CIRCUIT_STATE_OPEN)
            num++;
    } SMARTLIST_FOREACH_END (circ);
...
{
    if (have_performed_bandwidth_test || !check_whether_orport_reachable(get_options())) {
...
circuit_testing_failed(origin_circuit_t *circ, int at_last_hop)
{
    ...
    log_info(LD_GENERAL, "Our testing circuit (to see if your ORPort is reachable) "
...
circuit_build_failed(origin_circuit_t *circ)
{
    ...
    if (circ->cpath && circ->cpath->state != CPATH_STATE_OPEN && !circ->base_.received_destroy) {
...
{
    return (purpose >= CIRCUIT_PURPOSE_C_HS_MIN_ && purpose <= CIRCUIT_PURPOSE_C_HS_MAX_);
}
...
{
    return (purpose >= CIRCUIT_PURPOSE_S_HS_MIN_ && purpose <= CIRCUIT_PURPOSE_S_HS_MAX_);
}
...
{
    return (CIRCUIT_IS_ORIGIN(circ) && (CONST_TO_ORIGIN_CIRCUIT(circ)->rend_data != NULL));
}
...
{
    return (CIRCUIT_IS_ORIGIN(circ) && (CONST_TO_ORIGIN_CIRCUIT(circ)->hs_ident != NULL));
}
 ...
 */
static int
circuit_should_cannibalize_to_build(uint8_t purpose_to_build, int has_extend_info,
                                    int onehop_tunnel)
{
...
origin_circuit_t *
circuit_launch_by_extend_info(uint8_t purpose, extend_info_t *extend_info, int flags)
{
    ...
    if (!onehop_tunnel && (!router_have_minimum_dir_info() || !have_path)) {
        log_debug(LD_CIRC,
                  ...
                  "circuit launch.",
                  !router_have_minimum_dir_info() ? "fetched enough directory info"
     ...
     * then do so. */
    if (circuit_should_cannibalize_to_build(purpose, extend_info != NULL, onehop_tunnel)) {
        ...
        if (circ) {
            ...
            control_event_circuit_cannibalized(circ, old_purpose, &old_timestamp_began);
            ...
            case CIRCUIT_PURPOSE_S_ESTABLISH_INTRO:
                ...
                break;
            default:
                log_warn(LD_BUG, "unexpected purpose %d when cannibalizing a circ.", purpose);
                tor_fragile_assert();
                return NULL;
            }
            return circ;
    ...
    if (did_circs_fail_last_period && n_circuit_failures > MAX_CIRCUIT_FAILURES) {
 ...
 */
static int
circuit_get_open_circ_or_launch(entry_connection_t *conn, uint8_t desired_circuit_purpose,
                                origin_circuit_t **circp)
{
    ...
    check_exit_policy = conn->socks_request->command == SOCKS_COMMAND_CONNECT &&
                        !conn->use_begindir &&
    ...
    need_uptime =
        !conn->want_onehop && !conn->use_begindir &&
        smartlist_contains_int_as_string(options->LongLivedPorts, conn->socks_request->port);
    ...
    circ = circuit_get_best(conn, 1 /* Insist on open circuits */, desired_circuit_purpose,
        ...
        if (!connection_get_by_type(CONN_TYPE_DIR)) {
            ...
            if (entry_list_is_constrained(options)) {
                ...
                log_fn(severity, LD_APP | LD_DIR,
                       ...
                       !router_have_minimum_dir_info() ? "used client functionality lately"
                                                       : "received a consensus with exits",
    ...
    circ = circuit_get_best(conn, 0 /* don't insist on open circuits */, desired_circuit_purpose,
                            need_uptime, need_internal);
    if (circ)
        ...
        if (desired_circuit_purpose == CIRCUIT_PURPOSE_C_GENERAL ||
            ...
            if (conn->chosen_exit_name) {
                ...
                if (r && node_has_preferred_descriptor(r, conn->want_onehop ? 1 : 0)) {
                    ...
                    log_debug(LD_DIR, "considering %d, %s", want_onehop, conn->chosen_exit_name);
        ...
        if (desired_circuit_purpose == CIRCUIT_PURPOSE_C_GENERAL ||
            ...
            if (circ) {
                ...
                if (edge_conn->rend_data) {
                    ...
                    circ->hs_ident = hs_ident_circuit_new(&edge_conn->hs_ident->identity_pk);
                }
 ...
 */
static void
link_apconn_to_circ(entry_connection_t *apconn, origin_circuit_t *circ, crypt_path_t *cpath)
{
    ...
    if (optimistic_data_enabled() && (circ->base_.purpose == CIRCUIT_PURPOSE_C_GENERAL ||
...
hostname_in_track_host_exits(const or_options_t *options, const char *address)
{
    ...
    SMARTLIST_FOREACH_BEGIN (options->TrackHostExits, const char *, cp) {
        if (cp[0] == '.') { /* match end */
            if (cp[1] == '\0' || !strcasecmpend(address, cp) || !strcasecmp(address, &cp[1]))
                return 1;
 ...
 */
static void
consider_recording_trackhost(const entry_connection_t *conn, const origin_circuit_t *circ)
{
    const or_options_t *options = get_options();
    ...
    if (!options->TrackHostExits ||
        addressmap_have_mapping(conn->socks_request->address, options->TrackHostExitsExpire))
    ...
    base16_encode(fp, sizeof(fp), circ->build_state->chosen_exit->identity_digest, DIGEST_LEN);

    /* Add this exit/hostname pair to the addressmap. */
    tor_asprintf(&new_address, "%s.%s.exit", conn->socks_request->address, fp);

    addressmap_register(conn->socks_request->address, new_address,
                        time(NULL) + options->TrackHostExitsExpire, ADDRMAPSRC_TRACKEXIT, 0, 0);
}
 ...
 * <b>cpath</b> is NULL. */
int
connection_ap_handshake_attach_chosen_circuit(entry_connection_t *conn, origin_circuit_t *circ,
                                              crypt_path_t *cpath)
{
...
connection_ap_get_nonrend_circ_purpose(const entry_connection_t *conn)
{
    ...
    tor_assert_nonfatal(!connection_edge_is_rendezvous_stream(ENTRY_TO_EDGE_CONN(conn)));

    if (base_conn->linked_conn && base_conn->linked_conn->type == CONN_TYPE_DIR) {
        ...
        } else if (base_conn->linked_conn->purpose == DIR_PURPOSE_FETCH_RENDDESC_V2 ||
...
connection_ap_handshake_attach_circuit(entry_connection_t *conn)
{
    ...
    if (conn_age >= get_options()->SocksTimeout) {
        int severity =
            (tor_addr_is_null(&base_conn->addr) && !base_conn->port) ? LOG_INFO : LOG_NOTICE;
        log_fn(severity, LD_APP, "Tried for %d seconds to get a connection to %s:%d. Giving up.",
               conn_age, safe_str_client(conn->socks_request->address), conn->socks_request->port);
        return -1;
    }
    ...
    if (!connection_edge_is_rendezvous_stream(ENTRY_TO_EDGE_CONN(conn))) {
        ...
        if (conn->chosen_exit_name) {
            ...
            if (!node && !want_onehop) {
                ...
                log_fn(opt ? LOG_INFO : LOG_WARN, LD_APP,
                       "Requested exit point '%s' is not known. %s.", conn->chosen_exit_name,
                       opt ? "Trying others" : "Closing");
                if (opt) {
         ...
         * launch it
         */
        retval = circuit_get_open_circ_or_launch(conn, connection_ap_get_nonrend_circ_purpose(conn),
                                                 &circ);
        ...
        log_debug(LD_APP | LD_CIRC, "Attaching apconn to circ %u (stream %d sec old).",
        ...
        if (retval > 0) {
            ...
            log_info(LD_REND,
                     ...
                     (unsigned)TO_CIRCUIT(rendcirc)->n_circ_id, rendcirc->global_identifier,
                     conn_age);
        ...
        retval =
            circuit_get_open_circ_or_launch(conn, CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT, &introcirc);
        if (retval < 0)
            return -1; /* failed */
        ...
        if (retval > 0) {
            ...
            log_info(LD_REND,
                     ...
                     (unsigned)TO_CIRCUIT(introcirc)->n_circ_id, introcirc->global_identifier,
        ...
        if (rendcirc && introcirc && rendcirc->base_.purpose == CIRCUIT_PURPOSE_C_REND_READY) {
            log_info(LD_REND,
...
circuit_change_purpose(circuit_t *circ, uint8_t new_purpose)
{
    ...
    tor_assert(!!(CIRCUIT_IS_ORIGIN(circ)) == !!(CIRCUIT_PURPOSE_IS_ORIGIN(new_purpose)));
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        ...
        log_debug(LD_CIRC,
                  ...
                  "from \"%s\" (%d) to \"%s\" (%d)",
                  TO_ORIGIN_CIRCUIT(circ)->global_identifier, old_purpose_desc, circ->purpose,
                  circuit_purpose_to_string(new_purpose), new_purpose);
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        control_event_circuit_purpose_changed(TO_ORIGIN_CIRCUIT(circ), old_purpose);
...
circuit_sent_valid_data(origin_circuit_t *circ, uint16_t relay_body_len)
{
    ...
    tor_assertf_nonfatal(relay_body_len <= RELAY_PAYLOAD_SIZE,
                         "Wrong relay_body_len: %d (should be at most %d)", relay_body_len,
                         RELAY_PAYLOAD_SIZE);
    ...
    circ->n_overhead_written_circ_bw =
        tor_add_u32_nowrap(circ->n_overhead_written_circ_bw, RELAY_PAYLOAD_SIZE - relay_body_len);
}
...
circuit_read_valid_data(origin_circuit_t *circ, uint16_t relay_body_len)
{
    ...
    circ->n_overhead_read_circ_bw =
        tor_add_u32_nowrap(circ->n_overhead_read_circ_bw, RELAY_PAYLOAD_SIZE - relay_body_len);
}
