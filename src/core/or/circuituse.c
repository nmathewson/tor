        ...
        case CIRCUIT_PURPOSE_C_GENERAL:
            ...
            if (b->timestamp_dirty) {
                ...
                if (ob->build_state->is_internal)
                     ...
                     * think we can get rid of it. circuit_is_acceptable()
                     * already makes sure that is_internal is exactly what we
                     * need it to be. -RD */
                    return 1;
            }
            break;
        ...
        if (!any_opened_circs && victim->state != CIRCUIT_STATE_OPEN) {
            ...
            if (timercmp(&victim->timestamp_began, &close_cutoff, OP_GT)) {
                if (!TO_ORIGIN_CIRCUIT(victim)->relaxed_timeout) {
                     ...
                     * if we see enough of them. Note this means we also need to
                     * avoid double-counting below, too. */
    ...
    if (victim->purpose >= CIRCUIT_PURPOSE_C_INTRODUCING &&
      ...
      if (!victim->timestamp_dirty)
        ...
        if (victim->state == CIRCUIT_STATE_OPEN) {
            switch (victim->purpose) {
                ...
                case CIRCUIT_PURPOSE_PATH_BIAS_TESTING:
                    ...
                    TO_ORIGIN_CIRCUIT(victim)->path_state =
                        PATH_STATE_USE_FAILED;
                    break;
                case CIRCUIT_PURPOSE_C_INTRODUCING:
                    /* That purpose means that the intro point circuit has been
                     * opened successfully but the INTRODUCE1 cell hasn't been
                     * sent yet because the client is waiting for the rendezvous
                     * point circuit to open. Keep this circuit open while
                     * waiting for the rendezvous circuit. We let the circuit
                     * idle timeout take care of cleaning this circuit if it
                     * never used. */
                    continue;
        ...
        if (!(TO_ORIGIN_CIRCUIT(victim)->hs_circ_has_timed_out)) {
            switch (victim->purpose) {
                case CIRCUIT_PURPOSE_C_REND_READY:
                    /* We only want to spare a rend circ if it has been
                     * specified in an INTRODUCE1 cell sent to a hidden service.
                     * A circ's pending_final_cpath field is non-NULL iff it is
                     * a rend circ and we have tried to send an INTRODUCE1 cell
                     * specifying it. Thus, if the pending_final_cpath field
                     * *is* NULL, then we want to not spare it. */
                ...
                case CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT:
                case CIRCUIT_PURPOSE_C_REND_READY_INTRO_ACKED:
                    /* If we have reached this line, we want to spare the circ
                     * for now. */
                    log_info(LD_CIRC,
...
circuit_build_failed(origin_circuit_t *circ)
{
    ...
    if (circ->cpath && circ->cpath->state != CPATH_STATE_OPEN &&
         ...
         * If there's an OR connection to blame, blame it. Also, avoid this
         * relay for a while, and fail any one-hop directory fetches destined
         * for it. */
            ...
            if (conn->chosen_exit_name) {
                ...
                if (r && node_has_preferred_descriptor(
                    ...
                    if (want_onehop && conn->chosen_exit_name[0] == '$') {
                         ...
                         * encode both key formats. This is not absolutely
                         * critical since this is just for one-hop circuits, but
                         * we should still get it done */
                        char digest[DIGEST_LEN];
    ...
    if (!connection_edge_is_rendezvous_stream(ENTRY_TO_EDGE_CONN(conn))) {
        ...
        if (base_conn->linked_conn &&
            ...
            if (networkstatus_consensus_is_already_downloading(
                 ...
                 * proposal 210, and we're late to the party. Just close this
                 * conn. The circuit and TLS conn that we made will time out
                 * after a while if nothing else wants to use them. */
                log_info(LD_DIR,
        ...
        if (conn->chosen_exit_name) {
            ...
            if (!node && !want_onehop) {
                 ...
                 * keep this in mind when deciding to use BEGIN_DIR cells for
                 * other directory requests as well. -KL*/
                log_fn(opt ? LOG_INFO : LOG_WARN, LD_APP,
                       "Requested exit point '%s' is not known. %s.",
                       conn->chosen_exit_name,
                       opt ? "Trying others" : "Closing");
         ...
         * our call to circuit_get_open_circ_or_launch() changed the
