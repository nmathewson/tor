    ...
    if ((edge_conn->rend_data && !origin_circ->rend_data) ||
        ...
        (edge_conn->rend_data && origin_circ->rend_data &&
         rend_cmp_service_ids(
             rend_data_get_address(edge_conn->rend_data),
    ...
    if (purpose == CIRCUIT_PURPOSE_C_GENERAL ||
        ...
        if (build_state->onehop_tunnel) {
            ...
            if (build_state->chosen_exit) {
                ...
                if (tor_memneq(digest,
                               build_state->chosen_exit->identity_digest,
                               DIGEST_LEN))
            ...
            if (conn->want_onehop) {
                /* don't use three-hop circuits -- that could hurt our
                 * anonymity. */
                return 0;
            }
        }
                  ...
                  const entry_connection_t *conn)
{
    ...
    a_bits = connection_edge_update_circuit_isolation(
        conn, (origin_circuit_t *)oa, 1);
    b_bits = connection_edge_update_circuit_isolation(
        conn, (origin_circuit_t *)ob, 1);
            ...
            if (timercmp(&victim->timestamp_began, &close_cutoff, OP_GT)) {
                if (!TO_ORIGIN_CIRCUIT(victim)->relaxed_timeout) {
                    int first_hop_succeeded =
                        TO_ORIGIN_CIRCUIT(victim)->cpath->state ==
                        CPATH_STATE_OPEN;
                    if (!fixed_time) {
                        log_info(LD_CIRC,
                                 "No circuits are opened. Relaxing timeout "
                                 "for circuit %d "
                                 "(a %s %d-hop circuit in state %s with "
                                 "channel state %s).",
                                 TO_ORIGIN_CIRCUIT(victim)->global_identifier,
                                 circuit_purpose_to_string(victim->purpose),
                                 TO_ORIGIN_CIRCUIT(victim)->build_state
                                     ? TO_ORIGIN_CIRCUIT(victim)
                                           ->build_state->desired_path_len
                                     : -1,
                                 circuit_state_to_string(victim->state),
                                 victim->n_chan ? channel_state_to_string(
                                                      victim->n_chan->state)
                                                : "none");
                    }

                    /* We count the timeout here for CBT, because technically
                     * this was a timeout, and the timeout value needs to reset
                     * if we see enough of them. Note this means we also need
                     * to avoid double-counting below, too. */
                ...
                if (!fixed_time) {
                    log_fn_ratelim(
                        ...
                        "%d "
                        "(a %s %d-hop circuit in state %s with channel state "
                        "%s) to "
      ...
      if (!victim->timestamp_dirty)
        ...
        if (victim->state == CIRCUIT_STATE_OPEN) {
            ...
            default: /* most open circuits can be left alone. */
                continue; /* yes, continue inside a switch refers to the
                           * nearest enclosing loop. C is smart. */
            ...
            case CIRCUIT_PURPOSE_C_INTRODUCING:
                /* That purpose means that the intro point circuit has been
                 * opened successfully but the INTRODUCE1 cell hasn't been sent
                 * yet because the client is waiting for the rendezvous point
                 * circuit to open. Keep this circuit open while waiting for
                 * the rendezvous circuit. We let the circuit idle timeout take
                 * care of cleaning this circuit if it never used. */
                continue;
            ...
            if (circuit_timeout_want_to_count_circ(
                    TO_ORIGIN_CIRCUIT(victim)) &&
                ...
                /*
                 * If the circuit build time is much greater than we would have
                 * cut it off at, we probably had a suspend event along this
                 * codepath, and we should discard the value.
                 */
                if (timercmp(&victim->timestamp_began, &extremely_old_cutoff,
                             OP_LT)) {
                    log_notice(
                        LD_CIRC,
                        "Extremely large value for circuit build timeout: "
                        "%lds. "
        ...
        /* If this is a hidden service client circuit which is far enough along
         * in connecting to its destination, and we haven't already flagged it
         * as 'timed out', flag it so we'll launch another intro or rend circ,
         * but don't mark it for close yet.
         *
        ...
        if (!(TO_ORIGIN_CIRCUIT(victim)->hs_circ_has_timed_out)) {
            switch (victim->purpose) {
            case CIRCUIT_PURPOSE_C_REND_READY:
                /* We only want to spare a rend circ iff it has been specified
                 * in an INTRODUCE1 cell sent to a hidden service. */
...
circuit_remove_handled_ports(smartlist_t *needed_ports)
{
    ...
    for (i = 0; i < smartlist_len(needed_ports); ++i) {
        ...
        if (circuit_stream_is_being_handled(NULL, *port,
            ...
            log_debug(LD_CIRC, "Port %d is already being handled; removing.",
                      *port);
...
circuit_detach_stream(circuit_t *circ, edge_connection_t *conn)
{
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        ...
        if (removed) {
            ...
            /* If the stream was removed, and it was a rend stream, decrement
             * the number of streams on the circuit associated with the rend
             * service.
             */
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        ...
        if (circ->timestamp_dirty &&
            ...
            log_debug(LD_CIRC,
                      ...
                      circ->purpose);
            /* Don't do this magic for testing circuits. Their death is
             * governed by circuit_expire_building */
            ...
            if (timercmp(&circ->timestamp_began, &cutoff, OP_LT)) {
                if (circ->purpose == CIRCUIT_PURPOSE_C_GENERAL ||
                    ...
                    /* Server-side rend joined circuits can end up really old,
                     * because they are reused by clients for longer than
                     * normal. The client controls their lifespan. (They never
                     * become dirty, because connection_exit_begin_conn() never
                     * marks anything as dirty.) Similarly, server-side intro
                     * circuits last a long time. */
                    if (circ->purpose != CIRCUIT_PURPOSE_S_REND_JOINED &&
                        circ->purpose != CIRCUIT_PURPOSE_S_INTRO) {
                        log_notice(LD_CIRC,
                                   "Ancient non-dirty circuit %d is still "
                                   "around after "
...
circuit_expire_old_circuits_serverside(time_t now)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
         ...
         * on it, and it ends here, and it used a create_fast, mark it for
         * close.
         *
...
circuit_build_failed(origin_circuit_t *circ)
{
    ...
    if (circ->cpath && circ->cpath->state != CPATH_STATE_OPEN &&
        ...
        /* We failed at the first hop for some reason other than a DESTROY
         * cell. If there's an OR connection to blame, blame it. Also, avoid
         * this relay for a while, and fail any one-hop directory fetches
         * destined for it. */
    ...
    if (circuit_should_cannibalize_to_build(purpose, extend_info != NULL,
        ...
        if (circ) {
            ...
            log_info(LD_CIRC,
                     ...
                     "purpose %d (%s)",
                     TO_CIRCUIT(circ)->n_circ_id, circ->global_identifier,
                     purpose, circuit_purpose_to_string(purpose));
                                ...
                                origin_circuit_t **circp)
{
    ...
    circ =
        circuit_get_best(conn, 1 /* Insist on open circuits */,
    ...
    if (!want_onehop && (!router_have_minimum_dir_info() || !have_path)) {
        ...
        if (!connection_get_by_type(CONN_TYPE_DIR)) {
            ...
            if (entry_list_is_constrained(options)) {
                ...
                /* Getting directory documents doesn't help much if we have a
                 * limited number of guards */
                tor_assert_nonfatal(!options->UseBridges);
                tor_assert_nonfatal(!options->EntryNodes);
                /* Retry our directory fetches, so we have a fresh set of guard
                 * info */
                log_fn(severity, LD_APP | LD_DIR,
                       "Application request when we haven't %s. "
         ...
         * stream will be dealt with when router_have_minimum_dir_info becomes
         * 1, or when all directory attempts fail and
         * directory_all_unreachable() kills it.
         */
        return 0;
    }
    ...
    if (check_exit_policy) {
        if (!conn->chosen_exit_name) {
            ...
            if (router_exit_policy_all_nodes_reject(
                    addrp, conn->socks_request->port, need_uptime)) {
                log_notice(LD_APP,
            ...
            /* XXXX Duplicates checks in
             * connection_ap_handshake_attach_circuit: refactor into a single
             * function. */
            const node_t *node =
    ...
    circ =
        circuit_get_best(conn, 0 /* don't insist on open circuits */,
                         desired_circuit_purpose, need_uptime, need_internal);
    if (circ)
    ...
    if (!circ) {
        ...
        if (n_pending >= options->MaxClientCircuitsPending) {
            ...
            if ((m = rate_limit_log(&delay_limit, approx_time()))) {
                log_notice(LD_APP,
                           ...
                           "connection, but we already have %d "
        ...
        if (desired_circuit_purpose == CIRCUIT_PURPOSE_C_GENERAL ||
            ...
            if (conn->chosen_exit_name) {
                ...
                if (r && node_has_preferred_descriptor(
                             r, conn->want_onehop ? 1 : 0)) {
                    /* We might want to connect to an IPv6 bridge for loading
                       descriptors so we use the preferred address rather than
                       the primary. */
                    extend_info =
                        extend_info_from_node(r, conn->want_onehop ? 1 : 0);
                    if (!extend_info) {
                        log_warn(LD_CIRC,
                    ...
                    if (want_onehop && conn->chosen_exit_name[0] == '$') {
                         ...
                         * we don't have a routerinfo about. Make up an
                         * extend_info. */
                        /* XXX prop220: we need to make chosen_exit_name able
                         * to encode both key formats. This is not absolutely
                         * critical since this is just for one-hop circuits,
                         * but we should still get it done */
                        char digest[DIGEST_LEN];
                        ...
                        if (tor_addr_parse(&addr,
                                           conn->socks_request->address) < 0) {
                            log_info(
                                LD_DIR,
                        ...
                        extend_info =
                            extend_info_new(conn->chosen_exit_name + 1, digest,
                                            NULL, /* Ed25519 ID */
        ...
        if (desired_circuit_purpose == CIRCUIT_PURPOSE_C_GENERAL ||
             ...
             * If this stream has caused a _lot_ of circuits to be built,
             * that's a bad sign: we should tell the user. */
            if (conn->num_circuits_launched <
            ...
            if (circ) {
                ...
                if (edge_conn->rend_data) {
                    ...
                    circ->hs_ident = hs_ident_circuit_new(
                        &edge_conn->hs_ident->identity_pk);
                }
...
optimistic_data_enabled(void)
{
    ...
    if (options->OptimisticData < 0) {
         ...
         * obsolete. */
        const int32_t enabled = networkstatus_get_param(
            NULL, "UseOptimisticData", /*default*/ 1, 0, 1);
        return (int)enabled;
    }
...
connection_ap_get_nonrend_circ_purpose(const entry_connection_t *conn)
{
    ...
    if (base_conn->linked_conn &&
        ...
        if (base_conn->linked_conn->purpose ==
    ...
    if (!connection_edge_is_rendezvous_stream(ENTRY_TO_EDGE_CONN(conn))) {
        ...
        if (base_conn->linked_conn &&
            ...
            if (networkstatus_consensus_is_already_downloading(
                ...
                /* We're doing the "multiple consensus fetch attempts" game
                 * from proposal 210, and we're late to the party. Just close
                 * this conn. The circuit and TLS conn that we made will time
                 * out after a while if nothing else wants to use them. */
                log_info(LD_DIR,
         ...
         * open to that exit. See what exit we meant, and whether we can use
         * it.
         */
        if (conn->chosen_exit_name) {
            const node_t *node =
            ...
            int opt = conn->chosen_exit_optional;
            if (!node && !want_onehop) {
                /* We ran into this warning when trying to extend a circuit to
                 * a hidden service directory for which we didn't have a router
                 * descriptor. See flyspray task 767 for more details. We
                 * should keep this in mind when deciding to use BEGIN_DIR
                 * cells for other directory requests as well. -KL*/
                log_fn(opt ? LOG_INFO : LOG_WARN, LD_APP,
                       "Requested exit point '%s' is not known. %s.",
                       conn->chosen_exit_name,
                       opt ? "Trying others" : "Closing");
        ...
        /* At this point we need to re-check the state, since it's possible
         * that our call to circuit_get_open_circ_or_launch() changed the
         * connection's state from "CIRCUIT_WAIT" to "RENDDESC_WAIT" because we
         * decided to re-fetch the descriptor.
         */
        if (ENTRY_TO_CONN(conn)->state != AP_CONN_STATE_CIRCUIT_WAIT) {
            log_info(LD_REND,
        ...
        if (rendcirc && introcirc &&
            ...
            tor_assert(introcirc->base_.purpose ==
...
circuit_change_purpose(circuit_t *circ, uint8_t new_purpose)
{
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        ...
        /* Take specific actions if we are repurposing a hidden service
         * circuit. */
