...
stream_end_reason_to_string(int reason)
{
    ...
    default:
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Reason for ending (%d) not recognized.", reason);
        return "unknown";
    }
}
...
errno_to_stream_end_reason(int e)
{
    ...
    case EPIPE:
        ...
        E_CASE(EBADF)
            ...
            : S_CASE(EAFNOSUPPORT) : S_CASE(ENOTCONN) : return END_STREAM_REASON_INTERNAL;
        S_CASE(ENETUNREACH)
            : S_CASE(EHOSTUNREACH) : E_CASE(EACCES) : case EPERM : return END_STREAM_REASON_NOROUTE;
        ...
        S_CASE(ENOBUFS)
            ...
            : S_CASE(EADDRNOTAVAIL) : E_CASE(EMFILE) : return END_STREAM_REASON_RESOURCELIMIT;
    default:
        log_info(LD_EXIT,
...
errno_to_orconn_end_reason(int e)
{
    ...
    case EPIPE:
        ...
        S_CASE(ENOTCONN)
            : S_CASE(ENETUNREACH)
            : S_CASE(ENETDOWN) : S_CASE(EHOSTUNREACH) : return END_OR_CONN_REASON_NO_ROUTE;
        ...
        S_CASE(ENOBUFS)
            ...
            : E_CASE(EACCES)
            : E_CASE(EBADF)
            : E_CASE(EFAULT) : E_CASE(EINVAL) : return END_OR_CONN_REASON_RESOURCE_LIMIT;
    default:
        log_info(LD_OR, "Didn't recognize errno %d (%s).", e, tor_socket_strerror(e));
        return END_OR_CONN_REASON_MISC;
    }
}
