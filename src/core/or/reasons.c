...
stream_end_reason_to_socks5_response(int reason)
{
    switch (reason & END_STREAM_REASON_MASK) {
        ...
        case END_STREAM_REASON_DESTROY:
            return SOCKS5_GENERAL_ERROR;
        case END_STREAM_REASON_DONE:
            /* Note that 'DONE' usually indicates a successful close from the
             * other side of the stream... but if we receive it before a
             * connected cell -- that is, before we have sent a SOCKS reply --
             * that means that the other side of the circuit closed the
             * connection before telling us it was complete. */
            return SOCKS5_CONNECTION_REFUSED;
        case END_STREAM_REASON_TIMEOUT:
    ...
    switch (e) {
        case EPIPE:
            ...
            S_CASE(ENETUNREACH)
                : S_CASE(EHOSTUNREACH)
                : E_CASE(EACCES)
                : case EPERM : return END_STREAM_REASON_NOROUTE;
...
circuit_end_reason_to_control_string(int reason)
{
    ...
    switch (reason) {
        ...
        default:
            if (is_remote) {
                /*
                 * If it's remote, it's not a bug *here*, so don't use LD_BUG,
                 * but do note that the someone we're talking to is speaking the
                 * Tor protocol with a weird accent.
                 */
...
end_reason_to_http_connect_response_line(int endreason)
{
    ...
    switch (endreason) {
        ...
        default:
            tor_assert_nonfatal_unreached();
            return "HTTP/1.0 500 Internal Server Error (weird end "
                   "reason)\r\n\r\n";
    }
}
