...
#define PATHBIAS_SHOULDCOUNT_COUNTED 2
     ...
     * necessarily mean that we've <em>attached</em> any streams to the
     * circuit: we may only have marked up this circuit during the launch
     * process.
     */
     ...
     * value for some field ((isolation_flags_mixed & ISO_FOO) == 0), then
     * these elements hold the value for that field.
     *
     * Note again that "associated" is not the same as "attached": we
     * preliminarily associate streams with a circuit while the circuit is
     * being launched, so that we can tell whether we need to launch more
     * circuits.
     *
     * @{
     */
