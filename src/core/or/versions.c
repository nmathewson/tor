...
tor_version_is_obsolete(const char *myversion, const char *versionlist)
{
    ...
    int found_newer = 0, found_older = 0, found_newer_in_series = 0, found_any_in_series = 0, r,
        same;
 ...
 */
int
tor_version_parse_platform(const char *platform, tor_version_t *router_version, int strict)
{
    char tmp[128];
    char *s, *s2, *start;
    ...
    if (strict) {
        if (router_version->major < 0 || router_version->minor < 0 || router_version->micro < 0 ||
            router_version->patchlevel < 0 || router_version->svn_revision < 0) {
            return -1;
        }
    }
...
memoize_protover_summary(protover_summary_flags_t *out, const char *protocols)
{
    ...
    const protover_summary_flags_t *cached = strmap_get(protover_summary_map, protocols);
    ...
    out->supports_extend2_cells = protocol_list_supports_protocol(protocols, PRT_RELAY, 2);
    ...
    out->supports_ed25519_hs_intro = protocol_list_supports_protocol(protocols, PRT_HSINTRO, 4);
    out->supports_v3_hsdir =
        protocol_list_supports_protocol(protocols, PRT_HSDIR, PROTOVER_HSDIR_V3);
    out->supports_v3_rendezvous_point =
        protocol_list_supports_protocol(protocols, PRT_HSREND, PROTOVER_HS_RENDEZVOUS_POINT_V3);
    out->supports_hs_setup_padding =
        protocol_list_supports_protocol(protocols, PRT_PADDING, PROTOVER_HS_SETUP_PADDING);
 ...
 */
void
summarize_protover_flags(protover_summary_flags_t *out, const char *protocols, const char *version)
{
    tor_assert(out);
    memset(out, 0, sizeof(*out));
    if (protocols) {
