...
struct circuit_t {
    ...
    /** FIFO containing the digest of the cells that are just before a SENDME
     * is sent by the client. It is done at the last cell before our
     * package_window goes down to 0 which is when we expect a SENDME.
     *
    ...
    uint16_t marked_for_close; /**< Should we close this circuit at the end of
                                * the main loop? (If true, holds the line
                                * number where this circuit was marked.) */
    const char *marked_for_close_file; /**< For debugging: in which file was
                                        * this circuit marked for close? */
    ...
    /** Hashtable node: used to look up the circuit by its HS token using the
       HS circuitmap. */
    HT_ENTRY(circuit_t) hs_circuitmap_node;

    /** Adaptive Padding state machines: these are immutable. The state
     * machines that come from the consensus are saved to a global structure,
     * to avoid per-circuit allocations. This merely points to the global copy
     * in origin_padding_machines or relay_padding_machines that should never
