...
struct entry_port_cfg_t {
    ...
    /* Socks only: */
    /** When both no-auth and user/pass are advertised by a SOCKS client,
     * select no-auth. */
    ...
    /** @} */
    /** For a socks listeners: if we find an answer in our client-side DNS
     * cache, should we use it?
     *
     * @{ */
