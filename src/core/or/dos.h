...
typedef struct cc_client_stats_t {
    ...
    /* When was the last time we've refilled the circuit bucket? This is used
     * to know if we need to refill the bucket when a new circuit is seen. It
     * is synchronized using approx_time(). */
