...
circuit_update_channel_usage(circuit_t *circ, cell_t *cell)
{
    if (CIRCUIT_IS_ORIGIN(circ)) {
         ...
         * it until actual application traffic starts flowing (which is
         * controlled via consensus param nf_pad_before_usage).
         *
         * So: If we're an origin circuit and we've created a full length
         * circuit, then any CELL_RELAY cell means application data. Increase
         * the usage state of the channel to indicate this.
         *
        ...
        /* If we're a relay circuit, the question is more complicated.
         * Basically: we only want to pad connections that carry multihop
         * (anonymous) circuits.
         *
        ...
        if (!channel_is_client(or_circ->p_chan) ||
            ...
            if (cell->command == CELL_RELAY_EARLY) {
                if (or_circ->p_chan->channel_usage <
                    CHANNEL_USED_FOR_FULL_CIRCS) {
                    or_circ->p_chan->channel_usage =
                        CHANNEL_USED_FOR_FULL_CIRCS;
                }
                           ...
                           cell_direction_t cell_direction)
{
    ...
    if (recognized) {
        ...
        /* Recognized cell, the cell digest has been updated, we'll record it
         * for the SENDME if need be. */
        ...
        if (circ->purpose == CIRCUIT_PURPOSE_PATH_BIAS_TESTING) {
            ...
            /* We need to drop this cell no matter what to avoid code that
             * expects a certain purpose (such as the hidserv code). */
            return 0;
        }
        ...
        if (cell_direction == CELL_DIRECTION_IN) {
            ...
            reason = connection_edge_process_relay_cell(cell, circ, conn,
                                                        layer_hint);
            if (reason < 0) {
                /* If a client is trying to connect to unknown hidden service
                 * port, END_CIRC_AT_ORIGIN is sent back so we can then close
                 * the circuit.
                 * Do not log warn as this is an expected behavior for a
                 * service. */
                if (reason != END_CIRC_AT_ORIGIN) {
                    log_warn(LD_OR, "connection_edge_process_relay_cell (at "
                                    "origin) failed.");
                }
                return reason;
            }
    ...
    if (!chan) {
        ...
        if (!CIRCUIT_IS_ORIGIN(circ) && TO_OR_CIRCUIT(circ)->rend_splice &&
            ...
            tor_assert(splice_->base_.purpose ==
...
MOCK_IMPL(int, circuit_package_relay_cell,
          (cell_t * cell, circuit_t *circ, cell_direction_t cell_direction,
           crypt_path_t *layer_hint, streamid_t on_stream, const char *filename,
           int lineno))
{
           ...
           const char *filename, int lineno))
{
    ...
    if (cell_direction == CELL_DIRECTION_OUT) {
        ...
        if (origin_circ->remaining_relay_early_cells > 0 &&
            ...
            /* Memorize the command that is sent as RELAY_EARLY cell; helps
             * debug task 878. */
            origin_circ
                             ...
                             const char *payload, size_t payload_len)
{
    ...
    return relay_send_command_from_edge(fromconn->stream_id, circ,
                                        relay_command, payload, payload_len,
                                        cpath_layer);
}
        ...
        switch (reason) {
            case END_STREAM_REASON_EXITPOLICY: {
                ...
                if (rh->length >= 5) {
                    ...
                    if (rh->length == 5 || rh->length == 9) {
                        ...
                        if (rh->length == 9)
                            ttl = (int)ntohl(get_uint32(cell->payload +
                                                        RELAY_HEADER_SIZE + 5));
                        ...
                        if (rh->length == 21)
                            ttl = (int)ntohl(get_uint32(
                                cell->payload + RELAY_HEADER_SIZE + 17));
                    }
                    if (tor_addr_is_null(&addr)) {
                        ...
                        connection_mark_unattached_ap(
                            conn, END_STREAM_REASON_TORPROTOCOL);
                        return 0;
                    }
                    ...
                    client_dns_set_addressmap(
                        conn, conn->socks_request->address, &addr,
...

                    {
                        ...
                        if (strcmp(conn->socks_request->address, new_addr)) {
                            ...
                            control_event_stream_status(conn,
                                                        STREAM_EVENT_REMAP, 0);
                        }
                    }
                }
                /* check if the exit *ought* to have allowed it */

                adjust_exit_policy_from_exitpolicy_failure(circ, conn,
                                                           exitrouter, &addr);
                ...
                if (conn->chosen_exit_optional || conn->chosen_exit_retries) {
                     ...
                     * TrackHostExits for this address under a port that the
                     * exit relay allows, but then try the same address with a
                     * different port that it doesn't allow to exit. We
                     * shouldn't unregister the mapping, since it is probably
                     * still wanted on the original port. But now we give away
                     * to the exit relay that
                     * we probably have a TrackHostExits on it. So be it. */
                    conn->chosen_exit_retries = 0;
                    tor_free(conn->chosen_exit_name); /* clears it */
                }
                if (connection_ap_detach_retriable(conn, circ,
                                                   control_reason) >= 0)
                    return 0;
            ...
            case END_STREAM_REASON_NOROUTE:
                if (client_dns_incr_failures(conn->socks_request->address) <
                    ...
                    /* We haven't retried too many times; reattach the
                     * connection. */
                                           ...
                                           node_t *node, const tor_addr_t *addr)
{
    ...
    if (node) {
        tor_addr_t tmp;
        int asked_for_family =
            tor_addr_parse(&tmp, conn->socks_request->address);
        if (family == AF_UNSPEC) {
            make_reject_all = 1;
                                                ...
                                                smartlist_t *results)
{
    ...
    if (error_code) {
        ...
        connection_ap_handshake_socks_resolved(conn, error_code, 0, NULL, -1,
                                               -1);
        return;
    }
    ...
    if (get_options()->ClientDNSRejectInternalAddresses) {
        ...
        SMARTLIST_FOREACH_BEGIN(resolved_addresses, address_ttl_t *, addr)
        {
            if (addr->hostname == NULL &&
                tor_addr_is_internal(&addr->addr, 0)) {
                log_info(LD_APP,
        ...
        if (orig_len && smartlist_len(resolved_addresses) == 0) {
            log_info(LD_APP, "Got a resolved cell with only private addresses; "
                             "dropping it.");
            connection_ap_handshake_socks_resolved(
                entry_conn, RESOLVED_TYPE_ERROR_TRANSIENT, 0, NULL, 0,
                TIME_MAX);
            connection_mark_unattached_ap(entry_conn,
                                          END_STREAM_REASON_TORPROTOCOL);
            goto done;
    ...
    if (conn->base_.type == CONN_TYPE_AP &&
        ...
        if (tor_addr_family(&addr) != AF_UNSPEC) {
            ...
            if ((family == AF_INET && !entry_conn->entry_cfg.ipv4_traffic) ||
                ...
                log_fn(LOG_PROTOCOL_WARN, LD_APP,
                       "Got a connected cell to %s with unsupported address "
                       "family."
                       " Closing.",
                       fmt_addr(&addr));
                connection_edge_end(conn, END_STREAM_REASON_TORPROTOCOL);
            ...
            switch (dirconn->purpose) {
                case DIR_PURPOSE_FETCH_CERTIFICATE:
                    if (consensus_is_waiting_for_certs())
                        control_event_bootstrap(BOOTSTRAP_STATUS_LOADING_KEYS,
                                                0);
                    break;
                ...
                case DIR_PURPOSE_FETCH_MICRODESC:
                    if (TO_DIR_CONN(dirconn)->router_purpose ==
    ...
    switch (rh->command) {
        ...
        case RELAY_COMMAND_BEGIN_DIR:
            ...
            if (rh->command == RELAY_COMMAND_BEGIN_DIR &&
                ...
                /* Assign this circuit and its app-ward OR connection a unique
                 * ID, so that we can measure download times. The local edge and
                 * dir connection will be assigned the same ID when they are
                 * created and linked. */
        ...
        case RELAY_COMMAND_DATA:
            ...
            /* Update our circuit-level deliver window that we received a DATA
             * cell. If the deliver window goes below 0, we end the circuit and
             * stream due to a protocol failure. */
            ...
            /* Update our stream-level deliver window that we just received a
             * DATA cell. Going below 0 means we have a protocol level error so
             * the stream and circuit are closed. */
            ...
            if (!optimistic_data) {
                /* Only send a SENDME if we're not getting optimistic data;
                 * otherwise a SENDME could arrive before the CONNECTED.
                 */
        ...
        case RELAY_COMMAND_EXTEND2: {
            ...
            if (cell->command != CELL_RELAY_EARLY &&
                !networkstatus_get_param(NULL, "AllowNonearlyExtend", 0, 0,
                                         1)) {
#define EARLY_WARNING_INTERVAL 3600
                ...
                if (cell->command == CELL_RELAY) {
                    ++total_nonearly;
                    if ((m = rate_limit_log(&early_warning_limit,
                                            approx_time()))) {
                        double percentage =
                            ((double)total_nonearly) / total_n_extend;
                        percentage *= 100;
                        log_fn(LOG_PROTOCOL_WARN, domain,
                    ...
                    log_fn(LOG_WARN, domain,
                           "EXTEND cell received, in a cell with type %d! "
                           "Dropping.",
                           cell->command);
                }
                return 0;
        ...
        case RELAY_COMMAND_EXTENDED2:
            ...
            log_debug(domain, "Got an extended cell! Yay.");
            {
                ...
                if ((reason = circuit_finish_handshake(
                    ...
                    circuit_mark_for_close(circ, -reason);
                    return 0; /* We don't want to cause a warning, so we mark
                               * the circuit here. */
                }
            }
        ...
        case RELAY_COMMAND_CONNECTED:
            ...
            return 0;
        case RELAY_COMMAND_SENDME:
            return process_sendme_cell(rh, cell, circ, conn, layer_hint,
                                       domain);
        case RELAY_COMMAND_RESOLVE:
            if (layer_hint) {
                log_fn(LOG_PROTOCOL_WARN, LD_APP,
                                           ...
                                           circuit_t *on_circuit)
{
    ...
    if (n_available >= target_length) { /* A full payload is available. */
        ...
        if (package_partial)
            package_length =
                n_available; /* just take whatever's available now */
        else
...
repeat_connection_edge_package_raw_inbuf:
    ...
    if (connection_edge_send_command(conn, RELAY_COMMAND_DATA, payload,
                                     length) < 0) {
                                   ...
                                   circuit_t *circ, crypt_path_t *layer_hint)
{
    ...
    if (first_conn == NULL) {
        /* Don't bother to try to do the rest of this if there are no
     ...
     * don't need cryptographic randomness here. */
    {
        ...
        for (conn = first_conn; conn; conn = conn->next_stream) {
             ...
             *
             * (Note that we iterate over every stream on the circuit, so that
             * after we've considered the first stream, we've chosen it with
             * P=1; and after we consider the second stream, we've switched to
             * it with P=1/2 and stayed with the first stream with P=1/2; and
             * after we've considered the third stream, we've switched to it
             * with P=1/3 and remained with one of the first two streams with
             * P=(2/3), giving each one P=(1/2)(2/3) )=(1/3).) */
        }
    }
...
again:
    ...
    for (conn = first_conn; conn; conn = conn->next_stream) {
        ...
        if (!layer_hint || conn->cpath_layer == layer_hint) {
            ...
            /* If there's still data to read, we'll be coming back to this
             * stream. */
            if (connection_get_inbuf_len(TO_CONN(conn)))
                ++n_streams_left;

            /* If the circuit won't accept any more data, return without looking
             * at any more of the streams. Any connections that should be
             * stopped have already been stopped by
             * connection_edge_package_raw_inbuf. */
            if (circuit_consider_stop_edge_reading(circ, layer_hint))
                return -1;
            /* XXXX should we also stop immediately if we fill up the cell
             * queue? Probably. */
        }
    }
...
cell_queues_check_size(void)
{
    ...
    if (alloc >= get_options()->MaxMemInQueues_low_threshold) {
        ...
        if (alloc >= get_options()->MaxMemInQueues) {
             ...
             * descriptors, free them until we're down to 10%. Do the same for
             * geoip client cache. */
          ...
          (channel_t * chan, int max))
{
    ...
    while (n_flushed < max) {
        ...
        if (destroy_queue) {
            ...
            /* Send the DESTROY cell. It is very unlikely that this fails but
             * just in case, get rid of the channel. */
        ...
        /*
         * Get just one cell here; once we've sent it, that can change the
         * circuit selection, so we have to loop around for another even if this
        ...
        if (get_options()->CellStatistics ||
            ...
            if (get_options()->TestingEnableCellStatsEvent) {
                uint8_t command =
                    packed_cell_get_command(cell, chan->wide_circ_ids);
         ...
         * waiting to write to this circuit? */
