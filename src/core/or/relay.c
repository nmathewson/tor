...
static int circuit_resume_edge_reading_helper(edge_connection_t *conn, circuit_t *circ,
                                              crypt_path_t *layer_hint);
...
static void adjust_exit_policy_from_exitpolicy_failure(origin_circuit_t *circ,
                                                       entry_connection_t *conn, node_t *node,
 ...
 */
int
circuit_receive_relay_cell(cell_t *cell, circuit_t *circ, cell_direction_t cell_direction)
{
    channel_t *chan = NULL;
    ...
    if (relay_decrypt_cell(circ, cell, cell_direction, &layer_hint, &recognized) < 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "relay crypt failed. Dropping connection.");
    ...
    if (recognized) {
        ...
        if (cell_direction == CELL_DIRECTION_IN) {
            ...
            reason = connection_edge_process_relay_cell(cell, circ, conn, layer_hint);
            if (reason < 0) {
                ...
                if (reason != END_CIRC_AT_ORIGIN) {
                    log_warn(LD_OR, "connection_edge_process_relay_cell (at origin) failed.");
                }
                return reason;
            }
        }
    ...
    if (cell_direction == CELL_DIRECTION_OUT) {
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_OR, "Dropping unrecognized inbound cell on origin circuit.");
...
circuit_package_relay_cell,
          (cell_t * cell, circuit_t *circ, cell_direction_t cell_direction,
           crypt_path_t *layer_hint, streamid_t on_stream, const char *filename, int lineno))
{
    ...
    if (cell_direction == CELL_DIRECTION_OUT) {
        ...
        if (!chan) {
            log_warn(LD_BUG,
                     ...
                     filename, lineno, circuit_state_to_string(circ->state), circ->state,
        ...
        ocirc->n_written_circ_bw = tor_add_u32_nowrap(ocirc->n_written_circ_bw, CELL_PAYLOAD_SIZE);
                  ...
                  crypt_path_t *layer_hint)
{
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        for (tmpconn = TO_ORIGIN_CIRCUIT(circ)->p_streams; tmpconn;
            ...
            if (rh.stream_id == tmpconn->stream_id && !tmpconn->base_.marked_for_close &&
        ...
        for (tmpconn = TO_OR_CIRCUIT(circ)->n_streams; tmpconn; tmpconn = tmpconn->next_stream) {
            if (rh.stream_id == tmpconn->stream_id && !tmpconn->base_.marked_for_close) {
...
relay_command_to_string(uint8_t command)
{
    ...
    default:
        tor_snprintf(buf, sizeof(buf), "Unrecognized relay command %u", (unsigned)command);
        return buf;
    }
}
...
pad_cell_payload(uint8_t *cell_payload, size_t data_len)
{
    ...
    crypto_fast_rng_getbytes(get_thread_fast_rng(), cell_payload + pad_offset, pad_len);
}
...
relay_send_command_from_edge_,
          (streamid_t stream_id, circuit_t *circ, uint8_t relay_command, const char *payload,
           size_t payload_len, crypt_path_t *cpath_layer, const char *filename, int lineno))
{
    cell_t cell;
    relay_header_t rh;
    cell_direction_t cell_direction;
    ...
    if (relay_command == RELAY_COMMAND_END && circ->dirreq_id)
        geoip_change_dirreq_state(circ->dirreq_id, DIRREQ_TUNNELED, DIRREQ_END_CELL_SENT);
    ...
    if (cell_direction == CELL_DIRECTION_OUT) {
        ...
        if (origin_circ->remaining_relay_early_cells > 0 &&
            (relay_command == RELAY_COMMAND_EXTEND || relay_command == RELAY_COMMAND_EXTEND2 ||
             ...
             * task 878. */
            origin_circ->relay_early_commands[origin_circ->relay_early_cells_sent++] =
                relay_command;
            ...
            for (; i < origin_circ->relay_early_cells_sent; i++)
                smartlist_add(commands_list, (char *)relay_command_to_string(
    ...
    if (circuit_package_relay_cell(&cell, circ, cell_direction, cpath_layer, stream_id, filename,
                                   lineno) < 0) {
 ...
 */
int
connection_edge_send_command(edge_connection_t *fromconn, uint8_t relay_command,
                             const char *payload, size_t payload_len)
{
    ...
    if (fromconn->base_.marked_for_close) {
        log_warn(LD_BUG, "called on conn that's already marked for close at %s:%d.",
                 fromconn->base_.marked_for_close_file, fromconn->base_.marked_for_close);
        return 0;
    }
    ...
    if (!circ) {
        if (fromconn->base_.type == CONN_TYPE_AP) {
            ...
            connection_mark_unattached_ap(EDGE_TO_ENTRY_CONN(fromconn), END_STREAM_REASON_INTERNAL);
        } else {
    ...
    return relay_send_command_from_edge(fromconn->stream_id, circ, relay_command, payload,
...
{
    return reason == END_STREAM_REASON_HIBERNATING || reason == END_STREAM_REASON_RESOURCELIMIT ||
           reason == END_STREAM_REASON_EXITPOLICY || reason == END_STREAM_REASON_RESOLVEFAILED ||
           reason == END_STREAM_REASON_MISC || reason == END_STREAM_REASON_NOROUTE;
}
 ...
 */
static int
connection_ap_process_end_not_open(relay_header_t *rh, cell_t *cell, origin_circuit_t *circ,
                                   entry_connection_t *conn, crypt_path_t *layer_hint)
{
    ...
    if (rh->length > 0) {
        if (reason == END_STREAM_REASON_TORPROTOCOL || reason == END_STREAM_REASON_DESTROY) {
    ...
    if (edge_reason_is_retriable(reason) &&
        ...
        const char *chosen_exit_digest = circ->build_state->chosen_exit->identity_digest;
        log_info(LD_APP, "Address '%s' refused due to '%s'. Considering retrying.",
                 safe_str(conn->socks_request->address), stream_end_reason_to_string(reason));
        ...
        case END_STREAM_REASON_EXITPOLICY: {
            ...
            if (rh->length >= 5) {
                ...
                if (rh->length == 5 || rh->length == 9) {
                    tor_addr_from_ipv4n(&addr, get_uint32(cell->payload + RELAY_HEADER_SIZE + 1));
                    if (rh->length == 9)
                        ttl = (int)ntohl(get_uint32(cell->payload + RELAY_HEADER_SIZE + 5));
                } else if (rh->length == 17 || rh->length == 21) {
                    tor_addr_from_ipv6_bytes(&addr, (cell->payload + RELAY_HEADER_SIZE + 1));
                    if (rh->length == 21)
            ...
            adjust_exit_policy_from_exitpolicy_failure(circ, conn, exitrouter, &addr);
        ...
        case END_STREAM_REASON_NOROUTE:
            if (client_dns_incr_failures(conn->socks_request->address) < MAX_RESOLVE_FAILURES) {
    ...
    log_info(LD_APP, "Edge got end (%s) before we're connected. Marking for close.",
 ...
 **/
static void
adjust_exit_policy_from_exitpolicy_failure(origin_circuit_t *circ, entry_connection_t *conn,
                                           node_t *node, const tor_addr_t *addr)
{
    ...
    if (node) {
        ...
        if (family == AF_UNSPEC) {
            make_reject_all = 1;
        } else if (node_exit_policy_is_exact(node, family) && asked_for_family != -1 &&
...
{
    tor_addr_to_str(conn->socks_request->address, new_addr, sizeof(conn->socks_request->address),
                    1);
    control_event_stream_status(conn, STREAM_EVENT_REMAP, REMAP_STREAM_SOURCE_EXIT);
}
 ...
 * is set to -1). */
STATIC int
connected_cell_parse(const relay_header_t *rh, const cell_t *cell, tor_addr_t *addr_out,
                     int *ttl_out)
{
    uint32_t bytes;
 ...
 * return 0. */
STATIC int
resolved_cell_parse(const cell_t *cell, const relay_header_t *rh, smartlist_t *addresses_out,
                    int *errcode_out)
{
    const uint8_t *cp;
    uint8_t answer_type;
...
static void
connection_ap_handshake_socks_got_resolved_cell(entry_connection_t *conn, int error_code,
                                                smartlist_t *results)
{
    ...
    connection_ap_handshake_socks_resolved_addr(conn, &addr_best->addr, addr_best->ttl, -1);
 ...
 * stream. */
STATIC int
connection_edge_process_resolved_cell(edge_connection_t *conn, const cell_t *cell,
                                      const relay_header_t *rh)
{
    ...
    if (resolved_cell_parse(cell, rh, resolved_addresses, &errcode)) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Dropping malformed 'resolved' cell");
    ...
    if (get_options()->ClientDNSRejectInternalAddresses) {
        ...
        if (orig_len && smartlist_len(resolved_addresses) == 0) {
            log_info(LD_APP, "Got a resolved cell with only private addresses; "
                             "dropping it.");
            connection_ap_handshake_socks_resolved(entry_conn, RESOLVED_TYPE_ERROR_TRANSIENT, 0,
                                                   NULL, 0, TIME_MAX);
            connection_mark_unattached_ap(entry_conn, END_STREAM_REASON_TORPROTOCOL);
            goto done;
        }
    ...
    connection_ap_handshake_socks_got_resolved_cell(entry_conn, errcode, resolved_addresses);

    connection_mark_unattached_ap(entry_conn, END_STREAM_REASON_DONE |
                                                  END_STREAM_REASON_FLAG_ALREADY_SOCKS_REPLIED);

done:
    SMARTLIST_FOREACH(resolved_addresses, address_ttl_t *, addr, address_ttl_free(addr));
 ...
 */
static int
connection_edge_process_relay_cell_not_open(relay_header_t *rh, cell_t *cell, circuit_t *circ,
                                            edge_connection_t *conn, crypt_path_t *layer_hint)
{
    if (rh->command == RELAY_COMMAND_END) {
        if (CIRCUIT_IS_ORIGIN(circ) && conn->base_.type == CONN_TYPE_AP) {
            return connection_ap_process_end_not_open(rh, cell, TO_ORIGIN_CIRCUIT(circ),
                                                      EDGE_TO_ENTRY_CONN(conn), layer_hint);
        } else {
            /* we just got an 'end', don't need to send one */
            conn->edge_has_sent_end = 1;
            conn->end_reason = *(cell->payload + RELAY_HEADER_SIZE) | END_STREAM_REASON_FLAG_REMOTE;
            connection_mark_for_close(TO_CONN(conn));
            return 0;
        }
    }

    if (conn->base_.type == CONN_TYPE_AP && rh->command == RELAY_COMMAND_CONNECTED) {
        tor_addr_t addr;
        int ttl;
        entry_connection_t *entry_conn = EDGE_TO_ENTRY_CONN(conn);
        ...
        if (connected_cell_parse(rh, cell, &addr, &ttl) < 0) {
            log_fn(LOG_PROTOCOL_WARN, LD_APP, "Got a badly formatted connected cell. Closing.");
        ...
        if (tor_addr_family(&addr) != AF_UNSPEC) {
             ...
             * service streams.) */
            const sa_family_t family = tor_addr_family(&addr);
            if (tor_addr_is_null(&addr) || (get_options()->ClientDNSRejectInternalAddresses &&
                                            tor_addr_is_internal(&addr, 0))) {
            ...
            client_dns_set_addressmap(entry_conn, entry_conn->socks_request->address, &addr,
        ...
        if (TO_CONN(conn)->linked_conn && TO_CONN(conn)->linked_conn->type == CONN_TYPE_DIR) {
    ...
    if (conn->base_.type == CONN_TYPE_AP && rh->command == RELAY_COMMAND_RESOLVED) {
        return connection_edge_process_resolved_cell(conn, cell, rh);
    }

    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
           "Got an unexpected relay command %d, in state %d (%s). Dropping.", rh->command,
           conn->base_.state, conn_state_to_string(conn->base_.type, conn->base_.state));
...
static int
process_sendme_cell(const relay_header_t *rh, const cell_t *cell, circuit_t *circ,
                    edge_connection_t *conn, crypt_path_t *layer_hint, int domain)
{
    int ret;
    ...
    if (!rh->stream_id) {
        ...
        ret = sendme_process_circuit_level(layer_hint, circ, cell->payload + RELAY_HEADER_SIZE,
                                           rh->length);
        if (ret < 0) {
            return ret;
        }
    ...
    if (!conn) {
        if (CIRCUIT_IS_ORIGIN(circ)) {
            origin_circuit_t *ocirc = TO_ORIGIN_CIRCUIT(circ);
            if (connection_half_edge_is_valid_sendme(ocirc->half_streams, rh->stream_id)) {
        ...
        log_info(domain, "SENDME cell dropped, unknown stream (streamid %d).", rh->stream_id);
        return 0;
    }
 ...
 */
STATIC int
handle_relay_cell_command(cell_t *cell, circuit_t *circ, edge_connection_t *conn,
                          crypt_path_t *layer_hint, relay_header_t *rh, int optimistic_data)
{
    ...
    case RELAY_COMMAND_BEGIN_DIR:
        if (layer_hint && circ->purpose != CIRCUIT_PURPOSE_S_REND_JOINED) {
            log_fn(LOG_PROTOCOL_WARN, LD_APP, "Relay begin request unsupported at AP. Dropping.");
            return 0;
        }
        ...
        if (conn) {
            log_fn(LOG_PROTOCOL_WARN, domain, "Begin cell for known stream. Dropping.");
            return 0;
        }
    ...
    case RELAY_COMMAND_DATA:
        ...
        if (rh->stream_id == 0) {
            ...
            if (CIRCUIT_IS_ORIGIN(circ)) {
                origin_circuit_t *ocirc = TO_ORIGIN_CIRCUIT(circ);
                if (connection_half_edge_is_valid_data(ocirc->half_streams, rh->stream_id)) {
                    circuit_read_valid_data(ocirc, rh->length);
                    log_info(domain,
            ...
            log_info(domain, "data cell dropped, unknown stream (streamid %d).", rh->stream_id);
            return 0;
        }
        ...
        connection_buf_add((char *)(cell->payload + RELAY_HEADER_SIZE), rh->length, TO_CONN(conn));
    ...
    case RELAY_COMMAND_END:
        reason =
            rh->length > 0 ? get_uint8(cell->payload + RELAY_HEADER_SIZE) : END_STREAM_REASON_MISC;
        if (!conn) {
            if (CIRCUIT_IS_ORIGIN(circ)) {
                origin_circuit_t *ocirc = TO_ORIGIN_CIRCUIT(circ);
                if (connection_half_edge_is_valid_end(ocirc->half_streams, rh->stream_id)) {
                    ...
                    log_info(domain,
                             ...
                             "stream id %d",
                             stream_end_reason_to_string(reason), ocirc->global_identifier,
                             rh->stream_id);
                    return 0;
                }
            }
        ...
        log_info(domain,
                 TOR_SOCKET_T_FORMAT ": end cell (%s) for stream %d. "
                                     "Removing stream.",
                 conn->base_.s, stream_end_reason_to_string(reason), conn->stream_id);
        if (conn->base_.type == CONN_TYPE_AP) {
            entry_connection_t *entry_conn = EDGE_TO_ENTRY_CONN(conn);
            if (entry_conn->socks_request && !entry_conn->socks_request->has_finished)
...
#define EARLY_WARNING_INTERVAL 3600
            static ratelim_t early_warning_limit = RATELIM_INIT(EARLY_WARNING_INTERVAL);
            char *m;
            if (cell->command == CELL_RELAY) {
                ...
                log_fn(LOG_WARN, domain, "EXTEND cell received, in a cell with type %d! Dropping.",
                       cell->command);
            }
            return 0;
        }
    ...
    case RELAY_COMMAND_TRUNCATE:
        if (layer_hint) {
            log_fn(LOG_PROTOCOL_WARN, LD_APP, "'truncate' unsupported at origin. Dropping.");
            return 0;
        }
        if (circ->n_hop) {
            if (circ->n_chan)
        ...
        log_debug(LD_EXIT, "Processed 'truncate', replying.");
        {
            ...
            relay_send_command_from_edge(0, circ, RELAY_COMMAND_TRUNCATED, payload, sizeof(payload),
                                         NULL);
        }
        return 0;
    case RELAY_COMMAND_TRUNCATED:
        if (!layer_hint) {
            log_fn(LOG_PROTOCOL_WARN, LD_EXIT, "'truncated' unsupported at non-origin. Dropping.");
            return 0;
        }
        ...
        circuit_truncated(TO_ORIGIN_CIRCUIT(circ), get_uint8(cell->payload + RELAY_HEADER_SIZE));
        return 0;
    case RELAY_COMMAND_CONNECTED:
        if (conn) {
            log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
        ...
        if (CIRCUIT_IS_ORIGIN(circ)) {
            origin_circuit_t *ocirc = TO_ORIGIN_CIRCUIT(circ);
            if (connection_half_edge_is_valid_connected(ocirc->half_streams, rh->stream_id)) {
                circuit_read_valid_data(ocirc, rh->length);
                log_info(domain,
    ...
    case RELAY_COMMAND_RESOLVE:
        if (layer_hint) {
            log_fn(LOG_PROTOCOL_WARN, LD_APP, "resolve request unsupported at AP; dropping.");
            return 0;
        } else if (conn) {
            log_fn(LOG_PROTOCOL_WARN, domain, "resolve request for known stream; dropping.");
            return 0;
        } else if (circ->purpose != CIRCUIT_PURPOSE_OR) {
            log_fn(LOG_PROTOCOL_WARN, domain, "resolve request on circ with purpose %d; dropping",
                   circ->purpose);
            return 0;
        }
    ...
    case RELAY_COMMAND_RESOLVED:
        if (conn) {
            log_fn(LOG_PROTOCOL_WARN, domain, "'resolved' unsupported while open. Closing circ.");
            return -END_CIRC_REASON_TORPROTOCOL;
        }

        if (CIRCUIT_IS_ORIGIN(circ)) {
            origin_circuit_t *ocirc = TO_ORIGIN_CIRCUIT(circ);
            if (connection_half_edge_is_valid_resolved(ocirc->half_streams, rh->stream_id)) {
                circuit_read_valid_data(ocirc, rh->length);
                log_info(domain,
 ...
 */
STATIC int
connection_edge_process_relay_cell(cell_t *cell, circuit_t *circ, edge_connection_t *conn,
                                   crypt_path_t *layer_hint)
{
    ...
    num_seen++;
    log_debug(domain, "Now seen %d relay cells here (command %d, stream %d).", num_seen, rh.command,
              rh.stream_id);
    ...
    if (conn && !connection_state_is_open(TO_CONN(conn))) {
        if (conn->base_.type == CONN_TYPE_EXIT &&
            ...
            return connection_edge_process_relay_cell_not_open(&rh, cell, circ, conn, layer_hint);
        }
    }

    return handle_relay_cell_command(cell, circ, conn, layer_hint, &rh, optimistic_data);
}
 ...
 */
STATIC size_t
connection_edge_get_inbuf_bytes_to_package(size_t n_available, int package_partial,
                                           circuit_t *on_circuit)
{
    ...
    const bool force_random_bytes = (on_circuit->send_randomness_after_n_cells == 0) &&
 ...
 */
int
connection_edge_package_raw_inbuf(edge_connection_t *conn, int package_partial, int *max_cells)
{
    ...
    const int sending_optimistically =
        entry_conn && conn->base_.type == CONN_TYPE_AP && conn->base_.state != AP_CONN_STATE_OPEN;
...
repeat_connection_edge_package_raw_inbuf:
    ...
    if (conn->package_window <= 0) {
        log_info(domain, "called with package_window %d. Skipping.", conn->package_window);
        connection_stop_reading(TO_CONN(conn));
        return 0;
    }

    sending_from_optimistic = entry_conn && entry_conn->sending_optimistic_data != NULL;
    ...
    length = connection_edge_get_inbuf_bytes_to_package(bytes_to_process, package_partial, circ);
    if (!length)
        return 0;
    ...
    log_debug(domain, TOR_SOCKET_T_FORMAT ": Packaging %d bytes (%d waiting).", conn->base_.s,
...
circuit_resume_edge_reading(circuit_t *circ, crypt_path_t *layer_hint)
{
    ...
    if (CIRCUIT_IS_ORIGIN(circ))
        ...
        circuit_resume_edge_reading_helper(TO_OR_CIRCUIT(circ)->n_streams, circ, layer_hint);
}
 ...
 */
static int
circuit_resume_edge_reading_helper(edge_connection_t *first_conn, circuit_t *circ,
                                   crypt_path_t *layer_hint)
{
...
circuit_consider_stop_edge_reading(circuit_t *circ, crypt_path_t *layer_hint)
{
    ...
    if (!layer_hint) {
        or_circuit_t *or_circ = TO_OR_CIRCUIT(circ);
        log_debug(domain, "considering circ->package_window %d", circ->package_window);
    ...
    log_debug(domain, "considering layer_hint->package_window %d", layer_hint->package_window);
    if (layer_hint->package_window <= 0) {
        log_debug(domain, "yes, at-origin. stopped.");
        for (conn = TO_ORIGIN_CIRCUIT(circ)->p_streams; conn; conn = conn->next_stream) {
...
dump_cell_pool_usage(int severity)
{
    ...
    tor_log(severity, LD_MM, "%d cells allocated on %d circuits. %d cells leaked.", n_cells,
 ...
 */
void
cell_queue_append_packed_copy(circuit_t *circ, cell_queue_t *queue, int exitward,
                              const cell_t *cell, int wide_circ_ids, int use_stats)
{
...
void
destroy_cell_queue_append(destroy_cell_queue_t *queue, circid_t circid, uint8_t reason)
{
    ...
    if (alloc >= get_options()->MaxMemInQueues_low_threshold) {
        ...
        if (alloc >= get_options()->MaxMemInQueues) {
            ...
            if (geoip_client_cache_total > get_options()->MaxMemInQueues / 5) {
                const size_t bytes_to_remove =
                    geoip_client_cache_total - (size_t)(get_options()->MaxMemInQueues / 10);
...
{
    return last_time_under_memory_pressure + MEMORY_PRESSURE_INTERVAL < approx_time();
}
 ...
 */
void
update_circuit_on_cmux_(circuit_t *circ, cell_direction_t direction, const char *file, int lineno)
{
    channel_t *chan = NULL;
    ...
    if (!circuitmux_is_circuit_attached(cmux, circ)) {
        log_warn(LD_BUG, "called on non-attached circuit from %s:%d", file, lineno);
        return;
    }
 ...
 */
static int
set_streams_blocked_on_circ(circuit_t *circ, channel_t *chan, int block, streamid_t stream_id)
{
...
channel_flush_from_first_active_circuit, (channel_t * chan, int max))
{
    ...
    while (n_flushed < max) {
        ...
        if (get_options()->CellStatistics || get_options()->TestingEnableCellStatsEvent) {
            uint32_t timestamp_now = monotime_coarse_get_stamp();
            uint32_t msec_waiting = (uint32_t)monotime_coarse_stamp_units_to_approx_msec(
        ...
        if (queue->n == 0 && chan->dirreq_id)
            geoip_change_dirreq_state(chan->dirreq_id, DIRREQ_TUNNELED, DIRREQ_CIRC_QUEUE_FLUSHED);
...
static int32_t max_circuit_cell_queue_size = RELAY_CIRC_CELL_QUEUE_SIZE_DEFAULT;
...
relay_consensus_has_changed(const networkstatus_t *ns)
{
    ...
    max_circuit_cell_queue_size =
        networkstatus_get_param(ns, "circ_max_cell_queue_size", RELAY_CIRC_CELL_QUEUE_SIZE_DEFAULT,
                             ...
                             cell_direction_t direction, streamid_t fromstream)
{
    ...
    if (PREDICT_UNLIKELY(queue->n >= max_circuit_cell_queue_size)) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
               ...
               (exitward) ? "Outbound" : "Inbound", queue->n, max_circuit_cell_queue_size);
    ...
    cell_queue_append_packed_copy(circ, queue, exitward, cell, chan->wide_circ_ids, 1);
...
const uint8_t *
decode_address_from_payload(tor_addr_t *addr_out, const uint8_t *payload, int payload_len)
{
    if (payload_len < 2)
        return NULL;
