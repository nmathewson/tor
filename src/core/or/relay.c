                           ...
                           cell_direction_t cell_direction)
{
    ...
    if (recognized) {
        ...
        if (cell_direction == CELL_DIRECTION_OUT) {
            ...
            reason =
                connection_edge_process_relay_cell(cell, circ, conn, NULL);
            if (reason < 0) {
                log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
                                   ...
                                   crypt_path_t *layer_hint)
{
    ...
    if (edge_reason_is_retriable(reason) &&
        ...
        case END_STREAM_REASON_EXITPOLICY: {
            ...
            if (rh->length >= 5) {
                ...
                if (rh->length == 5 || rh->length == 9) {
                    ...
                    tor_addr_from_ipv6_bytes(
                        &addr, (cell->payload + RELAY_HEADER_SIZE + 1));
                    if (rh->length == 21)
                        ttl = (int)ntohl(get_uint32(cell->payload +
                                                    RELAY_HEADER_SIZE + 17));
                }
                ...
                client_dns_set_addressmap(conn, conn->socks_request->address,
                                          &addr, conn->chosen_exit_name, ttl);
            ...
            if (connection_ap_detach_retriable(conn, circ, control_reason) >=
                0)
                return 0;
    ...
    if (conn->base_.type == CONN_TYPE_AP &&
        ...
        if (tor_addr_family(&addr) != AF_UNSPEC) {
            ...
            client_dns_set_addressmap(
                entry_conn, entry_conn->socks_request->address, &addr,
    ...
    case RELAY_COMMAND_EXTENDED2:
        ...
        log_debug(domain, "Got an extended cell! Yay.");
        {
            ...
            if ((reason = circuit_finish_handshake(
                     TO_ORIGIN_CIRCUIT(circ), &extended_cell.created_cell)) <
                0) {
                circuit_mark_for_close(circ, -reason);
                return 0; /* We don't want to cause a warning, so we mark the
                           * circuit here. */
            }
        }
                                           ...
                                           circuit_t *on_circuit)
{
    ...
    if (package_length <= RELAY_PAYLOAD_LENGTH_FOR_RANDOM_SENDMES) {
        /* This cell will have enough randomness in the padding to make a
         * future sendme cell unpredictable. */
                                  ...
                                  int *max_cells)
{
    ...
    const unsigned domain =
        conn->base_.type == CONN_TYPE_AP ? LD_APP : LD_EXIT;
...
repeat_connection_edge_package_raw_inbuf:
    ...
    if (connection_edge_send_command(conn, RELAY_COMMAND_DATA, payload,
                                     length) < 0) {
        /* circuit got marked for close, don't continue, don't need to mark
         * conn */
        return 0;
    }
                                   ...
                                   circuit_t *circ, crypt_path_t *layer_hint)
{
     ...
     * don't need cryptographic randomness here. */
    {
        ...
        for (conn = first_conn; conn; conn = conn->next_stream) {
            ...
            /* Invariant: chosen_stream has been chosen uniformly at random
             * from among the first num_streams streams on first_conn.
             *
...
again:
    ...
    for (conn = first_conn; conn; conn = conn->next_stream) {
        ...
        if (!layer_hint || conn->cpath_layer == layer_hint) {
            ...
            /* If the circuit won't accept any more data, return without
             * looking at any more of the streams. Any connections that should
             * be stopped have already been stopped by
          ...
          (channel_t * chan, int max))
{
    ...
    while (n_flushed < max) {
         ...
         * circuit selection, so we have to loop around for another even if
         * this circuit has more than one.
         */
         ...
         * in which case that was an immediate double-free bug).
         */
        ...
        if (streams_blocked && queue->n <= CELL_QUEUE_LOWWATER_SIZE)
            set_streams_blocked_on_circ(circ, chan, 0,
                                        0); /* unblock streams */
