...
entry_connection_t *connection_ap_make_link(connection_t *partner, char *address, uint16_t port,
                                            const char *digest, int session_group,
                                            int isolation_flags, int use_begindir, int want_onehop);
void connection_ap_handshake_socks_reply(entry_connection_t *conn, char *reply, size_t replylen,
                                         int endreason);
...
void connection_ap_handshake_socks_resolved_addr(entry_connection_t *conn, const tor_addr_t *answer,
                                                 int ttl, time_t expires);
...
void connection_ap_mark_as_pending_circuit_(entry_connection_t *entry_conn, const char *file,
                                            int line);
...
#define CONNECTION_AP_EXPECT_NONPENDING(c)                                                         \
    do {                                                                                           \
        if (ENTRY_TO_CONN(c)->state == AP_CONN_STATE_CIRCUIT_WAIT) {                               \
            log_warn(LD_BUG, "At %s:%d: %p was unexpectedly in circuit_wait.", __FILE__, __LINE__, \
                     (c));                                                                         \
...
int connection_ap_handshake_rewrite_and_attach(entry_connection_t *conn, origin_circuit_t *circ,
                                               crypt_path_t *cpath);
...
int connection_edge_update_circuit_isolation(const entry_connection_t *conn, origin_circuit_t *circ,
                                             int dry_run);
...
STATIC int connected_cell_format_payload(uint8_t *payload_out, const tor_addr_t *addr,
                                         uint32_t ttl);
...
STATIC struct half_edge_t *connection_half_edge_find_stream_id(const smartlist_t *half_conns,
