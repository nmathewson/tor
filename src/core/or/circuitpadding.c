...
static inline circpad_circuit_state_t circpad_circuit_state(origin_circuit_t *circ);
...
static inline void
circpad_machine_update_state_length_for_nonpadding(circpad_machine_runtime_t *mi);
...
circpad_circuit_machineinfo_free_idx(circuit_t *circ, int idx)
{
    if (circ->padding_info[idx]) {
        log_fn(LOG_INFO, LD_CIRC, "Freeing padding info idx %d on circuit %u (%d)", idx,
...
circpad_marked_circuit_for_padding(circuit_t *circ, int reason)
{
    ...
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_BEGIN(i, circ)
    {
        ...
        log_info(LD_CIRC,
                 ...
                 CIRCUIT_IS_ORIGIN(circ) ? TO_ORIGIN_CIRCUIT(circ)->global_identifier : 0, i);
         ...
         * close it.
         */
        if (circ->padding_info[i]->last_cell_time_sec + (time_t)CIRCPAD_DELAY_MAX_SECS <
            approx_time()) {
            log_notice(LD_BUG,
...
free_circ_machineinfos_with_machine_num(circuit_t *circ, int machine_num)
{
    ...
    {
        if (circ->padding_machine[i] && circ->padding_machine[i]->machine_num == machine_num) {
...
{
    circpad_machine_runtime_t *mi = tor_malloc_zero(sizeof(circpad_machine_runtime_t));
...
circpad_machine_current_state(const circpad_machine_runtime_t *mi)
{
    ...
    if (mi->current_state == CIRCPAD_STATE_END) {
        ...
        log_fn(LOG_WARN, LD_CIRC, "Invalid circuit padding state %d", mi->current_state);
...
circpad_histogram_bin_to_usec(const circpad_machine_runtime_t *mi, circpad_hist_index_t bin)
{
    const circpad_state_t *state = circpad_machine_current_state(mi);
 ...
 */
STATIC circpad_delay_t
histogram_get_bin_upper_bound(const circpad_machine_runtime_t *mi, circpad_hist_index_t bin)
{
...
static circpad_delay_t
circpad_get_histogram_bin_midpoint(const circpad_machine_runtime_t *mi, int bin_index)
{
...
circpad_histogram_usec_to_bin(const circpad_machine_runtime_t *mi, circpad_delay_t usec)
{
    const circpad_state_t *state = circpad_machine_current_state(mi);
    circpad_delay_t rtt_add_usec = 0;
...
circpad_machine_setup_tokens(circpad_machine_runtime_t *mi)
{
    ...
    if (!mi->histogram || (mi->histogram && mi->histogram_len != state->histogram_len)) {
        ...
        mi->histogram = tor_malloc_zero(sizeof(circpad_hist_token_t) * state->histogram_len);
    }
    mi->histogram_len = state->histogram_len;

    memcpy(mi->histogram, state->histogram, sizeof(circpad_hist_token_t) * state->histogram_len);
}
...
circpad_choose_state_length(circpad_machine_runtime_t *mi)
{
    ...
    log_info(LD_CIRC, "State length sampled to %" PRIu64 " for circuit %u", mi->state_length,
             CIRCUIT_IS_ORIGIN(mi->on_circ) ? TO_ORIGIN_CIRCUIT(mi->on_circ)->global_identifier
                                            : 0);
}
 ...
 */
static circpad_delay_t
circpad_distribution_sample_iat_delay(const circpad_state_t *state, circpad_delay_t delay_shift)
{
...
circpad_machine_sample_delay(circpad_machine_runtime_t *mi)
{
    ...
    bin_choice = crypto_fast_rng_get_uint64(get_thread_fast_rng(), histogram_total_tokens);
    ...
    if (curr_bin >= CIRCPAD_INFINITY_BIN(state)) {
        if (state->token_removal != CIRCPAD_TOKEN_REMOVAL_NONE && mi->histogram[curr_bin] > 0) {
    ...
    return (circpad_delay_t)crypto_fast_rng_uint64_range(get_thread_fast_rng(), bin_start, bin_end);
}
...
{
    circpad_hist_index_t bin = circpad_histogram_usec_to_bin(mi, target_bin_usec);

    /* Don't remove from the infinity bin */
    for (; bin < CIRCPAD_INFINITY_BIN(mi); bin++) {
        if (mi->histogram[bin] && histogram_get_bin_upper_bound(mi, bin) >= target_bin_usec) {
            return bin;
        }
    }
                                  ...
                                  circpad_delay_t target_bin_usec)
{
    ...
    for (; bin >= 0; bin--) {
        if (mi->histogram[bin] && circpad_histogram_bin_to_usec(mi, bin) <= target_bin_usec) {
            return bin;
        }
    }
 ...
 */
STATIC void
circpad_machine_remove_higher_token(circpad_machine_runtime_t *mi, circpad_delay_t target_bin_usec)
{
    /* We need to remove the token from the first bin
     * whose upper bound is greater than the target, and that
     * has tokens remaining. */
    circpad_hist_index_t bin = circpad_machine_first_higher_index(mi, target_bin_usec);
 ...
 */
STATIC void
circpad_machine_remove_lower_token(circpad_machine_runtime_t *mi, circpad_delay_t target_bin_usec)
{
    circpad_hist_index_t bin = circpad_machine_first_lower_index(mi, target_bin_usec);
 ...
 */
STATIC void
circpad_machine_remove_closest_token(circpad_machine_runtime_t *mi, circpad_delay_t target_bin_usec,
                                     bool use_usec)
{
 ...
 */
static void
circpad_machine_remove_exact(circpad_machine_runtime_t *mi, circpad_delay_t target_bin_usec)
{
    circpad_hist_index_t bin = circpad_histogram_usec_to_bin(mi, target_bin_usec);
...
circpad_machine_count_padding_sent(circpad_machine_runtime_t *mi)
{
    ...
    if (mi->state_length != CIRCPAD_STATE_LENGTH_INFINITE && !BUG(mi->state_length <= 0)) {
        mi->state_length--;
    }
    ...
    if (circpad_is_token_removal_supported(mi)) {
        ...
        if (!BUG(mi->chosen_bin >= mi->histogram_len) && !BUG(mi->histogram[mi->chosen_bin] == 0)) {
...
circpad_machine_remove_token(circpad_machine_runtime_t *mi)
{
    ...
    target_bin_usec = (circpad_delay_t)MIN((current_time - mi->padding_scheduled_at_usec),
    ...
    default:
        tor_assert_nonfatal_unreached();
        log_warn(LD_BUG, "Circpad: Unknown token removal strategy %d", state->token_removal);
        break;
    }
}
...
circpad_send_command_to_hop,
          (origin_circuit_t * circ, uint8_t hopnum, uint8_t relay_command, const uint8_t *payload,
           ssize_t payload_len))
{
    ...
    if (!target_hop) {
        log_fn(LOG_WARN, LD_BUG, "Padding circuit %u has %d hops, not %d", circ->global_identifier,
    ...
    if (target_hop->state != CPATH_STATE_OPEN) {
        log_fn(LOG_WARN, LD_CIRC, "Padding circuit %u has %d hops, not %d", circ->global_identifier,
               circuit_get_cpath_opened_len(circ), hopnum);
        return -1;
    }

    /* Send the drop command to the second hop */
    ret = relay_send_command_from_edge(0, TO_CIRCUIT(circ), relay_command, (const char *)payload,
                                       payload_len, target_hop);
    return ret;
}
...
circpad_send_padding_cell_for_callback(circpad_machine_runtime_t *mi)
{
    ...
    if (mi->on_circ->marked_for_close) {
        log_fn(LOG_INFO, LD_CIRC, "Padding callback on circuit marked for close (%u). Ignoring.",
               CIRCUIT_IS_ORIGIN(mi->on_circ) ? TO_ORIGIN_CIRCUIT(mi->on_circ)->global_identifier
                                              : 0);
        return CIRCPAD_STATE_CHANGED;
    }
    ...
    if (CIRCUIT_IS_ORIGIN(mi->on_circ)) {
        circpad_send_command_to_hop(TO_ORIGIN_CIRCUIT(mi->on_circ),
                                    CIRCPAD_GET_MACHINE(mi)->target_hopnum, RELAY_COMMAND_DROP,
                                    NULL, 0);
        log_info(LD_CIRC,
                 "Callback: Sending padding to origin circuit %u"
                 " (%d) [length: %" PRIu64 "]",
                 TO_ORIGIN_CIRCUIT(mi->on_circ)->global_identifier, mi->on_circ->purpose,
                 mi->state_length);
    } else {
 ...
 */
static void
circpad_send_padding_callback(tor_timer_t *timer, void *args, const struct monotime_t *time)
{
    ...
    if (mi && mi->on_circ) {
        ...
        log_fn(LOG_WARN, LD_CIRC, "Circuit closed while waiting for padding timer.");
        tor_fragile_assert();
    }
...
{
    circpad_padding_disabled = networkstatus_get_param(ns, "circpad_padding_disabled", 0, 0, 1);

    circpad_padding_reduced = networkstatus_get_param(ns, "circpad_padding_reduced", 0, 0, 1);

    circpad_global_allowed_cells =
        networkstatus_get_param(ns, "circpad_global_allowed_cells", 0, 0, UINT16_MAX - 1);
    ...
    circpad_max_circ_queued_cells = networkstatus_get_param(
        ns, "circpad_max_circ_queued_cells", CIRCWINDOW_START_MAX, 0, 50 * CIRCWINDOW_START_MAX);
}
...
circpad_machine_reached_padding_limit(circpad_machine_runtime_t *mi)
{
    ...
    if (machine->max_padding_percent && mi->padding_sent >= machine->allowed_padding_count) {
        uint32_t total_cells = mi->padding_sent + mi->nonpadding_sent;

        /* Check the percent */
        if ((100 * (uint32_t)mi->padding_sent) / total_cells > machine->max_padding_percent) {
    ...
    if (circpad_global_max_padding_percent &&
        ...
        uint64_t total_cells = circpad_global_padding_sent + circpad_global_nonpadding_sent;
...
circpad_machine_schedule_padding, (circpad_machine_runtime_t * mi))
{
    ...
    if (!circpad_is_padding_allowed()) {
        ...
        log_fn_ratelim(&padding_lim, LOG_INFO, LD_CIRC,
                       "Padding has been disabled, but machine still on circuit %" PRIu64 ", %d",
    ...
    if (circpad_machine_reached_padding_limit(mi)) {
        if (CIRCUIT_IS_ORIGIN(mi->on_circ)) {
            log_fn(LOG_INFO, LD_CIRC, "Padding machine has reached padding limit on circuit %u",
                   TO_ORIGIN_CIRCUIT(mi->on_circ)->global_identifier);
        } else {
            static ratelim_t padding_lim = RATELIM_INIT(600);
            log_fn_ratelim(&padding_lim, LOG_INFO, LD_CIRC,
                           "Padding machine has reached padding limit on circuit %" PRIu64 ", %d",
                           mi->on_circ->n_chan ? mi->on_circ->n_chan->global_identifier : 0,
                           mi->on_circ->n_circ_id);
        }
        return CIRCPAD_STATE_UNCHANGED;
    ...
    log_fn(LOG_INFO, LD_CIRC, "\tPadding in %u usec on circuit %u", in_usec,
           CIRCUIT_IS_ORIGIN(mi->on_circ) ? TO_ORIGIN_CIRCUIT(mi->on_circ)->global_identifier : 0);
...
circpad_machine_spec_transitioned_to_end(circpad_machine_runtime_t *mi)
{
    ...
    log_fn(LOG_INFO, LD_CIRC, "Padding machine in end state on circuit %u (%d)",
           CIRCUIT_IS_ORIGIN(on_circ) ? TO_ORIGIN_CIRCUIT(on_circ)->global_identifier : 0,
           on_circ->purpose);
    ...
    if (machine->should_negotiate_end) {
        if (machine->is_origin_side) {
            ...
            circpad_negotiate_padding(TO_ORIGIN_CIRCUIT(on_circ), machine->machine_num,
                                      machine->target_hopnum, CIRCPAD_COMMAND_STOP);
        } else {
            circpad_circuit_machineinfo_free_idx(on_circ, machine->machine_index);
            circpad_padding_negotiated(on_circ, machine->machine_num, CIRCPAD_COMMAND_STOP,
                                       CIRCPAD_RESPONSE_OK);
          ...
          (circpad_machine_runtime_t * mi, circpad_event_t event))
{
    ...
    if (state->next_state[event] == CIRCPAD_STATE_IGNORE) {
         ...
         */
        log_fn(LOG_INFO, LD_CIRC, "Circuit %u circpad machine %d transitioning from %u to %u",
               CIRCUIT_IS_ORIGIN(mi->on_circ) ? TO_ORIGIN_CIRCUIT(mi->on_circ)->global_identifier
                                              : 0,
 ...
 */
static void
circpad_estimate_circ_rtt_on_received(circuit_t *circ, circpad_machine_runtime_t *mi)
{
    ...
    if (mi->last_received_time_usec) {
        ...
        if (circ->state == CIRCUIT_STATE_OPEN) {
            log_fn(LOG_INFO, LD_CIRC,
                   ...
                   circ->n_chan ? circ->n_chan->global_identifier : 0, circ->n_circ_id,
                   mi->rtt_estimate_usec);
 ...
 */
static void
circpad_estimate_circ_rtt_on_send(circuit_t *circ, circpad_machine_runtime_t *mi)
{
    /* Origin circuits don't estimate RTT. They could do it easily enough,
     * but they have no reason to use it in any delay calculations. */
    ...
    if (mi->last_received_time_usec) {
        circpad_time_t rtt_time = monotime_absolute_usec() - mi->last_received_time_usec;
        ...
        if (rtt_time >= INT32_MAX) {
            log_fn(LOG_WARN, LD_CIRC,
                   "Circuit padding RTT estimate overflowed: %" PRIu64 " vs %" PRIu64,
...
circpad_cell_event_nonpadding_sent(circuit_t *on_circ)
{
    ...
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_BEGIN(i, on_circ)
    {
        ...
        if (check_machine_token_supply(on_circ->padding_info[i]) == CIRCPAD_STATE_UNCHANGED) {
 ...
 */
int
circpad_check_received_cell(cell_t *cell, circuit_t *circ, crypt_path_t *layer_hint,
...
circpad_cell_event_nonpadding_received(circuit_t *on_circ)
{
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_BEGIN(i, on_circ)
    {
        ...
        circpad_machine_spec_transition(on_circ->padding_info[i], CIRCPAD_EVENT_NONPADDING_RECV);
    }
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_END;
}
...
circpad_cell_event_padding_sent(circuit_t *on_circ)
{
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_BEGIN(i, on_circ)
    {
        ...
        if (check_machine_token_supply(on_circ->padding_info[i]) == CIRCPAD_STATE_UNCHANGED) {
            ...
            on_circ->padding_info[i]->last_cell_time_sec = approx_time();
            circpad_machine_spec_transition(on_circ->padding_info[i], CIRCPAD_EVENT_PADDING_SENT);
        }
    }
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_END;
}
...
circpad_cell_event_padding_received(circuit_t *on_circ)
{
    ...
    {
        on_circ->padding_info[i]->last_cell_time_sec = approx_time();
        circpad_machine_spec_transition(on_circ->padding_info[i], CIRCPAD_EVENT_PADDING_RECV);
    }
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_END;
}
...
{
    if (circpad_machine_spec_transition(mi, CIRCPAD_EVENT_BINS_EMPTY) == CIRCPAD_STATE_CHANGED) {
        return CIRCPAD_STATE_CHANGED;
    } else {
 ...
 */
static inline bool
circpad_machine_conditions_met(origin_circuit_t *circ, const circpad_machine_spec_t *machine)
{
 ...
 */
STATIC void
circpad_add_matching_machines(origin_circuit_t *on_circ, smartlist_t *machines_sl)
{
    circuit_t *circ = TO_CIRCUIT(on_circ);
    ...
    FOR_EACH_CIRCUIT_MACHINE_BEGIN(i)
    {
        ...
        SMARTLIST_FOREACH_REVERSE_BEGIN(machines_sl, circpad_machine_spec_t *, machine)
        {
            ...
            if (machine->machine_index == i && circpad_machine_conditions_met(on_circ, machine)) {
                ...
                if (circ->padding_machine[i]) {
                    if (circ->padding_machine[i]->target_hopnum != machine->target_hopnum)
                        continue;
                ...
                if (circpad_negotiate_padding(on_circ, machine->machine_num, machine->target_hopnum,
                                              CIRCPAD_COMMAND_START) < 0) {
                    log_info(LD_CIRC, "Padding not negotiated. Cleaning machine from circuit %u",
...
circpad_padding_is_from_expected_hop(circuit_t *circ, crypt_path_t *from_hop)
{
    ...
    FOR_EACH_CIRCUIT_MACHINE_BEGIN(i)
    {
        ...
        target_hop =
            circuit_get_cpath_hop(TO_ORIGIN_CIRCUIT(circ), circ->padding_machine[i]->target_hopnum);
 ...
 */
void
circpad_deliver_recognized_relay_cell_events(circuit_t *circ, uint8_t relay_command,
 ...
 */
void
circpad_machine_states_init(circpad_machine_spec_t *machine, circpad_statenum_t num_states)
{
...
static void
circpad_setup_machine_on_circ(circuit_t *on_circ, const circpad_machine_spec_t *machine)
{
    if (CIRCUIT_IS_ORIGIN(on_circ) && !machine->is_origin_side) {
        log_fn(LOG_WARN, LD_BUG, "Can't set up non-origin machine on origin circuit!");
        return;
    }

    if (!CIRCUIT_IS_ORIGIN(on_circ) && machine->is_origin_side) {
        log_fn(LOG_WARN, LD_BUG, "Can't set up origin machine on non-origin circuit!");
        return;
    }
    ...
    if (CIRCUIT_IS_ORIGIN(on_circ)) {
        log_info(LD_CIRC, "Registering machine %s to origin circ %u (%d)", machine->name,
                 TO_ORIGIN_CIRCUIT(on_circ)->global_identifier, on_circ->purpose);
    } else {
        log_info(LD_CIRC, "Registering machine %s to non-origin circ (%d)", machine->name,
                 on_circ->purpose);
    }
...
padding_machine_state_is_valid(const circpad_state_t *state)
{
    ...
    for (b = 0; b < state->histogram_len; b++) {
        ...
        if (prev_bin_edge >= state->histogram_edges[b] && b > 0) {
            log_warn(LD_CIRC, "Histogram edges are not increasing [%u/%u]", prev_bin_edge,
...
void
circpad_register_padding_machine(circpad_machine_spec_t *machine, smartlist_t *machine_list)
{
    if (!padding_machine_is_valid(machine)) {
        log_warn(LD_CIRC, "Machine #%u is invalid. Ignoring.", machine->machine_num);
        return;
    }
...
{
    circpad_machine_spec_t *circ_client_machine = tor_malloc_zero(sizeof(circpad_machine_spec_t));
    ...
    circ_client_machine->states[CIRCPAD_STATE_START].next_state[CIRCPAD_EVENT_NONPADDING_RECV] =
        CIRCPAD_STATE_BURST;

    circ_client_machine->states[CIRCPAD_STATE_BURST].next_state[CIRCPAD_EVENT_NONPADDING_RECV] =
        CIRCPAD_STATE_BURST;
    circ_client_machine->states[CIRCPAD_STATE_BURST].next_state[CIRCPAD_EVENT_PADDING_RECV] =
        CIRCPAD_STATE_BURST;
    ...
    circ_client_machine->states[CIRCPAD_STATE_BURST].next_state[CIRCPAD_EVENT_NONPADDING_SENT] =
        CIRCPAD_STATE_CANCEL;

    circ_client_machine->states[CIRCPAD_STATE_BURST].next_state[CIRCPAD_EVENT_BINS_EMPTY] =
        CIRCPAD_STATE_END;

    circ_client_machine->states[CIRCPAD_STATE_BURST].token_removal = CIRCPAD_TOKEN_REMOVAL_CLOSEST;
    ...
    circpad_register_padding_machine(circ_client_machine, origin_padding_machines);
}
...
circpad_circ_responder_machine_init(void)
{
     ...
     * recieve */
    circ_responder_machine->states[CIRCPAD_STATE_START].next_state[CIRCPAD_EVENT_PADDING_RECV] =
        CIRCPAD_STATE_BURST;
    circ_responder_machine->states[CIRCPAD_STATE_START].next_state[CIRCPAD_EVENT_NONPADDING_RECV] =
        CIRCPAD_STATE_BURST;

    /* Inside the burst state we _stay_ in the burst state when a non-padding
     * is sent */
    circ_responder_machine->states[CIRCPAD_STATE_BURST].next_state[CIRCPAD_EVENT_NONPADDING_SENT] =
        CIRCPAD_STATE_BURST;

    /* Inside the burst state we transition to the gap state when we receive a
     * padding cell */
    circ_responder_machine->states[CIRCPAD_STATE_BURST].next_state[CIRCPAD_EVENT_PADDING_RECV] =
        CIRCPAD_STATE_GAP;
    ...
    circ_responder_machine->states[CIRCPAD_STATE_BURST].histogram_edges[1] = 1000000;
    ...
    circ_responder_machine->states[CIRCPAD_STATE_BURST].histogram_total_tokens = 1;

    /* From the gap state, we _stay_ in the gap state, when we receive padding
     * or non padding */
    circ_responder_machine->states[CIRCPAD_STATE_GAP].next_state[CIRCPAD_EVENT_PADDING_RECV] =
        CIRCPAD_STATE_GAP;
    circ_responder_machine->states[CIRCPAD_STATE_GAP].next_state[CIRCPAD_EVENT_NONPADDING_RECV] =
        CIRCPAD_STATE_GAP;

    /* And from the gap state, we go to the end, when the bins are empty or a
     * non-padding cell is sent */
    circ_responder_machine->states[CIRCPAD_STATE_GAP].next_state[CIRCPAD_EVENT_BINS_EMPTY] =
        CIRCPAD_STATE_END;
    circ_responder_machine->states[CIRCPAD_STATE_GAP].next_state[CIRCPAD_EVENT_NONPADDING_SENT] =
        CIRCPAD_STATE_END;
    ...
    circpad_register_padding_machine(circ_responder_machine, relay_padding_machines);
}
...
circpad_machines_free(void)
{
    if (origin_padding_machines) {
        SMARTLIST_FOREACH(origin_padding_machines, circpad_machine_spec_t *, m, tor_free(m->states);
                          tor_free(m));
        smartlist_free(origin_padding_machines);
    }

    if (relay_padding_machines) {
        SMARTLIST_FOREACH(relay_padding_machines, circpad_machine_spec_t *, m, tor_free(m->states);
                          tor_free(m));
...
circpad_node_supports_padding(const node_t *node)
{
    if (node->rs) {
        log_fn(LOG_INFO, LD_CIRC, "Checking padding: %s",
               node->rs->pv.supports_hs_setup_padding ? "supported" : "unsupported");
...
circpad_negotiate_padding(origin_circuit_t *circ, circpad_machine_num_t machine,
                          uint8_t target_hopnum, uint8_t command)
{
    ...
    if ((len = circpad_negotiate_encode(cell.payload, CELL_PAYLOAD_SIZE, &type)) < 0)
        return -1;
    ...
    return circpad_send_command_to_hop(circ, target_hopnum, RELAY_COMMAND_PADDING_NEGOTIATE,
                                       cell.payload, len);
}
 ...
 */
bool
circpad_padding_negotiated(circuit_t *circ, circpad_machine_num_t machine, uint8_t command,
                           uint8_t response)
{
    ...
    if ((len = circpad_negotiated_encode(cell.payload, CELL_PAYLOAD_SIZE, &type)) < 0)
        return 0;

    /* Use relay_send because we're from the middle to the origin. We don't
     * need to specify a target hop or layer_hint. */
    return relay_send_command_from_edge(0, circ, RELAY_COMMAND_PADDING_NEGOTIATED,
                                        (void *)cell.payload, (size_t)len, NULL) == 0;
}
...
circpad_handle_padding_negotiate(circuit_t *circ, cell_t *cell)
{
    ...
    if (circpad_negotiate_parse(&negotiate, cell->payload + RELAY_HEADER_SIZE,
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_CIRC, "Received malformed PADDING_NEGOTIATE cell; dropping.");
        return -1;
    }

    if (negotiate->command == CIRCPAD_COMMAND_STOP) {
        /* Free the machine corresponding to this machine type */
        if (free_circ_machineinfos_with_machine_num(circ, negotiate->machine_type)) {
            log_info(LD_CIRC, "Received STOP command for machine %u", negotiate->machine_type);
            goto done;
        }
        log_fn(LOG_PROTOCOL_WARN, LD_CIRC,
        ...
        SMARTLIST_FOREACH_BEGIN (relay_padding_machines, const circpad_machine_spec_t *, m) {
 ...
 */
signed_error_t
circpad_handle_padding_negotiated(circuit_t *circ, cell_t *cell, crypt_path_t *layer_hint)
{
    circpad_negotiated_t *negotiated;

    if (!CIRCUIT_IS_ORIGIN(circ)) {
        log_fn(LOG_PROTOCOL_WARN, LD_CIRC, "Padding negotiated cell unsupported at non-origin.");
        return -1;
    }

    /* Verify this came from the expected hop */
    if (!circpad_padding_is_from_expected_hop(circ, layer_hint)) {
        log_fn(LOG_PROTOCOL_WARN, LD_CIRC, "Padding negotiated cell from wrong hop on circuit %u",
               TO_ORIGIN_CIRCUIT(circ)->global_identifier);
        return -1;
    }
    ...
    if (negotiated->command == CIRCPAD_COMMAND_STOP) {
        log_info(LD_CIRC, "Received STOP command on PADDING_NEGOTIATED for circuit %u",
                 TO_ORIGIN_CIRCUIT(circ)->global_identifier);
...
{
    if (origin_padding_machines) {
        SMARTLIST_FOREACH_BEGIN (origin_padding_machines, circpad_machine_spec_t *, m) {
            machine_spec_free(m);
        } SMARTLIST_FOREACH_END (m);
        smartlist_free(origin_padding_machines);
    }
    if (relay_padding_machines) {
        SMARTLIST_FOREACH_BEGIN (relay_padding_machines, circpad_machine_spec_t *, m) {
            machine_spec_free(m);
        } SMARTLIST_FOREACH_END (m);
        smartlist_free(relay_padding_machines);
    }
