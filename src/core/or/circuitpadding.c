...
#    define FOR_EACH_CIRCUIT_MACHINE_BEGIN(loop_var)                \
        STMT_BEGIN                                                  \
            for (int loop_var = 0; loop_var < CIRCPAD_MAX_MACHINES; \
                 loop_var++) {
...
circpad_circuit_machineinfo_free_idx(circuit_t *circ, int idx)
{
    if (circ->padding_info[idx]) {
        log_fn(LOG_INFO, LD_CIRC,
               "Freeing padding info idx %d on circuit %u (%d)", idx,
...
circpad_marked_circuit_for_padding(circuit_t *circ, int reason)
{
    ...
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_BEGIN(i, circ)
    {
        ...
        /* If the machine does not want to control the circuit close itself,
                              ...
                              circpad_delay_t usec)
{
    ...
    /* Walk through the bins and check the upper bound of each bin, if 'usec'
     * is less-or-equal to that, return that bin. If rtt_estimate is enabled
     * then add that to the upper bound of each bin.
     *
    ...
    /* We don't want to return the infinity bin here, so if we still didn't
     * find the right bin, return the highest non-infinity bin */
...
circpad_is_token_removal_supported(circpad_machine_runtime_t *mi)
{
    ...
    if (mi->histogram == NULL) {
        ...
        /* Machines that do want token removal are less sensitive to
         * performance. Let's spend some time to check that our state is
         * consistent and sane */
        const circpad_state_t *state = circpad_machine_current_state(mi);
        if (BUG(!state)) {
            return 1;
        }
        tor_assert_nonfatal(state->token_removal !=
                                     ...
                                     bool use_usec)
{
    ...
    if (use_usec) {
        ...
        circpad_delay_t lower_usec =
            circpad_get_histogram_bin_midpoint(mi, lower);
        ...
        } else if (target_bin_usec - lower_usec >
                   higher_usec - target_bin_usec) {
            // Higher bin is closer
            ENSURE_BIN_CAPACITY(higher);
            bin_to_remove = higher;
        } else {
...
circpad_machine_remove_token(circpad_machine_runtime_t *mi)
{
    ...
    /* If we have scheduled padding some time in the future, we want to see
       what bin we are in at the current time */
    target_bin_usec =
...
circpad_send_padding_cell_for_callback(circpad_machine_runtime_t *mi)
{
    ...
    if (CIRCUIT_IS_ORIGIN(mi->on_circ)) {
        ...
        // If we're a non-origin circ, we can just send from here as if we're
        // the edge.
        if (TO_OR_CIRCUIT(circ)->p_chan_cells.n <=
            circpad_max_circ_queued_cells) {
            log_info(LD_CIRC,
                     "Callback: Sending padding to circuit (%d)"
                     " [length: %" PRIu64 "]",
                     mi->on_circ->purpose, mi->state_length);
            relay_send_command_from_edge(0, mi->on_circ, RELAY_COMMAND_DROP,
                                         NULL, 0, NULL);
            rep_hist_padding_count_write(PADDING_TYPE_DROP);
        } else {
            static ratelim_t cell_lim = RATELIM_INIT(600);
            log_fn_ratelim(
                &cell_lim, LOG_NOTICE, LD_CIRC,
...
circpad_new_consensus_params(const networkstatus_t *ns)
{
    ...
    circpad_global_max_padding_percent = networkstatus_get_param(
        ns, "circpad_global_max_padding_pct", 0, 0, 100);
    ...
    if (circpad_machine_reached_padding_limit(mi)) {
        if (CIRCUIT_IS_ORIGIN(mi->on_circ)) {
            ...
            log_fn_ratelim(
                ...
                ", %d",
                mi->on_circ->n_chan ? mi->on_circ->n_chan->global_identifier
                                    : 0,
                mi->on_circ->n_circ_id);
        }
        return CIRCPAD_STATE_UNCHANGED;
...
circpad_machine_spec_transitioned_to_end(circpad_machine_runtime_t *mi)
{
    ...
    log_fn(LOG_INFO, LD_CIRC,
           "Padding machine in end state on circuit %u (%d)",
    ...
    if (machine->should_negotiate_end) {
        if (machine->is_origin_side) {
            ...
            circpad_negotiate_padding(
                TO_ORIGIN_CIRCUIT(on_circ), machine->machine_num,
          ...
          (circpad_machine_runtime_t * mi, circpad_event_t event))
{
    ...
    if (state->next_state[event] == CIRCPAD_STATE_IGNORE) {
        ...
        /* See if we need to transition to any other states based on this
         * event. Whenever a transition happens, even to our own state, we
         * schedule padding.
         *
         * So if a state only wants to schedule padding for an event, it
         * specifies a transition to itself. All non-specified events are
         * ignored.
         */
        log_fn(LOG_INFO, LD_CIRC,
        ...
        if (circ->state == CIRCUIT_STATE_OPEN) {
            ...
            if (!mi->rtt_estimate_usec) {
                ...
                log_fn_ratelim(&rtt_lim, LOG_NOTICE, LD_BUG,
                               "Circuit got two cells back to back before "
                               "estimating RTT.");
            }
        }
    } else {
                                  ...
                                  circpad_machine_runtime_t *mi)
{
    ...
    if (mi->last_received_time_usec) {
        ...
        /* If the old RTT estimate is lower than this one, use this one,
         * because the circuit is getting longer. If this estimate is somehow
        ...
        /* If last_received_time_usec is zero, then we have gotten two cells
         * back to back. Stop estimating RTT in this case. Note that we only
                            ...
                            crypt_path_t *layer_hint, const relay_header_t *rh)
{
     ...
     * We suspect that continuing to pad is optimal against a passive
     * classifier, but as soon as the adversary is active (even as a client
     * adversary) this might change.
     *
...
circpad_cell_event_nonpadding_received(circuit_t *on_circ)
{
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_BEGIN(i, on_circ)
    {
        /* First, update any timestamps */
        on_circ->padding_info[i]->last_cell_time_sec = approx_time();
        circpad_estimate_circ_rtt_on_received(on_circ,
                                              on_circ->padding_info[i]);
...
circpad_shutdown_old_machines(origin_circuit_t *on_circ)
{
    ...
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_BEGIN(i, circ)
    {
        if (!circpad_machine_conditions_met(on_circ,
            ...
            circpad_negotiate_padding(
                on_circ, circ->padding_machine[i]->machine_num,
                circ->padding_machine[i]->target_hopnum, CIRCPAD_COMMAND_STOP);
        }
    }
    FOR_EACH_ACTIVE_CIRCUIT_MACHINE_END;
}
    ...
    FOR_EACH_CIRCUIT_MACHINE_BEGIN(i)
    {
                                        ...
                                        machine)
        {
            ...
            if (machine->machine_index == i &&
                 ...
                 * We will tear it down on error return, or if there is an
                ...
                if (circpad_negotiate_padding(on_circ, machine->machine_num,
                    ...
                    log_info(LD_CIRC,
                             "Padding not negotiated. Cleaning machine from "
                             "circuit %u",
                             CIRCUIT_IS_ORIGIN(circ)
                                 ? TO_ORIGIN_CIRCUIT(circ)->global_identifier
                                 : 0);
...
circpad_deliver_unrecognized_cell_events(circuit_t *circ, cell_direction_t dir)
{
    ...
    if (dir == CELL_DIRECTION_OUT) {
        /* When direction is out (away from origin), then we received
           non-padding cell coming from the origin to us. */
                                             ...
                                             crypt_path_t *layer_hint)
{
    if (relay_command == RELAY_COMMAND_DROP) {
        ...
        /* The cell should be recognized by now, which means that we are on the
           destination, which means that we received a padding cell. We might
           be the client or the Middle node, still, because leaky-pipe. */
...
circpad_deliver_sent_relay_cell_events(circuit_t *circ, uint8_t relay_command)
{
     ...
     * or processed here).
     *
     * We do generate events for PADDING_NEGOTIATE and PADDING_NEGOTIATED
     * cells.
     */
    if (relay_command == RELAY_COMMAND_DROP) {
...
padding_machine_state_is_valid(const circpad_state_t *state)
{
    ...
    for (b = 0; b < state->histogram_len; b++) {
        ...
        /* Also count the number of tokens as we go through the histogram
         * states */
...
circpad_circ_client_machine_init(void)
{
    ...
    /* We have 5 tokens in the histogram, which means that all circuits will
     * look like they have 7 hops (since we start this machine after the second
     * hop, and tokens are decremented for any valid hops, and fake extends are
     * used after that -- 2+5==7). */
    circ_client_machine->states[CIRCPAD_STATE_BURST].histogram[0] = 5;

    circ_client_machine->states[CIRCPAD_STATE_BURST].histogram_total_tokens =
        5;
...
circpad_circ_responder_machine_init(void)
{
    ...
    /* use_rtt_estimate tries to estimate how long padding cells take to go
       from C->M, and uses that as what as the base of the histogram */
    ...
    circ_responder_machine->states[CIRCPAD_STATE_BURST].histogram_edges[0] =
        500;
    ...
    circ_responder_machine->states[CIRCPAD_STATE_GAP].histogram_edges[1] =
        1000;
    circ_responder_machine->states[CIRCPAD_STATE_GAP].histogram_edges[2] =
        2000;
    circ_responder_machine->states[CIRCPAD_STATE_GAP].histogram_edges[3] =
        4000;
    circ_responder_machine->states[CIRCPAD_STATE_GAP].histogram_edges[4] =
        8000;
    circ_responder_machine->states[CIRCPAD_STATE_GAP].histogram_edges[5] =
        16000;
    ...
    circ_responder_machine->states[CIRCPAD_STATE_GAP].histogram_total_tokens =
        6;

    circ_responder_machine->states[CIRCPAD_STATE_GAP].token_removal =
        CIRCPAD_TOKEN_REMOVAL_CLOSEST_USEC;

    circ_responder_machine->machine_num =
        smartlist_len(relay_padding_machines);
...
err:
    ...
    circpad_padding_negotiated(
        circ, negotiate->machine_type, negotiate->command,
                                  ...
                                  crypt_path_t *layer_hint)
{
    ...
    if (negotiated->command == CIRCPAD_COMMAND_STOP) {
         ...
         * than this response. */
        free_circ_machineinfos_with_machine_num(circ,
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_CIRC,
               "Middle node did not accept our padding request on circuit %u "
               "(%d)",
