...
HT_PROTOTYPE(channel_gid_map, channel_s, gidmap_node, channel_id_hash,
             channel_id_eq)
HT_GENERATE2(channel_gid_map, channel_s, gidmap_node, channel_id_hash,
...
static HT_HEAD(channel_idmap,
               channel_idmap_entry_s) channel_identity_map = HT_INITIALIZER();
...
channel_state_can_transition(channel_state_t from, channel_state_t to)
{
    ...
    switch (from) {
        ...
        case CHANNEL_STATE_CLOSING:
            is_valid =
                (to == CHANNEL_STATE_CLOSED || to == CHANNEL_STATE_ERROR);
            break;
        case CHANNEL_STATE_ERROR:
            is_valid = 0;
            break;
        case CHANNEL_STATE_MAINT:
            is_valid = (to == CHANNEL_STATE_CLOSING ||
                        to == CHANNEL_STATE_ERROR || to == CHANNEL_STATE_OPEN);
            break;
        case CHANNEL_STATE_OPENING:
            is_valid = (to == CHANNEL_STATE_CLOSING ||
                        to == CHANNEL_STATE_ERROR || to == CHANNEL_STATE_OPEN);
        ...
        case CHANNEL_STATE_OPEN:
            is_valid = (to == CHANNEL_STATE_CLOSING ||
                        to == CHANNEL_STATE_ERROR || to == CHANNEL_STATE_MAINT);
            break;
        case CHANNEL_STATE_LAST:
        default:
            is_valid = 0;
...
channel_listener_dumpstats(int severity)
{
    if (all_listeners && smartlist_len(all_listeners) > 0) {
        ...
        tor_log(severity, LD_GENERAL,
                "%d are active and %d are done and waiting for cleanup",
                (active_listeners != NULL) ? smartlist_len(active_listeners)
                                           : 0,
