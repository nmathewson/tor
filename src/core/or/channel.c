                                ...
                                const ed25519_public_key_t *ed_id)
{
    ...
    if (ed_id && ed25519_public_key_is_zero(ed_id)) {
        /* Treat zero as meaning "We don't care about the presence or absence
         * of an Ed key", not "There must be no Ed key". */
        ed_id = NULL;
    }
...
#define MIN_RELAY_CONNECTIONS_TO_WARN 5
    ...
    if (total_relays > MIN_RELAY_CONNECTIONS_TO_WARN &&
        ...
        log_notice(
            LD_OR,
            "Your relay has a very large number of connections to other "
            "relays. "
                            ...
                            const ed25519_public_key_t *ed_identity)
{
    ...
    log_debug(LD_CHANNEL,
              ...
              chan, (chan->global_identifier),
              identity_digest ? hex_str(identity_digest, DIGEST_LEN)
                              : "(null)");
...
channel_do_open_actions(channel_t *chan)
{
    ...
    if (started_here) {
        ...
        if (channel_is_client(chan)) {
            ...
            /* Otherwise the underlying transport can't tell us this, so skip
             * it */
        }
    }
...
channel_get_for_extend,
          (const char *rsa_id_digest, const ed25519_public_key_t *ed_id,
           const tor_addr_t *target_ipv4_addr,
           const tor_addr_t *target_ipv6_addr, const char **msg_out,
           int *launch_out))
{
    ...
    for (; chan; chan = channel_next_with_rsa_identity(chan)) {
        tor_assert(
            tor_memeq(chan->identity_digest, rsa_id_digest, DIGEST_LEN));
        ...
        const bool matches_target = channel_matches_target_addr_for_extend(
            chan, target_ipv4_addr, target_ipv6_addr);
        ...
        /* If the connection is using a recent link protocol, only return
         * canonical connections, when the address is one of the addresses we
         * wanted.
         *
        ...
        if (!channel_is_canonical(chan) &&
            channel_is_canonical_is_reliable(chan) && !matches_target) {
            ++n_noncanonical;
            continue;
        }

        if (!best) {
            best = chan; /* If we have no 'best' so far, this one is good
                            enough. */
            continue;
        }
...
{
    /* Param 1 indicates the actual description */
    return chan->get_remote_descr(chan,
                                  GRD_FLAG_ORIGINAL | GRD_FLAG_ADDR_ONLY);
}
           ...
           int consider_identity))
{
    ...
    our_identity = started_here ? get_tlsclient_identity_key()
...
channel_rsa_id_group_set_badness(struct channel_list_t *lst, int force)
{
    ...
    if (PREDICT_LIKELY(!TOR_LIST_NEXT(chan, next_with_same_id))) {
        connection_or_single_set_badness_(time(NULL),
                                          BASE_CHAN_TO_TLS(chan)->conn, force);
        return;
    }
    ...
    /* XXXX 15056 we may want to do something special with connections that
     * have no set Ed25519 identity! */
