...
static void cbt_control_event_buildtimeout_set(const circuit_build_times_t *cbt,
...
int
circuit_build_times_disabled_(const or_options_t *options, int ignore_consensus)
{
    if (unit_tests) {
        return 0;
    } else {
        int consensus_disabled =
            ignore_consensus ? 0 : networkstatus_get_param(NULL, "cbtdisabled", 0, 0, 1);
        ...
        int single_onion_disabled = rend_service_allow_non_anonymous_connection(options);

        if (consensus_disabled || config_disabled || dirauth_disabled || state_disabled ||
...
circuit_build_times_max_timeouts(void)
{
    ...
    cbt_maxtimeouts =
        networkstatus_get_param(NULL, "cbtmaxtimeouts", CBT_DEFAULT_MAX_RECENT_TIMEOUT_COUNT,
...
{
    int32_t num = networkstatus_get_param(NULL, "cbtnummodes", CBT_DEFAULT_NUM_XM_MODES,
...
{
    int32_t num =
        networkstatus_get_param(NULL, "cbtmincircs", CBT_DEFAULT_MIN_CIRCUITS_TO_OBSERVE,
...
{
    int32_t num = networkstatus_get_param(NULL, "cbtquantile", CBT_DEFAULT_QUANTILE_CUTOFF,
...
circuit_build_times_close_quantile(void)
{
    ...
    param = networkstatus_get_param(NULL, "cbtclosequantile", CBT_DEFAULT_CLOSE_QUANTILE,
...
{
    int32_t num = networkstatus_get_param(NULL, "cbttestfreq", CBT_DEFAULT_TEST_FREQUENCY,
                                          CBT_MIN_TEST_FREQUENCY, CBT_MAX_TEST_FREQUENCY);

    if (!(get_options()->LearnCircuitBuildTimeout)) {
        log_debug(LD_BUG, "circuit_build_times_test_frequency() called, cbttestfreq is %d", num);
    }
...
{
    int32_t num = networkstatus_get_param(NULL, "cbtmintimeout", CBT_DEFAULT_TIMEOUT_MIN_VALUE,
                                          CBT_MIN_TIMEOUT_MIN_VALUE, CBT_MAX_TIMEOUT_MIN_VALUE);

    if (!(get_options()->LearnCircuitBuildTimeout)) {
        log_debug(LD_BUG, "circuit_build_times_min_timeout() called, cbtmintimeout is %d", num);
    }
    return num;
}
...
circuit_build_times_initial_timeout(void)
{
    ...
    int32_t param =
        networkstatus_get_param(NULL, "cbtinitialtimeout", CBT_DEFAULT_TIMEOUT_INITIAL_VALUE,
...
{
    int32_t num;
    num = networkstatus_get_param(ns, "cbtrecentcount", CBT_DEFAULT_RECENT_CIRCUITS,
 ...
 */
void
circuit_build_times_new_consensus_params(circuit_build_times_t *cbt, networkstatus_t *ns)
{
    int32_t num;
    ...
    if (!circuit_build_times_disabled(get_options())) {
        ...
        if (num > 0) {
            if (num != cbt->liveness.num_recent_circs) {
                ...
                if (cbt->liveness.timeouts_after_firsthop && cbt->liveness.num_recent_circs > 0) {
            ...
            log_warn(LD_CIRC, "The cbtrecentcircs consensus parameter came back zero!  "
...
circuit_build_times_get_initial_timeout(void)
{
    ...
    if (!unit_tests && options->CircuitBuildTimeout) {
        ...
        if (!circuit_build_times_disabled(options) && timeout < circuit_build_times_min_timeout()) {
...
circuit_build_times_mark_circ_as_measurement_only(origin_circuit_t *circ)
{
    ...
    if (!circ->relaxed_timeout) {
        int first_hop_succeeded = circ->cpath && circ->cpath->state == CPATH_STATE_OPEN;
...
circuit_build_times_handle_completed_hop(origin_circuit_t *circ)
{
    ...
    if (timediff > get_circuit_build_timeout_ms() && circuit_any_opened_circuits_cached()) {
    ...
    if (circuit_get_cpath_opened_len(circ) == DEFAULT_ROUTE_LEN) {
         ...
         */
        if (timediff < 0 || timediff > 2 * get_circuit_build_close_time_ms() + 1000) {
            log_notice(LD_CIRC,
            ...
            if (circ->base_.purpose != CIRCUIT_PURPOSE_C_MEASURE_TIMEOUT) {
                circuit_build_times_network_circ_success(get_circuit_build_times_mutable());
            }
        }
    }
}
 ...
 */
static uint32_t *
circuit_build_times_create_histogram(const circuit_build_times_t *cbt, build_time_t *nbins)
{
    uint32_t *histogram;
...
circuit_build_times_get_xm(circuit_build_times_t *cbt)
{
    ...
    for (i = 0; i < nbins; i++) {
        ...
        for (n = 1; n < num_modes; n++) {
            if (histogram[i] >= histogram[nth_max_bin[n]] &&
                (!histogram[nth_max_bin[n - 1]] || histogram[i] < histogram[nth_max_bin[n - 1]])) {
                nth_max_bin[n] = i;
            }
        }
    }
    ...
    if (bin_counts == 0) {
        ...
        log_warn(LD_CIRC,
                 ...
                 "have_timeout=%d, %lfms",
                 cbt->total_build_times, num_modes, cbt->have_computed_timeout, cbt->timeout_ms);
        goto done;
    }
 ...
 */
void
circuit_build_times_update_state(const circuit_build_times_t *cbt, or_state_t *state)
{
    uint32_t *histogram;
    build_time_t i = 0;
 ...
 */
static void
circuit_build_times_shuffle_and_store_array(circuit_build_times_t *cbt, build_time_t *raw_times,
                                            uint32_t num_times)
{
...
circuit_build_times_filter_timeouts(circuit_build_times_t *cbt)
{
    ...
    log_info(LD_CIRC,
             ...
             (int)(cbt->total_build_times * timeout_rate), cbt->total_build_times, num_filtered,
             max_timeout);
...
circuit_build_times_parse_state(circuit_build_times_t *cbt, or_state_t *state)
{
    ...
    for (line = state->BuildtimeHistogram; line; line = line->next) {
        ...
        if (smartlist_len(args) < 2) {
            ...
            build_time_t ms;
            int ok;
            ms = (build_time_t)tor_parse_ulong(ms_str, 10, 0, CBT_BUILD_TIME_MAX, &ok, NULL);
            if (!ok) {
                log_warn(LD_GENERAL, "Unable to parse circuit build times: "
            ...
            count = (uint32_t)tor_parse_ulong(count_str, 10, 0, UINT32_MAX, &ok, NULL);
            if (!ok) {
                log_warn(LD_GENERAL, "Unable to parse circuit build times: "
    ...
    log_info(LD_CIRC, "Loaded %d/%d values from %d lines in circuit time histogram", tot_values,
             cbt->total_build_times, N);
 ...
 */
STATIC double
circuit_build_times_calculate_timeout(circuit_build_times_t *cbt, double quantile)
{
    double ret;
 ...
 */
build_time_t
circuit_build_times_generate_sample(circuit_build_times_t *cbt, double q_lo, double q_hi)
{
 ...
 */
void
circuit_build_times_initial_alpha(circuit_build_times_t *cbt, double quantile, double timeout_ms)
{
    // Q(u) = Xm/((1-u)^(1/a))
 ...
 */
static void
circuit_build_times_network_close(circuit_build_times_t *cbt, int did_onehop, time_t start_time)
{
...
circuit_build_times_network_check_changed(circuit_build_times_t *cbt)
{
    ...
    if (cbt->liveness.timeouts_after_firsthop && cbt->liveness.num_recent_circs > 0) {
        memset(cbt->liveness.timeouts_after_firsthop, 0,
               sizeof(*cbt->liveness.timeouts_after_firsthop) * cbt->liveness.num_recent_circs);
    }
 ...
 */
int
circuit_build_times_count_close(circuit_build_times_t *cbt, int did_onehop, time_t start_time)
{
    if (circuit_build_times_disabled(get_options())) {
        cbt->close_ms = cbt->timeout_ms = circuit_build_times_get_initial_timeout();
        return 0;
...
circuit_build_times_set_timeout_worker(circuit_build_times_t *cbt)
{
    ...
    cbt->timeout_ms =
        circuit_build_times_calculate_timeout(cbt, circuit_build_times_quantile_cutoff());

    cbt->close_ms =
        circuit_build_times_calculate_timeout(cbt, circuit_build_times_close_quantile());
...
circuit_build_times_set_timeout(circuit_build_times_t *cbt)
{
    ...
    if (cbt->timeout_ms < circuit_build_times_min_timeout()) {
        log_info(LD_CIRC, "Set buildtimeout to low value %fms. Setting to %dms", cbt->timeout_ms,
    ...
    if (prev_timeout > tor_lround(cbt->timeout_ms / 1000)) {
        ...
        log_info(LD_CIRC, "Circuit timeout data: %fms, %fms, Xm: %d, a: %f, r: %f", cbt->timeout_ms,
        ...
        log_info(LD_CIRC,
                 ...
                 tor_lround(cbt->timeout_ms / 1000), cbt->timeout_ms, cbt->close_ms, cbt->Xm,
                 cbt->alpha, timeout_rate, cbt->total_build_times);
    }
}
...
static void
cbt_control_event_buildtimeout_set(const circuit_build_times_t *cbt, buildtimeout_set_event_t type)
{
    char *args = NULL;
    double qnt;
     ...
     * overflow. */
    const double total_circuits = ((double)cbt->num_circ_timeouts) + cbt->num_circ_succeeded;
    ...
    tor_asprintf(&args,
                 ...
                 (unsigned long)cbt->total_build_times, (unsigned long)cbt->timeout_ms,
                 (unsigned long)cbt->Xm, cbt->alpha, qnt, timeout_rate,
                 (unsigned long)cbt->close_ms, close_rate);
