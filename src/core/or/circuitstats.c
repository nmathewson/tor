    ...
    if (!circuit_build_times_disabled(get_options())) {
        ...
        if (num > 0) {
            if (num != cbt->liveness.num_recent_circs) {
                 ...
                 * or 0s, and is only used in a statistical test to determine
                 * when we should discard our history after a sufficient number
                 * of 1's have been reached, it is fine if order is not
                 * preserved or elements are lost.
                 *
...
circuit_build_times_parse_state(circuit_build_times_t *cbt, or_state_t *state)
{
    ...
    for (line = state->BuildtimeHistogram; line; line = line->next) {
        ...
        if (smartlist_len(args) < 2) {
            ...
            count = (uint32_t)tor_parse_ulong(count_str, 10, 0, UINT32_MAX, &ok,
                                              NULL);
            if (!ok) {
                log_warn(LD_GENERAL, "Unable to parse circuit build times: "
