...
get_param_cc_enabled, (const networkstatus_t *ns))
{
    ...
    return !!networkstatus_get_param(ns, "DoSCircuitCreationEnabled", DOS_CC_ENABLED_DEFAULT, 0, 1);
}
...
get_param_cc_min_concurrent_connection(const networkstatus_t *ns)
{
    ...
    return networkstatus_get_param(ns, "DoSCircuitCreationMinConnections",
                                   DOS_CC_MIN_CONCURRENT_CONN_DEFAULT, 1, INT32_MAX);
}
...
get_param_cc_circuit_rate(const networkstatus_t *ns)
{
    ...
    return networkstatus_get_param(ns, "DoSCircuitCreationRate", DOS_CC_CIRCUIT_RATE_DEFAULT, 1,
                                   INT32_MAX);
}
...
get_param_cc_circuit_burst(const networkstatus_t *ns)
{
    ...
    return networkstatus_get_param(ns, "DoSCircuitCreationBurst", DOS_CC_CIRCUIT_BURST_DEFAULT, 1,
                                   INT32_MAX);
}
...
get_param_cc_defense_type(const networkstatus_t *ns)
{
    ...
    return networkstatus_get_param(ns, "DoSCircuitCreationDefenseType", DOS_CC_DEFENSE_TYPE_DEFAULT,
...
get_param_cc_defense_time_period(const networkstatus_t *ns)
{
    ...
    return networkstatus_get_param(ns, "DoSCircuitCreationDefenseTimePeriod",
                                   DOS_CC_DEFENSE_TIME_PERIOD_DEFAULT, 0, INT32_MAX);
}
...
get_param_conn_enabled, (const networkstatus_t *ns))
{
    ...
    return !!networkstatus_get_param(ns, "DoSConnectionEnabled", DOS_CONN_ENABLED_DEFAULT, 0, 1);
}
...
get_param_conn_max_concurrent_count(const networkstatus_t *ns)
{
    ...
    return networkstatus_get_param(ns, "DoSConnectionMaxConcurrentCount",
                                   DOS_CONN_MAX_CONCURRENT_COUNT_DEFAULT, 1, INT32_MAX);
}
...
get_param_conn_defense_type(const networkstatus_t *ns)
{
    ...
    return networkstatus_get_param(ns, "DoSConnectionDefenseType", DOS_CONN_DEFENSE_TYPE_DEFAULT,
...
cc_stats_refill_bucket(cc_client_stats_t *stats, const tor_addr_t *addr)
{
    ...
    log_debug(LD_DOS,
              "DoS address %s has its circuit bucket value: %" PRIu32 ". Filling it to %" PRIu32
              ". Circuit rate is %" PRIu64 ". Elapsed time is %" PRIi64,
              fmt_addr(addr), stats->circuit_bucket, new_circuit_bucket_count, circuit_rate,
...
cc_mark_client(cc_client_stats_t *stats)
{
     ...
     * less predictable. */
    stats->marked_until_ts = approx_time() + dos_cc_defense_time_period +
...
dos_cc_new_create_cell(channel_t *chan)
{
    ...
    if (cc_has_exhausted_circuits(&entry->dos_stats)) {
        ...
        if (entry->dos_stats.cc_stats.marked_until_ts == 0) {
            log_debug(LD_DOS, "Detected circuit creation DoS by address: %s", fmt_addr(&addr));
            cc_num_marked_addrs++;
        }
...
dos_geoip_entry_about_to_free(const clientmap_entry_t *geoip_ent)
{
    ...
    SMARTLIST_FOREACH_BEGIN (get_connection_array(), connection_t *, conn) {
        if (conn->type == CONN_TYPE_OR) {
            ...
            if (!tor_addr_compare(&geoip_ent->addr, &or_conn->real_addr, CMP_EXACT)) {
...
dos_should_refuse_single_hop_client(void)
{
    ...
    return (int)networkstatus_get_param(NULL, "DoSRefuseSingleHopClientRendezvous", 0 /* default */,
                                        0, 1);
}
...
dos_log_heartbeat(void)
{
    ...
    tor_asprintf(&circ_stats_msg, " %" PRIu64 " circuits killed with too many cells.",
    ...
    if (dos_conn_enabled) {
        tor_asprintf(&conn_msg, " %" PRIu64 " connections closed.", conn_num_addr_rejected);
    }

    if (dos_should_refuse_single_hop_client()) {
        tor_asprintf(&single_hop_client_msg, " %" PRIu64 " single hop clients refused.",
                     num_single_hop_client_refused);
    }
    ...
    log_notice(LD_HEARTBEAT, "DoS mitigation since startup:%s%s%s%s%s", circ_stats_msg,
...
dos_new_client_conn(or_connection_t *or_conn, const char *transport_name)
{
    ...
    /* We are only interested in client connection from the geoip cache. */
    entry = geoip_lookup_client(&or_conn->real_addr, transport_name, GEOIP_CLIENT_CONNECT);
    if (BUG(entry == NULL)) {
