...
get_param_conn_defense_type(const networkstatus_t *ns)
{
    ...
    return networkstatus_get_param(
        ns, "DoSConnectionDefenseType", DOS_CONN_DEFENSE_TYPE_DEFAULT,
...
cc_stats_refill_bucket(cc_client_stats_t *stats, const tor_addr_t *addr)
{
    ...
    /* We've never filled the bucket so fill it with the maximum being the
     * burst and we are done. Note: If a relay's clock is ever 0, all clients
     * that were last refilled
     ...
     * when we should have been refilling it. But the fact that we didn't
    ...
    if (num_token > UINT32_MAX - stats->circuit_bucket) {
        ...
        /* We cap the bucket to the burst value else this could overflow
         * uint32_t over time. */
        new_circuit_bucket_count = MIN(
            stats->circuit_bucket + (uint32_t)num_token, dos_cc_circuit_burst);
    }
...
cc_channel_addr_is_marked(channel_t *chan)
{
    ...
    if (entry == NULL) {
        /* We can have a connection creating circuits but not tracked by the
         * geoip cache. Once this DoS subsystem is enabled, we can end up here
         * with no entry for the channel. */
        goto end;
    }
    now = approx_time();
...
{
    /* Looking at the consensus, is the connection mitigation subsystem
     * enabled? If not and it was enabled before, clean it up. */
...
dos_cc_new_create_cell(channel_t *chan)
{
    ...
    if (cc_has_exhausted_circuits(&entry->dos_stats)) {
         ...
         * Under heavy DDoS, logging each time we mark would results in lots
         * and lots of logs. */
...
dos_close_client_conn(const or_connection_t *or_conn)
{
     ...
     * lead to most likely a false positive. In theory, this should never
     * happen but lets be extra safe. */
...
dos_consensus_has_changed(const networkstatus_t *ns)
{
    ...
    /* We were already enabled or we just became enabled but either way, set
     * the consensus parameters for all subsystems. */
