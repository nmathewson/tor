    ...
    switch (cell_in->command) {
        case CELL_CREATE:
            if (tor_memeq(cell_in->payload, NTOR_CREATE_MAGIC, 16)) {
                create_cell_init(cell_out, CELL_CREATE,
                                 ONION_HANDSHAKE_TYPE_NTOR, NTOR_ONIONSKIN_LEN,
                                 cell_in->payload + 16);
            } else {
                create_cell_init(cell_out, CELL_CREATE,
            ...
            break;
        case CELL_CREATE_FAST:
            create_cell_init(cell_out, CELL_CREATE_FAST,
                             ONION_HANDSHAKE_TYPE_FAST, CREATE_FAST_LEN,
                             cell_in->payload);
            break;
        case CELL_CREATE2:
        ...
        switch (ls->ls_type) {
            case LS_IPV4:
                if (found_ipv4)
                    continue;
                found_ipv4 = 1;
                tor_addr_from_ipv4h(&cell_out->orport_ipv4.addr,
                                    ls->un_ipv4_addr);
                    ...
                    const uint8_t *payload, size_t payload_len)
{
    ...
    switch (command) {
        case RELAY_COMMAND_EXTENDED:
            ...
            memcpy(cell_out->created_cell.reply, payload,
...
created_cell_format(cell_t *cell_out, const created_cell_t *cell_in)
{
    ...
    switch (cell_in->cell_type) {
        ...
        case CELL_CREATED2:
            ...
            memcpy(cell_out->payload + 2, cell_in->reply,
                   cell_in->handshake_len);
            break;
        default:
            return -1;
                                       ...
                                       const or_options_t *options)
{
    ...
    return (int)networkstatus_get_param(
        ns, "ExtendByEd25519ID", 0 /* default */, 0 /* min */, 1 /*max*/);
}
    ...
    switch (cell_in->cell_type) {
        case RELAY_COMMAND_EXTEND: {
            ...
            *len_out = 6 + TAP_ONIONSKIN_CHALLENGE_LEN + DIGEST_LEN;
            ...
            if (cell_in->create_cell.handshake_type ==
                ONION_HANDSHAKE_TYPE_NTOR) {
                memcpy(p + 6, NTOR_CREATE_MAGIC, 16);
                memcpy(p + 22, cell_in->create_cell.onionskin,
                       NTOR_ONIONSKIN_LEN);
            } else {
        ...
        case RELAY_COMMAND_EXTEND2: {
            ...
            *command_out = RELAY_COMMAND_EXTEND2;
            ...
            link_specifier_t *ls;
            {
                ...
                ls->ls_len = 6;
                ls->un_ipv4_addr =
                    tor_addr_to_ipv4h(&cell_in->orport_ipv4.addr);
