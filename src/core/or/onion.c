 ...
 */
void
create_cell_init(create_cell_t *cell_out, uint8_t cell_type, uint16_t handshake_type,
                 uint16_t handshake_len, const uint8_t *onionskin)
{
...
parse_create2_payload(create_cell_t *cell_out, const uint8_t *p, size_t p_len)
{
    ...
    create_cell_init(cell_out, CELL_CREATE2, handshake_type, handshake_len, p + 4);
    return 0;
}
...
create_cell_parse(create_cell_t *cell_out, const cell_t *cell_in)
{
    ...
    case CELL_CREATE:
        if (tor_memeq(cell_in->payload, NTOR_CREATE_MAGIC, 16)) {
            create_cell_init(cell_out, CELL_CREATE, ONION_HANDSHAKE_TYPE_NTOR, NTOR_ONIONSKIN_LEN,
                             cell_in->payload + 16);
        } else {
        ...
        break;
    case CELL_CREATE_FAST:
        create_cell_init(cell_out, CELL_CREATE_FAST, ONION_HANDSHAKE_TYPE_FAST, CREATE_FAST_LEN,
                         cell_in->payload);
        break;
    case CELL_CREATE2:
        if (parse_create2_payload(cell_out, cell_in->payload, CELL_PAYLOAD_SIZE) < 0)
            return -1;
        break;
    default:
        return -1;
...
check_created_cell(const created_cell_t *cell)
{
    switch (cell->cell_type) {
    case CELL_CREATED:
        if (cell->handshake_len != TAP_ONIONSKIN_REPLY_LEN && cell->handshake_len != NTOR_REPLY_LEN)
            return -1;
        break;
...
check_extend_cell(const extend_cell_t *cell)
{
    ...
    if (cell->create_cell.cell_type == CELL_CREATE) {
        ...
        if (cell->cell_type != RELAY_COMMAND_EXTEND2 && cell->cell_type != RELAY_COMMAND_EXTEND)
            return -1;
    } else {
...
static int
extend_cell_from_extend1_cell_body(extend_cell_t *cell_out, const extend1_cell_body_t *cell)
{
    tor_assert(cell_out);
    tor_assert(cell);
    memset(cell_out, 0, sizeof(*cell_out));
    ...
    if (tor_memeq(cell->onionskin, NTOR_CREATE_MAGIC, 16)) {
        ...
        memcpy(cell_out->create_cell.onionskin, cell->onionskin + 16, NTOR_ONIONSKIN_LEN);
    } else {
        ...
        memcpy(cell_out->create_cell.onionskin, cell->onionskin, TAP_ONIONSKIN_CHALLENGE_LEN);
    }
...
static int
create_cell_from_create2_cell_body(create_cell_t *cell_out, const create2_cell_body_t *cell)
{
    tor_assert(cell_out);
    tor_assert(cell);
    ...
    memcpy(cell_out->onionskin, create2_cell_body_getconstarray_handshake_data(cell),
           cell->handshake_len);
    return 0;
}

static int
extend_cell_from_extend2_cell_body(extend_cell_t *cell_out, const extend2_cell_body_t *cell)
{
    tor_assert(cell_out);
    tor_assert(cell);
    ...
    for (i = 0; i < cell->n_spec; ++i) {
        ...
        case LS_IPV6:
            if (found_ipv6)
                continue;
            found_ipv6 = 1;
            tor_addr_from_ipv6_bytes(&cell_out->orport_ipv6.addr, ls->un_ipv6_addr);
    ...
    return create_cell_from_create2_cell_body(&cell_out->create_cell, cell->create2);
}
...
MOCK_IMPL(int,
extend_cell_parse,
          (extend_cell_t * cell_out, const uint8_t command, const uint8_t *payload,
           size_t payload_length))
{
    ...
    case RELAY_COMMAND_EXTEND: {
        ...
        if (extend1_cell_body_parse(&cell, payload, payload_length) < 0 || cell == NULL) {
            if (cell)
    ...
    case RELAY_COMMAND_EXTEND2: {
        ...
        if (extend2_cell_body_parse(&cell, payload, payload_length) < 0 || cell == NULL) {
            if (cell)
 ...
 * 0 on success, -1 on failure. */
int
extended_cell_parse(extended_cell_t *cell_out, const uint8_t command, const uint8_t *payload,
                    size_t payload_len)
{
    tor_assert(cell_out);
    tor_assert(payload);
...
static int
create_cell_format_impl(cell_t *cell_out, const create_cell_t *cell_in, int relayed)
{
    uint8_t *p;
    size_t space;
...
static int
should_include_ed25519_id_extend_cells(const networkstatus_t *ns, const or_options_t *options)
{
    if (options->ExtendByEd25519ID != -1)
        return options->ExtendByEd25519ID; /* The user has an opinion. */

    return (int)networkstatus_get_param(ns, "ExtendByEd25519ID", 0 /* default */, 0 /* min */,
                                        1 /*max*/);
}
...
int
extend_cell_format(uint8_t *command_out, uint16_t *len_out, uint8_t *payload_out,
    ...
    case RELAY_COMMAND_EXTEND: {
        ...
        *len_out = 6 + TAP_ONIONSKIN_CHALLENGE_LEN + DIGEST_LEN;
        ...
        if (cell_in->create_cell.handshake_type == ONION_HANDSHAKE_TYPE_NTOR) {
            ...
            memcpy(p + 6, cell_in->create_cell.onionskin, TAP_ONIONSKIN_CHALLENGE_LEN);
        }
    ...
    case RELAY_COMMAND_EXTEND2: {
        ...
        *command_out = RELAY_COMMAND_EXTEND2;
        ...
        if (tor_addr_port_is_valid_ap(&cell_in->orport_ipv6, 0)) {
            ...
            ls->ls_type = LS_IPV6;
            ls->ls_len = 18;
            tor_addr_copy_ipv6_bytes(ls->un_ipv6_addr, &cell_in->orport_ipv6.addr);
        ...
        create2_cell_body_setlen_handshake_data(cell->create2, cell_in->create_cell.handshake_len);
        ...
        ssize_t len_encoded = extend2_cell_body_encode(payload_out, RELAY_PAYLOAD_SIZE, cell);
        extend2_cell_body_free(cell);
 ...
 * RELAY_PAYLOAD_SIZE bytes available.  Return 0 on success, -1 on failure. */
int
extended_cell_format(uint8_t *command_out, uint16_t *len_out, uint8_t *payload_out,
    ...
    case RELAY_COMMAND_EXTENDED2: {
        ...
        *len_out = 2 + cell_in->created_cell.handshake_len;
        ...
        if (2 + cell_in->created_cell.handshake_len > RELAY_PAYLOAD_SIZE)
            return -1;
        memcpy(payload_out + 2, cell_in->created_cell.reply, cell_in->created_cell.handshake_len);
    } break;
    default:
        return -1;
    }
