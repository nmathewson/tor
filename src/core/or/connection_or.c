...
static int connection_or_check_valid_tls_handshake(or_connection_t *conn, int started_here,
 ...
 **/
static void
connection_or_set_identity_digest(or_connection_t *conn, const char *rsa_digest,
                                  const ed25519_public_key_t *ed_id)
{
    ...
    log_info(LD_HANDSHAKE, "Set identity digest for %p (%s): %s %s.", conn,
             escaped_safe_str(conn->base_.address), hex_str(rsa_digest, DIGEST_LEN),
             ed25519_fmt(ed_id));
    log_info(LD_HANDSHAKE, "   (Previously: %s %s)", hex_str(conn->identity_digest, DIGEST_LEN),
             chan ? ed25519_fmt(&chan->ed25519_identity) : "<null>");

    const int rsa_id_was_set = !tor_digest_is_zero(conn->identity_digest);
    const int ed_id_was_set = chan && !ed25519_public_key_is_zero(&chan->ed25519_identity);
    const int rsa_changed = tor_memneq(conn->identity_digest, rsa_digest, DIGEST_LEN);
    const int ed_changed =
        ed_id_was_set && (!ed_id || !ed25519_pubkey_eq(ed_id, &chan->ed25519_identity));
    ...
    if (tor_digest_is_zero(rsa_digest) && (!ed_id || ed25519_public_key_is_zero(ed_id)))
        return;
...
static void
connection_or_get_state_description(or_connection_t *orconn, char *buf, size_t buflen)
{
...
connection_or_report_broken_states(int severity, int domain)
{
    ...
    SMARTLIST_FOREACH_BEGIN (items, const broken_state_count_t *, c) {
        if (c_sl_idx > MAX_REASONS_TO_REPORT)
            break;
        tor_log(severity, domain, " %d connections died in state %s", (int)c->count, c->state);
 ...
 **/
void
connection_or_event_status(or_connection_t *conn, or_conn_status_event_t tp, int reason)
{
...
#define MAX_OR_INBUF_WHEN_NONOPEN 0
    ...
    if (buf_datalen(conn->base_.inbuf) > MAX_OR_INBUF_WHEN_NONOPEN) {
        log_fn(LOG_PROTOCOL_WARN, LD_NET,
               ...
               connection_or_nonopen_was_started_here(conn) ? "to" : "from", conn->base_.address,
               conn->base_.port, conn_state_to_string(conn->base_.type, conn->base_.state));
        connection_or_close_for_error(conn, 0);
        ret = -1;
    }
...
connection_or_finished_connecting(or_connection_t *or_conn)
{
    ...
    log_debug(LD_HANDSHAKE, "OR connect() to router at %s:%u finished.", conn->address, conn->port);
        ...
        if (connection_or_nonopen_was_started_here(or_conn)) {
            ...
            if (conn->state >= OR_CONN_STATE_TLS_HANDSHAKING) {
                ...
                if (!authdir_mode_tests_reachability(options))
                    control_event_bootstrap_prob_or(orconn_end_reason_to_control_string(reason),
                                                    reason, or_conn);
            }
        }
                                          ...
                                          const or_options_t *options)
{
    ...
    if (connection_or_digest_is_known_relay(conn->identity_digest)) {
        ...
        rate = options->PerConnBWRate
                   ...
                   : networkstatus_get_param(NULL, "perconnbwrate", (int)options->BandwidthRate, 1,
                                             INT32_MAX);
...
{
    if (bool_eq(is_canonical, or_conn->is_canonical) && or_conn->idle_timeout != 0) {
    ...
    or_conn->idle_timeout =
        channelpadding_get_channel_idle_timeout(TLS_CHAN_TO_BASE(or_conn->chan), is_canonical);

    log_info(LD_CIRC, "Channel %" PRIu64 " chose an idle timeout of %d.",
             or_conn->chan ? (TLS_CHAN_TO_BASE(or_conn->chan)->global_identifier) : 0,
             or_conn->idle_timeout);
}
...
void
connection_or_init_conn_from_address(or_connection_t *conn, const tor_addr_t *addr, uint16_t port,
                                     const char *id_digest, const ed25519_public_key_t *ed_id,
                                     int started_here)
{
    log_debug(LD_HANDSHAKE, "init conn from address %s: %s, %s (%d)", fmt_addr(addr),
              hex_str((const char *)id_digest, DIGEST_LEN), ed25519_fmt(ed_id), started_here);
...
connection_or_check_canonicity(or_connection_t *conn, int started_here)
{
    ...
    if (r) {
        ...
        conn->nickname[0] = '$';
        base16_encode(conn->nickname + 1, HEX_DIGEST_LEN + 1, conn->identity_digest, DIGEST_LEN);
 ...
 */
int
connection_or_single_set_badness_(time_t now, or_connection_t *or_conn, int force)
{
    /* XXXX this function should also be about channels? */
    if (or_conn->base_.marked_for_close || connection_or_is_bad_for_new_circs(or_conn))
        return 1;

    if (force || or_conn->base_.timestamp_created + TIME_BEFORE_OR_CONN_IS_TOO_OLD < now) {
        log_info(LD_OR,
...
connection_or_group_set_badness_(smartlist_t *group, int force)
{
    ...
    SMARTLIST_FOREACH_BEGIN (group, or_connection_t *, or_conn) {
        ...
        if (!best ||
            channel_is_better(TLS_CHAN_TO_BASE(or_conn->chan), TLS_CHAN_TO_BASE(best->chan))) {
            best = or_conn;
        }
    } SMARTLIST_FOREACH_END (or_conn);
    ...
    SMARTLIST_FOREACH_BEGIN (group, or_connection_t *, or_conn) {
        if (or_conn->base_.marked_for_close || connection_or_is_bad_for_new_circs(or_conn) ||
            ...
            continue;
        if (or_conn != best &&
            channel_is_better(TLS_CHAN_TO_BASE(best->chan), TLS_CHAN_TO_BASE(or_conn->chan))) {
            ...
            if (best->is_canonical) {
                ...
                connection_or_mark_bad_for_new_circs(or_conn);
            } else if (!tor_addr_compare(&or_conn->real_addr, &best->real_addr, CMP_EXACT)) {
                log_info(LD_OR,
                         "Marking OR conn to %s:%d as unsuitable for new circuits: "
...
static HT_HEAD(or_connect_failure_ht,
               or_connect_failure_entry_t) or_connect_failures_map = HT_INITIALIZER();

/* Helper: Hashtable equal function. Return 1 if equal else 0. */
static int
or_connect_failure_ht_eq(const or_connect_failure_entry_t *a, const or_connect_failure_entry_t *b)
{
...
HT_PROTOTYPE(or_connect_failure_ht, or_connect_failure_entry_t, node, or_connect_failure_ht_hash,
             or_connect_failure_ht_eq);

HT_GENERATE2(or_connect_failure_ht, or_connect_failure_entry_t, node, or_connect_failure_ht_hash,
...
static void
or_connect_failure_init(const char *identity_digest, const tor_addr_t *addr, uint16_t port,
                        or_connect_failure_entry_t *ocf)
{
    tor_assert(ocf);
    if (identity_digest) {
        memcpy(ocf->identity_digest, identity_digest, sizeof(ocf->identity_digest));
    }
    if (addr) {
...
or_connect_failure_new(const or_connection_t *or_conn)
{
    ...
    or_connect_failure_init(or_conn->identity_digest, &or_conn->real_addr, TO_CONN(or_conn)->port,
                            ocf);
    return ocf;
}
...
or_connect_failure_map_cleanup(time_t cutoff)
{
    ...
    for (ptr = HT_START(or_connect_failure_ht, &or_connect_failures_map); ptr != NULL; ptr = next) {
        entry = *ptr;
 ...
 */
void
connection_or_connect_failed(or_connection_t *conn, int reason, const char *msg)
{
           ...
           const ed25519_public_key_t *ed_id, channel_tls_t *chan))
{
    ...
    r = get_proxy_addrport(&proxy_addr, &proxy_port, &proxy_type, &is_pt, TO_CONN(conn));
    if (r == 0) {
        ...
        const char *transport_name =
            find_transport_name_by_bridge_addrport(&TO_CONN(conn)->addr, TO_CONN(conn)->port);
        ...
        if (transport_name) {
            log_warn(LD_GENERAL,
                     ...
                     fmt_addrport(&TO_CONN(conn)->addr, TO_CONN(conn)->port), transport_name,
                     transport_name);
    ...
    switch (connection_connect(TO_CONN(conn), conn->base_.address, &addr, port, &socket_error)) {
    case -1:
        ...
        connection_or_connect_failed(conn, errno_to_orconn_end_reason(socket_error),
...
connection_tls_start_handshake, (or_connection_t * conn, int receiving))
{
    ...
    log_debug(LD_HANDSHAKE, "starting TLS handshake on fd " TOR_SOCKET_T_FORMAT, conn->base_.s);
...
connection_tls_continue_handshake(or_connection_t *conn)
{
    ...
    CASE_TOR_TLS_ERROR_ANY:
        log_info(LD_OR, "tls error [%s]. breaking connection.", tor_tls_err_to_string(result));
        return -1;
    case TOR_TLS_DONE:
        if (!tor_tls_used_v1_handshake(conn->tls)) {
            if (!tor_tls_is_server(conn->tls)) {
                ...
                tor_tls_set_renegotiate_callback(conn->tls, connection_or_tls_renegotiated_cb,
                                                 conn);
                connection_or_change_state(conn, OR_CONN_STATE_TLS_SERVER_RENEGOTIATING);
...
{
    tor_assert(conn->base_.type == CONN_TYPE_OR || conn->base_.type == CONN_TYPE_EXT_OR);
    if (!conn->tls)
 ...
 */
static int
connection_or_check_valid_tls_handshake(or_connection_t *conn, int started_here,
                                        char *digest_rcvd_out)
{
    ...
    const char *safe_address =
        started_here ? conn->base_.address : safe_str_client(conn->base_.address);
    ...
    if (has_cert) {
        int v = tor_tls_verify(started_here ? severity : LOG_INFO, conn->tls, &identity_rcvd);
        if (started_here && v < 0) {
    ...
    if (started_here) {
        ...
        return connection_or_client_learned_peer_id(conn, (const uint8_t *)digest_rcvd_out, NULL);
    }
 ...
 */
int
connection_or_client_learned_peer_id(or_connection_t *conn, const uint8_t *rsa_peer_id,
                                     const ed25519_public_key_t *ed_peer_id)
{
    ...
    const int expected_ed_key = !ed25519_public_key_is_zero(&chan->ed25519_identity);

    log_info(LD_HANDSHAKE, "learned peer id for %p (%s): %s, %s", conn,
             safe_str_client(conn->base_.address), hex_str((const char *)rsa_peer_id, DIGEST_LEN),
             ed25519_fmt(ed_peer_id));

    if (!expected_rsa_key && !expected_ed_key) {
        log_info(LD_HANDSHAKE, "(we had no ID in mind when we made this "
                               "connection.");
        connection_or_set_identity_digest(conn, (const char *)rsa_peer_id, ed_peer_id);
        tor_free(conn->nickname);
        conn->nickname = tor_malloc(HEX_DIGEST_LEN + 2);
        conn->nickname[0] = '$';
        base16_encode(conn->nickname + 1, HEX_DIGEST_LEN + 1, conn->identity_digest, DIGEST_LEN);
        ...
        learned_router_identity(&conn->base_.addr, conn->base_.port, (const char *)rsa_peer_id,
                                ed_peer_id);
        changed_identity = 1;
    }

    const int rsa_mismatch =
        expected_rsa_key && tor_memneq(rsa_peer_id, conn->identity_digest, DIGEST_LEN);
    ...
    const int ed25519_mismatch =
        expected_ed_key &&
        (ed_peer_id == NULL || !ed25519_pubkey_eq(&chan->ed25519_identity, ed_peer_id));
    ...
    if (rsa_mismatch || ed25519_mismatch) {
        ...
        base16_encode(seen_rsa, sizeof(seen_rsa), (const char *)rsa_peer_id, DIGEST_LEN);
        base16_encode(expected_rsa, sizeof(expected_rsa), conn->identity_digest, DIGEST_LEN);
        if (ed_peer_id) {
        ...
        const int using_hardcoded_fingerprints =
            !networkstatus_get_reasonably_live_consensus(time(NULL), usable_consensus_flavor());
        const int is_fallback_fingerprint = router_digest_is_fallback_dir(conn->identity_digest);
        const int is_authority_fingerprint = router_digest_is_trusted_dir(conn->identity_digest);
        ...
        log_fn(severity, LD_HANDSHAKE,
               ...
               conn->base_.address, conn->base_.port, expected_rsa, expected_ed, seen_rsa, seen_ed,
               extra_log);

        /* Tell the new guard API about the channel failure */
        entry_guard_chan_failed(TLS_CHAN_TO_BASE(conn->chan));
        connection_or_event_status(conn, OR_CONN_EVENT_FAILED, END_OR_CONN_REASON_OR_IDENTITY);
        if (!authdir_mode_tests_reachability(options))
            control_event_bootstrap_prob_or("Unexpected identity in router certificate",
    ...
    if (authdir_mode_tests_reachability(options)) {
        dirserv_orconn_tls_done(&conn->base_.addr, conn->base_.port, (const char *)rsa_peer_id,
                                ed_peer_id);
    }
...
connection_tls_finish_handshake(or_connection_t *conn)
{
    ...
    log_debug(LD_HANDSHAKE,
              ...
              started_here ? "outgoing" : "incoming", conn, safe_str_client(conn->base_.address),
              tor_tls_get_ciphersuite_name(conn->tls));

    if (connection_or_check_valid_tls_handshake(conn, started_here, digest_rcvd) < 0)
        return -1;
    ...
    if (tor_tls_used_v1_handshake(conn->tls)) {
        conn->link_proto = 1;
        connection_or_init_conn_from_address(conn, &conn->base_.addr, conn->base_.port, digest_rcvd,
                                             NULL, 0);
 ...
 */
void
or_handshake_state_record_cell(or_connection_t *conn, or_handshake_state_t *state,
 ...
 */
void
or_handshake_state_record_var_cell(or_connection_t *conn, or_handshake_state_t *state,
...
static const int n_or_protocol_versions = (int)(sizeof(or_protocol_versions) / sizeof(uint16_t));
...
connection_or_send_versions(or_connection_t *conn, int v3_plus)
{
    ...
    tor_assert(conn->handshake_state && !conn->handshake_state->sent_versions_at);
...
connection_or_send_netinfo, (or_connection_t * conn))
{
    ...
    if ((public_server_mode(get_options()) || !conn->is_outgoing) &&
        ...
        netinfo_cell_add_my_addrs(netinfo_cell, netinfo_addr_from_tor_addr(&my_addr));

        if (!tor_addr_is_null(&me->ipv6_addr)) {
            netinfo_cell_add_my_addrs(netinfo_cell, netinfo_addr_from_tor_addr(&me->ipv6_addr));
        }
    }
