...
connection_or_finished_flushing(or_connection_t *conn)
{
    ...
    switch (conn->base_.state) {
        ...
        case OR_CONN_STATE_OR_HANDSHAKING_V3:
            break;
        default:
            log_err(LD_BUG, "Called in unexpected state %d.",
                    conn->base_.state);
            tor_fragile_assert();
            return -1;
    }
           ...
           const ed25519_public_key_t *ed_id, channel_tls_t *chan))
{
    ...
    if (r == 0) {
        ...
        if (transport_name) {
            ...
            control_event_bootstrap_prob_or(
                "Can't connect to bridge", END_OR_CONN_REASON_PT_MISSING, conn);
    ...
    switch (connection_connect(TO_CONN(conn), conn->base_.address, &addr, port,
        ...
        case -1:
            ...
            connection_or_connect_failed(
                conn, errno_to_orconn_end_reason(socket_error),
