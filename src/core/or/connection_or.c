...
connection_or_finished_flushing(or_connection_t *conn)
{
    ...
    case OR_CONN_STATE_PROXY_HANDSHAKING:
        ...
        if (conn->proxy_type == PROXY_HAPROXY) {
            tor_assert(TO_CONN(conn)->proxy_state ==
                                   ...
                                   const or_options_t *options)
{
    SMARTLIST_FOREACH(conns, connection_t *, conn, {
        if (connection_speaks_cells(conn))
            connection_or_update_token_buckets_helper(TO_OR_CONN(conn), 0,
                                                      options);
    });
}
...
connection_or_check_canonicity(or_connection_t *conn, int started_here)
{
    ...
    if (r) {
        ...
        if (!started_here) {
            ...
            /* XXXX <arma> i believe the reason we did this, originally, is
             * because we wanted to log what OR a connection was to, and if we
             * logged the right IP address and port 56244, that wouldn't be as
             * helpful. now we log the "right" port too, so we know if it's
             * moria1 or moria2.
             */
...
connection_or_group_set_badness_(smartlist_t *group, int force)
{
    ...
    SMARTLIST_FOREACH_BEGIN (group, or_connection_t *, or_conn) {
        ...
        if (or_conn->base_.state != OR_CONN_STATE_OPEN)
            continue; /* Don't mark anything bad until we have seen what
                       * happens when the connection finishes. */
        if (n_canonical && !or_conn->is_canonical) {
            ...
            log_info(
                ...
                "canonical, and we have another connection to that OR that "
                "is.",
                or_conn->base_.address, or_conn->base_.port, or_conn->base_.s,
                (int)(now - or_conn->base_.timestamp_created));
            connection_or_mark_bad_for_new_circs(or_conn);
     ...
     *   canonical connection to it), or where all the connections to the OR
     * are at noncanonical addresses and we have no good direct connection
     * (which means we aren't at risk of attaching circuits to it anyway).  As
...
or_connect_failure_map_cleanup(time_t cutoff)
{
    ...
    for (ptr = HT_START(or_connect_failure_ht, &or_connect_failures_map);
        ...
        if (entry->last_failed_connect_ts <= cutoff) {
            next = HT_NEXT_RMV(or_connect_failure_ht, &or_connect_failures_map,
                               ptr);
            tor_free(entry);
        } else {
            next =
                HT_NEXT(or_connect_failure_ht, &or_connect_failures_map, ptr);
        }
    }
}
...
should_connect_to_relay(const or_connection_t *or_conn)
{
    ...
    /* Opportunistically try to cleanup the failure cache. We do that at
     * regular interval so it doesn't grow too big. */
           ...
           const ed25519_public_key_t *ed_id, channel_tls_t *chan))
{
    ...
    conn->chan = chan;
    chan->conn = conn;
    connection_or_init_conn_from_address(conn, &addr, port, id_digest, ed_id,
                                         1);
    ...
    if (r == 0) {
        ...
        if (transport_name) {
            log_warn(LD_GENERAL,
                     ...
                     "using pluggable transport '%s', but we can't find a "
                     "pluggable "
    ...
    case -1:
         ...
         * error. No need to blame the guard in this case. Notify the
         * networking system of this failure. */
    ...
    case TOR_TLS_DONE:
        if (!tor_tls_used_v1_handshake(conn->tls)) {
            if (!tor_tls_is_server(conn->tls)) {
                ...
                connection_or_change_state(
                    conn, OR_CONN_STATE_TLS_SERVER_RENEGOTIATING);
                                     ...
                                     const ed25519_public_key_t *ed_peer_id)
{
    ...
    if (rsa_mismatch || ed25519_mismatch) {
        ...
        base16_encode(seen_rsa, sizeof(seen_rsa), (const char *)rsa_peer_id,
                      DIGEST_LEN);
        base16_encode(expected_rsa, sizeof(expected_rsa),
                      conn->identity_digest, DIGEST_LEN);
        if (ed_peer_id) {
        ...
        if (server_mode(options) || non_anonymous_mode) {
            ...
            if (using_hardcoded_fingerprints) {
                ...
                if (is_authority_fingerprint) {
                    ...
                    /* we expect a small number of fallbacks to change from
                     * their
                     * hard-coded fingerprints over the life of a release */
                    severity = LOG_INFO;
                    extra_log = " Tor will try a different fallback.";
                } else {
                    /* it's a bridge, it's either a misconfiguration, or
                     * unexpected */
                    severity = LOG_WARN;
                }
            } else {
                /* a relay has changed its fingerprint from the one in the
                 * consensus */
                severity = LOG_WARN;
            }
        }
    ...
    if (changed_identity) {
        ...
        connection_or_check_canonicity(conn,
          ...
          (const var_cell_t *cell, or_connection_t *conn))
{
    ...
    connection_buf_add((char *)cell->payload, cell->payload_len,
                       TO_CONN(conn));
    if (conn->base_.state == OR_CONN_STATE_OR_HANDSHAKING_V3)
        or_handshake_state_record_var_cell(conn, conn->handshake_state, cell,
                                           0);
    ...
    while (1) {
        ...
        if (connection_fetch_var_cell_from_buf(conn, &var_cell)) {
            ...
            size_t cell_network_size =
                get_cell_network_size(conn->wide_circ_ids);
            ...
            /* retrieve cell info from buf (create the host-order struct from
             * the network-order string) */
...
connection_or_send_netinfo, (or_connection_t * conn))
{
    ...
    if ((public_server_mode(get_options()) || !conn->is_outgoing) &&
        ...
        if (!tor_addr_is_null(&me->ipv6_addr)) {
            netinfo_cell_add_my_addrs(
                netinfo_cell, netinfo_addr_from_tor_addr(&me->ipv6_addr));
        }
    }
    ...
    if (netinfo_cell_encode(cell.payload, CELL_PAYLOAD_SIZE, netinfo_cell) <
        0) {
...
connection_or_send_certs_cell(or_connection_t *conn)
{
    ...
    if (tor_tls_get_my_certs(conn_in_server_mode, &global_link_cert,
                             &id_cert) < 0)
        return -1;
    ...
    const ed25519_keypair_t *ed_signing_key, int server)
{
...

    {
        ...
        their_digests = tor_x509_cert_get_id_digests(
            conn->handshake_state->certs->id_cert);
    ...
    if (old_tlssecrets_algorithm) {
        ...
        int r = tor_tls_export_key_material(
    ...
    const int AUTH_CELL_HEADER_LEN =
        4; /* 2 bytes of type, 2 bytes of length */
