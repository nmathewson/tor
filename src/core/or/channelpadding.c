    ...
    channel_t *chan, const channelpadding_negotiate_t *pad_vars)
{
    ...
    chan->padding_enabled =
        (pad_vars->command == CHANNELPADDING_COMMAND_START);
...
channelpadding_send_disable_command(channel_t *chan)
{
    ...
    channelpadding_negotiate_set_command(&disable,
                                   ...
                                   uint16_t high_timeout)
{
    ...
    channelpadding_negotiate_set_command(&enable,
...
channelpadding_send_padding_cell_for_callback(channel_t *chan)
{
    ...
    if (!chan || chan->state != CHANNEL_STATE_OPEN) {
        ...
        log_fn(LOG_INFO, LD_OR,
               "Scheduled a netflow padding cell, but connection already "
               "closed.");
        return;
    }
                                     ...
                                     const struct monotime_t *when)
{
    ...
    if (chan && CHANNEL_CAN_HANDLE_CELLS(chan)) {
        /* Hrmm.. It might be nice to have an equivalent to
         * assert_connection_ok for channels. Then we could get rid of the
         * channeltls dependency */
        tor_assert(TO_CONN(BASE_CHAN_TO_TLS(chan)->conn)->magic ==
                   OR_CONNECTION_MAGIC);
        assert_connection_ok(TO_CONN(BASE_CHAN_TO_TLS(chan)->conn),
                             approx_time());
...
channelpadding_compute_time_until_pad_for_netflow(channel_t *chan)
{
    ...
    if (monotime_coarse_is_zero(&chan->next_padding_time)) {
        ...
        monotime_coarse_add_msec(&chan->next_padding_time,
                                 &chan->timestamp_xfer, padding_timeout);
    }
    ...
    if (ms_till_pad > DFLT_NETFLOW_INACTIVE_KEEPALIVE_MAX) {
        tor_fragile_assert();
        log_warn(LD_BUG,
                 "Channel padding timeout scheduled %" PRId64
                 "ms in the future. "
    ...
    if (ms_till_pad < (TOR_HOUSEKEEPING_CALLBACK_MSEC +
         ...
         * calling the once-per-second callback due to other work taking too
         * long. See https://bugs.torproject.org/22212 and
         * https://bugs.torproject.org/16585. This is a systemic problem
         * with being single-threaded, but let's emit a notice if this
         * is long enough in the past that we might have missed a netflow
         * window, and allowed a router to emit a netflow frame, just so we
    ...
    if (chan->cmux && !chan->has_queued_writes(chan) &&
        ...
        if (is_client_channel || consensus_nf_pad_relays) {
            ...
            if (pad_time_ms == CHANNELPADDING_TIME_DISABLED) {
                ...
                /* We have to schedule a callback because we're called exactly
                 * once per second, but we don't want padding packets to go out
                 * exactly on an integer multiple of seconds. This callback
                 * will only be scheduled if we're within 1.1 seconds of the
                 * padding time.
                 */
                chan->currently_padding = 1;
