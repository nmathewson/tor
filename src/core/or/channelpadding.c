...
#define DFLT_NETFLOW_INACTIVE_KEEPALIVE_MAX 60000
    consensus_nf_ito_low = networkstatus_get_param(
        ns, "nf_ito_low", DFLT_NETFLOW_INACTIVE_KEEPALIVE_LOW, DFLT_NETFLOW_INACTIVE_KEEPALIVE_MIN,
        DFLT_NETFLOW_INACTIVE_KEEPALIVE_MAX);
    consensus_nf_ito_high =
        networkstatus_get_param(ns, "nf_ito_high", DFLT_NETFLOW_INACTIVE_KEEPALIVE_HIGH,
...
#define DFLT_NETFLOW_REDUCED_KEEPALIVE_MAX 60000
    ...
    consensus_nf_ito_high_reduced =
        networkstatus_get_param(ns, "nf_ito_high_reduced", DFLT_NETFLOW_REDUCED_KEEPALIVE_HIGH,
...
#define CONNTIMEOUT_RELAYS_MAX (7 * 24 * 60 * 60) // 1 week
    consensus_nf_conntimeout_relays =
        networkstatus_get_param(ns, "nf_conntimeout_relays", CONNTIMEOUT_RELAYS_DFLT,
...
#define CIRCTIMEOUT_CLIENTS_MAX (24 * 60 * 60) // 24 hours
    consensus_nf_conntimeout_clients =
        networkstatus_get_param(ns, "nf_conntimeout_clients", CIRCTIMEOUT_CLIENTS_DFLT,
                                CIRCTIMEOUT_CLIENTS_MIN, CIRCTIMEOUT_CLIENTS_MAX);

    consensus_nf_pad_before_usage = networkstatus_get_param(ns, "nf_pad_before_usage", 1, 0, 1);

    consensus_nf_pad_relays = networkstatus_get_param(ns, "nf_pad_relays", 0, 0, 1);

    consensus_nf_pad_single_onion =
        networkstatus_get_param(ns, CHANNELPADDING_SOS_PARAM, CHANNELPADDING_SOS_DEFAULT, 0, 1);
}
                                          ...
                                          const channelpadding_negotiate_t *pad_vars)
{
    ...
    if ((get_options()->BridgeRelay &&
        ...
        log_fn_ratelim(&relay_limit, LOG_PROTOCOL_WARN, LD_PROTOCOL,
                       ...
                       chan->get_remote_descr(chan, 0), hex_str(chan->identity_digest, DIGEST_LEN));
        return -1;
    }
    ...
    /* Min must not be lower than the current consensus parameter
       nf_ito_low. */
    chan->padding_timeout_low_ms = MAX(consensus_nf_ito_low, pad_vars->ito_low_ms);

    /* Max must not be lower than ito_low_ms */
    chan->padding_timeout_high_ms = MAX(chan->padding_timeout_low_ms, pad_vars->ito_high_ms);

    log_fn(LOG_INFO, LD_OR, "Negotiated padding=%d, lo=%d, hi=%d on %" PRIu64,
           chan->padding_enabled, chan->padding_timeout_low_ms, chan->padding_timeout_high_ms,
...
channelpadding_send_disable_command(channel_t *chan)
{
    ...
    tor_assert(chan);
    tor_assert(BASE_CHAN_TO_TLS(chan)->conn->link_proto >= MIN_LINK_PROTO_FOR_CHANNEL_PADDING);
    ...
    if (channelpadding_negotiate_encode(cell.payload, CELL_PAYLOAD_SIZE, &disable) < 0)
        return -1;
 ...
 */
int
channelpadding_send_enable_command(channel_t *chan, uint16_t low_timeout, uint16_t high_timeout)
{
    ...
    if (channelpadding_negotiate_encode(cell.payload, CELL_PAYLOAD_SIZE, &enable) < 0)
        return -1;
...
channelpadding_send_padding_cell_for_callback(channel_t *chan)
{
    ...
    if (!chan || chan->state != CHANNEL_STATE_OPEN) {
        ...
        log_fn(LOG_INFO, LD_OR, "Scheduled a netflow padding cell, but connection already closed.");
        return;
    }
    ...
    if (monotime_coarse_is_zero(&chan->next_padding_time) || chan->has_queued_writes(chan) ||
...

    {
        ...
        log_fn(LOG_INFO, LD_OR,
               ...
               (chan->global_identifier), safe_str_client(chan->get_remote_descr(chan, 0)),
 ...
 */
static void
channelpadding_send_padding_callback(tor_timer_t *timer, void *args, const struct monotime_t *when)
{
    ...
    if (chan && CHANNEL_CAN_HANDLE_CELLS(chan)) {
        ...
        tor_assert(TO_CONN(BASE_CHAN_TO_TLS(chan)->conn)->magic == OR_CONNECTION_MAGIC);
...
channelpadding_schedule_padding(channel_t *chan, int in_ms)
{
    ...
    if (chan->padding_timer) {
        timer_set_cb(chan->padding_timer, channelpadding_send_padding_callback, chan->timer_handle);
    } else {
        chan->padding_timer = timer_new(channelpadding_send_padding_callback, chan->timer_handle);
    }
...
channelpadding_compute_time_until_pad_for_netflow(channel_t *chan)
{
    ...
    if (monotime_coarse_is_zero(&chan->next_padding_time)) {
        ...
        int32_t padding_timeout = channelpadding_get_netflow_inactive_timeout_ms(chan);

        if (!padding_timeout)
            return CHANNELPADDING_TIME_DISABLED;

        monotime_coarse_add_msec(&chan->next_padding_time, &chan->timestamp_xfer, padding_timeout);
    }

    const int64_t ms_till_pad = monotime_coarse_diff_msec(&now, &chan->next_padding_time);
    ...
    /* If the timeout will expire before the next time we're called (1000ms
       ...
       then.
     */
    if (ms_till_pad < (TOR_HOUSEKEEPING_CALLBACK_MSEC + TOR_HOUSEKEEPING_CALLBACK_SLACK_MSEC)) {
...
#define NETFLOW_MISSED_WINDOW (150000 - DFLT_NETFLOW_INACTIVE_KEEPALIVE_HIGH)
        if (ms_till_pad < 0) {
            int severity = (ms_till_pad < -NETFLOW_MISSED_WINDOW) ? LOG_NOTICE : LOG_INFO;
            log_fn(severity, LD_OR, "Channel padding timeout scheduled %" PRId64 "ms in the past. ",
                   (-ms_till_pad));
            return 0; /* Clock jumped: Send padding now */
        }
 ...
 */
unsigned int
channelpadding_get_channel_idle_timeout(const channel_t *chan, int is_canonical)
{
...
#define CONNTIMEOUT_CLIENTS_BASE 180 // 3 to 4.5 min
        timeout = CONNTIMEOUT_CLIENTS_BASE + crypto_rand_int(CONNTIMEOUT_CLIENTS_BASE / 2);
     ...
     * set.
     */
    if (options->ReducedConnectionPadding && !options->CircuitsAvailableTimeout) {
        timeout /= 2;
    }
...
channelpadding_reduce_padding_on_channel(channel_t *chan)
{
    ...
    log_fn(LOG_INFO, LD_OR, "Reduced padding on channel %" PRIu64 ": lo=%d, hi=%d",
           (chan->global_identifier), chan->padding_timeout_low_ms, chan->padding_timeout_high_ms);
}
...
channelpadding_decide_to_pad_channel(channel_t *chan)
{
    ...
    if (rend_service_allow_non_anonymous_connection(options) && !consensus_nf_pad_single_onion) {
    ...
    if (chan->cmux && !chan->has_queued_writes(chan) && !circuitmux_num_cells(chan->cmux)) {
        ...
        if (is_client_channel || consensus_nf_pad_relays) {
            int64_t pad_time_ms = channelpadding_compute_time_until_pad_for_netflow(chan);
