...
struct channel_t {
    ...
    enum {
        /**
         * The channel is not open, or it has a full output buffer but no
         * queued cells.
         */
        SCHED_CHAN_IDLE = 0,
        /**
...
#define GRD_FLAG_ADDR_ONLY 2
     ...
     *
     * Note that this is the RSA identity that we hope the other side has --
     * not necessarily its true identity.  Don't believe this identity unless
     ...
     * never be used to satisfy an EXTEND request. */
...
#define CHANNEL_CAN_HANDLE_CELLS(chan)                    \
    (CHANNEL_IS_OPENING(chan) || CHANNEL_IS_OPEN(chan) || \
