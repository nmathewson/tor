            ...
            case SCHEDULER_KIST:
                if (!scheduler_can_use_kist()) {
                    ...
                    if (!have_logged_kist_suddenly_disabled) {
                         ...
                         * scheduler_can_use_kist() to return false, then this
                         * flag makes sure we only log this message once. If it
                         * was the consensus that switched from "yes use kist"
                         * to "no don't use kist", then we still set the flag so
                         * we log once, but we unset the flag elsewhere if we
                         * ever can_use_kist() again.
                         */
                 ...
                 * expect to ever end up here 2 - the consensus went from "yes
                 * use kist" to "no don't use kist". We might end up here if the
                 * consensus changes back to "yes", in which case we might want
                 * to warn the user again if it goes back to "no" yet again.
                 * Thus we unset the flag */
