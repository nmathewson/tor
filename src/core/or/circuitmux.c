...
circuitmux_detach_all_circuits(circuitmux_t *cmux, smartlist_t *detached_out)
{
    ...
    while (i) {
        ...
        if (!to_remove) {
            ...
            if (chan) {
                circ = circuit_get_by_circid_channel_even_if_marked(
                    to_remove->circ_id, chan);
                if (circ) {
                    /* Clear the circuit's mux for this direction */
                    if (to_remove->muxinfo.direction == CELL_DIRECTION_OUT) {
                        /*
                         * Update active_circuits et al.; this does policy
                         * notifies, so comes before freeing policy data
                         */
                    ...
                    if (to_remove->muxinfo.policy_data) {
                        /*
                         * If we have policy data, assert that we have the
                         * means to free it
                         */
                        tor_assert(cmux->policy);
                        tor_assert(cmux->policy->free_circ_data);
                        /* Call free_circ_data() */
                        cmux->policy->free_circ_data(
                            cmux, cmux->policy_data, circ,
...
circuitmux_set_policy(circuitmux_t *cmux, const circuitmux_policy_t *pol)
{
    ...
    while (i) {
        ...
        /*
         * Get the channel; since normal case is all circuits on the mux share
         * a channel, we cache last_chan_id_searched
         */
        ...
        /* Get the circuit */
        circ =
            circuit_get_by_circid_channel_even_if_marked((*i)->circ_id, chan);
        tor_assert(circ);
        ...
        if (new_pol && new_pol->alloc_circ_data) {
            /*
             * If alloc_circ_data is not null, we expect to get some
             * per-circuit policy data.  Assert that we also have
             * free_circ_data so we can free it when the time comes, and
             * allocate it.
             */
        ...
        if (circ->magic == OR_CIRCUIT_MAGIC) {
            ...
            if (TO_OR_CIRCUIT(circ)->p_chan) {
                ...
                hashent = HT_FIND(chanid_circid_muxinfo_map,
                                  cmux->chanid_circid_map, &search);
                /* Find anything? */
                if (hashent) {
                    /* Assert that the direction makes sense before we return
                     * it */
                    tor_assert(hashent->muxinfo.direction ==
                               CELL_DIRECTION_IN);
                }
            }
        }
          ...
          (circuitmux_t * cmux, circuit_t *circ, cell_direction_t direction))
{
    ...
    if (hashent) {
        ...
        /*
         * Looks okay; just update the cell count and active circuits if we
         * must
         */
          ...
          (circuitmux_t * cmux, circuit_t *circ))
{
     ...
     * We assert that it has been replaced with a valid value before it is
     * used.
     */
    ...
    if (hashent) {
        ...
        // This isn't sensitive, but we want to be sure to know if we're
        // accessing this accidentally.
...
circuitmux_make_circuit_active(circuitmux_t *cmux, circuit_t *circ)
{
    ...
    if (cmux->policy->notify_circ_active) {
        ...
        chanid_circid_muxinfo_t *hashent =
            circuitmux_find_map_entry(cmux, circ);
                             ...
                             unsigned int n_cells)
{
    ...
    /*
     * We call notify_xmit_cells() before making the circuit inactive if
     * needed, so the policy can always count on this coming in on an active
     * circuit.
     */
    ...
    if (cmux_1->policy && cmux_2->policy) {
        if (cmux_1->policy == cmux_2->policy) {
            ...
            if (policy->cmp_cmux) {
                ...
                /*
                 * Equivalent because the policy doesn't know how to compare
                 * between muxes.
                 */
                return 0;
            }
