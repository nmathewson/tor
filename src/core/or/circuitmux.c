...
static inline int chanid_circid_entries_eq(chanid_circid_muxinfo_t *a, chanid_circid_muxinfo_t *b);
static inline unsigned int chanid_circid_entry_hash(chanid_circid_muxinfo_t *a);
...
static inline int
chanid_circid_entries_eq(chanid_circid_muxinfo_t *a, chanid_circid_muxinfo_t *b)
{
...
{
    return (((unsigned int)(a->circ_id) << 8) ^ ((unsigned int)((a->chan_id >> 32) & 0xffffffff)) ^
            ((unsigned int)(a->chan_id & 0xffffffff)));
}

/* Emit a bunch of hash table stuff */
HT_PROTOTYPE(chanid_circid_muxinfo_map, chanid_circid_muxinfo_t, node, chanid_circid_entry_hash,
             chanid_circid_entries_eq);
HT_GENERATE2(chanid_circid_muxinfo_map, chanid_circid_muxinfo_t, node, chanid_circid_entry_hash,
             chanid_circid_entries_eq, 0.6, tor_reallocarray_, tor_free_);
...
circuitmux_detach_all_circuits(circuitmux_t *cmux, smartlist_t *detached_out)
{
    ...
    while (i) {
        ...
        if (!to_remove) {
            ...
            if (chan) {
                circ = circuit_get_by_circid_channel_even_if_marked(to_remove->circ_id, chan);
                if (circ) {
                    ...
                    /* Complain and move on */
                    log_warn(LD_CIRC, "Couldn't find circuit %u (for channel %" PRIu64 ")",
                             (unsigned)to_remove->circ_id, (to_remove->chan_id));
                }
            } else {
                /* Complain and move on */
                log_warn(LD_CIRC, "Couldn't find channel %" PRIu64 " (for circuit id %u)",
...
circuitmux_free_(circuitmux_t *cmux)
{
    ...
    if (cmux->destroy_cell_queue.n > 0) {
        ...
        log_debug(LD_CIRC,
                  ...
                  cmux, cmux->destroy_cell_queue.n, (cmux->destroy_ctr), (global_destroy_ctr));
    } else {
        log_debug(LD_CIRC,
...
circuitmux_set_policy(circuitmux_t *cmux, const circuitmux_policy_t *pol)
{
    ...
    while (i) {
        ...
        if (old_pol && old_pol->notify_circ_inactive && (*i)->muxinfo.cell_count > 0) {
            old_pol->notify_circ_inactive(cmux, old_pol_data, circ, (*i)->muxinfo.policy_data);
        }
        ...
        if ((*i)->muxinfo.policy_data) {
            ...
            /* Free it */
            old_pol->free_circ_data(cmux, old_pol_data, circ, (*i)->muxinfo.policy_data);
        ...
        if (new_pol && new_pol->alloc_circ_data) {
            ...
            (*i)->muxinfo.policy_data = new_pol->alloc_circ_data(
                cmux, new_pol_data, circ, (*i)->muxinfo.direction, (*i)->muxinfo.cell_count);
        }

        /* Need to make active on new policy? */
        if (new_pol && new_pol->notify_circ_active && (*i)->muxinfo.cell_count > 0) {
            new_pol->notify_circ_active(cmux, new_pol_data, circ, (*i)->muxinfo.policy_data);
        }
...
circuitmux_find_map_entry(circuitmux_t *cmux, circuit_t *circ)
{
    ...
    if (circ->n_chan) {
        ...
        /* Query */
        hashent = HT_FIND(chanid_circid_muxinfo_map, cmux->chanid_circid_map, &search);
    }
          ...
          (circuitmux_t * cmux, circuit_t *circ, cell_direction_t direction))
{
    ...
    tor_assert(cmux);
    tor_assert(circ);
    tor_assert(direction == CELL_DIRECTION_IN || direction == CELL_DIRECTION_OUT);
    ...
    if (hashent) {
         ...
         * counts.
         */
        log_debug(LD_CIRC, "Attaching circuit %u on channel %" PRIu64 " to cmux %p",
                  (unsigned)circ_id, (channel_id), cmux);
        ...
        if (cmux->policy->alloc_circ_data) {
            ...
            hashent->muxinfo.policy_data =
                cmux->policy->alloc_circ_data(cmux, cmux->policy_data, circ, direction, cell_count);
...
void
circuitmux_set_num_cells(circuitmux_t *cmux, circuit_t *circ, unsigned int n_cells)
{
    chanid_circid_muxinfo_t *hashent = NULL;
...
circuit_t *
circuitmux_get_first_active_circuit(circuitmux_t *cmux, destroy_cell_queue_t **destroy_queue_out)
{
    circuit_t *circ = NULL;
...
void
circuitmux_notify_xmit_cells(circuitmux_t *cmux, circuit_t *circ, unsigned int n_cells)
{
    chanid_circid_muxinfo_t *hashent = NULL;
    ...
    if (cmux->policy->notify_xmit_cells) {
        cmux->policy->notify_xmit_cells(cmux, cmux->policy_data, circ, hashent->muxinfo.policy_data,
                                        n_cells);
    }
...
/*DOCDOC */
void
circuitmux_append_destroy_cell(channel_t *chan, circuitmux_t *cmux, circid_t circ_id,
                               uint8_t reason)
{
...
int64_t
circuitmux_count_queued_destroy_cells(const channel_t *chan, const circuitmux_t *cmux)
{
    ...
    if (n_destroy_cells != destroy_queue_size || n_destroy_cells != manual_total ||
        ...
        log_warn(LD_BUG,
                 ...
                 (n_destroy_cells), (destroy_queue_size), (manual_total), (manual_total_in_map));
    }
...
circuitmux_compare_muxes, (circuitmux_t * cmux_1, circuitmux_t *cmux_2))
{
    ...
    if (cmux_1->policy && cmux_2->policy) {
        if (cmux_1->policy == cmux_2->policy) {
            ...
            if (policy->cmp_cmux) {
                /* Okay, we can compare! */
                return policy->cmp_cmux(cmux_1, cmux_1->policy_data, cmux_2, cmux_2->policy_data);
            } else {
                /*
