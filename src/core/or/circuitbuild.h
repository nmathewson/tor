...
extend_info_t *extend_info_new(const char *nickname, const char *rsa_id_digest,
                               const struct ed25519_public_key_t *ed_id, crypto_pk_t *onion_key,
...
MOCK_DECL(const node_t *, build_state_get_exit_node, (cpath_build_state_t * state));
const char *build_state_get_exit_nickname(cpath_build_state_t *state);

struct circuit_guard_state_t;

const node_t *choose_good_entry_server(uint8_t purpose, cpath_build_state_t *state,
                                       struct circuit_guard_state_t **guard_state_out);
void circuit_upgrade_circuits_from_guard_wait(void);

struct ed25519_public_key_t;

MOCK_DECL(channel_t *, channel_connect_for_circuit,
          (const tor_addr_t *addr, uint16_t port, const char *id_digest,
           const struct ed25519_public_key_t *ed_id));

struct create_cell_t;
MOCK_DECL(int, circuit_deliver_create_cell,
...
MOCK_DECL(STATIC int, count_acceptable_nodes, (const smartlist_t *nodes, int direct));
