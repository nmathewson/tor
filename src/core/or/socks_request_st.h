...
struct socks_request_t {
    ...
    unsigned int has_finished : 1; /**< Has the SOCKS handshake finished? Used
                                    * to make sure we send back a socks reply
                                    * for every connection. */
    unsigned int
        got_auth : 1; /**< Have we received any authentication data? */
