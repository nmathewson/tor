...
circpad_machine_client_hide_intro_circuits(smartlist_t *machines_sl)
{
     ...
     *
     * This means that up to the sixth cell (first line of each sequence
     * above), both general and intro circuits have identical cell sequences.
     * After that we want to mimic the second line sequence of
     *   -> [DATA] -> [DATA] -> DATA -> DATA...(inbound data cells continue)
     *
     * We achieve this by starting padding INTRODUCE1 has been sent. With
     * padding negotiation cells, in the common case of the second line looks
     * like:
     *   -> [INTRO1] -> [PADDING_NEGOTIATE] -> PADDING_NEGOTIATED -> INTRO_ACK
     *
     * Then, the middle node will send between INTRO_MACHINE_MINIMUM_PADDING
     * and INTRO_MACHINE_MAXIMUM_PADDING cells, to match the "...(inbound data
     * cells continue)" portion of the trace (aka the rest of an HTTPS response
     * body).
     */
     ...
     * sending PADDING_NEGOTIATE, and we stay there (without sending any
     * padding) until we receive a STOP from the other side. */
...
circpad_machine_relay_hide_intro_circuits(smartlist_t *machines_sl)
{
    ...
    /* We want to negotiate END from this side after all our padding is done,
     * so that the origin-side machine goes into END state, and eventually
     * closes the circuit. */
    ...
    /* For the relay-side, we want to transition from OBFUSCATE_CIRC_SETUP to
     * END state when the length finishes. */
     ...
     * flow:
     *
     * For the relay-side machine, we want to keep on sending padding bytes
     * even when nothing else happens on this circuit. */
    relay_machine->states[CIRCPAD_STATE_OBFUSCATE_CIRC_SETUP]
     ...
     * randomized. The relay side will send between
    ...
    /* Configure histogram */
    relay_machine->states[CIRCPAD_STATE_OBFUSCATE_CIRC_SETUP].histogram_len =
        2;
...
circpad_machine_client_hide_rend_circuits(smartlist_t *machines_sl)
{
    ...
    /* We only want to pad rendezvous circuits, and we want to start padding
     ...
     *
     * [EXTEND2] -> EXTENDED2 -> [EXTEND2] -> EXTENDED2 -> [EST_REND] ->
     * REND_EST
     *    -> REND2 -> [BEGIN]
     *
     * This means that up to the sixth cell (in the first line), both general
     * and rend circuits have identical cell sequences.
     *
    ...
    client_machine->states[CIRCPAD_STATE_OBFUSCATE_CIRC_SETUP].histogram_len =
        2;
    client_machine->states[CIRCPAD_STATE_OBFUSCATE_CIRC_SETUP]
        .histogram_edges[0] = 0;
    client_machine->states[CIRCPAD_STATE_OBFUSCATE_CIRC_SETUP]
    ...
    client_machine->states[CIRCPAD_STATE_OBFUSCATE_CIRC_SETUP].histogram[0] =
        1;
    client_machine->states[CIRCPAD_STATE_OBFUSCATE_CIRC_SETUP]
