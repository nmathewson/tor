...
struct or_connection_t {
    ...
    tor_addr_t
        real_addr; /**< The actual address that this connection came from
    ...
    /** True iff this is a client connection and its address has been put in
     * the geoip cache and handled by the DoS mitigation subsystem. We use this
     * to insure we have a coherent count of concurrent connection. */
    ...
    or_handshake_state_t
        *handshake_state; /**< If we are setting this connection
                           * up, state information to do so. */

    time_t
        timestamp_lastempty; /**< When was the outbuf last completely empty?*/

    token_bucket_rw_t bucket; /**< Used for rate limiting when the connection
                               * is in state CONN_OPEN. */
