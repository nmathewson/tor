 ...
 */
static inline int
chan_circid_entries_eq_(chan_circid_circuit_map_t *a, chan_circid_circuit_map_t *b)
{
...
static HT_HEAD(chan_circid_map, chan_circid_circuit_map_t) chan_circid_map = HT_INITIALIZER();
HT_PROTOTYPE(chan_circid_map, chan_circid_circuit_map_t, node, chan_circid_entry_hash_,
             chan_circid_entries_eq_);
HT_GENERATE2(chan_circid_map, chan_circid_circuit_map_t, node, chan_circid_entry_hash_,
...
static void
circuit_set_circid_chan_helper(circuit_t *circ, int direction, circid_t id, channel_t *chan)
{
    ...
    if (_last_circid_chan_ent &&
        ((old_id == _last_circid_chan_ent->circ_id && old_chan == _last_circid_chan_ent->chan) ||
    ...
    if (old_chan) {
         ...
         */
        if (old_id != 0 && (old_chan != chan || old_id != id) && !(circ->marked_for_close)) {
    ...
    *circid_ptr = id;
     ...
     * close.
     */
    if (chan && id != 0 && (old_chan != chan || old_id != id) && !(circ->marked_for_close)) {
 ...
 **/
int
circuit_event_status(origin_circuit_t *circ, circuit_status_event_t tp, int reason_code)
{
...
circuit_count_pending_on_channel(channel_t *chan)
{
    ...
    smartlist_free(sl);
    log_debug(LD_CIRC, "or_conn to %s, %d pending circs", channel_get_canonical_remote_descr(chan),
              cnt);
    return cnt;
}
...
circuit_remove_from_origin_circuit_list(origin_circuit_t *origin_circ)
{
    ...
    if (origin_idx < smartlist_len(global_origin_circuit_list)) {
        origin_circuit_t *replacement = smartlist_get(global_origin_circuit_list, origin_idx);
...
{
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_origin_circuit_list(), const origin_circuit_t *,
                             next_circ) {
...
origin_circuit_new(void)
{
    ...
    if (!circuit_build_times_disabled(get_options()) &&
        ...
        int prediction_time_remaining = predicted_ports_prediction_time_remaining(time(NULL));
        circ->circuit_idle_timeout =
            prediction_time_remaining + 1 + crypto_rand_int(1 + prediction_time_remaining / 20);
        ...
        log_info(LD_CIRC,
                 ...
                 "%d seconds of predictive building remaining.",
                 (circ->global_identifier), circ->circuit_idle_timeout, prediction_time_remaining);
    }
...
circuit_clear_testing_cell_stats(circuit_t *circ)
{
    if (!circ || !circ->testing_cell_stats)
        return;
    SMARTLIST_FOREACH(circ->testing_cell_stats, testing_cell_stats_entry_t *, ent, tor_free(ent));
 ...
 */
static void
circuit_dump_conn_details(int severity, circuit_t *circ, int conn_array_index, const char *type,
                          circid_t this_circid, circid_t other_circid)
{
    tor_log(severity, LD_CIRC,
            "Conn %d has %s circuit: circID %u "
            "(other side %u), state %d (%s), born %ld:",
            conn_array_index, type, (unsigned)this_circid, (unsigned)other_circid, circ->state,
            circuit_state_to_string(circ->state), (long)circ->timestamp_began.tv_sec);
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        ...
        if (CIRCUIT_IS_ORIGIN(circ)) {
            for (tmpconn = TO_ORIGIN_CIRCUIT(circ)->p_streams; tmpconn;
                ...
                if (TO_CONN(tmpconn) == conn) {
                    circuit_dump_conn_details(severity, circ, conn->conn_array_index, "App-ward",
                                              p_circ_id, n_circ_id);
                }
            }
        }
        ...
        if (!CIRCUIT_IS_ORIGIN(circ)) {
            for (tmpconn = TO_OR_CIRCUIT(circ)->n_streams; tmpconn;
                 tmpconn = tmpconn->next_stream) {
                if (TO_CONN(tmpconn) == conn) {
                    circuit_dump_conn_details(severity, circ, conn->conn_array_index, "Exit-ward",
                                              n_circ_id, p_circ_id);
                }
            }
        }
...
{
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        if (CIRCUIT_IS_ORIGIN(circ) && TO_ORIGIN_CIRCUIT(circ)->global_identifier == id) {
            if (circ->marked_for_close)
                return NULL;
            else
...
circuit_get_by_circid_channel_impl(circid_t circ_id, channel_t *chan, int *found_entry_out)
{
    chan_circid_circuit_map_t search;
    chan_circid_circuit_map_t *found;
    ...
    if (found && found->circuit) {
        log_debug(LD_CIRC,
                  ...
                  found->circuit, (unsigned)circ_id, (chan->global_identifier), chan);
        if (found_entry_out)
            *found_entry_out = 1;
    ...
    log_debug(LD_CIRC,
              ...
              " circ_id %u, channel ID %" PRIu64 " (%p)",
              found ? "placeholder" : "nothing", (unsigned)circ_id, (chan->global_identifier),
              chan);
...
circuit_get_by_edge_conn(edge_connection_t *conn)
{
    ...
    tor_assert(!circ || (CIRCUIT_IS_ORIGIN(circ) ? circ->magic == ORIGIN_CIRCUIT_MAGIC
...
circuit_unlink_all_from_channel(channel_t *chan, int reason)
{
...
#ifdef DEBUG_CIRCUIT_UNLINK_ALL
    {
        ...
        SMARTLIST_FOREACH(detached, circuit_t *, c,
                          if (c != smartlist_get(detached_2, c_sl_idx)) mismatch = 1;);
...
{
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        if (!circ->marked_for_close && circ->purpose == CIRCUIT_PURPOSE_C_REND_READY) {
            origin_circuit_t *ocirc = TO_ORIGIN_CIRCUIT(circ);
            ...
            if (!rend_cmp_service_ids(rend_data_get_address(rend_data),
                ...
                tor_memeq(ocirc->rend_data->rend_cookie, rend_data->rend_cookie, REND_COOKIE_LEN))
                return ocirc;
        }
    } SMARTLIST_FOREACH_END (circ);
    return NULL;
 ...
 */
origin_circuit_t *
circuit_get_next_intro_circ(const origin_circuit_t *start, bool want_client_circ)
{
    int idx = 0;
    smartlist_t *lst = circuit_get_global_list();
 ...
 */
origin_circuit_t *
circuit_get_next_by_pk_and_purpose(origin_circuit_t *start, const uint8_t *digest, uint8_t purpose)
{
    int idx;
    smartlist_t *lst = circuit_get_global_list();
 ...
 */
origin_circuit_t *
circuit_find_to_cannibalize(uint8_t purpose_to_produce, extend_info_t *info, int flags)
{
    ...
    purpose_to_search_for = get_circuit_purpose_needed_to_cannibalize(purpose_to_produce);
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ_) {
        if (CIRCUIT_IS_ORIGIN(circ_) && circ_->state == CIRCUIT_STATE_OPEN &&
            ...
            if ((!need_uptime || circ->build_state->need_uptime) &&
                ...
                (internal == circ->build_state->is_internal) && !circ->unusable_for_new_conns &&
                circ->remaining_relay_early_cells && !circ->build_state->onehop_tunnel &&
                ...
                if (info) {
                    ...
                    do {
                        const node_t *ri2;
                        if (tor_memeq(hop->extend_info->identity_digest, info->identity_digest,
                                      DIGEST_LEN))
                            goto next;
                        if (ri1 && (ri2 = node_get_by_id(hop->extend_info->identity_digest)) &&
                ...
                if (options->ExcludeNodes) {
                    ...
                    crypt_path_t *hop = circ->cpath;
                    do {
                        if (routerset_contains_extendinfo(options->ExcludeNodes, hop->extend_info))
                            goto next;
                        hop = hop->next;
                    } while (hop != circ->cpath);
                }

                if ((flags & CIRCLAUNCH_IS_V3_RP) && !circuit_can_be_cannibalized_for_v3_rp(circ)) {
...
circuit_find_circuits_to_upgrade_from_guard_wait(void)
{
     ...
     * run the algorithm. */
    if (!circuits_pending_other_guards || smartlist_len(circuits_pending_other_guards) == 0)
        return NULL;
...
circuit_get_cpath_opened_len(const origin_circuit_t *circ)
{
    ...
    if (circ && circ->cpath) {
        crypt_path_t *cpath, *cpath_next = NULL;
        for (cpath = circ->cpath; cpath->state == CPATH_STATE_OPEN && cpath_next != circ->cpath;
             cpath = cpath_next) {
            cpath_next = cpath->next;
            ++n;
        }
...
{
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
        if (CIRCUIT_IS_ORIGIN(circ) && !circ->marked_for_close && !circ->timestamp_dirty)
 ...
 */
void
circuit_synchronize_written_or_bandwidth(const circuit_t *c, circuit_channel_direction_t dir)
{
    uint64_t cells;
    uint64_t cell_size;
 ...
 */
MOCK_IMPL(void,
circuit_mark_for_close_, (circuit_t * circ, int reason, int line, const char *file))
{
    ...
    log_info(LD_GENERAL,
             ...
             circ->n_circ_id,
             CIRCUIT_IS_ORIGIN(circ) ? TO_ORIGIN_CIRCUIT(circ)->global_identifier : 0, file, line,
             orig_reason, reason);
}
...
circuit_about_to_free(circuit_t *circ)
{
     ...
     */
    if (circ->state != CIRCUIT_STATE_OPEN && circ->state != CIRCUIT_STATE_GUARD_WAIT) {
        if (CIRCUIT_IS_ORIGIN(circ)) {
            origin_circuit_t *ocirc = TO_ORIGIN_CIRCUIT(circ);
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        circuit_event_status(
            ...
            (circ->state == CIRCUIT_STATE_OPEN || circ->state == CIRCUIT_STATE_GUARD_WAIT)
                ? CIRC_EVENT_CLOSED
                : CIRC_EVENT_FAILED,
            orig_reason);
    }
...
{
    if (CIRCUIT_IS_ORIGIN(c)) {
        return circuit_get_streams_max_data_age(CONST_TO_ORIGIN_CIRCUIT(c)->p_streams, now);
    } else {
        return circuit_get_streams_max_data_age(CONST_TO_OR_CIRCUIT(c)->n_streams, now);
    }
}
...
circuits_handle_oom(size_t current_allocation)
{
    ...
    log_notice(LD_GENERAL,
               ...
               tor_compress_get_total_allocation(), tor_zlib_get_total_allocation(),
               tor_zstd_get_total_allocation(), tor_lzma_get_total_allocation(),
               rend_cache_get_total_allocation());

    {
        size_t mem_target =
            (size_t)(get_options()->MaxMemInQueues * FRACTION_OF_DATA_TO_RETAIN_ON_OOM);
...
done_recovering_mem:
    ...
    log_notice(LD_GENERAL,
               ...
               "connections.",
               mem_recovered, n_circuits_killed, smartlist_len(circlist) - n_circuits_killed,
               n_dirconns_killed);
}
...
assert_circuit_ok, (const circuit_t *c))
{
    ...
    tor_assert(c->purpose >= CIRCUIT_PURPOSE_MIN_ && c->purpose <= CIRCUIT_PURPOSE_MAX_);
    ...
    if (c->n_chan) {
        ...
        if (c->n_circ_id) {
            ...
            circuit_t *c2 = circuit_get_by_circid_channel_impl(c->n_circ_id, c->n_chan, NULL);
            tor_assert(c == c2);
        }
    }
    if (or_circ && or_circ->p_chan) {
        if (or_circ->p_circ_id) {
            /* ibid */
            circuit_t *c2 =
                circuit_get_by_circid_channel_impl(or_circ->p_circ_id, or_circ->p_chan, NULL);
            tor_assert(c == c2);
        }
    }
    if (or_circ)
    ...
    if (c->state == CIRCUIT_STATE_CHAN_WAIT && !c->marked_for_close) {
        tor_assert(circuits_pending_chans && smartlist_contains(circuits_pending_chans, c));
    } else {
        tor_assert(!circuits_pending_chans || !smartlist_contains(circuits_pending_chans, c));
    }
