        ...
        if (CIRCUIT_IS_ORIGIN(circ)) {
            for (tmpconn = TO_ORIGIN_CIRCUIT(circ)->p_streams; tmpconn;
                ...
                if (TO_CONN(tmpconn) == conn) {
                    circuit_dump_conn_details(severity, circ,
                                              conn->conn_array_index,
...
circuit_unlink_all_from_channel(channel_t *chan, int reason)
{
...
#ifdef DEBUG_CIRCUIT_UNLINK_ALL
    {
        ...
        SMARTLIST_FOREACH_BEGIN(circuit_get_global_list(), circuit_t *, circ)
        {
            if (circ->n_chan == chan || (!CIRCUIT_IS_ORIGIN(circ) &&
        ...
        if (CIRCUIT_IS_ORIGIN(circ_) && circ_->state == CIRCUIT_STATE_OPEN &&
            ...
            if ((!need_uptime || circ->build_state->need_uptime) &&
                ...
                if (options->ExcludeNodes) {
                     ...
                     * for general use.  (This may be possible if StrictNodes is
                     * 0, and we thought we needed to use an otherwise excluded
                     * node for, say, a directory operation.) */
                    crypt_path_t *hop = circ->cpath;
                    do {
    ...
    if (circ->purpose == CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT) {
        ...
        if (orig_reason != END_CIRC_REASON_IP_NOW_REDUNDANT &&
            ...
            log_info(LD_REND, "Failed intro circ %s to %s (awaiting ack). %s",
                     ...
                     safe_str_client(
                         build_state_get_exit_nickname(ocirc->build_state)),
                     timed_out ? "Recording timeout."
                               : "Removing from descriptor.");
            rend_client_report_intro_point_failure(
                ocirc->build_state->chosen_exit, ocirc->rend_data,
                timed_out ? INTRO_POINT_FAILURE_TIMEOUT
    ...
    if (!CIRCUIT_IS_ORIGIN(circ)) {
        ...
        while (or_circ->resolving_streams) {
            ...
            if (!conn->base_.marked_for_close) {
                 ...
                 * are closing because the circuit is getting torn down.  No
                 * need to send an end cell. */
