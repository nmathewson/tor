...
circuit_purpose_to_string(uint8_t purpose)
{
    ...
    case CIRCUIT_PURPOSE_C_REND_READY_INTRO_ACKED:
        return "Hidden service client: Pending rendezvous point (ack "
               "received)";
    case CIRCUIT_PURPOSE_C_REND_JOINED:
...
origin_circuit_new(void)
{
    ...
    if (!circuit_build_times_disabled(get_options()) &&
        ...
        // remaining, or else we'll end up with the case where a circuit times
        // out and another one is built, effectively doubling the timeout
        // window.
        //
...
circuit_free_(circuit_t *circ)
{
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        ...
        if (ocirc->build_state) {
            ...
            cpath_ref_decref(
                ocirc->build_state->service_pending_final_cpath_ref);
        }
        ...
        /* Finally, free the identifier of the circuit and nullify it so
         * multiple cleanup will work. */
    ...
    if (circ->sendme_last_digests) {
        SMARTLIST_FOREACH(circ->sendme_last_digests, uint8_t *, d,
                          tor_free(d));
        smartlist_free(circ->sendme_last_digests);
    }

    log_info(
        LD_CIRC, "Circuit %u (id: %" PRIu32 ") has been freed.", n_circ_id,
    ...
    if (should_free) {
         ...
         * the magic value so that when the reply comes back, we'll know to
         * discard the reply and free this structure.
         */
...
circuit_free_all(void)
{
...

    {
        ...
        for (elt = HT_START(chan_circid_map, &chan_circid_map); elt;
             elt = next) {
            c = *elt;
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ) {
            ...
            for (tmpconn = TO_ORIGIN_CIRCUIT(circ)->p_streams; tmpconn;
                ...
                if (TO_CONN(tmpconn) == conn) {
                    circuit_dump_conn_details(
                        severity, circ, conn->conn_array_index, "App-ward",
                        p_circ_id, n_circ_id);
                }
            }
        }
        ...
        if (!CIRCUIT_IS_ORIGIN(circ)) {
            for (tmpconn = TO_OR_CIRCUIT(circ)->n_streams; tmpconn;
                ...
                if (TO_CONN(tmpconn) == conn) {
                    circuit_dump_conn_details(
                        severity, circ, conn->conn_array_index, "Exit-ward",
                        n_circ_id, p_circ_id);
                }
            }
        }
...
circuit_unlink_all_from_channel(channel_t *chan, int reason)
{
...
#ifdef DEBUG_CIRCUIT_UNLINK_ALL
    {
        ...
        SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *,
                                 circ) {
            if (circ->n_chan == chan ||
        ...
        if (!circ->marked_for_close &&
            ...
            if (!rend_cmp_service_ids(
                ...
                tor_memeq(ocirc->rend_data->rend_cookie,
                          rend_data->rend_cookie, REND_COOKIE_LEN))
                return ocirc;
        }
    } SMARTLIST_FOREACH_END (circ);
    return NULL;
                            ...
                            bool want_client_circ)
{
    ...
    for (; idx < smartlist_len(lst); ++idx) {
        ...
        /* Depending on whether we are looking for client or service circs,
         * skip circuits with other purposes. */
        if (want_client_circ) {
                                   ...
                                   const uint8_t *digest, uint8_t purpose)
{
    ...
    for (; idx < smartlist_len(lst); ++idx) {
        ...
        if (circ->purpose != purpose)
            continue;
        /* At this point we should be able to get a valid origin circuit
         * because
                            ...
                            int flags)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circuit_get_global_list(), circuit_t *, circ_) {
        if (CIRCUIT_IS_ORIGIN(circ_) && circ_->state == CIRCUIT_STATE_OPEN &&
            ...
            /* Ignore any circuits for which we can't use the Guard. It is
             * possible that the Guard was removed from the samepled set after
             * the circuit was created so avoid using it. */
            ...
            if ((!need_uptime || circ->build_state->need_uptime) &&
                ...
                if (options->ExcludeNodes) {
                    /* Make sure no existing nodes in the circuit are excluded
                     * for general use.  (This may be possible if StrictNodes
                     * is 0, and we thought we needed to use an otherwise
                     * excluded node for, say, a directory operation.) */
                    crypt_path_t *hop = circ->cpath;
                    do {
                        if (routerset_contains_extendinfo(
                                options->ExcludeNodes, hop->extend_info))
                            goto next;
                        hop = hop->next;
                    } while (hop != circ->cpath);
                }
...
circuit_get_cpath_len(origin_circuit_t *circ)
{
    ...
    if (circ && circ->cpath) {
        ...
        for (cpath = circ->cpath; cpath_next != circ->cpath;
             cpath = cpath_next) {
          ...
          (circuit_t * circ, int reason, int line, const char *file))
{
    ...
    if (circ->marked_for_close) {
        log_warn(LD_BUG,
                 ...
                 " (first at %s:%d)",
                 file, line, circ->marked_for_close_file,
                 circ->marked_for_close);
        return;
    }
    ...
    if (reason < END_CIRC_REASON_MIN_ || reason > END_CIRC_REASON_MAX_) {
        if (!(orig_reason & END_CIRC_REASON_FLAG_REMOTE))
            log_warn(LD_BUG, "Reason %d out of range at %s:%d", reason, file,
                     line);
        reason = END_CIRC_REASON_NONE;
    }
    ...
    if (!CIRCUIT_IS_ORIGIN(circ)) {
        ...
        if (or_circ->rend_splice) {
            if (!or_circ->rend_splice->base_.marked_for_close) {
                /* do this after marking this circuit, to avoid infinite
                 * recursion. */
                circuit_mark_for_close(TO_CIRCUIT(or_circ->rend_splice),
                                       reason);
            }
    ...
    if (!CIRCUIT_IS_ORIGIN(circ)) {
        ...
        while (or_circ->resolving_streams) {
            ...
            if (!conn->base_.marked_for_close) {
                /* The client will see a DESTROY, and infer that the
                 * connections are closing because the circuit is getting torn
                 * down.  No need to send an end cell. */
        ...
        if (or_circ->p_chan) {
            ...
            if (!CHANNEL_CONDEMNED(or_circ->p_chan)) {
                channel_send_destroy(or_circ->p_circ_id, or_circ->p_chan,
                                     reason);
            }
            circuitmux_detach_circuit(or_circ->p_chan->cmux, circ);
            circuit_set_p_circid_chan(or_circ, 0, NULL);
...
marked_circuit_free_stream_bytes(circuit_t *c)
{
    if (CIRCUIT_IS_ORIGIN(c)) {
        return marked_circuit_streams_free_bytes(
            TO_ORIGIN_CIRCUIT(c)->p_streams);
    } else {
...
circuits_handle_oom(size_t current_allocation)
{
    ...
    SMARTLIST_FOREACH_BEGIN (circlist, circuit_t *, circ) {
        ...
        /* Free storage in any non-linked directory connections that have
         * buffered data older than this circuit. */
...
assert_circuit_ok, (const circuit_t *c))
{
    ...
    if (c->n_chan) {
        ...
        if (c->n_circ_id) {
            /* We use the _impl variant here to make sure we don't fail on
             * marked circuits, which would not be returned by the regular
             * function. */
            circuit_t *c2 = circuit_get_by_circid_channel_impl(
                c->n_circ_id, c->n_chan, NULL);
            tor_assert(c == c2);
        }
    }
