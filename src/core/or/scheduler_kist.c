...
{
    HT_FOREACH_FN(socket_table_s, &socket_table, free_socket_info_by_ent,
                  NULL);
...
update_socket_info_impl, (socket_table_ent_t * ent))
{
    ...
    /* Gather information */
    if (getsockopt(sock, SOL_TCP, TCP_INFO, (void *)&(tcp), &tcp_info_len) <
        0) {
        if (errno == EINVAL) {
            /* Oops, this option is not provided by the kernel, we'll have to
             * disable KIST entirely. This can happen if tor was built on a
             * machine with the support previously or if the kernel was updated
             * and lost the support. */
            log_notice(LD_SCHED,
     ...
     * that is near the amount that the socket would be able to immediately
     * send into the Internet.
     *
     * We first calculate how much the socket could send immediately (assuming
     * completely full packets) according to the congestion window and the
     ...
     * not have to wait for the scheduler to run again. The amount of extra
     * space is a factor of the current congestion window. With the suggested
     * sock_buf_size_factor value of 1.0, we allow at most 2*cwnd bytes to sit
     * in the kernel: 1 cwnd on the wire waiting for ACKs and 1 cwnd ready and
     * waiting to be sent when those ACKs finally come.
     *
     * In the below diagram, we see some bytes in the TCP-space (denoted by
     * '*') that have be sent onto the wire and are waiting for ACKs. We have a
     * little more room in "TCP space" that we can fill with data that will be
    ...
    /* The clamp_double_to_int64 makes sure the first part fits into an
     * int64_t. In fact, if sock_buf_size_factor is still forced to be >= 0 in
     * config.c, then it will be positive for sure. Then we subtract a
     * uint32_t. Getting a negative value is OK, see after how it is being
     * handled. */
    ...
    if ((tcp_space + extra_space) < 0) {
        /* This means that the "notsent" queue is just too big so we shouldn't
         * put more in the kernel for now. */
        ent->limit = 0;
    } else {
...
fallback:
    ...
    /* This function calls the specialized channel object (currently
     * channeltls) and ask how many cells it can write on the outbuf which we
     * then multiply by the size of the cells for this channel. The cast is
     * because this function requires a non-const channel object, meh. */
...
socket_can_write(socket_table_t *table, const channel_t *chan)
{
    ...
    if (SCHED_BUG(!ent, chan)) {
        return 1; // Just return true, saying that kist wouldn't limit the
                  // socket
    }
...
update_socket_info(socket_table_t *table, const channel_t *chan)
{
    ...
    log_debug(LD_SCHED,
              ...
              ", mss: %" PRIu32,
              ent->chan->global_identifier, ent->limit, ent->cwnd,
              ent->unacked, ent->notsent, ent->mss);
}
          ...
          (outbuf_table_t * table, channel_t *chan))
{
    ...
    /* CELL_MAX_NETWORK_SIZE * 8 because we only want to write the outbuf to
     * the kernel if there's 8 or more cells waiting */
...
kist_scheduler_schedule(void)
{
    ...
    /* If time is really monotonic, we can never have now being smaller than
     * the last scheduler run. The scheduler_last_run at first is set to 0.
    ...
    if (diff < sched_run_interval) {
        next_run.tv_sec = 0;
        /* Takes 1000 ms -> us. This will always be valid because diff can NOT
         * be negative and can NOT be bigger than sched_run_interval so values
         * can only go from 1000 usec (diff set to interval - 1) to 100000 usec
         * (diff set to 0) for the maximum allowed run interval (100ms). */
...
kist_scheduler_run(void)
{
    ...
    while (smartlist_len(cp) > 0) {
        ...
        if (SCHED_BUG(!chan, NULL)) {
            /* Some-freaking-how a NULL got into the channels_pending. That
             * should never happen, but it should be harmless to ignore it and
             * keep looping.
             */
            continue;
        }
        ...
        if (socket_can_write(&socket_table, chan)) {
            ...
            flush_result =
                (int)channel_flush_some_cells(chan, 1); // 1 for num cells
            /* XXX: While flushing cells, it is possible that the connection
             * write fails leading to the channel to be closed which triggers a
             * release and free its entry in the socket table. And because of a
             * engineering design issue, the error is not propagated back so we
             * don't get an error at this point. So before we continue, make
             * sure the channel is
             * open and if not just ignore it. See #23751. */
            if (!CHANNEL_IS_OPEN(chan)) {
                /* Channel isn't open so we put it back in IDLE mode. It is
                 * either renegotiating its TLS session or about to be
                 * released. */
            ...
            if (flush_result > 0) {
                update_socket_written(&socket_table, chan,
                                      flush_result * (CELL_MAX_NETWORK_SIZE +
                                                      TLS_PER_CELL_OVERHEAD));
            } else {
                /* XXX: This can happen because tor sometimes does flush in an
                 * opportunistic way cells from the circuit to the outbuf so
                 * the channel can end up here without having anything to flush
                 * nor needed to write to the kernel. Hopefully we'll fix that
                 * soon but for now we have to handle this case which happens
                 * kind of often. */
                log_debug(
                    LD_SCHED,
                    "We didn't flush anything on a chan that we think "
                    "can write and wants to write. The channel's state is "
                    "'%s' "
                ...
                scheduler_set_channel_state(chan,
        ...
        if (!channel_more_to_flush(chan) &&
            ...
            /*
             * You might think we should put the channel in SCHED_CHAN_IDLE.
             * And you're probably correct. While implementing KIST, we found
             * that the scheduling system would sometimes lose track of
             * channels when we did that. We suspect it has to do with the
             * difference between "can't write because socket/outbuf is full"
             * and KIST's "can't write because we've arbitrarily decided that
             * that's enough for now." Sometimes channels run out of cells at
             * the same time they hit their kist-imposed write limit and maybe
             * the rest of Tor doesn't put the channel back in pending when it
             * is supposed to.
             *
             * This should be investigated again. It is as simple as changing
             * SCHED_CHAN_WAITING_FOR_CELLS to SCHED_CHAN_IDLE and seeing if
             * Tor starts having serious throughput issues. Best done in
             * shadow/chutney.
             */
             ...
             * channels_pending, we would never exit the scheduling loop. We
             * need to add it to a temporary list of channels to be added to
             * channels_pending after the scheduling loop is over. They can
             * hopefully be taken care of in the next scheduling round.
             */
            if (!to_readd) {
    ...
    HT_FOREACH_FN(outbuf_table_s, &outbuf_table, each_channel_write_to_kernel,
                  NULL);
    /* We are done with it. */
    HT_FOREACH_FN(outbuf_table_s, &outbuf_table, free_outbuf_info_by_ent,
                  NULL);
        ...
        SMARTLIST_FOREACH_BEGIN (to_readd, channel_t *, readd_chan) {
            ...
            if (!smartlist_contains(cp, readd_chan)) {
                if (!SCHED_BUG(readd_chan->sched_heap_idx != -1, readd_chan)) {
                    /* XXXX Note that the check above is in theory redundant
                     * with the smartlist_contains check.  But let's make sure
                     * we're not messing anything up, and leave them both for
                     * now. */
                    smartlist_pqueue_add(cp, scheduler_compare_channels,
                                         offsetof(channel_t, sched_heap_idx),
                                         readd_chan);
...
scheduler_can_use_kist(void)
{
    ...
    /* We do have the support, time to check if we can get the interval that
     * the consensus can be disabling. */
