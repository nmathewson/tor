...
HT_PROTOTYPE(socket_table_s, socket_table_ent_t, node, socket_table_ent_hash, socket_table_ent_eq);
HT_GENERATE2(socket_table_s, socket_table_ent_t, node, socket_table_ent_hash, socket_table_ent_eq,
...
HT_GENERATE2(outbuf_table_s, outbuf_table_ent_t, node, outbuf_table_ent_hash, outbuf_table_ent_eq,
             0.6, tor_reallocarray, tor_free_);
...
free_socket_info_by_chan(socket_table_t *table, const channel_t *chan)
{
    ...
    if (!ent)
        return;
    log_debug(LD_SCHED, "scheduler free socket info for chan=%" PRIu64, chan->global_identifier);
...
update_socket_info_impl, (socket_table_ent_t * ent))
{
    ...
    tor_assert(ent);
    tor_assert(ent->chan);
    const tor_socket_t sock = TO_CONN(BASE_CHAN_TO_TLS((channel_t *)ent->chan)->conn)->s;
    struct tcp_info tcp;
    ...
    extra_space = clamp_double_to_int64((ent->cwnd * (int64_t)ent->mss) * sock_buf_size_factor) -
...
fallback:
    ...
    ent->limit = channel_num_cells_writeable((channel_t *)ent->chan) *
                 (get_cell_network_size(ent->chan->wide_circ_ids) + TLS_PER_CELL_OVERHEAD);
}
...
socket_can_write(socket_table_t *table, const channel_t *chan)
{
    ...
    int64_t kist_limit_space =
        (int64_t)(ent->limit - ent->written) / (CELL_MAX_NETWORK_SIZE + TLS_PER_CELL_OVERHEAD);
...
update_socket_info(socket_table_t *table, const channel_t *chan)
{
    ...
    log_debug(LD_SCHED,
              "chan=%" PRIu64 " updated socket info, limit: %" PRIu64 ", cwnd: %" PRIu32
              ", unacked: %" PRIu32 ", notsent: %" PRIu32 ", mss: %" PRIu32,
              ent->chan->global_identifier, ent->limit, ent->cwnd, ent->unacked, ent->notsent,
              ent->mss);
}
 ...
 */
MOCK_IMPL(int,
channel_should_write_to_kernel, (outbuf_table_t * table, channel_t *chan))
{
...
channel_write_to_kernel, (channel_t * chan))
{
    ...
    log_debug(LD_SCHED, "Writing %lu bytes to kernel for chan %" PRIu64,
              (unsigned long)channel_outbuf_length(chan), chan->global_identifier);
...
kist_scheduler_run(void)
{
    ...
    log_debug(LD_SCHED, "Running the scheduler. %d channels pending", smartlist_len(cp));
    ...
    while (smartlist_len(cp) > 0) {
        ...
        if (!channel_more_to_flush(chan) && !socket_can_write(&socket_table, chan)) {
    ...
    HT_FOREACH_FN(outbuf_table_s, &outbuf_table, each_channel_write_to_kernel, NULL);
    /* We are done with it. */
...
kist_scheduler_run_interval(void)
{
    ...
    return networkstatus_get_param(NULL, "KISTSchedRunInterval", KIST_SCHED_RUN_INTERVAL_DEFAULT,
...
scheduler_kist_set_lite_mode(void)
{
    ...
    log_info(LD_SCHED, "Setting KIST scheduler without kernel support (KISTLite mode)");
}
