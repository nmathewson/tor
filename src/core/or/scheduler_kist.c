...
kist_scheduler_run(void)
{
    ...
    while (smartlist_len(cp) > 0) {
        ...
        if (socket_can_write(&socket_table, chan)) {
             ...
             * sure the channel is open and if not just ignore it. See #23751.
             */
            ...
            if (flush_result > 0) {
                 ...
                 * channel can end up here without having anything to flush nor
                 * needed to write to the kernel. Hopefully we'll fix that soon
                 * but for now
        ...
        if (!channel_more_to_flush(chan) &&
             ...
             * you're probably correct. While implementing KIST, we found that
             * the scheduling system would sometimes lose track of channels when
             * we did that. We suspect it has to do with the difference between
             * "can't write because socket/outbuf is full" and KIST's "can't
             * write because we've arbitrarily decided that that's enough for
             * now." Sometimes channels run out of cells at the same time they
             * hit their kist-imposed write limit and maybe the rest of Tor
             * doesn't put the channel back in pending when it is supposed to.
             *
             ...
             * starts having serious throughput issues. Best done in
             * shadow/chutney.
             */
            scheduler_set_channel_state(chan, SCHED_CHAN_WAITING_FOR_CELLS);
