...
{
    return networkstatus_get_param(NULL, "sendme_emit_min_version", SENDME_EMIT_MIN_VERSION_DEFAULT,
...
get_accept_min_version(void)
{
    return networkstatus_get_param(NULL, "sendme_accept_min_version",
                                   SENDME_ACCEPT_MIN_VERSION_DEFAULT, SENDME_ACCEPT_MIN_VERSION_MIN,
...
pop_first_cell_digest(const circuit_t *circ)
{
    ...
    if (circ->sendme_last_digests == NULL || smartlist_len(circ->sendme_last_digests) == 0) {
        return NULL;
    }
...
v1_digest_matches(const uint8_t *circ_digest, const uint8_t *cell_digest)
{
    ...
    if (tor_memneq(circ_digest, cell_digest, TRUNNEL_SENDME_V1_DIGEST_LEN)) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "SENDME v1 cell digest do not match.");
        return false;
    }
...
STATIC bool
sendme_is_valid(const circuit_t *circ, const uint8_t *cell_payload, size_t cell_payload_len)
{
    ...
    case 0x00:
        ...
        break;
    default:
        log_warn(LD_PROTOCOL, "Unknown SENDME cell version %d received.", cell_version);
...
build_cell_payload_v1(const uint8_t *cell_digest, uint8_t *payload)
{
    ...
    memcpy(sendme_cell_getarray_data_v1_digest(cell), cell_digest, sendme_cell_get_data_len(cell));
...
static int
send_circuit_level_sendme(circuit_t *circ, crypt_path_t *layer_hint, const uint8_t *cell_digest)
{
    ...
    if (relay_send_command_from_edge(0, circ, RELAY_COMMAND_SENDME, (char *)payload, payload_len,
                                     layer_hint) < 0) {
        log_warn(LD_CIRC, "SENDME relay_send_command_from_edge failed. Circuit's closed.");
...
record_cell_digest_on_circ(circuit_t *circ, const uint8_t *sendme_digest)
{
    ...
    smartlist_add(circ->sendme_last_digests, tor_memdup(sendme_digest, DIGEST_LEN));
}
...
sendme_connection_edge_consider_sending(edge_connection_t *conn)
{
    ...
    while (conn->deliver_window <= (STREAMWINDOW_START - STREAMWINDOW_INCREMENT)) {
        ...
        if (connection_edge_send_command(conn, RELAY_COMMAND_SENDME, NULL, 0) < 0) {
...
int
sendme_process_circuit_level(crypt_path_t *layer_hint, circuit_t *circ, const uint8_t *cell_payload,
                             uint16_t cell_payload_len)
{
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        ...
        if ((layer_hint->package_window + CIRCWINDOW_INCREMENT) > CIRCWINDOW_START_MAX) {
 ...
 * circuit must be closed using the value as the reason for it. */
int
sendme_process_stream_level(edge_connection_t *conn, circuit_t *circ, uint16_t cell_body_len)
{
    tor_assert(conn);
    tor_assert(circ);
    ...
    if ((conn->package_window + STREAMWINDOW_INCREMENT) > STREAMWINDOW_START_MAX) {
    ...
    log_debug(CIRCUIT_IS_ORIGIN(circ) ? LD_APP : LD_EXIT,
              "stream-level sendme, package_window now %d.", conn->package_window);
    return 0;
}
...
sendme_record_cell_digest_on_circ(circuit_t *circ, crypt_path_t *cpath)
{
    ...
    if (cpath) {
        ...
        sendme_digest = relay_crypto_get_sendme_digest(&TO_OR_CIRCUIT(circ)->crypto);
    }
...
sendme_record_received_cell_digest(circuit_t *circ, crypt_path_t *cpath)
{
    ...
    if (!circuit_sendme_cell_is_next(cpath ? cpath->deliver_window : circ->deliver_window)) {
        return;
    }
...
sendme_record_sending_cell_digest(circuit_t *circ, crypt_path_t *cpath)
{
    ...
    /* Only record if the next cell is expected to be a SENDME. */
    if (!circuit_sendme_cell_is_next(cpath ? cpath->package_window : circ->package_window)) {
        goto end;
    }
