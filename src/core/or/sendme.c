...
cell_v1_is_valid(const sendme_cell_t *cell, const uint8_t *circ_digest)
{
    ...
    const uint8_t *cell_digest =
        sendme_cell_getconstarray_data_v1_digest(cell);
...
cell_version_can_be_handled(uint8_t cell_version)
{
     ...
     * handled by us and if not, regardless of the cell version we got, we
     * can't continue. */
                ...
                size_t cell_payload_len)
{
    ...
    /* Pop the first element that was added (FIFO). We do that regardless of
     * the version so we don't accumulate on the circuit if v0 is used by the
     * other end point. */
...
circuit_sendme_cell_is_next(int window)
{
     ...
     * next cell is a SENDME, the window (either package or deliver) hasn't
     * been decremented just yet so when this is called, we are currently
     * processing the "window - 1" cell.
     *
     * This function is used when recording a cell digest and this is done
     * quite low in the stack when decrypting or encrypting a cell. The window
     * is only updated once the cell is actually put in the outbuf. */
...
sendme_circuit_consider_sending(circuit_t *circ, crypt_path_t *layer_hint)
{
    ...
    while ((layer_hint ? layer_hint->deliver_window : circ->deliver_window) <=
        ...
        /* Current implementation is not suppose to send multiple SENDME at
         * once because this means we would use the same relay crypto digest
         * for each SENDME leading to a mismatch on the other side and the
         * circuit to collapse. Scream loudly if it ever happens so we can
         * address it. */
                             ...
                             uint16_t cell_payload_len)
{
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        ...
        if ((circ->package_window + CIRCWINDOW_INCREMENT) >
            ...
            log_fn_ratelim(&client_warn_ratelim, LOG_PROTOCOL_WARN,
                           LD_PROTOCOL,
                            ...
                            uint16_t cell_body_len)
{
    ...
    if ((conn->package_window + STREAMWINDOW_INCREMENT) >
        ...
        log_fn_ratelim(
            &stream_warn_ratelim, LOG_PROTOCOL_WARN, LD_PROTOCOL,
