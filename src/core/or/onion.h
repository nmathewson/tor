...
typedef struct extend_cell_t {
    ...
    /** An IPv6 address and port for the node we're connecting to. Not
     * currently used. */
     ...
     * create cells we generate ourselves, this create cell can have a
     * handshake type we don't recognize. */
...
MOCK_DECL(int, extend_cell_parse,
          (extend_cell_t * cell_out, const uint8_t command,
