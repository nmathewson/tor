...
secs_to_uptime(long secs)
{
    ...
    case 0:
        ...
        break;
    case 1:
        tor_asprintf(&uptime_string, "%ld day %d:%02d hours", days, hours, minutes);
        break;
    default:
        tor_asprintf(&uptime_string, "%ld days %d:%02d hours", days, hours, minutes);
        break;
    }
...
log_onion_service_stats(void)
{
    ...
    log_notice(LD_HEARTBEAT,
               ...
               num_services == 1 ? "" : "s", hs_stats_get_n_introduce2_v2_cells(),
               hs_stats_get_n_introduce2_v3_cells(), hs_stats_get_n_rendezvous_launches());
}
...
log_heartbeat(time_t now)
{
    ...
    if (stats_n_data_cells_packaged && !hibernating) {
        fullness_pct = 100 * (((double)stats_n_data_bytes_packaged) /
...
#define TLS_OVERHEAD_THRESHOLD 15
    ...
    const int severity =
        (fullness_pct < FULLNESS_PCT_THRESHOLD || overhead_pct > TLS_OVERHEAD_THRESHOLD)
            ? LOG_NOTICE
            : LOG_INFO;
    ...
    if (options->MainloopStats) {
        ...
        log_fn(LOG_NOTICE, LD_HEARTBEAT,
               ...
               "%" PRIu64 " idle returns.",
               (main_loop_success_count), (main_loop_error_count), (main_loop_idle_count));
    }
