...
struct connection_t {
    ...
    unsigned int
        purpose : 5; /**< Only used for DIR and EXIT types currently. */
    ...
    unsigned int inbuf_reached_eof : 1; /**< Boolean: did read() return 0 on
                                         * this conn? */
    /** Set to 1 when we're inside connection_flushed_some to keep us from
     * calling connection_handle_write() recursively. */
    unsigned int in_flushed_some : 1;
    /** True if connection_handle_write is currently running on this
     * connection.
     */
    ...
    size_t outbuf_flushlen; /**< How much data should we try to flush from the
                             * outbuf? */
    time_t timestamp_last_read_allowed; /**< When was the last time libevent
                                         * said we could read? */
    ...
    tor_addr_t
        addr; /**< IP that socket "s" is directly connected to;
