...
connection_edge_reached_eof(edge_connection_t *conn)
{
    ...
    log_info(LD_EDGE,
             "conn (fd " TOR_SOCKET_T_FORMAT ") reached eof. Closing.",
             conn->base_.s);
...
connection_edge_process_inbuf(edge_connection_t *conn, int package_partial)
{
    ...
    case AP_CONN_STATE_CONNECT_WAIT:
        if (connection_ap_supports_optimistic_data(EDGE_TO_ENTRY_CONN(conn))) {
            ...
            if (connection_edge_package_raw_inbuf(conn, package_partial,
                                                  NULL) < 0) {
                /* (We already sent an end cell if possible) */
                connection_mark_for_close(TO_CONN(conn));
                return -1;
        ...
        if ((m = rate_limit_log(&warn_limit, approx_time()))) {
            log_warn(LD_EDGE, "Onion service connection to %s failed (%s)",
                     (conn->base_.socket_family == AF_UNIX)
                         ...
                         : safe_str(fmt_addrport(&conn->base_.addr,
                                                 conn->base_.port)),
...
connection_edge_end(edge_connection_t *conn, uint8_t reason)
{
    ...
    if (conn->base_.marked_for_close) {
        log_warn(
            LD_BUG, "called on conn that's already marked for close at %s:%d.",
    ...
    if (reason == END_STREAM_REASON_EXITPOLICY &&
        ...
        set_uint32(payload + 1 + addrlen,
                   htonl(clip_dns_ttl(conn->address_ttl)));
    ...
    if (circ && !circ->marked_for_close) {
        ...
        connection_edge_send_command(conn, RELAY_COMMAND_END, payload,
                                     payload_len);
        /* We'll log warn if the connection was an hidden service and couldn't
         * be made because the service wasn't available. */
            ...
            log_warn(LD_BUG,
                     ...
                     CIRCUIT_IS_ORIGIN(circ)
                         ? pathbias_state_to_string(
                               TO_ORIGIN_CIRCUIT(circ)->path_state)
                         : "none");
        }
                                       ...
                                       const char *fname, int lineno)
{
    ...
    if (PREDICT_UNLIKELY(
        ...
        log_warn(LD_BUG, "(Previously called from %s:%d.)\n",
                 f2 ? f2 : "<NULL>", entry_conn->marked_pending_circ_line);
#endif /* defined(DEBUGGING_17659) */
                          ...
                          cpath_build_state_t *build_state)
{
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn) {
        ...
        if (tor_digest_is_zero(digest)) {
            ...
            if (tor_addr_parse(&addr, entry_conn->socks_request->address) <
                    0 ||
...
circuit_discard_optional_exit_enclaves(extend_info_t *info)
{
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn) {
        ...
        if (entry_conn->chosen_exit_optional) {
            log_info(
                LD_APP, "Giving up on enclave exit '%s' for destination %s.",
            ...
            consider_plaintext_ports(entry_conn,
                                     entry_conn->socks_request->port);
        }
        if (entry_conn->chosen_exit_retries) {
            if (--entry_conn->chosen_exit_retries == 0) { /* give up! */
                ...
                tor_free(entry_conn->chosen_exit_name); /* clears it */
                /* if this port is dangerous, warn or reject it now that we
                 * don't think it'll be using an enclave. */
                consider_plaintext_ports(entry_conn,
                                         entry_conn->socks_request->port);
            }
        }
...
consider_plaintext_ports(entry_connection_t *conn, uint16_t port)
{
    ...
    if (smartlist_contains_int_as_string(options->WarnPlaintextPorts, port)) {
        ...
        control_event_client_status(LOG_WARN,
                                    "DANGEROUS_PORT PORT=%d RESULT=%s", port,
                                    reject ? "REJECT" : "WARN");
    }

    if (reject) {
        log_info(LD_APP, "Port %d listed in RejectPlaintextPorts. Closing.",
                 port);
...
parse_extended_hostname(char *address, hostname_type_t *type_out)
{
    ...
    if (strcmp(s + 1, "onion")) {
        *type_out =
            NORMAL_HOSTNAME; /* neither .exit nor .onion, thus normal */
        goto success;
    }
...
failed:
    ...
    *s = '.';
    const bool is_onion =
        (*type_out == ONION_V2_HOSTNAME) || (*type_out == ONION_V3_HOSTNAME);
    log_warn(LD_APP, "Invalid %shostname %s; rejecting",
             is_onion ? "onion " : "", safe_str_client(address));
    return false;
}
                                ...
                                rewrite_result_t *out)
{
    ...
    if (socks->command == SOCKS_COMMAND_RESOLVE &&
        ...
        /* Check the suffix... */
        out->automap =
            addressmap_address_should_automap(socks->address, options);
        if (out->automap) {
            ...
            if (conn->socks_request->socks_version != 4) {
                if (!conn->entry_cfg.ipv4_traffic ||
                    (conn->entry_cfg.ipv6_traffic &&
                     conn->entry_cfg.prefer_ipv6) ||
                    conn->entry_cfg.prefer_ipv6_virtaddr)
                    addr_type = RESOLVED_TYPE_IPV6;
            }
            /* Okay, register the target address as automapped, and find the
             * new address we're supposed to give as a resolve answer.  (Return
             * a cached value if we've looked up this address before.
             */
    ...
    if (socks->command == SOCKS_COMMAND_RESOLVE_PTR) {
        ...
        if (addressmap_rewrite_reverse(socks->address, sizeof(socks->address),
            ...
            tor_snprintf(socks->address, sizeof(socks->address), "REVERSE[%s]",
                         out->orig_address);
            connection_ap_handshake_socks_resolved(
                conn, RESOLVED_TYPE_HOSTNAME, strlen(result),
        ...
        /* Hang on, did we find an answer saying that this is a reverse lookup
         * for an internal address?  If so, we should reject it if we're
         * configured to do so. */
        if (options->ClientDNSRejectInternalAddresses) {
            ...
            if (ok == 1 && tor_addr_is_internal(&addr, 0)) {
                connection_ap_handshake_socks_resolved(
                    conn, RESOLVED_TYPE_ERROR, 0, NULL, -1, TIME_MAX);
    ...
    if (socks->command != SOCKS_COMMAND_RESOLVE_PTR && !out->automap) {
        ...
        if (out->exit_source == ADDRMAPSRC_NONE) {
            /* If it wasn't a .exit before, maybe it turned into a .exit.
             * Remember the original source of a .exit. */
    ...
    if (!out->automap && address_is_in_virtual_range(socks->address)) {
         ...
         * client_dns_get_unmapped_address, but the mapping was discarded for
         * some reason.  Or the user typed in a virtual address range manually.
         * We *don't* want to send the address through Tor; that's likely to
         * fail, and may leak information.
         */
                           ...
                           origin_circuit_t *circ, hostname_type_t addresstype)
{
    ...
    if (refetch_desc || !descriptor_is_usable) {
        ...
        if (addresstype == ONION_V2_HOSTNAME) {
            ...
            /* Attempt to fetch the hsv3 descriptor. Check the retval to see
             * how it went and act accordingly. */
            int ret =
                hs_client_refetch_hsdesc(&edge_conn->hs_ident->identity_pk);
            switch (ret) {
            case HS_CLIENT_FETCH_MISSING_INFO:
                /* Keeping the connection in descriptor wait state is fine
                 * because once we get enough dirinfo or a new live consensus,
                 * the HS client subsystem is notified and every connection in
                 * that state will trigger a fetch for the service key. */
                                           ...
                                           crypt_path_t *cpath)
{
    ...
    if (addresstype == EXIT_HOSTNAME) {
        ...
        if (exit_source == ADDRMAPSRC_DNS || exit_source == ADDRMAPSRC_NONE) {
            /* It shouldn't be possible to get a .exit address from any of
             * these sources. */
            log_warn(LD_BUG,
        ...
        if (s) {
            ...
            if (node) {
                *socks->address = 0;
                node_get_address_string(node, socks->address,
    ...
    if (addresstype != ONION_V2_HOSTNAME && addresstype != ONION_V3_HOSTNAME) {
         ...
         *
         * If we can do DNS requests, and we can use at least one address
         * family, then we have to resolve the address first. Then we'll know
         * if it resolves to a usable address family. */
         ...
         * (For example, an attempt to look up the IP address for an IP
         * address.)
         */
        if (socks->command == SOCKS_COMMAND_RESOLVE) {
            tor_addr_t answer;
            /* Reply to resolves immediately if we can. */
            if (tor_addr_parse(&answer, socks->address) >=
                0) { /* is it an IP? */
                /* remember _what_ is supposed to have been resolved. */
                strlcpy(socks->address, rr.orig_address,
                        sizeof(socks->address));
             ...
             * Exceptions are begindir requests (where the address is
             * meaningless), or cases where you've hand-configured a particular
             * exit, thereby making the local address meaningful. */
            if (options->ClientRejectInternalAddresses &&
                ...
                if (tor_addr_hostname_is_local(socks->address) ||
                    ...
                    /* If this is an explicit private address with no chosen
                     * exit node, then we really don't want to try to connect
                     * to it.  That's probably an error. */
                    if (conn->is_transparent_ap) {
#define WARN_INTRVL_LOOP 300
                        static ratelim_t loop_warn_limit =
                            RATELIM_INIT(WARN_INTRVL_LOOP);
                        char *m;
                        if ((m = rate_limit_log(&loop_warn_limit,
                                                approx_time()))) {
                            log_warn(LD_NET,
                                     "Rejecting request for anonymous "
                                     "connection to private "
                                     "address %s on a TransPort or NATDPort.  "
                                     "Possible loop "
                                     "in your NAT rules?%s",
...
#define WARN_INTRVL_PRIV 300
                        static ratelim_t priv_warn_limit =
                            RATELIM_INIT(WARN_INTRVL_PRIV);
                        char *m;
                        if ((m = rate_limit_log(&priv_warn_limit,
                                                approx_time()))) {
                            log_warn(LD_NET,
                                     "Rejecting SOCKS request for anonymous "
                                     "connection to "
                                     "private address %s.%s",
                                     safe_str_client(socks->address), m);
                            tor_free(m);
                    ...
                    connection_mark_unattached_ap(
                        conn, END_STREAM_REASON_PRIVATE_ADDR);
                    return -1;
                }
             ...
             * fields in conn->entry_cfg to tell the exit what AF we want. */
            {
                ...
                if (tor_addr_parse(&addr, socks->address) >= 0) {
                    ...
                    if ((family == AF_INET && !conn->entry_cfg.ipv4_traffic) ||
                        (family == AF_INET6 &&
                         !conn->entry_cfg.ipv6_traffic)) {
                        /* You can't do an IPv4 address on a v6-only socks
                        ...
                        connection_mark_unattached_ap(
                            conn, END_STREAM_REASON_ENTRYPOLICY);
                        return -1;
                    } else if (family == AF_INET6 &&
                               socks->socks_version == 4) {
                        /* You can't make a socks4 request to an IPv6 address.
                         * Socks4 doesn't support that. */
                        log_warn(
                            LD_NET,
                        ...
                        /* You can't do any kind of Socks4 request when IPv4 is
                         * forbidden.
                         *
                        ...
                        /* Tell the exit: we won't accept any ipv4 connection
                         * to an IPv6 address. */
                        conn->entry_cfg.ipv4_traffic = 0;
                    } else if (family == AF_INET) {
                        /* Tell the exit: we won't accept any ipv6 connection
                         * to an IPv4 address. */
            ...
            if (!conn->use_begindir && !conn->chosen_exit_name && !circ) {
                ...
                const node_t *r = router_find_exact_exit_enclave(
        ...
        /* Okay. At this point we've set chosen_exit_name if needed, rewritten
         * the address, and decided not to reject it for any number of reasons.
         * Now mark the connection as waiting for a circuit, and try to attach
         * it!
         */
        base_conn->state = AP_CONN_STATE_CIRCUIT_WAIT;
        ...
        int rv;
        if (circ) {
            rv = connection_ap_handshake_attach_chosen_circuit(conn, circ,
                                                               cpath);
        } else {
            /* We'll try to attach it at the next event loop, or whenever
             * we call connection_ap_attach_pending() */
         ...
         * if it returned 1, we're attached.  Both are okay.  But if it
         * returned -1, there was an error, so make sure the connection is
         * marked, and return -1. */
        if (rv < 0) {
            if (!base_conn->marked_for_close)
                connection_mark_unattached_ap(conn,
...
#    ifdef TRANS_TPROXY
    if (get_options()->TransProxyType_parsed == TPT_TPROXY) {
        if (getsockname(ENTRY_TO_CONN(conn)->s, (struct sockaddr *)&orig_dst,
            ...
            log_warn(LD_NET, "getsockname() failed: %s",
    ...
    if (sockshere == 0) {
        ...
        if (!had_reply) {
            ...
            connection_ap_handshake_socks_reply(
                conn, NULL, 0, END_STREAM_REASON_SOCKSPROTOCOL);
        }
        connection_mark_unattached_ap(
            conn, END_STREAM_REASON_SOCKSPROTOCOL |
                      END_STREAM_REASON_FLAG_ALREADY_SOCKS_REPLIED);
...
connection_ap_process_natd(entry_connection_t *conn)
{
    ...
    *tbuf++ = '\0';
    ...
    socks->port =
        (uint16_t)tor_parse_long(tbuf, 10, 1, 65535, &port_ok, &daddr);
    if (!port_ok) {
        log_warn(LD_APP,
...
connection_ap_process_http_connect(entry_connection_t *conn)
{
    ...
    {
        char *authorization =
            http_get_header(headers, "Proxy-Authorization: ");
        if (authorization) {
          ...
          (entry_connection_t * ap_conn))
{
    ...
    if (begin_type == RELAY_COMMAND_BEGIN) {
         ...
         * Look at the linked directory connection to access the directory
         * purpose.
        ...
        /* If the linked connection has been unlinked by other code, we can't
         * send a begin cell on it. */
         ...
         * anonymity in directory_initiate_request(). */
        ...
        /* This code was written for the two connection types BEGIN and
         * BEGIN_DIR
         */
           ...
           const uint8_t *answer, int ttl, time_t expires))
{
    ...
    if (ttl >= 0) {
        if (answer_type == RESOLVED_TYPE_IPV4 && answer_len == 4) {
            ...
            if (!tor_addr_is_null(&a)) {
                client_dns_set_addressmap(conn, conn->socks_request->address,
                                          &a, conn->chosen_exit_name, ttl);
            }
    ...
    if (ENTRY_TO_EDGE_CONN(conn)->is_dns_request) {
        if (conn->dns_server_request) {
            ...
            dnsserv_resolved(conn, answer_type, answer_len, (char *)answer,
                             ttl);
            conn->socks_request->has_finished = 1;
            return;
        } else {
            /* This must be a request from the controller. Since answers to
             * those requests are not cached, they do not generate an ADDRMAP
             * event on their own. */
            tell_controller_about_resolved_result(
                conn, answer_type, answer_len, (char *)answer, ttl, expires);
            conn->socks_request->has_finished = 1;
            return;
        }
                                    ...
                                    size_t replylen, int endreason)
{
    ...
    if (status == SOCKS5_SUCCEEDED ||
        ...
        if (!conn->edge_.on_circuit ||
            ...
            // We don't want to mark optimistically opened streams as
            // successful.
            pathbias_mark_use_success(
                TO_ORIGIN_CIRCUIT(conn->edge_.on_circuit));
        }
    }
    ...
    if (conn->socks_request->listener_type ==
        ...
        if (tor_addr_family(&conn->edge_.base_.addr) == AF_INET) {
            ...
            buf[2] = 0;
            buf[3] = 4; /* ipv6 addr */
            /* 4 bytes for the header, 2 bytes for the port, 16 for the
             * address. */
            buf_len = 22;
        }
...
handle_hs_exit_conn(circuit_t *circ, edge_connection_t *conn)
{
    ...
    /* The circuit either has an hs identifier for v3+ or a rend_data for
     * legacy service. */
...
connection_exit_begin_conn(cell_t *cell, circuit_t *circ)
{
    ...
    if (rv < -1) {
        ...
        tor_free(bcell.address);
        relay_send_end_cell_from_edge(rh.stream_id, circ, end_reason,
                                      layer_hint);
        return 0;
    }
    ...
    if (!bcell.is_begindir) {
        ...
        if (or_circ && or_circ->p_chan) {
            ...
            if ((client_chan || (!connection_or_digest_is_known_relay(
                 ...
                 * attackers and users who'd be better off with, well,
                 * single-hop proxies. */
                log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
                       "Attempt by %s to open a stream %s. Closing.",
                       safe_str(channel_get_canonical_remote_descr(
                           or_circ->p_chan)),
                       client_chan ? "on first hop of circuit"
                                   : "from unknown relay");
                relay_send_end_cell_from_edge(
                    rh.stream_id, circ,
                    client_chan ? END_STREAM_REASON_TORPROTOCOL
    ...
    if (!options->IPv6Exit) {
        ...
        if (bcell.flags & BEGIN_FLAG_IPV4_NOT_OK) {
            tor_free(address);
            relay_send_end_cell_from_edge(
                rh.stream_id, circ, END_STREAM_REASON_EXITPOLICY, layer_hint);
            return 0;
        }
    }
...
connection_exit_connect(edge_connection_t *edge_conn)
{
...
#else
    {
         ...
         * tor_addr_make_unspec(conn->addr) (cf. the way we mark in the
         * incoming case in connection_handle_listener_read()), and
         * conn->address should have the socket path to connect to.
         */
    ...
    if (connection_edge_is_rendezvous_stream(edge_conn)) {
        ...
        connection_edge_send_command(edge_conn, RELAY_COMMAND_CONNECTED, NULL,
                                     0);
    } else { /* normal stream */
        uint8_t connected_payload[MAX_CONNECTED_CELL_PAYLOAD_LEN];
        int connected_payload_len = connected_cell_format_payload(
        ...
        if (connected_payload_len < 0) {
            ...
            circuit_detach_stream(circuit_get_by_edge_conn(edge_conn),
                                  edge_conn);
            connection_free(conn);
            return;
        }
                           ...
                           const node_t *exit_node)
{
    ...
    if (conn->chosen_exit_name) {
        ...
        if (!chosen_exit || tor_memneq(chosen_exit->identity,
            ...
            /* doesn't match */
            //      log_debug(LD_APP,"Requested node '%s', considering node
            //      '%s'. No.",
    ...
    if (conn->socks_request->command == SOCKS_COMMAND_CONNECT) {
        ...
        if (r == ADDR_POLICY_PROBABLY_REJECTED && !conn->chosen_exit_name)
            return 0; /* We don't know the addr, but the exit policy rejects
                       * most addresses with this port. Since the user didn't
                       * ask for this node, err on the side of caution. */
    } else if (SOCKS_COMMAND_IS_RESOLVE(conn->socks_request->command)) {
