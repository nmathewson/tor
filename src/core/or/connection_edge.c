...
{
    tor_assert(c->magic == EDGE_CONNECTION_MAGIC || c->magic == ENTRY_CONNECTION_MAGIC);
          ...
          (entry_connection_t * conn, int endreason, int line, const char *file))
{
    ...
    if (!conn->socks_request->has_finished) {
        if (endreason & END_STREAM_REASON_FLAG_ALREADY_SOCKS_REPLIED)
            log_warn(LD_BUG, "stream (marked at %s:%d) sending two socks replies?", file, line);
        ...
        if (SOCKS_COMMAND_IS_CONNECT(conn->socks_request->command))
            ...
            connection_ap_handshake_socks_resolved(conn, RESOLVED_TYPE_ERROR_TRANSIENT, 0, NULL, -1,
                                                   -1);
...
{
    if (connection_get_inbuf_len(TO_CONN(conn)) && connection_state_is_open(TO_CONN(conn))) {
        /* it still has stuff to process. don't let it die yet. */
        return 0;
    }
    log_info(LD_EDGE, "conn (fd " TOR_SOCKET_T_FORMAT ") reached eof. Closing.", conn->base_.s);
...
connection_edge_process_inbuf(edge_connection_t *conn, int package_partial)
{
    ...
    case AP_CONN_STATE_CONNECT_WAIT:
        if (connection_ap_supports_optimistic_data(EDGE_TO_ENTRY_CONN(conn))) {
            log_info(LD_EDGE,
                     ...
                     conn_state_to_string(conn->base_.type, conn->base_.state), package_partial,
    ...
    case AP_CONN_STATE_CONTROLLER_WAIT:
        log_info(LD_EDGE, "data from edge while in '%s' state. Leaving it on buffer.",
...
connection_edge_destroy(circid_t circ_id, edge_connection_t *conn)
{
    if (!conn->base_.marked_for_close) {
        ...
        if (conn->base_.type == CONN_TYPE_AP) {
            ...
            control_event_stream_status(entry_conn, STREAM_EVENT_CLOSED, END_STREAM_REASON_DESTROY);
 ...
 * as <b>reason</b> */
static int
relay_send_end_cell_from_edge(streamid_t stream_id, circuit_t *circ, uint8_t reason,
                              crypt_path_t *cpath_layer)
{
    ...
    return relay_send_command_from_edge(stream_id, circ, RELAY_COMMAND_END, payload, 1,
                                        cpath_layer);
}
...
warn_if_hs_unreachable(const edge_connection_t *conn, uint8_t reason)
{
    ...
    if (conn->base_.type == CONN_TYPE_EXIT && connection_edge_is_rendezvous_stream(conn) &&
        (reason == END_STREAM_REASON_CONNECTREFUSED || reason == END_STREAM_REASON_TIMEOUT)) {
...
connection_edge_end(edge_connection_t *conn, uint8_t reason)
{
    ...
    if (conn->base_.marked_for_close) {
        log_warn(LD_BUG, "called on conn that's already marked for close at %s:%d.",
    ...
    if (circ && !circ->marked_for_close) {
        log_debug(LD_EDGE, "Sending end on conn (fd " TOR_SOCKET_T_FORMAT ").", conn->base_.s);
...
connection_half_edge_add(const edge_connection_t *conn, origin_circuit_t *circ)
{
    ...
    if (connection_half_edge_find_stream_id(circ->half_streams, conn->stream_id)) {
        log_warn(LD_BUG, "Duplicate stream close for stream %d on circuit %d", conn->stream_id,
    ...
    half_conn->connected_pending = conn->base_.state == AP_CONN_STATE_CONNECT_WAIT;
 ...
 */
STATIC half_edge_t *
connection_half_edge_find_stream_id(const smartlist_t *half_conns, streamid_t stream_id)
{
    if (!half_conns)
        return NULL;

    return smartlist_bsearch(half_conns, &stream_id, connection_half_edge_compare_bsearch);
}
 ...
 */
int
connection_half_edge_is_valid_data(const smartlist_t *half_conns, streamid_t stream_id)
{
    half_edge_t *half = connection_half_edge_find_stream_id(half_conns, stream_id);
 ...
 */
int
connection_half_edge_is_valid_connected(const smartlist_t *half_conns, streamid_t stream_id)
{
    half_edge_t *half = connection_half_edge_find_stream_id(half_conns, stream_id);
 ...
 */
int
connection_half_edge_is_valid_sendme(const smartlist_t *half_conns, streamid_t stream_id)
{
    half_edge_t *half = connection_half_edge_find_stream_id(half_conns, stream_id);
 ...
 */
int
connection_half_edge_is_valid_end(smartlist_t *half_conns, streamid_t stream_id)
{
    half_edge_t *half;
    ...
    remove_idx =
        smartlist_bsearch_idx(half_conns, &stream_id, connection_half_edge_compare_bsearch, &found);
    if (!found)
        return 0;
 ...
 */
int
connection_half_edge_is_valid_resolved(smartlist_t *half_conns, streamid_t stream_id)
{
 ...
 * success. */
STATIC int
connected_cell_format_payload(uint8_t *payload_out, const tor_addr_t *addr, uint32_t ttl)
{
...
STATIC void
export_hs_client_circuit_id(edge_connection_t *edge_conn, hs_circuit_id_protocol_t protocol)
{
    ...
    /* Build the string */
    tor_asprintf(&buf, "PROXY TCP6 %s:%x:%x %s %d %d\r\n", src_ipv6_prefix, gid >> 16,
                 gid & 0x0000ffff, dst_ipv6, src_port, dst_port);
...
connection_edge_finished_connecting(edge_connection_t *edge_conn)
{
    ...
    log_info(LD_EXIT, "Exit connection to %s:%u (%s) established.", escaped_safe_str(conn->address),
             conn->port, safe_str(fmt_and_decorate_addr(&conn->addr)));
    ...
    if (connection_edge_is_rendezvous_stream(edge_conn)) {
        if (connection_edge_send_command(edge_conn, RELAY_COMMAND_CONNECTED, NULL, 0) < 0)
            return 0; /* circuit is closed, don't continue */
    } else {
        uint8_t connected_payload[MAX_CONNECTED_CELL_PAYLOAD_LEN];
        int connected_payload_len =
            connected_cell_format_payload(connected_payload, &conn->addr, edge_conn->address_ttl);
...
connection_ap_about_to_close(entry_connection_t *entry_conn)
{
    ...
    if (TO_CONN(edge_conn)->type == CONN_TYPE_AP) {
        connection_ap_warn_and_unmark_if_pending_circ(entry_conn, "about_to_close");
    }
#endif /* 1 */

    control_event_stream_bandwidth(edge_conn);
    control_event_stream_status(entry_conn, STREAM_EVENT_CLOSED, edge_conn->end_reason);
...
connection_ap_expire_beginning(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, base_conn) {
        ...
        severity = (tor_addr_is_null(&base_conn->addr) && !base_conn->port) ? LOG_INFO : LOG_NOTICE;
        ...
        if (AP_CONN_STATE_IS_UNATTACHED(base_conn->state)) {
            if (seconds_since_born >= options->SocksTimeout) {
                log_fn(severity, LD_APP,
                       ...
                       "Giving up. (%s)",
                       seconds_since_born, safe_str_client(entry_conn->socks_request->address),
        ...
        if (circ->purpose != CIRCUIT_PURPOSE_C_GENERAL &&
            ...
            log_warn(LD_BUG,
                     ...
                     "path_state %s.",
                     circuit_purpose_to_string(circ->purpose), circuit_state_to_string(circ->state),
        ...
        log_fn(cutoff < 15 ? LOG_INFO : severity, LD_APP,
               ...
               seconds_idle, safe_str_client(entry_conn->socks_request->address),
               conn->cpath_layer ? extend_info_describe(conn->cpath_layer->extend_info)
                                 : "*unnamed*");
 ...
 */
void
connection_ap_mark_as_pending_circuit_(entry_connection_t *entry_conn, const char *fname,
                                       int lineno)
{
    connection_t *conn = ENTRY_TO_CONN(entry_conn);
    ...
    if (PREDICT_UNLIKELY(smartlist_contains(pending_entry_connections, entry_conn))) {
        log_warn(LD_BUG,
...
/* DOCDOC */
void
connection_ap_warn_and_unmark_if_pending_circ(entry_connection_t *entry_conn, const char *where)
{
    if (pending_entry_connections && smartlist_contains(pending_entry_connections, entry_conn)) {
        log_warn(LD_BUG, "What was %p doing in pending_entry_connections in %s?", entry_conn,
                 where);
        connection_ap_mark_as_non_pending_circuit(entry_conn);
    }
}
...
void
connection_ap_fail_onehop(const char *failed_digest, cpath_build_state_t *build_state)
{
    entry_connection_t *entry_conn;
...
circuit_discard_optional_exit_enclaves(extend_info_t *info)
{
    ...
    SMARTLIST_FOREACH_BEGIN (conns, connection_t *, conn) {
        ...
        if (!entry_conn->chosen_exit_optional && !entry_conn->chosen_exit_retries)
            continue;
        r1 = node_get_by_nickname(entry_conn->chosen_exit_name, NNF_NO_WARN_UNNAMED);
 ...
 */
int
connection_ap_detach_retriable(entry_connection_t *conn, origin_circuit_t *circ, int reason)
{
    ...
    if (conn->pending_optimistic_data) {
        buf_set_to_copy(&conn->sending_optimistic_data, conn->pending_optimistic_data);
    }
...
consider_plaintext_ports(entry_connection_t *conn, uint16_t port)
{
    ...
    if (smartlist_contains_int_as_string(options->WarnPlaintextPorts, port)) {
        log_warn(LD_APP,
                 ...
                 "Internet reading!%s",
                 port, reject ? " Closing." : "");
        control_event_client_status(LOG_WARN, "DANGEROUS_PORT PORT=%d RESULT=%s", port,
                                    reject ? "REJECT" : "WARN");
    }
...
parse_extended_hostname(char *address, hostname_type_t *type_out)
{
    ...
    *s = 0; /* NUL-terminate it */
    ...
    if (strlcpy(query, q, HS_SERVICE_ADDR_LEN_BASE32 + 1) >= HS_SERVICE_ADDR_LEN_BASE32 + 1) {
        *type_out = BAD_HOSTNAME;
        goto failed;
    }
    if (q != address) {
...
failed:
    ...
    *s = '.';
    const bool is_onion = (*type_out == ONION_V2_HOSTNAME) || (*type_out == ONION_V3_HOSTNAME);
    log_warn(LD_APP, "Invalid %shostname %s; rejecting", is_onion ? "onion " : "",
 ...
 */
STATIC void
connection_ap_handshake_rewrite(entry_connection_t *conn, rewrite_result_t *out)
{
    ...
    log_debug(LD_APP, "Client asked for %s:%d", safe_str_client(socks->address), socks->port);
    ...
    if (socks->command != SOCKS_COMMAND_RESOLVE_PTR) {
        ...
        if (addressmap_rewrite(socks->address, sizeof(socks->address), rewrite_flags,
                               &out->map_expires, &out->exit_source)) {
     ...
     */
    if (socks->command == SOCKS_COMMAND_RESOLVE && tor_addr_parse(&addr_tmp, socks->address) < 0 &&
        ...
        if (out->automap) {
             ...
             */
            new_addr = addressmap_register_virtual_address(addr_type, tor_strdup(socks->address));
            if (!new_addr) {
                log_warn(LD_APP, "Unable to automap address %s", escaped_safe_str(socks->address));
                out->end_reason = END_STREAM_REASON_INTERNAL;
                out->should_close = 1;
                return;
            }
            log_info(LD_APP, "Automapping %s to %s", escaped_safe_str_client(socks->address),
    ...
    if (socks->command == SOCKS_COMMAND_RESOLVE_PTR) {
        ...
        if (addressmap_rewrite_reverse(socks->address, sizeof(socks->address), rewrite_flags,
                                       &out->map_expires)) {
            char *result = tor_strdup(socks->address);
            /* remember _what_ is supposed to have been resolved. */
            tor_snprintf(socks->address, sizeof(socks->address), "REVERSE[%s]", out->orig_address);
            connection_ap_handshake_socks_resolved(conn, RESOLVED_TYPE_HOSTNAME, strlen(result),
                                                   (uint8_t *)result, -1, out->map_expires);
            tor_free(result);
            out->end_reason = END_STREAM_REASON_DONE | END_STREAM_REASON_FLAG_ALREADY_SOCKS_REPLIED;
            out->should_close = 1;
            return;
        }
        ...
        if (options->ClientDNSRejectInternalAddresses) {
            ...
            if (ok == 1 && tor_addr_is_internal(&addr, 0)) {
                connection_ap_handshake_socks_resolved(conn, RESOLVED_TYPE_ERROR, 0, NULL, -1,
                                                       TIME_MAX);
                out->end_reason =
                    END_STREAM_REASON_SOCKSPROTOCOL | END_STREAM_REASON_FLAG_ALREADY_SOCKS_REPLIED;
                out->should_close = 1;
                return;
            }
        }
...
static int
connection_ap_handle_onion(entry_connection_t *conn, socks_request_t *socks, origin_circuit_t *circ,
                           hostname_type_t addresstype)
{
    ...
    if (addresstype == ONION_V2_HOSTNAME) { /* it's a v2 hidden service */
        ...
        rend_cache_lookup_result = rend_cache_lookup_entry(onion_address, -1, &entry);
        ...
        hs_ident_edge_conn_t *hs_conn_ident = tor_malloc_zero(sizeof(hs_ident_edge_conn_t));

        retval = hs_parse_address(socks->address, &hs_conn_ident->identity_pk, NULL, NULL);
        if (retval < 0) {
        ...
        if (cached_desc) {
            ...
            descriptor_is_usable =
                hs_client_any_intro_points_usable(&hs_conn_ident->identity_pk, cached_desc);
            log_info(LD_GENERAL, "Found %s descriptor in cache for %s. %s.",
                     (descriptor_is_usable) ? "usable" : "unusable", safe_str_client(onion_address),
    ...
    if (rend_cache_lookup_result < 0) {
        ...
        case EINVAL:
            ...
            log_warn(LD_BUG, "Invalid service name '%s'", safe_str_client(onion_address));
            connection_mark_unattached_ap(conn, END_STREAM_REASON_TORPROTOCOL);
            return -1;
        case ENOENT:
            ...
            break;
        default:
            log_warn(LD_BUG, "Unknown cache lookup error %d", rend_cache_lookup_result);
            return -1;
        }
    }
 ...
 */
int
connection_ap_handshake_rewrite_and_attach(entry_connection_t *conn, origin_circuit_t *circ,
                                           crypt_path_t *cpath)
{
    socks_request_t *socks = conn->socks_request;
    const or_options_t *options = get_options();
    ...
    if (addresstype == EXIT_HOSTNAME) {
        ...
        routerset_t *excludeset =
            options->StrictNodes ? options->ExcludeExitNodesUnion_ : options->ExcludeExitNodes;
        ...
        if (!node) {
            log_warn(LD_APP, "Unrecognized relay in exit address '%s.exit'. Refusing.",
                     safe_str_client(socks->address));
            connection_mark_unattached_ap(conn, END_STREAM_REASON_TORPROTOCOL);
            return -1;
        }
    ...
    if (addresstype != ONION_V2_HOSTNAME && addresstype != ONION_V3_HOSTNAME) {
        ...
        if (address_is_invalid_destination(socks->address, 1)) {
            control_event_client_status(LOG_WARN, "SOCKS_BAD_HOSTNAME HOSTNAME=%s",
                                        escaped(socks->address));
            log_warn(LD_APP, "Destination '%s' seems to be an invalid hostname. Failing.",
                     safe_str_client(socks->address));
            connection_mark_unattached_ap(conn, END_STREAM_REASON_TORPROTOCOL);
            return -1;
        }
        ...
        if (socks->command == SOCKS_COMMAND_RESOLVE) {
            ...
            if (tor_addr_parse(&answer, socks->address) >= 0) { /* is it an IP? */
                ...
                connection_ap_handshake_socks_resolved_addr(conn, &answer, -1, map_expires);
                connection_mark_unattached_ap(
                    conn, END_STREAM_REASON_DONE | END_STREAM_REASON_FLAG_ALREADY_SOCKS_REPLIED);
                return 0;
            }
            tor_assert(!automap);
...
#define WARN_INTRVL_PRIV 300
            ...
            if (!conn->use_begindir && !conn->chosen_exit_name && !circ) {
                ...
                const node_t *r = router_find_exact_exit_enclave(socks->address, socks->port);
                if (r) {
                    log_info(LD_APP, "Redirecting address %s to exit at enclave router %s",
                             safe_str_client(socks->address), node_describe(r));
                    /* use the hex digest, not nickname, in case there are two
                       routers with this nickname */
                    conn->chosen_exit_name = tor_strdup(hex_str(r->identity, DIGEST_LEN));
        ...
        tor_assert(addresstype == ONION_V2_HOSTNAME || addresstype == ONION_V3_HOSTNAME);
        tor_assert(!automap);
...
#    ifdef TRANS_TPROXY
    if (get_options()->TransProxyType_parsed == TPT_TPROXY) {
        if (getsockname(ENTRY_TO_CONN(conn)->s, (struct sockaddr *)&orig_dst, &orig_dst_len) < 0) {
...
#        endif /* defined(TRANS_NETFILTER_IPV6) */
    default:
        log_warn(LD_BUG, "Received transparent data from an unsuported socket family %d",
...
destination_from_pf(entry_connection_t *conn, socks_request_t *req)
{
    ...
    if (getsockname(ENTRY_TO_CONN(conn)->s, (struct sockaddr *)&proxy_addr, &proxy_addr_len) < 0) {
        int e = tor_socket_errno(ENTRY_TO_CONN(conn)->s);
        log_warn(LD_NET,
...
#    endif /* defined(__FreeBSD__) */
    ...
    if (proxy_sa->sa_family == AF_INET) {
        ...
        pnl.af = AF_INET6;
        const struct in6_addr *dest_in6 = tor_addr_to_in6(&ENTRY_TO_CONN(conn)->addr);
        if (BUG(!dest_in6))
            return -1;
 ...
 */
static int
connection_ap_get_original_destination(entry_connection_t *conn, socks_request_t *req)
{
#ifdef TRANS_NETFILTER
...
connection_ap_handshake_process_socks(entry_connection_t *conn)
{
    ...
    sockshere =
        fetch_from_buf_socks(base_conn->inbuf, socks, options->TestSocks, options->SafeSocks);
...
connection_ap_process_http_connect(entry_connection_t *conn)
{
    ...
    const int http_status =
        fetch_from_buf_http(ENTRY_TO_CONN(conn)->inbuf, &headers, 8192, &body, &bodylen, 1024, 0);
    if (http_status < 0) {
        /* Bad http status */
...
err:
     ...
     * reply. */
    conn->socks_request->has_finished = 1;
    connection_mark_unattached_ap(conn, END_STREAM_REASON_HTTPPROTOCOL |
                                            END_STREAM_REASON_FLAG_ALREADY_SOCKS_REPLIED);
...
connection_ap_supports_optimistic_data(const entry_connection_t *conn)
{
    ...
    /* We can only send optimistic data if we're connected to an open
       general circuit. */
    if (edge_conn->on_circuit == NULL || edge_conn->on_circuit->state != CIRCUIT_STATE_OPEN ||
...
connection_ap_get_begincell_flags(entry_connection_t *ap_conn)
{
    ...
    if (ap_conn->entry_cfg.ipv6_traffic && exitnode) {
        ...
        if (compare_tor_addr_to_node_policy(&a, ap_conn->socks_request->port, exitnode) !=
...
connection_ap_handshake_send_begin, (entry_connection_t * ap_conn))
{
    ...
    tor_snprintf(
        payload, RELAY_PAYLOAD_SIZE, "%s:%d",
        (circ->base_.purpose == CIRCUIT_PURPOSE_C_GENERAL) ? ap_conn->socks_request->address : "",
    ...
    log_info(LD_APP, "Sending relay cell %d on circ %u to begin stream %d.",
             (int)ap_conn->use_begindir, (unsigned)circ->base_.n_circ_id, edge_conn->stream_id);

    begin_type = ap_conn->use_begindir ? RELAY_COMMAND_BEGIN_DIR : RELAY_COMMAND_BEGIN;
    ...
    log_info(LD_APP, "Address/port sent, ap socket " TOR_SOCKET_T_FORMAT ", n_circ_id %u",
...
connection_ap_handshake_send_resolve(entry_connection_t *ap_conn)
{
    ...
    if (command == SOCKS_COMMAND_RESOLVE) {
        ...
        if (r <= 0) {
            log_warn(LD_APP, "Rejecting ill-formed reverse lookup of %s", safe_str_client(a));
            connection_mark_unattached_ap(ap_conn, END_STREAM_REASON_INTERNAL);
            return -1;
        }
        ...
        if (r < 0) {
            log_warn(LD_BUG, "Couldn't generate reverse lookup hostname of %s", safe_str_client(a));
            connection_mark_unattached_ap(ap_conn, END_STREAM_REASON_INTERNAL);
            return -1;
        }
    ...
    log_debug(LD_APP, "Sending relay cell to begin stream %d.", edge_conn->stream_id);

    if (connection_edge_send_command(edge_conn, RELAY_COMMAND_RESOLVE, string_addr, payload_len) <
        0)
        return -1; /* circuit is closed, don't continue */
    ...
    log_info(LD_APP, "Address sent for resolve, ap socket " TOR_SOCKET_T_FORMAT ", n_circ_id %u",
             base_conn->s, (unsigned)circ->base_.n_circ_id);
 ...
 */
entry_connection_t *
connection_ap_make_link(connection_t *partner, char *address, uint16_t port, const char *digest,
                        int session_group, int isolation_flags, int use_begindir, int want_onehop)
{
    ...
    log_info(LD_APP, "Making internal %s tunnel to %s:%d ...",
             want_onehop ? "direct" : "anonymized", safe_str_client(address), port);
    ...
    strlcpy(conn->socks_request->address, address, sizeof(conn->socks_request->address));
    ...
    if (use_begindir) {
        ...
        tor_assert(digest);
        base16_encode(conn->chosen_exit_name + 1, HEX_DIGEST_LEN + 1, digest, DIGEST_LEN);
    }
...
static void
tell_controller_about_resolved_result(entry_connection_t *conn, int answer_type, size_t answer_len,
                                      const char *answer, int ttl, time_t expires)
{
    expires = time(NULL) + ttl;
    if (answer_type == RESOLVED_TYPE_IPV4 && answer_len >= 4) {
        char *cp = tor_dup_ip(ntohl(get_uint32(answer)));
        control_event_address_mapped(conn->socks_request->address, cp, expires, NULL, 0);
        tor_free(cp);
        ...
        control_event_address_mapped(conn->socks_request->address, "<error>", time(NULL) + ttl,
                                     "error=yes", 0);
    }
}
 ...
 */
void
connection_ap_handshake_socks_resolved_addr(entry_connection_t *conn, const tor_addr_t *answer,
                                            int ttl, time_t expires)
{
    if (tor_addr_family(answer) == AF_INET) {
        uint32_t a = tor_addr_to_ipv4n(answer); /* network order */
        connection_ap_handshake_socks_resolved(conn, RESOLVED_TYPE_IPV4, 4, (uint8_t *)&a, ttl,
                                               expires);
    } else if (tor_addr_family(answer) == AF_INET6) {
        const uint8_t *a = tor_addr_to_in6_addr8(answer);
        connection_ap_handshake_socks_resolved(conn, RESOLVED_TYPE_IPV6, 16, a, ttl, expires);
    } else {
        log_warn(LD_BUG, "Got called with address of unexpected family %d",
                 tor_addr_family(answer));
        connection_ap_handshake_socks_resolved(conn, RESOLVED_TYPE_ERROR, 0, NULL, -1, -1);
    }
}
...
MOCK_IMPL(void,
connection_ap_handshake_socks_resolved,
          (entry_connection_t * conn, int answer_type, size_t answer_len, const uint8_t *answer,
           int ttl, time_t expires))
{
    char buf[384];
    size_t replylen;
    ...
    if (ENTRY_TO_EDGE_CONN(conn)->is_dns_request) {
        if (conn->dns_server_request) {
             ...
             * their own. */
            tell_controller_about_resolved_result(conn, answer_type, answer_len, (char *)answer,
                                                  ttl, expires);
            conn->socks_request->has_finished = 1;
            return;
        }
 ...
 */
void
connection_ap_handshake_socks_reply(entry_connection_t *conn, char *reply, size_t replylen,
                                    int endreason)
{
    char buf[256];
    ...
    if (!SOCKS_COMMAND_IS_RESOLVE(conn->socks_request->command)) {
        control_event_stream_status(
            conn, status == SOCKS5_SUCCEEDED ? STREAM_EVENT_SUCCEEDED : STREAM_EVENT_FAILED,
            endreason);
    }

    /* Flag this stream's circuit as having completed a stream successfully
     * (for path bias) */
    if (status == SOCKS5_SUCCEEDED || endreason == END_STREAM_REASON_RESOLVEFAILED ||
        endreason == END_STREAM_REASON_CONNECTREFUSED || endreason == END_STREAM_REASON_CONNRESET ||
        endreason == END_STREAM_REASON_NOROUTE || endreason == END_STREAM_REASON_RESOURCELIMIT) {
        if (!conn->edge_.on_circuit || !CIRCUIT_IS_ORIGIN(conn->edge_.on_circuit)) {
            if (endreason != END_STREAM_REASON_RESOLVEFAILED) {
                log_info(LD_BUG,
                         "No origin circuit for successful SOCKS stream %" PRIu64 ". Reason: %d",
 ...
 **/
STATIC int
begin_cell_parse(const cell_t *cell, begin_cell_t *bcell, uint8_t *end_reason_out)
{
    relay_header_t rh;
    ...
    *end_reason_out = END_STREAM_REASON_MISC;
    ...
    if (!nul) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Relay begin cell has no \\0. Closing.");
        *end_reason_out = END_STREAM_REASON_TORPROTOCOL;
        return -1;
    }

    if (tor_addr_port_split(LOG_PROTOCOL_WARN, (char *)(body), &bcell->address, &bcell->port) < 0) {
    ...
    if (bcell->port == 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Missing port in relay begin cell. Closing.");
...
handle_hs_exit_conn(circuit_t *circ, edge_connection_t *conn)
{
    ...
    if (origin_circ->rend_data) {
        ...
        conn->hs_ident = hs_ident_edge_conn_new(&origin_circ->hs_ident->identity_pk);
        tor_assert(connection_edge_is_rendezvous_stream(conn));
    ...
    if (ret < 0) {
         ...
         * the hidden service. */
        relay_send_end_cell_from_edge(conn->stream_id, circ, END_STREAM_REASON_DONE,
...
connection_exit_begin_conn(cell_t *cell, circuit_t *circ)
{
    ...
    if (!server_mode(options) && circ->purpose != CIRCUIT_PURPOSE_S_REND_JOINED) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Relay begin cell at non-server. Closing.");
        relay_send_end_cell_from_edge(rh.stream_id, circ, END_STREAM_REASON_EXITPOLICY, NULL);
        return 0;
    }
    ...
    if (!bcell.is_begindir) {
        ...
        if (or_circ && or_circ->p_chan) {
            ...
            if ((client_chan ||
                ...
                relay_send_end_cell_from_edge(
                    rh.stream_id, circ,
                    client_chan ? END_STREAM_REASON_TORPROTOCOL : END_STREAM_REASON_MISC, NULL);
                tor_free(address);
                return 0;
            }
        }
    } else if (rh.command == RELAY_COMMAND_BEGIN_DIR) {
        if (!directory_permits_begindir_requests(options) || circ->purpose != CIRCUIT_PURPOSE_OR) {
            relay_send_end_cell_from_edge(rh.stream_id, circ, END_STREAM_REASON_NOTDIRECTORY,
                                          layer_hint);
            return 0;
        }
        ...
        log_warn(LD_BUG, "Got an unexpected command %d", (int)rh.command);
        relay_send_end_cell_from_edge(rh.stream_id, circ, END_STREAM_REASON_INTERNAL, layer_hint);
        return 0;
    }
    ...
    if (!options->IPv6Exit) {
        ...
        if (bcell.flags & BEGIN_FLAG_IPV4_NOT_OK) {
            tor_free(address);
            relay_send_end_cell_from_edge(rh.stream_id, circ, END_STREAM_REASON_EXITPOLICY,
                                          layer_hint);
            return 0;
        }
    }
    ...
    if (we_are_hibernating()) {
        relay_send_end_cell_from_edge(rh.stream_id, circ, END_STREAM_REASON_HIBERNATING, NULL);
    ...
    if (rh.command == RELAY_COMMAND_BEGIN_DIR) {
        tor_addr_t tmp_addr;
        tor_assert(or_circ);
        if (or_circ->p_chan && channel_get_addr_if_possible(or_circ->p_chan, &tmp_addr)) {
    ...
    case -1: /* resolve failed */
        relay_send_end_cell_from_edge(rh.stream_id, circ, END_STREAM_REASON_RESOLVEFAILED, NULL);
...
connection_exit_begin_resolve(cell_t *cell, or_circuit_t *circ)
{
    ...
    dummy_conn->base_.address = tor_strndup((char *)cell->payload + RELAY_HEADER_SIZE, rh.length);
 ...
 */
static int
my_exit_policy_rejects(const tor_addr_t *addr, uint16_t port, const char **why_rejected)
{
...
connection_exit_connect(edge_connection_t *edge_conn)
{
    ...
    if (!connection_edge_is_rendezvous_stream(edge_conn) &&
        ...
        log_info(LD_EXIT, "%s:%d failed exit policy%s. Closing.",
                 escaped_safe_str_client(conn->address), conn->port, why_failed_exit_policy);
...
#else
    {
        ...
        result = connection_connect(conn, conn->address, addr, port, &socket_error);
#ifdef HAVE_SYS_UN_H
    } else {
        /*
    ...
    if (connection_edge_is_rendezvous_stream(edge_conn)) {
        ...
        connection_edge_send_command(edge_conn, RELAY_COMMAND_CONNECTED, (char *)connected_payload,
...
connection_exit_connect_dir(edge_connection_t *exitconn)
{
    ...
    if (connection_edge_send_command(exitconn, RELAY_COMMAND_CONNECTED, NULL, 0) < 0) {
        connection_mark_for_close(TO_CONN(exitconn));
 ...
 */
int
connection_ap_can_use_exit(const entry_connection_t *conn, const node_t *exit_node)
{
    const or_options_t *options = get_options();
    ...
    if (conn->chosen_exit_name) {
        const node_t *chosen_exit = node_get_by_nickname(conn->chosen_exit_name, 0);
        if (!chosen_exit || tor_memneq(chosen_exit->identity, exit_node->identity, DIGEST_LEN)) {
            /* doesn't match */
    ...
    if (conn->socks_request->command == SOCKS_COMMAND_CONNECT) {
        ...
        r = compare_tor_addr_to_node_policy(addrp, conn->socks_request->port, exit_node);
                                        ...
                                        const origin_circuit_t *circ)
{
    ...
    if ((iso & ISO_STREAM) &&
        (circ->associated_isolated_stream_global_id != ENTRY_TO_CONN(conn)->global_identifier))
        return 0;

    if ((iso & ISO_DESTPORT) && conn->socks_request->port != circ->dest_port)
        return 0;
    if ((iso & ISO_DESTADDR) && strcasecmp(conn->original_dest_address, circ->dest_address))
        return 0;
    if ((iso & ISO_SOCKSAUTH) &&
        (!memeq_opt(sr->username, sr->usernamelen, circ->socks_username,
                    circ->socks_username_len) ||
         !memeq_opt(sr->password, sr->passwordlen, circ->socks_password, circ->socks_password_len)))
        return 0;
    if ((iso & ISO_CLIENTPROTO) &&
        (conn->socks_request->listener_type != circ->client_proto_type ||
         conn->socks_request->socks_version != circ->client_proto_socksver))
        return 0;
    if ((iso & ISO_CLIENTADDR) && !tor_addr_eq(&ENTRY_TO_CONN(conn)->addr, &circ->client_addr))
        return 0;
 ...
 */
int
connection_edge_update_circuit_isolation(const entry_connection_t *conn, origin_circuit_t *circ,
                                         int dry_run)
{
    const socks_request_t *sr = conn->socks_request;
    if (!conn->original_dest_address) {
    ...
    if (!circ->isolation_values_set) {
        ...
        circ->socks_username = sr->username ? tor_memdup(sr->username, sr->usernamelen) : NULL;
        circ->socks_password = sr->password ? tor_memdup(sr->password, sr->passwordlen) : NULL;
