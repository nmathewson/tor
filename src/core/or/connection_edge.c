...
connection_edge_process_inbuf(edge_connection_t *conn, int package_partial)
{
    ...
    switch (conn->base_.state) {
        case AP_CONN_STATE_SOCKS_WAIT:
            if (connection_ap_handshake_process_socks(
                    EDGE_TO_ENTRY_CONN(conn)) < 0) {
                /* already marked */
                return -1;
            }
            return 0;
        ...
        case AP_CONN_STATE_HTTP_CONNECT_WAIT:
            if (connection_ap_process_http_connect(EDGE_TO_ENTRY_CONN(conn)) <
                0) {
                return -1;
            }
            return 0;
        ...
        case AP_CONN_STATE_CONNECT_WAIT:
            if (connection_ap_supports_optimistic_data(
...
connection_ap_handshake_rewrite(entry_connection_t *conn, rewrite_result_t *out)
{
    ...
    if (socks->command == SOCKS_COMMAND_RESOLVE &&
        ...
        if (out->automap) {
             ...
             * address we're supposed to give as a resolve answer.  (Return a
             * cached value if we've looked up this address before.
             */
    ...
    if (socks->command == SOCKS_COMMAND_RESOLVE_PTR) {
        ...
        if (addressmap_rewrite_reverse(socks->address, sizeof(socks->address),
            ...
            tor_snprintf(socks->address, sizeof(socks->address), "REVERSE[%s]",
                         out->orig_address);
            connection_ap_handshake_socks_resolved(
                conn, RESOLVED_TYPE_HOSTNAME, strlen(result), (uint8_t *)result,
                -1, out->map_expires);
            tor_free(result);
                           ...
                           origin_circuit_t *circ, hostname_type_t addresstype)
{
    ...
    if (refetch_desc || !descriptor_is_usable) {
        ...
        if (addresstype == ONION_V2_HOSTNAME) {
            ...
            /* Attempt to fetch the hsv3 descriptor. Check the retval to see how
             * it went and act accordingly. */
            int ret =
            ...
            switch (ret) {
                case HS_CLIENT_FETCH_MISSING_INFO:
                     ...
                     * because once we get enough dirinfo or a new live
                     * consensus, the HS client subsystem is notified and every
                     * connection in that state will trigger a fetch for the
                     * service key. */
                                           ...
                                           crypt_path_t *cpath)
{
    ...
    if (addresstype != ONION_V2_HOSTNAME && addresstype != ONION_V3_HOSTNAME) {
         ...
         * family, then we have to resolve the address first. Then we'll know if
         * it resolves to a usable address family. */
        ...
        if (socks->command == SOCKS_COMMAND_RESOLVE) {
            ...
            if (options->ClientRejectInternalAddresses && !conn->use_begindir &&
                ...
                if (tor_addr_hostname_is_local(socks->address) ||
                     ...
                     * exit node, then we really don't want to try to connect to
                     * it.  That's probably an error. */
             ...
             * fields in conn->entry_cfg to tell the exit what AF we want. */
            {
                ...
                if (tor_addr_parse(&addr, socks->address) >= 0) {
                    ...
                    if ((family == AF_INET && !conn->entry_cfg.ipv4_traffic) ||
                        ...
                        /* Tell the exit: we won't accept any ipv4 connection to
                         * an IPv6 address. */
                        conn->entry_cfg.ipv4_traffic = 0;
                    } else if (family == AF_INET) {
                        /* Tell the exit: we won't accept any ipv6 connection to
                         * an IPv4 address. */
...
destination_from_socket(entry_connection_t *conn, socks_request_t *req)
{
    ...
    switch (ENTRY_TO_CONN(conn)->socket_family) {
        ...
        case AF_INET6:
            rv = getsockopt(ENTRY_TO_CONN(conn)->s, SOL_IPV6,
                            IP6T_SO_ORIGINAL_DST, (struct sockaddr *)&orig_dst,
                            &orig_dst_len);
            break;
          ...
          (entry_connection_t * ap_conn))
{
    ...
    if (begin_type == RELAY_COMMAND_BEGIN) {
         ...
         * purpose. If a BEGINDIR connection is ever not linked, that's a bug.
         */
 ...
 * connection_ap_handshake_socks_resolved(). */
static void
tell_controller_about_resolved_result(entry_connection_t *conn, int answer_type,
                                      size_t answer_len, const char *answer,
...
connection_exit_begin_conn(cell_t *cell, circuit_t *circ)
{
    ...
    switch (dns_resolve(n_stream)) {
        ...
        case -1: /* resolve failed */
            relay_send_end_cell_from_edge(
                rh.stream_id, circ, END_STREAM_REASON_RESOLVEFAILED, NULL);
