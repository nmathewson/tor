...
#define MAX_CIRCID_ATTEMPTS 64
    ...
    do {
        if (++attempts > MAX_CIRCID_ATTEMPTS) {
            /* Make sure we don't loop forever because all circuit IDs are
             * used.
             *
             * Once, we would try until we had tried every possible circuit ID.
             * But that's quite expensive.  Instead, we try MAX_CIRCID_ATTEMPTS
             * random circuit IDs, and then give up.
             *
            ...
            /* analysis so far on 12184 suggests that we're running out of
               circuit IDs because it looks like we have too many pending
               destroy cells. Let's see how many we really have pending.
            */
            queued_destroys =
             ...
             * possible failure modes here.  You'll need to know how to use gdb
             * with Tor: this will make Tor exit with an assertion failure if
             * the cmux is corrupt. */
            if (0)
...
circuit_list_path_impl(origin_circuit_t *circ, int verbose, int verbose_names)
{
    ...
    do {
        ...
        if (verbose) {
            ...
            smartlist_add_asprintf(elements, "%s(%s)", elt,
                                   states[hop->state]);
            tor_free(elt);
        } else {
...
onion_populate_cpath(origin_circuit_t *circ)
{
    ...
    if (circuit_get_cpath_len(circ) == 1) {
        /* Allow for bootstrapping: when we're fetching directly from a
         * fallback, authority, or bridge, we have no way of knowing its ntor
         * onion key before we connect to it. So instead, we try connecting,
         * and end up using CREATE_FAST. */
        tor_assert(circ->cpath);
        tor_assert(circ->cpath->extend_info);
        const node_t *node =
            node_get_by_id(circ->cpath->extend_info->identity_digest);
        /* If we don't know the node and its descriptor, we must be
         * bootstrapping.
         */
...
circuit_n_chan_done(channel_t *chan, int status, int close_origin_circuits)
{
    ...
    SMARTLIST_FOREACH_BEGIN (pending_circs, circuit_t *, circ) {
        ...
        if (CIRCUIT_IS_ORIGIN(circ)) {
            if ((err_reason = circuit_send_next_onion_skin(
                     TO_ORIGIN_CIRCUIT(circ))) < 0) {
                log_info(LD_CIRC, "send_next_onion_skin failed; circuit "
                                  "marked for closing.");
                circuit_mark_for_close(circ, -err_reason);
                continue;
                /* XXX could this be bad, eg if next_onion_skin failed because
                 * conn died? */
            }
        } else {
            /* pull the create cell out of circ->n_chan_create_cell, and send
             * it */
                              ...
                              const extend_info_t *ei)
{
    ...
    /* torspec says: Clients SHOULD use the EXTEND format whenever sending a
     * TAP handshake... In other cases, clients SHOULD use EXTEND2. */
...
circuit_extend(cell_t *cell, circuit_t *circ)
{
    ...
    n_chan =
        channel_get_for_extend((const char *)ec.node_id, &ec.ed_pubkey,
    ...
    if (!n_chan) {
        ...
        circ->n_hop =
            extend_info_new(NULL /*nickname*/, (const char *)ec.node_id,
        ...
        if (should_launch) {
            ...
            n_chan = channel_connect_for_circuit(
                &ec.orport_ipv4.addr, ec.orport_ipv4.port,
                (const char *)ec.node_id, &ec.ed_pubkey);
            if (!n_chan) {
...
circuit_finish_handshake(origin_circuit_t *circ, const created_cell_t *reply)
{
...

    {
        ...
        if (onion_skin_client_handshake(
            ...
            if (msg)
                log_warn(LD_CIRC, "onion_skin_client_handshake failed: %s",
                         msg);
            return -END_CIRC_REASON_TORPROTOCOL;
        }
    }
                 ...
                 const uint8_t *rend_circ_nonce)
{
    ...
    if (created_cell_format(&cell, created_cell) < 0) {
        log_warn(LD_BUG, "couldn't format created cell (type=%d, len=%d)",
                 (int)created_cell->cell_type,
                 (int)created_cell->handshake_len);
        return -1;
    }
...
choose_good_exit_server_general(router_crn_flags_t flags)
{
    ...
    SMARTLIST_FOREACH_BEGIN (the_nodes, const node_t *, node) {
        ...
        if (node_is_unreliable(node, need_uptime, need_capacity, 0)) {
            ...
            continue; /* skip routers that are not suitable.  Don't worry if
                       ...
                       * we'll retry later in this function with need_update
                       * and need_capacity set to 0. */
        }
        if (!(node->is_valid)) {
        ...
        /* We do not allow relays that allow single hop exits by default.
         * Option
        ...
        SMARTLIST_FOREACH_BEGIN (connections, connection_t *, conn) {
            ...
            if (connection_ap_can_use_exit(TO_ENTRY_CONN(conn), node)) {
                ++n_supported[i];
                //        log_fn(LOG_DEBUG,"%s is supported. n_supported[%d]
                //        now %d.",
                //               router->nickname, i, n_supported[i]);
            } else {
                //        log_fn(LOG_DEBUG,"%s (index %d) would reject this
                //        stream.",
                //               router->nickname, i);
            }
    ...
    if (best_support > 0) {
        ...
        selected_node =
            node_sl_choose_by_bandwidth(supporting, WEIGHT_FOR_EXIT);
        smartlist_free(supporting);
    } else {
        /* Either there are no pending connections, or no routers even seem to
         * possibly support any of them.  Choose a router at random that
         * satisfies at least one predicted exit port. */
    ...
    tor_free(n_supported);
    if (selected_node) {
        log_info(LD_CIRC, "Chose exit server '%s'",
                        ...
                        int is_internal)
{
    ...
    case CIRCUIT_PURPOSE_C_GENERAL:
        if (is_internal) /* pick it like a middle hop */
            return router_choose_random_node(NULL, options->ExcludeNodes,
                                             flags);
        else
            return choose_good_exit_server_general(flags);
                      ...
                      int is_hs_v3_rp_circuit)
{
    ...
    if (state->onehop_tunnel) {
        ...
        int r =
            new_route_len(circ->base_.purpose, exit_ei, nodelist_get_list());
          ...
          (const smartlist_t *nodes, int direct))
{
    ...
    SMARTLIST_FOREACH_BEGIN (nodes, const node_t *, node) {
        //    log_debug(LD_CIRC,
        //              "Contemplating whether router %d (%s) is a new
        //              option.", i, r->nickname);
        if (!node->is_running)
        ...
        if (!node_has_preferred_descriptor(node, direct))
            continue;
        /* The node has a descriptor, so we can just check the ntor key
         * directly */
    ...
    //    log_debug(LD_CIRC,"I like %d. num_acceptable_routers now %d.",i,
    //    num);
                          ...
                          crypt_path_t *head, int cur_len)
{
    ...
    if (circuit_should_use_vanguards(purpose)) {
        return build_vanguard_middle_exclude_list(purpose, state, head,
                                                  cur_len);
    }

    excluded = smartlist_new();

    /* For non-vanguard circuits, add the exit and its family to the exclude
     * list (note that the exit/last hop is always chosen first in
                          ...
                          crypt_path_t *head, int cur_len)
{
    ...
    if (options->MiddleNodes) {
        ...
        choice =
            router_choose_random_node(excluded, options->ExcludeNodes, flags);
    }
    smartlist_free(excluded);
    return choice;
}
                         ...
                         circuit_guard_state_t **guard_state_out)
{
    ...
    if (state && (node = build_state_get_exit_node(state))) {
        /* Exclude the exit node from the state, if we have one.  Also exclude
         * its family. */
