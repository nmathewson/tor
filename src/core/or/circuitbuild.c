...
onion_populate_cpath(origin_circuit_t *circ)
{
    ...
    if (circuit_get_cpath_len(circ) == 1) {
         ...
         * onion key before we connect to it. So instead, we try connecting, and
         * end up using CREATE_FAST. */
...
circuit_n_chan_done(channel_t *chan, int status, int close_origin_circuits)
{
    ...
    SMARTLIST_FOREACH_BEGIN(pending_circs, circuit_t *, circ)
    {
        ...
        if (CIRCUIT_IS_ORIGIN(circ)) {
            ...
            if (circuit_deliver_create_cell(circ, circ->n_chan_create_cell, 1) <
                0) {
    ...
    SMARTLIST_FOREACH_BEGIN(the_nodes, const node_t *, node)
    {
        ...
        SMARTLIST_FOREACH_BEGIN(connections, connection_t *, conn)
        {
            ...
            if (connection_ap_can_use_exit(TO_ENTRY_CONN(conn), node)) {
                ++n_supported[i];
                //        log_fn(LOG_DEBUG,"%s is supported. n_supported[%d] now
                //        %d.",
...
static const node_t *
pick_vanguard_middle_node(const or_options_t *options, router_crn_flags_t flags,
                          int cur_len, const smartlist_t *excluded)
{
...
extend_info_new(const char *nickname, const char *rsa_id_digest,
                ...
                const curve25519_public_key_t *ntor_key, const tor_addr_t *addr,
                uint16_t port)
{
