 ...
 */
channel_t *
channel_tls_connect(const tor_addr_t *addr, uint16_t port, const char *id_digest,
                    const ed25519_public_key_t *ed_id)
{
    ...
    if (is_local_addr(addr)) {
        log_debug(LD_CHANNEL, "Marking new outgoing channel %" PRIu64 " at %p as local",
                  (chan->global_identifier), chan);
        channel_mark_local(chan);
    } else {
        log_debug(LD_CHANNEL, "Marking new outgoing channel %" PRIu64 " at %p as remote",
                  (chan->global_identifier), chan);
    ...
    log_debug(LD_CHANNEL, "Got orconn %p for channel with global id %" PRIu64, tlschan->conn,
...
channel_tls_start_listener(void)
{
    ...
    if (!channel_tls_listener) {
        ...
        listener->describe_transport = channel_tls_listener_describe_transport_method;

        channel_tls_listener = listener;

        log_debug(LD_CHANNEL, "Starting TLS channel listener %p with global id %" PRIu64, listener,
...
channel_tls_free_all(void)
{
    ...
    if (channel_tls_listener) {
        ...
        log_debug(LD_CHANNEL, "Closing channel_tls_listener with ID %" PRIu64 " at %p.",
...
channel_tls_handle_incoming(or_connection_t *orconn)
{
    ...
    if (is_local_addr(&(TO_CONN(orconn)->addr))) {
        log_debug(LD_CHANNEL, "Marking new incoming channel %" PRIu64 " at %p as local",
                  (chan->global_identifier), chan);
        channel_mark_local(chan);
    } else {
        log_debug(LD_CHANNEL, "Marking new incoming channel %" PRIu64 " at %p as remote",
                  (chan->global_identifier), chan);
        channel_mark_remote(chan);
    }
...
channel_tls_close_method(channel_t *chan)
{
    ...
    if (tlschan->conn)
        ...
        log_info(LD_CHANNEL, "Tried to close channel_tls_t %p with NULL conn", tlschan);
        channel_change_state(chan, CHANNEL_STATE_ERROR);
    }
}
...
channel_tls_get_overhead_estimate_method(channel_t *chan)
{
    ...
    log_debug(LD_CHANNEL, "Estimated overhead ratio for TLS chan %" PRIu64 " is %f",
...
channel_tls_get_remote_descr_method(channel_t *chan, int flags)
{
    ...
    if (tlschan->conn) {
        ...
        case 0:
            ...
            tor_snprintf(buf, TOR_ADDRPORT_BUF_LEN, "%s:%u", conn->address, conn->port);
            answer = buf;
            break;
...
channel_tls_has_queued_writes_method(channel_t *chan)
{
    ...
    outbuf_len = (tlschan->conn != NULL) ? connection_get_outbuf_len(TO_CONN(tlschan->conn)) : 0;
 ...
 */
static int
channel_tls_matches_extend_info_method(channel_t *chan, extend_info_t *extend_info)
{
    channel_tls_t *tlschan = BASE_CHAN_TO_TLS(chan);
 ...
 */
static int
channel_tls_write_packed_cell_method(channel_t *chan, packed_cell_t *packed_cell)
{
    tor_assert(chan);
    channel_tls_t *tlschan = BASE_CHAN_TO_TLS(chan);
    ...
    if (tlschan->conn) {
        connection_buf_add(packed_cell->body, cell_network_size, TO_CONN(tlschan->conn));
    } else {
        log_info(LD_CHANNEL,
 ...
 */
void
channel_tls_handle_state_change_on_orconn(channel_tls_t *chan, or_connection_t *conn, uint8_t state)
{
    channel_t *base_chan;
...
#    define PROCESS_CELL(tp, cl, cn)                                                               \
        STMT_BEGIN                                                                                 \
            {                                                                                      \
                ++num##tp;                                                                         \
                channel_tls_time_process_cell(cl, cn, &tp##time, channel_tls_process_##tp##_cell); \
            }                                                                                      \
        STMT_END
...
channel_tls_handle_cell(cell_t *cell, or_connection_t *conn)
{
    ...
    if (handshaking && cell->command != CELL_VERSIONS && cell->command != CELL_NETINFO) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
               "Received unexpected cell command %d in chan state %s / "
               "conn state %s; closing the connection.",
               (int)cell->command, channel_state_to_string(TLS_CHAN_TO_BASE(chan)->state),
...
channel_tls_handle_var_cell(var_cell_t *var_cell, or_connection_t *conn)
{
    ...
    if (now > current_second) { /* the second has rolled over */
        /* print stats */
        log_info(LD_OR, "At end of second: %d versions (%d ms), %d certs (%d ms)", num_versions,
                 versions_time / ((now - current_second) * 1000), num_certs,
                 certs_time / ((now - current_second) * 1000));
    ...
    if (!chan) {
        log_warn(LD_CHANNEL, "Got a var_cell_t on an OR connection with no channel");
        return;
    }
    ...
    case OR_CONN_STATE_OR_HANDSHAKING_V2:
        if (var_cell->command != CELL_VERSIONS) {
            log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
                   ...
                   conn_state_to_string(CONN_TYPE_OR, TO_CONN(conn)->state), TO_CONN(conn)->state,
    ...
    case OR_CONN_STATE_TLS_SERVER_RENEGOTIATING:
        if (!(command_allowed_before_handshake(var_cell->command))) {
            log_fn(
                ...
                "orconn state \"%s\" [%d], channel state \"%s\" [%d]; "
                "closing the connection.",
                (int)(var_cell->command), conn_state_to_string(CONN_TYPE_OR, TO_CONN(conn)->state),
                (int)(TO_CONN(conn)->state), channel_state_to_string(TLS_CHAN_TO_BASE(chan)->state),
                (int)(TLS_CHAN_TO_BASE(chan)->state));
    ...
    case OR_CONN_STATE_OR_HANDSHAKING_V3:
        if (var_cell->command != CELL_AUTHENTICATE)
            or_handshake_state_record_var_cell(conn, conn->handshake_state, var_cell, 1);
    ...
    case CELL_AUTHORIZE:
        ...
        break;
    default:
        log_fn(LOG_INFO, LD_PROTOCOL, "Variable-length cell of unknown type (%d) received.",
               (int)(var_cell->command));
        break;
    }
}
...
enter_v3_handshake_with_cell(var_cell_t *cell, channel_tls_t *chan)
{
    ...
    tor_assert(TO_CONN(chan->conn)->state == OR_CONN_STATE_TLS_HANDSHAKING ||
               TO_CONN(chan->conn)->state == OR_CONN_STATE_TLS_SERVER_RENEGOTIATING);
    ...
    or_handshake_state_record_var_cell(chan->conn, chan->conn->handshake_state, cell, 1);
    return 0;
}
...
channel_tls_process_versions_cell(var_cell_t *cell, channel_tls_t *chan)
{
    ...
    if (chan->conn->link_proto != 0 ||
        (chan->conn->handshake_state && chan->conn->handshake_state->received_versions)) {
        log_fn(LOG_PROTOCOL_WARN, LD_OR,
    ...
    case OR_CONN_STATE_TLS_SERVER_RENEGOTIATING:
    default:
        log_fn(LOG_PROTOCOL_WARN, LD_OR, "VERSIONS cell while in unexpected state");
        return;
    }
    ...
    if (chan->conn->link_proto == 2) {
        ...
        const int send_any = send_versions || send_certs || send_chall || send_netinfo;
        tor_assert(chan->conn->link_proto >= 3);

        log_info(LD_OR, "Negotiated version %d with %s:%d; %s%s%s%s%s", highest_supported_version,
...
channel_tls_process_padding_negotiate_cell(cell_t *cell, channel_tls_t *chan)
{
    ...
    if (channelpadding_negotiate_parse(&negotiation, cell->payload, CELL_PAYLOAD_SIZE) < 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_OR,
    ...
    channelpadding_update_padding_for_channel(TLS_CHAN_TO_BASE(chan), negotiation);
 ...
 */
static int
tor_addr_from_netinfo_addr(tor_addr_t *tor_addr, const netinfo_addr_t *netinfo_addr)
{
    tor_assert(tor_addr);
    ...
    if (type == NETINFO_ADDR_TYPE_IPV4 && len == 4) {
        ...
        const uint8_t *ipv6_bytes = netinfo_addr_getconstarray_addr_ipv6(netinfo_addr);
...
channel_tls_process_netinfo_cell(cell_t *cell, channel_tls_t *chan)
{
    ...
    if (chan->conn->link_proto < 2) {
        log_fn(LOG_PROTOCOL_WARN, LD_OR, "Received a NETINFO cell on %s connection; dropping.",
    ...
    tor_assert(chan->conn->handshake_state && chan->conn->handshake_state->received_versions);
    started_here = connection_or_nonopen_was_started_here(chan->conn);
    ...
    if (chan->conn->base_.state == OR_CONN_STATE_OR_HANDSHAKING_V3) {
        ...
        if (started_here) {
            ...
            if (!(chan->conn->handshake_state->authenticated)) {
                ...
                tor_assert(
                    fast_mem_is_zero((const char *)(chan->conn->handshake_state
    ...
    ssize_t parsed = netinfo_cell_parse(&netinfo_cell, cell->payload, CELL_PAYLOAD_SIZE);

    if (parsed < 0) {
        log_fn(LOG_PROTOCOL_WARN, LD_OR, "Failed to parse NETINFO cell - closing connection.");
        connection_or_close_for_error(chan->conn, 0);
        return;
    }
    ...
    const netinfo_addr_t *my_addr = netinfo_cell_getconst_other_addr(netinfo_cell);
    ...
    if (my_addr_type == NETINFO_ADDR_TYPE_IPV4 && my_addr_len == 4) {
        if (!get_options()->BridgeRelay && me && tor_addr_eq_ipv4h(&my_apparent_addr, me->addr)) {
            TLS_CHAN_TO_BASE(chan)->is_canonical_to_peer = 1;
        }
    } else if (my_addr_type == NETINFO_ADDR_TYPE_IPV6 && my_addr_len == 16) {
        if (!get_options()->BridgeRelay && me && !tor_addr_is_null(&me->ipv6_addr) &&
    ...
    for (uint8_t i = 0; i < n_other_addrs; i++) {
        ...
        const netinfo_addr_t *netinfo_addr = netinfo_cell_getconst_my_addrs(netinfo_cell, i);

        tor_addr_t addr;

        if (tor_addr_from_netinfo_addr(&addr, netinfo_addr) == -1) {
            log_fn(LOG_PROTOCOL_WARN, LD_OR, "Bad address in netinfo cell; Skipping.");
            continue;
        }
    ...
    if (me && !TLS_CHAN_TO_BASE(chan)->is_canonical_to_peer &&
        ...
        const char *descr = TLS_CHAN_TO_BASE(chan)->get_remote_descr(TLS_CHAN_TO_BASE(chan), 0);
        log_info(LD_OR,
...
#define NETINFO_NOTICE_SKEW 3600
    if (time_abs(apparent_skew) > NETINFO_NOTICE_SKEW &&
        (started_here || connection_or_digest_is_known_relay(chan->conn->identity_digest))) {
        int trusted = router_digest_is_trusted_dir(chan->conn->identity_digest);
        clock_skew_warning(TO_CONN(chan->conn), apparent_skew, trusted, LD_GENERAL, "NETINFO cell",
                           "OR");
    }
    ...
    if (connection_or_set_state_open(chan->conn) < 0) {
        ...
        log_info(LD_OR,
                 ...
                 safe_str_client(chan->conn->base_.address), chan->conn->base_.port,
                 (int)(chan->conn->link_proto), hex_str(identity_digest, DIGEST_LEN),
...
#define ERR(s)                                                                             \
    do {                                                                                   \
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Received a bad CERTS cell from %s:%d: %s", \
    ...
    chan->conn->handshake_state->certs->ed_rsa_crosscert_len = rsa_ed_cc_cert_len;
    rsa_ed_cc_cert = NULL;
     ...
     * to one. */
    if (started_here && router_digest_is_trusted_dir(TLS_CHAN_TO_BASE(chan)->identity_digest))
        severity = LOG_WARN;
    else
    ...
    or_handshake_certs_check_both(severity, chan->conn->handshake_state->certs, chan->conn->tls,
                                  time(NULL), &checked_ed_id, &checked_rsa_id);
    ...
    if (started_here) {
        ...
        if (checked_ed_id) {
            ...
            memcpy(&chan->conn->handshake_state->authenticated_ed25519_peer_id, checked_ed_id,
        ...
        if (connection_or_client_learned_peer_id(
                chan->conn, chan->conn->handshake_state->authenticated_rsa_peer_id, checked_ed_id) <
            0)
        ...
        log_info(LD_OR,
                 ...
                 checked_ed_id ? " and Ed25519" : "", safe_str(chan->conn->base_.address),
                 chan->conn->base_.port);
...
#define ERR(s)                                                                    \
    ...
    if (use_type >= 0) {
        log_info(LD_OR,
                 ...
                 safe_str(chan->conn->base_.address), chan->conn->base_.port, use_type);
...
#define ERR(s)                                                                                    \
    do {                                                                                          \
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Received a bad AUTHENTICATE cell from %s:%d: %s", \
               safe_str(chan->conn->base_.address), chan->conn->base_.port, (s));                 \
        connection_or_close_for_error(chan->conn, 0);                                             \
    ...
    expected_cell =
        connection_or_compute_authenticate_cell_body(chan->conn, authtype, NULL, NULL, 1);
    if (!expected_cell)
        ERR("Couldn't compute expected AUTHENTICATE cell body");

    int sig_is_rsa;
    if (authtype == AUTHTYPE_RSA_SHA256_TLSSECRET || authtype == AUTHTYPE_RSA_SHA256_RFC5705) {
    ...
    if (sig_is_rsa) {
        ...
        crypto_pk_t *pk = tor_tls_cert_get_key(chan->conn->handshake_state->certs->auth_cert);
        ...
        signed_len = crypto_pk_public_checksig(
            pk, signed_data, keysize, (char *)auth + V3_AUTH_BODY_LEN, authlen - V3_AUTH_BODY_LEN);
        crypto_pk_free(pk);
        if (signed_len < 0) {
    ...
    chan->conn->handshake_state->digest_received_data = 0;
    {
        ...
        if (!sig_is_rsa) {
            chan->conn->handshake_state->authenticated_ed25519 = 1;
            ed_identity_received = &chan->conn->handshake_state->certs->ed_id_sign->signing_key;
        ...
        memcpy(chan->conn->handshake_state->authenticated_rsa_peer_id, id_digests->d[DIGEST_SHA1],
               DIGEST_LEN);
        ...
        log_debug(LD_HANDSHAKE,
                  ...
                  safe_str(chan->conn->base_.address), __func__, ed_identity_received ? "" : "out");
        ...
        log_debug(LD_HANDSHAKE, "Got an AUTHENTICATE cell from %s:%d, type %d: Looks good.",
                  safe_str(chan->conn->base_.address), chan->conn->base_.port, authtype);
    }
