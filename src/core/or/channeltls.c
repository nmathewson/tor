...
channel_tls_is_canonical_method(channel_t *chan, int req)
{
    ...
    if (tlschan->conn) {
        switch (req) {
            ...
            case 1:
                /*
                 * Is the is_canonical bit reliable?  In protocols version 2 and
                 * up we get the canonical address from a NETINFO cell, but in
                 * older versions it might be based on an obsolete descriptor.
                 */
...
channel_tls_handle_cell(cell_t *cell, or_connection_t *conn)
{
    ...
    switch (cell->command) {
        ...
        case CELL_CREATED2:
            /*
             * These are all transport independent and we pass them up through
             * the channel_t mechanism.  They are ultimately handled in
             * command.c.
             */
...
channel_tls_handle_var_cell(var_cell_t *var_cell, or_connection_t *conn)
{
    ...
    switch (TO_CONN(conn)->state) {
        ...
        case OR_CONN_STATE_TLS_HANDSHAKING:
             ...
             * finished!" And we need to be accepting both at once to handle
             * both the v2 and v3 handshakes. */
        ...
        case OR_CONN_STATE_OPEN:
            if (conn->link_proto < 3) {
                log_fn(
                    ...
                    "state %s [%d], channel state %s [%d] with link protocol "
                    "%d; "
                    "ignoring it.",
                    (int)(var_cell->command),
                    conn_state_to_string(CONN_TYPE_OR, TO_CONN(conn)->state),
    ...
    if (chan->conn->base_.state == OR_CONN_STATE_OR_HANDSHAKING_V3) {
        ...
        if (started_here) {
            ...
            if (!(chan->conn->handshake_state->authenticated)) {
                 ...
                 * bridge relay, and we must not use it for EXTEND requests (nor
                 * could we, as there are no authenticated peer IDs) */
...
#define NETINFO_NOTICE_SKEW 3600
    ...
    if (!chan->conn->handshake_state->sent_netinfo) {
         ...
         * AUTH_CHALLENGE cell, then we would not previously have sent a NETINFO
         * cell. Do so now. */
        if (connection_or_send_netinfo(chan->conn) < 0) {
            connection_or_close_for_error(chan->conn, 0);
            return;
...
#define ERR(s)                                                                 \
    ...
    for (i = 0; i < n_certs; ++i) {
        ...
        switch (ct) {
            ...
            case CERT_ENCODING_X509: {
                ...
                if (!x509_cert) {
                    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
                           "Received undecodable certificate in CERTS cell "
                           "from %s:%d",
    ...
    if (started_here) {
        ...
        if (!public_server_mode(get_options())) {
             ...
             * aren't planning to authenticate at all.  At this point we know
             * who we are talking to, so we can just send a netinfo now. */
            send_netinfo = 1;
        }
    } else {
