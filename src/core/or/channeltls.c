...
channel_tls_get_remote_descr_method(channel_t *chan, int flags)
{
    ...
    if (tlschan->conn) {
        ...
        case 0:
            ...
            tor_snprintf(buf, TOR_ADDRPORT_BUF_LEN, "%s:%u", conn->address,
                         conn->port);
            answer = buf;
            break;
        case GRD_FLAG_ORIGINAL:
            ...
            tor_snprintf(buf, TOR_ADDRPORT_BUF_LEN, "%s:%u", addr_str,
                         conn->port);
            tor_free(addr_str);
            answer = buf;
            break;
                                       ...
                                       extend_info_t *extend_info)
{
    ...
    return (
        tor_addr_eq(&(extend_info->addr), &(TO_CONN(tlschan->conn)->addr)) &&
        ...
        STMT_BEGIN                                                       \
            {                                                            \
                ...
                channel_tls_time_process_cell(                           \
                    cl, cn, &tp##time, channel_tls_process_##tp##_cell); \
            }                                                            \
        STMT_END
...
channel_tls_process_versions_cell(var_cell_t *cell, channel_tls_t *chan)
{
...

    {
        ...
        for (i = 0; i < cell->payload_len / 2; ++i, cp += 2) {
            ...
            if (is_or_protocol_version_known(v) &&
                v > highest_supported_version)
                highest_supported_version = v;
        }
    }
    if (!highest_supported_version) {
        log_fn(LOG_PROTOCOL_WARN, LD_OR,
               "Couldn't find a version in common between my version list and "
               "the "
               "list in the VERSIONS cell; closing connection.");
        connection_or_close_for_error(chan->conn, 0);
        return;
    } else if (highest_supported_version == 1) {
        /* Negotiating version 1 makes no sense, since version 1 has no
         * VERSIONS cells. */
        log_fn(
            LOG_PROTOCOL_WARN, LD_OR,
    ...
    rep_hist_note_negotiated_link_proto(highest_supported_version,
                                        started_here);
...
channel_tls_process_netinfo_cell(cell_t *cell, channel_tls_t *chan)
{
    ...
    if (chan->conn->base_.state != OR_CONN_STATE_OR_HANDSHAKING_V2 &&
        chan->conn->base_.state != OR_CONN_STATE_OR_HANDSHAKING_V3) {
        log_fn(LOG_PROTOCOL_WARN, LD_OR,
               "Received a NETINFO cell on non-handshaking connection; "
               "dropping.");
        return;
    }
    tor_assert(chan->conn->handshake_state &&
        ...
        if (started_here) {
            ...
            if (!(chan->conn->handshake_state->authenticated)) {
                ...
                tor_assert(fast_mem_is_zero(
                    ...
                    32));
                /* If the client never authenticated, it's a tor client or
                 * bridge relay, and we must not use it for EXTEND requests
                 * (nor could we, as there are no authenticated peer IDs) */
                ...
                connection_or_init_conn_from_address(
                    chan->conn, &(chan->conn->base_.addr),
                    chan->conn->base_.port,
                    /* zero, checked above */
                    (const char *)(chan->conn->handshake_state
                                       ->authenticated_rsa_peer_id),
     ...
     * 255, and CELL_PAYLOAD_LEN - 6 is 503.  So we know that cp is < end. */
    ...
    if (me && !TLS_CHAN_TO_BASE(chan)->is_canonical_to_peer &&
        ...
        const char *descr = TLS_CHAN_TO_BASE(chan)->get_remote_descr(
            TLS_CHAN_TO_BASE(chan), 0);
        log_info(LD_OR,
...
#define NETINFO_NOTICE_SKEW 3600
    if (time_abs(apparent_skew) > NETINFO_NOTICE_SKEW &&
        ...
        int trusted =
            router_digest_is_trusted_dir(chan->conn->identity_digest);
        clock_skew_warning(TO_CONN(chan->conn), apparent_skew, trusted,
                           LD_GENERAL, "NETINFO cell", "OR");
    }
    ...
    if (!chan->conn->handshake_state->sent_netinfo) {
        /* If we were prepared to authenticate, but we never got an
         * AUTH_CHALLENGE cell, then we would not previously have sent a
         * NETINFO cell. Do so now. */
        if (connection_or_send_netinfo(chan->conn) < 0) {
            connection_or_close_for_error(chan->conn, 0);
            return;
        }
...
#define ERR(s)                                                              \
    ...
    for (i = 0; i < n_certs; ++i) {
        ...
        case CERT_ENCODING_X509: {
            tor_x509_cert_t *x509_cert =
                tor_x509_cert_decode(cert_body, cert_len);
            if (!x509_cert) {
                log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
                       "Received undecodable certificate in CERTS cell from "
                       "%s:%d",
    ...
    if (started_here) {
        ...
        if (!public_server_mode(get_options())) {
            /* If we initiated the connection and we are not a public server,
             * we aren't planning to authenticate at all.  At this point we
             * know who we are talking to, so we can just send a netinfo now.
             */
            send_netinfo = 1;
        }
    } else {
...
#define ERR(s)                                                              \
    ...
    if (BUG(bodylen < 24)) {
        // LCOV_EXCL_START
        ERR("Bodylen is somehow less than 24, which should really be "
            "impossible");
        // LCOV_EXCL_STOP
    }
    ...
    if (sig_is_rsa) {
        if (chan->conn->handshake_state->certs->ed_id_sign != NULL)
            ERR("RSA-signed AUTHENTICATE response provided with an ED25519 "
                "cert");
        ...
        /* Note that we deliberately allow *more* than DIGEST256_LEN bytes
         * here,
        ...
        if (ed25519_checksig(&sig, auth, authlen - ED25519_SIG_LEN, authkey) <
            0) {
