...
typedef struct scheduler_t {
     ...
     * where we register with libevent the next execution of run_sched_ev
     * [which ultimately calls run()]. */
    void (*schedule)(void);

    /* (Mandatory) This is the heart of a scheduler! This is where the
     * excitement happens! Here libevent has given us the chance to execute,
     * and we should do whatever we need to do in order to move some cells from
     * their circuit queues to output buffers in an intelligent manner. We
     * should do this quickly. When we are done, we'll try to schedule()
     * ourself
    ...
    /* (Optional) To be called when a channel is being freed. Sometimes
     * channels go away (for example: the relay on the other end is shutting
     * down). If the scheduler keeps any channel-specific state and has memory
     * to free when channels go away, implement this and free it here. */
