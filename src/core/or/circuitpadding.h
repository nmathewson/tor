...
typedef struct circpad_state_t {
     ...
     * is the max value of possible non-infinite delay allowed by this
     * histogram, and its upper edge is CIRCPAD_DELAY_INFINITE. You can tell if
     * the infinity bin is chosen by inspecting its bin index or inspecting its
     * upper edge.
     *
    ...
    /** Total number of tokens in this histogram. This is a constant and is
     * *not* decremented every time we spend a token. It's used for
     * initializing and refilling the histogram. */
    uint32_t histogram_total_tokens;

    /**
     * Represents a delay probability distribution (aka IAT distribution). It's
     * a parametrized way of encoding inter-packet delay information in
     ...
     *  value we can sample from the distribution. However, RTT measurements
     * and dist_added_shift gets applied on top of this value to derive the
     * final padding delay. */
    circpad_delay_t dist_max_sample_usec;
    /*  If a delay probability distribution is used and this is set, we will
     * add this value on top of the value sampled from the IAT distribution to
     ...
     * XXX: Are there any machines that actually want to set this to 0? There
     * may not be. OTOH, it's only a bit.. */
    ...
    /**
     * If true, estimate the RTT from this relay to the exit/website and add
     * that to start_usec for use as the histogram bin 0 start delay.
     *
...
typedef struct circpad_machine_runtime_t {
     ...
     *
     *  These timers stick around the machineinfo until the machineinfo's
     * circuit is closed, at which point the timer is cancelled. For this
     * reason it's safe to assume that the machineinfo exists if this timer
     * gets triggered. */
...
typedef struct circpad_machine_spec_t {
     ...
     * even if another part of Tor wants to close this circuit.
     *
     *  If this flag is set, the circuitpadding subsystem will close circuits
     * the moment the machine transitions to the END state, and only if the
     * circuit has already been asked to be closed by another part of Tor.
     *
