...
typedef struct circpad_state_t {
     ...
     * *not* decremented every time we spend a token. It's used for initializing
     * and refilling the histogram. */
...
typedef struct circpad_machine_runtime_t {
     ...
     * circuit is closed, at which point the timer is cancelled. For this reason
     * it's safe to assume that the machineinfo exists if this timer gets
     *  triggered. */
...
MOCK_DECL(STATIC signed_error_t, circpad_send_command_to_hop,
          (struct origin_circuit_t * circ, uint8_t hopnum,
           uint8_t relay_command, const uint8_t *payload, ssize_t payload_len));
