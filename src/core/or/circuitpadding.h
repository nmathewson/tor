...
typedef enum { CIRCPAD_STATE_UNCHANGED = 0, CIRCPAD_STATE_CHANGED = 1 } circpad_decision_t;
 ...
 * seconds.
 */
#define CIRCPAD_DELAY_MAX_SECS ((CIRCPAD_DELAY_INFINITE / CIRCPAD_DELAY_UNITS_PER_SECOND) + 1)
...
#define CIRCPAD_STATE_ALL                                                 \
    (CIRCPAD_CIRC_BUILDING | CIRCPAD_CIRC_OPENED | CIRCPAD_CIRC_STREAMS | \
     CIRCPAD_CIRC_NO_STREAMS | CIRCPAD_CIRC_HAS_RELAY_EARLY | CIRCPAD_CIRC_HAS_NO_RELAY_EARLY)
...
void circpad_deliver_recognized_relay_cell_events(struct circuit_t *circ, uint8_t relay_command,
...
signed_error_t circpad_handle_padding_negotiated(struct circuit_t *circ, struct cell_t *cell,
                                                 crypt_path_t *layer_hint);
signed_error_t circpad_negotiate_padding(struct origin_circuit_t *circ,
                                         circpad_machine_num_t machine, uint8_t target_hopnum,
                                         uint8_t command);
bool circpad_padding_negotiated(struct circuit_t *circ, circpad_machine_num_t machine,
                                uint8_t command, uint8_t response);

circpad_purpose_mask_t circpad_circ_purpose_to_mask(uint8_t circ_purpose);

int circpad_check_received_cell(cell_t *cell, circuit_t *circ, crypt_path_t *layer_hint,
                                const relay_header_t *rh);

MOCK_DECL(circpad_decision_t, circpad_machine_schedule_padding, (circpad_machine_runtime_t *));
...
#    define machine_spec_free(chan) FREE_AND_NULL(circpad_machine_spec_t, machine_spec_free_, (m))
...
STATIC circpad_machine_runtime_t *circpad_circuit_machineinfo_new(struct circuit_t *on_circ,
                                                                  int machine_index);
...
STATIC void circpad_machine_remove_closest_token(circpad_machine_runtime_t *mi,
                                                 circpad_delay_t target_bin_us, bool use_usec);
...
MOCK_DECL(STATIC signed_error_t, circpad_send_command_to_hop,
          (struct origin_circuit_t * circ, uint8_t hopnum, uint8_t relay_command,
           const uint8_t *payload, ssize_t payload_len));

MOCK_DECL(STATIC const node_t *, circuit_get_nth_node, (origin_circuit_t * circ, int hop));
