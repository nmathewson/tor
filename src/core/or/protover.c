                    ...
                    uint32_t *high_out)
{
     ...
     * we'd hit a NUL */
    low =
        (uint32_t)tor_parse_ulong(s, 10, 0, MAX_PROTOCOL_VERSION, &ok, &next);
    if (!ok)
        goto error;
    ...
    SMARTLIST_FOREACH_BEGIN (protos, const proto_entry_t *, ent) {
        ...
        SMARTLIST_FOREACH_BEGIN (ent->ranges, const proto_range_t *, range) {
            ...
            for (u = range->low; u <= range->high; ++u) {
                smartlist_add_asprintf(expanded, "%s=%lu", name,
                                       (unsigned long)u);
...
contract_protocol_list(const smartlist_t *proto_strings)
{
    ...
    SMARTLIST_FOREACH_BEGIN (all_names, const char *, name) {
        ...
        while (start_of_cur_series < smartlist_len(lst)) {
            ...
            int idx;
            for (idx = start_of_cur_series + 1; idx < smartlist_len(lst);
                 ++idx) {
...
protover_all_supported(const char *s, char **missing_out)
{
    ...
    SMARTLIST_FOREACH_BEGIN (entries, const proto_entry_t *, ent) {
        ...
        SMARTLIST_FOREACH_BEGIN (ent->ranges, const proto_range_t *, range) {
            ...
            for (i = range->low; i <= range->high; ++i) {
                if (!protover_is_supported_here(tp, i)) {
                    if (versions->low == 0 && versions->high == 0) {
                        versions->low = i;
                        /* Pre-emptively add the high now, just in case we're
                         * in a single version range (e.g. "Link=999"). */
                        versions->high = i;
                    }
                    /* If the last one to be unsupported is one less than the
                     * current one, we're in a continuous range, so set the
                     * high field. */
                    if ((versions->high && versions->high == i - 1) ||
                        /* Similarly, if the last high wasn't set and we're
                         * currently one higher than the low, add current index
                         * as the highest known high. */
                        (!versions->high && versions->low == i - 1)) {
                        versions->high = i;
                        continue;
                    }
                } else {
                    /* If we hit a supported version, and we previously had a
                     * range, we've hit a non-continuity. Copy the previous
                     * range and add it to the unsupported->ranges list and
                     * zero-out the previous range for the next iteration. */
                    if (versions->low != 0 && versions->high != 0) {
                        proto_range_t *versions_to_add =
            ...
            /* Once we've run out of versions to check, see if we had any
             * unsupported ones and, if so, add them to unsupported->ranges. */
            if (versions->low != 0 && versions->high != 0) {
    ...
    /* We keep the two smartlists separate so that we can free the
     * proto_entry_t we created and put in missing_some, so here we add them
     * together to build the string. */
    ...
    SMARTLIST_FOREACH(missing_some, proto_entry_t *, ent,
...
protover_free_all(void)
{
    if (supported_protocol_list) {
        ...
        SMARTLIST_FOREACH(entries, proto_entry_t *, ent,
                          proto_entry_free(ent));
        smartlist_free(entries);
