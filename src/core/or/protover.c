...
static int protocol_list_contains(const smartlist_t *protos, protocol_type_t pr,
                                  uint32_t ver);
            ...
            for (i = range->low; i <= range->high; ++i) {
                if (!protover_is_supported_here(tp, i)) {
                    if (versions->low == 0 && versions->high == 0) {
                        versions->low = i;
                        /* Pre-emptively add the high now, just in case we're in
                         * a single version range (e.g. "Link=999"). */
                        versions->high = i;
                    }
