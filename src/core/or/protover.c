...
static const struct {
    ...
    {PRT_LINK, "Link"},         {PRT_LINKAUTH, "LinkAuth"}, {PRT_RELAY, "Relay"},
    {PRT_DIRCACHE, "DirCache"}, {PRT_HSDIR, "HSDir"},       {PRT_HSINTRO, "HSIntro"},
    {PRT_HSREND, "HSRend"},     {PRT_DESC, "Desc"},         {PRT_MICRODESC, "Microdesc"},
    {PRT_PADDING, "Padding"},   {PRT_CONS, "Cons"},         {PRT_FLOWCTRL, "FlowCtrl"},
};
 ...
 */
static int
parse_version_range(const char *s, const char *end_of_range, uint32_t *low_out, uint32_t *high_out)
{
    uint32_t low, high;
    char *next = NULL;
    int ok;
 ...
 */
int
protocol_list_supports_protocol(const char *list, protocol_type_t tp, uint32_t version)
{
 ...
 */
int
protocol_list_supports_protocol_or_later(const char *list, protocol_type_t tp, uint32_t version)
{
    /* NOTE: This is a pretty inefficient implementation. If it ever shows
     * up in profiles, we should memoize it.
     */
...
get_supported_protocol_list(void)
{
    if (PREDICT_UNLIKELY(supported_protocol_list == NULL)) {
        supported_protocol_list = parse_protocol_list(protover_get_supported_protocols());
    }
...
proto_entry_encode_into(smartlist_t *chunks, const proto_entry_t *entry)
{
    ...
    SMARTLIST_FOREACH_BEGIN (entry->ranges, proto_range_t *, range) {
        ...
        if (range->low == range->high) {
            smartlist_add_asprintf(chunks, "%s%lu", comma, (unsigned long)range->low);
        } else {
            smartlist_add_asprintf(chunks, "%s%lu-%lu", comma, (unsigned long)range->low,
...
static int
protocol_list_contains(const smartlist_t *protos, protocol_type_t pr, uint32_t ver)
{
    if (BUG(protos == NULL)) {
...
protover_compute_for_old_tor(const char *version)
{
    ...
    if (tor_version_as_new_as(version, FIRST_TOR_VERSION_TO_ADVERTISE_PROTOCOLS)) {
        return "";
