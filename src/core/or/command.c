...
command_process_cell(channel_t *chan, cell_t *cell)
{
    ...
    if (now > current_second) { /* the second has rolled over */
        ...
        log_info(LD_OR,
                 ...
                 num_create, create_time / 1000, num_created, created_time / 1000, num_relay,
                 relay_time / 1000, num_destroy, destroy_time / 1000);
...
#    define PROCESS_CELL(tp, cl, cn)                                                       \
        STMT_BEGIN                                                                         \
            {                                                                              \
                ++num##tp;                                                                 \
                command_time_process_cell(cl, cn, &tp##time, command_process_##tp##_cell); \
            }                                                                              \
        STMT_END
...
command_process_create_cell(cell_t *cell, channel_t *chan)
{
    ...
    log_debug(LD_OR, "Got a CREATE cell for circ_id %u on channel %" PRIu64 " (%p)",
    ...
    if (circuit_id_in_use_on_channel(cell->circ_id, chan)) {
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
               ...
               "Dropping (age %d).",
               (unsigned)cell->circ_id, (int)(time(NULL) - channel_when_created(chan)));
        if (node) {
            char *p = esc_for_log(node_get_platform(node));
            log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "Details: router %s, platform %s.",
    ...
    if (we_are_hibernating()) {
        log_info(LD_OR, "Received create cell but we're shutting down. Sending back "
                        "destroy.");
    ...
    if (!server_mode(options) || (!public_server_mode(options) && channel_is_outgoing(chan))) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
    ...
    if (create_cell_parse(create_cell, cell) < 0) {
        ...
        log_fn(LOG_PROTOCOL_WARN, LD_OR, "Bogus/unrecognized create cell; closing.");
    ...
    if (create_cell->handshake_type != ONION_HANDSHAKE_TYPE_FAST) {
        ...
        len = onion_skin_server_handshake(ONION_HANDSHAKE_TYPE_FAST, create_cell->onionskin,
                                          create_cell->handshake_len, NULL, created_cell.reply,
                                          keys, CPATH_KEY_MATERIAL_LEN, rend_circ_nonce);
        tor_free(create_cell);
        if (len < 0) {
...
command_process_created_cell(cell_t *cell, channel_t *chan)
{
    ...
    if (circ->n_circ_id != cell->circ_id || circ->n_chan != chan) {
        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL, "got created cell from Tor client? Closing.");
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) { /* we're the OP. Handshake this. */
        ...
        if ((err_reason = circuit_finish_handshake(origin_circ, &extended_cell.created_cell)) < 0) {
        ...
        log_debug(LD_OR, "Converting created cell to extended relay cell, sending.");
        ...
        relay_send_command_from_edge(0, circ, command, (const char *)payload, len, NULL);
    }
}
...
command_process_relay_cell(cell_t *cell, channel_t *chan)
{
    ...
    if (!circ) {
        log_debug(LD_OR, "unknown circuit %u on connection from %s. Dropping.",
                  (unsigned)cell->circ_id, channel_get_canonical_remote_descr(chan));
        return;
    }
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) {
        ...
        ocirc->n_read_circ_bw = tor_add_u32_nowrap(ocirc->n_read_circ_bw, CELL_PAYLOAD_SIZE);
    ...
    if (cell->command == CELL_RELAY_EARLY) {
        if (direction == CELL_DIRECTION_IN) {
            ...
            if (CIRCUIT_IS_ORIGIN(circ)) {
                ...
                log_warn(LD_OR, " upstream=%s", channel_get_actual_remote_descr(circ->n_chan));
            }
            circuit_mark_for_close(circ, END_CIRC_REASON_TORPROTOCOL);
            return;
        } else {
            ...
            if (or_circ->remaining_relay_early_cells == 0) {
                log_fn(LOG_PROTOCOL_WARN, LD_OR,
                       ...
                       "  Closing circuit.",
                       (unsigned)cell->circ_id, safe_str(channel_get_canonical_remote_descr(chan)));
                circuit_mark_for_close(circ, END_CIRC_REASON_TORPROTOCOL);
                return;
            }
