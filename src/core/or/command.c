...
command_process_cell(channel_t *chan, cell_t *cell)
{
    ...
    if (now > current_second) { /* the second has rolled over */
        ...
        log_info(LD_OR,
                 ...
                 num_create, create_time / 1000, num_created,
                 created_time / 1000, num_relay, relay_time / 1000,
...
command_process_create_cell(cell_t *cell, channel_t *chan)
{
    ...
    if (create_cell->handshake_type != ONION_HANDSHAKE_TYPE_FAST) {
        ...
        /* This is a CREATE_FAST cell; we can handle it immediately without
         * using a CPU worker. */
        ...
        if (onionskin_answer(circ, &created_cell, (const char *)keys,
                             sizeof(keys), rend_circ_nonce) < 0) {
...
command_process_created_cell(cell_t *cell, channel_t *chan)
{
    ...
    if (CIRCUIT_IS_ORIGIN(circ)) { /* we're the OP. Handshake this. */
        ...
        if (extended_cell_format(&command, &len, payload, &extended_cell) <
            0) {
        ...
        relay_send_command_from_edge(0, circ, command, (const char *)payload,
                                     len, NULL);
    }
}
...
command_setup_listener(channel_listener_t *listener)
{
    ...
    channel_listener_set_listener_fn(listener,
