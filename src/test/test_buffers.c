...
test_buffer_time_tracking(void *arg)
{
    ...
    buf2 = buf_copy(buf);
    tt_assert(buf2);
    tt_int_op(1234, OP_EQ, buf_get_oldest_chunk_timestamp(buf2, START_TS + 1234));
...
static void
test_buffers_compress_fin_at_chunk_end_impl(compress_method_t method, compression_level_t level)
{
    char *msg = NULL;
    char *contents = NULL;
    char *expanded = NULL;
    ...
    tt_int_op(0, OP_EQ,
              tor_uncompress(&expanded, &out_len, contents + headerjunk, in_len - headerjunk,
                             method, 1, LOG_WARN));
                           ...
                           int finalize_with_nil)
{
    ...
    tt_int_op(buf_add_compress(buf, compress_state, msg + 128, 128, 0), OP_EQ, 0);
    tt_int_op(buf_add_compress(buf, compress_state, msg + 256, 256, 0), OP_EQ, 0);
    done = !finalize_with_nil;
    tt_int_op(buf_add_compress(buf, compress_state, "all done", 9, done), OP_EQ, 0);
    ...
    tt_int_op(0, OP_EQ, tor_uncompress(&expanded, &out_len, contents, in_len, method, 1, LOG_WARN));
...
struct testcase_t buffer_tests[] = {
    ...
    {"allocation_tracking", test_buffer_allocation_tracking, TT_FORK, NULL, NULL},
    ...
    {"compress/zlib", test_buffers_compress, TT_FORK, &passthrough_setup, (char *)"deflate"},
    {"compress/gzip", test_buffers_compress, TT_FORK, &passthrough_setup, (char *)"gzip"},
    {"compress/zstd", test_buffers_compress, TT_FORK, &passthrough_setup, (char *)"x-zstd"},
    {"compress/lzma", test_buffers_compress, TT_FORK, &passthrough_setup, (char *)"x-tor-lzma"},
    {"compress/none", test_buffers_compress, TT_FORK, &passthrough_setup, (char *)"identity"},
