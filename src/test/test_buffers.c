...
test_buffer_pullup(void *arg)
{
    ...
    /* Make room for 3000 bytes in the first chunk, so that the pullup-move
     * code can get tested. */
...
test_buffer_allocation_tracking(void *arg)
{
    ...
    tt_int_op(buf_get_total_allocation(), OP_EQ, 12288); /* that chunk was
                                                         really freed. */
...
test_buffer_time_tracking(void *arg)
{
    ...
    tt_int_op(1000, OP_EQ,
              buf_get_oldest_chunk_timestamp(buf, START_TS + 1000));
    ...
    tt_int_op(2000, OP_EQ,
              buf_get_oldest_chunk_timestamp(buf, START_TS + 2000));
                           ...
                           int finalize_with_nil)
{
    ...
    tt_int_op(buf_add_compress(buf, compress_state, "all done", 9, done),
              OP_EQ, 0);
...
test_buffers_find_contentlen(void *arg)
{
    ...
    for (i = 0; results[i].headers; ++i) {
        ...
        char *tmp =
            tor_memdup(results[i].headers, headerlen); /* ensure no eos */
