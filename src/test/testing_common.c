...
#define RAND_PATH_BYTES 5
...
#ifdef _WIN32
    {
        ...
        tor_snprintf(temp_dir, sizeof(temp_dir), "%s%stor_test_%d_%s", tmp, extra_backslash,
                     (int)getpid(), rnd32);
        r = mkdir(temp_dir);
    }
#elif defined(__ANDROID__)
    /* tor might not like the default perms, so create a subdir */
    tor_snprintf(temp_dir, sizeof(temp_dir), "/data/local/tmp/tor_%d_%d_%s", (int)getuid(),
                 (int)getpid(), rnd32);
...
get_fname_suffix(const char *name, const char *suffix)
{
    ...
    if (!name)
        return temp_dir;
    tor_snprintf(buf, sizeof(buf), "%s%s%s%s%s", temp_dir, PATH_SEPARATOR, name, suffix ? "_" : "",
                 suffix ? suffix : "");
    return buf;
}
...
main(int c, const char **v)
{
    ...
    if (skip_tests) {
        ...
        smartlist_split_string(skip, skip_tests, NULL, SPLIT_IGNORE_BLANK, -1);
        int n = 0;
