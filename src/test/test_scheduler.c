...
cleanup_scheduler_options(void)
{
    if (mocked_options.SchedulerTypes_) {
        SMARTLIST_FOREACH(mocked_options.SchedulerTypes_, int *, i,
                          tor_free(i));
...
channel_should_write_to_kernel_mock(outbuf_table_t *ot, channel_t *chan)
{
    ...
    (void)chan;
    return 1;
    /* We could make this more complicated if we wanted. But I don't think
     * doing so tests much of anything */
...
test_scheduler_loop_vanilla(void *arg)
{
     ...
     * channel_more_to_flush() returning false and
     * channel_num_cells_writeable() > 0/
     */
    tt_int_op(ch2->scheduler_state, OP_EQ, SCHED_CHAN_WAITING_FOR_CELLS);
...
test_scheduler_ns_changed(void *arg)
{
    ...
    /*
     * Currently no scheduler implementations use the old/new consensuses
     * passed in scheduler_notify_networkstatus_changed, so it is okay to pass
     * NULL.
     *
     * "But then what does test actually exercise???" It tests that
     * scheduler_notify_networkstatus_changed fetches the correct value from
     * the consensus, and then switches the scheduler if necessasry.
     */
...
test_scheduler_kist_pending_list(void *arg)
{
     ...
     * depending on the channel state, what will be the expected behavior of
     * the scheduler with that list.
     *
     ...
     * Essentially, this will articifically test cases of the KIST main loop
     * and entry point in the channel subsystem.
     *
    ...
    /* We'll flush one cell and make it that the socket can write but no more
     * to flush else we end up in an infinite loop. We expect the channel to be
     * put in waiting for cells state and the pending list empty. */
    ...
    /* Set the channel to a state where it doesn't want to write more. We
     * expect that the channel becomes idle. */
    scheduler_channel_doesnt_want_writes(chan1);
    tt_int_op(smartlist_len(get_channels_pending()), OP_EQ, 0);
    tt_int_op(chan1->scheduler_state, OP_EQ, SCHED_CHAN_IDLE);

    /* Some cells arrive on the channel now. We expect it to go back in waiting
     * to write. You might wonder why it is not put in the pending list?
     * Because once the channel becomes OPEN again (the doesn't want to write
     * event only occurs if the channel goes in MAINT mode), if there are cells
     * in the channel, the wants to write event is triggered thus putting the
     * channel in pending mode.
     *
     * Else, if no cells, it stays IDLE and then once a cell comes in, it
     * should go in waiting to write which is a BUG itself because the channel
     * can't be scheduled until a second cell comes in. Hopefully, #24554 will
     * fix that for KIST. */
    scheduler_channel_has_waiting_cells(chan1);
    tt_int_op(smartlist_len(get_channels_pending()), OP_EQ, 0);
    tt_int_op(chan1->scheduler_state, OP_EQ, SCHED_CHAN_WAITING_TO_WRITE);

    /* Second cell comes in, unfortunately, it won't get scheduled until a
     * wants to write event occurs like described above. */
    scheduler_channel_has_waiting_cells(chan1);
    tt_int_op(smartlist_len(get_channels_pending()), OP_EQ, 0);
    tt_int_op(chan1->scheduler_state, OP_EQ, SCHED_CHAN_WAITING_TO_WRITE);
     ...
     * exact channel was added back in the pending list because its state
     * wasn't PENDING.
     *
