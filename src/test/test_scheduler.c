...
static int32_t
mock_vanilla_networkstatus_get_param(const networkstatus_t *ns, const char *param_name,
...
static int32_t
mock_kist_networkstatus_get_param(const networkstatus_t *ns, const char *param_name,
                                  int32_t default_val, int32_t min_val, int32_t max_val)
{
    (void)ns;
    (void)default_val;
...
channel_flush_some_cells_mock_free_all(void)
{
    if (chans_for_flush_mock) {
        SMARTLIST_FOREACH_BEGIN (chans_for_flush_mock, flush_mock_channel_t *, flush_mock_ch) {
...
struct testcase_t scheduler_tests[] = {
    ...
    {"kist_pending_list", test_scheduler_kist_pending_list, TT_FORK, NULL, NULL},
    END_OF_TESTCASES};
