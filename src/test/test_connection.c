...
test_conn_get_basic_teardown(const struct testcase_t *tc, void *arg)
{
    ...
    if (conn->linked_conn) {
         ...
         * so we can't rely on connection_unregister_events() use of
         * event_del().
         */
        ...
        if (!conn->linked_conn->marked_for_close) {
            ...
            if (CONN_IS_EDGE(conn->linked_conn)) {
                ...
                if (conn->linked_conn->type == CONN_TYPE_AP) {
                    TO_ENTRY_CONN(conn->linked_conn)
                        ->socks_request->has_finished = 1;
                }
            }
...
test_conn_get_basic(void *arg)
{
    ...
    tt_assert(connection_get_by_type_addr_port_purpose(conn->type, &conn->addr,
                                                       ...
                                                       conn->purpose) == NULL);
    tt_assert(connection_get_by_type_addr_port_purpose(
                  conn->type, &conn->addr, conn->port, !conn->purpose) ==
              NULL);
...
test_failed_orconn_tracker(void *arg)
{
    ...
    /* Move time forward and check again: the cache should have been cleared
    ...
    /* Now mock the node_get_*by_id() functions to start using the node
