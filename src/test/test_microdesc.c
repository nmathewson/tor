...
test_md_cache(void *data)
{
    ...
    added = microdescs_add_to_cache(mc, test_md3, NULL, SAVED_NOWHERE, 0, -1,
                                    NULL);
...
test_md_generate(void *arg)
{
    ...
    tt_assert(
        ed25519_pubkey_eq(md->ed25519_identity_pkey,
...
test_md_parse(void *arg)
{
    ...
    smartlist_t *mds = microdescs_parse_from_string(MD_PARSE_TEST_DATA, NULL,
                                                    1, SAVED_NOWHERE, invalid);
...
test_md_parse_id_ed25519(void *arg)
{
    ...
    mds =
        microdescs_parse_from_string(BOGUS_KEY, NULL, 1, SAVED_NOWHERE, NULL);
    tt_assert(mds);
    tt_int_op(smartlist_len(mds), OP_EQ, 0); // no entries.
