...
static const char test_md1[] = "onion-key\n"
...
test_md_cache(void *data)
{
    ...
    crypto_digest256(d3, test_md3_noannotation, strlen(test_md3_noannotation), DIGEST_SHA256);

    mc = get_microdesc_cache();

    added = microdescs_add_to_cache(mc, test_md1, NULL, SAVED_NOWHERE, 0, time1, NULL);
    ...
    added = microdescs_add_to_cache(mc, test_md2, NULL, SAVED_NOWHERE, 0, time2, wanted);
    ...
    added = microdescs_add_to_cache(mc, test_md3, NULL, SAVED_NOWHERE, 0, -1, NULL);
    /* Must fail, since SAVED_NOWHERE precludes annotations */
    tt_int_op(0, OP_EQ, smartlist_len(added));
    smartlist_free(added);
    added = NULL;

    added = microdescs_add_to_cache(mc, test_md3_noannotation, NULL, SAVED_NOWHERE, 0, time3, NULL);
    /* Now it can work */
    tt_int_op(1, OP_EQ, smartlist_len(added));
    ...
    tt_mem_op(md3->body, OP_EQ, test_md3_noannotation, strlen(test_md3_noannotation));

    tor_asprintf(&fn, "%s" PATH_SEPARATOR "cached-microdescs.new", options->CacheDirectory);
    ...
    /* read the cache. */
    tor_asprintf(&fn, "%s" PATH_SEPARATOR "cached-microdescs", options->CacheDirectory);
    s = read_file_to_str(fn, RFTS_BIN, NULL);
...
static const char test_ri[] = "router test005r 127.0.0.1 5005 0 7005\n"
...
test_md_parse(void *arg)
{
    ...
    smartlist_t *mds =
        microdescs_parse_from_string(MD_PARSE_TEST_DATA, NULL, 1, SAVED_NOWHERE, invalid);
    tt_int_op(smartlist_len(mds), OP_EQ, 14);
    tt_int_op(smartlist_len(invalid), OP_EQ, 4);

    test_memeq_hex(smartlist_get(invalid, 0), "5d76bf1c6614e885614a1e0ad074e1ab"
                                              "4ea14655ebeefb1736a71b5ed8a15a51");
    test_memeq_hex(smartlist_get(invalid, 1), "2fde0ee3343669c2444cd9d53cbd39c6"
                                              "a7d1fc0513513e840ca7f6e68864b36c");
    test_memeq_hex(smartlist_get(invalid, 2), "20d1576c5ab11bbcff0dedb1db4a3cfc"
                                              "c8bc8dd839d8cbfef92d00a1a7d7b294");
    test_memeq_hex(smartlist_get(invalid, 3), "074770f394c73dbde7b44412e9692add"
...
test_md_parse_id_ed25519(void *arg)
{
    ...
    tt_mem_op(md->ed25519_identity_pkey, OP_EQ, "This isn't actually a public key",
              ED25519_PUBKEY_LEN);
    ...
    mds = microdescs_parse_from_string(DUPLICATE_KEY, NULL, 1, SAVED_NOWHERE, NULL);
    tt_assert(mds);
    ...
    const char BOGUS_KEY[] = "onion-key\n"
                             "-----BEGIN RSA PUBLIC KEY-----\n"
                             "MIGJAoGBAM7uUtq5F6h63QNYIvC+4NcWaD0DjtnrOORZMkdpJhinXUOwce3cD5Dj\n"
                             "sgdN1wJpWpTQMXJ2DssfSgmOVXETP7qJuZyRprxalQhaEATMDNJA/66Ml1jSO9mZ\n"
                             "+8Xb7m/4q778lNtkSbsvMaYD2Dq6k2QQ3kMhr9z8oUtX0XA23+pfAgMBAAE=\n"
...
#define ADD(hex)                                                                          \
    ...
    added =
        microdescs_add_to_cache(mc, MD_PARSE_TEST_DATA, NULL, SAVED_NOWHERE, 0, time(NULL), wanted);
...
test_md_corrupt_desc(void *arg)
{
    ...
    sl = microdescs_add_to_cache(get_microdesc_cache(), cp, cp + strlen(cp), SAVED_IN_JOURNAL, 0,
                                 time(NULL), NULL);
    tt_int_op(smartlist_len(sl), OP_EQ, 0);
