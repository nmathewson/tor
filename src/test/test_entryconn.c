...
static struct testcase_setup_t test_rewrite_setup = {entryconn_rewrite_setup,
...
test_entryconn_rewrite_basic(void *arg)
{
    ...
    strlcpy(ec->socks_request->address, "www.TORproject.org", sizeof(ec->socks_request->address));
...
test_entryconn_rewrite_automap_ipv4(void *arg)
{
    ...
    strlcpy(ec->socks_request->address, "WWW.MIT.EDU", sizeof(ec->socks_request->address));
    ...
    strlcpy(ec3->socks_request->address, "www.MIT.EDU", sizeof(ec3->socks_request->address));
...
test_entryconn_rewrite_cached_dns_ipv4(void *arg)
{
    ...
    addressmap_register("www.friendly.example.com", tor_strdup("240.240.241.241"), expires,
                        ADDRMAPSRC_DNS, 0, 0);
...
test_entryconn_rewrite_cached_dns_ipv6(void *arg)
{
    ...
    addressmap_register("www.friendly.example.com", tor_strdup("[::f00f]"), expires, ADDRMAPSRC_DNS,
                        0, 0);
...
test_entryconn_rewrite_unmapped_virtual(void *arg)
{
    ...
    strlcpy(ec->socks_request->address, "18.202.5.5", sizeof(ec->socks_request->address));
    ec->socks_request->command = SOCKS_COMMAND_CONNECT;
    connection_ap_handshake_rewrite(ec, &rr);
...
test_entryconn_rewrite_mapaddress(void *arg)
{
    ...
    strlcpy(ec->socks_request->address, "meta", sizeof(ec->socks_request->address));
    ec->socks_request->command = SOCKS_COMMAND_CONNECT;
    connection_ap_handshake_rewrite(ec, &rr);
...
test_entryconn_rewrite_reject_internal_reverse(void *arg)
{
    ...
    strlcpy(ec->socks_request->address, "10.0.0.1", sizeof(ec->socks_request->address));
...
test_entryconn_rewrite_automap_exit(void *arg)
{
    ...
    strlcpy(ec->socks_request->address, "website.example.exit", sizeof(ec->socks_request->address));
    ec->socks_request->command = SOCKS_COMMAND_RESOLVE;
    connection_ap_handshake_rewrite(ec, &rr);
...
test_entryconn_rewrite_mapaddress_exit(void *arg)
{
    ...
    /* Automap this on resolve. */
    strlcpy(ec->socks_request->address, "abc.example.com", sizeof(ec->socks_request->address));
    ec->socks_request->command = SOCKS_COMMAND_CONNECT;
    connection_ap_handshake_rewrite(ec, &rr);
...
test_entryconn_rewrite_mapaddress_automap_onion(void *arg)
{
    ...
    strlcpy(ec->socks_request->address, "foo.onion", sizeof(ec->socks_request->address));
    ec->socks_request->command = SOCKS_COMMAND_CONNECT;
    connection_ap_handshake_rewrite(ec, &rr);
    ...
    strlcpy(ec2->socks_request->address, "foo.onion", sizeof(ec2->socks_request->address));
...
static void
test_entryconn_rewrite_mapaddress_automap_onion_common(entry_connection_t *ec, int map_to_onion,
                                                       int map_to_address)
{
    entry_connection_t *ec2 = NULL;
    entry_connection_t *ec3 = NULL;
    ...
    strlcpy(ec->socks_request->address, "irc.example.com", sizeof(ec->socks_request->address));
    ec->socks_request->command = SOCKS_COMMAND_CONNECT;
    connection_ap_handshake_rewrite(ec, &rr);
    ...
    strlcpy(ec2->socks_request->address, "irc.example.com", sizeof(ec2->socks_request->address));
    ec2->socks_request->command = SOCKS_COMMAND_RESOLVE;
    connection_ap_handshake_rewrite(ec2, &rr);
...
struct testcase_t entryconn_tests[] = {
    REWRITE(rewrite_basic), REWRITE(rewrite_bad_dotexit), REWRITE(rewrite_automap_ipv4),
    ...
    REWRITE(rewrite_mapaddress_exit), REWRITE(rewrite_mapaddress_automap_onion),
    REWRITE(rewrite_mapaddress_automap_onion2), REWRITE(rewrite_mapaddress_automap_onion3),
