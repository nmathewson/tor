                              ...
                              const char *file)
{
    ...
    if (circ->marked_for_close) {
        TT_FAIL(
            ("Circuit already marked for close at %s:%d, but we are marking "
             "it again at %s:%d",
...
test_oom_streambuf(void *arg)
{
    ...
    for (i = 0; i < 4; ++i) {
        ...
        now_ns += 10 * 1000000;
        monotime_coarse_set_mock_time_nsec(now_ns);
        ec = dummy_edge_conn_new(c4, CONN_TYPE_EXIT, 1000,
                                 1000); /* Yes, 4 twice*/
        tt_assert(ec);
        smartlist_add(edgeconns, ec);
        now_ns += 10 * 1000000;
...
#define ts_is_approx(ts, val)                                         \
    do {                                                              \
        uint32_t x_ =                                                 \
            (uint32_t)monotime_coarse_stamp_units_to_approx_msec(ts); \
