...
static void
circuit_mark_for_close_dummy_(circuit_t *circ, int reason, int line, const char *file)
{
    (void)reason;
    if (circ->marked_for_close) {
        TT_FAIL(("Circuit already marked for close at %s:%d, but we are marking "
                 "it again at %s:%d",
                 circ->marked_for_close_file, (int)circ->marked_for_close, file, line));
    }
...
dummy_or_circuit_new(int n_p_cells, int n_n_cells)
{
    ...
    for (i = 0; i < n_p_cells; ++i) {
        ...
        cell_queue_append_packed_copy(TO_CIRCUIT(circ), &circ->p_chan_cells, 0, &cell, 1, 0);
    }
...
dummy_edge_conn_new(circuit_t *circ, int type, size_t in_bytes, size_t out_bytes)
{
...
test_oom_circbuf(void *arg)
{
    ...
    tt_int_op(cell_queues_get_total_allocation(), OP_EQ, packed_cell_mem_cost() * 70);
    ...
    tt_int_op(cell_queues_check_size(), OP_EQ, 0); /* We are still not OOM */
    tt_int_op(cell_queues_get_total_allocation(), OP_EQ, packed_cell_mem_cost() * 255);
    ...
    tt_int_op(cell_queues_get_total_allocation(), OP_EQ, packed_cell_mem_cost() * 257);
    ...
    tt_int_op(cell_queues_get_total_allocation(), OP_EQ, packed_cell_mem_cost() * (257 - 30));
...
test_oom_streambuf(void *arg)
{
    ...
    tt_int_op(cell_queues_get_total_allocation(), OP_EQ, packed_cell_mem_cost() * 80);
...
#define ts_is_approx(ts, val)                                                   \
    ...
    tt_int_op(cell_queues_get_total_allocation(), OP_EQ, packed_cell_mem_cost() * 85);
    tt_int_op(buf_get_total_allocation(), OP_EQ, 4096 * 17 * 2);
    ...
    SMARTLIST_FOREACH(edgeconns, edge_connection_t *, ec, connection_free_minimal(TO_CONN(ec)));
...
struct testcase_t oom_tests[] = {{"circbuf", test_oom_circbuf, TT_FORK, NULL, NULL},
