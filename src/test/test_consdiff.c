...
static int
consensus_compute_digest_as_signed_(const char *cons, consensus_digest_t *digest_out)
{
    size_t len = strlen(cons);
    char *tmp = tor_memdup(cons, len);
    // We use memdup here to ensure that the input is NOT nul-terminated.
...
test_consdiff_smartlist_slice(void *arg)
{
    ...
    tt_ptr_op(&items[5], OP_EQ, smartlist_get(sls->list, sls->offset + (sls->len - 1)));
    tor_free(sls);
...
test_consdiff_next_router(void *arg)
{
    ...
    smartlist_add_linecpy(sl, area, "r name hash+longer+than+27+chars+and+valid+base64 etc");
    smartlist_add_linecpy(sl, area, "foo");
    smartlist_add_linecpy(sl, area, "foo");
    smartlist_add_linecpy(sl, area, "r name hash+longer+than+27+chars+and+valid+base64 etc");
    smartlist_add_linecpy(sl, area, "foo");
...
test_consdiff_gen_diff(void *arg)
{
    ...
    tt_int_op(0, OP_EQ, consensus_compute_digest_as_signed_(cons1_str, &digests1));
    ...
    tt_assert(line_str_eq(smartlist_get(diff, 0), "network-status-diff-version 1"));
    tt_assert(line_str_eq(smartlist_get(diff, 1),
                          "hash "
    ...
    tt_assert(line_str_eq(smartlist_get(diff, 5), "directory-signature foo bar"));
    tt_assert(line_str_eq(smartlist_get(diff, 6), "."));
    tt_assert(line_str_eq(smartlist_get(diff, 7), "1a"));
    tt_assert(line_str_eq(smartlist_get(diff, 8), "r name aaaaaaaaaaaaaaaaa etc"));
...
struct testcase_t consdiff_tests[] = {CONSDIFF_LEGACY(smartlist_slice),
