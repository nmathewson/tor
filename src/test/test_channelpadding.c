...
{
    or_connection_t *conn = (or_connection_t *)connection_new(CONN_TYPE_OR, AF_INET);
...
{
    current_md_consensus = current_ns_consensus = tor_malloc_zero(sizeof(networkstatus_t));
...
{
    SMARTLIST_FOREACH(current_md_consensus->routerstatus_list, void *, r, tor_free(r));
...
test_channelpadding_killonehop(void *arg)
{
    ...
    smartlist_add(current_md_consensus->net_params, (void *)"nf_pad_single_onion=0");
    channelpadding_new_consensus_params(current_md_consensus);
    ...
    get_options_mutable()->HiddenServiceSingleHopMode = 0;
    get_options_mutable()->HiddenServiceNonAnonymousMode = 0;
    tt_int_op(channelpadding_decide_to_pad_channel(relay3_client), OP_EQ, CHANNELPADDING_WONTPAD);
...
test_channelpadding_consensus(void *arg)
{
    ...
    memcpy(relay->identity_digest, ((channel_tls_t *)chan)->conn->identity_digest, DIGEST_LEN);
    smartlist_add(current_md_consensus->routerstatus_list, relay);
    ...
    smartlist_add(current_md_consensus->net_params, (void *)"nf_pad_before_usage=0");
    channelpadding_new_consensus_params(current_md_consensus);
    tried_to_write_cell = 0;
    decision = channelpadding_decide_to_pad_channel(chan);
    tt_int_op(decision, OP_EQ, CHANNELPADDING_WONTPAD);
    ...
    smartlist_add(current_md_consensus->net_params, (void *)"nf_conntimeout_relays=600");
    channelpadding_new_consensus_params(current_md_consensus);
    ...
    options->ReducedConnectionPadding = 1;
    smartlist_add(current_md_consensus->net_params, (void *)"nf_conntimeout_clients=600");
    channelpadding_new_consensus_params(current_md_consensus);
    val = channelpadding_get_circuits_available_timeout();
...
test_channelpadding_negotiation(void *arg)
{
    ...
    tt_assert(relay3_client->padding_enabled);
    tt_int_op(channelpadding_update_padding_for_channel(client_relay3, &disable), OP_EQ, -1);
    tt_assert(client_relay3->padding_enabled);
    ...
    int64_t val64 = channelpadding_compute_time_until_pad_for_netflow(client_relay3);
