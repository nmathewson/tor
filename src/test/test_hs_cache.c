...
test_directory(void *arg)
{
    ...
    /* Store two descriptors and remove the expiring one only. */
    {
        ...
        ret = hs_cache_lookup_as_dir(3, helper_get_hsdir_query(desc_zero_lifetime), NULL);
        tt_int_op(ret, OP_EQ, 0);
...
helper_fetch_desc_from_hsdir(const ed25519_public_key_t *blinded_key)
{
    ...
    /* Read the descriptor that the HSDir just served us */
    {
        ...
        fetch_from_buf_http(TO_CONN(conn)->outbuf, &headers, MAX_HEADERS_SIZE, &received_desc,
                            &body_used, HS_DESC_MAX_LEN, 0);
        tor_free(headers);
    }
...
test_upload_and_download_hs_desc(void *arg)
{
    ...
    /* Generate a valid descriptor with normal values. */
    {
        ...
        retval = hs_desc_encode_descriptor(published_desc, &signing_kp, NULL, &published_desc_str);
        tt_int_op(retval, OP_EQ, 0);
    }
...
test_hsdir_revision_counter_check(void *arg)
{
    ...
    /* Fetch the published descriptor and validate the revision counter. */
    {
        ...
        retval = hs_desc_decode_descriptor(received_desc_str, &subcredential, NULL, &received_desc);
...
test_client_cache(void *arg)
{
    ...
    /* Generate a valid descriptor with normal values. */
    {
        ...
        retval = hs_desc_encode_descriptor(published_desc, &signing_kp, NULL, &published_desc_str);
        tt_int_op(retval, OP_EQ, 0);
        memcpy(&wanted_subcredential, &published_desc->subcredential, sizeof(hs_subcredential_t));
        tt_assert(!fast_mem_is_zero((char *)wanted_subcredential.subcred, DIGEST256_LEN));
    }
     ...
     * cleaned since we still in the same TP. */
    {
        ...
        tt_mem_op(cached_desc->subcredential.subcred, OP_EQ, wanted_subcredential.subcred,
                  SUBCRED_LEN);
    }
...
struct testcase_t hs_cache[] = {
    ...
    {"hsdir_revision_counter_check", test_hsdir_revision_counter_check, TT_FORK, NULL, NULL},
    {"upload_and_download_hs_desc", test_upload_and_download_hs_desc, TT_FORK, NULL, NULL},
