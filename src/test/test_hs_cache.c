...
test_directory(void *arg)
{
     ...
     * clean, found with a lookup and then cleaned by our OOM. */
    {
        ...
        ret = hs_cache_lookup_as_dir(3, helper_get_hsdir_query(desc1),
                                     &desc_out);
        tt_int_op(ret, OP_EQ, 1);
        tt_str_op(desc_out, OP_EQ, desc1_str);
        /* Tell our OOM to run and to at least remove a byte which will result
         * in removing the descriptor from our cache. */
    ...
    /* Store two descriptors and remove the expiring one only. */
    {
        ...
        ret = hs_cache_lookup_as_dir(
            3, helper_get_hsdir_query(desc_zero_lifetime), NULL);
        tt_int_op(ret, OP_EQ, 0);
    ...
    /* Test descriptor replacement with revision counter. */
    {
        ...
        ret = hs_desc_encode_descriptor(desc1, &signing_kp1, NULL,
                                        &new_desc_str);
        tt_int_op(ret, OP_EQ, 0);
...
test_hsdir_revision_counter_check(void *arg)
{
    ...
    /* Fetch the published descriptor and validate the revision counter. */
    {
        ...
        retval = hs_desc_decode_descriptor(received_desc_str, &subcredential,
                                           NULL, &received_desc);
...
test_client_cache(void *arg)
{
     ...
     * cleaned since we still in the same TP. */
    {
        ...
        tt_mem_op(cached_desc->subcredential.subcred, OP_EQ,
                  wanted_subcredential.subcred, SUBCRED_LEN);
    }
...
test_client_cache_decrypt(void *arg)
{
    ...
    search_desc_encoded =
        hs_cache_lookup_encoded_as_client(&service_kp.pubkey);
