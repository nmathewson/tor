...
test_compat_libevent_logging_callback(void *ignored)
{
    ...
    mock_clean_saved_logs();
    libevent_logging_callback(_EVENT_LOG_DEBUG, "012345678901234567890123456789"
...
test_compat_libevent_postloop_events(void *arg)
{
    ...
    timed =
        periodic_timer_new(tor_libevent_get_base(), &fifty_ms, increment_int_counter_cb, &counter);
...
struct testcase_t compat_libevent_tests[] = {
    ...
    {"postloop_events", test_compat_libevent_postloop_events, TT_FORK, NULL, NULL},
    END_OF_TESTCASES};
