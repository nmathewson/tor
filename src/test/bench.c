...
#define MICROCOUNT(start, end, iters) (NANOCOUNT((start), (end), (iters)) / 1000.0)
...
bench_aes(void)
{
    ...
    for (len = 1; len <= 8192; len *= 2) {
        ...
        tor_free(b1);
        tor_free(b2);
        printf("%d bytes: %.2f nsec per byte\n", len, NANOCOUNT(start, end, iters * len));
    }
...
bench_onion_TAP(void)
{
    ...
    for (i = 0; i < iters; ++i) {
        ...
        onion_skin_TAP_server_handshake(os, key, NULL, or, key_out, sizeof(key_out));
    }
    end = perftime();
    ...
    for (i = 0; i < iters; ++i) {
        char key_out[CPATH_KEY_MATERIAL_LEN];
        onion_skin_TAP_server_handshake(os, key2, key, or, key_out, sizeof(key_out));
    }
    end = perftime();
    ...
    for (i = 0; i < iters; ++i) {
        ...
        s = onion_skin_TAP_client_handshake(dh, or, key_out, sizeof(key_out), NULL);
        crypto_dh_free(dh);
        tor_assert(s == 0);
    }
    end = perftime();
...
bench_onion_ntor_impl(void)
{
    ...
    for (i = 0; i < iters; ++i) {
        ...
        onion_skin_ntor_server_handshake(os, keymap, NULL, nodeid, or, key_out, sizeof(key_out));
    }
    end = perftime();
    ...
    for (i = 0; i < iters; ++i) {
        uint8_t key_out[CPATH_KEY_MATERIAL_LEN];
        int s;
        s = onion_skin_ntor_client_handshake(state, or, key_out, sizeof(key_out), NULL);
        tor_assert(s == 0);
    }
    end = perftime();
    printf("Client-side, part 2: %f usec.\n", NANOCOUNT(start, end, iters) / 1e3);
...
bench_ed25519_impl(void)
{
    ...
    start = perftime();
    for (i = 0; i < iters; ++i) {
        ed25519_public_key_from_curve25519_public_key(&pubkey_tmp, &curve_kp.pubkey, 1);
    }
    end = perftime();
...
bench_dmap(void)
{
    ...
    for (i = 0; i < iters; ++i) {
        SMARTLIST_FOREACH(sl, const char *, cp, n += digestset_probably_contains(ds, cp));
        SMARTLIST_FOREACH(sl2, const char *, cp, n += digestset_probably_contains(ds, cp));
    }
    end = perftime();
...
bench_cell_ops(void)
{
    ...
    for (outbound = 0; outbound <= 1; ++outbound) {
        ...
        for (i = 0; i < iters; ++i) {
            ...
            relay_decrypt_cell(TO_CIRCUIT(or_circ), cell, d, &layer_hint, &recognized);
        }
        end = perftime();
...
bench_dh(void)
{
    ...
    for (i = 0; i < iters; ++i) {
        ...
        slen_a = crypto_dh_compute_secret(LOG_NOTICE, dh_a, dh_pubkey_b, sizeof(dh_pubkey_b),
                                          secret_a, sizeof(secret_a));
        slen_b = crypto_dh_compute_secret(LOG_NOTICE, dh_b, dh_pubkey_a, sizeof(dh_pubkey_a),
...
bench_ecdh_impl(int nid, const char *name)
{
    ...
    for (i = 0; i < iters; ++i) {
        ...
        slen_a =
            ECDH_compute_key(secret_a, DH1024_KEY_LEN, EC_KEY_get0_public_key(dh_b), dh_a, NULL);
        slen_b =
            ECDH_compute_key(secret_b, DH1024_KEY_LEN, EC_KEY_get0_public_key(dh_a), dh_b, NULL);
...
bench_md_parse(void)
{
    ...
    const char md_text[] = "@last-listed 2018-12-14 18:14:14\n"
                           "onion-key\n"
    ...
    start = perftime();
    for (int i = 0; i < N; ++i) {
        smartlist_t *s = microdescs_parse_from_string(md_text, NULL, 1, SAVED_IN_CACHE, NULL);
...
static struct benchmark_t benchmarks[] = {ENT(dmap),      ENT(siphash),    ENT(digest),  ENT(aes),
