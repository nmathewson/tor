...
bench_onion_ntor_impl(void)
{
    ...
    for (i = 0; i < iters; ++i) {
        uint8_t key_out[CPATH_KEY_MATERIAL_LEN];
        int s;
        s = onion_skin_ntor_client_handshake(state, or, key_out,
                                             sizeof(key_out), NULL);
        tor_assert(s == 0);
    }
    end = perftime();
...
bench_cell_aes(void)
{
    ...
    for (misalign = 0; misalign <= max_misalign; ++misalign) {
        ...
        printf("%d bytes, misaligned by %d: %.2f nsec per byte\n", len,
               misalign, NANOCOUNT(start, end, iters * len));
    }
            ...
            for (int j = 0; j < N; ++j) {
                switch (alg) {
                    ...
                    case DIGEST_SHA3_256:
                        failures +=
                            crypto_digest256(out, buf, lens[i], alg) < 0;
                        break;
                    case DIGEST_SHA512:
                    case DIGEST_SHA3_512:
                        failures +=
                            crypto_digest512(out, buf, lens[i], alg) < 0;
                        break;
                    default:
                        tor_assert(0);
                }
...
bench_cell_ops(void)
{
    ...
    for (outbound = 0; outbound <= 1; ++outbound) {
        ...
        printf(
            "%sbound cells: %.2f ns per cell. (%.2f ns per byte of payload)\n",
