 ...
 * writes to outbuf. */
static void
connection_write_to_buf_impl_replacement(const char *string, size_t len, connection_t *conn,
                                         int compressed)
{
    (void)compressed;
...
test_ext_or_write_command(void *arg)
{
    ...
    (void)arg;
    MOCK(connection_write_to_buf_impl_, connection_write_to_buf_impl_replacement);

    c1 = or_connection_new(CONN_TYPE_EXT_OR, AF_INET);
    tt_assert(c1);

    /* Length too long */
    tt_int_op(connection_write_ext_or_command(TO_CONN(c1), 100, "X", 100000), OP_LT, 0);

    /* Empty command */
    tt_int_op(connection_write_ext_or_command(TO_CONN(c1), 0x99, NULL, 0), OP_EQ, 0);
    ...
    /* Medium command. */
    tt_int_op(connection_write_ext_or_command(TO_CONN(c1), 0x99, "Wai\0Hello", 9), OP_EQ, 0);
    cp = buf_get_contents(TO_CONN(c1)->outbuf, &sz);
    ...
    memset(buf, 'x', 65535);
    tt_int_op(connection_write_ext_or_command(TO_CONN(c1), 0xf00d, buf, 65535), OP_EQ, 0);
    cp = buf_get_contents(TO_CONN(c1)->outbuf, &sz);
...
static int
write_bytes_to_file_fail(const char *fname, const char *str, size_t len, int bin)
{
    (void)fname;
    (void)str;
    (void)len;
...
test_ext_or_cookie_auth(void *arg)
{
    ...
    /* Wrong length */
    tt_int_op(-1, OP_EQ,
              handle_client_auth_nonce(client_nonce, 33, &client_hash, &reply, &reply_len));
    tt_int_op(-1, OP_EQ,
              handle_client_auth_nonce(client_nonce, 31, &client_hash, &reply, &reply_len));

    /* Now let's try this for real! */
    tt_int_op(0, OP_EQ,
              handle_client_auth_nonce(client_nonce, 32, &client_hash, &reply, &reply_len));
    ...
    crypto_hmac_sha256(hmac1, (char *)ext_or_auth_cookie, 32, server_hash_input, 46 + 32 + 32);
    crypto_hmac_sha256(hmac2, (char *)ext_or_auth_cookie, 32, client_hash_input, 46 + 32 + 32);
    ...
    tt_int_op(0, OP_EQ,
              handle_client_auth_nonce(client_nonce, 32, &client_hash2, &reply2, &reply_len));
...
test_ext_or_handshake(void *arg)
{
    ...
    tt_int_op(0, OP_EQ, connection_ext_or_process_inbuf(conn));
    CONTAINS("", 0);
    tt_int_op(TO_CONN(conn)->state, OP_EQ, EXT_OR_CONN_STATE_AUTH_WAIT_CLIENT_NONCE);
...
struct testcase_t extorport_tests[] = {
    ...
    {"cookie_auth_testvec", test_ext_or_cookie_auth_testvec, TT_FORK, NULL, NULL},
