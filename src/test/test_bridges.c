...
test_bridges_find_bridge_by_digest_known(void *arg)
{
    ...
    /* We have to call bridge_get_rsa_id_digest() here because the
     * bridge_info_t struct is opaquely defined in bridges.h. */
...
test_bridges_bridge_resolve_conflicts(void *arg)
{
    ...
    bridge_resolve_conflicts((const tor_addr_t *)addr, port, digest,
                             transport);

    /* The bridge should now be marked for removal, and removed when we sweep
     * the bridge_list */
    sweep_bridge_list();
...
test_bridges_get_transport_by_bridge_addrport_no_ptlist(void *arg)
{
    ...
    /* This will fail because the global transport_list has nothing in it, and
     * so transport_get_by_name() has nothing to return, even the the bridge
     * *did* say it had an obfs4 transport.
     */
