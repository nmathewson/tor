...
test_proto_http_valid(void *arg)
{
    ...
    for (i = 0; i < ARRAY_LENGTH(cases); ++i) {
        ...
        if (cases[i].should_detect_truncated) {
            tt_int_op(0, OP_EQ, fetch_from_buf_http(buf, &h, 1024 * 16, &b, &bl, 1024 * 16, 0));
        ...
        tt_int_op(1, OP_EQ, fetch_from_buf_http(buf, &h, 1024 * 16, &b, &bl, 1024 * 16, 0));
...
struct testcase_t proto_http_tests[] = {{"peek", test_proto_http_peek, 0, NULL, NULL},
