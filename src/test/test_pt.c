...
test_pt_parsing(void *arg)
{
    ...
    tt_int_op(1, OP_EQ, smartlist_len(mp->transports));
    {
        ...
        tt_str_op(transport_->extra_info_args, OP_EQ, "counterweight=3,sling=snappy");
    }
    reset_mp(mp);
...
test_pt_get_extrainfo_string(void *arg)
{
    ...
    tt_int_op(r, OP_EQ, 0);
    r = parse_smethod_line("SMETHOD celine 127.0.0.1:1723 ARGS:card=no-enemy", mp2);
    tt_int_op(r, OP_EQ, 0);
...
test_pt_configure_proxy(void *arg)
{
    ...
    { /* check that the transport info were saved properly in the tor state */
        ...
        smartlist_split_string(transport_info_sl, transport_in_state->value, NULL, 0, 0);
...
struct testcase_t pt_tests[] = {
    PT_LEGACY(parsing),
    PT_LEGACY(protocol),
    {"get_transport_options", test_pt_get_transport_options, TT_FORK, NULL, NULL},
    {"get_extrainfo_string", test_pt_get_extrainfo_string, TT_FORK, NULL, NULL},
