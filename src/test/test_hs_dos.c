...
setup_mock_consensus(void)
{
    ...
    smartlist_add(current_ns_consensus->net_params, (void *)"HiddenServiceEnableIntroDoSDefense=1");
...
test_validate_dos_extension_params(void *arg)
{
    ...
    ret = cell_dos_extension_parameters_are_valid(get_intro2_rate_consensus_param(NULL),
                                                  get_intro2_burst_consensus_param(NULL));
    tt_assert(ret);
...
struct testcase_t hs_dos_tests[] = {
    ...
    {"validate_dos_extension_params", test_validate_dos_extension_params, TT_FORK, NULL, NULL},
