...
test_add_onion_helper_keyarg_v3(void *arg)
{
    ...
    {
        /* The base64 key and hex key are the same. Hex key is 64 bytes long.
         * The sk has been generated randomly using python3. */
...
test_rend_service_parse_port_config(void *arg)
{
    ...
    rend_service_port_config_free(cfg);
    cfg =
        rend_service_parse_port_config("80,[2001:db8::1]:8080", sep, &err_msg);
    tt_assert(cfg);
    tt_ptr_op(err_msg, OP_EQ, NULL);
    rend_service_port_config_free(cfg);
    cfg = NULL;
...
test_add_onion_helper_clientauth(void *arg)
{
    ...
    tor_free(reply_str);
    client =
        add_onion_helper_clientauth("no*asterisks*allowed", &created, NULL);
...
test_download_status_consensus(void *arg)
{
    ...
    getinfo_helper_downloads(
        &dummy, "downloads/networkstatus/microdesc/running", &answer, &errmsg);
    tt_ptr_op(answer, OP_NE, NULL);
    tt_ptr_op(errmsg, OP_EQ, NULL);
