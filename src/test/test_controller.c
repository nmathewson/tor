...
test_add_onion_helper_keyarg_v2(void *arg)
{
    ...
    tor_free(reply_str);
    ret = add_onion_helper_keyarg("NEW:RSA1024", 0, &key_new_alg,
                                  &key_new_blob, &pk, &hs_version, NULL);
    tt_int_op(ret, OP_EQ, 0);
    ...
    pk.v2 = NULL;
    tor_free(key_new_blob);
    ret = add_onion_helper_keyarg("NEW:RSA1024", 1, &key_new_alg,
                                  &key_new_blob, &pk, &hs_version, NULL);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(hs_version, OP_EQ, HS_VERSION_TWO);
    tt_assert(pk.v2);
...
test_current_time(void *arg)
{
    ...
    getinfo_helper_current_time(&dummy, "current-time/local", &answer,
                                &errmsg);
    tt_ptr_op(answer, OP_NE, NULL);
    tt_ptr_op(errmsg, OP_EQ, NULL);
