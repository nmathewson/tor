...
#define PACK_CELL(id, cmd, body_s)                         \
    do {                                                   \
        ...
        memcpy(cell.payload + RELAY_HEADER_SIZE, (body_s), \
               sizeof((body_s)) - 1);                      \
...
#define ASSERT_COUNTED_BW()                                   \
    do {                                                      \
        tt_int_op(circ->n_delivered_read_circ_bw, OP_EQ,      \
                  delivered + rh.length);                     \
...
subtest_halfstream_insertremove(int num)
{
    ...
    tt_int_op(connection_half_edge_is_valid_data(circ->half_streams, 23),
              OP_EQ, 0);
    ...
    tt_int_op(connection_half_edge_is_valid_data(circ->half_streams, 22),
              OP_EQ, 0);
    ...
    tt_int_op(connection_half_edge_is_valid_data(circ->half_streams, 24),
              OP_EQ, 0);
...
test_circbw_relay(void *arg)
{
    ...
    /* Recording a cell, the window is updated after decryption so off by one
     * in order to record and then we process it with the proper window. */
...
#define ASSERT_RESOLVED_CALLED(atype, answer, ttl, expires)           \
    ...
    ASSERT_MARK_CALLED(END_STREAM_REASON_DONE |
                       END_STREAM_REASON_FLAG_ALREADY_SOCKS_REPLIED);
    ASSERT_RESOLVED_CALLED(RESOLVED_TYPE_HOSTNAME, "www.example.com", 65536,
                           -1);
