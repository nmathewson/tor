...
test_channel_state(void *arg)
{
    ...
    tt_int_op(channel_listener_state_is_valid(CHANNEL_LISTENER_STATE_CLOSED),
              OP_EQ, 1);
    tt_int_op(
        channel_listener_state_is_valid(CHANNEL_LISTENER_STATE_LISTENING),
        OP_EQ, 1);
    ...
    tt_int_op(
        channel_listener_state_can_transition(
            CHANNEL_LISTENER_STATE_CLOSED, CHANNEL_LISTENER_STATE_LISTENING),
        OP_EQ, 1);
    ...
    tt_int_op(
        channel_listener_state_can_transition(CHANNEL_LISTENER_STATE_CLOSING,
    ...
    tt_int_op(
        ...
        OP_EQ, 1);
    tt_int_op(
        channel_listener_state_can_transition(CHANNEL_LISTENER_STATE_ERROR,
    ...
    tt_int_op(
        ...
        OP_EQ, 1);
    tt_int_op(channel_listener_state_can_transition(
                  CHANNEL_LISTENER_STATE_LAST, INT_MAX),
              OP_EQ, 0);
    ...
    tt_str_op(
        channel_listener_state_to_string(CHANNEL_LISTENER_STATE_LISTENING),
        OP_EQ, "listening");
...
test_channel_for_extend(void *arg)
{
    ...
    /* Same creation time, num circuits will be used and they both have 0 so
     * the channel 2 should be picked due to how channel_is_better() works. */
    ...
    ret_chan = channel_get_for_extend(digest, &dumb_ed_id, &ipv4_addr,
                                      &ipv6_addr, &msg, &launch);
    tt_assert(!ret_chan);
