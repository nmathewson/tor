...
static int
mock_export_key_material(tor_tls_t *tls, uint8_t *secrets_out, const uint8_t *context,
...
test_link_handshake_certs_ok(void *arg)
{
     ...
     */
    tt_int_op(tor_tls_context_init(TOR_TLS_CTX_IS_PUBLIC_SERVER, key1, key2, 86400), OP_EQ, 0);
    ...
    tt_int_op(certs_cell_get_certs(cc1, 0)->cert_type, OP_EQ, CERTTYPE_RSA1024_ID_AUTH);
    tt_int_op(certs_cell_get_certs(cc1, 1)->cert_type, OP_EQ, CERTTYPE_RSA1024_ID_ID);

    tt_int_op(certs_cell_get_certs(cc2, 0)->cert_type, OP_EQ, CERTTYPE_RSA1024_ID_LINK);
    tt_int_op(certs_cell_get_certs(cc2, 1)->cert_type, OP_EQ, CERTTYPE_RSA1024_ID_ID);

    if (with_ed) {
        tt_int_op(certs_cell_get_certs(cc1, 2)->cert_type, OP_EQ, CERTTYPE_ED_ID_SIGN);
        tt_int_op(certs_cell_get_certs(cc1, 3)->cert_type, OP_EQ, CERTTYPE_ED_SIGN_AUTH);
        tt_int_op(certs_cell_get_certs(cc1, 4)->cert_type, OP_EQ, CERTTYPE_RSA1024_ID_EDID);

        tt_int_op(certs_cell_get_certs(cc2, 2)->cert_type, OP_EQ, CERTTYPE_ED_ID_SIGN);
        tt_int_op(certs_cell_get_certs(cc2, 3)->cert_type, OP_EQ, CERTTYPE_ED_SIGN_LINK);
        tt_int_op(certs_cell_get_certs(cc2, 4)->cert_type, OP_EQ, CERTTYPE_RSA1024_ID_EDID);
    }
    ...
    tt_assert(!fast_mem_is_zero((char *)c1->handshake_state->authenticated_rsa_peer_id, 20));
    ...
    tt_assert(fast_mem_is_zero((char *)c2->handshake_state->authenticated_rsa_peer_id, 20));
...
recv_certs_setup(const struct testcase_t *test)
{
    ...
    tt_int_op(tor_tls_context_init(TOR_TLS_CTX_IS_PUBLIC_SERVER, d->key1, d->key2, 86400), OP_EQ,
              0);
    if (is_ed) {
    ...
    if (is_ed) {
        ...
        memcpy(certs_cell_cert_getarray_body(ccc3), id_sign->encoded, ccc3->cert_len);
        memcpy(certs_cell_cert_getarray_body(ccc4), secondary->encoded, ccc4->cert_len);
...
static struct testcase_setup_t setup_recv_certs = {.setup_fn = recv_certs_setup,
...
CERTS_FAIL(badstate, require_failure_message = "We're not doing a v3 handshake!";
           d->c->base_.state = OR_CONN_STATE_CONNECTING;)
CERTS_FAIL(badproto, require_failure_message = "not using link protocol >= 3"; d->c->link_proto = 2)
CERTS_FAIL(duplicate, require_failure_message = "We already got one";
           d->c->handshake_state->received_certs_cell = 1)
CERTS_FAIL(already_authenticated, require_failure_message = "We're already authenticated!";
           d->c->handshake_state->authenticated = 1)
CERTS_FAIL(empty, require_failure_message = "It had no body"; d->cell->payload_len = 0)
CERTS_FAIL(bad_circid, require_failure_message = "It had a nonzero circuit ID";
           d->cell->circ_id = 1)
CERTS_FAIL(truncated_1, require_failure_message = "It couldn't be parsed"; d->cell->payload[0] = 5)
...
CERTS_FAIL(missing_signing_key, /* ed25519 */
           {
               ...
               tor_cert_t *bad_cert =
                   tor_cert_create(mk, CERT_TYPE_ID_SIGNING, &sk->pubkey, time(NULL), 86400,
...
CERTS_FAIL(wrong_labels_2, {
    ...
    memcpy(certs_cell_cert_getarray_body(certs_cell_get_certs(d->ccell, 1)), enca, lena);
...
test_link_handshake_send_authchallenge(void *arg)
{
    ...
    tt_int_op(tor_tls_context_init(TOR_TLS_CTX_IS_PUBLIC_SERVER, rsa0, rsa1, 86400), OP_EQ, 0);
...
AUTHCHALLENGE_FAIL(badstate, require_failure_message = "We're not currently doing a "
                                                       "v3 handshake";
                   d->c->base_.state = OR_CONN_STATE_CONNECTING)
AUTHCHALLENGE_FAIL(badproto, require_failure_message = "not using link protocol >= 3";
                   d->c->link_proto = 2)
AUTHCHALLENGE_FAIL(as_server, require_failure_message = "We didn't originate this "
                                                        "connection";
                   d->c->handshake_state->started_here = 0;
                   d->c->handshake_state->certs->started_here = 0;)
AUTHCHALLENGE_FAIL(duplicate, require_failure_message = "We already received one";
                   d->c->handshake_state->received_auth_challenge = 1)
AUTHCHALLENGE_FAIL(nocerts, require_failure_message = "We haven't gotten a CERTS "
                                                      "cell yet";
                   d->c->handshake_state->received_certs_cell = 0)
AUTHCHALLENGE_FAIL(tooshort, require_failure_message = "It was not well-formed";
                   d->cell->payload_len = 33)
AUTHCHALLENGE_FAIL(truncated, require_failure_message = "It was not well-formed";
                   d->cell->payload_len = 34)
AUTHCHALLENGE_FAIL(nonzero_circid, require_failure_message = "It had a nonzero circuit ID";
                   d->cell->circ_id = 1337)
...
static void
mock_set_circid_type(channel_t *chan, crypto_pk_t *identity_rcvd, int consider_identity)
{
    (void)chan;
    (void)identity_rcvd;
...
authenticate_data_setup(const struct testcase_t *test)
{
    ...
    if (is_ed) {
        d->c1->handshake_state->certs->ed_id_sign = tor_cert_dup(get_master_signing_key_cert());
        d->c2->handshake_state->certs->ed_id_sign = tor_cert_dup(get_master_signing_key_cert());
        d->c2->handshake_state->certs->ed_sign_auth = tor_cert_dup(get_current_auth_key_cert());
    } else {
...
static struct testcase_setup_t setup_authenticate = {.setup_fn = authenticate_data_setup,
...
test_link_handshake_auth_cell(void *arg)
{
    ...
    tt_int_op(ntohs(get_uint16(d->cell->payload + 2)), OP_EQ, d->cell->payload_len - 4);

    /* Check it out for plausibility... */
    auth_ctx_t ctx;
    ctx.is_ed = d->is_ed;
    tt_int_op(d->cell->payload_len - 4, OP_EQ,
              auth1_parse(&auth1, d->cell->payload + 4, d->cell->payload_len - 4, &ctx));
    tt_assert(auth1);
    ...
    if (d->is_ed) {
        ...
        tt_assert(!ed25519_checksig(&sig, start, end - start, &get_current_auth_keypair()->pubkey));
    } else {
        uint8_t sig[128];
        uint8_t digest[32];
        ...
        int n =
            crypto_pk_public_checksig(auth_pubkey, (char *)sig, sizeof(sig),
                                      (char *)auth1_getarray_sig(auth1), auth1_getlen_sig(auth1));
        tt_int_op(n, OP_EQ, 32);
        crypto_digest256((char *)digest, (const char *)start, end - start, DIGEST_SHA256);
...
AUTHENTICATE_FAIL(badstate, require_failure_message = "We're not doing a v3 handshake";
                  d->c2->base_.state = OR_CONN_STATE_CONNECTING)
AUTHENTICATE_FAIL(badproto, require_failure_message = "not using link protocol >= 3";
                  d->c2->link_proto = 2)
AUTHENTICATE_FAIL(atclient, require_failure_message = "We originated this connection";
...
AUTHENTICATE_FAIL(nocerts, require_failure_message = "We never got a certs cell";
                  d->c2->handshake_state->received_certs_cell = 0)
AUTHENTICATE_FAIL(noidcert, require_failure_message = "We never got an identity "
                                                      "certificate";
                  tor_x509_cert_free(d->c2->handshake_state->certs->id_cert);
                  d->c2->handshake_state->certs->id_cert = NULL)
AUTHENTICATE_FAIL(noauthcert, require_failure_message = "We never got an RSA "
...
AUTHENTICATE_FAIL(badtype, require_failure_message = "Authenticator type was not "
                                                     "recognized";
                  d->cell->payload[0] = 0xff)
AUTHENTICATE_FAIL(truncated_1, require_failure_message = "Authenticator was truncated";
                  d->cell->payload[2]++)
AUTHENTICATE_FAIL(truncated_2, require_failure_message = "Authenticator was truncated";
                  d->cell->payload[3]++)
AUTHENTICATE_FAIL(tooshort_1, require_failure_message = "Authenticator was too short";
                  tt_int_op(d->cell->payload_len, OP_GE, 260); d->cell->payload[2] -= 1;
...
AUTHENTICATE_FAIL(badsig_1,
                  if (d->is_ed) require_failure_message = "Ed25519 signature wasn't valid";
...
#    define TEST_RSA(name, flags)                                                         \
        {                                                                                 \
#            name, test_link_handshake_##name, (flags), &passthrough_setup, (void *)"RSA" \
        }

#    define TEST_ED(name, flags)                                                       \
        {                                                                              \
#            name "_ed25519", test_link_handshake_##name, (flags), &passthrough_setup, \
                (void *)"Ed25519"                                                      \
        }

#    define TEST_RCV_AUTHCHALLENGE(name)                                                         \
        {                                                                                        \
            "recv_authchallenge/" #name, test_link_handshake_recv_authchallenge_##name, TT_FORK, \
...
#    define TEST_RCV_CERTS_ED(name, type)                                                   \
        {                                                                                   \
            "recv_certs/" #name "_ed25519", test_link_handshake_recv_certs_##name, TT_FORK, \
                &setup_recv_certs, (void *)type                                             \
        }

#    define TEST_AUTHENTICATE(name)                                                               \
        {                                                                                         \
            "authenticate/" #name, test_link_handshake_auth_##name, TT_FORK, &setup_authenticate, \
                NULL                                                                              \
        }

#    define TEST_AUTHENTICATE_ED(name)                                                  \
        {                                                                               \
            "authenticate/" #name "_ed25519", test_link_handshake_auth_##name, TT_FORK, \
                &setup_authenticate, (void *)3                                          \
        }
#endif /* !defined(COCCI) */
...
struct testcase_t link_handshake_tests[] = {
    ...
    TEST_RCV_CERTS_RSA(ok_server, "RSA-Auth"), TEST_RCV_CERTS_ED(ok_server, "Ed25519-Auth"),
    TEST_RCV_CERTS(badstate), TEST_RCV_CERTS(badproto), TEST_RCV_CERTS(duplicate),
    TEST_RCV_CERTS(already_authenticated), TEST_RCV_CERTS(empty), TEST_RCV_CERTS(bad_circid),
    TEST_RCV_CERTS(truncated_1), TEST_RCV_CERTS(truncated_2), TEST_RCV_CERTS(truncated_3),
    TEST_RCV_CERTS_ED(truncated_4, "Ed25519-Link"), TEST_RCV_CERTS_ED(truncated_5, "Ed25519-Link"),
    TEST_RCV_CERTS_ED(truncated_6, "Ed25519-Link"), TEST_RCV_CERTS_ED(truncated_7, "Ed25519-Link"),
    TEST_RCV_CERTS(not_x509), TEST_RCV_CERTS(both_link), TEST_RCV_CERTS(both_id_rsa),
    ...
    TEST_RCV_CERTS_ED(link_mismatch, "Ed25519-Link"), TEST_RCV_CERTS_ED(bad_ed_sig, "Ed25519-Link"),
    ...
    TEST_RCV_CERTS_ED(expired_crosscert, "Ed25519-Link"), TEST_RCV_CERTS(wrong_labels_1),
    TEST_RCV_CERTS(wrong_labels_2), TEST_RCV_CERTS(wrong_labels_3),
    TEST_RCV_CERTS(server_missing_certs), TEST_RCV_CERTS(server_wrong_labels_1),
    ...
    TEST_AUTHENTICATE(cell), TEST_AUTHENTICATE_ED(cell), TEST_AUTHENTICATE(badstate),
    TEST_AUTHENTICATE(badproto), TEST_AUTHENTICATE(atclient), TEST_AUTHENTICATE(duplicate),
    TEST_AUTHENTICATE(already_authenticated), TEST_AUTHENTICATE(nocerts),
    TEST_AUTHENTICATE(noidcert), TEST_AUTHENTICATE(noauthcert), TEST_AUTHENTICATE(tooshort),
    TEST_AUTHENTICATE(badtype), TEST_AUTHENTICATE(truncated_1), TEST_AUTHENTICATE(truncated_2),
    TEST_AUTHENTICATE(tooshort_1), TEST_AUTHENTICATE(badcontent), TEST_AUTHENTICATE(badsig_1),
    TEST_AUTHENTICATE_ED(badsig_1), TEST_AUTHENTICATE_ED(missing_ed_id),
