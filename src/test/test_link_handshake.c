...
test_link_handshake_certs_ok(void *arg)
{
    ...
    if (with_ed) {
        ...
        mock_peer_cert = tor_x509_cert_dup(linkc); /* and when we do, the
                                                    * peer's cert is this... */
    }
...
recv_certs_setup(const struct testcase_t *test)
{
    ...
    if (is_ed) {
        ...
        const tor_cert_t *secondary = is_link ? get_current_link_cert_cert()
...
CERTS_FAIL(truncated_6, /* ed25519 */
           {
               ...
               certs_cell_cert_setlen_body(certs_cell_get_certs(d->ccell, 3),
                                           7);
...
CERTS_FAIL(truncated_7, /* ed25519 */
           {
               ...
               certs_cell_cert_setlen_body(certs_cell_get_certs(d->ccell, 4),
                                           7);
...
CERTS_FAIL(bad_ed_sig, /* ed25519 */
           {
               require_failure_message =
                   "At least one Ed25519 certificate was "
                   "badly signed";
...
CERTS_FAIL(bad_rsa_id_cert, /*ed25519*/
           {
                ...
                * won't let us just frob the old cert, since it will see that
                * the issuer & serial number are the same, which will make it
                * fail at an earlier stage than signature verification.) */
               const tor_x509_cert_t *idc;
...
CERTS_FAIL(expired_rsa_id, /* both */
           {
               ...
               certs_cell_cert_setlen_body(cert, encoded_len);
               certs_cell_cert_set_cert_len(cert, encoded_len);
               memcpy(certs_cell_cert_getarray_body(cert), encoded,
                      encoded_len);
               REENCODE();
               tor_x509_cert_free(newc);
           })
CERTS_FAIL(expired_ed_id, /* ed25519 */
           {
               ...
               /* We don't need to re-sign, since we check for expiration
                * first. */
               certs_cell_cert_t *cert = certs_cell_get_certs(d->ccell, 2);
               uint8_t *body = certs_cell_cert_getarray_body(cert);
               /* The expiration field is bytes [2..5].  It is in HOURS since
                * the epoch. */
...
test_link_handshake_auth_cell(void *arg)
{
    ...
    tt_int_op(d->cell->payload_len - 4, OP_EQ,
              auth1_parse(&auth1, d->cell->payload + 4,
    ...
    tt_mem_op(auth1->tlssecrets, OP_EQ, "int getRandomNumber(){return 4;}",
              32);
    ...
    if (d->is_ed) {
        ...
        int n = crypto_pk_public_checksig(
            auth_pubkey, (char *)sig, sizeof(sig),
...
#    define TEST_RCV_CERTS(name)                                        \
        {                                                               \
            "recv_certs/" #name, test_link_handshake_recv_certs_##name, \
                TT_FORK, &setup_recv_certs, (void *)"RSA-Link"          \
        }
...
#    define TEST_AUTHENTICATE_ED(name)                    \
        {                                                 \
            "authenticate/" #name "_ed25519",             \
                test_link_handshake_auth_##name, TT_FORK, \
