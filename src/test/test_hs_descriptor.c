...
test_cert_encoding(void *arg)
{
    ...
    /* Validated the certificate string. */
    {
         ...
         * 23:36", and the expiration date of the cert was two hours, the Tor
         * code will ceiling that and make it 02:00. Make sure that the right
...
test_decode_descriptor(void *arg)
{
    ...
    ret = hs_desc_decode_descriptor("hladfjlkjadf", &subcredential, NULL,
                                    &decoded);
    ...
    /* Decode a descriptor with _no_ introduction points. */
    {
        ...
        ret =
            hs_desc_decode_descriptor(encoded, &subcredential, NULL, &decoded);
        tt_int_op(ret, OP_EQ, HS_DESC_DECODE_OK);
        tt_assert(decoded);
    }
    ...
    /* Decode a descriptor with auth clients. */
    {
        ...
        crypto_strongest_rand(descriptor_cookie,
...
test_supported_version(void *arg)
{
    ...
    /* To early. */
    ret =
        hs_desc_is_supported_version(HS_DESC_SUPPORTED_FORMAT_VERSION_MIN - 1);
    tt_int_op(ret, OP_EQ, 0);
    /* One too new. */
    ret =
        hs_desc_is_supported_version(HS_DESC_SUPPORTED_FORMAT_VERSION_MAX + 1);
    tt_int_op(ret, OP_EQ, 0);
    /* Valid version. */
...
test_decode_invalid_intro_point(void *arg)
{
    ...
    /* Invalid auth key type. */
    {
        ...
        const char *bad_line =
            "auth-key\n"
            "-----BEGIN UNICORN CERT-----\n"
            "MIGJAoGBAO4bATcW8kW4h6RQQAKEgg+aXCpF4JwbcO6vGZtzXTDB+"
            "HdPVQzwqkbh\n"
            "XzFM6VGArhYw4m31wcP1Z7IwULir7UMnAFd7Zi62aYfU6l+"
            "Y1yAoZ1wzu1XBaAMK\n"
