...
test_cert_encoding(void *arg)
{
    ...
    cert = tor_cert_create(&kp, CERT_TYPE_SIGNING_AUTH, &signed_key, now, 3600 * 2,
...
#define PADDING_EXPECTED_LEN(l) \
    CEIL_DIV(l, HS_DESC_SUPERENC_PLAINTEXT_PAD_MULTIPLE) * HS_DESC_SUPERENC_PLAINTEXT_PAD_MULTIPLE
    ...
    { /* test #1: no padding */
        ...
        tt_int_op(
            fast_mem_is_zero((char *)padded_plaintext + plaintext_len, padded_len - plaintext_len),
            OP_EQ, 1);
...
test_encode_descriptor(void *arg)
{
...

    {
        ...
        ret = hs_desc_encode_descriptor(desc, &signing_kp, descriptor_cookie, &encoded);
        tt_int_op(ret, OP_EQ, 0);
        tt_assert(encoded);
...
test_decode_descriptor(void *arg)
{
    ...
    ret = hs_desc_decode_descriptor("hladfjlkjadf", &subcredential, NULL, &decoded);
    ...
    /* Decode a descriptor with _no_ introduction points. */
    {
        ...
        tt_int_op(ret, OP_EQ, 0);
        hs_helper_get_subcred_from_identity_keypair(&signing_kp_no_ip, &subcredential);
        desc_no_ip = hs_helper_build_hs_desc_no_ip(&signing_kp_no_ip);
        tt_assert(desc_no_ip);
        tor_free(encoded);
        ret = hs_desc_encode_descriptor(desc_no_ip, &signing_kp_no_ip, NULL, &encoded);
        tt_int_op(ret, OP_EQ, 0);
        tt_assert(encoded);
    ...
    /* Decode a descriptor with auth clients. */
    {
        ...
        hs_desc_build_authorized_client(&subcredential, &client_kp.pubkey,
                                        &auth_ephemeral_kp.seckey, descriptor_cookie, client);
        ...
        hs_descriptor_free(decoded);
        ret =
            hs_desc_decode_descriptor(encoded, &subcredential, &invalid_client_kp.seckey, &decoded);
        ...
        ret = hs_desc_decode_descriptor(encoded, &subcredential, &client_kp.seckey, &decoded);
        tt_int_op(ret, OP_EQ, HS_DESC_DECODE_OK);
        tt_assert(decoded);
...
test_decode_invalid_intro_point(void *arg)
{
    ...
    const char *intro_point = "introduction-point AQIUMDI5OUYyNjhGQ0E5RDU1Q0QxNTc=";
    const char *auth_key = "auth-key\n"
                           "-----BEGIN ED25519 CERT-----\n"
                           ...
                           "-----END ED25519 CERT-----";
    const char *enc_key = "enc-key ntor bpZKLsuhxP6woDQ3yVyjm5gUKSk7RjfAijT2qrzbQk0=";
    const char *enc_key_cert = "enc-key-cert\n"
                               "-----BEGIN ED25519 CERT-----\n"
    ...
    {
        smartlist_t *lines = smartlist_new();
        const char *bad_line = "enc-key unicorn bpZKLsuhxP6woDQ3yVyjm5gUKSk7RjfAijT2qrzbQk0=";
        /* Build intro point text. */
        smartlist_add(lines, (char *)intro_point);
        smartlist_add(lines, (char *)auth_key);
        smartlist_add(lines, (char *)bad_line);
...
test_build_authorized_client(void *arg)
{
    ...
    base16_decode((char *)&auth_ephemeral_sk, sizeof(auth_ephemeral_sk), ephemeral_sk_b16,
                  strlen(ephemeral_sk_b16));

    base16_decode((char *)descriptor_cookie, sizeof(descriptor_cookie), descriptor_cookie_b16,
                  strlen(descriptor_cookie_b16));

    base16_decode((char *)&client_auth_pk, sizeof(client_auth_pk), client_pubkey_b16,
                  strlen(client_pubkey_b16));

    testing_enable_prefilled_rng("\x01", 1);

    hs_desc_build_authorized_client(&subcredential, &client_auth_pk, &auth_ephemeral_sk,
    ...
    test_memeq_hex((char *)desc_client->encrypted_cookie, "B21222BE13F385F355BD07B2381F9F29");
...
struct testcase_t hs_descriptor[] = {
    ...
    {"decode_invalid_intro_point", test_decode_invalid_intro_point, TT_FORK, NULL, NULL},
    ...
    {"build_authorized_client", test_build_authorized_client, TT_FORK, NULL, NULL},
