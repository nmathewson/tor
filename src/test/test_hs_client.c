 ...
 * hidden service. */
static int
helper_get_circ_and_stream_for_test(origin_circuit_t **circ_out, connection_t **conn_out,
                                    int is_legacy)
{
    int retval;
    ...
    if (is_legacy) {
        ...
        or_circ->build_state->pending_final_cpath = tor_malloc_zero(sizeof(crypt_path_t));
        ...
        tt_assert(or_circ->build_state->pending_final_cpath->rend_dh_handshake_state);
...
test_e2e_rend_circuit_setup_legacy(void *arg)
{
    ...
    MOCK(connection_ap_handshake_send_begin, mock_connection_ap_handshake_send_begin);
    ...
    uint8_t rend_cell_body[DH1024_KEY_LEN + DIGEST_LEN] = {2};
    {
        ...
        crypto_dh_t *dh_state = or_circ->build_state->pending_final_cpath->rend_dh_handshake_state;
        /* compute and overwrite digest of cell body with the right value */
        retval =
            crypto_dh_compute_secret(LOG_PROTOCOL_WARN, dh_state, (char *)rend_cell_body,
                                     DH1024_KEY_LEN, keys, DIGEST_LEN + CPATH_KEY_MATERIAL_LEN);
        tt_int_op(retval, OP_GT, 0);
        memcpy(rend_cell_body + DH1024_KEY_LEN, keys, DIGEST_LEN);
    }

    /* Setup the circuit */
    retval = hs_circuit_setup_e2e_rend_circ_legacy_client(or_circ, rend_cell_body);
    tt_int_op(retval, OP_EQ, 0);
    ...
    tt_int_op(crypto_digest_get_algorithm(or_circ->cpath->pvt_crypto.f_digest), OP_EQ, DIGEST_SHA1);
    tt_int_op(crypto_digest_get_algorithm(or_circ->cpath->pvt_crypto.b_digest), OP_EQ, DIGEST_SHA1);
...
test_e2e_rend_circuit_setup(void *arg)
{
    ...
    /* Setup the circuit */
    retval = hs_circuit_setup_e2e_rend_circ(or_circ, ntor_key_seed, sizeof(ntor_key_seed), 0);
    tt_int_op(retval, OP_EQ, 0);
...
test_client_pick_intro(void *arg)
{
    ...
    /* 1) Add desc to HS cache */
    {
        ...
        const hs_descriptor_t *fetched_desc = hs_cache_lookup_as_client(&service_kp.pubkey);
        tt_assert(fetched_desc);
        tt_mem_op(fetched_desc->subcredential.subcred, OP_EQ, desc->subcredential.subcred,
                  SUBCRED_LEN);
        tt_assert(!fast_mem_is_zero((char *)fetched_desc->subcredential.subcred, DIGEST256_LEN));
        tor_free(encoded);
    }
     ...
     *   desc and get a random intro: check that we got _the chosen one_. */
    {
        ...
        extend_info_t *chosen_intro_ei = desc_intro_point_to_extend_info(chosen_intro_point);
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            tt_mem_op(ip->identity_digest, OP_EQ, chosen_intro_ei->identity_digest, DIGEST_LEN);
            extend_info_free(ip);
        }
     ...
     *    nothing is returned. */
    {
        ...
        SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points, hs_desc_intro_point_t *, ip) {
            ...
            if (intro_ei == NULL) {
                ...
                intro_ei = hs_get_extend_info_from_lspecs(ip->link_specifiers, &ip->onion_key, 1);
...
static void
mock_connection_mark_unattached_ap_(entry_connection_t *conn, int endreason, int line,
                                    const char *file)
{
    (void)line;
    (void)file;
...
static void
mock_connection_mark_unattached_ap_no_close(entry_connection_t *conn, int endreason, int line,
                                            const char *file)
{
    (void)conn;
    (void)endreason;
...
test_descriptor_fetch(void *arg)
{
    ...
    MOCK(networkstatus_get_live_consensus, mock_networkstatus_get_live_consensus_false);
    ret = hs_client_refetch_hsdesc(&service_pk);
...
test_parse_auth_file_content(void *arg)
{
    ...
    /* Wrong auth type. */
    tt_assert(
        !parse_auth_file_content("4acth47i6kxnvkewtm6q7ib2s3ufpo5sqbsnzjpbi7utijcltosqemad:x:"
                                 "x25519:zdsyvn2jq534ugyiuzgjy4267jbtzcjbsgedhshzx5mforyxtryq"));
    /* Wrong key type. */
...
mock_read_file_to_str(const char *filename, int flags, struct stat *stat_out)
{
    ...
    if (!strcmp(filename, get_fname("auth_keys" PATH_SEPARATOR "client1.auth_private"))) {
        ret = tor_strdup("4acth47i6kxnvkewtm6q7ib2s3ufpo5sqbsnzjpbi7utijcltosqemad:descriptor:"
                         "x25519:zdsyvn2jq534ugyiuzgjy4267jbtzcjbsgedhshzx5mforyxtryq");
        goto done;
    }

    if (!strcmp(filename, get_fname("auth_keys" PATH_SEPARATOR "dummy.xxx"))) {
        ret = tor_strdup("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx:descriptor:"
                         "x25519:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
        goto done;
    }

    if (!strcmp(filename, get_fname("auth_keys" PATH_SEPARATOR "client2.auth_private"))) {
        ret = tor_strdup("25njqamcweflpvkl73j4szahhihoc4xt3ktcgjnpaingr5yhkenl5sid:descriptor:"
...
#undef conf_fmt
    ...
    hs_parse_address("4acth47i6kxnvkewtm6q7ib2s3ufpo5sqbsnzjpbi7utijcltosqemad", &pk1, NULL, NULL);
    hs_parse_address("25njqamcweflpvkl73j4szahhihoc4xt3ktcgjnpaingr5yhkenl5sid", &pk2, NULL, NULL);
...
helper_build_socks_connection(const ed25519_public_key_t *service_pk, int conn_state)
{
...
test_desc_has_arrived_cleanup(void *arg)
{
     ...
     * descriptor state. */
    socks1 = helper_build_socks_connection(&signing_kp.pubkey, AP_CONN_STATE_RENDDESC_WAIT);
    tt_assert(socks1);
    socks2 = helper_build_socks_connection(&signing_kp.pubkey, AP_CONN_STATE_RENDDESC_WAIT);
    tt_assert(socks2);
    ...
    SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points, hs_desc_intro_point_t *, ip) {
        hs_cache_client_intro_state_note(&signing_kp.pubkey, &ip->auth_key_cert->signed_key,
                                         INTRO_POINT_FAILURE_GENERIC);
    } SMARTLIST_FOREACH_END (ip);
    ...
    hs_ident_dir_conn_init(&signing_kp.pubkey, &desc->plaintext_data.blinded_pubkey, &hs_dir_ident);
...
test_close_intro_circuits_new_desc(void *arg)
{
     ...
     * decryptable. Make sure the close circuit code path is not triggered. */
    {
        ...
        desc = hs_helper_build_hs_desc_with_client_auth(descriptor_cookie, &client_kp.pubkey,
                                                        &service_kp);
        tt_assert(desc);
        ret = hs_desc_encode_descriptor(desc, &service_kp, descriptor_cookie, &desc_encoded);
        tt_int_op(ret, OP_EQ, 0);
        /* Associate descriptor intro key with the dummy circuit. */
        const hs_desc_intro_point_t *ip = smartlist_get(desc->encrypted_data.intro_points, 0);
        tt_assert(ip);
        ocirc->hs_ident = hs_ident_circuit_new(&service_kp.pubkey);
        ed25519_pubkey_copy(&ocirc->hs_ident->intro_auth_pk, &ip->auth_key_cert->signed_key);
        hs_descriptor_free(desc);
    ...
    {
        const hs_desc_intro_point_t *ip = smartlist_get(desc1->encrypted_data.intro_points, 0);
        tt_assert(ip);
        ocirc->hs_ident = hs_ident_circuit_new(&service_kp.pubkey);
        ed25519_pubkey_copy(&ocirc->hs_ident->intro_auth_pk, &ip->auth_key_cert->signed_key);
    }
    ...
    /* Build the second descriptor for the same service and cache it. */
    {
         ...
         * bigger. */
        desc2->plaintext_data.revision_counter = desc1->plaintext_data.revision_counter + 1;
...
test_socks_hs_errors(void *arg)
{
    ...
    MOCK(networkstatus_get_live_consensus, mock_networkstatus_get_live_consensus);
    MOCK(connection_mark_unattached_ap_, mock_connection_mark_unattached_ap_no_close);
    MOCK(read_file_to_str, mock_read_file_to_str);
    MOCK(tor_listdir, mock_tor_listdir);
    MOCK(check_private_dir, mock_check_private_dir);
    ...
    socks_conn = helper_build_socks_connection(&service_kp.pubkey, AP_CONN_STATE_RENDDESC_WAIT);
    ...
    SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points, hs_desc_intro_point_t *, ip) {
        hs_cache_client_intro_state_note(&service_kp.pubkey, &ip->auth_key_cert->signed_key,
                                         INTRO_POINT_FAILURE_GENERIC);
    } SMARTLIST_FOREACH_END (ip);

    hs_client_dir_fetch_done(dir_conn, "Reason", desc_encoded, 200);
    tt_int_op(socks_conn->socks_request->socks_extended_error_code, OP_EQ, SOCKS5_HS_INTRO_FAILED);
    ...
    tt_int_op(socks_conn->socks_request->socks_extended_error_code, OP_EQ, SOCKS5_HS_REND_FAILED);
    ...
    ret = hs_client_decode_descriptor(desc_encoded, &service_kp.pubkey, &desc_out);
    tt_int_op(ret, OP_EQ, HS_DESC_DECODE_NEED_CLIENT_AUTH);
...
test_close_intro_circuit_failure(void *arg)
{
    ...
    circuit_mark_for_close(circ, END_CIRC_REASON_TIMEOUT);
    tt_assert(!hs_cache_client_intro_state_find(&service_kp.pubkey, &intro_kp.pubkey));
    /* Time to free. It should get removed. */
    circuit_free(circ);
    entry = hs_cache_client_intro_state_find(&service_kp.pubkey, &intro_kp.pubkey);
    tt_assert(entry);
...
struct testcase_t hs_client_tests[] = {
    {"e2e_rend_circuit_setup_legacy", test_e2e_rend_circuit_setup_legacy, TT_FORK, NULL, NULL},
    ...
    {"auth_key_filename_is_valid", test_auth_key_filename_is_valid, TT_FORK, NULL, NULL},
    {"parse_auth_file_content", test_parse_auth_file_content, TT_FORK, NULL, NULL},
    {"config_client_authorization", test_config_client_authorization, TT_FORK, NULL, NULL},
    {"desc_has_arrived_cleanup", test_desc_has_arrived_cleanup, TT_FORK, NULL, NULL},
    {"close_intro_circuit_failure", test_close_intro_circuit_failure, TT_FORK, NULL, NULL},
    {"close_intro_circuits_new_desc", test_close_intro_circuits_new_desc, TT_FORK, NULL, NULL},
    {"close_intro_circuits_cache_clean", test_close_intro_circuits_cache_clean, TT_FORK, NULL,
     NULL},
    ...
    {"purge_ephemeral_client_auth", test_purge_ephemeral_client_auth, TT_FORK, NULL, NULL},
