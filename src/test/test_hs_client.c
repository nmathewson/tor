                                    ...
                                    connection_t **conn_out, int is_legacy)
{
    ...
    if (is_legacy) {
        ...
        tt_assert(or_circ->build_state->pending_final_cpath
                      ->rend_dh_handshake_state);
        retval =
            crypto_dh_generate_public(or_circ->build_state->pending_final_cpath
                                          ->rend_dh_handshake_state);
...
test_e2e_rend_circuit_setup_legacy(void *arg)
{
    ...
    /** In this test we create a v2 legacy HS stream and a circuit with the
     * same hidden service destination. We make the stream wait for circuits to
     * be established to the hidden service, and then we complete the circuit
     * using the hs_circuit_setup_e2e_rend_circ_legacy_client() function. We
     * then check that the end-to-end cpath was setup correctly and that the
     * stream was attached to the circuit as expected. */
    ...
    uint8_t rend_cell_body[DH1024_KEY_LEN + DIGEST_LEN] = {2};
    {
        ...
        retval = crypto_dh_compute_secret(
            LOG_PROTOCOL_WARN, dh_state, (char *)rend_cell_body,
...
test_e2e_rend_circuit_setup(void *arg)
{
    ...
    /** In this test we create a prop224 v3 HS stream and a circuit with the
     * same hidden service destination. We make the stream wait for circuits to
     * be established to the hidden service, and then we complete the circuit
     * using the hs_circuit_setup_e2e_rend_circ() function. We then check that
     * the end-to-end cpath was setup correctly and that the stream was
     * attached to the circuit as expected. */
...
test_client_pick_intro(void *arg)
{
     ...
     *
     * 5) Set StrictNodes and put all our intro points in ExcludeNodes: see
    ...
    /* 1) Add desc to HS cache */
    {
        ...
        tt_mem_op(fetched_desc->subcredential.subcred, OP_EQ,
                  desc->subcredential.subcred, SUBCRED_LEN);
        tt_assert(!fast_mem_is_zero(
            (char *)fetched_desc->subcredential.subcred, DIGEST256_LEN));
        tor_free(encoded);
    }

    /* 2) Mark all intro points except _the chosen one_ as failed. Then query
     * the desc and get a random intro: check that we got _the chosen one_. */
    {
        /* Tell hs_get_extend_info_from_lspecs() to skip the private address
         * check.
         */
        ...
        SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points,
            ...
            ed25519_public_key_t *intro_auth_key =
        ...
        /* (We try several times, to make sure this behavior is consistent, and
         * to cover the different cases of client_get_random_intro().) */
        for (int i = 0; i < 64; ++i) {
            extend_info_t *ip = client_get_random_intro(&service_kp.pubkey);
            tor_assert(ip);
            tt_assert(
                !fast_mem_is_zero((char *)ip->identity_digest, DIGEST_LEN));
            tt_mem_op(ip->identity_digest, OP_EQ,
                      chosen_intro_ei->identity_digest, DIGEST_LEN);
            extend_info_free(ip);
        }
    ...
    /* 3) Clean the intro state cache and get an intro point */
    {
         ...
         * state. This should clean up the intro point failures and allow us to
         * get an intro. */
     ...
     * that nothing is returned. */
    {
        ...
        SMARTLIST_FOREACH_BEGIN (desc->encrypted_data.intro_points,
            ...
            /* desc_intro_point_to_extend_info() doesn't return IPv6 intro
             * points yet, because we can't extend to them. See #24404, #24451,
             * and #24181.
             */
            if (intro_ei == NULL) {
                /* Pretend we're making a direct connection, and that we can
                 * use IPv6
                 */
            ...
            if (intro_ei) {
                ...
                /* We need to decorate in case it is an IPv6 else
                 * routerset_parse() doesn't like it. */
                ptr = tor_addr_to_str(ip_addr, &intro_ei->addr,
                                      sizeof(ip_addr), 1);
...
#undef conf_fmt
    ...
    hs_parse_address(
        "4acth47i6kxnvkewtm6q7ib2s3ufpo5sqbsnzjpbi7utijcltosqemad", &pk1, NULL,
        NULL);
    hs_parse_address(
        "25njqamcweflpvkl73j4szahhihoc4xt3ktcgjnpaingr5yhkenl5sid", &pk2, NULL,
        NULL);
...
test_desc_has_arrived_cleanup(void *arg)
{
    ...
    /* Create two SOCKS connection for the same .onion both in the waiting for
     * a descriptor state. */
    ...
    connection_free_minimal(TO_CONN(dir_conn));
    tt_int_op(socks1->edge_.end_reason, OP_EQ,
              END_STREAM_REASON_RESOLVEFAILED);
    tt_int_op(socks2->edge_.end_reason, OP_EQ,
              END_STREAM_REASON_RESOLVEFAILED);
...
test_close_intro_circuits_new_desc(void *arg)
{
     ...
     * decryptable. Make sure the close circuit code path is not triggered. */
    {
        ...
        /* Clean cache with a future timestamp. It will trigger the clean up
         * and attempt to close the circuit but only if the descriptor is
         * decryptable. Cache object should be removed and circuit untouched.
         */
...
test_socks_hs_errors(void *arg)
{
    ...
    /* The caching will fail to decrypt because the descriptor_cookie used
     * above is not known to the HS subsystem. This will lead to a missing
     * client auth. */
    hs_client_dir_fetch_done(dir_conn, "Reason", desc_encoded, 200);
...
test_close_intro_circuit_failure(void *arg)
{
    ...
    /* Again, create and add to the global list a dummy client introduction
     * circuit at the INTRODUCING state but without a chosen_exit. In theory,
     * it can not happen but we'll make sure it doesn't end up in the failure
     * cache anyway. */
    circ = dummy_origin_circuit_new(0);
    tt_assert(circ);
    circ->purpose = CIRCUIT_PURPOSE_C_INTRODUCING;
...
test_purge_ephemeral_client_auth(void *arg)
{
    ...
    curve25519_secret_key_generate(&auth->enc_seckey, 0);
    hs_build_address(&service_kp.pubkey, HS_VERSION_THREE,
                     auth->onion_address);
    auth->flags = 0;
