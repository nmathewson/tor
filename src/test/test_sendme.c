...
test_v1_record_digest(void *arg)
{
     ...
     * in order to catched the CIRCWINDOW_INCREMENT-nth cell. Try something
     * that shouldn't be noted. */
...
test_v1_build_cell(void *arg)
{
    ...
    smartlist_add(circ->sendme_last_digests,
                  tor_memdup(digest, sizeof(digest)));
    ...
    expect_log_msg_containing(
        "We received a SENDME but we have no cell digests "
...
test_package_payload_len(void *arg)
{
     ...
     * sent a sufficiently random cell, we will not force this one to have a
     * gap.
     */
