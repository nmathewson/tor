...
test_libscrypt_eq_openssl(void *arg)
{
    ...
    libscrypt_retval = libscrypt_scrypt(
        ...
        (const uint8_t *)"NaCl", strlen("NaCl"), N, r, p, buf1, dk_len);
    openssl_retval =
    ...
    libscrypt_retval = libscrypt_scrypt(
        ...
        buf1, dk_len);
    openssl_retval = EVP_PBE_scrypt(
        (const char *)"pleaseletmein", strlen("pleaseletmein"),
        (const unsigned char *)"SodiumChloride", strlen("SodiumChloride"), N,
        r, p, maxmem, buf2, dk_len);
...
test_crypto_s2k_errors(void *arg)
{
    ...
    tt_int_op(S2K_BAD_LEN, OP_EQ,
              secret_to_key_derivekey(
                  buf, sizeof(buf), (const uint8_t *)"\x01\x02", 2, "ABC", 3));
    ...
    buf[0] = 2;
    tt_int_op(S2K_BAD_LEN, OP_EQ,
              secret_to_key_derivekey(buf2, sizeof(buf2), buf, sizeof(buf),
                                      "ABC", 3));
    ...
    /* It's a bad pbkdf2 buffer if it has an iteration count that would
     * overflow int32_t. */
    memset(buf, 0, sizeof(buf));
    buf[0] = 1; /* pbkdf2 */
...
test_crypto_pwbox(void *arg)
{
    ...
    for (i = 0; i < ARRAY_LENGTH(flags); ++i) {
        tt_int_op(0, OP_EQ,
                  crypto_pwbox(&boxed, &len, (const uint8_t *)msg, strlen(msg),
...
test_crypto_ed25519_fuzz_donna(void *arg)
{
    ...
    for (i = 0; i < iters; ++i) {
        ...
        tt_int_op(0, OP_EQ,
                  ed25519_keypair_from_curve25519_keypair(&kp_curve25519, &bit,
                                                          &ckp));
        ...
        tt_int_op(0, OP_EQ,
                  ed25519_public_blind(&pk_blind, &kp.pubkey, blinding));
        ...
        tt_mem_op(pk_curve25519.pubkey, OP_EQ, kp_curve25519.pubkey.pubkey,
                  32);
    }
