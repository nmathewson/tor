...
test_environment(void *arg)
{
    ...
    env = process_get_environment(process);
    tt_mem_op(env->windows_environment_block, OP_EQ, "FOO=bar\0HELLO=world\0", 20);
...
test_stringified_types(void *arg)
{
    ...
    tt_str_op("not running", OP_EQ, process_status_to_string(PROCESS_STATUS_NOT_RUNNING));
    tt_str_op("running", OP_EQ, process_status_to_string(PROCESS_STATUS_RUNNING));
...
test_line_protocol_simple(void *arg)
{
    ...
    tt_str_op(smartlist_get(process_data->stdout_data, 0), OP_EQ, "Hello stdout");
    tt_str_op(smartlist_get(process_data->stderr_data, 0), OP_EQ, "Hello stderr");
...
test_line_protocol_multi(void *arg)
{
    ...
    /* Check if the data is correct. */
    tt_str_op(smartlist_get(process_data->stdout_data, 0), OP_EQ, "Hello stdout");
    tt_str_op(smartlist_get(process_data->stdout_data, 1), OP_EQ, "Onion Onion Onion");
    ...
    tt_str_op(smartlist_get(process_data->stderr_data, 2), OP_EQ, "Onion Onion Onion");
