...
test_win32(void *arg)
{
     ...
     * MSDN but we don't exercise all quoting rules because
     * tor_join_win_cmdline will try to only generate simple cases for the
     * child process to parse;
    ...
    for (i = 0; cmdlines[i] != NULL; i++) {
        log_info(LD_GENERAL, "Joining argvs[%d], expecting <%s>", i,
                 cmdlines[i]);
