...
test_pe_launch(void *arg)
{
  ...
  for (int i = 0; periodic_events[i].name; ++i) {
    ...
    int mask =
        PERIODIC_EVENT_ROLE_CLIENT | PERIODIC_EVENT_ROLE_ALL | PERIODIC_EVENT_ROLE_NET_PARTICIPANT;
    for (int i = 0; mainloop_periodic_events[i].name; ++i) {
        periodic_event_item_t *item = &mainloop_periodic_events[i];
    ...
    tt_uint_op(roles, OP_EQ,
               PERIODIC_EVENT_ROLE_RELAY | PERIODIC_EVENT_ROLE_DIRSERVER | PERIODIC_EVENT_ROLE_ALL |
...
test_pe_get_roles(void *arg)
{
    ...
    const int ALL = PERIODIC_EVENT_ROLE_ALL | PERIODIC_EVENT_ROLE_NET_PARTICIPANT;
    ...
    roles = get_my_roles(options);
    tt_int_op(roles, OP_EQ,
              (PERIODIC_EVENT_ROLE_CLIENT | PERIODIC_EVENT_ROLE_RELAY | PERIODIC_EVENT_ROLE_BRIDGE |
               PERIODIC_EVENT_ROLE_DIRSERVER | ALL));
    tt_assert(roles & PERIODIC_EVENT_ROLE_ROUTER);
    /* Unset client so we can solely test Router role. */
    options->SocksPort_set = 0;
    roles = get_my_roles(options);
    tt_int_op(roles, OP_EQ, PERIODIC_EVENT_ROLE_ROUTER | PERIODIC_EVENT_ROLE_DIRSERVER | ALL);
    ...
    options->V3AuthoritativeDir = 1;
    roles = get_my_roles(options);
    tt_int_op(roles, OP_EQ, PERIODIC_EVENT_ROLE_DIRAUTH | PERIODIC_EVENT_ROLE_DIRSERVER | ALL);
    ...
    options->BridgeAuthoritativeDir = 1;
    roles = get_my_roles(options);
    tt_int_op(roles, OP_EQ, PERIODIC_EVENT_ROLE_BRIDGEAUTH | PERIODIC_EVENT_ROLE_DIRSERVER | ALL);
    tt_assert(roles & PERIODIC_EVENT_ROLE_AUTHORITIES);
    ...
    tt_int_op(roles, OP_EQ,
              (PERIODIC_EVENT_ROLE_BRIDGEAUTH | PERIODIC_EVENT_ROLE_RELAY |
               PERIODIC_EVENT_ROLE_HS_SERVICE | PERIODIC_EVENT_ROLE_DIRSERVER | ALL));
    tt_assert(roles & PERIODIC_EVENT_ROLE_AUTHORITIES);
...
struct testcase_t periodic_event_tests[] = {PE_TEST(initialize), PE_TEST(launch),
                                            PE_TEST(get_roles), PE_TEST(hs_service),
