...
test_crypto_dh(void *arg)
{
    ...
    s1len = crypto_dh_compute_secret(LOG_WARN, dh1_dup, p2, DH1024_KEY_LEN, s1, 50);
...

    {
        ...
        s1len = crypto_dh_compute_secret(LOG_WARN, dh1, p1, DH1024_KEY_LEN, s1, 50);
        tt_int_op(-1, OP_EQ, s1len);
        ...
        const char P_plus_one[] = "FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD129024E08"
                                  "8A67CC74020BBEA63B139B22514A08798E3404DDEF9519B3CD3A431B"
                                  "302B0A6DF25F14374FE1356D6D51C245E485B576625E7EC6F44C42E9"
                                  "A637ED6B0BFF5CB6F406B7EDEE386BFB5A899FA5AE9F24117C4B1FE6"
...

    {
        ...
        s1len = crypto_dh_compute_secret(LOG_WARN, dh2, p1, DH1024_KEY_LEN, s1, 50);
...
#    endif /* defined(OPENSSL_1_1_API) */
        ...
        s1len =
            crypto_dh_handshake(LOG_WARN, dh3, p2, DH1024_KEY_LEN, (unsigned char *)s1, sizeof(s1));
...
test_crypto_openssl_version(void *arg)
{
    ...
    if (strcmpstart(version, h_version)) { /* "-fips" suffix, etc */
        TT_DIE(("OpenSSL library version %s did not begin with header version %s.", version,
                h_version));
    }
...
#define RFC_4231_MAX_KEY_SIZE 131
    ...
    crypto_hmac_sha256(digest, "key", 3, "The quick brown fox jumps over the lazy dog", 43);
    tt_str_op(hex_str(digest, 32), OP_EQ,
    ...
    base16_decode(key, 25, "0102030405060708090a0b0c0d0e0f10111213141516171819", 50);
    memset(data, 0xcd, 50);
    ...
    memset(key, 0xaa, 131);
    crypto_hmac_sha256(digest, key, 131, "Test Using Larger Than Block-Size Key - Hash Key First",
                       54);
...
test_crypto_sha3(void *arg)
{
    ...
    const uint8_t keccak_kat_msg1080[] = {
        ,
    };
    ...
    const uint8_t keccak_kat_msg1088[] = {
        ,
    };
    ...
    const uint8_t keccak_kat_msg1096[] = {
        ,
    };
    ...
    const uint8_t keccak_kat_msg1144[] = {
        ,
    };
    ...
    const uint8_t keccak_kat_msg1152[] = {
        ,
    };
    ...
    const uint8_t keccak_kat_msg1160[] = {
        ,
    };
...
test_crypto_sha3_xof(void *arg)
{
    ...
    base16_decode((char *)msg, 255,
                  ...
                  510);
    const char *squeezed_hex = "8A5199B4A7E133E264A86202720655894D48CFF344A928CF8347F48379CE"
...
test_crypto_mac_sha3(void *arg)
{
    ...
    crypto_mac_sha3_256(hmac_test, sizeof(hmac_test), (uint8_t *)key, strlen(key), (uint8_t *)msg,
                        strlen(msg));
    ...
    /* Now let's try a manual H(len(k) || k || m) construction */
    {
        ...
        memcpy(all + sizeof(key_len_netorder), key_msg_concat, strlen(key_msg_concat));
...
test_crypto_pk(void *arg)
{
    ...
    tt_int_op(128, OP_EQ,
              crypto_pk_public_encrypt(pk2, data1, sizeof(data1), "Hello whirled.", 15,
                                       PK_PKCS1_OAEP_PADDING));
    tt_int_op(128, OP_EQ,
              crypto_pk_public_encrypt(pk1, data2, sizeof(data1), "Hello whirled.", 15,
                                       PK_PKCS1_OAEP_PADDING));
    /* oaep padding should make encryption not match */
    tt_mem_op(data1, OP_NE, data2, 128);
    tt_int_op(
        15, OP_EQ,
        crypto_pk_private_decrypt(pk1, data3, sizeof(data3), data1, 128, PK_PKCS1_OAEP_PADDING, 1));
    tt_str_op(data3, OP_EQ, "Hello whirled.");
    memset(data3, 0, 1024);
    tt_int_op(
        15, OP_EQ,
        crypto_pk_private_decrypt(pk1, data3, sizeof(data3), data2, 128, PK_PKCS1_OAEP_PADDING, 1));
    tt_str_op(data3, OP_EQ, "Hello whirled.");
    /* Can't decrypt with public key. */
    tt_int_op(
        -1, OP_EQ,
        crypto_pk_private_decrypt(pk2, data3, sizeof(data3), data2, 128, PK_PKCS1_OAEP_PADDING, 1));
    ...
    tt_int_op(crypto_pk_read_private_key_from_filename(pk2, get_fname("xyzzy")), OP_LT, 0);
    write_str_to_file(get_fname("xyzzy"), "foobar", 6);
    /* Failing case for read: no key. */
    tt_int_op(crypto_pk_read_private_key_from_filename(pk2, get_fname("xyzzy")), OP_LT, 0);
    tt_assert(!crypto_pk_read_private_key_from_filename(pk2, get_fname("pkey1")));
    tt_int_op(
        15, OP_EQ,
        crypto_pk_private_decrypt(pk2, data3, sizeof(data3), data1, 128, PK_PKCS1_OAEP_PADDING, 1));

    /* Now try signing. */
    strlcpy(data1, "Ossifrage", 1024);
    tt_int_op(128, OP_EQ, crypto_pk_private_sign(pk1, data2, sizeof(data2), data1, 10));
    tt_int_op(10, OP_EQ, crypto_pk_public_checksig(pk1, data3, sizeof(data3), data2, 128));
    tt_str_op(data3, OP_EQ, "Ossifrage");
    /* Try signing digests. */
    tt_int_op(128, OP_EQ, crypto_pk_private_sign_digest(pk1, data2, sizeof(data2), data1, 10));
    tt_int_op(20, OP_EQ, crypto_pk_public_checksig(pk1, data3, sizeof(data3), data2, 128));
    tt_int_op(0, OP_EQ, crypto_pk_public_checksig_digest(pk1, data1, 10, data2, 128));
    tt_int_op(-1, OP_EQ, crypto_pk_public_checksig_digest(pk1, data1, 11, data2, 128));
    ...
    for (i = 85; i < 140; ++i) {
        memset(data2, 0, 1024);
        memset(data3, 0, 1024);
        len = crypto_pk_obsolete_public_hybrid_encrypt(pk1, data2, sizeof(data2), data1, i,
                                                       PK_PKCS1_OAEP_PADDING, 0);
        tt_int_op(len, OP_GE, 0);
        len = crypto_pk_obsolete_private_hybrid_decrypt(pk1, data3, sizeof(data3), data2, len,
...
test_crypto_pk_pem_encrypted(void *arg)
{
       ...
       that's encrypted with a password. */
    {
        const char *s = "-----BEGIN RSA PRIVATE KEY-----\n"
                        ...
                        "-----END RSA PRIVATE KEY-----\n";
        tt_int_op(-1, OP_EQ, crypto_pk_read_private_key_from_string(pk, s, strlen(s)));
    }
    ...
    */
    {
        const char *s = "-----BEGIN RSA PUBLIC KEY-----\n"
                        ...
                        "iRqK\n"
                        "-----END RSA PUBLIC KEY-----\n";
        tt_int_op(-1, OP_EQ, crypto_pk_read_public_key_from_string(pk, s, strlen(s)));
    }
done:
    crypto_pk_free(pk);
}
...
test_crypto_pk_invalid_private_key(void *arg)
{
    ...
    tt_int_op(-1, OP_EQ, crypto_pk_read_private_key_from_string(pk, pem, strlen(pem)));
#ifdef ENABLE_NSS
...
test_crypto_digests(void *arg)
{
    ...
    k = crypto_pk_new();
    tt_assert(k);
    r = crypto_pk_read_private_key_from_string(k, AUTHORITY_SIGNKEY_3, strlen(AUTHORITY_SIGNKEY_3));
    tt_assert(!r);

    r = crypto_pk_get_digest(k, digest);
    tt_assert(r == 0);
    tt_mem_op(hex_str(digest, DIGEST_LEN), OP_EQ, AUTHORITY_SIGNKEY_A_DIGEST, HEX_DIGEST_LEN);

    r = crypto_pk_get_common_digests(k, &pkey_digests);
    tt_int_op(r, OP_EQ, 0);

    tt_mem_op(hex_str(pkey_digests.d[DIGEST_SHA1], DIGEST_LEN), OP_EQ, AUTHORITY_SIGNKEY_A_DIGEST,
              HEX_DIGEST_LEN);
...
test_crypto_aes_iv(void *arg)
{
    ...
    encrypted_size = crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 4095, plain, 4095);
    ...
    decrypted_size =
        crypto_cipher_decrypt_with_iv(key1, decrypted1, 4095, encrypted1, encrypted_size);

    tt_int_op(decrypted_size, OP_EQ, 4095);
    tt_assert(decrypted_size > 0);
    tt_mem_op(plain, OP_EQ, decrypted1, 4095);
    /* Encrypt a second time (with a new random initialization vector). */
    encrypted_size = crypto_cipher_encrypt_with_iv(key1, encrypted2, 16 + 4095, plain, 4095);

    tt_int_op(encrypted_size, OP_EQ, 16 + 4095);
    tt_assert(encrypted_size > 0);
    decrypted_size =
        crypto_cipher_decrypt_with_iv(key1, decrypted2, 4095, encrypted2, encrypted_size);
    tt_int_op(decrypted_size, OP_EQ, 4095);
    tt_assert(decrypted_size > 0);
    ...
    decrypted_size =
        crypto_cipher_decrypt_with_iv(key2, decrypted2, 4095, encrypted1, encrypted_size);
    tt_int_op(decrypted_size, OP_EQ, 4095);
    ...
    encrypted_size = crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 1, plain_1, 1);
    tt_int_op(encrypted_size, OP_EQ, 16 + 1);
    tt_assert(encrypted_size > 0);
    decrypted_size = crypto_cipher_decrypt_with_iv(key1, decrypted1, 1, encrypted1, encrypted_size);
    tt_int_op(decrypted_size, OP_EQ, 1);
    tt_assert(decrypted_size > 0);
    tt_mem_op(plain_1, OP_EQ, decrypted1, 1);
    /* Special length case: 15. */
    encrypted_size = crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 15, plain_15, 15);
    tt_int_op(encrypted_size, OP_EQ, 16 + 15);
    tt_assert(encrypted_size > 0);
    decrypted_size =
        crypto_cipher_decrypt_with_iv(key1, decrypted1, 15, encrypted1, encrypted_size);
    tt_int_op(decrypted_size, OP_EQ, 15);
    tt_assert(decrypted_size > 0);
    tt_mem_op(plain_15, OP_EQ, decrypted1, 15);
    /* Special length case: 16. */
    encrypted_size = crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 16, plain_16, 16);
    tt_int_op(encrypted_size, OP_EQ, 16 + 16);
    tt_assert(encrypted_size > 0);
    decrypted_size =
        crypto_cipher_decrypt_with_iv(key1, decrypted1, 16, encrypted1, encrypted_size);
    tt_int_op(decrypted_size, OP_EQ, 16);
    tt_assert(decrypted_size > 0);
    tt_mem_op(plain_16, OP_EQ, decrypted1, 16);
    /* Special length case: 17. */
    encrypted_size = crypto_cipher_encrypt_with_iv(key1, encrypted1, 16 + 17, plain_17, 17);
    tt_int_op(encrypted_size, OP_EQ, 16 + 17);
    tt_assert(encrypted_size > 0);
    decrypted_size =
        crypto_cipher_decrypt_with_iv(key1, decrypted1, 17, encrypted1, encrypted_size);
...
test_crypto_kdf_TAP(void *arg)
{
    ...
    (void)arg;
#define EXPAND(s) \
    r = crypto_expand_key_material_TAP((const uint8_t *)(s), strlen(s), key_material, 100)
    ...
    EXPAND("");
    tt_int_op(r, OP_EQ, 0);
    test_memeq_hex(key_material, "5ba93c9db0cff93f52b521d7420e43f6eda2784fbf8b4530d8"
    ...
    EXPAND("Tor");
    tt_int_op(r, OP_EQ, 0);
    test_memeq_hex(key_material, "776c6214fc647aaa5f683c737ee66ec44f03d0372e1cce6922"
    ...
    tt_int_op(r, OP_EQ, 0);
    test_memeq_hex(key_material, "a340b5d126086c3ab29c2af4179196dbf95e1c72431419d331"
...
#define EXPAND(s)                                                                                  \
    r = crypto_expand_key_material_rfc5869_sha256((const uint8_t *)(s), strlen(s), salt, salt_len, \
                                                  m_expand, m_expand_len, key_material, 100)

    /* Test vectors generated with ntor_ref.py */
    EXPAND("Tor");
    tt_int_op(r, OP_EQ, 0);
    test_memeq_hex(key_material, "5521492a85139a8d9107a2d5c0d9c91610d0f95989975ebee6"
    ...
    tt_int_op(r, OP_EQ, 0);
    test_memeq_hex(key_material, "a2aa9b50da7e481d30463adb8f233ff06e9571a0ca6ab6df0f"
...
test_crypto_hkdf_sha256_testvecs(void *arg)
{
    ...
    for (i = 0; vecs[i].ikm16; ++i) {
        ...
        int r = crypto_expand_key_material_rfc5869_sha256(
            (const uint8_t *)ikm, ikm_len, (const uint8_t *)salt, salt_len, (const uint8_t *)info,
            info_len, (uint8_t *)okm, okm_len);
        tt_int_op(r, OP_EQ, 0);
...
test_crypto_curve25519_testvec(void *arg)
{
    ...
    const char a16[] = "77076d0a7318a57d3c16c17251b26645df4c2f87ebc0992ab177fba51db92c2a";
    /* Alice's public key, X25519(a, 9): */
    const char a_pub16[] = "8520f0098930a754748b7ddcb43ef75a0dbf3a0d26381af4eba4a98eaa9b4e6a";
    /* Bob's private key, b: */
    const char b16[] = "5dab087e624a8a4b79e17f8b83800ee66f3bb1292618b6fd1c2f8b27ff88e0eb";
    /* Bob's public key, X25519(b, 9): */
    const char b_pub16[] = "de9edb7d7b7dc1b4d35b61c2ece435373f8343c85b78674dadfc7e146f882b4f";
    /* Their shared secret, K: */
    const char k16[] = "4a5d9d5ba4ce2de1728e3bf480350f25e07e21c947d19e3376f09b3c1e161742";
...
test_crypto_curve25519_persist(void *arg)
{
    ...
    tt_mem_op(keypair.pubkey.public_key, OP_EQ, keypair2.pubkey.public_key, CURVE25519_PUBKEY_LEN);
    tt_mem_op(keypair.seckey.secret_key, OP_EQ, keypair2.seckey.secret_key, CURVE25519_SECKEY_LEN);
...
test_crypto_ed25519_simple(void *arg)
{
    ...
    const uint8_t msg2[] = "Microsoft Windows extends the features of the DOS operating system, "
    ...
    /* Test the string-prefixed sign/checksig functions */
    {
        ...
        tt_int_op(0, OP_EQ, ed25519_sign_prefixed(&sig1, msg, msg_len, "always in the mood", &kp1));
        ...
        tt_int_op(0, OP_EQ,
                  ed25519_sign(&manual_sig, (uint8_t *)prefixed_msg, strlen(prefixed_msg), &kp1));
        ...
        tt_int_op(0, OP_EQ,
                  ed25519_checksig_prefixed(&sig1, msg, msg_len, "always in the mood", &pub1));

        /* Test that checksig with wrong prefix fails. */
        tt_int_op(-1, OP_EQ,
                  ed25519_checksig_prefixed(&sig1, msg, msg_len, "always in the moo", &pub1));
        tt_int_op(-1, OP_EQ,
                  ed25519_checksig_prefixed(&sig1, msg, msg_len, "always in the moon", &pub1));
        tt_int_op(-1, OP_EQ,
                  ed25519_checksig_prefixed(&sig1, msg, msg_len, "always in the mood!", &pub1));
    }
...
test_crypto_ed25519_test_vectors(void *arg)
{
    ...
    struct {
        ...
        const char *sig;
        const char *msg;
    } items[] = {/* These test vectors were generated with the "ref" implementation of
...
test_crypto_ed25519_encode(void *arg)
{
    ...
    /* Test known value. */
    tt_int_op(0, OP_EQ,
              ed25519_public_from_base64(&pk, "lVIuIctLjbGZGU5wKMNXxXlSE3cW4kaqkqm04u6pxvM"));
    test_memeq_hex(pk.pubkey, "95522e21cb4b8db199194e7028c357c57952137716e246aa92a9b4e2eea9c6f3");
...
test_crypto_ed25519_convert(void *arg)
{
    ...
    for (i = 0; i < N; ++i) {
        ...
        tt_int_op(
            0, OP_EQ,
            ed25519_keypair_from_curve25519_keypair(&ed25519_keypair, &bit, &curve25519_keypair));
        tt_int_op(0, OP_EQ,
                  ed25519_public_key_from_curve25519_public_key(&ed25519_pubkey,
                                                                &curve25519_keypair.pubkey, bit));
        ...
        tt_int_op(0, OP_EQ, ed25519_checksig(&sig, msg, sizeof(msg), &ed25519_pubkey));

        tt_int_op(-1, OP_EQ, ed25519_checksig(&sig, msg, sizeof(msg) - 1, &ed25519_pubkey));
        sig.sig[0] ^= 15;
        tt_int_op(-1, OP_EQ, ed25519_checksig(&sig, msg, sizeof(msg), &ed25519_pubkey));
    }
...
{
    const uint8_t msg[] = "Eyes I dare not meet in dreams / In death's dream kingdom";
    ...
    for (i = 0; i < N; ++i) {
        ...
        tt_int_op(0, OP_EQ,
                  ed25519_keypair_blind(&ed25519_keypair_blinded, &ed25519_keypair, blinding));

        tt_int_op(0, OP_EQ,
                  ed25519_public_blind(&ed25519_pubkey_blinded, &ed25519_keypair.pubkey, blinding));

        tt_mem_op(ed25519_pubkey_blinded.pubkey, OP_EQ, ed25519_keypair_blinded.pubkey.pubkey, 32);

        tt_int_op(0, OP_EQ, ed25519_sign(&sig, msg, sizeof(msg), &ed25519_keypair_blinded));

        tt_int_op(0, OP_EQ, ed25519_checksig(&sig, msg, sizeof(msg), &ed25519_pubkey_blinded));

        tt_int_op(-1, OP_EQ, ed25519_checksig(&sig, msg, sizeof(msg) - 1, &ed25519_pubkey_blinded));
        sig.sig[0] ^= 15;
        tt_int_op(-1, OP_EQ, ed25519_checksig(&sig, msg, sizeof(msg), &ed25519_pubkey_blinded));
    }
...
test_crypto_ed25519_blinding_fail(void *arg)
{
    ...
    const char badkey[] = "e19c65de75c68cf3b7643ea732ba9eb1a3d20d6d57ba223c2ece1df66feb5af0";
    retval = base16_decode((char *)pub.pubkey, sizeof(pub.pubkey), badkey, strlen(badkey));
    ...
    const char goodkey[] = "4ba2e44760dff4c559ef3c38768c1c14a8a54740c782c8d70803e9d6e3ad8794";
    retval = base16_decode((char *)pub.pubkey, sizeof(pub.pubkey), goodkey, strlen(goodkey));
    tt_int_op(retval, OP_EQ, sizeof(pub.pubkey));
    retval = ed25519_public_blind(&pub_blinded, &pub, param);
...
#define EQ(a, h) test_memeq_hex((const char *)(a), (h))
        ...
        tt_int_op(0, OP_EQ, ed25519_keypair_from_curve25519_keypair(&keypair, &sign, &curvekp));
        tt_int_op(
            0, OP_EQ,
            ed25519_public_key_from_curve25519_public_key(&pkfromcurve, &curvekp.pubkey, sign));
        ...
        /* Blinding */
        tt_int_op(0, OP_EQ, ed25519_keypair_blind(&blind_keypair, &keypair, blinding_param));
...
test_crypto_ed25519_storage(void *arg)
{
    ...
    tt_int_op(0, OP_EQ, ed25519_seckey_write_to_file(&keypair->seckey, fname_1, "foo"));
    tt_int_op(0, OP_EQ, ed25519_pubkey_write_to_file(&keypair->pubkey, fname_2, "bar"));
...
test_crypto_siphash(void *arg)
{
    ...
    */
    ...
    const struct sipkey K = {UINT64_C(0x0706050403020100), UINT64_C(0x0f0e0d0c0b0a0908)};
    uint8_t input[64];
    int i, j;
...
test_crypto_ed25519_validation(void *arg)
{
    ...
    { /* Point with torsion component (order 8l) */
        const char badkey[] = "300ef2e64e588e1df55b48e4da0416ffb64cc85d5b00af6463d5cc6c2b1c185e";
        retval = base16_decode((char *)pub1.pubkey, sizeof(pub1.pubkey), badkey, strlen(badkey));
    ...
    { /* Point with torsion component (order 4l) */
        const char badkey[] = "f43e3a046db8749164c6e69b193f1e942c7452e7d888736f40b98093d814d5e7";
        retval = base16_decode((char *)pub1.pubkey, sizeof(pub1.pubkey), badkey, strlen(badkey));
        tt_int_op(retval, OP_EQ, sizeof(pub1.pubkey));
        tt_int_op(ed25519_validate_pubkey(&pub1), OP_EQ, -1);
    }
    ...
    { /* Point with torsion component (order 2l) */
        const char badkey[] = "c9fff3af0471c28e33e98c2043e44f779d0427b1e37c521a6bddc011ed1869af";
        retval = base16_decode((char *)pub1.pubkey, sizeof(pub1.pubkey), badkey, strlen(badkey));
        tt_int_op(retval, OP_EQ, sizeof(pub1.pubkey));
        tt_int_op(ed25519_validate_pubkey(&pub1), OP_EQ, -1);
    }
    ...
    { /* This one is a good key */
        const char goodkey[] = "4ba2e44760dff4c559ef3c38768c1c14a8a54740c782c8d70803e9d6e3ad8794";
        retval = base16_decode((char *)pub1.pubkey, sizeof(pub1.pubkey), goodkey, strlen(goodkey));
        tt_int_op(retval, OP_EQ, sizeof(pub1.pubkey));
        tt_int_op(ed25519_validate_pubkey(&pub1), OP_EQ, 0);
    }
...
#    define ED25519_TEST_ONE(name, fl, which)                                               \
        {                                                                                   \
#            name "/ed25519_" which, test_crypto_ed25519_##name, (fl), &ed25519_test_setup, \
                (void *)which                                                               \
        }

#    define ED25519_TEST(name, fl) \
        ED25519_TEST_ONE(name, (fl), "donna"), ED25519_TEST_ONE(name, (fl), "ref10")
#endif /* !defined(COCCI) */
...
struct testcase_t crypto_tests[] = {
    ...
    {"aes128_ctr_testvec", test_crypto_aes_ctr_testvec, 0, &passthrough_setup, (void *)"128"},
    {"aes192_ctr_testvec", test_crypto_aes_ctr_testvec, 0, &passthrough_setup, (void *)"192"},
    {"aes256_ctr_testvec", test_crypto_aes_ctr_testvec, 0, &passthrough_setup, (void *)"256"},
    CRYPTO_LEGACY(sha),
    CRYPTO_LEGACY(pk),
    ...
    {"pk_invalid_private_key", test_crypto_pk_invalid_private_key, 0, NULL, NULL},
    ...
    CRYPTO_LEGACY(dh),
    {"aes_iv_AES", test_crypto_aes_iv, TT_FORK, &passthrough_setup, (void *)"aes"},
    {"aes_iv_EVP", test_crypto_aes_iv, TT_FORK, &passthrough_setup, (void *)"evp"},
    ...
    {"curve25519_basepoint", test_crypto_curve25519_basepoint, TT_FORK, NULL, NULL},
