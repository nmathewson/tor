...
test_crypto_dh(void *arg)
{
    ...
    s1len =
        crypto_dh_compute_secret(LOG_WARN, dh1, p2, DH1024_KEY_LEN, s1, 50);
    s2len =
        crypto_dh_compute_secret(LOG_WARN, dh2, p1, DH1024_KEY_LEN, s2, 50);
    tt_assert(s1len > 0);
    ...
    s1len = crypto_dh_compute_secret(LOG_WARN, dh1_dup, p2, DH1024_KEY_LEN, s1,
                                     50);
    ...
    {
        /* provoke an error in the openssl DH_compute_key function; make sure
         * we survive. */
        tt_assert(!crypto_dh_get_public(dh1, p1, DH1024_KEY_LEN));

        crypto_dh_free(dh2);
        dh2 = crypto_dh_new(DH_TYPE_CIRCUIT); /* no private key set */
        s1len = crypto_dh_compute_secret(LOG_WARN, dh2, p1, DH1024_KEY_LEN, s1,
                                         50);
...
#    endif /* defined(OPENSSL_1_1_API) */
        ...
        /* right-pad. */
        BN_bn2bin(pk,
                  (unsigned char *)(p2 + DH1024_KEY_LEN - BN_num_bytes(pk)));
...
test_crypto_openssl_version(void *arg)
{
    ...
    if (strcmpstart(version, h_version)) { /* "-fips" suffix, etc */
        TT_DIE((
            "OpenSSL library version %s did not begin with header version %s.",
            version, h_version));
    }
...
test_crypto_aes_ctr_testvec(void *arg)
{
    ...
    c = crypto_cipher_new_with_iv_and_bits((uint8_t *)key, (uint8_t *)iv,
                                           bits);
...
test_crypto_sha3(void *arg)
{
    ...
    /* Attempt to exercise the incremental hashing code by creating a
     * randomized 30 KiB buffer, and hashing rand[1, 5 * Rate] bytes at a time.
     * SHA3-512 is used because it has a lowest rate of the family (the code is
     * common, but the slower rate exercises more of it).
     */
...
test_crypto_pk(void *arg)
{
    ...
    tt_int_op(
        crypto_pk_read_private_key_from_filename(pk2, get_fname("xyzzy")),
        OP_LT, 0);
...
test_crypto_pk_pem_encrypted(void *arg)
{
       ...
       that's encrypted with a password. */
    {
        const char *s =
            ...
            "\n"
            "5NiEFTkWj31EhrvrYcuiJtQ/iEbABxZULFWFeJ058rb+1izBz5rScqnEacIS/"
            "3Go\n"
...
test_crypto_digest_names(void *arg)
{
    ...
    int i;
    for (i = 0; names[i].n; ++i) {
        tt_str_op(names[i].n, OP_EQ,
                  crypto_digest_algorithm_get_name(names[i].a));
...
test_crypto_formats(void *arg)
{
    ...
    tt_int_op(-1, OP_EQ,
              base16_decode(data2, 8, data1, 15)); /* odd input len */
    tt_int_op(-1, OP_EQ,
...
test_crypto_curve25519_impl(void *arg)
{
    ...
    for (loop = 0; loop < loop_max; ++loop) {
        ...
        if (randomize_high_bit) {
             ...
             * we're doing this variant test, we randomize the high bit of e2k,
             * and make sure that the handshake still works out the same as it
             * would otherwise. */
            uint8_t byte;
...
test_crypto_ed25519_simple(void *arg)
{
    ...
    const uint8_t msg2[] =
        ...
        "yet is compatible with most existing applications that run under "
        "DOS.";
    ...
    /* Test the string-prefixed sign/checksig functions */
    {
        ...
        tt_int_op(0, OP_EQ,
                  ed25519_sign_prefixed(&sig1, msg, msg_len,
                                        "always in the mood", &kp1));

        /* First, check that ed25519_sign_prefixed() returns the exact same sig
           as if we had manually prefixed the msg ourselves. */
...
test_crypto_ed25519_test_vectors(void *arg)
{
    ...
    for (i = 0; items[i].pk; ++i) {
        ...
        base16_decode((char *)msg, msg_len, items[i].msg,
                      strlen(items[i].msg));
...
test_crypto_ed25519_convert(void *arg)
{
    ...
    for (i = 0; i < N; ++i) {
        ...
        tt_mem_op(ed25519_pubkey.pubkey, OP_EQ, ed25519_keypair.pubkey.pubkey,
                  32);
...
test_crypto_ed25519_blinding(void *arg)
{
    ...
    for (i = 0; i < N; ++i) {
        ...
        tt_int_op(-1, OP_EQ,
                  ed25519_checksig(&sig, msg, sizeof(msg) - 1,
...
#define EQ(a, h) test_memeq_hex((const char *)(a), (h))
        ...
        tt_int_op(0, OP_EQ,
                  ed25519_keypair_from_curve25519_keypair(&keypair, &sign,
                                                          &curvekp));
        tt_int_op(0, OP_EQ,
                  ed25519_public_key_from_curve25519_public_key(
