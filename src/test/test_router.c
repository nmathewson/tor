...
test_router_dump_router_to_string_no_bridge_distribution_method(void *arg)
{
    ...
    crypto_pk_t *onion_pkey = router_get_rsa_onion_pkey(router->onion_pkey, router->onion_pkey_len);
    desc = router_dump_router_to_string(router, router->identity_pkey, onion_pkey, &ntor_keypair,
                                        &signing_keypair);
...
test_router_check_descriptor_bandwidth_changed(void *arg)
{
    ...
    expect_log_msg_not_containing("Measured bandwidth has changed; rebuilding descriptor.");
    ...
    setup_full_capture_of_logs(LOG_INFO);
    check_descriptor_bandwidth_changed(time(NULL));
    expect_log_msg_containing("Measured bandwidth has changed; rebuilding descriptor.");
    teardown_capture_of_logs();
...
test_router_mark_if_too_old(void *arg)
{
    ...
    mark_my_descriptor_dirty_if_too_old(now);
    tt_i64_op(desc_clean_since, OP_EQ, 0);
    tt_str_op(desc_dirty_reason, OP_EQ, "version listed in consensus is quite old");
...
test_router_get_my_family(void *arg)
{
    ...
    set_server_identity_key_digest_testing((const uint8_t *)"holeinthebottomofthe");
...
#define CLEAR()                                              \
    ...
    config_line_append(&options->MyFamily, "MyFamily", "$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
    ...
    tt_str_op(join, OP_EQ,
              ...
              "$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA "
              "bagel");
    expect_single_log_msg_containing("There is a router named \"BAGEL\" in my declared family, but "
    ...
    config_line_append(&options->MyFamily, "MyFamily", "$686F6C65696E746865626F74746F6D6F66746865");
    sl = get_my_declared_family(options);
    tt_ptr_op(sl, OP_EQ, NULL);
    expect_no_log_entry();
...
test_router_get_advertised_or_port(void *arg)
{
    ...
    router_get_advertised_ipv6_or_ap(opts, &ipv6);
    tt_str_op(fmt_addrport(&ipv6.addr, ipv6.port), OP_EQ, "[1234::5678]:54321");
