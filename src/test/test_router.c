...
rtr_tests_router_get_my_routerinfo(void)
{
    ...
    if (!mock_routerinfo) {
        /* Mock the published timestamp, otherwise
         * router_dump_router_to_string() will poop its pants. */
        time(&now);

        /* We'll need keys, or router_dump_router_to_string() would return
         * NULL. */
...
test_router_get_advertised_or_port(void *arg)
{
    ...
    tt_int_op(54321, OP_EQ,
              router_get_advertised_or_port_by_af(opts, AF_INET6));
