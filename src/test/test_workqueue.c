...
workqueue_do_rsa(void *state, void *work)
{
    ...
    len = crypto_pk_private_sign(rsa, (char *)sig, 256, (char *)rw->msg, rw->msglen);
    if (len < 0) {
        rw->msglen = 0;
        return WQ_RPL_ERROR;
    }
...
{
    int add_rsa = opt_ratio_rsa == 0 || tor_weak_random_range(&weak_rng, opt_ratio_rsa) == 0;
    ...
    if (add_rsa) {
        ...
        w->msglen = 20;
        ++rsa_sent;
        return threadpool_queue_work_priority(tp, WQ_PRI_MED, workqueue_do_rsa, handle_reply, w);
    } else {
...
replysock_readable_cb(threadpool_t *tp)
{
    ...
    if (opt_verbose) {
        ...
        if (opt_n_cancel)
            printf(" (%d cancelled, %d uncancellable)", n_successful_cancel, n_failed_cancel);
        puts("");
    }
#ifdef TRACK_RESPONSES
    tor_mutex_acquire(&bitmap_mutex);
...
main(int argc, char **argv)
{
    ...
    if (opt_n_threads < 1 || opt_n_items < 1 || opt_n_inflight < 1 || opt_n_lowwater < 0 ||
        opt_n_cancel > opt_n_inflight || opt_n_inflight > MAX_INFLIGHT || opt_ratio_rsa < 0) {
        help();
        return 1;
    }
