...
#define SPIN() SPIN_UNTIL(ti->addend == 0, 0)
    ...
    if (!timeout) {
        ...
        SPIN_UNTIL((ti->n_timeouts == 2 || time(NULL) >= started_at + GIVE_UP_AFTER_SEC), 10);
...
struct testcase_t thread_tests[] = {
    THREAD_TEST(basic),
    {"conditionvar", test_threads_conditionvar, TT_FORK, &passthrough_setup, (void *)"no-tv"},
    {"conditionvar_timeout", test_threads_conditionvar, TT_FORK, &passthrough_setup, (void *)"tv"},
    END_OF_TESTCASES};
