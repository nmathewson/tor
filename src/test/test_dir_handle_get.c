...
test_dir_handle_get_bad_request(void *data)
{
    ...
    fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, NULL, NULL, 1, 0);
...
test_dir_handle_get_v1_command(void *data)
{
    ...
    fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body, &body_used,
                        body_len + 1, 0);
...
test_dir_handle_get_robots_txt(void *data)
{
    ...
    fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, &body, &body_used, 29, 0);
...
test_dir_handle_get_rendezvous2_on_encrypted_conn_with_invalid_desc_id(void *data)
{
    ...
    tt_int_op(directory_handle_command_get(conn, RENDEZVOUS2_GET("invalid-desc-id"), NULL, 0),
              OP_EQ, 0);
    fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE, NULL, NULL, 1, 0);
...
static const char microdesc[] = "onion-key\n"
...
test_dir_handle_get_micro_d(void *data)
{
    ...
    list = microdescs_add_to_cache(mc, microdesc, NULL, SAVED_NOWHERE, 0, time(NULL), NULL);
...
test_dir_handle_get_networkstatus_bridges(void *data)
{
    ...
    crypto_digest256(mock_options->BridgePassword_AuthDigest_, "abcdefghijklm12345", 18,
                     DIGEST_SHA256);
...
static char TEST_DESCRIPTOR[] = "@uploaded-at 2014-06-08 19:20:11\n"
                                "@source \"127.0.0.1\"\n"
...
test_dir_handle_get_server_descriptors_authority(void *data)
{
    ...
    mock_routerinfo->cache_info.signed_descriptor_body = tor_strdup(TEST_DESCRIPTOR);
    mock_routerinfo->cache_info.signed_descriptor_len = strlen(TEST_DESCRIPTOR) - annotation_len;
...
#define HEX2 "Deadbeef99999991111119999911111111f00ba4"
    ...
    char req[155];
    tor_snprintf(req, sizeof(req), SERVER_DESC_GET("fp/%s+" HEX1 "+" HEX2), hex_digest);
    tt_int_op(directory_handle_command_get(conn, req, NULL, 0), OP_EQ, 0);
...
test_dir_handle_get_server_descriptors_d(void *data)
{
    ...
    tor_snprintf(req_header, sizeof(req_header), SERVER_DESC_GET("d/%s+" HEX1 "+" HEX2),
                 hex_digest);
    ...
    tt_str_op(body, OP_EQ,
              router->cache_info.signed_descriptor_body + router->cache_info.annotations_len);
    tt_ptr_op(conn->spool, OP_EQ, NULL);
...
static const char TEST_CERT_IDENT_KEY[] = "D867ACF56A9D229B35C25F0090BC9867E906BE69";
...
test_dir_handle_get_server_keys_authority(void *data)
{
    ...
    mock_cert = authority_cert_parse_from_string(TEST_CERTIFICATE, strlen(TEST_CERTIFICATE), NULL);
...
test_dir_handle_get_server_keys_fpsk(void *data)
{
    ...
    char req[115];
    tor_snprintf(req, sizeof(req), GET("/tor/keys/fp-sk/%s-%s"), TEST_CERT_IDENT_KEY,
                 TEST_SIGNING_KEY);
...
#define NETWORK_STATUS "some network status string"
    ...
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(
                  conn, GET("/tor/status-vote/current/consensus-ns/" HEX1 "+" HEX2), NULL, 0));
...
test_dir_handle_get_status_vote_current_consensus_ns_not_found(void *data)
{
    ...
    tt_int_op(
        0, OP_EQ,
        directory_handle_command_get(conn, GET("/tor/status-vote/current/consensus-ns"), NULL, 0));
...
#define NETWORK_STATUS "some network status string"
    ...
    tt_int_op(
        0, OP_EQ,
        directory_handle_command_get(conn, GET("/tor/status-vote/current/consensus"), NULL, 0));
...
#define NETWORK_STATUS "some network status string"
  ...
  dirserv_set_cached_consensus_networkstatus(NETWORK_STATUS, "ns", &digests,
    ...
    fetch_from_buf_http(TO_CONN(conn)->outbuf, header, MAX_HEADERS_SIZE, body, body_len,
                        strlen(NETWORK_STATUS) + 7, 0);
...
test_dir_handle_get_status_vote_current_not_found(void *data)
{
    ...
    conn = new_dir_conn();
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(conn, GET("/tor/status-vote/current/" HEX1), NULL, 0));
...
status_vote_current_d_test(char **header, char **body, size_t *body_l)
{
    ...
    conn = new_dir_conn();
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(conn, GET("/tor/status-vote/current/d/" VOTE_DIGEST),
                                           NULL, 0));

    fetch_from_buf_http(TO_CONN(conn)->outbuf, header, MAX_HEADERS_SIZE, body, body_l,
                        strlen(VOTE_BODY_V3) + 1, 0);
    tt_assert(header);
...
status_vote_next_d_test(char **header, char **body, size_t *body_l)
{
    ...
    tt_int_op(
        0, OP_EQ,
        directory_handle_command_get(conn, GET("/tor/status-vote/next/d/" VOTE_DIGEST), NULL, 0));
...
test_dir_handle_get_status_vote_d(void *data)
{
    ...
    int status_out = 0;
    struct pending_vote_t *pv = dirvote_add_vote(VOTE_BODY_V3, 0, &msg_out, &status_out);
    tt_assert(pv);
...
test_dir_handle_get_status_vote_next_not_found(void *data)
{
    ...
    conn = new_dir_conn();
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(conn, GET("/tor/status-vote/next/" HEX1), NULL, 0));
...
status_vote_next_consensus_test(char **header, char **body, size_t *body_used)
{
    ...
    conn = new_dir_conn();
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(conn, GET("/tor/status-vote/next/consensus"), NULL, 0));

    fetch_from_buf_http(TO_CONN(conn)->outbuf, header, MAX_HEADERS_SIZE, body, body_used, 18, 0);
done:
    UNMOCK(connection_write_to_buf_impl_);
    connection_free_minimal(TO_CONN(conn));
}
...
test_dir_handle_get_status_vote_current_authority_not_found(void *data)
{
    ...
    tt_int_op(
        0, OP_EQ,
        directory_handle_command_get(conn, GET("/tor/status-vote/current/authority"), NULL, 0));
...
test_dir_handle_get_status_vote_next_authority_not_found(void *data)
{
    ...
    conn = new_dir_conn();
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(conn, GET("/tor/status-vote/next/authority"), NULL, 0));
...
test_dir_handle_get_status_vote_next_bandwidth_not_found(void *data)
{
    ...
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(conn, GET("/tor/status-vote/next/bandwdith"), NULL, 0));
...
status_vote_next_consensus_signatures_test(char **header, char **body, size_t *body_used)
{
    ...
    conn = new_dir_conn();
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(conn, GET("/tor/status-vote/next/consensus-signatures"),
                                           NULL, 0));

    fetch_from_buf_http(TO_CONN(conn)->outbuf, header, MAX_HEADERS_SIZE, body, body_used, 22, 0);
...
test_dir_handle_get_status_vote_next_authority(void *data)
{
    ...
    struct pending_vote_t *vote = dirvote_add_vote(VOTE_BODY_V3, 0, &msg_out, &status_out);
    tt_assert(vote);
...
test_dir_handle_get_status_vote_next_bandwidth(void *data)
{
    ...
    init_mock_options();
    MOCK(get_options, mock_get_options);
    mock_options->V3BandwidthsFile = tor_strdup(get_fname_rnd("V3BandwidthsFile"));
    ...
    conn = new_dir_conn();
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(conn, GET("/tor/status-vote/next/bandwidth"), NULL, 0));
    ...
    tt_int_op(
        0, OP_EQ,
        directory_handle_command_get(conn, GET("/tor/status-vote/next/bandwidth.z"), NULL, 0));
...
test_dir_handle_get_status_vote_too_late(void *data)
{
    ...
    vote = dirvote_add_vote(VOTE_BODY_V3, fetch_missing + vote_interval, &msg_out, &status_out);
    tt_assert(!vote);
    tt_int_op(status_out, OP_EQ, 400);
    tt_str_op(msg_out, OP_EQ, "Posted vote received too late, would be dangerous to count it");

    /* Just after fetch missing */
    vote = dirvote_add_vote(VOTE_BODY_V3, fetch_missing + 1, &msg_out, &status_out);
    tt_assert(!vote);
    tt_int_op(status_out, OP_EQ, 400);
    tt_str_op(msg_out, OP_EQ, "Posted vote received too late, would be dangerous to count it");

    /* On fetch missing */
    vote = dirvote_add_vote(VOTE_BODY_V3, fetch_missing, &msg_out, &status_out);
    tt_assert(vote);
    ...
    vote = dirvote_add_vote(VOTE_BODY_V3, voting_starts + 1, &msg_out, &status_out);
    tt_assert(vote);
    ...
    /* On voting starts */
    vote = dirvote_add_vote(VOTE_BODY_V3, voting_starts, &msg_out, &status_out);
    tt_assert(vote);
...
#    define DIR_HANDLE_CMD_ARG(name, flags, arg) DIR_HANDLE_CMD(name, flags)
#else
#    define DIR_HANDLE_CMD_ARG(name, flags, arg)                                                  \
        {                                                                                         \
#            name "/" arg, test_dir_handle_get_##name, (flags), &passthrough_setup, (void *)(arg) \
        }
#endif /* defined(COCCI) */
