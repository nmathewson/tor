...
#define RECENT_TIME -10
    ...
    tt_int_op(rend_cache_store_v2_desc_as_dir(desc_holder->desc_str), OP_EQ,
              0);
...
test_dir_handle_get_server_descriptors_all(void *data)
{
    ...
    // TODO: Is this a BUG?
    // It requires strlen(signed_descriptor_len)+1 as body_len but returns a
    // body which is smaller than that by annotation_len bytes
    fetch_from_buf_http(TO_CONN(conn)->outbuf, &header, MAX_HEADERS_SIZE,
                        &body, &body_used, 1024 * 1024, 0);
...
test_dir_handle_get_server_descriptors_authority(void *data)
{
    ...
    // the digest is mine (the channel is unnecrypted, so we must allow
    // sending)
    ...
    // TODO: Is this a BUG?
    // It requires strlen(TEST_DESCRIPTOR)+1 as body_len but returns a body
...
test_dir_handle_get_server_descriptors_d(void *data)
{
    ...
    tt_int_op(directory_handle_command_get(conn, req_header, NULL, 0), OP_EQ,
              0);
...
test_dir_handle_get_server_keys_all(void *data)
{
    ...
    tt_int_op(0, OP_EQ,
              trusted_dirs_load_certs_from_string(
                  TEST_CERTIFICATE, TRUSTED_DIRS_CERTS_SRC_DL_BY_ID_DIGEST, 1,
                  NULL));
...
test_dir_handle_get_server_keys_authority(void *data)
{
    ...
    mock_cert = authority_cert_parse_from_string(
        TEST_CERTIFICATE, strlen(TEST_CERTIFICATE), NULL);
...
test_dir_handle_get_server_keys_fp(void *data)
{
    ...
    conn = new_dir_conn();
    char req[71];
    tor_snprintf(req, sizeof(req), GET("/tor/keys/fp/%s"),
                 TEST_CERT_IDENT_KEY);
    tt_int_op(directory_handle_command_get(conn, req, NULL, 0), OP_EQ, 0);
...
#define NETWORK_STATUS "some network status string"
    ...
    tt_int_op(0, OP_EQ,
              directory_handle_command_get(
                  conn, GET("/tor/status-vote/current/consensus-microdesc"),
                  NULL, 0));
...
test_dir_handle_get_status_vote_current_consensus_ns(void *data)
{
    ...
    status_vote_current_consensus_ns_test(&header, &comp_body,
                                          &comp_body_used);
    tt_assert(header);
    ...
    tor_uncompress(&body, &body_used, comp_body, comp_body_used, compression,
                   0, LOG_PROTOCOL_WARN);
...
test_dir_handle_get_status_vote_d(void *data)
{
    ...
    struct pending_vote_t *pv =
        dirvote_add_vote(VOTE_BODY_V3, 0, &msg_out, &status_out);
    tt_assert(pv);
...
test_dir_handle_get_status_vote_too_late(void *data)
{
    ...
    vote = dirvote_add_vote(VOTE_BODY_V3, fetch_missing + vote_interval,
                            &msg_out, &status_out);
    tt_assert(!vote);
    ...
    vote = dirvote_add_vote(VOTE_BODY_V3, fetch_missing + 1, &msg_out,
                            &status_out);
    tt_assert(!vote);
    tt_int_op(status_out, OP_EQ, 400);
    tt_str_op(msg_out, OP_EQ,
    ...
    /* On fetch missing */
    vote =
        dirvote_add_vote(VOTE_BODY_V3, fetch_missing, &msg_out, &status_out);
    tt_assert(vote);
    ...
    vote = dirvote_add_vote(VOTE_BODY_V3, voting_starts + 1, &msg_out,
                            &status_out);
    tt_assert(vote);
    ...
    /* On voting starts */
    vote =
        dirvote_add_vote(VOTE_BODY_V3, voting_starts, &msg_out, &status_out);
    tt_assert(vote);
