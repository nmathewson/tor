...
test_dir_handle_get_status_vote_too_late(void *data)
{
  ...
  vote =
      dirvote_add_vote(VOTE_BODY_V3, fetch_missing + 1, &msg_out, &status_out);
  tt_assert(!vote);
  tt_int_op(status_out, OP_EQ, 400);
  tt_str_op(msg_out, OP_EQ,
            "Posted vote received too late, would be dangerous to count it");
  ...
  vote =
      dirvote_add_vote(VOTE_BODY_V3, voting_starts + 1, &msg_out, &status_out);
  tt_assert(vote);
...
#  define DIR_HANDLE_CMD_ARG(name, flags, arg)                                \
    {                                                                         \
#      name "/" arg, test_dir_handle_get_##name, (flags), &passthrough_setup, \
          (void *)(arg)                                                       \
    }
#endif /* defined(COCCI) */
