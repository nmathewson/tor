...
init_authority_state(void)
{
    ...
    mock_cert = authority_cert_parse_from_string(
...
test_get_sr_protocol_phase(void *arg)
{
    ...
    {
        retval =
            parse_rfc1123_time("Wed, 20 Apr 2015 23:59:00 UTC", &the_time);
    ...
    {
        retval =
            parse_rfc1123_time("Wed, 20 Apr 2015 00:00:00 UTC", &the_time);
        tt_int_op(retval, OP_EQ, 0);
    ...
    {
        retval =
            parse_rfc1123_time("Wed, 20 Apr 2015 00:00:01 UTC", &the_time);
        tt_int_op(retval, OP_EQ, 0);
    ...
    {
        retval =
            parse_rfc1123_time("Wed, 20 Apr 2015 11:59:00 UTC", &the_time);
        tt_int_op(retval, OP_EQ, 0);
    ...
    {
        retval =
            parse_rfc1123_time("Wed, 20 Apr 2015 12:00:00 UTC", &the_time);
        tt_int_op(retval, OP_EQ, 0);
    ...
    {
        retval =
            parse_rfc1123_time("Wed, 20 Apr 2015 12:00:01 UTC", &the_time);
        tt_int_op(retval, OP_EQ, 0);
    ...
    {
        retval =
            parse_rfc1123_time("Wed, 20 Apr 2015 13:00:00 UTC", &the_time);
        tt_int_op(retval, OP_EQ, 0);
...
test_get_start_time_of_current_run(void *arg)
{
    ...
    {
        /* We want the local time to be past midnight, but the current
         * consensus to have valid-after 23:00 (e.g. this can happen if we
         * fetch a new consensus at 00:08 before dircaches have a chance to get
         * the midnight consensus).
         *
         * Basically, we want to cause a desynch between ns->valid_after
         * (23:00) and the voting_schedule.interval_starts (01:00), to make
         * sure that sr_state_get_start_time_of_current_protocol_run() handles
         * it gracefully: It should actually follow the local consensus time
         * and not the voting
...
test_sr_commit(void *arg)
{
     ...
     * vote coming from the network. */
    {
        ...
        /* Do we have a valid encoded commit and reveal. Note the following
         * only tests if the generated values are correct. Their could be a bug
         * in the decode function but we test them separately. */
        tt_int_op(0, OP_EQ,
     ...
     * takes from a vote line and see if we can parse it correctly. */
    {
        ...
        smartlist_add_strdup(
            args, crypto_digest_algorithm_get_name(our_commit->alg));
...
test_sr_setup_srv(int also_current)
{
    ...
    if (also_current) {
        ...
        srv->num_reveals = 128;
        memcpy(srv->value, "NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN",
               sizeof(srv->value));
...
test_vote(void *arg)
{
    ...
    { /* Now test the vote generation */
        ...
        ret =
            smartlist_split_string(chunks, lines, "\n", SPLIT_IGNORE_BLANK, 0);
        ...
        tt_str_op(smartlist_get(tokens, 0), OP_EQ,
...
test_sr_setup_commits(void)
{
    ...
    { /* Commit from auth 'a' */
        ...
        base16_encode(commit_a->rsa_identity_hex,
                      sizeof(commit_a->rsa_identity_hex),
    ...
    { /* Commit from auth 'b' */
        ...
        base16_encode(commit_b->rsa_identity_hex,
                      sizeof(commit_b->rsa_identity_hex),
    ...
    { /* Commit from auth 'c' */
        ...
        base16_encode(commit_c->rsa_identity_hex,
                      sizeof(commit_c->rsa_identity_hex),
    ...
    { /* Commit from auth 'd' */
        ...
        base16_encode(commit_d->rsa_identity_hex,
                      sizeof(commit_d->rsa_identity_hex),
...
#define SRV_2 "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB"
    ...
    /* We will now lower the AuthDirNumSRVAgreements requirement by tweaking
     * the
...
test_utils_auth(void *arg)
{
...

    {
        ...
        tt_mem_op(sr_state_get_previous_srv(), OP_NE,
                  sr_state_get_current_srv(), sizeof(sr_srv_t));
        /* Set the current to the current: the protocol never replaces an SRV
         * with the same value */
...
test_state_transition(void *arg)
{
    ...
    /* Test SRV rotation in our state. */
    {
        ...
        tt_assert(cur);
        /* After, the previous SRV should be the same as the old current SRV,
         * and the current SRV should be set to NULL */
        state_rotate_srv();
...
test_keep_commit(void *arg)
{
    ...
    tt_int_op(
        should_keep_commit(commit, commit->rsa_identity, SR_PHASE_COMMIT),
        OP_EQ, 0);
...
test_state_update(void *arg)
{
...

    {
        ...
        state = get_sr_state();
        set_sr_phase(SR_PHASE_COMMIT);
        /* We'll cheat a bit here and reset the creation time of the state
         * which will avoid us to compute a valid_after time that fits the
         * commit phase. */
        state->valid_after = 0;
