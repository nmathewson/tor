...
test_get_start_time_of_current_run(void *arg)
{
    ...
    {
        /* We want the local time to be past midnight, but the current consensus
         * to have valid-after 23:00 (e.g. this can happen if we fetch a new
         * consensus at 00:08 before dircaches have a chance to get the midnight
         * consensus).
         *
         ...
         * sr_state_get_start_time_of_current_protocol_run() handles it
         * gracefully: It should actually follow the local consensus time and
         * not the voting schedule (which is designed for authority voting
         * purposes). */
        retval = parse_rfc1123_time("Mon, 20 Apr 2015 00:00:00 UTC",
                                    &mock_consensus.fresh_until);
        tt_int_op(retval, OP_EQ, 0);
