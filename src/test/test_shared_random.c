...
init_authority_state(void)
{
    ...
    mock_cert = authority_cert_parse_from_string(AUTHORITY_CERT_1, strlen(AUTHORITY_CERT_1), NULL);
    tt_assert(mock_cert);
...
test_get_state_valid_until_time(void *arg)
{
    ...
    retval = parse_rfc1123_time("Mon, 20 Apr 2015 01:00:00 UTC", &mock_consensus.fresh_until);
    tt_int_op(retval, OP_EQ, 0);

    retval = parse_rfc1123_time("Mon, 20 Apr 2015 00:00:00 UTC", &mock_consensus.valid_after);
    tt_int_op(retval, OP_EQ, 0);
...
test_get_start_time_of_current_run(void *arg)
{
    ...
    {
        retval = parse_rfc1123_time("Mon, 20 Apr 2015 23:59:59 UTC", &current_time);
        tt_int_op(retval, OP_EQ, 0);
        dirauth_sched_recalculate_timing(get_options(), current_time);
        run_start_time = sr_state_get_start_time_of_current_protocol_run();
...

    {
        ...
        retval = parse_rfc1123_time("Mon, 20 Apr 2015 00:00:00 UTC", &mock_consensus.fresh_until);
        tt_int_op(retval, OP_EQ, 0);

        retval = parse_rfc1123_time("Mon, 19 Apr 2015 23:00:00 UTC", &mock_consensus.valid_after);
        tt_int_op(retval, OP_EQ, 0);

        retval = parse_rfc1123_time("Mon, 20 Apr 2015 00:08:00 UTC", &current_time);
        tt_int_op(retval, OP_EQ, 0);
     ...
     * takes 10 seconds * 24 rounds = 4 mins */
    {
        ...
        retval = parse_rfc1123_time("Mon, 20 Apr 2015 00:15:32 UTC", &current_time);
        tt_int_op(retval, OP_EQ, 0);
        dirauth_sched_recalculate_timing(get_options(), current_time);
        run_start_time = sr_state_get_start_time_of_current_protocol_run();
...
test_get_start_time_functions(void *arg)
{
    ...
    time_t start_time_of_protocol_run = sr_state_get_start_time_of_current_protocol_run();
    tt_assert(start_time_of_protocol_run);

    /* Check that the round start time of the beginning of the run, is itself */
    tt_int_op(dirauth_sched_get_cur_valid_after_time(), OP_EQ, start_time_of_protocol_run);
...
test_sr_commit(void *arg)
{
     ...
     * vote coming from the network. */
    {
        ...
        tt_assert(!fast_mem_is_zero(our_commit->hashed_reveal, sizeof(our_commit->hashed_reveal)));
        ...
        tt_int_op(0, OP_EQ, reveal_decode(our_commit->encoded_reveal, &test_commit));
        tt_int_op(0, OP_EQ, commit_decode(our_commit->encoded_commit, &test_commit));
     ...
     * takes from a vote line and see if we can parse it correctly. */
    {
        ...
        smartlist_add_strdup(args, crypto_digest_algorithm_get_name(our_commit->alg));
...
test_encoding(void *arg)
{
    ...
    static const char *encoded_reveal = "AAAAAFavXpZJxbwTupvaJCTeIUCQmOPxAMblc7ChL5H2nZKuGchdaA==";
    static const char *encoded_commit = "AAAAAFavXpbkBMzMQG7aNoaGLFNpm2Wkk1ozXhuWWqL//GynltxVAg==";
    ...
    ret = crypto_digest256(hashed_rand, raw_rand, sizeof(raw_rand), SR_DIGEST_ALG);
    tt_int_op(0, OP_EQ, ret);
    /* Hash reveal value. */
    tt_int_op(SR_REVEAL_BASE64_LEN, OP_EQ, strlen(encoded_reveal));
    ret = crypto_digest256(hashed_reveal, encoded_reveal, strlen(encoded_reveal), SR_DIGEST_ALG);
    tt_int_op(0, OP_EQ, ret);
    ...
    /* Test our commit/reveal decode functions. */
    {
        ...
        tt_mem_op(hashed_rand, OP_EQ, parsed_commit.random_number, sizeof(hashed_rand));
        ...
        tt_mem_op(hashed_reveal, OP_EQ, parsed_commit.hashed_reveal, sizeof(hashed_reveal));
    }
    ...
    /* Test our commit/reveal encode functions. */
    {
        ...
        memcpy(parsed_commit.random_number, hashed_rand, sizeof(parsed_commit.random_number));
...

    {
        ...
        memcpy(parsed_commit.hashed_reveal, hashed_reveal, sizeof(parsed_commit.hashed_reveal));
...
test_vote(void *arg)
{
    ...
    MOCK(trusteddirserver_get_by_v3_auth_digest, trusteddirserver_get_by_v3_auth_digest_m);
    ...
    { /* Now test the vote generation */
        ...
        base16_decode(digest, sizeof(digest), smartlist_get(tokens, 3), HEX_DIGEST_LEN);
        ...
        tt_str_op(smartlist_get(tokens, 2), OP_EQ, "WlpaWlpaWlpaWlpaWlpaWlpaWlpaWlpaWlpaWlpaWlo=");
        ...
        tt_str_op(smartlist_get(tokens, 2), OP_EQ, "Tk5OTk5OTk5OTk5OTk5OTk5OTk5OTk5OTk5OTk5OTk4=");
...
test_sr_setup_commits(void)
{
    ...
    { /* Commit from auth 'a' */
        ...
        base16_encode(commit_a->rsa_identity_hex, sizeof(commit_a->rsa_identity_hex),
                      commit_a->rsa_identity, sizeof(commit_a->rsa_identity));
        strlcpy(commit_a->encoded_reveal,
                "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
                sizeof(commit_a->encoded_reveal));
        memcpy(commit_a->hashed_reveal, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
    ...
    { /* Commit from auth 'b' */
        ...
        base16_encode(commit_b->rsa_identity_hex, sizeof(commit_b->rsa_identity_hex),
                      commit_b->rsa_identity, sizeof(commit_b->rsa_identity));
        strlcpy(commit_b->encoded_reveal,
                "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB",
                sizeof(commit_b->encoded_reveal));
        memcpy(commit_b->hashed_reveal, "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB",
    ...
    { /* Commit from auth 'c' */
        ...
        base16_encode(commit_c->rsa_identity_hex, sizeof(commit_c->rsa_identity_hex),
                      commit_c->rsa_identity, sizeof(commit_c->rsa_identity));
    ...
    { /* Commit from auth 'd' */
        ...
        base16_encode(commit_d->rsa_identity_hex, sizeof(commit_d->rsa_identity_hex),
                      commit_d->rsa_identity, sizeof(commit_d->rsa_identity));
        strlcpy(commit_d->encoded_reveal,
                "DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD",
                sizeof(commit_d->encoded_reveal));
        memcpy(commit_d->hashed_reveal, "DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD",
...
#define SRV_TEST_VECTOR "2A9B1D6237DAB312A40F575DA85C147663E7ED3F80E9555395F15B515C74253D"
...
get_test_vote_with_curr_srv(const char *srv)
{
    ...
    memcpy(vote->sr_info.current_srv->value, srv, sizeof(vote->sr_info.current_srv->value));
...
#define SRV_2 "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB"
    ...
    SMARTLIST_FOREACH(votes, networkstatus_t *, vote, networkstatus_vote_free(vote));
...
test_sr_svr_dup(void *arg)
{
    ...
    const char *srv_value = "1BDB7C3E973936E4D13A49F37C859B3DC69C429334CF9412E3FEF6399C52D47A";
...
test_commitments_are_the_same(void *arg)
{
    ...
    const char *payload = "\x5d\xb9\x60\xb6\xcc\x51\x68\x52\x31\xd9\x88\x88\x71\x71\xe0\x30"
...
test_utils_auth(void *arg)
{
...

    {
        ...
        tt_mem_op(sr_state_get_previous_srv(), OP_NE, sr_state_get_current_srv(), sizeof(sr_srv_t));
        ...
        tt_mem_op(sr_state_get_previous_srv(), OP_EQ, sr_state_get_current_srv(), sizeof(sr_srv_t));
    }

    {
        ...
        tt_mem_op(&old_current_srv, OP_EQ, sr_state_get_current_srv(), sizeof(sr_srv_t));
    }
...
test_keep_commit(void *arg)
{
    ...
    tt_int_op(should_keep_commit(commit, commit->rsa_identity, SR_PHASE_COMMIT), OP_EQ, 0);
    ...
    tt_int_op(should_keep_commit(commit, commit->rsa_identity, SR_PHASE_COMMIT), OP_EQ, 1);
    ...
    tt_int_op(should_keep_commit(commit, commit->rsa_identity, SR_PHASE_REVEAL), OP_EQ, 0);
    ...
    tt_int_op(should_keep_commit(commit, commit->rsa_identity, SR_PHASE_REVEAL), OP_EQ, 1);
     ...
     * hashed reveal. */
    {
        ...
        assert_log_predicate(mock_saved_log_n_entries() == 2, ("expected 2 log entries"));
        teardown_capture_of_logs();
        memcpy(commit->hashed_reveal, place_holder.hashed_reveal, sizeof(commit->hashed_reveal));
    }
    /* We shouldn't keep a commit that has no reveal. */
    tt_int_op(should_keep_commit(dup_commit, dup_commit->rsa_identity, SR_PHASE_REVEAL), OP_EQ, 0);
...
struct testcase_t sr_tests[] = {
    ...
    {"get_start_time_of_current_run", test_get_start_time_of_current_run, TT_FORK, NULL, NULL},
    {"get_start_time_functions", test_get_start_time_functions, TT_FORK, NULL, NULL},
    {"get_sr_protocol_duration", test_get_sr_protocol_duration, TT_FORK, NULL, NULL},
    {"get_state_valid_until_time", test_get_state_valid_until_time, TT_FORK, NULL, NULL},
    ...
    {"sr_get_majority_srv_from_votes", test_sr_get_majority_srv_from_votes, TT_FORK, NULL, NULL},
    {"sr_svr_dup", test_sr_svr_dup, TT_FORK, NULL, NULL},
    {"commitments_are_the_same", test_commitments_are_the_same, TT_FORK, NULL, NULL},
    {"commit_is_authoritative", test_commit_is_authoritative, TT_FORK, NULL, NULL},
