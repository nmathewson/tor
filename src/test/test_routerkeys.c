...
test_routerkeys_ed_certs(void *args)
{
    ...
    for (int i = 0; i <= 1; ++i) {
        ...
        parsed_cert[i] =
            tor_cert_parse(cert[i]->encoded, cert[i]->encoded_len);
        ...
        /* Wrong key */
        tt_int_op(tor_cert_checksig(parsed_cert[i], &kp2.pubkey, now), OP_LT,
                  0);
        ...
        /* Right key */
        tt_int_op(tor_cert_checksig(parsed_cert[i], &kp1.pubkey, now), OP_EQ,
                  0);
        tt_uint_op(parsed_cert[i]->sig_bad, OP_EQ, 0);
...
test_routerkeys_ed_key_init_basic(void *arg)
{
    ...
    kp1 = ed_key_init_from_file(fname1, INIT_ED_KEY_CREATE, LOG_INFO, NULL,
                                now, 0, 7, &cert, NULL);
    tt_assert(kp1 != NULL);
    tt_assert(cert == NULL);
    ...
    kp2 = ed_key_init_from_file(fname1, 0, LOG_INFO, NULL, now, 0, 6, &cert,
                                NULL);
    tt_assert(kp2 == NULL);

    /* Load successfully if we're not picky, whether we say "create" or not. */
    kp2 = ed_key_init_from_file(fname1, INIT_ED_KEY_CREATE, LOG_INFO, NULL,
                                now, 0, 7, &cert, NULL);
    tt_assert(kp2 != NULL);
    tt_assert(cert == NULL);
    ...
    kp2 = ed_key_init_from_file(fname1, 0, LOG_INFO, NULL, now, 0, 7, &cert,
                                NULL);
    tt_assert(kp2 != NULL);
    tt_assert(cert == NULL);
    tt_mem_op(kp1, OP_EQ, kp2, sizeof(*kp1));
    ...
    /* ... even without create... */
    kp3 = ed_key_init_from_file(fname2, INIT_ED_KEY_NEEDCERT, LOG_INFO, kp1,
    ...
    kp3 = ed_key_init_from_file(fname2, INIT_ED_KEY_NEEDCERT, LOG_INFO, kp1,
                                now, 7200, 7, NULL, NULL);
    tt_mem_op(kp2, OP_EQ, kp3, sizeof(*kp2));
    ed25519_keypair_free(kp3);
    kp3 = NULL;

    /* Fail if we're told the wrong signing key */
    kp3 = ed_key_init_from_file(fname2, INIT_ED_KEY_NEEDCERT, LOG_INFO, kp2,
                                now, 7200, 7, &cert2, NULL);
    tt_assert(kp3 == NULL);
...
test_routerkeys_ed_key_init_split(void *arg)
{
    ...
    kp1 = ed_key_init_from_file(fname1, flags, LOG_INFO, NULL, now, 0, 7,
                                &cert, NULL);
    tt_assert(kp1 == NULL);
    tt_assert(cert == NULL);
    ...
    kp2 = ed_key_init_from_file(fname1, flags, LOG_INFO, NULL, now, 0, 7,
                                &cert, NULL);
    tt_assert(kp2 != NULL);
    tt_assert(cert == NULL);
    ...
    kp2 = ed_key_init_from_file(fname1, flags, LOG_INFO, NULL, now, 0, 99,
                                &cert, NULL);
    tt_assert(kp2 == NULL);
...
test_routerkeys_rsa_ed_crosscert(void *arg)
{
    ...
    /* Truncated object */
    tt_int_op(-2, OP_EQ,
              rsa_ed25519_crosscert_check(cc, cc_len - 2, rsa, &ed,
                                          expires_in - 10));
