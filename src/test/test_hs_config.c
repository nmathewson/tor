...
test_invalid_service(void *arg)
{
    ...
    /* Try with a missing port configuration. */
    {
        ...
        expect_log_msg_containing(
            "HiddenServiceVersion must be between 2 and 3");
...
test_dos_parameters(void *arg)
{
    ...
    /* Valid configuration. */
    {
        ...
        tt_int_op(ret, OP_EQ, 0);
        expect_log_msg_containing(
            "Service INTRO2 DoS defenses rate set to: 42");
        expect_log_msg_containing(
            "Service INTRO2 DoS defenses burst set to: 87");
        teardown_capture_of_logs();
    }
    ...
    /* Invalid burst. Value of 2^38. Max allowed is 2^31. */
    {
        ...
        tt_int_op(ret, OP_EQ, -1);
        expect_log_msg_containing(
            "HiddenServiceEnableIntroDoSBurstPerSec must "
            "be between 0 and 2147483647, "
            "not 274877906944");
        teardown_capture_of_logs();
    }
    ...
    /* Negative value. */
    {
        ...
        tt_int_op(ret, OP_EQ, -1);
        expect_log_msg_containing(
            "HiddenServiceEnableIntroDoSRatePerSec must be "
