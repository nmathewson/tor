...
test_container_smartlist_strings(void *arg)
{
    ...
    smartlist_split_string(sl, " ab\tc \td ef  ", NULL, SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 0);
    tt_int_op(4, OP_EQ, smartlist_len(sl));
    ...
    smartlist_split_string(sl, "ghi\tj", NULL, SPLIT_SKIP_SPACE | SPLIT_IGNORE_BLANK, 0);
    ...
    /* Test bsearch. */
    smartlist_sort(sl, compare_strs_);
    tt_str_op("nickm", OP_EQ, smartlist_bsearch(sl, "zNicKM", cmp_without_first_));
    ...
    /* Test bsearch_idx */
    {
        ...
        tmp = smartlist_new();
        tt_int_op(0, OP_EQ, smartlist_bsearch_idx(tmp, "foo", compare_strs_for_bsearch_, &f));
        tt_int_op(f, OP_EQ, 0);
        smartlist_insert(tmp, 0, (void *)("bar"));
        tt_int_op(1, OP_EQ, smartlist_bsearch_idx(tmp, "foo", compare_strs_for_bsearch_, &f));
        tt_int_op(f, OP_EQ, 0);
        tt_int_op(0, OP_EQ, smartlist_bsearch_idx(tmp, "aaa", compare_strs_for_bsearch_, &f));
        tt_int_op(f, OP_EQ, 0);
        tt_int_op(0, OP_EQ, smartlist_bsearch_idx(tmp, "bar", compare_strs_for_bsearch_, &f));
        tt_int_op(f, OP_EQ, 1);
        /* ... and one for length 2 */
        smartlist_insert(tmp, 1, (void *)("foo"));
        tt_int_op(1, OP_EQ, smartlist_bsearch_idx(tmp, "foo", compare_strs_for_bsearch_, &f));
        tt_int_op(f, OP_EQ, 1);
        tt_int_op(2, OP_EQ, smartlist_bsearch_idx(tmp, "goo", compare_strs_for_bsearch_, &f));
        tt_int_op(f, OP_EQ, 0);
        smartlist_free(tmp);
    }
    ...
    /* Test uniq() */
    smartlist_split_string(sl, "50,noon,radar,a,man,a,plan,a,canal,panama,radar,noon,50", ",", 0,
                           0);
    smartlist_sort(sl, compare_strs_);
    ...
    smartlist_split_string(sl, "Some say the Earth will end in ice and some in fire", " ", 0, 0);
...
{
    smartlist_t *sl = smartlist_new();
    smartlist_t *sl2 = smartlist_new(), *sl3 = smartlist_new(), *sl4 = smartlist_new();
    char *joined = NULL;
    /* unique, sorted. */
    (void)arg;
    ...
    SMARTLIST_FOREACH(sl3, const char *, cp,
                      tt_assert(smartlist_contains(sl2, cp) && !smartlist_contains_string(sl, cp)));
    SMARTLIST_FOREACH(sl4, const char *, cp,
                      tt_assert(smartlist_contains(sl, cp) && smartlist_contains_string(sl2, cp)));
...
test_container_digestset(void *arg)
{
    ...
    SMARTLIST_FOREACH(included, const char *, cp, if (digestset_probably_contains(set, cp)) ok = 0);
    tt_assert(ok);
...
test_container_fp_pair_map(void *arg)
{
    ...
    tt_ptr_op(fp_pair_map_get(map, &fp2), OP_EQ, NULL);
    tt_ptr_op(fp_pair_map_get_by_digests(map, fp5.first, fp5.second), OP_EQ, v104);
...
struct testcase_t container_tests[] = {CONTAINER_LEGACY(smartlist_basic),
