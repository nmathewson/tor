...
static int testing_circuitset_teardown(const struct testcase_t *testcase, void *ptr);
...
testing_circuitset_setup(const struct testcase_t *testcase)
{
    ...
    for (i = 0; i < 3; ++i) {
        ...
        relay_crypto_init(&hop->pvt_crypto, KEY_MATERIAL[i], sizeof(KEY_MATERIAL[i]), 0, 0);
...
static const struct testcase_setup_t relaycrypt_setup = {testing_circuitset_setup,
...
test_relaycrypt_outbound(void *arg)
{
    ...
    for (i = 0; i < 50; ++i) {
        ...
        relay_encrypt_cell_outbound(&encrypted, cs->origin_circ, cs->origin_circ->cpath->prev);

        for (j = 0; j < 3; ++j) {
            crypt_path_t *layer_hint = NULL;
            char recognized = 0;
            int r = relay_decrypt_cell(TO_CIRCUIT(cs->or_circ[j]), &encrypted, CELL_DIRECTION_OUT,
...
test_relaycrypt_inbound(void *arg)
{
    ...
    for (i = 0; i < 50; ++i) {
        ...
        int r;
        for (j = 1; j >= 0; --j) {
            r = relay_decrypt_cell(TO_CIRCUIT(cs->or_circ[j]), &encrypted, CELL_DIRECTION_IN,
                                   &layer_hint, &recognized);
            tt_int_op(r, OP_EQ, 0);
            tt_ptr_op(layer_hint, OP_EQ, NULL);
        ...
        relay_decrypt_cell(TO_CIRCUIT(cs->origin_circ), &encrypted, CELL_DIRECTION_IN, &layer_hint,
                           &recognized);
        tt_int_op(r, OP_EQ, 0);
...
struct testcase_t relaycrypt_tests[] = {TEST(outbound), TEST(inbound), END_OF_TESTCASES};
