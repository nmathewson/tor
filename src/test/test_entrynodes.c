...
big_fake_network_setup(const struct testcase_t *testcase)
{
    ...
    for (i = 0; i < N_NODES; ++i) {
        ...
        n->md->onion_curve25519_pkey = tor_malloc_zero(sizeof(curve25519_public_key_t));
        curve25519_secret_key_generate(&curve25519_secret_key, 0);
        curve25519_public_key_generate(n->md->onion_curve25519_pkey, &curve25519_secret_key);
...
test_entry_guard_parse_from_state_minimal(void *arg)
{
    ...
    test_mem_op_hex(eg->identity, OP_EQ, "596f75206d6179206e656564206120686f626279");
...
test_entry_guard_parse_from_state_maximal(void *arg)
{
    ...
    eg = entry_guard_parse_from_state("in=fred "
                                      "rsa_id=706C75727079666C75727079736C75727079646F "
    ...
    test_mem_op_hex(eg->identity, OP_EQ, "706C75727079666C75727079736C75727079646F");
...
test_entry_guard_parse_from_state_failure(void *arg)
{
    ...
    /* no selection */
    eg = entry_guard_parse_from_state("rsa_id=596f75206d6179206e656564206120686f626270");
...
test_entry_guard_parse_from_state_full(void *arg)
{
    ...
    const char STATE[] = "Guard in=default rsa_id=214F44BD5B638E8C817D47FF7C97397790BF0345 "
    ...
    tt_int_op(r, OP_EQ, 0);
    guard_selection_t *gs_br = get_guard_selection_by_name("bridges", GS_TYPE_BRIDGE, 0);
    ...
    guard_selection_t *gs_df = get_guard_selection_by_name("default", GS_TYPE_NORMAL, 0);
    guard_selection_t *gs_wb = get_guard_selection_by_name("wobblesome", GS_TYPE_NORMAL, 0);
...
test_entry_guard_choose_selection_initial(void *arg)
{
    ...
    const char *name = choose_guard_selection(get_options(), dummy_consensus, NULL, &type);
...
#define NUM 7
    ...
    SMARTLIST_FOREACH(big_fake_net_nodes, node_t *, node, { memset(node->identity, 0xff, 20); });
    entry_guards_update_filtered_sets(gs);
    for (i = 0; i < NUM; ++i) {
...
test_entry_guard_expand_sample(void *arg)
{
    ...
    tt_int_op(DFLT_MIN_FILTERED_SAMPLE_SIZE, OP_EQ, num_reachable_filtered_guards(gs, NULL));
    ...
    guard->is_usable_filtered_guard = 0;
    tt_int_op(DFLT_MIN_FILTERED_SAMPLE_SIZE - 3, OP_EQ, num_reachable_filtered_guards(gs, NULL));
    ...
    tt_int_op(num_reachable_filtered_guards(gs, NULL), OP_LT, DFLT_MIN_FILTERED_SAMPLE_SIZE);
    ...
    tt_int_op(num_reachable_filtered_guards(gs, NULL), OP_LE, DFLT_MIN_FILTERED_SAMPLE_SIZE);
    /* but we definitely didn't exceed the sample maximum. */
    const int n_guards = 271 / 2;
    tt_int_op(smartlist_len(gs->sampled_entry_guards), OP_LE, (int)(n_guards * .3));
...
test_entry_guard_update_from_consensus_repair(void *arg)
{
    ...
    expect_log_msg_containing("was unlisted, but with unlisted_since_date unset");
...
test_entry_guard_manage_primary(void *arg)
{
    ...
    /* Do some dirinfo checks */
    {
        ...
        char *dir_info_str = guard_selection_get_err_str_if_dir_info_missing(gs, 0, 0, 0);
...
test_entry_guard_select_for_circuit_no_confirmed(void *arg)
{
    ...
    entry_guard_t *g = select_entry_guard_for_circuit(gs, GUARD_USAGE_TRAFFIC, NULL, &state);
    ...
    entry_guard_t *g2 = select_entry_guard_for_circuit(gs, GUARD_USAGE_TRAFFIC, NULL, &state);
...
test_entry_guard_select_for_circuit_confirmed(void *arg)
{
    ...
    tt_assert(!g2->is_primary);
    tt_ptr_op(g2, OP_NE, g);
    tt_int_op(g2->confirmed_idx, OP_EQ, smartlist_len(gs->primary_entry_guards) + 1);
    ...
    g = smartlist_get(gs->confirmed_entry_guards, smartlist_len(gs->primary_entry_guards) + 2);
    rst = guard_create_exit_restriction((uint8_t *)g->identity);
    g2 = select_entry_guard_for_circuit(gs, GUARD_USAGE_TRAFFIC, rst, &state);
    tt_ptr_op(g2, OP_NE, NULL);
    tt_ptr_op(g2, OP_NE, g);
    tt_int_op(g2->confirmed_idx, OP_EQ, smartlist_len(gs->primary_entry_guards) + 3);

    // If we make every confirmed guard become pending then we start poking
    // other guards.
    const int n_remaining_confirmed = N_CONFIRMED - 3 - smartlist_len(gs->primary_entry_guards);
...
test_entry_guard_select_for_circuit_highlevel_primary(void *arg)
{
     ...
     */
    int r = entry_guard_pick_for_circuit(gs, GUARD_USAGE_TRAFFIC, NULL, &node, &guard);
    ...
    r = entry_guard_pick_for_circuit(gs, GUARD_USAGE_TRAFFIC, NULL, &node, &guard);
    tt_int_op(r, OP_EQ, 0);
    tt_assert(node);
    tt_assert(guard);
    tt_int_op(guard->state, OP_EQ, GUARD_CIRC_STATE_USABLE_ON_COMPLETION);
...
test_entry_guard_select_for_circuit_highlevel_primary_retry(void *arg)
{
    ...
    r = entry_guard_pick_for_circuit(gs, GUARD_USAGE_TRAFFIC, NULL, &node, &guard2);
    tt_int_op(r, OP_EQ, 0);
...
test_entry_guard_drop_guards(void *arg)
{
    ...
    r = entry_guard_pick_for_circuit(gs, GUARD_USAGE_TRAFFIC, NULL, &node, &guard);
    tt_int_op(r, OP_EQ, 0);
    tt_int_op(smartlist_len(gs->sampled_entry_guards), OP_GE, DFLT_MIN_FILTERED_SAMPLE_SIZE);
...
upgrade_circuits_setup(const struct testcase_t *testcase)
{
    ...
    guard_selection_t *gs = data->gs = guard_selection_new("default", GS_TYPE_NORMAL);
    circuit_guard_state_t *guard;
    const node_t *node;
    entry_guard_t *g;
    int i;
    ...
    entry_guard_pick_for_circuit(gs, GUARD_USAGE_TRAFFIC, NULL, &node, &data->guard1_state);
    ...
    entry_guard_pick_for_circuit(gs, GUARD_USAGE_TRAFFIC, NULL, &node, &data->guard2_state);
    ...
    tor_assert(data->guard1_state->state == GUARD_CIRC_STATE_USABLE_IF_NO_BETTER_GUARD);
    tor_assert(data->guard2_state->state == GUARD_CIRC_STATE_USABLE_IF_NO_BETTER_GUARD);
    ...
    if (make_circ1_succeed) {
        ...
        tor_assert(data->guard1_state->state == GUARD_CIRC_STATE_WAITING_FOR_BETTER_GUARD);
    }
    update_approx_time(data->start + 33);
    if (make_circ2_succeed) {
        r = entry_guard_succeeded(&data->guard2_state);
        tor_assert(r == GUARD_MAYBE_USABLE_LATER);
        tor_assert(data->guard2_state->state == GUARD_CIRC_STATE_WAITING_FOR_BETTER_GUARD);
    }
...
test_entry_guard_upgrade_a_circuit(void *arg)
{
    ...
    int r;
    r = entry_guards_upgrade_waiting_circuits(data->gs, data->all_origin_circuits, result);
    tt_int_op(r, OP_EQ, 1);
...
test_entry_guard_should_expire_waiting(void *arg)
{
    ...
    fake_state->state_set_at = approx_time() - DFLT_NONPRIMARY_GUARD_IDLE_TIMEOUT - 1;
    ...
    fake_state->state = GUARD_CIRC_STATE_WAITING_FOR_BETTER_GUARD;
    fake_state->state_set_at = approx_time() - DFLT_NONPRIMARY_GUARD_IDLE_TIMEOUT + 5;
    tt_assert(!entry_guard_state_should_expire(fake_state));
...
test_entry_guard_outdated_dirserver_exclusion(void *arg)
{
    ...
    guard_selection_t *gs = get_guard_selection_by_name("default", GS_TYPE_NORMAL, 1);
...

    {
        ...
        initiate_descriptor_downloads(NULL, DIR_PURPOSE_FETCH_MICRODESC, digests, 3, 7, 0);
...
test_entry_guard_vanguard_path_selection(void *arg)
{
    ...
    retval = routerset_contains_extendinfo(options->HSLayer2Nodes, l2_node->extend_info);
    tt_int_op(retval, OP_EQ, 4);
    /* test that L3 node is _not_ contained in HSLayer2Node */
    retval = routerset_contains_extendinfo(options->HSLayer2Nodes, l3_node->extend_info);
    tt_int_op(retval, OP_LT, 4);

    /* Test that L3 is indeed HSLayer3Node */
    retval = routerset_contains_extendinfo(options->HSLayer3Nodes, l3_node->extend_info);
    tt_int_op(retval, OP_EQ, 4);
    /* test that L2 node is _not_ contained in HSLayer3Node */
    retval = routerset_contains_extendinfo(options->HSLayer3Nodes, l2_node->extend_info);
    tt_int_op(retval, OP_LT, 4);
...
static const struct testcase_setup_t big_fake_network = {big_fake_network_setup,
                                                         big_fake_network_cleanup};

static const struct testcase_setup_t upgrade_circuits = {upgrade_circuits_setup,
...
#    define BFN_TEST(name)                                                                    \
        EN_TEST_BASE(name, TT_FORK, &big_fake_network, NULL),                                 \
            {#name "_reasonably_future", test_entry_guard_##name, TT_FORK, &big_fake_network, \
             (void *)(REASONABLY_FUTURE)},                                                    \
        {                                                                                     \
#            name "_reasonably_past", test_entry_guard_##name, TT_FORK, &big_fake_network,    \
                (void *)(REASONABLY_PAST)                                                     \
        }

#    define UPGRADE_TEST(name, arg)                                                           \
        EN_TEST_BASE(name, TT_FORK, &upgrade_circuits, arg),                                  \
            {#name "_reasonably_future", test_entry_guard_##name, TT_FORK, &upgrade_circuits, \
             (void *)(arg REASONABLY_FUTURE)},                                                \
        {                                                                                     \
#            name "_reasonably_past", test_entry_guard_##name, TT_FORK, &upgrade_circuits,    \
                (void *)(arg REASONABLY_PAST)                                                 \
        }
#endif /* !defined(COCCI) */
...
struct testcase_t entrynodes_tests[] = {
    ...
    UPGRADE_TEST(upgrade_not_blocked_by_restricted_circ_complete, "c1-done c2-done"),
