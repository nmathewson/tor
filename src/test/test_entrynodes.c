...
big_fake_network_setup(const struct testcase_t *testcase)
{
    ...
    if (argument) {
        reasonably_future_consensus =
            strstr(argument, REASONABLY_FUTURE) != NULL;
    ...
    if (reasonably_future_consensus) {
        ...
        /* Make the dummy consensus valid from 16 hours ago, but expired 12
         * hours ago. */
...
test_entry_guard_add_single_guard(void *arg)
{
    ...
    tt_ptr_op(g1, OP_EQ,
              get_sampled_guard_with_id(gs, (uint8_t *)n1->identity));
...
#define NUM 7
    ...
    for (i = 0; i < NUM; ++i) {
        n[i] =
            smartlist_get(big_fake_net_nodes, i * 2); // even ones are guards.
...
test_entry_guard_update_from_consensus_status(void *arg)
{
    ...
    /* Don't randomly backdate stuff; it will make correctness harder to
     * check.*/
    ...
    update_approx_time(start + 30);
    {
        ...
        dummy_consensus = NULL;
        sampled_guards_update_from_consensus(gs);
        tt_i64_op(smartlist_len(gs->sampled_entry_guards), OP_EQ,
                  n_sampled_pre);
...
test_entry_guard_sample_reachable_filtered(void *arg)
{
    ...
    for (j = 0; tests[j].flag >= 0; ++j) {
        ...
        for (i = 0; i < N; ++i) {
            g = sample_reachable_filtered_entry_guards(gs, NULL,
                                                       excluded_flags);
            tor_assert(g);
        ...
        for (i = 0; i < n_guards; ++i) {
            const int should_be_set =
                (i != excluded_idx && i != 3); // filtered out.
...
test_entry_guard_manage_primary(void *arg)
{
    ...
    });
    {
        entry_guard_t *prev_last_guard =
            smartlist_get(prev_guards, n_primary - 1);
    ...
    {
        /* Check that we have all required dirinfo for the primaries (that's
         * done in big_fake_network_setup()) */
        char *dir_info_str =
        ...
        /* Change the first primary's identity digest so that the mocked
         * functions can't find its descriptor */
...
test_entry_guard_select_for_circuit_no_confirmed(void *arg)
{
    ...
    // If we say that the first primary guard was last tried a long time ago,
    // we should get an automatic retry on it.
    ...
    // As a bonus, maybe we should be retrying the primary guards. Let's say
    // so.
...
test_entry_guard_select_for_circuit_confirmed(void *arg)
{
    ...
    tt_assert(g);
    tt_assert(!g->is_primary);
    tt_int_op(g->confirmed_idx, OP_EQ,
              smartlist_len(gs->primary_entry_guards));
    ...
    for (i = 0; i < n_remaining_confirmed; ++i) {
        g = select_entry_guard_for_circuit(gs, GUARD_USAGE_TRAFFIC, NULL,
                                           &state);
...
test_entry_guard_select_for_circuit_highlevel_confirm_other(void *arg)
{
    ...
    /* At the start, we have no confirmed guards.  We'll mark the primary
     * guards down, then confirm something else.  As soon as we do, it should
     * become primary, and we should get it next time. */
    ...
    u = entry_guard_succeeded(&guard);
    /* We're on the internet (by fiat), so this guard will get called
     * "confirmed" and should immediately become primary.
     */
    tt_int_op(guard->state, OP_EQ, GUARD_CIRC_STATE_COMPLETE);
...
test_entry_guard_upgrade_a_circuit(void *arg)
{
    ...
    int r;
    r = entry_guards_upgrade_waiting_circuits(
        data->gs, data->all_origin_circuits, result);
    tt_int_op(r, OP_EQ, 1);
...
test_entry_guard_upgrade_not_blocked_by_restricted_circ_complete(void *arg)
{
    ...
    /* Now, we try again. Since circ2 has a restriction that circ1 doesn't
     * obey, circ2 _is_ eligible for upgrade. */
    smartlist_clear(result);
    r = entry_guards_upgrade_waiting_circuits(
        data->gs, data->all_origin_circuits, result);
    tt_int_op(r, OP_EQ, 1);
...
test_entry_guard_outdated_dirserver_exclusion(void *arg)
{
     ...
     *  2. Order another microdescriptor fetch and make sure that primary
     * guards get skipped since they failed previous fetches.
     */
...

    {
        ...
        SMARTLIST_FOREACH_BEGIN (gs->primary_entry_guards,
                                 const entry_guard_t *, g) {
...

    {
        ...
        /* ... and check that because we failed to fetch microdescs from all
         * our
         * primaries, we didn't end up selecting a primary for fetching dir
         * info */
...
helper_setup_vanguard_list(or_options_t *options)
{
    ...
    for (i = 0; i < 10; i += 2) {
        ...
        routerset_parse(options->HSLayer2Nodes, vanguard_node->rs->nickname,
                        "l2");
    }
    ...
    for (i = 10; i < 20; i += 2) {
        node_t *vanguard_node = smartlist_get(big_fake_net_nodes, i);
        tt_assert(vanguard_node->is_possible_guard);
        routerset_parse(options->HSLayer3Nodes, vanguard_node->rs->nickname,
                        "l3");
    }
