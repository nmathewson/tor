...
test_conscache_cleanup(void *arg)
{
    (void)arg;
    const int N = 20;
    consensus_cache_entry_t **ents = tor_calloc(N, sizeof(consensus_cache_entry_t *));
...
test_conscache_filter(void *arg)
{
    ...
    for (i = 0; i < N; ++i) {
        ...
        uint8_t *body = tor_malloc(bodylen);
        memset(body, i, bodylen);
        consensus_cache_entry_t *ent = consensus_cache_add(cache, labels, body, bodylen);
        tor_free(body);
        config_free_lines(labels);
        tt_assert(ent);
        consensus_cache_entry_decref(ent);
...
struct testcase_t conscache_tests[] = {ENT(open_failure), ENT(simple_usage), ENT(cleanup),
                                       ENT(filter), END_OF_TESTCASES};
