...
#define STR_DESC_ID_BASE32 "hba3gmcgpfivzfhx5rtfqkfdhv65yrj3"
    ...
    expected_msg =
        ...
        " NO_AUTH " STR_HSDIR_EXIST_LONGNAME " " STR_DESC_ID_BASE32 "\r\n";
...
test_hs_rend_data(void *arg)
{
    ...
    for (rep = 0; rep < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; rep++) {
        int ret =
            rend_compute_v2_desc_id(desc_id, client_v2->onion_address,
    ...
    for (rep = 0; rep < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; rep++) {
        tt_int_op(tor_digest_is_zero(service_v2->descriptor_id[rep]), OP_EQ,
                  1);
    }
    ...
    tt_mem_op(service_dup_v2->rend_pk_digest, OP_EQ,
              service_v2->rend_pk_digest,
...
#define TEST_COOKIE_ENCODED_INVALID "YWJjZGVmZ2hpamtsbW5vcD"
    ...
    encoded_cookie =
        rend_auth_encode_cookie(TEST_COOKIE_RAW, REND_STEALTH_AUTH);
...
test_single_onion_poisoning(void *arg)
{
    ...
    ret =
        hs_check_service_private_dir(mock_options->User, service_1->directory,
    ...
    ret =
        hs_check_service_private_dir(mock_options->User, service_2->directory,
    ...
    /* Now remove the poisoning from the first service, and we have the
     * opposite problem. */
...
test_prune_services_on_reload(void *arg)
{
    ...
    /* Create a non ephemeral service with the _same_ path as so we can test
     * the
    ...
    {
        /* This test will make sure that only the ephemeral service is kept if
         * the new list is empty. The old list should contain only the non
         * ephemeral one. */
        smartlist_add(old, s1);
        smartlist_add(old, e1);
        set_rend_service_list(old);
    ...
    {
        /* This test makes sure that the new list stays the same even from the
         * old list being completely different. */
        smartlist_add(new, s1);
