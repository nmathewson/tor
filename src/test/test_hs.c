...
#define HSDIR_EXIST_ID                         \
    ...
    "\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA"
#define STR_HSDIR_EXIST_LONGNAME "$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=TestDir"
...
#define HSDIR_NONE_EXIST_ID                    \
    ...
    "\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB\xBB"
#define STR_HSDIR_NONE_EXIST_LONGNAME "$BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB"
...
#define STR_DESC_ID_BASE32 "hba3gmcgpfivzfhx5rtfqkfdhv65yrj3"
    ...
    strncpy(rend_query.onion_address, STR_HS_ADDR, REND_SERVICE_ID_LEN_BASE32 + 1);
    rend_query.auth_type = REND_NO_AUTH;
    rend_query.base_.hsdirs_fp = smartlist_new();
    smartlist_add(rend_query.base_.hsdirs_fp, tor_memdup(HSDIR_EXIST_ID, DIGEST_LEN));

    /* Compute descriptor ID for replica 0, should be STR_DESC_ID_BASE32. */
    ret =
        rend_compute_v2_desc_id(rend_query.descriptor_id[0], rend_query.onion_address, NULL, 0, 0);
    tt_int_op(ret, OP_EQ, 0);
    base32_encode(desc_id_base32, sizeof(desc_id_base32), rend_query.descriptor_id[0], DIGEST_LEN);
    ...
    control_event_hs_descriptor_requested(rend_query.onion_address, rend_query.auth_type,
                                          HSDIR_EXIST_ID, STR_DESC_ID_BASE32, NULL);
    expected_msg = "650 HS_DESC REQUESTED " STR_HS_ADDR " NO_AUTH " STR_HSDIR_EXIST_LONGNAME
                   " " STR_DESC_ID_BASE32 "\r\n";
    ...
    control_event_hsv2_descriptor_received(rend_query.onion_address, &rend_query.base_,
                                           HSDIR_EXIST_ID);
    expected_msg = "650 HS_DESC RECEIVED " STR_HS_ADDR " BASIC_AUTH " STR_HSDIR_EXIST_LONGNAME
                   " " STR_DESC_ID_BASE32 "\r\n";
    tt_assert(received_msg);
    tt_str_op(received_msg, OP_EQ, expected_msg);
    tor_free(received_msg);
    ...
    control_event_hsv2_descriptor_failed(&rend_query.base_, HSDIR_NONE_EXIST_ID, "QUERY_REJECTED");
    expected_msg = "650 HS_DESC FAILED " STR_HS_ADDR " STEALTH_AUTH " STR_HSDIR_NONE_EXIST_LONGNAME
    ...
    control_event_hsv2_descriptor_failed(&rend_query.base_, HSDIR_EXIST_ID, "QUERY_REJECTED");
    expected_msg = "650 HS_DESC FAILED " STR_HS_ADDR " UNKNOWN " STR_HSDIR_EXIST_LONGNAME
    ...
    rend_query.auth_type = REND_NO_AUTH;
    control_event_hsv2_descriptor_failed(&rend_query.base_, NULL, "QUERY_NO_HSDIR");
    ...
    rend_query.auth_type = REND_NO_AUTH;
    control_event_hsv2_descriptor_failed(&rend_query.base_, NULL, "QUERY_RATE_LIMITED");
    expected_msg = "650 HS_DESC FAILED " STR_HS_ADDR " NO_AUTH "
    ...
    char *exp_msg;
    control_event_hs_descriptor_content(rend_query.onion_address, STR_HS_CONTENT_DESC_ID, NULL,
                                        NULL);
    tor_asprintf(&exp_msg, "650+HS_DESC_CONTENT " STR_HS_ADDR " " STR_HS_CONTENT_DESC_ID " UNKNOWN"
                           "\r\n\r\n.\r\n650 OK\r\n");
    tt_assert(received_msg);
    ...
    control_event_hs_descriptor_content(rend_query.onion_address, STR_HS_CONTENT_DESC_ID,
                                        HSDIR_EXIST_ID, hs_desc_content_control);
...
test_hs_rend_data(void *arg)
{
    ...
    base32_decode(desc_id, sizeof(desc_id), STR_DESC_ID_BASE32, REND_DESC_ID_V2_LEN_BASE32);
    memset(client_cookie, 'e', sizeof(client_cookie));

    client = rend_data_client_create(STR_HS_ADDR, desc_id, client_cookie, REND_NO_AUTH);
    tt_assert(client);
    ...
    tt_mem_op(client_v2->descriptor_cookie, OP_EQ, client_cookie, sizeof(client_cookie));
    ...
    for (rep = 0; rep < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; rep++) {
        tt_mem_op(client_dup_v2->descriptor_id[rep], OP_EQ, client_v2->descriptor_id[rep],
                  DIGEST_LEN);
    }
    /* The rest should be zeroed because this is a client request. */
    ...
    tt_mem_op(client_v2->desc_id_fetch, OP_EQ, desc_id, sizeof(desc_id));
    tt_int_op(fast_mem_is_zero(client_v2->descriptor_cookie, sizeof(client_v2->descriptor_cookie)),
              OP_EQ, 1);
    tt_assert(client->hsdirs_fp);
    tt_int_op(smartlist_len(client->hsdirs_fp), OP_EQ, 0);
    for (rep = 0; rep < REND_NUMBER_OF_NON_CONSECUTIVE_REPLICAS; rep++) {
    ...
    service = rend_data_service_create(STR_HS_ADDR, rend_pk_digest, rend_cookie, REND_NO_AUTH);
    tt_assert(service);
    ...
    tt_mem_op(service_v2->rend_pk_digest, OP_EQ, rend_pk_digest, sizeof(rend_pk_digest));
...
#define TEST_COOKIE_ENCODED_INVALID "YWJjZGVmZ2hpamtsbW5vcD"
    ...
    re = rend_auth_decode_cookie(TEST_COOKIE_ENCODED, raw_cookie, &auth_type, &err_msg);
    tt_assert(!re);
    ...
    re = rend_auth_decode_cookie(TEST_COOKIE_ENCODED_STEALTH, raw_cookie, &auth_type, &err_msg);
    tt_assert(!re);
    tt_ptr_op(err_msg, OP_EQ, NULL);
    tt_mem_op(raw_cookie, OP_EQ, TEST_COOKIE_RAW, REND_DESC_COOKIE_LEN);
    ...
    re = rend_auth_decode_cookie(TEST_COOKIE_ENCODED "==", raw_cookie, NULL, &err_msg);
    tt_assert(!re);
    tt_ptr_op(err_msg, OP_EQ, NULL);
    tt_mem_op(raw_cookie, OP_EQ, TEST_COOKIE_RAW, REND_DESC_COOKIE_LEN);

    /* Decoding with an unknown type should fail */
    re = rend_auth_decode_cookie(TEST_COOKIE_ENCODED_INVALID, raw_cookie, &auth_type, &err_msg);
...
test_single_onion_poisoning(void *arg)
{
    ...
    rend_service_port_config_t *port1 = rend_service_parse_port_config("80", " ", &err_msg);
    tt_assert(port1);
    tt_ptr_op(err_msg, OP_EQ, NULL);
    smartlist_add(service_1->ports, port1);

    rend_service_port_config_t *port2 = rend_service_parse_port_config("90", " ", &err_msg);
...
struct testcase_t hs_tests[] = {
    ...
    {"hs_parse_static_v2_desc", test_hs_parse_static_v2_desc, TT_FORK, NULL, NULL},
    {"hs_desc_event", test_hs_desc_event, TT_FORK, NULL, NULL},
    {"hs_auth_cookies", test_hs_auth_cookies, TT_FORK, NULL, NULL},
    {"single_onion_poisoning_create_dir_none", test_single_onion_poisoning, TT_FORK,
     &passthrough_setup, (void *)(CREATE_HS_DIR_NONE)},
    {"single_onion_poisoning_create_dir1", test_single_onion_poisoning, TT_FORK, &passthrough_setup,
     (void *)(CREATE_HS_DIR1)},
    {"single_onion_poisoning_create_dir2", test_single_onion_poisoning, TT_FORK, &passthrough_setup,
     (void *)(CREATE_HS_DIR2)},
    {"single_onion_poisoning_create_dir_both", test_single_onion_poisoning, TT_FORK,
     &passthrough_setup, (void *)(CREATE_HS_DIR1 | CREATE_HS_DIR2)},
    {"prune_services_on_reload", test_prune_services_on_reload, TT_FORK, NULL, NULL},
