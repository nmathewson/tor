...
test_addr_basic(void *arg)
{
    ...
    /* Test inet_ntop */
    {
        ...
        tt_str_op(tor_inet_ntop(AF_INET, &in, tmpbuf, strlen(ip) + 1), OP_EQ,
                  ip);

        /* too short buffer */
        tt_ptr_op(tor_inet_ntop(AF_INET, &in, tmpbuf, strlen(ip)), OP_EQ,
                  NULL);
    }
            ...
            tt_assert_test_fmt_type(                                    \
                ...
                {                                                       \
                    ...
                    for (int ii_ = 0; ii_ < 16; ++ii_) {                \
                        tor_snprintf(cp, 3, "%02x",                     \
                                     (unsigned)value_->s6_addr[ii_]);   \
                        cp += 2;                                        \
                        if (ii_ != 15)                                  \
                            *cp++ = ':';                                \
                    }                                                   \
...
#    define test_addr_compare(a, op, b)                                      \
        STMT_BEGIN                                                           \
            tt_int_op(tor_inet_pton(AF_INET6, a, &t1.addr.in6_addr), OP_EQ,  \
                      1);                                                    \
            tt_int_op(tor_inet_pton(AF_INET6, b, &t2.addr.in6_addr), OP_EQ,  \
                      1);                                                    \
...
#    define test_addr_compare_masked(a, op, b, m)                            \
        STMT_BEGIN                                                           \
            ...
            if (!(r op 0))                                                   \
                TT_DIE(("Failed: tor_addr_compare_masked(%s,%s,%d) %s 0", a, \
                        b, m, #op));                                         \
        STMT_END
#endif /* !defined(COCCI) */
...
#define test_addr_mask_ports_parse(xx, f, ip1, ip2, ip3, ip4, mm, pt1, pt2) \
    STMT_BEGIN                                                              \
        tt_int_op(                                                          \
            tor_addr_parse_mask_ports(xx, 0, &t1, &mask, &port1, &port2),   \
            OP_EQ, f);                                                      \
...
test_addr_ip6_helpers(void *arg)
{
    ...
    (void)arg;
    {
        ...
        tt_str_op(tor_inet_ntop(AF_INET6, &a1, buf, strlen(ip) + 1), OP_EQ,
                  ip);
        ...
        tt_ptr_op(tor_inet_ntop(AF_INET6, &a2, buf, strlen(ip_ffff)), OP_EQ,
                  NULL);
    }
    ...
    r = tor_inet_pton(AF_INET6, "0102:0304:0506:0708:090A:0B0C:0D0E:0F10",
                      &a1);
    tt_int_op(r, OP_EQ, 1);
    for (i = 0; i < 16; ++i) {
    ...
    tt_int_op(0, OP_EQ, i);
    i = tor_addr_parse_PTR_name(
        &t1, "9999999999999999999999999999.in-addr.arpa", AF_UNSPEC, 1);
    tt_int_op(-1, OP_EQ, i);
    ...
    r = tor_addr_parse_mask_ports(
        "[ffff:1111:1111:1111:1111:1111:1111:1111:1]", 0, &t1, NULL, NULL,
        NULL);
    tt_int_op(r, OP_EQ, -1);
    r = tor_addr_parse_mask_ports(
    ...
    tt_int_op(r, OP_EQ, -1);
    r = tor_addr_parse_mask_ports("*4/16", TAPMP_EXTENDED_STAR, &t1, &mask,
                                  NULL, NULL);
    tt_int_op(r, OP_EQ, -1);
    r = tor_addr_parse_mask_ports("*6/30", TAPMP_EXTENDED_STAR, &t1, &mask,
                                  NULL, NULL);
    tt_int_op(r, OP_EQ, -1);
    /* Basic mask tests*/
    ...
    r = tor_addr_parse_mask_ports("[efef::]/112", 0, &t1, &mask, &port1,
                                  &port2);
    tt_int_op(r, OP_EQ, AF_INET6);
    ...
    r = tor_addr_parse_mask_ports("*:8000-9000", TAPMP_EXTENDED_STAR, &t1,
                                  &mask, &port1, &port2);
    ...
    r = tor_addr_parse_mask_ports("*6", TAPMP_EXTENDED_STAR, &t1, &mask,
                                  &port1, &port2);
    tt_int_op(r, OP_EQ, AF_INET6);
    tt_int_op(tor_addr_family(&t1), OP_EQ, AF_INET6);
...
#define TEST_ADDR_V6_PARSE_CANONICAL(addr_str, fmt_decorated)              \
    STMT_BEGIN                                                             \
        ...
        TEST_ADDR_LOOKUP_FMT(addr_str, AF_UNSPEC, AF_INET6, fmt_decorated, \
                             addr_str);                                    \
        TEST_ADDR_PORT_LOOKUP_FMT(addr_str, AF_INET6, fmt_decorated,       \
                                  addr_str, 0);                            \
        TEST_ADDR_PORT_PARSE_XFAIL(addr_str, -1);                          \
...
test_addr_parse(void *arg)
{
     ...
     * We reject it either way, but some OS host lookup routines accept it as
    ...
    for (ipv6 = 0; ipv6 <= 1; ++ipv6) {
        for (bits = 0; bits < 18; ++bits) {
            ...
            for (iter = 0; iter < 128; ++iter) {
                 ...
                 */
                tt_int_op(0, OP_EQ,
                          tor_addr_compare_masked(&a, &cfg[ipv6].addr, bits,
                                                  CMP_EXACT));
