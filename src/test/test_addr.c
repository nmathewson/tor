...
#    define test_op_ip6_(a, op, b, e1, e2)                                            \
        STMT_BEGIN                                                                    \
            tt_assert_test_fmt_type(                                                  \
                ...
                (fast_memcmp(val1_->s6_addr, val2_->s6_addr, 16) op 0), char *, "%s", \
                {                                                                     \
                    char *cp;                                                         \
...
#define test_addr_mask_ports_parse(xx, f, ip1, ip2, ip3, ip4, mm, pt1, pt2)                \
    STMT_BEGIN                                                                             \
        tt_int_op(tor_addr_parse_mask_ports(xx, 0, &t1, &mask, &port1, &port2), OP_EQ, f); \
...
test_addr_ip6_helpers(void *arg)
{
    ...
    (void)arg;
    {
        ...
        tt_str_op(tor_inet_ntop(AF_INET6, &a2, buf, strlen(ip_ffff) + 1), OP_EQ, ip_ffff);
    ...
    tt_int_op(sizeof(struct sockaddr_in), OP_EQ,
              tor_addr_to_sockaddr(&t1, 1234, (struct sockaddr *)&sa_storage, sizeof(sa_storage)));
    ...
    tt_int_op(sizeof(struct sockaddr_in6), OP_EQ,
              tor_addr_to_sockaddr(&t1, 9999, (struct sockaddr *)&sa_storage, sizeof(sa_storage)));
    ...
    /* shortened words. */
    test_pton6_same("0001:0099:BEEF:0000:0123:FFFF:0001:0001", "1:99:BEEF:0:0123:FFFF:1:1");
    /* zeros at the beginning */
    test_pton6_same("0000:0000:0000:0000:0009:C0A8:0001:0001", "::9:c0a8:1:1");
    test_pton6_same("0000:0000:0000:0000:0009:C0A8:0001:0001", "::9:c0a8:0.1.0.1");
    /* zeros in the middle. */
    test_pton6_same("fe80:0000:0000:0000:0202:1111:0001:0001", "fe80::202:1111:1:1");
    /* zeros at the end. */
    ...
    test_ntop6_reduces("0001:0099:BEEF:0006:0123:FFFF:0001:0001", "1:99:beef:6:123:ffff:1:1");
    ...
    test_ntop6_reduces("0000:0000:0000:0000:0009:C0A8:0001:0001", "::9:c0a8:1:1");
    test_ntop6_reduces("fe80:0000:0000:0000:0202:1111:0001:0001", "fe80::202:1111:1:1");
    test_ntop6_reduces("1000:0001:0000:0007:0000:0000:0000:0000", "1000:1:0:7::");
    ...
    tt_int_op(0, OP_EQ, i);
    i = tor_addr_parse_PTR_name(&t1, "9999999999999999999999999999.in-addr.arpa", AF_UNSPEC, 1);
    tt_int_op(-1, OP_EQ, i);
    ...
    tt_int_op(tor_addr_to_PTR_name(rbuf, strlen("3.2.1.127.in-addr.arpa") - 1, &t1), OP_EQ, -1);
...

    {
        ...
        tt_int_op(tor_addr_to_PTR_name(rbuf, sizeof(rbuf), &t1), OP_EQ, strlen(addr_PTR));
        tt_str_op(rbuf, OP_EQ, addr_PTR);
    }

    /* XXXX turn this into a separate function; it's not all IPv6. */
    /* test tor_addr_parse_mask_ports */
    test_addr_mask_ports_parse("[::f]/17:47-95", AF_INET6, 0, 0, 0, 0x0000000f, 17, 47, 95);
    tt_str_op(p1, OP_EQ, "::f");
    // test_addr_parse("[::fefe:4.1.1.7/120]:999-1000");
    // test_addr_parse_check("::fefe:401:107", 120, 999, 1000);
    test_addr_mask_ports_parse("[::ffff:4.1.1.7]/120:443", AF_INET6, 0, 0, 0x0000ffff, 0x04010107,
                               120, 443, 443);
    tt_str_op(p1, OP_EQ, "::ffff:4.1.1.7");
    test_addr_mask_ports_parse("[abcd:2::44a:0]:2-65000", AF_INET6, 0xabcd0002, 0, 0, 0x044a0000,
                               128, 2, 65000);

    tt_str_op(p1, OP_EQ, "abcd:2::44a:0");
    /* Try some long addresses. */
    r = tor_addr_parse_mask_ports("[ffff:1111:1111:1111:1111:1111:1111:1111]", 0, &t1, NULL, NULL,
                                  NULL);
    tt_int_op(r, OP_EQ, AF_INET6);
    r = tor_addr_parse_mask_ports("[ffff:1111:1111:1111:1111:1111:1111:11111]", 0, &t1, NULL, NULL,
                                  NULL);
    tt_int_op(r, OP_EQ, -1);
    r = tor_addr_parse_mask_ports("[ffff:1111:1111:1111:1111:1111:1111:1111:1]", 0, &t1, NULL, NULL,
                                  NULL);
    tt_int_op(r, OP_EQ, -1);
    ...
    tt_int_op(r, OP_EQ, -1);
    r = tor_addr_parse_mask_ports("[f:f:f:f:f::]/255.255.0.0", 0, &t1, NULL, NULL, NULL);
    tt_int_op(r, OP_EQ, -1);
    ...
    tt_int_op(r, OP_EQ, -1);
    r = tor_addr_parse_mask_ports("*4/16", TAPMP_EXTENDED_STAR, &t1, &mask, NULL, NULL);
    tt_int_op(r, OP_EQ, -1);
    r = tor_addr_parse_mask_ports("*6/30", TAPMP_EXTENDED_STAR, &t1, &mask, NULL, NULL);
    tt_int_op(r, OP_EQ, -1);
    /* Basic mask tests*/
    ...
    r = tor_addr_parse_mask_ports("*:8000-9000", TAPMP_EXTENDED_STAR, &t1, &mask, &port1, &port2);
    ...
    r = tor_addr_parse_mask_ports("*4:6667", TAPMP_EXTENDED_STAR, &t1, &mask, &port1, &port2);
    tt_int_op(r, OP_EQ, AF_INET);
    tt_int_op(tor_addr_family(&t1), OP_EQ, AF_INET);
    tt_int_op(tor_addr_to_ipv4h(&t1), OP_EQ, 0);
    tt_int_op(mask, OP_EQ, 0);
    ...
    r = tor_addr_parse_mask_ports("*6", TAPMP_EXTENDED_STAR, &t1, &mask, &port1, &port2);
    tt_int_op(r, OP_EQ, AF_INET6);
    tt_int_op(tor_addr_family(&t1), OP_EQ, AF_INET6);
    ...
    tt_int_op(TOR_ADDR_BUF_LEN, OP_GE, sizeof("ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255"));
    ...
    TT_BLATHER(("v4 address: %s (family=%d)", fmt_addr(&t1), tor_addr_family(&t1)));
    TT_BLATHER(("v6 address: %s (family=%d)", fmt_addr(&t2), tor_addr_family(&t2)));
 ...
 */
#define TEST_ADDR_PARSE_FMT(addr_str, expect_family, fmt_decorated, expect_str) \
    STMT_BEGIN                                                                  \
 ...
 */
#define TEST_ADDR_PORT_PARSE_FMT(addr_port_str, default_port, expect_family, fmt_decorated, \
                                 expect_str, expect_port)                                   \
    STMT_BEGIN                                                                              \
        r = tor_addr_port_parse(LOG_DEBUG, addr_port_str, &addr, &port, default_port);      \
        tt_int_op(r, OP_EQ, 0);                                                             \
 ...
 *  - the fmt_decorated result of tor_addr_to_str() is expect_str.
 */
#define TEST_ADDR_LOOKUP_FMT(addr_str, require_family, expect_family, fmt_decorated, expect_str) \
    STMT_BEGIN                                                                                   \
 ...
 *  - the port is expect_port.
 */
#define TEST_ADDR_PORT_LOOKUP_FMT(addr_port_str, expect_family, fmt_decorated, expect_str, \
                                  expect_port)                                             \
    STMT_BEGIN                                                                             \
...
#define TEST_HOST_PORT_LOOKUP(host_port_str, expect_success_port)                               \
    STMT_BEGIN                                                                                  \
        ...
        if (r == -1) {                                                                          \
            ...
            tt_assert(tor_addr_family(&addr) == AF_INET || tor_addr_family(&addr) == AF_INET6); \
...
#define TEST_ADDR_V6_PARSE_CANONICAL(addr_str, fmt_decorated)                            \
    STMT_BEGIN                                                                           \
        ...
        TEST_ADDR_PORT_PARSE_FMT(addr_str, 222, AF_INET6, fmt_decorated, addr_str, 222); \
        TEST_ADDR_LOOKUP_FMT(addr_str, AF_INET6, AF_INET6, fmt_decorated, addr_str);     \
        TEST_ADDR_LOOKUP_FMT(addr_str, AF_UNSPEC, AF_INET6, fmt_decorated, addr_str);    \
        TEST_ADDR_PORT_LOOKUP_FMT(addr_str, AF_INET6, fmt_decorated, addr_str, 0);       \
        TEST_ADDR_PORT_PARSE_XFAIL(addr_str, -1);                                        \
...
#define TEST_ADDR_V6_PARSE(addr_str, fmt_decorated, expect_str)                            \
    STMT_BEGIN                                                                             \
        ...
        TEST_ADDR_PORT_PARSE_FMT(addr_str, 333, AF_INET6, fmt_decorated, expect_str, 333); \
        TEST_ADDR_LOOKUP_FMT(addr_str, AF_INET6, AF_INET6, fmt_decorated, expect_str);     \
        TEST_ADDR_LOOKUP_FMT(addr_str, AF_UNSPEC, AF_INET6, fmt_decorated, expect_str);    \
        TEST_ADDR_PORT_LOOKUP_FMT(addr_str, AF_INET6, fmt_decorated, expect_str, 0);       \
        TEST_ADDR_PORT_PARSE_XFAIL(addr_str, -1);                                          \
        TEST_ADDR_V4_LOOKUP_XFAIL(addr_str);                                               \
        TEST_ADDR_LOOKUP_XFAIL(addr_str, AF_INET);                                         \
    STMT_END
...
#define TEST_ADDR_V4_PORT_PARSE(addr_port_str, expect_str, expect_port)                    \
    STMT_BEGIN                                                                             \
        TEST_ADDR_PORT_PARSE_FMT(addr_port_str, -1, AF_INET, 0, expect_str, expect_port);  \
        TEST_ADDR_PORT_PARSE_FMT(addr_port_str, 444, AF_INET, 0, expect_str, expect_port); \
        TEST_ADDR_PORT_LOOKUP_FMT(addr_port_str, AF_INET, 0, expect_str, expect_port);     \
...
#define TEST_ADDR_V6_PORT_PARSE(addr_port_str, expect_str, expect_port)                     \
    STMT_BEGIN                                                                              \
        TEST_ADDR_PORT_PARSE_FMT(addr_port_str, -1, AF_INET6, 0, expect_str, expect_port);  \
        TEST_ADDR_PORT_PARSE_FMT(addr_port_str, 555, AF_INET6, 0, expect_str, expect_port); \
        TEST_ADDR_PORT_LOOKUP_FMT(addr_port_str, AF_INET6, 0, expect_str, expect_port);     \
        TEST_ADDR_PARSE_XFAIL(addr_port_str);                                               \
        TEST_ADDR_V4_LOOKUP_XFAIL(addr_port_str);                                           \
        TEST_ADDR_LOOKUP_XFAIL(addr_port_str, AF_INET6);                                    \
        TEST_ADDR_LOOKUP_XFAIL(addr_port_str, AF_UNSPEC);                                   \
...
test_addr_parse(void *arg)
{
    ...
    TEST_ADDR_V6_PORT_PARSE("[11:22:33:44:55:66:77:88]:99", "11:22:33:44:55:66:77:88", 99);
...
static void
update_difference(int ipv6, uint8_t *d, const tor_addr_t *a, const tor_addr_t *b)
{
    ...
    for (ipv6 = 0; ipv6 <= 1; ++ipv6) {
        for (bits = 0; bits < 18; ++bits) {
            ...
            for (iter = 0; iter < 128; ++iter) {
                ...
                tt_int_op(0, OP_EQ, tor_addr_compare_masked(&a, &cfg[ipv6].addr, bits, CMP_EXACT));
...
test_virtaddrmap_persist(void *data)
{
    ...
    // Try a hostname.
    a = addressmap_register_virtual_address(RESOLVED_TYPE_HOSTNAME, tor_strdup("foobar.baz"));
    tt_assert(a);
    ...
    b = addressmap_register_virtual_address(RESOLVED_TYPE_HOSTNAME, tor_strdup("nescio.baz"));
    tt_assert(a);
    tt_assert(b);
    ...
    tt_int_op(0, OP_EQ, parse_virtual_addr_network("192.168.0.0/16", AF_INET, 0, NULL));
    a = addressmap_register_virtual_address(RESOLVED_TYPE_IPV4, tor_strdup("foobar.baz"));
    tt_assert(a);
    tt_assert(!strcmpstart(a, "192.168."));
    tor_addr_parse(&addr, a);
    tt_int_op(AF_INET, OP_EQ, tor_addr_family(&addr));

    b = addressmap_register_virtual_address(RESOLVED_TYPE_IPV4, tor_strdup("quuxit.baz"));
    tt_str_op(b, OP_NE, a);
    ...
    a = addressmap_register_virtual_address(RESOLVED_TYPE_IPV4, tor_strdup("wumble.onion"));
    b = addressmap_register_virtual_address(RESOLVED_TYPE_IPV4, tor_strdup("wumpus.onion"));
    tt_str_op(a, OP_EQ, "192.168.3.4");
    tt_str_op(b, OP_EQ, "192.168.7.240");
    testing_disable_prefilled_rng();

    // Now try IPv6!
    tt_int_op(0, OP_EQ, parse_virtual_addr_network("1010:F000::/20", AF_INET6, 0, NULL));
    a = addressmap_register_virtual_address(RESOLVED_TYPE_IPV6, tor_strdup("foobar.baz"));
    tt_assert(a);
    tt_assert(!strcmpstart(a, "[1010:f"));
    tor_addr_parse(&addr, a);
    tt_int_op(AF_INET6, OP_EQ, tor_addr_family(&addr));

    b = addressmap_register_virtual_address(RESOLVED_TYPE_IPV6, tor_strdup("quuxit.baz"));
    tt_str_op(b, OP_NE, a);
    ...
    a = addressmap_register_virtual_address(RESOLVED_TYPE_IPV6, tor_strdup("wuffle.baz"));
    b = addressmap_register_virtual_address(RESOLVED_TYPE_IPV6, tor_strdup("gribble.baz"));
    ...
    a = addressmap_register_virtual_address(RESOLVED_TYPE_IPV4, tor_strdup("might-work.onion"));
    if (a) {
    ...
    // out "1" forever.
    b = addressmap_register_virtual_address(RESOLVED_TYPE_IPV4, tor_strdup("wont-work.onion"));
    tt_assert(b == NULL);
...
#define get_ipv4(test_addr, str, iprv)                       \
    STMT_BEGIN                                               \
...
#define get_ipv6(test_addr, str, iprv)                                  \
    STMT_BEGIN                                                          \
        test_addr = tor_malloc(sizeof(tor_addr_t));                     \
