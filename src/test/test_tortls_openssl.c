...
test_tortls_classify_client_ciphers(void *ignored)
{
    ...
    SSL_CIPHER *one = get_cipher_by_name(TLS1_TXT_DHE_RSA_WITH_AES_128_SHA),
               ...
               *three = get_cipher_by_name(SSL3_TXT_EDH_RSA_DES_192_CBC3_SHA), *four = NULL;
...
#    ifndef OPENSSL_1_1_API
    tt_assert(!(tls->ssl->s3->flags & SSL3_FLAGS_ALLOW_UNSAFE_LEGACY_RENEGOTIATION));
#    endif
...
test_tortls_unblock_renegotiation(void *ignored)
{
    ...
    tt_uint_op(SSL_get_options(tls->ssl) & SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION, OP_EQ,
...
static X509 *
fixed_tor_tls_create_certificate(crypto_pk_t *rsa, crypto_pk_t *rsa_sign, const char *cname,
                                 const char *cname_sign, unsigned int cert_lifetime)
{
    ...
    (void)cert_lifetime;
    X509 *result =
        fixed_tor_tls_create_certificate_result[fixed_tor_tls_create_certificate_result_index++];
    if (result)
...
test_tortls_context_new(void *ignored)
{
    ...
    crypto_pk_t *pk1, *pk2, *pk3, *pk4, *pk5, *pk6, *pk7, *pk8, *pk9, *pk10, *pk11, *pk12, *pk13,
        *pk14, *pk15, *pk16, *pk17, *pk18;
