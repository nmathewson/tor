...
test_cfmt_begin_cells(void *arg)
{
    ...
    memset(&bcell, 0x7f, sizeof(bcell));
    make_relay_cell(&cell, RELAY_COMMAND_BEGIN, "[2620::6b0:b:1a1a:0:26e5:480e]:80", 34);
    tt_int_op(0, OP_EQ, begin_cell_parse(&cell, &bcell, &end_reason));
...
test_cfmt_connected_cells(void *arg)
{
    ...
    make_relay_cell(&cell, RELAY_COMMAND_CONNECTED, "\x02\x03\x04\x05\x00\x00\x0e\x10", 8);
    relay_header_unpack(&rh, cell.payload);
    r = connected_cell_parse(&rh, &cell, &addr, &ttl);
    tt_int_op(r, OP_EQ, 0);
    tt_int_op(tor_addr_family(&addr), OP_EQ, AF_INET);
    ...
    make_relay_cell(&cell, RELAY_COMMAND_CONNECTED, "\x02\x03\x04\x05\xf0\x00\x00\x00", 8);
    relay_header_unpack(&rh, cell.payload);
    r = connected_cell_parse(&rh, &cell, &addr, &ttl);
    tt_int_op(r, OP_EQ, 0);
    tt_int_op(tor_addr_family(&addr), OP_EQ, AF_INET);
    ...
    rh.length = connected_cell_format_payload(cell.payload + RELAY_HEADER_SIZE, &addr, 3600);
    tt_int_op(rh.length, OP_EQ, 25);
    test_memeq_hex(cell.payload + RELAY_HEADER_SIZE, "00000000"
                                                     "06"
...
test_cfmt_extend_cells(void *arg)
{
    ...
    tt_int_op(0, OP_EQ,
              extend_cell_parse(&ec, RELAY_COMMAND_EXTEND, p, 26 + TAP_ONIONSKIN_CHALLENGE_LEN));
    ...
    tt_int_op(0, OP_EQ, extend_cell_parse(&ec, RELAY_COMMAND_EXTEND2, p, 35 + NTOR_ONIONSKIN_LEN));
    ...
    test_memeq_hex(p2,
                   ...
                   "0214"
                   "616e7468726f706f6d6f727068697a6174696f6e"
                   /*IPv6 address */
                   "0112"
                   "20020000000000000000000000f0c51e"
                   "1112"
    ...
    tt_int_op(0, OP_EQ, extend_cell_parse(&ec, RELAY_COMMAND_EXTEND2, p2, p2_len));
    tt_mem_op("brownshoesdontmakeit/brownshoesd", OP_EQ, ec.ed_pubkey.pubkey, 32);
    ...
    tt_int_op(0, OP_EQ, extend_cell_parse(&ec, RELAY_COMMAND_EXTEND2, p, sizeof(p)));
    tt_int_op(RELAY_COMMAND_EXTEND2, OP_EQ, ec.cell_type);
    tt_assert(fast_mem_is_zero((const char *)&ec.orport_ipv4.addr, sizeof(tor_addr_t)));
    tt_int_op(0, OP_EQ, ec.orport_ipv4.port);
    tt_str_op("7878:7878:7878:7878:7878:7878:7878:7878", OP_EQ, fmt_addr(&ec.orport_ipv6.addr));
    ...
    tt_int_op(-1, OP_EQ, extend_cell_parse(&ec, RELAY_COMMAND_EXTEND2, p, sizeof(p)));
...
test_cfmt_extended_cells(void *arg)
{
    ...
    tt_int_op(0, OP_EQ,
              extended_cell_parse(&ec, RELAY_COMMAND_EXTENDED, p, TAP_ONIONSKIN_REPLY_LEN));
    ...
    memcpy(p, "\x01\xf0", 2);
    tt_int_op(0, OP_EQ, extended_cell_parse(&ec, RELAY_COMMAND_EXTENDED2, p, sizeof(p)));

    /* Now try a too-long extended2 cell. That's the only misparse I can think
     * of. */
    memcpy(p, "\x01\xf1", 2);
    tt_int_op(-1, OP_EQ, extended_cell_parse(&ec, RELAY_COMMAND_EXTENDED2, p, sizeof(p)));
...
struct testcase_t cell_format_tests[] = {TEST(relay_header, 0),    TEST(begin_cells, 0),
