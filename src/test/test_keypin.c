...
test_keypin_parse_line(void *arg)
{
    ...
    /* Good line */
    ent = keypin_parse_journal_line("aGVyZSBpcyBhIGdvb2Qgc2hhMSE "
    ...
    ent = keypin_parse_journal_line("aGVyZSBpcyBhIGdv!2Qgc2hhMSE "
                                    "VGhpcyBlZDI1NTE5IHNjb2ZmcyBhdCB0aGUgc2hhMS4");
    tt_ptr_op(ent, OP_EQ, NULL);
...
test_keypin_parse_file(void *arg)
{
    ...
    const char data3[] = "Tm8gZHJhZ29uIGNhbiByZXNpc3Q IHRoZSBmYXNjaW5hdGlvbiBvZiByaWRkbGluZyB0YWw";
...
#define ADD(a, b) keypin_check_and_add((const uint8_t *)(a), (const uint8_t *)(b), 0)
...
test_keypin_add_entry(void *arg)
{
    ...
    tt_int_op(KEYPIN_ADDED, OP_EQ, ADD("ambassadors-at-large", "bread-and-butter thing-in-itself"));
    tt_int_op(KEYPIN_ADDED, OP_EQ, ADD("gentleman-adventurer", "cloak-and-dagger what's-his-face"));

    tt_int_op(KEYPIN_FOUND, OP_EQ, ADD("ambassadors-at-large", "bread-and-butter thing-in-itself"));
    tt_int_op(KEYPIN_FOUND, OP_EQ, ADD("ambassadors-at-large", "bread-and-butter thing-in-itself"));
    tt_int_op(KEYPIN_FOUND, OP_EQ, ADD("gentleman-adventurer", "cloak-and-dagger what's-his-face"));

    tt_int_op(KEYPIN_ADDED, OP_EQ, ADD("Johnnies-come-lately", "run-of-the-mill root-mean-square"));
...
test_keypin_journal(void *arg)
{
    ...
    tt_int_op(KEYPIN_ADDED, OP_EQ, ADD("king-of-the-herrings", "good-for-nothing attorney-at-law"));
    tt_int_op(KEYPIN_ADDED, OP_EQ, ADD("yellowish-red-yellow", "salt-and-pepper high-muck-a-muck"));
    tt_int_op(KEYPIN_FOUND, OP_EQ, ADD("yellowish-red-yellow", "salt-and-pepper high-muck-a-muck"));
    ...
    tt_int_op(0, OP_EQ, keypin_open_journal(fname));
    tt_int_op(KEYPIN_FOUND, OP_EQ, ADD("yellowish-red-yellow", "salt-and-pepper high-muck-a-muck"));
    tt_int_op(KEYPIN_ADDED, OP_EQ, ADD("theatre-in-the-round", "holier-than-thou jack-in-the-box"));
    tt_int_op(KEYPIN_ADDED, OP_EQ, ADD("no-deposit-no-return", "across-the-board will-o-the-wisp"));
    tt_int_op(KEYPIN_MISMATCH, OP_EQ,
    ...
    tt_int_op(KEYPIN_MISMATCH, OP_EQ,
              ADD("intellectualizations", "salt-and-pepper high-muck-a-muck"));
    tt_int_op(KEYPIN_FOUND, OP_EQ, ADD("theatre-in-the-round", "holier-than-thou jack-in-the-box"));
    tt_int_op(KEYPIN_MISMATCH, OP_EQ,
...
struct testcase_t keypin_tests[] = {TEST(parse_line, 0), TEST(parse_file, TT_FORK),
                                    TEST(add_entry, TT_FORK), TEST(journal, TT_FORK),
                                    END_OF_TESTCASES};
