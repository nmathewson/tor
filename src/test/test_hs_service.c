...
test_service_intro_point(void *arg)
{
    ...
    /* Test simple creation of an object. */
    {
         ...
         * because there could be a gap between setting now and the time taken
         * in service_intro_point_new. On ARM and other older CPUs, it can be
         * surprisingly slow... */
...
test_helper_functions(void *arg)
{
    ...
    ip = helper_create_service_ip();
    /* Immediately add the intro point to the service so the free service at
     * the end cleans it as well. */
    service_intro_point_add(service->desc_current->intro_points.map, ip);
    ...
    /* Testing get_node_from_intro_point() */
    {
        ...
        SMARTLIST_FOREACH_BEGIN (ip->base.link_specifiers, link_specifier_t *,
            ...
            if (link_specifier_get_ls_type(ls) == LS_LEGACY_ID) {
                /* Change legacy id in link specifier which is not the mock
                 * node. */
    ...
    {
        /* Put the start of the retry period back in time, we should be
         * allowed. to launch intro circuit. */
...
test_intro_circuit_opened(void *arg)
{
    ...
    expect_log_msg_containing(
        "Introduction circuit 0 established for service");
    teardown_capture_of_logs();
...
test_service_event(void *arg)
{
     ...
     * here that should get cleaned up. */
    {
        ...
        tt_int_op(digest256map_size(service->desc_current->intro_points.map),
                  OP_EQ, 0);
        /* No removal but no circuit so this means the IP object will stay in
         * the descriptor map so we can retry it. */
        ip = helper_create_service_ip();
        service_intro_point_add(service->desc_current->intro_points.map, ip);
        run_housekeeping_event(now);
        tt_int_op(digest256map_size(service->desc_current->intro_points.map),
...
test_rotate_descriptors(void *arg)
{
     ...
     * the descriptor certificate which is important to be now else the
     * decoding
...
test_build_update_descriptors(void *arg)
{
    ...
    tt_int_op(digest256map_size(service->desc_current->intro_points.map),
              OP_EQ, 0);
    tt_int_op(digest256map_size(service->desc_next->intro_points.map), OP_EQ,
              0);
    ...
    /* Now, we'll setup a node_t. */
    {
        ...
        tt_assert(node);
        node->is_running = node->is_valid = node->is_fast = node->is_stable =
            1;
    }
    ...
    expect_log_msg_containing(
        "just picked 1 intro points and wanted 3 for next "
    ...
    tt_int_op(digest256map_size(service->desc_next->intro_points.map), OP_EQ,
              1);
    /* Get the IP object. Because we don't have the auth key of the IP, we
     * can't query it so get the first element in the map. */
    {
        void *obj = NULL;
        const uint8_t *key;
        ...
        tt_assert(obj);
        ip_cur = obj;
        /* Get also the IP from the next descriptor. We'll make sure it's not
         * the same object as in the current descriptor. */
...
test_upload_descriptors(void *arg)
{
    ...
    tt_u64_op(service->desc_current->next_upload_time, OP_EQ, 0);
    /* We'll simulate that we've opened our intro point circuit and that we
     * only want one intro point. */
...
test_rendezvous1_parsing(void *arg)
{
...

    {
        ...
        ed25519_public_key_generate(&service->keys.identity_pk,
                                    &service->keys.identity_sk);
        memcpy(service->onion_address, test_addr,
...

    {
        ...
        service_circ = helper_create_origin_circuit(
            CIRCUIT_PURPOSE_S_CONNECT_REND, flags);
...
test_export_client_circuit_id(void *arg)
{
    ...
    conn =
        test_conn_get_connection(AP_CONN_STATE_CIRCUIT_WAIT, CONN_TYPE_AP, 0);
    ...
    /* Export circuit ID */
    export_hs_client_circuit_id(edge_conn,
...
test_intro2_handling(void *arg)
{
    ...
    MOCK(launch_rendezvous_point_circuit,
     ...
     *  includes instances X and Y. We then setup onionbalance on X and test
     * that Alice who addresses Z can communicate with X through INTRO2 cells.
     *
    ...
    alice_ip = hs_helper_build_intro_point(
        &signing_kp, now, "1.2.3.4", 0, &x_ip->auth_key_kp, &x_ip->enc_key_kp);
    ...
    retval =
        hs_circ_send_introduce1(intro_circ, &rend_circ, alice_ip, &x_subcred);
    ...
    /* Handle the cell */
    retval =
        hs_circ_handle_introduce2(&x_service, intro_circ, x_ip, &x_subcred,
    ...
    /* Act II:
     *
     * We now create an onionbalance setup with Z being the frontend and X and
     * Y being the backend instances. Make sure that Alice can talk with the
    ...
    memset(x_service.desc_current->desc->subcredential.subcred, 'C',
           SUBCRED_LEN);
    ...
    memset(relay_payload, 0, sizeof(relay_payload));
    retval =
        hs_circ_send_introduce1(intro_circ, &rend_circ, alice_ip, &z_subcred);
    ...
    retval =
        hs_circ_handle_introduce2(&x_service, intro_circ, x_ip, &z_subcred,
                                  (uint8_t *)relay_payload, relay_payload_len);
    tt_int_op(retval, OP_EQ, 0);
    ...
    /* Now cleanup the intro point replay cache but not the service replay
       cache and see that this one triggers this time. */
    replaycache_free(x_ip->replay_cache);
    x_ip->replay_cache = replaycache_new(0, 0);
    setup_full_capture_of_logs(LOG_INFO);
    retval =
     ...
     * subcred (should fail since Y is just another instance and not the
     * frontend service!) */
    memset(relay_payload, 0, sizeof(relay_payload));
    retval =
        hs_circ_send_introduce1(intro_circ, &rend_circ, alice_ip, &y_subcred);
    tt_int_op(retval, OP_EQ, 0);
    ...
    retval =
        hs_circ_handle_introduce2(&x_service, intro_circ, x_ip, &y_subcred,
                                  (uint8_t *)relay_payload, relay_payload_len);
    tt_int_op(retval, OP_EQ, -1);
