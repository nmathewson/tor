...
static void
mock_circuit_mark_for_close(circuit_t *circ, int reason, int line, const char *file)
{
    (void)circ;
    (void)reason;
    (void)line;
...
static int
mock_relay_send_command_from_edge(streamid_t stream_id, circuit_t *circ, uint8_t relay_command,
...
test_e2e_rend_circuit_setup(void *arg)
{
    ...
    /* Setup the circuit: do the ntor key exchange */
    {
        ...
        retval = hs_circuit_setup_e2e_rend_circ(or_circ, ntor_key_seed, sizeof(ntor_key_seed), 1);
        tt_int_op(retval, OP_EQ, 0);
    }
    ...
    tt_int_op(crypto_digest_get_algorithm(or_circ->cpath->pvt_crypto.f_digest), OP_EQ,
              DIGEST_SHA3_256);
    tt_int_op(crypto_digest_get_algorithm(or_circ->cpath->pvt_crypto.b_digest), OP_EQ,
              DIGEST_SHA3_256);
...
helper_clone_authorized_client(const hs_service_authorized_client_t *client)
{
    ...
    memcpy(client_out->client_pk.public_key, client->client_pk.public_key, CURVE25519_PUBKEY_LEN);
...
helper_create_service(void)
{
    ...
    ed25519_public_key_generate(&service->keys.identity_pk, &service->keys.identity_sk);
...
helper_create_service_ip(void)
{
    ...
    memset(link_specifier_getarray_un_legacy_id(ls), 'A', link_specifier_getlen_un_legacy_id(ls));
    smartlist_add(ip->base.link_specifiers, ls);
...
#undef conf_fmt
    ...
    tt_assert(!fast_mem_is_zero((char *)s->keys.identity_sk.seckey, ED25519_SECKEY_LEN));
    tt_assert(!fast_mem_is_zero((char *)s->keys.identity_pk.pubkey, ED25519_PUBKEY_LEN));
...
test_parse_authorized_client(void *arg)
{
    ...
    /* Wrong auth type. */
    tt_assert(
        !parse_authorized_client("x:x25519:dz4q5xqlb4ldnbs72iarrml4ephk3du4i7o2cgiva5lwr6wkquja"));
    /* Wrong key type. */
...
mock_read_file_to_str(const char *filename, int flags, struct stat *stat_out)
{
    ...
    if (!strcmp(filename, get_fname("hs3" PATH_SEPARATOR "authorized_clients" PATH_SEPARATOR
                                    "client1.auth"))) {
    ...
    if (!strcmp(filename,
                get_fname("hs3" PATH_SEPARATOR "authorized_clients" PATH_SEPARATOR "dummy.xxx"))) {
        ret = tor_strdup("descriptor:x25519:"
...
#undef conf_fmt
    ...
    tt_int_op(smartlist_len(service->config.clients), OP_EQ, smartlist_len(pubkey_b32_list));
    ...
    SMARTLIST_FOREACH_BEGIN (pubkey_b32_list, char *, pubkey_b32) {
        ...
        int is_found = 0;
        base32_decode((char *)pubkey.public_key, sizeof(pubkey.public_key), pubkey_b32,
                      strlen(pubkey_b32));

        SMARTLIST_FOREACH_BEGIN (service->config.clients, hs_service_authorized_client_t *,
                                 client) {
...
test_service_intro_point(void *arg)
{
    ...
    /* Test simple creation of an object. */
    {
        ...
        tt_int_op(fast_mem_is_zero((const char *)&ip->auth_key_kp, sizeof(ed25519_keypair_t)),
                  OP_EQ, 0);
        /* The introduce2_max MUST be in that range. */
        tt_u64_op(ip->introduce2_max, OP_GE, INTRO_POINT_MIN_LIFETIME_INTRODUCTIONS);
        tt_u64_op(ip->introduce2_max, OP_LE, INTRO_POINT_MAX_LIFETIME_INTRODUCTIONS);
         ...
         * surprisingly slow... */
        tt_u64_op(ip->time_to_expire, OP_GE, now + INTRO_POINT_LIFETIME_MIN_SECONDS - 500);
        /* We add 500 seconds, because this time we're testing against the
         * maximum allowed time. */
        tt_u64_op(ip->time_to_expire, OP_LE, now + INTRO_POINT_LIFETIME_MAX_SECONDS + 500);
     ...
     * created object (non legacy). */
    {
        ...
        hs_service_descriptor_t *desc_lookup = service_desc_find_by_intro(service, ip);
        tt_mem_op(service->desc_current, OP_EQ, desc_lookup, sizeof(hs_service_descriptor_t));
...
test_helper_functions(void *arg)
{
    ...
    /* Testing get_objects_from_ident(). */
    {
        ...
        tt_mem_op(desc_lookup, OP_EQ, service->desc_current, sizeof(hs_service_descriptor_t));
        /* Reset */
        s_lookup = NULL;
        ip_lookup = NULL;
        desc_lookup = NULL;
    ...
    /* Testing can_service_launch_intro_circuit() */
    {
        ...
        service->state.intro_circ_retry_started_time = (now - INTRO_CIRC_RETRY_PERIOD - 1);
    ...
    /* Testing intro_point_should_expire(). */
    {
        ...
        tt_u64_op(ip->introduce2_max, OP_GE, INTRO_POINT_MIN_LIFETIME_INTRODUCTIONS);
        tt_u64_op(ip->introduce2_max, OP_LE, INTRO_POINT_MAX_LIFETIME_INTRODUCTIONS);
        tt_u64_op(ip->time_to_expire, OP_GE, now + INTRO_POINT_LIFETIME_MIN_SECONDS);
        tt_u64_op(ip->time_to_expire, OP_LE, now + INTRO_POINT_LIFETIME_MAX_SECONDS);
...
test_intro_circuit_opened(void *arg)
{
    ...
    circ = helper_create_origin_circuit(CIRCUIT_PURPOSE_S_ESTABLISH_INTRO, flags);
    ...
    {
        service = helper_create_service();
        ed25519_pubkey_copy(&circ->hs_ident->identity_pk, &service->keys.identity_pk);
    ...
    /* Set an IP object now for this circuit. */
    {
        ...
        ed25519_pubkey_copy(&circ->hs_ident->intro_auth_pk, &ip->auth_key_kp.pubkey);
    }
...
test_closing_intro_circs(void *arg)
{
    ...
    hs_circuitmap_register_intro_circ_v3_service_side(intro_circ, &ip->auth_key_kp.pubkey);
    tmp_circ = hs_circuitmap_get_intro_circ_v3_service_side(&ip->auth_key_kp.pubkey);
    ...
    intro_circ = origin_circuit_init(CIRCUIT_PURPOSE_S_ESTABLISH_INTRO, flags);
    intro_circ->hs_ident = hs_ident_circuit_new(&service->keys.identity_pk);
    ed25519_pubkey_copy(&intro_circ->hs_ident->intro_auth_pk, &ip->auth_key_kp.pubkey);
    /* Register circuit in the circuitmap . */
    hs_circuitmap_register_intro_circ_v3_service_side(intro_circ, &ip->auth_key_kp.pubkey);
    tmp_circ = hs_circuitmap_get_intro_circ_v3_service_side(&ip->auth_key_kp.pubkey);
    tt_ptr_op(tmp_circ, OP_EQ, intro_circ);
...
test_service_event(void *arg)
{
     ...
     * here that should get cleaned up. */
    {
        ...
        tt_int_op(digest256map_size(service->desc_current->intro_points.map), OP_EQ, 0);
        ...
        service_intro_point_add(service->desc_current->intro_points.map, ip);
        run_housekeeping_event(now);
        tt_int_op(digest256map_size(service->desc_current->intro_points.map), OP_EQ, 1);
        ...
        service_intro_point_add(service->desc_current->intro_points.map, ip);
        ed25519_pubkey_copy(&circ->hs_ident->intro_auth_pk, &ip->auth_key_kp.pubkey);
        hs_circuitmap_register_intro_circ_v3_service_side(circ, &ip->auth_key_kp.pubkey);
        run_housekeeping_event(now);
        tt_int_op(digest256map_size(service->desc_current->intro_points.map), OP_EQ, 1);
...
test_rotate_descriptors(void *arg)
{
    ...
    tt_int_op(ret, OP_EQ, 0);
    ret = parse_rfc1123_time("Sat, 26 Oct 1985 14:00:00 UTC", &mock_ns.fresh_until);
    tt_int_op(ret, OP_EQ, 0);
    ...
    service->state.next_rotation_time = next_rotation_time = mock_ns.valid_after + (11 * 60 * 60);
    ...
    tt_assert(service->desc_current);
    tt_assert(service->desc_next);
    tt_u64_op(service->desc_current->time_period_num, OP_EQ, hs_get_previous_time_period_num(0));
    tt_u64_op(service->desc_next->time_period_num, OP_EQ, hs_get_time_period_num(0));
    ...
    ret = parse_rfc1123_time("Sat, 27 Oct 1985 01:00:00 UTC", &mock_ns.valid_after);
    tt_int_op(ret, OP_EQ, 0);
    ret = parse_rfc1123_time("Sat, 27 Oct 1985 02:00:00 UTC", &mock_ns.fresh_until);
    tt_int_op(ret, OP_EQ, 0);
    dirauth_sched_recalculate_timing(get_options(), mock_ns.valid_after);
    ...
    build_all_descriptors(now);
    tt_mem_op(service->desc_current, OP_EQ, desc_next, sizeof(*desc_next));
    tt_u64_op(service->desc_current->time_period_num, OP_EQ, hs_get_time_period_num(0));
    tt_u64_op(service->desc_next->time_period_num, OP_EQ, hs_get_next_time_period_num(0));
    tt_assert(service->desc_next);
...
test_build_update_descriptors(void *arg)
{
    ...
    ret = parse_rfc1123_time("Sat, 26 Oct 1985 03:00:00 UTC", &mock_ns.valid_after);
    tt_int_op(ret, OP_EQ, 0);
    ret = parse_rfc1123_time("Sat, 26 Oct 1985 04:00:00 UTC", &mock_ns.fresh_until);
    tt_int_op(ret, OP_EQ, 0);
    dirauth_sched_recalculate_timing(get_options(), mock_ns.valid_after);
    ...
    /* Now, we'll setup a node_t. */
    {
        ...
        tt_int_op(ret, OP_EQ, 0);
        ri.onion_curve25519_pkey = tor_malloc_zero(sizeof(curve25519_public_key_t));
        ri.onion_pkey = tor_malloc_zero(140);
        curve25519_public_key_generate(ri.onion_curve25519_pkey, &curve25519_secret_key);
        memset(ri.cache_info.identity_digest, 'A', DIGEST_LEN);
     ...
     * query it so get the first element in the map. */
    {
        void *obj = NULL;
        const uint8_t *key;
        digest256map_iter_t *iter = digest256map_iter_init(service->desc_current->intro_points.map);
    ...
    tt_assert(
        !fast_mem_is_zero((char *)ip_cur->enc_key_kp.seckey.secret_key, CURVE25519_SECKEY_LEN));
    tt_assert(
        !fast_mem_is_zero((char *)ip_cur->enc_key_kp.pubkey.public_key, CURVE25519_PUBKEY_LEN));
    tt_u64_op(ip_cur->time_to_expire, OP_GE, now + INTRO_POINT_LIFETIME_MIN_SECONDS);
    tt_u64_op(ip_cur->time_to_expire, OP_LE, now + INTRO_POINT_LIFETIME_MAX_SECONDS);
 ...
 *  test. */
static int
mock_relay_send_rendezvous1(streamid_t stream_id, circuit_t *circ, uint8_t relay_command,
...
{
    int retval;
    static const char *test_addr = "4acth47i6kxnvkewtm6q7ib2s3ufpo5sqbsnzjpbi7utijcltosqemad.onion";
    hs_service_t *service = NULL;
...

    {
        ...
        service_circ = helper_create_origin_circuit(CIRCUIT_PURPOSE_S_CONNECT_REND, flags);
        ...
        retval = hs_ntor_service_get_rendezvous1_keys(&ip_auth_kp.pubkey, &ip_enc_kp, &ephemeral_kp,
                                                      &client_kp.pubkey, &hs_ntor_rend_cell_keys);
        tt_int_op(retval, OP_EQ, 0);
    ...
    client_circ = helper_create_origin_circuit(CIRCUIT_PURPOSE_C_REND_READY_INTRO_ACKED, flags);
    /* fix up its circ ident */
    ed25519_pubkey_copy(&client_circ->hs_ident->intro_auth_pk, &ip_auth_kp.pubkey);
    memcpy(&client_circ->hs_ident->rendezvous_client_kp, &client_kp,
           sizeof(client_circ->hs_ident->rendezvous_client_kp));
    memcpy(&client_circ->hs_ident->intro_enc_pk.public_key, &ip_enc_kp.pubkey.public_key,
    ...
    retval = handle_rendezvous2(client_circ, (uint8_t *)rend1_payload + 20, rend1_payload_len - 20);
    tt_int_op(retval, OP_EQ, 0);
...
mock_build_state_get_exit_node(cpath_build_state_t *state)
{
    ...
    if (!fake_node) {
        ...
        fake_node->ri->onion_curve25519_pkey = tor_malloc_zero(sizeof(curve25519_public_key_t));
        curve25519_public_key_generate(fake_node->ri->onion_curve25519_pkey, &seckey);
    }
...
test_intro2_handling(void *arg)
{
    ...
    MOCK(relay_send_command_from_edge_, mock_relay_send_command_from_edge);
    MOCK(node_get_link_specifier_smartlist, mock_node_get_link_specifier_smartlist);
    ...
    hs_helper_get_subcred_from_identity_keypair(&x_identity_keypair, &x_subcred);
    ...
    hs_helper_get_subcred_from_identity_keypair(&z_identity_keypair, &z_subcred);
    ...
    hs_helper_get_subcred_from_identity_keypair(&y_identity_keypair, &y_subcred);

    /* Create Alice's intro point */
    hs_desc_intro_point_t *alice_ip;
    ed25519_keypair_t signing_kp;
    ed25519_keypair_generate(&signing_kp, 0);
    alice_ip = hs_helper_build_intro_point(&signing_kp, now, "1.2.3.4", 0, &x_ip->auth_key_kp,
                                           &x_ip->enc_key_kp);
    ...
    retval = hs_circ_send_introduce1(intro_circ, &rend_circ, alice_ip, &x_subcred);
    ...
    /* Handle the cell */
    retval = hs_circ_handle_introduce2(&x_service, intro_circ, x_ip, &x_subcred,
    ...
    smartlist_add(x_service.config.ob_master_pubkeys, &z_identity_keypair.pubkey);
    ...
    memset(relay_payload, 0, sizeof(relay_payload));
    retval = hs_circ_send_introduce1(intro_circ, &rend_circ, alice_ip, &z_subcred);
    ...
    retval = hs_circ_handle_introduce2(&x_service, intro_circ, x_ip, &z_subcred,
                                       (uint8_t *)relay_payload, relay_payload_len);
    tt_int_op(retval, OP_EQ, 0);
     ...
     * service!) */
    memset(relay_payload, 0, sizeof(relay_payload));
    retval = hs_circ_send_introduce1(intro_circ, &rend_circ, alice_ip, &y_subcred);
    tt_int_op(retval, OP_EQ, 0);
    ...
    retval = hs_circ_handle_introduce2(&x_service, intro_circ, x_ip, &y_subcred,
                                       (uint8_t *)relay_payload, relay_payload_len);
    tt_int_op(retval, OP_EQ, -1);
...
test_cannot_upload_descriptors(void *arg)
{
    ...
    /* 1. Testing missing intro points reason. */
    {
        ...
        expect_log_msg_containing("Service [scrubbed] can't upload its current descriptor: "
    ...
    {
        MOCK(networkstatus_get_live_consensus, mock_networkstatus_get_live_consensus_null);
        setup_full_capture_of_logs(LOG_INFO);
        run_upload_descriptor_event(now);
        expect_log_msg_containing("Service [scrubbed] can't upload its current descriptor: "
                                  "No live consensus");
...
struct testcase_t hs_service_tests[] = {
    {"e2e_rend_circuit_setup", test_e2e_rend_circuit_setup, TT_FORK, NULL, NULL},
    {"load_keys", test_load_keys, TT_FORK, NULL, NULL},
    {"client_filename_is_valid", test_client_filename_is_valid, TT_FORK, NULL, NULL},
    {"parse_authorized_client", test_parse_authorized_client, TT_FORK, NULL, NULL},
    {"load_keys_with_client_auth", test_load_keys_with_client_auth, TT_FORK, NULL, NULL},
    ...
    {"bad_introduce2", test_bad_introduce2, TT_FORK, NULL, NULL},
    {"service_event", test_service_event, TT_FORK, NULL, NULL},
    {"rotate_descriptors", test_rotate_descriptors, TT_FORK, NULL, NULL},
    {"build_update_descriptors", test_build_update_descriptors, TT_FORK, NULL, NULL},
    {"build_descriptors", test_build_descriptors, TT_FORK, NULL, NULL},
    {"upload_descriptors", test_upload_descriptors, TT_FORK, NULL, NULL},
    {"cannot_upload_descriptors", test_cannot_upload_descriptors, TT_FORK, NULL, NULL},
    {"rendezvous1_parsing", test_rendezvous1_parsing, TT_FORK, NULL, NULL},
    {"authorized_client_config_equal", test_authorized_client_config_equal, TT_FORK, NULL, NULL},
    {"export_client_circuit_id", test_export_client_circuit_id, TT_FORK, NULL, NULL},
