                     ...
                     const char *format, va_list ap)
{
    switch (CALLED(logv)) {
        ...
        case 3:
            ...
            tt_ptr_op(suffix, OP_EQ, NULL);
            tt_str_op(format, OP_EQ,
                      "Circuit handshake stats since last time: %d/%d TAP, "
                      "%d/%d NTor.");
            tt_int_op(va_arg(ap, int), OP_EQ,
                     ...
                     const char *format, va_list ap)
{
    switch (CALLED(logv)) {
        ...
        case 1:
            ...
            tt_ptr_op(suffix, OP_EQ, NULL);
            tt_str_op(format, OP_EQ,
                      "Heartbeat: Accounting enabled. Sent: %s, Received: %s, "
                      "Used: %s / "
                      "%s, Rule: %s. The current accounting interval ends on "
                      "%s, in %s.");
    ...
    switch (CALLED(logv)) {
        ...
        case 1:
            ...
            tt_str_op(format, OP_EQ,
                      "Average packaged cell fullness: %2.3f%%. "
                      "TLS write overhead: %.f%%");
            tt_int_op(fabs(va_arg(ap, double) - 100.0) <= DBL_EPSILON, OP_EQ,
                      1);
