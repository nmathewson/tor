...
test_status_hb_fails(void *arg)
{
    ...
    MOCK(tls_get_write_overhead_ratio, status_hb_fails_tls_get_write_overhead_ratio);
...
static void status_hb_not_in_consensus_logv(int severity, log_domain_mask_t domain,
...
test_status_hb_not_in_consensus(void *arg)
{
    ...
    MOCK(tls_get_write_overhead_ratio, status_hb_not_in_consensus_tls_get_write_overhead_ratio);
...
static void
status_hb_not_in_consensus_logv(int severity, log_domain_mask_t domain, const char *funcname,
                                const char *suffix, const char *format, va_list ap)
{
    ...
    case 0:
        ...
        tt_str_op(format, OP_EQ, "Heartbeat: It seems like we are not in the cached consensus.");
        break;
    case 1:
        tt_int_op(severity, OP_EQ, LOG_NOTICE);
        tt_u64_op(domain, OP_EQ, LD_HEARTBEAT);
    ...
    case 3:
        tt_int_op(severity, OP_EQ, LOG_NOTICE);
        tt_u64_op(domain, OP_EQ, LD_HEARTBEAT);
        tt_ptr_op(strstr(funcname, "rep_hist_log_circuit_handshake_stats"), OP_NE, NULL);
        tt_ptr_op(suffix, OP_EQ, NULL);
        tt_str_op(format, OP_EQ, "Circuit handshake stats since last time: %d/%d TAP, %d/%d NTor.");
    ...
    case 4:
        tt_int_op(severity, OP_EQ, LOG_NOTICE);
        tt_u64_op(domain, OP_EQ, LD_HEARTBEAT);
        tt_ptr_op(strstr(funcname, "rep_hist_log_link_protocol_counts"), OP_NE, NULL);
        break;
    case 5:
        tt_int_op(severity, OP_EQ, LOG_NOTICE);
        tt_u64_op(domain, OP_EQ, LD_HEARTBEAT);
        tt_str_op(format, OP_EQ, "DoS mitigation since startup:%s%s%s%s%s");
        tt_str_op(va_arg(ap, char *), OP_EQ, " 0 circuits killed with too many cells.");
...
static void status_hb_simple_logv(int severity, log_domain_mask_t domain, const char *funcname,
                                  const char *suffix, const char *format, va_list ap);
...
test_status_hb_simple(void *arg)
{
    ...
    MOCK(tls_get_write_overhead_ratio, status_hb_simple_tls_get_write_overhead_ratio);
...
static void status_hb_calls_log_accounting_logv(int severity, log_domain_mask_t domain,
                                                const char *funcname, const char *suffix,
                                                const char *format, va_list ap);
...
test_status_hb_calls_log_accounting(void *arg)
{
    ...
    MOCK(tls_get_write_overhead_ratio, status_hb_calls_log_accounting_tls_get_write_overhead_ratio);
    ...
    MOCK(accounting_is_enabled, status_hb_calls_log_accounting_accounting_is_enabled);
    MOCK(accounting_get_end_time, status_hb_calls_log_accounting_accounting_get_end_time);
...
static void
status_hb_calls_log_accounting_logv(int severity, log_domain_mask_t domain, const char *funcname,
                                    const char *suffix, const char *format, va_list ap)
{
...
static void status_hb_packaged_cell_fullness_logv(int severity, log_domain_mask_t domain,
                                                  const char *funcname, const char *suffix,
                                                  const char *format, va_list ap);
...
test_status_hb_packaged_cell_fullness(void *arg)
{
    ...
    MOCK(we_are_hibernating, status_hb_packaged_cell_fullness_we_are_hibernating);
    MOCK(public_server_mode, status_hb_packaged_cell_fullness_public_server_mode);
    ...
    MOCK(accounting_is_enabled, status_hb_packaged_cell_fullness_accounting_is_enabled);
    log_global_min_severity_ = LOG_DEBUG;
...
static void status_hb_tls_write_overhead_logv(int severity, log_domain_mask_t domain,
                                              const char *funcname, const char *suffix,
                                              const char *format, va_list ap);
...
test_status_hb_tls_write_overhead(void *arg)
{
    ...
    MOCK(tls_get_write_overhead_ratio, status_hb_tls_write_overhead_tls_get_write_overhead_ratio);
    ...
    MOCK(accounting_is_enabled, status_hb_tls_write_overhead_accounting_is_enabled);
    stats_n_data_cells_packaged = 0;
    log_global_min_severity_ = LOG_DEBUG;
...
static void
status_hb_tls_write_overhead_logv(int severity, log_domain_mask_t domain, const char *funcname,
                                  const char *suffix, const char *format, va_list ap)
{
