...
static void
test_util_read_until_eof_impl(const char *fname, size_t file_len, size_t read_limit)
{
    char *fifo_name = NULL;
    char *test_str = NULL;
    char *str = NULL;
...
#define CHECK_TIMEGM_ARG_OUT_OF_RANGE(msg) \
    ...
    tt_assert(b_time.tm_year == (1970 - 1900) || b_time.tm_year == (1969 - 1900));
    ...
    if (sizeof(time_t) == 4 || sizeof(time_t) == 8) {
        ...
        tor_gmtime_r(&t_res, &b_time);
        CHECK_POSSIBLE_EINVAL();
        tt_assert(b_time.tm_year == (1970 - 1900) || b_time.tm_year == (1935 - 1900));

        t_res = INT32_MIN;
        CAPTURE();
        tor_gmtime_r(&t_res, &b_time);
        CHECK_POSSIBLE_EINVAL();
        tt_assert(b_time.tm_year == (1970 - 1900) || b_time.tm_year == (1901 - 1900));
    }
...
#if SIZEOF_TIME_T == 4 || SIZEOF_TIME_T == 8
    {
        ...
        t_res = INT32_MAX;
        tor_gmtime_r(&t_res, &b_time);
        tt_assert(b_time.tm_year == (2037 - 1900) || b_time.tm_year == (2038 - 1900));
    }
    {
        /* as above but with localtime. */
        t_res = 3 * (1 << 29);
...
#if SIZEOF_TIME_T == 8
    {
        ...
        tor_gmtime_r(&t_res, &b_time);
        CHECK_POSSIBLE_EINVAL();
        tt_assert(b_time.tm_year == (2037 - 1900) || b_time.tm_year == (9999 - 1900));
...
#    endif
    ...
    tt_int_op(t_res, OP_EQ, (time_t)1091580502UL);
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, zz Aug 2004 99-99x99 GMT", &t_res));
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 32 Mar 2011 00:00:00 GMT", &t_res));
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 30 Mar 2011 24:00:00 GMT", &t_res));
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 30 Mar 2011 23:60:00 GMT", &t_res));
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 30 Mar 2011 23:59:62 GMT", &t_res));
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 30 Mar 1969 23:59:59 GMT", &t_res));
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 30 Ene 2011 23:59:59 GMT", &t_res));
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 30 Mar 2011 23:59:59 GM", &t_res));
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 30 Mar 1900 23:59:59 GMT", &t_res));

    /* Leap year. */
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 29 Feb 2011 16:00:00 GMT", &t_res));
    tt_int_op(0, OP_EQ, parse_rfc1123_time("Wed, 29 Feb 2012 16:00:00 GMT", &t_res));

    /* Leap second plus one */
    tt_int_op(-1, OP_EQ, parse_rfc1123_time("Wed, 30 Mar 2011 23:59:61 GMT", &t_res));
...
#define T(s)                                     \
    ...
    tt_int_op(-1, OP_EQ, parse_http_time("Sunday, 32 Aug 2004 00:48:22 GMT", &a_time));
    tt_int_op(-1, OP_EQ, parse_http_time("Sunday, 3 Aug 1869 00:48:22 GMT", &a_time));
    tt_int_op(-1, OP_EQ, parse_http_time("Sunday, 32-Aug-94 00:48:22 GMT", &a_time));
    ...
    tt_int_op(-1, OP_EQ, parse_http_time("Wednesday,,04 Aug 1994 00:48:22 GMT", &a_time));

    tt_int_op(0, OP_EQ, parse_http_time("Wednesday, 04 Aug 1994 00:48:22 GMT", &a_time));
    tt_int_op((time_t)775961302UL, OP_EQ, tor_timegm(&a_time));
    T("1994-08-04 00:48:22");
    tt_int_op(0, OP_EQ, parse_http_time("Wednesday, 4 Aug 1994 0:48:22 GMT", &a_time));
    tt_int_op((time_t)775961302UL, OP_EQ, tor_timegm(&a_time));
    T("1994-08-04 00:48:22");
    tt_int_op(0, OP_EQ, parse_http_time("Miercoles, 4 Aug 1994 0:48:22 GMT", &a_time));
    tt_int_op((time_t)775961302UL, OP_EQ, tor_timegm(&a_time));
    T("1994-08-04 00:48:22");
    tt_int_op(0, OP_EQ, parse_http_time("Wednesday, 04-Aug-94 00:48:22 GMT", &a_time));
    tt_int_op((time_t)775961302UL, OP_EQ, tor_timegm(&a_time));
    T("1994-08-04 00:48:22");
...
test_util_strmisc(void *arg)
{
    ...
    tt_str_op("\"\\\\ \\n\\r\\t\\\"\\'\"", OP_EQ, escaped("\\ \n\r\t\"'"));
    tt_str_op("\"unnecessary \\'backslashes\\'\"", OP_EQ, escaped("unnecessary \'backslashes\'"));
    ...
    /* Test hex_str */
    {
        ...
        tt_str_op(hex_str(binary_data, 17), OP_EQ, "000102030405060708090A0B0C0D0E0F10");
...
test_util_parse_integer(void *arg)
{
    ...
    tt_assert(UINT64_C(12345678901) == tor_parse_uint64("12345678901", 10, 0, UINT64_MAX, &i, &cp));
    tt_int_op(1, OP_EQ, i);
    tt_str_op(cp, OP_EQ, "");
    tt_assert(UINT64_C(0) == tor_parse_uint64("12345678901", 10, 500, INT32_MAX, &i, &cp));
    tt_int_op(0, OP_EQ, i);
...
#define TOOBIG "100000000000000000000000000"
        ...
        tt_int_op(i, OP_EQ, 0);
        tt_u64_op(UINT64_C(0), OP_EQ, tor_parse_uint64(TOOBIG, 10, 0, UINT64_MAX, &i, NULL));
        tt_int_op(i, OP_EQ, 0);
    }
done:;
}
...
test_util_pow2(void *arg)
{
    ...
    tt_u64_op(round_to_power_of_2(UINT64_C(40000000000000000)), OP_EQ, UINT64_C(1) << 55);
    tt_u64_op(round_to_power_of_2(UINT64_C(0xffffffffffffffff)), OP_EQ, UINT64_C(1) << 63);
...
test_util_compress_impl(compress_method_t method)
{
    ...
    tt_assert(!tor_uncompress(&buf3, &len2, buf2, len1 - 16, method, 0, LOG_INFO));
    ...
    tor_free(buf3);
    if (method != NO_METHOD) {
        tt_assert(tor_uncompress(&buf3, &len2, buf2, len1 - 16, method, 1, LOG_INFO));
...
static void
test_util_compress_stream_impl(compress_method_t method, compression_level_t level)
{
    ...
    len2 = 21;
    tt_int_op(tor_compress_process(state, &cp1, &len1, &ccp2, &len2, 0), OP_EQ, TOR_COMPRESS_OK);
    tt_int_op(0, OP_EQ, len2); /* Make sure we compressed it all. */
    tt_assert(cp1 > buf1);

    len2 = 0;
    cp2 = cp1;
    tt_int_op(tor_compress_process(state, &cp1, &len1, &ccp2, &len2, 1), OP_EQ, TOR_COMPRESS_DONE);
    ...
    tt_assert(!tor_uncompress(&buf3, &len2, buf1, 1024 - len1, method, 1, LOG_WARN));
...
static const struct testcase_setup_t compress_setup = {compression_test_setup,
...
test_util_compress(void *arg)
{
    ...
    compression_level_t levels[] = {BEST_COMPRESSION, HIGH_COMPRESSION, MEDIUM_COMPRESSION,
                                    LOW_COMPRESSION};
...
test_util_decompress_junk_impl(compress_method_t method)
{
    ...
    strlcpy(input, "This shouldn't be a compressed string by any means.", sizeof(input));
    sz = strlen(input);
...
test_util_decompress_dos_impl(compress_method_t method)
{
    ...
    tt_int_op(r, OP_EQ, -1);
    expect_log_msg_containing("other Tors would think this was a compression bomb");
    teardown_capture_of_logs();
...
test_util_gzip_compression_bomb(void *arg)
{
    ...
    setup_full_capture_of_logs(LOG_WARN);
    tt_int_op(-1, OP_EQ, tor_compress(&result, &result_len, one_mb, one_million, ZLIB_METHOD));
    expect_single_log_msg_containing("We compressed something and got an insanely high "
                                     "compression factor; other Tors would think this "
                                     "was a compression bomb.");
    teardown_capture_of_logs();

    /* Here's a compression bomb that we made manually. */
    const char compression_bomb[1039] = {0x78, 0xDA, 0xED, 0xC1, 0x31, 0x01, 0x00, 0x00, 0x00,
                                         0xC2, 0xA0, 0xF5, 0x4F, 0x6D, 0x08, 0x5F, 0xA0 /* .... */};
    tt_int_op(
        -1, OP_EQ,
        tor_uncompress(&result, &result_len, compression_bomb, 1039, ZLIB_METHOD, 0, LOG_WARN));
...
{
    char *out = NULL;
    const char *inp = "..This is a test\r\n.of the emergency \n..system.\r\n\rZ.\r\n";
    size_t sz;

    (void)arg;
    sz = read_escaped_data(inp, strlen(inp), &out);
    tt_str_op(out, OP_EQ, ".This is a test\nof the emergency \n.system.\n\rZ.\n");
...
test_util_sscanf(void *arg)
{
    ...
    tt_int_op(-1, OP_EQ, tor_sscanf("wrong", "%5c", s1)); /* %c cannot have a number. */
  ...
  test_eq(0, tor_sscanf("prettylongstring", "%10s", NULL));
    ...
    /* Excess data */
    tt_int_op(0, OP_EQ, tor_sscanf("hello 3", "%u", &u1)); /* have to match the start */
    tt_int_op(0, OP_EQ, tor_sscanf(" 3 hello", "%u", &u1));
    tt_int_op(0, OP_EQ, tor_sscanf(" 3 hello", "%2u", &u1)); /* not even in this case */
    tt_int_op(1, OP_EQ, tor_sscanf("3 hello", "%u", &u1)); /* but trailing is alright */

    /* Numbers (ie. %u) */
    tt_int_op(0, OP_EQ, tor_sscanf("hello world 3", "hello worlb %u", &u1)); /* d vs b */
    ...
    tt_int_op(3, OP_EQ, tor_sscanf("67:8:099", "%2u:%2u:%3u", &u1, &u2, &u3)); /* 0s */
    ...
    tt_int_op(3, OP_EQ, tor_sscanf("12:03:000000000000000099", "%2u:%2u:%u", &u1, &u2, &u3));
    tt_int_op(12u, OP_EQ, u1);
    tt_int_op(3u, OP_EQ, u2);
    tt_int_op(99u, OP_EQ, u3);
    ...
    tt_str_op(s2, OP_EQ, "");
    tt_int_op(3, OP_EQ, tor_sscanf("hi", "%9s%9s%3s", s1, s2, s3)); /* %s can be empty. */
    tt_str_op(s1, OP_EQ, "hi");
    tt_str_op(s2, OP_EQ, "");
    tt_str_op(s3, OP_EQ, "");

    tt_int_op(3, OP_EQ, tor_sscanf("1.2.3", "%u.%u.%u%c", &u1, &u2, &u3, &ch));
    tt_int_op(4, OP_EQ, tor_sscanf("1.2.3 foobar", "%u.%u.%u%c", &u1, &u2, &u3, &ch));
    tt_int_op(' ', OP_EQ, ch);
    ...
    r = tor_sscanf("-9223372036854775808. 9223372036854775807.", "%d. %d.", &int1, &int2);
    tt_int_op(r, OP_EQ, 2);
    ...
    /* and the first failure stops further processing */
    r = tor_sscanf("-9223372036854775808. 9223372036854775808.", "%d. %d.", &int1, &int2);
    tt_int_op(r, OP_EQ, 1);

    r = tor_sscanf("-9223372036854775809. 9223372036854775807.", "%d. %d.", &int1, &int2);
    tt_int_op(r, OP_EQ, 0);

    r = tor_sscanf("9223372036854775808. -9223372036854775809.", "%d. %d.", &int1, &int2);
    tt_int_op(r, OP_EQ, 0);
#endif /* SIZEOF_INT == 4 || ... */
    ...
    /* %ld */
    /* INT64_MIN and INT64_MAX should work */
    r = tor_sscanf("-9223372036854775808. 9223372036854775807.", "%ld. %ld.", &lng1, &lng2);
    tt_int_op(r, OP_EQ, 2);
    ...
    /* and the first failure stops further processing */
    r = tor_sscanf("-9223372036854775808. 9223372036854775808.", "%ld. %ld.", &lng1, &lng2);
    tt_int_op(r, OP_EQ, 1);

    r = tor_sscanf("-9223372036854775809. 9223372036854775807.", "%ld. %ld.", &lng1, &lng2);
    tt_int_op(r, OP_EQ, 0);

    r = tor_sscanf("9223372036854775808. -9223372036854775809.", "%ld. %ld.", &lng1, &lng2);
    tt_int_op(r, OP_EQ, 0);
#endif /* SIZEOF_LONG == 4 || ... */

    r = tor_sscanf("123.456 .000007 -900123123.2000787 00003.2", "%lf %lf %lf %lf", &d1, &d2, &d3,
                   &d4);
    tt_int_op(r, OP_EQ, 4);
    test_feq(d1, 123.456);
    test_feq(d2, .000007);
...
#define tt_ci_char_op(a, op, b) tt_char_op(TOR_TOLOWER((int)a), op, TOR_TOLOWER((int)b))
...
#define TL_ T_ " " L_
    ...
    format_time_interval(dbuf, sizeof(dbuf), 24 * 60 * 60 + 60);
    tt_int_op(strnlen(dbuf, DBUF_SIZE), OP_LE, DBUF_SIZE - 1);
    r = tor_sscanf(dbuf, TL_ " " TL_ " " TL_, &day, label_d, &hour, label_h, &min, label_m);
    if (r == -1) {
...
test_util_datadir(void *arg)
{
    ...
    tor_snprintf(buf, sizeof(buf), "%s" PATH_SEPARATOR "cache" PATH_SEPARATOR "thingy", temp_dir);
    tt_str_op(f, OP_EQ, buf);
    tor_free(f);
    f = get_datadir_fname2_suffix("cache", "thingy", ".foo");
    tor_snprintf(buf, sizeof(buf), "%s" PATH_SEPARATOR "cache" PATH_SEPARATOR "thingy.foo",
                 temp_dir);
    tt_str_op(f, OP_EQ, buf);
    tor_free(f);
...
test_util_strtok(void *arg)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        tor_snprintf(buf, sizeof(buf), "%sGraved on the dark  in gestures of descent%s", pad1,
                     pad1);
        tor_snprintf(buf2, sizeof(buf2), "%sthey.seemed;;their!.own;most.perfect;monument%s", pad2,
                     pad2);
...
test_util_string_is_utf8(void *ptr)
{
    ...
    tt_int_op(1, OP_EQ, string_is_utf8_no_bom("loveandrespect", strlen("loveandrespect")));
...
#define LOREMIPSUM "Lorem ipsum dolor sit amet, consectetur adipisicing elit"
    ...
    r = tor_asprintf(&cp, "1: %s 2: %s 3: %s", LOREMIPSUM, LOREMIPSUM, LOREMIPSUM);
    tt_assert(cp);
    tt_int_op(strlen(cp), OP_EQ, r);
...
test_util_ftruncate(void *ptr)
{
    ...
    tt_int_op(strlen(message), OP_EQ, write_all_to_fd(fd, message, strlen(message)));
    ...
    tt_int_op(strlen(message2), OP_EQ, write_all_to_fd(fd, message2, strlen(message2)));
...
test_util_format_dec_number(void *ptr)
{
    ...
    for (i = 0; test_data[i].str != NULL; ++i) {
        ...
        len = format_dec_number_sigsafe(test_data[i].x, buf, (int)(strlen(test_data[i].str) + 1));
        tt_int_op(len, OP_EQ, strlen(buf));
        tt_str_op(buf, OP_EQ, test_data[i].str);
    }
...
test_util_eat_whitespace(void *ptr)
{
    ...
    tt_ptr_op(str + strlen(str) - 1, OP_EQ, eat_whitespace_eos_no_nl(str, str + strlen(str)));

    strlcpy(str, " \t\r ", sizeof(str));
    tt_ptr_op(str + strlen(str), OP_EQ, eat_whitespace(str));
    tt_ptr_op(str + strlen(str), OP_EQ, eat_whitespace_eos(str, str + strlen(str)));
    tt_ptr_op(str + strlen(str), OP_EQ, eat_whitespace_no_nl(str));
    tt_ptr_op(str + strlen(str), OP_EQ, eat_whitespace_eos_no_nl(str, str + strlen(str)));
    ...
    tt_ptr_op(str + sizeof(ws), OP_EQ, eat_whitespace_eos_no_nl(str, str + strlen(str)));
...
test_util_make_environment(void *ptr)
{
    ...
    const char expected_windows_env_block[] =
        ...
        "\000";
    size_t expected_windows_env_block_len = sizeof(expected_windows_env_block) - 1;
...
test_util_set_env_var_in_sl(void *ptr)
{
    ...
    smartlist_t *merged_env_vars = smartlist_new_from_text_lines(base_env_vars_string);
    smartlist_t *new_env_vars = smartlist_new_from_text_lines(new_env_vars_string);
    ...
    SMARTLIST_FOREACH(
        new_env_vars, char *, env_var,
        set_environment_variable_in_smartlist(merged_env_vars, env_var, tor_free_, 1));
...
test_util_round_to_next_multiple_of(void *arg)
{
    ...
    tt_u64_op(round_uint64_to_next_multiple_of(UINT64_MAX, 2), OP_EQ, UINT64_MAX);
    ...
    tt_int_op(round_uint32_to_next_multiple_of(UINT32_MAX, 2), OP_EQ, UINT32_MAX);
...
test_util_laplace(void *arg)
{
     ...
     */
    tt_i64_op(INT64_MIN + 20, OP_EQ, add_laplace_noise(20, 0.0, delta_f, epsilon));
    ...
    tt_i64_op(INT64_MIN, OP_EQ, add_laplace_noise(0, 0.0, noscale_df, noscale_eps));

    /* is it clipped to INT64_MIN? */
    tt_i64_op(INT64_MIN, OP_EQ, add_laplace_noise(-1, 0.0, noscale_df, noscale_eps));
    tt_i64_op(INT64_MIN, OP_EQ, add_laplace_noise(INT64_MIN, 0.0, noscale_df, noscale_eps));
    /* ... even when scaled? */
    ...
    tt_i64_op(-35, OP_EQ, add_laplace_noise(0, min_dbl_error, noscale_df, noscale_eps));
    ...
    tt_i64_op(INT64_MIN, OP_EQ, add_laplace_noise(INT64_MIN, min_dbl_error, DBL_MAX, 1));
    ...
    tt_i64_op(35, OP_EQ, add_laplace_noise(0, max_dbl_lt_one, noscale_df, noscale_eps));

    /* is it clipped to INT64_MAX? */
    tt_i64_op(INT64_MAX, OP_EQ,
              add_laplace_noise(INT64_MAX - 35, max_dbl_lt_one, noscale_df, noscale_eps));
    tt_i64_op(INT64_MAX, OP_EQ,
              add_laplace_noise(INT64_MAX - 34, max_dbl_lt_one, noscale_df, noscale_eps));
    tt_i64_op(INT64_MAX, OP_EQ,
              add_laplace_noise(INT64_MAX, max_dbl_lt_one, noscale_df, noscale_eps));
    /* ... even when scaled? */
    tt_i64_op(INT64_MAX, OP_EQ, add_laplace_noise(INT64_MAX, max_dbl_lt_one, delta_f, epsilon));
    tt_i64_op((INT64_MIN + INT64_MAX), OP_EQ,
              add_laplace_noise(INT64_MIN, max_dbl_lt_one, DBL_MAX, 1));
    tt_i64_op(INT64_MAX, OP_EQ, add_laplace_noise(INT64_MAX, max_dbl_lt_one, DBL_MAX, 1));
...
test_util_clamp_double_to_int64(void *arg)
{
    ...
    tt_i64_op(INT64_MIN, OP_EQ, clamp_double_to_int64(-1.0 * pow(2.0, 64.0) - 1.0));
    tt_i64_op(INT64_MIN, OP_EQ, clamp_double_to_int64(-1.0 * pow(2.0, 63.0) - 1.0));
    tt_i64_op(((uint64_t)-1) << 53, OP_EQ, clamp_double_to_int64(-1.0 * pow(2.0, 53.0)));
    ...
    tt_i64_op(0, OP_EQ, clamp_double_to_int64(0.9));
    tt_i64_op(1, OP_EQ, clamp_double_to_int64(1.0));
    tt_i64_op((((int64_t)1) << 53) - 1, OP_EQ, clamp_double_to_int64(pow(2.0, 53.0) - 1.0));
...
test_util_socket(void *arg)
{
    ...
    if (fd1 < 0 && (err == SOCK_ERRNO(EPROTONOSUPPORT) || err == SOCK_ERRNO(EAFNOSUPPORT))) {
...
test_util_hostname_validation(void *arg)
{
    ...
    tt_assert(!string_is_valid_nonrfc_hostname("..washingtonpost.is.better.com"));
...
test_util_monotonic_time(void *arg)
{
    ...
    uint64_t coarse_stamp_diff = monotime_coarse_stamp_units_to_approx_msec(stamp2 - stamp1);
...
#    define COMPRESS(name, identifier)                                                        \
        {                                                                                     \
            ("compress/" #name), test_util_compress, 0, &compress_setup, (char *)(identifier) \
        }

#    define COMPRESS_CONCAT(name, identifier)                                                  \
        {                                                                                      \
            ("compress_concat/" #name), test_util_decompress_concatenated, 0, &compress_setup, \
                (char *)(identifier)                                                           \
        }
...
struct testcase_t util_tests[] = {
    ...
    {"socket_ipv4", test_util_socket, TT_FORK, &passthrough_setup, (void *)"4"},
    {"socket_ipv6", test_util_socket, TT_FORK, &passthrough_setup, (void *)"6"},
    {"socketpair", test_util_socketpair, TT_FORK, &passthrough_setup, (void *)"0"},
    {"socketpair_ersatz", test_util_socketpair, TT_FORK, &passthrough_setup, (void *)"1"},
    UTIL_TEST(max_mem, 0),
