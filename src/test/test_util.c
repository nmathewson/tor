...
#ifdef _WIN32
    /* Would you believe that tv_sec is a long on windows? Of course you
     * would.*/
#    define TV_SEC_MAX LONG_MAX
#    define TV_SEC_MIN LONG_MIN
#else
    /* Some BSDs have struct timeval.tv_sec 64-bit, but time_t (and long)
     * 32-bit Which means TIME_MAX is not actually the maximum value of tv_sec.
...
test_util_parse_integer(void *arg)
{
    ...
    tt_int_op(68284L, OP_EQ,
              tor_parse_long("10abc", 16, 0, 70000, NULL, NULL));
    tt_int_op(68284L, OP_EQ,
              tor_parse_long("10ABC", 16, 0, 70000, NULL, NULL));
...
#define TOOBIG "100000000000000000000000000"
        ...
        tt_int_op(i, OP_EQ, 0);
        tt_int_op(0UL, OP_EQ,
                  tor_parse_ulong(TOOBIG, 10, 0, ULONG_MAX, &i, NULL));
        tt_int_op(i, OP_EQ, 0);
...
test_util_compress_impl(compress_method_t method)
{
    ...
    if (method == NO_METHOD) {
        ...
        tt_int_op(detect_compression_method(buf2, len1), OP_EQ,
                  UNKNOWN_METHOD);
    } else {
    ...
    tor_free(buf3);
    if (method != NO_METHOD) {
        tt_assert(tor_uncompress(&buf3, &len2, buf2, len1 - 16, method, 1,
                                 LOG_INFO));
...
test_util_decompress_junk_impl(compress_method_t method)
{
    ...
    /* Now try again, with a compressed object that starts out good and turns
       to junk. */
    crypto_rand(input, sizeof(input));
...
test_util_gzip_compression_bomb(void *arg)
{
     ...
     * one. Most compression formats support them, but they can be a DOS
     * vector. In Tor we try not to generate them, and we don't accept them.
     */
    (void)arg;
...
test_util_sscanf(void *arg)
{
    ...
    (void)arg;
    tt_int_op(-1, OP_EQ,
              tor_sscanf("123", "%i", &r)); /* %i is not supported */
    tt_int_op(-1, OP_EQ,
              tor_sscanf("wrong", "%5c", s1)); /* %c cannot have a number. */
    tt_int_op(-1, OP_EQ,
              tor_sscanf("hello", "%s", s1)); /* %s needs a number. */
  ...
  test_eq(0, tor_sscanf("prettylongstring", "%10s", NULL));
    ...
    tt_int_op(3, OP_EQ,
              tor_sscanf("!12:3:456", "!%2u:%2u:%3u", &u1, &u2, &u3));
    tt_int_op(12u, OP_EQ, u1);
    ...
    tt_int_op(2, OP_EQ,
              tor_sscanf("76trombones", "%6u%9s", &u1, s1)); /* %u%s */
...
test_util_memarea(void *arg)
{
    ...
    tt_int_op(used, OP_GE,
              total); /* not EQ, because of alignment and headers*/
...
test_util_datadir(void *arg)
{
    ...
    tor_snprintf(buf, sizeof(buf),
                 "%s" PATH_SEPARATOR "cache" PATH_SEPARATOR "thingy",
                 temp_dir);
    tt_str_op(f, OP_EQ, buf);
    tor_free(f);
...
test_util_strtok(void *arg)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        tor_snprintf(buf, sizeof(buf),
                     "%sGraved on the dark  in gestures of descent%s", pad1,
                     pad1);
...
test_util_find_str_at_start_of_line(void *ptr)
{
    ...
    tt_ptr_op(NULL, OP_EQ,
              find_str_at_start_of_line(short_string, "nonsense"));
    ...
    tt_ptr_op(long_string, OP_EQ,
              find_str_at_start_of_line(long_string, "how"));
...
test_util_memcpy_iftrue_timei(void *arg)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        /* We just copied buf1 into buf3.  Now we're going to copy buf2 into
           buf2, iff our coin flip comes up heads. */
...
test_util_make_environment(void *ptr)
{
    ...
    tt_want(tor_memeq(expected_windows_env_block,
                   ...
                   smartlist_len(env_vars_in_unixoid_env_block_sorted));
    {
        ...
        for (i = 0; i < len; ++i) {
            tt_want_str_op(
                smartlist_get(env_vars_sorted, i), OP_EQ,
...
test_util_laplace(void *arg)
{
    ...
    tt_i64_op(INT64_MIN, OP_EQ,
              add_laplace_noise(0, min_dbl_error, DBL_MAX, 1));
...
test_util_socketpair(void *arg)
{
    ...
    if (ersatz && socketpair_result < 0) {
        /* In my testing, an IPv6-only FreeBSD jail without ::1 returned
         * EINVAL.
...
test_util_hostname_validation(void *arg)
{
    ...
    tt_assert(
        string_is_valid_nonrfc_hostname("multiple-words-with-hypens.jp"));
    ...
    tt_assert(
        string_is_valid_nonrfc_hostname("core3_euw1.fabrik.nytimes.com"));
    ...
    tt_assert(
        string_is_valid_nonrfc_hostname("core9_euw1.fabrik.nytimes.com."));
    tt_assert(
    ...
    // We shouldn't for SOCKS (spec says "contains a fully-qualified domain
    // name" but only test pathologically malformed traling '.' cases for now.
    tt_assert(!string_is_valid_nonrfc_hostname("."));
    tt_assert(!string_is_valid_nonrfc_hostname(".."));
...
test_util_map_anon_nofork(void *arg)
{
    ...
    if (inherit == INHERIT_RES_ZERO) {
        ...
        tt_int_op(inherit, OP_EQ, INHERIT_RES_KEEP);
        tt_int_op((int)r, OP_EQ, 1); // child should send us a byte.
        tt_int_op(buf[0], OP_EQ,
                  TEST_VALUE); // that byte should be TEST_VALUE.
    }
