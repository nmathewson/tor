...
test_socks_5_malformed_commands(void *ptr)
{
    ...
    /** SOCKS 5 Send CONNECT [01] to IP address 2.2.2.2:4369, with SafeSocks
     * set
     */
    ADD_DATA(buf, "\x05\x01\x00");
    ADD_DATA(buf, "\x05\x01\x00\x01\x02\x02\x02\x02\x11\x11");
...
test_socks_truncated(void *ptr)
{
    const struct {
        ...
        {NONE, "\x04\x01\x09\x09\x00\x00\x00\x01hi\x00www.example.com\x00",
         27},
        /* SOCKS5 */
        /* initial handshake */
        {NONE, "\x05\x00", 2},
    ...
    for (i = 0; i < ARRAY_LENGTH(commands); ++i) {
        for (j = 0; j < commands[i].len; ++j) {
            ...
            TT_BLATHER(("Checking command %u, length %u, omitting char %u", i,
                        j, (unsigned)commands[i].body[j]));
...
test_socks_client_v5_auth(void *arg)
{
    ...
    tt_int_op(1, OP_EQ,
              fetch_from_buf_socks_client(
                  buf, PROXY_SOCKS5_WANT_AUTH_RFC1929_OK, &reason));
    tt_ptr_op(reason, OP_EQ, NULL);
    tt_int_op(buf_datalen(buf), OP_EQ, 0);
...
test_socks_client_v5_connect(void *arg)
{
    ...
    tt_int_op(1, OP_EQ,
              fetch_from_buf_socks_client(buf, PROXY_SOCKS5_WANT_CONNECT_OK,
                                          &reason));
    tt_ptr_op(reason, OP_EQ, NULL);
    tt_int_op(buf_datalen(buf), OP_EQ, 0);
