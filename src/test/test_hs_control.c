...
test_hs_desc_event(void *arg)
{
    ...
    hs_control_desc_event_upload(onion_address, HSDIR_EXIST_ID, &blinded_pk,
                                 hsdir_index_store);
    tor_asprintf(&expected_msg,
                 "650 HS_DESC UPLOAD %s UNKNOWN " STR_HSDIR_EXIST_LONGNAME
                 " %s "
    ...
    tor_asprintf(&expected_msg,
                 "650 HS_DESC FAILED %s NO_AUTH " STR_HSDIR_EXIST_LONGNAME
                 " %s "
                 "REASON=BAD_DESC\r\n",
                 onion_address, base64_blinded_pk);
    tt_assert(received_msg);
...
test_hs_control_store_permanent_creds(void *arg)
{
    ...
    creds_fname = tor_strdup(get_fname("permanent_credentials/"
                                       "2fvhjskjet3n5syd6yfg5lhvwcs62bojmthr35"
                                       "ko5bllr3iqdb4ctdyd.auth_private"));
    ...
    /* Now for our next act!!! Actually get the HS client subsystem to parse
     * the whole directory and make sure that it extracted the right
     * credential! */
