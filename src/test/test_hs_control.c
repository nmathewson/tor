...
#define HSDIR_INDEX_FETCH_HEX "4343434343434343434343434343434343434343434343434343434343434343"
#define HSDIR_INDEX_STORE_HEX "4444444444444444444444444444444444444444444444444444444444444444"
...
test_hs_desc_event(void *arg)
{
    ...
    memcpy(&ident.identity_pk, &identity_kp.pubkey, sizeof(ed25519_public_key_t));
    memcpy(&ident.blinded_pk, &blinded_pk, sizeof(blinded_pk));

    /* HS_DESC REQUESTED ... */
    hs_control_desc_event_requested(&identity_kp.pubkey, base64_blinded_pk, &hsdir_rs);
    ...
    hs_control_desc_event_upload(onion_address, HSDIR_EXIST_ID, &blinded_pk, hsdir_index_store);
    tor_asprintf(&expected_msg,
    ...
    tor_asprintf(&expected_msg,
                 "650 HS_DESC RECEIVED %s NO_AUTH " STR_HSDIR_EXIST_LONGNAME " %s\r\n",
                 onion_address, base64_blinded_pk);
    tt_assert(received_msg);
    tt_str_op(received_msg, OP_EQ, expected_msg);
    tor_free(received_msg);
    ...
    tor_asprintf(&expected_msg, "650 HS_DESC UPLOADED %s UNKNOWN " STR_HSDIR_EXIST_LONGNAME "\r\n",
                 onion_address);
    tt_assert(received_msg);
    tt_str_op(received_msg, OP_EQ, expected_msg);
    tor_free(received_msg);
...
test_hs_control_good_onion_client_auth_add(void *arg)
{
    ...
    { /* Setup the services */
        retval = hs_parse_address("2fvhjskjet3n5syd6yfg5lhvwcs62bojmthr35ko5bllr3iqdb4ctdyd",
                                  &service_identity_pk_2fv, NULL, NULL);
        tt_int_op(retval, OP_EQ, 0);

        retval = hs_parse_address("jt4grrjwzyz3pjkylwfau5xnjaj23vxmhskqaeyfhrfylelw4hvxcuyd",
...
test_hs_control_bad_onion_client_auth_add(void *arg)
{
    ...
    /* Register first service */
    args = tor_strdup("badaddr x25519:iJ1tjKCrMAbiFT2bVrCjhbfMDnE1fpaRbIS5ZHKUvEQ=");
...
test_hs_control_store_permanent_creds(void *arg)
{
    ...
    /* Check file contents! */
    creds_fname = tor_strdup(
        get_fname("permanent_credentials/"
...
struct testcase_t hs_control_tests[] = {
    ...
    {"hs_control_good_onion_client_auth_add", test_hs_control_good_onion_client_auth_add, TT_FORK,
     NULL, NULL},
    {"hs_control_bad_onion_client_auth_add", test_hs_control_bad_onion_client_auth_add, TT_FORK,
     NULL, NULL},
    {"hs_control_store_permanent_creds", test_hs_control_store_permanent_creds, TT_FORK, NULL,
     NULL},
    {"hs_control_add_onion_with_bad_pubkey", test_hs_control_add_onion_with_bad_pubkey, TT_FORK,
     NULL, NULL},
