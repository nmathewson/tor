...
test_address_udp_socket_trick_whitebox(void *arg)
{
    ...
    hack_retval = get_interface_address6_via_udp_socket_hack(
        LOG_DEBUG, AF_INET, addr_from_hack);
    ...
    hack_retval = get_interface_address6_via_udp_socket_hack(
        LOG_DEBUG, AF_INET6, addr_from_hack);
...
test_address_get_if_addrs6_list_internal(void *arg)
{
    ...
    if (smartlist_len(mock_saved_logs()) == 1) {
        expect_log_msg_containing_either4(
            "connect() failed", "unable to create socket",
