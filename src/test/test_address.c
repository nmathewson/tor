 ...
 */
static uint8_t
sockaddr_in_are_equal(struct sockaddr_in *sockaddr1, struct sockaddr_in *sockaddr2)
{
 ...
 */
static uint8_t
sockaddr_in6_are_equal(struct sockaddr_in6 *sockaddr1, struct sockaddr_in6 *sockaddr2)
{
    return ((sockaddr1->sin6_family == sockaddr2->sin6_family) &&
            (sockaddr1->sin6_port == sockaddr2->sin6_port) &&
            (tor_memeq(sockaddr1->sin6_addr.s6_addr, sockaddr2->sin6_addr.s6_addr, 16)));
}
...
test_address_ifaddrs_to_smartlist(void *arg)
{
    ...
    tor_inet_pton(AF_INET6, "2001:db8:8714:3a90::12", &(ipv6_sockaddr->sin6_addr));
    ...
    addr_len = tor_addr_to_sockaddr(tor_addr, 0, sockaddr_to_check, sizeof(struct sockaddr_in));

    tt_int_op(addr_len, OP_EQ, sizeof(struct sockaddr_in));
    tt_assert(sockaddr_in_are_equal((struct sockaddr_in *)sockaddr_to_check, ipv4_sockaddr_local));

    tor_addr = smartlist_get(smartlist, 1);
    addr_len = tor_addr_to_sockaddr(tor_addr, 0, sockaddr_to_check, sizeof(struct sockaddr_in));

    tt_int_op(addr_len, OP_EQ, sizeof(struct sockaddr_in));
    tt_assert(sockaddr_in_are_equal((struct sockaddr_in *)sockaddr_to_check, ipv4_sockaddr_remote));

    tor_addr = smartlist_get(smartlist, 2);
    addr_len = tor_addr_to_sockaddr(tor_addr, 0, sockaddr_to_check, sizeof(struct sockaddr_in6));

    tt_int_op(addr_len, OP_EQ, sizeof(struct sockaddr_in6));
    tt_assert(sockaddr_in6_are_equal((struct sockaddr_in6 *)sockaddr_to_check, ipv6_sockaddr));
...
test_address_ip_adapter_addresses_to_smartlist(void *arg)
{
    ...
    addrs1->FirstUnicastAddress = unicast11 = tor_malloc_zero(sizeof(IP_ADAPTER_UNICAST_ADDRESS));
    sockaddr_test1 = sockaddr_in_from_string("86.59.30.40", NULL);
    unicast11->Address.lpSockaddr = (LPSOCKADDR)sockaddr_test1;

    unicast11->Next = unicast12 = tor_malloc_zero(sizeof(IP_ADAPTER_UNICAST_ADDRESS));
    ...
    addrs2->FirstUnicastAddress = unicast21 = tor_malloc_zero(sizeof(IP_ADAPTER_UNICAST_ADDRESS));
...
test_address_ifreq_to_smartlist(void *arg)
{
    ...
    addr_len = tor_addr_to_sockaddr(tor_addr, 0, (struct sockaddr *)sockaddr_to_check,
                                    sizeof(struct sockaddr_in));
...
static tor_socket_t
pretend_to_connect(tor_socket_t sock, const struct sockaddr *address, socklen_t address_len)
{
    (void)address;
    (void)address_len;
...
static int
fake_getsockname(tor_socket_t sock, struct sockaddr *address, socklen_t *address_len)
{
...
test_address_udp_socket_trick_whitebox(void *arg)
{
    ...
    struct sockaddr_in6 *ipv6_to_check = tor_malloc_zero(sizeof(struct sockaddr_in6));
    ...
    hack_retval = get_interface_address6_via_udp_socket_hack(LOG_DEBUG, AF_INET, addr_from_hack);
    ...
    hack_retval = get_interface_address6_via_udp_socket_hack(LOG_DEBUG, AF_INET6, addr_from_hack);
...
test_address_udp_socket_trick_blackbox(void *arg)
{
  ...
  tt_assert( (retval == -1 && retval_reference == -1) ||
    ...
    retval = get_interface_address6_via_udp_socket_hack(LOG_DEBUG, AF_INET + AF_INET6, &addr4);
...
test_address_get_if_addrs6_list_internal(void *arg)
{
    ...
    if (smartlist_len(mock_saved_logs()) == 1) {
        expect_log_msg_containing_either4("connect() failed", "unable to create socket",
...
static int
mock_get_interface_address6_via_udp_socket_hack_fail(int severity, sa_family_t family,
                                                     tor_addr_t *addr)
{
    (void)severity;
    (void)family;
 ...
 * IPv6 address. */
#define CHECK_RI_ADDR(ipv4_addr_str, rv) TEST_ROUTER_VALID_ADDRESS_HELPER(ipv4_addr_str, "::", rv)
...
#define ADDRESS_TEST_STR_ARG(name, flags, str_arg)                                           \
    {                                                                                        \
#        name "/" str_arg, test_address_##name, flags, &passthrough_setup, (void *)(str_arg) \
    }
