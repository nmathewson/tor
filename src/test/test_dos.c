...
test_dos_conn_creation(void *arg)
{
    ...
    tt_int_op(DOS_CONN_DEFENSE_NONE, OP_EQ, dos_conn_addr_get_defense_type(addr));

    /* Register another conn and check that new conns are not allowed anymore */
    dos_new_client_conn(&or_conn, NULL);
    tt_int_op(DOS_CONN_DEFENSE_CLOSE, OP_EQ, dos_conn_addr_get_defense_type(addr));
...
test_dos_circuit_creation(void *arg)
{
    ...
    uint32_t min_conc_conns_for_cc = get_param_cc_min_concurrent_connection(NULL);
...
test_dos_bucket_refill(void *arg)
{
    ...
    clientmap_entry_t *entry = geoip_lookup_client(addr, NULL, GEOIP_CLIENT_CONNECT);
    tt_assert(entry);
