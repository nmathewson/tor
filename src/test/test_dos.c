...
test_dos_circuit_creation(void *arg)
{
    ...
    /* Register another CREATE cell that will push us over the limit. Check
     * that the cell gets refused. */
    dos_cc_new_create_cell(chan);
...
test_dos_bucket_refill(void *arg)
{
    (void)arg;
    int i;
    /* For this test, this variable is set to the current circ count of the
     * token bucket. */
    ...
    /* Check that the circuit rate is a positive number and smaller than the
     * max circuit count */
    ...
    /* Send a create cell: then check that the circ token bucket got
     * initialized and one circ was subtracted. */
    dos_cc_new_create_cell(chan);
    ...
    /* Now progress time a week forward, and check that the token bucket does
    ...
    /* Progress time forward one sec again, refill the bucket and check that
     * the refill happened correctly. */
    now += 1;
    update_approx_time(now);
    cc_stats_refill_bucket(&dos_stats->cc_stats, addr);
    /* check refill */
