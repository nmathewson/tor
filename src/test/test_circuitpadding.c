    ...
    if (circ == client_side) {
        if (cell->payload[0] == RELAY_COMMAND_PADDING_NEGOTIATE) {
            ...
            circpad_deliver_recognized_relay_cell_events(
                relay_side, cell->payload[0], NULL);
        }
        n_client_cells++;
...
helper_create_machine_with_big_histogram(circpad_removal_t removal_strategy)
{
    ...
    burst_state->next_state[CIRCPAD_EVENT_NONPADDING_RECV] =
        CIRCPAD_STATE_BURST;
...
test_circuitpadding_token_removal_higher(void *arg)
{
    ...
    /* This is the right order to remove tokens from this histogram. That is,
     * we first remove tokens from the 4th bin since 57 usec is nearest to the
     * 4th bin midpoint (31 + (62-31)/2 == 46). Then we remove from the 3rd bin
     * for the same reason, then from the 5th, etc. */
    ...
    for (i = 0; i < sizeof(bin_removal_order) / sizeof(int); i++) {
        ...
        /* Test that we cleaned out this bin. Don't do this in the case of the
           last bin since the tokens will get refilled */
...
test_circuitpadding_token_removal_exact(void *arg)
{
    ...
    /* Ensure that we will not remove any other tokens even tho we try to,
     * since this is what the exact strategy dictates */
    mi->padding_scheduled_at_usec = current_time - 57;
    circpad_cell_event_nonpadding_sent(client_side);
    for (int i = 0; i < BIG_HISTOGRAM_LEN; i++) {
        if (i != 4) {
...
test_circuitpadding_tokens(void *arg)
{
    ...
    for (circpad_delay_t i = 0; i <= state->histogram_edges[0]; i++) {
        int bin =
            circpad_histogram_usec_to_bin(client_side->padding_info[0], i);
        circpad_delay_t usec =
            circpad_histogram_bin_to_usec(client_side->padding_info[0], bin);
        int bin2 =
            circpad_histogram_usec_to_bin(client_side->padding_info[0], usec);
                           ...
                           int padding)
{
    ...
    hop->extend_info =
        extend_info_new(padding ? "padding" : "non-padding", digest, NULL,
                        NULL, NULL, &addr, padding);
...
test_circuitpadding_sample_distribution(void *arg)
{
    ...
    /* For every state, sample a bunch of values from the distribution and
     * ensure they fall within range. */
...
test_circuitpadding_machine_rate_limiting(void *arg)
{
    ...
    MOCK(circpad_machine_spec_transition,
    ...
    /* Send 99 padding cells which is below circpad_global_allowed_cells=100,
     * so the rate limit will not trigger */
    for (i = 0; i < 99; i++) {
        circpad_send_padding_cell_for_callback(mi);
    }
    retval = circpad_machine_reached_padding_limit(mi);
...
test_circuitpadding_global_rate_limiting(void *arg)
{
     ...
     * non-padding cells, the padding traffic is still 51% of total traffic so
    ...
    /* Send another non-padding cell to bring the padding traffic to 50% of
     * total traffic and get past the limit */
    circpad_cell_event_nonpadding_sent(relay_side);
    retval = circpad_machine_reached_padding_limit(mi);
    tt_int_op(retval, OP_EQ, 0);
...
test_circuitpadding_reduce_disable(void *arg)
{
    ...
    smartlist_free(vote1.net_params);
    vote1.net_params = smartlist_new();
    smartlist_split_string(vote1.net_params, "circpad_padding_disabled=1",
                           NULL, 0, 0);
...
test_circuitpadding_manage_circuit_lifetime(void *arg)
{
     ...
     * managing the lifetime manually, but the circuit was tagged as such by
     * the circpadding subsystem */
    tt_int_op(client_side->marked_for_close, OP_EQ, 0);
...
helper_test_hs_machines(bool test_intro_circs)
{
    ...
    /* Intro machines are at START state, but rend machines have already
     * skipped to OBFUSCATE_CIRC_SETUP because of the sent PADDING_NEGOTIATE.
     */
    tt_int_op(client_side->padding_info[0]->current_state, OP_EQ,
    ...
    if (test_intro_circs) {
        ...
        /* See that the machine has been teared down after all the length has
         * been exhausted (the padding info should now be null on both sides)
         */
        tt_ptr_op(relay_side->padding_info[0], OP_EQ, NULL);
        tt_ptr_op(client_side->padding_info[0], OP_EQ, NULL);
    } else {
        int i;
        /* Send state_length worth of padding and see that the state goes to
         * END */
