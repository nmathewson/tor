...
#define MONOTIME_MOCK_START (monotime_absolute_nsec() + TOR_NSEC_PER_USEC * TOR_USEC_PER_SEC)
...
static void simulate_single_hop_extend(circuit_t *client, circuit_t *mid_relay, int padding);
...
static void
circuitmux_attach_circuit_mock(circuitmux_t *cmux, circuit_t *circ, cell_direction_t direction)
{
    (void)cmux;
    (void)circ;
    (void)direction;
...
static int
circuit_package_relay_cell_mock(cell_t *cell, circuit_t *circ, cell_direction_t cell_direction,
                                crypt_path_t *layer_hint, streamid_t on_stream,
                                const char *filename, int lineno)
{
    (void)cell;
    ...
    if (circ == client_side) {
        if (cell->payload[0] == RELAY_COMMAND_PADDING_NEGOTIATE) {
            ...
            int is_target_hop = circpad_padding_is_from_expected_hop(circ, layer_hint);
            ...
            circpad_deliver_recognized_relay_cell_events(relay_side, cell->payload[0], NULL);
        }
        n_client_cells++;
        ...
        if (cell->payload[0] == RELAY_COMMAND_PADDING_NEGOTIATED) {
            ...
            circpad_deliver_recognized_relay_cell_events(
                client_side, cell->payload[0], TO_ORIGIN_CIRCUIT(client_side)->cpath->next);
        }
...
circpad_send_command_to_hop_mock(origin_circuit_t *circ, uint8_t hopnum, uint8_t relay_command,
...
test_circuitpadding_rtt(void *arg)
{
    ...
    client_side->padding_info[0] = circpad_circuit_machineinfo_new(client_side, 0);
    ...
    tt_int_op(circpad_histogram_bin_to_usec(relay_side->padding_info[0], 0), OP_EQ,
              relay_side->padding_info[0]->rtt_estimate_usec +
                  circpad_machine_current_state(relay_side->padding_info[0])->histogram_edges[0]);
    ...
    tt_int_op(relay_side->padding_info[0]->rtt_estimate_usec, OP_EQ, rtt_estimate);
    tt_u64_op(relay_side->padding_info[0]->last_received_time_usec, OP_EQ, 0);
    tt_int_op(relay_side->padding_info[0]->stop_rtt_update, OP_EQ, 1);
    tt_int_op(circpad_histogram_bin_to_usec(relay_side->padding_info[0], 0), OP_EQ,
              relay_side->padding_info[0]->rtt_estimate_usec +
    ...
    tt_int_op(circpad_histogram_bin_to_usec(client_side->padding_info[0], 0), OP_NE,
              client_side->padding_info[0]->rtt_estimate_usec);
    tt_int_op(circpad_histogram_bin_to_usec(client_side->padding_info[0], 0), OP_EQ,
              circpad_machine_current_state(client_side->padding_info[0])->histogram_edges[0]);
done:
...
helper_create_basic_machine(void)
{
    ...
    circ_client_machine.states[CIRCPAD_STATE_BURST].token_removal = CIRCPAD_TOKEN_REMOVAL_HIGHER;
...
helper_create_machine_with_big_histogram(circpad_removal_t removal_strategy)
{
    ...
    circpad_state_t *burst_state = &circ_client_machine.states[CIRCPAD_STATE_BURST];
...
test_circuitpadding_token_removal_higher(void *arg)
{
    ...
    circpad_cell_event_nonpadding_received(client_side);
    tt_int_op(client_side->padding_info[0]->current_state, OP_EQ, CIRCPAD_STATE_BURST);
    ...
    for (int i = 0; i <= BIG_HISTOGRAM_LEN; i++) {
        tt_uint_op(bin_left_bounds[i], OP_EQ, circpad_histogram_bin_to_usec(mi, i));
    }

    /* Test right boundaries of each histogram bin: */
    const circpad_delay_t bin_right_bounds[] = {0,   6,   14,  30,  61,
                                                124, 249, 499, 999, CIRCPAD_DELAY_INFINITE - 1};
    for (int i = 0; i < BIG_HISTOGRAM_LEN; i++) {
        tt_uint_op(bin_right_bounds[i], OP_EQ, histogram_get_bin_upper_bound(mi, i));
    }
    ...
    for (i = 0; i < sizeof(bin_removal_order) / sizeof(int); i++) {
        ...
        log_debug(LD_GENERAL, "Testing that %d attempt removes %d bin", i, bin_to_remove);
...
test_circuitpadding_token_removal_lower(void *arg)
{
    ...
    tt_int_op(client_side->padding_info[0]->current_state, OP_EQ, CIRCPAD_STATE_BURST);
    circ_client_machine.states[CIRCPAD_STATE_BURST].histogram_edges[BIG_HISTOGRAM_LEN - 2] = 100;
...
test_circuitpadding_tokens(void *arg)
{
    ...
    for (int bin = 0; bin < 2; bin++) {
        circpad_delay_t usec = circpad_histogram_bin_to_usec(client_side->padding_info[0], bin);
        int bin2 = circpad_histogram_usec_to_bin(client_side->padding_info[0], usec);
    ...
    for (int bin = 2; bin < state->histogram_len - 1; bin++) {
        ...
        bin2 = circpad_histogram_usec_to_bin(client_side->padding_info[0], usec + 3);
        tt_int_op(bin, OP_EQ, bin2);

        bin2 = circpad_histogram_usec_to_bin(client_side->padding_info[0], usec - 1);
    ...
    for (circpad_delay_t i = state->histogram_edges[0] + 1;
         i <= state->histogram_edges[0] + state->histogram_edges[state->histogram_len - 2]; i++) {
        int bin = circpad_histogram_usec_to_bin(client_side->padding_info[0], i);
        circpad_delay_t usec = circpad_histogram_bin_to_usec(client_side->padding_info[0], bin);
        int bin2 = circpad_histogram_usec_to_bin(client_side->padding_info[0], usec);
        tt_int_op(bin, OP_EQ, bin2);
    ...
    /* 2.a. Normal higher bin */
    {
        ...
        circpad_machine_remove_higher_token(mi, circpad_histogram_bin_to_usec(mi, 2) + 1);
        tt_int_op(mi->histogram[3], OP_EQ, 2);
    ...
    /* 2.b. Higher Infinity bin */
    {
        ...
        circpad_machine_remove_higher_token(mi, circpad_histogram_bin_to_usec(mi, 5) + 1000000);
    ...
    {
        tt_int_op(mi->histogram[3], OP_EQ, 2);
        circpad_machine_remove_lower_token(mi, circpad_histogram_bin_to_usec(mi, 3) + 1);
        circpad_machine_remove_lower_token(mi, circpad_histogram_bin_to_usec(mi, 3) + 1);
        tt_int_op(mi->histogram[3], OP_EQ, 0);
        tt_int_op(mi->histogram[2], OP_EQ, 2);
        circpad_machine_remove_lower_token(mi, circpad_histogram_bin_to_usec(mi, 3) + 1);
    ...
    tt_int_op(mi->histogram[2], OP_EQ, 2);
    circpad_machine_remove_closest_token(mi, circpad_histogram_bin_to_usec(mi, 2) + 1, 0);
    circpad_machine_remove_closest_token(mi, circpad_histogram_bin_to_usec(mi, 2) + 1, 0);
    tt_int_op(mi->histogram[2], OP_EQ, 0);
    tt_int_op(mi->histogram[3], OP_EQ, 2);
    circpad_machine_remove_closest_token(mi, circpad_histogram_bin_to_usec(mi, 2) + 1, 0);
    ...
    tt_int_op(mi->histogram[0], OP_EQ, 1);
    circpad_machine_remove_closest_token(mi, circpad_histogram_bin_to_usec(mi, 0) / 3, 1);
    tt_int_op(mi->histogram[0], OP_EQ, 0);
    tt_int_op(mi->histogram[2], OP_EQ, 2);
    circpad_machine_remove_closest_token(mi, circpad_histogram_bin_to_usec(mi, 0) / 3, 1);
    circpad_machine_remove_closest_token(mi, circpad_histogram_bin_to_usec(mi, 0) / 3, 1);
    tt_int_op(mi->histogram[2], OP_EQ, 0);
    tt_int_op(mi->histogram[3], OP_EQ, 2);
    circpad_machine_remove_closest_token(mi, circpad_histogram_bin_to_usec(mi, 4), 1);
    circpad_machine_remove_closest_token(mi, circpad_histogram_bin_to_usec(mi, 4), 1);
    tt_int_op(mi->histogram[3], OP_EQ, 0);
    tt_int_op(mi->histogram[4], OP_EQ, 2);
    circpad_machine_remove_closest_token(mi, circpad_histogram_bin_to_usec(mi, 4), 1);
...
test_circuitpadding_wronghop(void *arg)
{
    ...
    circpad_deliver_recognized_relay_cell_events(client_side, RELAY_COMMAND_DROP,
    ...
    circpad_negotiate_padding(TO_ORIGIN_CIRCUIT(client_side), 255, 2, CIRCPAD_COMMAND_STOP);
    ...
    circpad_padding_negotiated(relay_side, CIRCPAD_MACHINE_CIRC_SETUP, CIRCPAD_COMMAND_START,
                               CIRCPAD_RESPONSE_OK);
...
test_circuitpadding_negotiation(void *arg)
{
    ...
    circpad_negotiate_padding(TO_ORIGIN_CIRCUIT(client_side), CIRCPAD_MACHINE_CIRC_SETUP, 2,
...
simulate_single_hop_extend(circuit_t *client, circuit_t *mid_relay, int padding)
{
    ...
    hop->extend_info = extend_info_new(padding ? "padding" : "non-padding", digest, NULL, NULL,
                                       NULL, &addr, padding);
...
{
    circpad_machine_spec_t *ret = tor_malloc_zero(sizeof(circpad_machine_spec_t));

    /* Start, burst */
    circpad_machine_states_init(ret, 2);

    ret->states[CIRCPAD_STATE_START].next_state[CIRCPAD_EVENT_PADDING_SENT] = CIRCPAD_STATE_BURST;

    ret->states[CIRCPAD_STATE_BURST].next_state[CIRCPAD_EVENT_PADDING_SENT] = CIRCPAD_STATE_BURST;

    ret->states[CIRCPAD_STATE_BURST].next_state[CIRCPAD_EVENT_LENGTH_COUNT] = CIRCPAD_STATE_END;

    ret->states[CIRCPAD_STATE_BURST].next_state[CIRCPAD_EVENT_BINS_EMPTY] = CIRCPAD_STATE_END;
...
helper_create_conditional_machines(void)
{
    ...
    add->conditions.state_mask =
        CIRCPAD_CIRC_BUILDING | CIRCPAD_CIRC_NO_STREAMS | CIRCPAD_CIRC_HAS_RELAY_EARLY;
    ...
    add->conditions.state_mask =
        CIRCPAD_CIRC_OPENED | CIRCPAD_CIRC_STREAMS | CIRCPAD_CIRC_HAS_NO_RELAY_EARLY;
    add->conditions.purpose_mask = CIRCPAD_PURPOSE_ALL;
    circpad_register_padding_machine(add, origin_padding_machines);
...
test_circuitpadding_state_length(void *arg)
{
    ...
    client_machine->states[CIRCPAD_STATE_BURST].token_removal = CIRCPAD_TOKEN_REMOVAL_EXACT;
...
test_circuitpadding_conditions(void *arg)
{
    ...
    circpad_machine_event_circ_has_no_relay_early(TO_ORIGIN_CIRCUIT(client_side));
    tt_int_op(client_side->padding_machine[0]->machine_num, OP_EQ, 2);
    tt_int_op(relay_side->padding_machine[0]->machine_num, OP_EQ, 2);
...
static circpad_decision_t
circpad_machine_spec_transition_mock(circpad_machine_runtime_t *mi, circpad_event_t event)
{
    (void)mi;
    (void)event;
...
test_circuitpadding_reduce_disable(void *arg)
{
    ...
    networkstatus_t vote1;
    vote1.net_params = smartlist_new();
    smartlist_split_string(vote1.net_params, "circpad_padding_reduced=1", NULL, 0, 0);
    ...
    tt_int_op(circpad_machine_reached_padding_limit(client_side->padding_info[0]), OP_EQ, 0);
    tt_int_op(circpad_machine_reached_padding_limit(relay_side->padding_info[0]), OP_EQ, 0);

    /* Test that machines get torn down when padding is disabled */
    SMARTLIST_FOREACH(vote1.net_params, char *, cp, tor_free(cp));
    smartlist_free(vote1.net_params);
    vote1.net_params = smartlist_new();
    smartlist_split_string(vote1.net_params, "circpad_padding_disabled=1", NULL, 0, 0);

    /* Register reduced padding machine with the padding subsystem */
    circpad_new_consensus_params(&vote1);

    tt_int_op(circpad_machine_schedule_padding(client_side->padding_info[0]), OP_EQ,
              CIRCPAD_STATE_UNCHANGED);
    tt_int_op(circpad_machine_schedule_padding(relay_side->padding_info[0]), OP_EQ,
              CIRCPAD_STATE_UNCHANGED);
...
test_circuitpadding_manage_circuit_lifetime(void *arg)
{
    ...
    circuit_mark_for_close(client_side, END_CIRC_REASON_FLAG_REMOTE | END_CIRC_REASON_NONE);
    tt_ptr_op(client_side->padding_info[0], OP_NE, NULL);
    ...
    mocked_timeofday = client_side->timestamp_dirty + get_options()->MaxCircuitDirtiness + 2;
...
helper_test_hs_machines(bool test_intro_circs)
{
    ...
    client_side->purpose =
        test_intro_circs ? CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT : CIRCUIT_PURPOSE_C_REND_JOINED;
    circpad_add_matching_machines(origin_client_side, origin_padding_machines);
    tt_ptr_op(client_side->padding_info[0], OP_NE, NULL);
    tt_ptr_op(client_side->padding_machine[0], OP_NE, NULL);
    ...
    circpad_machine_runtime_t *client_machine_runtime = client_side->padding_info[0];
    circpad_machine_runtime_t *relay_machine_runtime = relay_side->padding_info[0];
    ...
    if (test_intro_circs) {
        ...
        tt_i64_op(relay_machine_runtime->state_length, OP_GE, INTRO_MACHINE_MINIMUM_PADDING);
        tt_i64_op(relay_machine_runtime->state_length, OP_LT, INTRO_MACHINE_MAXIMUM_PADDING);
    } else {
        tt_i64_op(client_machine_runtime->state_length, OP_EQ, 1);
    ...
    if (test_intro_circs) {
        ...
        /* See that the machine has been teared down after all the length has been
         * exhausted. */
        tt_int_op(client_side->padding_info[0]->current_state, OP_EQ, CIRCPAD_STATE_END);
    }
...
struct testcase_t circuitpadding_tests[] = {
    ...
    TEST_CIRCUITPADDING(circuitpadding_ignore_non_padding_cells, TT_FORK), END_OF_TESTCASES};
