...
test_bwmgt_token_buf_refill(void *arg)
{
    ...
    /* No time: nothing happens. */
    {
        ...
        tt_int_op(0, OP_EQ,
                  token_bucket_rw_refill(&b, START_TS + BW_SEC * 3 / 2));
...
test_bwmgt_dir_conn_global_write_low(void *arg)
{
    ...
    /* Add all configured authorities (hardcoded) before we set the consensus
     * so the address set exists. */
    ...
    addr_family =
        tor_addr_parse(&conn->addr, "[2001:638:a000:4140::ffff:189]");
