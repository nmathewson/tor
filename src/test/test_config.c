...
#define addressmap_rewrite(a, s, eo, ao) addressmap_rewrite((a), (s), ~0, (eo), (ao))
...
static void
good_bridge_line_test(const char *string, const char *test_addrport, const char *test_digest,
                      const char *test_transport, const smartlist_t *test_socks_args)
{
    char *tmp = NULL;
...
test_config_parse_bridge_line(void *arg)
{
    ...
    good_bridge_line_test("transport [::1]", "[::1]:443", NULL, "transport", NULL);

    good_bridge_line_test("transport 192.0.2.1:12 "
                          "4352e58420e68f5e40bf7c74faddccd9d1349413",
                          "192.0.2.1:12", "4352e58420e68f5e40bf7c74faddccd9d1349413", "transport",
                          NULL);
...

    {
        ...
        good_bridge_line_test("transport 192.0.2.1:12 twoandtwo=five z=z", "192.0.2.1:12", NULL,
                              "transport", sl_tmp);
...

    {
        ...
        good_bridge_line_test("transport 192.0.2.1:12 "
                              ...
                              "192.0.2.1:12", "4352e58420e68f5e40bf7c74faddccd9d1349666",
                              "transport", sl_tmp);
    ...
    good_bridge_line_test("192.0.2.1:1231 "
                          "4352e58420e68f5e40bf7c74faddccd9d1349413",
                          "192.0.2.1:1231", "4352e58420e68f5e40bf7c74faddccd9d1349413", NULL, NULL);
    ...
    bad_bridge_line_test("obfs2 2.2.2.2:1231 aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
...
test_config_parse_transport_options_line(void *arg)
{
    ...
    { /* correct line, but wrong transport specified */
        options_sl = get_options_from_transport_options_line("trebuchet k=v", "rook");
        tt_ptr_op(options_sl, OP_EQ, NULL);
    }
    ...
    { /* correct -- no transport specified */
        ...
        options_sl = get_options_from_transport_options_line("rook ladi=dadi weliketo=party", NULL);
    ...
    { /* correct -- correct transport specified */
        ...
        options_sl =
            get_options_from_transport_options_line("rook ladi=dadi weliketo=party", "rook");
        tt_assert(options_sl);
        tt_assert(smartlist_strings_eq(options_sl, sl_tmp));
        SMARTLIST_FOREACH(sl_tmp, char *, s, tor_free(s));
        smartlist_free(sl_tmp);
...
static void pt_kickstart_proxy_mock(const smartlist_t *transport_list, char **proxy_argv,
                                    int is_server);
static int transport_add_from_config_mock(const tor_addr_t *addr, uint16_t port, const char *name,
                                          int socks_ver);
...
static void
pt_kickstart_proxy_mock(const smartlist_t *transport_list, char **proxy_argv, int is_server)
{
    (void)transport_list;
    (void)proxy_argv;
    (void)is_server;
...
static int
transport_add_from_config_mock(const tor_addr_t *addr, uint16_t port, const char *name,
                               int socks_ver)
{
    (void)addr;
    (void)port;
...
test_config_parse_transport_plugin_line(void *arg)
{
    ...
    r = pt_parse_transport_line(options, "transport_1 exec /usr/bin/fake-transport", 1, 0);
    tt_int_op(r, OP_EQ, 0);
    r = pt_parse_transport_line(options, "transport_1 exec /usr/bin/fake-transport", 1, 1);
    tt_int_op(r, OP_EQ, 0);
    r = pt_parse_transport_line(options, "transport_1,transport_2 exec /usr/bin/fake-transport", 1,
                                0);
    tt_int_op(r, OP_EQ, 0);
    r = pt_parse_transport_line(options, "transport_1,transport_2 exec /usr/bin/fake-transport", 1,
                                1);
    ...
    r = pt_parse_transport_line(options, "transport_* exec /usr/bin/fake-transport", 1, 0);
    tt_int_op(r, OP_LT, 0);
    r = pt_parse_transport_line(options, "transport_* exec /usr/bin/fake-transport", 1, 1);
    tt_int_op(r, OP_LT, 0);
    ...
    r = pt_parse_transport_line(options, "transport_1,transport_2 socks5 1.2.3.4:567", 1, 0);
    tt_int_op(r, OP_LT, 0);
    r = pt_parse_transport_line(options, "transport_1,transport_2 proxy 1.2.3.4:567", 1, 1);
    tt_int_op(r, OP_LT, 0);
    /* No port error exit */
    ...
    r = pt_parse_transport_line(options, "transport_1 exec /usr/bin/fake-transport", 0, 1);
    tt_int_op(r, OP_EQ, 0);
    tt_assert(pt_kickstart_proxy_mock_call_count == old_pt_kickstart_proxy_mock_call_count + 1);
    ...
    old_pt_kickstart_proxy_mock_call_count = pt_kickstart_proxy_mock_call_count;
    old_transport_add_from_config_mock_call_count = transport_add_from_config_mock_call_count;
    old_transport_is_needed_mock_call_count = transport_is_needed_mock_call_count;
    r = pt_parse_transport_line(options, "transport_1 exec /usr/bin/fake-transport", 0, 0);
    /* Should have succeeded */
    tt_int_op(r, OP_EQ, 0);
    /* transport_is_needed() should have been called */
    tt_assert(transport_is_needed_mock_call_count == old_transport_is_needed_mock_call_count + 1);
    /*
     * pt_kickstart_proxy() and transport_add_from_config() should
     * not have been called.
     */
    tt_assert(pt_kickstart_proxy_mock_call_count == old_pt_kickstart_proxy_mock_call_count);
 ...
 */
static int
get_interface_address6_replacement(int severity, sa_family_t family, tor_addr_t *addr)
{
    (void)severity;
 ...
 */
static int
get_interface_address6_failure(int severity, sa_family_t family, tor_addr_t *addr)
{
    (void)severity;
    (void)addr;
...
test_config_resolve_my_address(void *arg)
{
    ...
    retval = resolve_my_address(LOG_NOTICE, options, &resolved_addr, &method_used, &hostname_out);
    ...
    tt_want(retval == 0);
    tt_want_int_op(n_gethostname_replacement, OP_EQ, prev_n_gethostname_replacement + 1);
    tt_want_int_op(n_get_interface_address, OP_EQ, prev_n_get_interface_address + 1);
    ...
    tt_want(n_get_interface_address_failure == prev_n_get_interface_address_failure + 1);
    tt_want(n_gethostname_replacement == prev_n_gethostname_replacement + 1);
    tt_int_op(retval, OP_EQ, -1);
    ...
    retval = resolve_my_address(LOG_DEBUG, options, &resolved_addr, &method_used, &hostname_out);
    ...
    tt_want(n_gethostname_replacement == prev_n_gethostname_replacement + 1);
    tt_want(n_hostname_localhost == prev_n_hostname_localhost + 1);
    tt_want(n_get_interface_address6_failure == prev_n_get_interface_address6_failure + 1);
...
test_config_adding_trusted_dir_server(void *arg)
{
    ...
    ds = trusted_dir_server_new("ds", "127.0.0.1", 9059, 9060, NULL, digest, NULL, V3_DIRINFO, 1.0);
    tt_assert(ds);
    dir_server_add(ds);
    ...
    ds =
        trusted_dir_server_new("ds", "127.0.0.1", 9059, 9060, &ipv6, digest, NULL, V3_DIRINFO, 1.0);
    tt_assert(ds);
    dir_server_add(ds);
...
test_config_parsing_trusted_dir_server(void *arg)
{
    ...
    rv = parse_dir_authority_line(TEST_DIR_AUTH_LINE_START TEST_DIR_AUTH_LINE_END, V3_DIRINFO, 1);
    tt_int_op(rv, OP_EQ, 0);

    /* parse a trusted dir server with an IPv6 address and port */
    rv = parse_dir_authority_line(
        TEST_DIR_AUTH_LINE_START TEST_DIR_AUTH_IPV6_FLAG TEST_DIR_AUTH_LINE_END, V3_DIRINFO, 1);
    tt_int_op(rv, OP_EQ, 0);
...
test_config_parsing_invalid_dir_address(void *arg)
{
    ...
    rv = parse_dir_authority_line(TEST_DIR_AUTH_LINE_START TEST_DIR_AUTH_LINE_END_BAD_IP,
                                  V3_DIRINFO, 1);
    tt_int_op(rv, OP_EQ, -1);

    rv = parse_dir_authority_line(TEST_DIR_AUTH_LINE_START TEST_DIR_AUTH_LINE_END_WITH_DNS_ADDR,
                                  V3_DIRINFO, 1);
    tt_int_op(rv, OP_EQ, -1);
...
test_config_parsing_fallback_dir_server(void *arg)
{
    ...
    /* parse a trusted dir server with an IPv6 address and port */
    rv = parse_dir_fallback_line(TEST_DIR_FALLBACK_LINE TEST_DIR_FALLBACK_IPV6_FLAG, 1);
    tt_int_op(rv, OP_EQ, 0);
...
add_default_fallback_dir_servers_known_default(void)
{
    ...
    for (i = 0; fallback[i]; i++) {
        if (parse_dir_fallback_line(fallback[i], 0) < 0) {
            log_err(LD_BUG, "Couldn't parse internal FallbackDir line %s", fallback[i]);
        }
    }
...
test_config_adding_dir_servers(void *arg)
{
    ...
    test_dir_authority->value =
        tor_strdup("D0 orport=9000 "
                   "v3ident=0023456789012345678901234567890123456789 "
                   "127.0.0.1:60090 0123 4567 8901 2345 6789 0123 4567 8901 2345 6789");

    config_line_t *test_alt_bridge_authority = tor_malloc_zero(sizeof(config_line_t));
    test_alt_bridge_authority->key = tor_strdup("AlternateBridgeAuthority");
    test_alt_bridge_authority->value =
        tor_strdup("B1 orport=9001 bridge "
                   "127.0.0.1:60091 1123 4567 8901 2345 6789 0123 4567 8901 2345 6789");

    config_line_t *test_alt_dir_authority = tor_malloc_zero(sizeof(config_line_t));
    test_alt_dir_authority->key = tor_strdup("AlternateDirAuthority");
    test_alt_dir_authority->value =
        tor_strdup("A2 orport=9002 "
    ...
    config_line_t *test_fallback_directory = tor_malloc_zero(sizeof(config_line_t));
    test_fallback_directory->key = tor_strdup("FallbackDir");
    test_fallback_directory->value =
        tor_strdup("127.0.0.1:60093 orport=9003 id=0323456789012345678901234567890123456789");
...
#define n_default_authorities ((n_default_alt_bridge_authority) + (n_default_alt_dir_authority))
       ...
       AlternateDirAuthority Not Set, FallbackDir Set */
    {
...

        {
            ...
            SMARTLIST_FOREACH(fallback_servers, dir_server_t *, ds,
                              /* increment the found counter if dir_port matches */
                              found_non_default_fallback += (ds->dir_port == 60093 ? 1 : 0));
            ...
            SMARTLIST_FOREACH(fallback_servers, dir_server_t *, ds,
                              /* increment the found counter if dir_port matches */
                              found_default_fallback += (ds->dir_port == 60099 ? 1 : 0));
    ...
    */
       ...
       AlternateDirAuthority Not Set, FallbackDir Set */
    {
...

        {
            ...
            /* (No D0), B1, (No A2), Default v3 Non-Bridge Authorities,
             * Custom Fallback */
            tt_assert(smartlist_len(fallback_servers) == 2 + n_default_alt_dir_authority);
    ...
    */
       ...
       AlternateDirAuthority & FallbackDir Set */
    {
...

        {
            ...
            tt_assert(smartlist_len(fallback_servers) == 2 + n_default_alt_bridge_authority);
     ...
     AlternateDirAuthority Set, FallbackDir Not Set */
    {
...

        {
            ...
            tt_assert(smartlist_len(fallback_servers) == 1 + n_default_alt_bridge_authority);
...
static int
mock_router_pick_published_address(const or_options_t *options, uint32_t *addr, int cache_only)
{
    (void)options;
    (void)addr;
    (void)cache_only;
...
test_config_directory_fetch(void *arg)
{
    ...
    MOCK(router_my_exit_policy_is_reject_star, mock_router_my_exit_policy_is_reject_star);
    ...
    tt_int_op(networkstatus_consensus_can_use_multiple_directories(options), OP_EQ, 1);
    ...
    tt_int_op(dirclient_fetches_from_authorities(options), OP_EQ, 1);
    tt_int_op(networkstatus_consensus_can_use_multiple_directories(options), OP_EQ, 0);
...
test_config_port_cfg_line_extract_addrport(void *arg)
{
    ...
    tt_int_op(port_cfg_line_extract_addrport("hello", &a, &unixy, &rest), OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 0);
    tt_str_op(a, OP_EQ, "hello");
    tt_str_op(rest, OP_EQ, "");
    tor_free(a);

    tt_int_op(port_cfg_line_extract_addrport(" flipperwalt gersplut", &a, &unixy, &rest), OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 0);
    tt_str_op(a, OP_EQ, "flipperwalt");
    tt_str_op(rest, OP_EQ, "gersplut");
    tor_free(a);

    tt_int_op(port_cfg_line_extract_addrport(" flipperwalt \t gersplut", &a, &unixy, &rest), OP_EQ,
              0);
    tt_int_op(unixy, OP_EQ, 0);
    tt_str_op(a, OP_EQ, "flipperwalt");
    tt_str_op(rest, OP_EQ, "gersplut");
    ...
    tt_int_op(port_cfg_line_extract_addrport("unix:flipperwalt \t gersplut", &a, &unixy, &rest),
              OP_EQ, 0);
    ...
    tt_int_op(port_cfg_line_extract_addrport("lolol", &a, &unixy, &rest), OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 0);
    tt_str_op(a, OP_EQ, "lolol");
    tt_str_op(rest, OP_EQ, "");
    tor_free(a);

    tt_int_op(port_cfg_line_extract_addrport("unix:lolol", &a, &unixy, &rest), OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 1);
    tt_str_op(a, OP_EQ, "lolol");
    tt_str_op(rest, OP_EQ, "");
    tor_free(a);
    ...
    tt_int_op(port_cfg_line_extract_addrport("foobar:lolol", &a, &unixy, &rest), OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 0);
    ...
    tt_int_op(port_cfg_line_extract_addrport("unix:\"lolol\" foo ", &a, &unixy, &rest), OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 1);
    tt_str_op(a, OP_EQ, "lolol");
    tt_str_op(rest, OP_EQ, "foo ");
    tor_free(a);

    tt_int_op(port_cfg_line_extract_addrport("unix:\"lol ol\" foo ", &a, &unixy, &rest), OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 1);
    tt_str_op(a, OP_EQ, "lol ol");
    tt_str_op(rest, OP_EQ, "foo ");
    tor_free(a);

    tt_int_op(port_cfg_line_extract_addrport("unix:\"lol\\\" ol\" foo ", &a, &unixy, &rest), OP_EQ,
              0);
    tt_int_op(unixy, OP_EQ, 1);
    tt_str_op(a, OP_EQ, "lol\" ol");
    tt_str_op(rest, OP_EQ, "foo ");
    ...
    tt_int_op(port_cfg_line_extract_addrport("unix:\"lol\\0\" ol foo ", &a, &unixy, &rest), OP_EQ,
              -1);
    tor_free(a);
...
test_config_parse_port_config__ports__no_ports_given(void *data)
{
    ...
    smartlist_clear(slout);
    ret =
        port_parse_config(slout, NULL, "DNS", 0, "/foo/bar/unixdomain", 42, CL_PORT_IS_UNIXSOCKET);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
...
test_config_parse_port_config__ports__ports_given(void *data)
{
    ...
    ret = port_parse_config(NULL, config_port_valid, "DNS", CONN_TYPE_AP_DNS_LISTENER, NULL, 0, 0);
    tt_int_op(ret, OP_EQ, -1);

    // Test valid unix domain
    SMARTLIST_FOREACH(slout, port_cfg_t *, pf, port_cfg_free(pf));
    smartlist_clear(slout);
    ret = port_parse_config(slout, config_port_valid, "SOCKS", CONN_TYPE_AP_LISTENER, NULL, 0, 0);
#ifdef _WIN32
    tt_int_op(ret, OP_EQ, -1);
#else
    tt_int_op(ret, OP_EQ, 0);
    ...
    config_free_lines(config_port_invalid);
    config_port_invalid = NULL;
    config_port_invalid = mock_config_line("SOCKSPort", "unix:/tmp/foo/bar NoIPv4Traffic "
                                                        "NoIPv6Traffic "
                                                        "NoOnionTraffic");
    ret = port_parse_config(NULL, config_port_invalid, "SOCKS", CONN_TYPE_AP_LISTENER, NULL, 0,
    ...
    config_free_lines(config_port_invalid);
    config_port_invalid = NULL;
    config_port_invalid = mock_config_line("DNSPort", "127.0.0.1:80 NoDNSRequest");
    ret = port_parse_config(NULL, config_port_invalid, "DNS", CONN_TYPE_AP_DNS_LISTENER, NULL, 0,
                            CL_PORT_TAKES_HOSTNAMES);
    tt_int_op(ret, OP_EQ, -1);
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", CONN_TYPE_AP_DNS_LISTENER, NULL, 0,
                            CL_PORT_TAKES_HOSTNAMES);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    config_free_lines(config_port_invalid);
    config_port_invalid = NULL;
    config_port_invalid = mock_config_line("SOCKSPort", "NoIPv6Traffic "
    ...
    ret = port_parse_config(slout, config_port_valid, "SOCKS", CONN_TYPE_AP_LISTENER, NULL, 0,
                            CL_PORT_TAKES_HOSTNAMES);
#ifdef _WIN32
    tt_int_op(ret, OP_EQ, -1);
#else
    ...
    ret = port_parse_config(NULL, config_port_invalid, "DNS", 0, "127.0.0.3", 0, 0);
    tt_int_op(ret, OP_EQ, -1);
    ...
    ret = port_parse_config(NULL, config_port_valid, "DNS", 0, "127.0.0.3", 0, 0);
    tt_int_op(ret, OP_EQ, 0);
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.3", 0, 0);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    ret = port_parse_config(slout, config_port_valid, "SOCKS", CONN_TYPE_AP_LISTENER, "127.0.0.42",
                            0, CL_PORT_TAKES_HOSTNAMES);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    tt_int_op(port_cfg->entry_cfg.prefer_ipv6, OP_EQ, 1);
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.42", 0, 0);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    ret = port_parse_config(slout, config_port_invalid, "DNS", 0, "127.0.0.42", 0, 0);
    tt_int_op(ret, OP_EQ, -1);

    // Test success with warn non-local control
    SMARTLIST_FOREACH(slout, port_cfg_t *, pf, port_cfg_free(pf));
    smartlist_clear(slout);
    ret = port_parse_config(slout, config_port_valid, "Control", CONN_TYPE_CONTROL_LISTENER,
    ...
    SMARTLIST_FOREACH(slout, port_cfg_t *, pf, port_cfg_free(pf));
    smartlist_clear(slout);
    ret = port_parse_config(slout, config_port_valid, "ExtOR", CONN_TYPE_EXT_OR_LISTENER,
                            "127.0.0.42", 0, CL_PORT_WARN_NONLOCAL);
    tt_int_op(ret, OP_EQ, 0);
    ...
    ret = port_parse_config(slout, config_port_invalid, "DNS", 0, "127.0.0.44", 0,
    ...
    config_port_invalid = mock_config_line("DNSPort", "42 SessionGroup=123 "
                                                      "SessionGroup=321");
    ret = port_parse_config(slout, config_port_invalid, "DNS", 0, "127.0.0.44", 0, 0);
    tt_int_op(ret, OP_EQ, -1);
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.44", 0, 0);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.46", 0, 0);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    ret = port_parse_config(NULL, config_port_invalid, "DNS", 0, "127.0.0.46", 0, 0);
    ...
    ret = port_parse_config(slout, config_port_invalid, "DNS", 0, "127.0.0.46", 0, 0);
    tt_int_op(ret, OP_EQ, -1);
    ...
    ret = port_parse_config(slout, config_port_valid, "SOCKS", CONN_TYPE_AP_LISTENER, "127.0.0.46",
                            0, CL_PORT_DFLT_GROUP_WRITABLE);
#ifdef _WIN32
    tt_int_op(ret, OP_EQ, -1);
#else
    tt_int_op(ret, OP_EQ, 0);
...
test_config_parse_port_config__ports__server_options(void *data)
{
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, NULL, 0, CL_PORT_SERVER_OPTIONS);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    config_port_invalid = mock_config_line("DNSPort", "127.0.0.124:656 NoListen "
                                                      "NoAdvertise");
    ret = port_parse_config(slout, config_port_invalid, "DNS", 0, NULL, 0, CL_PORT_SERVER_OPTIONS);
    tt_int_op(ret, OP_EQ, -1);
    ...
    ret =
        port_parse_config(slout, config_port_invalid, "ORPort", 0, NULL, 0, CL_PORT_SERVER_OPTIONS);
    tt_int_op(ret, OP_EQ, -1);
...
test_config_get_first_advertised(void *data)
{
    ...
    port = get_first_advertised_port_by_type_af(CONN_TYPE_OR_LISTENER, AF_INET);
    tt_int_op(port, OP_EQ, 0);
    addr = get_first_advertised_addr_by_type_af(CONN_TYPE_OR_LISTENER, AF_INET);
    tt_ptr_op(addr, OP_EQ, NULL);

    port = get_first_advertised_port_by_type_af(CONN_TYPE_OR_LISTENER, AF_INET6);
    tt_int_op(port, OP_EQ, 0);
    addr = get_first_advertised_addr_by_type_af(CONN_TYPE_OR_LISTENER, AF_INET6);
    tt_ptr_op(addr, OP_EQ, NULL);

    config_line_append(&opts->ORPort_lines, "ORPort", "[1234::5678]:8080");
    config_line_append(&opts->ORPort_lines, "ORPort", "1.2.3.4:9999 noadvertise");
    config_line_append(&opts->ORPort_lines, "ORPort", "5.6.7.8:9911 nolisten");

    r = parse_ports(opts, 0, &msg, &n, &w);
    tt_assert(r == 0);

    // UNSPEC gets us nothing.
    port = get_first_advertised_port_by_type_af(CONN_TYPE_OR_LISTENER, AF_UNSPEC);
    tt_int_op(port, OP_EQ, 0);
    addr = get_first_advertised_addr_by_type_af(CONN_TYPE_OR_LISTENER, AF_UNSPEC);
    tt_ptr_op(addr, OP_EQ, NULL);
...
{
    int ret;
    const char *severity_log_lines[] = {"debug file /tmp/debug.log", "debug\tfile /tmp/debug.log",
...
test_config_include_limit(void *data)
{
    ...
    tor_snprintf(torrc_contents, sizeof(torrc_contents), "%%include %s", torrc_path);
    tt_int_op(write_str_to_file(torrc_path, torrc_contents, 0), OP_EQ, 0);

    tt_int_op(config_get_lines_include(torrc_contents, &result, 0, NULL, NULL), OP_EQ, -1);
...
test_config_include_does_not_exist(void *data)
{
    ...
    char torrc_contents[1000];
    tor_snprintf(torrc_contents, sizeof(torrc_contents), "%%include %s", missing_path);
...
test_config_include_error_in_included_file(void *data)
{
    ...
    char torrc_contents[1000];
    tor_snprintf(torrc_contents, sizeof(torrc_contents), "%%include %s", invalid_path);
...
test_config_include_empty_file_folder(void *data)
{
    ...
    int include_used;
    tt_int_op(config_get_lines_include(torrc_contents, &result, 0, &include_used, NULL), OP_EQ, 0);
...
test_config_include_no_permission(void *data)
{
    ...
    int include_used;
    tt_int_op(config_get_lines_include(torrc_contents, &result, 0, &include_used, NULL), OP_EQ, -1);
    tt_ptr_op(result, OP_EQ, NULL);
...
test_config_include_recursion_before_after(void *data)
{
    ...
    int include_used;
    tt_int_op(config_get_lines_include(file_contents, &result, 0, &include_used, NULL), OP_EQ, 0);
...
test_config_include_flag_both_without(void *data)
{
    ...
    int ret = options_init_from_string(conf_empty, conf_empty, CMD_RUN_UNITTESTS, NULL, &errmsg);
    tt_int_op(ret, OP_EQ, 0);
...
test_config_include_flag_torrc_only(void *data)
{
    ...
    int ret = options_init_from_string(conf_empty, conf_include, CMD_RUN_UNITTESTS, NULL, &errmsg);
    tt_int_op(ret, OP_EQ, 0);
...
test_config_include_flag_defaults_only(void *data)
{
    ...
    int ret = options_init_from_string(conf_include, conf_empty, CMD_RUN_UNITTESTS, NULL, &errmsg);
    tt_int_op(ret, OP_EQ, 0);
...
test_config_include_opened_file_list(void *data)
{
    ...
    tor_asprintf(&path, "%s" PATH_SEPARATOR "%s", subfolder, "01_file_in_subfolder");
    tt_int_op(write_str_to_file(path, "Test 1\n", 0), OP_EQ, 0);
    ...
    int include_used;
    tt_int_op(config_get_lines_include(torrc_contents, &result, 0, &include_used, opened_files),
              OP_EQ, 0);
    tt_ptr_op(result, OP_NE, NULL);
    tt_int_op(include_used, OP_EQ, 1);
...
#define MEGABYTE(x) (UINT64_C(x) << 20)
    ...
    tt_u64_op(compute_real_max_mem_in_queues(0, 0), OP_EQ, 3 * (GIGABYTE(1) / 4));
    ...
    tt_u64_op(compute_real_max_mem_in_queues(0, 0), OP_EQ, 2 * (GIGABYTE(8) / 5));

    /* We are running on a machine with 16 GB of RAM. */
    total_system_memory_output = GIGABYTE(16);

    /* We will have 0.4 * RAM available. */
    tt_u64_op(compute_real_max_mem_in_queues(0, 0), OP_EQ, 2 * (GIGABYTE(16) / 5));
    ...
    tt_u64_op(compute_real_max_mem_in_queues(0, 0), OP_EQ, MAX_DEFAULT_MEMORY_QUEUE_SIZE);
