                      ...
                      const smartlist_t *test_socks_args)
{
    ...
    if (test_socks_args)
        tt_assert(
            smartlist_strings_eq(test_socks_args, bridge_line->socks_args));
...
{
    (void)arg;
    good_bridge_line_test("192.0.2.1:4123", "192.0.2.1:4123", NULL, NULL,
                          NULL);
...
test_config_parse_transport_plugin_line(void *arg)
{
    ...
    r = pt_parse_transport_line(options, "transport_1 socks4 1.2.3.4:567", 1,
                                0);
    tt_int_op(r, OP_EQ, 0);
    r = pt_parse_transport_line(options, "transport_1 socks5 1.2.3.4:567", 1,
                                0);
    tt_int_op(r, OP_EQ, 0);
    /* Proxy case for server transport */
    r = pt_parse_transport_line(options, "transport_1 proxy 1.2.3.4:567", 1,
                                1);
    tt_int_op(r, OP_EQ, 0);
    ...
    r = pt_parse_transport_line(options, "transport_1 proxy 1.2.3.4:567", 0,
                                1);
    tt_int_op(r, OP_EQ, 0);
    ...
    old_transport_is_needed_mock_call_count =
        transport_is_needed_mock_call_count;
    r = pt_parse_transport_line(options, "transport_1 socks5 1.2.3.4:567", 0,
                                0);
    /* Should have succeeded */
    tt_int_op(r, OP_EQ, 0);
    /*
...
test_config_resolve_my_address(void *arg)
{
    ...
    /*
     * CASE 7:
     * We want resolve_my_address() to try and get network interface address
     * via get_interface_address() if hostname returned by tor_gethostname()
     * cannot be resolved into IP address.
     */
...
test_config_adding_trusted_dir_server(void *arg)
{
    ...
    rv = tor_addr_port_parse(LOG_WARN, "[::1]:9061", &ipv6.addr, &ipv6.port,
                             -1);
...
test_config_parsing_trusted_dir_server(void *arg)
{
    ...
    rv = parse_dir_authority_line(
        TEST_DIR_AUTH_LINE_START TEST_DIR_AUTH_IPV6_FLAG
...
test_config_adding_dir_servers(void *arg)
{
     ...
     *
     * Let's count these cases using binary, with 1 meaning set & 0 meaning
     * NULL So 1001 or case 9 is: DirAuthorities set, AlternateBridgeAuthority
     * NULL, AlternateDirAuthority NULL FallbackDir set The valid cases are
     * cases 0-9 counting using this method, as every case greater than or
     * equal to 10 = 1010 is invalid.
     *
     ...
     *           AlternateBridgeAuthority and AlternateDirAuthority are both
     * set)
     *
     ...
     *  - Use FallbackDir if it is set, otherwise No Default Fallback
     * Directories Cases expected to yield this outcome: 2 & 3 (the 2 cases
     * where DirAuthorities and AlternateBridgeAuthority are both NULL, but
     * AlternateDirAuthority is set)
     *
...
#define n_default_authorities \
     ...
     */
    {
        ...
        tt_assert(networkstatus_consensus_can_use_extra_fallbacks(options) ==
                  1);

        {
            /* fallback_dir_servers */
            const smartlist_t *fallback_servers =
            ...
            /* If we have no default authorities, something has gone really
             * wrong */
    ...
    */
       ...
       AlternateDirAuthority Not Set, FallbackDir Set */
    {
...

        {
             ...
             * default v3 non-Bridge directory authorities, so let's assume
             * that if the total count above is correct, we have the right
             * ones.
             */
        }
    ...
    /*
     3. Outcome: Use Set Alternate Directory Authority
       - Use Default Bridge Authorities
       - Use FallbackDir if it is set, otherwise No Default Fallback
     Directories Cases expected to yield this outcome: 2 & 3 (the 2 cases where
     DirAuthorities and AlternateBridgeAuthority are both NULL, but
     AlternateDirAuthority is set)
    */
       ...
       AlternateDirAuthority & FallbackDir Set */
    {
...

        {
            ...
            /* There's no easy way of checking that we have included all the
             * default Bridge authorities (except for hard-coding tonga's
             * details), so let's assume that if the total count above is
             * correct, we have the right ones.
             */
        }
    ...
    */
    ...
    /*
     5. Outcome: Use All Defaults
       - Use Default Bridge & Directory Authorities, Default Fallback
     Directories Cases expected to yield this outcome: 0 (DirAuthorities,
     AlternateBridgeAuthority, AlternateDirAuthority and FallbackDir are all
     NULL)
    */
...
test_config_directory_fetch(void *arg)
{
    ...
    /* Dir servers fetch the consensus from the authorities, unless they are
     * not advertising themselves (hibernating) or have no routerinfo or are
     * not advertising their dirport, and never use multiple directories for
...
test_config_port_cfg_line_extract_addrport(void *arg)
{
    ...
    tt_int_op(port_cfg_line_extract_addrport("lolol", &a, &unixy, &rest),
              OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 0);
    ...
    tt_int_op(
        port_cfg_line_extract_addrport("foobar:lolol", &a, &unixy, &rest),
        OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 0);
    ...
    tt_int_op(port_cfg_line_extract_addrport("unix:\"lolol\" foo ", &a, &unixy,
                                             &rest),
              OP_EQ, 0);
    tt_int_op(unixy, OP_EQ, 1);
    tt_str_op(a, OP_EQ, "lolol");
...
test_config_parse_port_config__ports__ports_given(void *data)
{
    ...
    ret = port_parse_config(NULL, config_port_invalid, "DNS", 0, "127.0.0.3",
                            0, 0);
    tt_int_op(ret, OP_EQ, -1);
    ...
    ret = port_parse_config(NULL, config_port_valid, "DNS", 0, "127.0.0.3", 0,
                            0);
    tt_int_op(ret, OP_EQ, 0);
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.3", 0,
                            0);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.42",
                            0, 0);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    config_port_valid = mock_config_line("DNSPort", "42 IPv4Traffic "
                                                    "IPv6Traffic");
    ret =
        port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.44", 0,
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.44",
                            0, 0);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.45",
                            0, CL_PORT_IS_UNIXSOCKET);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 0);
    ...
    ret = port_parse_config(slout, config_port_valid, "DNS", 0, "127.0.0.46",
                            0, 0);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(slout), OP_EQ, 1);
    port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
    ...
    ret = port_parse_config(NULL, config_port_invalid, "DNS", 0, "127.0.0.46",
                            0, 0);
...
test_config_get_first_advertised(void *data)
{
    ...
    addr =
        get_first_advertised_addr_by_type_af(CONN_TYPE_OR_LISTENER, AF_INET);
    tt_ptr_op(addr, OP_EQ, NULL);

    port =
        get_first_advertised_port_by_type_af(CONN_TYPE_OR_LISTENER, AF_INET6);
    tt_int_op(port, OP_EQ, 0);
    addr =
        get_first_advertised_addr_by_type_af(CONN_TYPE_OR_LISTENER, AF_INET6);
    tt_ptr_op(addr, OP_EQ, NULL);
    ...
    port =
        get_first_advertised_port_by_type_af(CONN_TYPE_OR_LISTENER, AF_UNSPEC);
    tt_int_op(port, OP_EQ, 0);
    addr =
        get_first_advertised_addr_by_type_af(CONN_TYPE_OR_LISTENER, AF_UNSPEC);
    tt_ptr_op(addr, OP_EQ, NULL);
...
test_config_include_recursion_before_after(void *data)
{
    ...
    int i;
    // Loop backwards so file_contents has the contents of the first file by
    // the end of the loop
    for (i = limit; i > 0; i--) {
        if (i < limit) {
        ...
        if (i > 1) {
            ...
            tt_int_op(write_str_to_file(file_path, file_contents, 0), OP_EQ,
                      0);
            tor_free(file_path);
        }
    }
    ...
    int include_used;
    tt_int_op(config_get_lines_include(file_contents, &result, 0,
                                       &include_used, NULL),
              OP_EQ, 0);
...
test_config_include_flag_both_without(void *data)
{
    ...
    int ret = options_init_from_string(conf_empty, conf_empty,
...
#define MEGABYTE(x) (UINT64_C(x) << 20)
     ...
     * MaxMemInQueues here, even though we should only have had 0.75 * 256 =
     * 192 MB available. */
...
test_config_kvline_parse(void *arg)
{
    ...
    lines =
        kvline_parse("AB=C CDE=\"F G\" \"GHI\" ", KV_QUOTED | KV_OMIT_KEYS);
    tt_assert(lines);
    ...
    lines =
        kvline_parse("A\"B=C CDE=\"F\" \"GHI\" ", KV_QUOTED | KV_OMIT_KEYS);
    tt_assert(!lines);
