...
test_crypto_rng_strongest(void *arg)
{
    ...
    if (how == NULL) {
        ...
        broken = break_strongest_rng_syscall = break_strongest_rng_fallback =
            1;
    }
    ...
    for (i = 0; i < 100; ++i) { /* 2^-100 chances just don't happen. */
        ...
        if (how == NULL) {
            ...
            if (r == -1) {
                ...
                /* This function is allowed to break, but only if it always
                 * breaks. */
                tt_int_op(i, OP_EQ, 0);
                tt_skip();
            } else {
...
test_crypto_rng_fast_whitebox(void *arg)
{
    ...
    /* Make sure that refilling works as expected: skip all but the last 5
     * bytes of this steam. */
