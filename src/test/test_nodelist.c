...
#define N_NODES 5
    ...
    /* Make a bunch of dummy objects that we can play around with.  Only set
    ...
    for (i = 0; i < N_NODES; ++i) {
        ...
        ri[i]->cache_info.signing_key_cert =
    ...
    tt_ptr_op(NULL, OP_EQ,
              node_get_by_ed25519_id(md[0]->ed25519_identity_pkey));
        ...
        for (int log_at_warn = 0; log_at_warn <= 1; ++log_at_warn) {
            ...
            if (log_at_warn) {
                expect_log_msg_containing(
                    "$616273656e746d696e6465646e6573736573");
...
test_nodelist_router_describe(void *arg)
{
    ...
    /* We don't test the no-nickname and no-IP cases, because they're covered
     * by format_node_description(), and we don't expect to see them in Tor
     * code. */
