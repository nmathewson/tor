...
#define N_NODES 5
    ...
    for (i = 0; i < N_NODES; ++i) {
        ...
        crypto_rand((char *)md[i]->ed25519_identity_pkey, sizeof(ed25519_public_key_t));
        crypto_rand(rs[i]->identity_digest, sizeof(rs[i]->identity_digest));
        memcpy(ri[i]->cache_info.identity_digest, rs[i]->identity_digest, DIGEST_LEN);
        ...
        memcpy(&ri[i]->cache_info.signing_key_cert->signing_key, md[i]->ed25519_identity_pkey,
    ...
    memcpy(md[4]->ed25519_identity_pkey, md[3]->ed25519_identity_pkey,
           sizeof(ed25519_public_key_t));
    memcpy(&ri[4]->cache_info.signing_key_cert->signing_key, md[3]->ed25519_identity_pkey,
           sizeof(ed25519_public_key_t));
...
test_nodelist_nodefamily(void *arg)
{
    ...
    /* binary ID digests */
    uint8_t d1[DIGEST_LEN], d2[DIGEST_LEN], d3[DIGEST_LEN], d4[DIGEST_LEN], d5[DIGEST_LEN];
...
test_nodelist_nodefamily_parse_err(void *arg)
{
    ...
    nodefamily_t *nf1 = NULL;
    char *enc = NULL;
    const char *semibogus = "sdakljfdslkfjdsaklfjdkl9sdf " // too long for nickname
...
test_nodelist_format_node_description(void *arg)
{
    ...
    rv = format_node_description(ndesc, mock_digest, mock_nickname, &mock_null_ip, 0);
    tt_ptr_op(rv, OP_EQ, ndesc);
    tt_str_op(ndesc, OP_EQ, "$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA~TestOR7890123456789");

    /* Try some real IP addresses */
    rv = format_node_description(ndesc, mock_digest, NULL, &mock_ipv4, 0);
    tt_ptr_op(rv, OP_EQ, ndesc);
    tt_str_op(ndesc, OP_EQ, "$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA at 111.222.233.244");

    rv = format_node_description(ndesc, mock_digest, mock_nickname, &mock_ipv6, 0);
    tt_ptr_op(rv, OP_EQ, ndesc);
    tt_str_op(ndesc, OP_EQ,
...
test_nodelist_router_describe(void *arg)
{
    ...
    tor_addr_parse(&mock_ri_dual.ipv6_addr, "[1111:2222:3333:4444:5555:6666:7777:8888]");
...
test_nodelist_routerstatus_describe(void *arg)
{
    ...
    memcpy(mock_rs_dual.identity_digest,
           ...
           "\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA\xAA",
           sizeof(mock_rs_dual.identity_digest));
    strlcpy(mock_rs_dual.nickname, "TestOR7890123456789", sizeof(mock_rs_dual.nickname));
    tor_addr_parse(&mock_ipv4, "111.222.233.244");
    mock_rs_dual.addr = tor_addr_to_ipv4h(&mock_ipv4);
    tor_addr_parse(&mock_rs_dual.ipv6_addr, "[1111:2222:3333:4444:5555:6666:7777:8888]");
...
test_nodelist_extend_info_describe(void *arg)
{
    ...
    strlcpy(mock_ei_ipv4.nickname, "TestOR7890123456789", sizeof(mock_ei_ipv4.nickname));
    ...
    tor_addr_parse(&mock_ei_ipv6.addr, "[1111:2222:3333:4444:5555:6666:7777:8888]");
...
#define FORMAT()                                                                           \
    STMT_BEGIN                                                                             \
        tor_free(fmt_orig);                                                                \
        tor_free(fmt);                                                                     \
        fmt_orig = routerstatus_format_entry(&rs_orig, NULL, NULL, NS_CONTROL_PORT, NULL); \
...
struct testcase_t nodelist_tests[] = {NODE(node_get_verbose_nickname_by_id_null_node, TT_FORK),
