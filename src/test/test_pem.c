...
static const char expected[] = "-----BEGIN WOMBAT QUOTE-----\n"
...
test_crypto_pem_encode(void *arg)
{
    ...
    int n2 = pem_encode(buf, sizeof(buf), (const unsigned char *)example_pre, strlen(example_pre),
                        "WOMBAT QUOTE");
    ...
    n2 =
        pem_encode(buf, n, (const unsigned char *)example_pre, strlen(example_pre), "WOMBAT QUOTE");
    tt_int_op(n2, OP_EQ, 0);
    tt_str_op(buf, OP_EQ, expected);

    /* Make sure it fails if the buffer is too short. */
    memset(buf, 0, sizeof(buf));
    n2 = pem_encode(buf, n - 1, (const unsigned char *)example_pre, strlen(example_pre),
                    "WOMBAT QUOTE");
...
test_crypto_pem_decode(void *arg)
{
    ...
    int n = pem_decode(buf, sizeof(buf), expected, strlen(expected), "WOMBAT QUOTE");
    ...
    n = pem_decode(buf, strlen(example_pre), expected, strlen(expected), "WOMBAT QUOTE");
    tt_int_op(n, OP_EQ, strlen(example_pre));
    tt_mem_op(buf, OP_EQ, example_pre, n);
    ...
    memset(buf, 0xff, sizeof(buf));
    n = pem_decode(buf, strlen(example_pre) - 1, expected, strlen(expected), "WOMBAT QUOTE");
    tt_int_op(n, OP_EQ, -1);
...
test_crypto_pem_decode_crlf(void *arg)
{
    ...
    *outp = 0;
    ...
    int n = pem_decode(buf, sizeof(buf), crlf_version, strlen(crlf_version), "WOMBAT QUOTE");
    tt_int_op(n, OP_EQ, strlen(example_pre));
    tt_mem_op(buf, OP_EQ, example_pre, n);
...
struct testcase_t pem_tests[] = {{"encode", test_crypto_pem_encode, 0, NULL, NULL},
