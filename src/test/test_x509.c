...
test_x509_consume_ec_cert(void *arg)
{
    ...
    const char certificate[] = "MIIBEzCBugIJAIdl5svgOZ0OMAoGCCqGSM49BAMCMBIxEDAOBgNVBAMMB1Rlc3Rp\n"
...
test_x509_expiration(void *arg)
{
    ...
    const char certificate[] = "MIICzjCCAbYCCQDxIONWIQ9OGDANBgkqhkiG9w0BAQsFADApMQswCQYDVQQGEwJV\n"
    ...
    tt_assert(tor_tls_cert_is_valid(LOG_ERR, cert, cert, now - TOR_X509_FUTURE_SLOP, 0));
    tt_assert(tor_tls_cert_is_valid(LOG_ERR, cert, cert,
                                    now + 365 * 86400 + TOR_X509_PAST_SLOP - 3600, 0));

    tt_assert(!tor_tls_cert_is_valid(LOG_INFO, cert, cert, now - TOR_X509_FUTURE_SLOP - 3600, 0));
    tt_assert(
        !tor_tls_cert_is_valid(LOG_INFO, cert, cert, now + 365 * 86400 + TOR_X509_FUTURE_SLOP, 0));
...
struct testcase_t x509_tests[] = {TEST(cert_new_failing_digest), TEST(consume_ec_cert),
