...
#define CHECK_COUNTRY(country, val)                                                         \
    do {                                                                                    \
        ...
        tt_str_op(country, OP_EQ, geoip_get_country_name(geoip_get_country_by_ipv4(val)));  \
        /* test ipv6 country lookup */                                                      \
        SET_TEST_IPV6(val);                                                                 \
        tt_str_op(country, OP_EQ, geoip_get_country_name(geoip_get_country_by_ipv6(&in6))); \
    } while (0)
...
test_geoip(void *arg)
{
    ...
    char *s = NULL, *v = NULL;
    const char *bridge_stats_1 = "bridge-stats-end 2010-08-12 13:27:30 (86400 s)\n"
    ...
    geoip_entry_stats_init(now);
    SET_TEST_ADDRESS(100);
    geoip_note_client_seen(GEOIP_CLIENT_CONNECT, &addr, NULL, now - (12 * 60 * 60));
    ...
    geoip_entry_stats_init(now);
    SET_TEST_ADDRESS(100);
    geoip_note_client_seen(GEOIP_CLIENT_CONNECT, &addr, NULL, now - (3 * 60 * 60));
...
test_geoip_load_file(void *arg)
{
    ...
    tt_int_op(-1, OP_EQ, geoip_load_file(AF_INET, "/you/did/not/put/a/file/here/I/hope", LOG_INFO));
    ...
    tt_str_op("0000000000000000000000000000000000000000", OP_EQ, geoip_db_digest(AF_INET));
    ...
    tt_str_op("0000000000000000000000000000000000000000", OP_NE, geoip_db_digest(AF_INET));
...
test_geoip6_load_file(void *arg)
{
    ...
    /* A nonexistant filename should fail. */
    tt_int_op(-1, OP_EQ,
              geoip_load_file(AF_INET6, "/you/did/not/put/a/file/here/I/hope", LOG_INFO));
    ...
    const char CONTENT[] = "2001:4830:6010::,2001:4830:601f:ffff:ffff:ffff:ffff:ffff,GB\n"
    ...
    /* The digest should be set.... */
    tt_str_op("0000000000000000000000000000000000000000", OP_NE, geoip_db_digest(AF_INET6));
    ...
    tt_int_op(-1, OP_EQ, geoip_get_country_by_ipv6(&iaddr6));
    tt_str_op("0000000000000000000000000000000000000000", OP_EQ, geoip_db_digest(AF_INET6));
...
struct testcase_t geoip_tests[] = {{"geoip", test_geoip, TT_FORK, NULL, NULL},
