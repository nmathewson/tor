                                      ...
                                      enum networkstatus_type_t ns_type)
{
    ...
    networkstatus_t *result = networkstatus_parse_vote_from_string(tmp, len, eos_out, ns_type);
    if (eos_out && *eos_out) {
...
test_dir_nicknames(void *arg)
{
    ...
    /* valid */
    tt_assert(is_legal_nickname_or_hexdigest("$AAAAAAAA01234AAAAAAAAAAAAAAAAAAAAAAAAAAA"));
    tt_assert(is_legal_nickname_or_hexdigest("$AAAAAAAA01234AAAAAAAAAAAAAAAAAAAAAAAAAAA=fred"));
    tt_assert(is_legal_nickname_or_hexdigest("$AAAAAAAA01234AAAAAAAAAAAAAAAAAAAAAAAAAAA~fred"));
    /* too short */
    tt_assert(!is_legal_nickname_or_hexdigest("$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"));
    /* illegal char */
    tt_assert(!is_legal_nickname_or_hexdigest("$AAAAAAzAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"));
    /* hex part too long */
    tt_assert(!is_legal_nickname_or_hexdigest("$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"));
    tt_assert(!is_legal_nickname_or_hexdigest("$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=fred"));
    /* Bad nickname */
    tt_assert(!is_legal_nickname_or_hexdigest("$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="));
    tt_assert(!is_legal_nickname_or_hexdigest("$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA~"));
    tt_assert(
        !is_legal_nickname_or_hexdigest("$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA~hyphen-"));
    tt_assert(!is_legal_nickname_or_hexdigest("$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA~"
 ...
 */
static routerinfo_t *
basic_routerinfo_new(const char *nickname, uint32_t ipv4_addr, uint16_t or_port, uint16_t dir_port,
                     uint32_t bandwidthrate, uint32_t bandwidthburst, uint32_t bandwidthcapacity,
                     time_t published_on, crypto_pk_t **rsa_onion_keypair_out)
{
    char platform[256];
...
get_new_router_line(const routerinfo_t *r1)
{
    ...
    tor_asprintf(&line, "router %s %s %d 0 %d\n", r1->nickname, fmt_addr32(r1->addr), r1->or_port,
                 r1->dir_port);
    tor_assert(line);
...
get_new_bandwidth_line(const routerinfo_t *r1)
{
    ...
    tor_asprintf(&line, "bandwidth %u %u %u\n", r1->bandwidthrate, r1->bandwidthburst,
...
get_new_rsa_key_block(const char *key_name, crypto_pk_t *pk1)
{
    ...
    tor_assert(!crypto_pk_write_public_key_to_string(pk1, &pk1_str, &pk1_str_len));
    tor_assert(pk1_str);
...
get_new_onion_key_block(const routerinfo_t *r1)
{
    char *block = NULL;
    tor_assert(r1);
    crypto_pk_t *pk_tmp = router_get_rsa_onion_pkey(r1->onion_pkey, r1->onion_pkey_len);
...
get_new_ntor_onion_key_line(const curve25519_public_key_t *ntor_onion_pubkey)
{
    ...
    rv = base64_encode(cert_buf, sizeof(cert_buf), (const char *)ntor_onion_pubkey->public_key, 32,
 ...
 */
static void
setup_mocks_for_fresh_descriptor(const routerinfo_t *r1, crypto_pk_t *rsa_onion_keypair)
{
    ...
    if (rsa_onion_keypair) {
        ...
        mocked_onionkey = router_get_rsa_onion_pkey(r1->onion_pkey, r1->onion_pkey_len);
    }
...
#define CHECK_ROUTERINFO_CONSISTENCY(r1, rp1)                                           \
    STMT_BEGIN                                                                          \
        ...
        tt_int_op(crypto_pk_cmp_keys(rp1->identity_pkey, r1->identity_pkey), OP_EQ, 0); \
...
test_dir_formats_rsa(void *arg)
{
    ...
    r1 = basic_routerinfo_new("Magri", 0xc0a80001u /* 192.168.0.1 */, 9000, 9003, 1000, 5000, 10000,
                              0, NULL);
    ...
    tt_assert(rv == 0);
    msg = "";
    rv = routerinfo_incompatible_with_extrainfo(r1->identity_pkey, e1, &r1->cache_info, &msg);
...
test_dir_formats_rsa_ed25519(void *arg)
{
     ...
     * IPv6 */
    r2 = basic_routerinfo_new("Fred", 0x0a030201u /* 10.3.2.1 */, 9005, 0, 3000, 3000, 3000, 5,
                              &r2_onion_pkey);
    ...
    ed25519_secret_key_from_seed(&kp1.seckey, (const uint8_t *)"YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY");
    ed25519_public_key_generate(&kp1.pubkey, &kp1.seckey);
    ed25519_secret_key_from_seed(&kp2.seckey, (const uint8_t *)"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
    ed25519_public_key_generate(&kp2.pubkey, &kp2.seckey);
    r2->cache_info.signing_key_cert = tor_cert_create(&kp1, CERT_TYPE_ID_SIGNING, &kp2.pubkey, now,
                                                      86400, CERT_FLAG_INCLUDE_SIGNING_KEY);
    ...
    buf =
        router_dump_router_to_string(r2, r2->identity_pkey, r2_onion_pkey, &r2_onion_keypair, &kp2);
    tt_assert(buf);
    ...
    base64_encode(cert_buf, sizeof(cert_buf),
                  ...
                  r2->cache_info.signing_key_cert->encoded_len, BASE64_ENCODE_MULTILINE);
    ...
    int rsa_cc_len;
    rsa_cc =
        make_tap_onion_key_crosscert(r2_onion_pkey, &kp1.pubkey, r2->identity_pkey, &rsa_cc_len);
    tt_assert(rsa_cc);
    base64_encode(cert_buf, sizeof(cert_buf), (char *)rsa_cc, rsa_cc_len, BASE64_ENCODE_MULTILINE);
    ...
    int ntor_cc_sign;
    {
        ...
        ntor_cc = make_ntor_onion_key_crosscert(&r2_onion_keypair, &kp1.pubkey,
                                                r2->cache_info.published_on,
                                                get_onion_key_lifetime(), &ntor_cc_sign);
        tt_assert(ntor_cc);
        base64_encode(cert_buf, sizeof(cert_buf), (char *)ntor_cc->encoded, ntor_cc->encoded_len,
                      BASE64_ENCODE_MULTILINE);
    ...
    tt_mem_op(rp2->onion_curve25519_pkey->public_key, OP_EQ, r2->onion_curve25519_pkey->public_key,
    ...
    MOCK(router_build_fresh_unsigned_routerinfo, mock_router_build_fresh_unsigned_routerinfo);
...
#define CHECK_FAIL(s, againval)                                             \
    ...
    routerinfo_free(ri);
    ri = router_parse_entry_from_string(EX_RI_MINIMAL, NULL, 0, 0, "@purpose bridge\n", NULL);
...
#define ADD(name)                                                                              \
    do {                                                                                       \
        ...
        tt_assert(!crypto_pk_read_public_key_from_string(pk, name##_KEY, strlen(name##_KEY))); \
...
test_dir_parse_router_list(void *arg)
{
    ...
    cp = list;
    tt_int_op(0, OP_EQ,
              router_parse_list_from_string(&cp, NULL, dest, SAVED_NOWHERE, 0, 0, NULL, invalid));
    tt_int_op(2, OP_EQ, smartlist_len(dest));
    tt_ptr_op(cp, OP_EQ, list + strlen(list));

    routerinfo_t *r = smartlist_get(dest, 0);
    tt_mem_op(r->cache_info.signed_descriptor_body, OP_EQ, EX_RI_MINIMAL, strlen(EX_RI_MINIMAL));
    r = smartlist_get(dest, 1);
    tt_mem_op(r->cache_info.signed_descriptor_body, OP_EQ, EX_RI_MAXIMAL, strlen(EX_RI_MAXIMAL));

    tt_int_op(2, OP_EQ, smartlist_len(invalid));
    test_memeq_hex(smartlist_get(invalid, 0), "ab9eeaa95e7d45740185b4e519c76ead756277a9");
    ...
    cp = list;
    tt_int_op(0, OP_EQ,
              router_parse_list_from_string(&cp, NULL, dest, SAVED_NOWHERE, 1, 0, NULL, invalid));
    tt_int_op(2, OP_EQ, smartlist_len(dest));
    extrainfo_t *e = smartlist_get(dest, 0);
    tt_mem_op(e->cache_info.signed_descriptor_body, OP_EQ, EX_EI_MAXIMAL, strlen(EX_EI_MAXIMAL));
    e = smartlist_get(dest, 1);
    tt_mem_op(e->cache_info.signed_descriptor_body, OP_EQ, EX_EI_MINIMAL, strlen(EX_EI_MINIMAL));

    tt_int_op(2, OP_EQ, smartlist_len(invalid));
    test_memeq_hex(smartlist_get(invalid, 0), "d5df4aa62ee9ffc9543d41150c9864908e0390af");
    test_memeq_hex(smartlist_get(invalid, 1), "f61efd2a7f4531f3687a9043e0de90a862ec64ba");
    ...
    if (map) {
        digestmap_free_((digestmap_t *)map, routerinfo_free_wrapper_);
        router_get_routerlist()->identity_map = (struct digest_ri_map_t *)digestmap_new();
    }
...
#define ADD(str)                                                            \
    ...
    list = smartlist_join_strings(chunks, "", 0, NULL);
    tt_int_op(1, OP_EQ,
              router_load_routers_from_string(list, NULL, SAVED_IN_JOURNAL, wanted, 1, NULL));
    ...
    tt_str_op(smartlist_get(wanted, 0), OP_EQ, "E0A3753CEFD54128EAB239F294954121DB23D2EF");
...
#define ADD(str)                                                               \
    ...
    /* Wanted still contains "BAD_SIG2" */
    tt_int_op(smartlist_len(wanted), OP_EQ, 1);
    tt_str_op(smartlist_get(wanted, 0), OP_EQ, "16D387D3A58F7DB3CF46638F8D0B90C45C7D769C");
...
#define tt_versionstatus_op(vs1, op, vs2)                                                          \
    tt_assert_test_type(vs1, vs2, #vs1 " " #op " " #vs2, version_status_t, (val1_ op val2_), "%d", \
                        TT_EXIT_TEST_FUNCTION)
#define test_v_i_o(val, ver, lst) tt_versionstatus_op(val, OP_EQ, tor_version_is_obsolete(ver, lst))
    ...
    test_v_i_o(VS_NEW_IN_SERIES, "0.1.0.3", "Tor 0.1.0.2,Tor 0.0.9.5,Tor 0.1.1.0");
    ...
    test_v_i_o(VS_UNRECOMMENDED, "0.1.0.1", "Tor 0.1.0.2,Tor 0.0.9.5,Tor 0.1.1.0");
    /* On list, not newer than any on same series. */
    test_v_i_o(VS_UNRECOMMENDED, "0.1.0.1", "Tor 0.1.0.2,Tor 0.0.9.5,Tor 0.1.1.0");
    tt_int_op(0, OP_EQ, tor_version_as_new_as("Tor 0.0.5", "0.0.9pre1-cvs"));
    tt_int_op(1, OP_EQ,
              tor_version_as_new_as("Tor 0.0.8 on Darwin 64-121-192-100.c3-0."
    ...
    tt_int_op(1, OP_EQ, tor_version_as_new_as("Tor 0.2.1.0-dev (r100)", "Tor 0.2.1.0-dev (r99)"));
    tt_int_op(1, OP_EQ,
              tor_version_as_new_as("Tor 0.2.1.0-dev (r100) on Banana Jr",
                                    "Tor 0.2.1.0-dev (r99) on Hal 9000"));
    tt_int_op(1, OP_EQ,
              tor_version_as_new_as("Tor 0.2.1.0-dev (r100)", "Tor 0.2.1.0-dev on Colossus"));
    tt_int_op(0, OP_EQ, tor_version_as_new_as("Tor 0.2.1.0-dev (r99)", "Tor 0.2.1.0-dev (r100)"));
    tt_int_op(
        0, OP_EQ,
        tor_version_as_new_as("Tor 0.2.1.0-dev (r99) on MCP", "Tor 0.2.1.0-dev (r100) on AM"));
    tt_int_op(0, OP_EQ, tor_version_as_new_as("Tor 0.2.1.0-dev", "Tor 0.2.1.0-dev (r99)"));
    tt_int_op(1, OP_EQ, tor_version_as_new_as("Tor 0.2.1.1", "Tor 0.2.1.0-dev (r99)"));
    /* And git revisions */
    tt_int_op(1, OP_EQ,
    ...
    tt_int_op(1, OP_EQ, tor_version_as_new_as("Tor 0.2.9.9 (git-56788a2489127072)", "Tor 0.2.9.9"));
     ...
     * anyway) */
    tt_int_op(1, OP_EQ,
              tor_version_as_new_as("Tor 0.2.9.9 (git-56788a2489127072d513cf4baf35a8ff475f3c7b)",
                                    "Tor 0.2.9.9 (git-56788a2489127072)"));
    tt_int_op(1, OP_EQ, tor_version_as_new_as("Tor 0.2.9.9 (git-0102)", "Tor 0.2.9.9 (git-03)"));
    tt_int_op(1, OP_EQ, tor_version_as_new_as("Tor 0.2.9.9 (git-0102)", "Tor 0.2.9.9 (git-00)"));
    tt_int_op(1, OP_EQ, tor_version_as_new_as("Tor 0.2.9.9 (git-01)", "Tor 0.2.9.9 (git-00)"));
    tt_int_op(0, OP_EQ, tor_version_as_new_as("Tor 0.2.9.9 (git-00)", "Tor 0.2.9.9 (git-01)"));
    ...
    tt_int_op(0, OP_EQ, tor_version_as_new_as("Tor 0.0.0.0", "Tor 2147483647.0.0.0"));
    tt_int_op(1, OP_EQ, tor_version_as_new_as("Tor 2147483647.0.0.0", "Tor 0.0.0.0"));
    ...
    tt_int_op(0, OP_EQ, tor_version_as_new_as("Tor 0.0.0.0", "Tor 0.-2147483648.0.0"));
    expect_single_log_msg_containing("unparseable");
    mock_clean_saved_logs();
    tt_int_op(0, OP_EQ, tor_version_as_new_as("Tor 0.2147483647.0.0", "Tor 0.-1.0.0"));
    expect_single_log_msg_containing("unparseable");
    mock_clean_saved_logs();
    tt_int_op(0, OP_EQ, tor_version_as_new_as("Tor 0.2147483647.0.0", "Tor 0.-2147483648.0.0"));
    expect_single_log_msg_containing("unparseable");
    mock_clean_saved_logs();
    tt_int_op(1, OP_EQ, tor_version_as_new_as("Tor 4294967295.0.0.0", "Tor 0.0.0.0"));
    expect_no_log_entry();
    tt_int_op(0, OP_EQ, tor_version_as_new_as("Tor 0.4294967295.0.0", "Tor 0.-4294967295.0.0"));
    expect_single_log_msg_containing("unparseable");
    mock_clean_saved_logs();
    ...
    tt_int_op(0, OP_EQ, tor_version_parse("0.5.6.7 (git-0010203040506070)", &ver1));
    /* length limit is 40 hex digits */
    tt_int_op(0, OP_EQ,
              tor_version_parse("0.5.6.7 (git-000102030405060708090a0b0c0d0e0f10111213)", &ver1));
    tt_int_op(-1, OP_EQ,
              tor_version_parse("0.5.6.7 (git-000102030405060708090a0b0c0d0e0f1011121314)", &ver1));
done:
    teardown_capture_of_logs();
}
...
#define HEX256_1 "f3f3f3f3fbbbbf3f3f3f3fbbbf3f3f3f3fbbbbf3f3f3f3fbbbf3f3f3f3fbbbbf"
#define HEX256_2 "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccCCc"
#define HEX256_3 "0123456789ABCdef0123456789ABCdef0123456789ABCdef0123456789ABCdef"
...
#define B64_256_2 "zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMw"
    ...
    dir_split_resource_into_fingerprints(HEX1 "+bogus+" HEX2 "+" HEX256_1, sl, NULL, DSR_HEX);
    tt_int_op(smartlist_len(sl), OP_EQ, 2);
    test_mem_op_hex(smartlist_get(sl, 0), OP_EQ, HEX1);
    test_mem_op_hex(smartlist_get(sl, 1), OP_EQ, HEX2);
    SMARTLIST_FOREACH(sl, char *, cp, tor_free(cp));
    ...
    /* Decode long hex */
    dir_split_resource_into_fingerprints(HEX256_1 "+" HEX256_2 "+" HEX2 "+" HEX256_3, sl, NULL,
                                         DSR_HEX | DSR_DIGEST256);
    tt_int_op(smartlist_len(sl), OP_EQ, 3);
    ...
    /* Decode hex and sort. */
    dir_split_resource_into_fingerprints(HEX1 "+" HEX2 "+" HEX3 "+" HEX2, sl, NULL,
    ...
    dir_split_resource_into_fingerprints(HEX256_1 "+" HEX256_2 "+" HEX256_3 "+" HEX256_1, sl, NULL,
    ...
    dir_split_resource_into_fingerprints(B64_256_1, sl, NULL, DSR_BASE64 | DSR_DIGEST256);
...
test_dir_measured_bw_kb(void *arg)
{
    ...
    const char *lines_fail[] = {
        ...
        "", "\n", " \n ", " \n\n",
        /* Test assorted noise */
        " node_id= ", "node_id==$557365204145532d32353620696e73746561642e bw==1024\n",
    ...
    for (i = 0; strcmp(lines_pass[i], "end"); i++) {
        ...
        tt_assert(strcmp(mbwl.node_hex, "557365204145532d32353620696e73746561642e") == 0);
    }
...
test_dir_measured_bw_kb_line_is_after_headers(void *arg)
{
    (void)arg;
    measured_bw_line_t mbwl;
    const char *line_pass = "node_id=$557365204145532d32353620696e73746561642e bw=1024\n";
    int i;
    const char *lines_fail[] = {"node_id=$557365204145532d32353620696e73746561642e \n", "bw=1024\n",
                                "rtt=300\n", "end"};
...
test_dir_dirserv_read_measured_bandwidths(void *arg)
{
    ...
    const char *bw_file_headers_str_v110_no_timestamp = "version=1.1.0 software=sbws "
                                                        "software_version=0.1.0 "
                                                        "earliest_bandwidth=2018-05-08T16:13:26 "
                                                        "file_created=2018-04-16T21:49:18 "
                                                        "generator_started=2018-05-08T16:13:25 "
                                                        "latest_bandwidth=2018-04-16T20:49:18";
    const char *relay_lines_v100 = "node_id=$557365204145532d32353620696e73746561642e bw=1024 "
    ...
    for (i = 0; i < MAX_BW_FILE_HEADER_COUNT_IN_VOTE - 8 - 1; i++) {
        strlcat(bw_file_headers_str_long, "foo=bar ", sizeof(bw_file_headers_str_long));
    }
    strlcat(bw_file_headers_str_long, "foo=bar", sizeof(bw_file_headers_str_long));
    ...
    tt_int_op(-1, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, bw_file_headers, NULL));
    ...
    bw_file_headers = smartlist_new();
    tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, bw_file_headers, NULL));
    ...
    write_str_to_file(fname, content, 0);
    tor_free(content);
    tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, NULL, NULL));
    ...
    bw_file_headers = smartlist_new();
    tor_asprintf(&content, "%s%s%s", header_lines_v100, header_lines_v110, relay_lines_v100);
    write_str_to_file(fname, content, 0);
    tor_free(content);
    tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, bw_file_headers, NULL));
    ...
    bw_file_headers = smartlist_new();
    tor_asprintf(&content, "%s%s%s", header_lines_v100, relay_lines_v100, header_lines_v110);
    write_str_to_file(fname, content, 0);
    tor_free(content);
    tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, bw_file_headers, NULL));
    ...
    bw_file_headers = smartlist_new();
    tor_asprintf(&content, "%s%s%s%s", header_lines_v100, relay_lines_v100, header_lines_v110,
                 relay_lines_v100);
    write_str_to_file(fname, content, 0);
    tor_free(content);
    tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, bw_file_headers, NULL));
    ...
    bw_file_headers = smartlist_new();
    tor_asprintf(&content, "%s%s", header_lines_v110_no_terminator, relay_lines_v110);
    write_str_to_file(fname, content, 0);
    tor_free(content);
    tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, bw_file_headers, NULL));
     ...
     * the bad relay lines. */
    bw_file_headers = smartlist_new();
    tor_asprintf(&content, "%s%s%s", header_lines_v110_no_terminator, relay_lines_bad,
                 relay_lines_v110);
    write_str_to_file(fname, content, 0);
    tor_free(content);
    tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, bw_file_headers, NULL));
    ...
    bw_file_headers = smartlist_new();
    tor_asprintf(&content, "%s%s%s", header_lines_v110_no_terminator, header_lines_long,
                 relay_lines_v110);
    write_str_to_file(fname, content, 0);
    tor_free(content);
    tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, bw_file_headers, NULL));
...
test_dir_param_voting(void *arg)
{
    ...
    smartlist_split_string(vote1.net_params, "ab=90 abcd=20 cw=50 x-yz=-99", NULL, 0, 0);
    smartlist_split_string(vote2.net_params, "ab=27 cw=5 x-yz=88", NULL, 0, 0);
    smartlist_split_string(vote3.net_params, "abcd=20 c=60 cw=500 x-yz=-9 zzzzz=101", NULL, 0, 0);
    smartlist_split_string(vote4.net_params, "ab=900 abcd=200 c=1 cw=51 x-yz=100", NULL, 0, 0);
    ...
    tt_int_op(100, OP_EQ, networkstatus_get_overridable_param(&vote4, -1, "x-yz", 50, 0, 300));
    tt_int_op(30, OP_EQ, networkstatus_get_overridable_param(&vote4, 30, "x-yz", 50, 0, 300));
    tt_int_op(0, OP_EQ, networkstatus_get_overridable_param(&vote4, -101, "foobar", 0, -100, 8));
    tt_int_op(-99, OP_EQ, networkstatus_get_overridable_param(&vote4, -99, "foobar", 0, -100, 8));
...
test_dir_param_voting_lookup(void *arg)
{
    ...
    tt_int_op(1000, OP_EQ, dirvote_get_intermediate_param_value(lst, "ab", 1000));
    ...
    tor_capture_bugs_(1);
    tt_int_op(-100, OP_EQ, dirvote_get_intermediate_param_value(lst, "moomin", -100));
    tt_int_op(smartlist_len(tor_get_captured_bug_log_()), OP_EQ, 1);
    tt_str_op(smartlist_get(tor_get_captured_bug_log_(), 0), OP_EQ, "n_found == 0");
    tor_end_capture_bugs_();
    /* There is no 'fred=', so that is treated as not existing. */
    tt_int_op(-100, OP_EQ, dirvote_get_intermediate_param_value(lst, "fred", -100));
    /* jack is truncated */
    tor_capture_bugs_(1);
    tt_int_op(-100, OP_EQ, dirvote_get_intermediate_param_value(lst, "jack", -100));
    tt_int_op(smartlist_len(tor_get_captured_bug_log_()), OP_EQ, 1);
    tt_str_op(smartlist_get(tor_get_captured_bug_log_(), 0), OP_EQ, "!(!ok)");
    tor_end_capture_bugs_();
    /* electricity and opa aren't integers. */
    tor_capture_bugs_(1);
    tt_int_op(-100, OP_EQ, dirvote_get_intermediate_param_value(lst, "electricity", -100));
    tt_int_op(smartlist_len(tor_get_captured_bug_log_()), OP_EQ, 1);
    tt_str_op(smartlist_get(tor_get_captured_bug_log_(), 0), OP_EQ, "!(!ok)");
    tor_end_capture_bugs_();

    tor_capture_bugs_(1);
    tt_int_op(-100, OP_EQ, dirvote_get_intermediate_param_value(lst, "opa", -100));
    tt_int_op(smartlist_len(tor_get_captured_bug_log_()), OP_EQ, 1);
    tt_str_op(smartlist_get(tor_get_captured_bug_log_(), 0), OP_EQ, "!(!ok)");
    tor_end_capture_bugs_();
...
vote_tweaks_for_v3ns(networkstatus_t *v, int voter, time_t now)
{
    ...
    if (voter == 1) {
        ...
        if (voter == 3) {
            ...
            tt_assert(router_add_to_routerlist(dir_common_generate_ri_from_rs(vrs), &msg, 0, 0) >=
                      0);
        }
    }
...
test_routerstatus_for_v3ns(routerstatus_t *rs, time_t now)
{
    ...
    if (tor_memeq(rs->identity_digest,
                  ...
                  DIGEST_LEN)) {
        tt_mem_op(rs->identity_digest, OP_EQ,
                  "\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3\x3", DIGEST_LEN);
        tt_mem_op(rs->descriptor_digest, OP_EQ, "NNNNNNNNNNNNNNNNNNNN", DIGEST_LEN);
        ...
        tt_mem_op(rs->identity_digest, OP_EQ,
                  "\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5\x5", DIGEST_LEN);
        tt_str_op(rs->nickname, OP_EQ, "router1");
        tt_mem_op(rs->descriptor_digest, OP_EQ, "MMMMMMMMMMMMMMMMMMMM", DIGEST_LEN);
        tt_int_op(rs->published_on, OP_EQ, now - 1000);
        tt_int_op(rs->addr, OP_EQ, 0x99009901);
...
test_dir_networkstatus_compute_bw_weights_v10(void *arg)
{
    ...
    tt_str_op(smartlist_get(chunks, 0), OP_EQ,
                     ...
                     void (*rs_test)(routerstatus_t *rs, time_t now))
{
    ...
    networkstatus_t *vote = NULL, *v1 = NULL, *v2 = NULL, *v3 = NULL, *con = NULL, *con_md = NULL;
    vote_routerstatus_t *vrs;
    routerstatus_t *rs;
    int idx, n_rs, n_vrs;
    ...
    cert1 = mock_cert =
        authority_cert_parse_from_string(AUTHORITY_CERT_1, strlen(AUTHORITY_CERT_1), NULL);
    tt_assert(cert1);
    cert2 = authority_cert_parse_from_string(AUTHORITY_CERT_2, strlen(AUTHORITY_CERT_2), NULL);
    tt_assert(cert2);
    cert3 = authority_cert_parse_from_string(AUTHORITY_CERT_3, strlen(AUTHORITY_CERT_3), NULL);
    tt_assert(cert3);
    ...
    tt_assert(!crypto_pk_read_private_key_from_string(sign_skey_1, AUTHORITY_SIGNKEY_1, -1));
    tt_assert(!crypto_pk_read_private_key_from_string(sign_skey_2, AUTHORITY_SIGNKEY_2, -1));
    tt_assert(!crypto_pk_read_private_key_from_string(sign_skey_3, AUTHORITY_SIGNKEY_3, -1));

    tt_assert(!crypto_pk_cmp_keys(sign_skey_1, cert1->signing_key));
    tt_assert(!crypto_pk_cmp_keys(sign_skey_2, cert2->signing_key));

    tt_assert(
        !dir_common_construct_vote_1(&vote, cert1, sign_skey_1, vrs_gen, &v1, &n_vrs, now, 1));
    tt_assert(v1);
    ...
    tt_assert(
        !dir_common_construct_vote_2(&vote, cert2, sign_skey_2, vrs_gen, &v2, &n_vrs, now, 1));
    tt_assert(v2);
    ...
    tt_assert(
        !dir_common_construct_vote_3(&vote, cert3, sign_skey_3, vrs_gen, &v3, &n_vrs, now, 1));
    tt_assert(v3);
    ...
    consensus_text =
        networkstatus_compute_consensus(votes, 3, cert3->identity_key, sign_skey_3,
                                        "AAAAAAAAAAAAAAAAAAAA", sign_skey_leg1, FLAV_NS);
    tt_assert(consensus_text);
    con = networkstatus_parse_vote_from_string_(consensus_text, NULL, NS_TYPE_CONSENSUS);
    tt_assert(con);
    // log_notice(LD_GENERAL, "<<%s>>\n<<%s>>\n<<%s>>\n",
    //           v1_text, v2_text, v3_text);
    consensus_text_md =
        networkstatus_compute_consensus(votes, 3, cert3->identity_key, sign_skey_3,
                                        "AAAAAAAAAAAAAAAAAAAA", sign_skey_leg1, FLAV_MICRODESC);
    tt_assert(consensus_text_md);
    con_md = networkstatus_parse_vote_from_string_(consensus_text_md, NULL, NS_TYPE_CONSENSUS);
    tt_assert(con_md);
    ...
    tt_assert(fast_memcmp(cert2->cache_info.identity_digest, cert1->cache_info.identity_digest,
                          DIGEST_LEN) < 0);
    tt_assert(fast_memcmp(cert1->cache_info.identity_digest, cert3->cache_info.identity_digest,
                          DIGEST_LEN) < 0);
...

    {
        ...
        consensus_text2 = networkstatus_compute_consensus(votes, 3, cert2->identity_key,
                                                          sign_skey_2, NULL, NULL, FLAV_NS);
        consensus_text_md2 = networkstatus_compute_consensus(
            votes, 3, cert2->identity_key, sign_skey_2, NULL, NULL, FLAV_MICRODESC);
        smartlist_shuffle(votes);
        consensus_text3 = networkstatus_compute_consensus(votes, 3, cert1->identity_key,
                                                          sign_skey_1, NULL, NULL, FLAV_NS);
        consensus_text_md3 = networkstatus_compute_consensus(
            votes, 3, cert1->identity_key, sign_skey_1, NULL, NULL, FLAV_MICRODESC);
        tt_assert(consensus_text2);
        tt_assert(consensus_text3);
        tt_assert(consensus_text_md2);
        tt_assert(consensus_text_md3);
        con2 = networkstatus_parse_vote_from_string_(consensus_text2, NULL, NS_TYPE_CONSENSUS);
        con3 = networkstatus_parse_vote_from_string_(consensus_text3, NULL, NS_TYPE_CONSENSUS);
        con_md2 =
            networkstatus_parse_vote_from_string_(consensus_text_md2, NULL, NS_TYPE_CONSENSUS);
        con_md3 =
            networkstatus_parse_vote_from_string_(consensus_text_md3, NULL, NS_TYPE_CONSENSUS);
        tt_assert(con2);
        tt_assert(con3);
        tt_assert(con_md2);
        tt_assert(con_md3);
        ...
        tt_int_op(dsig1->valid_until, OP_EQ, con3->valid_until);
        {
            ...
            tt_assert(dsig_digests);
            tt_mem_op(dsig_digests->d[DIGEST_SHA256], OP_EQ, con_md3->digests.d[DIGEST_SHA256],
                      DIGEST256_LEN);
        }
        {
            ...
            tt_mem_op(sig->identity_digest, OP_EQ, cert1->cache_info.identity_digest, DIGEST_LEN);
        ...
        tt_int_op(1, OP_EQ,
                  networkstatus_add_detached_signatures(con2, dsig1, "test", LOG_INFO, &msg));
        tor_free(detached_text2);
        tt_int_op(1, OP_EQ,
                  networkstatus_add_detached_signatures(con_md2, dsig1, "test", LOG_INFO, &msg));
        tor_free(detached_text2);
        detached_text2 = get_detached_sigs(con2, con_md2);
        // printf("\n<%s>\n", detached_text2);
        ...
        */
        tt_int_op(2, OP_EQ, smartlist_len((smartlist_t *)strmap_get(dsig2->signatures, "ns")));
        tt_int_op(2, OP_EQ,
                  smartlist_len((smartlist_t *)strmap_get(dsig2->signatures, "microdesc")));
        ...
        /* Add to con. */
        tt_int_op(2, OP_EQ,
                  networkstatus_add_detached_signatures(con, dsig2, "test", LOG_INFO, &msg));
        /* Check signatures */
        voter = smartlist_get(con->voters, 1);
        sig = smartlist_get(voter->sigs, 0);
        tt_assert(sig);
...
{
    (void)arg;
    test_a_networkstatus(dir_common_gen_routerstatus_for_v3ns, vote_tweaks_for_v3ns,
                         test_vrs_for_v3ns, test_consensus_for_v3ns, test_routerstatus_for_v3ns);
}
...
gen_routerstatus_for_umbw(int idx, time_t now)
{
    ...
    routerstatus_t *rs;
    tor_addr_t addr_ipv6;
    uint32_t max_unmeasured_bw_kb =
        (alternate_clip_bw > 0) ? alternate_clip_bw : DEFAULT_MAX_UNMEASURED_BW_KB;
    ...
    case 1:
        ...
        rs->ipv6_orport = 4711;
        rs->is_exit = rs->is_stable = rs->is_fast = rs->is_flagged_running = rs->is_valid =
    ...
    case 2:
        ...
        rs->or_port = 400;
        rs->dir_port = 9999;
        rs->is_authority = rs->is_exit = rs->is_stable = rs->is_fast = rs->is_flagged_running =
...
test_dir_clip_unmeasured_bw_kb(void *arg)
{
    ...
    (void)arg;
    alternate_clip_bw = 0;
    test_a_networkstatus(gen_routerstatus_for_umbw, vote_tweaks_for_umbw, test_vrs_for_umbw,
                         test_consensus_for_umbw, test_routerstatus_for_umbw);
}
...
test_dir_bwauth_bw_file_digest256(void *arg)
{
    ...
    tt_int_op(-1, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, NULL, digest));
    ...
    tt_int_op(0, OP_EQ, dirserv_read_measured_bandwidths(fname, NULL, NULL, digest));
    tt_mem_op(digest, OP_EQ, digest_expected, DIGEST256_LEN);
...
static void
reset_routerstatus(routerstatus_t *rs, const char *hex_identity_digest, uint32_t ipv4_addr)
{
    memset(rs, 0, sizeof(routerstatus_t));
    base16_decode(rs->identity_digest, sizeof(rs->identity_digest), hex_identity_digest,
                  HEX_DIGEST_LEN);
...
test_dir_dirserv_set_routerstatus_testing(void *arg)
{
    ...
    /* Init options */
    dirauth_options_t *dirauth_options = tor_malloc_zero(sizeof(dirauth_options_t));
    ...
    tt_int_op(routerset_contains_routerstatus(routerset_none, rs_a, 0), OP_EQ, 0);
    tt_int_op(routerset_contains_routerstatus(routerset_none, rs_b, 0), OP_EQ, 0);
...
test_dir_purpose_needs_anonymity_returns_true_for_bridges(void *arg)
{
    ...
    tt_int_op(1, OP_EQ, purpose_needs_anonymity(0, ROUTER_PURPOSE_BRIDGE, "foobar"));
    tt_int_op(
        1, OP_EQ,
        purpose_needs_anonymity(DIR_PURPOSE_HAS_FETCHED_RENDDESC_V2, ROUTER_PURPOSE_BRIDGE, NULL));
done:;
}
...
test_dir_purpose_needs_anonymity_returns_false_for_own_bridge_desc(void *arg)
{
    (void)arg;
    tt_int_op(0, OP_EQ,
              purpose_needs_anonymity(DIR_PURPOSE_FETCH_SERVERDESC, ROUTER_PURPOSE_BRIDGE,
                                      "authority.z"));
done:;
}
...
test_dir_purpose_needs_anonymity_returns_true_for_sensitive_purpose(void *arg)
{
    ...
    tt_int_op(
        1, OP_EQ,
        purpose_needs_anonymity(DIR_PURPOSE_HAS_FETCHED_RENDDESC_V2, ROUTER_PURPOSE_GENERAL, NULL));
    tt_int_op(1, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_UPLOAD_RENDDESC_V2, 0, NULL));
    tt_int_op(1, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_FETCH_RENDDESC_V2, 0, NULL));
done:;
}
...
test_dir_purpose_needs_anonymity_ret_false_for_non_sensitive_conn(void *arg)
{
    ...
    tt_int_op(0, OP_EQ,
              purpose_needs_anonymity(DIR_PURPOSE_UPLOAD_DIR, ROUTER_PURPOSE_GENERAL, NULL));
    tt_int_op(0, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_UPLOAD_VOTE, 0, NULL));
    tt_int_op(0, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_UPLOAD_SIGNATURES, 0, NULL));
    tt_int_op(0, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_FETCH_STATUS_VOTE, 0, NULL));
    tt_int_op(0, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_FETCH_DETACHED_SIGNATURES, 0, NULL));
    tt_int_op(0, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_FETCH_CONSENSUS, 0, NULL));
    tt_int_op(0, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_FETCH_CERTIFICATE, 0, NULL));
    tt_int_op(0, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_FETCH_SERVERDESC, 0, NULL));
    tt_int_op(0, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_FETCH_EXTRAINFO, 0, NULL));
    tt_int_op(0, OP_EQ, purpose_needs_anonymity(DIR_PURPOSE_FETCH_MICRODESC, 0, NULL));
done:;
}
...
test_dir_fetch_type(void *arg)
{
    ...
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_EXTRAINFO, ROUTER_PURPOSE_BRIDGE, NULL), OP_EQ,
              EXTRAINFO_DIRINFO | BRIDGE_DIRINFO);
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_EXTRAINFO, ROUTER_PURPOSE_GENERAL, NULL), OP_EQ,
              EXTRAINFO_DIRINFO | V3_DIRINFO);

    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_SERVERDESC, ROUTER_PURPOSE_BRIDGE, NULL), OP_EQ,
              BRIDGE_DIRINFO);
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_SERVERDESC, ROUTER_PURPOSE_GENERAL, NULL), OP_EQ,
              V3_DIRINFO);

    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_STATUS_VOTE, ROUTER_PURPOSE_GENERAL, NULL), OP_EQ,
              V3_DIRINFO);
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_DETACHED_SIGNATURES, ROUTER_PURPOSE_GENERAL, NULL),
              OP_EQ, V3_DIRINFO);
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_CERTIFICATE, ROUTER_PURPOSE_GENERAL, NULL), OP_EQ,
              V3_DIRINFO);

    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_CONSENSUS, ROUTER_PURPOSE_GENERAL, "microdesc"),
              OP_EQ, V3_DIRINFO | MICRODESC_DIRINFO);
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_CONSENSUS, ROUTER_PURPOSE_GENERAL, NULL), OP_EQ,
              V3_DIRINFO);

    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_MICRODESC, ROUTER_PURPOSE_GENERAL, NULL), OP_EQ,
              MICRODESC_DIRINFO);

    /* This will give a warning, because this function isn't supposed to be
     * used for HS descriptors. */
    setup_full_capture_of_logs(LOG_WARN);
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_RENDDESC_V2, ROUTER_PURPOSE_GENERAL, NULL), OP_EQ,
              NO_DIRINFO);
...
#define GOOD(s) tt_int_op(1, OP_EQ, validate_recommended_package_line(s));
    ...
    smartlist_add(votes, tor_malloc_zero(sizeof(networkstatus_t)));
    smartlist_add(votes, tor_malloc_zero(sizeof(networkstatus_t)));
    SMARTLIST_FOREACH(votes, networkstatus_t *, ns, ns->package_lines = smartlist_new());

#define ADD(i, s) \
    smartlist_add(((networkstatus_t *)smartlist_get(votes, (i)))->package_lines, (void *)(s));
...
download_status_random_backoff_helper(int min_delay)
{
    download_status_t dls_random = {
        0, 0, 0, DL_SCHED_GENERIC, DL_WANT_AUTHORITY, DL_SCHED_INCREMENT_FAILURE, 0, 0};
    int increment = -1;
    int old_increment = -1;
    ...
    int n_attempts = 0;
    do {
        increment = download_status_schedule_get_delay(&dls_random, min_delay, current_time);

        log_debug(LD_DIR, "Min: %d, Inc: %d, Old Inc: %d", min_delay, increment, old_increment);
...
{
    (void)arg;
    download_status_t dls_exp = {
        0, 0, 0, DL_SCHED_GENERIC, DL_WANT_ANY_DIRSERVER, DL_SCHED_INCREMENT_ATTEMPT, 0, 0};
    ...
    /* we really want to test that it's equal to time(NULL) + delay0, but that's
     * an unrealiable test, because time(NULL) might change. */
    tt_assert(download_status_get_next_attempt_at(&dls_exp) >= current_time + no_delay);
    ...
    /* we really want to test that it's equal to time(NULL) + delay0, but that's
     * an unrealiable test, because time(NULL) might change. */
    tt_assert(download_status_get_next_attempt_at(&dls_exp) >= current_time + delay0);
    tt_assert(download_status_get_next_attempt_at(&dls_exp) != TIME_MAX);
    tt_int_op(download_status_get_n_failures(&dls_exp), OP_EQ, 0);
    tt_int_op(download_status_get_n_attempts(&dls_exp), OP_EQ, 0);
    tt_int_op(mock_get_options_calls, OP_GE, 1);
...
test_dir_authdir_type_to_string(void *data)
{
    ...
    tt_str_op(res = authdir_type_to_string(NO_DIRINFO), OP_EQ, "[Not an authority]");
    tor_free(res);

    tt_str_op(res = authdir_type_to_string(EXTRAINFO_DIRINFO), OP_EQ, "[Not an authority]");
    tor_free(res);

    tt_str_op(res = authdir_type_to_string(MICRODESC_DIRINFO), OP_EQ, "[Not an authority]");
    tor_free(res);
    ...
    tt_str_op(res = authdir_type_to_string(V3_DIRINFO | BRIDGE_DIRINFO | EXTRAINFO_DIRINFO), OP_EQ,
              "V3, Bridge");
done:
    tor_free(res);
}
...
test_dir_should_not_init_request_to_ourselves(void *data)
{
    ...
    ourself = trusted_dir_server_new("ourself", "127.0.0.1", 9059, 9060, NULL, digest, NULL,
                                     V3_DIRINFO, 1.0);
...
test_dir_should_not_init_request_to_dir_auths_without_v3_info(void *data)
{
    ...
    dirinfo_type_t dirinfo_type = BRIDGE_DIRINFO | EXTRAINFO_DIRINFO | MICRODESC_DIRINFO;
    (void)data;
    ...
    ds = trusted_dir_server_new("ds", "10.0.0.1", 9059, 9060, NULL, "12345678901234567890", NULL,
                                dirinfo_type, 1.0);
    tt_assert(ds);
    dir_server_add(ds);
...
static int
mock_check_private_dir(const char *dirname, cpd_check_t check, const char *effective_user)
{
    (void)dirname;
    (void)check;
    (void)effective_user;
...
static char *
mock_get_datadir_fname(const or_options_t *options, directory_root_t roottype, const char *sub1,
...
test_dir_populate_dump_desc_fifo(void *data)
{
    ...
    ent = dump_desc_populate_one_file(dirname, "unparseable-desc.08AE85E90461F59E");
    tt_ptr_op(ent, OP_EQ, NULL);
    ...
    ent = dump_desc_populate_one_file(
        dirname, "unparseable-desc.08AE85E90461F59EDF0981323F3A70D02B55AB54B44B04F"
    ...
    ent = dump_desc_populate_one_file(
        dirname, "unparseable-desc.68219B8BGE64B705A6FFC728C069DC596216D60A7D7520C"
                 "D5ECE250D912E686B");
    tt_ptr_op(ent, OP_EQ, NULL);
    ...
    /* Read fails */
    ent = dump_desc_populate_one_file(
        dirname, "unparseable-desc.DF0981323F3A70D02B55AB54B44B04F287D72F7B72F242E"
                 "85C8CB0EDA8854A99");
    tt_ptr_op(ent, OP_EQ, NULL);
...
static int
mock_networkstatus_consensus_can_use_extra_fallbacks(const or_options_t *options)
{
    (void)options;
...
test_dir_find_dl_min_delay(void *data)
{
    ...
    MOCK(networkstatus_consensus_is_bootstrapping, mock_networkstatus_consensus_is_bootstrapping);
    ...
    mock_options->ClientBootstrapConsensusAuthorityDownloadInitialDelay = client_boot_auth_cons;
    mock_options->ClientBootstrapConsensusFallbackDownloadInitialDelay = client_boot_fallback_cons;
    ...
    if (networkstatus_consensus_is_bootstrapping(time(NULL))) {
        if (networkstatus_consensus_can_use_extra_fallbacks(mock_options)) {
            ...
            /* client */
            mock_options->ClientOnly = 1;
            tt_int_op(find_dl_min_delay(&dls, mock_options), OP_EQ, client_boot_auth_cons);
            mock_options->ClientOnly = 0;
            ...
            /* client */
            mock_options->ClientOnly = 1;
            tt_int_op(find_dl_min_delay(&dls, mock_options), OP_EQ, client_boot_fallback_cons);
            mock_options->ClientOnly = 0;
            ...
            /* client */
            mock_options->ClientOnly = 1;
            tt_int_op(find_dl_min_delay(&dls, mock_options), OP_EQ, client_boot_auth_only_cons);
            mock_options->ClientOnly = 0;
...
test_dir_matching_flags(void *arg)
{
    ...
    const char *ex_noflags = "r example hereiswhereyouridentitygoes 2015-08-30 12:00:00 "
                             "192.168.0.1 9001 0\n"
                             "m thisoneislongerbecauseitisa256bitmddigest33\n"
                             "s\n";
    const char *cp = ex_noflags;
    rs_noflags =
        routerstatus_parse_entry_from_string(area, &cp, cp + strlen(cp), tokens, NULL, NULL,
    ...
    STMT_BEGIN                                                                                  \
        {                                                                                       \
            tor_asprintf(&s,                                                                    \
                         ...
                         string);                                                               \
            cp = s;                                                                             \
            rs = routerstatus_parse_entry_from_string(area, &cp, cp + strlen(cp), tokens, NULL, \
                                                      NULL, MAX_SUPPORTED_CONSENSUS_METHOD,     \
                                                      FLAV_MICRODESC);                          \
...
test_dir_assumed_flags(void *arg)
{
    ...
    const char *str1 = "r example hereiswhereyouridentitygoes 2015-08-30 12:00:00 "
                       "192.168.0.1 9001 0\n"
                       "m thisoneislongerbecauseitisa256bitmddigest33\n"
                       "s Fast Guard Stable\n";
    ...
    const char *cp = str1;
    rs = routerstatus_parse_entry_from_string(area, &cp, eos, tokens, NULL, NULL, 24,
                                              FLAV_MICRODESC);
    tt_assert(rs);
    tt_assert(rs->is_flagged_running);
    tt_assert(rs->is_valid);
...
test_dir_post_parsing(void *arg)
{
    ...
    /* Test the version parsing from an HS descriptor publish request. */
    {
        ...
        tt_int_op(version, OP_EQ, -1);
        tt_ptr_op(end, OP_EQ, NULL);
        version = parse_hs_version_from_post("/random/blah/tor/hs/3/publish", prefix, &end);
        tt_int_op(version, OP_EQ, -1);
        tt_ptr_op(end, OP_EQ, NULL);
        version = parse_hs_version_from_post("/tor/hs/3/publish/random/junk", prefix, &end);
        tt_int_op(version, OP_EQ, 3);
        ...
        tt_ptr_op(end, OP_EQ, NULL);
        /* INT_MAX */
        version = parse_hs_version_from_post("/tor/hs/2147483647/publish", prefix, &end);
        tt_int_op(version, OP_EQ, INT_MAX);
        tt_str_op(end, OP_EQ, "/publish");
        /* INT_MAX + 1*/
        version = parse_hs_version_from_post("/tor/hs/2147483648/publish", prefix, &end);
        tt_int_op(version, OP_EQ, -1);
        tt_ptr_op(end, OP_EQ, NULL);
    }
...
test_dir_platform_str(void *arg)
{
    ...
    tt_int_op(tor_version_parse_platform("Tor 0.3.3.3 (foo) (git-xyzzy) on a potato", &ver, 1),
              OP_EQ, 1);
done:;
}
...
test_dir_format_versions_list(void *arg)
{
    ...
    expect_single_log_msg_containing("Recommended version \"alpha-complex\" does not look valid.");
...
test_dir_add_fingerprint(void *arg)
{
    ...
    /* RSA test - successful */
    ret = add_rsa_fingerprint_to_dir("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", list, 0);
    tt_int_op(ret, OP_EQ, 0);

    /* RSA test - failure */
    ret = add_rsa_fingerprint_to_dir("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ", list, 0);
    tt_int_op(ret, OP_EQ, -1);
...
test_dir_dirserv_router_get_status(void *arg)
{
    ...
    ed25519_secret_key_from_seed(&kp2.seckey, (const uint8_t *)"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
    ed25519_public_key_generate(&kp2.pubkey, &kp2.seckey);
    ri->cache_info.signing_key_cert = tor_cert_create(&kp1, CERT_TYPE_ID_SIGNING, &kp2.pubkey, now,
                                                      86400, CERT_FLAG_INCLUDE_SIGNING_KEY);
...
test_dir_dirserv_would_reject_router(void *arg)
{
    ...
    ed25519_secret_key_from_seed(&kp.seckey, (const uint8_t *)"YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY");
...
test_dir_dirserv_add_own_fingerprint(void *arg)
{
    ...
    tt_assert(digest256map_get(list->status_by_digest256, get_master_identity_key()->pubkey));
...
struct testcase_t dir_tests[] = {
    ...
    DIR_ARG(formats_rsa, TT_FORK, ""), DIR_ARG(formats_rsa_ed25519, TT_FORK, ""),
    /* on a bridge */
    DIR_ARG(formats_rsa, TT_FORK, "b"), DIR_ARG(formats_rsa_ed25519, TT_FORK, "b"),
    /* extrainfo with basic stats */
    DIR_ARG(formats_rsa, TT_FORK, "e"), DIR_ARG(formats_rsa_ed25519, TT_FORK, "e"),
    DIR_ARG(formats_rsa, TT_FORK, "be"), DIR_ARG(formats_rsa_ed25519, TT_FORK, "be"),
    /* extrainfo with all stats */
    DIR_ARG(formats_rsa, TT_FORK, "es"), DIR_ARG(formats_rsa_ed25519, TT_FORK, "es"),
    DIR_ARG(formats_rsa, TT_FORK, "bes"), DIR_ARG(formats_rsa_ed25519, TT_FORK, "bes"),
    DIR(routerinfo_parsing, 0), DIR(extrainfo_parsing, 0), DIR(parse_router_list, TT_FORK),
    DIR(load_routers, TT_FORK), DIR(load_extrainfo, TT_FORK), DIR(getinfo_extra, 0),
    DIR_LEGACY(versions), DIR_LEGACY(fp_pairs), DIR(split_fps, 0), DIR_LEGACY(measured_bw_kb),
    DIR_LEGACY(measured_bw_kb_line_is_after_headers), DIR_LEGACY(measured_bw_kb_cache),
    DIR_LEGACY(dirserv_read_measured_bandwidths), DIR(bwauth_bw_file_digest256, 0),
    DIR_LEGACY(param_voting), DIR(param_voting_lookup, 0), DIR_LEGACY(v3_networkstatus),
    DIR(random_weighted, 0), DIR(scale_bw, 0), DIR_LEGACY(clip_unmeasured_bw_kb),
    DIR_LEGACY(clip_unmeasured_bw_kb_alt), DIR(fmt_control_ns, 0),
    DIR(dirserv_set_routerstatus_testing, TT_FORK), DIR(http_handling, 0),
    ...
    DIR(purpose_needs_anonymity_ret_false_for_non_sensitive_conn, 0), DIR(post_parsing, 0),
    DIR(fetch_type, 0), DIR(packages, 0), DIR(download_status_random_backoff, 0),
    DIR(download_status_random_backoff_ranges, 0), DIR(download_status_increment, TT_FORK),
    DIR(authdir_type_to_string, 0), DIR(conn_purpose_to_string, 0),
    DIR(should_use_directory_guards, 0), DIR(should_not_init_request_to_ourselves, TT_FORK),
    ...
    DIR(populate_dump_desc_fifo_2, 0), DIR_ARG(find_dl_min_delay, TT_FORK, "bfd"),
    DIR_ARG(find_dl_min_delay, TT_FORK, "bad"), DIR_ARG(find_dl_min_delay, TT_FORK, "cfd"),
    DIR_ARG(find_dl_min_delay, TT_FORK, "cad"), DIR_ARG(find_dl_min_delay, TT_FORK, "bfr"),
    DIR_ARG(find_dl_min_delay, TT_FORK, "bar"), DIR_ARG(find_dl_min_delay, TT_FORK, "cfr"),
    DIR_ARG(find_dl_min_delay, TT_FORK, "car"), DIR(assumed_flags, 0), DIR(matching_flags, 0),
    DIR(networkstatus_compute_bw_weights_v10, 0), DIR(platform_str, 0),
    DIR(format_versions_list, TT_FORK), DIR(add_fingerprint, TT_FORK),
    DIR(dirserv_load_fingerprint_file, TT_FORK), DIR(dirserv_router_get_status, TT_FORK),
    DIR(dirserv_would_reject_router, TT_FORK), DIR(dirserv_add_own_fingerprint, TT_FORK),
    END_OF_TESTCASES};
