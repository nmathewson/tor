...
cleanup_mock_configured_ports(void)
{
    ...
    if (mocked_configured_ports) {
        SMARTLIST_FOREACH(mocked_configured_ports, port_cfg_t *, p,
                          tor_free(p));
                                 ...
                                 crypto_pk_t *rsa_onion_keypair)
{
    ...
    /* router_build_fresh_signed_extrainfo() requires
     * get_server_identity_key(). Use the same one as the call to
     * router_dump_router_to_string() above.
     */
...
#define CHECK_ROUTERINFO_CONSISTENCY(r1, rp1)                                \
    STMT_BEGIN                                                               \
        ...
        crypto_pk_t *r1_onion_pkey =                                         \
            router_get_rsa_onion_pkey(r1->onion_pkey, r1->onion_pkey_len);   \
        tt_int_op(crypto_pk_cmp_keys(rp1_onion_pkey, r1_onion_pkey), OP_EQ,  \
                  0);                                                        \
...
test_dir_formats_rsa(void *arg)
{
    ...
    r1 = basic_routerinfo_new("Magri", 0xc0a80001u /* 192.168.0.1 */, 9000,
                              9003, 1000, 5000, 10000, 0, NULL);
    ...
    buf =
        router_dump_router_to_string(r1, r1->identity_pkey, NULL, NULL, NULL);
    tt_assert(buf);
    ...
    smartlist_add(chunks,
                  get_new_ntor_onion_key_line(&r1_onion_keypair.pubkey));
    ...
    ep1 = extrainfo_parse_entry_from_string((const char *)cp, NULL, 1, NULL,
                                            NULL);
...
test_dir_formats_rsa_ed25519(void *arg)
{
     ...
     * IPv6 */
    r2 = basic_routerinfo_new("Fred", 0x0a030201u /* 10.3.2.1 */, 9005, 0,
                              3000, 3000, 3000, 5, &r2_onion_pkey);
    ...
    smartlist_add_strdup(chunks, "master-key-ed25519 ");
    {
        ...
        ed25519_public_to_base64(
            k, &r2->cache_info.signing_key_cert->signing_key);
    ...
    smartlist_add(chunks, get_new_bridge_distribution_request_line(options));
    smartlist_add(chunks,
                  get_new_ntor_onion_key_line(&r2_onion_keypair.pubkey));
    ...
    buf[strlen(buf2)] = '\0'; /* Don't compare either sig; they're never the
                               * same twice */
    ...
    buf =
        router_dump_router_to_string(r2, r2->identity_pkey, NULL, NULL, NULL);
    tt_assert(buf);
    ...
    ep2 = extrainfo_parse_entry_from_string((const char *)cp, NULL, 1, NULL,
                                            NULL);
...
#define ADD(name)                                                      \
    do {                                                               \
        ...
        tt_assert(!crypto_pk_read_public_key_from_string(              \
            pk, name##_KEY, strlen(name##_KEY)));                      \
        tt_int_op(20, OP_EQ,                                           \
                  base16_decode(d, 20, name##_FP, strlen(name##_FP))); \
...
test_dir_parse_router_list(void *arg)
{
    ...
    cp = list;
    tt_int_op(0, OP_EQ,
              router_parse_list_from_string(&cp, NULL, dest, SAVED_NOWHERE, 0,
                                            0, NULL, invalid));
    ...
    cp = list;
    tt_int_op(0, OP_EQ,
              router_parse_list_from_string(&cp, NULL, dest, SAVED_NOWHERE, 1,
                                            0, NULL, invalid));
    tt_int_op(2, OP_EQ, smartlist_len(dest));
...
#define ADD(str)                                                     \
    do {                                                             \
        tt_int_op(0, OP_EQ,                                          \
                  router_get_extrainfo_hash(str, strlen(str), buf)); \
        smartlist_add_strdup(wanted, hex_str(buf, DIGEST_LEN));      \
    } while (0)
...
#define test_v_i_o(val, ver, lst) \
    ...
    tt_int_op(1, OP_EQ,
              tor_version_as_new_as(
                  ...
                  "Tor 0.2.9.9 (git-56788a2489127072)"));
    tt_int_op(1, OP_EQ,
              tor_version_as_new_as("Tor 0.2.9.9 (git-0102)",
                                    "Tor 0.2.9.9 (git-03)"));
    tt_int_op(1, OP_EQ,
              tor_version_as_new_as("Tor 0.2.9.9 (git-0102)",
                                    "Tor 0.2.9.9 (git-00)"));
    ...
    mock_clean_saved_logs();
    tt_int_op(0, OP_EQ,
              tor_version_as_new_as("Tor 0.2147483647.0.0",
    ...
    expect_no_log_entry();
    tt_int_op(0, OP_EQ,
              tor_version_as_new_as("Tor 0.4294967295.0.0",
...
#define B64_256_2 "zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMw"
    ...
    /* uniq strings. */
    dir_split_resource_into_fingerprints("A+C+B+A+B+B", sl, NULL,
                                         DSR_SORT_UNIQ);
    tt_int_op(smartlist_len(sl), OP_EQ, 3);
    tt_str_op(smartlist_get(sl, 0), OP_EQ, "A");
    ...
    /* Decode base64 */
    dir_split_resource_into_fingerprints(B64_1 "-" B64_2, sl, NULL,
                                         DSR_BASE64);
    tt_int_op(smartlist_len(sl), OP_EQ, 2);
    test_mem_op_hex(smartlist_get(sl, 0), OP_EQ, HEX1);
    test_mem_op_hex(smartlist_get(sl, 1), OP_EQ, HEX2);
...
test_dir_measured_bw_kb_line_is_after_headers(void *arg)
{
    ...
    for (i = 0; strcmp(lines_fail[i], "end"); i++) {
        ...
        expect_log_msg_containing(
            "Missing bw or node_id in bandwidth file line:");
        mock_clean_saved_logs();
    }
...
test_dir_dirserv_read_measured_bandwidths(void *arg)
{
    ...
    teardown_capture_of_logs();
    bw_file_headers_str =
        smartlist_join_strings(bw_file_headers, " ", 0, NULL);
...
test_dir_measured_bw_kb_cache(void *arg)
{
    ...
    tt_assert(
        dirserv_query_measured_bw_cache_kb(mbwl[0].node_id, &bw, &as_of));
    ...
    tt_int_op(bw, OP_EQ, 20);
    tt_assert(
        dirserv_query_measured_bw_cache_kb(mbwl[0].node_id, NULL, &as_of));
    tt_int_op(as_of, OP_EQ, MBWC_INIT_TIME);
    /* Now expire it */
    ...
    tt_assert(
        !dirserv_query_measured_bw_cache_kb(mbwl[0].node_id, NULL, NULL));
...
test_dir_param_voting(void *arg)
{
    ...
    smartlist_split_string(
        vote3.net_params, "abcd=20 c=60 cw=500 x-yz=-9 zzzzz=101", NULL, 0, 0);
    ...
    tt_int_op(0, OP_EQ,
              networkstatus_get_overridable_param(&vote4, -101, "foobar", 0,
                                                  -100, 8));
    tt_int_op(-99, OP_EQ,
              networkstatus_get_overridable_param(&vote4, -99, "foobar", 0,
                                                  -100, 8));
...
test_dir_param_voting_lookup(void *arg)
{
    ...
    tt_int_op(9, OP_EQ,
              dirvote_get_intermediate_param_value(lst, "abc", 1000));
    tt_int_op(99, OP_EQ,
...
vote_tweaks_for_v3ns(networkstatus_t *v, int voter, time_t now)
{
    ...
    if (voter == 1) {
        ...
        mbw.bw_kb = 1024;
        tt_int_op(measured_bw_line_apply(&mbw, v->routerstatus_list), OP_EQ,
                  1);
...
test_dir_networkstatus_compute_bw_weights_v10(void *arg)
{
    ...
    tt_str_op(
        ...
        "Wbe=0 Wbg=0 Wbm=10000 Wdb=10000 Web=10000 Wed=2000 Wee=10000 "
        "Weg=2000 "
    ...
    /* case 2b3 */
    /* XXX I can't get a combination of values that hits this case without
     * error, so this just tests that it fails. See #20285. Also see #20284 as
     * 2b3 does not follow dir-spec. */
    ...
    tt_str_op(
        ...
        "Wem=10000 Wgb=10000 Wgd=883 Wgg=6327 Wgm=6327 Wmb=10000 Wmd=883 "
        "Wme=0 "
        "Wmg=3673 Wmm=10000\n");
    SMARTLIST_FOREACH(chunks, char *, cp, tor_free(cp));
    smartlist_clear(chunks);
    ...
    tt_str_op(
        smartlist_get(chunks, 0), OP_EQ,
        "bandwidth-weights Wbd=3333 "
        "Wbe=0 Wbg=0 Wbm=10000 Wdb=10000 Web=10000 Wed=3333 Wee=10000 "
        "Weg=3333 "
                     ...
                     void (*rs_test)(routerstatus_t *rs, time_t now))
{
    ...
    networkstatus_t *con2 = NULL, *con_md2 = NULL, *con3 = NULL,
                    *con_md3 = NULL;
    ...
    tt_assert(!crypto_pk_read_private_key_from_string(
        sign_skey_1, AUTHORITY_SIGNKEY_1, -1));
    tt_assert(!crypto_pk_read_private_key_from_string(
        sign_skey_2, AUTHORITY_SIGNKEY_2, -1));
    tt_assert(!crypto_pk_read_private_key_from_string(
        sign_skey_3, AUTHORITY_SIGNKEY_3, -1));
    ...
    tt_int_op(4, OP_EQ,
              smartlist_len(con->voters)); /*3 voters, 1 legacy key.*/
...

    {
        ...
        con3 = networkstatus_parse_vote_from_string_(consensus_text3, NULL,
                                                     NS_TYPE_CONSENSUS);
        con_md2 = networkstatus_parse_vote_from_string_(
            consensus_text_md2, NULL, NS_TYPE_CONSENSUS);
        con_md3 = networkstatus_parse_vote_from_string_(
            consensus_text_md3, NULL, NS_TYPE_CONSENSUS);
        tt_assert(con2);
        tt_assert(con3);
        tt_assert(con_md2);
        tt_assert(con_md3);
        ...
        tt_mem_op(&con->digests, OP_EQ, &con2->digests,
                  sizeof(common_digests_t));
        tt_mem_op(&con->digests, OP_EQ, &con3->digests,
                  sizeof(common_digests_t));
        ...
        }
        {
            ...
            tt_mem_op(sig->identity_digest, OP_EQ,
                      cert1->cache_info.identity_digest, DIGEST_LEN);
        ...
        { char hd[64]; base16_encode(hd, sizeof(hd), vi->identity_digest,
        DIGEST_LEN); printf("%s\n", hd);
          });
        */
        tt_int_op(
...
test_dir_dirserv_set_routerstatus_testing(void *arg)
{
    ...
    tt_int_op(routerset_contains_routerstatus(routerset_all, rs_a, 0), OP_EQ,
              3);
    tt_int_op(routerset_contains_routerstatus(routerset_all, rs_b, 0), OP_EQ,
              3);
...
test_dir_purpose_needs_anonymity_returns_true_for_bridges(void *arg)
{
    ...
    tt_int_op(1, OP_EQ,
              purpose_needs_anonymity(0, ROUTER_PURPOSE_BRIDGE, NULL));
    tt_int_op(1, OP_EQ,
...
test_dir_purpose_needs_anonymity_ret_false_for_non_sensitive_conn(void *arg)
{
    ...
    tt_int_op(0, OP_EQ,
              purpose_needs_anonymity(DIR_PURPOSE_FETCH_DETACHED_SIGNATURES, 0,
                                      NULL));
    tt_int_op(0, OP_EQ,
...
test_dir_fetch_type(void *arg)
{
    ...
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_EXTRAINFO,
                             ROUTER_PURPOSE_BRIDGE, NULL),
    ...
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_CONSENSUS,
                             ROUTER_PURPOSE_GENERAL, "microdesc"),
              OP_EQ, V3_DIRINFO | MICRODESC_DIRINFO);
    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_CONSENSUS,
                             ROUTER_PURPOSE_GENERAL, NULL),
              OP_EQ, V3_DIRINFO);

    tt_int_op(dir_fetch_type(DIR_PURPOSE_FETCH_MICRODESC,
                             ROUTER_PURPOSE_GENERAL, NULL),
...
{
    download_status_t dls_random = {0,
                                    0,
                                    0,
                                    DL_SCHED_GENERIC,
                                    DL_WANT_AUTHORITY,
    ...
    do {
        ...
        log_debug(LD_DIR, "Min: %d, Inc: %d, Old Inc: %d", min_delay,
...
test_dir_download_status_increment(void *arg)
{
    ...
    mock_options->TestingClientDownloadInitialDelay =
        schedule_no_initial_delay;
    mock_get_options_calls = 0;
    /* we really want to test that it's equal to time(NULL) + delay0, but
     * that's an unrealiable test, because time(NULL) might change. */
...
test_dir_should_not_init_request_to_dir_auths_without_v3_info(void *data)
{
    ...
    ds = trusted_dir_server_new("ds", "10.0.0.1", 9059, 9060, NULL,
                                "12345678901234567890", NULL, dirinfo_type,
                                1.0);
    tt_assert(ds);
    dir_server_add(ds);
...
test_dir_dump_unparseable_descriptors(void *data)
{
    ...
    const char *test_desc_1 = "The most merciful thing in the world, I think, "
                              "is the inability of the "
                              "human mind to correlate all its contents. We "
                              "live on a placid island of"
                              " ignorance in the midst of black seas of "
                              "infinity, and it was not meant"
                              " that we should voyage far. The sciences, each "
                              "straining in its own dir"
                              "ection, have hitherto harmed us little; but "
                              "some day the piecing togeth"
                              "er of dissociated knowledge will open up such "
                              "terrifying vistas of real"
                              "ity, and of our frightful position therein, "
                              "that we shall either go mad"
                              "from the revelation or flee from the light "
                              "into the peace and safety of"
                              "a new dark age.";
    uint8_t test_desc_1_hash[DIGEST256_LEN];
    char test_desc_1_hash_str[HEX_DIGEST256_LEN + 1];
    /* strlen(test_desc_2) = 650 bytes */
    const char *test_desc_2 = "I think their predominant colour was a "
                              "greyish-green, though they had w"
                              "hite bellies. They were mostly shiny and "
                              "slippery, but the ridges of th"
                              "eir backs were scaly. Their forms vaguely "
                              "suggested the anthropoid, whi"
                              "le their heads were the heads of fish, with "
                              "prodigious bulging eyes tha"
                              "t never closed. At the sides of their necks "
                              "were palpitating gills, and"
                              "their long paws were webbed. They hopped "
                              "irregularly, sometimes on two "
                              "legs and sometimes on four. I was somehow glad "
                              "that they had no more th"
                              "an four limbs. Their croaking, baying voices, "
                              "clearly wed tar articulat"
                              "e speech, held all the dark shades of "
                              "expression which their staring fa"
                              "ces lacked.";
    ...
    const char *test_desc_3 =
        ...
        "de"
        "finite structure or building, he dwells only on broad impressions of "
        "va"
        "st angles and stone surfaces - surfaces too great to belong to "
        "anything"
        "right or proper for this earth, and impious with horrible images and "
        "hi"
        "eroglyphs. I mention his talk about angles because it suggests "
        "somethin"
        "g Wilcox had told me of his awful dreams. He said that the geometry "
        "of "
        "the dream-place he saw was abnormal, non-Euclidean, and loathsomely "
        "red"
        "olent of spheres and dimensions apart from ours. Now an unlettered "
        "seam"
    ...
    const char *test_desc_4 = "So we glanced back simultaneously, it would "
                              "appear; though no doubt the"
                              "incipient motion of one prompted the imitation "
                              "of the other. As we did "
                              "so we flashed both torches full strength at "
                              "the momentarily thinned mis"
                              "t; either from sheer primitive anxiety to see "
                              "all we could, or in a les"
                              "s primitive but equally unconscious effort to "
                              "dazzle the entity before "
                              "we dimmed our light and dodged among the "
                              "penguins of the labyrinth cent"
                              "er ahead. Unhappy act! Not Orpheus himself, or "
                              "Lot's wife, paid much mo"
                              "re dearly for a backward glance. And again "
                              "came that shocking, wide-ran"
...
test_dir_find_dl_min_delay(void *data)
{
    ...
    const int client_boot_fallback_cons = 70, bridge = 80,
              bridge_bootstrap = 90;
    ...
    mock_options->TestingBridgeBootstrapDownloadInitialDelay =
        bridge_bootstrap;
...
test_dir_assumed_flags(void *arg)
{
    ...
    const char *cp = str1;
    rs = routerstatus_parse_entry_from_string(area, &cp, eos, tokens, NULL,
                                              NULL, 24, FLAV_MICRODESC);
    tt_assert(rs);
    tt_assert(rs->is_flagged_running);
    tt_assert(rs->is_valid);
    tt_assert(!rs->is_exit);
...
test_dir_post_parsing(void *arg)
{
    ...
    /* Test the version parsing from an HS descriptor publish request. */
    {
        ...
        int version =
            parse_hs_version_from_post("/tor/hs//publish", prefix, &end);
        tt_int_op(version, OP_EQ, -1);
        tt_ptr_op(end, OP_EQ, NULL);
        version =
            parse_hs_version_from_post("/tor/hs/a/publish", prefix, &end);
        tt_int_op(version, OP_EQ, -1);
        tt_ptr_op(end, OP_EQ, NULL);
        version =
            parse_hs_version_from_post("/tor/hs/3/publish", prefix, &end);
        tt_int_op(version, OP_EQ, 3);
        tt_str_op(end, OP_EQ, "/publish");
        version =
            parse_hs_version_from_post("/tor/hs/42/publish", prefix, &end);
        ...
        tt_ptr_op(end, OP_EQ, NULL);
        version =
            parse_hs_version_from_post("/tor/hs/3/publish", "blah", &end);
        tt_int_op(version, OP_EQ, -1);
        tt_ptr_op(end, OP_EQ, NULL);
        /* Missing the '/' at the end of the prefix. */
        version =
            parse_hs_version_from_post("/tor/hs/3/publish", "/tor/hs", &end);
        tt_int_op(version, OP_EQ, -1);
        tt_ptr_op(end, OP_EQ, NULL);
        ...
        version =
            parse_hs_version_from_post("/tor/hs/-1/publish", prefix, &end);
        tt_int_op(version, OP_EQ, -1);
        tt_ptr_op(end, OP_EQ, NULL);
        /* INT_MAX */
...
test_dir_platform_str(void *arg)
{
    ...
    TT_BLATHER(
        ("%d.%d.%d.%d", ver.major, ver.minor, ver.micro, ver.patchlevel));
...
test_dir_format_versions_list(void *arg)
{
    ...
    tor_free(s);
    config_line_append(&lines, "ignored",
                       "0.3.4.1, 0.2.9.111-alpha, 4.4.4-rc");
    s = format_recommended_version_list(lines, 1);
    ...
    mock_clean_saved_logs();
    config_line_append(&lines, "ignored",
                       "0.1.2.3, alpha-complex, 0.1.1.8-rc");
    s = format_recommended_version_list(lines, 1);
...
test_dir_add_fingerprint(void *arg)
{
    ...
    /* RSA test - successful */
    ret = add_rsa_fingerprint_to_dir(
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", list, 0);
    tt_int_op(ret, OP_EQ, 0);

    /* RSA test - failure */
    ret = add_rsa_fingerprint_to_dir(
        "ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ", list, 0);
    tt_int_op(ret, OP_EQ, -1);
