...
test_new_route_len_safe_exit(void *arg)
{
    ...
    r = new_route_len(CIRCUIT_PURPOSE_S_ESTABLISH_INTRO, &dummy_ei, &dummy_nodes);
    tt_int_op(DEFAULT_ROUTE_LEN, OP_EQ, r);
...
#    endif
    ...
    tt_str_op(smartlist_get(tor_get_captured_bug_log_(), 0), OP_EQ, "!(exit_ei && !known_purpose)");
...
test_upgrade_from_guard_wait(void *arg)
{
    ...
    orig_circ->guard_state =
        circuit_guard_state_new(guard, GUARD_CIRC_STATE_WAITING_FOR_BETTER_GUARD, NULL);
...
static int
mock_node_supports_ed25519_link_authentication(const node_t *node, int compatible_with_us)
{
    (void)node;
...
test_circuit_extend_add_ed25519(void *arg)
{
    ...
    MOCK(node_supports_ed25519_link_authentication, mock_node_supports_ed25519_link_authentication);
    ...
    expect_log_msg("Client asked me to extend without specifying an id_digest.\n");
...
test_circuit_choose_ip_ap_for_extend(void *arg)
{
    ...
    can_extend_over_ipv6_result = false;
    mock_router_can_extend_over_ipv6_calls = 0;
    tt_ptr_op(circuit_choose_ip_ap_for_extend(&ipv4_ap, &ipv6_ap), OP_EQ, &ipv4_ap);
    tt_int_op(mock_router_can_extend_over_ipv6_calls, OP_EQ, 1);
...
static void
mock_circuit_mark_for_close_(circuit_t *circ, int reason, int line, const char *cfile)
{
    (void)circ;
    (void)reason;
    (void)line;
...
static channel_t *
mock_channel_connect_for_circuit(const tor_addr_t *addr, uint16_t port, const char *id_digest,
...
static int
mock_extend_cell_parse(extend_cell_t *cell_out, const uint8_t command, const uint8_t *payload_in,
                       size_t payload_len)
{
    (void)command;
    (void)payload_in;
...
static channel_t *
mock_channel_get_for_extend(const char *rsa_id_digest, const ed25519_public_key_t *ed_id,
...
static int
mock_circuit_deliver_create_cell(circuit_t *circ, const struct create_cell_t *create_cell,
                                 int relayed)
{
    (void)create_cell;
...
test_circuit_extend(void *arg)
{
    ...
    MOCK(channel_get_canonical_remote_descr, mock_channel_get_canonical_remote_descr);
    ...
    tor_addr_parse(&mock_extend_cell_parse_cell_out.orport_ipv4.addr, PUBLIC_IPV4);
...
test_onionskin_answer(void *arg)
{
    ...
    /* Circuit must be non-NULL */
    tor_capture_bugs_(1);
    tt_int_op(onionskin_answer(NULL, created_cell, keys, CPATH_KEY_MATERIAL_LEN, rend_circ_nonce),
              OP_EQ, -1);
    tt_int_op(smartlist_len(tor_get_captured_bug_log_()), OP_EQ, 1);
    tt_str_op(smartlist_get(tor_get_captured_bug_log_(), 0), OP_EQ,
              "!(ASSERT_PREDICT_UNLIKELY_(!circ))");
    ...
    tor_capture_bugs_(1);
    tt_int_op(onionskin_answer(or_circ, NULL, keys, CPATH_KEY_MATERIAL_LEN, rend_circ_nonce), OP_EQ,
              -1);
    tt_int_op(smartlist_len(tor_get_captured_bug_log_()), OP_EQ, 1);
    tt_str_op(smartlist_get(tor_get_captured_bug_log_(), 0), OP_EQ,
    ...
    tor_capture_bugs_(1);
    tt_int_op(
        onionskin_answer(or_circ, created_cell, NULL, CPATH_KEY_MATERIAL_LEN, rend_circ_nonce),
        OP_EQ, -1);
    tt_int_op(smartlist_len(tor_get_captured_bug_log_()), OP_EQ, 1);
    tt_str_op(smartlist_get(tor_get_captured_bug_log_(), 0), OP_EQ,
    ...
    tor_capture_bugs_(1);
    tt_int_op(onionskin_answer(or_circ, created_cell, keys, CPATH_KEY_MATERIAL_LEN, NULL), OP_EQ,
              -1);
    tt_int_op(smartlist_len(tor_get_captured_bug_log_()), OP_EQ, 1);
    tt_str_op(smartlist_get(tor_get_captured_bug_log_(), 0), OP_EQ,
    ...
    tt_int_op(
        onionskin_answer(or_circ, created_cell, keys, CPATH_KEY_MATERIAL_LEN, rend_circ_nonce),
        OP_EQ, -1);
...
#    define TEST_CIRCUIT_PASSTHROUGH(name, flags, arg)                                   \
        {                                                                                \
#            name "/" arg, test_circuit_##name, flags, &passthrough_setup, (void *)(arg) \
        }
#endif
