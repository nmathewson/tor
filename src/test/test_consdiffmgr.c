...
static struct testcase_setup_t setup_diffmgr = {consdiffmgr_test_setup, consdiffmgr_test_teardown};
...
mock_cpuworker_queue_work(workqueue_priority_t prio, enum workqueue_reply_t (*fn)(void *, void *),
...
lookup_diff_from(consensus_cache_entry_t **out, consensus_flavor_t flav, const char *str1)
{
    uint8_t digest[DIGEST256_LEN];
    if (router_get_networkstatus_v3_sha3_as_signed(digest, str1, strlen(str1)) < 0) {
        TT_FAIL(("Unable to compute sha3-as-signed"));
        return CONSDIFF_NOT_FOUND;
    }
    return consdiffmgr_find_diff_from(out, flav, DIGEST_SHA3_256, digest, sizeof(digest),
                                      NO_METHOD);
}

static int
lookup_apply_and_verify_diff(consensus_flavor_t flav, const char *str1, const char *str2)
{
...
test_consdiffmgr_make_diffs(void *arg)
{
    ...
    router_get_networkstatus_v3_sha3_as_signed(md_ns_sha3, md_ns_body, strlen(md_ns_body));
    networkstatus_vote_free(ns);
    tt_int_op(r, OP_EQ, 0);
    ...
    diff_status = consdiffmgr_find_diff_from(&diff, FLAV_MICRODESC, DIGEST_SHA3_256, md_ns_sha3,
                                             DIGEST256_LEN, NO_METHOD);
...
#define N 6
    ...
    tt_int_op(0, OP_EQ, lookup_apply_and_verify_diff(FLAV_NS, ns_body[0], ns_body[5]));
    tt_int_op(0, OP_EQ, lookup_apply_and_verify_diff(FLAV_NS, ns_body[1], ns_body[5]));

    tt_int_op(0, OP_EQ, lookup_apply_and_verify_diff(FLAV_MICRODESC, md_body[1], md_body[4]));
    tt_int_op(0, OP_EQ, lookup_apply_and_verify_diff(FLAV_MICRODESC, md_body[2], md_body[4]));
    tt_int_op(0, OP_EQ, lookup_apply_and_verify_diff(FLAV_MICRODESC, md_body[3], md_body[4]));

    /* Self-to-self diff won't be present */
    consensus_cache_entry_t *ent;
    tt_int_op(CONSDIFF_NOT_FOUND, OP_EQ, lookup_diff_from(&ent, FLAV_NS, ns_body[5]));
    /* No diff from 2 has been added yet */
    tt_int_op(CONSDIFF_NOT_FOUND, OP_EQ, lookup_diff_from(&ent, FLAV_NS, ns_body[2]));
    /* No diff arriving at old things. */
    tt_int_op(-1, OP_EQ, lookup_apply_and_verify_diff(FLAV_MICRODESC, md_body[1], md_body[2]));
    /* No backwards diff */
    tt_int_op(-1, OP_EQ, lookup_apply_and_verify_diff(FLAV_MICRODESC, md_body[4], md_body[3]));
    ...
    tt_int_op(0, OP_EQ, lookup_apply_and_verify_diff(FLAV_NS, ns_body[2], ns_body[5]));
...
test_consdiffmgr_diff_failure(void *arg)
{
    ...
    consensus_cache_entry_t *ent;
    tt_int_op(CONSDIFF_NOT_FOUND, OP_EQ, lookup_diff_from(&ent, FLAV_NS, "foo bar baz\n"));
...
#define N 3
    ...
    tt_int_op(0, OP_EQ, lookup_apply_and_verify_diff(FLAV_MICRODESC, md_body[0], md_body[2]));
    tt_int_op(0, OP_EQ, lookup_apply_and_verify_diff(FLAV_MICRODESC, md_body[1], md_body[2]));
...
test_consdiffmgr_cleanup_no_valid_after(void *arg)
{
     ...
     * valid-after. */
    config_line_prepend(&labels, "document-type", "consensus");
    config_line_prepend(&labels, "confrooble-voolid-oofter", "2010-10-10T09:08:07");
    ent = consensus_cache_add(cache, labels, (const uint8_t *)"Foo", 3);
    tt_assert(ent);
    consensus_cache_entry_decref(ent);
...
#define N 4
    ...
    tt_int_op(CONSDIFF_AVAILABLE, OP_EQ, lookup_diff_from(&hold_ent, FLAV_MICRODESC, md_body[1]));
    ...
    tt_int_op(0, OP_EQ, consdiffmgr_add_consensus(md_body[3], md_ns[3]));
    tt_int_op(2 * n_diff_compression_methods() + (n_consensus_compression_methods() - 1), OP_EQ,
              consdiffmgr_cleanup());

    tt_int_op(CONSDIFF_NOT_FOUND, OP_EQ, lookup_diff_from(&ent, FLAV_MICRODESC, md_body[0]));
    /* This one is marked deletable but still in the hashtable */
    tt_int_op(CONSDIFF_AVAILABLE, OP_EQ, lookup_diff_from(&ent, FLAV_MICRODESC, md_body[1]));
    tt_int_op(CONSDIFF_NOT_FOUND, OP_EQ, lookup_diff_from(&ent, FLAV_MICRODESC, md_body[2]));
    ...
    consdiffmgr_rescan();
    tt_int_op(CONSDIFF_IN_PROGRESS, OP_EQ, lookup_diff_from(&ent, FLAV_MICRODESC, md_body[0]));
    tt_int_op(CONSDIFF_IN_PROGRESS, OP_EQ, lookup_diff_from(&ent, FLAV_MICRODESC, md_body[1]));
    tt_int_op(CONSDIFF_IN_PROGRESS, OP_EQ, lookup_diff_from(&ent, FLAV_MICRODESC, md_body[2]));
...
test_consdiffmgr_validate(void *arg)
{
    ...
    config_line_prepend(&lines, "sha3-digest", "now is the winter of our dicotheque");
    ent = consensus_cache_add(cache, lines, (const uint8_t *)"Hi there", 8);
    consensus_cache_entry_decref(ent);
    config_free_lines(lines);
    lines = NULL;
...
#endif
    TEST(sha3_helper), TEST(add), TEST(make_diffs), TEST(diff_rules), TEST(diff_failure),
    TEST(diff_pending), TEST(cleanup_old), TEST(cleanup_bad_valid_after),
    TEST(cleanup_no_valid_after), TEST(cleanup_old_diffs), TEST(validate),
