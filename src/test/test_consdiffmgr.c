...
consdiffmgr_test_setup(const struct testcase_t *arg)
{
    ...
    get_options_mutable()->CacheDirectory =
        ddir_fname; // now owns the pointer.
...
fake_ns_body_new(consensus_flavor_t flav, time_t valid_after)
{
    ...
    tor_asprintf(&consensus,
                 ...
                 flavor_string, valid_after_string, random_stuff,
                 random_stuff2);
                 ...
                 const char *str1)
{
    ...
    if (router_get_networkstatus_v3_sha3_as_signed(digest, str1,
                                                   strlen(str1)) < 0) {
                             ...
                             const char *str2)
{
    ...
    char *applied =
        consensus_diff_apply(str1, strlen(str1), diff_string, size);
    tor_free(diff_owned);
    if (applied == NULL)
        return -1;
...
test_consdiffmgr_add(void *arg)
{
    ...
    tt_ptr_op(cdm_cache_lookup_consensus(FLAV_MICRODESC, now - 60), OP_EQ,
              NULL);
...
test_consdiffmgr_validate(void *arg)
{
    ...
    /* Put these: objects in the cache: one with a good sha3, one with bad
     * sha3, one with a wrong sha3, and one with no sha3. */
