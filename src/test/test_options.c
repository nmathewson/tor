...
dump_logs(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (messages, logmsg_t *, x) {
        smartlist_add_asprintf(msgs, "[%s] %s", log_level_to_string(x->severity), x->msg);
...
static int
test_options_checklog(const char *configuration, int expect_log_severity, const char *expect_log)
{
    int found = 0, ret = -1;
    ...
    if (!found) {
        ...
        TT_DIE(("Expected log message [%s] %s from <%s>, but got <%s>.",
                log_level_to_string(expect_log_severity), expect_log, configuration, actual_log));
    }
    ret = 0;
...
test_options_checkmsgs(const char *configuration, const char *expect_errmsg,
                       int expect_log_severity, const char *expect_log, char *msg)
{
    if (expect_errmsg && !msg) {
        TT_DIE(
            ("Expected error message <%s> from <%s>, but got none.", expect_errmsg, configuration));
    } else if (expect_errmsg && !strstr(msg, expect_errmsg)) {
        TT_DIE(("Expected error message <%s> from <%s>, but got <%s>.", expect_errmsg,
                configuration, msg));
    } else if (!expect_errmsg && msg) {
        TT_DIE(("Expected no error message from <%s> but got <%s>.", configuration, msg));
    }
    if (expect_log) {
        return test_options_checklog(configuration, expect_log_severity, expect_log);
    }
    return 0;
...
static void
test_options_validate_impl(const char *configuration, const char *expect_errmsg,
                           int expect_log_severity, const char *expect_log, int phase)
{
    ...
    if (phase == PH_GETLINES) {
        if (test_options_checkmsgs(configuration, expect_errmsg, expect_log_severity, expect_log,
                                   msg))
            goto done;
    }
    if (r)
...
#define WANT_ERR(config, msg, ph) test_options_validate_impl((config), (msg), 0, NULL, (ph))
...
#define OK(config, ph) test_options_validate_impl((config), NULL, 0, NULL, (ph))
...
test_options_validate(void *arg)
{
    ...
    WANT_ERR_LOG("ServerTransportOptions trebuchet", "ServerTransportOptions did not parse",
    ...
    WANT_ERR_LOG("ServerTransportOptions trebuchet slingsnappy",
                 "ServerTransportOptions did not parse", LOG_WARN, "\"slingsnappy\" is not a k=v",
                 PH_VALIDATE);

    WANT_ERR("DirPort 8080\nDirCache 0", "DirPort configured but DirCache disabled.", PH_VALIDATE);
    WANT_ERR("BridgeRelay 1\nDirCache 0", "We're a bridge but DirCache is disabled.", PH_VALIDATE);

    WANT_ERR("HeartbeatPeriod 21 snarks", "Unknown unit in 21 snarks", PH_ASSIGN);
    WANT_ERR("LogTimeGranularity 21 snarks", "Unknown unit in 21 snarks", PH_ASSIGN);
...
test_options_validate__data_directory(void *ignored)
{
    ...
    int ret;
    char *msg;
    options_test_data_t *tdata = get_options_test_data("DataDirectory longreallyl"
...
test_options_validate__contactinfo(void *ignored)
{
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg("Your ContactInfo config option is not set. Please strongly "
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_no_log_msg("Your ContactInfo config option is not set. Please strongly "
                      "consider setting it, so we can contact you if your relay is "
                      "misconfigured, end-of-life, or something else goes wrong. It "
                      "is also possible that your relay might get rejected from the "
                      "network due to a missing valid contact address.\n");
...
test_options_validate__authdir(void *ignored)
{
    ...
    options_test_data_t *tdata =
        get_options_test_data(ENABLE_AUTHORITY_V3_MIN "Address this.should.not!exist!.example.org");
...
test_options_validate__relay_with_hidden_services(void *ignored)
{
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg("Tor is currently configured as a relay and a hidden service. "
...
test_options_validate__exclude_nodes(void *ignored)
{
    ...
    tt_str_op((char *)(smartlist_get(tdata->opt->ExcludeExitNodesUnion_->list, 0)), OP_EQ,
              "{us} {cn}");
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_no_log_msg("You have asked to exclude certain relays from all positions "
                      "in your circuits. Expect hidden services and other Tor "
                      "features to be broken in unpredictable ways.\n");
    tor_free(msg);
...
test_options_validate__use_bridges(void *ignored)
{
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, -1);
    tt_str_op(msg, OP_EQ, "Setting UseBridges requires also setting UseEntryGuards.");
    tor_free(msg);
...
test_options_validate__path_bias(void *ignored)
{
    ...
    options_test_data_t *tdata = get_options_test_data("PathBiasNoticeRate 1.1\n");
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, -1);
...
test_options_validate__rend(void *ignored)
{
    ...
    free_options_test_data(tdata);
    tdata = get_options_test_data("UseEntryGuards 0\n"
    ...
    free_options_test_data(tdata);
    tdata = get_options_test_data("UseEntryGuards 1\n"
                                  "HiddenServiceDir /Library/Tor/var/lib/tor/hidden_service/\n"
                                  "HiddenServicePort 80 127.0.0.1:8080\n");
    mock_clean_saved_logs();
    ret = options_validate(NULL, tdata->opt, &msg);
...
test_options_validate__single_onion(void *ignored)
{
    ...
    tdata = get_options_test_data("HiddenServiceDir /Library/Tor/var/lib/tor/hidden_service/\n"
                                  "HiddenServicePort 80 127.0.0.1:8080\n");
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    tt_ptr_op(msg, OP_EQ, NULL);
...
test_options_validate__control(void *ignored)
{
    ...
    free_options_test_data(tdata);
    tdata = get_options_test_data("__HashedControlSessionPassword something_incorrect\n");
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, -1);
    tt_str_op(msg, OP_EQ,
    ...
    free_options_test_data(tdata);
    tdata = get_options_test_data("__OwningControllerProcess something_incorrect\n");
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, -1);
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg("ControlPort is open, but no authentication method has been "
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_no_log_msg("ControlPort is open, but no authentication method has been "
                      "configured.  This means that any program on your computer can "
                      "reconfigure your Tor.  That's bad!  You should upgrade your Tor "
                      "controller as soon as possible.\n");
    tor_free(msg);
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg("ControlSocket is world writable, but no authentication method has"
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_no_log_msg("ControlSocket is world writable, but no authentication method has"
                      " been configured.  This means that any program on your computer "
                      "can reconfigure your Tor.  That's bad!  You should upgrade your "
                      "Tor controller as soon as possible.\n");
    tor_free(msg);
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg("CookieAuthFileGroupReadable is set, but will have no effect: you "
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_no_log_msg("CookieAuthFileGroupReadable is set, but will have no effect: you "
                      "must specify an explicit CookieAuthFile to have it "
                      "group-readable.\n");
    tor_free(msg);
...
test_options_validate__families(void *ignored)
{
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg("Listing a family for a bridge relay is not supported: it can "
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_no_log_msg("Listing a family for a bridge relay is not supported: it can "
                      "reveal bridge fingerprints to censors. You should also make sure "
                      "you aren't listing this bridge's fingerprint in any other "
                      "MyFamily.\n");
    tor_free(msg);
...
test_options_validate__dir_auth(void *ignored)
{
    ...
    tt_str_op(msg, OP_EQ,
              ...
              "details.");
    expect_log_msg("You cannot set both DirAuthority and Alternate*Authority.\n");
    tor_free(msg);
    ...
    free_options_test_data(tdata);
    tdata = get_options_test_data(VALID_ALT_DIR_AUTH VALID_ALT_BRIDGE_AUTH "TestingTorNetwork 1\n");
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    tor_free(msg);
...
test_options_validate__transport(void *ignored)
{
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg("You need at least a single managed-proxy to specify a transport "
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_no_log_msg("You need at least a single managed-proxy to specify a transport "
                      "listen address. The ServerTransportListenAddr line will be "
                      "ignored.\n");
...
test_options_validate__constrained_sockets(void *ignored)
{
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    expect_no_log_msg("You have requested constrained socket buffers while also serving"
...
test_options_validate__v3_auth(void *ignored)
{
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data(ENABLE_AUTHORITY_V3 "TestingV3AuthInitialVoteDelay 1\n" VALID_DIR_AUTH
                                                  "TestingTorNetwork 1\n");
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, -1);
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data(ENABLE_AUTHORITY_V3 "TestingV3AuthInitialDistDelay 1\n" VALID_DIR_AUTH
                                                  "TestingTorNetwork 1\n");
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, -1);
...
#define TEST_TESTING_OPTION(name, accessor, low_val, high_val, err_low, EXTRA_OPT_STR)       \
    STMT_BEGIN                                                                               \
        free_options_test_data(tdata);                                                       \
        tdata = get_options_test_data(EXTRA_OPT_STR VALID_DIR_AUTH "TestingTorNetwork 1\n"); \
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("TestingEnableConnBwEvent 1\n" VALID_DIR_AUTH "TestingTorNetwork 1\n"
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("TestingEnableTbEmptyEvent 1\n" VALID_DIR_AUTH "TestingTorNetwork 1\n"
                              "___UsingTestNetworkDefaults 0\n");
...
static void
mock_add_stream_log_impl(const log_severity_list_t *sev, const char *name, int fd)
{
...
test_options_init_logs_quiet(void *arg)
{
    ...
    tt_int_op(a->fd, OP_EQ, fileno(stdout));
    tt_u64_op(a->sev.masks[SEVERITY_MASK_IDX(LOG_INFO)], OP_EQ, 0);
    tt_u64_op(a->sev.masks[SEVERITY_MASK_IDX(LOG_NOTICE)], OP_EQ, LD_ALL_DOMAINS);
    tt_u64_op(a->sev.masks[SEVERITY_MASK_IDX(LOG_WARN)], OP_EQ, LD_ALL_DOMAINS);
    clear_added_logs();
...
struct testcase_t options_tests[] = {
    ...
    {"init_logs/granularity", test_options_init_logs_granularity, TT_FORK, NULL, NULL},
