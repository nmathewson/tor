...
test_options_validate(void *arg)
{
    ...
    WANT_ERR("HeartbeatPeriod 21 snarks", "Unknown unit in 21 snarks",
             PH_ASSIGN);
    WANT_ERR("LogTimeGranularity 21 snarks", "Unknown unit in 21 snarks",
             PH_ASSIGN);
...
get_options_test_data(const char *conf)
{
    ...
    if (rv != 0) {
        ...
        result = NULL;
        /* Callers expect a non-NULL result, so just die if we can't provide
         * one.
         */
        tor_assert(0);
    }
...
test_options_validate__authdir(void *ignored)
{
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data(ENABLE_AUTHORITY_MIN "Address 100.200.10.1\n"
                                                   "TestingTorNetwork 1\n");
    mock_clean_saved_logs();
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, -1);
...
test_options_validate__exclude_nodes(void *ignored)
{
    ...
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, 0);
    tt_int_op(smartlist_len(tdata->opt->ExcludeExitNodesUnion_->list), OP_EQ,
              1);
    tt_str_op(
...
test_options_validate__node_families(void *ignored)
{
    ...
    char *msg;
    options_test_data_t *tdata =
        get_options_test_data("NodeFamily flux, flax\n"
    ...
    tt_str_op((char *)(smartlist_get(
                  ((routerset_t *)smartlist_get(tdata->opt->NodeFamilySets, 0))
                      ->list,
                  0)),
              OP_EQ, "flux");
    tt_str_op((char *)(smartlist_get(
              ...
              OP_EQ, "flax");
    tt_str_op((char *)(smartlist_get(
                  ((routerset_t *)smartlist_get(tdata->opt->NodeFamilySets, 1))
                      ->list,
                  0)),
              OP_EQ, "somewhere");
    tor_free(msg);
...
test_options_validate__conn_limit(void *ignored)
{
    ...
    tt_int_op(ret, OP_EQ, -1);
    tt_str_op(msg, OP_EQ,
              "ConnLimit must be greater than 0, but was set to 0");
    tor_free(msg);
...
test_options_validate__ports(void *ignored)
{
    ...
    char *msg;
    options_test_data_t *tdata =
        get_options_test_data("FirewallPorts 65537\n");
    ...
    tt_int_op(ret, OP_EQ, -1);
    tt_str_op(msg, OP_EQ,
              "Port '112233' out of range in RejectPlaintextPorts");
    tor_free(msg);
...
test_options_validate__safe_logging(void *ignored)
{
    ...
    escaped(
        NULL); // This will free the leaking memory from the previous escaped
    free_options_test_data(tdata);
    tor_free(msg);
}
    ...
    STMT_BEGIN                                                               \
        ...
        if (msg) {                                                           \
            ...
            tor_free(msg);                                                   \
                                                                             ...
                                                                             \
        free_options_test_data(tdata);                                       \
        tdata = get_options_test_data(#varname                               \
                                      " " #varval "\n"                       \
...
#define ENSURE_BANDWIDTH_PARAM(p, EXTRA_OPT_STR)                             \
    STMT_BEGIN                                                               \
        ...
        ret = options_validate(NULL, tdata->opt, &msg);                      \
        tt_int_op(ret, OP_EQ, -1);                                           \
        tt_mem_op(msg, OP_EQ, #p " (3221225471) must be at most 2147483647", \
                  40);                                                       \
        tor_free(msg);                                                       \
    STMT_END
...
test_options_validate__accounting(void *ignored)
{
    ...
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg(
        "Using accounting with multiple "
...
test_options_validate__proxy(void *ignored)
{
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("HttpProxyAuthenticator "
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("HttpsProxyAuthenticator "
                              "onetwothreonetwothreonetwothreonetwothreonetw"
                              "othreonetwothreonetwothreonetwothreonetwothre"
                              "onetwothreonetwothreonetwothreonetwothreonetw"
                              "othreonetwothreonetwothreonetwothreonetwothre"
...
test_options_validate__control(void *ignored)
{
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("HashedControlPassword something_incorrect\n");
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, -1);
    tt_str_op(msg, OP_EQ,
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("HashedControlPassword 16:872860B76453A77D60CA"
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("__HashedControlSessionPassword 16:872860B7645"
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("ControlPort 127.0.0.1:1234\n"
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("ControlSocket unix:/tmp WorldWritable\n"
                              "HashedControlPassword 16:872860B76453A77D60CA"
                              "2BB8C1A7042072093276A3D701AD684053EC4C\n");
    mock_clean_saved_logs();
    ret = options_validate(NULL, tdata->opt, &msg);
...
test_options_validate__dir_auth(void *ignored)
{
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("TestingTorNetwork 1\n" VALID_ALT_BRIDGE_AUTH);
    ret = options_validate(NULL, tdata->opt, &msg);
    tt_int_op(ret, OP_EQ, -1);
    tt_str_op(
        msg, OP_EQ,
...
#define TEST_TESTING_OPTION(name, accessor, low_val, high_val, err_low, \
    ...
    free_options_test_data(tdata);
    tdata =
        get_options_test_data("TestingEnableCellStatsEvent 1\n" VALID_DIR_AUTH
                              "TestingTorNetwork 1\n"
                              "___UsingTestNetworkDefaults 0\n");
...
test_options_init_logs_quiet(void *arg)
{
    ...
    tt_u64_op(a->sev.masks[SEVERITY_MASK_IDX(LOG_WARN)], OP_EQ,
              LD_ALL_DOMAINS);
    clear_added_logs();
    ...
    tt_u64_op(a->sev.masks[SEVERITY_MASK_IDX(LOG_INFO)], OP_EQ,
              LD_ALL_DOMAINS);
    tt_u64_op(a->sev.masks[SEVERITY_MASK_IDX(LOG_NOTICE)], OP_EQ,
              LD_ALL_DOMAINS);
    tt_u64_op(a->sev.masks[SEVERITY_MASK_IDX(LOG_WARN)], OP_EQ,
...
#    define LOCAL_VALIDATE_TEST(name)                                         \
        {                                                                     \
            "validate__" #name, test_options_validate__##name, TT_FORK, NULL, \
                NULL                                                          \
        }
#endif
