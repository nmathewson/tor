...
b16_dec(const chunk_t *inp)
{
    ...
    int r =
        base16_decode((char *)ch->buf, ch->len, (char *)inp->buf, inp->len);
    if (r >= 0) {
        ch->len = r;
    } else {
        chunk_free(ch);
...
b32_dec(const chunk_t *inp)
{
    ...
    int r =
        base32_decode((char *)ch->buf, ch->len, (char *)inp->buf, inp->len);
    if (r >= 0) {
        ch->len = r;
    } else {
        chunk_free(ch);
...
b64_dec(const chunk_t *inp)
{
    ...
    int r =
        base64_decode((char *)ch->buf, ch->len, (char *)inp->buf, inp->len);
    if (r >= 0) {
        ch->len = r;
    } else {
        chunk_free(ch);
