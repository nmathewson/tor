...
static int
mock_check_tap_onion_key_crosscert__nocheck(const uint8_t *crosscert, int crosscert_len,
...
static int
mock_router_produce_hash_final__nohash(char *digest, const char *start, size_t len,
...
fuzz_init(void)
{
    ...
    MOCK(check_tap_onion_key_crosscert, mock_check_tap_onion_key_crosscert__nocheck);
...
fuzz_main(const uint8_t *data, size_t sz)
{
    ...
    ri = router_parse_entry_from_string((const char *)str, str + sz, 0, 0, 0, NULL);
    if (ri) {
