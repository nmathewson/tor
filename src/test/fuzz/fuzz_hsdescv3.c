...
static int
mock_rsa_ed25519_crosscert_check(const uint8_t *crosscert, const size_t crosscert_len,
...
static size_t
mock_decrypt_desc_layer(const hs_descriptor_t *desc, const uint8_t *descriptor_cookie,
                        bool is_superencrypted_layer, char **decrypted_out)
{
    ...
    if (encrypted_blob_size < overhead)
        return 0;
    *decrypted_out = tor_memdup_nulterm(encrypted_blob + HS_DESC_ENCRYPTED_SALT_LEN,
