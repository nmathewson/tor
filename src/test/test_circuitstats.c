...
subtest_fourhop_circuit(struct timeval circ_start_time, int should_timeout)
{
    ...
    circuit_build_times_handle_completed_hop(or_circ);
    tt_int_op(get_circuit_build_times()->total_build_times, OP_EQ, !should_timeout);
...
test_circuitstats_hoplen(void *arg)
{
    ...
    circuit_expire_building();
    tt_int_op(get_circuit_build_times()->total_build_times, OP_EQ, 1);
    tt_int_op(TO_CIRCUIT(fourhop)->purpose, OP_NE, CIRCUIT_PURPOSE_C_MEASURE_TIMEOUT);
    ...
    fourhop = subtest_fourhop_circuit(circ_start_time, 0);
    tt_int_op(TO_CIRCUIT(fourhop)->purpose, OP_EQ, CIRCUIT_PURPOSE_C_MEASURE_TIMEOUT);
    tt_int_op(get_circuit_build_times()->total_build_times, OP_EQ, 1);
    circuit_expire_building();
    tt_int_op(get_circuit_build_times()->total_build_times, OP_EQ, 1);
...
struct testcase_t circuitstats_tests[] = {TEST_CIRCUITSTATS(circuitstats_hoplen, TT_FORK),
                                          END_OF_TESTCASES};
