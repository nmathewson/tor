...
static int dns_resolve_dns_resolve_impl(edge_connection_t *exitconn, int is_resolve,
 ...
 */
static int
dns_resolve_dns_resolve_impl(edge_connection_t *exitconn, int is_resolve, or_circuit_t *oncirc,
...
static void
dns_resolve_send_resolved_hostname_cell(edge_connection_t *conn, const char *hostname)
{
    conn_for_resolved_cell = conn;
...
test_dns_resolve(void *arg)
{
    ...
    prev_n_send_resolved_hostname_cell_replacement = n_send_resolved_hostname_cell_replacement;
    ...
    tt_int_op(retval, OP_EQ, 1);
    tt_assert(conn_for_resolved_cell == exitconn);
    tt_int_op(n_send_resolved_cell_replacement, OP_EQ, prev_n_send_resolved_cell_replacement + 1);
    ...
    tt_int_op(prev_n_send_resolved_cell_replacement, OP_EQ, n_send_resolved_cell_replacement);
...
test_dns_impl_malformed_ptr(void *arg)
{
    ...
    TO_CONN(exitconn)->address = tor_strdup("z01234567890123456789.in-addr.arpa");
...
test_dns_impl_cache_hit_pending(void *arg)
{
    ...
    strlcpy(cache_entry->address, TO_CONN(exitconn)->address, sizeof(cache_entry->address));
...
test_dns_impl_cache_hit_cached(void *arg)
{
    ...
    MOCK(set_exitconn_info_from_resolve, dns_impl_cache_hit_cached_set_exitconn_info_from_resolve);

    dns_init();

    dns_insert_cache_entry(cache_entry);

    retval = dns_resolve_impl(exitconn, 1, on_circ, NULL, &made_pending, &resolve_out);
