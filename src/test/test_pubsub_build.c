...
static dispatch_typefns_t stringfns = {.free_fn = ex_str_free, .fmt_fn = ex_str_fmt};
...
static void
seed_dispatch_builder(pubsub_builder_t *b, unsigned fl1, unsigned fl2, unsigned fl3, unsigned fl4)
{
...
test_pubsub_build_types_ok(void *arg)
{
    ...
    tt_assert(dispatcher->typefns[get_msg_type_id("string")].fmt_fn == ex_str_fmt);
...
test_pubsub_build_missing_pubsub(void *arg)
{
    ...
    expect_log_msg_containing("Message \"bunch_of_coconuts\" has publishers, but no subscribers.");
    expect_log_msg_containing("Message \"yes_we_have_no\" has subscribers, but no publishers.");
...
test_pubsub_build_stub_pubsub(void *arg)
{
    ...
    // 1 subscriber.
    tt_int_op(1, OP_EQ, dispatcher->table[get_message_id("yes_we_have_no")]->n_enabled);
    // no subscribers
    tt_ptr_op(NULL, OP_EQ, dispatcher->table[get_message_id("bunch_of_coconuts")]);
...
test_pubsub_build_channels_conflict(void *arg)
{
...

    {
         ...
         * the wrong channel */
        pubsub_add_pub_(c, &btmp, get_channel_id("hithere"), get_message_id("bunch_of_coconuts"),
...
test_pubsub_build_pubsub_multi(void *arg)
{
...

    {
        c = pubsub_connector_for_subsystem(b, get_subsys_id("sys3"));
        DISPATCH_ADD_SUB(c, main, bunch_of_coconuts);
        pubsub_add_pub_(c, &btmp, get_channel_id("main"), get_message_id("yes_we_have_no"),
                        get_msg_type_id("string"), 0 /* flags */, "somewhere.c", 22);
        pubsub_connector_free(c);
    };
    ...
    for (i = 1; i < 100; ++i) {
        ...
        if (i % 7) {
            ...
            pubsub_add_sub_(c, some_other_coconut_hook, get_channel_id("main"),
                            get_message_id("bunch_of_coconuts"), get_msg_type_id("int"),
                            0 /* flags */, "somewhere.c", 22);
        }
        pubsub_connector_free(c);
        tor_free(sysname);
    };
...
test_pubsub_build_excl_ok(void *arg)
{
    ...
    tt_int_op(1, OP_EQ, dispatcher->table[get_message_id("yes_we_have_no")]->n_enabled);
    // 1 subscriber.
    tt_int_op(1, OP_EQ, dispatcher->table[get_message_id("bunch_of_coconuts")]->n_enabled);
