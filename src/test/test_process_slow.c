...
run_main_loop(process_data_t *process_data)
{
    ...
    main_loop_timeout_timer =
        periodic_timer_new(tor_libevent_get_base(), &interval, main_loop_timeout_cb, process_data);
...
test_callbacks(void *arg)
{
    ...
    tor_snprintf(argv0_expected, sizeof(argv0_expected), "argv[0] = '%s'", filename);

    tt_str_op(smartlist_get(process_data->stdout_data, 0), OP_EQ, argv0_expected);
    tt_str_op(smartlist_get(process_data->stdout_data, 1), OP_EQ,
              "argv[1] = 'This is the first one'");
    tt_str_op(smartlist_get(process_data->stdout_data, 2), OP_EQ, "argv[2] = 'Second one'");
    tt_str_op(smartlist_get(process_data->stdout_data, 3), OP_EQ, "argv[3] = 'Third: Foo bar baz'");
    ...
    tt_str_op(smartlist_get(process_data->stdout_data, 6), OP_EQ, "This is a new line");
    ...
    /* Check stderr output. */
    tt_str_op(smartlist_get(process_data->stderr_data, 0), OP_EQ, "Output on stderr");
    tt_str_op(smartlist_get(process_data->stderr_data, 1), OP_EQ, "This is a new line");
