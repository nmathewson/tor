...
test_clist_maps(void *arg)
{
    ...
    tt_ptr_op(circuit_get_by_circid_channel(200, ch1), OP_EQ,
              TO_CIRCUIT(or_c1));
    tt_ptr_op(circuit_get_by_circid_channel(200, ch2), OP_EQ,
              TO_CIRCUIT(or_c2));
    tt_ptr_op(circuit_get_by_circid_channel(100, ch2), OP_EQ,
              TO_CIRCUIT(or_c1));
    ...
    tt_assert(!circuit_id_in_use_on_channel(100, ch2));
    tt_ptr_op(circuit_get_by_circid_channel(500, ch3), OP_EQ,
              TO_CIRCUIT(or_c1));
    ...
    tt_ptr_op(circuit_get_by_circid_channel(200, ch2), OP_EQ,
              TO_CIRCUIT(or_c2));
    tt_ptr_op(circuit_get_by_circid_channel(100, ch1), OP_EQ,
              TO_CIRCUIT(or_c2));
...
test_hs_circuitmap_isolation(void *arg)
{
...

    {
        ...
        tt_ptr_op(NULL, OP_EQ,
                  hs_circuitmap_get_intro_circ_v2_service_side(tok1));
...

    {
        ...
        tt_ptr_op(circ3, OP_EQ,
                  hs_circuitmap_get_intro_circ_v2_relay_side(tok2));
        ...
        /* Check that the getter returns circ4; the last circuit registered
         * with that token. */
