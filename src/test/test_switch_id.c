...
#    endif
    ...
    case TEST_ROOT_CAN_BIND_LOW:
        ...
        break;
    case TEST_SETUID:
        /* Succeed if we can do a setuid with no capability retention, and
         * doing so makes us lose the ability to bind low ports */
    case TEST_SETUID_KEEPCAPS:
        /* Succeed if we can do a setuid with capability retention, and doing
         * so does not make us lose the ability to bind low ports */
        {
            int keepcaps = (test_id == TEST_SETUID_KEEPCAPS);
            okay = switch_id(username,
                             keepcaps ? SWITCH_ID_KEEP_BINDLOW : 0) == 0;
            if (okay) {
