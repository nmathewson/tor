...
#define ADD_PUBLISH(binding_suffix, subsys, channel, msg, flags)                                \
    STMT_BEGIN                                                                                  \
        {                                                                                       \
            ...
            pubsub_add_pub_(con, &pub_binding_##binding_suffix, get_channel_id(#channel),       \
                            get_message_id(#msg), get_msg_type_id("string"), (flags), __FILE__, \
                            __LINE__);                                                          \
...
setup_dispatcher(const struct testcase_t *testcase)
{
...

    {
        ...
        pubsub_connector_register_type_(con, get_msg_type_id("string"), &stringfns, "nowhere.c",
                                        99);
        pubsub_connector_free(con);
    }
    ...
    for (int i = 0; i < 1000 - 5; ++i) {
        ...
        pubsub_add_sub_(con, recv_msg_copy_string, get_channel_id("main"),
                        get_message_id("message5"), get_msg_type_id("string"), 0, "here", 100);
        pubsub_connector_free(con);
        tor_free(sys);
    }
...
static const struct testcase_setup_t dispatcher_setup = {setup_dispatcher, cleanup_dispatcher};
...
test_pubsub_msg_alertfns(void *arg)
{
    ...
    tt_int_op(0, OP_EQ, dispatch_set_alert_fn(d, get_channel_id("main"), alertfn_generic, &ch1_a));
    tt_int_op(0, OP_EQ, dispatch_set_alert_fn(d, get_channel_id("other"), alertfn_generic, &ch2_a));
...
struct testcase_t pubsub_msg_tests[] = {T(minimal),  T(send_to_stub), T(cancel_msgs),
                                        T(alertfns), T(many_hooks),   END_OF_TESTCASES};
