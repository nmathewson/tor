    ...
    do {                                                                     \
        ...
        if (!(relerr(check_expected, check_actual) <= relerr_bound)) {       \
            log_warn(LD_GENERAL,                                             \
                     ...
                     " = %.17e > %.17e\n",                                   \
                     __func__, __LINE__, (unsigned)i, str_expected,          \
                     check_expected, str_actual, check_actual, check_relerr, \
                     relerr_bound);                                          \
            ok = false;                                                      \
        }                                                                    \
    } while (0)
    ...
    do {                                                                     \
        ...
        if (!(check_a <= check_b)) {                                         \
            log_warn(LD_GENERAL, "%s:%d: case %u: %s=%.17e > %s=%.17e\n",    \
                     __func__, __LINE__, (unsigned)i, str_a, check_a, str_b, \
                     check_b);                                               \
            ok = false;                                                      \
        }                                                                    \
    } while (0)
...
test_log_logistic(void *arg)
{
    ...
    static const struct {
        ...
        double x;
        /* 'p' is the probability that a random variable X for a given
         * LogLogistic
         * probability ditribution will take value less-or-equal to x */
        double p;
        /* 'np' is the probability that a random variable X for a given
         * LogLogistic probability distribution will take value
...
test_genpareto(void *arg)
{
    ...
    struct {
        /* xi is the 'xi' parameter of the generalized Pareto distribution, and
         * the rest are the same as in the above tests */
        double xi, x, p, np;
    } cases[] = {
        {0, 0, 0, 1},
    ...
    for (i = 0; i < arraycount(xi_array); i++) {
        for (j = 0; j <= 100; j++) {
            ...
            /* This is actually a check against 0, but we do <= so that the
               compiler does not raise a -Wfloat-equal */
...
test_uniform_interval(void *arg)
{
    (void)arg;
    struct {
        /* Sample from a uniform distribution with parameters 'a' and 'b',
         * using 't' as the sampling index. */
        double t, a, b;
    } cases[] = {
        {0, 0, 0},
        {0, 0, 1},
...
write_stochastic_warning(void)
{
    if (tinytest_cur_test_has_failed()) {
        printf(
            ...
            "time to time, with some low probability. If you see it fail "
            "more\n"
