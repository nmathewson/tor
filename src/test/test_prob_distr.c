    ...
    do {                                                                                        \
        ...
        if (!(relerr(check_expected, check_actual) <= relerr_bound)) {                          \
            log_warn(LD_GENERAL,                                                                \
                     ...
                     " = %.17e > %.17e\n",                                                      \
                     __func__, __LINE__, (unsigned)i, str_expected, check_expected, str_actual, \
                     check_actual, check_relerr, relerr_bound);                                 \
            ok = false;                                                                         \
        }                                                                                       \
    } while (0)
    ...
    do {                                                                                      \
        ...
        if (!(check_a <= check_b)) {                                                          \
            log_warn(LD_GENERAL, "%s:%d: case %u: %s=%.17e > %s=%.17e\n", __func__, __LINE__, \
                     (unsigned)i, str_a, check_a, str_b, check_b);                            \
            ok = false;                                                                       \
        }                                                                                     \
    } while (0)
...
test_logit_logistic(void *arg)
{
    ...
    for (i = 0; i <= 100; i++) {
        ...
        CHECK_RELERR(logithalf(p0 * (0.5 - 1 / (1 + M_E))), sample_logistic(0, 1, p0));
        CHECK_RELERR(-logithalf(p0 * (0.5 - 1 / (1 + M_E))), sample_logistic(1, 1, p0));
    }
    ...
    for (i = 0; i < arraycount(xi_array); i++) {
        for (j = 0; j <= 100; j++) {
            ...
            if (fabs(xi_array[i]) <= 0) {
                ...
                CHECK_RELERR(
                    (j == 0 ? DBL_MIN : expm1(-xi_array[i] * log1p(-p0 / 2)) / xi_array[i]),
 ...
 */
static void
bin_cdfs(const struct dist_t *dist, double lo, double hi, double *logP, size_t n)
{
 ...
 */
static void
bin_samples(const struct dist_t *dist, double lo, double hi, size_t *C, size_t n)
{
    const double w = (hi - lo) / (n - 2);
    size_t i;
