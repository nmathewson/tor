...
test_policies_general(void *arg)
{
    ...
    p = router_parse_addr_policy_item_from_string("reject 169.254.0.0/16:*",
                                                  -1, &malformed_list);
    tt_ptr_op(p, OP_NE, NULL);
    smartlist_add(policy5, p);
...
test_policies_reject_port_address(void *arg)
{
    ...
    /* test that an IPv4 port is rejected on an IPv4-only exit, but an IPv6
     * port is NOT rejected (all IPv6 addresses are rejected by
...
test_policies_reject_interface_address(void *arg)
{
    ...
    /* test that only IPv4 interface addresses are rejected on an IPv4-only
...
test_policies_getinfo_helper_policies(void *arg)
{
    ...
    rv =
        getinfo_helper_policies(NULL, "exit-policy/default", &answer, &errmsg);
...
test_policies_fascist_firewall_allows_address(void *arg)
{
    ...
    tt_int_op(fascist_firewall_allows_address(&ipv6_addr, port, policy, 0, 0),
              OP_EQ, 1);
    tt_int_op(
        fascist_firewall_allows_address(&r_ipv4_addr, port, policy, 0, 0),
        OP_EQ, 0);
    tt_int_op(
        fascist_firewall_allows_address(&r_ipv6_addr, port, policy, 0, 0),
        OP_EQ, 0);
    ...
    tt_int_op(fascist_firewall_allows_address(&ipv6_addr, port, policy, 1, 0),
              OP_EQ, 0);
    tt_int_op(
        fascist_firewall_allows_address(&r_ipv4_addr, port, policy, 1, 0),
        OP_EQ, 0);
    tt_int_op(
        fascist_firewall_allows_address(&r_ipv6_addr, port, policy, 1, 0),
        OP_EQ, 0);
    ...
    tt_int_op(fascist_firewall_allows_address(&ipv6_addr, port, policy, 1, 1),
              OP_EQ, 1);
    tt_int_op(
        fascist_firewall_allows_address(&r_ipv4_addr, port, policy, 1, 1),
        OP_EQ, 0);
    tt_int_op(
        fascist_firewall_allows_address(&r_ipv6_addr, port, policy, 1, 1),
        OP_EQ, 0);
    ...
    tt_int_op(fascist_firewall_allows_address(NULL, port, policy, 0, 0), OP_EQ,
              0);
    tt_int_op(
        fascist_firewall_allows_address(&n_ipv4_addr, port, policy, 0, 0),
        OP_EQ, 0);
    tt_int_op(
        fascist_firewall_allows_address(&n_ipv6_addr, port, policy, 0, 0),
        OP_EQ, 0);
    ...
    tt_int_op(fascist_firewall_allows_address(&ipv6_addr, port, NULL, 0, 0),
              OP_EQ, 1);
    tt_int_op(
        fascist_firewall_allows_address(&ipv4_addr, port, e_policy, 0, 0),
        OP_EQ, 1);
    tt_int_op(
        fascist_firewall_allows_address(&ipv6_addr, port, e_policy, 0, 0),
        OP_EQ, 1);
...
#define CHECK_CHOSEN_ADDR_LS(fake_ls, pref_only, expect_rv, expect_ap)     \
    STMT_BEGIN                                                             \
        ...
        chosen_ls_ap.port = 0;                                             \
        setup_full_capture_of_logs(LOG_WARN);                              \
        fascist_firewall_choose_address_ls(fake_ls, pref_only,             \
                                           &chosen_ls_ap);                 \
...
#define CHECK_HS_EXTEND_INFO_ADDR_LS_EXPECT_MSG(fake_ls, msg_level, msg) \
    STMT_BEGIN                                                           \
        ...
        extend_info_t *ei =                                              \
            hs_get_extend_info_from_lspecs(fake_ls, &pubkey, 0);         \
        tt_ptr_op(ei, OP_EQ, NULL);                                      \
...
test_policies_fascist_firewall_choose_address(void *arg)
{
    ...
    CHECK_CHOSEN_ADDR_NODE(fake_node, FIREWALL_OR_CONNECTION, 0, 1,
                           ipv4_or_ap);
    CHECK_CHOSEN_ADDR_NODE(fake_node, FIREWALL_OR_CONNECTION, 1, 1,
                           ipv4_or_ap);
