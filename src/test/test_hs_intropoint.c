                         ...
                         trn_cell_establish_intro_t **cell_out)
{
    ...
    /* Ensure that *cell_out is NULL such that we can use to check if we need
     * to free `cell` in case of an error. */
    *cell_out = NULL;
...
new_establish_intro_encoded_cell(const char *circ_nonce, uint8_t *cell_out)
{
    ...
    tt_assert(ip);
    cell_len =
        hs_cell_build_establish_intro(circ_nonce, &config, ip, cell_out);
    tt_i64_op(cell_len, OP_GT, 0);
...
test_establish_intro_wrong_purpose(void *arg)
{
    ...
    retval =
        hs_intro_received_establish_intro(intro_circ, cell_body, cell_len);
...
test_establish_intro_wrong_mac(void *arg)
{
    ...
    /* We need to resign the payload with that change. */
    {
        ...
        /* New keypair for the signature since we don't have access to the
         * private key material generated earlier when creating the cell. */
        ...
        cell_len = trn_cell_establish_intro_encode(cell_body,
                                                   sizeof(cell_body), cell);
        tt_i64_op(cell_len, OP_GT, 0);
...
test_intro_point_registration(void *arg)
{
    ...
    MOCK(hs_intro_send_intro_established_cell,
...
test_received_introduce1_handling(void *arg)
{
    ...
    /* Valid case. */
    {
        ...
        hs_circuitmap_register_intro_circ_v3_relay_side(service_circ,
                                                        &auth_key);
