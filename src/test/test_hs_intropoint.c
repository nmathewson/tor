...
static size_t
new_establish_intro_cell(const char *circ_nonce, trn_cell_establish_intro_t **cell_out)
{
    ssize_t cell_len = 0;
...
helper_create_intro_circuit(void)
{
    ...
    token_bucket_ctr_init(&circ->introduce2_bucket, 100, 100, (uint32_t)approx_time());
done:
    return circ;
}
...
test_establish_intro_wrong_mac(void *arg)
{
    ...
    uint8_t *handshake_ptr = trn_cell_establish_intro_getarray_handshake_mac(cell);
    ...
    /* We need to resign the payload with that change. */
    {
        ...
        cell_len = trn_cell_establish_intro_encode(cell_body, sizeof(cell_body), cell);
        tt_i64_op(cell_len, OP_GT, 0);

        retval = ed25519_sign_prefixed(&sig, cell_body,
                                       cell_len - (ED25519_SIG_LEN + sizeof(cell->sig_len)),
...
test_establish_intro_wrong_sig(void *arg)
{
    ...
    /* Receive the cell. Should fail. */
    setup_full_capture_of_logs(LOG_INFO);
    retval = hs_intro_received_establish_intro(intro_circ, cell_body, (size_t)cell_len);
...
helper_establish_intro_v2(or_circuit_t *intro_circ)
{
    ...
    cell_len = rend_service_encode_establish_intro_cell((char *)cell_body, sizeof(cell_body), key1,
                                                        circ_nonce);
    tt_int_op(cell_len, OP_GT, 0);
...
test_intro_point_registration(void *arg)
{
    ...
    /* Create a v3 intro point */
    {
        ...
        get_auth_key_from_cell(&auth_key, RELAY_COMMAND_ESTABLISH_INTRO, establish_intro_cell);
        returned_intro_circ = hs_circuitmap_get_intro_circ_v3_relay_side(&auth_key);
    ...
    /* Create a v2 intro point */
    {
        ...
        tt_int_op(retval, OP_EQ, 0);
        returned_intro_circ = hs_circuitmap_get_intro_circ_v2_relay_side((uint8_t *)key_digest);
...
test_received_introduce1_handling(void *arg)
{
    ...
    /* Valid case. */
    {
        ...
        ssize_t encoded_len = trn_cell_introduce1_encode(request, request_len, cell);
        ...
        ed25519_public_key_t auth_key;
        const uint8_t *cell_auth_key = trn_cell_introduce1_getconstarray_auth_key(cell);
...
test_received_establish_intro_dos_ext(void *arg)
{
    ...
    tt_int_op(intro_circ->introduce2_bucket.cfg.burst, OP_EQ,
              HS_CONFIG_V3_DOS_DEFENSE_BURST_PER_SEC_DEFAULT);
    tt_int_op(intro_circ->introduce2_dos_defense_enabled, OP_EQ, HS_CONFIG_V3_DOS_DEFENSE_DEFAULT);
...
static struct testcase_setup_t test_setup = {hs_subsystem_setup_fn, hs_subsystem_cleanup_fn};

struct testcase_t hs_intropoint_tests[] = {
    {"intro_point_registration", test_intro_point_registration, TT_FORK, NULL, &test_setup},

    {"receive_establish_intro_wrong_keytype", test_establish_intro_wrong_keytype, TT_FORK, NULL,
     &test_setup},

    {"receive_establish_intro_wrong_keytype2", test_establish_intro_wrong_keytype2, TT_FORK, NULL,
     &test_setup},

    {"receive_establish_intro_wrong_purpose", test_establish_intro_wrong_purpose, TT_FORK, NULL,
     &test_setup},

    {"receive_establish_intro_wrong_sig", test_establish_intro_wrong_sig, TT_FORK, NULL,
     &test_setup},

    {"receive_establish_intro_wrong_sig_len", test_establish_intro_wrong_sig_len, TT_FORK, NULL,
     &test_setup},

    {"receive_establish_intro_wrong_auth_key_len", test_establish_intro_wrong_auth_key_len, TT_FORK,
     NULL, &test_setup},

    {"receive_establish_intro_wrong_mac", test_establish_intro_wrong_mac, TT_FORK, NULL,
     &test_setup},

    {"introduce1_suitable_circuit", test_introduce1_suitable_circuit, TT_FORK, NULL, &test_setup},

    {"introduce1_is_legacy", test_introduce1_is_legacy, TT_FORK, NULL, &test_setup},

    {"introduce1_validation", test_introduce1_validation, TT_FORK, NULL, &test_setup},

    {"received_introduce1_handling", test_received_introduce1_handling, TT_FORK, NULL, &test_setup},

    {"received_establish_intro_dos_ext", test_received_establish_intro_dos_ext, TT_FORK, NULL,
     &test_setup},
