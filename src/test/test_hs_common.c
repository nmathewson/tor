...
test_time_period(void *arg)
{
     ...
     * next time period, must be the same time period number as the one
     * returned from hs_get_next_time_period_num() */
    {
        time_t next_tp_start =
...
test_start_time_of_next_time_period(void *arg)
{
    ...
    {
        /* Now pretend we are on a testing network and alter the voting
           schedule to be every 10 seconds. This means that a time period has
           length 10*24 seconds (4 minutes). It also means that we apply a
           rotational offset of 120 seconds to the time period, so that it
           starts at 00:02:00 instead of 00:00:00. */
        ...
        retval =
            parse_rfc1123_time("Wed, 13 Apr 2016 00:00:00 UTC", &fake_time);
        tt_int_op(retval, OP_EQ, 0);
        next_tp_start_time = hs_get_start_time_of_next_time_period(fake_time);
        /* Compare it with the correct result */
        format_iso_time(tbuf, next_tp_start_time);
        ...
        retval =
            parse_rfc1123_time("Wed, 13 Apr 2016 00:02:00 UTC", &fake_time);
        tt_int_op(retval, OP_EQ, 0);
        next_tp_start_time = hs_get_start_time_of_next_time_period(fake_time);
        /* Compare it with the correct result */
        format_iso_time(tbuf, next_tp_start_time);
...
test_responsible_hsdirs(void *arg)
{
    ...
    hs_get_responsible_hsdirs(&pubkey, time_period_num, 0, 0,
                              responsible_dirs);
...
test_hid_serv_request_tracker(void *arg)
{
    ...
    const char *req_key_str_first = "vd4zb6zesaubtrjvdqcr2w7x7lhw2up4Xnw4526Th"
                                    "UNbL5o1go+EdUuEqlKxHkNbnK41pRzizzs";
    const char *req_key_str_second = "g53o7iavcd62oihswhr24u6czmqws5kpXnw4526T"
                                     "hUNbL5o1go+EdUuEqlKxHkNbnK41pRzizzs";
    ...
    hs_clean_last_hid_serv_requests(now + 3 +
...
{
    if (mock_service_ns) {
        SMARTLIST_FOREACH(mock_service_ns->routerstatus_list, routerstatus_t *,
                          rs, routerstatus_free(rs));
    ...
    if (mock_client_ns) {
        SMARTLIST_FOREACH(mock_client_ns->routerstatus_list, routerstatus_t *,
                          rs, routerstatus_free(rs));
...
run_reachability_scenario(const reachability_cfg_t *cfg, int num_scenario)
{
    ...
    /* Client ONLY use the current time period. This is the whole point of
     * these reachability test that is to make sure the client can always reach
     * the service using only its current time period. */
     ...
     * next descriptor. Depending on the scenario, the client timing indicate
     * if it is fetching the current or the next descriptor so we use the
     * "client_fetch_next_desc" to know which one the client is trying to get
     * to confirm that the service computes the same hashring for the same
     * blinded key and service time period function. */
                       ...
                       int client_position, int client_fetches_next_desc)
{
     ...
     *   4) Try to fetch descriptor as client, and CHECK that the HSDir picked
     * by the client was also picked by service.
     */
    ...
    desc =
        client_fetches_next_desc ? service->desc_next : service->desc_current;
    ...
    for (int y = 0; y < 6; y++) {
        ...
        /* CHECK: Go through the hsdirs chosen by the service and make sure
         * that it contains the one picked by the client! */
    ...
    /* At the end: free all services and initialize the subsystem again, we
     * will need it for next scenario. */
    cleanup_nodelist();
    hs_service_free_all();
    hs_service_init();
    SMARTLIST_FOREACH(ns->routerstatus_list, routerstatus_t *, rs,
...
test_client_service_hsdir_set_sync(void *arg)
{
    ...
    /*  a) Scenario where both client and service are in the time segment
     * between SRV#N and TP#N. At this time the client fetches the first HS
     * desc:
     *
     *  +------------------------------------------------------------------+
     *  |                                                                  |
    ...
    /*  b) Scenario where both client and service are in the time segment
     * between TP#N and SRV#N+1. At this time the client fetches the second HS
     *     desc:
     *
     *  +------------------------------------------------------------------+
     *  |                                                                  |
     ...
     *     between TP#N and SRV#N+1. Client is forward in time so it fetches
     * the second HS desc.
     *
     *  +------------------------------------------------------------------+
     *  |                                                                  |
    ...
    /*  d) Scenario where service is between TP#N and SRV#N+1, but client is
     *     between SRV#N and TP#N. Client is backwards in time so it fetches
     * the first HS desc.
     *
     *  +------------------------------------------------------------------+
     *  |                                                                  |
