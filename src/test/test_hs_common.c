...
test_start_time_of_next_time_period(void *arg)
{
    ...
    {
        /* Now pretend we are on a testing network and alter the voting schedule
           to be every 10 seconds. This means that a time period has length
           10*24 seconds (4 minutes). It also means that we apply a rotational
           offset of 120 seconds to the time period, so that it starts at
           00:02:00 instead of 00:00:00. */
...
test_hid_serv_request_tracker(void *arg)
{
    ...
    const char *req_key_str_first = "vd4zb6zesaubtrjvdqcr2w7x7lhw2up4Xnw4526ThU"
                                    "NbL5o1go+EdUuEqlKxHkNbnK41pRzizzs";
    const char *req_key_str_second = "g53o7iavcd62oihswhr24u6czmqws5kpXnw4526Th"
                       ...
                       int client_position, int client_fetches_next_desc)
{
    ...
    for (int y = 0; y < 6; y++) {
        ...
        /* CHECK: Go through the hsdirs chosen by the service and make sure that
         * it contains the one picked by the client! */
