...
test_validate_address(void *arg)
{
    ...
    setup_full_capture_of_logs(LOG_WARN);
    ret = hs_address_is_valid("p3xnclpu4mu22dwaurjtsybyqk4xfjmcfz6z62yl24uwmhjatiwnlnadb");
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg_containing("Invalid length");
    teardown_capture_of_logs();
    ...
    setup_full_capture_of_logs(LOG_WARN);
    ret = hs_address_is_valid("l5satjgud6gucryazcyvyvhuxhr74u6ygigiuyixe3a6ysis67ororad");
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg_containing("invalid checksum");
    teardown_capture_of_logs();

    setup_full_capture_of_logs(LOG_WARN);
    ret = hs_address_is_valid("btojiu7nu5y5iwut64eufevogqdw4wmqzugnoluw232r4t3ecsfv37ad");
    tt_int_op(ret, OP_EQ, 0);
    expect_log_msg_containing("invalid checksum");
    teardown_capture_of_logs();
    ...
    /* Valid address. */
    ret = hs_address_is_valid("25njqamcweflpvkl73j4szahhihoc4xt3ktcgjnpaingr5yhkenl5sid");
...
mock_write_str_to_file(const char *path, const char *str, int bin)
{
    ...
    tt_str_op(str, OP_EQ, "25njqamcweflpvkl73j4szahhihoc4xt3ktcgjnpaingr5yhkenl5sid.onion\n");
...
test_build_address(void *arg)
{
    ...
    char pubkey_hex[] = "d75a980182b10ab7d54bfed3c964073a0ee172f3daa62325af021a68f707511a";
    ...
    static const char *test_addr = "25njqamcweflpvkl73j4szahhihoc4xt3ktcgjnpaingr5yhkenl5sid";

    /* Let's try to build the same onion address as the script */
    base16_decode((char *)pubkey.pubkey, sizeof(pubkey.pubkey), pubkey_hex, strlen(pubkey_hex));
...
test_time_period(void *arg)
{
    ...
    { /* Check start time of next time period */
        retval = parse_rfc1123_time("Wed, 13 Apr 2016 12:00:00 UTC", &correct_time);
        tt_int_op(retval, OP_EQ, 0);
    ...
    { /* Check start time of next time period again */
        retval = parse_rfc1123_time("Wed, 14 Apr 2016 12:00:00 UTC", &correct_time);
        tt_int_op(retval, OP_EQ, 0);
...
static void
helper_add_hsdir_to_networkstatus(networkstatus_t *ns, int identity_idx, const char *nickname,
                                  int is_hsdir)
{
    ...
    memset(&ri->cache_info.signing_key_cert->signing_key, identity_idx, ED25519_PUBKEY_LEN);
...
static int
mock_hs_desc_encode_descriptor(const hs_descriptor_t *desc, const ed25519_keypair_t *signing_kp,
...
test_hid_serv_request_tracker(void *arg)
{
    ...
    retval = hs_lookup_last_hid_serv_request(hsdir, req_key_str_second, now + 1, 0);
    tt_int_op(retval, OP_EQ, 0);
    tt_int_op(strmap_size(request_tracker), OP_EQ, 1);

    /* Let's lookup a real hidserv request */
    retval = hs_lookup_last_hid_serv_request(hsdir, req_key_str_first, now + 2, 0);
    ...
    retval = hs_lookup_last_hid_serv_request(hsdir2, req_key_str_first, now + 3, 1);
    ...
    retval = hs_lookup_last_hid_serv_request(hsdir3, req_key_str_small, now + 4, 1);
...
test_parse_extended_hostname(void *arg)
{
    ...
    char address8[] = "www.25njqamcweflpvkl73j4szahhihoc4xt3ktcgjnpaingr5yhkenl5sid.onion";
    char address9[] = "www.15njqamcweflpvkl73j4szahhihoc4xt3ktcgjnpaingr5yhkenl5sid.onion";
    ...
    tt_str_op(address8, OP_EQ, "25njqamcweflpvkl73j4szahhihoc4xt3ktcgjnpaingr5yhkenl5sid");
...
are_responsible_hsdirs_equal(void)
{
    ...
    SMARTLIST_FOREACH_BEGIN (client_responsible_hsdirs, const routerstatus_t *, c_rs) {
        SMARTLIST_FOREACH_BEGIN (service_responsible_hsdirs, const routerstatus_t *, s_rs) {
            if (tor_memeq(c_rs->identity_digest, s_rs->identity_digest, DIGEST_LEN)) {
                count++;
                break;
            }
...
run_reachability_scenario(const reachability_cfg_t *cfg, int num_scenario)
{
    ...
    dirauth_sched_recalculate_timing(get_options(), mock_service_ns->valid_after);
    /* Check that service is in the right time period point */
    tt_int_op(hs_in_period_between_tp_and_srv(mock_service_ns, 0), OP_EQ, cfg->service_in_new_tp);
    ...
    dirauth_sched_recalculate_timing(get_options(), mock_client_ns->valid_after);
    /* Check that client is in the right time period point */
    tt_int_op(hs_in_period_between_tp_and_srv(mock_client_ns, 0), OP_EQ, cfg->client_in_new_tp);
    ...
    MOCK(networkstatus_get_latest_consensus, mock_networkstatus_get_latest_consensus_client);
    ...
    hs_build_blinded_pubkey(&service->keys.identity_pk, NULL, 0, client_tp, &client_blinded_pk);
    hs_get_responsible_hsdirs(&client_blinded_pk, client_tp, 0, 1, client_responsible_hsdirs);
    ...
    MOCK(networkstatus_get_live_consensus, mock_networkstatus_get_live_consensus_service);
    MOCK(networkstatus_get_latest_consensus, mock_networkstatus_get_latest_consensus_service);
    ...
    hs_build_blinded_pubkey(&service->keys.identity_pk, NULL, 0, service_tp, &service_blinded_pk);
    ...
    hs_get_responsible_hsdirs(&service_blinded_pk, service_tp, cfg->client_fetch_next_desc, 0,
    ...
    tt_mem_op(&client_blinded_pk, OP_EQ, &service_blinded_pk, ED25519_PUBKEY_LEN);
...
static void
helper_client_pick_hsdir(const ed25519_public_key_t *onion_identity_pk, char *hsdir_digest_out)
{
...
static void
helper_test_hsdir_sync(networkstatus_t *ns, int service_position, int client_position,
                       int client_fetches_next_desc)
{
    ...
    for (int y = 0; y < 6; y++) {
        ...
        helper_client_pick_hsdir(&service->keys.identity_pk, client_hsdir_b64_digest);

        /* CHECK: Go through the hsdirs chosen by the service and make sure that it
         * contains the one picked by the client! */
        retval = smartlist_contains_string(desc->previous_hsdirs, client_hsdir_b64_digest);
...
struct testcase_t hs_common_tests[] = {
    ...
    {"start_time_of_next_time_period", test_start_time_of_next_time_period, TT_FORK, NULL, NULL},
    ...
    {"hid_serv_request_tracker", test_hid_serv_request_tracker, TT_FORK, NULL, NULL},
    {"parse_extended_hostname", test_parse_extended_hostname, TT_FORK, NULL, NULL},
    {"time_between_tp_and_srv", test_time_between_tp_and_srv, TT_FORK, NULL, NULL},
    {"reachability", test_reachability, TT_FORK, NULL, NULL},
    {"client_service_hsdir_set_sync", test_client_service_hsdir_set_sync, TT_FORK, NULL, NULL},
