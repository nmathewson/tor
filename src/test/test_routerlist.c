...
static void
mock_get_from_dirserver(uint8_t dir_purpose, uint8_t router_purpose, const char *resource,
...
static void
mock_initiate_descriptor_downloads(const routerstatus_t *source, int purpose, smartlist_t *digests,
...
test_routerlist_launch_descriptor_downloads(void *arg)
{
    ...
    launch_descriptor_downloads(DIR_PURPOSE_FETCH_MICRODESC, downloadable, NULL, now);
...
construct_consensus(char **consensus_text_md, time_t now)
{
    ...
    tt_assert(!dir_common_authority_pk_init(&cert1, &cert2, &cert3, &sign_skey_1, &sign_skey_2,
                                            &sign_skey_3));
    sign_skey_leg = pk_generate(4);

    dir_common_construct_vote_1(&vote, cert1, sign_skey_1, &dir_common_gen_routerstatus_for_v3ns,
                                &v1, &n_vrs, now, 1);
    ...
    dir_common_construct_vote_2(&vote, cert2, sign_skey_2, &dir_common_gen_routerstatus_for_v3ns,
                                &v2, &n_vrs, now, 1);
    networkstatus_vote_free(vote);
    tt_assert(v2);
    tt_int_op(n_vrs, OP_EQ, 4);
    ...
    dir_common_construct_vote_3(&vote, cert3, sign_skey_3, &dir_common_gen_routerstatus_for_v3ns,
                                &v3, &n_vrs, now, 1);
    ...
    *consensus_text_md =
        networkstatus_compute_consensus(votes, 3, cert1->identity_key, sign_skey_1,
                                        "AAAAAAAAAAAAAAAAAAAA", sign_skey_leg, FLAV_MICRODESC);
...
test_router_pick_directory_server_impl(void *arg)
{
    ...
    tt_assert(consensus_text_md);
    con_md = networkstatus_parse_vote_from_string(consensus_text_md, strlen(consensus_text_md),
    ...
    tt_assert(!networkstatus_consensus_is_bootstrapping(con_md->valid_until + 24 * 60 * 60));
...
mock_circuit_guard_state_new(entry_guard_t *guard, unsigned state, entry_guard_restriction_t *rst)
{
    (void)guard;
    (void)state;
    (void)rst;
...
test_directory_guard_fetch_with_no_dirinfo(void *arg)
{
    ...
    routerset_parse(get_options_mutable()->EntryNodes, "2121212121212121212121212121212121212121",
                    "foo");
 ...
 * mocked_connection. */
static connection_t *
mock_connection_get_by_type_addr_port_purpose(int type, const tor_addr_t *addr, uint16_t port,
                                              int purpose)
{
    (void)type;
    (void)addr;
...
test_routerlist_router_is_already_dir_fetching(void *arg)
{
    ...
    zero_port_ap.port = 0;
    MOCK(connection_get_by_type_addr_port_purpose, mock_connection_get_by_type_addr_port_purpose);
 ...
 * expected. */
static void
mock_clock_skew_warning(const connection_t *conn, long apparent_skew, int trusted,
...
test_skew_common(void *arg, time_t now, unsigned long *offset)
{
    ...
    *offset = strtoul(arg, NULL, 10);
    ...
    retval =
        networkstatus_set_current_consensus(consensus, strlen(consensus), "microdesc", 0, NULL);
...
static void
test_warn_early_consensus_yes(const networkstatus_t *c, time_t now, long offset)
{
    mock_apparent_skew = 0;
    setup_capture_of_logs(LOG_WARN);
    warn_early_consensus(c, "microdesc", now + offset);
...
struct testcase_t routerlist_tests[] = {
    ...
    {"directory_guard_fetch_with_no_dirinfo", test_directory_guard_fetch_with_no_dirinfo, TT_FORK,
     NULL, NULL},
