...
test_router_pick_directory_server_impl(void *arg)
{
    ...
    /* With no consensus, we must be bootstrapping, regardless of time or
     * flavor
     */
    ...
    tt_assert(
        networkstatus_consensus_is_bootstrapping(now + 2 * 24 * 60 * 60));
    tt_assert(
        networkstatus_consensus_is_bootstrapping(now - 2 * 24 * 60 * 60));
...
test_directory_guard_fetch_with_no_dirinfo(void *arg)
{
     ...
     * sure that Tor will use the specified entry guard instead of relying on
