...
test_rend_cache_store_v2_desc_as_client(void *data)
{
    ...
    ret = rend_cache_store_v2_desc_as_client(
        "invalid descriptor", "3xqunszqnaolrrfmtzgaki7mxelgvkje",
        mock_rend_query, NULL);
    tt_int_op(ret, OP_EQ, -1);

    // TODO: it doesn't seem to be possible to test invalid service ID
    // condition. that means it is likely not possible to have that condition
    // without earlier conditions failing first (such as signature checking of
    // the desc)
...
test_rend_cache_init(void *data)
{
    ...
    tt_assert_msg(!rend_cache_v2_dir, "rend_cache_v2_dir should be NULL "
                                      "when starting");
    tt_assert_msg(!rend_cache_failure,
...
test_rend_cache_failure_intro_lookup(void *data)
{
    ...
    ret =
        cache_failure_intro_lookup((const uint8_t *)key_ip_two, "foo1", NULL);
    tt_int_op(ret, OP_EQ, 0);

    // Test found
    ret =
        cache_failure_intro_lookup((const uint8_t *)key_ip_one, "foo1", NULL);
    tt_int_op(ret, OP_EQ, 1);
