...
static const int TIME_IN_THE_PAST = -(REND_CACHE_MAX_AGE + REND_CACHE_MAX_SKEW + 60);
...
test_rend_cache_lookup_entry(void *data)
{
    ...
    rend_cache_store_v2_desc_as_client(desc_holder->desc_str, desc_id_base32, mock_rend_query,
                                       NULL);
...
test_rend_cache_store_v2_desc_as_client(void *data)
{
    ...
    mock_rend_query = mock_rend_data(service_id);
    base32_encode(desc_id_base32, sizeof(desc_id_base32), desc_holder->desc_id, DIGEST_LEN);
    ret = rend_cache_store_v2_desc_as_client(desc_holder->desc_str, desc_id_base32, mock_rend_query,
                                             &entry);
    ...
    ret = rend_cache_store_v2_desc_as_client(
        desc_holder->desc_str, "!xqunszqnaolrrfmtzgaki7mxelgvkj", mock_rend_query, NULL);
    tt_int_op(ret, OP_EQ, -1);
#endif

    // Test invalid descriptor
    ret = rend_cache_store_v2_desc_as_client(
        "invalid descriptor", "3xqunszqnaolrrfmtzgaki7mxelgvkje", mock_rend_query, NULL);
    tt_int_op(ret, OP_EQ, -1);
    ...
    rend_cache_init();
    strncpy(TO_REND_DATA_V2(mock_rend_query)->onion_address, "abc", REND_SERVICE_ID_LEN_BASE32 + 1);
    ret = rend_cache_store_v2_desc_as_client(desc_holder->desc_str, desc_id_base32, mock_rend_query,
                                             NULL);
    tt_int_op(ret, OP_EQ, -1);
    rend_cache_free_all();
...
test_rend_cache_store_v2_desc_as_client_with_different_time(void *data)
{
    ...
    rend_cache_store_v2_desc_as_client(desc_holder_newer->desc_str, desc_id_base32, mock_rend_query,
                                       NULL);
    ret = rend_cache_store_v2_desc_as_client(desc_holder_older->desc_str, desc_id_base32,
                                             mock_rend_query, NULL);
    tt_int_op(ret, OP_EQ, 0);
    ...
    rend_cache_init();
    rend_cache_store_v2_desc_as_client(desc_holder_older->desc_str, desc_id_base32, mock_rend_query,
                                       NULL);
    ret = rend_cache_store_v2_desc_as_client(desc_holder_newer->desc_str, desc_id_base32,
                                             mock_rend_query, NULL);
    tt_int_op(ret, OP_EQ, 0);
...
test_rend_cache_decrement_allocation(void *data)
{
    ...
    expect_single_log_msg_containing("Underflow in rend_cache_decrement_allocation");
...
test_rend_cache_increment_allocation(void *data)
{
    ...
    expect_single_log_msg_containing("Overflow in rend_cache_increment_allocation");
    teardown_capture_of_logs();
...
test_rend_cache_failure_intro_add(void *data)
{
    ...
    cache_failure_intro_add((const uint8_t *)identity, "foo2", INTRO_POINT_FAILURE_TIMEOUT);
...
test_rend_cache_intro_failure_note(void *data)
{
    ...
    // Test not found
    rend_cache_intro_failure_note(INTRO_POINT_FAILURE_TIMEOUT, (const uint8_t *)key, "foo2");
    fail_entry = strmap_get_lc(rend_cache_failure, "foo2");
    tt_assert(fail_entry);
    tt_int_op(digestmap_size(fail_entry->intro_failures), OP_EQ, 1);
    ...
    // Test found
    rend_cache_intro_failure_note(INTRO_POINT_FAILURE_UNREACHABLE, (const uint8_t *)key, "foo2");
...
struct testcase_t rend_cache_tests[] = {
    ...
    {"store_v2_desc_as_client_with_different_time",
     test_rend_cache_store_v2_desc_as_client_with_different_time, 0, NULL, NULL},
    ...
    {"store_v2_desc_as_dir_with_different_content",
     test_rend_cache_store_v2_desc_as_dir_with_different_content, 0, NULL, NULL},
