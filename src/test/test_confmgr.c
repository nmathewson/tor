...
#define PV(name, type, dflt) CONFIG_VAR_ETYPE(pasture_cfg_t, #name, type, name, 0, dflt)
#define LV(name, type, dflt) CONFIG_VAR_ETYPE(llama_cfg_t, #name, type, name, 0, dflt)
#define AV(name, type, dflt) CONFIG_VAR_ETYPE(alpaca_cfg_t, #name, type, name, 0, dflt)
static const config_var_t pasture_vars[] = {PV(address, STRING, NULL), PV(opentopublic, BOOL, "1"),
                                            END_OF_CONFIG_VARS};
static const config_var_t llama_vars[] = {LV(llamaname, STRING, NULL), LV(eats_meat, BOOL, NULL),
                                          LV(cuteness, POSINT, "100"), END_OF_CONFIG_VARS};
static const config_var_t alpaca_vars[] = {AV(alpacaname, STRING, NULL),
                                           AV(fuzziness, POSINT, "50"), AV(n_wings, POSINT, "0"),
                                           END_OF_CONFIG_VARS};

static config_deprecation_t llama_deprecations[] = {{"eats_meat", "Llamas are herbivores."},
                                                    {NULL, NULL}};

static config_deprecation_t alpaca_deprecations[] = {{"n_wings", "Alpacas are quadrupeds."},
                                                     {NULL, NULL}};
...
legacy_validate_pasture(const void *old_, void *obj, char **msg_out)
{
    ...
    if (old && old->address && (!p->address || strcmp(old->address, p->address))) {
...
test_confmgr_init(void *arg)
{
    ...
    tt_str_op("Llamas are herbivores.", OP_EQ, config_find_deprecation(mgr, "EATS_MEAT"));
    tt_str_op("Alpacas are quadrupeds.", OP_EQ, config_find_deprecation(mgr, "N_WINGS"));
...
parse_and_validate(config_mgr_t *mgr, const char *inp, const pasture_cfg_t *old, char **msg_out)
{
    pasture_cfg_t *p = config_new(mgr);
...
struct testcase_t confmgr_tests[] = {CONFMGR_TEST(init, 0),     CONFMGR_TEST(magic, 0),
                                     CONFMGR_TEST(parse, 0),    CONFMGR_TEST(dump, 0),
                                     CONFMGR_TEST(validate, 0), END_OF_TESTCASES};
