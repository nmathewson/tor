                            ...
                            const curve25519_keypair_t *intro_enc_kp)
{
    ...
    /* Encryption key. */
    {
        ...
        cross_cert = tor_cert_create(
            signing_kp, CERT_TYPE_CROSS_HS_IP_KEYS, &ed25519_kp.pubkey,
            time(NULL), HS_DESC_CERT_LIFETIME, CERT_FLAG_INCLUDE_SIGNING_KEY);
                             ...
                             const ed25519_keypair_t *signing_kp)
{
    ...
    smartlist_add(desc->encrypted_data.intro_auth_types,
                  tor_strdup("ed25519"));
    desc->encrypted_data.intro_points = smartlist_new();
    if (!no_ip) {
        /* Add four intro points. */
        smartlist_add(desc->encrypted_data.intro_points,
                      hs_helper_build_intro_point(signing_kp, now, "1.2.3.4",
                                                  0, NULL, NULL));
        smartlist_add(desc->encrypted_data.intro_points,
                      hs_helper_build_intro_point(signing_kp, now, "[2600::1]",
                                                  0, NULL, NULL));
        smartlist_add(desc->encrypted_data.intro_points,
                      hs_helper_build_intro_point(signing_kp, now, "3.2.1.4",
                                                  1, NULL, NULL));
        smartlist_add(desc->encrypted_data.intro_points,
                      hs_helper_build_intro_point(signing_kp, now, "5.6.7.8",
                                                  1, NULL, NULL));
    }
                     ...
                     const hs_descriptor_t *desc2)
{
    ...
    tt_mem_op(desc1->superencrypted_data.auth_ephemeral_pubkey.public_key,
              OP_EQ,
    ...
    /* Auth clients. */
    {
        ...
        for (int i = 0; i < smartlist_len(desc1->superencrypted_data.clients);
            ...
            tt_mem_op(client1->encrypted_cookie, OP_EQ,
                      client2->encrypted_cookie,
    ...
    if (desc1->encrypted_data.intro_auth_types &&
        ...
        for (int i = 0;
             i < smartlist_len(desc1->encrypted_data.intro_auth_types); i++) {
            tt_str_op(
                smartlist_get(desc1->encrypted_data.intro_auth_types, i),
                OP_EQ,
    ...
    /* Introduction points. */
    {
        ...
        for (int i = 0; i < smartlist_len(desc1->encrypted_data.intro_points);
            ...
            if (ip1->legacy.key) {
                ...
                tt_mem_op(&ip1->enc_key, OP_EQ, &ip2->enc_key,
                          CURVE25519_PUBKEY_LEN);
            }
