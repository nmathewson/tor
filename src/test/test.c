...
test_onion_handshake(void *arg)
{
    ...
    for (i = 1; i <= 3; ++i) {
        ...
        memset(s_keys, 0, 40);
        tt_assert(!onion_skin_TAP_server_handshake(c_buf, k1, k2, s_buf,
                                                   s_keys, 40));
...
test_bad_onion_handshake(void *arg)
{
    ...
    tt_int_op(-1, OP_EQ,
              onion_skin_TAP_server_handshake(junk_buf2, pk, NULL, s_buf,
                                              s_keys, 40));
...
test_fast_handshake(void *arg)
{
    ...
    tt_int_op(0, OP_EQ,
              fast_server_handshake(client_handshake, server_handshake, s_keys,
                                    100));
    const char *msg = NULL;
    tt_int_op(
        0, OP_EQ,
...
test_rend_fns(void *arg)
{
    ...
    for (i = 0; i < smartlist_len(parsed->intro_nodes); i++) {
        rend_intro_point_t *par_intro = smartlist_get(parsed->intro_nodes, i),
                           *gen_intro =
                               smartlist_get(generated->intro_nodes, i);
        extend_info_t *par_info = par_intro->extend_info;
        extend_info_t *gen_info = gen_intro->extend_info;
        tt_assert(
            !crypto_pk_cmp_keys(gen_info->onion_key, par_info->onion_key));
